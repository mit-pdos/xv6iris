(* ====================================================================== *)
(*  OFF THE BUILD.  This file's row in _CoqProject is left out: it is the  *)
(*  MULTI-HART builder, and it is written against the old [TEST_RUN] --    *)
(*  the one whose [outcome : model_outcome] made the top-level theorem a   *)
(*  statement about two data rather than about [prim_step].  That type is  *)
(*  gone, so this does not compile.                                        *)
(*                                                                         *)
(*  IT IS KEPT, UNEDITED BELOW, because the port is real work and this is  *)
(*  its starting point: [VConc]'s [crun]/[cfinish] already run on          *)
(*  [gstate], so a multi-hart run should reuse [VExecStep.enode_prim]      *)
(*  directly rather than needing a second bridge.  Until then the twelve   *)
(*  conc_* runs are proved through the SINGLE-hart theorem, which is a     *)
(*  true but weaker thing to know about a race.                            *)
(* ====================================================================== *)
