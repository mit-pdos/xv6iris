...
(*                                                                         *)
(* THE UART HALF IS NOT INNOCENT EITHER, and it is why this source makes a  *)
(* good probe.  [DevModel.uart_isr]'s own comment says the THRE interrupt   *)
(* is modelled as a LEVEL that an ISR read does not clear, where a real     *)
(* 16550 latches it and clears it on that read.  Nothing here reads the     *)
(* ISR, so nothing here depends on that -- but it means the level under     *)
(* test stays asserted for as long as IER bit 1 is set, which is what makes *)
(* the gateway's behaviour visible at all.  Testing the ISR itself belongs  *)
(* to a `uart` test, not a `plic` one.                                      *)
(* ---------------------------------------------------------------------- *)
