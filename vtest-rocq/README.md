...
4. `make vtest-check`, and `make vtest-check-ci` for what CI will say.  CI
   requires every test in `_CoqProject` to pass, so a divergence found here is
   pinned on both sides (below) rather than left red.
