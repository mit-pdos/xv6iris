...
| arm | refuted by |
| --- | --- |
| `panic("kerneltrap: not from supervisor mode")` | the entry mstatus has `_get_Mstatus_SPP = 'b"1"` — the trap came from S-mode, which `trap_ms` in the handler contract already says |
