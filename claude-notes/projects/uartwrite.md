# uartwrite — the interrupt-driven UART output path

`void uartwrite(char buf[], int n)` @ `KernelSyms.uartwrite = 0x800008dc`,
52 instructions, an 80-byte frame.  **PROVEN and linked** (`SpecUartwrite.v` /
`WpUartwriteDecode.v` / `ProofUartwrite.v` / `LinkUartwrite.v`, over the new
definitional layer `UartTxInv.v`).
...
- **uartintr** is the other half of `tx_res` and is unproven.  It is what makes
  the invariant's `tx_busy == 0 ⟹ uart_out_lb` implication ever TRUE again
  after uartwrite sets the flag; until it is proven, a `uartwrite` that is
  called twice can only be shown to make progress on its first byte in any
  execution where nothing clears `tx_busy` (the spec is still sound — it says
  what holds when the call returns, not that it returns).
