...
1. **The slot cells on the ledger (the pin-return crossing; A6.124's
   ...
   forgetting, RwF §1's bridges deleted.  Sized in A6.124.  **Steps 1–5-OUT of the (b′)
   plan are LANDED (r42–r46): the pin-return crossing for HART-WRITTEN
   cells is closed.  What is left in RwF is the two DEVICE-WRITTEN
   hand-backs (`vdrwf_plist_mem`, the IN buffer; the status byte at ~1208)
   and `ProofVirtioDiskIntr`'s reclaim -- all the same crossing, waiting on
   item 2's instrument.  Nothing buildable remains in this item without the
   ruling.  Old text of
...
2. **RULED (§0.41′, 2026-08-29): the used index is a RELEASE cell** -- an
   ...
   pure arm (§1) and the `TsoCtx` kit (§4 step 1) are LANDED (r47–r49);
   the engine node and the release-read leaf are LANDED too (r50); what
   remains is the virtio side, A6.126 §4 items 3–6 (the used-index word
   minted as a release window at init, the device's completion as a
   second message through `ledger_store_rel_ok`, positions exposed in
   `slot_done_res`, the floor beside `disk_done_lb`, the handler's read
   through `_au_rel`, RwF's two hand-backs).  Greens
