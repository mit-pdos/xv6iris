...
- **THE DEVICE-CONFORMANCE TESTS ARE IN CI, AND ADDING ONE MEANS ADDING A ROW.** `.github/workflows/ci.yml`'s last step runs `make vtest-check-ci`: every test in `vtest-rocq/_CoqProject` compiled with `-k` against the CHECKED-IN QEMU captures (no QEMU, nothing regenerated), then `tools/vtest/vtest_status.py` reports per test and decides against `vtest-rocq/expected-pass.txt` — a bare name there must pass, a `!Name  reason` row is known red and forgiven, and **a test the file does not mention fails CI**, which is what a new test hits if only `_CoqProject` is updated. The target deletes `vtest-rocq/*.vo` first on purpose: a failed recompile leaves the previous `.vo` in place (`.DELETE_ON_ERROR` takes the `.glob`, not an untouched `.vo`), so on a warm tree a newly red test would otherwise be reported green off the last run's output. ~600 s of CPU, one 33 s file wide.
