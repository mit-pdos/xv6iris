...
- **`kernel-rocq/KernelInstrs.v` SITS ON THE 8 MB STACK BOUNDARY, AND ITS OVERFLOW IS NONDETERMINISTIC.** The file ends in one `list_to_map [...]` of ~23.7k entries, and elaborating it is stack-depth-linear, so `Error: Stack overflow` at the closing `].` (~line 32375) is a COIN FLIP on an 8 MB stack: the same file, same flags, same machine gave 1 failure in 3 runs, while the pre-bump copy passed — which makes it look exactly like "the bump broke the dump" when it is neither the bump nor the file. Do not bisect the dump. `ulimit -s 65536` makes it deterministic (5/5), and the GCP VM builds it in ~18 s at the default limit, so a local failure is not a reason to doubt a green VM build. Note this is NOT the `nat`-literal trap above (there is no unary chain here — it is list depth), and no `Z`-restating fixes it; the generator would have to chunk the list.
...
- **A SET EQUATION WHOSE RIGHT SIDE MENTIONS `set_seq 0 (S n)` COSTS MINUTES PER BUILD.** `VirtioQueue`'s `vpo_pend_dom` was `dom (vp_pend pr) = set_seq 0 (vp_np pr) ∖ vp_srv pr`; at the PUBLISH the published count is `S (vp_np pr)`, and any conversion that touches that goal — `cbn [vp_np vp_pend vp_srv]` is enough — unfolds `set_seq 0 (S n)` one step into `{[0+n]} ∪ set_seq 1 n` and then normalises the union into the `gset`'s map representation. The file went from seconds to **six minutes and 2 GB**, with the `.v.timing` file (which is block-buffered, so its last entry lags by up to 4 KB of source) pointing at whatever tactic happened to be near the real one. The fix is to state such an invariant POINTWISE — `forall q, q ∈ dom (vp_pend pr) <-> (q < vp_np pr)%nat /\ q ∉ vp_srv pr` — which says the same thing and never builds a set; the same rule is why the projection lemmas (`vppq_*`) exist rather than `unfold ...; cbn [...]`. Diagnose it the same way: `ps -o pid,etime,rss -C rocqworker` on the VM shows one worker pinned at a stable multi-GB RSS in `R` state, and killing and re-running with `TIMING=1` bounds the region.
