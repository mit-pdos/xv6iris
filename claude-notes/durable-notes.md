...
- **A "CLEAN" `iris/` REBUILD IS NOT CLEAN IF `model-xv6iris/*.v` MOVED — the model `.vo` is outside `iris/CoqMakefile`'s rule set.**  Deleting `iris/*.vo` and rebuilding still LINKS against whatever `model-xv6iris/*.vo` is on disk, and if the model *source* was edited or reverted since, every number that build produces is about the OLD model — silently, since the digests are internally consistent.  Measured 2026-08-26 in the flip workspace: a model-source revert at 18:37 left the 11:15 `.vo` in place, and a full "clean" rebuild validated a machine that no longer existed (tso-machine-flip.md A6.39); it surfaced only when a proof's `cbn` produced a constructor the restored model cannot emit.  Rule: any time `model-xv6iris/*.v` changes IN EITHER DIRECTION (edit, revert, restore-from-backup), rebuild the model first (`rv64d_types.v → riscv_extras.v → rv64d.v`) and verify `.vo` postdates `.v` before trusting any downstream build — same tell as the `git pull` entry above, but the pull is not the only way to get there.
