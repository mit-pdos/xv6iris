...
- **A C POINTER CAST IN A COMMENT CLOSES THE COMMENT.** Writing `[wait((int *)0)]` or `[(char *)buf]` inside `(* … *)` ends the comment at the cast's `*)`, and the rest of the prose is then parsed as Rocq — which surfaces as `Syntax error: illegal begin of vernac` at whatever line the prose happens to reach, not at the cast. (Cost two build rounds on 2026-08-30, in `SpecKwait.v` and `UsysMemOk.v`.) Say "a null status pointer" rather than pasting the cast. The sibling trap is a `"` in a comment: Rocq starts a string, and you get `because it occurs in a non-terminated string of the comment` — harmless as a warning but it means the rest of that comment is lexed as a string.
