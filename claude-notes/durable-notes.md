...
- **`destruct (decide P)` in one file does NOT reduce a `decide P` baked into ANOTHER file's definition** (seen on `InodeInv.blk_res`/`ind_blk`/`ind_tok`): the two `Decision P` instance terms differ even though they print identically, so the `if` survives the destruct — and a following `iIntros "_"` then silently discards the unreduced hypothesis instead of failing. Use `rewrite (decide_True _ _ H)` / `rewrite (decide_False _ _ H)`, where the instance is an evar and unifies with the definition's own.
