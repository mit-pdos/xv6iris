...
- **A `nat` EQUALITY WHOSE RHS IS A LARGE LITERAL NEEDS `Z`, NOT A BIGGER STACK.** Any route to closing such a goal — `reflexivity`, `vm_compute`, even `vm_cast_no_check` — eventually makes something materialize a literal-deep unary successor chain, and that overflows a normal 8 MB stack outright (`ProofWriteiParts.v`'s old `wi_maxfile_bsize : (MAXFILE * BSIZE)%nat = 274432%nat` died this way, deterministically, in under 4 s and under 1 GB RSS — so it does NOT look like memory pressure or a `-j` artifact, it looks like a broken proof in a file you did not touch). `Z` literals are binary `Z.pos` trees (~log2 depth), so state the fact at `Z.of_nat (… )  = <literal>` instead (`rewrite Nat2Z.inj_mul` first if the LHS is a product) and close with `vm_compute; reflexivity` — the `nat`-side factors being unfolded stay small, and the literal itself is never forced into unary form.
  - **BUT THE TREE AS IT STANDS DOES NOT ALWAYS FOLLOW THAT RULE, SO `ulimit -s` IS A REAL BUILD PREREQUISITE.** `ProofWriteiParts.v` today carries BOTH forms: the `Z` one (`wi_maxfile_bsize`) *and* a `nat` twin `wi_maxfile_bsize_nat : (MAXFILE * BSIZE)%nat = 274432%nat` closed by bare `reflexivity`, because the loop's range bound lives at `nat`. Its comment argues the numeral notation keeps the literal as `Nat.of_num_uint`; that is true of the *statement* but not of the *conversion* `reflexivity` then has to run, so on an **8 MB stack (the common default — check with `ulimit -s`) the file dies with a bare `Error: Stack overflow.`** and takes the rest of the build with it. With `ulimit -s unlimited` the same file compiles in **7 s**. So: a stack overflow in a file you did not touch is not evidence of a broken tree — **raise the stack and re-run before investigating**, and set `ulimit -s unlimited` in any shell that runs a full build (including `nohup`/background ones). Restating that lemma in the `Z` form above would retire the requirement; nobody has.
