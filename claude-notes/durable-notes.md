...
- **The assumption audit is `make audit`, NOT part of `make proofs`.** `Print Assumptions` on the system theorem lives in `iris/SystemAssumptions.v` — a file `iris/_CoqProject` deliberately does not list (the commented-out row there is what keeps `proof_coverage.py --check` from calling it a stray). The statement alone measured 95 s of `SystemAdequacy.v`'s 98.6 s, on the serial build tail; CI runs it after every build and prints it in the run's step summary. Do not put it back in a `_CoqProject`-listed file, and do not add a second `Print Assumptions` beside it — consecutive calls share nothing, so a second one doubles the bill. [`optimization.md`](optimization.md) §"`Print Assumptions` is a whole-tree walk" has the measurements.
- **THE DEVICE-CONFORMANCE TESTS ARE IN CI, AND EVERY ONE OF THEM MUST PASS.** `.github/workflows/ci.yml`'s last step runs `make vtest-check-ci`: every test in `vtest-rocq/_CoqProject` compiled with `-k` against the CHECKED-IN QEMU captures (no QEMU, nothing regenerated), then `tools/vtest/vtest_status.py` writes the per-area table with each failure's error into the step summary and fails the job on any red test. There is no expected-red list because a known divergence is pinned on BOTH sides and proved unequal (`tools/vtest/README.md`, "Recording a divergence") — green today, red the day the model moves — so **a device change that "fixes" a pinned finding turns a vtest RED in CI, and the fix includes revisiting that test file.** The target deletes `vtest-rocq/*.vo` first on purpose: a failed recompile leaves the previous `.vo` in place (`.DELETE_ON_ERROR` takes the `.glob`, not an untouched `.vo`), so on a warm tree a newly red test would otherwise be reported green off the last run's output. ~600 s of CPU, one 33 s file wide.
...
- **A FUEL CONSTANT THAT IS A UNARY `nat` MAKES `Qed` DIVERGE, AND THE ONLY TELL IS A `Stack overflow` AT `Qed`.** `UkAbi.uk_slen_fuel : nat := Z.to_nat (2 ^ 31)` is a *term*, harmless until something forces it into constructor form. Nothing evaluates it directly — but the KERNEL will, on its own initiative, if you ever ask it to convert two *different* constants that both unfold to `uscan _ _ uk_slen_fuel`. Concretely: `uk_slens M av i` is `uk_slen M (uk_argv_p M av i)` by delta+beta, so `unfold uk_slens in H` looks free. It is not — `unfold … in H` leaves the proof term referring to `H` at its ORIGINAL type, so `Qed` hands the kernel `uk_slens M av i ≡? uk_slen M (uk_argv_p M av i)`, it unfolds both, lands on two `uscan` applications, and iota-reduces the fuel. With the default stack that is `Error: Stack overflow` **at the `Qed` line**, which reads like a big proof term; with `ulimit -s unlimited` it is an unbounded run instead, which is the real diagnosis (verified 2026-08-30, `iris/UEchoKernel.v`). Two rules: **state everything in the form the source lemma hands you** (`uk_slens M av i`, never the unfolded twin), and when a `Qed` overflows, re-run once with `ulimit -s unlimited` — if it turns into a hang rather than a pass, the term is not merely deep and you are looking for a conversion, not for a smaller proof. Bisect by splitting the lemma: each `Qed` then names itself.
- **A C POINTER CAST IN A COMMENT CLOSES THE COMMENT.** Writing `[wait((int *)0)]` or `[(char *)buf]` inside `(* … *)` ends the comment at the cast's `*)`, and the rest of the prose is then parsed as Rocq — which surfaces as `Syntax error: illegal begin of vernac` at whatever line the prose happens to reach, not at the cast. (Cost two build rounds on 2026-08-30, in `SpecKwait.v` and `UsysMemOk.v`.) Say "a null status pointer" rather than pasting the cast. The sibling trap is a `"` in a comment: Rocq starts a string, and you get `because it occurs in a non-terminated string of the comment` — a WARNING while the quotes are unbalanced only inside one comment, and a **`Syntax Error: Lexer: Unterminated string` AT END OF FILE** as soon as a later comment has the matching quote, because everything between the two is then one string. So the warning is the cheap tell and the error is hundreds of lines from its cause: grep the file for `"` inside `(* *)` rather than reading backwards from the reported line. (Bit twice on 2026-08-31 in `iris/UkShRun.v`, once each way.)
- **`vm_compute` ON A GOAL WITH A FREE VARIABLE HANGS THE SAME WAY — and `try` is how it sneaks in.** The evar case below is the famous one, but a *universally quantified* variable does it too, and the shape that produces it is a tidy-looking `destruct`-and-discharge: `assert (Ho : forall j, (j < 8)%nat -> uoff_sdsp (mword_of_int (Z.of_nat j)) = 8 * Z.of_nat j)` proved by `destruct j as [|[|…[|j]]]; try (vm_compute; reflexivity)`. The eight concrete cases are ground and fine; the RESIDUAL case still has a free `j`, `try` hands it to `vm_compute` anyway, and that one branch took a `rocqworker` to **31.5 GB in six minutes** with the file merely looking slow (verified 2026-08-30, `iris/UkEcho.v`). Two lessons: never let `try (vm_compute; …)` see a case you did not check is ground, and prefer N closed `assert`s over one quantified lemma when N is small — the closed form is also the one `rewrite` can actually match, since the goal carries `mword_of_int 7`, not `mword_of_int (Z.of_nat 7)`. Diagnose exactly as below: `run-on-gcp --no-sync ps -eo pid,etime,rss,comm --sort=-rss`, and read the culprit off `/proc/<pid>/cmdline`.
- **`eapply` + `vm_compute` IS A 37 GB HANG, and it looks like a slow file.** `vm_compute` on a goal carrying an EVAR is the same trap as the section-variable one below: it does not fail, it normalises forever. The shape that produces it is innocent — `eapply lem; [vm_compute; reflexivity | …]` where `lem`'s first premise determines an argument (`plic_prio_src off = Some ?i`) — and the symptom is a single `rocqworker` sitting at tens of GB with the build apparently stuck on whatever file `make` last announced. Diagnose with `ps -o pid,etime,rss -C rocqworker --sort=-rss` (on the build VM, `run-on-gcp --no-sync`); the RSS column names the file. Fix by giving the argument: `apply (lem p _ 10%N)`, which leaves the `vm_compute` goal ground.
- **A COMMENT-ONLY EDIT INVALIDATES A `.vo`, and the failure is the same "inconsistent assumptions" as a real change.** Coq's library digest covers source LOCATIONS, so adding a line of comment to `DevModel.v` produces a `.vo` with a different md5 than the byte-identical-in-content one already on disk, and every dependent built against the old one then refuses to load beside it (`Compiled library xv6iris.WpUart … makes inconsistent assumptions over library xv6iris.DevModel`). Two consequences: never `touch` a `.vo` to dodge a rebuild after "only a comment" (verified 2026-08-24 — the md5s differ), and never edit a file low in the cone while its dependents are compiling, or the run poisons itself the same way.
...
- **AFTER ANY `git pull`, THE IMAGE IS THE FIRST SUSPECT — and every way it goes wrong surfaces as the SAME bogus address failure** at the bottom of the tree (`Unable to unify "2147558264" with "2147558418"` in `ProcGeom.v` / `ColdBoot.v` / `BootReset.v`, taking all 145 `Code*.v` with it). It reads like a broken proof or a corrupt checkout and is neither. **Sequence after any pull that touches `kernel-rocq/`: `make xv6-rev-check` → `make kernel-rocq` → the `iris/` build.** Three independent causes, all with that one symptom:
  - `xv6-riscv/` is gitignored, so a pull that bumps `XV6_REV` leaves your clone on the OLD revision — and `make proofs`' dump rules then see an ELF newer than the freshly-checked-out `kernel-rocq/*.v` and **silently re-dump from the stale ELF, clobbering the tracked image**. Recover: `git checkout -- kernel-rocq/`, fix the clone (`git -C xv6-riscv fetch && git -C xv6-riscv checkout --detach $XV6_REV`), rebuild the ELF, `make dump-force`, and confirm `git status kernel-rocq/` is **clean** — a byte-identical re-dump is the proof that the toolchain and the revision agree.
    - **"Rebuild the ELF" MEANS `make -C xv6-riscv clean` FIRST, and skipping it fails in a way that names the wrong cause.** A plain `make -C xv6-riscv kernel/kernel` after a `checkout --detach` reuses object files compiled at the OLD revision, so the pin's `DETFLAGS` (`-ffile-prefix-map=$(CURDIR)=.`) never reaches them. `make dump-force` then dies on `rocq-raw: … embeds its own build directory (…/xv6-riscv), so this dump would differ between build trees`, which reads as "you are on a revision without the flag" when you are in fact on the right revision with a dirty build tree. Worse, `dump-force` does `rm -f` before regenerating, so the failure leaves the later dumps (`KernelElfRaw.v`, `FsImgRaw.v`, all of `user-rocq/`) DELETED rather than stale — `git checkout -- kernel-rocq/ user-rocq/` before retrying. The tell that the clean took: `grep -c ffile-prefix-map` on the xv6 build log is nonzero (53 at `4aab0eb8`).
    - The same clone goes stale **on the build VM**, independently and silently; `claude-notes/remote-build-gcp.md` has the remote form. Fixing both is worth it for its own sake: five scratch-path dumps (`--out /tmp/...`, never `dump-force`, which would move mtimes and rebuild the world) that `md5sum`-match the tracked files prove reproducibility on two machines sharing nothing but the pin.
  - **`kernel-rocq/*.vo` is rebuilt by nothing in `iris/`**, and neither `dump-force` nor `make -C iris -f CoqMakefile` has a rule for it — so new tracked sources sit beside whatever `.vo` you last built and every proof loads the OLD image. `make xv6-rev-check` and `make check-decode` both PASS (they read the `.v`), which is the tell. Compare `ls -la kernel-rocq/*.v kernel-rocq/*.vo` mtimes; it is a two-second check and it is the first thing to do. **`make vtest-deps` makes this WORSE before it makes it better**: it rebuilds exactly `KernelSyms/KernelInstrs/KernelData`, leaving `KernelElfRaw.vo` and `FsImgRaw.vo` from the previous image, and the resulting *half*-refreshed set fails in `ElfKernel.v` as `"eq_refl" has type "true = true" while it is expected to have type "bool_decide (elf_file_image kernel_elf = kernel_bytes ∪ kernel_data) = true"` — a new ELF image compared against an old raw dump. `make -C kernel-rocq -f CoqMakefile -j…` (its own CoqMakefile, so no dump rule can run) fixes it in seconds.
...
- **`List.rev` IS QUADRATIC, AND UNDER `vm_compute` ON WHOLE-IMAGE DATA THAT IS THE WHOLE BUDGET.** Stdlib's `rev` is `rev l ++ [x]`-shaped; one `rev` of a 55k list cost ~55 s inside a `vm_compute` whose every other step (55k `PrimString.get`s, a 55k-entry `gmap` build, the decidable equality) totalled ~3 s. Never build-then-reverse: build in final order with fuel counting DOWN and cons at the front, or `rev_append`. Worked shapes: `PStringBytes.pstring_hex_aux`, `ElfFile.elf_table`; the rest of the measured rules for computing over a whole binary image are in [`design/elf.md`](design/elf.md).
- **A `nat` EQUALITY WHOSE RHS IS A LARGE LITERAL NEEDS `Z`, NOT A BIGGER STACK.** Any route to closing such a goal — `reflexivity`, `vm_compute`, even `vm_cast_no_check` — eventually materializes a literal-deep unary successor chain and overflows a normal 8 MB stack outright, deterministically, in under 4 s and under 1 GB RSS. So it does NOT look like memory pressure or a `-j` artifact; it looks like a broken proof in a file you did not touch. `Z` literals are binary `Z.pos` trees (~log2 depth), so state the fact at `Z.of_nat (…) = <literal>` (`rewrite Nat2Z.inj_mul` first if the LHS is a product) and close with `vm_compute; reflexivity`.
  - **THE SAME TRAP FIRES ON AN INEQUALITY, AND ON `length`.** It is not
    specific to `=`, and the operands need not be literals — `Z.to_nat sz <=
    Z.to_nat (fs_nblk sz) * BSIZE` over a real file's `sz` is the same
    unary-chain blowup, and so is any `vm_compute` that reaches a `length`
    of an image-sized list. Both have canonical symbolic answers in tree:
    `FsImg.fs_nblocks_cover_nat` goes through `Nat2Z.inj_le` to
    `fs_nblk_cover` and costs nothing at any size, and `ElfUser.init_elf_length`
    (and its `sync`/`echo`/`sh` siblings) states the length at `Z` for exactly
    this reason. **Never `vm_compute` a `nat` goal whose operands come off a
    whole-image computation** — `apply Nat2Z.inj_le` first, then work in `Z`.
    The failure surfaces as `Error: Stack overflow` in a file you did not
    touch, and it is machine-dependent only in the sense that a bigger
    `ulimit -s` hides it.
...
- **THE DUMPED MAPS ARE CHUNKED, AND THAT IS WHAT KEEPS THEM OFF THE STACK LIMIT — DO NOT UN-CHUNK ONE.** A Rocq list literal `[e1; e2; …; eN]` is a right-nested `cons` chain and Rocq recurses once per element to build it, so a FLAT literal costs stack PROPORTIONAL TO N and blows up in whichever phase runs first (the reported location is the whole command when it dies in the parser, a single entry when it dies in the elaborator — same cause, two very different-looking errors). Until 2026-08-25 `kernel-rocq/KernelInstrs.v` ended in ONE `list_to_map [...]` of 23.7k entries and sat exactly on the boundary: `Error: Stack overflow` at the closing `].` was a COIN FLIP at `ulimit -s 8192` (1 failure in 3 identical runs) and CERTAIN at 4096 — which reads exactly like "the bump broke the dump" when it is neither the bump nor the file. `tools/dump_elf.py` now emits every per-byte/per-instruction map as `<name>_chunk0 … _chunkK` of `ROCQ_LIST_CHUNK` (= 1000) entries, rejoined by `list_to_map (List.concat [chunk0; …])`; `kernel_data` (11.8k entries, the measured 4 MB threshold) and the `user-rocq/` dumps get the same treatment. Measured on the VM: `KernelInstrs.v` compiles at a **1 MB** `ulimit -s` and got FASTER, 17.5 s → 13.7 s (`KernelData.v` 8.2 s → 6.5 s), `.vo` size unchanged. The chunks are plain `Definition`s, so reduction sees the same list and every `vm_compute` consumer is untouched; the ENTRY LINES are byte-identical, so the three regex readers of the dump (`tools/gen_code.py`, `tools/fix_proof_imms.py`, `tools/proof_coverage.py`) keep working and `make check-decode` stays empty across the reformat. Note this is NOT the `nat`-literal trap above (there is no unary chain here — it is list depth), and no `Z`-restating fixes it. If a dump ever overflows again, the lever is `ROCQ_LIST_CHUNK`, not `ulimit`.
- **`set_solver` WALKS THE WHOLE CONTEXT, SO ABSTRACTING THE GOAL IS ONLY HALF THE FIX.** `set … ; clearbody …` turns a goal over `pa_range`/`dom`/`range_map` atoms into eight opaque `gset` variables, and the goal then LOOKS trivial -- but `set_solver` also `set_unfold`s and case-splits every hypothesis in scope, and a context carrying a ∀→∃ fact over five conjuncts plus a dozen `dom`/`range_map` equations never returns (verified 2026-09-01, `VirtioProto.virtio_proto_deposit_acc`'s `Hhiff`: 12 s as a scratch file with a smaller context, unbounded inside the real proof). Follow the abstraction with `clear - <goal's variables>.` before `set_solver`/`naive_solver`. To find such a sentence in a 7000-line file, do not bisect serially: register N truncation probes (`lines 1..L` + `Abort.`) as separate `_CoqProject` files, `make -jN -k` them under one `timeout`, and read off which `.vo` appeared; a `Show.` before the suspect tactic in one probe says whether the goal or the context is the problem. One round of 13 probes pinned a 620-line lemma to a block, a second round of 4 to the sentence.
...
- **A COMPILE THAT NEVER FINISHES IS LOCALISED BY `coqc -time`, WHICH STREAMS** — the LAST LINE IN THE LOG IS THE STALLING SENTENCE, so redirect to a file and `tail` it. (Map `Chars A - B` to a line with `head -c B <f>.v | wc -l`.) **The failure mode to suspect first is a MIS-STATED `∀`-PREMISE**: a premise that binds variables it then does not use in its body (`∀ aq0 rl0, … <the OUTER aq rl> …`) can never be supplied, so `iMod` spins forever on the unification instead of failing. Grep for that shape; if the same names are in scope outside, Rocq's renaming (`aq0`) is the only hint in the error message, and only if you get one at all.
  - **The second shape to suspect is a DEFINITION THAT NOBODY EVER COMPUTES BUT THE UNIFIER WILL.** A `gset` built by a filter over a literal range — `list_to_set (List.filter f (seqZ 0 (8 * Z.of_nat 1024%nat)))` — is a perfectly good *specification* and a catastrophic *reduct*: `Z.of_nat` on a 1024-deep unary numeral, then an 8192-element list, then 8192 nested `gset` unions over a STUCK filter. Nothing needs its value; every consumer goes through lookup lemmas. But the tactic unifier delta-unfolds it while checking a **one-step** conversion between two spellings of the same set, and the compile then runs for fifteen minutes with no error. **`Global Opaque` any definition whose body is a big-op/fold over a literal-sized index range, right after its lookup lemmas** (`vm_compute` ignores `Opaque`, so an image sweep elsewhere in the tree is unaffected), and do not give it an abbreviation — a second name is a second delta step at every use site. Worked instance: `FsImg.fs_bmap_set`. **The better fix, where the
    definition has one EXPENSIVE subterm repeated by the indexing, is to
    spell the definition so that subterm occurs ONCE.** `FsImg.fs_slot_list`
    is the worked case: every `fs_slot` at an indirect index carries its own
    copy of `fs_ind_ents P dn`, so the natural `fs_slot <$> seq 0 269` form
    makes ONE conversion check expand 256 copies of a 256-element list and a
    single `f_equal` then runs for a quarter of an hour with no error; the
    concatenation form (`addrs[12] :: (direct fmap ++ fs_ind_ents P dn)`)
    computes the same list, keeps `unfold` usable, and is instant.
...
- **`destruct (decide P)` in one file does NOT reduce a `decide P` baked into ANOTHER file's definition** (seen on `InodeInv.blk_res`/`ind_blk`): the two `Decision P` instance terms differ even though they print identically, so the `if` survives the destruct — and a following `iIntros "_"` then silently discards the unreduced hypothesis instead of failing. Use `rewrite (decide_True _ _ H)` / `rewrite (decide_False _ _ H)`, where the instance is an evar and unifies with the definition's own.
- **Bare `NoDup` can resolve to Stdlib's `List.NoDup`, not stdpp's `base.NoDup`** — different inductives, and every stdpp lemma (`NoDup_cons_1_1`, `NoDup_seq`, `NoDup_elements`, …) is about stdpp's. In a file whose import chain pulls `Stdlib.Lists.List` on top (InodeInv.v and FsBoot.v both do), write `base.NoDup` explicitly in new statements; the symptom is `NoDup_*` lemmas that "don't apply" to a hypothesis that prints identically.
- **A new Iris leaf file needs `From iris.base_logic.lib Require Import iprop`** (or something that exports it) — with the proofmode alone, a `[∗ set] …, P ∗ Q` body fails to elaborate (`uPred (iResUR ?Σ)` vs `bi_car ?PROP1`).
- **Importing the iris proofmode re-opens `nat_scope` on top of the file's scope stack**, so a `Local Open Scope Z_scope` issued BEFORE the import silently loses: `=?`/`<=` become the nat operators and goals go wrong without a parse error. Re-issue `Local Open Scope Z_scope` after the import; and keep vanilla-`rewrite … by …` pure lemmas ABOVE the import (ssreflect rejects that form — see the bullet below).
...
- **`cbn` OR `set_solver` POINTED AT A PROJECTION OF A CONCRETE RECORD WHOSE FIELDS ARE EMPTY SETS/MAPS DOES NOT TERMINATE.** `VirtioQueue.vproto_ok_init` proves `vproto_ok c vproto0 …` where `vproto0 = VProto 0 0 0 None ∅ ∅ ∅ ∅ ∅`, and its obligations were discharged with `cbn in Hq` (no whitelist) and bare `set_solver` on goals like `vp_lo vproto0 ∉ vp_srv vproto0`. To reduce a projection those tactics normalise the WHOLE constructor application, empty `gset`/`gmap` fields included, and the file went from six minutes to **over an hour at a flat 1.96 GB**. The fix is to name every projection once, by `reflexivity`, and to `rewrite` with those facts only: `assert (Hsrv0 : vp_srv vproto0 = (∅ : gset nat)) by reflexivity`, then `rewrite Hsrv0` where it is needed. The same rule is why `vppq_*`/`vps_*` exist rather than `unfold …; cbn […]` at each use.
