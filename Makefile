...
#   make vtest-passes  build every run's TEST_PASSES instantiation and
#                      rewrite vtest-rocq/PASSING.txt with the ones that hold
...
vtest-passes: $(VTEST)/CoqMakefile
	-$(RUN) $(MAKE) -C $(VTEST) -f CoqMakefile -j$(JOBS) -k \
	  $$(cd $(VTEST) && ls *Pass.v | sed 's/\.v$$/.vo/')
	@cd $(VTEST) && { \
	  echo '# GENERATED by `make vtest-passes`.  One line per RUN whose'; \
	  echo '# [TEST_PASSES] instantiation COMPILES.  The table reads this so'; \
	  echo '# it is available without a build; the build rewrites it.'; \
	  for f in *Pass.v; do \
	    [ -f "$${f%.v}.vo" ] && echo "$${f%Pass.v}"; \
	  done; } > PASSING.txt
	$(PYTHON) tools/vtest/vtest.py table
