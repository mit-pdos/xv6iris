...
4. **Big literals.** FsDurImg reads the image decoder (`img_state`) and must never force the 2 MB literal: `Himg` is a premise (D34). Run CF under `ulimit -v 40000000; timeout 300` (memory note lean-runaway-memory).
