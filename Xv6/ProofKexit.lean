...
theorem kx_fileclose (FC : FsEntry filecloseAddr) (Γ : SchedNames) [ClaimIs (hlc := hlc) GF Γ]
    ...
    kctx c kk ∗ pcIs c KA.«fileclose» ∗ procsInv Γ ∗ trapCsrs c ∗ cpuClaim c (procAddr j) ∗
      intrRes c ∗
  ...
  rw [hp] at h
  rw [show filecloseAddr = KA.«fileclose» from rfl] at h
  exact h
...
theorem kx_loop (FC : FsEntry filecloseAddr) (Γ : SchedNames) [ClaimIs (hlc := hlc) GF Γ]
    ...
    (hΨ : ∀ (c' : CPU) (kk : KCtx), kxFrame kk j status spval availval →
        (kctx c' kk ∗ pcIs c' (KA.«kexit» + 0x4c#64) ∗ procsInv Γ ∗ trapCsrs c' ∗
    ...
    ∀ (n fd : Nat), fd + n + 1 = NOFILE → ∀ (c : CPU) (k : KCtx) (L : List (BitVec 64)),
      ...
      (kctx c k ∗ pcIs c (KA.«kexit» + 0x3e#64) ∗ procsInv Γ ∗ trapCsrs c ∗ cpuClaim c (procAddr j) ∗
  ...
  have hbody : ∀ (fd : Nat) (hfdlt : fd < NOFILE) (L : List (BitVec 64)) (hlen : L.length = NOFILE)
      (TT : ∀ (c'' : CPU) (k'' : KCtx), kxFrame k'' j status spval availval →
        ...
        (kctx c'' k'' ∗ pcIs c'' (KA.«kexit» + 0x38#64) ∗ procsInv Γ ∗ trapCsrs c'' ∗
          cpuClaim c'' (procAddr j) ∗ intrRes c'' ∗
    ...
    icases BigSepL.bigSepL_insert_acc
      ...
      with ⟨Hcell, Hback⟩
    -- ld a0,0(s1)
    k_step (wp_s_ld c _ (KA.«kexit» + 0x3e#64) true 0#12 10#5 9#5 (by decide) (by decide)
    ...
    -- beqz a0
    k_step (wp_s_branch c _ (KA.«kexit» + 0x40#64) true 8184#13 10#5 0#5 (by decide) bop.BEQ)
    ...
    · -- open file: fileclose, then null the slot
      ...
      -- jal fileclose
      k_step (wp_s_jal c _ (KA.«kexit» + 0x42#64) false 8412#21 1#5 (by decide))
        from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
        with [kexit_br_211e, KCtx.setReg_sie, KCtx.setReg_proc]
      iintro Hk Hpc
      -- fileclose(a0)
      have hfc := kx_fileclose (hlc := hlc) (GF := GF) FC Γ c
        ((k.setReg 10#5 (L[fd]'hfdlt2)).setReg 1#5 (KA.«kexit» + 0x46#64)) j hj
      ...
      have hkframe1 : kxFrame ((k.setReg 10#5 (L[fd]'hfdlt2)).setReg 1#5 (KA.«kexit» + 0x46#64)) j status spval availval :=
      ...
      ihave Hpc := (show pcIs (GF := GF) c2 (jumpPc (((k.setReg 10#5 (L[fd]'hfdlt2)).setReg 1#5
          (KA.«kexit» + 0x46#64)).regs 1#5)) ⊢ pcIs c2 (KA.«kexit» + 0x46#64) from by rw [hjump]) $$ Hpc
      -- sd zero,0(s1)
      k_step (wp_s_sd c2 _ (KA.«kexit» + 0x46#64) false 0#12 9#5 0#5 (by decide) (L[fd]'hfdlt2))
        from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
        with [KCtx.rget_withRegs', hR9, kx_add0, KCtx.rget_zero, KCtx.setReg_sie, KCtx.setReg_proc]
      iintro Hk Hpc Hcell
      ihave Hbig := Hback $$ %(0#64) Hcell
      -- j 0x80002136
      k_step (wp_s_j c2 _ (KA.«kexit» + 0x4a#64) true 2097134#21)
        from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  ...
  | zero =>
    ...
    icases kctx_kernelText _ _ $$ Hk with ⟨#Htext, Hk⟩
    -- addi s1,8
    k_step (wp_s_addi c'' _ (KA.«kexit» + 0x38#64) true 8#12 9#5 9#5 (by decide))
      from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
      with [KCtx.setReg_sie, KCtx.setReg_proc]
    iintro Hk Hpc
    -- beq s1,s2 (taken)
    k_step (wp_s_branch c'' _ (KA.«kexit» + 0x3a#64) false 18#13 9#5 18#5 (by decide) bop.BEQ)
      from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
...
theorem kx_rest (AC : ACQUIRE) (RE : RELEASE) (RP : REPARENT) (WU : WAKEUP) (SC : SCHED) [FsEnv]
  ...
  icases kctx_kernelText _ _ $$ Hk with ⟨#Htext, Hk⟩
  -- jal begin_op
  k_step (wp_s_jal c _ (KA.«kexit» + 0x4c#64) false 7192#21 1#5 (by decide))
  ...
  have hjp1 : jumpPc ((k.setReg 1#5 (KA.«kexit» + 0x50#64)).regs 1#5) = (KA.«kexit» + 0x50#64) := by
    rw [KCtx.setReg_regs, RegMap.set_apply, if_pos rfl]; decide
  ihave Hpc := (show pcIs (GF := GF) c1 (jumpPc ((k.setReg 1#5 (KA.«kexit» + 0x50#64)).regs 1#5)) ⊢
      pcIs c1 (KA.«kexit» + 0x50#64) from by rw [hjp1]) $$ Hpc
  have hf1 := kxFrame_cross hf1 spie1 spp1 R1 hcs1
  -- ld a0,336(s3)
  k_step (wp_s_ld c1 _ (KA.«kexit» + 0x50#64) false 336#12 10#5 19#5 (by decide) (by decide)
      (DFrac.own 1) V.cwd)
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  ...
  -- jal iput
  k_step (wp_s_jal c1 _ (KA.«kexit» + 0x54#64) false 4904#21 1#5 (by decide))
  ...
  have hf1b : kxFrame (((k.setReg 1#5 (KA.«kexit» + 0x50#64)).withSpie spie1 spp1).withRegs
      ((R1.set 10#5 V.cwd).set 1#5 (KA.«kexit» + 0x58#64))) j status spval availval :=
    kx_setReg_frame _ j status spval 1#5 (KA.«kexit» + 0x58#64) hf1a
      (by decide) (by decide) (by decide) (by decide)
  iapply (kx_fs iputAddr KA.«iput» rfl FsEnv.iput Γ status spval availval c1 _ j hj hf1b)
    $$ [- $Hk $Hpc $Hpinv $Htc $Hclaim $Hres]
  iapply wpNext_intro_pin
  iintro %c2 %hpin2
  iintro %spie2 %spp2 %R2 Hk Hpc Htc Hclaim Hres %hcs2
  ihave Hpc := (kx_pcIs_jump c2 _ (R1.set 10#5 V.cwd) (KA.«kexit» + 0x58#64) (by decide)) $$ Hpc
  have hf2 := kxFrame_cross hf1b spie2 spp2 R2 hcs2
  -- jal end_op
  k_step (wp_s_jal c2 _ (KA.«kexit» + 0x58#64) false 7320#21 1#5 (by decide))
  ...
  ihave Hpc := (kx_pcIs_jump c3 _ R2 (KA.«kexit» + 0x5c#64) (by decide)) $$ Hpc
  have hf3 := kxFrame_cross hf2a spie3 spp3 R3 hcs3
  have h19R3 : R3 19#5 = procAddr j := hf3.2.2.2.2.2.2.2.1
  -- sd zero,336(s3): p->cwd = 0
  k_step (wp_s_sd c3 _ (KA.«kexit» + 0x5c#64) false 336#12 19#5 0#5 (by decide) V.cwd)
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  ...
  k_step (wp_s_auipc c3 _ (KA.«kexit» + 0x60#64) false 16#20 10#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc] with [KCtx.setReg_sie, KCtx.setReg_proc]
  iintro Hk Hpc
  k_step (wp_s_addi c3 _ (KA.«kexit» + 0x64#64) false 746#12 10#5 10#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
    with [kexit_br_1034a, KCtx.rget_eq, kx_wl_addr1, KCtx.setReg_sie, KCtx.setReg_proc]
  iintro Hk Hpc
  k_step (wp_s_jal c3 _ (KA.«kexit» + 0x68#64) false 2091762#21 1#5 (by decide))
  ...
  have hacw : ∀ (k' : KCtx) (hsie' : k'.sie = false) (hnoff' : k'.noff + 1 < 2 ^ 31)
      ...
      kctx c3 k' ∗ pcIs c3 KA.«acquire» ∗ isLock γw waitLockAddr "wait_lock" waitLockPay ∗
      (∀ R' : RegMap,
  ...
  ihave Hpc := (kx_pcIs_jump c3 _ _ (KA.«kexit» + 0x6c#64) (by decide)) $$ Hpc
  ...
  -- c.mv a0,s3
  k_step (wp_s_add c3 _ (KA.«kexit» + 0x6c#64) true 10#5 0#5 19#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
    with [KCtx.rget_eq, hW19, KCtx.rget_zero]
  iintro Hk Hpc
  -- jal reparent
  k_step (wp_s_jal c3 _ (KA.«kexit» + 0x6e#64) false 2096956#21 1#5 (by decide))
  ...
  subst c5
  ihave Hpc := (kx_pcIs_jump c3 _ _ (KA.«kexit» + 0x72#64) (by decide)) $$ Hpc
  ...
  k_step (wp_s_ld c3 _ (KA.«kexit» + 0x72#64) false 56#12 10#5 19#5 (by decide) (by decide) (DFrac.own 1)
  ...
  -- jal wakeup
  k_step (wp_s_jal c3 _ (KA.«kexit» + 0x76#64) false 2096846#21 1#5 (by decide))
  ...
  subst c6
  ihave Hpc := (kx_pcIs_jump c3 _ _ (KA.«kexit» + 0x7a#64) (by decide)) $$ Hpc
  ...
  k_step (wp_s_add c3 _ (KA.«kexit» + 0x7a#64) true 10#5 0#5 19#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
    with [KCtx.rget_eq, hd6_19, KCtx.rget_zero]
  iintro Hk Hpc
  k_step (wp_s_jal c3 _ (KA.«kexit» + 0x7c#64) false 2091742#21 1#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc] with [kexit_br_ffffffffffffeb5a, KCtx.setReg_sie, KCtx.setReg_proc]
  iintro Hk Hpc
  ...
  have hacp : ∀ (k' : KCtx) (hsie' : k'.sie = false) (hnoff' : k'.noff + 1 < 2 ^ 31)
      ...
      kctx c3 k' ∗ pcIs c3 KA.«acquire» ∗ isLock (Γ.lock j) (procAddr j) "proc" (procLockPay Γ j) ∗
      (∀ R' : RegMap,
  ...
  ihave Hpc := (kx_pcIs_jump c3 _ _ (KA.«kexit» + 0x80#64) (by decide)) $$ Hpc
  ...
  k_step (wp_s_sw c3 _ (KA.«kexit» + 0x80#64) false 44#12 19#5 20#5 (by decide) xs)
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
    with [KCtx.rget_eq, hR7_19, kx_pXstate]
  iintro Hk Hpc Hxstate
  -- c.li a5,5
  k_step (wp_s_addi c3 _ (KA.«kexit» + 0x84#64) true 5#12 15#5 0#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc] with [KCtx.rget_eq]
  iintro Hk Hpc
  -- sw a5,24(s3): p->state = ZOMBIE
  k_step (wp_s_sw c3 _ (KA.«kexit» + 0x86#64) false 24#12 19#5 15#5 (by decide) RUNNING)
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  ...
  k_step (wp_s_auipc c3 _ (KA.«kexit» + 0x8a#64) false 16#20 10#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc] with [KCtx.setReg_sie, KCtx.setReg_proc]
  iintro Hk Hpc
  k_step (wp_s_addi c3 _ (KA.«kexit» + 0x8e#64) false 704#12 10#5 10#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
    with [kexit_br_1034a, KCtx.rget_eq, kx_wl_addr2, KCtx.setReg_sie, KCtx.setReg_proc]
  iintro Hk Hpc
  k_step (wp_s_jal c3 _ (KA.«kexit» + 0x92#64) false 2091856#21 1#5 (by decide))
  ...
  have hrew : ∀ (k' : KCtx) (hsie' : k'.sie = false) (hnoff' : 1 ≤ k'.noff)
      ...
      kctx c3 k' ∗ pcIs c3 KA.«release» ∗ isLock γw waitLockAddr "wait_lock" waitLockPay ∗
  ...
  ihave Hpc := (kx_pcIs_jump c3 _ _ (KA.«kexit» + 0x96#64) (by decide)) $$ Hpc
  -- jal sched
  k_step (wp_s_jal c3 _ (KA.«kexit» + 0x96#64) false 2096474#21 1#5 (by decide))
  ...
  have hsc : ∀ (k' : KCtx) (ch' : BitVec 64) (hK' : schedSlots ≤ k'.avail) (hsie' : k'.sie = false)
      ...
      kctx c3 k' ∗ pcIs c3 KA.«sched» ∗ procsInv Γ ∗ procHeld Γ c3 j ZOMBIE ch' ∗
...
theorem kexit_proof (MP : MYPROC) (AC : ACQUIRE) (RE : RELEASE) (RP : REPARENT) (WU : WAKEUP)
  ...
  k_step (wp_s_sd cpu _ (KA.«kexit» + 0x2#64) true 40#12 2#5 1#5 (by decide) w0)
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  iintro Hk Hpc F0
  k_step (wp_s_sd cpu _ (KA.«kexit» + 0x4#64) true 32#12 2#5 8#5 (by decide) w1)
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  iintro Hk Hpc F1
  k_step (wp_s_sd cpu _ (KA.«kexit» + 0x6#64) true 24#12 2#5 9#5 (by decide) w2)
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  iintro Hk Hpc F2
  k_step (wp_s_sd cpu _ (KA.«kexit» + 0x8#64) true 16#12 2#5 18#5 (by decide) w3)
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  iintro Hk Hpc F3
  k_step (wp_s_sd cpu _ (KA.«kexit» + 0xa#64) true 8#12 2#5 19#5 (by decide) w4)
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  iintro Hk Hpc F4
  k_step (wp_s_sd cpu _ (KA.«kexit» + 0xc#64) true 0#12 2#5 20#5 (by decide) w5)
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  iintro Hk Hpc F5
  k_step (wp_s_addi cpu _ (KA.«kexit» + 0xe#64) true 48#12 8#5 2#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  iintro Hk Hpc
  -- c.mv s4,a0: s4 = status (= a0 at entry)
  k_step (wp_s_add cpu _ (KA.«kexit» + 0x10#64) true 20#5 0#5 10#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc] with [KCtx.rget_eq, KCtx.rget_zero]
  iintro Hk Hpc
  -- jal myproc
  k_step (wp_s_jal cpu _ (KA.«kexit» + 0x12#64) false 2095224#21 1#5 (by decide))
  ...
  k_step (wp_s_add cpu _ (KA.«kexit» + 0x16#64) true 19#5 0#5 10#5 (by decide))
  ...
  k_step (wp_s_auipc cpu _ (KA.«kexit» + 0x18#64) false 8#20 15#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  iintro Hk Hpc
  ...
  k_step (wp_s_addi cpu _ (KA.«kexit» + 0x20#64) false 208#12 9#5 10#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc] with [KCtx.rget_eq, h10]
  iintro Hk Hpc
  -- addi s2,a0,336: s2 = &cwd
  k_step (wp_s_addi cpu _ (KA.«kexit» + 0x24#64) false 336#12 18#5 10#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc] with [KCtx.rget_eq, h10]
  iintro Hk Hpc
  -- bne a5,a0: p != initproc, so jump to the loop at 0x8000213c
  k_step (wp_s_branch cpu _ (KA.«kexit» + 0x28#64) false 22#13 15#5 10#5 (by decide) bop.BNE)
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
