...
theorem dirlookup_short (PA : PANIC) (c : CPU) (k : KCtx) (spie spp : Bool) (R : RegMap)
    (hK : dirlookupSlots ≤ k.avail) (hsie : k.sie = false) (hnoff : k.noff = 0)
    (hlocks : k.locks = []) :
    kctx c (((k.withSpie spie spp).pushed 12).withRegs R) ∗ pcIs c (KA.«dirlookup» + 0x46#64) ∗
    panicEnv
    ⊢ wpLoop (GF := GF) c := by
  iintro ⟨Hk, Hpc, #Hpe⟩
  ...
  k_step (wp_s_auipc c _ (KA.«dirlookup» + 0x46#64) false 4#20 10#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  iintro Hk Hpc
  k_step (wp_s_addi c _ (KA.«dirlookup» + 0x4a#64) false 3002#12 10#5 10#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  iintro Hk Hpc
  k_step (wp_s_jal c _ (KA.«dirlookup» + 0x4e#64) false 2084618#21 1#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc] with [dirlookup_br_panic]
  iintro Hk Hpc
  iapply (dirlookup_panic PA c _ ?pa ?pK ?pn ?ppr ?pu) $$ [$Hk $Hpc $Hpe $Hmsg]
...
theorem dirlookup_read (RD : READI) (NC : NAMECMP) (IG : IGET) (PA : PANIC)
    ...
    kctx c (((k.withSpie spie spp).pushed 12).withRegs R) ∗ pcIs c (KA.«dirlookup» + 0x5c#64) ∗
  ...
  iintro ⟨Hk, Hpc, Hframe, Hde, Htc, Hcl, Hir, Hkeep, Hin, #Henv, Hnext, IH⟩
  icases kctx_kernelText _ _ $$ Hk with ⟨#Htext, Hk⟩
  -- +0x5c .. +0x64  the arguments
  k_step (wp_s_add c _ (KA.«dirlookup» + 0x5c#64) true 14#5 0#5 19#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  iintro Hk Hpc
  k_step (wp_s_add c _ (KA.«dirlookup» + 0x5e#64) true 13#5 0#5 9#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  iintro Hk Hpc
  k_step (wp_s_add c _ (KA.«dirlookup» + 0x60#64) true 12#5 0#5 20#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  iintro Hk Hpc
  k_step (wp_s_addi c _ (KA.«dirlookup» + 0x62#64) true 0#12 11#5 0#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  iintro Hk Hpc
  k_step (wp_s_add c _ (KA.«dirlookup» + 0x64#64) true 10#5 0#5 18#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  iintro Hk Hpc
  k_step (wp_s_jal c _ (KA.«dirlookup» + 0x66#64) false 2096524#21 1#5 (by decide))
  ...
  iapply (dirlookup_readi RD Γ c _ γl pd pav pu j γkl γk ip bm data dn (16 * i) bs pidv dqp dqd
      hs.hj ?gproc ?gK ?gsie ?gnoff ?glocks ?gtier hs.hgeom hs.hwf hs.hcov hs.hsz (by omega) hpd
      ?ga0 ?ga1 ?ga3 ?ga4 hbl)
    $$ [- $Hk $Hpc]
  rotate_right 1
  ...
  case gsie => k_norm_g; exact hsie
  case gnoff => k_norm_g; exact hs.hnoff
  case glocks => k_norm_g; exact hs.hlocks
  ...
  iintro %c1 %hp1 %spie1 %spp1 %R1 %tot %hcs1 %⟨hra0, htot⟩ Hk Hpc Htc Hcl Hir Hdev Hmeta Hmap
    Hblk Hbuf Hpid Hbsl
  ...
  have hr1 : dirlookupRegs k ip R1 i :=
    ⟨b2.trans r2, b8.trans r8, b9.trans r9, b18.trans r18, b19.trans r19, b20.trans r20,
      ...
      b27.trans r27⟩
  have hpin := dirlookup_pin hs.hj k hs.hproc c1 cpu
  ...
  · -- THE SHORT READ: dirlookup DIVERGES
    ...
    k_step (wp_s_branch c1 _ (KA.«dirlookup» + 0x6a#64) false 8156#13 10#5 19#5 (by decide)
        bop.BNE)
      from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
      with [hra0, b19, r19, hbne tot (by omega), decide_eq_true hne]
    iintro Hk Hpc
    iapply (dirlookup_short PA c1 k spie1 spp1 _ hs.hK hsie hs.hnoff hs.hlocks)
      $$ [$Hk $Hpc $Hpe]
  ...
  k_step (wp_s_branch c1 _ (KA.«dirlookup» + 0x6a#64) false 8156#13 10#5 19#5 (by decide) bop.BNE)
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  ...
  iapply (dirlookup_name NC IG Γ c1 cpu k spie1 spp1 R1 j γl pd pav pu γkl γk ip dinum bm data dn
      dr fn hasp pofv pidv dqp dqd dqn i fuel v10 hs hr1 hlt hrec hnone hfu hpin)
    $$ [$Hk $Hpc $Hframe $Hde $Htc $Hcl $Hir $Hkeep $Hin $Henv $Hnext $IH]
...
theorem dirlookup_loop (RD : READI) (NC : NAMECMP) (IG : IGET) (PA : PANIC)
    ...
    ∀ fuel : Nat, dirlookupEnv (hlc := hlc) (GF := GF) Γ γl pd pav pu γkl γk -∗
      dirlookupLoop cpu k ip dinum bm data dn dr fn hasp pofv pidv dqp dqd dqn fuel := by
  intro fuel
  induction fuel with
  | zero =>
    iintro #Henv
  ...
  | succ f ih =>
    iintro #Henv
    iapply dirlookupLoop_intro
    iintro %c %spie %spp %R %i %v10 %bs %⟨hr, hlt, hnone, hfu⟩ Hk Hpc Hframe Hde Htc Hcl Hir
      Hkeep Hin Hnext
    ihave IH := ih $$ Henv
    iapply (dirlookup_read RD NC IG PA Γ c cpu k spie spp R j γl pd pav pu γkl γk ip dinum bm data
