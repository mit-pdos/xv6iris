...
`umBelow V.sz P'` that `procPriv` carries cannot be re-established here.
The user arm therefore hands the private block back as `procPrivExt`,
which is `procPriv` minus exactly that conjunct; `procPrivExt_close`
turns it back into `procPriv` for a caller that knows the bound, and
...
`procPriv` with `umBelow V.sz V.upt` dropped and the table at `P'`. -/
...
theorem procPriv_to_ext (pa : BitVec 64) (pid : BitVec 32) (V : ProcPriv)
    (M : Nat → List (BitVec 8)) :
    procPriv (GF := GF) pa pid V M ⊢ procPrivExt pa pid V V.upt M := by
  unfold procPriv procPrivExt
...
theorem procFields_upt (pa : BitVec 64) (dq : DFrac) (V : ProcPriv) (P' : UPtd) :
    procFields (GF := GF) pa dq { V with upt := P' } = procFields pa dq V := rfl
...
theorem procPrivExt_close (pa : BitVec 64) (pid : BitVec 32) (V : ProcPriv) (P' : UPtd)
    ...
    procPrivExt (GF := GF) pa pid V P' M' ⊢ procPriv pa pid { V with upt := P' } M' := by
  unfold procPriv procPrivExt
  simp only [procFields]
  iintro ⟨%hf, Hpid, Hfields, Hpt, Htf⟩
  isplitl []
...
def wp_either_copyout_body {hlc : HasLC} {GF : BundledGFunctors} [MachGS hlc GF] [Xv6G GF] [CurCtx]
    ...
    (V : ProcPriv) (M : Nat → List (BitVec 8)) (user : Bool) (dqs : DFrac)
  ...
  (if user then procPriv (procAddr j) pid V M else byteBuf (k.regs 11#5) (DFrac.own 1) olds) ∗
...
structure EITHER_COPYOUT : Prop where
  wp_either_copyout : ∀ {hlc : HasLC} {GF : BundledGFunctors} [MachGS hlc GF] [Xv6G GF] [CurCtx]
    (cpu : CPU) (k : KCtx) (γl : GName) (γk : KmemNames) (j : Nat) (pid : BitVec 32)
    (V : ProcPriv) (M : Nat → List (BitVec 8)) (user : Bool) (dqs : DFrac) (bs olds : List (BitVec 8))
    hj hproc hnoff hK hlk huser hlen hlen' holds,
    wp_either_copyout_body (hlc := hlc) (GF := GF) cpu k γl γk j pid V M user dqs bs olds
      hj hproc hnoff hK hlk huser hlen hlen' holds
...
def wp_either_copyin_body {hlc : HasLC} {GF : BundledGFunctors} [MachGS hlc GF] [Xv6G GF] [CurCtx]
  ...
  (if user then procPriv (procAddr j) pid V M else byteBuf (k.regs 12#5) dqs bs) ∗
  wpNext k.sie k.proc cpu (fun cpu' => iprop(∀ spie : Bool, ∀ spp : Bool, ∀ R' : RegMap,
    ⌜k.sie = false → spie = k.spie ∧ spp = k.spp⌝ -∗
    kctx cpu' ((k.withSpie spie spp).withRegs R') -∗ pcIs cpu' (jumpPc (k.regs 1#5)) -∗
    (if user then
      (∃ (P' : UPtd) (bs' : List (BitVec 8)),
        ⌜V.upt.ext P' ∧
          ((R' 10#5 = 0#64 ∧ bs' = umemRead (viewFaulted V.upt P' M) (k.regs 12#5).toNat old.length) ∨
           (R' 10#5 = -1#64 ∧ ∃ d, d ≤ old.length ∧
              bs' = umemRead (viewFaulted V.upt P' M) (k.regs 12#5).toNat d ++ old.drop d))⌝ ∗
...
structure EITHER_COPYIN : Prop where
  wp_either_copyin : ∀ {hlc : HasLC} {GF : BundledGFunctors} [MachGS hlc GF] [Xv6G GF] [CurCtx]
    (cpu : CPU) (k : KCtx) (γl : GName) (γk : KmemNames) (j : Nat) (pid : BitVec 32)
    (V : ProcPriv) (M : Nat → List (BitVec 8)) (user : Bool) (dqs : DFrac) (bs old : List (BitVec 8))
    hj hproc hnoff hK hlk huser hlen hlen' hbs,
    wp_either_copyin_body (hlc := hlc) (GF := GF) cpu k γl γk j pid V M user dqs bs old
      hj hproc hnoff hK hlk huser hlen hlen' hbs
