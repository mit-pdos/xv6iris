...
theorem dirlookup_setup (RD : READI) (NC : NAMECMP) (IG : IGET) (PA : PANIC)
    ...
    (c cpu : CPU) (k : KCtx) (γl : GName) (pd pav pu : BitVec 64) (j : Nat)
    ...
    (hc : c = cpu) (ha0 : k.regs 10#5 = ip)
    (hX : X = (k.regs.set 2#5 (k.regs 2#5 + 0xFFFFFFFFFFFFFFA0#64)).set 8#5 (k.regs 2#5)) :
    kctx c ((k.pushed 12).withRegs X) ∗ pcIs c (KA.«dirlookup» + 0x16#64) ∗
  ...
  iintro ⟨Hk, Hpc, Hframe, Hde, Htc, Hcl, Hir, Hkeep, Hin, #Henv, Hnext⟩
  ...
  k_step (wp_s_lh c _ (KA.«dirlookup» + 0x16#64) false 68#12 14#5 10#5 (by decide) (by decide)
      (DFrac.own 1) dn.diType)
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc] with [ha0, iType]
  iintro Hk Hpc Hty
  ihave Hmeta := Hmcl $$ Hty
  k_step (wp_s_addi c _ (KA.«dirlookup» + 0x1a#64) true 1#12 15#5 0#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  iintro Hk Hpc
  k_step (wp_s_branch c _ (KA.«dirlookup» + 0x1c#64) false 30#13 14#5 15#5 (by decide) bop.BNE)
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc] with [hty, dirlookup_bne_type]
  iintro Hk Hpc
  -- +0x20  c.mv s2,a0 ; +0x22  c.mv s5,a1 ; +0x24  c.mv s7,a2
  k_step (wp_s_add c _ (KA.«dirlookup» + 0x20#64) true 18#5 0#5 10#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  iintro Hk Hpc
  k_step (wp_s_add c _ (KA.«dirlookup» + 0x22#64) true 21#5 0#5 11#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  iintro Hk Hpc
  k_step (wp_s_add c _ (KA.«dirlookup» + 0x24#64) true 23#5 0#5 12#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  iintro Hk Hpc
  -- +0x26  c.lw a5,76(a0)
  icases dirlookup_meta_size ip dn $$ Hmeta with ⟨Hsz, Hmcl⟩
  k_step (wp_s_lw c _ (KA.«dirlookup» + 0x26#64) true 76#12 15#5 10#5 (by decide) (by decide)
  ...
  k_step (wp_s_addi c _ (KA.«dirlookup» + 0x28#64) true 0#12 9#5 0#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  iintro Hk Hpc
  k_step (wp_s_addi c _ (KA.«dirlookup» + 0x2a#64) false 4000#12 20#5 8#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  iintro Hk Hpc
  k_step (wp_s_addi c _ (KA.«dirlookup» + 0x2e#64) true 16#12 19#5 0#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  iintro Hk Hpc
  k_step (wp_s_addi c _ (KA.«dirlookup» + 0x30#64) false 4002#12 22#5 8#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  iintro Hk Hpc
  k_step (wp_s_addi c _ (KA.«dirlookup» + 0x34#64) true 0#12 10#5 0#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  iintro Hk Hpc
  ...
  have hpin := dirlookup_pin hs.hj k hs.hproc c c
  -- +0x36  c.bnez a5,+0x5c
  by_cases hz : dn.diSize.toNat = 0
  · k_step (wp_s_branch c _ (KA.«dirlookup» + 0x36#64) true 38#13 15#5 0#5 (by decide) bop.BNE)
    ...
    k_step (wp_s_j c _ (KA.«dirlookup» + 0x38#64) true 94#21)
      from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
    iintro Hk Hpc
    ...
    iapply (dirlookup_tail c c k k.spie k.spp _ ip dinum bm data dn dr fn hasp pofv pidv dqp dqd
        dqn false 0 0 1 v10 bs 0#64
        (by have := hs.hK; unfold dirlookupSlots at this; omega) hsie hs.hal ?t2 ?t10 ?t24 ?t25
        ?t26 ?t27 hpin)
  ...
  · k_step (wp_s_branch c _ (KA.«dirlookup» + 0x36#64) true 38#13 15#5 0#5 (by decide) bop.BNE)
    ...
    ihave IH := dirlookup_loop RD NC IG PA Γ c k j γl pd pav pu γkl γk ip dinum bm data dn dr fn
    ...
    iapply IH $$ %c %k.spie %k.spp %_ %0 %v10 %bs [] Hk Hpc Hframe Hde Htc Hcl Hir Hkeep Hin Hnext
    ipureintro
...
theorem dirlookup_main (RD : READI) (NC : NAMECMP) (IG : IGET) (PA : PANIC)
  ...
  iintro ⟨Hk, Hpc, #Hpi, Htc, Hcl, Hir, #Hpe, #Hbc, #Hdc, #Hkl, #Hav, Hdev, Hmeta, Hmap, Hblk,
    Hnm, Hpf, Hpid, Hbsl, #Hit2, #Hiti, #Hinv, Hsl, Hlk, Hdi, Hnext⟩
  icases kctx_kernelText _ _ $$ Hk with ⟨#Htext, Hk⟩
  ihave Hnext := dirlookup_post_of_spec cpu k ip dinum bm data dn dr fn hasp pofv pidv dqp dqd
    dqn $$ Hnext
  ihave Hkeep : dirlookupKeep k ip dinum bm data dn dr fn pidv dqp dqd dqn
    $$ [Hdev Hmeta Hmap Hblk Hnm Hpid Hbsl Hlk Hdi]
  · unfold dirlookupKeep; iframe
  ...
  iintro %c1 %hp1 Hk Hpc ⟨%w9, %w10, %w11, Hframe⟩
  ...
  k_norm
  subst hc1
  iapply (dirlookup_setup RD NC IG PA Γ _ _ k γl pd pav pu j γkl γk ip dinum bm data dn dr fn
      hasp pofv pidv dqp dqd dqn _ w9 _ hs hpd rfl ha0 rfl)
    $$ [$Hk $Hpc $Hframe $Hde $Htc $Hcl $Hir $Hkeep $Hin $Henv $Hnext]
...
theorem dirlookup_proof (RD : READI) (NC : NAMECMP) (IG : IGET) (PA : PANIC) : DIRLOOKUP :=
  ⟨fun Γ _ cpu k γl pd pav pu j γkl γk ip dinum bm data dn dr fn hasp pofv pidv dqp dqd dqn hj
    hproc hK hsie hnoff hlocks htier htype hgeom hwf hcov hsz hholes hinums hdisj horph hdrnz
    hdrnl hpd ha0 hpoff =>
  dirlookup_main RD NC IG PA Γ cpu k γl pd pav pu j γkl γk ip dinum bm data dn dr fn hasp pofv pidv
    dqp dqd dqn hj hproc hK hsie hnoff hlocks htier htype hgeom hwf hcov hsz hholes hinums hdisj
