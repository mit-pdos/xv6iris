...
(* translate-abstract wrapper: whatever makes [translate] error with
   Needs_Update (a TLB hit or the walk above), [translateAddr] for a
   fetch turns it into [E_Fetch_Page_Fault], no state change *)
Lemma exec_translateAddr_fetch_u_needs_update_of_translate
    (vpn : mword 27) (root : mword 44) (mxr do_sum : bool) (va satp0 : mword 64) s :
...
Proof.
  intros Hcp HSXL Hsatp Hmode Hasid Hppn Htranslate Hcanon Hvpn_def.
    ...
    { destruct (vec_access_dec tlbvec (tlb_hash (__id 39) vpn)) as [ent|] eqn:Hslot.
      ...
      - (* empty slot: lookup misses, walk the page table *)
        ...
        exact (exec_translate_walk_user_needs_update vpn root (uw_pte2 i) (uw_pte1 i) (uw_pte0 i)
                 false false H2i H2nl H1i H1nl H0i H0nl (fun s => Hchk false false s) H0N
                 (mword_of_int 0) pte' sigma Lmisa Hlk Hupd' Hrd2 Hrd1 Hrd0 Lmenv). }
    exact (exec_translateAddr_fetch_u_needs_update_of_translate vpn root false false va satp0 sigma
             Lpriv LSXL Lsatp Hsatpmode Hasid Hroot Htranslate Hcanon Hvpn_def).
  Qed.
