...
Lemma il_align_arith (kk : Z) :
  0 <= kk -> kk < 30 ->
  (2147582464 + 1112 * kk + 88) `mod` 4 = 0
  /\ 0 <= 2147582464 + 1112 * kk + 88
  /\ 2147582464 + 1112 * kk + 88 < 18446744073709551616.
Proof.
  intros H1 H2. split_and!; [| lia | lia].
  replace (2147582464 + 1112 * kk + 88)
    with ((536895638 + 278 * kk) * 4) by lia.
  apply Z_mod_mult.
Qed.
...
Proof.
  ...
  destruct (il_align_arith (Z.of_nat k)
    ...
    as (Hm & Hlo & Hhi).
  replace (0x800181e8 + 24 + 1112 * Z.of_nat k + Z.of_nat 88)
    with (2147582464 + 1112 * Z.of_nat k + 88) by lia.
...
Section ProofInitlog.
  ...
  Proof.
    ...
    iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.initlog + 0x1e)) Ra1 Ra1
              (mword_of_int 2436 : mword 12) R7 (K - 6)%nat b
              ltac:(vm_compute; discriminate) ltac:(rdok)
              with "Hcg Hpc Hi1e [-]").
    iIntros (CID13 Hs13) "Hcg Hpc".
    set (R8 := <[Regidx Ra1 := regval_into_reg
                  (add_vec (rget R7 Ra1)
                     (sign_extend' 64 (mword_of_int 2436 : mword 12)))]> R7).
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.initlog + 0x24)) Rra
              (mword_of_int 2084798 : mword 21) R9 (K - 6)%nat b
              ltac:(vm_compute; discriminate) ltac:(rdok)
    ...
    assert (Htgtil : add_vec (mword_of_int (KernelSyms.initlog + 0x24) : mword 64)
                       (sign_extend' 64 (mword_of_int 2084798 : mword 21))
