...
(* THE MINT JUSTIFICATION (header, THE ONE DELTA): the write delta cannot
   ...
   reuse refused. *)
Lemma delta_write_no_shrink (av : aview) (i : Z) (off : nat)
...
Section SysOpenAU.
  ...
  Definition aopen_commit Γ (E : coPset)
      (Φ : aview -> Z -> anode -> iProp Σ) : iProp Σ :=
    (∀ (av : aview) (i : Z) (a : anode),
       ⌜av !! i = Some a⌝ -∗
       astate Γ av ={E}=∗ astate Γ av ∗ Φ av i a)%I.

  Lemma aopen_commit_at_weaken Γ E Φ :
  ...
  Lemma aopen_commit_at_unit Γ E :
  ...
  Lemma aopen_commit_at_pinned Γ E (q : Qp) (jpin : Z) (b : anode)
  ...
  Lemma atrunc_commit_at_unit Γ E :
  ...
  Lemma atrunc_commit_at_pinned Γ E (q : Qp) (jpin : Z) (b : anode)
  ...
  (* ONE SHOT, instantiated by the walk at the string argstr fetched and
     ...
     THE WALK PREMISE). *)
  Definition open_walk_pre_era (γfs : fs_names)
  ...
  Definition open_au_pre_plain Γ (γfs : fs_names)
  ...
  Definition open_au_pre_create Γ (γfs : fs_names)
      (P Pmiss : nat -> Z -> iProp Σ)
  ...
  Lemma open_fd_frags_any (γ : gname) (sts : list fdstate) :
  ...
  (* THE SUCCESS ARMS' SHARED TAIL, [SpecSysOpen.sys_open_post]'s success
     ...
     [fd_frags_acc]. *)
  Definition open_fd_ok (γf : gname) (p : mword 64) (pid : mword 32)
  ...
  Definition open_post_ok_plain Γ (γf : gname) (p : mword 64)
  ...
  Definition open_post_fail_plain Γ (γfs : fs_names)
      (P Pmiss : nat -> Z -> iProp Σ)
      (Φo : aview -> Z -> anode -> iProp Σ)
      (Φt : aview -> Z -> list (bv 8) -> iProp Σ) : iProp Σ :=
  ...
  Definition open_arms_plain Γ (γfs : fs_names) (γf : gname)
  ...
  (* ret = fd: FRESH (the fused delta fired at the entry write; the
     ...
     SpecCreate's F-OK) *)
  Definition open_post_ok_create Γ (γf : gname) (p : mword 64)
      (pid : mword 32) (vom : mword 64)
      (P : nat -> Z -> iProp Σ)
      (Φok Φex : aview -> Z -> fname -> Z -> iProp Σ)
      (Φo : aview -> Z -> anode -> iProp Σ)
  ...
  Definition open_post_fail_create Γ (γfs : fs_names)
      (P Pmiss : nat -> Z -> iProp Σ)
      (Φok Φex : aview -> Z -> fname -> Z -> iProp Σ)
      (Φo : aview -> Z -> anode -> iProp Σ)
      (Φt : aview -> Z -> list (bv 8) -> iProp Σ) : iProp Σ :=
  ...
  Definition open_arms_create Γ (γfs : fs_names) (γf : gname)
      (p : mword 64) (pid : mword 32) (vom : mword 64)
      (P Pmiss : nat -> Z -> iProp Σ)
      (Φok Φex : aview -> Z -> fname -> Z -> iProp Σ)
      (Φo : aview -> Z -> anode -> iProp Σ)
