...
(* LOOP.                                                                   *)
(*                                                                        *)
(*   parseexec @0x56c -- the loop that calls [gettoken] once per token and *)
...
(*ALIASES-END*)
  (* ===================================================================== *)
  (* §11 parseexec @0x56c -- the ARGUMENT LOOP.                             *)
  (*                                                                       *)
  ...
  Proof using .
    ...
    all: assert (Ekk : (cur' + ushp_skipws (len - cur') cur' f)%nat = cur')
      by (rewrite Hz'; lia).
    (* ---- 0x5fe  c.mv a2,s6 ---- *)
    all: iApply (wp_uk_cmv N h mc (mword_of_int 0x5fe) a2_idx
    ...
    (* ---- 0x600  c.mv a1,s5 ---- *)
    all: iApply (wp_uk_cmv N h1 n1 (mword_of_int 0x600) a1_idx
    ...
    all: assert (Hk2 : forall r : mword 5, ucallee_saved_idx r = true ->
      ...
      by (intros r Hr;
          ...
          exact (Hk1 r Hr)).
    (* ---- 0x602  c.mv a0,s4 ---- *)
    all: iApply (wp_uk_cmv N h2 n2 (mword_of_int 0x602) a0_idx
    ...
    all: assert (Hk3 : forall r : mword 5, ucallee_saved_idx r = true ->
      ...
      by (intros r Hr;
          ...
          exact (Hk2 r Hr)).
    (* ---- 0x604  jal 424 <peek> ---- *)
    all: iApply (wp_uk_jal N h3 n3 (mword_of_int 0x604)
    ...
    all: assert (Hk4 : forall r : mword 5, ucallee_saved_idx r = true ->
      ...
      by (intros r Hr;
          ...
          exact (Hk3 r Hr)).
    all: assert (Eret4 : ret_pc (n4 !!! Regidx ra_idx) = mword_of_int 0x608);
    ...
    all: iApply (wp_uk_cbnez N h5 n5 (mword_of_int 0x608)
    ...
    all: iApply (wp_uk_cmv N h6 n5 (mword_of_int 0x60a) a3_idx
    ...
    all: assert (Hk6 : forall r : mword 5, ucallee_saved_idx r = true ->
      ...
      by (intros r Hr;
          ...
          exact (Hk5 r Hr)).
    all: iApply (wp_uk_cmv N h7 n6 (mword_of_int 0x60c) a2_idx
    ...
    all: assert (Hk7 : forall r : mword 5, ucallee_saved_idx r = true ->
      ...
      by (intros r Hr;
          rewrite (upd_ne n6 (Regidx a2_idx) (Regidx r) _
                     (ushp_cs_ne r a2_idx Hr ltac:(vm_compute; reflexivity)));
          exact (Hk6 r Hr)).
    all: iApply (wp_uk_cmv N h8 n7 (mword_of_int 0x60e) a1_idx
                   s5_idx (mword_of_int (s0 + Z.of_nat len)) (24 + nn)
                   ltac:(unfold unot_sp; vm_compute; discriminate)
                   ltac:(vm_compute; discriminate)
    ...
    all: assert (Hk8 : forall r : mword 5, ucallee_saved_idx r = true ->
      ...
      by (intros r Hr;
          rewrite (upd_ne n7 (Regidx a1_idx) (Regidx r) _
                     (ushp_cs_ne r a1_idx Hr ltac:(vm_compute; reflexivity)));
          exact (Hk7 r Hr)).
    all: iApply (wp_uk_cmv N h9 n8 (mword_of_int 0x610) a0_idx
                   s4_idx (mword_of_int ps) (24 + nn)
                   ltac:(unfold unot_sp; vm_compute; discriminate)
                   ltac:(vm_compute; discriminate)
    ...
    all: iApply (wp_uk_jal N h10 n9 (mword_of_int 0x612)
  ...
  (* THE FRAME IS SPLIT IN TWO, BOTH WAYS.  gcc spills ra/s0/s1/s4/s5 before
     ...
     come back at 0x64c and the five at 0x5d6, and the [c.j 0x5d4] between
  ...
  Proof.
    ...
    iApply (wp_kshp_execcmd h13 m10 s0 (8 + nn) with "Hcode HM Hpx Hpay Hrun").
