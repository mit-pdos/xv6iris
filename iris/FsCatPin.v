(* ====================================================================== *)
(*  FsCatPin.v -- THE ERA-0 /cat PINS, THE /echo TRIPLE REPLAYED FOR CAT   *)
(* ====================================================================== *)
...
(*  WHAT THIS FILE IS.  [FsInitPin.v] proved, about era 0's durable map
    ...
    (inum 13) and [FsEchoPin.v] for ECHO (inum 4); THIS FILE IS THE SAME
    THREE SENTENCES FOR CAT, at inum 3 with [ElfUser.cat_elf]'s bytes,
    plus the transport corollaries -- and it is written, like [FsEchoPin],
    to REUSE both files rather than to restate them.

    WHY IT IS WANTED.  The FILE application
    (claude-notes/design/app-file.md) has the shell answer the console
    line "cat f" with [exec("cat")], and a verified shell answers kexec's
    contract for cat out of its own pins, so the lemma names below are
    that bundle's inputs -- deliberately [FsEchoPin]'s shapes with [echo]
    replaced by [cat] throughout.  The file application's own claim
    ([FileFsPure.file_fs_pure]) carries [era0_cat_pins] beside /init's,
    /sh's and /echo's.
    ...
    WHAT IS REUSED, AND WHY NOTHING IS RESTATED.
      - [FsInitPin] section 1's [img_*] lemmas ([img_astep_root],
        ...
        pay for a computation.  Cat is a fourth literal corollary and pays
      ...
      - [FsInitPinBoot]'s [era0_boot_snap_ok] / [era0_recovery_D] /
        ...
        pins what inside it.
      - No landed statement moves.  This file is a FIFTH LEAF over the
    ...
    THE TWO MEASURED TRAPS, both inherited and both respected here.
      (1) [FsInitPin] section 3's PERFORMANCE RULE.  [Some (NFile _)] must
          ...
          which is QUADRATIC in the file size.  Cat is 36,728 bytes.
    ...
    WHAT IS NOT CLAIMED.  Nothing here says cat is still at inum 3 after
...
Require Import FsImgCheck.     (* [fname_cat], [fsimg_cat_path],
                                  [fsimg_cat_type], [fsimg_cat_at]       *)
...
(* ====================================================================== *)
(*  1.  CAT'S THREE NAMES, AND ITS DURABLE ROW                             *)
(*                                                                        *)
(*  [CAT_INO] is READ OFF THE IMAGE, not chosen: [FsImgCheck.             *)
(*  fsimg_cat_path] -- which this file cites and does not re-run -- says   *)
(*  "cat" resolves, in the root directory, to 3.  [cat_bytes] is the       *)
(*  tracked raw, so every [ElfUser] theorem about [cat_elf]                *)
(*  (well-formedness, the file-backed image [CatInstrs.cat_bytes ∪         *)
(*  CatData.cat_data], the entry) is a theorem about the bytes pinned      *)
(*  below -- that is [FsImgCheck.fsimg_cat_ok]'s chain, and                *)
(*  [cat_bytes_elf] is the one line that joins it to this file.            *)
(* ====================================================================== *)

Definition CAT_INO : Z := 3.
Definition cat_path : list fname := [fname_cat].
Definition cat_bytes : list (bv 8) := ElfUser.cat_elf.

(* THE TIE TO THE ELF LAYER.  [FsInitPin] names /init's bytes as
   [init_bytes], [FsShPin] names the shell's as [sh_bytes] and
   [FsEchoPin] echo's as [echo_bytes]; no file names one for cat, so
   [cat_bytes] is defined fresh here and this is its join. *)
Lemma cat_bytes_elf : cat_bytes = ElfUser.cat_elf.
Proof. reflexivity. Qed.

(* THE DURABLE ROW.  [FsInitPin.img_dur_node] at cat's inum: era 0's map
   denotes the image's node there, at every state it denotes. *)
Lemma era0_dur_cat :
  dur_node era0_D CAT_INO (img_node fsimg_P fsimg_sb CAT_INO).
Proof.
  apply (img_dur_node FsImgDisk.fsimg_dk XV6_DISK_BYTES fsimg_sb fsimg_nib
           fsimg_cov CAT_INO fsimg_image_wf).
  cbv [CAT_INO fsimg_nib]. lia.
Qed.

(* ====================================================================== *)
(*  2.  THE LITERAL IMAGE'S TWO READINGS, AT CAT                           *)
(*                                                                        *)
...
(*  [FsImgCheck.fsimg_cat_type] pays.  NO file's contents are forced: the  *)
(*  bytes come in through [fsimg_cat_at], which [FsImgCheck] already       *)
(*  proved and which is CITED.                                             *)
(* ====================================================================== *)

Lemma fsimg_cat_size :
  bv_unsigned (di_size (fs_dinode fsimg_P fsimg_sb CAT_INO)) = 36776.
Proof. vm_eq. Qed.

Lemma fsimg_cat_nlink :
  Z.to_nat (bv_unsigned (di_nlink (fs_dinode fsimg_P fsimg_sb CAT_INO)))
  = 1%nat.
Proof. vm_eq. Qed.

(* ...so /cat is LINKED, which is what its row needs *)
Lemma fsimg_cat_nlink_nz :
  bv_unsigned (di_nlink (fs_dinode fsimg_P fsimg_sb CAT_INO)) <> 0.
Proof.
  intros Hz. pose proof fsimg_cat_nlink as H1. rewrite Hz in H1.
  simpl in H1. discriminate H1.
Qed.

(* cat is an eighth of what a file may hold; [FsInitPin.maxfile_bytes] is
   the bound, cited *)
Lemma fsimg_cat_size_bound :
  bv_unsigned (di_size (fs_dinode fsimg_P fsimg_sb CAT_INO))
  <= Z.of_nat MAXFILE * Z.of_nat BSIZE.
Proof. rewrite fsimg_cat_size maxfile_bytes. lia. Qed.

(* the two side conditions of [FsInitPin.node_at_nondir] at cat, hoisted
...
Lemma fsimg_cat_type_nz :
  bv_unsigned (di_type (fs_dinode fsimg_P fsimg_sb CAT_INO)) <> 0.
Proof. cbv [CAT_INO]. rewrite fsimg_cat_type. cbv [T_FILE_z]. lia. Qed.

Lemma fsimg_cat_type_nd :
  bv_unsigned (di_type (fs_dinode fsimg_P fsimg_sb CAT_INO)) <> T_DIR_z.
Proof.
  cbv [CAT_INO]. rewrite fsimg_cat_type. cbv [T_FILE_z T_DIR_z]. lia.
Qed.
...
(* THE FILE'S BYTES, off [FsImgCheck]'s own equality.  [FsInitPin] section
   ...
   36,728-byte literal is never entered. *)
Lemma fsimg_cat_file_bytes :
  file_bytes (fs_data_of fsimg_P (fs_dinode fsimg_P fsimg_sb CAT_INO))
    (Z.to_nat (bv_unsigned (di_size (fs_dinode fsimg_P fsimg_sb CAT_INO))))
  = cat_bytes.
Proof.
  apply nfile_inj.
  transitivity (node_at fsimg_P fsimg_sb CAT_INO).
  - symmetry.
    exact (node_at_nondir fsimg_P fsimg_sb CAT_INO
             fsimg_cat_type_nz fsimg_cat_type_nd).
  - exact fsimg_cat_at.
Qed.

(* THE IMAGE'S /cat ROW, as an abstract node.  This is the CONTENT PIN's
   whole content, with no state and no map in it yet. *)
Lemma fsimg_cat_abs :
  abs_of (img_node fsimg_P fsimg_sb CAT_INO)
  = Some (MkAnode (AFile cat_bytes) 1%nat).
Proof.
  rewrite (img_abs_file fsimg_P fsimg_sb CAT_INO fsimg_cat_type
             fsimg_cat_size_bound fsimg_cat_nlink_nz).
  rewrite fsimg_cat_file_bytes fsimg_cat_nlink. reflexivity.
Qed.
...
Theorem era0_cat_path_pin (S : fs_state_rec) :
  snap_ok S era0_D ->
  apath_at (abs_view (fss_inodes S)) FsImg.ROOTINO cat_path = Some CAT_INO.
Proof.
  intros HS.
  apply (img_apath_root fsimg_P fsimg_sb _ fname_cat CAT_INO fsimg_wf_ok).
  - cbv [fsimg_sb FsImg.ROOTINO FsImg.sb_ninodes]. lia.
  - exact (era0_root_row S HS).
  - exact fsimg_cat_path.
Qed.

(* ---- THE CONTENT PIN ------------------------------------------------ *)

Theorem era0_cat_content_pin (S : fs_state_rec) :
  snap_ok S era0_D ->
  abs_view (fss_inodes S) !! CAT_INO = Some (MkAnode (AFile cat_bytes) 1%nat).
Proof.
  intros HS.
  rewrite (era0_arow S CAT_INO _ HS era0_dur_cat) fsimg_cat_abs.
  reflexivity.
Qed.
...
(*  [FsAbsPins.apr_walk] takes exactly one pure premise about the abstract
    ...
    At era 0, for cat, that list is [[ROOTINO; 3]].  *)
Theorem era0_cat_arun (S : fs_state_rec) :
  snap_ok S era0_D ->
  arun (abs_view (fss_inodes S)) FsImg.ROOTINO cat_path
       [FsImg.ROOTINO; CAT_INO].
Proof.
  intros HS.
  (* [eapply]: [ARun_cons]'s hop target [c] is not in its conclusion, so it
     is fixed by the TAIL run ([ARun_nil] at [[CAT_INO]]) and read back
     into the hop's goal ([FsInitPin.era0_init_arun]'s note). *)
  eapply ARun_cons; [| apply ARun_nil].
  rewrite (img_astep_root fsimg_P fsimg_sb _ fname_cat fsimg_wf_ok).
  - exact fsimg_cat_path.
  - cbv [fsimg_sb FsImg.ROOTINO FsImg.sb_ninodes]. lia.
  - exact (era0_root_row S HS).
Qed.
...
(*  replayed at cat.  Every transport lemma below is CITED from there:     *)
(*  the identification "the map this boot founds at IS [era0_D]" is a fact *)
(*  about the map, so it serves /cat's pins as it serves /init's, and      *)
(*  this file adds no reasoning about the boot at all.                     *)
(* ====================================================================== *)

Definition era0_cat_pins (av : aview) : Prop :=
  (* the PATH pin: "cat" resolves, in the root, to inum 3 *)
  apath_at av FsImg.ROOTINO cat_path = Some CAT_INO
  (* the CONTENT pin: inum 3 holds [cat_elf]'s bytes, at nlink 1 *)
  /\ av !! CAT_INO = Some (MkAnode (AFile cat_bytes) 1%nat)
  (* ...and the WALK, which is exactly [FsAbsPins.apr_walk]'s premise *)
  /\ arun av FsImg.ROOTINO cat_path [FsImg.ROOTINO; CAT_INO].
...
Lemma era0_cat_pins_of_snap (S : fs_state_rec) :
  snap_ok S era0_D -> era0_cat_pins (abs_view (fss_inodes S)).
Proof.
  intros HS. split; [exact (era0_cat_path_pin S HS) |].
  split; [exact (era0_cat_content_pin S HS) | exact (era0_cat_arun S HS)].
Qed.
...
Theorem era0_boot_cat_pins (dk : Z -> bv 8) (ndisk : nat) (S : fs_state_rec)
    (Pb : Z -> list (bv 8)) (sb : fs_sb) (nib : nat) (cov : gset Z) :
  fs_boot_snap_wf dk ndisk S Pb sb nib cov ->
  fs_blocks dk = fsimg_P ->
  cov = fsimg_cov ->
  era0_cat_pins (abs_view (fss_inodes S)).
Proof.
  intros Hb Hdk Hcov.
  exact (era0_cat_pins_of_snap S
...
(*  [SystemAdequacy.xv6_boot_era] starts from [FsCrash.P_fs_lend]'s epoch,
    ...
    epoch recovers to.                                                    *)
Theorem era0_recovery_cat_pins (dk : Z -> bv 8) (D : gmap Z (list (bv 8)))
    (S : fs_state_rec) :
  fs_blocks dk = fsimg_P ->
  ...
  era0_cat_pins (abs_view (fss_inodes S)).
Proof.
  intros Hdk Hrec HS. apply era0_cat_pins_of_snap.
  assert (HD : D = era0_D).
  { apply era0_recovery_D. rewrite -Hdk. exact Hrec. }
  rewrite -HD. exact HS.
Qed.

(* ---- 4c.  CRASH BEFORE CAT EVER RUNS -- and it costs nothing.
...
Corollary era0_reboot_cat_pins (dk dk' : Z -> bv 8)
  ...
  fs_recovery (fs_blocks dk') D' fsimg_cov (FsImg.sb_logstart fsimg_sb) ->
  snap_ok S D' ->
  D' = D /\ D' = era0_D /\ era0_cat_pins (abs_view (fss_inodes S)).
Proof.
...
(*                                                                        *)
(*  [FsInitPin] section 6 and [FsInitPinBoot] section 4, at cat.  ALL ARE  *)
...
Section Era0CatEpoch.
  ...
  Proof using .
    ...
    iDestruct (fs_snap_read_ok_keep _ _ _ _ _ era0_dblk_full with "H")
      as "[%Hok H]".
    iFrame "H". iPureIntro. exact (era0_cat_pins_of_snap S Hok).
  Qed.
End Era0CatEpoch.
...
Section Era0CatLive.
  ...
  Lemma astate_era0_cat_pins Γ (S : fs_state_rec) :
    snap_ok S era0_D ->
  ...
  Proof using .
    ...
    exact (era0_cat_pins_of_snap S HS).
  Qed.

  (* ...and off the boot's own bundle, so the consumer (the pinned-exec
     gate: sh's [exec("cat")]) owes only the era-0 disk equation. *)
  Lemma astate_era0_boot_cat_pins Γ (dk : Z -> bv 8) (ndisk : nat)
  ...
  Proof using .
    ...
    exact (era0_boot_cat_pins dk ndisk S Pb sb nib cov Hb Hdk Hcov).
  Qed.

  (* THE CONTENT PIN AS AN AGREEMENT: a client-held share of inum 3 IS
     cat's bytes.  [FsAbs.astate_nview] is the agreement; the pin
     supplies the row.  [Some_inj], NOT [injection] -- section "THE TWO
     MEASURED TRAPS" (1): the row carries the 36,728-byte literal. *)
  Lemma nview_era0_cat Γ (S : fs_state_rec) (q : Qp) (a : anode) :
    snap_ok S era0_D ->
    astate Γ (abs_view (fss_inodes S)) -∗ nview Γ q CAT_INO a -∗
      ⌜a = MkAnode (AFile cat_bytes) 1%nat⌝.
  Proof using .
    intros HS. iIntros "Hst Hn".
    iDestruct (astate_nview with "Hst Hn") as %Hav.
    iPureIntro.
    rewrite (era0_cat_content_pin S HS) in Hav.
    apply Some_inj in Hav. symmetry. exact Hav.
  Qed.

  Lemma nview_era0_boot_cat Γ (dk : Z -> bv 8) (ndisk : nat)
      (S : fs_state_rec) (Pb : Z -> list (bv 8)) (sb : fs_sb) (nib : nat)
...
(*                                                                        *)
(*    Print Assumptions era0_cat_path_pin.                                *)
(*    Print Assumptions era0_cat_content_pin.                             *)
(*    Print Assumptions era0_cat_arun.                                    *)
(* ====================================================================== *)
