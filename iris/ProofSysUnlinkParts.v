...
Section ProofSysUnlinkFrame.
  ...
  Lemma su_frame_carve (sp0 : mword 64) :
    ...
    bytes_own (KTR := kt) (DfracOwn 1) (pa_stk sp0 8) 16 ∗
    bytes_own (KTR := kt) (DfracOwn 1) (pa_stk sp0 10) 16 ∗
    bytes_own (KTR := kt) (DfracOwn 1) (pa_stk sp0 26) 128 ∗
    (∃ w : mword 64, (pa_stk sp0 27) ↦₈[kt] w) ∗
    bytes_own (KTR := kt) (DfracOwn 1) (pa_stk sp0 29) 16 ∗
    (∃ w : mword 64, (pa_stk sp0 30) ↦₈[kt] w).
  Proof.
    ...
    iDestruct (slotsn_bytes_own (KTR := kt) sp0 8 2 ltac:(lia) with "[H7 H8]")
      as "[%HalD HbD]".
    { cbn [seq]. iSplitL "H8"; [iExact "H8" |]. iSplitL "H7"; [iExact "H7" |].
      done. }
    change 16%nat with (8 * 2)%nat at 1.
    iDestruct (slotsn_bytes_own (KTR := kt) sp0 10 2 ltac:(lia) with "[H9 H10]")
      as "[%HalN HbN]".
    ...
    iDestruct (slotsn_bytes_own (KTR := kt) sp0 29 2 ltac:(lia) with "[H28 H29]")
      as "[%HalE HbE]".
  ...
  Lemma su_frame_join (sp0 : mword 64) (w1 w2 w3 w4 w5 w6 w27 w30 : mword 64) :
    ...
    (pa_stk sp0 5) ↦₈[kt] w5 -∗ (pa_stk sp0 6) ↦₈[kt] w6 -∗
    bytes_own (KTR := kt) (DfracOwn 1) (pa_stk sp0 8) 16 -∗
    bytes_own (KTR := kt) (DfracOwn 1) (pa_stk sp0 10) 16 -∗
    bytes_own (KTR := kt) (DfracOwn 1) (pa_stk sp0 26) 128 -∗
  ...
  Proof.
    ...
    iDestruct (bytes_own_slotsn (KTR := kt) sp0 8 2 ltac:(lia) HalD with "[HbD]") as "HsD".
    { change (8 * 2)%nat with 16%nat. iExact "HbD". }
    iDestruct (bytes_own_slotsn (KTR := kt) sp0 10 2 ltac:(lia) HalN with "[HbN]") as "HsN".
    { change (8 * 2)%nat with 16%nat. iExact "HbN". }
    iDestruct (bytes_own_slotsn (KTR := kt) sp0 26 16 ltac:(lia) HalP with "[HbP]") as "HsP".
    { change (8 * 16)%nat with 128%nat. iExact "HbP". }
    iDestruct (bytes_own_slotsn (KTR := kt) sp0 29 2 ltac:(lia) HalE with "[HbE]") as "HsE".
  ...
  Lemma su_off_split (sp0 : mword 64) (w : mword 64) :
    (pa_stk sp0 27) ↦₈[kt] w ⊢
    (pa_stk sp0 27) ↦₄[kt] word_lo w ∗ (pa_add (pa_stk sp0 27) 4) ↦₄[kt] word_hi w.
  ...
  Lemma su_off_join (sp0 : mword 64) (lo hi : bv 32) :
    ...
    (pa_stk sp0 27) ↦₄[kt] lo -∗ (pa_add (pa_stk sp0 27) 4) ↦₄[kt] hi -∗
    (pa_stk sp0 27) ↦₈[kt] word_of_words lo hi.
...
Section ProofSysUnlinkEpilogue.
  ...
  Lemma su_epilogue `{GEN : GenId} `{CID0 : CpuId}
    ...
    ([∗ list] jj ∈ seq 0 16, pa_add (pa_stk sp0 10) jj ↦ₘ[kt] bn jj) -∗
