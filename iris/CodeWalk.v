(* CodeWalk.v -- the machine code of xv6's walk() (kernel/vm.c), the
   3-level Sv39 page-table walk with allocation: the per-instruction [instr]
   facts (wdec_* RVC decode facts via rvc_oneshot + wi_* instr lemmas via
   mk_rvc/mk_base), consumed by the wp_walk proof.

   INSTRUCTION TABLE (from kernel-rocq/KernelInstrs.v at KernelSyms.walk =
   0x80000f5c; offsets relative; B = 4-byte base, C = RVC.  The ALU leaves
   for srl/andi/ori/c.slli/c.srli are wp_{srl,andi,ori,cslli,csrli}_s_r in
   WpSmodePtAlu.v):
