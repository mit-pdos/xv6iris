...
Section ProofSysSbrk.
  ...
  Local Lemma ss_eager `{CID0 : CpuId} (γa γf : gname)
    ...
    sie_cap_gpr kt Me (av - 6)%nat b p -∗
    cpu_own 0%nat eb p b lks -∗
    kernel_text -∗
    pc_is (mword_of_int (KernelSyms.sys_sbrk + 0x58) : mword 64) -∗
    proc_priv γf p pid V -∗
    kalloc_env γa None -∗
    word4_pointsto (KTR := kt) (pa_stk sp0 5) (DfracOwn 1) nw -∗
    wp_next (CID0 := CID0) b p (fun (CID1 : CpuId) =>
      ∀ (Mf : regfile) (P' : uptd) (szv' rv : mword 64),
        ...
        sie_cap_gpr kt Mf (av - 6)%nat b p -∗
        cpu_own 0%nat eb p b lks -∗
        pc_is (mword_of_int (KernelSyms.sys_sbrk + 0x64) : mword 64) -∗
  ...
  Proof.
    ...
    iApply (Growproc.wp_growproc_sconf kt γa γf G2 (av - 6)%nat eb p pid V b lks
              ltac:(lia)
  ...
  Proof.
    ...
    iEval (rewrite (stack_own_split (KTR := kt) sp0 4 6 H46)) in "Hframe".
    ...
    iApply (Argint.wp_argint_sconf kt (CID := CIDs8) M5 (av - 6)%nat 0%nat eb p 0%nat
    ...
    iApply (Argint.wp_argint_sconf kt (CID := CIDs11) A3 (av - 6)%nat 0%nat eb p 1%nat
    ...
    iApply (Myproc.wp_myproc_sconf kt (CID := CIDs12) B1 (av - 6)%nat 0%nat eb p b lks
              ss_n0 ltac:(lia)
    ...
    iAssert (∀ (CIDx : CpuId) (Mf : regfile) (P' : uptd) (szv' rv : mword 64),
        ...
        sie_cap_gpr kt (CID := CIDx) Mf (av - 6)%nat b p -∗
        cpu_own (CID := CIDx) 0%nat eb p b lks -∗
        pc_is (mword_of_int (KernelSyms.sys_sbrk + 0x64) : mword 64) -∗
        proc_priv γf p pid (upd_sz (upd_upt V P') szv') -∗
        (∃ w5 : mword 64, word_pointsto (KTR := kt) (pa_stk sp0 5) (DfracOwn 1) w5) -∗
    ...
    iApply (Myproc.wp_myproc_sconf kt (CID := CIDs30) L5 (av - 6)%nat 0%nat eb p b lks
              ss_n0 ltac:(lia)
              with "Hcg Hcpu Htext Hpc").
