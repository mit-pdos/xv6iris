...
Section WpSmodePtMemLeaves.
 ...
 HSIE HMPRV HSXL Hmm HPBMTE Hmenvval0
  ...
  Lemma wp_clw_s_pt (root_ppn : mword 44) (Φ : mval -> iProp Σ)
    ...
    WP (Loop : expr riscv_lang) {{ Φ }}.
  Proof.
    exact (wp_clw_s_r (kpt_regime root_ppn) Φ pc rd rs1 imm m v mstatus0 mie_v mdv0 menvcfg0 (dq:=dq) (dqm:=dqm)).
  Qed.

  Lemma wp_lw_s_r (R : s_regime) (Φ : mval -> iProp Σ)
      (pc : mword 64) (rd rs1 : mword 5) (imm : mword 12)
      (m : gmap regidx (mword 64)) (v : mword 32)
      (mstatus0 mie_v mdv0 menvcfg0 : mword 64)
      {dq dqm : dfrac} :
  ...
  Proof.
    ...
    iDestruct "Hhwc" as (misa0 mseccfg0 pmar0 elp0)
      "(#Hmisa & #Hmseccfg & #Hpma & #Hhtif & #Help & %HmisaS & %HmisaC &
        %HmisaU & %HmisaM & %Hpma_all & %Hseccfg1 & %Hseccfg2 & %Help_np & %HmisaA & %Hmisa_val0 & %Hmseccfg_val0)".
    destruct (Hpma_all pa 4) as (region_ld & Hmatch_ld0 & _ & Hread_ld & _).
    iApply (wp_instr_s_config_regime R Φ pc false (LOAD (imm, Regidx rs1, Regidx rd, false, 4))
              mstatus0 mie_v mdv0 menvcfg0
 HSIE HMPRV HSXL Hmm HPBMTE Hmenvval0
              with "Hhw Hinv Hhs Hpriv Hms Hmie Hmdl Hmenv Htlbinv Hpc Hinstr").
    iIntros (σ Hpceq)
  ...
  Lemma wp_lw_s_pt (root_ppn : mword 44) (Φ : mval -> iProp Σ)
    ...
    WP (Loop : expr riscv_lang) {{ Φ }}.
  Proof.
    exact (wp_lw_s_r (kpt_regime root_ppn) Φ pc rd rs1 imm m v mstatus0 mie_v mdv0 menvcfg0 (dq:=dq) (dqm:=dqm)).
  Qed.
 ...
 HSIE HMPRV HSXL Hmm HPBMTE Hmenvval0
  ...
  Lemma wp_csw_s_pt (root_ppn : mword 44) (Φ : mval -> iProp Σ)
    ...
    WP (Loop : expr riscv_lang) {{ Φ }}.
  Proof.
    exact (wp_csw_s_r (kpt_regime root_ppn) Φ pc rs2 rs1 imm m vold mstatus0 mie_v mdv0 menvcfg0 (dq:=dq)).
  Qed.
 ...
 HSIE HMPRV HSXL Hmm HPBMTE Hmenvval0
  ...
  Lemma wp_sw_s_pt (root_ppn : mword 44) (Φ : mval -> iProp Σ)
    ...
    WP (Loop : expr riscv_lang) {{ Φ }}.
  Proof.
    exact (wp_sw_s_r (kpt_regime root_ppn) Φ pc rs2 rs1 imm m vold mstatus0 mie_v mdv0 menvcfg0 (dq:=dq)).
  Qed.
 ...
 HSIE HMPRV HSXL Hmm HPBMTE Hmenvval0
  ...
  Lemma wp_sb_s_r (R : s_regime) (γ : gname) (Φ : mval -> iProp Σ)
      (pc : mword 64) (rs2 rs1 : mword 5) (imm : mword 12)
  ...
  Proof.
    ...
    iApply (wp_instr_s_config_regime R Φ pc false (STORE (imm, Regidx rs2, Regidx rs1, 1))
              mstatus0 mie_v mdv0 menvcfg0
 HSIE HMPRV HSXL Hmm HPBMTE Hmenvval0
              with "Hhw Hinv Hhs Hpriv Hms Hmie Hmdl Hmenv Htlbinv Hpc Hinstr").
    iIntros (σ Hpceq)
    ...
    iDestruct (sr_transform R (Store Data)
                 ...
                 with "Hreg Htlbinv") as %Htea.
    iMod (sr_absorb R (Store Data) a8 s_pc
            (or_intror (or_intror (or_introl eq_refl))) Hrama8 Lmisa_pc' Lmenv_pc' Lhtif_pc Lpriv_pc LSXL_pc
            (exec_effectivePrivilege_store_S (register_lookup mstatus s_pc.(sregs)) s_pc
               ltac:(rewrite Lms_pc; exact HMPRV))
            (exec_is_shadow_stack_store s_pc)
            Lpma_pc' with "Hreg Hmem Htlbinv")
      as (s_tr) "(%Htr0 & %Hmdevtr & %Hshtr & %Hgr & Hreg & Hmem & Htlbinv)".
    destruct Hgr as (HA1 & Hord1 & HX1 & HW1 & HR1 & Hcov1).
    pose proof (ram_pmp_match_w pa (vec_access_dec (register_lookup pmpaddr_n s_tr.(sregs)) 0) 1
                  ltac:(lia) ltac:(vm_compute; reflexivity) Hlo Hfit Hcov1) as Hrange_st.
    pose proof (pt_regs_preserved _ _ Hshtr) as Hprestr.
    assert (Lpriv_tr : register_lookup cur_privilege s_tr.(sregs) = Supervisor)
      by (rewrite (Hprestr cur_privilege ltac:(vm_compute; reflexivity)); exact Lpriv_pc).
    ...
    { rewrite (exec_execute_STORE_1_gpr_S_walk_pt rs2 rs1 imm region_st s_pc s_tr
                 Htea
                 ltac:(rewrite Lva; apply is_aligned_vaddr_1) ltac:(rewrite Lva; exact Htr_pc)
                 Lpriv_tr ltac:(rewrite Lms_tr; exact HMPRV)
                 HA1 Hord1
                 ltac:(rewrite Lva; exact Hrange_st) HW1
  ...
  Lemma wp_sb_s_pt (root_ppn : mword 44) (γ : gname) (Φ : mval -> iProp Σ)
    ...
    WP (Loop : expr riscv_lang) {{ Φ }}.
  Proof.
    exact (wp_sb_s_r (kpt_regime root_ppn) γ Φ pc rs2 rs1 imm m vold (dq:=dq)).
  Qed.
