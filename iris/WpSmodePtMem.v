...
(* ---- state-generic width-4/1 towers ---- *)
  ...
  Proof.
    ...
    rewrite (exec_bind_Some _ _ _ _ _
      (exec_vmem_read_4_gpr_S_walk_pt rs1 offset v region satp0 s s' Hcp HSXL Hsatp Hmode Hmprv Hmxr Hpmm Halign Htr Hcp' Hmprv' HA Hord Hrange HR Hmatch Hpalign Hread Hc Hsig Hh Hdev Hbytes)).
    cbn match.
  ...
  Proof.
    ...
    cbn match.
    rewrite (exec_bind_Some _ _ _ _ _
      (exec_vmem_write_4_gpr_S_walk_pt rs1 offset _ region satp0 s s' Hcp HSXL Hsatp Hmode Hmprv Hmxr Hpmm Halign Htr Hcp' Hmprv' HA Hord Hrange HW Hmatch Hpalign Hwrite Hc Hsig Hh Hdev)).
    cbn match.
    apply exec_returnM.
  Qed.
  ...
  Proof.
    ...
    cbn match.
    rewrite (exec_bind_Some _ _ _ _ _
      (exec_vmem_write_1_gpr_S_walk_pt rs1 offset _ region satp0 s s' Hcp HSXL Hsatp Hmode Hmprv Hmxr Hpmm Halign Htr Hcp' Hmprv' HA Hord Hrange HW Hmatch Hpalign Hwrite Hc Hsig Hh Hdev)).
    cbn match.
    apply exec_returnM.
  Qed.
...
Section WpSmodePtMemLeaves.
  ...
  Lemma wp_clw_s_pt (root_ppn : mword 44) (Φ : mval -> iProp Σ)
 ...
 HSIE HMPRV HSXL Hmm HPBMTE Hmenvval0
  ...
  Lemma wp_lw_s_pt (root_ppn : mword 44) (Φ : mval -> iProp Σ)
      (pc : mword 64) (rd rs1 : mword 5) (imm : mword 12)
      (m : gmap regidx (mword 64)) (v : mword 32)
      (mstatus0 mie_v mdv0 menvcfg0 : mword 64)
      {dq dqm : dfrac} :
  ...
  Proof.
    ...
    iDestruct "Hhwc" as (misa0 mseccfg0 pmar0 elp0)
      "(#Hmisa & #Hmseccfg & #Hpma & #Hhtif & #Help & %HmisaS & %HmisaC &
        %HmisaU & %HmisaM & %Hpma_all & %Hseccfg1 & %Hseccfg2 & %Help_np & %HmisaA & %Hmisa_val0 & %Hmseccfg_val0)".
    destruct (Hpma_all pa 4) as (region_ld & Hmatch_ld0 & _ & Hread_ld & _).
    iApply (wp_instr_s_config_tlbinv_pt root_ppn Φ pc false (LOAD (imm, Regidx rs1, Regidx rd, false, 4))
              mstatus0 mie_v mdv0 menvcfg0
 HSIE HMPRV HSXL Hmm HPBMTE Hmenvval0
              with "Hhw Hinv Hhs Hpriv Hms Hmie Hmdl Hmenv Htlbinv Hpc Hinstr").
    iIntros (σ Hpceq)
 ...
 HSIE HMPRV HSXL Hmm HPBMTE Hmenvval0
  ...
  Lemma wp_ld_s_pt (root_ppn : mword 44) (Φ : mval -> iProp Σ)
      (pc : mword 64) (rd rs1 : mword 5) (imm : mword 12)
      (m : gmap regidx (mword 64)) (v : mword 64)
      (mstatus0 mie_v mdv0 menvcfg0 : mword 64)
      {dq dqm : dfrac} :
    ...
    WP (Loop : expr riscv_lang) {{ Φ }}.
  Proof.
    exact (wp_ld_s_r (kpt_regime root_ppn) Φ pc rd rs1 imm m v mstatus0 mie_v mdv0 menvcfg0 (dq:=dq) (dqm:=dqm)).
  Qed.

  Lemma wp_csw_s_pt (root_ppn : mword 44) (Φ : mval -> iProp Σ)
  ...
  Proof.
    ...
    iApply (wp_instr_s_config_tlbinv_pt root_ppn Φ pc true (STORE (imm, Regidx rs2, Regidx rs1, 4))
              mstatus0 mie_v mdv0 menvcfg0
 HSIE HMPRV HSXL Hmm HPBMTE Hmenvval0
              with "Hhw Hinv Hhs Hpriv Hms Hmie Hmdl Hmenv Htlbinv Hpc Hinstr").
    iIntros (σ Hpceq)
  ...
  Lemma wp_sw_s_pt (root_ppn : mword 44) (Φ : mval -> iProp Σ)
      (pc : mword 64) (rs2 rs1 : mword 5) (imm : mword 12)
      (m : gmap regidx (mword 64)) (vold : bv 32)
      (mstatus0 mie_v mdv0 menvcfg0 : mword 64)
      {dq : dfrac} :
  ...
  Proof.
    ...
    iDestruct "Hhwc" as (misa0 mseccfg0 pmar0 elp0)
      "(#Hmisa & #Hmseccfg & #Hpma & #Hhtif & #Help & %HmisaS & %HmisaC &
        %HmisaU & %HmisaM & %Hpma_all & %Hseccfg1 & %Hseccfg2 & %Help_np & %HmisaA & %Hmisa_val0 & %Hmseccfg_val0)".
    destruct (Hpma_all pa 4) as (region_st & Hmatch_st0 & _ & _ & Hwrite_st & _).
    iApply (wp_instr_s_config_tlbinv_pt root_ppn Φ pc false (STORE (imm, Regidx rs2, Regidx rs1, 4))
              mstatus0 mie_v mdv0 menvcfg0
 HSIE HMPRV HSXL Hmm HPBMTE Hmenvval0
              with "Hhw Hinv Hhs Hpriv Hms Hmie Hmdl Hmenv Htlbinv Hpc Hinstr").
    iIntros (σ Hpceq)
 ...
 HSIE HMPRV HSXL Hmm HPBMTE Hmenvval0
  ...
  Lemma wp_sd_s_pt (root_ppn : mword 44) (Φ : mval -> iProp Σ)
      (pc : mword 64) (rs2 rs1 : mword 5) (imm : mword 12)
      (m : gmap regidx (mword 64)) (vold : mword 64)
      (mstatus0 mie_v mdv0 menvcfg0 : mword 64)
      {dq : dfrac} :
    ...
    WP (Loop : expr riscv_lang) {{ Φ }}.
  Proof.
    exact (wp_sd_s_r (kpt_regime root_ppn) Φ pc rs2 rs1 imm m vold mstatus0 mie_v mdv0 menvcfg0 (dq:=dq)).
  Qed.

  (* width-1 store leaf: sb rs2, imm(rs1) to a RAM byte, γ-form.  Same
     absorption recipe; the byte window is a single [↦ₘ] cell. *)
  Lemma wp_sb_s_pt (root_ppn : mword 44) (γ : gname) (Φ : mval -> iProp Σ)
      (pc : mword 64) (rs2 rs1 : mword 5) (imm : mword 12)
  ...
  Proof.
    ...
    assert (Hrama8 : addr_is_ram a8) by (rewrite <- Hpa_a8; exact Hrampa).
    ...
    iApply (wp_instr_s_config_tlbinv_pt root_ppn Φ pc false (STORE (imm, Regidx rs2, Regidx rs1, 1))
              mstatus0 mie_v mdv0 menvcfg0
 HSIE HMPRV HSXL Hmm HPBMTE Hmenvval0
              with "Hhw Hinv Hhs Hpriv Hms Hmie Hmdl Hmenv Htlbinv Hpc Hinstr").
    iIntros (σ Hpceq)
    ...
    assert (Lpma_pc' : pma_allows_all (register_lookup pma_regions s_pc.(sregs)))
      by (rewrite Lpma_pc; exact Hpma_all).
    iMod (tlb_inv_pt_translateAddr_store root_ppn a8 s_pc
            Hrama8 Lmisa_pc' Lmenv_pc' Lhtif_pc Lpriv_pc LSXL_pc
            (exec_effectivePrivilege_store_S (register_lookup mstatus s_pc.(sregs)) s_pc
               ltac:(rewrite Lms_pc; exact HMPRV))
            (exec_is_shadow_stack_store s_pc)
            Lpma_pc' with "Hreg Hmem Htlbinv")
    ...
    { replace (add_vec_int (bits_of_virtaddr (Virtaddr a8)) (0 * 1)) with a8
      ...
      replace pa with a8 by (symmetry; exact Hpa_a8).
      exact Htr0. }
    pose (s_x := MState s_tr.(sregs) (write_bytes s_tr.(mem) pa 1 storeval) s_tr.(mdev)).
    assert (Hstore : exec (execute (STORE (imm, Regidx rs2, Regidx rs1, 1))) s_pc
                     = Some (RETIRE_SUCCESS, s_x)).
    { rewrite (exec_execute_STORE_1_gpr_S_walk_pt rs2 rs1 imm region_st satp1 s_pc s_tr
                 Lpriv_pc ltac:(rewrite Lms_pc; exact HSXL) Lsatp_pc Hmode
                 ltac:(rewrite Lms_pc; exact HMPRV) ltac:(rewrite Lms_pc; exact HMXR)
                 ltac:(rewrite Lmenv_pc; exact Hpmm)
                 ...
                 Lpriv_tr ltac:(rewrite Lms_tr; exact HMPRV)
                 ltac:(rewrite Lpmpc_tr; exact HA0) ltac:(rewrite Lpmpaddr_tr; exact Hord0)
                 ltac:(rewrite Lpmpaddr_tr Lva; exact Hrange_st) ltac:(rewrite Lpmpc_tr; exact HW)
                 ltac:(rewrite Lpma_tr Lva; exact Hmatch_st0) ltac:(rewrite Lva; apply is_aligned_paddr_1)
    ...
    iApply ("Hcont" with "Hsm Htlbinv [$Hpc' $Hnpc] [Hfmap] Hbyte").
