...
(* ---- state-generic width-4/1 towers ---- *)
  ...
  Proof.
    ...
    rewrite execR_liftR.
    rewrite (exec_vmem_read_addr_4_S_walk_pt a8 v region s s' pa Halign Hcp' Hmprv' Htr HA Hord Hrange HR Hmatch Hpalign Hread Hc Hsig Hh Hdev Hbytes).
    reflexivity.
  Qed.
  End RWgS4walkPt.
  ...
  Proof.
    ...
    rewrite (exec_bind_Some _ _ _ _ _
      (exec_vmem_read_4_gpr_S_walk_pt rs1 offset v region s s' pa Htea Halign Htr Hcp' Hmprv' HA Hord Hrange HR Hmatch Hpalign Hread Hc Hsig Hh Hdev Hbytes)).
    cbn match.
    ...
    { eapply execR_untilMT_1.
      ...
      - cbn match.
        ...
        { match goal with
          ...
          rewrite (execR_liftR_seq _ _ _ _ _
            (exec_mem_write_value_4_S PBMT_PMA pa region data
  ...
  Proof.
    ...
    rewrite (execR_bind_Some _ _ _ _ _ (execR_returnR_fwd (Virtaddr a8) s)).
    rewrite execR_liftR.
    rewrite (exec_vmem_write_addr_4_S_walk_pt a8 data region s s' pa Halign Hcp' Hmprv' Htr HA Hord Hrange HW Hmatch Hpalign Hwrite Hc Hsig Hh Hdev).
    reflexivity.
  Qed.
  End VWgS4walkPt.
  ...
  Proof.
    ...
    cbn match.
    rewrite (exec_bind_Some _ _ _ _ _
      (exec_vmem_write_4_gpr_S_walk_pt rs1 offset _ region s s' pa Htea Halign Htr Hcp' Hmprv' HA Hord Hrange HW Hmatch Hpalign Hwrite Hc Hsig Hh Hdev)).
    cbn match.
    apply exec_returnM.
  Qed.
      ...
      - cbn match.
        ...
        { match goal with
          ...
          2: { symmetry.
               ...
               apply bv_wrap_bv_unsigned. }
          rewrite (execR_liftR_seq _ _ _ _ _
            (exec_mem_write_value_1_S PBMT_PMA pa region data
               (register_lookup mstatus s'.(sregs)) s' HA Hord Hrange HW Hmatch Hpalign Hwrite Hc Hsig Hh Hdev eq_refl Hmprv Hcp)).
          cbn match.
          apply execR_returnR_fwd. }
        rewrite (execR_bind_Some _ _ _ _ _ Hwrloop).
  ...
  Proof.
    ...
    rewrite (execR_bind_Some _ _ _ _ _ (execR_returnR_fwd (Virtaddr a8) s)).
    rewrite execR_liftR.
    rewrite (exec_vmem_write_addr_1_S_walk_pt a8 data region s s' pa Halign Hcp' Hmprv' Htr HA Hord Hrange HW Hmatch Hpalign Hwrite Hc Hsig Hh Hdev).
    reflexivity.
  Qed.
  End VWgS1walkPt.
  ...
  Proof.
    ...
    cbn match.
    rewrite (exec_bind_Some _ _ _ _ _
      (exec_vmem_write_1_gpr_S_walk_pt rs1 offset _ region s s' pa Htea Halign Htr Hcp' Hmprv' HA Hord Hrange HW Hmatch Hpalign Hwrite Hc Hsig Hh Hdev)).
    cbn match.
    apply exec_returnM.
  Qed.
...
Section WpSmodePtMemLeaves.
 ...
 HSIE HMPRV HSXL Hmm HPBMTE Hmenvval0
    ...
    destruct (Hpma_all (pa_of ppn a8) 4) as (region_ld & Hmatch_ld0 & _ & Hread_ld & _).
 ...
 HSIE HMPRV HSXL Hmm HPBMTE Hmenvval0
    ...
    destruct (Hpma_all (pa_of ppn a8) 8) as (region_ld & Hmatch_ld0 & _ & Hread_ld & _).
    assert (Hlo : (ram_base <= uint (pa_of ppn a8))%Z) by (destruct Hram0 as [Hl _]; exact Hl).
    ...
    { rewrite <- Hev.
      apply (exec_execute_LOAD_8_gpr_S_walk_pt rs1 rd imm v region_ld s_pc s_tr (pa_of ppn a8) Hrd
               Htea
               ltac:(rewrite Lva subrange_id sign_extend'_id; exact Halign4)
               ltac:(rewrite Lva; rewrite subrange_id; rewrite sign_extend'_id; cbn [bits_of_virtaddr]; rewrite avi0_mul8; exact Htr_pc)
               Lpriv_tr ltac:(rewrite Lms_tr; exact HMPRV)
               HA1 Hord1
               Hrange_ld HR1
               ltac:(rewrite Lpma_tr; exact Hmatch_ld0)
 ...
 HSIE HMPRV HSXL Hmm HPBMTE Hmenvval0
    ...
    destruct (Hpma_all (pa_of ppn a8) 4) as (region_st & Hmatch_st0 & _ & _ & Hwrite_st & _).
    assert (Hlo : (ram_base <= uint (pa_of ppn a8))%Z) by (destruct Hram0 as [Hl _]; exact Hl).
    assert (Hfit : (uint (pa_of ppn a8) + 4 <= ram_base + ram_size)%Z).
    { assert (Hnw : (uint (pa_of ppn a8) + Z.of_nat 3 < 18446744073709551616)%Z).
      { destruct Hram0 as [_ Hh]. unfold ram_base, ram_size in Hh. change (Z.of_nat 3) with 3. lia. }
 ...
 HSIE HMPRV HSXL Hmm HPBMTE Hmenvval0
    ...
    destruct (Hpma_all (pa_of ppn a8) 8) as (region_st & Hmatch_st0 & _ & _ & Hwrite_st & _).
    assert (Hlo : (ram_base <= uint (pa_of ppn a8))%Z) by (destruct Hram0 as [Hl _]; exact Hl).
    assert (Hfit : (uint (pa_of ppn a8) + 8 <= ram_base + ram_size)%Z).
    { assert (Hnw : (uint (pa_of ppn a8) + Z.of_nat 7 < 18446744073709551616)%Z).
      { destruct Hram0 as [_ Hh]. unfold ram_base, ram_size in Hh. change (Z.of_nat 7) with 7. lia. }
