(* WpUserMem.v -- the memory USTEP arms (width-8 loads and stores).
...
Section WpUserMem.
  ...
  Proof.
    ...
    assert (Hnlpad : is_lpad_instruction (LOAD (imm, Regidx rs1, Regidx rd, false, 8))
                       = false) by reflexivity.
    iApply (wp_instr_u_hit_data va vpn ie w (LOAD (imm, Regidx rs1, Regidx rd, false, 8))
              ms_v tlbvec E Φ HN Hvec Hchk0 HupdN Hpbmt0 Hcw HSXL Hval Hcanon
              Hvpn_def Hpaal HnotRVC Hdec Hnlpad
  ...
  (* COMBINED width-8 LOAD (all 4 fetch×data TLB combos). *)
  Lemma ustep_ld_u
      (va : mword 64) (vpn : mword 27) (ie : uwalk_info) (w : mword 32)
      (vpnD : mword 27) (ieD : uwalk_info)
      (imm : mword 12) (rs1 rd : mword 5) (v : mword 64)
  ...
  Proof.
    ...
    assert (Hnlpad : is_lpad_instruction (LOAD (imm, Regidx rs1, Regidx rd, false, 8))
                       = false) by reflexivity.
    iApply (wp_instr_u_data va vpn ie w (LOAD (imm, Regidx rs1, Regidx rd, false, 8))
...
Lemma ustep_sd_data_miss
  ...
  Proof.
    ...
    assert (Hnlpad : is_lpad_instruction (STORE (imm, Regidx rs2, Regidx rs1, 8))
                       = false) by reflexivity.
    iApply (wp_instr_u_hit_data va vpn ie w (STORE (imm, Regidx rs2, Regidx rs1, 8))
              ms_v tlbvec E Φ HN Hvec Hchk0 HupdN Hpbmt0 Hcw HSXL Hval Hcanon
              Hvpn_def Hpaal HnotRVC Hdec Hnlpad
              with "Hhw Hinv Hhs Hpriv Hms Htlbc Hpcr Hcode Hupt Hcfg").
    iIntros (σ Hpceq Hag Hpins) "Htlbc Hupt [Hreg [Hmem Hdev]]".
  ...
  (* COMBINED width-8 STORE (all 4 fetch×data TLB combos). *)
Lemma ustep_sd_u
      (va : mword 64) (vpn : mword 27) (ie : uwalk_info) (w : mword 32)
      (vpnD : mword 27) (ieD : uwalk_info)
      (imm : mword 12) (rs2 rs1 : mword 5) (vold : mword 64)
  ...
  Proof.
    ...
    assert (Hnlpad : is_lpad_instruction (STORE (imm, Regidx rs2, Regidx rs1, 8))
                       = false) by reflexivity.
    iApply (wp_instr_u_data va vpn ie w (STORE (imm, Regidx rs2, Regidx rs1, 8))
              ms_v tlbvec E Φ HN Hok Hsome Hchk0 HupdN Hcw HSXL Hval Hcanon
              Hvpn_def Hpaal HnotRVC Hdec Hnlpad
              with "Hhw Hinv Hhs Hpriv Hms Htlbc Hpcr Hcode Hupt Hcfg").
    iIntros (σ Hpceq Hag Hpins) "Htlbc Hupt [Hreg [Hmem Hdev]]".
  ...
  Proof.
    ...
    { iDestruct (big_sepM_lookup (fun a b => (a ↦ₘ□ b)%I) code _ _ (Hcwd 0%nat ltac:(lia)) with "Hcode")
      ...
      iPureIntro. exact Hr0. }
    iAssert (⌜addr_is_ram (pa_add paD 7)⌝)%I as %HramD7.
    { iDestruct (big_sepM_lookup (fun a b => (a ↦ₘ□ b)%I) code _ _ (Hcwd 7%nat ltac:(lia)) with "Hcode")
        as "Hb7".
      iDestruct (mem_ram with "Hb7") as %Hr7. iPureIntro. exact Hr7. }
    pose proof (addr_is_ram_not_in_clint _ HramD) as HncD.
    pose proof (addr_is_ram_not_in_sig _ HramD) as HnsD.
