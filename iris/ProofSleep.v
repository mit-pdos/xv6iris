...
Section SleepJoin.
  ...
  Proof.
    ...
    iApply (Release.wp_release_sconf KT1 γl (proc_addr j) "proc"%string (proc_lock_pay γs γl (proc_addr j)) D1 0 eb pj (av - 4)%nat
...
Section ProofSleepBody.
  ...
  Proof.
    ...
    iDestruct (cpu_own_transport CID7 CID9 0 eb pj eb ltac:(wp_next_chain)
                 with "Hcpu") as "Hcpu".
    iApply (Acquire.wp_acquire_sconf KT1 γl "proc"%string (proc_lock_pay γs γl (proc_addr j)) B1 0 eb pj (av - 4)%nat eb lks
              ltac:(lia)
              ltac:(lia)
              Hno
