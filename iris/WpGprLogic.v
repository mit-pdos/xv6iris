...
Section ForwardOrGpr.
  ...
  Proof using All.
    ...
    assert (LmidA : register_lookup (R_bitvector_64 mideleg) sAor.(sregs) = zeros' 64).
    { unfold sAor, set_reg; cbn [sregs]. rewrite irrelevant_register_set;
    ...
    { apply exec_dispatchInterrupt_none.
      apply (exec_getPendingSet_machine_none sAor _ (exec_currentlyEnabled_S sAor) LmidA LmIEA). }
...
Section WpOrGpr.
  ...
  Proof.
    ...
    iExists (sFcor s pc b1 rs2 rs1 rd mst0). iSplitR.
    { iPureIntro.
      rewrite <- (sFor_eq s pc b1 rs2 rs1 rd mst0 Lpc Lmst).
...
Section WpOrGprDemo.
  ...
  Definition wp_or_x5_x6_x7 (pc : mword 64) (w : mword 32) :=
    wp_or_gpr pc w (mword_of_int 7) (mword_of_int 6) (mword_of_int 5).
  (* `add x28, x1, x2` : rd=x28, rs1=x1, rs2=x2.  SAME lemma, different regs. *)
  Definition wp_or_x28_x1_x2 (pc : mword 64) (w : mword 32) :=
    wp_or_gpr pc w (mword_of_int 2) (mword_of_int 1) (mword_of_int 28).
...
Section ForwardAndGpr.
  ...
  Proof using All.
    ...
    { unfold sAand, set_reg; cbn [sregs]. rewrite irrelevant_register_set;
        [ exact Lhs | vm_compute; reflexivity ]. }
    assert (LmidA : register_lookup (R_bitvector_64 mideleg) sAand.(sregs) = zeros' 64).
    { unfold sAand, set_reg; cbn [sregs]. rewrite irrelevant_register_set;
        [ exact Lmideleg | vm_compute; reflexivity ]. }
    assert (LmIEA : eq_vec (_get_Mstatus_MIE
    ...
    { apply exec_dispatchInterrupt_none.
      apply (exec_getPendingSet_machine_none sAand _ (exec_currentlyEnabled_S sAand) LmidA LmIEA). }
...
Section WpAndGpr.
  ...
  Proof.
    ...
    iDestruct (fetch_from_pts_minstret pc w region_f pmpcfg0 pmar0 b1 s
                 ...
                 with "Hreg Hmem Hpc Hpriv Hpmpc Hpma Hhtif Hibytes") as %Hfetch_at.
    iApply fupd_mask_intro; [apply empty_subseteq|]. iIntros "Hclose".
    iExists (sFcand s pc b1 rs2 rs1 rd mst0). iSplitR.
    { iPureIntro.
      rewrite <- (sFand_eq s pc b1 rs2 rs1 rd mst0 Lpc Lmst).
...
Section WpAndGprDemo.
  ...
  Definition wp_and_x5_x6_x7 (pc : mword 64) (w : mword 32) :=
    wp_and_gpr pc w (mword_of_int 7) (mword_of_int 6) (mword_of_int 5).
  (* `add x28, x1, x2` : rd=x28, rs1=x1, rs2=x2.  SAME lemma, different regs. *)
  Definition wp_and_x28_x1_x2 (pc : mword 64) (w : mword 32) :=
    wp_and_gpr pc w (mword_of_int 2) (mword_of_int 1) (mword_of_int 28).
...
Section ForwardOriGpr.
  ...
  Proof using All.
    ...
    { unfold sAori, set_reg; cbn [sregs]. rewrite irrelevant_register_set;
        [ exact Lhs | vm_compute; reflexivity ]. }
    assert (LmidA : register_lookup (R_bitvector_64 mideleg) sAori.(sregs) = zeros' 64).
    { unfold sAori, set_reg; cbn [sregs]. rewrite irrelevant_register_set;
        [ exact Lmideleg | vm_compute; reflexivity ]. }
    assert (LmIEA : eq_vec (_get_Mstatus_MIE
    ...
    { apply exec_dispatchInterrupt_none.
      apply (exec_getPendingSet_machine_none sAori _ (exec_currentlyEnabled_S sAori) LmidA LmIEA). }
...
Section WpOriGpr.
  ...
  Lemma wp_ori_gpr (pc : mword 64) (w : mword 32) (rs1 rd : mword 5) (imm : mword 12)
      (m : gmap register_bitvector_64 (mword 64)) (va vd : mword 64)
      (b1 : bool) (npc0 mst0 mstatus0 : mword 64)
      (mc : mword 32) (mcfg : mword 64)
      (pmpcfg0 : type_of_register pmpcfg_n) (pmar0 : list PMA_Region)
      (mi0 : bool) (elp0 : mword 1) E (Phi : mval -> iProp Σ) :
    ...
    WP (Loop : expr riscv_lang) @ E {{ Phi }}.
  Proof.
    iIntros (Hr1 Hrd Hm1 Hmd Hpmaall Hpmpf Halignf Hbit0f Hbit1f Hvalignf HnotRVCf Hdec Hb1 HmIE Help)
      "Hpc Hfile Hnpc Hmi Hmst Hpriv Hhs Hmdl Hms Help Hmcinh Hmcfg Hpmpc Hpma Hhtif Hibytes Hcont".
    destruct (Hpmaall (fetch_pa pc) 4) as (region_f & Hmatchf & Hexecf & _ & _).
    iApply wp_exec_step. iIntros (s ns κs nt) "[Hreg Hmem]".
    iDestruct (reg_valid with "Hreg Hpc")    as %Lpc.
    ...
    iDestruct (fetch_from_pts_minstret pc w region_f pmpcfg0 pmar0 b1 s
                 ...
                 with "Hreg Hmem Hpc Hpriv Hpmpc Hpma Hhtif Hibytes") as %Hfetch_at.
    iApply fupd_mask_intro; [apply empty_subseteq|]. iIntros "Hclose".
    iExists (sFcori s pc b1 rs1 rd imm mst0). iSplitR.
    { iPureIntro.
      rewrite <- (sFori_eq s pc b1 rs1 rd imm mst0 Lpc Lmst).
...
Section WpOriGprDemo.
  ...
  Definition wp_ori_x5_x6 (pc : mword 64) (w : mword 32) (imm : mword 12) :=
    wp_ori_gpr pc w (mword_of_int 6) (mword_of_int 5) imm.   (* addi x5, x6, imm *)
  Definition wp_ori_x28_x1 (pc : mword 64) (w : mword 32) (imm : mword 12) :=
    wp_ori_gpr pc w (mword_of_int 1) (mword_of_int 28) imm.  (* addi x28, x1, imm *)
  Goal gpr_of_Z (uint (mword_of_int 6 : mword 5)) = x6
    /\ gpr_of_Z (uint (mword_of_int 28 : mword 5)) = x28
