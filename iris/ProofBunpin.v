...
Section ProofBunpin.
  ...
  Proof.
    ...
    iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.bunpin + 0x10)) Ra0 Ra0 (mword_of_int 0x462 : mword 12)
    ...
    set (R5 := <[Regidx Ra0 := regval_into_reg
                  (add_vec (R4 !!! Regidx Ra0) (sign_extend' 64 (mword_of_int 1122 : mword 12)))]> R4).
    ...
    (* ===== +0x14 jal ra,acquire ===== *)
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.bunpin + 0x14)) Rra (mword_of_int 0x1fde44 : mword 21)
    ...
    assert (Htgtacq : add_vec (mword_of_int (KernelSyms.bunpin + 0x14) : mword 64)
                        (sign_extend' 64 (mword_of_int 0x1fde44 : mword 21))
    ...
    iDestruct (cpu_own_transport CID CID9 n eb p b ltac:(wp_next_chain)
                 with "Hcnt") as "Hcnt".
    (* BOX v2: refs-- is lemma (d) -- no withdraw, no floor; the plain
       acquire over the λ payload suffices. *)
    ...
    assert (Hs64 : sign_extend' 64 (mword_of_int 64 : mword 12) = (mword_of_int 64 : mword 64))
      by (apply bv_eq; vm_compute; reflexivity).
    (* ENDGAME R2: the decrement is [bcache_scan2_decr] -- the box counts
       down (the last reference takes the resting content back into L1's
       None arm); the caller stores the decremented word. *)
    ...
    iDestruct (bcache_scan2_decr bn V Mg ord devs bnos tl k q dev bno ⊤
                 ltac:(solve_ndisj) Hk
                 with "Hbox Hfl Hllbtl Hscan Href")
      as (cnt) "(%Hcntb & Hcell & Hclose)".
    set (cw := (mword_of_int (Z.pos cnt) : mword 32)).
    (* +0x18 c.lw a5,64(s1) *)
    ...
    iEval (rewrite /cw (decr32_pos cnt Hcntb)) in "Hcell".
    iApply fupd_wp.
    iMod ("Hclose" with "Hcell") as "[Hafter Hbslot]".
    iModIntro.
    ...
    iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.bunpin + 0x22)) Ra0 Ra0 (mword_of_int 0x450 : mword 12)
    ...
    set (D4 := <[Regidx Ra0 := regval_into_reg
                  (add_vec (D3 !!! Regidx Ra0) (sign_extend' 64 (mword_of_int 1104 : mword 12)))]> D3).
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.bunpin + 0x26)) Rra (mword_of_int 0x1fdeba : mword 21)
    ...
    assert (Htgtrel : add_vec (mword_of_int (KernelSyms.bunpin + 0x26) : mword 64)
                        (sign_extend' 64 (mword_of_int 0x1fdeba : mword 21))
