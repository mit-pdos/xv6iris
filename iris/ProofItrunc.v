...
Section ItruncCont.
  ...
  Definition it_cont `{GEN : GenId} `{CID0 : CpuId}
      ...
      (m : regfile) (K : nat) (b : bool) (eb : bool) (lks : gset string) : iProp Σ :=
...
Section ItruncTail.
  ...
  Local Lemma it_tail `{GEN : GenId} `{CID0 : CpuId} 
      ...
      (m M : regfile) (K : nat) (b : bool) (eb : bool) (lks : gset string) :
    (K_itrunc <= K)%nat ->
  ...
  Proof.
    ...
    pose proof HK as HK'. 
    iIntros "Hcg Hcnt Hextc Hextm #Htext #Hkd Hpc #Hpanic #Hpenv #Hbio #Hlctx #Hprocs Hframe Hppid Hidev Hinum Hsbb Hsbi Hmeta Hmap Hblks Hbmr
...
Section ItruncDLoop.
  ...
  Local Lemma it_dloop `{GEN : GenId} `{CID0 : CpuId} 
      ...
      (w : nat)
      (m : regfile) (K : nat) (b : bool) (eb : bool) (fuel : nat) (lks : gset string) :
    (K_itrunc <= K)%nat ->
    log_geom_ok cov logstart ->
    0 < size <= BPB ->
    0 <= bmapstart ->
    ...
    it_dexit (CID0 := CID0) γ γfs bn cov logstart bmapstart size used dev
             ip bm data pidv dq dqd dqb jx crb Sb e0 w m K b eb lks -∗
    WP (Loop : expr riscv_lang).
  Proof.
    ...
    iIntros "Hcg Hcnt Hextc Hextm #Htext #Hkd Hpc #Hpanic #Hpenv #Hbio #Hlctx #Hprocs Hppid Hidev Hsbb #Hdevi #Hdgeom #Hdlock Hsl Hst Hexit".
    iDestruct (cpu_own_eb_agree with "Hcg Hcnt") as %Hbm.
    pose proof HK as HK'. 
    ...
    - (* ---------- SKIP: the slot is already empty ---------- *)
      ...
      + (* more entries to go: round again *)
        ...
        iApply (IH CID4 (S k) L1 Hk' Hf' HL1sp HL1thr HL1s1 HL1s2 HL1s3 Hlkbelow
                  with "Hcg Hcnt Hextc Hextm Htext Hkd Hpc Hpanic Hpenv Hbio Hlctx Hprocs Hppid Hidev Hsbb Hdevi Hdgeom Hdlock Hsl
                        Hst Hexit").
    - (* ---------- FREE: the slot names a block ---------- *)
      ...
      iApply (BF.wp_bfree_gen γs jx γl γu γd γk pd pav pu bn γ γfs
                ...
                Hj Hgl HL3a0 HL3a1
                Hlkbelow
                with "Hcg Hcnt Hextc Hextm Htext Hkd Hpc Hpanic Hpenv Hbio Hlctx Hsbb [Hbmr] Hfsb Htok
...
Section ItruncELoop.
  ...
  Local Lemma it_eloop `{GEN : GenId} `{CID0 : CpuId} 
    ...
    it_eexit (CID0 := CID0) γ γfs bn γd cov logstart bmapstart size used dev
             ip bm data kk dsk pidv dq dqd dqb jx crb Sb e0 w m K b eb lks -∗
    WP (Loop : expr riscv_lang).
  Proof.
    ...
    iIntros "Hcg Hcnt Hextc Hextm #Htext #Hkd Hpc #Hpanic #Hpenv #Hbio #Hlctx #Hprocs Hppid Hidev Hsbb #Hdevi #Hdgeom #Hdlock Hsl Hbuf Hst Hexit".
    iDestruct (cpu_own_eb_agree with "Hcg Hcnt") as %Hbm.
    pose proof HK as HK'. 
...
Section ItruncIArm.
  ...
  Local Lemma it_iarm `{GEN : GenId} `{CID0 : CpuId}       (γs : list gname) (jx : nat) (γl : gname)
    ...
    it_armexit (CID0 := CID0) γ γfs bn cov logstart bmapstart size used dev
               ip bm pidv dq dqd dqb jx crb Sb e0 w m K b eb lks -∗
    WP (Loop : expr riscv_lang).
  Proof.
    intros HK Hgeom Hsize Hbm0 Hbmcov Hbmlog Hwf Hrange Hblen Hindnz Hj Hgl
           Hsp Hthr Hs3 Ha1 Hlkbelow.
    pose proof HK as HK'. 
    iIntros "Hcg Hcnt Hextc Hextm #Htext #Hkd Hpc #Hpanic #Hpenv #Hbio #Hlctx #Hprocs Hppid Hidev Hsbb #Hdevi #Hdgeom #Hdlock Hslot6 Hsl Hmap
    ...
    iApply (BR.wp_bread_sconf γs jx γl γu γd γk pd pav pu bn
              ...
              ltac:(lkbelow)
              with "Hcg Hcnt Hextc Hextm Htext Hkd Hpc Hpanic Hpenv Hbio Hppid Hprocs Hdevi Hdgeom Hdlock Hsl1").
    all: try lkbelow.
    ...
    iApply (it_eloop (CID0 := CID8) γs jx γl γu γd γk pd pav pu bn γ γfs
              ...
              with "Hcg Hcnt Hextc Hextm Htext Hkd Hpc Hpanic Hpenv Hbio Hlctx Hprocs Hppid Hidev Hsbb Hdevi Hdgeom Hdlock Hsl Hbuf Hst").
    ...
    iApply (BF.wp_bfree_gen γs jx γl γu γd γk pd pav pu bn γ γfs
              ...
              pidv dq dqb C2 (K - 6)%nat eb b lks
              ...
              Hj Hgl HC2a0 HC2a1
              Hlkbelow
              with "Hcg Hcnt Hextc Hextm Htext Hkd Hpc Hpanic Hpenv Hbio Hlctx Hsbb Hbmr Hindblk
...
Section ItruncMain.
  ...
  Proof.
    ...
    iIntros "Hcg Hcnt Hextc Hextm #Htext #Hkd Hpc #Hpanic #Hpenv #Hbio #Hlctx Hidev Hinum Hmeta Hmap
    ...
    iApply (it_dloop (CID0 := CID11x) γs j γl γu γd γk pd pav pu bn γ γfs
              ...
              with "Hcg Hcnt Hextc Hextm Htext Hkd Hpc Hpanic Hpenv Hbio Hlctx Hprocs
