...
Section ItruncTail.
  ...
  Local Lemma it_tail `{GEN : GenId} `{CID0 : CpuId} 
      ...
      (logstart bmapstart inodestart size : Z) (nib : nat)
      (dev : mword 32)
    ...
    it_cont (CID0 := CID0) γ γi bn logstart bmapstart inodestart size
  ...
  Proof.
   ...
   iDestruct (cpu_own_transport CID0 CID3 0 eb (proc_addr j) b 
    ...
    iApply (IU.wp_iupdate_credgen γs j γl γu γd γk pd pav pu bn γ γi
              logstart inodestart nib dev ip inum (di_trunc dn) dn0
    ...
    iDestruct (cpu_own_transport CID4 CID11 0 eb (proc_addr j) b 
                 ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
    ...
    iApply ("Hcont" $! P6 with "[%] Hcg Hcnt Hextc Hextm Hpc Hppid Hidev Hinum
                                 Hsbb Hsbi Hmeta Hmap Hblks Hdn [Hsl Hslp]
                                 [Hop]").
...
Section ItruncDLoop.
  ...
  Definition it_dexit `{GEN : GenId} `{CID0 : CpuId} 
    ...
    wp_next true (proc_addr j) (fun (CID : CpuId) =>
      ∀ Mx : regfile,
        ...
        sb_bmapstart ↦₄{dqb} (mword_of_int bmapstart : mword 32) -∗
        bslots 2 -∗
        it_dir_state γ fsc_fs ip bm data fsc_cov logstart bmapstart size bn
                     crb Sb e0 w NDIRECT -∗
        WP (Loop : expr riscv_lang))%I.
  ...
  Local Lemma it_dloop `{GEN : GenId} `{CID0 : CpuId} 
      ...
      (logstart bmapstart size : Z) (dev : mword 32)
    ...
    is_lock γk d_lock "virtio_disk"%string (disk_res γd pd pav pu) -∗
    bslots 2 -∗
    it_dir_state γ fsc_fs ip bm data fsc_cov logstart bmapstart size bn crb Sb e0 w k -∗
    it_dexit (CID0 := CID0) γ bn logstart bmapstart size dev
  ...
  Proof.
    ...
    iIntros "Hcg Hcnt Hextc Hextm #Htext #Hkd Hpc #Hpenv #Hbio #Hlctx #Hprocs Hppid Hidev Hsbb #Hbmi #Hdevi #Hdgeom #Hdlock Hsl Hst Hexit".
    iDestruct (cpu_own_eb_agree with "Hcg Hcnt") as %Hbm.
    pose proof HK as HK'. 
    ...
    iDestruct (inode_map_dir_acc fsc_fs ip (bm_dir_zeroed bm k) k Hzlen
    ...
    - (* ---------- SKIP: the slot is already empty ---------- *)
      ...
      iAssert (it_dir_state γ fsc_fs ip bm data fsc_cov logstart bmapstart size
                            bn crb Sb e0 w (S k))
      ...
      + (* more entries to go: round again *)
        ...
        iApply (IH CID4 (S k) L1 Hk' Hf' HL1sp HL1thr HL1s1 HL1s2 HL1s3 Hlkbelow
                  with "Hcg Hcnt Hextc Hextm Htext Hkd Hpc Hpenv Hbio Hlctx Hprocs Hppid Hidev Hsbb Hbmi Hdevi Hdgeom Hdlock Hsl
                        Hst Hexit").
    - (* ---------- FREE: the slot names a block ---------- *)
      ...
      assert (Hgk : blkmap_get (bm_dir_zeroed bm k) k = bm_dir bm !!! k)
        by exact Hcur.
      iDestruct (inode_blocks_take fsc_fs (bm_dir_zeroed bm k)
      ...
      iApply (BF.wp_bfree_gen γs jx γl γu γd γk pd pav pu bn γ fsc_fs
                fsc_cov logstart bmapstart size dev
      ...
      { iApply (iti_1c with "Htext"). }
        ...
        iEval (rewrite Htgt32') in "Hpc".
        rewrite /it_dexit.
        iDestruct (cpu_own_transport CID4 CID8 0 eb (proc_addr jx) b
                     ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
...
Section ItruncELoop.
  ...
  Definition it_eexit `{GEN : GenId} `{CID0 : CpuId} 
      (γ : log_names) (bn : bio_names) (γd : disk_names)
      (logstart bmapstart size : Z)
      (dev : mword 32) (ip : mword 64) (bm : blkmap)
    ...
    wp_next true (proc_addr j) (fun (CID : CpuId) =>
      ∀ Mx : regfile,
        ...
        it_ent_state γ fsc_fs bm data fsc_cov logstart bmapstart size
  ...
  Local Lemma it_eloop `{GEN : GenId} `{CID0 : CpuId} 
    ...
    bslots 2 -∗
    buf_own (bpa kk) (bm_ind bm) dsk (ind_bytes (bm_ent bm)) -∗
    it_ent_state γ fsc_fs bm data fsc_cov logstart bmapstart size crb Sb e0 w q -∗
    it_eexit (CID0 := CID0) γ bn γd logstart bmapstart size dev
  ...
  Proof.
    ...
    iIntros "Hcg Hcnt Hextc Hextm #Htext #Hkd Hpc #Hpenv #Hbio #Hlctx #Hprocs Hppid Hidev Hsbb #Hbmi #Hdevi #Hdgeom #Hdlock Hsl Hbuf Hst Hexit".
    iDestruct (cpu_own_eb_agree with "Hcg Hcnt") as %Hbm.
    pose proof HK as HK'. 
    pose proof (blkmap_wf_ent_len _ _ _ Hwf) as Hentlen.
    iDestruct (it_ent_state_open with "Hst") as "(Hres & Hpaid)".
    ...
    iDestruct (it_ent_res_peel fsc_fs bm data q ltac:(exact Hq) with "Hres")
      as "[Hblk Hres]".
    (* ===== +0x6e c.beqz a1 ===== *)
    destruct (decide (bv_unsigned (bm_ent bm !!! q : mword 32) = 0))
      as [Hzero|Hnzero].
    - (* ---------- SKIP ---------- *)
      iAssert (it_ent_state γ fsc_fs bm data fsc_cov logstart bmapstart size crb Sb e0 w (S q))
      ...
      { iApply (iti_68 with "Htext"). }
        ...
        rewrite /it_eexit.
        iDestruct (cpu_own_transport CID0 CIDb 0 eb (proc_addr jx) b
                     ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
    ...
    - (* ---------- FREE ---------- *)
      ...
      iDestruct (blk_res_nz fsc_fs (bm_ent bm !!! q) (data (NDIRECT + q)%nat)
      ...
      { iApply (iti_68 with "Htext"). }
        ...
        rewrite /it_eexit.
        iDestruct (cpu_own_transport CIDf CIDr 0 eb (proc_addr jx) b
                     ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
...
Section ItruncIArm.
  ...
  Local Lemma it_iarm `{GEN : GenId} `{CID0 : CpuId}       (γs : list gname) (jx : nat) (γl : gname)
    ...
    bm_paidS γ bmapstart crb w Sb e0 -∗
    it_armexit (CID0 := CID0) γ bn logstart bmapstart size dev
  ...
  Proof.
    ...
    pose proof HK as HK'. 
    iIntros "Hcg Hcnt Hextc Hextm #Htext #Hkd Hpc #Hpenv #Hbio #Hlctx #Hprocs Hppid Hidev Hsbb #Hbmi #Hdevi #Hdgeom #Hdlock Hslot6 Hsl Hmap
              Hres Hpaid Hexit".
    iDestruct (cpu_own_eb_agree with "Hcg Hcnt") as %Hbm.
    pose proof (blkmap_wf_dir_len _ _ _ Hwf) as Hdirlen.
    pose proof (blkmap_wf_ent_len _ _ _ Hwf) as Hentlen.
    ...
    iDestruct (inode_map_ind_acc fsc_fs ip (bm_dir_zeroed bm NDIRECT) Hzlen
    ...
    assert (Hentz : bm_ent (bm_dir_zeroed bm NDIRECT) = bm_ent bm)
      by (rewrite /bm_dir_zeroed; reflexivity).
    iDestruct (ind_res_nz fsc_fs (bm_dir_zeroed bm NDIRECT)
    ...
    iApply (BR.wp_bread_sconf γs jx γl γu γd γk pd pav pu bn
              (fs_view fsc_fs γd dev fsc_cov) pidv dev (bm_ind bm : mword 32) dq
    ...
    iMod (bm_held_content ⊤ bn fsc_fs γd (DfracOwn 1) dev fsc_cov kk pidv dev
    ...
    iAssert (it_ent_state γ fsc_fs bm data fsc_cov logstart bmapstart size crb Sb e0 w 0)
      with "[Hres Hpaid]" as "Hst".
    { iApply (it_ent_state_close with "Hres Hpaid"). }
    ...
    iDestruct (cpu_claim_ext_transport CID4 CID8 eb (proc_addr jx)
                 ltac:(rewrite Hbm; wp_next_chain) with "Hextm") as "Hextm".
    iApply (it_eloop (CID0 := CID8) γs jx γl γu γd γk pd pav pu bn γ
              logstart bmapstart size dev ip bm data kk
    ...
    iApply (wp_cmv_s_sconf (mword_of_int (IT + 0x7a)) Ra0 Rs4
              Mx (K - 6)%nat b ltac:(nz) ltac:(rdok) with "Hcg Hpc []").
    ...
    iApply (wp_lw_s_sconf (kt := KT1) (ktd := KT0) (mword_of_int (IT + 0x80)) Ra1 Rs3
              ...
              ltac:(nz) ltac:(rdok) with "Hcg Hpc [] Hindcell").
    { iApply (iti_80 with "Htext"). }
    ...
    iApply (wp_sw_s_sconf (mword_of_int (IT + 0x8c)) Rz Rs3
              ...
              (bm_ind bm : mword 32) b with "Hcg Hpc [] Hindcell").
    ...
    iApply (wp_cldsp_s_sconf (mword_of_int (IT + 0x90))
              ...
              ltac:(nz) ltac:(rdok) with "Hcg Hpc [] [Hslot6]").
    ...
    rewrite /it_armexit.
    iDestruct (cpu_own_transport CID16 CID19 0 eb (proc_addr jx) b
                 ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
...
Section ItruncMain.
  ...
  Proof.
    ...
    assert (Hrange : forall i : nat, (i <= MAXFILE)%nat ->
              ...
              bv_unsigned (bm_slot bm i) < size).
    { intros i Hi Hnz.
      exact (proj2 (blkmap_slot_inrange fsc_cov logstart size bm
    ...
    iApply (wp_caddi16sp_push_s_sconf pcE (mword_of_int 61 : mword 6)
              m K 6 b ltac:(lia) Hpush with "Hcg Hpc []").
    ...
    iApply (wp_cmv_s_sconf (mword_of_int (IT + 0x0e)) Rs3 Ra0
              Q0 (K - 6)%nat b ltac:(nz) ltac:(rdok) with "Hcg Hpc []").
    ...
    iDestruct "Hsl" as "[Hsl Hslp]".
    iDestruct (cpu_own_transport CID CID11x 0 eb (proc_addr j) b
                 ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
    iDestruct (trap_csrs_ext_transport CID CID11x eb (proc_addr j)
                 ltac:(rewrite Hbm; wp_next_chain) with "Hextc") as "Hextc".
    iDestruct (cpu_claim_ext_transport CID CID11x eb (proc_addr j)
                 ltac:(rewrite Hbm; wp_next_chain) with "Hextm") as "Hextm".
    iApply (it_dloop (CID0 := CID11x) γs j γl γu γd γk pd pav pu bn γ
              logstart bmapstart size dev ip bm data
    ...
    - (* ---------- NO indirect block: straight to the tail ----------
      ...
      pose proof (blkmap_wf_no_ind fsc_cov logstart bm Hwf Hnoind) as Hentzero.
      ...
      iPoseProof (log_credit_mono γ cru Sb Sq e0 (IBLOCK inum inodestart) Hqsub
                    with "Hcru") as "#Hcru1".
      iDestruct (cpu_own_transport CID12x CID14x 0 eb (proc_addr j) b
                   ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
      ...
      iApply (it_tail (CID0 := CID14x) γs j γl γu γd γk pd pav pu bn γ γi
                logstart bmapstart inodestart size nib dev ip inum
                dn dn0 bm
                ...
                with "Hcg Hcnt Hextc Hextm Htext Hkd Hpc Hpenv Hbio Hlctx Hprocs
                      ...
                      Hmeta Hmap Hblks Hireg Hdn Hdevi Hdgeom Hdlock
      ...
      iApply ("Hcont" $! mf with "[%] Hsie Hcnt Hextc Hextm Hpc Hppid Hidev Hinum
                                  Hsbb Hsbi Hmeta Hmap Hblks Hdn Hsl
                                  [Hop]");
        [exact Hcs |].
    ...
    - (* ---------- there is one: take the arm ---------- *)
      ...
      iDestruct (inode_blocks_to_ent_res fsc_fs bm data Hdirlen Hentlen
                   with "Hblks") as "Hres0".
      rewrite /it_frame.
      iDestruct "Hframe" as "(Hf1 & Hf2 & Hf3 & Hf4 & Hf5 & Hf6)".
      iDestruct (cpu_own_transport CID12x CID14y 0 eb (proc_addr j) b
                   ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
      ...
      iDestruct (cpu_claim_ext_transport CID12x CID14y eb (proc_addr j)
                   ltac:(rewrite Hbm; wp_next_chain) with "Hextm") as "Hextm".
      iApply (it_iarm (CID0 := CID14y) γs j γl γu γd γk pd pav pu bn γ
                logstart bmapstart size dev ip bm data
      ...
      iApply (it_tail (CID0 := CID15y) γs j γl γu γd γk pd pav pu bn γ γi
                ...
                with "Hcg Hcnt Hextc Hextm Htext Hkd Hpc Hpenv Hbio Hlctx Hprocs
                      ...
                      Hsbi Hmeta Hmap Hblks Hireg Hdn Hdevi Hdgeom Hdlock
                      Hsl Hcru2 Hop [Hcont]").
