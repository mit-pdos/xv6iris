...
Section ProofKalloc.
  ...
  Proof.
    ...
    iDestruct (cpu_own_transport CID CID8 n eb p b ltac:(wp_next_chain)
                 with "Hcnt") as "Hcnt".
    iApply (Acquire.wp_acquire_sconf kt γl "kmem"%string (λ ξ : CtxId, kmem_res (XIk := ξ) γk fl) mA
    ...
    - (* ===== EMPTY: head=nullp, taken to +0x4c ===== *)
      ...
      iApply (Release.wp_release_sconf kt γl (mword_of_int KernelSyms.kmem) "kmem"%string (λ ξ : CtxId, kmem_res (XIk := ξ) γk fl) E3
    ...
    - (* ===== NONEMPTY: head=pg, pop + release + memset(p,5,4096) ===== *)
      ...
      (* the acquire handed the window index out as [trap_res b + N]; release
         ...
         acquire/release pair compose back to [N]. *)
      iEval (rewrite Hbmatch) in "Hcg".
      iApply (Release.wp_release_sconf kt γl (mword_of_int KernelSyms.kmem) "kmem"%string (λ ξ : CtxId, kmem_res (XIk := ξ) γk fl) R12
                n eb p (K - 4)%nat ({["kmem"]} ∪ lks)
