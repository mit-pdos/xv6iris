...
Proof.
  rewrite (ByteCursor.add_vec_comm zero_reg x).
...
Section ProofVirtioDiskRwE.
  ...
  (* ------------------------------------------------------------------- *)
  Definition vdrw_p5_exit (γ γk : gname) (Φ : mval -> iProp Σ)
    ...
    (∀ (M : regfile) (q np nr : nat) (fl pk : gmap nat dclaim)
       ...
       trap_csrs_pay 0 eb -∗
       pc_is (mword_of_int (VRW + 0x1b0) : mword 64) -∗
  ...
  Definition vdrw_p5_loop (γ γk : gname) (Φ : mval -> iProp Σ)
      (γs : list gname) (j : nat) (γd : disk_names)
      (pd pav pu : SailStdpp.Values.mword 64) (K : nat) (eb : bool) (C : iProp Σ)
      (sp0 b : Arch.pa) (wr sector : SailStdpp.Values.mword 64)
      (bs_buf bs_disk : list (bv 8)) (h m2 t : nat) (q : nat) (pin : _) : iProp Σ :=
    (∀ M : regfile,
       ⌜vdrw_regs M sp0 b wr sector
       ...
       cpu_own γ 1 eb (proc_addr j) C -∗
       trap_csrs_pay 0 eb -∗
       pc_is (mword_of_int (VRW + 0x1a0) : mword 64) -∗
       own_ctx (p_context (proc_addr j)) -∗
       ▷ sched_vc γ Φ γs (a_cpu_ctx cid_word) (proc_addr j) -∗
       locked γk cpu_id -∗
       ...
       ⌜pin ∖ range_map (d_ring pav (q `mod` 8)) 2
        ...
        /\ pm_ok (vdrwd_pinr_regions pd b h m2 t wr sector
                    (vdrwd_bufwin b wr bs_buf))⌝ -∗
       vdrw_p5_exit γ γk Φ γs j γd pd pav pu K eb C sp0 b wr sector bs_buf bs_disk -∗
       WP (Loop : expr riscv_lang) {{ Φ }})%I.
  ...
  (* ------------------------------------------------------------------- *)
  Lemma wp_vdrw_p5_seam (γ γk : gname) (Φ : mval -> iProp Σ)
      (γs : list gname) (j : nat) (γl : gname)
      (γu : uart_names) (γd : disk_names)
      (pd pav pu : SailStdpp.Values.mword 64) (K : nat) (eb : bool) (C : iProp Σ)
      (sp0 b : Arch.pa) (wr sector : SailStdpp.Values.mword 64)
      (bs_buf bs_disk : list (bv 8)) :
    ...
    vdrw_p5_exit γ γk Φ γs j γd pd pav pu K eb C sp0 b wr sector bs_buf bs_disk -∗
    P4.vdrw_p4_exit γ γk Φ γs j γd pd pav pu K eb C sp0 b wr sector bs_buf bs_disk.
  Proof.
    intros HK Hj Hjl.
    iIntros "#Htext #Hpanic #Hpinv #Hdinv #Hlk Hexit".
    rewrite /P4.vdrw_p4_exit.
    iIntros (M q np nr fl pk tr fr h m2 t pin)
            "%Hregs %Ha1 %Hok %Hpinr %Hal Hcg Hown Hpay Hpc Hctx Hsched Htok Hbody Hclaim Hrm Hrt Hidx".
    ...
    iAssert (vdrw_p5_loop γ γk Φ γs j γd pd pav pu K eb C sp0 b wr sector
               bs_buf bs_disk h m2 t q pin)%I with "[]" as "Hloop".
    { iLöb as "IH". rewrite {2}/vdrw_p5_loop.
      iIntros (M') "%Hinv Hcg Hown Hpay Hpc Hctx Hsched Htok HR Hclaim Hrm Hrt Hidx
                    %HokL %HalL %HpinrL HexitL".
      destruct Hinv as (HregsL & Hs1L & Hs2L).
      pose proof HregsL as HregsL'.
      destruct HregsL' as (HspL & Hs0L & Hs3L & Hs6L & Hs7L & HtpL).
      ...
      (* ---- +0x1a0  c.mv a1,s2 ---- *)
      iApply (wp_cmv_s_sconf γ Φ (mword_of_int (VRW + 0x1a0) : mword 64) Ra1 Rs2 M'
                (K - 12)%nat ltac:(vm_compute; discriminate)
                ltac:(vm_compute; discriminate) with "Hcg Hpc Hi1a0 [-]").
      iIntros "Hcg Hpc".
      ...
      assert (Hp1a2 : add_vec_int (mword_of_int (VRW + 0x1a0) : mword 64) 2
                      = mword_of_int (VRW + 0x1a2)) by pcstep.
      iEval (rewrite Hp1a2) in "Hpc".
      (* ---- +0x1a2  c.mv a0,s3 ---- *)
      iApply (wp_cmv_s_sconf γ Φ (mword_of_int (VRW + 0x1a2) : mword 64) Ra0 Rs3 L1
                (K - 12)%nat ltac:(vm_compute; discriminate)
                ltac:(vm_compute; discriminate) with "Hcg Hpc Hi1a2 [-]").
      iIntros "Hcg Hpc".
      set (L2 := <[Regidx Ra0 := regval_into_reg
                    (add_vec zero_reg (L1 !!! Regidx Rs3))]> L1).
      change (<[Regidx Ra0 := regval_into_reg
                    (add_vec zero_reg (L1 !!! Regidx Rs3))]> L1) with L2.
      assert (Hp1a4 : add_vec_int (mword_of_int (VRW + 0x1a2) : mword 64) 2
                      = mword_of_int (VRW + 0x1a4)) by pcstep.
      iEval (rewrite Hp1a4) in "Hpc".
      (* ---- +0x1a4  jal ra,sleep ---- *)
      iApply (wp_jal_s_sconf γ Φ (mword_of_int (VRW + 0x1a4) : mword 64) Rra
                (mword_of_int 2082322 : mword 21) L2 (K - 12)%nat
                ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                ltac:(vm_compute; reflexivity) with "Hcg Hpc Hi1a4 [-]").
      iIntros "Hcg Hpc".
      set (L3 := <[Regidx Rra := regval_into_reg
                    (add_vec_int (mword_of_int (VRW + 0x1a4) : mword 64) 4)]> L2).
      change (<[Regidx Rra := regval_into_reg
                    (add_vec_int (mword_of_int (VRW + 0x1a4) : mword 64) 4)]> L2) with L3.
      assert (Hjsl : add_vec (mword_of_int (VRW + 0x1a4) : mword 64)
      ...
      { rewrite /L3 upd_ne; [| reg_neq]. rewrite /L2 upd_ne; [| reg_neq].
        rewrite /L1 upd_ne; [| reg_neq]. exact HtpL. }
      ...
      assert (HL3ra : L3 !!! Regidx Rra
                      = add_vec_int (mword_of_int (VRW + 0x1a4) : mword 64) 4)
      ...
      iApply (wp_lw_s_sconf γ Φ (mword_of_int (VRW + 0x1a8) : mword 64) Ra5 Rs3
                (mword_of_int 4 : mword 12) Mf (K - 12)%nat dv (dqm := DfracOwn 1)
                ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                with "Hcg Hpc Hi1a8 [Hbd] [-]").
      { iEval (rewrite Haddr). rewrite /V. cbn [dc_buf]. iExact "Hbd". }
      iIntros "Hcg Hpc Hbd".
      ...
      assert (Hp1ac : add_vec_int (mword_of_int (VRW + 0x1a8) : mword 64) 4
                      = mword_of_int (VRW + 0x1ac)) by pcstep.
      ...
      + (* still in flight: TAKEN, back to +0x1a0 *)
        iApply (wp_beq_taken_s_sconf γ Φ (mword_of_int (VRW + 0x1ac) : mword 64)
                  (mword_of_int 8180 : mword 13) Rs1 Ra5 L4 (K - 12)%nat
                  ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                  ltac:(rewrite HL4a5 HL4s1 vdrwe_sext_one; exact vdrwe_eq_one)
                  ltac:(vm_compute; reflexivity) with "Hcg Hpc Hi1ac [-]").
        iNext. iIntros "Hcg Hpc".
        assert (Hback : add_vec (mword_of_int (VRW + 0x1ac) : mword 64)
                          (sign_extend' 64 (mword_of_int 8180 : mword 13))
                        = mword_of_int (VRW + 0x1a0)) by pcstep.
        ...
        * split_and!; [| exact HL4s1 |].
          -- unfold vdrw_regs. destruct HMfregs as (F1 & F2 & F3 & F4 & F5 & F6).
             split_and!;
               ...
               | rewrite /L4 upd_ne; [exact F6 | reg_neq] ].
          -- rewrite /L4 upd_ne; [exact HMfs2 | reg_neq].
        * exact HokL.
        * exact HalL.
        * exact HpinrL.
      + (* parked: FALL THROUGH to +0x1b0 *)
        iApply (wp_beq_fall_s_sconf γ Φ (mword_of_int (VRW + 0x1ac) : mword 64)
                  (mword_of_int 8180 : mword 13) Rs1 Ra5 L4 (K - 12)%nat
                  ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                  ltac:(rewrite HL4a5 HL4s1 vdrwe_sext_zero; exact vdrwe_eq_zero_one)
                  with "Hcg Hpc Hi1ac [-]").
        iIntros "Hcg Hpc".
        assert (Hp1b0 : add_vec_int (mword_of_int (VRW + 0x1ac) : mword 64) 4
                        = mword_of_int (VRW + 0x1b0)) by pcstep.
        iEval (rewrite Hp1b0) in "Hpc".
        iApply ("HexitL" $! L4 q np' nr' fl' pk' tr' fr' h m2 t pin with
                  "[%] [%] [%] [%] [%] Hcg Hown Hpay Hpc Hctx Hsched Htok Hbody Hclaim
                   Hrm Hrt Hidx").
        * unfold vdrw_regs. destruct HMfregs as (F1 & F2 & F3 & F4 & F5 & F6).
          split_and!;
            [ rewrite /L4 upd_ne; [exact F1 | reg_neq]
    ...
    (* ---- +0x186  lui a5,0x10001 ---- *)
    iApply (wp_lui_s_sconf γ Φ (mword_of_int (VRW + 0x186) : mword 64) Ra5
              (mword_of_int 65537 : mword 20)
              (mword_of_int 0x10001000 : SailStdpp.Values.mword 64) M (K - 12)%nat
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
    ...
    assert (Hp18a : add_vec_int (mword_of_int (VRW + 0x186) : mword 64) 4
                    = mword_of_int (VRW + 0x18a)) by pcstep.
    iEval (rewrite Hp18a) in "Hpc".
    (* ---- +0x18a  sw x0,80(a5) : *R(QUEUE_NOTIFY) = 0 ---- *)
    iDestruct (sie_cap_gpr_x0 γ N1 (K - 12)%nat (mword_of_int 0 : mword 5)
    ...
    assert (Ha8 : sign_extend' 64 (subrange_vec_dec
                    (add_vec (N1 !!! Regidx Ra5) (sign_extend' 64 (mword_of_int 80 : mword 12)))
                    (xlen - 0 - 1) 0)
                  = (mword_of_int 0x10001050 : SailStdpp.Values.mword 64)).
    { rewrite subrange_id sign_extend'_id. exact Hnaddr. }
    ...
    iApply (wp_sw_virtio_dev_s_sconf γ γu γd Φ (mword_of_int (VRW + 0x18a) : mword 64)
              ...
              ltac:(rewrite Ha8 Hsw; exact Hwrite)
              with "Hcg Hpc Hi18a Hdinv [-]").
    iIntros "Hcg Hpc".
    assert (Hp18e : add_vec_int (mword_of_int (VRW + 0x18a) : mword 64) 4
                    = mword_of_int (VRW + 0x18e)) by pcstep.
    ...
    iApply (wp_lw_s_sconf γ Φ (mword_of_int (VRW + 0x18e) : mword 64) Ra5 Rs3
              (mword_of_int 4 : mword 12) N1 (K - 12)%nat dv (dqm := DfracOwn 1)
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi18e [Hbd] [-]").
    { iEval (rewrite Haddr0). rewrite /V. cbn [dc_buf]. iExact "Hbd". }
    iIntros "Hcg Hpc Hbd".
    ...
    assert (Hp192 : add_vec_int (mword_of_int (VRW + 0x18e) : mword 64) 4
                    = mword_of_int (VRW + 0x192)) by pcstep.
    iEval (rewrite Hp192) in "Hpc".
    (* ---- +0x192 / +0x196  s2 := &disk.vdisk_lock ---- *)
    iApply (wp_auipc_s_sconf γ Φ (mword_of_int (VRW + 0x192) : mword 64) Rs2
              (mword_of_int 30 : mword 20) N2 (K - 12)%nat
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi192 [-]").
    iIntros "Hcg Hpc".
    set (N3 := <[Regidx Rs2 := regval_into_reg
                  (add_vec (mword_of_int (VRW + 0x192) : mword 64)
                           (auipc_off (mword_of_int 30 : mword 20)))]> N2).
    change (<[Regidx Rs2 := regval_into_reg
                  (add_vec (mword_of_int (VRW + 0x192) : mword 64)
                           (auipc_off (mword_of_int 30 : mword 20)))]> N2) with N3.
    assert (Hp196 : add_vec_int (mword_of_int (VRW + 0x192) : mword 64) 4
                    = mword_of_int (VRW + 0x196)) by pcstep.
    iEval (rewrite Hp196) in "Hpc".
    iApply (wp_addi4_s_sconf γ Φ (mword_of_int (VRW + 0x196) : mword 64) Rs2 Rs2
              (mword_of_int 3166 : mword 12) N3 (K - 12)%nat
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi196 [-]").
    iIntros "Hcg Hpc".
    ...
    assert (Hp19a : add_vec_int (mword_of_int (VRW + 0x196) : mword 64) 4
                    = mword_of_int (VRW + 0x19a)) by pcstep.
    ...
    iApply (wp_cmv_s_sconf γ Φ (mword_of_int (VRW + 0x19a) : mword 64) Rs1 Ra1 N4
              (K - 12)%nat ltac:(vm_compute; discriminate)
              ltac:(vm_compute; discriminate) with "Hcg Hpc Hi19a [-]").
    iIntros "Hcg Hpc".
    ...
    assert (Hp19c : add_vec_int (mword_of_int (VRW + 0x19a) : mword 64) 2
                    = mword_of_int (VRW + 0x19c)) by pcstep.
    iEval (rewrite Hp19c) in "Hpc".
    (* ---- +0x19c  bne a5,a1 ---- *)
    destruct Hdv as [-> | [-> Hpq] ].
    - (* IN FLIGHT: fall through into the wait loop *)
      iApply (wp_bne_fall_s_sconf γ Φ (mword_of_int (VRW + 0x19c) : mword 64)
                (mword_of_int 20 : mword 13) Ra1 Ra5 N5 (K - 12)%nat
                ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                ltac:(rewrite HN5a5 HN5a1 vdrwe_sext_one; exact vdrwe_neq_one)
                with "Hcg Hpc Hi19c [-]").
      iIntros "Hcg Hpc".
      assert (Hp1a0 : add_vec_int (mword_of_int (VRW + 0x19c) : mword 64) 4
                      = mword_of_int (VRW + 0x1a0)) by pcstep.
    ...
    - (* ALREADY PARKED: the branch is taken, straight to +0x1b0 *)
      iApply (wp_bne_taken_s_sconf γ Φ (mword_of_int (VRW + 0x19c) : mword 64)
                (mword_of_int 20 : mword 13) Ra1 Ra5 N5 (K - 12)%nat
                ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                ltac:(rewrite HN5a5 HN5a1 vdrwe_sext_zero; exact vdrwe_neq_zero_one)
                ltac:(vm_compute; reflexivity) with "Hcg Hpc Hi19c [-]").
      iNext. iIntros "Hcg Hpc".
      assert (Hb1b0 : add_vec (mword_of_int (VRW + 0x19c) : mword 64)
