...
(* into [assert]s (execcmd's own shape) does too.                         *)
(* Nor is SEVEN-vs-THREE the discriminator: [wp_kshp_peek_epi] applies     *)
(* [wp_kshp_restore] to the SAME seven-entry list in milliseconds.  What   *)
(* is left is that spill's frame premise carries an EXISTENTIAL UNDER THE  *)
(* BIG-OP where restore's does not; the worklist names the next probe.     *)
(*                                                                        *)
