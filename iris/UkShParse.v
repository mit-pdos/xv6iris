...
(* WHICH TACTIC, re-measured 2026-09-01 and now pinned exactly: it is the  *)
(* body's [iApply (wp_kshp_spill spn (2 + nn) [seven entries] ...)].  Cut  *)
(* immediately BEFORE it the file is green in 13.8 s at 1.1 GB; cut        *)
(* immediately AFTER it -- with neither its instruction bullet nor the     *)
(* [iIntros] that follows -- it runs to 17.5 GB and is still climbing when *)
(* killed at 300 s.  Round 2 blamed the tactic AFTER the spill run's       *)
(* continuation and was off by one: the bullet, the [iIntros] and the      *)
(* [cbn [length]] are innocent, they were only ever measured downstream of *)
(* a tactic that had already exploded.                                     *)
(*                                                                        *)
(* WHAT IT IS NOT.  Five hypotheses tested and all five REFUTED, each by   *)
...
(* reproduces it, and so does clearing every PURE one and [clearbody] on   *)
(* every [set]; (d) not the durable notes' unsealed-big-op rule --         *)
(* [Typeclasses Opaque] on [ubytesq]/[ubytes]/[uwordq]/[uword]/[ustr]/     *)
(* [ustack]/[ustack_body] (none of which carries a seal today) changes     *)
(* nothing; (e) NOT the end-pc -- giving both runs an explicit [pend]      *)
(* parameter with the premise [pcs (length rs) = pend], literal at every   *)
(* call site, costs the SAME 17 GB, and hoisting the two [ltac:] premises  *)
(* into [assert]s (execcmd's own shape) does too; (f) NOT the existential  *)
(* under the big-op and NOT the arity -- a STANDALONE rig applying         *)
(* [wp_kshp_spill] at k = 3..7 in a two-line context, with peek's own      *)
(* list, pcs, addresses and [ltac:] premises, measures 0.075 / 0.096 /     *)
(* 0.118 / 0.139 / 0.167 s, i.e. LINEAR at ~24 ms a spill, and an          *)
(* exists-FREE variant of the lemma measures the same to within noise.     *)
(* Adding [shp_code] to that rig's persistent context: 0.170 s.  Moving    *)
(* the rig to the END of this file: 0.171 s.  So the same lemma at the     *)
(* same seven entries is a SIXTH OF A SECOND here and 17.5 GB in peek --   *)
(* a factor of >1800 -- and the difference is not the lemma, the list, the *)
(* premises, the context or the file.                                      *)
(* WHAT IS LEFT: the rig writes [Hrun], [Hsl], [m] and [spn] OUT BY HAND   *)
(* where peek gets the same propositions from TACTICS, and only five       *)
(* tactics separate the two states.  Bisect FORWARD from the rig, one      *)
(* tactic at a time.  Named favourite: peek pushes with [c.addi16sp]       *)
(* ([wp_uk_caddi16sp_dn]) where execcmd -- the fast 3-spill site -- pushes *)
(* with [c.addi], and [wp_kshp_peek_epi] -- the fast 7-entry restore --    *)
(* has no push in front of it at all.  The worklist has the detail.        *)
(*                                                                        *)
