...
(* ===================================================================== *)
(* §9 The width-1 clones (LB/SB).  Width-1 alignment is trivially true    *)
(*    (Z.rem _ 1 = 0), and the read brick missing tree-wide              *)
(*    ([exec_read_ram_plain_1]) is cloned here from the width-2 one.      *)
(* ===================================================================== *)
...
Lemma off1_bound (va : mword 64) :
  uint (subrange_vec_dec va 11 0) + 1 <= 4096.
Proof.
  rewrite uint_subrange11.
  pose proof (Z.mod_pos_bound (uint va) 4096 ltac:(lia)) as Hb. lia.
Qed.

Lemma u_walk_pa_window_1 (pte0 : mword 64) (va : mword 64) (j : nat) :
  (j < 1)%nat ->
  pa_add (u_walk_pa pte0 va) j = u_walk_pa pte0 (add_vec_int va (Z.of_nat j)).
Proof.
  intros Hj.
  pose proof (off1_bound va) as Hb.
  exact (pa_window _ va j ltac:(lia)).
Qed.

Lemma pa1_aligned (a : mword 64) :
  is_aligned_paddr (Physaddr a) 1 = true.
Proof.
  unfold is_aligned_paddr. apply Z.eqb_eq. apply Z.rem_1_r.
Qed.

Lemma nth_byte_assemble1 (bs : list (bv 8)) (j : nat) :
  length bs = 1%nat -> (j < 1)%nat ->
  nth_byte (Z_to_bv 8 (assemble_bytes bs) : bv 8) j = bs !!! j.
Proof.
...
Lemma exec_pmaCheck_ram_load_1 (addr : mword 64) (pbmt : page_based_mem_type)
    (region : PMA_Region) s :
  matching_pma_region (register_lookup pma_regions s.(sregs)) (Physaddr addr) 1
    = Some region ->
...
Lemma exec_checked_mem_read_ram_1_U (pbmt : page_based_mem_type) (addr : mword 64)
    (region : PMA_Region) (w : bv 8) s :
  pmpAddrMatchType_encdec_backwards
    (_get_Pmpcfg_ent_A (vec_access_dec (register_lookup pmpcfg_n s.(sregs)) 0)) = TOR ->
  zopz0zKzJ_u (zeros' 64) (vec_access_dec (register_lookup pmpaddr_n s.(sregs)) 0) = false ->
  pmpRangeMatch (Z.mul (uint (zeros' 64 : mword 64)) 4)
  ...
  exec (checked_mem_read (Load Data) pbmt User (Physaddr addr) 1 false false false false)
       s = Some (Ok (w, default_meta), s).
Proof.
  intros HA Hord Hrange HR Hmatch Halign Hread Hc Hsig Hh Hdev Hbytes.
  unfold checked_mem_read.
  ...
  2:{ unfold phys_access_check.
      rewrite (exec_bind_Some _ _ _ _ _
                 (exec_pmpCheck_user_grant_load addr 1 s HA Hord Hrange HR)).
      cbn match.
...
Lemma exec_mem_read_data_1_U (pbmt : page_based_mem_type) (addr : mword 64)
    (region : PMA_Region) (w : bv 8) s :
  pmpAddrMatchType_encdec_backwards
    (_get_Pmpcfg_ent_A (vec_access_dec (register_lookup pmpcfg_n s.(sregs)) 0)) = TOR ->
  zopz0zKzJ_u (zeros' 64) (vec_access_dec (register_lookup pmpaddr_n s.(sregs)) 0) = false ->
...
Proof.
  ...
  2:{ unfold mem_read_priv_meta. cbn [orb andb].
      ...
      2:{ cbn match. apply exec_checked_mem_read_ram_1_U with (region := region); assumption. }
      cbn match. unfold mem_read_callback. apply exec_returnM. }
  cbn [MemoryOpResult_drop_meta]. apply exec_returnM.
Qed.

Lemma exec_checked_mem_write_ram_1_U (pbmt : page_based_mem_type) (addr : mword 64)
  ...
  dev_addr addr = false ->
  exec (checked_mem_write (Physaddr addr) 1 data (Store Data) pbmt User tt false false false) s
...
Proof.
  ...
  2:{ unfold phys_access_check.
      rewrite (exec_bind_Some _ _ _ _ _
                 (exec_pmpCheck_user_grant_store addr 1 s HA Hord Hrange HW)).
      cbn match.
...
Lemma exec_mem_write_value_1_U (pbmt : page_based_mem_type) (addr : mword 64)
    (region : PMA_Region) (data : bv 8) s :
  pmpAddrMatchType_encdec_backwards
    (_get_Pmpcfg_ent_A (vec_access_dec (register_lookup pmpcfg_n s.(sregs)) 0)) = TOR ->
  zopz0zKzJ_u (zeros' 64) (vec_access_dec (register_lookup pmpaddr_n s.(sregs)) 0) = false ->
...
Proof.
  ...
  unfold mem_write_value_priv_meta. cbn [orb andb].
  rewrite (exec_bind_Some _ _ _ _ _
            (exec_checked_mem_write_ram_1_U pbmt addr region data s
               HA Hord Hrange HW Hmatch Halign Hwrite Hc Hsig Hh Hdev)).
  cbn match. unfold mem_write_callback. apply exec_returnm.
Qed.

Section UserMemPtGhost1.
  Context `{!riscvGS Σ}.
  Context `{CID : CpuId}.

Lemma udata_read_word_1 (um : gmap (mword 27) (mword 64)) (data : gset Arch.pa)
      (w va : mword 64) (σ' : mstate) :
    um !! svpn_of va = Some w ->
    udata_cov um data ->
    is_aligned_vaddr (Virtaddr va) 1 = true ->
    ...
    ⌜exists dv : bv 8,
       (forall j : nat, (N.of_nat j < 1)%N ->
          σ'.(mem) !! pa_add (u_walk_pa w va) j = Some (nth_byte dv j))
       /\ addr_is_ram (u_walk_pa w va)
       /\ addr_is_ram (pa_add (u_walk_pa w va) 0)⌝.
  Proof.
    iIntros (Hl Hcov Hal) "Hmem Hdata".
    iDestruct "Hdata" as (dm) "[%Hdom Hbytes]".
    set (pa := u_walk_pa w va).
    assert (Hin : forall j : nat, (j < 1)%nat -> pa_add pa j ∈ dom dm).
    { intros j Hj. rewrite Hdom.
      unfold pa. rewrite (u_walk_pa_window_1 _ _ _ Hj).
      exact (Hcov (svpn_of va) w (add_vec_int va (Z.of_nat j)) Hl). }
    assert (Hb : forall j : nat, (j < 1)%nat -> exists b, dm !! pa_add pa j = Some b).
    { intros j Hj. apply elem_of_dom. apply Hin. exact Hj. }
    destruct (Hb 0%nat ltac:(lia)) as [b0 Hb0].
    set (dv := Z_to_bv 8 (assemble_bytes [b0]) : bv 8).
    assert (Hw : forall j : nat, (j < 1)%nat ->
              nth_byte dv j = [b0] !!! j).
    { intros j Hj. apply nth_byte_assemble1; [reflexivity | exact Hj]. }
    iAssert (⌜forall (a : Arch.pa) (b : bv 8), dm !! a = Some b ->
               σ'.(mem) !! a = Some b /\ addr_is_ram a⌝)%I as %Hmm.
    { iIntros (a b Hab).
      iDestruct (big_sepM_lookup_acc _ _ _ _ Hab with "Hbytes") as "[Hab Hrest]".
    ...
    split; [ | split;
      [ rewrite <- (pa_add_0 pa); exact (proj2 (Hmm _ _ Hb0))
      | exact (proj2 (Hmm _ _ Hb0)) ] ].
    intros j HjN.
    assert (Hj : (j < 1)%nat) by lia.
    rewrite Hw; [ | exact Hj ].
    destruct j as [|]; try lia;
      cbn [lookup_total list_lookup_total];
      [ exact (proj1 (Hmm _ _ Hb0)) ].
  Qed.

Lemma udata_own_store_1 (data : gset Arch.pa) (pa : Arch.pa)
      (v : bv 8) (m : _) :
    (forall j : nat, (j < 1)%nat -> pa_add pa j ∈ data) ->
    gen_heap_interp (hG := riscv_memGS) m -∗ udata_own data ==∗
    gen_heap_interp (hG := riscv_memGS) (write_bytes m pa 1 v) ∗ udata_own data.
  Proof.
    intros Hin.
    apply (udata_own_upd data (seq 0 1) pa v m).
    intros j Hj. apply Hin.
    apply elem_of_seq in Hj. lia.
  Qed.
...
Lemma user_pt_load_data_1 (uroot tfp : mword 44)
  ...
  Proof.
    ...
    { intros r Hne.
      destruct Hsregs as [Heq | (tv & Heq)]; rewrite Heq;
        [ reflexivity | apply irrelevant_register_set; exact Hne ]. }
    iDestruct (udata_read_word_1 um data w va σ' Hl Hcov Hal with "Hgh Hdata")
      as %(dv & Hbytes & Hram0 & Hram7).
    iModIntro.
    iExists dv, σ'.
    iSplit; [ iPureIntro; exact Htr | ].
    ...
    { set (pa := u_walk_pa w va) in *.
      destruct ((ltac:(rewrite (Tr pma_regions ltac:(vm_compute; reflexivity)); exact Hall)
                 : pma_allows_all (register_lookup pma_regions σ'.(sregs))) pa 1)
        as (region & Hpmam & _ & Hrd & _).
      pose proof (addr_is_ram_not_in_clint _ Hram0) as Hnc.
      pose proof (addr_is_ram_not_in_sig _ Hram0) as Hns.
      assert (Hrange : pmpRangeMatch (Z.mul (uint (zeros' 64 : mword 64)) 4)
      ...
      { rewrite (Tr pmpaddr_n ltac:(vm_compute; reflexivity)).
        exact (ram_fetch_pmp pa _ 1 0 ltac:(lia) ltac:(lia)
                 ltac:(vm_compute; reflexivity) ltac:(reflexivity)
                 Hram0 Hram7 Hcovp). }
      exact (exec_mem_read_data_1_U PBMT_PMA pa region dv σ'
               (ltac:(rewrite (Tr pmpcfg_n ltac:(vm_compute; reflexivity)); exact HA))
               (ltac:(rewrite (Tr pmpaddr_n ltac:(vm_compute; reflexivity)); exact Hord))
               Hrange
               (ltac:(rewrite (Tr pmpcfg_n ltac:(vm_compute; reflexivity)); exact HR))
               Hpmam
               (pa1_aligned pa) Hrd
               (within_clint_false pa 1 σ' Hnc ltac:(lia))
               (within_sig_false pa 1 σ' Hns ltac:(lia))
               (within_htif_false pa 1 σ'
                  (ltac:(rewrite (Tr htif_tohost_base ltac:(vm_compute; reflexivity)); exact Hhtif)))
               (addr_is_ram_not_dev _ Hram0)
               Hbytes
               (ltac:(rewrite (Tr mstatus ltac:(vm_compute; reflexivity)); exact Hmprv))
...
Lemma user_pt_store_data_1 (uroot tfp : mword 44)
  ...
  Proof.
    ...
    set (pa := u_walk_pa w va) in *.
    (* the physical write fact at σ' *)
    assert (Hwr : exec (mem_write_value (Physaddr pa) 1 v (Store Data)
                          PBMT_PMA false false false) σ'
                  = Some (Ok true,
                          MState σ'.(sregs) (write_bytes σ'.(mem) pa 1 v) σ'.(mdev))).
    { destruct ((ltac:(rewrite (Tr pma_regions ltac:(vm_compute; reflexivity)); exact Hall)
                 : pma_allows_all (register_lookup pma_regions σ'.(sregs))) pa 1)
        as (region & Hpmam & _ & _ & Hwrb).
      pose proof (addr_is_ram_not_in_clint _ Hram0) as Hnc.
      ...
      { rewrite (Tr pmpaddr_n ltac:(vm_compute; reflexivity)).
        exact (ram_fetch_pmp pa _ 1 0 ltac:(lia) ltac:(lia)
                 ltac:(vm_compute; reflexivity) ltac:(reflexivity)
                 Hram0 Hram7 Hcovp). }
      exact (exec_mem_write_value_1_U PBMT_PMA pa region v σ'
               (ltac:(rewrite (Tr pmpcfg_n ltac:(vm_compute; reflexivity)); exact HA))
               (ltac:(rewrite (Tr pmpaddr_n ltac:(vm_compute; reflexivity)); exact Hord))
               Hrange
               (ltac:(rewrite (Tr pmpcfg_n ltac:(vm_compute; reflexivity)); exact HW))
               Hpmam
               (pa1_aligned pa) (proj1 Hwrb)
               (within_clint_false pa 1 σ' Hnc ltac:(lia))
               (within_sig_false pa 1 σ' Hns ltac:(lia))
               (within_htif_writable_false pa 1 σ'
                  (ltac:(rewrite (Tr htif_tohost_base ltac:(vm_compute; reflexivity)); exact Hhtif)))
               (addr_is_ram_not_dev _ Hram0)
               (ltac:(rewrite (Tr mstatus ltac:(vm_compute; reflexivity)); exact Hmprv))
               (ltac:(rewrite (Tr cur_privilege ltac:(vm_compute; reflexivity)); exact Hcp))). }
    (* ghost update of the written window *)
    iMod (udata_own_store_1 data pa v σ'.(mem)
            (fun j Hj => ltac:(
               rewrite (u_walk_pa_window_1 w va j Hj);
               exact (Hcov (svpn_of va) w (add_vec_int va (Z.of_nat j)) Hl)))
            with "Hgh Hdata") as "[Hgh Hdata]".
    iModIntro.
    iExists σ'.
