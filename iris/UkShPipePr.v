(* ===================================================================== *)
(* UkShPipePr.v -- parseredirs' ZERO-TURN AT THE '|', lane SH-PARSE-PIPE   *)
(* part 2 (design/app-pipe.md SS5.1).                                      *)
(*                                                                        *)
...
(* is NOT A SYMBOL, which the '|' falsifies.  SH-PARSE's one-line-of-N     *)
(* shape, a third time -- the walk spends that premise in EXACTLY ONE      *)
(* line of its 584, through [UkShRedirPr.ushs_peek_res_nsym], to make the  *)
(* peek answer 0.  So this is that walk at the WEAKEST fact instead:       *)
(*                                                                        *)
(*   ushp_peek_res len f <the scanned cursor> 2 (ushp_lit ushp_T_redir) = 0 *)
(*                                                                        *)
(* which [UkShPipeEx.ushq_peek_redir_miss_pipe] supplies at the pipe shape *)
(* and which the landed premise implies at every other cursor.  Nothing    *)
(* else in the walk changed: same instructions, same frame, same answer.   *)
(* ===================================================================== *)
...
Section UkShPipePr.
  ...
  Proof using .
    ...
    iApply (UkShRedirPr.wp_kshp_frame_pro_at N 14 3 [(ra_idx, mword_of_int 13 : mword 6);
    ...
    (* ---- 0x4ac  addi s6,s6,-440 -- the table base 0x12f0 ---- *)
    ...
    iIntros (h10) "Hrun".
    (* ---- 0x4de  li s5,60 -- the '<' the dead switch compares against ---- *)
    iApply (wp_uk_li N h10 m10 (mword_of_int 0x4de)
    ...
    rewrite Eret.
    rewrite Hpmiss in Ha0n0.
    (* ---- 0x4ec  c.beqz a0 -- TAKEN: the loop never turns ---- *)
    iApply (wp_uk_cbeqz N h16 n0 (mword_of_int 0x4ec)
    ...
    iIntros (h17) "Hrun".
    (* ---- 0x550  c.mv a0,s4 -- the answer is the cmd we were handed ---- *)
    ...
    iApply (wp_uk_cmv N h17 n0 (mword_of_int 0x550) a0_idx s4_idx
              (mword_of_int cmd) (8 + (2 + nn))
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate)
