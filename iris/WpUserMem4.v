(* WpUserMem4.v -- width-4 memory USTEP arms (LW/LWU loads, SW stores).
   Companion to WpUserMem (width 8): the same TLB-hit data-access arms at
   access width 4, riding the mode-neutral width-4 tower (MemData4) via its
   U-mode instantiation (UmodeData4).  The load arm is sign-generic
   (is_unsigned), covering both LW and LWU (and, by expansion, C_LW/
   C_LWSP).                                                               *)
...
Section WpUserMem4.
  ...
  Local Notation wp_instr_u_data := (WpUserComputeMiss.wp_instr_u_data U).
  ...
  Proof.
    ...
    iApply (wp_instr_u_hit va vpn ie w (LOAD (imm, Regidx rs1, Regidx rd, is_unsigned, 4))
    ...
    iExists ms_v, sc_v, stval_v, sepc_v, (add_vec_int va 4),
            (<[Regidx rd := regval_into_reg (extend_value is_unsigned v)]> g), tlbvec.
  ...
  (* ================================================================== *)
  (* Width-4 LOAD, fetch-HIT + data-TLB-MISS.  Same as [ustep_lw_data]    *)
  (* except the data vpnD's TLB slot is EMPTY, so the data translate       *)
  (* WALKS the page table (reading the owned PT slots via upt_inv) and      *)
  (* FILLS the slot.  Rides [wp_instr_u_hit_data], which hands the tlb      *)
  (* fragment + upt_inv into the execute callback so the fill is legal.     *)
  (* ================================================================== *)
  Lemma ustep_lw_data_miss
      (va : mword 64) (vpn : mword 27) (ie : uwalk_info) (w : mword 32)
      (vpnD : mword 27) (ieD : uwalk_info)
      (imm : mword 12) (rs1 rd : mword 5) (is_unsigned : bool) (v : mword 32)
      (ms_v sc_v stval_v sepc_v : mword 64)
  ...
  Proof.
    ...
    assert (Hnlpad : is_lpad_instruction (LOAD (imm, Regidx rs1, Regidx rd, is_unsigned, 4))
                       = false) by reflexivity.
    iApply (wp_instr_u_hit_data va vpn ie w (LOAD (imm, Regidx rs1, Regidx rd, is_unsigned, 4))
              ms_v tlbvec E Φ HN Hvec Hchk0 HupdN Hpbmt0 Hcw HSXL Hval Hcanon
              Hvpn_def Hpaal HnotRVC Hdec Hnlpad
  ...
  (* ================================================================= *)
  (* COMBINED width-4 LOAD: rides wp_instr_u_data, so it needs NO fetch  *)
  (* or data TLB-hit hypothesis -- it dispatches the fetch (in the       *)
  (* engine) and the DATA slot (here, on the fetch-filled TLB) across     *)
  (* hit / empty-miss / colliding-miss.                                   *)
  (* ================================================================= *)
  Lemma ustep_lw_u
      (va : mword 64) (vpn : mword 27) (ie : uwalk_info) (w : mword 32)
      (vpnD : mword 27) (ieD : uwalk_info)
  ...
  Proof.
    ...
    assert (Hnlpad : is_lpad_instruction (LOAD (imm, Regidx rs1, Regidx rd, is_unsigned, 4))
                       = false) by reflexivity.
    iApply (wp_instr_u_data va vpn ie w (LOAD (imm, Regidx rs1, Regidx rd, is_unsigned, 4))
  ...
  Proof.
    ...
    iAssert (⌜addr_is_ram (pa_add paD 3)⌝)%I as %HramD7.
    { iDestruct (big_sepM_lookup (fun a b => (a ↦ₘ□ b)%I) code _ _
                   (Hcwd 3%nat ltac:(lia)) with "Hcode") as "Hb7".
      iDestruct (mem_ram with "Hb7") as %Hr7. iPureIntro. exact Hr7. }
    pose proof (addr_is_ram_not_in_clint _ HramD) as HncD.
    pose proof (addr_is_ram_not_in_sig _ HramD) as HnsD.
    destruct (Hpma_all paD 4) as (regionD & HpmamD & _ & HreadD & _ & _).
  ...
  Proof.
    ...
    assert (Hnlpad : is_lpad_instruction (STORE (imm, Regidx rs2, Regidx rs1, 4))
                       = false) by reflexivity.
    iApply (wp_instr_u_hit va vpn ie w (STORE (imm, Regidx rs2, Regidx rs1, 4))
              ms_v tlbvec E Φ HN Hvec Hchk0 HupdN Hpbmt0 Hcw HSXL Hval Hcanon
              Hvpn_def Hpaal HnotRVC Hdec Hnlpad
              with "Hhw Hinv Hhs Hpriv Hms Htlbc Hpcr Hcode Hcfg").
    iIntros (σ Hpceq Hag Hpins) "[Hreg [Hmem Hdev]]".
  ...
  (* STORE, fetch-HIT + data-MISS (width 4): the data EA translate walks the *)
  (* owned page table and fills the (cold) data TLB slot, then the write body *)
  (* runs at the filled state s'. *)
  Lemma ustep_sw_data_miss
      (va : mword 64) (vpn : mword 27) (ie : uwalk_info) (w : mword 32)
      (vpnD : mword 27) (ieD : uwalk_info)
      (imm : mword 12) (rs2 rs1 : mword 5) (vold : mword 32)
      (ms_v sc_v stval_v sepc_v : mword 64)
  ...
  Proof.
    ...
    assert (Hnlpad : is_lpad_instruction (STORE (imm, Regidx rs2, Regidx rs1, 4))
                       = false) by reflexivity.
    iApply (wp_instr_u_hit_data va vpn ie w (STORE (imm, Regidx rs2, Regidx rs1, 4))
              ms_v tlbvec E Φ HN Hvec Hchk0 HupdN Hpbmt0 Hcw HSXL Hval Hcanon
              Hvpn_def Hpaal HnotRVC Hdec Hnlpad
              with "Hhw Hinv Hhs Hpriv Hms Htlbc Hpcr Hcode Hupt Hcfg").
    iIntros (σ Hpceq Hag Hpins) "Htlbc Hupt [Hreg [Hmem Hdev]]".
  ...
  (* COMBINED width-4 STORE (all 4 fetch×data TLB combos). *)
  Lemma ustep_sw_u
      (va : mword 64) (vpn : mword 27) (ie : uwalk_info) (w : mword 32)
      (vpnD : mword 27) (ieD : uwalk_info)
      (imm : mword 12) (rs2 rs1 : mword 5) (vold : mword 32)
    ...
    WP (Loop : expr riscv_lang) @ E {{ Φ }}.
  Proof.
    intros eaF paD vNew HN Hok Hsome Hchk0 HupdN Hcw HSXL HMPRV HMXR Hval
           Hcanon Hvpn_def Hpaal HnotRVC Hdec
           HsomeD HchkD HupdD HpbmtD
           HalignD HcanonD Hvpn_defD HpaalD.
    iIntros "#Hhw #Hinv Hhs Hpriv Hms Hsc Hstv Hsepc Htlbc [Hpcr Hnpc]
    ...
    assert (Hnlpad : is_lpad_instruction (STORE (imm, Regidx rs2, Regidx rs1, 4))
                       = false) by reflexivity.
    iApply (wp_instr_u_data va vpn ie w (STORE (imm, Regidx rs2, Regidx rs1, 4))
              ms_v tlbvec E Φ HN Hok Hsome Hchk0 HupdN Hcw HSXL Hval Hcanon
              Hvpn_def Hpaal HnotRVC Hdec Hnlpad
              with "Hhw Hinv Hhs Hpriv Hms Htlbc Hpcr Hcode Hupt Hcfg").
    iIntros (σ Hpceq Hag Hpins) "Htlbc Hupt [Hreg [Hmem Hdev]]".
  ...
  Proof.
    ...
    iAssert (⌜forall j : nat, (N.of_nat j < 4)%N ->
               σ.(mem) !! (pa_add paD j) = Some (nth_byte v j)⌝)%I as %HbfD.
    { iIntros (j Hj).
      iDestruct (big_sepM_lookup (fun a b => (a ↦ₘ□ b)%I) code _ _ (Hcwd j ltac:(lia)) with "Hcode")
        as "Hbj".
      iDestruct (mem_valid with "Hmem Hbj") as %Hmj. iPureIntro. exact Hmj. }
    iAssert (⌜addr_is_ram paD⌝)%I as %HramD.
    { iDestruct (big_sepM_lookup (fun a b => (a ↦ₘ□ b)%I) code _ _ (Hcwd 0%nat ltac:(lia)) with "Hcode")
        as "Hb0".
      iDestruct (mem_ram with "Hb0") as %Hr0. rewrite pa_add_0 in Hr0.
      iPureIntro. exact Hr0. }
    iAssert (⌜addr_is_ram (pa_add paD 3)⌝)%I as %HramD7.
    { iDestruct (big_sepM_lookup (fun a b => (a ↦ₘ□ b)%I) code _ _ (Hcwd 3%nat ltac:(lia)) with "Hcode")
        as "Hb7".
      iDestruct (mem_ram with "Hb7") as %Hr7. iPureIntro. exact Hr7. }
    pose proof (addr_is_ram_not_in_clint _ HramD) as HncD.
    pose proof (addr_is_ram_not_in_sig _ HramD) as HnsD.
