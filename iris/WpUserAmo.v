(* WpUserAmo.v -- width-4 ATOMIC USTEP arm (AMOSWAP.W).
...
Require Import WpPushOffMem UmodeAmo4.
...
Section WpUserAmo.
  ...
  (* AMOSWAP.W: read the old 4-byte data word into rd, write low32(rs2) back.
     Standalone spatial (owns the target window) like ustep_sw, plus a
     register write of the loaded value like the data-page load arm. *)
  Lemma ustep_amoswap_w
    ...
    user_code -∗
    (* the target window's OLD bytes (the loaded value), owned *)
  ...
  Proof.
    intros eaF paD vN HN Hok Hvec Hchk0 HupdN Hpbmt0 Hcw HSXL HMPRV HMXR Hval
    ...
    iApply (wp_instr_u_hit va vpn ie w (AMO (AMOSWAP, true, false, Regidx rs2, Regidx rs1, 4, Regidx rd))
    ...
    iExists ms_v, sc_v, stval_v, sepc_v, (add_vec_int va 4),
            (<[Regidx rd := regval_into_reg (sign_extend' 64
               (autocast (T := mword) (vold : mword (8 * 4)) : mword (4 * 8)))]> g), tlbvec.
  ...
  (* ================================================================= *)
  (* AMOSWAP.W with a COLD data TLB: the EA translate walks the page    *)
  (* table off the owned upt_inv slots and fills the TLB (s1 -> s'),     *)
  (* then the atomic read+write runs at s'.  Data-miss twin of           *)
  (* ustep_amoswap_w.                                                    *)
  (* ================================================================= *)
  Lemma ustep_amoswap_w_data_miss
      (va : mword 64) (vpn : mword 27) (ie : uwalk_info) (w : mword 32)
      (vpnD : mword 27) (ieD : uwalk_info)
      (rs2 rs1 rd : mword 5) (vold : mword 32)
      (ms_v sc_v stval_v sepc_v : mword 64)
  ...
  Proof.
    ...
    assert (Hnlpad : is_lpad_instruction (AMO (AMOSWAP, true, false, Regidx rs2, Regidx rs1, 4, Regidx rd))
                       = false) by reflexivity.
    iApply (wp_instr_u_hit_data va vpn ie w (AMO (AMOSWAP, true, false, Regidx rs2, Regidx rs1, 4, Regidx rd))
              ms_v tlbvec E Φ HN Hvec Hchk0 HupdN Hpbmt0 Hcw HSXL Hval Hcanon
              Hvpn_def Hpaal HnotRVC Hdec Hnlpad
  ...
  Lemma ustep_amoswap_w_u
    ...
    WP (Loop : expr riscv_lang) @ E {{ Φ }}.
  Proof.
    intros eaF paD vN HN Hok Hsome Hchk0 HupdN Hcw HSXL HMPRV HMXR Hval
           Hcanon Hvpn_def Hpaal HnotRVC Hdec Hrd
    ...
    assert (Hnlpad : is_lpad_instruction (AMO (AMOSWAP, true, false, Regidx rs2, Regidx rs1, 4, Regidx rd))
                       = false) by reflexivity.
    iApply (wp_instr_u_data va vpn ie w (AMO (AMOSWAP, true, false, Regidx rs2, Regidx rs1, 4, Regidx rd))
