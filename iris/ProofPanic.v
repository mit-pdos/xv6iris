(* ProofPanic.v -- panic()'s whole-function proof (SpecPanic.v), as a sealed
   ...
   RESTORED AT 163d39b.  06ea57f had commented the two calls out and this
   proof was cut to the five instructions that were left; the second UART
   put them back, byte for byte -- the new panic differs from the pre-06ea57f
   one in NOTHING but the two [addi] immediates that materialise the .rodata
   addresses (2040 -> 2004 at +0x10, 2034 -> 1998 at +0x1e), panic having
...
(* ===================================================================== *)
(* The two .rodata addresses, DERIVED rather than transcribed: [etext] is the
   base of .rodata, so an ordinary text-growing bump carries these for free
   and only a .rodata reordering touches the offset (xv6-bump-playbook.md
   4b).  [ltac:(eval vm_compute ...)] keeps the body a plain [Z] literal for
   the [lia]/[vm_compute] side conditions downstream.

   Both offsets were re-derived from the image at 163d39b: the auipc/addi
   pair at +0x0c/+0x10 computes (panic + 0xc) + (6 << 12) + 2004 = etext +
   0x18, the pair at +0x1a/+0x1e computes (panic + 0x1a) + (6 << 12) + 1998
   = etext + 0x20, and KernelData.v has "panic: " and "%s\n" exactly there,
   each NUL-terminated and NUL-preceded, and nowhere else in the image. *)
Definition pn_hdr_a : Z :=                    (* "panic: " (a0 at +0x10) *)
  ltac:(let x := eval vm_compute in (KernelSyms.etext + 0x18)%Z in exact x).
Definition pn_fmt_a : Z :=                    (* "%s\n"    (a0 at +0x1e) *)
  ltac:(let x := eval vm_compute in (KernelSyms.etext + 0x20)%Z in exact x).
  ...
  Proof.
    ...
    set (P4 := <[Regidx Ra0 := regval_into_reg
                  (add_vec (P3 !!! Regidx Ra0)
                     (sign_extend' 64 (mword_of_int 2004 : mword 12)))]> P3).
