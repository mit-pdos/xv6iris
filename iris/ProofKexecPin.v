...
Definition wp_kexec_pinned_run_body
  ...
  wp_next true pj (fun (CID : CpuId) =>
    kexec_closer (kxp_entry_ok pb) (fun _ : KexecOkQ.kxf_cause => Logic.True)
...
Section KexecPinMain.
  ...
  (* the failure-side plug (S5): the pinned contract, like the landed one,
     says nothing about why a run gave up. *)
 ...
 plen pfun na avf alen aslen afun
  ...
  Proof.
    ...
    iApply (PA.kxc_phaseAp (CID0 := CID0) (kxp_entry_ok pb) QTF pb ds
              gs jp gl pd pav pu
 gf
    ...
    iApply (PB.kxc_b1 (CID0 := CIDa) (kxp_entry_ok pb) QTF gs jp gl pd pav pu
 gf
 ...
 kf qf sf gyf loyf tlyf inumf dnf bmf datl n2
      ...
      iApply (kxc_cd (CID0 := CIDy) (kxp_entry_ok pb) QTF jp gf
 plen pfun na avf alen aslen afun
                ...
                (fun _ U' _ => Q_pin_of_hdr pb ef Hhp U') I (fun _ _ _ => I)
    ...
    - (* ---- OUTPUT 2: the phdr loop's body, entered at i = 0, sz = 0 ---- *)
      ...
      iApply (PB3.kxc_b2 (CID0 := CIDl) (kxp_entry_ok pb) QTF gs jp gl pd pav pu
                gilf gislf gf
 kf qf sf gyf loyf tlyf inumf dnf bmf datl n2
                plen pfun na avf alen aslen afun pidv U eb dqb dqs dqa dqpv dqas
