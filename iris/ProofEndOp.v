...
Lemma eo_reloc_log_0c :
  add_vec (add_vec (mword_of_int (KernelSyms.end_op + 0x0c) : mword 64)
          ...
          (sign_extend' 64 (mword_of_int 1664 : mword 12)) = log_addr.
...
Lemma eo_reloc_log_ac :
  add_vec (add_vec (mword_of_int (KernelSyms.end_op + 0xac) : mword 64)
                   (auipc_off (mword_of_int 30 : mword 20)))
          (sign_extend' 64 (mword_of_int 1504 : mword 12)) = log_addr.
Proof. rewrite /log_addr. apply bv_eq; vm_compute; reflexivity. Qed.

Lemma eo_reloc_lhn :
  add_vec (add_vec (mword_of_int (KernelSyms.end_op + 0x10e) : mword 64)
                   (auipc_off (mword_of_int 30 : mword 20)))
          (sign_extend' 64 (mword_of_int 1450 : mword 12)) = lh_n_pa.
Proof.
  rewrite /lh_n_pa /log_pa /log_addr /pa_add /add_vec_int.
  apply bv_eq; vm_compute; reflexivity.
Qed.
...
(* A WRITE TO THE LOG REGION DOES NOT MOVE THE HOME RESTRICTION (lane CE).
   The copy loop's every [log_write] lands in a log SLOT, so the map the
   commit's snapshot premise is stated at -- the logged view restricted to
   the home blocks -- is literally the same term at every iteration.  This
   is what lets [eo_loop] carry the file system's law as a pure hypothesis
   across the whole fuel induction. *)
Lemma eo_home_restrict_upd (L : gmap Z (list (bv 8))) (cov : gset Z)
    (ls b : Z) (bs : list (bv 8)) :
  b ∉ fs_home_set cov ls ->
  fs_restrict (dv_of_D (<[b := bs]> L)) (fs_home_set cov ls)
  = fs_restrict (dv_of_D L) (fs_home_set cov ls).
Proof.
  intros Hb. apply fs_restrict_ext. intros x Hx.
  rewrite /dv_of_D lookup_insert_ne; [reflexivity |].
  intros ->. exact (Hb Hx).
Qed.
...
(* ===================================================================== *)
(*  THE INSTALL PASS'S PICTURE IS [LogDefs.lm_install].                   *)
(*                                                                       *)
(*  install_trans's crash permits are CURSOR-INDEXED                     *)
...
(*  [Lw i].  That chain is [lm_install] over the header's own decoded     *)
(*  write set [map uint W] -- the same pass the recovering install runs   *)
(*  ([ProofInitlog]), block-keyed.  What is here is its three readings IN *)
(*  THE COMMIT TAIL'S VOCABULARY: a write set of WORDS, whose             *)
(*  duplicate-freedom this proof holds as a [NoDup] rather than as the    *)
(*  injectivity [lm_install_hit] takes.  The third is half of the         *)
(*  CAUGHT-UP fact ([FsCrash.fs_clear_keep_seq_permit]'s premise): home = *)
(*  slot at every entry, by computation on the chained value and nothing  *)
(*  about the disk.                                                      *)
(* ===================================================================== *)
Lemma eo_install_miss (W : list (mword 32)) (Lw : nat -> list (bv 8))
    (M : log_mirror) (t : nat) (c : Z) :
  (t <= length W)%nat ->
  (forall (i : nat) (w : mword 32), (i < t)%nat -> W !! i = Some w ->
     uint w <> c) ->
  lm_view (lm_install M (map uint W) Lw t) c = lm_view M c.
Proof.
  intros Ht Hne.
  apply (lm_install_miss M (map uint W) Lw t c
           ltac:(rewrite length_map; lia)).
  intros i b Hi Hb.
  destruct (eo_map_lookup_inv W i b Hb) as (w & Hw & ->).
  exact (Hne i w Hi Hw).
Qed.

Lemma eo_install_hdr (W : list (mword 32)) (Lw : nat -> list (bv 8))
    (M : log_mirror) (ls : Z) (t : nat) :
  (t <= length W)%nat ->
  (forall (i : nat) (w : mword 32), (i < t)%nat -> W !! i = Some w ->
     uint w <> log_hdr_bno ls) ->
  lm_hdr (lm_install M (map uint W) Lw t) ls = lm_hdr M ls.
Proof.
  intros Ht Hne. rewrite /lm_hdr (eo_install_miss W Lw M t _ Ht Hne) //.
Qed.

Lemma eo_install_hit (W : list (mword 32)) (Lw : nat -> list (bv 8))
    (M : log_mirror) (t j : nat) (w : mword 32) :
  NoDup (map uint W) ->
  (t <= length W)%nat ->
  (j < t)%nat -> W !! j = Some w ->
  lm_view (lm_install M (map uint W) Lw t) (uint w) = Lw j.
Proof.
  intros Hnd Ht Hj Hw.
  exact (lm_install_hit M (map uint W) Lw t j (uint w)
           ltac:(intros i k c Hi Hk;
                 exact (NoDup_lookup (map uint W) i k c
                          ltac:(by apply NoDup_ListNoDup) Hi Hk))
           ltac:(rewrite length_map; lia) Hj (eo_map_lookup W j w Hw)).
Qed.
...
Section EndOpDefs.
  ...
  (* THE TWO SPELLINGS OF THE LOGGED VIEW ON THE HOME SET.  [LogDefs.lm_logged]
     is what the log's parked payload is re-indexed at; [fs_restrict (dv_of_D
     L) …] is what [FsCrash]'s commit permit concludes.  They are the same
     term up to delta (durable-disk 1d'), and this is the bridge that lets a
  ...
  Lemma eo_open_of_batch
    ...
    ∃ (W : list (mword 32)) (L : gmap Z (list (bv 8))) (D : gmap Z bool)
      ...
      (* ROW (b) LEAVES THE CHECKOUT (durable-disk 1b).  It is what the
         commit permit turns into [D' = L|home], so it has to travel beside
         the mirror's value all the way to [eo_commit]; [LB] rides with it
         because [log_res]'s ledger tie is stated over that name. *)
  ...
  Proof.
    ...
    iSplitR; [iPureIntro; rewrite -HLB; exact Hmtie|].
  ...
  (* the same at the OPENED batch, which is the form the commit arm meets:
     [eo_open]'s fourth conjunct IS the cache authority. *)
  Lemma eo_open_snap_law `{GEN : GenId} (γ : log_names) (bn : bio_names) (γfs : fs_names)
      (cov : gset Z) (logstart : Z) (dev : mword 32)
      (om : gmap nat op_entry) (T : gmap nat unit)
      (n : nat) (W : list (mword 32)) (L : gmap Z (list (bv 8)))
      (Db : gmap Z bool) (Lw : nat -> list (bv 8)) (t : nat) :
    size T = size om -> om = ∅ ->
    log_ctx γ bn γfs cov logstart dev -∗
...
Section EndOpBlocks.
  ...
  Local Lemma eo_epi `{GEN : GenId} `{CID0 : CpuId} 
      (j : nat) (pidv : mword 32) (dq : dfrac)
      (m M : regfile) (K : nat) (eb : bool) (b : bool) (lks : gset string) (Upr : ustate) :
    (K_end_op <= K)%nat ->
    eo_regsE m M ->
  ...
  Local Lemma eo_tail `{GEN : GenId} `{CID0 : CpuId} 
      ...
      (m M : regfile) (K : nat) (eb : bool) (lks : gset string) (Upr : ustate) :
    (K_end_op <= K)%nat ->
    eo_regsE m M ->
  ...
  Proof.
    ...
    iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.end_op + 0x46)) Rs1 Rs1
              (mword_of_int 1610 : mword 12) E1 (K - 8)%nat eb
    ...
    pose (E2 := <[Regidx Rs1 := regval_into_reg
                  (add_vec (E1 !!! Regidx Rs1 : mword 64)
                     (sign_extend' 64 (mword_of_int 1610 : mword 12)))]> E1).
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.end_op + 0x4c)) Rra
              (mword_of_int 2084466 : mword 21) E3 (K - 8)%nat eb
              ltac:(vm_compute; discriminate) ltac:(rdok)
    ...
    assert (Htgt4c : add_vec (mword_of_int (KernelSyms.end_op + 0x4c) : mword 64)
                       (sign_extend' 64 (mword_of_int 2084466 : mword 21))
    ...
    iDestruct (cpu_own_transport CID0 CIDa4 0 eb (proc_addr j) eb 
                 ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
    ...
    iDestruct (cpu_claim_ext_transport CIDa4 CIDb1 eb (proc_addr j)
                 ltac:(wp_next_chain) with "Hextm") as "Hextm".
    iDestruct (eo_cont_shift (CIDa := CIDa4) (CIDb := CIDb1)  j pidv dq m K eb eb lks Upr
                 ltac:(wp_next_chain) with "Hcont") as "Hcont".
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.end_op + 0x5c)) Rra
              (mword_of_int 2089480 : mword 21) F3 (trap_res eb + (K - 8))%nat false
              ltac:(vm_compute; discriminate) ltac:(rdok)
              ltac:(vm_compute; reflexivity) with "Hcg Hpc []").
    { iApply (eoi_5c with "Htext"). }
    iApply wp_next_off_intro. iIntros "Hcg Hpc".
    ...
    assert (Htgt5c : add_vec (mword_of_int (KernelSyms.end_op + 0x5c) : mword 64)
                       (sign_extend' 64 (mword_of_int 2089480 : mword 21))
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.end_op + 0x62)) Rra
              (mword_of_int 2084580 : mword 21) G1 (trap_res eb + (K - 8))%nat false
              ltac:(vm_compute; discriminate) ltac:(rdok)
              ltac:(vm_compute; reflexivity) with "Hcg Hpc []").
    { iApply (eoi_62 with "Htext"). }
    iApply wp_next_off_intro. iIntros "Hcg Hpc".
    ...
    assert (Htgt62 : add_vec (mword_of_int (KernelSyms.end_op + 0x62) : mword 64)
                       (sign_extend' 64 (mword_of_int 2084580 : mword 21))
    ...
    clear Htgt66.
    iDestruct (cpu_own_transport CIDc1 CIDc2 0 eb (proc_addr j) eb
                 ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
    iDestruct (trap_csrs_ext_transport CIDc1 CIDc2 eb (proc_addr j)
                 ltac:(wp_next_chain) with "Hextc") as "Hextc".
    iDestruct (cpu_claim_ext_transport CIDc1 CIDc2 eb (proc_addr j)
                 ltac:(wp_next_chain) with "Hextm") as "Hextm".
    iDestruct (eo_cont_shift (CIDa := CIDb1) (CIDb := CIDc2)  j pidv dq m K eb eb lks Upr
                 ltac:(wp_next_chain) with "Hcont") as "Hcont".
    iApply (eo_epi (CID0 := CIDc2)  j pidv dq m mr K eb eb lks Upr HK Hregs2
  ...
  Local Lemma eo_commit `{GEN : GenId} `{CID0 : CpuId} 
    ...
    (* ROW (b) AT THE COMMIT (durable-disk 1b).  Off the batch's own blocks
       the logged view IS the era's picture of the durable disk.  Together
  ...
  Proof.
    ...
    iDestruct (cpu_own_transport CID0 CIDa1 0 eb (proc_addr j) eb
                 ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
    iDestruct (trap_csrs_ext_transport CID0 CIDa1 eb (proc_addr j)
                 ltac:(wp_next_chain) with "Hextc") as "Hextc".
    iDestruct (cpu_claim_ext_transport CID0 CIDa1 eb (proc_addr j)
                 ltac:(wp_next_chain) with "Hextm") as "Hextm".
    iDestruct (eo_cont_shift (CIDa := CID0) (CIDb := CIDa1)  j pidv dq m K eb eb lks Upr
                 ltac:(wp_next_chain) with "Hcont") as "Hcont".
    ...
    { iIntros (bs' Hlen' Hhn' Hdec').
      iApply (fs_commit_L_seq_permit cov logstart Mc (lm_view Mc) L n
                ...
                ltac:(intros b Hb HbW; apply (Hrow b Hb); intros Hc;
                      apply elem_of_list_to_set in Hc; exact (HbW Hc))
                (* the entries: the logged value of a home block is what the
                   copy loop put in its slot *)
                ltac:(intros i b Hb;
                      destruct (eo_map_lookup_inv W i b Hb) as (v & Hv & ->);
                      rewrite (HMcslot i
    ...
    assert (HMihdr : forall t : nat, (t <= n)%nat ->
              lm_hdr (lm_install (lm_upd Mc (log_hdr_bno logstart) bs1)
                        (map uint W) Lw t) logstart
              = (n, map uint W)).
    { intros t Ht.
      rewrite (eo_install_hdr W Lw
                 (lm_upd Mc (log_hdr_bno logstart) bs1) logstart t ltac:(lia)
    ...
    assert (Hcaught : forall (jj : nat) (b : Z), map uint W !! jj = Some b ->
              lm_view (lm_install (lm_upd Mc (log_hdr_bno logstart) bs1)
                         (map uint W) Lw n) b
              = lm_view (lm_install (lm_upd Mc (log_hdr_bno logstart) bs1)
                           (map uint W) Lw n)
    ...
    iDestruct (cpu_claim_ext_transport CIDb1 CIDa3 eb (proc_addr j)
                 ltac:(wp_next_chain) with "Hextm") as "Hextm".
    iDestruct (eo_cont_shift (CIDa := CIDa1) (CIDb := CIDa3)  j pidv dq m K eb eb lks Upr
                 ltac:(wp_next_chain) with "Hcont") as "Hcont".
    ...
    { iModIntro. iIntros (i w) "%Hwi %Hlen'".
      cbn [lm_install].
      rewrite (list_lookup_total_correct (map uint W) i (uint w)
                 (eo_map_lookup W i w Hwi)).
    ...
    assert (Hlhna : add_vec (rget A4 Ra5) (sign_extend' 64 (mword_of_int 1450 : mword 12))
                    = lh_n_pa).
    ...
    iApply (wp_sw_zero_s_sconf (mword_of_int (KernelSyms.end_op + 0x112)) Ra5
              (mword_of_int 1450 : mword 12) A4 (K - 8)%nat
    ...
    iDestruct (cpu_claim_ext_transport CIDb2 CIDa6 eb (proc_addr j)
                 ltac:(wp_next_chain) with "Hextm") as "Hextm".
    iDestruct (eo_cont_shift (CIDa := CIDa3) (CIDb := CIDa6)  j pidv dq m K eb eb lks Upr
                 ltac:(wp_next_chain) with "Hcont") as "Hcont".
    ...
    assert (Hfinhdr : lm_hdr (lm_upd
                          (lm_install (lm_upd Mc (log_hdr_bno logstart) bs1)
                             (map uint W) Lw n)
                          (log_hdr_bno logstart) bs2) logstart = (0%nat, [])).
    { rewrite /lm_hdr lm_upd_view_eq. apply hdr_dec_zero.
      rewrite Hhdrn2. reflexivity. }
    (* ROW (b) AT THE DEPOSIT (durable-disk 1b), by
       [LogInv.log_mirror_tie_deposit] off the chain this proof carried: the
       install pass wrote [Lw j] to the home block of every entry, nothing
       in the cycle touched any OTHER home block (the fills go to slots, the
       commit and the clear to the header), and the copy loop's ghost step
       says [L] agrees with [Lw] at every entry.  The two header inserts are
       off the row's domain by [home_set_ne_hdr]. *)
    assert (Hdep : log_mirror_tie_body
                     (lm_upd (lm_install (lm_upd Mc (log_hdr_bno logstart) bs1)
                                (map uint W) Lw n)
                        (log_hdr_bno logstart) bs2)
                     (<[log_hdr_bno logstart := bs2]>
                        (<[log_hdr_bno logstart := bs1]> L))
                     cov logstart ∅).
    { apply (log_mirror_tie_deposit Mc _ _ cov logstart
               (list_to_set (map uint W)) (map uint W) Lw eq_refl).
      - (* the checkout's row, at the [L] the deposit parks *)
        intros b Hb HbW.
        pose proof (FsCrash.home_set_ne_hdr cov logstart b Hb) as Hbh.
        rewrite !(lookup_insert_ne _ _ _ _ (not_eq_sym Hbh)).
        exact (Hrow b Hb HbW).
      - (* every LOGGED home block ends at its logged content *)
        intros jj b Hb.
        destruct (eo_map_lookup_inv W jj b Hb) as (v & Hv & ->).
        assert (Hjlt : (jj < n)%nat) by (apply lookup_lt_Some in Hv; lia).
        rewrite (lm_upd_view_ne _ _ _ _ (Hnehdr jj v Hv)).
        exact (eo_install_hit W Lw
                 (lm_upd Mc (log_hdr_bno logstart) bs1) n jj v
                 Hnd ltac:(lia) Hjlt Hv).
      - (* ...and no other home block moved at all *)
        intros b Hb HbW.
        pose proof (FsCrash.home_set_ne_hdr cov logstart b Hb) as Hbh.
        rewrite (lm_upd_view_ne _ _ _ _ Hbh).
        rewrite (eo_install_miss W Lw
      ...
      - (* the logged view, at the entries the batch wrote *)
        intros jj b Hb.
        destruct (eo_map_lookup_inv W jj b Hb) as (v & Hv & ->).
        rewrite !(lookup_insert_ne _ _ _ _ (not_eq_sym (Hnehdr jj v Hv))).
        exact (HLw jj v Hv). }
    (* THE PAYLOAD, RE-PARKED AT THE NEW COMMITTED VIEW (durable-disk 1d').
       The deposited picture's header is CLEAN and row (b) holds at the empty
       batch, so [LogDefs.lm_committed_clean] says its committed view IS the
       logged view -- and the two header inserts the cycle made to [L] are
       off the home set, so that is the very index the law produced. *)
    assert (Hhdrout : log_hdr_bno logstart ∉ fs_home_set cov logstart)
      by exact (eo_hdr_not_home cov logstart).
    assert (Hdeprow : forall b : Z, b ∈ fs_home_set cov logstart ->
              (<[log_hdr_bno logstart := bs2]>
                 (<[log_hdr_bno logstart := bs1]> L)) !! b
              = Some (lm_view (lm_upd
                        (lm_install (lm_upd Mc (log_hdr_bno logstart) bs1)
                           (map uint W) Lw n)
                        (log_hdr_bno logstart) bs2) b))
      by (intros b Hb; exact (Hdep b Hb (not_elem_of_empty b))).
    assert (Hdepidx : lm_committed (lm_upd
                        (lm_install (lm_upd Mc (log_hdr_bno logstart) bs1)
                           (map uint W) Lw n)
                        (log_hdr_bno logstart) bs2) cov logstart
                      = lm_logged L cov logstart).
    { rewrite (lm_committed_clean _ _ cov logstart Hfinhdr Hdeprow)
              (lm_logged_insert_ne _ cov logstart (log_hdr_bno logstart) bs2
                 Hhdrout).
    ...
    clear Htgt120.
    iDestruct (cpu_own_transport CIDb3 CIDa10 0 eb (proc_addr j) eb
                 ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
    ...
    iDestruct (cpu_claim_ext_transport CIDb3 CIDa10 eb (proc_addr j)
                 ltac:(wp_next_chain) with "Hextm") as "Hextm".
    iDestruct (eo_cont_shift (CIDa := CIDa6) (CIDb := CIDa10)  j pidv dq m K eb eb lks Upr
                 ltac:(wp_next_chain) with "Hcont") as "Hcont".
  ...
  Local Lemma eo_loop `{GEN : GenId} 
    ...
    (forall i : nat, (i < t)%nat ->
       lm_view Mc (log_slot_bno logstart i) = Lw i) ->
    (* ROW (b) RIDES THE COPY LOOP UNTOUCHED (durable-disk 1b): every write
  ...
  Proof.
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.end_op + 0xc2)) Rra
              (mword_of_int 2092464 : mword 21) A4 (K - 8)%nat eb
              ltac:(vm_compute; discriminate) ltac:(rdok)
              ltac:(vm_compute; reflexivity) with "Hcg Hpc []").
    ...
    assert (Htgtc2 : add_vec (mword_of_int (KernelSyms.end_op + 0xc2) : mword 64)
                       (sign_extend' 64 (mword_of_int 2092464 : mword 21))
    ...
    { rewrite /eo_regs. split.
      ...
      - intros c Hcs N2 N8 N9 N18 N19 N20 N21.
        ...
        rewrite /A1 upd_ne; [| regne]. exact (Hthr c Hcs N2 N8 N9 N18 N19 N20 N21). }
    iDestruct (cpu_own_transport CID0 CIDa5 0 eb (proc_addr j) eb
                 ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
    iDestruct (trap_csrs_ext_transport CID0 CIDa5 eb (proc_addr j)
                 ltac:(wp_next_chain) with "Hextc") as "Hextc".
    iDestruct (cpu_claim_ext_transport CID0 CIDa5 eb (proc_addr j)
                 ltac:(wp_next_chain) with "Hextm") as "Hextm".
    iDestruct (eo_cont_shift (CIDa := CID0) (CIDb := CIDa5)  j pidv dq m K eb eb lks Upr
                 ltac:(wp_next_chain) with "Hcont") as "Hcont".
    iApply (BR.wp_bread_sconf γs j γl γu γd γk pd pav pu bn
              (fs_view γfs γd dev cov) pidv dev bnol dq A5 (K - 8)%nat eb eb lks Upr
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.end_op + 0xd0)) Rra
              (mword_of_int 2092450 : mword 21) B3 (K - 8)%nat eb
              ltac:(vm_compute; discriminate) ltac:(rdok)
              ltac:(vm_compute; reflexivity) with "Hcg Hpc []").
    ...
    assert (Htgtd0 : add_vec (mword_of_int (KernelSyms.end_op + 0xd0) : mword 64)
                       (sign_extend' 64 (mword_of_int 2092450 : mword 21))
                     = mword_of_int KernelSyms.bread)
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    iDestruct (cpu_own_transport CIDb1 CIDa9 0 eb (proc_addr j) eb
                 ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
    iDestruct (trap_csrs_ext_transport CIDb1 CIDa9 eb (proc_addr j)
                 ltac:(wp_next_chain) with "Hextc") as "Hextc".
    iDestruct (cpu_claim_ext_transport CIDb1 CIDa9 eb (proc_addr j)
                 ltac:(wp_next_chain) with "Hextm") as "Hextm".
    iDestruct (eo_cont_shift (CIDa := CIDa5) (CIDb := CIDa9)  j pidv dq m K eb eb lks Upr
                 ltac:(wp_next_chain) with "Hcont") as "Hcont".
    iApply (BR.wp_bread_sconf γs j γl γu γd γk pd pav pu bn
              (fs_view γfs γd dev cov) pidv dev w dq B4 (K - 8)%nat eb eb lks Upr
              ltac:(pose proof (eo_Kbread K HK); lia)
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.end_op + 0xe2)) Rra
              (mword_of_int 2084604 : mword 21) G4 (K - 8)%nat eb
              ltac:(vm_compute; discriminate) ltac:(rdok)
              ltac:(vm_compute; reflexivity) with "Hcg Hpc []").
    ...
    assert (Htgte2 : add_vec (mword_of_int (KernelSyms.end_op + 0xe2) : mword 64)
                       (sign_extend' 64 (mword_of_int 2084604 : mword 21))
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.end_op + 0xe8)) Rra
              (mword_of_int 2092640 : mword 21) H1 (K - 8)%nat eb
              ltac:(vm_compute; discriminate) ltac:(rdok)
              ltac:(vm_compute; reflexivity) with "Hcg Hpc []").
    ...
    assert (Htgte8 : add_vec (mword_of_int (KernelSyms.end_op + 0xe8) : mword 64)
                       (sign_extend' 64 (mword_of_int 2092640 : mword 21))
    ...
    iDestruct (cpu_own_transport CIDb2 CIDa17 0 eb (proc_addr j) eb
                 ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
    ...
    iDestruct (cpu_claim_ext_transport CIDb2 CIDa17 eb (proc_addr j)
                 ltac:(wp_next_chain) with "Hextm") as "Hextm".
    iDestruct (eo_cont_shift (CIDa := CIDa9) (CIDb := CIDa17)  j pidv dq m K eb eb lks Upr
                 ltac:(wp_next_chain) with "Hcont") as "Hcont".
    iApply (BW.wp_bwrite_sconf γs j γl γu γd γk pd pav pu bn
              (fs_view γfs γd dev cov) k1 pidv dev bnol dq H2 (K - 8)%nat eb
              bs2 bsd1 eb
              (log_mirror_half (lm_upd Mc (log_slot_bno logstart t) bs2)) lks Upr
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.end_op + 0xee)) Rra
              (mword_of_int 2092684 : mword 21) H3 (K - 8)%nat eb
              ltac:(vm_compute; discriminate) ltac:(rdok)
              ltac:(vm_compute; reflexivity) with "Hcg Hpc []").
    ...
    assert (Htgtee : add_vec (mword_of_int (KernelSyms.end_op + 0xee) : mword 64)
                       (sign_extend' 64 (mword_of_int 2092684 : mword 21))
    ...
    iDestruct (cpu_own_transport CIDb3 CIDa19 0 eb (proc_addr j) eb
                 ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
    ...
    iDestruct (cpu_claim_ext_transport CIDb3 CIDa19 eb (proc_addr j)
                 ltac:(wp_next_chain) with "Hextm") as "Hextm".
    iDestruct (eo_cont_shift (CIDa := CIDa17) (CIDb := CIDa19)  j pidv dq m K eb eb lks Upr
                 ltac:(wp_next_chain) with "Hcont") as "Hcont".
    iApply (BL.wp_brelse_sconf γs bn (fs_view γfs γd dev cov) k2 pidv dev w dq
              H4 (K - 8)%nat eb (proc_addr j) bs2 bsd2 d2 eb lks Upr
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.end_op + 0xf4)) Rra
              (mword_of_int 2092678 : mword 21) H5 (K - 8)%nat eb
              ltac:(vm_compute; discriminate) ltac:(rdok)
              ltac:(vm_compute; reflexivity) with "Hcg Hpc []").
    ...
    assert (Htgtf4 : add_vec (mword_of_int (KernelSyms.end_op + 0xf4) : mword 64)
                       (sign_extend' 64 (mword_of_int 2092678 : mword 21))
                     = mword_of_int KernelSyms.brelse)
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    iDestruct (cpu_own_transport CIDb4 CIDa21 0 eb (proc_addr j) eb
                 ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
    ...
    iDestruct (cpu_claim_ext_transport CIDa19 CIDa21 eb (proc_addr j)
                 ltac:(wp_next_chain) with "Hextm") as "Hextm".
    iDestruct (eo_cont_shift (CIDa := CIDa19) (CIDb := CIDa21)  j pidv dq m K eb eb lks Upr
                 ltac:(wp_next_chain) with "Hcont") as "Hcont".
    iApply (BL.wp_brelse_sconf γs bn (fs_view γfs γd dev cov) k1 pidv dev bnol dq
              H6 (K - 8)%nat eb (proc_addr j) bs2 bs2 d1 eb lks Upr
    ...
    { intros b Hb HbW.
      assert (Hbs : b <> log_slot_bno logstart t)
        by (apply (FsCrash.home_ne_slot logstart b t
                     (FsCrash.home_set_not_region cov logstart b Hb));
            exact (eo_t_lt_lb t n Ht Hn30)).
      rewrite Hubnol (lookup_insert_ne _ _ _ _ (not_eq_sym Hbs)).
      rewrite /Mc' (lm_upd_view_ne Mc (log_slot_bno logstart t) b bs2 Hbs).
      exact (Hrow b Hb HbW). }
    (* the tie at the bumped cursor: the fill wrote a log SLOT *)
    assert (Hslotout : log_slot_bno logstart t ∉ fs_home_set cov logstart).
    { intros Hin. apply (FsCrash.home_set_not_region cov logstart _ Hin).
      apply eo_slot_in_region. exact (eo_t_lt_lb t n Ht Hn30). }
    ...
    - (* the back edge is TAKEN *)
      ...
      clear Htgt100.
      iDestruct (cpu_own_transport CIDb5 CIDa25 0 eb (proc_addr j) eb
                   ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
      ...
      iDestruct (cpu_claim_ext_transport CIDa21 CIDa25 eb (proc_addr j)
                   ltac:(wp_next_chain) with "Hextm") as "Hextm".
      iDestruct (eo_cont_shift (CIDa := CIDa21) (CIDb := CIDa25)  j pidv dq m K eb eb lks Upr
                   ltac:(wp_next_chain) with "Hcont") as "Hcont".
  ...
  Proof.
    ...
    pose proof (locks_below_not_elem _ _ Hbelow) as Hfresh.
    pose proof Hregs as (Hsp & Hthr).
    iIntros "Hcg Hcnt Hpay Hextc Hextm Htok HRres #Htext Hpc #Hlctx #Hprocs Hppid Hframe Hjunk Hcont".
    iDestruct "Hlctx" as "(#Hlock & #Hdevc & #Hstc & _)".
    iDestruct (procs_inv_len γs with "Hprocs") as %Hlen.
    (* ===== +0x7a / +0x7e : a0 := &log ===== *)
    ...
    iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.end_op + 0x7e)) Ra0 Ra0
              (mword_of_int 1554 : mword 12) E1 (trap_res eb + (K - 8))%nat false
              ltac:(vm_compute; discriminate) ltac:(rdok) with "Hcg Hpc []").
    { iApply (eoi_7e with "Htext"). }
    iApply wp_next_off_intro. iIntros "Hcg Hpc".
    pose (E2 := <[Regidx Ra0 := regval_into_reg
                  (add_vec (E1 !!! Regidx Ra0 : mword 64)
                     (sign_extend' 64 (mword_of_int 1554 : mword 12)))]> E1).
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.end_op + 0x82)) Rra
              (mword_of_int 2089442 : mword 21) E2 (trap_res eb + (K - 8))%nat false
              ltac:(vm_compute; discriminate) ltac:(rdok)
              ltac:(vm_compute; reflexivity) with "Hcg Hpc []").
    { iApply (eoi_82 with "Htext"). }
    iApply wp_next_off_intro. iIntros "Hcg Hpc".
    ...
    assert (Htgt82 : add_vec (mword_of_int (KernelSyms.end_op + 0x82) : mword 64)
                       (sign_extend' 64 (mword_of_int 2089442 : mword 21))
                     = mword_of_int KernelSyms.wakeup)
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.end_op + 0x8a)) Ra0 Ra0
              (mword_of_int 1542 : mword 12) G1 (trap_res eb + (K - 8))%nat false
              ltac:(vm_compute; discriminate) ltac:(rdok) with "Hcg Hpc []").
    { iApply (eoi_8a with "Htext"). }
    iApply wp_next_off_intro. iIntros "Hcg Hpc".
    pose (G2 := <[Regidx Ra0 := regval_into_reg
                  (add_vec (G1 !!! Regidx Ra0 : mword 64)
                     (sign_extend' 64 (mword_of_int 1542 : mword 12)))]> G1).
    assert (HG2a0 : G2 !!! Regidx Ra0 = log_addr).
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.end_op + 0x8e)) Rra
              (mword_of_int 2084536 : mword 21) G2 (trap_res eb + (K - 8))%nat false
              ltac:(vm_compute; discriminate) ltac:(rdok)
              ltac:(vm_compute; reflexivity) with "Hcg Hpc []").
    { iApply (eoi_8e with "Htext"). }
    iApply wp_next_off_intro. iIntros "Hcg Hpc".
    ...
    assert (Htgt8e : add_vec (mword_of_int (KernelSyms.end_op + 0x8e) : mword 64)
                       (sign_extend' 64 (mword_of_int 2084536 : mword 21))
                     = mword_of_int KernelSyms.release)
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    iDestruct (cpu_claim_ext_transport CID0 CIDc1 eb (proc_addr j)
                 ltac:(wp_next_chain) with "Hextm") as "Hextm".
    iDestruct (eo_cont_shift (CIDa := CID0) (CIDb := CIDc1)  j pidv dq m K eb eb lks Upr
                 ltac:(wp_next_chain) with "Hcont") as "Hcont".
    iApply (eo_epi (CID0 := CIDc1)  j pidv dq m mr K eb eb lks Upr HK Hregs2
              with "Hcg Hcnt Hextc Hextm Htext Hpc Hppid Hframe Hjunk Hcont").
  Qed.
...
Section ProofEndOp.
  ...
  Proof.
    ...
    iDestruct (cpu_own_eb_agree with "Hcg Hcnt") as %Hbm.
    cbn in Hbm. subst b.
    iPoseProof (log_ctx_lock with "Hlctx") as "#Hlock".
    iAssert (eo_cont (CID0 := CID)  j pidv dq m K eb eb lks Upr)%I
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.end_op + 0x16)) Rra
              (mword_of_int 2084520 : mword 21) R5 (K - 8)%nat eb
              ltac:(vm_compute; discriminate) ltac:(rdok)
              ltac:(vm_compute; reflexivity) with "Hcg Hpc []").
    ...
    assert (Htgt16 : add_vec (mword_of_int (KernelSyms.end_op + 0x16) : mword 64)
                       (sign_extend' 64 (mword_of_int 2084520 : mword 21))
                     = mword_of_int KernelSyms.acquire)
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    iDestruct (cpu_own_transport CID CID10 0 eb (proc_addr j) eb
                 ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
    ...
    iDestruct (cpu_claim_ext_transport CID10 CIDq eb (proc_addr j)
                 ltac:(wp_next_chain) with "Hextm") as "Hextm".
    iDestruct (eo_cont_shift (CIDa := CID10) (CIDb := CIDq)  j pidv dq m K eb eb lks Upr
                 ltac:(wp_next_chain) with "Hcont") as "Hcont".
    assert (Hqs1 : macq !!! Regidx Rs1 = log_addr)
    ...
    - (* ---- THE COMMIT ARM: outstanding hit zero ---- *)
      (* ================================================================ *)
      (*  THE FILE SYSTEM'S LAW IS READ HERE (lane CE, plan section 3).    *)
      (*                                                                  *)
      (*  It needs the WAL's TRANSACTION AUTHORITY, and that authority     *)
      (*  lives inside [LogInv.log_res] -- behind the log lock, which      *)
      (*  end_op releases a dozen instructions below and only re-acquires  *)
      (*  after the whole commit has run.  So the reading happens HERE, in *)
      (*  the accounting critical section, at the one instant where the    *)
      (*  ledger is provably empty (this arm IS [out - 1 = 0], and         *)
      ...
      assert (Hommt0 : delete i0 om = ∅)
        by (apply map_size_empty_iff; lia).
      iApply fupd_wp.
      ...
      iApply (wp_bnez_x0_fall_s_sconf (mword_of_int (KernelSyms.end_op + 0x26))
                ...
                ltac:(vm_compute; discriminate) Hnz26 with "Hcg Hpc []").
      { iApply (eoi_26 with "Htext"). }
      iApply wp_next_off_intro. iIntros "Hcg Hpc".
      ...
      iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.end_op + 0x2e)) Rs1 Rs1
                (mword_of_int 1634 : mword 12) U1 (trap_res eb + (K - 8))%nat false
                ltac:(vm_compute; discriminate) ltac:(rdok) with "Hcg Hpc []").
      { iApply (eoi_2e with "Htext"). }
      iApply wp_next_off_intro. iIntros "Hcg Hpc".
      pose (U2 := <[Regidx Rs1 := regval_into_reg
                    (add_vec (U1 !!! Regidx Rs1 : mword 64)
                       (sign_extend' 64 (mword_of_int 1634 : mword 12)))]> U1).
      ...
      iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.end_op + 0x38)) Rra
                (mword_of_int 2084622 : mword 21) U4 (trap_res eb + (K - 8))%nat false
                ltac:(vm_compute; discriminate) ltac:(rdok)
                ltac:(vm_compute; reflexivity) with "Hcg Hpc []").
      { iApply (eoi_38 with "Htext"). }
      iApply wp_next_off_intro. iIntros "Hcg Hpc".
      ...
      assert (Htgt38 : add_vec (mword_of_int (KernelSyms.end_op + 0x38) : mword 64)
                         (sign_extend' 64 (mword_of_int 2084622 : mword 21))
                       = mword_of_int KernelSyms.release)
        by (apply bv_eq; vm_compute; reflexivity).
      ...
      iDestruct (eo_cont_shift (CIDa := CIDq) (CIDb := CIDr)  j pidv dq m K eb eb lks Upr
                   ltac:(wp_next_chain) with "Hcont") as "Hcont".
      (* ---- the batch, already opened above (lane CE) ---- *)
      ...
      clear Hpp3e.
      iAssert (eo_open bn γfs cov logstart nl W L Dd (fun _ => []) 0)
      ...
      + (* n = 0: the whole commit body is skipped *)
        ...
        subst W. subst nl.
        iDestruct (cpu_own_transport CIDr CIDs2 0 eb (proc_addr j) eb
                     ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
        iDestruct (trap_csrs_ext_transport CIDr CIDs2 eb (proc_addr j)
                     ltac:(wp_next_chain) with "Hextc") as "Hextc".
        iDestruct (cpu_claim_ext_transport CIDr CIDs2 eb (proc_addr j)
                     ltac:(wp_next_chain) with "Hextm") as "Hextm".
        iDestruct (eo_cont_shift (CIDa := CIDr) (CIDb := CIDs2)  j pidv dq m K eb eb lks Upr
                     ltac:(wp_next_chain) with "Hcont") as "Hcont".
      ...
      + (* n > 0: save s3/s4/s5, set up the cursors, and run the copy loop *)
        ...
        clear Hppb4.
        iDestruct (cpu_own_transport CIDr CIDs9 0 eb (proc_addr j) eb
                     ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
    ...
    - (* ---- THE FAST PATH: other operations are still open ---- *)
      ...
      iApply (eo_fast (CID0 := CIDq)  γs j γl bn γ γfs cov logstart dev pidv dq
                m T4 K eb lks Upr HK HT4regsE Hbelow
                with "Hcg Hcnt Hpay Hextc Hextm Htok HRres Htext Hpc Hlctx Hprocs Hppid Hframe Hjunk Hcont").
  Qed.
