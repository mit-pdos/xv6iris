...
Section ProofConsoleread.
    ...
    (wp_next (CID0 := CID0) true (proc_addr jp) (fun (CIDx : CpuId) =>
     (* the image moves: either_copyout writes user memory, so the five
       ...
       ∀ (M : regfile) (P' : uptd) (Mo : gmap Z (bv 8)) (nc : Z)
         ...
         cr_winR cn Wd ord (cr_fault U (m0 !!! Regidx Ra1))
  ...
  Proof.
    ...
    iApply fupd_wp.
    iMod (cr_winO_of_winR cn Wd γc ord (cr_fault U (m0 !!! Regidx Ra1))
  ...
  Proof.
    ...
    { (* ============ c == C('D'): the end-of-file break at +0xe2 ======== *)
      ...
      { (* nc >= n: nothing copied, so no push-back.  -> +0xf6 ---- *)
        ...
        iApply fupd_wp.
        iMod (cr_pop_swallow cn Wd γc ord (cr_fault U (m0 !!! Regidx Ra1))
                rr ww ee bs ts
  ...
  Proof.
    ...
    iAssert (cr_ret (CID0 := CID) cn Wd ord (cr_fault U (m !!! Regidx Ra1))
