...
Section ForwardMulGpr.
  ...
  Proof using All.
    ...
    assert (LSA : eq_vec (_get_Misa_S (register_lookup misa sAmg.(sregs))) ('b"1") = true).
    { unfold sAmg, set_reg; cbn [sregs]. rewrite irrelevant_register_set;
...
Section WpMulGpr.
  ...
  Proof.
    ...
    iModIntro.
    iExists (sFcmg s pc b1 rs2 rs1 rd (register_lookup minstret s.(sregs))). iSplitR.
    { iPureIntro.
      rewrite <- (sFmg_eq s pc b1 rs2 rs1 rd (register_lookup minstret s.(sregs)) Lpc eq_refl).
...
Section WpMulGprDemo.
  ...
  Definition wp_mul_x5_x6_x7 (pc : mword 64) (w : mword 32) :=
    wp_mul_gpr (dqc:=DfracOwn 1) pc w (mword_of_int 7) (mword_of_int 6) (mword_of_int 5).
  (* `mul x28, x1, x2` : rd=x28, rs1=x1, rs2=x2.  SAME lemma, different regs. *)
  Definition wp_mul_x28_x1_x2 (pc : mword 64) (w : mword 32) :=
    wp_mul_gpr (dqc:=DfracOwn 1) pc w (mword_of_int 2) (mword_of_int 1) (mword_of_int 28).
