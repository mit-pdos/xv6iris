...
Definition wp_push_off_sconf_body `{!riscvGS Σ, !xv6G Σ} `{GEN : GenId} `{CID : CpuId} `{XI : CurCtx} (kt : ktier) (m : regfile) (av : nat)
...
Definition wp_pop_off_sconf_body `{!riscvGS Σ, !xv6G Σ} `{GEN : GenId} `{CID : CpuId} `{XI : CurCtx} (kt : ktier) (m : regfile) (av : nat)
...
Module Type PUSHOFF.
  Parameter wp_push_off_sconf :
    forall `{!riscvGS Σ, !xv6G Σ} (kt : ktier) `{GEN : GenId} `{CID : CpuId} `{XI : CurCtx} (m : regfile) (av : nat)
      (n : nat) (eb : bool) (p : mword 64) (b : bool) (lks : gset string),
