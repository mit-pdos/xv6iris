...
Section VRU4Walk.
  ...
  Lemma exec_execute_LOAD_2_U_walk (is_unsigned : bool) :
    exec (execute (LOAD (imm, Regidx rs1, Regidx rd, is_unsigned, 2))) s
      = Some (RETIRE_SUCCESS,
              set_reg s' (R_bitvector_64 (gpr_of_Z (uint rd)))
                (regval_into_reg (extend_value is_unsigned data2))).
  Proof.
    ...
    assert (Hpmam' : matching_pma_region (register_lookup pma_regions s'.(sregs)) (Physaddr pa) 2 = Some region)
    ...
    assert (Hbytes' : forall j : nat, (N.of_nat j < 2)%N ->
              s'.(mem) !! (pa_add pa j) = Some (nth_byte v j)).
    { intros j Hj. unfold s', set_reg; cbn [mem]. exact (Hbytes j Hj). }
    exact (exec_execute_LOAD_2_gpr_walk User is_unsigned rs1 rd imm ea v region s s' pa
             Hrd Htea Halign Htr Hcp' Hmprv' Hpmp Hpmam' Hpalign Hread
             (within_clint_false pa 2 s' Hnc ltac:(lia))
             (within_sig_false pa 2 s' Hns ltac:(lia))
             (within_htif_false pa 2 s' Lhtif')
...
Section VRU4WalkNomatch.
  ...
  Lemma exec_execute_LOAD_2_U_walk_nomatch (is_unsigned : bool) :
    exec (execute (LOAD (imm, Regidx rs1, Regidx rd, is_unsigned, 2))) s
      = Some (RETIRE_SUCCESS,
              set_reg s' (R_bitvector_64 (gpr_of_Z (uint rd)))
                (regval_into_reg (extend_value is_unsigned data2))).
...
Section VWU4Walk.
  ...
  Lemma exec_execute_STORE_2_U_walk :
    exec (execute (STORE (imm, Regidx rs2, Regidx rs1, 2))) s
      = Some (RETIRE_SUCCESS,
              MState s'.(sregs) (write_bytes s'.(mem) pa 2
                (autocast (T := mword) (subrange_vec_dec vrs2 (Z.sub (Z.mul 2 8) 1) 0) : mword 16)) s'.(mdev)).
  Proof.
    ...
    assert (Lhtif' : register_lookup htif_tohost_base s'.(sregs) = None)
      by (unfold s', set_reg; cbn [sregs]; rewrite irrelevant_register_set; [ exact Hhtif | vm_compute; reflexivity ]).
    exact (exec_execute_STORE_2_gpr_walk User rs2 rs1 imm ea region s s' pa
             Htea Halign Htr Hcp' Hmprv' Hpmp Hpmam' Hpalign Hwrite
             (within_clint_false pa 2 s' Hnc ltac:(lia))
             (within_sig_false pa 2 s' Hns ltac:(lia))
             (within_htif_false pa 2 s' Lhtif' : exec (within_htif_writable (Physaddr pa) 2) s' = Some (false, s'))
...
Section VWU4WalkNomatch.
  ...
  Lemma exec_execute_STORE_2_U_walk_nomatch :
    exec (execute (STORE (imm, Regidx rs2, Regidx rs1, 2))) s
      = Some (RETIRE_SUCCESS,
              MState s'.(sregs) (write_bytes s'.(mem) pa 2
                (autocast (T := mword) (subrange_vec_dec vrs2 (Z.sub (Z.mul 2 8) 1) 0) : mword 16)) s'.(mdev)).
