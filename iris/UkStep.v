...
(*                                                                         *)
(*   uk_step_obl π Kc sz fdv cw gn cs M m pc   the caller's fetch-onward obligation, at    *)
...
(*   uk_ih π Kc fdv cw gn cs M m pc         the Löb hypothesis: from the bundle [uvb]    *)
...
(* here [R := (Kc ∧ ukc π M sz fdv cw gn cs m pc) ∗ Rut pt ∗ Rfd fdv ∗ ukb C pt Rfd Rut sz π fdv cw gn cs] -- the     *)
...
(* [ukc π M sz fdv cw gn cs m pc] and NOT the leaf's post-instruction [Kc].  Both cannot   *)
...
Section UkBundle.
  ...
  Lemma uvb_elim (C : ucfg) (pt : uptd) (Rfd : list fdstate -> iProp Σ) (Rut : uptd -> iProp Σ)
      (sz : Z)
      (π : gmap (mword 27) uperm) (M : gmap Z (bv 8)) (m : regfile) (pc : mword 64) (fdv : list fdstate) (cw : Z) (gn : gname) (cs : gset gname) :
    uvb C pt Rfd Rut sz π fdv cw gn cs M m pc -∗
    ∃ Mp : gmap Z (bv 8),
    ⌜uk_pt_pure pt sz M Mp⌝ ∗
    uv_amb ∗ uv_regs ∗
    utlb_inv_pt (ud_root pt) (ud_tfp pt) (ud_um pt) ∗ umem_x pt Mp ∗
    (* the descriptor fragments come out beside the image, and go back in
       ...
       as it threads [umem] *)
    Rfd fdv ∗
    user_cfg C ∗ gpr_file m ∗ pc_is pc ∗ Rut pt ∗ ▷ ukb C pt Rfd Rut sz π fdv cw gn cs.
  Proof.
  ...
  Lemma uvb_intro (C : ucfg) (pt : uptd) (Rfd : list fdstate -> iProp Σ) (Rut : uptd -> iProp Σ)
      ...
      (pc : mword 64) (fdv : list fdstate) (cw : Z) (gn : gname) (cs : gset gname) :
    uk_pt_pure pt sz M Mp ->
    uv_amb -∗ uv_regs -∗
    utlb_inv_pt (ud_root pt) (ud_tfp pt) (ud_um pt) -∗ umem_x pt Mp -∗
    Rfd fdv -∗
  ...
  Proof.
    ...
    { iFrame "Htlb".
      ...
      rewrite /umem_lazy_x /umem_own_x. iExists Mp.
  ...
  Lemma uvb_pure (C : ucfg) (pt : uptd) (Rfd : list fdstate -> iProp Σ) (Rut : uptd -> iProp Σ) (sz : Z)
      (π : gmap (mword 27) uperm) (M : gmap Z (bv 8)) (m : regfile) (pc : mword 64) (fdv : list fdstate) (cw : Z) (gn : gname) (cs : gset gname) :
    uvb C pt Rfd Rut sz π fdv cw gn cs M m pc -∗
    ⌜exists Mp : gmap Z (bv 8), uk_pt_pure pt sz M Mp⌝ ∗ uvb C pt Rfd Rut sz π fdv cw gn cs M m pc.
  Proof.
    rewrite /uvb /uvb_F.
    iIntros "(Hamb & Hur & %Hsz & Hpt & Hfrag & Hcfg & Hg & Hpc & Hrut & Hk)".
    iAssert (⌜exists Mp : gmap Z (bv 8), uk_pt_pure pt sz M Mp⌝ ∗
             user_ptm_inv_x pt sz M)%I with "[Hpt]" as "[%Hp Hpt]".
    { rewrite /user_ptm_inv_x /umem_lazy_x.
      iDestruct "Hpt" as "(Htlb & Hlz & %Hinj & %Hacc)".
      iDestruct "Hlz" as (Mp) "(%Hsub & %Himg & %Hz & [%Hdom Hmem])".
      iSplitR.
      { iPureIntro. exists Mp.
        exact (ukp_intro pt sz M Mp Hsub Himg Hz Hdom Hinj Hacc Hsz). }
      rewrite /user_ptm_inv_x /umem_lazy_x /umem_own_x. iFrame "Htlb".
  ...
  Lemma uvb_x0 (C : ucfg) (pt : uptd) (Rfd : list fdstate -> iProp Σ) (Rut : uptd -> iProp Σ) (sz : Z)
      (π : gmap (mword 27) uperm) (M : gmap Z (bv 8)) (m : regfile) (pc : mword 64) (fdv : list fdstate) (cw : Z) (gn : gname) (cs : gset gname) :
    uvb C pt Rfd Rut sz π fdv cw gn cs M m pc -∗
    ⌜m !!! Regidx (mword_of_int 0) = zero_reg⌝ ∗ uvb C pt Rfd Rut sz π fdv cw gn cs M m pc.
  Proof.
    rewrite /uvb /uvb_F.
    iIntros "(Hamb & Hur & %Hsz & Hpt & Hfrag & Hcfg & Hg & Hpc & Hrut & Hk)".
  ...
  Lemma trapped_of_uv_trap_frame (C : ucfg) (pt : uptd) (Rut : uptd -> iProp Σ)
      ...
      (sz : Z) (π : gmap (mword 27) uperm) (fdv : list fdstate) (cw : Z) (gn : gname) (cs : gset gname) :
    uk_pt_pure pt sz M Mp ->
    m !!! Regidx (mword_of_int 0) = zero_reg ->
    uv_trap_frame C pt sc stv pc m Mp -∗ Rut pt -∗
    trapped_machine C pt Rut sz sc stv (uvis_of_run m pc M π sz fdv cw gn cs).
  Proof.
...
Section UkObl.
  ...
  Definition uk_step_obl (π : gmap (mword 27) uperm) (Kc : iProp Σ)
    ...
    (∀ (R : iProp Σ) (CIDo : CpuId) (XIo : TsoCtx.CurCtx)
       ...
       (R -∗ (TsoCtx.own_context (CID := CIDo) XIo -∗ Rut pt) ∗ Rfd fdv ∗ ukb (CID := CIDo) C pt Rfd Rut sz π fdv cw gn cs ∗ uk_paycont Q gn (Kc ∧ ukc π M sz fdv cw gn cs m pc)) -∗
       resv_any cpu_id -∗
       hreg_frame rsA u_Drw -∗
       hreg_frame_ro (u_Df (uc_dqc C)) rsA u_Dro -∗
       TsoCtx.own_context (CID := CIDo) XIo -∗
       uv_bytes (XI := XIo) pt Mp t -∗
  ...
  Definition uk_ih (π : gmap (mword 27) uperm) (Kc : iProp Σ)
    ...
    (∀ (h : CpuId) (xi : TsoCtx.CurCtx) (C : ucfg) (pt : uptd) (Rfd : list fdstate -> iProp Σ)
       ...
       uvb (CID := h) (XI := xi) C pt Rfd Rut sz π fdv cw gn cs M m pc -∗
       □ uk_step_obl π Kc Q sz fdv cw gn cs M m pc -∗
       (* THE PAYMENT RIDES THE STEP'S OWN LATER, with the continuation it
          is handed back into: the payload this builds is proved under the
  ...
  Definition uk_payload `{CID : CpuId} (sz : Z) (π : gmap (mword 27) uperm)
      (fdv : list fdstate) (cw : Z) (gn : gname) (cs : gset gname) (Kc : iProp Σ)
      (Q : Z -> iProp Σ)
      (M : gmap Z (bv 8)) (m : regfile) (pc : mword 64)
      (C : ucfg) (pt : uptd) (Rfd : list fdstate -> iProp Σ)
      (Rut : uptd -> iProp Σ) : iProp Σ :=
    (uk_paycont Q gn (Kc ∧ ukc π M sz fdv cw gn cs m pc) ∗
     (TsoCtx.own_context TsoCtx.cur_ctx -∗ Rut pt) ∗
     Rfd fdv ∗ ukb C pt Rfd Rut sz π fdv cw gn cs)%I.
...
Section UkArms.
  ...
  Lemma uk_psi_active (R : iProp Σ) (Rut : uptd -> iProp Σ) (sz : Z)
      ...
      (rs2 : regstate) (fdv : list fdstate) (cw : Z) (gn : gname) (cs : gset gname) :
    ...
    (R -∗ (TsoCtx.own_context XI -∗ Rut pt) ∗ Rfd fdv ∗ ukb C pt Rfd Rut sz π fdv cw gn cs ∗
          (uvb C pt Rfd Rut sz π fdv cw gn cs M m' npc -∗ WP (Loop : expr riscv_lang))) -∗
    uv_psi C R rs2.
  Proof.
    ...
    iDestruct (uv_land_close C pt Mp m' npc t usatp pcfg paddr User rs2 rs3
      ...
      as "(Hhs & Hpriv & Hms & Hsc & Hstval & Hsepc & Hpc & Hgpr & Hcfg &
           Hutlb & Humem)".
    iApply "Hcont".
    iApply (uvb_intro C pt Rfd Rut sz π M Mp m' npc fdv cw gn cs Hpure
  ...
  Lemma uk_arm_intr (Rut : uptd -> iProp Σ) (sz : Z) (π : gmap (mword 27) uperm)
      (Kc : iProp Σ) (Q : Z -> iProp Σ) (M Mp : gmap Z (bv 8)) (m : regfile)
      (pc : mword 64) (t : ptree) (usatp : mword 64)
      (pcfg : type_of_register pmpcfg_n) (paddr : type_of_register pmpaddr_n)
      (rs1 rsA : regstate) (i : InterruptType) (fdv : list fdstate) (cw : Z) (gn : gname) (cs : gset gname) :
    uv_pre C pt Mp m pc t rs1 rsA usatp pcfg paddr ->
    uk_pt_pure pt sz M Mp ->
    ...
    uv_bytes pt Mp t -∗
    uv_res pt Mp t usatp pcfg paddr -∗
    uv_step_post C (uk_payload sz π fdv cw gn cs Kc Q M m pc C pt Rfd Rut) rs1
  ...
  Proof.
    ...
    iFrame "Hrw Hro".
    iApply (uv_psi_trap C pt (uk_payload sz π fdv cw gn cs Kc Q M m pc C pt Rfd Rut) Mp m t usatp pcfg paddr
    ...
    iDestruct ("Hbak" with "Hctx") as "Hrut".
    iApply ("Hkb" $! (uvis_of_run m pc M π sz fdv cw gn cs)
              (utrap_scause (Interrupt i) (register_lookup (R_bitvector_64 scause) rsA))
              (tval None) with "[%] [%] [%] [%] [%] [%] [Hframe Hrut Hfdr Hkc Hpayv]");
      [ reflexivity | reflexivity | reflexivity | reflexivity
      | reflexivity | reflexivity | ].
    iSplitL "Hframe Hrut".
    { iApply (trapped_of_uv_trap_frame C pt Rut _ _ m pc M Mp sz π fdv cw gn cs Hpure Hx0
                with "Hframe Hrut"). }
    ...
    iSplitL "Hpayv";
      [ iApply (uexec_pay_dep_ne _ (uvis_of_run m pc M π sz fdv cw gn cs) _ (sfam_at Q sfam_pt)
                  (utrap_scause_intr_ne i
                     (register_lookup (R_bitvector_64 scause) rsA))
                  (sexit_pay_at Q sfam_pt) with "Hmyp Hpayv") | ].
    iIntros "Hpayv".
    rewrite (uslot_run m pc M π sz fdv cw gn cs Hx0 Hal2).
...
Section UkStepEngine.
  ...
  Lemma wp_uk_step_gen (π : gmap (mword 27) uperm) (Kc : iProp Σ)
      (Q : Z -> iProp Σ)
      (M : gmap Z (bv 8)) (m : regfile) (pc : mword 64) (fdv : list fdstate) (cw : Z) (gn : gname) (cs : gset gname) :
    is_aligned_vaddr (Virtaddr pc) 2 = true ->
    ⊢ uk_ih π Kc Q fdv cw gn cs M m pc.
  Proof.
    intros Hal2.
    rewrite /uk_ih.
    iLöb as "IH".
    ...
    iApply (swp_exec_step_full u_Drw u_Dro (u_Df (uc_dqc C)) RS (wrap_pre RS)
              (uv_land RS) (uv_psi C (uk_payload sz π fdv cw gn cs Kc Q M m pc C pt Rfd Rut))
              ...
              with "Hcert Hresv Hrw Hro [Hmm Hcl Hctx] [Hpay3 Hbak Hfdv Hk]").
    - (* ================= THE BODY SLOT ================= *)
      ...
      2: iApply (swp_run_hart_active_res u_Drw u_Dro (u_Df (uc_dqc C))
                   ...
                   (fun (ii : InterruptType) (pr : Privilege) =>
                      uv_step_post C (uk_payload sz π fdv cw gn cs Kc Q M m pc C pt Rfd Rut) RS
      ...
      + (* ---- THE DISPATCH ---- *)
        ...
        * assert (Hsup : pr = Supervisor).
          ...
          iApply (uk_arm_intr C pt Rfd Rut sz π Kc Q M Mp m pc t usatp pcfg paddr RS
                    (wrap_pre RS) ii fdv cw gn cs Hpre Hpure Hal2
      ...
      + (* ---- THE FETCH: the caller's obligation ---- *)
        ...
        iDestruct "HWd" as "(Hfrag & Hctx & Hmm & Hres)".
        iDestruct (resv_any_intro cpu_id None with "Hfrag") as "Hany".
        iApply ("Hobl" $! (uk_payload sz π fdv cw gn cs Kc Q M m pc C pt Rfd Rut) CID XIv C pt Rfd Rut HRut Mp t RS
...
Section UkFunnel.
  ...
  Lemma wp_uk_step (Kc : iProp Σ) (Q : Z -> iProp Σ) (M : gmap Z (bv 8)) (m : regfile) (pc : mword 64) (fdv : list fdstate) (cw : Z) (gn : gname) (cs : gset gname) :
    is_aligned_vaddr (Virtaddr pc) 2 = true ->
    uvb C pt Rfd Rut sz π fdv cw gn cs M m pc -∗ □ uk_step_obl π Kc Q sz fdv cw gn cs M m pc -∗
    ▷ (my_pay gn Q ∗ Q (-1) ∗ (Q (-1) -∗ Kc)) -∗ WP (Loop : expr riscv_lang).
  Proof.
    intros Hal2.
    iIntros "Hb #Hobl Hpay3".
    iPoseProof (wp_uk_step_gen π Kc Q M m pc fdv cw gn cs Hal2) as "H". rewrite /uk_ih.
...
Section UkPostFetch.
  ...
  Lemma uk_retire_post_fetch (R : iProp Σ) (Rut : uptd -> iProp Σ) (sz : Z)
      ...
      (usatp : mword 64) (pcfg : type_of_register pmpcfg_n)
      (paddr : type_of_register pmpaddr_n) (rs1 rs2 : regstate) (fdv : list fdstate) (cw : Z) (gn : gname) (cs : gset gname) :
    uv_wrok wr ->
    uv_redirect i o ->
    (forall s_pc : mstate,
    ...
    gen_cert -∗ uv_amb -∗
    (R -∗ (TsoCtx.own_context XI -∗ Rut pt) ∗ Rfd fdv ∗ ukb C pt Rfd Rut sz π fdv cw gn cs ∗
          (uvb C pt Rfd Rut sz π fdv cw gn cs M (uv_upd m wr) (uv_next jt (add_vec_int pc k)) -∗
  ...
  Proof.
    ...
    iApply (uk_psi_active C pt Rfd R Rut sz π M Mp (uv_upd m wr)
              ...
              (uv_post_rs rsx jt wr) fdv cw gn cs
...
Section UkObligation.
  ...
  Lemma uk_obl_base (R : iProp Σ) (Rut : uptd -> iProp Σ) (sz : Z)
      ...
      (pcfg : type_of_register pmpcfg_n) (paddr : type_of_register pmpaddr_n)
      (rs1 rsA : regstate) (fdv : list fdstate) (cw : Z) (gn : gname) (cs : gset gname) :
    uv_pre C pt Mp m pc t rs1 rsA usatp pcfg paddr ->
    uk_pt_pure pt sz M Mp ->
    udecode_base w i ->
    ...
    (R -∗ (TsoCtx.own_context XI -∗ Rut pt) ∗ Rfd fdv ∗ ukb C pt Rfd Rut sz π fdv cw gn cs ∗
          (uvb C pt Rfd Rut sz π fdv cw gn cs M (uv_upd m wr) (uv_next jt (add_vec_int pc 4)) -∗
           WP (Loop : expr riscv_lang))) -∗
    resv_any cpu_id -∗
    hreg_frame rsA u_Drw -∗ hreg_frame_ro (u_Df (uc_dqc C)) rsA u_Dro -∗
    TsoCtx.own_context XI -∗
  ...
  Proof.
    ...
    iApply (uk_retire_post_fetch C pt Rfd R Rut sz π M Mp m pc 4 i o jt wr
              (zero_extend' 32 w) t' usatp pcfg paddr rs1 rs2 fdv cw gn cs
  ...
  Lemma uk_obl_rvc (R : iProp Σ) (Rut : uptd -> iProp Σ) (sz : Z)
    ...
    (R -∗ (TsoCtx.own_context XI -∗ Rut pt) ∗ Rfd fdv ∗ ukb C pt Rfd Rut sz π fdv cw gn cs ∗
          (uvb C pt Rfd Rut sz π fdv cw gn cs M (uv_upd m wr) (uv_next jt (add_vec_int pc 2)) -∗
           WP (Loop : expr riscv_lang))) -∗
    resv_any cpu_id -∗
    hreg_frame rsA u_Drw -∗ hreg_frame_ro (u_Df (uc_dqc C)) rsA u_Dro -∗
    TsoCtx.own_context XI -∗
  ...
  Proof.
    ...
    iApply (uk_retire_post_fetch C pt Rfd R Rut sz π M Mp m pc 2 i o jt wr
              (zero_extend' 32 h) t' usatp pcfg paddr rs1 rs2 fdv cw gn cs
              Hwrok Hred Hg1 Hg2 Hexec
              (T2 _ _ u_in_PC ltac:(vm_compute; reflexivity) LpcA)
              (T2 _ _ u_in_hart ltac:(vm_compute; reflexivity) LhsA)
              Lcp2
...
Section UkRetire.
  ...
  Context `{!ctokG Σ}.
  Context {SG : uexecSG Σ}.
  (* THE PAYLOAD THE RUN KEEPS, as an IMPLICIT section variable: the leaves
     below thread it (their continuation is [UexecRet.ukcq] at it) and no
     call site names it -- it is read off the continuation the caller
     supplies. *)
  ...
  Lemma wp_uk_retire_later (M : gmap Z (bv 8))
      (m : regfile) (pc : mword 64) (fdv : list fdstate) (cw : Z) (gn : gname) (cs : gset gname)
    ...
    (forall s_pc : mstate,
       ...
       exec (execute (uv_exp i o)) s_pc
         = Some (RETIRE_SUCCESS, uv_post s_pc jt wr)) ->
    uvb C pt Rfd Rut sz π fdv cw gn cs M m pc -∗
    ▷ ukcq Qp π M sz fdv cw gn cs (uv_upd m wr) (uv_next jt (add_vec_int pc (if is_rvc then 2 else 4))) -∗
    WP (Loop : expr riscv_lang).
  Proof.
    ...
    iAssert (R -∗ (TsoCtx.own_context (CID := CIDo) XIo -∗ Rut' pt') ∗ Rfd' fdv ∗
             ukb C' pt' Rfd' Rut' sz π fdv cw gn cs ∗
             (uvb (CID := CIDo) C' pt' Rfd' Rut' sz π fdv cw gn cs M (uv_upd m wr)
    ...
    - iDestruct "Hf" as (h) "[[%HisRVC %Hdecrvc] Hbridge]".
      iApply (uk_obl_rvc C' pt' Rfd' R Rut' sz π M Mp' m pc h i o jt wr t usatp pcfg paddr
                rs1 rsA fdv cw gn cs Hpre Hpure Hdecrvc Hwrok Hred Hg1 Hg2 Hexec
                with "Hcert Hamb Hbridge Hk Hany Hrw Hro Hctx Hmm Hres").
    - iDestruct "Hf" as (w) "[[%HnRVC %Hdecbase] Hbridge]".
      iApply (uk_obl_base C' pt' Rfd' R Rut' sz π M Mp' m pc w i o jt wr t usatp pcfg paddr
                rs1 rsA fdv cw gn cs Hpre Hpure Hdecbase Hwrok Hred Hg1 Hg2 Hexec
                with "Hcert Hamb Hbridge Hk Hany Hrw Hro Hctx Hmm Hres").
  Qed.

  (* the later-free restatement: the shape every leaf takes *)
  Lemma wp_uk_retire (M : gmap Z (bv 8)) (m : regfile)
      (pc : mword 64) (fdv : list fdstate) (cw : Z) (gn : gname) (cs : gset gname) (is_rvc : bool) (i : instruction)
      (o : option instruction) (jt : option (mword 64))
      (wr : option (mword 5 * mword 64)) :
    uk_instr π M pc is_rvc i ->
    uv_redirect i o ->
  ...
  Proof.
    intros Hui Hred Hlpad Hwrok Hg1 Hg2 Hexec.
    iIntros "Hb Hcont".
    iApply (wp_uk_retire_later M m pc fdv cw gn cs is_rvc i o jt wr
...
Section UkEcallPost.
  ...
  Lemma uk_ecall_post_fetch (R : iProp Σ) (Rut : uptd -> iProp Σ) (sz : Z)
    ...
    uk_pt_pure pt sz M Mp ->
    gen_cert -∗
    (R -∗ (TsoCtx.own_context XI -∗ Rut pt) ∗ Rfd fdv ∗ ukb C pt Rfd Rut sz π fdv cw gn cs ∗ uexec_ret uecall_scause (uvis_of_run m pc M π sz fdv cw gn cs)) -∗
    resv_any cpu_id -∗
    TsoCtx.own_context XI -∗
    uv_bytes pt Mp t' -∗
    uv_res pt Mp t' usatp pcfg paddr -∗
...
Section UkEcall.
  ...
  Lemma wp_uk_ecall (M : gmap Z (bv 8)) (m : regfile) (pc : mword 64) (fdv : list fdstate) (cw : Z) (gn : gname) (cs : gset gname) :
    ...
    my_pay gn Qp -∗ Qp (-1) -∗
    (Qp (-1) -∗ uexec_ret uecall_scause (uvis_of_run m pc M π sz fdv cw gn cs)) -∗
    WP (Loop : expr riscv_lang).
  Proof.
    intros Hui Hg.
    pose proof (Hui pt sz (loop_ok_wf C pt Hlo) Hpm) as Hui0.
    ...
    iIntros (R CIDo XIo C' pt' Rfd' Rut' HRut' Mp' t rs1 rsA usatp pcfg paddr)
      "%Hlo' %Hpm' %Hpure %Hpre #Hamb Hk Hany Hrw Hro Hctx Hmm Hres".
    iAssert (R -∗ (TsoCtx.own_context (CID := CIDo) XIo -∗ Rut' pt') ∗ Rfd' fdv ∗
             ukb (CID := CIDo) C' pt' Rfd' Rut' sz π fdv cw gn cs ∗
             uexec_ret uecall_scause (uvis_of_run m pc M π sz fdv cw gn cs))%I with "[Hk]" as "Hk".
    { iIntros "HR". iDestruct ("Hk" with "HR") as "(Hbak & Hfdr & Hkb & Hkc)".
      iDestruct "Hkc" as "(_ & Hpayv & Hkc)".
      iDestruct ("Hkc" with "Hpayv") as "Hkc".
    ...
    2:{ destruct (is_aligned_vaddr (Virtaddr pc) 4) eqn:Hal4.
        - iPoseProof (uv_swp_fetch4 (CID := CIDo) (XI := XIo) pt' Mp' t (uc_dqc C')
    ...
    iApply (uk_ecall_post_fetch C' pt' Rfd' R Rut' sz π M Mp' m pc (zero_extend' 32 w) t' usatp
              pcfg paddr rs1 rs2 fdv cw gn cs Hg
              (T2 _ _ u_in_PC ltac:(vm_compute; reflexivity) LpcA)
              (T2 _ _ u_in_hart ltac:(vm_compute; reflexivity) LhsA)
              Lcp2
              ltac:(rewrite (T2 _ _ u_in_mst ltac:(vm_compute; reflexivity)
