...
Section UkBundle.
  ...
  Lemma uvb_elim (C : ucfg) (pt : uptd) (Rfd : list fdstate -> iProp Σ) (Rut : uptd -> iProp Σ)
    ...
    (* the descriptor fragments come out beside the image, and go back in
       ...
       as it threads [umem] *)
    Rfd fdv ∗
    user_cfg C ∗ gpr_file m ∗ pc_is pc ∗ Rut pt ∗ ▷ ukb C pt Rfd Rut sz π fdv cw gn cs pidv false secc_all.
  Proof using .
  ...
  Lemma uvb_pure (C : ucfg) (pt : uptd) (Rfd : list fdstate -> iProp Σ) (Rut : uptd -> iProp Σ) (sz : Z)
      (π : gmap (mword 27) uperm) (M : gmap Z (bv 8)) (m : regfile) (pc : mword 64) (fdv : list fdstate) (cw : Z) (gn : gname) (cs : gset gname) (pidv : mword 32) :
    uvb C pt Rfd Rut sz π fdv cw gn cs pidv false secc_all M m pc -∗
    ⌜exists Mp : gmap Z (bv 8), uk_pt_pure pt sz M Mp⌝ ∗ uvb C pt Rfd Rut sz π fdv cw gn cs pidv false secc_all M m pc.
  Proof using .
    rewrite /uvb /uvb_F.
    iIntros "(Hamb & Hur & %Hsz & Hpt & Hfrag & Hcfg & Hg & Hpc & Hrut & Hk)".
  ...
  Lemma uvb_x0 (C : ucfg) (pt : uptd) (Rfd : list fdstate -> iProp Σ) (Rut : uptd -> iProp Σ) (sz : Z)
      (π : gmap (mword 27) uperm) (M : gmap Z (bv 8)) (m : regfile) (pc : mword 64) (fdv : list fdstate) (cw : Z) (gn : gname) (cs : gset gname) (pidv : mword 32) :
    uvb C pt Rfd Rut sz π fdv cw gn cs pidv false secc_all M m pc -∗
    ⌜m !!! Regidx (mword_of_int 0) = zero_reg⌝ ∗ uvb C pt Rfd Rut sz π fdv cw gn cs pidv false secc_all M m pc.
  Proof using .
    rewrite /uvb /uvb_F.
    iIntros "(Hamb & Hur & %Hsz & Hpt & Hfrag & Hcfg & Hg & Hpc & Hrut & Hk)".
  ...
  Lemma trapped_of_uv_trap_frame (C : ucfg) (pt : uptd) (Rut : uptd -> iProp Σ)
      ...
      (sz : Z) (π : gmap (mword 27) uperm) (fdv : list fdstate) (cw : Z) (gn : gname) (cs : gset gname) (pidv : mword 32) :
    uk_pt_pure pt sz M Mp ->
    m !!! Regidx (mword_of_int 0) = zero_reg ->
    uv_trap_frame C pt sc stv pc m Mp -∗ Rut pt -∗
    trapped_machine C pt Rut sz sc stv (uvis_of_run m pc M π sz fdv cw gn cs pidv false secc_all).
  Proof using .
...
Section UkObl.
  ...
  Definition uk_step_obl (π : gmap (mword 27) uperm) (Kc : iProp Σ)
    ...
    (∀ (R : iProp Σ) (CIDo : CpuId) (XIo : CtxIdDefs.CurCtx)
       ...
       (R -∗ (TsoCtx.own_context (CID := CIDo) XIo -∗ Rut pt) ∗ Rfd fdv ∗ ukb (CID := CIDo) C pt Rfd Rut sz π fdv cw gn cs pidv false secc_all ∗ uk_paycont Q gn (Kc ∧ ukc π M sz fdv cw gn cs pidv false secc_all m pc)) -∗
       resv_any cpu_id -∗
  ...
  Definition uk_ih (π : gmap (mword 27) uperm) (Kc : iProp Σ)
    ...
    (∀ (h : CpuId) (xi : CtxIdDefs.CurCtx) (C : ucfg) (pt : uptd) (Rfd : list fdstate -> iProp Σ)
       ...
       (* ...AND THE FILL IS EMPTY (lane KILL-PAY, milestone LAZY-ROW).
          ...
          arms. *)
       ⌜lazy_free (ud_um pt) sz⌝ -∗
       uvb (CID := h) (XI := xi) C pt Rfd Rut sz π fdv cw gn cs pidv false secc_all M m pc -∗
  ...
  Definition uk_payload `{CID : CpuId} (sz : Z) (π : gmap (mword 27) uperm)
      (fdv : list fdstate) (cw : Z) (gn : gname) (cs : gset gname) (pidv : mword 32) (Kc : iProp Σ)
      (Q : Z -> iProp Σ)
      (M : gmap Z (bv 8)) (m : regfile) (pc : mword 64)
      (C : ucfg) (pt : uptd) (Rfd : list fdstate -> iProp Σ)
      (Rut : uptd -> iProp Σ) : iProp Σ :=
    (uk_paycont Q gn (Kc ∧ ukc π M sz fdv cw gn cs pidv false secc_all m pc) ∗
     (TsoCtx.own_context CtxIdDefs.cur_ctx -∗ Rut pt) ∗
     Rfd fdv ∗ ukb C pt Rfd Rut sz π fdv cw gn cs pidv false secc_all)%I.
...
Section UkArms.
  ...
  Lemma uk_psi_active (R : iProp Σ) (Rut : uptd -> iProp Σ) (sz : Z)
      ...
      (rs2 : regstate) (fdv : list fdstate) (cw : Z) (gn : gname) (cs : gset gname) (pidv : mword 32) :
    ...
    (R -∗ (TsoCtx.own_context XI -∗ Rut pt) ∗ Rfd fdv ∗ ukb C pt Rfd Rut sz π fdv cw gn cs pidv false secc_all ∗
          (uvb C pt Rfd Rut sz π fdv cw gn cs pidv false secc_all M m' npc -∗ mWP (Loop : expr riscv_lang))) -∗
    uv_psi C R rs2.
  Proof using .
    ...
    iDestruct (uv_land_close C pt Mp m' npc t usatp pcfg paddr User rs2 rs3
      ...
      as "(Hhs & Hpriv & Hms & Hsc & Hstval & Hsepc & Hpc & Hgpr & Hcfg &
           Hutlb & Humem)".
    iApply "Hcont".
    iApply (uvb_intro C pt Rfd Rut sz π M Mp m' npc fdv cw gn cs pidv Hpure
  ...
  Lemma uk_arm_intr (Rut : uptd -> iProp Σ) (sz : Z) (π : gmap (mword 27) uperm)
      ...
      (pcfg : type_of_register pmpcfg_n) (paddr : type_of_register pmpaddr_n)
      (rs1 rsA : regstate) (i : InterruptType) (fdv : list fdstate) (cw : Z) (gn : gname) (cs : gset gname) (pidv : mword 32) :
    uv_pre C pt Mp m pc t rs1 rsA usatp pcfg paddr ->
    uk_pt_pure pt sz M Mp ->
  ...
  Proof using .
    ...
    iFrame "Hrw Hro".
    iApply (uv_psi_trap C pt (uk_payload sz π fdv cw gn cs pidv Kc Q M m pc C pt Rfd Rut) Mp m t usatp pcfg paddr
    ...
    iDestruct ("Hbak" with "Hctx") as "Hrut".
    iApply ("Hkb" $! (uvis_of_run m pc M π sz fdv cw gn cs pidv false secc_all)
              (utrap_scause (Interrupt i) (register_lookup (R_bitvector_64 scause) rsA))
    ...
    iApply (bi.equiv_entails_1_2 _ _
              (uexec_ret_transparent _ (uvis_of_run m pc M π sz fdv cw gn cs pidv false secc_all)
    ...
    iSplitR;
      [ iApply (uexec_pay_dep_ne _ (uvis_of_run m pc M π sz fdv cw gn cs pidv false secc_all) _ (sfam_at Q sfam_pt)
...
Section UkStepEngine.
  ...
  Proof using .
    ...
    iApply (swp_exec_step_full u_Drw u_Dro (u_Df (uc_dqc C)) RS (wrap_pre RS)
              (uv_land RS) (uv_psi C (uk_payload sz π fdv cw gn cs pidv Kc Q M m pc C pt Rfd Rut))
    ...
    - (* ================= THE BODY SLOT ================= *)
      ...
      2: iApply (swp_run_hart_active_res u_Drw u_Dro (u_Df (uc_dqc C))
                   ...
                   (fun (ii : InterruptType) (pr : Privilege) =>
                      uv_step_post C (uk_payload sz π fdv cw gn cs pidv Kc Q M m pc C pt Rfd Rut) RS
...
Section UkFunnel.
  ...
  Lemma wp_uk_step (Kc : iProp Σ) (Q : Z -> iProp Σ) (M : gmap Z (bv 8)) (m : regfile) (pc : mword 64) (fdv : list fdstate) (cw : Z) (gn : gname) (cs : gset gname) (pidv : mword 32) :
    is_aligned_vaddr (Virtaddr pc) 2 = true ->
    uvb C pt Rfd Rut sz π fdv cw gn cs pidv false secc_all M m pc -∗ □ uk_step_obl π Kc Q sz fdv cw gn cs pidv M m pc -∗
...
Section UkPostFetch.
  ...
  Lemma uk_retire_post_fetch (R : iProp Σ) (Rut : uptd -> iProp Σ) (sz : Z)
      ...
      (usatp : mword 64) (pcfg : type_of_register pmpcfg_n)
      (paddr : type_of_register pmpaddr_n) (rs1 rs2 : regstate) (fdv : list fdstate) (cw : Z) (gn : gname) (cs : gset gname) (pidv : mword 32) :
    uv_wrok wr ->
    uv_redirect i o ->
    (forall s_pc : mstate,
    ...
    gen_cert -∗ uv_amb -∗
    (R -∗ (TsoCtx.own_context XI -∗ Rut pt) ∗ Rfd fdv ∗ ukb C pt Rfd Rut sz π fdv cw gn cs pidv false secc_all ∗
          (uvb C pt Rfd Rut sz π fdv cw gn cs pidv false secc_all M (uv_upd m wr) (uv_next jt (add_vec_int pc k)) -∗
  ...
  Proof using .
    ...
    iApply (uk_psi_active C pt Rfd R Rut sz π M Mp (uv_upd m wr)
              ...
              (uv_post_rs rsx jt wr) fdv cw gn cs pidv
...
Section UkObligation.
  ...
  Lemma uk_obl_base (R : iProp Σ) (Rut : uptd -> iProp Σ) (sz : Z)
      ...
      (pcfg : type_of_register pmpcfg_n) (paddr : type_of_register pmpaddr_n)
      (rs1 rsA : regstate) (fdv : list fdstate) (cw : Z) (gn : gname) (cs : gset gname) (pidv : mword 32) :
    uv_pre C pt Mp m pc t rs1 rsA usatp pcfg paddr ->
    uk_pt_pure pt sz M Mp ->
    udecode_base w i ->
    ...
    (R -∗ (TsoCtx.own_context XI -∗ Rut pt) ∗ Rfd fdv ∗ ukb C pt Rfd Rut sz π fdv cw gn cs pidv false secc_all ∗
          (uvb C pt Rfd Rut sz π fdv cw gn cs pidv false secc_all M (uv_upd m wr) (uv_next jt (add_vec_int pc 4)) -∗
           mWP (Loop : expr riscv_lang))) -∗
    resv_any cpu_id -∗
    hreg_frame rsA u_Drw -∗ hreg_frame_ro (u_Df (uc_dqc C)) rsA u_Dro -∗
    TsoCtx.own_context XI -∗
  ...
  Lemma uk_obl_rvc (R : iProp Σ) (Rut : uptd -> iProp Σ) (sz : Z)
    ...
    (R -∗ (TsoCtx.own_context XI -∗ Rut pt) ∗ Rfd fdv ∗ ukb C pt Rfd Rut sz π fdv cw gn cs pidv false secc_all ∗
          (uvb C pt Rfd Rut sz π fdv cw gn cs pidv false secc_all M (uv_upd m wr) (uv_next jt (add_vec_int pc 2)) -∗
           mWP (Loop : expr riscv_lang))) -∗
    resv_any cpu_id -∗
    hreg_frame rsA u_Drw -∗ hreg_frame_ro (u_Df (uc_dqc C)) rsA u_Dro -∗
    TsoCtx.own_context XI -∗
...
Section UkRetire.
  ...
  Proof using HRut Hlf0 Hlo Hpm.
    ...
    iAssert (R -∗ (TsoCtx.own_context (CID := CIDo) XIo -∗ Rut' pt') ∗ Rfd' fdv ∗
             ukb C' pt' Rfd' Rut' sz π fdv cw gn cs pidv false secc_all ∗
             (uvb (CID := CIDo) C' pt' Rfd' Rut' sz π fdv cw gn cs pidv false secc_all M (uv_upd m wr)
    ...
    - iDestruct "Hf" as (h) "[[%HisRVC %Hdecrvc] Hbridge]".
      iApply (uk_obl_rvc C' pt' Rfd' R Rut' sz π M Mp' m pc h i o jt wr t usatp pcfg paddr
                rs1 rsA fdv cw gn cs pidv Hpre Hpure Hdecrvc Hwrok Hred Hg1 Hg2 Hexec
    ...
    - iDestruct "Hf" as (w) "[[%HnRVC %Hdecbase] Hbridge]".
      iApply (uk_obl_base C' pt' Rfd' R Rut' sz π M Mp' m pc w i o jt wr t usatp pcfg paddr
                rs1 rsA fdv cw gn cs pidv Hpre Hpure Hdecbase Hwrok Hred Hg1 Hg2 Hexec
                with "Hcert Hamb Hbridge Hk Hany Hrw Hro Hctx Hmm Hres").
  Qed.

  (* the later-free restatement: the shape every leaf takes *)
  Lemma wp_uk_retire (M : gmap Z (bv 8)) (m : regfile)
      (pc : mword 64) (fdv : list fdstate) (cw : Z) (gn : gname) (cs : gset gname) (pidv : mword 32) (is_rvc : bool) (i : instruction)
      (o : option instruction) (jt : option (mword 64))
      (wr : option (mword 5 * mword 64)) :
    uk_instr π M pc is_rvc i ->
    uv_redirect i o ->
...
Section UkEcallPost.
  ...
  Lemma uk_ecall_post_fetch (R : iProp Σ) (Rut : uptd -> iProp Σ) (sz : Z)
    ...
    uk_pt_pure pt sz M Mp ->
    gen_cert -∗
    (R -∗ (TsoCtx.own_context XI -∗ Rut pt) ∗ Rfd fdv ∗ ukb C pt Rfd Rut sz π fdv cw gn cs pidv false secc_all ∗ uexec_ret uecall_scause (uvis_of_run m pc M π sz fdv cw gn cs pidv false secc_all)) -∗
    resv_any cpu_id -∗
    TsoCtx.own_context XI -∗
    uv_bytes pt Mp t' -∗
    uv_res pt Mp t' usatp pcfg paddr -∗
...
Section UkEcall.
  ...
  Lemma wp_uk_ecall (M : gmap Z (bv 8)) (m : regfile) (pc : mword 64) (fdv : list fdstate) (cw : Z) (gn : gname) (cs : gset gname) (pidv : mword 32) :
    ...
    my_pay gn Qp -∗
    uexec_ret uecall_scause (uvis_of_run m pc M π sz fdv cw gn cs pidv false secc_all) -∗
    mWP (Loop : expr riscv_lang).
  Proof using HRut Hlf0 Hlo Hpm.
    intros Hui Hg.
    pose proof (Hui pt sz (loop_ok_wf C pt Hlo) Hpm) as Hui0.
    ...
    iIntros (R CIDo XIo C' pt' Rfd' Rut' HRut' Mp' t rs1 rsA usatp pcfg paddr)
      "%Hlo' %Hpm' %Hlf' %Hpure %Hpre #Hamb Hk Hany Hrw Hro Hctx Hmm Hres".
    iAssert (R -∗ (TsoCtx.own_context (CID := CIDo) XIo -∗ Rut' pt') ∗ Rfd' fdv ∗
             ukb (CID := CIDo) C' pt' Rfd' Rut' sz π fdv cw gn cs pidv false secc_all ∗
             uexec_ret uecall_scause (uvis_of_run m pc M π sz fdv cw gn cs pidv false secc_all))%I with "[Hk]" as "Hk".
    { iIntros "HR". iDestruct ("Hk" with "HR") as "(Hbak & Hfdr & Hkb & Hkc)".
      iDestruct "Hkc" as "(_ & Hkc)".
    ...
    iApply (uk_ecall_post_fetch C' pt' Rfd' R Rut' sz π M Mp' m pc (zero_extend' 32 w) t' usatp
              pcfg paddr rs1 rs2 fdv cw gn cs pidv Hg
              (T2 _ _ u_in_PC ltac:(vm_compute; reflexivity) LpcA)
              (T2 _ _ u_in_hart ltac:(vm_compute; reflexivity) LhsA)
              Lcp2
              ltac:(rewrite (T2 _ _ u_in_mst ltac:(vm_compute; reflexivity)
