...
Section BioInitAt.
  ...
  (* relaxed-ww (§2.14): FENCE-BOUND like [BioInv.bio_init] -- the lock
     births and the deposits run at a release fence, over the creator's
     flushed token *)
  Lemma bio_init_at `{CID : RiscvLang.CpuId} (bn : bio_names) (V : bio_view Σ) E (Df : nat) :
    (0 ∉ bv_cov V) ->
    ...
    own_context_flushed cur_ctx Df ∗ bio_ctx bn V ∗ bslots BSLOTS_FS.
  Proof.
    ...
    { iApply big_sepL_intro. iIntros "!>" (idx k Hk) "Hrun [[Hf (Hp & Ho & _)] Hrp]".
      iMod (sl_fresh_new_genl_at2 E Df (bn_slk bn k) (buf_lock (bnode k))
              "buffer"%string (bslp bn k) sl_untracked with "Hp Hf Hrun [Ho Hrp]")
        as "(Hrun & Hlk)".
    ...
    (* the bcache lock at its published gname, minted WITH the fold at the
       boot floor slot *)
    iMod (newlock_at_llb E Df (bn_lk bn) bcache_addr "bcache"%string
    ...
    iModIntro. iFrame "Hrun". rewrite /bio_ctx.
