...
Section ProofKfree.
  ...
  Proof.
    ...
    assert (Hprlo : KernelSyms.end_ <= uint p) by (destruct Hpv as [_ [Hlo _]]; exact Hlo).
    assert (Hprhi : uint p < 0x88000000) by (destruct Hpv as [_ [_ Hhi]]; exact Hhi).
    assert (Hsltu14 : zopz0zI_u p (mword_of_int KernelSyms.end_ : mword 64) = false).
    { unfold zopz0zI_u. apply Z.ltb_ge.
      replace (uint (mword_of_int KernelSyms.end_ : mword 64)) with KernelSyms.end_ by (vm_compute; reflexivity).
      lia. }
    ...
    iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.kfree + 0x10)) (mword_of_int 15 : mword 5) (mword_of_int 15 : mword 5) (mword_of_int 0xbc4 : mword 12)
              R3 (K - 4)%nat b ltac:(vm_compute; discriminate) ltac:(rdok)
              with "Hcg Hpc []").
    { iApply (kfi_10 with "Htext"). }
    iIntros (CID8 Hs8) "Hcg Hpc".
    set (R4 := <[Regidx (mword_of_int 15 : mword 5) := regval_into_reg (add_vec (R3 !!! Regidx (mword_of_int 15 : mword 5)) (sign_extend' 64 (mword_of_int 3012 : mword 12)))]> R3).
    iEval (rgne) in "Hcg".
    ...
    assert (Hend : R4 !!! Regidx (mword_of_int 15 : mword 5) = mword_of_int KernelSyms.end_).
    ...
    iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.kfree + 0x3a)) (mword_of_int 18 : mword 5) (mword_of_int 18 : mword 5) (mword_of_int 0x96a : mword 12)
              S1 (K - 4)%nat b ltac:(vm_compute; discriminate) ltac:(rdok)
              with "Hcg Hpc []").
    { iApply (kfi_3a with "Htext"). }
    iIntros (CID23 Hs23) "Hcg Hpc".
    set (S2 := <[Regidx (mword_of_int 18 : mword 5) := regval_into_reg (add_vec (S1 !!! Regidx (mword_of_int 18 : mword 5)) (sign_extend' 64 (mword_of_int 2410 : mword 12)))]> S1).
    iEval (rgne) in "Hcg".
    ...
    iDestruct (cpu_own_transport CID CID25 n eb pcur b ltac:(wp_next_chain)
                 with "Hcnt") as "Hcnt".
    iApply (Acquire.wp_acquire_sconf kt γl "kmem"%string (λ ξ : CtxId, kmem_res (XIk := ξ) γk fl) Kacq
    ...
    iApply (Release.wp_release_sconf kt γl lk "kmem"%string (λ ξ : CtxId, kmem_res (XIk := ξ) γk fl) Rrel
