(* SpecPlicinit.v -- the public interface of plicinit, stated independently of
   ...
   priorities of both UARTs and of the VIRTIO interrupt to 1 (non-zero =
   enabled):
     ...
     <plicinit>:
       ...
       sw   a5,48(a4)       *(PLIC + UART1_IRQ*4)   = 1   (source 12 priority)
       sw   a5,4(a4)        *(PLIC + VIRTIO0_IRQ*4) = 1   (source 1  priority)
       ...epilogue...

   The three stores are S-mode 32-bit MMIO writes through the kernel page
   table's PLIC identity mapping.
   ...
   and borrows the fragment around each of its three writes.
   ...
   [PlicPlan.plic_write_prio_ok] for a source-priority write, at each of the
   three source ids.
...
(* the three PLIC interrupt-source ids xv6 raises to priority 1 --
