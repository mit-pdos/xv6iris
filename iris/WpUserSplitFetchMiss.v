(* WpUserSplitFetchMiss.v -- the U-mode RETIRE engine for a 32-bit F_Base
   instruction fetched at a pc == 2 (mod 4) address, TLB-MISS twin of
   [WpUserSplitFetch.wp_instr_u_split].

   The split (2+2) fetch runs from a COLD (or colliding) TLB slot for [vpn].
   In the NON-STRADDLING case (both halfwords live in the SAME page, so the
   low half at [va] and the high half at [va+2] share the vpn), the low half
   walks the owned page table (empty/colliding hash slot), FILLS the TLB with
   [upt_entry vpn ie], and lands in the filled state [σ'].  The high half at
   [va+2] then HITS that just-filled entry -- a state-preserving translate
   [σ' -> σ'].  So the two abstract halfword translations thread
   [σ -> σ' -> σ'], and [exec_fetch_F_Base_2_U_gen] (which is already
   state-abstract in [s s1 s2]) assembles the whole fetch.

   Exactly like [wp_instr_u_miss], the continuation gets the tlb cell back at
   the FILLED vector, with [upt_inv] re-established.  The decode/execute
   continuation is byte-identical to [wp_instr_u_split]'s, so the retire arms
   ride it unchanged.

   SCOPE: only the non-straddling case (vpnL = vpnH = vpn).  The straddling
   case (va at a page boundary, va+2 in the next page) is a documented gap. *)
...
Section WpUserSplitFetchMiss.
  ...
  (* ------------------------------------------------------------------ *)
  (* Non-straddling split-fetch MISS engine.  Analog of [wp_instr_u_miss] *)
  (* for the 2-aligned (pc == 2 mod 4) split fetch: the low half walks +   *)
  (* fills the TLB for [vpn], the high half (same page) hits the fresh      *)
  (* entry.  Continuation hands back the FILLED tlb + [upt_inv], exactly    *)
  (* as [wp_instr_u_miss].                                                  *)
  (* ------------------------------------------------------------------ *)
  Lemma wp_instr_u_split_miss
  ...
  (* ================================================================== *)
  (* THE COMBINED non-straddling split-fetch ENGINE.  Given only          *)
  (* [spec!!vpn = Some ie] and a [upt_tlb_ok] TLB (which the EMPTY TLB     *)
  (* satisfies), it dispatches on the fetch TLB slot for [vpn]:            *)
  (*   - a matching cached entry -> the HIT engine [wp_instr_u_split]      *)
  (*     (the stored entry is forced to be [upt_entry vpn ie], so the      *)
  (*     filled TLB collapses back to the original via [vec64_update_same]);*)
  (*   - an empty or colliding slot -> the MISS engine                     *)
  (*     [wp_instr_u_split_miss] (walk + fill).                            *)
  (* Both branches present ONE uniform continuation: the FILLED TLB and    *)
  (* [upt_inv].  Mirror of [WpUserComputeMiss.wp_instr_u].                 *)
  (* ================================================================== *)
  Lemma wp_instr_u_split_combined
      (va : mword 64) (vpn : mword 27) (ie : uwalk_info)
      (w : mword 32) (ii : instruction) (ms_v : mword 64)
      (tlbvec : vec (option TLB_Entry) (2 ^ 6))
      E (Φ : mval -> iProp Σ) :
  ...
  Proof.
    ...
    - (* the slot is occupied *)
      ...
      + (* HIT: the stored entry is forced to be [upt_entry vpn ie] *)
        ...
        iApply (wp_instr_u_split va vpn vpn ie ie w ii ms_v tlbvec E Φ HN
      ...
      + (* colliding miss: the walk fills a fresh entry *)
        iApply (wp_instr_u_split_miss va vpn ie w ii ms_v tlbvec E Φ HN Hsome
