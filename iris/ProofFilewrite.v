...
Section ProofFilewrite.
  ...
  Local Lemma fw_env_out_fs (fn' : fwrite_names) (Cf' : fcontent)
      (used' : gset Z) :
    fc_type Cf' = FD_INODE ->
    filewrite_fs_out fn' used' -∗ filewrite_env_out fn' Cf' used'.
  Proof.
  ...
  Proof.
    ...
    iDestruct "Hout" as "(%Hsub & Hsbi & Hsbsz & Hsbb & Hbmres & Hbsl)".
    ...
    iApply (wp_jal_s_sconf (mword_of_int (FW + 0x84)) Rra
              (mword_of_int 2095318 : mword 21) B0 (K - 12)%nat b
              ltac:(vm_compute; discriminate) ltac:(rdok)
    ...
    assert (Htgtbo : add_vec (mword_of_int (FW + 0x84) : mword 64)
              (sign_extend' 64 (mword_of_int 2095318 : mword 21))
   ...
   iDestruct (cpu_own_transport CID0 CIDa1 0%nat eb (proc_addr jx) C b 
    ...
    iDestruct (proc_priv_core_pid (proc_addr jx) pidv (upd_upt V PI) with "Hpriv")
      as "[Hppid Hpbk2]".
    iDestruct (cpu_own_transport CIDbo CIDa3 0%nat eb (proc_addr jx) C b lks
                 ltac:(rewrite Hb; wp_next_chain) with "Hcnt") as "Hcnt".
    ...
    iApply (wp_jal_s_sconf (mword_of_int (FW + 0xa0)) Rra
              (mword_of_int 2093852 : mword 21) Q5 (K - 12)%nat b
              ltac:(vm_compute; discriminate) ltac:(rdok)
    ...
    assert (Htgtwi : add_vec (mword_of_int (FW + 0xa0) : mword 64)
              (sign_extend' 64 (mword_of_int 2093852 : mword 21))
    ...
    iDestruct (cpu_own_transport CIDil CIDa9 0%nat eb (proc_addr jx) C b lks
                 ltac:(rewrite Hb; wp_next_chain) with "Hcnt") as "Hcnt".
    ...
    iDestruct (cpu_own_transport CIDwi CIDb4 0%nat eb (proc_addr jx) C b lks
                 ltac:(rewrite Hb; wp_next_chain) with "Hcnt") as "Hcnt".
    ...
    iApply (wp_jal_s_sconf (mword_of_int (FW + 0xbc)) Rra
              (mword_of_int 2095402 : mword 21) miu (K - 12)%nat b
              ltac:(vm_compute; discriminate) ltac:(rdok)
    ...
    assert (Htgteo : add_vec (mword_of_int (FW + 0xbc) : mword 64)
              (sign_extend' 64 (mword_of_int 2095402 : mword 21))
    ...
    iDestruct (cpu_own_transport CIDiu CIDb5 0%nat eb (proc_addr jx) C b lks
                 ltac:(rewrite Hb; wp_next_chain) with "Hcnt") as "Hcnt".
    ...
    iAssert (filewrite_fs_out fn used')
      with "[Hsbi Hsbsz Hsbb Hbmres Hbsl]" as "Hout".
    { rewrite /filewrite_fs_out.
      iSplitR; [iPureIntro; exact (transitivity Hsub Hsub2) |].
      iFrame "Hsbi Hsbsz Hsbb Hbmres Hbsl". }
    ...
    - (* ============ THE FULL CHUNK: fall to +0xc4 ============ *)
      ...
      + (* ---- EXHAUSTED: i = n, the write completed ---- *)
        ...
        iApply (wp_cj_s_sconf (mword_of_int (FW + 0xe4))
                  ...
                  with "Hcg Hpc Hie4").
        iIntros (CIDc5 Hsc5). iApply bi.later_intro. iIntros "Hcg Hpc".
        ...
        iDestruct (cpu_own_transport CIDeo CIDe 0%nat eb (proc_addr jx) C b lks
                     ltac:(rewrite Hb; wp_next_chain) with "Hcnt") as "Hcnt".
      ...
      + (* ---- NOT EXHAUSTED: the FALL is the back edge to +0xcc ---- *)
        ...
        iDestruct (cpu_own_transport CIDeo CIDc3 0%nat eb (proc_addr jx) C b lks
                     ltac:(rewrite Hb; wp_next_chain) with "Hcnt") as "Hcnt".
    ...
    - (* ============ THE SHORT WRITE (and writei's -1): +0xea ============ *)
      ...
      iApply (wp_bne_taken_s_sconf (mword_of_int (FW + 0xc0))
                ...
                with "Hcg Hpc Hic0").
      iApply bi.later_intro. iIntros (CIDc1 Hsc1) "Hcg Hpc".
          ...
          -- (* ======================= FD_INODE =======================
             ...
             { iIntros (CID13 Hs13) "Hcg Hpc".
               ...
               - (* ---- n <= 0, so n = 0: the loop is never entered ---- *)
                 ...
                 iIntros (CIDe Hse mfin rv) "%Hcsr Hcg Hpc".
                 destruct Hcsr as (Hcsf & Hrv & Hdisj).
                 iDestruct (cpu_own_transport CID CIDe 0%nat eb pj C b lks
                              ltac:(rewrite Hb; wp_next_chain) with "Hcnt") as "Hcnt".
                 iSpecialize ("Hcont" $! CIDe with "[]"); [iPureIntro; wp_next_chain|].
                 assert (HVid : upd_upt V (pv_upt V) = V) by apply fw_upd_upt_id.
               ...
               - (* ---- 0 < n: the five late spills, the two 3072s, and
                 ...
                 iDestruct (cpu_own_transport CID CID28 0%nat eb pj C b lks
                              ltac:(rewrite Hb; wp_next_chain) with "Hcnt") as "Hcnt".
