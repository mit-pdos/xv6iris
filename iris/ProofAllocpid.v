...
Section ProofAllocpid.
  ...
  Proof.
    ...
    iDestruct (cpu_own_transport CID CID8 n eb p b ltac:(wp_next_chain)
                 with "Hcpu") as "Hcpu".
    iApply (Acquire.wp_acquire_sconf kt γp "nextpid"%string nextpid_res A4 n eb p (av - 4)%nat b lks
    ...
    iApply (wp_clw_s_sconf (kt := kt) (ktd := KT0) (mword_of_int (KernelSyms.allocpid + 0x1e)) ai_s1 ai_a5
    ...
    iApply (wp_csw_s_sconf (kt := kt) (ktd := KT0) (mword_of_int (KernelSyms.allocpid + 0x24)) ai_a4 ai_a5
    ...
    iApply (Release.wp_release_sconf kt γp alp_pid_lock "nextpid"%string nextpid_res B7 n eb p (av - 4)%nat
              ({["nextpid"]} ∪ lks)
