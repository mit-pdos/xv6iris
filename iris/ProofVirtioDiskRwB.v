  ...
  Proof.
    ...
    { iLöb as "IH". rewrite /vdrw_p2_loop.
      ...
      iDestruct "Hbody" as "(%Hdfl & %Hdpk & %Hdtr & %Hcoh & %Htok1 & %Htok2 & %Htok3 &
                             Hpub & Hlb & Hdfl & Hcl & Huidx & Hflight & Hparked &
                             Hbun & Hring & Havh)".
      ...
      { (* ============ all three won: hand the seam to P3 ============ *)
        ...
        iApply ("Hexit" $! M1 np nr fl pk tr fr h m2 t with
                  "[%] [%] [%] [%] Hcg Hown Htc Hclm Hpc Htok
                   [Hpub Hlb Hdfl Hcl Huidx Hflight Hparked Hbun Hring Havh] Hbh Hbm Hbt Hidx").
        - split;
        ...
        - exact Hal.
        - rewrite /vdrw_body. iFrame "Hpub Hlb Hdfl Hcl Huidx Hflight Hparked Hbun Hring Havh".
      ...
      iAssert ( ∀ (Mz : regfile),
                  ...
                  vdrw_p2_exit CID γk γs j γd pd pav pu K eb sp0 b wr sector m0 lks -∗
                  WP (Loop : expr riscv_lang))%I
        with "[Hpub Hlb Hdfl Hcl Huidx Hflight Hparked Hring Havh IH]" as "Hsleep".
      { iIntros (Mz) "%Hcsz Hcg Hown Htc Hclm Hpc Htok Hbun Hscr Hexit".
        ...
        iAssert (disk_res γd pd pav pu)
          with "[Hpub Hlb Hdfl Hcl Huidx Hflight Hparked Hbun Hring Havh]" as "HR".
        { iApply (vdrw_body_close γd pd pav pu np nr fl pk tr fr).
          rewrite /vdrw_body.
          iFrame "Hpub Hlb Hdfl Hcl Huidx Hflight Hparked Hbun Hring Havh".
        ...
        iApply (Release.wp_release_sconf KT1 γk d_lock "virtio_disk"%string (disk_res_at γd pd pav pu) C3 0%nat eb (proc_addr j) (K - 12)%nat
                  ({["virtio_disk"]} ∪ lks)
        ...
        iDestruct (cpu_own_transport CIDsl CIDd3 0 eb (proc_addr j) eb
                     ltac:(wp_next_chain) with "Hown") as "Hown".
        iApply (Acquire.wp_acquire_sconf KT1 γk "virtio_disk"%string (disk_res_at γd pd pav pu) D3 0%nat eb (proc_addr j) (K - 12)%nat eb lks
