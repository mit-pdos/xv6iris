  ...
  Proof.
    ...
    { iLöb as "IH". rewrite /vdrw_p2_loop.
      ...
      { iIntros (Mz) "%Hcsz Hcg Hown Htc Hclm Hpc Htok Hbun Hscr Hexit".
        ...
        iApply (Release.wp_release_sconf KT1 γk d_lock "virtio_disk"%string (λ ξ : CtxId, disk_res (XI := ξ) γd pd pav pu) C3 0%nat eb (proc_addr j) (K - 12)%nat
                  ({["virtio_disk"]} ∪ lks)
        ...
        iDestruct (cpu_own_transport CIDsl CIDd3 0 eb (proc_addr j) eb
                     ltac:(wp_next_chain) with "Hown") as "Hown".
        iApply (Acquire.wp_acquire_sconf KT1 γk "virtio_disk"%string (λ ξ : CtxId, disk_res (XI := ξ) γd pd pav pu) D3 0%nat eb (proc_addr j) (K - 12)%nat eb lks
