...
Section ProofSysKill.
  ...
  Proof.
    ...
    (* +0x16 jal ra,kkill *)
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.sys_kill + 0x16)) Rra (mword_of_int 2094610 : mword 21)
              B1 (av - 4)%nat b
              ltac:(vm_compute; discriminate) ltac:(rdok) ltac:(vm_compute; reflexivity)
              with "Hcg Hpc []").
    ...
    assert (Hjkk : add_vec (mword_of_int (KernelSyms.sys_kill + 0x16) : mword 64)
                     (sign_extend' 64 (mword_of_int 2094610 : mword 21)) = mword_of_int KernelSyms.kkill)
      by (apply bv_eq; vm_compute; reflexivity).
