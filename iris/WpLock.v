...
Require Import TsoCtxLedger.  (* [ctx_stamp] is FENCE-BOUND under two logs (relaxed-ww.md §2.3/§2.4) *)
...
Section Lock.
  ...
  (* THE LOCK'S CONTEXT, PARKED UNDER THE HOLDER
     ...
     ([lock_pay_born]), STAMPED in the invariant's free arm while the lock is
     free ([lock_pay]), RESUMED by the winner at acquire -- which moves the
     payload out of it and parks it under its own context ([lock_pay_take])
     -- and resumed again at release, which moves the payload back and
     stamps it ([lock_pay_intro]).  So the lock path uses exactly the moves
     a thread uses at swtch and no deposit into, or absorb out of, a stamped
     root.  The parked token rides here, in the holder's [locked]; its
     identity is existential because nothing outside the lock kit ever names
     it. *)
  Definition lock_ctx_held : iProp Σ := (∃ ξL : CtxId, ctx_parked ξL cur_ctx)%I.
  Definition locked (γ : gname) (i : CPU) : iProp Σ :=
    (locked_core γ i ∗ lock_ctx_held)%I.
  (* the same, in the window where [lk->cpu] is still 0 (internal to the
     ...
     is minted, so the two arms of the held interval agree.  The lock's
     context is NOT in it: acquire parks the context after the AMO and hands
  ...
  Proof. apply _. Qed.
  Global Instance lock_ctx_held_timeless : Timeless lock_ctx_held.
  Proof. rewrite /lock_ctx_held. apply _. Qed.
  ...
  Lemma locked_split γ i : locked γ i ⊣⊢ locked_core γ i ∗ lock_ctx_held.
  ...
  Proof.
    iIntros "[H1 _] [H2 _]". iApply (locked_core_exclusive with "H1 H2").
  Qed.
  ...
  Lemma lkcpu_read_not_mine `{CID : CpuId} (g : gstate) (lk : mword 64)
    ...
    TsoCtx.ledger_anchor (hart_agent cpu_id) lo t -∗
    ...
    ⌜forall (tv : nat), (g.(gtv) cpu_id <= tv)%nat -> forall w : mword 64,
       tso_read_bytes g.(gimg) g.(glog) g.(gdlog) (hart_agent cpu_id) tv (lock_cpu lk)
  ...
  (* >>> A6.115 (approved on A6.114's pricing): THE CELL'S PER-AGENT ANCHOR.
     ...
     collapse into one. <<< *)
  (* TWO LOGS: the anchor is [TsoCtx.ledger_anchor] -- the floor itself or
     the agent's own message -- since a timestamp is never compared with a
     position (relaxed-ww.md §2.7). *)
  Definition lk_own_anchored (lo : nat) (own : agent -> option nat) : iProp Σ :=
    (∀ (h : agent) (t : nat),
       ⌜own h = Some t⌝ -∗ TsoCtx.ledger_anchor h lo t)%I.
  ...
  Proof.
    ...
    iApply TsoCtx.ledger_anchor_floor.
  Qed.
  ...
  (* TWO LOGS (relaxed-ww.md §2.8): the floor's left arm is a DRAIN
     witness -- the floor message drained under ξ's bound -- and the right
     arm is ξ's own write: exactly [TsoCtx.key_at] at the floor message,
     the justification every ctx fact carries.  The byte is existential;
     the clean arm does not depend on it. *)
  Definition lk_floor (ξ : TsoCtx.CtxId) (lo : nat) : iProp Σ :=
    (∃ a : Arch.pa, TsoCtx.key_at ξ (lo, a))%I.

  Global Instance lk_floor_persistent ξ lo : Persistent (lk_floor ξ lo).
  Proof. apply _. Qed.

  Lemma lk_floor_0 ξ : ⊢ lk_floor ξ 0.
  Proof. iExists inhabitant. iApply TsoCtx.key_at_0. Qed.

  Lemma lk_floor_of_key ξ lo (a : Arch.pa) : TsoCtx.key_at ξ (lo, a) -∗ lk_floor ξ lo.
  Proof. iIntros "#H". by iExists a. Qed.

  (* the clean arm: the floor message is drained at [p], under ξ's bound *)
  Lemma lk_floor_of_dpos ξ lo (p : nat) :
    TsoGhost.dpos_ev dpos_name lo p -∗ TsoCtx.ctx_floor ξ p -∗ lk_floor ξ lo.
  Proof.
    iIntros "#Hdp #Hfl". iExists inhabitant. rewrite /TsoCtx.key_at. iLeft.
    iExists p. by iFrame "Hdp Hfl".
  Qed.

  Lemma lk_floor_of_wrote ξ lo (a : Arch.pa) :
    TsoCtx.ctx_wrote ξ lo a -∗ lk_floor ξ lo.
  Proof. iIntros "#Hw". iExists a. rewrite /TsoCtx.key_at. iRight. iExact "Hw". Qed.
  ...
  Proof.
    iIntros (ξ ξ') "Hd (%a & #Hk)". rewrite /TsoCtx.key_at.
    iDestruct "Hk" as "[(%p & #Hdp & #Hfl) | #Hw]".
    - iDestruct (TsoCtx.ctx_floor_dom with "Hd Hfl") as "[Hd #Hfl']".
      iModIntro. iFrame "Hd". iExists a. iLeft. iExists p. by iFrame "Hdp Hfl'".
    - iDestruct (TsoCtx.ctx_dom_wrote_floor with "Hd Hw") as "[Hd #Hk']".
      iModIntro. iFrame "Hd". iExists a. iExact "Hk'".
  Qed.
  ...
  Lemma lk_floor_vis `{CID : CpuId} (ξ : TsoCtx.CtxId) (lo : nat) :
    ...
    TsoCtx.own_context ξ ∗ ∃ K : nat,
      ...
      TsoCtx.ledger_vis (hart_agent cpu_id) K lo.
  Proof.
    iIntros "Hrun (%a & #Hk)". rewrite /TsoCtx.key_at.
    iDestruct "Hk" as "[(%p & #Hdp & #Hfl) | #Hw]".
    - iDestruct (TsoCtx.own_context_floor_view with "Hrun Hfl")
        as "[Hrun (%K & #HK & %HpK)]".
      iFrame "Hrun". iExists K. iFrame "HK". iApply TsoCtx.ledger_vis_ev.
      iApply (TsoGhost.dpos_ev_mono _ _ _ _ HpK with "Hdp").
  ...
  (* the same reclamation [lk_cpu_fresh_free] does for the owner cell: a
     ...
     relaxed-ww STAGE E: the free tier is ξ-indexed (relaxed-ww.md §2.10),
     so the bytes go home as the destroyer's OWN ctx bytes, which needs the
  ...
  (* A6.120: THE WINNER'S FORM of the record -- the same record, plus the
     ...
     [hart_view_lb K ∗ ⌜T ≤ K⌝] by [TsoCtx.own_context_floor_view]); it is
     what lets the winner resume the lock's context ([lock_pay_take]). *)
  Definition lock_pay_won (R : CtxId -> iProp Σ) : iProp Σ :=
    (∃ (ξ : CtxId) (T : nat),
       ctx_stamped ξ T ∗ TsoCtx.ctx_floor cur_ctx T ∗ R ξ)%I.

  (* THE HOOK: how a releaser FINISHES its payload at the lock's stamped
     context.  Given the record just stamped and the payload it moved in,
     the hook may raise the stamp and returns the payload in the shape the
     invariant states.  The identity is the ordinary release; the floor fold
     ([lock_hook_llb]) is the other instance, and it is why a hook exists:
     a payload row [ctx_floor ξ tl] above the releaser's view can be minted
     only on a hartless record, so the one moment it can be minted is here,
     between the stamp and the store.  The next winner cashes the row
     against its own running token ([TsoCtx.own_context_floor_view]) after
     [lock_pay_take] has moved the payload to it. *)
  (* TWO LOGS (relaxed-ww.md §2.14): the hook runs AT THE RELEASE FENCE,
     where the stamp it depends on is produced.  What it sees of the fence
     is the FLUSHED TOKEN [own_context_flushed cur_ctx Df] -- the
     releaser's running token with every dirty key drained under [Df] --
     and nothing of the machine: no [gstate], no [own_drained], no interp.
     The hook hands the token back flushed (a client with more to deposit
     at the same fence deposits again) and EXPORTS [Q]: what the deposit
     tells the continuation (the boxes' chains, §2.6); the identity hook
     exports [emp]. *)
  Definition lock_ctx_hook `{CID : CpuId} (E : coPset) (R Rin : CtxId -> iProp Σ)
      (Q : iProp Σ) : iProp Σ :=
    (∀ (ξ : CtxId) (T Df : nat),
       own_context_flushed cur_ctx Df -∗ ctx_stamped ξ T -∗ Rin ξ ={E}=∗
       own_context_flushed cur_ctx Df ∗
       ∃ T' : nat, ctx_stamped ξ T' ∗ R ξ ∗ Q)%I.

  Lemma lock_hook_id `{CID : CpuId} (E : coPset) (R : CtxId -> iProp Σ) :
    ⊢ lock_ctx_hook E R R emp.
  Proof.
    rewrite /lock_ctx_hook. iIntros (ξ T Df) "Hrun Hs HR". iModIntro.
    iFrame "Hrun". iExists T. iFrame.
  Qed.

  (* the floor fold: the record's stamp rises to the receipt's position for
     free ([TsoCtx.ctx_stamped_raise] -- a stamped context has no hart) and
     the floor it hands back completes the row *)
  Lemma lock_hook_llb `{CID : CpuId} (E : coPset) (Rin R : CtxId -> iProp Σ) (tl : nat) :
    (forall ξ : CtxId, Rin ξ ∗ TsoCtx.ctx_floor ξ tl ⊢ R ξ) ->
    TsoGhost.llb dlen_name tl -∗ lock_ctx_hook E R Rin emp.
  Proof.
    iIntros (Hfold) "#Hllb". rewrite /lock_ctx_hook.
    iIntros (ξ T Df) "Hrun Hs HR".
    iMod (TsoCtx.ctx_stamped_raise ξ T tl with "Hllb Hs") as "[Hs #Hfl]".
    iModIntro. iFrame "Hrun". iExists (Nat.max T tl). rewrite bi.sep_emp. iFrame "Hs".
    iApply Hfold. iFrame "HR Hfl".
  Qed.

  (* THE CREATOR'S MINT: the lock's context is born as a RUNNING twin of the
     creator ([TsoCtx.own_context_twin_flushed]), takes the payload by the
     same-hart move, and is stamped.  [own_context] is CpuId-indexed and this
     section binds no [CpuId], so the creator's hart is a parameter -- the
     token the caller hands over is at ITS hart, not at some ambient one. *)
  (* TWO LOGS (relaxed-ww.md §2.4, "Birth"; §2.14): the record is the
     creator's pending stores and has no position, so its stamp --
     publication -- is FENCE-BOUND: this runs with the creator's FLUSHED
     token in hand, i.e. at a release fence on the creator's hart.  Which
     fence, and how the born record travels to it, is RULING C (the
     owner's; (c1) a born token, (c2) a born free arm).  The statement
     below is the common core both rulings share. *)
  Lemma lock_pay_born `{CID : CpuId} (E : coPset) (Df : nat)
      (Rin R : CtxId -> iProp Σ) (Q : iProp Σ) `{!CtxMorph Rin} :
    own_context_flushed cur_ctx Df -∗ Rin cur_ctx -∗ lock_ctx_hook E R Rin Q ={E}=∗
    own_context_flushed cur_ctx Df ∗ lock_pay R ∗ Q.
  Proof.
    iIntros "Hrun HR Hhook".
    iMod (own_context_twin_flushed cur_ctx Df with "Hrun") as "[Hrun (%ξL & HξL)]".
    iMod (ctx_move_flushed (R := Rin) cur_ctx ξL Df with "Hrun HξL HR") as "(Hrun & HξL & HR)".
    iMod (ctx_stamp_flushed ξL Df with "HξL") as "Hst".
    iMod ("Hhook" with "Hrun Hst HR") as "(Hrun & %T' & Hst & HR & HQ)".
    iModIntro. iFrame "Hrun HQ". iExists ξL, T'. iFrame "Hst HR".
  Qed.

  Lemma lock_pay_born_id `{CID : CpuId} (E : coPset) (Df : nat)
      (R : CtxId -> iProp Σ) `{!CtxMorph R} :
    own_context_flushed cur_ctx Df -∗ R cur_ctx ={E}=∗
    own_context_flushed cur_ctx Df ∗ lock_pay R.
  Proof.
    iIntros "Hrun HR".
    iMod (lock_pay_born E Df R R emp with "Hrun HR []") as "(Hrun & HR & _)";
      [ iApply lock_hook_id | ].
    iModIntro. iFrame.
  Qed.

  (* THE RELEASER'S MOVE: resume the lock's context from under the holder,
     move the payload into it, stamp it, run the hook.  What comes out is
     the free arm's record, and the hook's export. *)
  (* TWO LOGS: the stamp is publication at release's fence, so the whole
     move runs over the releaser's flushed token; the resumed lock context
     inherits the flush ([TsoCtx.ctx_resume_flushed]) and the same-hart
     move keeps it ([TsoCtx.ctx_move_flushed]). *)
  Lemma lock_pay_intro `{CID : CpuId} (E : coPset) (Df : nat)
      (Rin R : CtxId -> iProp Σ) (Q : iProp Σ) `{!CtxMorph Rin} :
    own_context_flushed cur_ctx Df -∗ lock_ctx_held -∗ Rin cur_ctx -∗
    lock_ctx_hook E R Rin Q ={E}=∗
    own_context_flushed cur_ctx Df ∗ lock_pay R ∗ Q.
  Proof.
    iIntros "Hrun (%ξL & Hpk) HR Hhook".
    iMod (ctx_resume_flushed ξL cur_ctx Df with "Hrun Hpk") as "[Hrun HξL]".
    iMod (ctx_move_flushed (R := Rin) cur_ctx ξL Df with "Hrun HξL HR") as "(Hrun & HξL & HR)".
    iMod (ctx_stamp_flushed ξL Df with "HξL") as "Hst".
    iMod ("Hhook" with "Hrun Hst HR") as "(Hrun & %T' & Hst & HR & HQ)".
    iModIntro. iFrame "Hrun HQ". iExists ξL, T'. iFrame "Hst HR".
  ...
  (* THE WINNER'S MOVE: the AMO leaf hands the record with the winner's
     floor at its stamp ([lock_pay_won]); the running token cashes the floor
     into the view receipt ([TsoCtx.own_context_floor_view]), the lock's
     context resumes on this hart ([TsoCtx.ctx_unstamp]), the payload moves
     to the winner ([TsoCtx.ctx_move]), and the emptied lock context parks
     under the winner -- the token [locked] carries until release. *)
  Lemma lock_pay_take `{CID : CpuId} (R : CtxId -> iProp Σ) `{!CtxMorph R} :
    own_context cur_ctx -∗ lock_pay_won R ==∗
    own_context cur_ctx ∗ R cur_ctx ∗ lock_ctx_held.
  Proof.
    iIntros "Hrun (%ξL & %T & Hst & #Hfl & HR)".
    iDestruct (TsoCtx.own_context_floor_view cur_ctx T with "Hrun Hfl")
      as "[Hrun (%K & #HK & %HTK)]".
    iAssert (hart_view_lb K) as "#HK'".
    { iEval (rewrite TsoCtx.hart_view_lb_unseal /TsoCtx.hart_view_lb_def). iExact "HK". }
    iMod (ctx_unstamp ξL T K HTK with "HK' Hst") as "HξL".
    iMod (ctx_move (R := R) ξL cur_ctx with "HξL Hrun HR") as "(HξL & Hrun & HR)".
    iMod (ctx_park ξL cur_ctx with "Hrun HξL") as "[Hrun Hpk]".
    iModIntro. iFrame "Hrun HR". iExists ξL. iFrame "Hpk".
  Qed.
  ...
  (* the boot-static producer: floor 0 is free, so every lock whose owner
     cell is an era-image cell has an opener for nothing (A6.101).  (The
     absorbed opener [lock_openable_c] of A6.112 is gone: retired in place
     by A6.120 with no producer and no consumer, and its floor conjunct
     named an issue position.) *)
  Lemma lock_openable_inv_0 γ lk s R :
    inv lockN (lock_inv γ lk s R 0) -∗ lock_openable γ lk s R False.
  Proof.
    iIntros "#Hi". iExists 0%nat. iSplitR; [ iApply lk_floor_0 | ].
    iIntros "!>" (E T HE) "_ HT".
  ...
  (* THE FINISHER IS TWO-PART -- a PRELUDE and a BODY at the store -- because
     its two arms want the payload in two different shapes:
       close   : the invariant's free arm is the lock's stamped context with
                 the payload in it, so the payload must be MOVED there and
                 the context stamped ([lock_pay_intro]), which needs the
                 running token -- borrowable from [sie_cap] in straight-line
                 code (SieCapCtx) and NOT at the store, where the step engine
                 holds it inside its atomic update -- and the lock's parked
                 context, which the cpu clear hands back out of [locked];
       destroy : the caller's completion wand speaks at [cur_ctx]; the lock's
                 context dies with the lock.
     So the finisher CHOOSES the shape [Pay]: the prelude produces it with the
     token and the parked context in hand, the body consumes it at the store.
     [lock_finisher_body] is what the word-clear leaf takes; nothing outside
     this file looks inside a finisher. *)
  ...
  (* the general finisher: the caller has closed over its payload, so the
     prelude takes only the fence's flushed token and the lock's parked
     context.  This is what the generic release is proved against. *)
  (* TWO LOGS (relaxed-ww.md §2.14): the prelude runs AT release's
     `fence rw,w`, over the flushed token the fence leaf produces -- the
     lifting ([WpSconfFencePub.wp_fence_rel_flush_s_sconf]) does the flush;
     nothing here sees the machine. *)
  Definition lock_finisher_pay `{CID : CpuId} (γ : gname) (lk : mword 64)
      (s : string) (R : CtxId → iProp Σ) (D Out : iProp Σ)
      (E : coPset) : iProp Σ :=
    (∃ Pay : iProp Σ,
       (∀ Df : nat,
          own_context_flushed cur_ctx Df -∗ lock_ctx_held ={E}=∗
          own_context_flushed cur_ctx Df ∗ Pay) ∗
       lock_finisher_body γ lk s R D Out E Pay)%I.

  (* the finisher with the payload still to be handed in, at the caller's own
     context -- the shape the generic release's consumers name *)
  Definition lock_finisher `{CID : CpuId} (γ : gname) (lk : mword 64)
      (s : string) (R : CtxId → iProp Σ) (D Out : iProp Σ)
      (E : coPset) : iProp Σ :=
    (∃ Pay : iProp Σ,
       (∀ Df : nat,
          own_context_flushed cur_ctx Df -∗ lock_ctx_held -∗ R cur_ctx ={E}=∗
          own_context_flushed cur_ctx Df ∗ Pay) ∗
       lock_finisher_body γ lk s R D Out E Pay)%I.

  Lemma lock_finisher_to_pay `{CID : CpuId} γ lk s R D Out E :
    R cur_ctx -∗ lock_finisher γ lk s R D Out E -∗ lock_finisher_pay γ lk s R D Out E.
  Proof.
    iIntros "HR (%Pay & Hpre & Hbody)". iExists Pay. iFrame "Hbody".
    iIntros (Df) "Hrun Hheld". iApply ("Hpre" with "Hrun Hheld HR").
  Qed.

  (* the closing body: the free arm is rebuilt from the pieces and the record *)
  ...
  (* the same, with the hook's export riding beside the record and leaving
     as the leaf's output *)
  Lemma lock_finisher_close_body_q γ lk s R D (Q : iProp Σ) E :
    ⊢ lock_finisher_body γ lk s R D Q E (lock_pay R ∗ Q).
  Proof.
    iIntros (lo) "[Hclose _] Hauth Hfrag [#Hc4 Hword] [#Hc8 Hcpu] _ [HR HQ]".
    iDestruct "Hauth" as (B) "Hauth".
    iMod ("Hclose" with "[Hauth Hfrag Hword Hcpu HR]") as "_"; [| iModIntro; iExact "HQ"].
    iNext. rewrite /lock_inv /lock_body. iFrame "Hc4 Hc8".
    iExists (mword_of_int 0 : mword 32), None, B.
    rewrite lk_cpu_res_free. iFrame "Hword Hcpu Hauth".
    iLeft. by iFrame "Hfrag HR".
  ...
  (* put it back, with the payload finished by a hook at the lock's stamped
     context: the ordinary release at the identity hook, the floor fold at
     [lock_hook_llb], a box deposit at its own hook.  The hook's export [Q]
     is the leaf's output.  [D] is not used, only not taken. *)
  Lemma lock_finisher_close_hook `{CID : CpuId} γ lk s (Rin R : CtxId → iProp Σ)
      (Q : iProp Σ) `{!CtxMorph Rin} D E :
    Rin cur_ctx -∗ lock_ctx_hook E R Rin Q -∗ lock_finisher_pay γ lk s R D Q E.
  Proof.
    iIntros "HR Hhook". iExists (lock_pay R ∗ Q)%I. iSplitL.
    { iIntros (Df) "Hrun Hheld".
      iMod (lock_pay_intro E Df Rin R Q with "Hrun Hheld HR Hhook") as "(Hrun & HR & HQ)".
      iModIntro. iFrame. }
    iApply lock_finisher_close_body_q.
  Qed.

  (* put it back: today's release, and equally the release of an object that
     merely still has other holders. *)
  Lemma lock_finisher_close `{CID : CpuId} γ lk s R `{!CtxMorph R} D E :
    ⊢ lock_finisher γ lk s R D emp E.
  Proof.
    iExists (lock_pay R). iSplitR.
    { iIntros (Df) "Hrun Hheld HR".
      iMod (lock_pay_intro E Df R R emp with "Hrun Hheld HR []") as "(Hrun & HR & _)";
        [ iApply lock_hook_id | ].
      iModIntro. iFrame. }
  ...
  (* destroy it and keep the storage.  The certificate [D] is assembled HERE,
     ...
     context, which is the context its completion wand speaks at, and the
     lock's context is dropped with the lock. *)
  ...
  Proof.
    ...
    { iIntros (Df) "Hrun _ HR". iModIntro. iFrame "Hrun HR". }
  ...
  (* the free arm is the lock's stamped context with the payload in it, so
     the creator mints that context ([lock_pay_born]).  The token is taken
     and handed straight back. *)
  (* TWO LOGS (relaxed-ww.md §2.14): every construction below runs at a
     FENCE on the creator's hart (the record's publication, [lock_pay_born])
     and sees of it only the creator's FLUSHED token; how the creator gets
     there is RULING C's. *)
  Lemma lock_inv_alloc `{CID : CpuId} (E : coPset) (Df : nat) (lo : nat)
      (lk : mword 64) (s : string) (R : CtxId → iProp Σ) `{!CtxMorph R} :
    own_context_flushed cur_ctx Df -∗
    lk ↦₄ (mword_of_int 0 : mword 32) -∗
    lk_cpu_fresh lo lk -∗
    R cur_ctx ={E}=∗
    own_context_flushed cur_ctx Df ∗ ∃ γ : gname, lock_inv γ lk s R lo.
  Proof.
  ...
  Lemma newlock_d `{CID : CpuId} E (lo : nat) (lk : mword 64) (s : string) :
    lk ↦₄ (mword_of_int 0 : mword 32) -∗
    lk_cpu_fresh lo lk ==∗
    ∃ γ : gname, ∀ (Df : nat) (R : CtxId → iProp Σ) (D : iProp Σ),
      ⌜CtxMorph R⌝ -∗ own_context_flushed cur_ctx Df -∗
      R cur_ctx ={E}=∗
      own_context_flushed cur_ctx Df ∗ inv lockN (lock_inv γ lk s R lo ∨ D).
  Proof.
    ...
    iMod (own_alloc ((((●E (None : leibnizO lock_state)),
      ...
      [ split; apply excl_auth_valid | ].
    iDestruct (own_op with "H") as "[Ha Hf]".
    iModIntro. iExists γ. iIntros (Df R D) "%HmR Hrun HR".
    iMod (lock_pay_born_id (CtxMorph0 := HmR) E Df R with "Hrun HR") as "(Hrun & HR)".
    iFrame "Hrun".
  ...
  Lemma newlock `{CID : CpuId} E (Df : nat) (lk : mword 64) (s : string)
      (R : CtxId → iProp Σ) `{!CtxMorph R} :
    lock_name lk s -∗
    own_context_flushed cur_ctx Df -∗
    lk ↦₄ (mword_of_int 0 : mword 32) -∗
    lk_cpu_ready lk -∗
    R cur_ctx ={E}=∗
    own_context_flushed cur_ctx Df ∗ ∃ γ : gname, is_lock γ lk s R.
  Proof.
    iIntros "#Hnm Hrun Hword Hready HR".
    rewrite /lk_cpu_ready /lk_cpu_ready_at.
    iDestruct "Hready" as (lo) "[Hcpu #Hfl]".
    iMod (lock_inv_alloc E Df lo lk s R with "Hrun Hword Hcpu HR") as "(Hrun & Hbody)".
  ...
  Lemma newlock_delayed `{CID : CpuId} E (lk : mword 64) (s : string) :
    ...
    lk ↦₄ (mword_of_int 0 : mword 32) -∗
    lk_cpu_ready lk ==∗
    ∃ γ : gname, ∀ (Df : nat) (R : CtxId → iProp Σ),
      ⌜CtxMorph R⌝ -∗ own_context_flushed cur_ctx Df -∗
  ...
  Proof.
    ...
    iMod (own_alloc ((((●E (None : leibnizO lock_state)),
      ...
      [ split; apply excl_auth_valid | ].
    iDestruct (own_op with "H") as "[Ha Hf]".
    iModIntro. iExists γ. iIntros (Df R) "%HmR Hrun HR".
    iMod (lock_pay_born_id (CtxMorph0 := HmR) E Df R with "Hrun HR") as "(Hrun & HR)".
    iFrame "Hrun".
  ...
  (* BOX v2 boot (endgame §3.6): the creator cannot floor its own stores,
     so the lock is minted WITH the fold ([lock_hook_llb]): the payload row
     [Rdep] at [cur_ctx] plus [llb tl] and the one-line entailment give the
     floored [R]. *)
  Lemma newlock_delayed_llb `{CID : CpuId} E (lk : mword 64) (s : string) :
    lock_name lk s -∗
    lk ↦₄ (mword_of_int 0 : mword 32) -∗
    lk_cpu_ready lk ==∗
    ∃ γ : gname, ∀ (Df : nat) (R Rdep : CtxId → iProp Σ) (tl : nat),
      ⌜CtxMorph Rdep⌝ -∗
      ⌜forall ξ : CtxId, Rdep ξ ∗ TsoCtx.ctx_floor ξ tl ⊢ R ξ⌝ -∗
      TsoGhost.llb dlen_name tl -∗ own_context_flushed cur_ctx Df -∗
  ...
  Proof.
    ...
    iMod (own_alloc ((((●E (None : leibnizO lock_state)),
      ...
      [ split; apply excl_auth_valid | ].
    iDestruct (own_op with "H") as "[Ha Hf]".
    iModIntro. iExists γ. iIntros (Df R Rdep tl) "%HmR %Hfold #Hllb Hrun HR".
    iMod (lock_pay_born (CtxMorph0 := HmR) E Df Rdep R emp with "Hrun HR [Hllb]") as "(Hrun & HR & _)".
