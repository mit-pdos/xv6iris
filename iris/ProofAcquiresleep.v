(* ProofAcquiresleep.v -- acquiresleep() over the SIE-agnostic sconf world.
   ...
   THE HART-GENERIC PROTOCOL (claude-notes/projects/sched-hart-generic.md).
   sleep() PARKS: it does not return on the hart it was called from, so
   everything after it -- the +0x24 reload, the back edge, the whole exit
   path -- runs at whichever hart's scheduler dispatched this thread.  Because
   the park is INSIDE the retry loop, it is the LOOP INVARIANT that has to be
   hart-generic, not merely a trailing half:

     * [asl_regs] takes the hart it is about; its tp conjunct reads
       [cid_word_of h] and its preserved set drops tp ([callee_saved_notp]).
     * [asl_loop] and [asl_exit] are CID-FREE propositions quantified over
       [∀ (h : CPU) (g : gname) (M : regfile)] -- the loop-carried register /
       cpu_own / trap-CSR / pc tier all live at (h,g).  That is what makes the
       iLöb IH re-enterable at the RESUMING hart: [iLöb] is taken on the
       ∀h-quantified [asl_loop], so the IH is available at every hart.
     * a Section-fixed [CID] cannot be instantiated from inside its own
       section, so each straight-line stretch is its OWN lemma with [CID] a
       BINDER -- [asl_exit_body] (+0x28..ret), [asl_post_sleep_body]
       (+0x24..the branch) and [asl_loop_body] (+0x1c..the sleep call), each
       applied once at [(CID := h)].  The proposition-level [asl_loop] /
       [asl_exit] are what cross between them.
     * the trap CSRs ride the loop: acquire(&slk->lk) mints
       [trap_csrs_pay 0 eb] at the entry hart, sleep carries it across the
       park and hands it back at the resuming hart, and the final
       release(&slk->lk) spends it there.  acquiresleep is therefore
       trap-CSR-balanced and its own contract does not mention them.
     * [panic_wp_any] is what acquire and sleep now take; acquiresleep has no
...
(* ===================================================================== *)
(*  The loop/exit register-map invariant, AT THE HART IT IS ABOUT.        *)
(*                                                                        *)
(*  s1 = slk, s2 = sl_lk slk, sp = the pushed frame base, tp = hart [h]'s  *)
(*  id, and s3..s11 preserved from the entry map [m].  tp is the one       *)
(*  conjunct the park changes, so it names [h] rather than [m]'s tp, and    *)
(*  the preserved set is threaded with [callee_saved_notp] across a park.   *)
(* ===================================================================== *)
Definition asl_regs (h : CPU) (m M : regfile) (slk spd : mword 64) : Prop :=
  M !!! Regidx (mword_of_int 9 : mword 5) = slk /\
  M !!! Regidx (mword_of_int 18 : mword 5) = sl_lk slk /\
  M !!! Regidx csp_rs1 = spd /\
  M !!! Regidx (mword_of_int 4 : mword 5) = cid_word_of h /\
...
(* a same-hart hop (a leaf write, or a non-parking callee) *)
Lemma asl_regs_cs (h : CPU) (m M1 M2 : regfile) (slk spd : mword 64) :
  callee_saved M1 M2 -> asl_regs h m M1 slk spd -> asl_regs h m M2 slk spd.
Proof.
...
(* THE PARKING HOP: the hart genuinely changes, so the invariant moves to the
   new hart and the preserved set is the tp-free one -- exactly what
   sleep()'s postcondition hands over. *)
Lemma asl_regs_notp (h h' : CPU) (m M1 M2 : regfile) (slk spd : mword 64) :
  callee_saved_notp M1 M2 ->
  M2 !!! Regidx (mword_of_int 4 : mword 5) = cid_word_of h' ->
  asl_regs h m M1 slk spd -> asl_regs h' m M2 slk spd.
Proof.
  intros Hcs Htp Ha. unfold asl_regs in *.
  destruct Ha as (A&B&C&_&E&F&G&H&I&J&K&L&N).
  repeat split.
  - rewrite (callee_saved_notp_lookup Hcs (mword_of_int 9 : mword 5) ltac:(vm_compute; reflexivity)). exact A.
  - rewrite (callee_saved_notp_lookup Hcs (mword_of_int 18 : mword 5) ltac:(vm_compute; reflexivity)). exact B.
  - rewrite (callee_saved_notp_lookup Hcs csp_rs1 ltac:(vm_compute; reflexivity)). exact C.
  - exact Htp.
  - rewrite (callee_saved_notp_lookup Hcs (mword_of_int 19 : mword 5) ltac:(vm_compute; reflexivity)). exact E.
  - rewrite (callee_saved_notp_lookup Hcs (mword_of_int 20 : mword 5) ltac:(vm_compute; reflexivity)). exact F.
  - rewrite (callee_saved_notp_lookup Hcs (mword_of_int 21 : mword 5) ltac:(vm_compute; reflexivity)). exact G.
  - rewrite (callee_saved_notp_lookup Hcs (mword_of_int 22 : mword 5) ltac:(vm_compute; reflexivity)). exact H.
  - rewrite (callee_saved_notp_lookup Hcs (mword_of_int 23 : mword 5) ltac:(vm_compute; reflexivity)). exact I.
  - rewrite (callee_saved_notp_lookup Hcs (mword_of_int 24 : mword 5) ltac:(vm_compute; reflexivity)). exact J.
  - rewrite (callee_saved_notp_lookup Hcs (mword_of_int 25 : mword 5) ltac:(vm_compute; reflexivity)). exact K.
  - rewrite (callee_saved_notp_lookup Hcs (mword_of_int 26 : mword 5) ltac:(vm_compute; reflexivity)). exact L.
  - rewrite (callee_saved_notp_lookup Hcs (mword_of_int 27 : mword 5) ltac:(vm_compute; reflexivity)). exact N.
Qed.
...
Section AslProps.
  ...
  (* The shared exit-path continuation (control at +0x28), and the wait-loop
     invariant (control at +0x1c).  Both are HART-FREE propositions
     quantified over the hart and SIE ghost their instance runs at: the park
  ...
  Definition asl_exit
    ...
    (∀ (h : CPU) (g : gname) (M : regfile),
      ⌜ asl_regs h m M slk spd ⌝ -∗
      ...
      locked γl h -∗ sleeplocked γsl -∗
      sl_pid slk ↦₄ (mword_of_int 0 : mword 32) -∗ R -∗
      slk ↦₄ (mword_of_int 0 : mword 32) -∗
      p_pid (proc_addr j) ↦₄{dq} pidv -∗
      own_ctx (p_context (proc_addr j)) -∗
      ▷ sched_vc_at Φ γs h g (a_cpu_ctx (cid_word_of h)) (proc_addr j) -∗
      cpu_own (CID := h) g 1 eb (proc_addr j) C -∗
      trap_csrs_pay (CID := h) 0 eb -∗
      sie_cap_gpr (CID := h) g M (av - 4)%nat -∗
      pc_is (CID := h) (mword_of_int (ASL + 0x28)) -∗
      WP (LoopE h : expr riscv_lang) {{ Φ }})%I.
  ...
  Definition asl_loop
    ...
    (∀ (h : CPU) (g : gname) (M : regfile),
      ...
      trap_csrs_pay (CID := h) 0 eb -∗
      sie_cap_gpr (CID := h) g M (av - 4)%nat -∗
      pc_is (CID := h) (mword_of_int (ASL + 0x1c)) -∗
      asl_exit Φ γs j γl γsl R m pidv av dq slk spd sp0 eb C -∗
      WP (LoopE h : expr riscv_lang) {{ Φ }})%I.
...
(* ===================================================================== *)
(*  THE STRAIGHT-LINE STRETCHES, EACH WITH [CID] AS A BINDER.             *)
(* ===================================================================== *)
Section AslBodies.
  Context `{!riscvGS Σ, !sieG Σ, !lockG Σ, !fdslotG Σ}.

  (* ---- the exit path: +0x28 (locked:=1) .. +0x44 (c.ret) ---- *)
  Lemma asl_exit_body `{CID : CpuId}
      (g : gname) (Φ : mval -> iProp Σ) (γs : list gname) (j : nat)
  ...
  Proof.
    intros pj Hav Heb Hspd Hsp0 Hasl.
    destruct Hasl as (Hs1 & Hs2 & Hsp & HtpM & H19 & H20 & H21 & H22 & H23 & H24 & H25 & H26 & H27).
    iIntros "#Htext #Hslk Hr24 Hr16 Hr8 Hr0 Htok Hstok Hspid HR Hw Hpid Hctx Hsched Hown Hpay Hcg Hpc Hcont".
    ...
    (* +0x28 c.li a5,1 *)
    iApply (wp_cli_s_sconf g Φ (mword_of_int (ASL + 0x28)) (mword_of_int 15 : mword 5) (mword_of_int 1 : mword 6) (mword_of_int 1 : mword 64)
    ...
    iApply (wp_csw_s_sconf g Φ (mword_of_int (ASL + 0x2a)) (mword_of_int 15 : mword 5) (mword_of_int 9 : mword 5)
    ...
    (* +0x2c jal ra,myproc *)
    iApply (wp_jal_s_sconf g Φ (mword_of_int (ASL + 0x2c)) (mword_of_int 1 : mword 5) (mword_of_int 2087436 : mword 21)
    ...
    iApply (Myproc.wp_myproc_sconf g Φ E2 (av - 4)%nat 1%nat eb pj C
              ltac:(exact HE2tp)
              ltac:(lia)
              ltac:(lia)
    ...
    { rewrite Hmfma0. rewrite /p_pid.
      ...
      reflexivity. }
    iApply (wp_clw_s_sconf g Φ (mword_of_int (ASL + 0x30)) (mword_of_int 15 : mword 5) (mword_of_int 10 : mword 5)
    ...
    { rewrite HE3s1. rewrite /sl_pid.
      ...
      reflexivity. }
    iApply (wp_csw_s_sconf g Φ (mword_of_int (ASL + 0x32)) (mword_of_int 15 : mword 5) (mword_of_int 9 : mword 5)
    ...
    iApply (wp_cmv_s_sconf g Φ (mword_of_int (ASL + 0x34)) (mword_of_int 10 : mword 5) (mword_of_int 18 : mword 5)
    ...
    (* +0x36 jal ra,release *)
    iApply (wp_jal_s_sconf g Φ (mword_of_int (ASL + 0x36)) (mword_of_int 1 : mword 5) (mword_of_int 2084238 : mword 21)
    ...
    iApply (Release.wp_release_sconf g Φ γl (sl_lk slk) "sleep lock"%string (sl_res γsl slk R) E5
              0%nat eb pj C (av - 4)%nat
              Hrel_lka HE5tp ltac:(lia)
    ...
    iApply (wp_cldsp_s_sconf g Φ (mword_of_int (ASL + 0x3a)) (mword_of_int 3 : mword 6) (mword_of_int 1 : mword 5)
    ...
    iApply (wp_cldsp_s_sconf g Φ (mword_of_int (ASL + 0x3c)) (mword_of_int 2 : mword 6) (mword_of_int 8 : mword 5)
    ...
    (* +0x3e c.ldsp s1,8(sp) *)
    iApply (wp_cldsp_s_sconf g Φ (mword_of_int (ASL + 0x3e)) (mword_of_int 1 : mword 6) (mword_of_int 9 : mword 5)
    ...
    (* +0x40 c.ldsp s2,0(sp) *)
    iApply (wp_cldsp_s_sconf g Φ (mword_of_int (ASL + 0x40)) (mword_of_int 0 : mword 6) (mword_of_int 18 : mword 5)
    ...
    iApply (wp_caddi16sp_pop_s_sconf g Φ (mword_of_int (ASL + 0x42)) (mword_of_int 2 : mword 6) Q40 (av - 4)%nat 4 Hpop
    ...
    iApply (wp_cret_s_sconf g Φ (mword_of_int (ASL + 0x44)) (mword_of_int 1 : mword 5) Q42 av
    ...
    (* the postcondition: tp is THIS hart's (the resuming one), so the
       register fact is [callee_saved_notp] plus the tp pin. *)
    ...
    assert (Ctp : Q42 !!! Regidx (mword_of_int 4 : mword 5) = cid_word_of cpu_id).
    { rewrite (Hthr (mword_of_int 4 : mword 5) ltac:(vm_compute; reflexivity) ltac:(reg_neq) ltac:(reg_neq) ltac:(reg_neq) ltac:(reg_neq) ltac:(reg_neq) ltac:(reg_neq) ltac:(reg_neq)).
      exact HtpM. }
    iApply ("Hcont" $! cpu_id g Q42 with "[%] [%] Hcg Hown Hpc Hstok Hspid HR Hpid Hctx Hsched").
  ...
  (* ---- the post-park stretch: +0x24 (reload) .. +0x26 (the branch).
     THIS runs at the hart sleep() resumed on, and its two arms are the two
     hart-generic propositions: the exit path, or the loop's own Löb IH. ---- *)
  Lemma asl_post_sleep_body `{CID : CpuId}
      (g : gname) (Φ : mval -> iProp Σ) (γs : list gname) (j : nat)
    ...
    asl_regs cpu_id m M slk spd ->
    kernel_text -∗
    ▷ asl_loop Φ γs j γl γsl R m pidv av dq slk spd sp0 eb C -∗
    asl_exit Φ γs j γl γsl R m pidv av dq slk spd sp0 eb C -∗
    pa_stk sp0 1 ↦₈ (m !!! Regidx (mword_of_int 1 : mword 5)) -∗
    pa_stk sp0 2 ↦₈ (m !!! Regidx (mword_of_int 8 : mword 5)) -∗
    pa_stk sp0 3 ↦₈ (m !!! Regidx (mword_of_int 9 : mword 5)) -∗
    ...
    trap_csrs_pay 0 eb -∗
    sie_cap_gpr g M (av - 4)%nat -∗
    pc_is (mword_of_int (ASL + 0x24)) -∗
    WP (Loop : expr riscv_lang) {{ Φ }}.
  Proof.
    intros pj Hav Heb Hasl.
    iIntros "#Htext IH Hexit Hr24 Hr16 Hr8 Hr0 Htok HRc Hpid Hctx Hsched Hown Hpay Hcg Hpc".
    assert (HaslM : asl_regs cpu_id m M slk spd) by exact Hasl.
    destruct Hasl as (Hs1 & Hs2 & Hsp & HtpM & H19 & H20 & H21 & H22 & H23 & H24 & H25 & H26 & H27).
    ...
    assert (Hlw24 : add_vec (M !!! Regidx (mword_of_int 9 : mword 5)) (sign_extend' 64 (zero_extend' 12 (concat_vec (mword_of_int 0 : mword 5) ('b"00")))) = slk).
    { rewrite Hs1. replace (sign_extend' 64 (zero_extend' 12 (concat_vec (mword_of_int 0 : mword 5) ('b"00"))) : mword 64) with (mword_of_int 0 : mword 64) by (apply bv_eq; vm_compute; reflexivity). apply kv_addv_zero. }
    iApply (wp_clw_s_sconf g Φ (mword_of_int (ASL + 0x24)) (mword_of_int 15 : mword 5) (mword_of_int 9 : mword 5)
              (zero_extend' 12 (concat_vec (mword_of_int 0 : mword 5) ('b"00"))) M (av - 4)%nat vp (dqm := DfracOwn 1)
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
    ...
    assert (HaslLa5 : asl_regs cpu_id m La5 slk spd) by (apply (asl_regs_cs cpu_id m M La5 slk spd HcsMLa5 HaslM)).
    ...
    - (* FREE: vp = 0 -> bnez falls through to +0x28 -> exit *)
      iApply (wp_cbnez_fall_s_sconf g Φ (mword_of_int (ASL + 0x26)) (mword_of_int 251 : mword 8) (Cregidx (mword_of_int 7)) (mword_of_int 15 : mword 5)
      ...
      rewrite /asl_exit.
      iApply ("Hexit" $! cpu_id g La5 with "[%] Hr24 Hr16 Hr8 Hr0 Htok Hstok Hspid HRu Hwp Hpid Hctx Hsched Hown Hpay Hcg Hpc").
      exact HaslLa5.
    - (* HELD: vp <> 0 -> bnez TAKEN, back edge to +0x1c (the Löb IH).  Hand the
      ...
      iApply (wp_cbnez_taken_s_sconf g Φ (mword_of_int (ASL + 0x26)) (mword_of_int 251 : mword 8) (Cregidx (mword_of_int 7)) (mword_of_int 15 : mword 5)
                La5 (av - 4)%nat ltac:(vm_compute; reflexivity) ltac:(vm_compute; discriminate)
                ltac:(rewrite HLa5_15; exact Hvph)
                ltac:(vm_compute; reflexivity)
                with "Hcg Hpc Hi26 [Hr24 Hr16 Hr8 Hr0 Htok Hpid Hctx Hsched Hown Hpay IH Hexit Hheldw]").
      iNext. iIntros "Hcg Hpc".
      (* iNext strips the [▷] of [▷ sched_vc_at] and of the IH; cpu_own /
         trap_csrs_pay are opaque Definitions so their contents are untouched --
         re-wrap only the scheduler valid-context. *)
      iAssert (▷ sched_vc_at Φ γs cpu_id g (a_cpu_ctx cid_word) pj)%I with "[Hsched]" as "Hsched".
      ...
      rewrite /asl_loop.
      iApply ("IH" $! cpu_id g La5 with "[%] Hr24 Hr16 Hr8 Hr0 Htok Hheldw Hpid Hctx Hsched Hown Hpay Hcg Hpc Hexit").
      exact HaslLa5.
  Qed.

  (* ---- one loop iteration: +0x1c .. the sleep() call, which parks ---- *)
  Lemma asl_loop_body `{CID : CpuId}
      (g : gname) (Φ : mval -> iProp Σ) (γs : list gname) (j : nat)
    ...
    WP (Loop : expr riscv_lang) {{ Φ }}.
  Proof.
    intros pj Hav Heb Hj Hjpl Hasl.
    iIntros "#Htext #Hslk #Hpanic #Hpinv IH Hr24 Hr16 Hr8 Hr0 Htok Hheld Hpid Hctx Hsched Hown Hpay Hcg Hpc Hexit".
    assert (HaslM : asl_regs cpu_id m M slk spd) by exact Hasl.
    destruct Hasl as (Hs1 & Hs2 & Hsp & HtpM & H19 & H20 & H21 & H22 & H23 & H24 & H25 & H26 & H27).
    ...
    iApply (wp_cmv_s_sconf g Φ (mword_of_int (ASL + 0x1c)) (mword_of_int 11 : mword 5) (mword_of_int 18 : mword 5)
    ...
    iApply (wp_cmv_s_sconf g Φ (mword_of_int (ASL + 0x1e)) (mword_of_int 10 : mword 5) (mword_of_int 9 : mword 5)
    ...
    (* +0x20 jal ra,sleep *)
    iApply (wp_jal_s_sconf g Φ (mword_of_int (ASL + 0x20)) (mword_of_int 1 : mword 5) (mword_of_int 2088986 : mword 21)
    ...
    assert (HaslL2 : asl_regs cpu_id m L2 slk spd) by (apply (asl_regs_cs cpu_id m M L2 slk spd HcsML2 HaslM)).
    ...
    iApply (Sleep.wp_sleep_sconf g Φ γs j γpl γl (sl_lk slk) "sleep lock"%string (sl_res γsl slk R) L2 (av - 4)%nat eb C
              HL2tp Hj Hjpl Hsl_lka Heb
              ltac:(lia)
              with "Hcg Hown Hpay Htext Hpc Hpinv [] Htok HRc Hpanic Hctx Hsched [-]").
    { iApply (is_sleeplock_lock with "Hslk"). }
    (* SLEEP RETURNS ON HART [h], UNDER GHOST [g2].  Everything after the park
       runs there, inside [asl_post_sleep_body] at [(CID := h)]. *)
    iIntros (h g2 mfs) "%Hs_cs %Hs_tp Hcg Hown Hpay Hpc Htok HRc Hctx Hsched".
    ...
    assert (HaslMfs : asl_regs h m mfs slk spd)
      by (apply (asl_regs_notp cpu_id h m L2 mfs slk spd Hs_cs Hs_tp HaslL2)).
    iApply (asl_post_sleep_body (CID := h) g2 Φ γs j γl γsl R m mfs pidv av dq slk spd sp0 eb C
              Hav Heb HaslMfs
              with "Htext IH Hexit Hr24 Hr16 Hr8 Hr0 Htok HRc Hpid Hctx Hsched Hown Hpay Hcg Hpc").
  Qed.
...
Section ProofAcquiresleep.
  ...
  Proof.
    ...
    intros pcE slk pj ret_tgt Htp Hj Hav Heb.
    set (sp0 := (m !!! Regidx csp_rs1 : mword 64)).
    set (spd := add_vec sp0 (sign_extend' 64 (sign_extend' 12 (mword_of_int 32 : mword 6)))).
    iIntros "Hcg Hown #Htext Hpc #Hslk #Hpanic Hpid #Hpinv Hctx Hsched Hcont".
    (* the loop/exit invariants state the scheduler slot at an explicit hart *)
    iAssert (▷ sched_vc_at Φ γs cpu_id γ (a_cpu_ctx cid_word) pj)%I with "[Hsched]" as "Hsched".
    { iExact "Hsched". }
    ...
    { iApply (panic_wp_any_at cpu_id with "Hpanic"). }
    ...
    assert (Hasl_acq : asl_regs cpu_id m Macq slk spd).
    { unfold asl_regs.
      repeat split.
    ...
    (* ============ the hart-generic EXIT continuation (+0x28 -> ret) ============ *)
    iAssert (asl_exit Φ γs j γl γsl R m pidv av dq slk spd sp0 eb C) with "[Hcont]" as "Hexit".
    { rewrite /asl_exit.
      iIntros (h g M) "%HaslE Hr24 Hr16 Hr8 Hr0 Htok Hstok Hspid HRx Hw Hpid Hctx Hsched Hown Hpay Hcg Hpc".
      iApply (asl_exit_body (CID := h) g Φ γs j γl γsl s R m M pidv av dq slk spd sp0 eb C
                Hav Heb Hspd Hsp0 HaslE
                with "Htext Hslk Hr24 Hr16 Hr8 Hr0 Htok Hstok Hspid HRx Hw Hpid Hctx Hsched Hown Hpay Hcg Hpc Hcont"). }

    (* ============ the WAIT LOOP (iLöb over the ∀h-generic invariant) ============ *)
    iAssert (asl_loop Φ γs j γl γsl R m pidv av dq slk spd sp0 eb C) with "[]" as "Hloop".
    { iLöb as "IH". rewrite /asl_loop.
      iIntros (h g M) "%HaslL Hr24 Hr16 Hr8 Hr0 Htok Hheld Hpid Hctx Hsched Hown Hpay Hcg Hpc Hexit".
      iApply (asl_loop_body (CID := h) g Φ γs j γpl γl γsl s R m M pidv av dq slk spd sp0 eb C
                Hav Heb Hj Hjpl HaslL
                with "Htext Hslk Hpanic Hpinv IH Hr24 Hr16 Hr8 Hr0 Htok Hheld Hpid Hctx Hsched Hown Hpay Hcg Hpc Hexit"). }
    ...
    assert (HaslMe : asl_regs cpu_id m Me slk spd) by (apply (asl_regs_cs cpu_id m Macq Me slk spd HcsAcqMe HaslAcqW)).
    ...
    - (* FREE at entry: v0 = 0 -> c.beqz TAKEN -> +0x28 (exit) *)
      ...
      (* iNext (from beqz-taken) strips [▷ sched_vc_at]; cpu_own/pay are opaque *)
      iAssert (▷ sched_vc_at Φ γs cpu_id γ (a_cpu_ctx cid_word) pj)%I with "[Hsched]" as "Hsched".
      { iApply bi.later_intro. iExact "Hsched". }
      rewrite /asl_exit.
      iApply ("Hexit" $! cpu_id γ Me with "[%] Hr24 Hr16 Hr8 Hr0 Htok Hstok Hspid HRu Hw0 Hpid Hctx Hsched Hown Hpay Hcg Hpc").
      exact HaslMe.
    - (* HELD at entry: v0 <> 0 -> c.beqz falls through -> +0x1c (the loop) *)
      ...
      rewrite /asl_loop.
      iApply ("Hloop" $! cpu_id γ Me with "[%] Hr24 Hr16 Hr8 Hr0 Htok Hheldw Hpid Hctx Hsched Hown Hpay Hcg Hpc Hexit").
      exact HaslMe.
  Qed.
