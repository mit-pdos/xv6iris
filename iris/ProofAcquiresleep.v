...
Section AslProps.
  ...
  Definition asl_nloop_l `{GEN : GenId} `{CID : CpuId} `{XI : CurCtx}
    ...
    (∀ (M : regfile),
      ...
      sie_cap_gpr KT1 M (av - 4)%nat false (proc_addr j) -∗
      pc_is (mword_of_int (KernelSyms.acquiresleep + 0x1c)) -∗
      asl_nexit_l γl γsl R H q X Tl m j pidv av Vpr slk spd sp0 eb n lks -∗
      WP (Loop : expr riscv_lang))%I.
...
Section ProofAcquiresleep.
  ...
  Lemma wp_acquiresleep_genl_llb_sconf
    ...
    : wp_acquiresleep_genl_llb_sconf_body γs j γl γsl s R H q m pidv Vpr av eb b lks Tl.
  Proof.
  ...
  Lemma asl_nested_core_llb
    ...
    X -∗
    proc_priv_bare pj pidv Vpr -∗
    asl_nloop_l γl γsl R H q X Tl m j pidv av Vpr slk
      (add_vec (m !!! Regidx csp_rs1 : mword 64)
         (sign_extend' 64 (sign_extend' 12 (mword_of_int 32 : mword 6))))
      (m !!! Regidx csp_rs1 : mword 64) eb n lks -∗
    wp_next false pj (fun (CID : CpuId) =>
  ...
  Proof.
    ...
    (* ============ the EXIT continuation (+0x36 -> ret) ============ *)
    iAssert (asl_nexit_l γl γsl R H q X Tl m j pidv av Vpr slk spd sp0 eb n lks) with "[Hcont]" as "Hnexit".
