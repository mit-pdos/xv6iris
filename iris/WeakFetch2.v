...
Proof.
  ...
  pma_ok_eff_peel Hmatch Hexec (exec_eff_is_mag_applicable_fetch 2 s) Halign.
Qed.

(** [RiscvFetchExec]'s own [Local Lemma avi0_mul2], restated (it is [Local]
    there, so not importable). *)
Local Lemma avi0_mul2 (a : SailStdpp.Values.mword 64) : add_vec_int a (0 * 2) = a.
...
Proof.
  ...
  assert (Hmmio : exec_eff (within_mmio_readable (Physaddr addr) 2) s
                  = Some (false, s, [])).
  ...
  rewrite (execR_eff_liftR_seq _ _ _ _ _
             (exec_eff_split_misaligned_unsplit addr 2 0 s)). cbn beta.
  ...
  match goal with |- context[Defs.bind (Defs.untilMT ?vs ?m ?c ?b) _] =>
    assert (Hu : execR_eff (Defs.untilMT vs m c b) s
                 = Some (inr (w, true, 0), s, [WEread wak_plain addr 2])) end.
  { eapply execR_eff_untilMT_1; [ reflexivity | | apply execR_eff_returnR ].
    ...
    match goal with
      ...
      assert (Hrd : execR_eff (Defs.bind (Defs.liftR (read_ram rk pa wd mt)) k1) s
                    = Some (inr w, s, [WEread wak_plain addr 2])) end.
