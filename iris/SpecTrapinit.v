(* SpecTrapinit.v -- the public interface of Trapinit, stated independently of
...
Definition time_name_str : Z := 0x80007258%Z.
...
(* trapinit() = initlock(&tickslock, "time").  It takes no arguments and
   ...
   caller hands the word and the cpu field in raw and gets them back zeroed, and
   the name field comes back sealed as the persistent [lock_name lk "time"].
...
Definition wp_trapinit_sconf_body `{!riscvGS Σ, !xv6G Σ} `{GEN : GenId} `{CID : CpuId} `{XI : CurCtx} (m : regfile) (K : nat) (vlock : bv 32) (vname vcpu : bv 64) (b : bool) (p : mword 64) :=
