...
Section ProofKilled.
  ...
  Proof.
    ...
    { rewrite /A2 upd_ne; [| vm_compute; discriminate].
      ...
      rewrite /M1 upd_ne; [| vm_compute; discriminate]. exact Ha0. }
    (* +0x0e: jal ra,acquire *)
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.killed + 0x0e)) kl_ra (mword_of_int 2091616 : mword 21)
              A2 (av - 4)%nat b
    ...
    assert (Hjacq : add_vec (mword_of_int (KernelSyms.killed + 0x0e) : mword 64) (sign_extend' 64 (mword_of_int 2091616 : mword 21)) = mword_of_int KernelSyms.acquire)
    ...
    iDestruct (cpu_own_transport CID CID8 n eb p b ltac:(wp_next_chain)
                 with "Hcpu") as "Hcpu".
    iApply (Acquire.wp_acquire_sconf KT1 γl "proc"%string (λ ξ : CtxId, proc_lock_res (XI := ξ) γs γl (proc_addr j)) B1 n eb p (av - 4)%nat b lks
              Hn ltac:(lia)
              Hfresh
    ...
    (* +0x18: jal ra,release *)
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.killed + 0x18)) kl_ra (mword_of_int 2091742 : mword 21)
    ...
    assert (Hjrel : add_vec (mword_of_int (KernelSyms.killed + 0x18) : mword 64) (sign_extend' 64 (mword_of_int 2091742 : mword 21)) = mword_of_int KernelSyms.release)
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (Release.wp_release_sconf KT1 γl (proc_addr j) "proc"%string (λ ξ : CtxId, proc_lock_res (XI := ξ) γs γl (proc_addr j)) C4 n eb p (av - 4)%nat ({["proc"]} ∪ lks)
              Hlka ltac:(lia)
