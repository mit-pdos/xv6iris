(* ProofNamexTr.v -- the whole-function proof of namex AT THE TRACE
   ...
   This is a SECOND walk of the same 334 bytes at
   [SpecNamexTr.wp_namex_tr_body]: the same blocks, the same callee
   contracts, the same budget and ledger arithmetic -- all of it reusing
   [ProofNamexParts] and [ProofNamex]'s own top-level pure lemmas verbatim
   -- with exactly three differences.

   (1) THE LOOP INVARIANT CARRIES A CURSOR.  [nx_loop_body] and
       [nx_rest_body] below replace [inode_held ipv] with
       [SpecNameiTr.inode_held_at ipv dcur] -- the same four conjuncts plus
       the pure tie [bv_unsigned inum = dcur] -- and gain two trace rows:
       [P (length es0) dcur] and the UNFIRED suffix
       [nx_hops_from P Pmiss pl (length es0)].  [es0] is the walk's own
       already-consumed prefix, which the landed invariant carries anyway,
       so the hop index costs nothing new.

   (2) ONE FIRE, IN dirlookup's CONTINUATION.  See the comment at the fire
       point itself.  The whole of the ghost trace is that one [iMod].

   (3) THREE ARMS ARE DEAD, AND ARE REFUTED RATHER THAN PROVED.  The
       ruling fixes the contract at ABSOLUTE PATHS, namei side: the
       [pfun 0 = SLASH] premise kills the relative start (myproc / idup),
       and [a1 = 0] kills [L_par] and the nameiparent tail of [L_done].
       The functor still takes all nine callee modules so the Link file is
       [LinkNamex.v]'s shape verbatim; [IU] and [IP] are simply unused.

   The block decomposition, the [Htail] / [Hloop] / [Hrest] / [Hdlblk]
   assertions and the performance rules behind them are
   [ProofNamex.v]'s -- read that file's header first; nothing here
   restates it. *)
...
(* ===================================================================== *)
(*  THE TRACE'S THREE LEMMAS -- everything the ghost side of the walk     *)
(*  needs, and the only genuinely new proof content in this file.         *)
(* ===================================================================== *)
...
Section NamexTrHops.
  ...
  (* PEEL THE HEAD HOP.  [nx_hops_from .. k] is the big-op over
     ...
     element it is about to look up, that list is a cons and the family
     splits into THIS hop and the suffix from [S k].  The index shift
     [k + S j = S k + j] is the whole content. *)
  Lemma nxt_hops_cons (P Pmiss : nat -> Z -> iProp Σ)
      (pl : list (bv 8)) (k : nat) (s : fname) (rest : list fname) :
    drop k (path_elems pl) = s :: rest ->
    nx_hops_from P Pmiss pl k -∗
    nx_hop P Pmiss k s ∗ nx_hops_from P Pmiss pl (S k).
  Proof.
    iIntros (Hd) "H". rewrite /nx_hops_from.
  ...
  (* ...AND ONE THAT MISSES: same lend, and the caller's own [Pmiss]
     receipt comes back instead of a stepped cursor. *)
...
Module NamexTrProof (MP : MYPROC) (ID : IDUP) (IG : IGET) (MM : MEMMOVE)
                    (IL : ILOCK) (IU : IUNLOCK) (IUP : IUNLOCKPUT)
                    (DL : DIRLOOKUP) (IP : IPUT) : NAMEX_TR.
...
Section ProofNamexTrMain.
  ...
  Definition nx_loop_body
    ...
    (* THE CURSOR RIDES THE INVARIANT (namei-pinned-lookup.md §4).  [dcur]
       ...
       [nx_hops_from P Pmiss pl (length es0)].  Every exit reads its own
  ...
  Proof.
    ...
    iClear "Hi000 Hi002 Hi004 Hi006 Hi008 Hi00a Hi00c Hi00e Hi010 Hi012 Hi014 Hi016 Hi018 Hi01a".
    ...
    { iModIntro.
      ...
      iClear "Hj5c Hj5e Hj60 Hj62 Hj64 Hj66 Hj68 Hj6a Hj6c Hj6e Hj70 Hj72 Hj74 Hj76 Hj78".
      iEval (rgne) in "Hpc".
...
"[%] [%] Hcg Hcnt Hextc Hclmc Hpc
          ...
          * iIntros (CIDb Hsb a Mb) "%B1 %B2 %B3 %B4 %B5 %B6 %B7 %B8
            ...
            -- iIntros (CIDe Hse e Me) "%E1 %E2 %E3 %E4 %E5 %E6
               ...
               iAssert (wp_next (CID0 := CIDl) true (proc_addr j)
                          (fun CIDt : CpuId =>
                             nx_rest_body j b K m sp0 pv nb plen a e pfun ipv ncur
                                          ...
                                          eb g gfs bn cov logstart bmapstart
               ...
               { iIntros (CIDt Hst Mt nf') "%Hregt %Hviewt Hcg Hcnt Hextc Hclmc Hpc
                          ...
                          Hpath Hname Hbslot Hlog HP Hhops".
                     ...
                     * iIntros (CIDf Hsf mf) "%Hcsf %Hfa0 Hcg Hcnt Hextc Hclmc Hpc".
                     ...
                     (* ---- everything else reaches dirlookup at +0xde ---- *)
                       ...
                       { iIntros (CIDz Hsz Mz) "%Hregz Hcg Hcnt Hextc Hclmc Hpc Hpath".
                         ...
                         iDestruct (nxt_hops_cons P Pmiss pl (length es0)
                                      (bname 14 nf') (path_elems (drop o2 pl))
                         ...
                         - (* ============ FOUND: recurse on the child ==== *)
                           ...
                           iApply fupd_wp.
                           iMod (nxt_hop_hit P Pmiss (length es0)
                                   (bname 14 nf') (bv_unsigned iinum)
                                   (dv_of dnl datl)
                                   (bv_unsigned (dir_inum datl kdir)) Hents
                                   with "Hhop HP Hdview") as "[Hdview HP]".
                           iModIntro.
                         ...
                         - (* ============ MISS: iunlockput and return 0 === *)
                           ...
                           iApply fupd_wp.
                           iMod (nxt_hop_miss P Pmiss (length es0)
                                   (bname 14 nf') (bv_unsigned iinum)
                                   (dv_of dnl datl) Hents
                                   with "Hhop HP Hdview") as "[Hdview HP]".
                           iModIntro.
                           iAssert (ic_loaded gfs gi cov logstart ik iinum dnl
                                      bml)
               ...
               ++ (* ================ LONG: 14 <= len ================= *)
                  ...
                  iApply ("Hrest" $! T5 (fun jj => pfun (a + jj)%nat)
                            ...
                            Hb1 Hb2 Hb3 Hb4 Hb5 Hb6 Hb7 Hb8 Hb9 Hb10 Hb11 Hb12
                            Hip Hisl Hbmap Hinos Hppid Hcwdr
                            Hpath Hdst Hbslot Hlog HP Hhops").
                  ** exact HT5regs.
                  ** exact Hview. }
    ...
    iClear "Hi01c Hi01e Hi020 Hi022 Hi026".
    ...
    + (* ---------------- THE ABSOLUTE ARM: iget(ROOTDEV, ROOTINO) ------- *)
      ...
      iClear "Hi048 Hi04a Hi04c Hi050 Hi052".
      ...
      iApply ("Hloop" $! 0%nat (ientry kig) A8 n Sb [] nfun false
                ...
                with "[%] [%] [%] [%] [%] [%] [%] [%] [%] [%] Hcg Hcnt Hextc Hclmc Hpc
                      ...
                      Hname Hbslot Hlog HP0 Hhops0 Hcont").
      - lia.
      - lia.
