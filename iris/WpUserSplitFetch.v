(* WpUserSplitFetch.v -- the U-mode RETIRE engine for a 32-bit F_Base
   instruction fetched at a pc == 2 (mod 4) address: the split (2+2)
   fetch, low half at [va] (physaddr [pal] via [vpnL]/[ieL]) and high
   half at [va+2] (physaddr [pah] via [vpnH]/[ieH]), each translated
   through its OWN TLB hit (the two halves may live in different pages).

   [wp_instr_u_split] is the analog of [WpUserBase.wp_instr_u_hit] (the
   4-aligned single-translate fetch-hit success engine); the ONLY
   structural change is single-translate -> two-translate on the fetch,
   wrapping [exec_fetch_F_Base_2_U_gen] (UmodeFetchC.v) instead of
   [exec_fetch_F_Base_4_U_gen].  Because a TLB-hit translate is
   state-preserving, both halfword translates thread s -> s -> s, so the
   whole step runs at the single fetch state [σ], exactly as the hit
   engine does.  Its decode/execute continuation is IDENTICAL to
   [wp_instr_u_hit]'s, so the retire arms ride it unchanged -- only the
   fetch-hit premise bundle is doubled (one hit fact per halfword).      *)
...
Section WpUserSplitFetch.
  ...
  (* ------------------------------------------------------------------ *)
  (* The U-mode RETIRE engine for the 2-aligned (pc == 2 mod 4) 32-bit   *)
  (* F_Base split fetch.  Analog of [wp_instr_u_hit]: same decode/execute *)
  (* continuation, same WP shape, same frame; the ONLY change is the      *)
  (* fetch -- TWO independent TLB hits (low half va -> pal via vpnL/ieL,  *)
  (* high half va+2 -> pah via vpnH/ieH), wrapping                         *)
  (* [exec_fetch_F_Base_2_U_gen] rather than [exec_fetch_F_Base_4_U_gen]. *)
  (* ------------------------------------------------------------------ *)
  Lemma wp_instr_u_split
