(* ProofNamei.v -- namei over the SIE-agnostic sconf world.
   ...
   26 bytes, eleven instructions.  A FOUR-slot frame: ra@24 and s0@16 are
   the two saves, and the low two slots -- sp+0 .. sp+15, i.e. [pa_stk sp0 4]
   and [pa_stk sp0 3] -- ARE the [name[14]] local.  [addi a2,s0,-32] is the
   address of it, computed off the frame pointer.
   ...
   which is why namei's postcondition has no name clause and no [nf] binder,
   and why [npar := false] discharges namex's conditional one vacuously. *)
...
Definition nam_thr (m M : regfile) : Prop :=
  forall c : mword 5, is_cs_idx c = true ->
    c <> csp_rs1 -> c <> (mword_of_int 8 : mword 5) ->
    M !!! Regidx c = (m !!! Regidx c : mword 64).

Definition nam_sp (m M : regfile) : Prop :=
  M !!! Regidx csp_rs1 = pa_stk (m !!! Regidx csp_rs1 : mword 64) 4.

(* the four c.sdsp / c.ldsp displacements off the pushed sp *)
Lemma nam_frm (X : mword 64) (u : mword 6) (k : nat) :
...
Lemma nam_frm1 (X : mword 64) :
  add_vec (pa_stk X 4) (zero_extend' 64 (concat_vec (mword_of_int 3 : mword 6) ('b"000")))
  = pa_stk X 1.
Proof. apply nam_frm. apply bv_eq; vm_compute; reflexivity. Qed.

Lemma nam_frm2 (X : mword 64) :
  add_vec (pa_stk X 4) (zero_extend' 64 (concat_vec (mword_of_int 2 : mword 6) ('b"000")))
  = pa_stk X 2.
Proof. apply nam_frm. apply bv_eq; vm_compute; reflexivity. Qed.

(* [addi a2,s0,-32] off the frame pointer (which is the ENTRY sp) is the
   BASE of the name buffer, i.e. the lowest slot of the frame *)
Lemma nam_buf (X : mword 64) :
  add_vec X (sign_extend' 64 (mword_of_int 4064 : mword 12)) = pa_stk X 4.
Proof. apply stk_push. apply bv_eq; vm_compute; reflexivity. Qed.

(* K_namei's single premise, turned into the three bounds the callee and
...
Lemma nam_a1_false :
  eq_vec (mword_of_int 0 : mword 64) (zero_reg : mword 64) = negb false.
Proof. vm_compute. reflexivity. Qed.

Module NameiProof (NX : NAMEX) : NAMEI.
...
Section ProofNameiMain.
  ...
  Proof.
    ...
    iIntros "Hcg Hcnt #Htext Hpc #Hpanic #Hbio #Hlogc #Hkenv #Hitb2 #Hitbl
              ...
              Hbits Hppid Hcwdc Hcwdr Hpath Hbslot Hislot Hlog Hcont".
    ...
    iApply (NX.wp_namex_gen gs j gl gu gd gk pd pav pu bn g gfs gi cn gtl
              ...
              plen pfun nfun false n Sb pidv dq dqb dqs dqc R5 (K - 4)%nat eb C b
    ...
    assert (Cs11 : (P3 !!! Regidx (mword_of_int 27 : mword 5) : mword 64)
      ...
      by (apply Hfin; namidx).
    (* the two arms, re-stated at the FINAL register file.  [npar = false]
       makes namex's name clause vacuous, so it is simply DROPPED. *)
    iAssert (if ok
    ...
    { destruct ok.
      - iDestruct "Hok" as "[%Hp [Hheld Hsl]]". iFrame "Hheld Hsl".
        iPureIntro. rewrite HP3a0. exact (proj1 Hp).
    ...
    iApply ("Hcont" $! P3 n' used' Sb' ok ipv
  ...
  Proof.
    ...
    iApply (wp_namei_gen gs j gl gu gd gk pd pav pu bn g gfs gi cn gtl
              ga gf cov logstart bmapstart inodestart nib
              size dev used cwdv plen pfun n Sb0
              pidv dq dqb dqs dqc m K eb C b
              HK Hdev Hnib Hroot Hnib0 Hlg Hsize Hbmap0 Hbmapcov Hbmaplog Hinos0 Hcovb Hiregb Hcstr Hplen Hbud Hj Hgs Heb
              with "Hcg Hcnt Htext Hpc Hpanic Hbio Hlogc Hkenv Hitb2 Hitbl Hesc Hslks Hireg Hprocs Hdev Hgeom Hdlk Hbmap Hinos Hbits Hppid Hcwdc Hcwdr Hpath Hbslot Hislot Hlog [Hcont]").
    iEval (rewrite /wp_next).
    iIntros (CIDf) "%Hchain".
    iIntros (mf n' used' Sb' ok ipv)
      "%Hcs Hcg Hcnt Hpc Hbmap Hinos %Husub Hbits Hppid Hcwdc Hcwdr
       Hpath Hbslot %Hssub %Hbnd Hlog Hok".
    iSpecialize ("Hcont" $! CIDf with "[%]"); [exact Hchain|].
    iApply ("Hcont" $! mf n' used' ok ipv
              with "[%] Hcg Hcnt Hpc Hbmap Hinos [%] Hbits Hppid Hcwdc Hcwdr
                    Hpath Hbslot [%] [Hlog] Hok").
    { exact Hcs. }
    { exact Husub. }
