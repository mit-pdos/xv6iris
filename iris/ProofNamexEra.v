(* ProofNamexEra.v -- the whole-function proof of namex AT THE ERA-FRAGMENT
   TRACE CONTRACT ([SpecNamexEra.wp_namex_era_body]).

   Worklist: claude-notes/projects/fs-syscall-specs.md, lane A item (iii),
   the user's 2026-08-28 ruling (OPTION (b)).

   THIS FILE IS A COPY-ADAPT, AND THAT IS THE RULED APPROACH.  It is
   [ProofNamexTr.v] with the FIRE SITES changed and NOTHING ELSE: same
   block decomposition, same callee contracts, same budget and ledger
   ...
   (1) THE TRACE ROW IS AT A DIFFERENT LEND.  [SpecNameiTr.nx_hops_from P
       Pmiss pl k] becomes [FsAbsEra.ex_hops_from fsc_fs P Pmiss pl k] in
       the two loop invariants ([nx_loop_body], [nx_rest_body]) and in the
       contract.  Both ARE [FsAbs.ax_hops_from] at a different [F]
       ([FsAbsEra.ex_hops_is_ax_hops]), so the peel, the index shift and
       the failure arms' refunds are unchanged as PROOFS as well as
       statements.

   (2) THE THREE GHOST LEMMAS MOVED OUT.  [ProofNamexTr]'s
   ...
   (3) THE FIRE LENDS THE ERA LEG, NOT THE CONTENTS.  At dirlookup's
       ...
       conjuncts as one hypothesis [Hfview] = [fv_ride * top_frag] (the
       flip IcacheEscrow made at durable-disk 2b-inode-3).  The frozen
       proof fires off [Hdview] ([dv_ride], the N-1 carrier) and casts on
       its two custody fractions; this one splits [Hfview], fires off the
       [top_frag] half [Htop], and re-frames [Hfv Htop] where the frozen
       proof re-frames [Hfview].  [Hdview] is never touched -- it goes
       back into the re-packed [ic_loaded] untouched, which is why the two
       [ic_loaded_flat] rebuilds below are otherwise identical.

       There is no custody case split here: namex's [ilock] takes the
       WRITE arm, so the era leg is at [DfracOwn 1] whole, and
       [FsAbsEra.elend_fire_hit] splits it in half (lend half, keep half)
       so the node the caller returns is pinned to the one it lent.

   (4) ONE NEW PURE FACT, [Htydz] : [bv_unsigned (di_type dnl) = T_DIR_z],
       one line off the [Htyd] the walk already derived from its own
       [ip->type == T_DIR] test sixty lines up.  It is what lets the lend
       carry DIRECTORY-NESS, which is what lets it discharge
       [FsAbs.lend_agrees] where [dv_half] could only reach
       [FsAbs.lend_reads] ([FsAbsSeam], finding 2).

   THE RELATIVE START IS NOW PROVED (lane A-iii, 2026-08-28), so the arm
   count is TWO dead, not three: [a1 = 0] still kills [L_par] and the
   nameiparent tail of [L_done], but the [pfun 0 = SLASH] premise is gone
   and the myproc / idup arm is back, restored from [ProofNamex]'s own
   text.  What it needed was not a reading of the cwd but a trace that is
   PARAMETRIC in the start ([FsAbsStart.ex_start]): the walk opens idup's
   returned package, reads the slot's inum off the witness, and fires the
   caller's one shot there.  The absolute arm fires the same premise at
   ROOTINO and is otherwise unchanged, line for line.  The NAMEIPARENT
   side landed separately ([ProofNparEra], the same walk at [a1 <> 0] with
   the family re-indexed to the parent prefix) and takes the relative
   start the same way.

   The block decomposition, the [Htail] / [Hloop] / [Hrest] / [Hdlblk]
   assertions and the performance rules behind them are [ProofNamex.v]'s --
   read that file's header first; nothing here restates it. *)
...
Require Import FsTree.        (* [fname], [dir_view], the [dv_lookup_*] family *)
...
(* ===================================================================== *)
(*  THE TRACE'S THREE LEMMAS ARE NOT HERE.                                *)
(*                                                                        *)
(*  [ProofNamexTr] proves [nxt_hops_cons] / [nxt_hop_hit] /               *)
(*  [nxt_hop_miss] at the top of the file, as its only new proof content. *)
(*  The era walk's three are [FsAbsEra.ex_hops_cons] /                    *)
(*  [FsAbsEra.elend_fire_hit] / [FsAbsEra.elend_fire_miss] and they live  *)
(*  in the spec layer, because the lend has clients that need them        *)
(*  without this file's cone.  Nothing else about the ghost side of the   *)
(*  walk differs; see the header's item (3).                              *)
(* ===================================================================== *)

Module NamexEraProof (MP : MYPROC) (ID : IDUP) (IG : IGET) (MM : MEMMOVE)
                    (IL : ILOCK) (IU : IUNLOCK) (IUP : IUNLOCKPUT)
                    (DL : DIRLOOKUP) (IP : IPUT) : NAMEX_ERA.
...
Section ProofNamexTrMain.
  ...
  Definition nx_loop_body
    ...
    (* THE CURSOR RIDES THE INVARIANT (namei-pinned-lookup.md §4).  [dcur]
       is the INUM the walk currently stands at -- exposed by
       [inode_held_at] rather than hidden inside [inode_held]'s existential
       -- and [length es0] is the number of hops already fired, so the two
       trace rows are [P (length es0) dcur] and the UNFIRED suffix
       [ex_hops_from fsc_fs P Pmiss pl (length es0)].  Every exit reads its own
       postcondition arm straight off that pair. *)
    (∀ (off : nat) (ipv : mword 64) (Ml : regfile) (ncur : nat)
     ...
     (* the set form beside the transaction's token: this walk's [ilock]
        takes the write arm (durable-disk B''-tx) *)
     log_opSt icfg_log ncur Scur -∗
     P (length es0) dcur -∗
     ex_hops_from fsc_fs P Pmiss pl (length es0) -∗
  ...
  (* THE WALK, AT THE TRACE CONTRACT.  Instruction for instruction the
     ...
     two trace rows and the five exits that read them.  The three arms the
     ruling kills -- the relative start, [L_par], and the nameiparent tail
     of [L_done] -- are refuted from the [pfun 0 = SLASH] / [a1 = 0]
 ...
 plen pfun nfun n Sb P Pmiss
  ...
  Proof.
    ...
    intros pcE pjv pv nb ret_tgt pl L
           ...
           Hbmaplog Hinos0 Hcovb Hiregb Hcstr Hplen Hbud Hj Hgs Ha1
           Hbelow.
    ...
    iIntros "Hcg Hcnt Hextc Hclmc #Htext #Hkd Hpc #Hpenv #Hbio #Hlogc #Hkenv #Hitb2 #Hitbl
              ...
              #Hbits Hppid Hcwdr Hpath Hname Hbslot Hislot [Hlog Htx] Htr Hcont".
      ...
      - iIntros (CIDl Hsl off ipv Ml ncur Scur es0 nf wc dcur)
        ...
        + (* ================= EXIT A: nothing but separators left ======= *)
          ...
          * (* ---- the +0x140 tail ---- *)
            ...
            rewrite Hlr in HbC.
            (* THE WALK IS DONE: the suffix has no elements left, so the
               cursor's index IS [L] and the pin the contract promises is
               [P L dcur] at the reference the loop still holds. *)
            assert (HLk : length (path_elems pl) = length es0).
            { rewrite Hes0 Hpel app_nil_r. reflexivity. }
...
"[%] [%] Hcg Hcnt Hextc Hclmc Hpc
               ...
               ++ iIntros (CIDf Hsf mf) "%Hcsf %Hfa0 Hcg Hcnt Hextc Hclmc Hpc".
                  ...
                  ** (* THE PIN, at the inum the walk stands on *)
                     iExists dcur.
                     iSplitR; [iPureIntro; exact Hfa0 |].
                     rewrite HLk. iFrame "Hip HP Hisl".
 ...
 pidv dq dqb dqs dqpv
               ...
               { iIntros (CIDt Hst Mt nf') "%Hregt %Hviewt Hcg Hcnt Hextc Hclmc Hpc
                 ...
                 - iIntros (CIDr Hsr o2 Mr) "%J1 %J2 %J3 %J4 %J5 %J6
                   ...
                   rewrite Hlrs in HbC.
                   (* ---- THE LEVEL'S THREE TRACE FACTS ----
                      [Hkltl]: the death index is inside the path -- this
                      level runs, so an element is left, which is exactly the
                      [k < L] the failure arms owe.
                      [Hdrop]: the hop family's head at [length es0] IS the
                      name the memmove just wrote.  This is the whole content
                      of "the hop names are the elements themselves"
                      (SpecNameiTr.v's header): [Hviewt] is
                      [PathElems.skipelem_name_view] at BOTH memmove shapes,
                      and [Hpe2] is the walk's own [path_elems] step.
                      [Hlen1]: the back edge's index arithmetic. *)
                   assert (Hkltl : (length es0 < length (path_elems pl))%nat).
                   { rewrite Hes0 length_app Hlrs. lia. }
                   assert (Hdrop : drop (length es0) (path_elems pl)
                            = bname 14 nf' :: path_elems (drop o2 pl)).
                   { rewrite Hes0 Hpe2 Hviewt drop_app_length. reflexivity. }
                   assert (Hlen1 : length (es0 ++ [take 14
                              (bview (e - a)%nat (fun i => pfun (a + i)%nat))])
                            = S (length es0)).
                   ...
                   assert (Hib' : bv_unsigned iinum < 16 * Z.of_nat icfg_nib)
                     by (exact Hib).
                   destruct (Hiregb iinum Hib') as [Hibc Hibl].
                   iDestruct (nx_esc_acc ik Hik
                                with "Hesc") as "#Hesck".
 ...
 gilk gislk
                               ...
                               with "Hcg Hcnt Hextc Hclmc Htext Hkd Hpc Hpenv Hbio Hlogc
                                     ...
                                     [%] Hflkp Hclaimsnx Hdep Hoffd Hidev Hiinum Hivalid Hload
                     ...
                     * iIntros (CIDf Hsf mf) "%Hcsf %Hfa0 Hcg Hcnt Hextc Hclmc Hpc".
                       ...
                       ** (* THE LEFT DISJUNCT.  [L_nlink] runs AT the guard,
                             two instructions before dirlookup, so hop
                             [length es0] never fired: the caller gets its
                             cursor back beside the WHOLE unfired suffix. *)
                          iSplitR; [iPureIntro; exact Hfa0 |].
                          iSplitL "Hisl"; [iExact "Hisl" |].
                          iExists (length es0), dcur.
                          iSplitR; [iPureIntro; exact Hkltl |].
                          iLeft. iFrame "HP Hhops".
                     }
                     ...
                     iApply fupd_wp.
                     iMod (InodeRegion.ireg_obs_mint ⊤ fsc_ireg fsc_fs icfg_ist icfg_nib
                             iinum dnl icfg_log enx ltac:(solve_ndisj) Hib' eq_refl
                             (nx_nlink_nz _ Hnl0)
                             with "Hireg Hdiat Hlbnx") as "[Hdiat #Hobs]".
                     iModIntro.
                     (* the [crz] premise the two credited sites present:
                        the observation alone.  The region's two ambient
                        ties used to ride beside it and are [True] now. *)
                     iAssert (nlz_obs (bv_unsigned iinum) enx)%I as "#Hcrz".
                     { iFrame "Hobs". }
                     ...
                     (* ---- everything else reaches dirlookup at +0xde ---- *)
                       ...
                       assert (Hbwf : blkmap_wf fsc_cov fsc_logst bml)
                       ...
                       assert (Hdio : dir_inums_ok datl
                                 (dir_nrec (bv_unsigned (di_size dnl))) icfg_nib).
                       {
 ...
 (ientry ik) iinum
                         ...
                         (* ================== THE FIRE POINT ==================
                            ...
                            legal exactly because nothing of the walk's is open
                            here (the same move the nlink guard's
                            [ireg_obs_mint] makes sixty lines up).  The bridge
                         ...
                         iDestruct (ex_hops_cons fsc_fs P Pmiss pl (length es0)
                                      (bname 14 nf') (path_elems (drop o2 pl))
                                      Hdrop with "Hhops") as "[Hhop Hhops]".
                         (* THE ERA LEG, OUT OF THE PAYLOAD'S LAST CONJUNCT.
                            [Hfview] is [fv_ride * top_frag] (the flip
                            IcacheEscrow made at durable-disk 2b-inode-3);
                            its second half is what THIS walk lends where the
                         ...
                         assert (Htydz : bv_unsigned (di_type dnl) = T_DIR_z)
                           by (rewrite Htyd; vm_compute; reflexivity).
                         destruct found.
                         - (* ============ FOUND: recurse on the child ==== *)
                           ...
                           { apply (dv_lookup_found _ datl
                                      ...
                                      _ kdir); [reflexivity | exact Hsome]. }
                           iApply fupd_wp.
                           iMod (elend_fire_hit fsc_fs fsc_cov fsc_logst
                                   P Pmiss (length es0)
                           ...
                           assert (Hcinb : bv_unsigned
                                     (zero_extend' 32
                                      ...
                                      : mword 32) < 16 * Z.of_nat icfg_nib).
                           { rewrite (dlk_zext32_unsigned
                                        (dir_inum datl kdir)).
                             exact (Hdio kdir Hklt Hklive). }
                           iAssert (inode_held_at (ientry kslot)
                                      (bv_unsigned (dir_inum datl kdir)))
                             with "[Href2 Hru2]" as "Hip2".
                           { rewrite /inode_held_at.
                             iExists kslot, qq,
                               (zero_extend' 32
                                  (dir_inum datl kdir : mword 16)).
                             iSplitR; [done |].
 ...
 ik (iq/2)%Qp (iq/2)%Qp gkp lokp tlkp
                                     ...
                                     with "Hcg Hcnt Hextc Hclmc Htext Hkd Hpc Hpenv Hbio
                                           ...
                                           Hslkk Hslkd [%] Hflkp Hclaimsnx Hdep Hoffd Hidev
                           ...
                           iApply ("IHl" $! o2 (ientry kslot) GB4 nup Sup
                                     ...
                                     with "[%] [%] [%] [%] [%] [%] [%] [%]
                                           ...
                                           Hbmap Hinos Hppid Hcwdr Hpath Hname Hbslot [$Hlog $Htx]
                                           HP Hhops Hcont").
                           -- lia.
                           -- exact J2.
                           -- exact Hes2.
                         ...
                         - (* ============ MISS: iunlockput and return 0 === *)
                           ...
                           { apply (dv_lookup_none _ datl
                                      (dir_nrec (bv_unsigned (di_size dnl))));
                               [reflexivity | exact Hnone]. }
                           iApply fupd_wp.
                           iMod (elend_fire_miss fsc_fs fsc_cov fsc_logst
                                   P Pmiss (length es0)
                                   (bname 14 nf') (bv_unsigned iinum)
                                   dnl bml datl
                                   Hiok Htydz (nx_nlink_nz _ Hnl0) Hents
 ...
 ik (iq/2)%Qp (iq/2)%Qp gkp lokp tlkp
                           ...
                           -- iIntros (CIDf Hsf mf) "%Hcsf %Hfa0 Hcg Hcnt Hextc Hclmc Hpc".
                              ...
                              ++ exact (nx_wi_spend n ncur nup wc wup true
                                          HbA HbB Hwupc
                                          (proj1 Hbdup) (proj2 Hbdup)).
                              ++ (* THE RIGHT DISJUNCT.  Hop [length es0] DID
                                    fire, and the name was absent: the caller
                                    holds the [Pmiss] receipt its own step
                                    produced, beside the suffix from
                                    [S (length es0)]. *)
                                 iSplitR; [iPureIntro; exact Hfa0 |].
                                 iSplitL "Hisl"; [iExact "Hisl" |].
                                 iExists (length es0), (bv_unsigned iinum).
                                 iSplitR; [iPureIntro; exact Hkltl |].
                                 iRight. iFrame "HP Hhops". }
 ...
 gilk gislk
                     ...
                     * iIntros (CIDf Hsf mf) "%Hcsf %Hfa0 Hcg Hcnt Hextc Hclmc Hpc".
                       ...
                       ** exact (nx_wi_spend n ncur nup wc wup false
                                   HbA HbB Hwupc (proj1 Hbdup) (proj2 Hbdup)).
                       ** (* THE LEFT DISJUNCT.  [L_notdir] runs before the
                             nlink guard and long before dirlookup, so hop
                             [length es0] never fired: cursor and whole
                             suffix go back to the caller. *)
                          iSplitR; [iPureIntro; exact Hfa0 |].
                          iSplitL "Hisl"; [iExact "Hisl" |].
                          iExists (length es0), dcur.
                          iSplitR; [iPureIntro; exact Hkltl |].
               ...
               ++ (* ================ LONG: 14 <= len ================= *)
                  ...
                  assert (Htgmm : add_vec
                            ...
                            (sign_extend' 64 (mword_of_int 2085732 : mword 21))
                          = mword_of_int KernelSyms.memmove) by pcw.
                  iApply (wp_jal_s_sconf (mword_of_int (NX + 0xa8)) Rra
                            (mword_of_int 2085732 : mword 21) T3
                            (K - 12)%nat b
                            ltac:(nz) ltac:(rdok)
                            ltac:(vm_compute; reflexivity)
                            with "Hcg Hpc []").
                  ...
                  iApply ("Hrest" $! T5 (fun jj => pfun (a + jj)%nat)
                            ...
                            Hb1 Hb2 Hb3 Hb4 Hb5 Hb6 Hb7 Hb8 Hb9 Hb10 Hb11 Hb12
                            Hip Hisl Hbmap Hinos Hppid Hcwdr
                            Hpath Hdst Hbslot [$Hlog $Htx] HP Hhops").
                  ** exact HT5regs.
                  ** exact Hview. }
    ...
    + (* ---------------- THE ABSOLUTE ARM: iget(ROOTDEV, ROOTINO) ------- *)
      ...
      iAssert (iname fsc_ireg fsc_fs icfg_ist ROOTINO RootL) as "Hlicr";
      ...
      iAssert (inode_held_at (ientry kig) (bv_unsigned ROOTINO))
        with "[Href Hru]" as "Hip".
      ...
      iDestruct (wp_next_shift (b := true) (CIDa := CID23) (CIDb := CIDK5)
                   ltac:(wp_next_chain) with "Hcont") as "Hcont".
      (* THE ONE SHOT, FIRED AT THE ROOT.  [FsAbsStart.ex_start] is
         universally quantified over the start inum; this arm's is
         ROOTINO, and the tie ([the path begins with SLASH] -> [r =
         ROOTINO]) is discharged by [reflexivity] because [r] IS ROOTINO
         here.  The relative arm below fires the same premise at idup's
         own inum, where the implication is refuted instead. *)
      iApply fupd_wp.
      ...
      iApply ("Hloop" $! 0%nat (ientry kig) A8 n Sb [] nfun false
                ...
                with "[%] [%] [%] [%] [%] [%] [%] [%] [%] [%] Hcg Hcnt Hextc Hclmc Hpc
                      ...
                      Hname Hbslot [$Hlog $Htx] HP0 Hhops0 Hcont").
      - lia.
      - lia.
    ...
    + (* ------- THE RELATIVE ARM: idup(myproc()->cwd) (lane A-iii) ------
         RESTORED from [ProofNamex.wp_namex_gen]'s own arm, instruction for
         instruction; the era proof used to REFUTE it from [pfun 0 = SLASH]
         and that premise is gone.  Only two things are new, and both are
         at the join: the starting reference is read AT ITS INUM out of
         idup's package, and the caller's deferred trace is fired there. *)
      ...
      assert (Htgtmp : add_vec (mword_of_int (NX + 0x2e) : mword 64)
                         (sign_extend' 64 (mword_of_int 2088926 : mword 21))
                       = mword_of_int KernelSyms.myproc) by pcw.
      iApply (wp_jal_s_sconf (mword_of_int (NX + 0x2e)) Rra
                (mword_of_int 2088926 : mword 21) R7 (K - 12)%nat b
                ltac:(nz) ltac:(rdok) ltac:(vm_compute; reflexivity)
                with "Hcg Hpc []").
      ...
      iDestruct (proc_priv_bare_cwd (proc_addr j) pidv Upr with "Hppid")
        as "[Hcwdc Hcwdbk]".
      iApply (wp_ld_s_sconf (kt := KT1) (ktd := KT0) (mword_of_int (NX + 0x32)) Ra0 Ra0
                (mword_of_int 336 : mword 12) mf1 (K - 12)%nat (pv_cwd (us_V Upr)) b
                (dqm := DfracOwn 1)
      ...
      rewrite us_cwd_id.
      pose (B2 := <[Regidx Ra0 := regval_into_reg (pv_cwd (us_V Upr))]> mf1).
      assert (HB2a0 : B2 !!! Regidx Ra0 = pv_cwd (us_V Upr))
      ...
      (* THE WALK'S STARTING REFERENCE, AND WHERE THE Q-c GAP CLOSES.
         idup's second package hides its inum -- but the walk is the one
         holding it, so it OPENS the existential right here and reads the
         slot's inum off the witness.  [inode_held_at] is that package
      ...
      iDestruct "Hip0" as (ik iq iinum) "(%Hipe & %Hikl & %Hiinb & %Hiipos & %Hiinz & Hirefp)".
      iAssert (inode_held_at (ientry ck) (bv_unsigned iinum))
        with "[Hirefp]" as "Hip".
      { rewrite /inode_held_at. iExists ik, iq, iinum.
        iSplitR; [iPureIntro; exact Hipe |].
        iSplitR; [iPureIntro; exact Hikl |].
        iSplitR; [iPureIntro; exact Hiinb |].
        iSplitR; [iPureIntro; exact Hiipos |].
        iSplitR; [iPureIntro; reflexivity |]. iExact "Hirefp". }
      (* THE ONE SHOT, FIRED AT THE CWD.  The tie is REFUTED on this arm
         rather than used: [pfun 0 <> SLASH] and the buffer's head IS
         [pfun 0] ([FsAbsStart.bview_head_slash]), so the caller's ROOTINO
