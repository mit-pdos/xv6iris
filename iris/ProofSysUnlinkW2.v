(* ProofSysUnlinkW2.v -- the unlink AU walk's BLOCK W2: ilock(dp), the
   two namecmp refusals, dirlookup(dp, name, &off).

     W2  +0x30 .. +0x6e   (ARM C at +0x44, ARM C' at +0x58, ARM D at +0x6e)

   [ProofSysUnlinkPure.su_w2]'s copy-adapt, at [SysUnlinkDefs]'s contract.
...
Module SysUnlinkW2 (Ilock : ILOCK) (Namecmp : NAMECMP)
                     (Dirlookup : DIRLOOKUP)
...
Section ProofSysUnlinkW2.
 ...
 kk qi s gy loy tly inum dn bm u pidv (DfracOwn (1/4)) dqb dqs
  ...
  (*                                                                     *)
  (*    +0x30 jal ilock                                                  *)
  (*    +0x34 auipc a1,2 ; +0x38 addi a1,a1,1656   ("." @ 0x800075d0)    *)
  (*    +0x3c addi a0,s0,-80 ; +0x40 jal namecmp                          *)
  (*    +0x44 beq a0,x0 -> +0x15a                        [ARM C]         *)
  (*    +0x48 auipc a1,2 ; +0x4c addi a1,a1,1644   (".." @ 0x800075d8)   *)
 ...
 (dqb : dfrac) (dqs : dfrac)
  ...
  Proof.
    ...
    iApply (wp_addi4_s_sconf (CID := CID3) (mword_of_int (SU + 0x38)) Ra1 Ra1
              (mword_of_int 1516 : mword 12) R1 (K - 30)%nat b
              ltac:(nz) ltac:(rdok) with "Hcg Hpc []").
    { iApply (suli_038 with "Htext"). }
    iIntros (CID4 Hq4) "Hcg Hpc".
    set (R2 := <[Regidx Ra1 := regval_into_reg
                  (add_vec (R1 !!! Regidx Ra1)
                     (sign_extend' 64 (mword_of_int 1516 : mword 12)))]> R1).
