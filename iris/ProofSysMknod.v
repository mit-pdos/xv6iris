(* ProofSysMknod.v -- sys_mknod over the SIE-agnostic sconf world.
   ...
   ([ProcInv.proc_priv_pid]); the trapframe quarter AND the page go out to
...
Section ProofSysMknodFrame.
  ...
  Lemma mn_frame_carve (sp0 : mword 64) :
    ...
    (∃ w : mword 64, (pa_stk sp0 20) ↦₈[kt] w) ∗
    bytes_own (KTR := kt) (DfracOwn 1) (pa_stk sp0 18) 128.
  Proof.
    ...
    iDestruct (slotsn_bytes_own (KTR := kt) sp0 18 16 ltac:(lia)
  ...
  Lemma mn_frame_join (sp0 : mword 64) (w1 w2 w19 w20 : mword 64) :
    ...
    (pa_stk sp0 19) ↦₈[kt] w19 -∗ (pa_stk sp0 20) ↦₈[kt] w20 -∗
    bytes_own (KTR := kt) (DfracOwn 1) (pa_stk sp0 18) 128 -∗
    stack_own (KTR := kt) sp0 20.
  Proof.
    intro Hal. iIntros "H1 H2 H19 H20 Hb".
    change 128%nat with (8 * 16)%nat.
    iDestruct (bytes_own_slotsn (KTR := kt) sp0 18 16 ltac:(lia) Hal with "Hb") as "Hs".
    cbn [seq].
...
Section ProofSysMknodEpilogue.
  ...
  Lemma mn_epilogue `{GEN : GenId} `{CID0 : CpuId}
    ...
    (pa_stk sp0 19) ↦₈[kt] w19 -∗
    (pa_stk sp0 20) ↦₈[kt] w20 -∗
    ([∗ list] jj ∈ seq 0 128, pa_add (pa_stk sp0 18) jj ↦ₘ[kt] bf jj) -∗
  ...
  Proof.
    ...
    iPoseProof (smni_50 with "Htext") as "Hi50".
    iPoseProof (smni_52 with "Htext") as "Hi52".
    iPoseProof (smni_54 with "Htext") as "Hi54".
    iPoseProof (smni_56 with "Htext") as "Hi56".
...
Section ProofSysMknodM1Tail.
  ...
  Lemma mn_m1_tail `{GEN : GenId} `{CID0 : CpuId}
    ...
    (forall i, (i < 16)%nat ->
       is_aligned_paddr (Physaddr (pa_stk sp0 (18 - i)%nat)) 8 = true) ->
    sie_cap_gpr kt M (K - 20) b (proc_addr jx) -∗
  ...
  Proof.
    ...
    iPoseProof (smni_5c with "Htext") as "Hi5c".
    iPoseProof (smni_5e with "Htext") as "Hi5e".
    (* ===== +0x40 jal ra,end_op ===== *)
    iApply (wp_jal_s_sconf (CID := CID0) (mword_of_int (MN + 0x58)) Rra
              (mword_of_int 2091514 : mword 21) M (K - 20)%nat b
...
Section ProofSysMknodBody.
  ...
  Lemma wp_sys_mknod_sconf `{GEN : GenId} `{CID0 : CpuId}
    ...
    wp_sys_mknod_sconf_body kt gf ga gpr gs j gl gu gd gk pd pav pu bn g gfs gi
                            cn gtl cov logstart bmapstart inodestart nib
                            ninodes size dev used ns dqb dqs dqbs dqn v0 v1 v2
                            pid V m K eb b lks.
  Proof.
    ...
    iPoseProof (smni_00 with "Htext") as "Hi00".
    iPoseProof (smni_02 with "Htext") as "Hi02".
    iPoseProof (smni_04 with "Htext") as "Hi04".
    iPoseProof (smni_06 with "Htext") as "Hi06".
    iPoseProof (smni_08 with "Htext") as "Hi08".
    iPoseProof (smni_0c with "Htext") as "Hi0c".
    iPoseProof (smni_10 with "Htext") as "Hi10".
    iPoseProof (smni_12 with "Htext") as "Hi12".
    iPoseProof (smni_16 with "Htext") as "Hi16".
    iPoseProof (smni_1a with "Htext") as "Hi1a".
    iPoseProof (smni_1c with "Htext") as "Hi1c".
    iPoseProof (smni_20 with "Htext") as "Hi20".
    iPoseProof (smni_24 with "Htext") as "Hi24".
    iPoseProof (smni_28 with "Htext") as "Hi28".
    iPoseProof (smni_2a with "Htext") as "Hi2a".
    iPoseProof (smni_2e with "Htext") as "Hi2e".
    iPoseProof (smni_32 with "Htext") as "Hi32".
    iPoseProof (smni_36 with "Htext") as "Hi36".
    iPoseProof (smni_3a with "Htext") as "Hi3a".
    iPoseProof (smni_3c with "Htext") as "Hi3c".
    iPoseProof (smni_40 with "Htext") as "Hi40".
    iPoseProof (smni_44 with "Htext") as "Hi44".
    iPoseProof (smni_46 with "Htext") as "Hi46".
    iPoseProof (smni_4a with "Htext") as "Hi4a".
    iPoseProof (smni_4e with "Htext") as "Hi4e".
    (* ================= +0x00 c.addi16sp sp,-160 ================= *)
    iApply (wp_caddi16sp_push_s_sconf pcE (mword_of_int 54 : mword 6) m K 20 b
              ltac:(lia) (mn_push sp0) with "Hcg Hpc Hi00").
    ...
    iApply (wp_jal_s_sconf (CID := CID4) (mword_of_int (MN + 0x08)) Rra
              (mword_of_int 2091454 : mword 21) M2 (K - 20)%nat b
              ltac:(nz) ltac:(rdok) ltac:(vm_compute; reflexivity)
              with "Hcg Hpc Hi08").
    ...
    iDestruct (cpu_own_transport CID0 CID5 0 eb pj b ltac:(wp_next_chain)
                 with "Hown") as "Hown".
    iApply (BeginOp.wp_begin_op_sconf kt (CID := CID5) gs j gl bn g gfs cov logstart
    ...
    iApply (wp_jal_s_sconf (CID := CID8) (mword_of_int (MN + 0x12)) Rra
              (mword_of_int 2086328 : mword 21) M5 (K - 20)%nat b
              ltac:(nz) ltac:(rdok) ltac:(vm_compute; reflexivity)
              with "Hcg Hpc Hi12").
    ...
    assert (Hjai1 : add_vec (mword_of_int (MN + 0x12) : mword 64)
                      (sign_extend' 64 (mword_of_int 2086328 : mword 21))
    ...
    iDestruct (cpu_own_transport CID6 CID9 0 eb pj b ltac:(wp_next_chain)
                 with "Hown") as "Hown".
    iApply (Argint.wp_argint_sconf kt (CID := CID9) M6 (K - 20)%nat 0%nat eb pj
    ...
    iApply (wp_jal_s_sconf (CID := CID12) (mword_of_int (MN + 0x1c)) Rra
              (mword_of_int 2086318 : mword 21) M8 (K - 20)%nat b
              ltac:(nz) ltac:(rdok) ltac:(vm_compute; reflexivity)
              with "Hcg Hpc Hi1c").
    ...
    assert (Hjai2 : add_vec (mword_of_int (MN + 0x1c) : mword 64)
                      (sign_extend' 64 (mword_of_int 2086318 : mword 21))
                    = mword_of_int KernelSyms.argint) by pcw.
    ...
    iDestruct (cpu_own_transport CID10 CID13 0 eb pj b ltac:(wp_next_chain)
                 with "Hown") as "Hown".
    iApply (Argint.wp_argint_sconf kt (CID := CID13) M9 (K - 20)%nat 0%nat eb pj
    ...
    iApply (wp_jal_s_sconf (CID := CID17) (mword_of_int (MN + 0x2a)) Rra
              (mword_of_int 2086360 : mword 21) M12 (K - 20)%nat b
              ltac:(nz) ltac:(rdok) ltac:(vm_compute; reflexivity)
              with "Hcg Hpc Hi2a").
    ...
    assert (Hjas : add_vec (mword_of_int (MN + 0x2a) : mword 64)
                     (sign_extend' 64 (mword_of_int 2086360 : mword 21))
    ...
    iDestruct (cpu_own_transport CID14 CID18 0 eb pj b ltac:(wp_next_chain)
                 with "Hown") as "Hown".
    iApply (Argstr.wp_argstr_sconf kt (CID := CID18) ga gf M13 (K - 20)%nat 0%nat eb pj
    ...
    - (* ---- the string fetched: the [bltz] FALLS THROUGH ---- *)
      ...
      iDestruct (cpu_own_transport CID19 CID25 0 eb pj b
                   ltac:(wp_next_chain) with "Hown") as "Hown".
      iApply (Create.wp_create_sconf kt (CID := CID25) gs j gl gu gd gk pd pav pu
      ...
      iIntros (CID26 Hq26 mcr ok made kk qi ss gy inum dn bm un1 Sb1 ns1 used1)
      ...
      + (* ---------- create SUCCEEDED: the LOCKED inode ---------- *)
        ...
        iApply (wp_jal_s_sconf (CID := CID27) (mword_of_int (MN + 0x46)) Rra
                  (mword_of_int 2089322 : mword 21) mcr (K - 20)%nat b
                  ltac:(nz) ltac:(rdok) ltac:(vm_compute; reflexivity)
                  with "Hcg Hpc Hi46").
        ...
        assert (Hjiu : add_vec (mword_of_int (MN + 0x46) : mword 64)
                         (sign_extend' 64 (mword_of_int 2089322 : mword 21))
        ...
        iDestruct (cpu_own_transport CID26 CID28 0 eb pj b
                     ltac:(wp_next_chain) with "Hown") as "Hown".
        iApply (Iunlockput.wp_iunlockput_sconf kt (CID := CID28) gs j gl gu gd gk
        ...
        iApply (wp_jal_s_sconf (CID := CID29) (mword_of_int (MN + 0x4a)) Rra
                  (mword_of_int 2091528 : mword 21) miu (K - 20)%nat b
                  ltac:(nz) ltac:(rdok) ltac:(vm_compute; reflexivity)
                  with "Hcg Hpc Hi4a").
        ...
        assert (Hjeo : add_vec (mword_of_int (MN + 0x4a) : mword 64)
                         (sign_extend' 64 (mword_of_int 2091528 : mword 21))
                       = mword_of_int KernelSyms.end_op) by pcw.
        iEval (rewrite Hjeo) in "Hpc".
        ...
        iDestruct (cpu_own_transport CID29 CID30 0 eb pj b
                     ltac:(wp_next_chain) with "Hown") as "Hown".
        iApply (EndOp.wp_end_op_sconf kt (CID := CID30) gs j gl gu gd gk pd pav pu
        ...
        { rewrite Heb /trap_csrs_ext. done. }
        { rewrite Heb /cpu_claim_ext. done. }
        iIntros (CID31 Hq31 meo) "%Hcseo Hcg Hown _ _ Hpc Hpidq".
      ...
      + (* ---------- ARM B: create returned 0 ---------- *)
        ...
        iDestruct (cpu_own_transport CID26 CID27 0 eb pj b
                     ltac:(wp_next_chain) with "Hown") as "Hown".
        iApply (mn_m1_tail (CID0 := CID27) gs j gl gu gd gk pd pav pu bn g gfs
    ...
    - (* ================= ARM A: argstr returned -1 =================
      ...
      iDestruct (cpu_own_transport CID19 CID20 0 eb pj b
                   ltac:(wp_next_chain) with "Hown") as "Hown".
      iApply (mn_m1_tail (CID0 := CID20) gs j gl gu gd gk pd pav pu bn g gfs
