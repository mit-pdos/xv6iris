 ...
 ns dqb dqs dqbs dqn v0 v1 v2
  ...
  Proof using .
    ...
    iApply (wp_jal_s_sconf (CID := CID8) (mword_of_int (MN + 0x12)) Rra
              (mword_of_int 2086176 : mword 21) M5 (K - 20)%nat b
              ltac:(nz) ltac:(rdok) ltac:(vm_compute; reflexivity)
              with "Hcg Hpc []").
    ...
    assert (Hjai1 : add_vec (mword_of_int (MN + 0x12) : mword 64)
                      (sign_extend' 64 (mword_of_int 2086176 : mword 21))
    ...
    iApply (wp_jal_s_sconf (CID := CID12) (mword_of_int (MN + 0x1c)) Rra
              (mword_of_int 2086166 : mword 21) M8 (K - 20)%nat b
              ltac:(nz) ltac:(rdok) ltac:(vm_compute; reflexivity)
              with "Hcg Hpc []").
    ...
    assert (Hjai2 : add_vec (mword_of_int (MN + 0x1c) : mword 64)
                      (sign_extend' 64 (mword_of_int 2086166 : mword 21))
                    = mword_of_int KernelSyms.argint) by pcw.
    ...
    iApply (wp_jal_s_sconf (CID := CID17) (mword_of_int (MN + 0x2a)) Rra
              (mword_of_int 2086208 : mword 21) M12 (K - 20)%nat b
              ltac:(nz) ltac:(rdok) ltac:(vm_compute; reflexivity)
              with "Hcg Hpc []").
    ...
    assert (Hjas : add_vec (mword_of_int (MN + 0x2a) : mword 64)
                     (sign_extend' 64 (mword_of_int 2086208 : mword 21))
   ...
   un1 pid (DfracOwn (1/4))
                    ...
                    with "Hcg Hown [] [] Htext Hdata Hpc Hpe Hbio Hlog Hseam Hgen
                          ...
                          Hbuf
                          [Hpback Hbsl Hsbn Hsbi Hsbs Hsbb Hir Hcont Hcf]").
          { rewrite Heb /trap_csrs_ext. done. }
          { rewrite Heb /cpu_claim_ext. done. }
...
Section MknodStable.
  ...
  Lemma mkr_acre_forget Γ (E : coPset) (c : absnode) (Nm : fname -> Prop)
      (root : Z)
  ...
  Proof using .
    ...
    iIntros "Hcm".
    iApply (mkr_acre_compose Γ E c avc Nm root ps ds Pd Farm F.(pf_recv) Hr
              with "Hc Hcm").
  Qed.
  ...
  Lemma mkr_acre_forget_at Γ (E : coPset) (c : absnode) (Nm : fname -> Prop)
      (root : Z)
      (ps : list fname) (ds : list Z) (Pd : Z -> iProp Σ)
      (Farm : pfam Σ (aview -> Z -> iProp Σ))
      (F : pfam Σ (aview -> Z -> fname -> Z -> iProp Σ)) :
  ...
  Proof using .
    ...
    iApply (pf_at_mono_pair (acre_commit_at_nm Γ E c Nm Pd Farm)
              ...
              (mkr_fam root ps ds F) F eq_refl with "[] Hcm").
    iIntros "Hcm".
    iApply (mkr_acre_forget Γ E c Nm root ps ds Pd Farm F.(pf_recv) with "Hcm").
  Qed.
  ...
  Proof using .
    ...
    iSplitL "HΦ"; [iExact "HΦ" |]. iExact "Hun".
  Qed.
  ...
  Lemma mkr_fail_arm Γ (γfs : fs_names) (cw : Z)
    ...
    ⊢ mknod_stable_fail Γ ma mi (npar_nm M pv) root ps ds Farm Fun Fok Fex.
  Proof using .
    ...
    { iLeft. iSplitL "Hacre".
      - (* the bundle's commit is at the GUARDED cursor and the stable
        ...
        iApply (mknod_acre_triv_in Γ M pv ma mi (npar_nm M pv) Farm Fok).
  ...
  Lemma mkr_arms Γ (γfs : fs_names) (cw : Z)
    ...
    ⊢ mknod_stable_arms Γ ma mi (npar_nm M pv) root ps ds Farm Fun Fok Fex r.
  Proof using .
...
Section MknodStableWp.
  ...
  Proof using .
    ...
    { rewrite /mknod_au_at.
      ...
      iSplitL "Hacre".
      - iApply (mkr_acre_compose_at _ _ _ avc _ root ps ds _ _ _ Hrun
                  with "Hchain [Hacre]").
        iApply (mknod_acre_triv_out _ (us_M U) v0 _ _ _ Farm Fok with "Hacre").
      - iSplitL "Hdl".
