(* WpUserMemC2.v -- width-2 COMPRESSED memory USTEP arms (C_LH/C_LHU loads,
   C_SH stores; Zcb).  Each ExecuteAs-expands to the width-2 base LH/LHU/SH,
   over UmodeData2's exec_execute_{LOAD,STORE}_2_U; sign-generic loads. *)
...
Section WpUserMemC2.
  ...
  Proof.
    ...
    iApply (wp_instr_c_hit va vpn ie h ii (LOAD (imm, Regidx rs1, Regidx rd, is_unsigned, 2))
  ...
  Lemma ustep_c_sh
    ...
    (* the pure expansion to the base instruction *)
    (forall s : mstate, exec (execute ii) s
       = Some (ExecuteAs (STORE (imm, Regidx rs2, Regidx rs1, 2)), s)) ->
    spec !! vpnD = Some ieD ->
    vec_access_dec tlbvec (tlb_hash (__id 39) vpnD) = Some (upt_entry vpnD ieD) ->
  ...
  Proof.
    ...
    iDestruct "Hhwc" as (misa0 mseccfg0 pmar0 elp0)
      "(#Hmisa & #Hmseccfg & #Hpma & #Hhtif & #Help & %HmisaS & %HmisaC &
        %HmisaU & %HmisaM & %Hpma_all & %Hseccfg1 & %Hseccfg2 & %Help_np &
        %HmisaA & %Hmisa_val0 & %Hmseccfg_val0)".
    iApply (wp_instr_c_hit va vpn ie h ii (STORE (imm, Regidx rs2, Regidx rs1, 2))
              ms_v tlbvec E Φ HN Hvec Hchk0 HupdN Hpbmt0 HSXL Hcanon
              Hvpn_def Hmode HisRVC Hdec Hexp
              with "Hhw Hinv Hhs Hpriv Hms Htlbc Hpcr Hcode Hcfg").
    iIntros (σ Hpceq Hag Hpins) "[Hreg [Hmem Hdev]]".
