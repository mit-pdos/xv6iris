(* WpUserLrscMiss.v -- combined (fetch-hit x data-miss) LR/SC FAULT arms.
   A standalone spatial arm (like ustep_sw): AMOSWAP.W reads the old
   4-byte data-window value into rd AND writes the low 32 bits of rs2
   back, so it owns the target window and rebuilds user_data from the new
   window.  Rides the mode-neutral AMO tower (MemAmo4) via UmodeAmo4.    *)
...
Section WpUserLrscMiss.
  ...
  Notation sc_cause := (rv64d_types.Exception (E_SAMO_Access_Fault tt)).


  Lemma ustep_lr_fault_dm
  ...
  Proof.
    ...
    (* ==== DATA-side WALK (empty slot): translate walks + fills s_x -> s' ==== *)
    iDestruct (upt_walk_read_ptes root slots spec vpnD ieD s_x HsomeD
                 ...
                 ltac:(rewrite LpmpaX; exact Hpmp_cov)
                 Hpter
    ...
    (* s' register-lookup transports (only tlb differs from s_x) *)
    assert (LprivX' : register_lookup cur_privilege s'.(sregs) = User)
      by (unfold s', set_reg; cbn [sregs]; rewrite irrelevant_register_set; [ exact LprivX | vm_compute; reflexivity ]).
    assert (LmsX' : register_lookup mstatus s'.(sregs) = ms_v)
      by (unfold s', set_reg; cbn [sregs]; rewrite irrelevant_register_set; [ exact LmsX | vm_compute; reflexivity ]).
    assert (LscX' : register_lookup scause s'.(sregs) = sc_v)
      by (unfold s', set_reg; cbn [sregs]; rewrite irrelevant_register_set; [ exact LscX | vm_compute; reflexivity ]).
    assert (LstvecX' : register_lookup stvec s'.(sregs) = stvec_v)
      by (unfold s', set_reg; cbn [sregs]; rewrite irrelevant_register_set; [ exact LstvecX | vm_compute; reflexivity ]).
    assert (LelpX' : register_lookup elp s'.(sregs) = elp0)
      by (unfold s', set_reg; cbn [sregs]; rewrite irrelevant_register_set; [ exact LelpX | vm_compute; reflexivity ]).
    assert (LpcXx' : register_lookup PC s'.(sregs) = va)
      by (unfold s', set_reg; cbn [sregs]; rewrite irrelevant_register_set; [ exact LpcXx | vm_compute; reflexivity ]).
    assert (LmisaSX' : eq_vec (_get_Misa_S (register_lookup misa s'.(sregs))) ('b"1") = true)
      by (unfold s', set_reg; cbn [sregs]; rewrite irrelevant_register_set; [ exact LmisaSX | vm_compute; reflexivity ]).
    ...
    assert (LpmpcX' : register_lookup pmpcfg_n s'.(sregs) = pmpcfg0)
      by (unfold s', set_reg; cbn [sregs]; rewrite irrelevant_register_set; [ exact LpmpcX | vm_compute; reflexivity ]).
    assert (LpmpaX' : register_lookup pmpaddr_n s'.(sregs) = pmpaddr00)
      by (unfold s', set_reg; cbn [sregs]; rewrite irrelevant_register_set; [ exact LpmpaX | vm_compute; reflexivity ]).
    ...
    { unfold s'; lk. unfold s_x; lk. rewrite Lmedl. exact Hdel_loadaf. }
    (* the data translate WALK at s_x -> s' *)
    ...
    { rewrite Heaeq. rewrite <- Hpaw.
      exact (exec_translateAddr_loadres_walk_u vpnD root (uw_pte2 ieD) (uw_pte1 ieD) (uw_pte0 ieD)
               ...
               H2i H2nl H1i H1nl H0i H0nl HchkD H0N LmisaX LprivX HSXLX HMPRVX LsatpX Hsatpmode Hasid Hroot
               eq_refl ltac:(rewrite LtlbX; exact HvecD) HupdD Hrd2 Hrd1 Hrd0 LmenvX
               ltac:(vm_compute; reflexivity) HcanonD Hvpn_defD). }
    (* the EA transform (identity) at s_x -- pre-translate, unchanged *)
    ...
    (* the data pmp grant at s' (post-fill) *)
    ...
    (* ---- the faulting execute (WALK form): LOADRES -> Trap, s_x -> s' ---- *)
    ...
    assert (Hha : exec (run_hart_active 0) σ = Some (Step_Execute (resf, zero_extend' 32 w), s')).
    { apply (exec_hart_active_progress_base_gen User σ σ s' w
    ...
    assert (LmedlX'' : bit_to_bool (access_vec_dec (register_lookup medeleg s'.(sregs))
                       (uint (exceptionType_bits_forwards (E_Load_Access_Fault tt)))) = true)
      by exact LmedlX'.
    pose proof (exec_exception_handler_ne_M s' lr_cause User va
    ...
    (* ---- ghost updates: tlb fill (nextPC already ticked), then the trap-CSR tower ---- *)
    ...
    { unfold s_trap, s9x, s', s_x. lk. exact Lpc. }
    rewrite LpcT.
    iSplitL "Hpcr"; [iExact "Hpcr" |].
    iSplitL "Hreg Hmem Hdev".
    { rewrite /mstate_interp. unfold s_trap, s9x, s', s_x, set_reg; cbn [sregs mem mdev].
  ...
  Proof.
    ...
    assert (LpmaX' : register_lookup pma_regions s'.(sregs) = pmar0)
      by (unfold s', set_reg; cbn [sregs]; rewrite irrelevant_register_set; [ exact LpmaX | vm_compute; reflexivity ]).
    assert (LmedlX' : bit_to_bool (access_vec_dec (register_lookup medeleg s'.(sregs))
                       (uint (exceptionType_bits_forwards (E_SAMO_Access_Fault tt)))) = true).
    { unfold s'; lk. unfold s_x; lk. rewrite Lmedl. exact Hdel_samoaf. }
    (* the data translate WALK at s_x -> s' *)
    ...
    assert (Hexec_sc : exec (execute (STORECON (aq, rl, Regidx rs2, Regidx rs1, 4, Regidx rd))) s_x
                       = Some (resf, s')).
    ...
    assert (Hha : exec (run_hart_active 0) σ = Some (Step_Execute (resf, zero_extend' 32 w), s')).
    { apply (exec_hart_active_progress_base_gen User σ σ s' w
               (STORECON (aq, rl, Regidx rs2, Regidx rs1, 4, Regidx rd)) va resf
               Lpriv Hdisp Hfetch Hdec' Hlpad Hnlpad Lpc).
      - exact Hexec_sc.
      - exact I. }
    (* ---- the Trap arm of try_step_dispatch, at s' ---- *)
    assert (LmedlX'' : bit_to_bool (access_vec_dec (register_lookup medeleg s'.(sregs))
                       (uint (exceptionType_bits_forwards (E_SAMO_Access_Fault tt)))) = true)
      by exact LmedlX'.
    pose proof (exec_exception_handler_ne_M s' sc_cause User va
                  (xtval_exception_value (E_SAMO_Access_Fault tt) eaF)
                  ms_v sc_v stvec_v elp0
                  (or_introl eq_refl) LprivX' LmsX' LscX' LstvecX' LelpX' LmisaSX' Htvd
                  (make_sync_exception (E_SAMO_Access_Fault tt) eaF)
                  eq_refl eq_refl eq_refl LmedlX'') as Hehe.
    match type of Hehe with _ = Some (_, ?T) => set (s9x := T) in Hehe end.
    set (s_trap := set_reg s9x nextPC (stvec_base stvec_v)).
    assert (Hdispb : exec (try_step_dispatch (Step_Execute (resf, zero_extend' 32 w))) s'
    ...
    iMod (reg_update _ scause _
            (update_subrange_vec_dec sc_v (64 - 1) (64 - 1)
               (bool_to_bit (trapCause_is_interrupt sc_cause)))
            with "Hreg Hsc") as "[Hreg Hsc]".
    iMod (reg_update _ scause _ (utrap_scause sc_cause sc_v)
            with "Hreg Hsc") as "[Hreg Hsc]".
    iMod (reg_update _ mstatus _
            (update_subrange_vec_dec (update_subrange_vec_dec ms_v 23 23 elp0) 5 5
               (_get_Mstatus_SIE (update_subrange_vec_dec ms_v 23 23 elp0)))
    ...
    iMod (reg_update _ mstatus _ (utrap_ms ms_v elp0) with "Hreg Hms") as "[Hreg Hms]".
    iMod (reg_update _ stval _
            (tval (xtval_exception_value (E_SAMO_Access_Fault tt) eaF))
            with "Hreg Hstv") as "[Hreg Hstv]".
    iMod (reg_update _ sepc _ va with "Hreg Hsepc") as "[Hreg Hsepc]".
    iMod (reg_update _ cur_privilege _ Supervisor with "Hreg Hpriv") as "[Hreg Hpriv]".
    iMod (reg_update _ nextPC _ (stvec_base stvec_v) with "Hreg Hnpc") as "[Hreg Hnpc]".
  ...
  Lemma ustep_lr_fault_nomatch
      (va : mword 64) (vpn : mword 27) (ie : uwalk_info) (w : mword 32)
      (vpnD : mword 27) (ieD : uwalk_info) (entD : TLB_Entry)
      (rs1 rd : mword 5) (aq rl : bool)
      (ms_v sc_v stval_v sepc_v : mword 64)
      (g : gmap regidx (mword 64))
      (tlbvec : vec (option TLB_Entry) (2 ^ 6))
    ...
    (* data-side facts *)
    spec !! vpnD = Some ieD ->
    vec_access_dec tlbvec (tlb_hash (__id 39) vpnD) = Some entD ->
    match_TLB_Entry entD (mword_of_int 0 : mword 16) (sign_extend' (57 - 12) vpnD) = false ->
    uw_check_ok (LoadReserved Data) ieD ->
    update_PTE_Bits (uw_pte0 ieD) (LoadReserved Data) = None ->
    _get_PTE_Ext_PBMT (ext_bits_of_PTE (uw_pte0 ieD)) = ('b"00" : mword 2) ->
    is_aligned_vaddr (Virtaddr eaF) 4 = true ->
  ...
  Proof.
    intros eaF paD HN Hok Hvec Hchk0 HupdN Hpbmt0 Hcw HSXL HMPRV HMXR Hval Hcanon
           Hvpn_def Hpaal HnotRVC Hdec
           HsomeD HvecD HnmD HchkD HupdD HpbmtD HalignD HcanonD Hvpn_defD HpaalD
           HramD HramD3 Hresv Hdel_loadaf.
    iIntros "#Hhw #Hinv Hhs Hpriv Hms Hsc Hstv Hsepc Htlbc Hpc Hgpr Hupt
             #Hcode Hdata Hcfg Hcont".
    iPoseProof "Hhw" as "#Hhwc".
    ...
    { rewrite Heaeq. rewrite <- Hpaw.
      exact (exec_translateAddr_loadres_walk_u_nomatch entD vpnD root (uw_pte2 ieD) (uw_pte1 ieD) (uw_pte0 ieD)
               eaF satp0 MENVCFG_S (register_lookup tlb s_x.(sregs)) s_x
               H2i H2nl H1i H1nl H0i H0nl HchkD H0N LmisaX LprivX HSXLX HMPRVX LsatpX Hsatpmode Hasid Hroot
               eq_refl ltac:(rewrite LtlbX; exact HvecD) HnmD HupdD Hrd2 Hrd1 Hrd0 LmenvX
               ltac:(vm_compute; reflexivity) HcanonD Hvpn_defD). }
    (* the EA transform (identity) at s_x -- pre-translate, unchanged *)
    assert (Htea_x : exec (transform_effective_address
              (Virtaddr (add_vec (if Z.eqb (uint rs1) 0 then zero_reg
