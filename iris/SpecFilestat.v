...
Section SpecFilestat.
  ...
  Definition filestat_env (fn : fstat_names) (st : fdstate) : iProp Σ :=
    (match st with
     | FdOpen _ _ (FdInode _ _) | FdOpen _ _ (FdDevice _) => filestat_fs_env fn
     | _ => emp
     end)%I.

  Definition filestat_env_out (fn : fstat_names) (st : fdstate) : iProp Σ :=
    (match st with
     | FdOpen _ _ (FdInode _ _) | FdOpen _ _ (FdDevice _) => filestat_fs_out fn
     | _ => emp
     end)%I.
  ...
  Lemma filestat_env_none fn st :
    match st with
    | FdOpen _ _ (FdInode _ _) | FdOpen _ _ (FdDevice _) => False
    | _ => True
