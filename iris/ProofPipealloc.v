...
Section ProofPipealloc.
  ...
  (* THE [eb]-GUARD FROM THE [b]-GUARD.  The [trap_csrs_ext] / [cpu_claim_ext]
     transports ask for [eb = false \/ p = zero_reg -> ...] while every
     per-instruction chain fact is stated over [b], and pipealloc's contract
     does not pin [n = 0], so the usual [eb = b] is not available here.  It is
     not needed: [CpuOwn.cpu_own_eb_agree] gives
     [match n with O => eb | S _ => false end = b] at ANY level, and that
     already yields [eb = false -> b = false] -- at level 0 because the two
     indices agree, above it because [b] is [false] outright.  Weaken the
     guard with that and hand the [b]-spelled goal to [wp_next_chain].
     [Hbf] is the hypothesis of that type. *)
  Local Ltac ext_chain Hbf :=
    let Hd := fresh "Hd" in
    let Hdb := fresh "Hdb" in
    intros Hd;
    lazymatch type of Hbf with
    | _ = false -> ?bb = false =>
        lazymatch type of Hd with
        | _ \/ ?pp = ?zz =>
            assert (Hdb : bb = false \/ pp = zz)
              by (destruct Hd as [Hf | Hz];
                  [ left; exact (Hbf Hf) | right; exact Hz ]);
            clear Hd; revert Hdb; wp_next_chain
        end
    end.
  ...
  Proof.
    ...
    assert (Hbf : eb = false -> b = false)
      by (intros He; rewrite -Hbm; destruct n; [exact He | reflexivity]).
    ...
    assert (Hpp02 : add_vec_int (pcE : mword 64) 2 = mword_of_int (KernelSyms.pipealloc + 0x02))
    ...
    iApply (wp_caddi4spn_s_sconf (mword_of_int (KernelSyms.pipealloc + 0x0a)) (Cregidx (mword_of_int 0))
    ...
    assert (Hpp0c : add_vec_int (mword_of_int (KernelSyms.pipealloc + 0x0a) : mword 64) 2 = mword_of_int (KernelSyms.pipealloc + 0x0c))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp0c) in "Hpc".
    (* +0x0c c.mv s1,a0   (s1 := f0) *)
    iApply (wp_cmv_s_sconf (mword_of_int (KernelSyms.pipealloc + 0x0c)) Rs1 Ra0
    ...
    assert (Hpp0e : add_vec_int (mword_of_int (KernelSyms.pipealloc + 0x0c) : mword 64) 2 = mword_of_int (KernelSyms.pipealloc + 0x0e))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp0e) in "Hpc".
    (* +0x0e c.mv s4,a1   (s4 := f1) *)
    iApply (wp_cmv_s_sconf (mword_of_int (KernelSyms.pipealloc + 0x0e)) Rs4 Ra1
    ...
    { rewrite /R4 upd_ne; [| vm_compute; discriminate].
      ...
      rewrite /R2 upd_ne; [| vm_compute; discriminate].
      rewrite /R1 upd_ne; [reflexivity | vm_compute; discriminate]. }
    assert (Hpp10 : add_vec_int (mword_of_int (KernelSyms.pipealloc + 0x0e) : mword 64) 2 = mword_of_int (KernelSyms.pipealloc + 0x10))
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    (* THE THREE BLOCK CONTINUATIONS MOVE WITH pipealloc's OWN CROSSING, to
       the literal [true]: each of them is reached from a path that has
       crossed a fileclose, whose crossing is [true] and therefore carries no
       chain fact at [b = false].  Each also THREADS the complement -- takes
       it as an argument and hands it on -- rather than capturing it in the
       closure: a bundle built at the entry hart cannot transport a
       hart-indexed resource to the arbitrary hart it is later consumed at. *)
    set (EPI := (wp_next (CID0 := CID) true p (fun (CIDe : CpuId) =>
    ...
    { rewrite /EPI.
      iIntros (CIDe Hbe mj res) "(%Hjsp & %Hja0 & %Hjthr) Hcg Hpc Hcnt Hextc Hextm Hslots Hpost".
      ...
      iApply (wp_cldsp_s_sconf (mword_of_int (KernelSyms.pipealloc + 0xb8)) (mword_of_int 5 : mword 6) Rra
      ...
      assert (Hpbba : add_vec_int (mword_of_int (KernelSyms.pipealloc + 0xb8) : mword 64) 2 = mword_of_int (KernelSyms.pipealloc + 0xba))
        by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hpbba) in "Hpc".
      (* +0xba c.ldsp s0,32(sp) *)
      iApply (wp_cldsp_s_sconf (mword_of_int (KernelSyms.pipealloc + 0xba)) (mword_of_int 4 : mword 6) Rs0
      ...
      assert (Hpbbc : add_vec_int (mword_of_int (KernelSyms.pipealloc + 0xba) : mword 64) 2 = mword_of_int (KernelSyms.pipealloc + 0xbc))
        by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hpbbc) in "Hpc".
      (* +0xbc c.ldsp s1,24(sp) *)
      iApply (wp_cldsp_s_sconf (mword_of_int (KernelSyms.pipealloc + 0xbc)) (mword_of_int 3 : mword 6) Rs1
      ...
      assert (Hpbbe : add_vec_int (mword_of_int (KernelSyms.pipealloc + 0xbc) : mword 64) 2 = mword_of_int (KernelSyms.pipealloc + 0xbe))
        by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hpbbe) in "Hpc".
      (* +0xbe c.ldsp s4,0(sp) *)
      iApply (wp_cldsp_s_sconf (mword_of_int (KernelSyms.pipealloc + 0xbe)) (mword_of_int 0 : mword 6) Rs4
      ...
      assert (Hpbc0 : add_vec_int (mword_of_int (KernelSyms.pipealloc + 0xbe) : mword 64) 2 = mword_of_int (KernelSyms.pipealloc + 0xc0))
        by (apply bv_eq; vm_compute; reflexivity).
      ...
      iAssert (stack_own (KTR := kt) sp0 6) with "[Hr40 Hr32 Hr24 Hs4c Hs5c Hr0]" as "Hframe6".
      ...
      assert (Hpbc2 : add_vec_int (mword_of_int (KernelSyms.pipealloc + 0xc0) : mword 64) 2 = mword_of_int (KernelSyms.pipealloc + 0xc2))
        by (apply bv_eq; vm_compute; reflexivity).
      ...
      iDestruct (trap_csrs_ext_transport CIDe CIDf6 eb p ltac:(ext_chain Hbf)
                   with "Hextc") as "Hextc".
      iDestruct (cpu_claim_ext_transport CIDe CIDf6 eb p ltac:(ext_chain Hbf)
                   with "Hextm") as "Hextm".
      iSpecialize ("Hcont" $! CIDf6 with "[%]"); [wp_next_chain|].
      iApply ("Hcont" $! P5 with "Hcg Hcnt Hextc Hextm Hpc [%] [Hpost]").
    ...
    (* The TYPE conjunct is what the fileclose in the tail needs: the file
       was allocated and not yet typed, so [fileclose_env] is [emp] for it
    ...
    { iSplit; [iExact "Hepi"|]. rewrite /T8.
      ...
      iApply (wp_ld_s_sconf (kt := kt) (ktd := KT0) (mword_of_int (KernelSyms.pipealloc + 0xa8)) Ra5 Rs4 (mword_of_int 0 : mword 12)
                Mt (K - 6)%nat x b
                ltac:(vm_compute; discriminate) ltac:(rdok)
      ...
      assert (Hpaac : add_vec_int (mword_of_int (KernelSyms.pipealloc + 0xa8) : mword 64) 4 = mword_of_int (KernelSyms.pipealloc + 0xac))
        by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hpaac) in "Hpc".
      (* +0xac c.li a0,-1 *)
      iApply (wp_cli_s_sconf (mword_of_int (KernelSyms.pipealloc + 0xac)) Ra0 (mword_of_int 63 : mword 6)
      ...
      assert (Hpaae : add_vec_int (mword_of_int (KernelSyms.pipealloc + 0xac) : mword 64) 2 = mword_of_int (KernelSyms.pipealloc + 0xae))
        by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hpaae) in "Hpc".
      (* +0xae c.beqz a5 *)
      iDestruct "Hx" as "[[%Hx0 Hunit1] | Hx]".
      - (* *f1 holds 0: nothing else to close, straight to the epilogue *)
        iApply (wp_cbeqz_taken_s_sconf (mword_of_int (KernelSyms.pipealloc + 0xae)) (mword_of_int 5 : mword 8)
        ...
        iApply ("Hepi" $! U2 (mword_of_int (-1) : mword 64) with "[%] Hcg Hpc Hcnt Hextc Hextm Hslots [Hav Hunit0 Hunit1 Hcell0 Hcell1]").
      ...
      - (* *f1 holds a live file: close it too *)
        ...
        (* +0xb0 c.mv a0,a5 *)
        iApply (wp_cmv_s_sconf (mword_of_int (KernelSyms.pipealloc + 0xb0)) Ra0 Ra5
        ...
        assert (Hpab2 : add_vec_int (mword_of_int (KernelSyms.pipealloc + 0xb0) : mword 64) 2 = mword_of_int (KernelSyms.pipealloc + 0xb2))
          by (apply bv_eq; vm_compute; reflexivity).
        iEval (rewrite Hpab2) in "Hpc".
        (* +0xb2 jal ra,fileclose *)
        iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.pipealloc + 0xb2)) Rra (mword_of_int 0x1ffc2e : mword 21)
        ...
        assert (Htgtfc : add_vec (mword_of_int (KernelSyms.pipealloc + 0xb2) : mword 64)
                           (sign_extend' 64 (mword_of_int 0x1ffc2e : mword 21))
        ...
        (* fileclose THREADS the complement -- it is at the top level of its
           ...
           in beside [cpu_own] and comes back re-indexed. *)
        iDestruct (trap_csrs_ext_transport CIDt CIDt5 eb p ltac:(ext_chain Hbf)
                     with "Hextc") as "Hextc".
        iDestruct (cpu_claim_ext_transport CIDt CIDt5 eb p ltac:(ext_chain Hbf)
                     with "Hextm") as "Hextm".
        iApply (Fileclose.wp_fileclose_sconf kt γfl γf k1 1%Qp Cf1 inhabitant on (∅ : gset Z) U4 n eb p (K - 6)%nat b lks
                  ltac:(lia) Hnoffpos HU4a0 Hbelow
                  with "Hcg Hcnt Hextc Hextm Htext Hkdata Hpc Hftab Hpe Href1 []").
        all: try lkbelow.
        ...
        iIntros (CIDt6 Hst6 mr) "Hcg Hcnt Hextc Hextm Hpc %Hfcpins Hunit1 _".
        ...
        (* +0xb6 c.li a0,-1 *)
        iApply (wp_cli_s_sconf (mword_of_int (KernelSyms.pipealloc + 0xb6)) Ra0 (mword_of_int 63 : mword 6)
        ...
        iDestruct (trap_csrs_ext_transport CIDt6 CIDt7 eb p ltac:(ext_chain Hbf)
                     with "Hextc") as "Hextc".
        iDestruct (cpu_claim_ext_transport CIDt6 CIDt7 eb p ltac:(ext_chain Hbf)
                     with "Hextm") as "Hextm".
        iSpecialize ("Hepi" $! CIDt7 with "[%]"); [wp_next_chain|].
        iApply ("Hepi" $! U5 (mword_of_int (-1) : mword 64) with "[%] Hcg Hpc Hcnt Hextc Hextm Hslots [Hav Hunit0 Hunit1 Hcell0 Hcell1]").
        { split.
    ...
    { iSplit; [iDestruct "HK1" as "[$ _]"|].
      ...
      iIntros (CIDu Hbu Mt k0 Cf0) "(%Hk0ty & %Htsp & %Hts4 & %Hta0 & %Htthr) Hcg Hpc Hcnt Hextc Hextm Href0 Hslots Hcell0 Hcell1 Hav".
      (* +0xa4 jal ra,fileclose *)
      iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.pipealloc + 0xa4)) Rra (mword_of_int 0x1ffc3c : mword 21)
      ...
      assert (Htgtfc : add_vec (mword_of_int (KernelSyms.pipealloc + 0xa4) : mword 64)
                         (sign_extend' 64 (mword_of_int 0x1ffc3c : mword 21))
                       = mword_of_int KernelSyms.fileclose)
        by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Htgtfc) in "Hpc".
      ...
      iDestruct (cpu_claim_ext_transport CIDu CIDu1 eb p ltac:(ext_chain Hbf)
                   with "Hextm") as "Hextm".
      iApply (Fileclose.wp_fileclose_sconf kt γfl γf k0 1%Qp Cf0 inhabitant on (∅ : gset Z) V1 n eb p (K - 6)%nat b lks
                ltac:(lia) Hnoffpos HV1a0 Hbelow
                with "Hcg Hcnt Hextc Hextm Htext Hkdata Hpc Hftab Hpe Href0 []").
      all: try lkbelow.
      ...
      iIntros (CIDu2 Hsu2 mr) "Hcg Hcnt Hextc Hextm Hpc %Hfcpins Hunit0 _".
      ...
      iApply ("Ht8" $! mr with "[%] Hcg Hpc Hcnt Hextc Hextm Hslots Hcell0 Hunit0 Hcell1 Hav").
      split.
      { rewrite (callee_saved_lookup Hfcpins_cs csp_rs1 ltac:(vm_compute; reflexivity)).
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.pipealloc + 0x18)) Rra (mword_of_int 0x1ffc24 : mword 21)
    ...
    assert (Htgtfa : add_vec (mword_of_int (KernelSyms.pipealloc + 0x18) : mword 64)
                       (sign_extend' 64 (mword_of_int 0x1ffc24 : mword 21))
    ...
    iDestruct (cpu_own_transport CID CID11 n eb p b ltac:(wp_next_chain)
                 with "Hcnt") as "Hcnt".
    iApply (Filealloc.wp_filealloc_sconf kt γfl γf mA n eb p (K - 6)%nat b lks
    ...
    { (* PATH A: no file at all.  Both cells hold 0; jump to +0xa8. *)
      iApply (wp_cbeqz_taken_s_sconf (mword_of_int (KernelSyms.pipealloc + 0x1e)) (mword_of_int 69 : mword 8)
      ...
      (* filealloc does not mention the complement, so it rode along in the
         ...
         filealloc's own guard fact and every leaf step. *)
      iDestruct (trap_csrs_ext_transport CID CID14 eb p ltac:(ext_chain Hbf)
                   with "Hextc") as "Hextc".
      iDestruct (cpu_claim_ext_transport CID CID14 eb p ltac:(ext_chain Hbf)
                   with "Hextm") as "Hextm".
      iSpecialize ("Ht8" $! CID14 with "[%]"); [wp_next_chain|].
      iApply ("Ht8" $! mB with "[%] Hcg Hpc Hcnt Hextc Hextm [Hr16 Hr8] [Hc0] Hslota' [Hc1 Hslotb] Hav").
    ...
    (* ================================================================= *)
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.pipealloc + 0x20)) Rra (mword_of_int 0x1ffc1c : mword 21)
    ...
    assert (Htgtfa2 : add_vec (mword_of_int (KernelSyms.pipealloc + 0x20) : mword 64)
                        (sign_extend' 64 (mword_of_int 0x1ffc1c : mword 21))
                      = mword_of_int KernelSyms.filealloc)
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    iDestruct (cpu_own_transport CID12 CID15 n eb p b ltac:(wp_next_chain)
                 with "Hcnt") as "Hcnt".
    iApply (Filealloc.wp_filealloc_sconf kt γfl γf mC n eb p (K - 6)%nat b lks
              ltac:(lia) Hnoffpos ltac:(lkbelow)
    ...
    { (* PATH B: only the first file was taken.  Close it: jump to +0xa0. *)
      iApply (wp_cbeqz_taken_s_sconf (mword_of_int (KernelSyms.pipealloc + 0x28)) (mword_of_int 60 : mword 8)
      ...
      iApply (wp_cld_s_sconf (kt := kt) (ktd := KT0) (mword_of_int (KernelSyms.pipealloc + 0xa0)) Ra0 Rs1 (mword_of_int 0 : mword 12)
      ...
      assert (Hppa2 : add_vec_int (mword_of_int (KernelSyms.pipealloc + 0xa0) : mword 64) 2 = mword_of_int (KernelSyms.pipealloc + 0xa2))
        by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hppa2) in "Hpc".
      (* +0xa2 c.beqz a0 -- DEAD: a file slot's address is never null *)
      iApply (wp_cbeqz_fall_s_sconf (mword_of_int (KernelSyms.pipealloc + 0xa2)) (mword_of_int 17 : mword 8)
      ...
      iApply ("Ht4" $! B1 k0 Cf0 with "[%] Hcg Hpc Hcnt Hextc Hextm Href0 [Hr16 Hr8] [Hc0] [Hc1 Hslotb'] Hav").
    ...
    iApply (wp_csdsp_s_sconf (mword_of_int (KernelSyms.pipealloc + 0x2a)) (mword_of_int 2 : mword 6) Rs2
    ...
    (* +0x2c jal ra,kalloc *)
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.pipealloc + 0x2c)) Rra (mword_of_int 0x1fc6c2 : mword 21)
    ...
    assert (Htgtka : add_vec (mword_of_int (KernelSyms.pipealloc + 0x2c) : mword 64)
                       (sign_extend' 64 (mword_of_int 0x1fc6c2 : mword 21))
    ...
    iDestruct (cpu_own_transport CID16 CID20 n eb p b ltac:(wp_next_chain)
                 with "Hcnt") as "Hcnt".
    iApply (Kalloc.wp_kalloc_sconf kt γkl γk fl mE on n eb p (K - 6)%nat b lks
    ...
    (* +0x30 c.mv s2,a0 *)
    iApply (wp_cmv_s_sconf (mword_of_int (KernelSyms.pipealloc + 0x30)) Rs2 Ra0
    ...
    assert (Hpp32 : add_vec_int (mword_of_int (KernelSyms.pipealloc + 0x30) : mword 64) 2 = mword_of_int (KernelSyms.pipealloc + 0x32))
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    { (* PATH C: no page.  Reload *f0, restore s2, and close both files. *)
      iApply (wp_cbeqz_taken_s_sconf (mword_of_int (KernelSyms.pipealloc + 0x32)) (mword_of_int 49 : mword 8)
      ...
      iApply (wp_cld_s_sconf (kt := kt) (ktd := KT0) (mword_of_int (KernelSyms.pipealloc + 0x94)) Ra0 Rs1 (mword_of_int 0 : mword 12)
      ...
      assert (Hpp96 : add_vec_int (mword_of_int (KernelSyms.pipealloc + 0x94) : mword 64) 2 = mword_of_int (KernelSyms.pipealloc + 0x96))
        by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hpp96) in "Hpc".
      (* +0x96 c.beqz a0 -- DEAD *)
      iApply (wp_cbeqz_fall_s_sconf (mword_of_int (KernelSyms.pipealloc + 0x96)) (mword_of_int 3 : mword 8)
      ...
      iApply (wp_cldsp_s_sconf (mword_of_int (KernelSyms.pipealloc + 0x98)) (mword_of_int 2 : mword 6) Rs2
      ...
      assert (Hpp9a : add_vec_int (mword_of_int (KernelSyms.pipealloc + 0x98) : mword 64) 2 = mword_of_int (KernelSyms.pipealloc + 0x9a))
        by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hpp9a) in "Hpc".
      (* +0x9a c.j +0xa4 *)
      iApply (wp_cj_s_sconf (mword_of_int (KernelSyms.pipealloc + 0x9a))
      ...
      iDestruct (trap_csrs_ext_transport CID CID27 eb p ltac:(ext_chain Hbf)
                   with "Hextc") as "Hextc".
      iDestruct (cpu_claim_ext_transport CID CID27 eb p ltac:(ext_chain Hbf)
                   with "Hextm") as "Hextm".
      iSpecialize ("Ht4" $! CID27 with "[%]"); [wp_next_chain|].
      iApply ("Ht4" $! C2 k0 Cf0 with "[%] Hcg Hpc Hcnt Hextc Hextm Href0 [Hr16 Hr8] [Hc0] [Hc1 Href1] Hav").
    ...
    iApply (wp_cbeqz_fall_s_sconf (mword_of_int (KernelSyms.pipealloc + 0x32)) (mword_of_int 49 : mword 8)
              (Cregidx (mword_of_int 2)) Ra0 G1 (K - 6)%nat b
              ltac:(vm_compute; reflexivity) ltac:(vm_compute; discriminate)
    ...
    iApply (wp_csdsp_s_sconf (mword_of_int (KernelSyms.pipealloc + 0x34)) (mword_of_int 1 : mword 6) Rs3
    ...
    assert (Hpp36 : add_vec_int (mword_of_int (KernelSyms.pipealloc + 0x34) : mword 64) 2 = mword_of_int (KernelSyms.pipealloc + 0x36))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp36) in "Hpc".
    (* +0x36 c.li s3,1 *)
    iApply (wp_cli_s_sconf (mword_of_int (KernelSyms.pipealloc + 0x36)) Rs3 (mword_of_int 1 : mword 6)
    ...
    assert (Hpp3c : add_vec_int (mword_of_int (KernelSyms.pipealloc + 0x38) : mword 64) 4 = mword_of_int (KernelSyms.pipealloc + 0x3c))
      by (apply bv_eq; vm_compute; reflexivity).
    assert (Hpp40 : add_vec_int (mword_of_int (KernelSyms.pipealloc + 0x3c) : mword 64) 4 = mword_of_int (KernelSyms.pipealloc + 0x40))
      by (apply bv_eq; vm_compute; reflexivity).
    assert (Hpp44 : add_vec_int (mword_of_int (KernelSyms.pipealloc + 0x40) : mword 64) 4 = mword_of_int (KernelSyms.pipealloc + 0x44))
      by (apply bv_eq; vm_compute; reflexivity).
    assert (Hpp48 : add_vec_int (mword_of_int (KernelSyms.pipealloc + 0x44) : mword 64) 4 = mword_of_int (KernelSyms.pipealloc + 0x48))
      by (apply bv_eq; vm_compute; reflexivity).
    assert (Hpp38 : add_vec_int (mword_of_int (KernelSyms.pipealloc + 0x36) : mword 64) 2 = mword_of_int (KernelSyms.pipealloc + 0x38))
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (wp_auipc_s_sconf (mword_of_int (KernelSyms.pipealloc + 0x48)) Ra1 (mword_of_int 3 : mword 20)
    ...
    iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.pipealloc + 0x4c)) Ra1 Ra1 (mword_of_int 0x17e : mword 12)
    ...
    set (G4 := <[Regidx Ra1 := regval_into_reg
                  (add_vec (G3 !!! Regidx Ra1) (sign_extend' 64 (mword_of_int 382 : mword 12)))]> G3).
    ...
    assert (Hpp50 : add_vec_int (mword_of_int (KernelSyms.pipealloc + 0x4c) : mword 64) 4 = mword_of_int (KernelSyms.pipealloc + 0x50))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp50) in "Hpc".
    (* +0x50 jal ra,initlock *)
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.pipealloc + 0x50)) Rra (mword_of_int 0x1fc6f8 : mword 21)
    ...
    assert (Htgtil : add_vec (mword_of_int (KernelSyms.pipealloc + 0x50) : mword 64)
                       (sign_extend' 64 (mword_of_int 0x1fc6f8 : mword 21))
    ...
    iApply (Initlock.wp_initlock_sconf kt G5 vlock vname vcpu "pipe"%string (K - 6)%nat b p
    ...
    iApply (wp_cld_s_sconf (kt := kt) (ktd := KT0) (mword_of_int (KernelSyms.pipealloc + 0x54)) Ra5 Rs1 (mword_of_int 0 : mword 12)
    ...
    assert (Hst56 : add_vec_int (mword_of_int (KernelSyms.pipealloc + 0x54) : mword 64) 2 = mword_of_int (KernelSyms.pipealloc + 0x56))
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (wp_cld_s_sconf (kt := kt) (ktd := KT0) (mword_of_int (KernelSyms.pipealloc + 0x5a)) Ra5 Rs1 (mword_of_int 0 : mword 12)
    ...
    assert (Hst5c : add_vec_int (mword_of_int (KernelSyms.pipealloc + 0x5a) : mword 64) 2 = mword_of_int (KernelSyms.pipealloc + 0x5c))
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (wp_cld_s_sconf (kt := kt) (ktd := KT0) (mword_of_int (KernelSyms.pipealloc + 0x60)) Ra5 Rs1 (mword_of_int 0 : mword 12)
    ...
    assert (Hst62 : add_vec_int (mword_of_int (KernelSyms.pipealloc + 0x60) : mword 64) 2 = mword_of_int (KernelSyms.pipealloc + 0x62))
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (wp_cld_s_sconf (kt := kt) (ktd := KT0) (mword_of_int (KernelSyms.pipealloc + 0x66)) Ra5 Rs1 (mword_of_int 0 : mword 12)
    ...
    assert (Hst68 : add_vec_int (mword_of_int (KernelSyms.pipealloc + 0x66) : mword 64) 2 = mword_of_int (KernelSyms.pipealloc + 0x68))
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (wp_ld_s_sconf (kt := kt) (ktd := KT0) (mword_of_int (KernelSyms.pipealloc + 0x6c)) Ra5 Rs4 (mword_of_int 0 : mword 12)
    ...
    assert (Hst70 : add_vec_int (mword_of_int (KernelSyms.pipealloc + 0x6c) : mword 64) 4 = mword_of_int (KernelSyms.pipealloc + 0x70))
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (wp_ld_s_sconf (kt := kt) (ktd := KT0) (mword_of_int (KernelSyms.pipealloc + 0x74)) Ra5 Rs4 (mword_of_int 0 : mword 12)
    ...
    assert (Hst78 : add_vec_int (mword_of_int (KernelSyms.pipealloc + 0x74) : mword 64) 4 = mword_of_int (KernelSyms.pipealloc + 0x78))
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (wp_ld_s_sconf (kt := kt) (ktd := KT0) (mword_of_int (KernelSyms.pipealloc + 0x7c)) Ra5 Rs4 (mword_of_int 0 : mword 12)
    ...
    assert (Hst80 : add_vec_int (mword_of_int (KernelSyms.pipealloc + 0x7c) : mword 64) 4 = mword_of_int (KernelSyms.pipealloc + 0x80))
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (wp_ld_s_sconf (kt := kt) (ktd := KT0) (mword_of_int (KernelSyms.pipealloc + 0x84)) Ra5 Rs4 (mword_of_int 0 : mword 12)
    ...
    assert (Hst88 : add_vec_int (mword_of_int (KernelSyms.pipealloc + 0x84) : mword 64) 4 = mword_of_int (KernelSyms.pipealloc + 0x88))
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (wp_cli_s_sconf (mword_of_int (KernelSyms.pipealloc + 0x8c)) Ra0 (mword_of_int 0 : mword 6)
    ...
    assert (Hnx8e : add_vec_int (mword_of_int (KernelSyms.pipealloc + 0x8c) : mword 64) 2 = mword_of_int (KernelSyms.pipealloc + 0x8e))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hnx8e) in "Hpc".
    (* +0x8e c.ldsp s2,16(sp) *)
    iApply (wp_cldsp_s_sconf (mword_of_int (KernelSyms.pipealloc + 0x8e)) (mword_of_int 2 : mword 6) Rs2
    ...
    assert (Hnx90 : add_vec_int (mword_of_int (KernelSyms.pipealloc + 0x8e) : mword 64) 2 = mword_of_int (KernelSyms.pipealloc + 0x90))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hnx90) in "Hpc".
    (* +0x90 c.ldsp s3,8(sp) *)
    iApply (wp_cldsp_s_sconf (mword_of_int (KernelSyms.pipealloc + 0x90)) (mword_of_int 1 : mword 6) Rs3
    ...
    assert (Hnx92 : add_vec_int (mword_of_int (KernelSyms.pipealloc + 0x90) : mword 64) 2 = mword_of_int (KernelSyms.pipealloc + 0x92))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hnx92) in "Hpc".
    (* +0x92 c.j +0xb8 *)
    iApply (wp_cj_s_sconf (mword_of_int (KernelSyms.pipealloc + 0x92))
    ...
    iDestruct (trap_csrs_ext_transport CID CID53 eb p ltac:(ext_chain Hbf)
                 with "Hextc") as "Hextc".
    iDestruct (cpu_claim_ext_transport CID CID53 eb p ltac:(ext_chain Hbf)
                 with "Hextm") as "Hextm".
    iSpecialize ("Hepi" $! CID53 with "[%]"); [wp_next_chain|].
    iApply ("Hepi" $! O3 (zero_reg : mword 64) with "[%] Hcg Hpc Hcnt Hextc Hextm [Hr16 Hr8]").
