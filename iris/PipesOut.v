...
(* ===================================================================== *)
(*  1.  THE SINGLE-WRITER STEPS, over any content function and admission *)
(* ===================================================================== *)
Section pipes_writes.
  ...
  Proof using .
    intros Hn Hpin0 Hb.
    iIntros "#Hpin Ht #Hpslb #Hcslb #Hilb Hcl". rewrite !pecl'_gen.
    iDestruct "Hcl" as "[Hc | Hp]".
    - iAssert (gcW GP k tt) as "HW"; [done |].
      iMod (gcl_step_write PM GP tt GA k v P b ps0 cs0 tt I0 ho H Hn Hpin0 Hb
    ...
    - iDestruct "Hp" as (v2 w so r gb pre tm)
      ...
      rewrite -(pipes_st_tt so) in Hb.
      destruct (lm_write_stage_byte PM ps0 (gs_ps PM so) cs0 (gs_cs PM so)
                  (gs_state PM tt so) (gs_E PM so) (gs_w PM so) I0 P b
                  Hpsp Hpin0 Hcsp Hn HI0 HP Hb) as [HlenE _].
      destruct Hopen as (_ & Hop & _).
      destruct (lm_blk_open_cs PM tt so r pre Hop) as (Hq & Hrr & Hnn).
  ...
  Proof using .
    ...
    iDestruct "Hcl" as "[Hc | Hp]".
    - iAssert (gcW GP k tt) as "HW"; [done |].
      iMod (gcl_step_write_blk PM GP PB tt GA k v P a b ps0 cs0 tt I0 ho H
  ...
  Proof using .
    ...
    iDestruct "Hcl" as "[Hc | Hp]".
    - iAssert (gcW GP k tt) as "HW"; [done |].
      iMod (gcl_step_write_pro PM GP tt GA k v P a b ps0 cs0 tt I0 ho CH
    ...
    - iDestruct "Hp" as (v2 w so r gb pre tm)
      ...
      iDestruct (inp_lb_le with "Hdll Hilb") as %HI0dl.
      assert (HI0 : I0 `prefix_of` (snd <$> gs_E PM so)).
      { etrans; [exact HI0dl | exact (gcl_pure_o_dl_E PM tt _ _ _ _ _ _ Hopen2)]. }
      destruct Hopen2 as (_ & Hop & _).
      pose proof Hop as (_ & Hwpre' & Hne' & _).
      destruct (lm_blk_open_cs PM tt so r pre Hop) as (Hqq & Hrr & Hne0').
      assert (Hwne : (1 <= length (gs_w PM so))%nat).
      { rewrite Hwpre'. destruct pre; [by destruct (Hne' eq_refl) | cbn; lia]. }
      rewrite (pipes_st_tt so) /lm_pcount in HP.
      destruct (decide ((snd <$> gs_E PM so) = I0)) as [Heq | Hne2].
      ...
      + pose proof (lm_proc_stream_before PM (gs_ps PM so) (gs_cs PM so) tt I0
        ...
        pose proof (prefix_length _ _
          (lm_proc_stream_prefix PM ps0 (gs_ps PM so) cs0 (gs_cs PM so) tt I0
