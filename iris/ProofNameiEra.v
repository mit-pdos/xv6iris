(* ProofNameiEra.v -- namei at the ERA-FRAGMENT TRACE contract.

   Worklist: claude-notes/projects/fs-syscall-specs.md, lane A item (iii),
   the user's 2026-08-28 ruling (OPTION (b)).

   A COPY-ADAPT OF [ProofNameiTr.v], and the diff is the names alone.
   namei is

     struct inode* namei(char *path) { char name[DIRSIZ]; return namex(path, 0, name); }

   so this file threads the two trace premises into the call and hands the
   two changed postcondition arms back; it fires no hop and owns no
   fragment of any ghost, exactly as the frozen one does not.  The lend
   therefore does not appear in this proof AT ALL -- what changes is which
   module type the callee is taken at ([SpecNamexEra.NAMEX_ERA] instead of
   [SpecNamexEra.NAMEX_ERA]) and which family the relayed rows are spelled
   at ([FsAbsEra.ex_hops_from] instead of [SpecNameiTr.nx_hops_from]).
   Everything else, including the FRAME CARVE (the low two slots ARE
   [name[14]]) and every pure side condition, is [ProofNameiTr.v] and
   [ProofNamei.v] verbatim.

   The [pfun 0 = SLASH] premise is GONE (lane A-iii, the relative start)
   and the two trace premises are one, [FsAbsStart.ex_start]; both are
   namex's business and this file relays the survivor verbatim. *)
...
Module NameiEraProof (NX : NAMEX_ERA) : NAMEI_ERA.
...
Section ProofNameiTrMain.
  ...
  (* THE WRAPPER, AT THE TRACE CONTRACT.  [ProofNamei.wp_namei_gen]'s
     ...
     are threaded straight into [NX.wp_namex_era] and its two postcondition
 ...
 plen pfun n Sb P Pmiss
  ...
  Proof.
    ...
    iIntros "Hcg Hcnt Hextc Hclmc #Htext #Hkd Hpc #Hpenv #Hbio #Hlogc #Hkenv #Hitb2 #Hitbl
              ...
              #Hbits Hppid Hcwdr Hpath Hbslot Hislot Hlog Htr Hcont".
             ...
             then ∃ iL : Z,
                  ...
                  (∃ (k : nat) (d : Z), ⌜(k < L)%nat⌝ ∗
                     ((P k d ∗ ex_hops_from fsc_fs P Pmiss pl k) ∨
                      (Pmiss k d ∗ ex_hops_from fsc_fs P Pmiss pl (S k)))))%I
      with "[Hok]" as "Hok".
    { destruct ok.
