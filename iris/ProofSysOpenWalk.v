(* ProofSysOpenWalk.v -- THE else ARM (+0xdc .. +0xfa) AND ARMS B-FAIL /
   ...
   is peeled ([ProofSysOpenShared.so_flat]) and the commit reads the row
...
Module SysOpenWalk (Iunlock : IUNLOCK) (Iunlockput : IUNLOCKPUT)
...
Module Join := SysOpenJoin Iunlock Iunlockput EndOp Fileclose Itrunc
                             Filealloc Fdalloc.
Module Alloc := SysOpenAlloc Iunlock Iunlockput EndOp Fileclose Itrunc
                               Filealloc Fdalloc.
Module Tails := SysOpenTails Iunlock Iunlockput EndOp Fileclose.

Section ProofSysOpenWalk.
 ...
 inum pidv (DfracOwn (1/4)) dqs
    ...
    (* ---- THE TERMINAL OBSERVATION (SysOpenDefs item 2): fired the
