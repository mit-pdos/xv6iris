(* ProofFilewriteAU.v -- filewrite's ATOMIC-UPDATE arm: [ProofFilewrite.v]'s
   walk, on the inode arm only, with the per-chunk two-phase commit FUSED
   INTO THE ROW RETAG.  Seals [SpecFilewriteAU.FILEWRITE_AU].

   Worklist: claude-notes/projects/fs-syscall-specs.md, lane W.  A PARALLEL
   FORM beside [ProofFilewrite.FilewriteProof] -- R10: that file does not
   move, and [LinkFilewrite.v] keeps instantiating it.

   ==== WHAT IS THE SAME AND WHAT IS NOT ================================

   THE SAME: every instruction.  The walk below is [ProofFilewrite]'s, line
   for line, and the whole prefix of this file (the pure arithmetic, the
   register lemmas, the two small sections) is its prefix VERBATIM so that
   the two can be diffed.  [ProofFilewriteParts]'s blocks -- [fw_pro],
   [fw_epi], [fw_tail], [fw_rest6] -- are reused as they stand: they take an
   abstract continuation and never mention filewrite's post, which is
   exactly what makes an AU walk affordable (the sys_open AU lane's rule).

   NOT THE SAME, and there are only five differences:

   1. THE THREE NON-INODE ARMS ARE DEAD BY PREMISE.  [FILEWRITE_AU] pins
      [st = FdOpen rb true (FdInode i)], so [fdstate_ok] gives
      [fc_type Cf = FD_INODE] and [fc_writable Cf = 1]: the [f->writable]
      test at +0x04, the FD_PIPE compare at +0x20, the FD_DEVICE compare at
      +0x26 and the ELSE arm's [panic] are all refuted rather than walked.
      The functor therefore takes FIVE parameters, not eight -- Pipewrite,
      Consolewrite and Panic are gone, and with Consolewrite goes the ONE
      axiom [LinkFilewrite]'s cone rests on.

   2. THE LOOP CARRIES [SpecFilewriteAU.fw_au_raw].  [fw_loop] gains the
      chunk index [p], the fired total [t], the entry tie
      [t = iz /\ t = FW_MAX * p], and the chain itself (at [x = 0]: no
      partial arm spent on any entry); its continuation drops
   ...
   3. THE FIRE REPLACES THE RETAG, AND MOVES THE OFFSET'S HALF.
      ...
      return and [iunlock], BEFORE the cell's checkin, which re-forms the
   ...
   5. [fw_test]'S POST IS ONE CONJUNCT WIDER: [c = nz - iz \/ c = FW_MAX],
      ...
      values) and what the back edge needs to re-prove the tie -- a chunk
      that does not exhaust the count IS the cap.

   THE TYPE READING, AND WHY IT IS NOW AVAILABLE.  The fire needs
   [bv_unsigned (di_type dnl) = T_FILE_z].  [inode_rec_local] enumerates the
   four legal type words; [InodeLock.inode_ok] rules out 0; the carve's
   fourth output rules out T_DIR on a writable fd; and the carve's FIFTH
   output -- the owner's [file_payload] strengthening, 2026-08-29 -- rules
   out T_DEVICE on an FD_INODE fd.  That is the whole of what killed
   [SpecSysWriteAUEra]'s third arm.

   THE INUM BRIDGE.  [fdstate_ok]'s [FdInode] arm ties the contract's [i] to
   the payload's [inum], but [file_pay_st_ok] and
   ...
   a sixth output -- the two read off ONE [pn] -- so the loop gets
   [i = bv_unsigned inum] once per iteration and the receipts speak about
   the caller's file. *)
...
(* AU EDIT (difference 1): the CONVERSE constant.  The landed walk never
   needed it -- its [f->writable] arm is live -- but [FILEWRITE_AU]'s
   premise pins the byte to 1, so the taken arm has to be refuted. *)
Lemma fw_zext8_one `{XI : CurCtx} :
...
Module FilewriteAUProof (Ilock : ILOCK) (Writei : WRITEI)
                        ...
                        (EndOp : END_OP) : FILEWRITE_AU.

Section ProofFilewriteAU.
      ...
      ∀ (c : Z) (P : regfile),
        ⌜(0 < c <= SpecFilewrite.FW_MAX)%Z /\ (c <= nz - iz)%Z
          (* AU EDIT (difference 5): the chunk is one of the two the code
             ...
             [FW_MAX * p]. *)
          /\ (c = nz - iz \/ c = SpecFilewrite.FW_MAX)%Z
  ...
  Local Lemma fw_loop `{CID0 : CpuId}
    ...
    (* ---- AU EDIT (difference 2): the carried commit state, at the
       ...
       chunk exits) ---- *)
    fw_au_raw (fs_gamma_L fsc_fs) nx γx n (us_M U) (m !!! Regidx Ra1) Φw t p 0%nat -∗
    ...
    wp_next true pj (fun (CID : CpuId) =>
      ...
      ∀ (mf : regfile) (r : mword 64) (P' : uptd),
        ...
        write_arms_at (fs_gamma_L fsc_fs) nx γx n (us_M U) (m !!! Regidx Ra1) Φw r -∗
        WP (Loop : expr riscv_lang)) -∗
    WP (Loop : expr riscv_lang).
  Proof.
 ...
 gf
    ...
    { rewrite Hnum.
      ...
      - (* ---- THE CHUNK FIRES ---- *)
        ...
        iMod (wrf_awrite_fire fsc_fs ⊤ (bv_unsigned inum) γx p Φw _
    ...
    - (* ============ THE FULL CHUNK: fall to +0xc4 ============ *)
      ...
      + (* ---- EXHAUSTED: i = n, the write completed ---- *)
        ...
        { (* THE ARMS, AT THE EXHAUSTED EXIT.  The chunk that reached it was
          ...
          iApply (fw_au_raw_ok (fs_gamma_L fsc_fs) nx γx n (us_M U) (m !!! Regidx Ra1) Φw pf with "Hau"). }
    ...
    - (* ====== THE SHORT WRITE (and writei's -1): straight to +0xe2 ======
      ...
      { rewrite /write_arms_at. iRight.
        ...
        iApply (fw_au_raw_fail (fs_gamma_L fsc_fs) nx γx n (us_M U) (m !!! Regidx Ra1) Φw tf pf xf
                  Hfailex with "Hau"). }
  Qed.
  ...
  Proof.
    ...
    (* AU EDIT (difference 1): THE TWO FIELD READINGS THE PREMISE FORCES.
       [st] is an open, WRITABLE inode descriptor, so [fdstate_ok] pins the
       two words the dispatch tests -- which is what kills the [f->writable]
       arm, the two type compares and the panic. *)
    pose proof Hok as Hokc. rewrite Hst in Hokc.
    destruct Hokc as (Hrdc & Hwrc & Htyi0 & Hnxeq & Hgxeq).
    iEval (rewrite /file_fields) in "Hrfields".
    iDestruct "Hrfields" as "(Hcty & Hcrd & Hcwr & Hcpp & Hcip & Hcmaj)".
    (* =================================================================
    ...
    - (* ================= NOT WRITABLE IS DEAD ======================
         AU EDIT (difference 1).  [st = FdOpen rb true (FdInode i)] pins
         [f->writable] to the byte 1, so the [beq a5,x0] at +0x04 cannot be
         taken and the -1 return at +0x13a is unreachable. *)
      exfalso.
      assert (Hw1 : fc_writable Cf = (mword_of_int 1 : mword 8)) by exact Hwrc.
      rewrite Hw1 fw_zext8_one in Hwrz. discriminate.
    - (* ===============================================================
      ...
      { (* ---- n < 0 : the guard FIRES, and +0x11a is [fw_m1j] ---- *)
        ...
        { (* AU EDIT: THE NEGATIVE-COUNT EXIT.  The loop is never entered,
             so the bundle refunds WHOLE and the receipts are empty --
             which is [write_post_fail_at]'s second disjunct, the one that
             exists for exactly this arm. *)
          rewrite /write_arms_at. iRight.
          iSplitR; [iPureIntro; reflexivity |].
          iApply (fw_au_raw_fail Γfs i γo n (us_M U) (m !!! Regidx Ra1) Φw 0%Z 0%nat 0%nat
      ...
      + (* ==================== FD_PIPE IS DEAD ======================
           AU EDIT (difference 1).  [FILEWRITE_AU] pins the descriptor to
           [FdOpen rb true (FdInode i)], so [fdstate_ok] reads
           [f->type = FD_INODE] and this compare cannot have succeeded.
           The landed walk's whole pipewrite block goes with it, and with
           it the [PIPEWRITE] functor parameter. *)
        exfalso. apply eq_vec_true_iff in Hp1. rewrite Htyi0 in Hp1.
        apply (f_equal bv_unsigned) in Hp1. by vm_compute in Hp1.
      + (* ---- +0x24 c.li a4,3 ; +0x26 beq a5,a4 -> +0x5c (FD_DEVICE) ---- *)
        ...
        * (* =================== FD_DEVICE IS DEAD =====================
             AU EDIT (difference 1), the same refutation at the second
             compare.  The console arm -- the devsw dispatch, the major
             range test, the null-slot arm and [CONSOLEWRITE]'s AXIOM --
             is out of this contract's domain by premise, which is why
             this functor has five parameters and not eight. *)
          exfalso. apply eq_vec_true_iff in Hp3. rewrite Htyi0 in Hp3.
          apply (f_equal bv_unsigned) in Hp3. by vm_compute in Hp3.
          ...
          -- (* ======================= FD_INODE =======================
             ...
             { iIntros (CID13 Hs13) "Hcg Hpc".
               ...
               - (* ---- n <= 0, so n = 0: the loop is never entered ---- *)
                 ...
                 { rewrite HVid. iExact "Hpriv". }
                 { by iApply filewrite_env_out_of_env. }
                 { (* AU EDIT: THE ZERO TRIP.  [n = 0], so the write is
                      complete before it starts: no receipts, no chunks
                      ([wchunks 0 = 0]), and the total IS the count. *)
                   ...
                   rewrite Hnz0.
                   iApply (fw_au_raw_ok Γfs i γo 0%Z (us_M U) (m !!! Regidx Ra1) Φw 0%nat).
                   iApply (fw_au_raw_init with "Hbundle"). }
               - (* ---- 0 < n: the five late spills, the two 3072s, and
                 ...
                 (* AU EDIT: the state's two fields are the CONTRACT's now,
                    so the landed re-derivation through [fdstate_ok_inode]
                    is gone: [rx := rb] and [nx := i] come straight off
                    [Hst]. *)
                 iApply (fw_loop (CID0 := CID28) γf γs j γlp k q st rb
                           i γo fn pidv U
                           m K eb n b sp0 w12 pj lks Φw
                           HK Hk Hj Hgs Hlens Hfnj Hfnps Hn01 Heb Hst Hspm
                           ltac:(reflexivity)
                           E1 E2 E3 E4 E5 E6
                 ...
                 { exact Hup. }
                 { exact Hra. } }
          -- (* ================= THE ELSE ARM IS DEAD ==================
                AU EDIT (difference 1).  [f->type] IS [FD_INODE], so the
                [bne] at +0x2c falls and [panic("filewrite")] is never
                reached -- which retires the [PANIC] functor parameter
                too. *)
             exfalso.
             assert (Hcon : eq_vec (fc_type Cf) (mword_of_int 2 : mword 32)
                            = true) by (apply eq_vec_true_iff; exact Htyi0).
             rewrite Hcon in Hp2. discriminate.
  Qed.
End ProofFilewriteAU.
End FilewriteAUProof.
