(* ProofSysWriteAU.v -- sys_write's ATOMIC-UPDATE walk: the whole-function WP
   that seals [SpecSysWriteAUEra.SYSWRITE_AU_ERA], over [SpecFilewriteAU]'s
   per-chunk AU form of filewrite.

   Worklist: claude-notes/projects/fs-syscall-specs.md, lane W (the write AU
   prover).  A PARALLEL FORM beside [ProofSysWrite.SysWriteProof] -- R10:
   the landed proof and its contract do not move, and this file's walk is
   that proof's, instruction for instruction.

   ==== WHAT IS ACTUALLY DIFFERENT ======================================

   FOUR THINGS, and they all live in the last forty lines of the walk; the
   twenty-five instructions in between are the landed proof VERBATIM (the
   reuse rule: a landed tail that concludes abstractly is reused, and only
   the blocks that fire a commit or mint the armed post are re-derived).
   ...
   1. THE argfd FAILURE ARM IS DEAD.  The contract's pure premise
      ...
      whole -1-without-a-call branch, which is why the arms below are TWO
      and not the landed contract's blanket.

   2. THE DESCRIPTOR'S STATE IS PINNED BY THE CALLER'S FRAGMENT.  The B1
      seam ([ProcInv.proc_priv_lend]) hands out the reference at an
      EXISTENTIAL [stf] together with [FdSlots.fd_st_auth] at that [stf];
      the contract's own [fd_st] fragment plus [FdSlots.fd_st_agree] pins
      [stf = FdOpen rb true (FdInode i)].  That is the whole fd bridge --
      no new carrier, no new agreement law -- and it is what lets the AU
      form of filewrite be called at all (its domain premise IS that
      equation).  The fragment is threaded through untouched and returned:
      a write moves the offset and the bytes, never the descriptor.

   3. THE CALLEE IS [SpecFilewriteAU.FILEWRITE_AU].  Same arguments as the
      landed call plus [rb], [i] and the receipt family [Φw]; the caller's
      bundle goes down unchanged (this shell fires nothing itself -- every
      instant is inside the loop) and the armed post comes back on [rv].

   4. THE POST IS THE ARMED ONE.  [write_arms_at] REPLACES the landed
      [⌜sys_write_ret ...⌝]; each arm pins a0, so the blanket is implied
      and no caller loses anything.
   ...
   [sw_tail] -- is copied because [ProofSysWrite]'s module is SEALED and its
   [Local] helpers are therefore not reachable; the two pure address lemmas
   ARE top-level there and are reused by [Require].

   BINDERS: [ProofSysWrite]'s section list VERBATIM.  NO [!icacheG Σ]:
   [fileG] bundles it. *)
...
Module SysWriteAUProof (Argaddr : ARGADDR) (Argint : ARGINT) (Argfd : ARGFD)
                    (FilewriteAU : FILEWRITE_AU) : SYSWRITE_AU_ERA.
...
Section ProofSysWriteAU.
  ...
  Proof.
    ...
    (* [v1] is a BINDER now (RULING A): the arms speak about the bytes at
       that address, so the argument cannot stay existential. *)
    ...
    - (* ================= SUCCESS: the descriptor resolved ============= *)
      ...
      (* a1 IS the trapframe's argument 1 (RULING A): the arms name the bytes
         at that address, so the walk has to say which address it is. *)
      ...
      (* ---- THE FD BRIDGE.  The lend hands the reference out at an
         ...
         call inside [FILEWRITE_AU]'s domain (open, WRITABLE, an inode
      ...
      (* the arms come back stated at filewrite's a1; [HS4a1] says that IS
         the syscall's own argument 1 (RULING A). *)
      ...
      (* THE ARMED POST goes straight through: this shell fires nothing --
         every instant is inside filewrite's chunk loop -- so [Harms] is
         handed on exactly as it came back, and the descriptor's fragment
         with it. *)
