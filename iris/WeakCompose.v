...
Proof.
  ...
  destruct Hst as
    ...
    |cfg ag st' dd Hag Hps
    |cfg ag aq base tvs asrc st' dd Hag Hps Hok
    |cfg ag rl base data asrc vsrc k R st' dd
         Hag Hps Hne Hres Hrb Hrlen Hex Hkc]; destruct dd;
  ...
  - exact (PFInstr (pstep_unit (pstep_xv6 next)) lbl_class_p i
             ...
             (lift_ag ag) (PHart st') tt Hlk Hps).
  (* THE RMW SPLIT (S2) *)
  - exact (PFExLoad (pstep_unit (pstep_xv6 next)) lbl_class_p i
             (WPCfgU (pc_img cfg) (pc_log cfg) ((lift_ag <$> pc_ags cfg) ++ dks))
             (lift_ag ag) aq base tvs asrc (PHart st') tt Hlk Hps Hok).
  - exact (PFExStore (pstep_unit (pstep_xv6 next)) lbl_class_p i
             (WPCfgU (pc_img cfg) (pc_log cfg) ((lift_ag <$> pc_ags cfg) ++ dks))
             (lift_ag ag) rl base data asrc vsrc k R (PHart st') tt
             Hlk Hps Hne Hres Hrb Hrlen Hex Hkc).
Qed.
