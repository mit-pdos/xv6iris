...
Section UkGen.
  ...
  Definition uvb_F' `{CID : CpuId} `{XI : CtxIdDefs.CurCtx}
    ...
    (uv_amb ∗ uv_regs ∗ ⌜usz_ok sz⌝ ∗ user_ptm_inv_x pt sz M ∗
     Rfd fdv ∗ user_cfg C ∗
     gpr_file m ∗ pc_is pc ∗ Rut pt ∗ ukont_F' C pt Rfd Rut sz π fdv cw gn cs pidv)%I.
  ...
  Definition ukc' (π : gmap (mword 27) uperm) (M : gmap Z (bv 8))
    ...
    (∀ (h : CpuId) (xi : CtxIdDefs.CurCtx) (C : ucfg) (pt : uptd)
       ...
       (* the fill row, as [UexecRet.ukc] carries it; the primed bundle is
  ...
  Lemma ukc'_run (m : regfile) (pc : mword 64) (M : gmap Z (bv 8))
    ...
    X (uvis_of_run m pc M π szv fdv cw gn cs pidv false secc_all) ⊣⊢ ukc' π M szv fdv cw gn cs pidv m pc.
...
Section UkGenBundle.
  ...
  Lemma uvb_elim' (C : ucfg) (pt : uptd) (Rfd : list fdstate -> iProp Σ) (Rut : uptd -> iProp Σ)
    ...
    (* the descriptor fragments come out beside the image, and go back in
       ...
       as it threads [umem] *)
    Rfd fdv ∗
    user_cfg C ∗ gpr_file m ∗ pc_is pc ∗ Rut pt ∗ ▷ ukb_F' C pt Rfd Rut sz π fdv cw gn cs pidv.
  Proof using .
...
Section UkGenObl.
  ...
  Definition uk_step_obl' (π : gmap (mword 27) uperm) (Kc : iProp Σ)
    ...
    (∀ (R : iProp Σ) (CIDo : CpuId) (XIo : CtxIdDefs.CurCtx)
       ...
       (R -∗ (TsoCtx.own_context (CID := CIDo) XIo -∗ Rut pt) ∗ Rfd fdv ∗ ukb_F' (CID := CIDo) C pt Rfd Rut sz π fdv cw gn cs pidv ∗ uk_paycont' Qp gn (Kc ∧ ukc' π M sz fdv cw gn cs pidv m pc)) -∗
       resv_any cpu_id -∗
...
Section UkGenArms.
  ...
  Proof using Ret_transparent X_unfold.
    ...
    iFrame "Hrw Hro".
    iApply (uv_psi_trap C pt (uk_payload' sz π fdv cw gn cs pidv Kc Qp M m pc C pt Rfd Rut) Mp m t usatp pcfg paddr
    ...
    iApply (bi.equiv_entails_1_2 _ _
              (Ret_transparent _ (uvis_of_run m pc M π sz fdv cw gn cs pidv false secc_all)
    ...
    iSplitR;
      [ iApply (uexec_pay_dep_ne _ (uvis_of_run m pc M π sz fdv cw gn cs pidv false secc_all) _ (sfam_at Qp sfam_pt)
...
Section UkGenStepEngine.
  ...
  Lemma wp_uk_step_gen' (π : gmap (mword 27) uperm) (Kc : iProp Σ)
    ...
    is_aligned_vaddr (Virtaddr pc) 2 = true ->
    ⊢ uk_ih' π Kc Qp fdv cw gn cs pidv M m pc.
  Proof using Ret_transparent X_unfold.
    intros Hal2.
    rewrite /uk_ih'.
    iLöb as "IH".
    iIntros (CID XIv C pt Rfd Rut HRut sz) "%HQ %Hlo %Hpm %Hlf Hb #Hobl Hpay3".
    ...
    iApply (swp_exec_step_full u_Drw u_Dro (u_Df (uc_dqc C)) RS (wrap_pre RS)
              (uv_land RS) (uv_psi C (uk_payload' sz π fdv cw gn cs pidv Kc Qp M m pc C pt Rfd Rut))
    ...
    - (* ================= THE BODY SLOT ================= *)
      ...
      2: iApply (swp_run_hart_active_res u_Drw u_Dro (u_Df (uc_dqc C))
                   ...
                   (fun (ii : InterruptType) (pr : Privilege) =>
                      uv_step_post C (uk_payload' sz π fdv cw gn cs pidv Kc Qp M m pc C pt Rfd Rut) RS
      ...
      + (* ---- THE FETCH: the caller's obligation ---- *)
        ...
        iDestruct "HWd" as "(Hfrag & Hctx & Hmm & Hres)".
        iDestruct (resv_any_intro cpu_id None with "Hfrag") as "Hany".
        iApply ("Hobl" $! (uk_payload' sz π fdv cw gn cs pidv Kc Qp M m pc C pt Rfd Rut) CID XIv C pt Rfd Rut HRut Mp t RS
                  (wrap_pre RS) usatp pcfg paddr
                  with "[%] [%] [%] [%] [%] [%] Hamb [] Hany Hrw Hro Hctx Hmm Hres");
          [ exact HQ | exact Hlo | exact Hpm | exact Hlf | exact Hpure | exact Hpre | ].
...
Section UkGenFunnel.
  ...
  (* the fill row (lane KILL-PAY, milestone LAZY-ROW), on [Hlo]/[Hpm]'s
     footing: a section hypothesis, because the section's [pt] is fixed *)
  ...
  Proof using HQ0 HRut Hlf0 Hlo Hpm Ret_transparent X_unfold.
    ...
    iPoseProof (wp_uk_step_gen' π Kc Qp M m pc fdv cw gn cs pidv Hal2) as "H". rewrite /uk_ih'.
    iApply ("H" $! CID XI C pt Rfd Rut HRut sz
              with "[%] [%] [%] [%] Hb Hobl Hpay3");
      [ exact HQ0 | exact Hlo | exact Hpm | exact Hlf0 ].
  Qed.
...
Section UkGenEcallPost.
  ...
  Lemma uk_ecall_post_fetch' (R : iProp Σ) (Rut : uptd -> iProp Σ) (sz : Z)
    ...
    uk_pt_pure pt sz M Mp ->
    gen_cert -∗
    (R -∗ (TsoCtx.own_context XI -∗ Rut pt) ∗ Rfd fdv ∗ ukb_F' C pt Rfd Rut sz π fdv cw gn cs pidv ∗ RetF X uecall_scause (uvis_of_run m pc M π sz fdv cw gn cs pidv false secc_all)) -∗
    resv_any cpu_id -∗
    TsoCtx.own_context XI -∗
    uv_bytes pt Mp t' -∗
    uv_res pt Mp t' usatp pcfg paddr -∗
...
Section UkGenEcall.
  ...
  Hypothesis (HRut : forall pt' : uptd,
                       ⊢ Rut pt' -∗ TsoCtx.own_context XI ∗
                                    (TsoCtx.own_context XI -∗ Rut pt')).
  Hypothesis (HQ0 : Q XI).
  (* the fill row (lane KILL-PAY, milestone LAZY-ROW) *)
  Hypothesis (Hlf0 : lazy_free (ud_um pt) sz).
  ...
  Lemma wp_uk_ecall' (M : gmap Z (bv 8)) (m : regfile) (pc : mword 64) (fdv : list fdstate) (cw : Z) (gn : gname) (cs : gset gname) (pidv : mword 32) :
    ...
    (* the payment enters here and the return is behind it -- see
       [UkStep.wp_uk_ecall] *)
    my_pay gn Qp -∗
    RetF X uecall_scause (uvis_of_run m pc M π sz fdv cw gn cs pidv false secc_all) -∗
  ...
  Proof using HQ0 HRut Hlf0 Hlo Hpm Ret_transparent X_unfold.
    ...
    iIntros (R CIDo XIo C' pt' Rfd' Rut' HRut' Mp' t rs1 rsA usatp pcfg paddr)
      "%HQo %Hlo' %Hpm' %Hlf' %Hpure %Hpre #Hamb Hk Hany Hrw Hro Hctx Hmm Hres".
    iAssert (R -∗ (TsoCtx.own_context (CID := CIDo) XIo -∗ Rut' pt') ∗ Rfd' fdv ∗
             ukb_F' (CID := CIDo) C' pt' Rfd' Rut' sz π fdv cw gn cs pidv ∗
             RetF X uecall_scause (uvis_of_run m pc M π sz fdv cw gn cs pidv false secc_all))%I with "[Hk]" as "Hk".
  ...
  Proof using HQ0 HRut Hlf0 Hlo Hpm Ret_transparent X_unfold.
    ...
    { iIntros "HR". iDestruct ("Hk" with "HR") as "(Hbak & Hfdr & Hkb & Hkc)".
      ...
      iApply ("Hkc" $! CIDo XIo C' pt' Rfd' Rut' HRut' with "[%] [%] [%] [%] Hb");
        [ exact HQo | exact Hlo' | exact Hpm' | exact Hlf' ]. }
...
Section UkGenPlain.
  ...
  Proof using .
    ...
    - iIntros "H" (h xi C pt Rfd Rut HRut) "%Hlo %Hpm %Hlf Hb".
      iApply ("H" $! h xi C pt Rfd Rut HRut with "[%] [%] [%] [%] Hb");
        [ exact I | exact Hlo | exact Hpm | exact (Hlf eq_refl) ].
  Qed.
  ...
  Proof using . exact (uexec_ret_transparent sc W). Qed.
  ...
  Definition uk_ecall_ty `{CID : CpuId} `{XI : CurCtx}
      ...
      (HRut : forall pt' : uptd,
                ⊢ Rut pt' -∗ TsoCtx.own_context XI ∗
                             (TsoCtx.own_context XI -∗ Rut pt'))
      (* the fill row (lane KILL-PAY, milestone LAZY-ROW), which both
         engines' ecall drivers now take -- see [UkStep.uk_ih] *)
      (Hlf : lazy_free (ud_um pt) sz)
      (Qp : Z -> iProp Σ) : Prop :=
  ...
  Proof using .
    exact (UkStep.wp_uk_ecall (Qp := Qp) C pt Rfd Rut π sz Hlo Hpm HRut Hlf).
  Qed.
