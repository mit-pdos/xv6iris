(* ProofSysUnlinkShared.v -- the walk's SHARED layer: the return
   continuation, the four per-slot / per-record projections the blocks
   want, and the name tie.

   THREE FILES BELOW IT CARRY THE REST, and each block file requires all
   four:
     ...
     - [ProofSysUnlinkTails.v] -- EVERY exit block.  The tails conclude
       ...
       the walk supplies one that pays [unlink_arms] at [-1].
     - [ProofSysUnlinkPure.v] -- the top-level pure lemmas (the two name
       literals, the isdirempty index arithmetic, the zeroing's cost
       figures).
   ...
   What this file adds is what the blocks cannot share through those three:
     ...
     - THE RETURN CONTINUATION IS NOT HERE: it is the contract's own
       ...
       ⌜sys_unlink_ret⌝.
     - the per-slot / per-record projections, hoisted out of the blocks'
       functors so they are visible to all five: none of them mentions a
       functor parameter.
     - [su_last_of_npar], the name tie
       ([ProofCreateShared.cr_last_of_npar]'s shape): nameiparent's name
       clause is [nameiparent_of pl es e] and the arms speak of
       [last (path_elems pl)], which is one [last_snoc]. *)
...
(* the payload's own vocabulary, IMPORTED BEFORE [FsBlocks] on purpose --
   ProofSysUnlinkPure's banner: the [FsState*] stack exports [fs_view] and
