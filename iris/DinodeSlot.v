...
Local Lemma iu_align_arith (kk qq off dv : Z) :
  ...
  (2147582512 + 1112 * kk + (88 + (64 * qq + off))) `mod` dv = 0
  /\ 0 <= 2147582512 + 1112 * kk + (88 + (64 * qq + off))
  /\ 2147582512 + 1112 * kk + (88 + (64 * qq + off)) < 18446744073709551616.
Proof.
  intros H1 H2 H3 H4 H5 H6 Hd Hoff. split_and!; [| lia | lia].
  assert (Hz : 2147582512 + 1112 * kk + (88 + (64 * qq + off))
               = (536895650 + 278 * kk + 16 * qq) * 4 + off) by lia.
  rewrite Hz. destruct Hd as [-> | ->].
  - rewrite Z.add_mod; [| lia]. rewrite Hoff.
    replace ((536895650 + 278 * kk + 16 * qq) * 4) with
      ((536895650 + 278 * kk + 16 * qq) * 2 * 2) by lia.
...
Proof.
  ...
  destruct (iu_align_arith (Z.of_nat k) (Z.of_nat q) (Z.of_nat off) dv
    ...
    as (Hmod & Hlo & Hhi).
  replace (0x80018218 + 24 + 1112 * Z.of_nat k + Z.of_nat (88 + (64 * q + off)))
    with (2147582512 + 1112 * Z.of_nat k
