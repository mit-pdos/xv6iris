(* WpUserMem1.v -- width-1 memory USTEP arms (LB/LBU loads, SB stores),
   over UmodeData2's exec_execute_{LOAD,STORE}_1_U (mode-neutral MemData1
   tower).  The load arm is sign-generic (is_unsigned), covering LB and LBU. *)
...
Require Import WpPushOffMem MemData1 UmodeData1.
...
Section WpUserMem1.
  ...
  (* width-1 ghost window update (public upd_window_bw specialized) *)
  ...
  Proof.
    ...
    iApply (wp_instr_u_hit va vpn ie w (LOAD (imm, Regidx rs1, Regidx rd, is_unsigned, 1))
    ...
    { iDestruct (big_sepM_lookup (fun a b => (a ↦ₘ□ b)%I) code _ _
                   (Hcwd 0%nat ltac:(lia)) with "Hcode") as "Hb7".
  ...
  Proof.
    ...
    assert (Hnlpad : is_lpad_instruction (STORE (imm, Regidx rs2, Regidx rs1, 1))
                       = false) by reflexivity.
    iApply (wp_instr_u_hit va vpn ie w (STORE (imm, Regidx rs2, Regidx rs1, 1))
              ms_v tlbvec E Φ HN Hvec Hchk0 HupdN Hpbmt0 Hcw HSXL Hval Hcanon
              Hvpn_def Hpaal HnotRVC Hdec Hnlpad
              with "Hhw Hinv Hhs Hpriv Hms Htlbc Hpcr Hcode Hcfg").
    iIntros (σ Hpceq Hag Hpins) "[Hreg [Hmem Hdev]]".
  ...
  Proof.
    ...
    assert (Hnlpad : is_lpad_instruction (LOAD (imm, Regidx rs1, Regidx rd, is_unsigned, 1))
                       = false) by reflexivity.
    iApply (wp_instr_u_hit_data va vpn ie w (LOAD (imm, Regidx rs1, Regidx rd, is_unsigned, 1))
              ms_v tlbvec E Φ HN Hvec Hchk0 HupdN Hpbmt0 Hcw HSXL Hval Hcanon
              Hvpn_def Hpaal HnotRVC Hdec Hnlpad
  ...
  Proof.
    ...
    assert (Hnlpad : is_lpad_instruction (LOAD (imm, Regidx rs1, Regidx rd, is_unsigned, 1))
                       = false) by reflexivity.
    iApply (wp_instr_u_data va vpn ie w (LOAD (imm, Regidx rs1, Regidx rd, is_unsigned, 1))
  ...
  Proof.
    ...
    assert (Hnlpad : is_lpad_instruction (STORE (imm, Regidx rs2, Regidx rs1, 1))
                       = false) by reflexivity.
    iApply (wp_instr_u_hit_data va vpn ie w (STORE (imm, Regidx rs2, Regidx rs1, 1))
              ms_v tlbvec E Φ HN Hvec Hchk0 HupdN Hpbmt0 Hcw HSXL Hval Hcanon
              Hvpn_def Hpaal HnotRVC Hdec Hnlpad
              with "Hhw Hinv Hhs Hpriv Hms Htlbc Hpcr Hcode Hupt Hcfg").
    iIntros (σ Hpceq Hag Hpins) "Htlbc Hupt [Hreg [Hmem Hdev]]".
  ...
  Proof.
    ...
    assert (Hnlpad : is_lpad_instruction (STORE (imm, Regidx rs2, Regidx rs1, 1))
                       = false) by reflexivity.
    iApply (wp_instr_u_data va vpn ie w (STORE (imm, Regidx rs2, Regidx rs1, 1))
              ms_v tlbvec E Φ HN Hok Hsome Hchk0 HupdN Hcw HSXL Hval Hcanon
              Hvpn_def Hpaal HnotRVC Hdec Hnlpad
              with "Hhw Hinv Hhs Hpriv Hms Htlbc Hpcr Hcode Hupt Hcfg").
    iIntros (σ Hpceq Hag Hpins) "Htlbc Hupt [Hreg [Hmem Hdev]]".
  ...
  Proof.
    ...
    iDestruct "Hhwc" as (misa0 mseccfg0 pmar0 elp0)
      "(#Hmisa & #Hmseccfg & #Hpma & #Hhtif & #Help & %HmisaS & %HmisaC &
        %HmisaU & %HmisaM & %Hpma_all & %Hseccfg1 & %Hseccfg2 & %Help_np &
        %HmisaA & %Hmisa_val0 & %Hmseccfg_val0)".
    iApply (wp_instr_c_data va vpn ie h ii (LOAD (imm, Regidx rs1, Regidx rd, is_unsigned, 1))
