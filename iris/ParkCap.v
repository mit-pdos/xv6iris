...
Section ParkCap.
  ...
  Definition park_pkg `{XI : CurCtx}
    ...
    (kernel_text ∗
     ...
     (* THE LATER IS ON THIS ROW ALONE (tso-port.md §0.16′ step (ii)): the
        park's cap is the site where three of this package's rows would be
        DEPOSITED into the child's fresh context, and nothing under a [▷] can
        be deposited ([ctx_dom] is not persistent and there is no crossing).
        The closer is the only row that names [W], hence the only one the
        fixpoint needs guarded. *)
  ...
  (* (tso-port.md §0.12′ ruling 1, the ∀-parker variant).                   *)
  (* ===================================================================== *)
  ...
  Definition park_cap
    ...
    (* THE PARKER'S HART IS ∀-QUANTIFIED BESIDE ITS CONTEXT, and that is
       ...
       hart-free) even though the park now borrows an [own_context], which
    ...
    (□ ∀ (hp : CpuId) (ξp : CtxId) (γw γft γf γtl : gname) (pa ks : mword 64)
       ...
       (* NO LONGER UNDER A [▷]: this is where the package's rows would be
          deposited into the child's fresh context, and a deposit cannot run
          under a later.  The package's own closer row carries the later. *)
       park_pkg (XI := ξp) URB W γs γw γft γf γtl pa ks pid av -∗
  ...
  Lemma park_token_park `{CID : CpuId} `{XI : CurCtx}
    ...
    ut_wf N ->
    length rest = 12%nat ->
    (* BORROWED, and handed straight back: the cap is the site of the
       child-context deposit and [TsoCtx.ctx_deposit] runs at the
       depositor's own authority.  Both parkers ([ProofUserinit],
  ...
  Proof.
    (* [procs_inv] AND [park_globals] ARE PREMISES NOW, not read out of
       ...
       the record-carried half (UsertrapRes.v, "THE RESUMER'S HALF").  Both
       parkers hold them at their own context anyway -- [procs_inv] is what
       they built the child's slot out of. *)
    ...
    iApply ("Hcap" $! cpu_id cur_ctx (un_w N) (un_ft N) (un_f N) (un_tk N) (un_pj N)
              (un_ks N) rest (un_pid N) V KSTACK_AV
