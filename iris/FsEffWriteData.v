(* FsEffWriteData.v -- durable-disk stage F2, effect 2: writing one
   covered content block of a live non-directory (writei's per-block net).
   The shared ground lives in FsEffBase.v. *)
...
Section EffWriteData.
  ...
  (* ==================================================================== *)
  (*  13.  EFFECT 2 -- WRITING A FILE'S CONTENT                            *)
  (*                                                                       *)
  (*  One covered content block of a live non-directory inode gets new     *)
  (*  bytes, and the size moves within the same block count (writei's      *)
  (*  net per-block step; a multi-block write is a composition).           *)
  (* ==================================================================== *)

  Definition eff_write_file_data (i : Z) (fbn : nat) (bs : list (bv 8))
      (sz' : Z) : Z -> list (bv 8) :=
    fs_upd (eff_dinode P sb i
  ...
  Proof.
    ...
    set (P' := eff_write_file_data i fbn bs sz').
    assert (Htype' : di_type dn' = di_type dn) by reflexivity.
    assert (Haddrs' : di_addrs dn' = di_addrs dn) by reflexivity.
    assert (Hnlink' : di_nlink dn' = di_nlink dn) by reflexivity.
    assert (Hsz'u : bv_unsigned (di_size dn') = sz').
    { unfold dn'. cbn [di_size di_set_size]. apply Z_to_bv_small.
      unfold FS_MAXFILE, BSIZE_z in Hsz'. lia. }
    ...
    pose proof (dok_at i Hi Hlive) as Hdi.
    pose proof (fdi_size _ _ _ Hdi) as Hcap.
    ...
    assert (HoutA : forall b : Z,
              ...
              P' b = P b).
    { intros b Hb1 Hb2. unfold P', eff_write_file_data.
    ...
    { apply HoutA; unfold fs_data_start in *; lia. }
    ...
    { intros z Hz.
      ...
      - apply fs_dinode_ext. unfold P', eff_write_file_data.
        apply fs_upd_ne. fold dn. rewrite HaE.
    ...
    (* the untouched-inode bundle for every z <> i *)
    ...
    { intros z Hz Hne Hnz. apply inode_untouched; try assumption.
      ...
      - intros ->. exact (blocks_cross z i a Hz Hi Hne Hnz Hlive Hb Ha_in).
      - pose proof (blocks_range z b Hz Hnz Hb) as Hbr.
        unfold fs_data_start in Hbr. lia. }
    (* inode [i]'s own readers *)
    assert (HindI : fs_ind_ents P' dn' = fs_ind_ents P dn).
    { transitivity (fs_ind_ents P' dn);
        [exact (fs_ind_ents_meta P' dn dn' Haddrs') |].
      apply fs_ind_ents_ext. intros Hnz12. apply HoutA.
      - intros Hc.
        pose proof (slot_inj_at i Hi Hlive) as Hinj.
        assert (Hfe : fs_slot P dn FS_MAXFILE = fs_slot P dn fbn).
        { rewrite fs_slot_max, (fs_slot_blk dn fbn HfbnM), HaE. exact Hc. }
        assert (Hnz' : fs_slot P dn FS_MAXFILE <> 0)
          by (rewrite fs_slot_max; exact Hnz12).
        pose proof (Hinj FS_MAXFILE fbn ltac:(lia) ltac:(lia) Hnz'
                      Hfe) as Hcc.
        lia.
    ...
    { unfold fs_rtickets. apply tick_mjoin_ext.
      ...
      - rewrite (Hdec (Z.of_nat x) ltac:(lia)), decide_False by exact Hxi.
        reflexivity.
      - intros _ k Hk.
    ...
    - apply fs_inodes_dwf_intro. intros z Hz Hnz'.
      ...
      destruct (decide (z = i)) as [-> | Hne]; [exact HdwfI |].
    ...
    - exists u. split.
      + assert (Hused : fs_used_blocks P' sb = fs_used_blocks P sb).
        { unfold fs_used_blocks. f_equal. apply list_fmap_ext.
          ...
          - rewrite Heq. fold dn. rewrite Htype'.
          ...
          - destruct (bv_unsigned (di_type (fs_dinode P sb (Z.of_nat (0 + idx))))
            ...
            destruct (Hunt (Z.of_nat (0 + idx)) ltac:(lia) Hne
    ...
    - apply root_wf_untouched.
      ...
      + intros k Hk.
        ...
        destruct (Hunt ROOTINO
                    ltac:(pose proof Hnin1; unfold ROOTINO; lia) Hrne Hrl)
          as (_ & Hdata & _ & _).
        exact (Hdata k).
    - apply fs_dots_all_intro. intros z Hz Hdty.
      ...
      destruct (Hunt z Hz Hne Hzl) as (_ & Hdata & _ & _).
    ...
    - exists rd.
      ...
      + (* the per-reachable-dir bundle *)
        ...
        destruct (Hunt z Hzr Hne Hzl) as (_ & Hdata & _ & _).
...
Lemma eff_write_file_data_wfv (P : Z -> list (bv 8)) (sb : fs_sb)
    (i : Z) (fbn : nat) (bs : list (bv 8))
      (sz' : Z) :
...
Proof.
  ...
  apply (eff_write_file_data_wf P sb Hp Hsb HW3 u Hu Hbm HW7 HW8 nib Hnibz Hreg rd Hrd Hdok Hlkg Horph); assumption.
Qed.
