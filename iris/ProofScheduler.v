...
(* ===================================================================== *)
(* EXPLICIT-CPUID PORT: BLOCKED.  Two findings, the second a design gap.  *)
(*                                                                        *)
(* (A) [WpSconfCsr.wp_csrci_sstatus_x0_s_sconf] (the +0x8a intr_off leaf)  *)
(*     was NOT updated when [cpu_own] moved into [sie_arm]'s enabled arm.  *)
(*     Its sibling [wp_csrsi_sstatus_x0_enable_s_sconf] was: that leaf     *)
(*     CONSUMES [if eb then emp else (intr_count 0 false ∗ trap_csrs ∗     *)
(*     cpu_cells 0 true p)] and returns ONLY [sie_cap_gpr m n true p].     *)
(*     The disable leaf then demands a SEPARATE [intr_count 0 true]        *)
(*     alongside [sie_cap_gpr m n b p], which nothing at +0x8a can supply  *)
(*     (and which nobody can hold: at b = true both count eighths are      *)
(*     inside the arm -- explicit-cpuid.md, surprise #4).  It also RETURNS *)
(*     one eighth too many ([intr_count 0 false] AND [cpu_hart 0 true p],  *)
(*     i.e. the same eighth at two values).  The fix is the shape its two  *)
(*     siblings already use: premise [intr_count_pre b 0 true], post       *)
(*     [intr_count 0 false ∗ trap_csrs ∗ cpu_cells_pay b p].               *)
(*                                                                        *)
(* (B) THE SCHEDULER CANNOT SURVIVE A [wp_next true].  It holds resources  *)
(*     PINNED to its hart -- [own_ctx (a_cpu_ctx cid_word)], the loop      *)
(*     invariant's [s4 + 48 = a_cpu_proc cid_word] / [s6 = a_cpu_ctx       *)
(*     cid_word], its own record's [Some (cpu_id, sie_gname)] admissibility *)
(*     index -- and there are three crossing points it cannot avoid:       *)
(*       - +0x8a, the intr_off: its entry index IS true (the +0x86 csrsi   *)
(*         just enabled), so the crossing is unconditional, at every eb;   *)
(*       - the release in the tail, whose [wp_next] index is [outb = ebx]  *)
(*         and ebx = true after any dispatch (the parking precondition is  *)
(*         eb = true, so swtch hands the scheduler back at eb' = true);    *)
(*       - the acquire in the scan, entered at b = ebc = that same true.   *)
(*     Nothing re-establishes the pins at the resumed hart, and nothing    *)
(*     refutes the migration, so the loop invariants are unprovable.       *)
(*                                                                        *)
(*     WHY IT IS SAFE IN REALITY, and the fix that expresses it: the       *)
(*     scheduler thread has c->proc == 0, so kerneltrap's                  *)
(*     [if (which_dev == 2 && myproc() != 0 && ...) yield()] never fires   *)
(*     and the hart CANNOT change.  That datum is already threaded -- it   *)
(*     is [sie_cap_gpr]'s / [sie_arm]'s [p], which is [zero_reg] at every  *)
(*     point of this proof where b can be true (the dispatch window where  *)
(*     p = proc_addr jj runs entirely at noff >= 1, hence b = false).  So  *)
(*     the crossing should be conditioned on p as well as on b:            *)
(*                                                                        *)
(*       Definition wp_next `{CID0} (b : bool) (p : mword 64) K :=         *)
(*         (∀ CID, ⌜ b = false \/ p = zero_reg ->                          *)
(*                  (CID : CPU) = (CID0 : CPU) ⌝ -∗ K CID)%I.              *)
(*       Lemma wp_next_idle : p = zero_reg -> wp_next b p K ⊣⊢ K CID0.     *)
(*                                                                        *)
(*     [p] is an implicit section [Context] in every Wp* leaf, so no       *)
(*     consumer call site changes; [wp_next_idle] then collapses EVERY     *)
(*     step of this file exactly as [wp_next_off] does, and the port       *)
(*     becomes purely mechanical.  Retagging [p] in a b = false bundle is  *)
(*     conversion ([sie_arm false p] ignores p) and stays sound: whenever  *)
(*     b = true the arm's [cpu_hart 0 true p] pins p to the real c->proc.  *)
(*     The soundness obligation is on kerneltrap / [intr_handler_spec]     *)
(*     (Stage 2): "no current proc => the trap returns on the same hart".  *)
(* ===================================================================== *)
...
Section ProofScheduler.
  ...
  Lemma wp_scheduler_sconf (γ : gname) (Φ : mval -> iProp Σ)
  ...
  Proof.
    ...
    iDestruct "Hcpu" as "(%Hbnd0 & Hnoff & Hint & Hcnt & Hproc & Hfree)".
    ...
    iApply (wp_caddi16sp_push_s_sconf γ Φ pcE (mword_of_int 59 : mword 6) m av 10
    ...
    assert (Hpc02 : add_vec_int (pcE : mword 64) 2 = mword_of_int (SC + 0x02))
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (SC + 0x02)) (mword_of_int 9 : mword 6) Rra
              A0 (av - 10)%nat u1 with "Hcg Hpc Hi02 [Hf1] [-]").
    { iEval (rewrite HcspA0 -Hb1). iExact "Hf1". }
    iIntros "Hcg Hpc _".
    assert (Hpc04 : add_vec_int (mword_of_int (SC + 0x02) : mword 64) 2 = mword_of_int (SC + 0x04))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpc04) in "Hpc".
    (* +0x04 c.sdsp s0,64 *)
    iPoseProof (schi_04 with "Htext") as "Hi04".
    iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (SC + 0x04)) (mword_of_int 8 : mword 6) Rs0
              A0 (av - 10)%nat u2 with "Hcg Hpc Hi04 [Hf2] [-]").
    { iEval (rewrite HcspA0 -Hb2). iExact "Hf2". }
    iIntros "Hcg Hpc _".
    assert (Hpc06 : add_vec_int (mword_of_int (SC + 0x04) : mword 64) 2 = mword_of_int (SC + 0x06))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpc06) in "Hpc".
    (* +0x06 c.sdsp s1,56 *)
    iPoseProof (schi_06 with "Htext") as "Hi06".
    iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (SC + 0x06)) (mword_of_int 7 : mword 6) Rs1
              A0 (av - 10)%nat u3 with "Hcg Hpc Hi06 [Hf3] [-]").
    { iEval (rewrite HcspA0 -Hb3). iExact "Hf3". }
    iIntros "Hcg Hpc _".
    assert (Hpc08 : add_vec_int (mword_of_int (SC + 0x06) : mword 64) 2 = mword_of_int (SC + 0x08))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpc08) in "Hpc".
    (* +0x08 c.sdsp s2,48 *)
    iPoseProof (schi_08 with "Htext") as "Hi08".
    iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (SC + 0x08)) (mword_of_int 6 : mword 6) Rs2
              A0 (av - 10)%nat u4 with "Hcg Hpc Hi08 [Hf4] [-]").
    { iEval (rewrite HcspA0 -Hb4). iExact "Hf4". }
    iIntros "Hcg Hpc _".
    assert (Hpc0a : add_vec_int (mword_of_int (SC + 0x08) : mword 64) 2 = mword_of_int (SC + 0x0a))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpc0a) in "Hpc".
    (* +0x0a c.sdsp s3,40 *)
    iPoseProof (schi_0a with "Htext") as "Hi0a".
    iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (SC + 0x0a)) (mword_of_int 5 : mword 6) Rs3
              A0 (av - 10)%nat u5 with "Hcg Hpc Hi0a [Hf5] [-]").
    { iEval (rewrite HcspA0 -Hb5). iExact "Hf5". }
    iIntros "Hcg Hpc _".
    assert (Hpc0c : add_vec_int (mword_of_int (SC + 0x0a) : mword 64) 2 = mword_of_int (SC + 0x0c))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpc0c) in "Hpc".
    (* +0x0c c.sdsp s4,32 *)
    iPoseProof (schi_0c with "Htext") as "Hi0c".
    iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (SC + 0x0c)) (mword_of_int 4 : mword 6) Rs4
              A0 (av - 10)%nat u6 with "Hcg Hpc Hi0c [Hf6] [-]").
    { iEval (rewrite HcspA0 -Hb6). iExact "Hf6". }
    iIntros "Hcg Hpc _".
    assert (Hpc0e : add_vec_int (mword_of_int (SC + 0x0c) : mword 64) 2 = mword_of_int (SC + 0x0e))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpc0e) in "Hpc".
    (* +0x0e c.sdsp s5,24 *)
    iPoseProof (schi_0e with "Htext") as "Hi0e".
    iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (SC + 0x0e)) (mword_of_int 3 : mword 6) Rs5
              A0 (av - 10)%nat u7 with "Hcg Hpc Hi0e [Hf7] [-]").
    { iEval (rewrite HcspA0 -Hb7). iExact "Hf7". }
    iIntros "Hcg Hpc _".
    assert (Hpc10 : add_vec_int (mword_of_int (SC + 0x0e) : mword 64) 2 = mword_of_int (SC + 0x10))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpc10) in "Hpc".
    (* +0x10 c.sdsp s6,16 *)
    iPoseProof (schi_10 with "Htext") as "Hi10".
    iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (SC + 0x10)) (mword_of_int 2 : mword 6) Rs6
              A0 (av - 10)%nat u8 with "Hcg Hpc Hi10 [Hf8] [-]").
    { iEval (rewrite HcspA0 -Hb8). iExact "Hf8". }
    iIntros "Hcg Hpc _".
    assert (Hpc12 : add_vec_int (mword_of_int (SC + 0x10) : mword 64) 2 = mword_of_int (SC + 0x12))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpc12) in "Hpc".
    (* +0x12 c.sdsp s7,8 *)
    iPoseProof (schi_12 with "Htext") as "Hi12".
    iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (SC + 0x12)) (mword_of_int 1 : mword 6) Rs7
              A0 (av - 10)%nat u9 with "Hcg Hpc Hi12 [Hf9] [-]").
    { iEval (rewrite HcspA0 -Hb9). iExact "Hf9". }
    iIntros "Hcg Hpc _".
    assert (Hpc14 : add_vec_int (mword_of_int (SC + 0x12) : mword 64) 2 = mword_of_int (SC + 0x14))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpc14) in "Hpc".
    (* +0x14 c.sdsp s8,0 *)
    iPoseProof (schi_14 with "Htext") as "Hi14".
    iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (SC + 0x14)) (mword_of_int 0 : mword 6) Rs8
              A0 (av - 10)%nat u10 with "Hcg Hpc Hi14 [Hf10] [-]").
    { iEval (rewrite HcspA0 -Hb10). iExact "Hf10". }
    iIntros "Hcg Hpc _".
    assert (Hpc16 : add_vec_int (mword_of_int (SC + 0x14) : mword 64) 2 = mword_of_int (SC + 0x16))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpc16) in "Hpc".
    (* +0x16 c.addi4spn s0,sp,80 -- the frame pointer, a dead value here *)
    iPoseProof (schi_16 with "Htext") as "Hi16".
    iApply (wp_caddi4spn_s_sconf γ Φ (mword_of_int (SC + 0x16)) (Cregidx (mword_of_int 0))
    ...
    assert (Hpc18 : add_vec_int (mword_of_int (SC + 0x16) : mword 64) 2 = mword_of_int (SC + 0x18))
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (wp_cmv_s_sconf γ Φ (mword_of_int (SC + 0x18)) Ra5 Rtp A1 (av - 10)%nat
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi18 [-]").
    iIntros "Hcg Hpc".
    set (A2 := <[Regidx Ra5 := regval_into_reg (add_vec zero_reg (A1 !!! Regidx Rtp))]> A1).
    change (<[Regidx Ra5 := regval_into_reg (add_vec zero_reg (A1 !!! Regidx Rtp))]> A1) with A2.
    assert (Hpc1a : add_vec_int (mword_of_int (SC + 0x18) : mword 64) 2 = mword_of_int (SC + 0x1a))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpc1a) in "Hpc".
    (* +0x1a sext.w a5 *)
    iPoseProof (schi_1a with "Htext") as "Hi1a".
    iApply (wp_caddiw_s_sconf γ Φ (mword_of_int (SC + 0x1a)) Ra5 (mword_of_int 0 : mword 6)
              A2 (av - 10)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi1a [-]").
    iIntros "Hcg Hpc".
    ...
    assert (Hpc1c : add_vec_int (mword_of_int (SC + 0x1a) : mword 64) 2 = mword_of_int (SC + 0x1c))
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    { rewrite /A3 upd_eq /A2 upd_eq HA1tp. reflexivity. }
    (* +0x1c slli s6,a5,7 *)
    iPoseProof (schi_1c with "Htext") as "Hi1c".
    iApply (wp_slli_s_sconf γ Φ (mword_of_int (SC + 0x1c)) Rs6 Ra5 (mword_of_int 7 : mword 6)
              (mycpu_a5 cid_word) A3 (av - 10)%nat
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              ltac:(rewrite HA3a5; reflexivity)
              with "Hcg Hpc Hi1c [-]").
    iIntros "Hcg Hpc".
    set (A4 := <[Regidx Rs6 := regval_into_reg (mycpu_a5 cid_word)]> A3).
    change (<[Regidx Rs6 := regval_into_reg (mycpu_a5 cid_word)]> A3) with A4.
    assert (Hpc20 : add_vec_int (mword_of_int (SC + 0x1c) : mword 64) 4 = mword_of_int (SC + 0x20))
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (wp_auipc_s_sconf γ Φ (mword_of_int (SC + 0x20)) Ra4 (mword_of_int 0x10 : mword 20)
              A4 (av - 10)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi20 [-]").
    iIntros "Hcg Hpc".
    set (A5 := <[Regidx Ra4 := regval_into_reg
        (add_vec (mword_of_int (SC + 0x20) : mword 64) (auipc_off (mword_of_int 0x10 : mword 20)))]> A4).
    change (<[Regidx Ra4 := regval_into_reg
        (add_vec (mword_of_int (SC + 0x20) : mword 64) (auipc_off (mword_of_int 0x10 : mword 20)))]> A4) with A5.
    assert (Hpc24 : add_vec_int (mword_of_int (SC + 0x20) : mword 64) 4 = mword_of_int (SC + 0x24))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpc24) in "Hpc".
    (* +0x24 addi a4,a4,1454 *)
    iPoseProof (schi_24 with "Htext") as "Hi24".
    iApply (wp_addi4_s_sconf γ Φ (mword_of_int (SC + 0x24)) Ra4 Ra4 (mword_of_int 1454 : mword 12)
              A5 (av - 10)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi24 [-]").
    iIntros "Hcg Hpc".
    ...
    assert (Hpc28 : add_vec_int (mword_of_int (SC + 0x24) : mword 64) 4 = mword_of_int (SC + 0x28))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpc28) in "Hpc".
    assert (HA6a4 : A6 !!! Regidx Ra4
                    = add_vec (add_vec (mword_of_int (SC + 0x20) : mword 64) (auipc_off (mword_of_int 0x10 : mword 20)))
    ...
    iApply (wp_cadd_s_sconf γ Φ (mword_of_int (SC + 0x28)) Ra4 Rs6 A6 (av - 10)%nat
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi28 [-]").
    iIntros "Hcg Hpc".
    ...
    assert (Hpc2a : add_vec_int (mword_of_int (SC + 0x28) : mword 64) 2 = mword_of_int (SC + 0x2a))
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (wp_sd_zero_s_sconf γ Φ (mword_of_int (SC + 0x2a)) Ra4 (mword_of_int 48 : mword 12)
              A7 (av - 10)%nat p0 with "Hcg Hpc Hi2a [Hproc] [-]").
    { iEval (rewrite Hrec_proc0). iExact "Hproc". }
    iIntros "Hcg Hpc Hproc".
    iEval (rewrite Hrec_proc0) in "Hproc".
    assert (Hpc2e : add_vec_int (mword_of_int (SC + 0x2a) : mword 64) 4 = mword_of_int (SC + 0x2e))
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (wp_auipc_s_sconf γ Φ (mword_of_int (SC + 0x2e)) Ra4 (mword_of_int 0x10 : mword 20)
              A7 (av - 10)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi2e [-]").
    iIntros "Hcg Hpc".
    set (A8 := <[Regidx Ra4 := regval_into_reg
        (add_vec (mword_of_int (SC + 0x2e) : mword 64) (auipc_off (mword_of_int 0x10 : mword 20)))]> A7).
    change (<[Regidx Ra4 := regval_into_reg
        (add_vec (mword_of_int (SC + 0x2e) : mword 64) (auipc_off (mword_of_int 0x10 : mword 20)))]> A7) with A8.
    assert (Hpc32 : add_vec_int (mword_of_int (SC + 0x2e) : mword 64) 4 = mword_of_int (SC + 0x32))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpc32) in "Hpc".
    (* +0x32 addi a4,a4,1496 *)
    iPoseProof (schi_32 with "Htext") as "Hi32".
    iApply (wp_addi4_s_sconf γ Φ (mword_of_int (SC + 0x32)) Ra4 Ra4 (mword_of_int 1496 : mword 12)
              A8 (av - 10)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi32 [-]").
    iIntros "Hcg Hpc".
    ...
    assert (Hpc36 : add_vec_int (mword_of_int (SC + 0x32) : mword 64) 4 = mword_of_int (SC + 0x36))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpc36) in "Hpc".
    assert (HA9a4 : A9 !!! Regidx Ra4
                    = add_vec (add_vec (mword_of_int (SC + 0x2e) : mword 64) (auipc_off (mword_of_int 0x10 : mword 20)))
    ...
    iApply (wp_cadd_s_sconf γ Φ (mword_of_int (SC + 0x36)) Rs6 Ra4 A9 (av - 10)%nat
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi36 [-]").
    iIntros "Hcg Hpc".
    ...
    assert (Hpc38 : add_vec_int (mword_of_int (SC + 0x36) : mword 64) 2 = mword_of_int (SC + 0x38))
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (wp_cli_s_sconf γ Φ (mword_of_int (SC + 0x38)) Rs8 (mword_of_int 4 : mword 6)
              (sign_extend' 64 (RUNNING : mword 32)) A10 (av - 10)%nat
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
    ...
    assert (Hpc3a : add_vec_int (mword_of_int (SC + 0x38) : mword 64) 2 = mword_of_int (SC + 0x3a))
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (wp_cslli_s_sconf γ Φ (mword_of_int (SC + 0x3a)) (Regidx Ra5) Ra5 (mword_of_int 7 : mword 6)
              A11 (av - 10)%nat eq_refl ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi3a [-]").
    iIntros "Hcg Hpc".
    ...
    assert (Hpc3c : add_vec_int (mword_of_int (SC + 0x3a) : mword 64) 2 = mword_of_int (SC + 0x3c))
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (wp_auipc_s_sconf γ Φ (mword_of_int (SC + 0x3c)) Rs4 (mword_of_int 0x10 : mword 20)
              A12 (av - 10)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi3c [-]").
    iIntros "Hcg Hpc".
    set (A13 := <[Regidx Rs4 := regval_into_reg
        (add_vec (mword_of_int (SC + 0x3c) : mword 64) (auipc_off (mword_of_int 0x10 : mword 20)))]> A12).
    change (<[Regidx Rs4 := regval_into_reg
        (add_vec (mword_of_int (SC + 0x3c) : mword 64) (auipc_off (mword_of_int 0x10 : mword 20)))]> A12) with A13.
    assert (Hpc40 : add_vec_int (mword_of_int (SC + 0x3c) : mword 64) 4 = mword_of_int (SC + 0x40))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpc40) in "Hpc".
    (* +0x40 addi s4,s4,1426 *)
    iPoseProof (schi_40 with "Htext") as "Hi40".
    iApply (wp_addi4_s_sconf γ Φ (mword_of_int (SC + 0x40)) Rs4 Rs4 (mword_of_int 1426 : mword 12)
              A13 (av - 10)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi40 [-]").
    iIntros "Hcg Hpc".
    ...
    assert (Hpc44 : add_vec_int (mword_of_int (SC + 0x40) : mword 64) 4 = mword_of_int (SC + 0x44))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpc44) in "Hpc".
    assert (HA14s4 : A14 !!! Regidx Rs4
                     = add_vec (add_vec (mword_of_int (SC + 0x3c) : mword 64) (auipc_off (mword_of_int 0x10 : mword 20)))
    ...
    iApply (wp_cadd_s_sconf γ Φ (mword_of_int (SC + 0x44)) Rs4 Ra5 A14 (av - 10)%nat
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi44 [-]").
    iIntros "Hcg Hpc".
    ...
    assert (Hpc46 : add_vec_int (mword_of_int (SC + 0x44) : mword 64) 2 = mword_of_int (SC + 0x46))
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (wp_cli_s_sconf γ Φ (mword_of_int (SC + 0x46)) Rs7 (mword_of_int 1 : mword 6)
              (mword_of_int 1 : mword 64) A15 (av - 10)%nat
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              ltac:(apply bv_eq; vm_compute; reflexivity)
              with "Hcg Hpc Hi46 [-]").
    iIntros "Hcg Hpc".
    ...
    assert (Hpc48 : add_vec_int (mword_of_int (SC + 0x46) : mword 64) 2 = mword_of_int (SC + 0x48))
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    assert (HP_tp : A16 !!! Regidx Rtp = cid_word).
    { rewrite /A16 upd_ne; [| vm_compute; discriminate].
      rewrite /A15 upd_ne; [| vm_compute; discriminate].
      rewrite /A14 upd_ne; [| vm_compute; discriminate].
      rewrite /A13 upd_ne; [| vm_compute; discriminate].
      ...
      rewrite /A2 upd_ne; [| vm_compute; discriminate]. exact HA1tp. }
    (* refold the cpu bundle at [zero_reg] with an [emp] context slot. *)
    iAssert (cpu_own γ 0 false zero_reg emp) with "[Hnoff Hint Hcnt Hproc]" as "Hcpu".
    { rewrite /cpu_own. iFrame "Hnoff Hint Hcnt Hproc". iPureIntro; vm_compute; reflexivity. }
    (* ================================================================== *)
    ...
    iAssert (□ ( ∀ (jj : nat) (γl : gname) (Mt : regfile) (ebx : bool),
        ...
        ( ( ⌜(S jj < NPROC)%nat⌝ -∗ ∀ (Mn : regfile),
              ...
              sie_cap_gpr γ Mn (av - 10)%nat -∗
              pc_is (mword_of_int (SC + 0x58)) -∗
              cpu_own γ 0 ebx zero_reg emp -∗
          ...
          ∧ ( ∀ (Me : regfile),
              ...
              sie_cap_gpr γ Me (av - 10)%nat -∗
              pc_is (mword_of_int (SC + 0x7e)) -∗
              cpu_own γ 0 ebx zero_reg emp -∗
              (if ebx then emp else trap_csrs) -∗
              own_ctx (a_cpu_ctx cid_word) -∗
              WP (Loop : expr riscv_lang) {{ Φ }} ) ) -∗
    ...
    { iModIntro.
      ...
      (* +0x4a c.mv a0,s1 *)
      iApply (wp_cmv_s_sconf γ Φ (mword_of_int (SC + 0x4a)) Ra0 Rs1 Mt (av - 10)%nat
                ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                with "Hcg Hpc Hi4a [-]").
      iIntros "Hcg Hpc".
      ...
      assert (Hq4c : add_vec_int (mword_of_int (SC + 0x4a) : mword 64) 2 = mword_of_int (SC + 0x4c))
        by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hq4c) in "Hpc".
      (* +0x4c jal release *)
      iApply (wp_jal_s_sconf γ Φ (mword_of_int (SC + 0x4c)) Rra (mword_of_int 2092746 : mword 21)
                T0 (av - 10)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                ltac:(vm_compute; reflexivity) with "Hcg Hpc Hi4c [-]").
      iIntros "Hcg Hpc".
      set (T1 := <[Regidx Rra := regval_into_reg (add_vec_int (mword_of_int (SC + 0x4c) : mword 64) 4)]> T0).
      change (<[Regidx Rra := regval_into_reg (add_vec_int (mword_of_int (SC + 0x4c) : mword 64) 4)]> T0) with T1.
      assert (Hqrl : add_vec (mword_of_int (SC + 0x4c) : mword 64) (sign_extend' 64 (mword_of_int 2092746 : mword 21))
                     = mword_of_int KernelSyms.release) by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hqrl) in "Hpc".
      assert (HT1ra : T1 !!! Regidx Rra = add_vec_int (mword_of_int (SC + 0x4c) : mword 64) 4)
      ...
      { rewrite /T1 upd_ne; [| vm_compute; discriminate].
        rewrite /T0 upd_ne; [| vm_compute; discriminate]. exact Hptp. }
      ...
      assert (Hq50 : ret_pc (T1 !!! Regidx Rra) = mword_of_int (SC + 0x50))
      ...
      (* +0x50 addi s1,s1,360 *)
      iApply (wp_addi4_s_sconf γ Φ (mword_of_int (SC + 0x50)) Rs1 Rs1 (mword_of_int 360 : mword 12)
                mr (av - 10)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                with "Hcg Hpc Hi50 [-]").
      iIntros "Hcg Hpc".
      ...
      assert (Hq54 : add_vec_int (mword_of_int (SC + 0x50) : mword 64) 4 = mword_of_int (SC + 0x54))
        by (apply bv_eq; vm_compute; reflexivity).
      ...
      assert (HT2tp : T2 !!! Regidx Rtp = cid_word)
        by (rewrite (HT2 Rtp ltac:(vm_compute; reflexivity) ltac:(vm_compute; discriminate)); exact Hptp).
      (* +0x54 beq s1,s2 : the scan's end test *)
      destruct (decide (S jj = NPROC)) as [Hend | Hne].
      - (* scan finished: branch TAKEN to +0x7e *)
        iApply (wp_beq_taken_s_sconf γ Φ (mword_of_int (SC + 0x54)) (mword_of_int 42 : mword 13)
                  Rs2 Rs1 T2 (av - 10)%nat
                  ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                  ltac:(rewrite HT2s1 HT2s2 Hend; apply eq_vec_true_iff; reflexivity)
                  ltac:(vm_compute; reflexivity)
                  with "Hcg Hpc Hi54 [-]").
        iApply bi.later_intro. iIntros "Hcg Hpc".
        assert (Hq7e : add_vec (mword_of_int (SC + 0x54) : mword 64) (sign_extend' 64 (mword_of_int 42 : mword 13))
                       = mword_of_int (SC + 0x7e)) by (apply bv_eq; vm_compute; reflexivity).
      ...
      - (* keep scanning: branch FALLS to +0x58 *)
        ...
        iApply (wp_beq_fall_s_sconf γ Φ (mword_of_int (SC + 0x54)) (mword_of_int 42 : mword 13)
                  Rs2 Rs1 T2 (av - 10)%nat
                  ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                  ltac:(rewrite HT2s1 HT2s2; apply sc_eq_vec_ne; exact (sc_proc_addr_ne_end (S jj) HSjj))
                  with "Hcg Hpc Hi54 [-]").
        iIntros "Hcg Hpc".
        assert (Hq58 : add_vec_int (mword_of_int (SC + 0x54) : mword 64) 4 = mword_of_int (SC + 0x58))
          by (apply bv_eq; vm_compute; reflexivity).
    ...
    iAssert (□ ( ∀ (fuel jj : nat) (M : regfile) (ebc : bool),
        ...
        sie_cap_gpr γ M (av - 10)%nat -∗
        pc_is (mword_of_int (SC + 0x58)) -∗
        cpu_own γ 0 ebc zero_reg emp -∗
    ...
    { iModIntro. iIntros (fuel). iInduction fuel as [|fuel IH] "IH";
      ...
      (* +0x58 c.mv a0,s1 *)
      iApply (wp_cmv_s_sconf γ Φ (mword_of_int (SC + 0x58)) Ra0 Rs1 M (av - 10)%nat
                ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                with "Hcg Hpc Hi58 [-]").
      iIntros "Hcg Hpc".
      ...
      assert (Hr5a : add_vec_int (mword_of_int (SC + 0x58) : mword 64) 2 = mword_of_int (SC + 0x5a))
        by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hr5a) in "Hpc".
      (* +0x5a jal acquire *)
      iApply (wp_jal_s_sconf γ Φ (mword_of_int (SC + 0x5a)) Rra (mword_of_int 2092596 : mword 21)
                M0 (av - 10)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                ltac:(vm_compute; reflexivity) with "Hcg Hpc Hi5a [-]").
      iIntros "Hcg Hpc".
      set (M1 := <[Regidx Rra := regval_into_reg (add_vec_int (mword_of_int (SC + 0x5a) : mword 64) 4)]> M0).
      change (<[Regidx Rra := regval_into_reg (add_vec_int (mword_of_int (SC + 0x5a) : mword 64) 4)]> M0) with M1.
      assert (Hraq : add_vec (mword_of_int (SC + 0x5a) : mword 64) (sign_extend' 64 (mword_of_int 2092596 : mword 21))
                     = mword_of_int KernelSyms.acquire) by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hraq) in "Hpc".
      assert (HM1ra : M1 !!! Regidx Rra = add_vec_int (mword_of_int (SC + 0x5a) : mword 64) 4)
      ...
      assert (Hr5e : ret_pc (M1 !!! Regidx Rra) = mword_of_int (SC + 0x5e))
      ...
      assert (Hrec_st : add_vec (macq !!! Regidx Rs1) (sign_extend' 64 (mword_of_int 24 : mword 12))
                        = p_state (proc_addr jj)) by (rewrite Hq1; apply sc_state_addr).
      iApply (wp_clw_s_sconf γ Φ (mword_of_int (SC + 0x5e)) Ra5 Rs1 (mword_of_int 24 : mword 12)
                macq (av - 10)%nat st ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
      ...
      assert (Hr60 : add_vec_int (mword_of_int (SC + 0x5e) : mword 64) 2 = mword_of_int (SC + 0x60))
        by (apply bv_eq; vm_compute; reflexivity).
      ...
      assert (HM2tp : M2 !!! Regidx Rtp = cid_word)
        by (rewrite (HM2 Rtp ltac:(vm_compute; reflexivity)); exact Hptp).
      ...
      - (* not RUNNABLE: skip straight to the release tail at +0x4a *)
        iApply (wp_bne_taken_s_sconf γ Φ (mword_of_int (SC + 0x60)) (mword_of_int 8170 : mword 13)
                  Rs3 Ra5 M2 (av - 10)%nat
                  ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                  ltac:(rewrite HM2a5 HM2s3; exact Hcmp)
                  ltac:(vm_compute; reflexivity)
                  with "Hcg Hpc Hi60 [-]").
        iApply bi.later_intro. iIntros "Hcg Hpc".
        assert (Hr4a : add_vec (mword_of_int (SC + 0x60) : mword 64) (sign_extend' 64 (mword_of_int 8170 : mword 13))
      ...
      - (* RUNNABLE: dispatch it *)
        ...
        iApply (wp_bne_fall_s_sconf γ Φ (mword_of_int (SC + 0x60)) (mword_of_int 8170 : mword 13)
                  ...
                  with "Hcg Hpc Hi60 [-]").
        iIntros "Hcg Hpc".
        assert (Hr64 : add_vec_int (mword_of_int (SC + 0x60) : mword 64) 4 = mword_of_int (SC + 0x64))
          by (apply bv_eq; vm_compute; reflexivity).
        ...
        iEval (rewrite /proc_slots needs_ctx_RUNNABLE inv_dormant_RUNNABLE) in "Hslot".
        iDestruct "Hslot" as "[Hvc _]".
        (* +0x64 sw s8,24(s1) : p->state = RUNNING *)
        iApply (wp_sw_s_sconf γ Φ (mword_of_int (SC + 0x64)) Rs8 Rs1 (mword_of_int 24 : mword 12)
                  M2 (av - 10)%nat RUNNABLE with "Hcg Hpc Hi64 [Hstate] [-]").
        { iEval (rewrite HM2s1 sc_state_addr). iExact "Hstate". }
        iIntros "Hcg Hpc Hstate".
        iEval (rewrite HM2s1 sc_state_addr HM2s8) in "Hstate".
        assert (Hr68 : add_vec_int (mword_of_int (SC + 0x64) : mword 64) 4 = mword_of_int (SC + 0x68))
          by (apply bv_eq; vm_compute; reflexivity).
        iEval (rewrite Hr68) in "Hpc".
        (* +0x68 sd s1,48(s4) : c->proc = p *)
        iDestruct (cpu_own_set_proc γ 1 ebc zero_reg (proc_addr jj) emp with "Hcpu") as "[Hproc Hback]".
        iApply (wp_sd_s_sconf γ Φ (mword_of_int (SC + 0x68)) Rs1 Rs4 (mword_of_int 48 : mword 12)
                  M2 (av - 10)%nat zero_reg with "Hcg Hpc Hi68 [Hproc] [-]").
        { iEval (rewrite HM2s4). iExact "Hproc". }
        iIntros "Hcg Hpc Hproc".
        iEval (rewrite HM2s4 HM2s1) in "Hproc".
        iDestruct ("Hback" with "Hproc") as "Hcpu".
        assert (Hr6c : add_vec_int (mword_of_int (SC + 0x68) : mword 64) 4 = mword_of_int (SC + 0x6c))
          by (apply bv_eq; vm_compute; reflexivity).
        iEval (rewrite Hr6c) in "Hpc".
        (* +0x6c addi a1,s1,96 : &p->context *)
        iApply (wp_addi4_s_sconf γ Φ (mword_of_int (SC + 0x6c)) Ra1 Rs1 (mword_of_int 96 : mword 12)
                  M2 (av - 10)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                  with "Hcg Hpc Hi6c [-]").
        iIntros "Hcg Hpc".
        ...
        assert (Hr70 : add_vec_int (mword_of_int (SC + 0x6c) : mword 64) 4 = mword_of_int (SC + 0x70))
          by (apply bv_eq; vm_compute; reflexivity).
        iEval (rewrite Hr70) in "Hpc".
        (* +0x70 c.mv a0,s6 : &c->context *)
        iApply (wp_cmv_s_sconf γ Φ (mword_of_int (SC + 0x70)) Ra0 Rs6 M3 (av - 10)%nat
                  ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                  with "Hcg Hpc Hi70 [-]").
        iIntros "Hcg Hpc".
        ...
        assert (Hr72 : add_vec_int (mword_of_int (SC + 0x70) : mword 64) 2 = mword_of_int (SC + 0x72))
          by (apply bv_eq; vm_compute; reflexivity).
        iEval (rewrite Hr72) in "Hpc".
        (* +0x72 jal swtch *)
        iApply (wp_jal_s_sconf γ Φ (mword_of_int (SC + 0x72)) Rra (mword_of_int 1452 : mword 21)
                  M4 (av - 10)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                  ltac:(vm_compute; reflexivity) with "Hcg Hpc Hi72 [-]").
        iIntros "Hcg Hpc".
        set (Mc := <[Regidx Rra := regval_into_reg (add_vec_int (mword_of_int (SC + 0x72) : mword 64) 4)]> M4).
        change (<[Regidx Rra := regval_into_reg (add_vec_int (mword_of_int (SC + 0x72) : mword 64) 4)]> M4) with Mc.
        assert (Hrsw : add_vec (mword_of_int (SC + 0x72) : mword 64) (sign_extend' 64 (mword_of_int 1452 : mword 21))
        ...
        assert (HMcra : Mc !!! Regidx Rra = add_vec_int (mword_of_int (SC + 0x72) : mword 64) 4)
        ...
        { rewrite /Mc upd_ne; [| vm_compute; discriminate].
          rewrite /M4 upd_ne; [| vm_compute; discriminate].
          rewrite /M3 upd_ne; [| vm_compute; discriminate]. exact HM2tp. }
        ...
        assert (Hr76 : ret_pc (m' !!! Regidx Rra) = mword_of_int (SC + 0x76))
        ...
        { rewrite Hm20.
          ...
          exact HM2s4. }
        iDestruct (cpu_own_set_proc γ 1 eb' (proc_addr jj) zero_reg emp with "Hcpu") as "[Hproc Hback]".
        iApply (wp_sd_zero_s_sconf γ Φ (mword_of_int (SC + 0x76)) Rs4 (mword_of_int 48 : mword 12)
                  m' (av - 10)%nat (proc_addr jj) with "Hcg Hpc Hi76 [Hproc] [-]").
        { iEval (rewrite Hm's4). iExact "Hproc". }
        iIntros "Hcg Hpc Hproc".
        iEval (rewrite Hm's4) in "Hproc".
        iDestruct ("Hback" with "Hproc") as "Hcpu".
        assert (Hr7a : add_vec_int (mword_of_int (SC + 0x76) : mword 64) 4 = mword_of_int (SC + 0x7a))
          by (apply bv_eq; vm_compute; reflexivity).
        iEval (rewrite Hr7a) in "Hpc".
        (* +0x7a c.mv s5,s7 : found := 1 *)
        iApply (wp_cmv_s_sconf γ Φ (mword_of_int (SC + 0x7a)) Rs5 Rs7 m' (av - 10)%nat
                  ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                  with "Hcg Hpc Hi7a [-]").
        iIntros "Hcg Hpc".
        ...
        assert (Hr7c : add_vec_int (mword_of_int (SC + 0x7a) : mword 64) 2 = mword_of_int (SC + 0x7c))
          by (apply bv_eq; vm_compute; reflexivity).
        ...
        { rewrite /M5 upd_ne; [| vm_compute; discriminate]. exact Htpv. }
        ...
        iApply (wp_cj_s_sconf γ Φ (mword_of_int (SC + 0x7c))
                  (sign_extend' 21 (concat_vec (mword_of_int 2023 : mword 11) ('b"0"))) M5 (av - 10)%nat
                  ltac:(vm_compute; reflexivity) with "Hcg Hpc Hi7c [-]").
        iApply bi.later_intro. iIntros "Hcg Hpc".
        assert (Hr4a2 : add_vec (mword_of_int (SC + 0x7c) : mword 64)
    ...
    iAssert (□ ( ∀ (M : regfile) (eb : bool),
        ⌜ add_vec (M !!! Regidx Rs4) (sign_extend' 64 (mword_of_int 48 : mword 12)) = a_cpu_proc cid_word
          ...
          /\ M !!! Regidx Rtp = cid_word ⌝ -∗
        sie_cap_gpr γ M (av - 10)%nat -∗
        pc_is (mword_of_int (SC + 0x86)) -∗
        cpu_own γ 0 eb zero_reg emp -∗
    ...
    { iModIntro. iLöb as "IHo".
      ...
      destruct Hpo as (Ho4 & Ho6 & Ho7 & Ho8 & Hotp).
      ...
      (* the flip leaves take [intr_count] separately; open the bundle *)
      iDestruct "Hcpu" as "(%Hbo1 & Hnoff & Hint & Hcnt & Hproc & _)".
      (* +0x86 csrsi sstatus,2 : intr_on *)
      iApply (wp_csrsi_sstatus_x0_enable_s_sconf γ Φ (mword_of_int (SC + 0x86)) eb M (av - 10)%nat
                with "Hcg Hcnt Hcsrs Havail Hpc Hi86 [-]").
      iIntros (ms1) "%Hmsf1 Hcg Hcnt Hpc".
      assert (Ho8a : add_vec_int (mword_of_int (SC + 0x86) : mword 64) 4 = mword_of_int (SC + 0x8a))
        by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Ho8a) in "Hpc".
      (* +0x8a csrci sstatus,2 : intr_off *)
      iApply (wp_csrci_sstatus_x0_s_sconf γ Φ (mword_of_int (SC + 0x8a)) M (av - 10)%nat
                with "Hcg Hcnt Hpc Hi8a [-]").
      iIntros (ms2) "%Hmsf2 Hcg Hcnt Hcsrs Hpc".
      ...
      iApply (wp_cli_s_sconf γ Φ (mword_of_int (SC + 0x8e)) Rs5 (mword_of_int 0 : mword 6)
                (zero_reg : mword 64) M (av - 10)%nat
                ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                ltac:(apply bv_eq; vm_compute; reflexivity)
                with "Hcg Hpc Hi8e [-]").
      iIntros "Hcg Hpc".
      ...
      assert (Ho90 : add_vec_int (mword_of_int (SC + 0x8e) : mword 64) 2 = mword_of_int (SC + 0x90))
        by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Ho90) in "Hpc".
      (* +0x90 auipc s1,0x11 *)
      iApply (wp_auipc_s_sconf γ Φ (mword_of_int (SC + 0x90)) Rs1 (mword_of_int 0x11 : mword 20)
                B0 (av - 10)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                with "Hcg Hpc Hi90 [-]").
      iIntros "Hcg Hpc".
      set (B1 := <[Regidx Rs1 := regval_into_reg
          (add_vec (mword_of_int (SC + 0x90) : mword 64) (auipc_off (mword_of_int 0x11 : mword 20)))]> B0).
      change (<[Regidx Rs1 := regval_into_reg
          (add_vec (mword_of_int (SC + 0x90) : mword 64) (auipc_off (mword_of_int 0x11 : mword 20)))]> B0) with B1.
      assert (Ho94 : add_vec_int (mword_of_int (SC + 0x90) : mword 64) 4 = mword_of_int (SC + 0x94))
        by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Ho94) in "Hpc".
      (* +0x94 addi s1,s1,2414 : &proc[0] *)
      iApply (wp_addi4_s_sconf γ Φ (mword_of_int (SC + 0x94)) Rs1 Rs1 (mword_of_int 2414 : mword 12)
                B1 (av - 10)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                with "Hcg Hpc Hi94 [-]").
      iIntros "Hcg Hpc".
      ...
      assert (Ho98 : add_vec_int (mword_of_int (SC + 0x94) : mword 64) 4 = mword_of_int (SC + 0x98))
        by (apply bv_eq; vm_compute; reflexivity).
      ...
      iApply (wp_cli_s_sconf γ Φ (mword_of_int (SC + 0x98)) Rs3 (mword_of_int 3 : mword 6)
                (sign_extend' 64 (RUNNABLE : mword 32)) B2 (av - 10)%nat
                ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                ltac:(apply bv_eq; vm_compute; reflexivity)
                with "Hcg Hpc Hi98 [-]").
      iIntros "Hcg Hpc".
      ...
      assert (Ho9a : add_vec_int (mword_of_int (SC + 0x98) : mword 64) 2 = mword_of_int (SC + 0x9a))
        by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Ho9a) in "Hpc".
      (* +0x9a auipc s2,0x16 *)
      iApply (wp_auipc_s_sconf γ Φ (mword_of_int (SC + 0x9a)) Rs2 (mword_of_int 0x16 : mword 20)
                B3 (av - 10)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                with "Hcg Hpc Hi9a [-]").
      iIntros "Hcg Hpc".
      set (B4 := <[Regidx Rs2 := regval_into_reg
          (add_vec (mword_of_int (SC + 0x9a) : mword 64) (auipc_off (mword_of_int 0x16 : mword 20)))]> B3).
      change (<[Regidx Rs2 := regval_into_reg
          (add_vec (mword_of_int (SC + 0x9a) : mword 64) (auipc_off (mword_of_int 0x16 : mword 20)))]> B3) with B4.
      assert (Ho9e : add_vec_int (mword_of_int (SC + 0x9a) : mword 64) 4 = mword_of_int (SC + 0x9e))
        by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Ho9e) in "Hpc".
      (* +0x9e addi s2,s2,868 : &proc[NPROC] *)
      iApply (wp_addi4_s_sconf γ Φ (mword_of_int (SC + 0x9e)) Rs2 Rs2 (mword_of_int 868 : mword 12)
                B4 (av - 10)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                with "Hcg Hpc Hi9e [-]").
      iIntros "Hcg Hpc".
      ...
      assert (Hoa2 : add_vec_int (mword_of_int (SC + 0x9e) : mword 64) 4 = mword_of_int (SC + 0xa2))
        by (apply bv_eq; vm_compute; reflexivity).
      ...
      { rewrite (HB5thr Rtp ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                   ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)). exact Hotp. }
      (* +0xa2 c.j -0x4a : enter the scan at j = 0 *)
      iApply (wp_cj_s_sconf γ Φ (mword_of_int (SC + 0xa2))
                (sign_extend' 21 (concat_vec (mword_of_int 2011 : mword 11) ('b"0"))) B5 (av - 10)%nat
                ltac:(vm_compute; reflexivity) with "Hcg Hpc Hia2 [-]").
      iApply bi.later_intro. iIntros "Hcg Hpc".
      assert (Ho58 : add_vec (mword_of_int (SC + 0xa2) : mword 64)
                       (sign_extend' 64 (sign_extend' 21 (concat_vec (mword_of_int 2011 : mword 11) ('b"0"))))
                     = mword_of_int (SC + 0x58)) by (apply bv_eq; vm_compute; reflexivity).
      ...
      destruct Hpe as (He4 & He6 & He7 & He8 & Hetp).
      destruct (neq_vec (Me !!! Regidx Rs5) zero_reg) eqn:Hs5v.
      + (* a proc ran: straight back to the loop head *)
        iApply (wp_bnez_x0_taken_s_sconf γ Φ (mword_of_int (SC + 0x7e)) (mword_of_int 8 : mword 13)
                  Rs5 Me (av - 10)%nat ltac:(vm_compute; discriminate) Hs5v
                  ltac:(vm_compute; reflexivity) with "Hcg Hpc Hi7e [-]").
        iNext. iIntros "Hcg Hpc".
        assert (Hb86 : add_vec (mword_of_int (SC + 0x7e) : mword 64) (sign_extend' 64 (mword_of_int 8 : mword 13))
                       = mword_of_int (SC + 0x86)) by (apply bv_eq; vm_compute; reflexivity).
      ...
      + (* nothing runnable: SIE is provably off, so wfi is legal *)
        ...
        subst eb2.
        iApply (wp_bnez_x0_fall_s_sconf γ Φ (mword_of_int (SC + 0x7e)) (mword_of_int 8 : mword 13)
                  Rs5 Me (av - 10)%nat ltac:(vm_compute; discriminate) Hs5v
                  with "Hcg Hpc Hi7e [-]").
        iIntros "Hcg Hpc".
        assert (Hb82 : add_vec_int (mword_of_int (SC + 0x7e) : mword 64) 4 = mword_of_int (SC + 0x82))
          by (apply bv_eq; vm_compute; reflexivity).
        iEval (rewrite Hb82) in "Hpc".
        (* +0x82 wfi *)
        iDestruct "Hcpu" as "(%Hbo2 & Hnoff & Hint & Hcnt & Hproc & _)".
        iApply (wp_wfi_s_sconf γ Φ (mword_of_int (SC + 0x82)) Me (av - 10)%nat
                  with "Hcg Hcnt Hpc Hi82 [-]").
        iNext. iIntros "Hcg Hcnt Hpc".
        assert (Hb86b : add_vec_int (mword_of_int (SC + 0x82) : mword 64) 4 = mword_of_int (SC + 0x86))
          by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (wp_cj_s_sconf γ Φ (mword_of_int (SC + 0x48))
              (sign_extend' 21 (concat_vec (mword_of_int 31 : mword 11) ('b"0"))) A16 (av - 10)%nat
              ltac:(vm_compute; reflexivity) with "Hcg Hpc Hi48 [-]").
    iApply bi.later_intro. iIntros "Hcg Hpc".
    assert (Hpc86 : add_vec (mword_of_int (SC + 0x48) : mword 64)
