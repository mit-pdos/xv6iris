...
Section ForwardCsrrGpr.
  ...
  Proof using All.
    ...
    assert (LSA : eq_vec (_get_Misa_S (register_lookup misa sAcg.(sregs))) ('b"1") = true).
    { unfold sAcg, set_reg; cbn [sregs]. rewrite irrelevant_register_set;
...
Section WpCsrrGpr.
  ...
  Lemma wp_csrr_gpr (pc : mword 64) (w : mword 32) (rd : mword 5)
      (m : gmap register_bitvector_64 (mword 64)) (vd mhartid_in misa0 mdv0 : mword 64)
    ...
    minstret_inv -∗
    PC ↦ᵣ pc -∗ gpr_file m -∗ mhartid ↦ᵣ mhartid_in -∗ reg_pointsto misa dqc misa0 -∗ nextPC ↦ᵣ npc0 -∗
    ...
    ▷ ( PC ↦ᵣ add_vec_int pc 4 -∗
        ...
        mhartid ↦ᵣ mhartid_in -∗ reg_pointsto misa dqc misa0 -∗
  ...
  Proof.
    iIntros (HN Hrd Hmd HS Hpmaall Hpmpf Halignf Hbit0f Hbit1f Hvalignf HnotRVCf Hdec Hb1 HmIE Help)
      "#Hinv Hpc Hfile Hmh Hmisa Hnpc Hpriv Hhs Hmdl Hms Help Hmcinh Hmcfg Hpmpc Hpma Hhtif Hibytes Hcont".
    ...
    iDestruct (reg_valid_dq with "Hreg Hmh")    as %Lmh.
    ...
    iModIntro.
    iExists (sFccg s pc b1 rd (register_lookup minstret s.(sregs))). iSplitR.
    { iPureIntro.
      rewrite <- (sFcg'_eq s pc b1 rd (register_lookup minstret s.(sregs)) Lpc eq_refl).
    ...
    - iMod (reg_update _ minstret _ (add_vec_int (register_lookup minstret s.(sregs)) 1) with "Hreg Hmst") as "[Hreg Hmst]".
      ...
      iApply ("Hcont" with "Hpc Hfile Hmh Hmisa Hnpc Hpriv Hhs Hmdl Hms Help Hmcinh Hmcfg Hpmpc Hpma Hhtif Hibytes").
    - iModIntro. unfold set_reg; cbn [sregs mem]. iFrame "Hreg Hmem".
...
Section WpCsrrGprDemo.
  Context `{!riscvGS Σ}.
  Context {dqc : dfrac}.
  Definition wp_csrr_x5  (pc : mword 64) (w : mword 32) := wp_csrr_gpr (dqc:=DfracOwn 1) pc w (mword_of_int 5).
  Definition wp_csrr_x28 (pc : mword 64) (w : mword 32) := wp_csrr_gpr (dqc:=DfracOwn 1) pc w (mword_of_int 28).
