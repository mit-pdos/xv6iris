...
Section KforkArms.
  ...
  (* =================================================================== *)
  Lemma kfork_arm2
      (γa : gname) (γk : gname * gname) (γf γl2 : gname) (γs : list gname) 
  ...
  Proof.
    ...
    iAssert (∃ w4', word_pointsto (KTR := KT1) (pa_stk sp0 4) (DfracOwn 1) w4')%I with "[Hb4]" as "Hb4x".
    { iExists w4. iExact "Hb4". }
    iAssert (∃ w5', word_pointsto (KTR := KT1) (pa_stk sp0 5) (DfracOwn 1) w5')%I with "[Hb5]" as "Hb5x".
  ...
  (* =================================================================== *)
  Lemma kfork_arm1
      (γa : gname) (γk : gname * gname) (γf : gname) 
      (m : regfile) (K lvl : nat) (eb b : bool) (pme : mword 64)
      (pid_p : mword 32) (Vp : pprivate)
  ...
  (* =================================================================== *)
  Lemma kfork_arm3
      (γa : gname) (γk : gname * gname) (γf γil γic γw γl : gname) (γs : list gname)
      (logstart : Z)
  ...
  Proof.
    ...
    - iApply wp_next_off_intro. iIntros (Mx2) "%Hregs2 Hsc Hown Hpcx Hpvx Hpvcx".
      ...
      iApply (B4.kfk_b4 γf γil γic logstart inodestart nib
                pid_p pid_c Vp
...
Section KforkMain.
 ...
 :
    wp_kfork_sconf_body γa γk γp γw γl γf γil γic γs logstart
  ...
  Proof.
    ...
    iApply (B6.kfk_prologue γa γk γp γw γl γf γil γic γs logstart nib
              ...
              (wp_next b pme (fun (CID : CpuId) =>
                 (∀ mr : regfile,
                    ...
                    kfork_post γa γk γf lvl eb pme b pid_p Vp
    ...
    - (* ---- arm 1: allocproc found no free slot, +0x10a ---- *)
      ...
      iAssert (kalloc_env_at γa γk None) with "[Hke]" as "Hke".
      { iDestruct "Hke" as "[$ | (% & _ & $)]". }
      iApply (kfork_arm1 (CID0 := CID1) γa γk γf m K lvl eb b pme pid_p Vp
    ...
    - (* ---- arm 2: uvmcopy failed, +0x7c ---- *)
      ...
      iApply (kfork_arm2 (CID0 := CID2) γa γk γf γl2 γs m K lvl eb b pme
    ...
    - (* ---- arm 3: uvmcopy succeeded, the copy loop's head at +0x4a ---- *)
      ...
      iApply (kfork_arm3 (CID0 := CID3) γa γk γf γil γic γw γl γs
                logstart inodestart nib m K lvl eb b pme
                pid_p Vp (m !!! Regidx csp_rs1) (m !!! Regidx Rra)
                (m !!! Regidx Rs0) (m !!! Regidx Rs1) (m !!! Regidx Rs5)
