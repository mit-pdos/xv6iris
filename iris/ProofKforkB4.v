...
Section KforkB4Proof.
  ...
  Proof.
    ...
    assert (Hjidup : add_vec (mword_of_int (KF + 0xa8) : mword 64)
                       (sign_extend' 64 (mword_of_int 5342 : mword 21))
                     = mword_of_int KernelSyms.idup)
      by (apply bv_eq; vm_compute; reflexivity).
    iApply (wp_jal_s_sconf (mword_of_int (KF + 0xa8)) Rra (mword_of_int 5342 : mword 21)
    ...
    (* THE idup CALL.                                                 *)
    (* ------------------------------------------------------------- *)
    iApply (ID.wp_idup_sconf γil γic inodestart nib
