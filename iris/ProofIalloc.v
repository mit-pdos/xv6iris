...
(*  [auipc a0,0x4 / addi a0,a0,850] at +0x72 resolves to 0x80007428.      *)
...
Definition ia_msg_addr : Z := 0x80007458.
 ...
 (ty : mword 16)
  ...
  Proof.
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.ialloc + 0x7a)) Rra
              (mword_of_int 2085668 : mword 21) Q8 (K - 8)%nat b
    ...
    assert (Htgtpk : add_vec (mword_of_int (KernelSyms.ialloc + 0x7a) : mword 64)
                       (sign_extend' 64 (mword_of_int 2085668 : mword 21))
    ...
    iDestruct (cpu_own_transport CID10 CID11 0 true (proc_addr j) b
                 ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
    iApply (ia_epilogue (CID0 := CID11) j ty u Sb
              t qt pidv dq dqs dqn m QB K b lks Upr HK Hty Htyk HQBsp HQBthr
...
(*  is [InodeRegion.ireg_claim_au ⊤ fsc_ireg fsc_fs inodestart nib inum            *)
 ...
 (ty : mword 16)
  ...
  Proof.
    ...
    iIntros "Hcg Hcnt #Htext #Hkdata Hpc #Hpanenv #Hbio #Hlctx #Hireg #Hiopen
              ...
              Hframe Hppid Hsbn Hsbi Hsl Hop Htx Hheld Hcont".
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.ialloc + 0x90)) Rra
              (mword_of_int 2087680 : mword 21) W2 (K - 8)%nat b
              ltac:(nz) ltac:(rdok) ltac:(vm_compute; reflexivity)
              with "Hcg Hpc []").
    ...
    assert (Htgtms : add_vec (mword_of_int (KernelSyms.ialloc + 0x90) : mword 64)
                       (sign_extend' 64 (mword_of_int 2087680 : mword 21))
    ...
    iApply (LW.wp_log_write_au_range fsc_bio icfg_log fsc_fs fsc_disk fsc_cov fsc_logst icfg_dev kk pidv bno
              ...
              false Sb e0 0%nat (⊤ ∖ ↑iregN) (iclaim (bv_unsigned inum) ty t qt)
              ...
              ltac:(lkbelow)
              with "Hcg Hcnt Htext Hpc Hbio Hlctx Hsl Hlb0 Hcrd HopS [Htx] Hheld").
    all: try lkbelow.
    { iEval (rewrite Hbno).
    ...
    iApply (BL.wp_brelse_sconf γs fsc_bio (fs_view fsc_fs fsc_disk icfg_dev fsc_cov) kk
              ...
              (diblk_bytes (<[DinodeEnc.islot inum := ialloc_fresh ty]> ds))
              bsd true b lks Upr HKbl Hkk HW7a0
    ...
    iEval (change (runit (is_claim (ClaimL ty t qt)) (bv_unsigned inum))
    ...
    iDestruct (cpu_own_transport CID16 CID23 0 true (proc_addr j) b
                 ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
    iApply (ia_epilogue (CID0 := CID23) j ty u Sb
              t qt pidv dq dqs dqn m V6 K b lks Upr HK Hty Htyk HV6sp HV6thr
              with "Hcg Hcnt Htext Hpc Hframe Hppid Hsbn Hsbi Hsl
 ...
 (ty : mword 16) (u : nat) (Sb : gset Z)
    ...
    (∀ fuel : nat, wp_next (CID0 := CIDe) b (proc_addr j) (fun (CIDl : CpuId) =>
       ∀ (Ml : regfile) (inum : mword 32) (CIDc : CpuId),
         ⌜(Z.to_nat (fsc_ninodes - bv_unsigned inum) <= fuel)%nat⌝ -∗
         ⌜0 < bv_unsigned inum < fsc_ninodes⌝ -∗
         ⌜ia_sp m Ml⌝ -∗
         ⌜ia_thr8 m Ml⌝ -∗
    ...
    - (* ---- FUEL S: one turn of the loop ---- *)
      ...
      + (* ---- TAKEN: THE CLAIM at +0x88 ---- *)
        ...
        iDestruct (cpu_own_transport CID6 CID13 0 true (proc_addr j) b
                     ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
        iApply (ia_claim (CID0 := CID13) γs j γl pd pav pu
 ty inum ds u Sb
                  t qt kk bno bsd0 d0 pidv dq dqs dqn m GA K b lks
                  Upr HK Hgeom HGAsp HGAthr HGAs1 HGAs3 HGAs5 HGAs6 HGAs2 Hkk
                  ...
                  with "Hcg Hcnt Htext Hkdata Hpc Hpanenv Hbio Hlctx Hireg Hiopen
                        ...
                        Hppid Hsbn Hsbi Hsl Hop Htx Hheld [Hcont]").
      ...
      + (* ---- FALL: brelse, inum++, and the loop test ---- *)
        ...
        iApply (BL.wp_brelse_sconf γs fsc_bio (fs_view fsc_fs fsc_disk icfg_dev fsc_cov) kk
                  ...
                  (diblk_bytes ds) bsd0 d0 b lks Upr HKbl Hkk HGBa0
        ...
        assert (HGDa4 : GD !!! Regidx Ra4 = (mword_of_int fsc_ninodes : mword 64)).
        ...
        assert (HGEa4 : GE !!! Regidx Ra4 = (mword_of_int fsc_ninodes : mword 64))
        ...
        destruct (Z.ltb (bv_unsigned inum + 1) fsc_ninodes) eqn:Hlt.
        * (* still in range: back to +0x30 with the fuel one lower *)
          ...
          { rgne. rgne. rewrite HGEa5 HGEa4.
            rewrite (ia_bltu_moi (bv_unsigned inum + 1) fsc_ninodes
...
Section IallocMain.
 ...
 ty u Sb
  ...
  Proof.
    ...
    subst eb.
    pose proof HK as HK'. 
    (* the bound as a NAMED hypothesis -- see [ia_arms]' note: a class field
       in an explicit-argument position leaves its instance an evar until the
       whole application is elaborated, so an inline [ltac:] side condition
       cannot see it. *)
    assert (Hn64 : 0 <= fsc_ninodes < 18446744073709551616)
      by (change (2^31)%Z with 2147483648%Z in Hn31; lia).
    ...
    { rewrite (ia_bgeu_moi 1 fsc_ninodes ltac:(lia) Hn64).
    ...
    iAssert (ia_cont (CID0 := CID) ty u Sb
               pidv dq dqs dqn j m K b lks Upr t qt)%I with "[Hcont]" as "Hcont";
    ...
    assert (Hnadr : add_vec (rget R3 Ra4)
                      (sign_extend' 64 (mword_of_int 1972 : mword 12))
                    = sb_ninodes).
    ...
    iApply (wp_lw_s_sconf (kt := KT1) (ktd := KT0) (mword_of_int (KernelSyms.ialloc + 0xc)) Ra4 Ra4
              (mword_of_int 1972 : mword 12) R3 (K - 8)%nat
              (mword_of_int fsc_ninodes : mword 32) b
              ltac:(nz) ltac:(rdok) with "Hcg Hpc [] Hsbn").
    ...
    assert (HR4a4 : R4 !!! Regidx Ra4 = (mword_of_int fsc_ninodes : mword 64))
    ...
    assert (HR5a4 : R5 !!! Regidx Ra4 = (mword_of_int fsc_ninodes : mword 64))
    ...
    iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.ialloc + 0x2c)) Rs4 Rs4
              (mword_of_int 1928 : mword 12) R9 (K - 8)%nat b
              ltac:(nz) ltac:(rdok) with "Hcg Hpc []").
    { iApply (iali_2c with "Htext"). }
    iIntros (CID19 Hq19) "Hcg Hpc".
    set (RA := <[Regidx Rs4 := regval_into_reg
                  (add_vec (rget R9 Rs4)
                     (sign_extend' 64 (mword_of_int 1928 : mword 12)))]> R9).
 ...
 ty u Sb
    ...
    - iDestruct "Harm" as "(%Hp & Hiref & Htxc & HopS)".
      iSplitR; [iPureIntro; exact Hp |].
      iSplitL "Hiref"; [iExact "Hiref" |].
      iSplitL "Htxc"; [iExact "Htxc" |].
      iApply (log_opS_op with "HopS Htx").
  Qed.
