...
Section IallocEpilogue.
  ...
  Local Lemma ia_epilogue `{GEN : GenId} `{CID0 : CpuId}
    ...
    ia_cont (kt := kt) (CID0 := CID0) γ bn inodestart ninodes nib dev ty u Sb
...
Section IallocOut.
  ...
  Proof.
    ...
    iAssert (ia_frame (kt := kt) m) with "[Hf1 Hf2 Hf3 Hf4 Hf5 Hf6 Hf7 Hf8]" as "Hframe".
    { rewrite /ia_frame.
...
Section IallocClaim.
  ...
  Proof.
    ...
    iApply (MS.wp_memset_sconf kt W3 (K - 8)%nat 64%nat
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.ialloc + 0x9a)) Rra
              (mword_of_int 3238 : mword 21) W4 (K - 8)%nat b
              ltac:(nz) ltac:(rdok) ltac:(vm_compute; reflexivity)
              with "Hcg Hpc Hi9a").
    iIntros (CID8 Hq8) "Hcg Hpc".
    ...
    assert (Htgtlw : add_vec (mword_of_int (KernelSyms.ialloc + 0x9a) : mword 64)
                       (sign_extend' 64 (mword_of_int 3238 : mword 21))
  ...
  Local Lemma ia_scan `{GEN : GenId} `{CIDe : CpuId}
    ...
    (∀ fuel : nat, wp_next (CID0 := CIDe) b (proc_addr j) (fun (CIDl : CpuId) =>
       ∀ (Ml : regfile) (inum : mword 32) (CIDc : CpuId),
         ...
         iref_slot -∗
         log_opS γ (S u) Sb -∗
         ia_cont (kt := kt) (CID0 := CIDc) γ bn inodestart ninodes nib dev ty u Sb
                 pidv dq dqs dqn j m K b lks -∗
         WP (Loop : expr riscv_lang))).
  Proof.
    ...
    - (* ---- FUEL S: one turn of the loop ---- *)
      ...
      iApply (wp_lw_s_sconf (kt := kt) (ktd := KT0) (mword_of_int (KernelSyms.ialloc + 0x34)) Ra5 Rs4
      ...
      + (* ---- FALL: brelse, inum++, and the loop test ---- *)
        ...
        iApply (wp_lw_s_sconf (kt := kt) (ktd := KT0) (mword_of_int (KernelSyms.ialloc + 0x5a)) Ra4 Rs4
...
Section IallocMain.
  ...
  Proof.
    ...
    iAssert (ia_cont (kt := kt) (CID0 := CID) γ bn inodestart ninodes nib dev ty u Sb
    ...
    assert (Hnadr : add_vec (rget R3 Ra4)
                      (sign_extend' 64 (mword_of_int 2110 : mword 12))
                    = sb_ninodes).
    { rgne. rewrite HR3a4. rewrite /sb_ninodes /pa_add /add_vec_int. pcw. }
    iEval (rewrite -Hnadr) in "Hsbn".
    iApply (wp_lw_s_sconf (kt := kt) (ktd := KT0) (mword_of_int (KernelSyms.ialloc + 0xc)) Ra4 Ra4
    ...
    iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.ialloc + 0x2c)) Rs4 Rs4
              (mword_of_int 2066 : mword 12) R9 (K - 8)%nat b
              ltac:(nz) ltac:(rdok) with "Hcg Hpc Hi2c").
    iIntros (CID19 Hq19) "Hcg Hpc".
    set (RA := <[Regidx Rs4 := regval_into_reg
                  (add_vec (rget R9 Rs4)
                     (sign_extend' 64 (mword_of_int 2066 : mword 12)))]> R9).
