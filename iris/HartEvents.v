...
Section events.
  ...
  (* ------------------------------------------------------------------ *)
  Lemma wp_hart_ram_read (n : N) (req : Interface.ReadReq.t n) (m : M unit) :
    ...
    (∀ σ, mstate_interp σ ={⊤,∅}=∗
       ∃ w : bv (8 * n),
         ...
         ▷ (|={∅,⊤}=> mstate_interp σ ∗
              WP (HartE gen_id cpu_id (hread_resume (bv_unsigned w) m)
                  : expr riscv_lang))) -∗
  ...
  Proof.
    ...
    iModIntro. iExists (K (inl (w, None))), σ.
    iSplitR.
    { iPureIntro. rewrite Hm /mnode_step. cbn beta iota.
    ...
    rewrite Hm /mnode_step in Hstep. cbn beta iota in Hstep.
  ...
  (* ------------------------------------------------------------------ *)
  Lemma wp_hart_ram_write (n : N) (req : Interface.WriteReq.t n) (m : M unit) :
    ...
    (∀ σ, mstate_interp σ ={⊤,∅}=∗
       ▷ (|={∅,⊤}=> mstate_interp
            ...
            WP (HartE gen_id cpu_id (hwrite_resume m) : expr riscv_lang))) -∗
    WP (HartE gen_id cpu_id m : expr riscv_lang).
  Proof.
  ...
  (* ------------------------------------------------------------------ *)
  Lemma wp_hart_dev_read (n : N) (req : Interface.ReadReq.t n) (m : M unit) :
    hread_req_at n m = Some req ->
  ...
  Proof.
    ...
    iModIntro. iExists (K (inl (w, None))), (MState σ.(sregs) σ.(mem) d').
    iSplitR.
    { iPureIntro. rewrite Hm /mnode_step. cbn beta iota.
      rewrite Hdev. cbn beta iota.
      exists w, d'. done. }
  ...
  (* MMIO WRITE.                                                          *)
  (* ------------------------------------------------------------------ *)
  Lemma wp_hart_dev_write (n : N) (req : Interface.WriteReq.t n) (m : M unit) :
    hwrite_req_at n m = Some req ->
  ...
  Proof.
    ...
    iModIntro. iExists (K (inl None)), (MState σ.(sregs) σ.(mem) d').
    iSplitR.
    { iPureIntro. rewrite Hm /mnode_step. cbn beta iota.
      rewrite Hdev. cbn beta iota.
      exists d'. done. }
