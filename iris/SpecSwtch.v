(* SpecSwtch.v -- the public interface of Swtch, stated independently of its
   ...
   chain payload [P cpu_id sie_gname Ao newc oldc tp p]; the machine ends up
   running new's saved WP.  The target's admissibility index [An] must admit
   THIS hart (the pure [adm An cpu_id sie_gname] premise), and the caller's
   continuation is what the OLD record is built from, so it carries the index
   [Ao] that record is deposited at -- and is quantified over the hart [h] and
   SIE ghost [g] that record may later be resumed on.  The interface is
   ...
   (SwtchCtx.v, unconverted by this sweep -- its [gname] slot is the
   admissibility ghost, a DIFFERENT thing from the now-canonical SIE ghost),
   not an ordinary trapped step. *)
...
Definition wp_swtch_sconf_body `{!riscvGS Σ, !sieG Σ} `{CID : CpuId}
  ...
  (* THE ADMISSIBILITY OBLIGATION: the target record must admit resumption
     ...
     pinned to a cpu context ([An = Some (cpu_id, sie_gname)]) by [adm_pin].
     [Ao] is the index the caller's OWN record is built at from the
     continuation below, so the continuation's [⌜adm Ao h g⌝] is exactly what
     that record promises. *)
  adm An cpu_id sie_gname ->
  kernel_text -∗
  ...
  P cpu_id sie_gname Ao newc oldc (rget m0 (mword_of_int 4 : mword 5)) p -∗
  ( ∀ (h : CPU) (g : gname) (m : regfile) (eb' : bool),
      ...
      (∃ (A' : ctx_adm) (cret : mword 64),
         ...
         P h g A' oldc cret (rget (CID := h) m (mword_of_int 4 : mword 5)) p) -∗
