...
Section ForwardJALg.
  ...
  Proof using All.
    ...
    assert (LmidA : register_lookup (R_bitvector_64 mideleg) sAjg.(sregs) = zeros' 64).
    { unfold sAjg, set_reg; cbn [sregs]. rewrite irrelevant_register_set;
    ...
    { apply exec_dispatchInterrupt_none.
      apply (exec_getPendingSet_machine_none sAjg _ (exec_currentlyEnabled_S sAjg) LmidA LmIEA). }
...
Section WpJalGpr.
  ...
  Lemma wp_jal_gpr (pc : mword 64) (w : mword 32) (imm : mword 21) (rd : mword 5)
    ...
    WP (Loop : expr riscv_lang) @ E {{ Phi }}.
  Proof.
    iIntros (Hrd Hmd Hpmaall Hpmpf Halignf Hbit0f Hbit1f Hvalignf HnotRVCf Hdec Hal0 Hal1 Hb1 HmIE Help)
