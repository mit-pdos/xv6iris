...
Section WpJalGpr.
  ...
  Lemma wp_jal_gpr (pc : mword 64) (w : mword 32) (imm : mword 21) (rd : mword 5)
    ...
    WP (Loop : expr riscv_lang) @ E {{ Phi }}.
  Proof.
    iIntros (HN Hrd Hmd HS Hpmaall Hpmpf Halignf Hbit0f Hbit1f Hvalignf HnotRVCf Hdec Hal0 Hal1 Hb1 HmIE Help)
    ...
    iModIntro.
    iExists (sFcjg s pc b1 imm rd (register_lookup minstret s.(sregs))). iSplitR.
    { iPureIntro.
      rewrite <- (sFj_eq_gpr s pc b1 imm rd (register_lookup minstret s.(sregs)) Lpc eq_refl).
...
Section WpJalGprDemo.
  ...
  Definition wp_jal_x1 (pc : mword 64) (w : mword 32) (imm : mword 21) :=
    wp_jal_gpr (dqc:=DfracOwn 1) pc w imm (mword_of_int 1).
  Definition wp_jal_x5 (pc : mword 64) (w : mword 32) (imm : mword 21) :=
    wp_jal_gpr (dqc:=DfracOwn 1) pc w imm (mword_of_int 5).
