...
Section SsProps.
  ...
  Definition ss_loop `{GEN : GenId} (CID0 : CPU)
    ...
    (wp_next (CID0 := CID0) true (proc_addr j) (fun (CID : CpuId) =>
      ...
      ss_exit CID0 j γ bn γfs cov logstart m K eb C spd sp0 -∗
      WP (Loop : expr riscv_lang)))%I.
...
Section SsBodies.
  ...
  Lemma ss_tail_body `{GEN : GenId} `{CID : CpuId} (CID0 : CPU) (j : nat)
    ...
    WP (Loop : expr riscv_lang).
  Proof.
    intros pj HK Hanch Hspd Hsp0 Hss.
  ...
  Lemma ss_loop_body `{GEN : GenId} `{CID : CpuId} (CID0 : CPU)
    ...
    panic_wp_any -∗
    procs_inv γs -∗
    ▷ ss_loop CID0 j γ bn γfs cov logstart m K eb C spd sp0 -∗
    ss_exit CID0 j γ bn γfs cov logstart m K eb C spd sp0 -∗
    pa_stk sp0 1 ↦₈ (m !!! Regidx Rra) -∗
    pa_stk sp0 2 ↦₈ (m !!! Regidx Rs0) -∗
    pa_stk sp0 3 ↦₈ (m !!! Regidx Rs1) -∗
    ...
    WP (Loop : expr riscv_lang).
  Proof.
    intros pj HK Hj Hjl Hanch Hspd Hss.
    ...
    iApply (SleepPrepare.wp_sleep_prepare_sconf γs j γl A1
              ...
              Hj Hjl HA1nz ss_noff2 ltac:(pose proof (ss_K22 K HK); lia)
    ...
    iApply (Sleep.wp_sleep_sconf γs j γl A4 (K - 4)%nat eb C Hj Hjl
              ltac:(pose proof (ss_K22 K HK); lia)
    ...
    iApply (Acquire.wp_acquire_sconf (ln_lk γ) "log"%string
              ...
              ss_noff1 ltac:(pose proof (ss_K10 K HK); lia)
...
Section ProofSysSync.
  ...
  Proof.
    ...
    iAssert (ss_exit CID j γ bn γfs cov logstart m K eb C spd sp0) with "[Hcont]" as "Hexit".
    { rewrite /ss_exit.
      iIntros (CIDx Hsx Mx) "%HssE Hr24 Hr16 Hr8 Hr0 Htok Hres Hown Htc Hclm Hcg Hpc".
      iApply (ss_tail_body (CID := CIDx) CID j γ bn γfs cov logstart dev m Mx K eb C spd sp0
                HK Hsx Hspd Hsp0 HssE
                with "Htext Hlog Hr24 Hr16 Hr8 Hr0 Htok Hres Hown Htc Hclm Hcg Hpc Hcont"). }
    iAssert (ss_loop CID j γ bn γfs cov logstart m K eb C spd sp0) with "[]" as "Hloop".
    { iLöb as "IH". rewrite /ss_loop.
      iIntros (CIDy Hsy My) "%HssL Hr24 Hr16 Hr8 Hr0 Htok Hres Hown Htc Hclm Hcg Hpc Hexit".
      iApply (ss_loop_body (CID := CIDy) CID γs j γl γ bn γfs cov logstart dev m My K eb C spd sp0
                HK Hj Hjl Hsy Hspd HssL
    ...
    - (* ============== COMMITTING: straight to the spill block ============ *)
      ...
      iApply (ss_entry_body (CID := CIDa) CID j γ bn γfs cov logstart m G2 K eb C spd sp0
    ...
    - (* ============== NOT COMMITTING: test [outstanding] ================= *)
      ...
      + (* ---- SOMETHING IS PENDING: fall into the spill block ---- *)
        ...
        iApply (ss_entry_body (CID := CIDa) CID j γ bn γfs cov logstart m G4 K eb C spd sp0
