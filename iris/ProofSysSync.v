...
Lemma ss_reloc_a0_08 :
  add_vec (add_vec (mword_of_int (SS + 0x08) : mword 64)
          ...
          (sign_extend' 64 (mword_of_int 1254 : mword 12)) = log_addr.
...
Lemma ss_reloc_cmt_14 :
  add_vec (add_vec (mword_of_int (SS + 0x14) : mword 64)
                   (auipc_off (mword_of_int 30 : mword 20)))
          (sign_extend' 64 (mword_of_int 1274 : mword 12)) = l_cmt.
Proof.
...
Lemma ss_reloc_out_1e :
  add_vec (add_vec (mword_of_int (SS + 0x1e) : mword 64)
                   (auipc_off (mword_of_int 30 : mword 20)))
          (sign_extend' 64 (mword_of_int 1260 : mword 12)) = l_out.
Proof.
...
Lemma ss_reloc_nc_2e :
  add_vec (add_vec (mword_of_int (SS + 0x2e) : mword 64)
                   (auipc_off (mword_of_int 30 : mword 20)))
          (sign_extend' 64 (mword_of_int 1256 : mword 12)) = l_ncommit.
Proof.
...
Lemma ss_reloc_s1_36 :
  add_vec (add_vec (mword_of_int (SS + 0x36) : mword 64)
                   (auipc_off (mword_of_int 30 : mword 20)))
          (sign_extend' 64 (mword_of_int 1208 : mword 12)) = log_addr.
Proof. rewrite /log_addr. apply bv_eq; vm_compute; reflexivity. Qed.

Lemma ss_reloc_a0_5e :
  add_vec (add_vec (mword_of_int (SS + 0x5e) : mword 64)
                   (auipc_off (mword_of_int 30 : mword 20)))
          (sign_extend' 64 (mword_of_int 1168 : mword 12)) = log_addr.
Proof. rewrite /log_addr. apply bv_eq; vm_compute; reflexivity. Qed.
...
Section SsProps.
  ...
  Definition ss_exit `{GEN : GenId} (CID0 : CPU)
      ...
      (cov : gset Z) (logstart : Z)
      (m : regfile) (K : nat) (eb : bool) (C : iProp Σ) (lks : gset nat)
  ...
  Definition ss_loop `{GEN : GenId} (CID0 : CPU)
    ...
    (wp_next (CID0 := CID0) true (proc_addr j) (fun (CID : CpuId) =>
      ...
      ss_exit CID0 j γ bn γfs cov logstart m K eb C lks spd sp0 -∗
      WP (Loop : expr riscv_lang)))%I.
...
Section SsBodies.
  ...
  Lemma ss_tail_body `{GEN : GenId} `{CID : CpuId} (CID0 : CPU) (j : nat)
    ...
    (* the round trip's cancellation ([release]'s postcondition subtracts
       "log" back off): [locks_add_del_below] needs [lock_rank "log" ∉ lks],
  ...
  Proof.
    ...
    iApply (wp_addi4_s_sconf (mword_of_int (SS + 0x62)) Ra0 Ra0
              (mword_of_int 1168 : mword 12) X1 (trap_res eb + (K - 4))%nat false
              ltac:(vm_compute; discriminate) ltac:(rdok)
              with "Hcg Hpc Hi62").
    iApply wp_next_off_intro. iIntros "Hcg Hpc".
    iEval (rgne) in "Hcg".
    set (X2 := <[Regidx Ra0 := regval_into_reg
        (add_vec (X1 !!! Regidx Ra0) (sign_extend' 64 (mword_of_int 1168 : mword 12)))]> X1).
    change (<[Regidx Ra0 := regval_into_reg
        (add_vec (X1 !!! Regidx Ra0) (sign_extend' 64 (mword_of_int 1168 : mword 12)))]> X1) with X2.
    ...
    iEval (rewrite (locks_add_del_below (lock_rank "log") lks Hbelow)) in "Hown".
    ...
    iDestruct (cpu_own_transport CIDr CIDe4 0 eb pj C eb ltac:(wp_next_chain)
                 with "Hown") as "Hown".
  ...
  Lemma ss_loop_body `{GEN : GenId} `{CID : CpuId} (CID0 : CPU)
    ...
    panic_wp_any -∗
    procs_inv γs -∗
    ▷ ss_loop CID0 j γ bn γfs cov logstart m K eb C lks spd sp0 -∗
    ss_exit CID0 j γ bn γfs cov logstart m K eb C lks spd sp0 -∗
    pa_stk sp0 1 ↦₈ (m !!! Regidx Rra) -∗
    pa_stk sp0 2 ↦₈ (m !!! Regidx Rs0) -∗
    pa_stk sp0 3 ↦₈ (m !!! Regidx Rs1) -∗
  ...
  Proof.
    ...
    iApply (SleepPrepare.wp_sleep_prepare_sconf γs j γl A1
              (trap_res eb + (K - 4))%nat 1%nat eb C false ({[lock_rank "log"]} ∪ lks)
    ...
    (* ========================== sleep() ========================== *)
    iDestruct (cpu_own_transport CIDr CIDj 0 eb pj C eb ltac:(wp_next_chain) with "Hown") as "Hown".
    ...
    (* -------------------- acquire(&log.lock) -------------------- *)
    iDestruct (cpu_own_transport CIDs CIDn 0 eb pj C eb ltac:(wp_next_chain) with "Hown") as "Hown".
  ...
  Proof.
    ...
    assert (Hnca : add_vec (rget Y1 Rs2) (sign_extend' 64 (mword_of_int 1256 : mword 12)) = l_ncommit).
    { rgne. rewrite HY1s2. exact ss_reloc_nc_2e. }
    iEval (rewrite -Hnca) in "Hnc".
    iApply (wp_lw_s_sconf (mword_of_int (SS + 0x32)) Rs2 Rs2
              (mword_of_int 1256 : mword 12) Y1 (trap_res eb + (K - 4))%nat nc false
              (dqm := DfracOwn 1)
              ltac:(vm_compute; discriminate) ltac:(rdok)
    ...
    iApply (wp_addi4_s_sconf (mword_of_int (SS + 0x3a)) Rs1 Rs1
              (mword_of_int 1208 : mword 12) Y3 (trap_res eb + (K - 4))%nat false
              ltac:(vm_compute; discriminate) ltac:(rdok)
              with "Hcg Hpc Hi3a").
    iApply wp_next_off_intro. iIntros "Hcg Hpc".
    iEval (rgne) in "Hcg".
    set (Y4 := <[Regidx Rs1 := regval_into_reg
        (add_vec (Y3 !!! Regidx Rs1) (sign_extend' 64 (mword_of_int 1208 : mword 12)))]> Y3).
    change (<[Regidx Rs1 := regval_into_reg
        (add_vec (Y3 !!! Regidx Rs1) (sign_extend' 64 (mword_of_int 1208 : mword 12)))]> Y3) with Y4.
...
Section ProofSysSync.
  ...
  Proof.
    ...
    iApply (wp_addi4_s_sconf (mword_of_int (SS + 0x0c)) Ra0 Ra0
              (mword_of_int 1254 : mword 12) R3 (K - 4)%nat eb
              ltac:(vm_compute; discriminate) ltac:(rdok)
              with "Hcg Hpc Hi0c").
    iIntros (CID6 Hs6) "Hcg Hpc".
    iEval (rgne) in "Hcg".
    set (R4 := <[Regidx Ra0 := regval_into_reg
        (add_vec (R3 !!! Regidx Ra0) (sign_extend' 64 (mword_of_int 1254 : mword 12)))]> R3).
    change (<[Regidx Ra0 := regval_into_reg
        (add_vec (R3 !!! Regidx Ra0) (sign_extend' 64 (mword_of_int 1254 : mword 12)))]> R3) with R4.
    ...
    (* ===================== acquire(&log.lock) ===================== *)
    iDestruct (cpu_own_transport CID CID7 0 eb pj C eb ltac:(wp_next_chain)
                 with "Hown") as "Hown".
    ...
    iAssert (ss_exit CID j γ bn γfs cov logstart m K eb C lks spd sp0) with "[Hcont]" as "Hexit".
    { rewrite /ss_exit.
      iIntros (CIDx Hsx Mx) "%HssE Hr24 Hr16 Hr8 Hr0 Htok Hres Hown Htc Hclm Hcg Hpc".
      iApply (ss_tail_body (CID := CIDx) CID j γ bn γfs cov logstart dev m Mx K eb C lks spd sp0
                HK Hsx Hspd Hsp0 HssE Hbelow
                with "Htext Hlog Hr24 Hr16 Hr8 Hr0 Htok Hres Hown Htc Hclm Hcg Hpc Hcont"). }
    iAssert (ss_loop CID j γ bn γfs cov logstart m K eb C lks spd sp0) with "[]" as "Hloop".
    { iLöb as "IH". rewrite /ss_loop.
      iIntros (CIDy Hsy My) "%HssL Hr24 Hr16 Hr8 Hr0 Htok Hres Hown Htc Hclm Hcg Hpc Hexit".
      iApply (ss_loop_body (CID := CIDy) CID γs j γl γ bn γfs cov logstart dev m My K eb C lks spd sp0
    ...
    assert (Hcmta : add_vec (rget G1 Ra5) (sign_extend' 64 (mword_of_int 1274 : mword 12)) = l_cmt).
    { rgne. rewrite HG1a5. exact ss_reloc_cmt_14. }
    iEval (rewrite -Hcmta) in "Hcmt".
    iApply (wp_lw_s_sconf (mword_of_int (SS + 0x18)) Ra5 Ra5
              (mword_of_int 1274 : mword 12) G1 (trap_res eb + (K - 4))%nat
    ...
    - (* ============== COMMITTING: straight to the spill block ============ *)
      ...
      iApply (ss_entry_body (CID := CIDa) CID j γ bn γfs cov logstart m G2 K eb C lks spd sp0
    ...
    - (* ============== NOT COMMITTING: test [outstanding] ================= *)
      ...
      assert (Houta : add_vec (rget G3 Ra5) (sign_extend' 64 (mword_of_int 1260 : mword 12)) = l_out).
      { rgne. rewrite HG3a5. exact ss_reloc_out_1e. }
      iEval (rewrite -Houta) in "Hout".
      iApply (wp_lw_s_sconf (mword_of_int (SS + 0x22)) Ra5 Ra5
                (mword_of_int 1260 : mword 12) G3 (trap_res eb + (K - 4))%nat
      ...
      + (* ---- SOMETHING IS PENDING: fall into the spill block ---- *)
        ...
        iApply (ss_entry_body (CID := CIDa) CID j γ bn γfs cov logstart m G4 K eb C lks spd sp0
