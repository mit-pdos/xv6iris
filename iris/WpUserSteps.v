...
Require Import WpUserSplitTrap.
...
Section WpUserSteps.
  ...
  Local Notation ustep_c_j := (WpUserCtrlU.ustep_c_j U).
  Local Notation ustep_c_jr := (WpUserCtrlU.ustep_c_jr U).
  ...
  Local Notation ustep_u_split_nop := (WpUserSplitCompute.ustep_u_split_nop U).
  ...
  Local Notation ustep_u_split_highfault_u := (WpUserSplitFault.ustep_u_split_highfault_u U).
  ...
  Definition ustep_case (va ms_v : mword 64) (g : gmap regidx (mword 64))
    ...
    (exists vpn i vpnD ieD (h : mword 16) (ii : instruction) (imm : mword 12) (rs1 rd : mword 5) (v : mword 32),
       ...
       (forall j : nat, (j < 4)%nat -> code !! pa_add paD j = Some (nth_byte v j)))
    \/
    (* 41: fetch hit, retiring width-2 LH/LHU from a code page *)
    (exists vpn i vpnD ieD (w : mword 32) (imm : mword 12) (rs1 rd : mword 5) (is_unsigned : bool) (v : mword 16),
       ...
       (forall j : nat, (j < 2)%nat -> code !! pa_add paD j = Some (nth_byte v j)))
    \/
    (* 42: RVC fetch hit, compressed width-2 C_LH/C_LHU from a code page *)
    (exists vpn i vpnD ieD (h : mword 16) (ii : instruction) (imm : mword 12) (rs1 rd : mword 5) (is_unsigned : bool) (v : mword 16),
       ...
       (forall j : nat, (j < 2)%nat -> code !! pa_add paD j = Some (nth_byte v j)))
    \/
    (* 43: fetch hit, retiring width-1 LB/LBU from a code page *)
    ...
    \/
    (* 44: RVC fetch hit, compressed width-1 C_LBU from a code page *)
    ...
    (exists vpn ie (w : mword 32) (imm : mword 12) (rs1 : mword 5),
       ...
       bit_to_bool (access_vec_dec (jalr_target (g !!! Regidx rs1) imm) 1) = false)
    \/
    (* 49: RVC fetch hit, retiring C_J (no link, aligned target) *)
    (exists vpn ie (h : mword 16) (imm : mword 11),
    ...
    \/
    (* 52: 2-aligned (pc == 2 mod 4) 32-bit split-fetch, retiring straight-line
       ITYPE compute (two per-half TLB hits; rides wp_instr_u_split) *)
    (exists (vpnL vpnH : mword 27) (ieL ieH : uwalk_info) (w : mword 32) (op : iop)
            (f : mword 64 -> mword 12 -> mword 64)
            (imm : mword 12) (rs1 rd : mword 5),
    ...
    (exists (vpnL vpnH : mword 27) (ieL ieH : uwalk_info) (ilo : mword 16),
       ...
       autocast (T := mword) (subrange_vec_dec
         (subrange_vec_dec (bits_of_virtaddr (Virtaddr (add_vec_int va 2))) (Z.sub 39 1) 0) (Z.sub 39 1) pagesize_bits) = vpnH)
    \/
    (* 54: 2-aligned (pc == 2 mod 4) 32-bit split-fetch, retiring straight-line
       ITYPE compute, FETCH-UNCONDITIONAL.  Non-straddling (a single vpn/ie),
       spec-based: rides [ustep_u_split_compute_u] (via
       [wp_instr_u_split_combined]) so it classifies from an EMPTY TLB. *)
    (exists (vpn : mword 27) (ie : uwalk_info) (w : mword 32) (op : iop)
            (f : mword 64 -> mword 12 -> mword 64)
            (imm : mword 12) (rs1 rd : mword 5),
       spec !! vpn = Some ie /\
       uw_check_ok (InstructionFetch tt) ie /\
                  ...
                  if Z.eqb (uint rd') 0 then s
                         ...
                         (regval_into_reg
                            (f (if Z.eqb (uint rs1') 0 then zero_reg
                               ...
                               imm')))))
    \/
    (* 55: 2-aligned split-fetch, retiring GENERIC single-source compute,
       FETCH-UNCONDITIONAL.  Non-straddling (a single vpn/ie), spec-based:
       rides [ustep_u_split_compute1_u] (via [wp_instr_u_split_combined]).
       Covers every single-source 32-bit compute family (abstract [mk]/[F]). *)
    (exists (vpn : mword 27) (ie : uwalk_info) (w : mword 32)
       ...
       is_lpad_instruction (mk rs1 rd) = false)
    \/
    (* 56: 2-aligned split-fetch, retiring GENERIC two-source compute,
       FETCH-UNCONDITIONAL.  Non-straddling (a single vpn/ie), spec-based:
       rides [ustep_u_split_rtype2_u] (via [wp_instr_u_split_combined]).
       Covers every two-source 32-bit compute family (abstract [mk2]/[f]). *)
    (exists (vpn : mword 27) (ie : uwalk_info) (w : mword 32)
    ...
    (exists (vpn : mword 27) (ie : uwalk_info) (w : mword 32) (op : bop)
       ...
       bit_to_bool (access_vec_dec (add_vec va (sign_extend' 64 imm)) 1) = false)
    \/
    (* 61: 2-aligned (pc == 2 mod 4) 32-bit split-fetch illegal-in-U,
       FETCH-UNCONDITIONAL.  Rides [ustep_u_split_illegal_u]; execute is
       Illegal_Instruction (delivered via handle_exception). *)
    (exists (vpn : mword 27) (ie : uwalk_info) (w : mword 32) (ii : instruction),
       (forall s, register_lookup cur_privilege s.(sregs) = User ->
          exec (execute ii) s = Some (Illegal_Instruction tt, s)) /\
       is_lpad_instruction ii = false /\
       spec !! vpn = Some ie /\
       ...
       (forall s0, agree_on D_u s0 dstateU ->
          exec (ext_decode w) s0 = Some (ii, s0)))
    \/
    (* 62: 2-aligned (pc == 2 mod 4) 32-bit split-fetch EBREAK-in-U,
       FETCH-UNCONDITIONAL.  Rides [ustep_u_split_ebreak_u]; execute is a
       software-breakpoint Trap (cause E_Breakpoint, stval = va). *)
    (exists (vpn : mword 27) (ie : uwalk_info) (w : mword 32) (ii : instruction),
       (forall s, register_lookup cur_privilege s.(sregs) = User ->
          register_lookup PC s.(sregs) = va ->
          exec (execute ii) s
    ...
    (exists (vpn : mword 27) (ie : uwalk_info),
       ...
       is_aligned_paddr (Physaddr (u_pa (upt_entry vpn ie) (add_vec_int va 2) vpn)) 2 = true)
    \/
    (* 64: 2-aligned split-fetch HIGH-half page fault, FETCH-UNCONDITIONAL
       (spec-based low half).  Rides [ustep_u_split_highfault_u]. *)
    (exists (vpnL vpnH : mword 27) (ieL ieH : uwalk_info) (ilo : mword 16),
       spec !! vpnL = Some ieL /\
       uw_check_ok (InstructionFetch tt) ieL /\
       update_PTE_Bits (uw_pte0 ieL) (InstructionFetch tt) = None /\
       ...
       autocast (T := mword) (subrange_vec_dec
         (subrange_vec_dec (bits_of_virtaddr (Virtaddr (add_vec_int va 2))) (Z.sub 39 1) 0) (Z.sub 39 1) pagesize_bits) = vpnH)
    \/
    (* 65: 2-aligned (pc == 2 mod 4) 32-bit split-fetch, GENERIC rd=x0 NO-OP
       (HINT / nop such as [addi x0,x1,5]), FETCH-UNCONDITIONAL.  Non-
       straddling (a single vpn/ie), spec-based: state-preserving retire
       with NO register write.  Rides [ustep_u_split_nop] (via
       [wp_instr_u_split_combined]).  This is the 2-aligned analogue of the
       4-aligned generic no-state-change disjunct; it makes the 2-aligned
       compute classification TOTAL over rd (rd=x0 has no route in the
       rd<>0 compute disjuncts 54/55/56). *)
    (exists (vpn : mword 27) (ie : uwalk_info) (w : mword 32) (ii : instruction),
       spec !! vpn = Some ie /\
       uw_check_ok (InstructionFetch tt) ie /\
       update_PTE_Bits (uw_pte0 ie) (InstructionFetch tt) = None /\
  ...
  Proof.
                                                                                                      ...
                                                                                                      | [ Hodd
                                                                                                        | [ (vpnL & vpnH & ieL & ieH & w & op & f & imm & rs1 & rd &
                                                                                                          ...
                                                                                                          | [ (vpnL & vpnH & ieL & ieH & ilo &
                                                                                                             ...
                                                                                                             HcanonL & Hvpn_defL & HalignL &
                                                                                                             HcanonH & Hvpn_defH)
                                                                                                            | [ (vpn & ie & w & op & f & imm & rs1 & rd &
                                                                                                               Hsome & Hchk0 & HupdN & HcwL & HcwH &
                                                                                                               Hbit0 & Hbit1 & Hvalign4 &
                                                                                                               HcanonL & Hvpn_defL & HalignL &
                                                                                                               HcanonH & Hvpn_defH & HalignH &
                                                                                                               HnotRVC & Hdec & Hrd & Hexec_op)
                                                                                                              | [ (vpn & ie & w & mk & F & rs1 & rd &
                                                                                                                 Hsome & Hchk0 & HupdN & HcwL & HcwH &
                                                                                                                 Hbit0 & Hbit1 & Hvalign4 &
                                                                                                                 HcanonL & Hvpn_defL & HalignL &
                                                                                                                 HcanonH & Hvpn_defH & HalignH &
                                                                                                                          ...
                                                                                                                          | [ (vpn & ie & w & ii & Hexec_op & Hlpad & Hsome & Hchk0 & HupdN & Hpbmt0 &
                                                                                                                            ...
                                                                                                                            | [ (vpn & ie & w & ii & Hexec_op & Hlpad & Hsome & Hchk0 & HupdN & Hpbmt0 &
                                                                                                                              ...
                                                                                                                              | [ (vpn & ie & Hsome & Hchk0 & HupdN & Hpbmt0 &
                                                                                                                                 ...
                                                                                                                                 HcanonH & Hvpn_defH & HalignH)
                                                                                                                                | [ (vpnL & vpnH & ieL & ieH & ilo &
                                                                                                                                   HsomeL & Hchk0L & HupdNL & Hpbmt0L & HcwL & HnotRVC &
                                                                                                                                   HsomeH & HdenH &
                                                                                                                                   Hbit0 & Hbit1 & Hvalign4 &
                                                                                                                                   HcanonL & Hvpn_defL & HalignL &
                                                                                                                                   HcanonH & Hvpn_defH)
                                                                                                                                  | (vpn & ie & w & ii &
                                                                                                                                     ...
                                                                                                                                     Hbit0 & Hbit1 & Hvalign4 &
                                                                                                                                     HcanonL & Hvpn_defL & HalignL &
                                                                                                                                     HcanonH & Hvpn_defH & HalignH & HnotRVC &
                                                                                                                                     Hdec & Hexec_op & Hnlpad) ] ] ] ] ] ] ] ] ] ] ] ] ] ] ] ] ] ] ] ] ] ] ] ] ] ] ]
                                                                              ] ] ] ] ] ] ] ] ] ] ] ] ] ] ] ] ] ] ] ] ] ]
                                  ] ] ] ] ] ] ] ] ] ] ] ] ] ] ].
    - (* non-canonical *)
    ...
    - (* ODD pc -> fetch-address-misaligned fault (cause 0) *)
      ...
      iApply (ustep_fetch_misalign va ms_v sc_v stval_v sepc_v g tlbvec E Φ
                ...
                with "Hhw Hinv Hhs Hpriv Hms Hsc Hstv Hsepc Htlbc Hpc Hgpr Hupt
                      Hcode Hdata Hcfg HkT").
    - (* 2-aligned 32-bit split-fetch retiring ITYPE op *)
      iDestruct "Hk" as "[HkP _]".
      iApply (ustep_u_split_compute op f va vpnL vpnH ieL ieH w imm rs1 rd
    ...
    - (* 53: 2-aligned 32-bit split-fetch, HIGH-half page fault *)
      ...
      iApply (ustep_split_highfault va vpnL vpnH ieL ieH ilo
                ...
                with "Hhw Hinv Hhs Hpriv Hms Hsc Hstv Hsepc Htlbc Hpc Hgpr Hupt
                      Hcode Hdata Hcfg HkT").
    - (* 54: 2-aligned 32-bit split-fetch retiring ITYPE, FETCH-UNCONDITIONAL *)
      iDestruct "Hk" as "[HkP _]".
      iApply (ustep_u_split_compute_u op f va vpn ie w imm rs1 rd
                ms_v sc_v stval_v sepc_v g tlbvec E Φ HN Hexec_op Hok
                Hsome Hchk0 HupdN HcwL HcwH
                HSXL Hbit0 Hbit1 Hvalign4
                HcanonL Hvpn_defL HalignL
                ...
                with "Hhw Hinv Hhs Hpriv Hms Hsc Hstv Hsepc Htlbc Hpc Hgpr Hupt
                      Hcode Hdata Hcfg HkP").
    - (* 55: 2-aligned split-fetch retiring GENERIC single-source, FETCH-UNCONDITIONAL *)
      iDestruct "Hk" as "[HkP _]".
      iApply (ustep_u_split_compute1_u mk F va vpn ie w rs1 rd
                ms_v sc_v stval_v sepc_v g tlbvec E Φ HN Hexec_op Hnlpad Hok
                Hsome Hchk0 HupdN HcwL HcwH
                HSXL Hbit0 Hbit1 Hvalign4
                HcanonL Hvpn_defL HalignL
                HcanonH Hvpn_defH HalignH
                ...
                with "Hhw Hinv Hhs Hpriv Hms Hsc Hstv Hsepc Htlbc Hpc Hgpr Hupt
                      Hcode Hdata Hcfg HkP").
    - (* 56: 2-aligned split-fetch retiring GENERIC two-source, FETCH-UNCONDITIONAL *)
      iDestruct "Hk" as "[HkP _]".
      iApply (ustep_u_split_rtype2_u mk2 f va vpn ie w rs2 rs1 rd
                ms_v sc_v stval_v sepc_v g tlbvec E Φ HN Hexec_op Hnlpad Hok
                Hsome Hchk0 HupdN HcwL HcwH
                HSXL Hbit0 Hbit1 Hvalign4
                HcanonL Hvpn_defL HalignL
    ...
    - (* 60: 2-aligned 32-bit split-fetch BTYPE taken (4-aligned target), FETCH-UNCONDITIONAL *)
      ...
      iApply (ustep_u_split_branch_taken_u op c va vpn ie w imm rs2 rs1
                ...
                with "Hhw Hinv Hhs Hpriv Hms Hsc Hstv Hsepc Htlbc Hpc Hgpr Hupt
                      Hcode Hdata Hcfg HkP").
    - (* 61: 2-aligned 32-bit split-fetch illegal-in-U, FETCH-UNCONDITIONAL *)
      iDestruct "Hk" as "[_ HkT]".
      iApply (ustep_u_split_illegal_u ii va vpn ie w
                ms_v sc_v stval_v sepc_v g tlbvec E Φ HN Hexec_op Hlpad Hok
                ...
                with "Hhw Hinv Hhs Hpriv Hms Hsc Hstv Hsepc Htlbc Hpc Hgpr Hupt
                      Hcode Hdata Hcfg HkT").
    - (* 62: 2-aligned 32-bit split-fetch EBREAK-in-U, FETCH-UNCONDITIONAL *)
      iDestruct "Hk" as "[_ HkT]".
      iApply (ustep_u_split_ebreak_u ii va vpn ie w
                ms_v sc_v stval_v sepc_v g tlbvec E Φ HN Hexec_op Hlpad Hok
                Hsome Hchk0 HupdN Hpbmt0 HcwL HcwH
                HSXL Hbit0 Hbit1 Hvalign4
                HcanonL Hvpn_defL HalignL
    ...
    - (* 64: 2-aligned split-fetch HIGH-half page fault, FETCH-UNCONDITIONAL *)
      iDestruct "Hk" as "[_ HkT]".
      iApply (ustep_u_split_highfault_u va vpnL vpnH ieL ieH ilo
                ms_v sc_v stval_v sepc_v g tlbvec E Φ HN
                HsomeL Hchk0L HupdNL Hpbmt0L HcwL HnotRVC HsomeH HdenH Hok HSXL
                Hbit0 Hbit1 Hvalign4
                HcanonL Hvpn_defL HalignL HcanonH Hvpn_defH
                with "Hhw Hinv Hhs Hpriv Hms Hsc Hstv Hsepc Htlbc Hpc Hgpr Hupt
                      Hcode Hdata Hcfg HkT").
    - (* 65: 2-aligned split-fetch generic rd=x0 NO-OP, FETCH-UNCONDITIONAL *)
      iDestruct "Hk" as "[HkP _]".
      iApply (ustep_u_split_nop va vpn ie w ii
                ms_v sc_v stval_v sepc_v g tlbvec E Φ HN Hexec_op Hnlpad Hok
                Hsome Hchk0 HupdN HcwL HcwH
                HSXL Hbit0 Hbit1 Hvalign4
                HcanonL Hvpn_defL HalignL
