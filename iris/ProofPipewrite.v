(* ProofPipewrite.v -- the whole-function WP for xv6's pipewrite().
   ...
   - the while loop is UNBOUNDED (the sleep arm re-runs the same iteration),
     ...
     which takes the loop assertion itself as a premise.  Both back edges pass
     through an [iNext] before applying the Löb IH: the +0xcc [c.j], and (on
     the sleep arm) the +0x98 [beq]-taken, whose later is stripped BEFORE
     wakeup/sleep run.  Each [iNext] also strips the [▷] off [▷ sched_vc], so
     it is re-introduced immediately after (the ProofAcquiresleep repair).
...
Require Import WpPipewriteDecode.
...
Proof.
  ...
  rewrite (sext64_moi32_unsigned (i + 1) ltac:(rewrite H231; lia)).
...
Local Notation PW := KernelSyms.pipewrite.
...
Definition pw_base_regs `{CID : CpuId} (m M : regfile) (spr : mword 64) : Prop :=
...
Lemma pw_base_regs_cs `{CID : CpuId} (m M1 M2 : regfile) (spr : mword 64) :
  callee_saved M1 M2 -> pw_base_regs m M1 spr -> pw_base_regs m M2 spr.
Proof.
  intros Hcs (A & B & C & D & E & F & G & H). unfold pw_base_regs.
  split_and!.
  - rewrite (callee_saved_lookup Hcs csp_rs1 ltac:(vm_compute; reflexivity)). exact A.
  - rewrite (callee_saved_lookup Hcs (mword_of_int 4) ltac:(vm_compute; reflexivity)). exact B.
...
Definition pw_min_regs `{CID : CpuId} (m M : regfile) (spr pi : mword 64) : Prop :=
  M !!! Regidx csp_rs1 = spr /\
  M !!! Regidx Rtp = cid_word /\
  M !!! Regidx Rs1 = pi /\
  M !!! Regidx Rs11 = m !!! Regidx Rs11.

Lemma pw_min_regs_cs `{CID : CpuId} (m M1 M2 : regfile) (spr pi : mword 64) :
  callee_saved M1 M2 -> pw_min_regs m M1 spr pi -> pw_min_regs m M2 spr pi.
Proof.
  intros Hcs (A & B & C & D). unfold pw_min_regs. split_and!.
  - rewrite (callee_saved_lookup Hcs csp_rs1 ltac:(vm_compute; reflexivity)). exact A.
  - rewrite (callee_saved_lookup Hcs (mword_of_int 4) ltac:(vm_compute; reflexivity)). exact B.
...
Definition pw_loop_regs `{CID : CpuId} (m M : regfile)
...
Section PwPieces.
  ...
  (* copyin CONSUMES [kalloc_env] and hands nothing back, and the loop calls it
     ...
     conjunct is persistent ([is_lock], [kalloc_sealed], [panic_wp]), which is
...
Section PwConts.
  ...
  Definition pw_epi (γ γf : gname) (Φ : mval -> iProp Σ) (γs : list gname) (j : nat)
    ...
    (∀ (M : regfile) (P' : uptd),
       ...
       pw_frame7 m sp0 -∗
       stack_own (pa_stk sp0 7%nat) 7%nat -∗
       sie_cap_gpr γ M (av - 14)%nat -∗
       cpu_own γ 0%nat eb (proc_addr j) C -∗
       pc_is (mword_of_int (PW + 0x58) : mword 64) -∗
       pipe_ref γp w q -∗
  ...
  Definition pw_tail (γ γf : gname) (Φ : mval -> iProp Σ) (γs : list gname) (j : nat)
    ...
    (∀ (M : regfile) (P' : uptd),
       ...
       stack_own (pa_stk sp0 7%nat) 7%nat -∗
       sie_cap_gpr γ M (av - 14)%nat -∗
       cpu_own γ 1%nat eb (proc_addr j) C -∗
       trap_csrs_pay 0%nat eb -∗
       locked γl cpu_id -∗
       pipe_res γp pi -∗
       pc_is (mword_of_int (PW + 0xd8) : mword 64) -∗
       pipe_ref γp w q -∗
       proc_priv γf (proc_addr j) pid (upd_upt V P') -∗
       own_ctx (p_context (proc_addr j)) -∗
       ▷ sched_vc γ Φ γs (a_cpu_ctx cid_word) (proc_addr j) -∗
  ...
  Definition pw_minus1 (γ γf : gname) (Φ : mval -> iProp Σ) (γs : list gname) (j : nat)
      (γl : gname) (γp : pipe_names) (w : bool) (q : Qp)
      (m : regfile) (av : nat) (eb : bool) (C : iProp Σ)
      (pid : mword 32) (V : pprivate) (n : Z) (sp0 pi : mword 64) : iProp Σ :=
    (∀ (M : regfile) (P' : uptd),
       ...
       locked γl cpu_id -∗
       pipe_res γp pi -∗
       pc_is (mword_of_int (PW + 0x46) : mword 64) -∗
       pipe_ref γp w q -∗
       proc_priv γf (proc_addr j) pid (upd_upt V P') -∗
       own_ctx (p_context (proc_addr j)) -∗
       ▷ sched_vc γ Φ γs (a_cpu_ctx cid_word) (proc_addr j) -∗
  ...
  Definition pw_exits (γ γf : gname) (Φ : mval -> iProp Σ) (γs : list gname) (j : nat)
      (γl : gname) (γp : pipe_names) (w : bool) (q : Qp)
      (m : regfile) (av : nat) (eb : bool) (C : iProp Σ)
      (pid : mword 32) (V : pprivate) (n : Z) (sp0 pi : mword 64) : iProp Σ :=
    (pw_tail γ γf Φ γs j γl γp w q m av eb C pid V n sp0 pi
     ∧ pw_minus1 γ γf Φ γs j γl γp w q m av eb C pid V n sp0 pi)%I.

  (* +0x7e: the loop BODY, entered with 0 <= i < n. *)
  Definition pw_loop (γ γa γf : gname) (Φ : mval -> iProp Σ) (γs : list gname) (j : nat)
      (γl : gname) (γp : pipe_names) (w : bool) (q : Qp)
      (m : regfile) (av : nat) (eb : bool) (C : iProp Σ)
    ...
    (∀ (i : Z) (M : regfile) (Pc : uptd),
       ...
       locked γl cpu_id -∗
       pipe_res γp pi -∗
       pc_is (mword_of_int (PW + 0x7e) : mword 64) -∗
       pipe_ref γp w q -∗
       proc_priv γf (proc_addr j) pid (upd_upt V Pc) -∗
       kalloc_env γa None cid_word -∗
       own_ctx (p_context (proc_addr j)) -∗
       ▷ sched_vc γ Φ γs (a_cpu_ctx cid_word) (proc_addr j) -∗
       pw_exits γ γf Φ γs j γl γp w q m av eb C pid V n sp0 pi -∗
       WP (Loop : expr riscv_lang) {{ Φ }})%I.
...
Section PwRestore.
  ...
  Lemma pw_restore5 (γ : gname) (Φ : mval -> iProp Σ)
    ...
    ( ∀ M' : regfile, ⌜ pw_restored m M M' ⌝ -∗
        sie_cap_gpr γ M' K -∗ pc_is p5 -∗ pw_frame5 m sp0 -∗
  ...
  Proof.
    ...
    iApply (wp_cldsp_s_sconf γ Φ p0 (mword_of_int 6 : mword 6) Rs6 M K (m !!! Regidx Rs6)
    ...
    iApply (wp_cldsp_s_sconf γ Φ p1 (mword_of_int 5 : mword 6) Rs7 N1 K (m !!! Regidx Rs7)
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi1 F9 [-]").
    iIntros "Hcg Hpc F9".
    ...
    iApply (wp_cldsp_s_sconf γ Φ p2 (mword_of_int 4 : mword 6) Rs8 N2 K (m !!! Regidx Rs8)
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
    ...
    iApply (wp_cldsp_s_sconf γ Φ p3 (mword_of_int 3 : mword 6) Rs9 N3 K (m !!! Regidx Rs9)
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
    ...
    iApply (wp_cldsp_s_sconf γ Φ p4 (mword_of_int 2 : mword 6) Rs10 N4 K (m !!! Regidx Rs10)
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
...
Section PwGuard.
  ...
  Lemma pw_guard_step (γ γa γf : gname) (Φ : mval -> iProp Σ) (γs : list gname) (j : nat)
      (γl : gname) (γp : pipe_names) (w : bool) (q : Qp)
      (m : regfile) (av : nat) (eb : bool) (C : iProp Σ)
    ...
    locked γl cpu_id -∗
    pipe_res γp pi -∗
    pc_is (mword_of_int (PW + 0x7a) : mword 64) -∗
    pipe_ref γp w q -∗
    proc_priv γf (proc_addr j) pid (upd_upt V Pc) -∗
    kalloc_env γa None cid_word -∗
    own_ctx (p_context (proc_addr j)) -∗
    ▷ sched_vc γ Φ γs (a_cpu_ctx cid_word) (proc_addr j) -∗
    pw_exits γ γf Φ γs j γl γp w q m av eb C pid V n sp0 pi -∗
    pw_loop γ γa γf Φ γs j γl γp w q m av eb C pid V n sp0 pi addr -∗
    WP (Loop : expr riscv_lang) {{ Φ }}.
  Proof.
    intros Hn0 Hn31 Hi Hext Hregs.
    ...
    destruct Hregs' as (Hsp & Htp & Hs0 & Hs1 & Hs2 & Hs3 & Hs4 & Hs5 &
    ...
    iIntros "#Htext HF7 HF5 HCH Hcg Hown Hpay Hlocked Hres Hpc Href Hpriv #Henv Hctx Hsched HEX HLP".
    ...
    - (* ==== i >= n : the copy is done, restore s6..s10 and take the tail ==== *)
      ...
      assert (Hgt : zopz0zKzJ_s (M !!! Regidx Rs2) (M !!! Regidx Rs4) = true)
        by exact Hcmp.
      assert (Hal : eq_vec (access_vec_dec (add_vec (mword_of_int (PW + 0x7a) : mword 64)
                       (sign_extend' 64 (mword_of_int 84 : mword 13))) 0) ('b"0") = true)
        by (vm_compute; reflexivity).
      iApply (wp_bge_taken_s_sconf γ Φ (mword_of_int (PW + 0x7a)) (mword_of_int 84 : mword 13)
                Rs4 Rs2 M (av - 14)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                Hgt Hal with "Hcg Hpc Hi7a [-]").
      iNext. iIntros "Hcg Hpc".
      iAssert (▷ sched_vc γ Φ γs (a_cpu_ctx cid_word) (proc_addr j))%I with "[Hsched]" as "Hsched".
      { iNext. iExact "Hsched". }
      assert (Hjce : add_vec (mword_of_int (PW + 0x7a) : mword 64)
                       (sign_extend' 64 (mword_of_int 84 : mword 13))
                     = (mword_of_int (PW + 0xce) : mword 64)) by (apply bv_eq; vm_compute; reflexivity).
      ...
      assert (Ed0 : add_vec_int (mword_of_int (PW + 0xce) : mword 64) 2 = mword_of_int (PW + 0xd0)) by (apply bv_eq; vm_compute; reflexivity).
      assert (Ed2 : add_vec_int (mword_of_int (PW + 0xd0) : mword 64) 2 = mword_of_int (PW + 0xd2)) by (apply bv_eq; vm_compute; reflexivity).
      assert (Ed4 : add_vec_int (mword_of_int (PW + 0xd2) : mword 64) 2 = mword_of_int (PW + 0xd4)) by (apply bv_eq; vm_compute; reflexivity).
      assert (Ed6 : add_vec_int (mword_of_int (PW + 0xd4) : mword 64) 2 = mword_of_int (PW + 0xd6)) by (apply bv_eq; vm_compute; reflexivity).
      assert (Ed8 : add_vec_int (mword_of_int (PW + 0xd6) : mword 64) 2 = mword_of_int (PW + 0xd8)) by (apply bv_eq; vm_compute; reflexivity).
      iApply (pw_restore5 γ Φ (mword_of_int (PW + 0xce)) (mword_of_int (PW + 0xd0))
                (mword_of_int (PW + 0xd2)) (mword_of_int (PW + 0xd4)) (mword_of_int (PW + 0xd6))
                (mword_of_int (PW + 0xd8)) M m (av - 14)%nat sp0
                Hsp Ed0 Ed2 Ed4 Ed6 Ed8
                with "Hcg Hpc Hj0 Hj1 Hj2 Hj3 Hj4 HF5 [-]").
      iIntros (M') "%Hrst Hcg Hpc HF5".
      ...
      assert (Htp' : M' !!! Regidx Rtp = cid_word).
      { rewrite (Rrest (mword_of_int 4) ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                   ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
      ...
      iDestruct "HEX" as "[TAIL _]".
      iApply ("TAIL" $! M' Pc with "[%] [%] [%] [%] HF7 [HF5 HCH] Hcg Hown Hpay Hlocked Hres Hpc Href Hpriv Hctx Hsched").
    ...
    - (* ==== i < n : fall through into the body ==== *)
      ...
      assert (Hgf : zopz0zKzJ_s (M !!! Regidx Rs2) (M !!! Regidx Rs4) = false)
        by exact Hcmp.
      iApply (wp_bge_fall_s_sconf γ Φ (mword_of_int (PW + 0x7a)) (mword_of_int 84 : mword 13)
                Rs4 Rs2 M (av - 14)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                Hgf with "Hcg Hpc Hi7a [-]").
      iIntros "Hcg Hpc".
      assert (Hpp7e : add_vec_int (mword_of_int (PW + 0x7a) : mword 64) 4 = mword_of_int (PW + 0x7e))
        by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hpp7e) in "Hpc".
      iApply ("HLP" $! i M Pc with "[%] [%] [%] HF7 HF5 HCH Hcg Hown Hpay Hlocked Hres Hpc Href Hpriv Henv Hctx Hsched HEX").
      + lia.
      + exact Hext.
      + exact Hregs.
  Qed.
...
Section ProofPipewrite.
  ...
  Lemma wp_pipewrite_sconf (γ : gname) (γa : gname) (γf : gname) (Φ : mval -> iProp Σ)
  ...
  Proof.
    ...
    intros pcE pj pi ret_tgt Htp Hj Hjlp Hlen Ha2 Hnrange Hav.
    ...
    (* ================================================================= *)
    iAssert (pw_epi γ γf Φ γs j γp w q m av eb C pid V n sp0) with "[Hcont]" as "EPI".
    { rewrite /pw_epi. iIntros (M P') "%Hbr %Hrw %Hext (Hc1 & Hc2 & Hc3 & Hc4 & Hc5 & Hc6 & Hc7) Hhi Hcg Hown Hpc Href Hpriv Hctx Hsched".
      destruct Hbr as (Hsp & Htpm & B6 & B7 & B8 & B9 & B10 & B11).
      ...
      iApply (wp_cmv_s_sconf γ Φ (mword_of_int (PW + 0x58)) Ra0 Rs2 M (av - 14)%nat
                ltac:(nz) ltac:(nz) with "Hcg Hpc Hi58 [-]").
      iIntros "Hcg Hpc".
      ...
      assert (Hpp5a : add_vec_int (mword_of_int (PW + 0x58) : mword 64) 2 = mword_of_int (PW + 0x5a)) by (apply bv_eq; vm_compute; reflexivity).
      ...
      iApply (wp_cldsp_s_sconf γ Φ (mword_of_int (PW + 0x5a)) (mword_of_int 13 : mword 6) Rra
                E1 (av - 14)%nat (m !!! Regidx Rra) ltac:(nz) ltac:(nz)
      ...
      assert (Hpp5c : add_vec_int (mword_of_int (PW + 0x5a) : mword 64) 2 = mword_of_int (PW + 0x5c)) by (apply bv_eq; vm_compute; reflexivity).
      ...
      iApply (wp_cldsp_s_sconf γ Φ (mword_of_int (PW + 0x5c)) (mword_of_int 12 : mword 6) Rs0
                E2 (av - 14)%nat (m !!! Regidx Rs0) ltac:(nz) ltac:(nz)
      ...
      assert (Hpp5e : add_vec_int (mword_of_int (PW + 0x5c) : mword 64) 2 = mword_of_int (PW + 0x5e)) by (apply bv_eq; vm_compute; reflexivity).
      ...
      iApply (wp_cldsp_s_sconf γ Φ (mword_of_int (PW + 0x5e)) (mword_of_int 11 : mword 6) Rs1
                E3 (av - 14)%nat (m !!! Regidx Rs1) ltac:(nz) ltac:(nz)
      ...
      assert (Hpp60 : add_vec_int (mword_of_int (PW + 0x5e) : mword 64) 2 = mword_of_int (PW + 0x60)) by (apply bv_eq; vm_compute; reflexivity).
      ...
      iApply (wp_cldsp_s_sconf γ Φ (mword_of_int (PW + 0x60)) (mword_of_int 10 : mword 6) Rs2
                E4 (av - 14)%nat (m !!! Regidx Rs2) ltac:(nz) ltac:(nz)
      ...
      assert (Hpp62 : add_vec_int (mword_of_int (PW + 0x60) : mword 64) 2 = mword_of_int (PW + 0x62)) by (apply bv_eq; vm_compute; reflexivity).
      ...
      iApply (wp_cldsp_s_sconf γ Φ (mword_of_int (PW + 0x62)) (mword_of_int 9 : mword 6) Rs3
                E5 (av - 14)%nat (m !!! Regidx Rs3) ltac:(nz) ltac:(nz)
      ...
      assert (Hpp64 : add_vec_int (mword_of_int (PW + 0x62) : mword 64) 2 = mword_of_int (PW + 0x64)) by (apply bv_eq; vm_compute; reflexivity).
      ...
      iApply (wp_cldsp_s_sconf γ Φ (mword_of_int (PW + 0x64)) (mword_of_int 8 : mword 6) Rs4
                E6 (av - 14)%nat (m !!! Regidx Rs4) ltac:(nz) ltac:(nz)
      ...
      assert (Hpp66 : add_vec_int (mword_of_int (PW + 0x64) : mword 64) 2 = mword_of_int (PW + 0x66)) by (apply bv_eq; vm_compute; reflexivity).
      ...
      iApply (wp_cldsp_s_sconf γ Φ (mword_of_int (PW + 0x66)) (mword_of_int 7 : mword 6) Rs5
                E7 (av - 14)%nat (m !!! Regidx Rs5) ltac:(nz) ltac:(nz)
      ...
      assert (Hpp68 : add_vec_int (mword_of_int (PW + 0x66) : mword 64) 2 = mword_of_int (PW + 0x68)) by (apply bv_eq; vm_compute; reflexivity).
      ...
      iApply (wp_caddi16sp_pop_s_sconf γ Φ (mword_of_int (PW + 0x68)) (mword_of_int 7 : mword 6)
                E8 (av - 14)%nat 14%nat Hpop with "Hcg Hpc Hi68 Hframe [-]").
      iIntros "Hcg Hpc".
      ...
      assert (Hpp6a : add_vec_int (mword_of_int (PW + 0x68) : mword 64) 2 = mword_of_int (PW + 0x6a)) by (apply bv_eq; vm_compute; reflexivity).
      ...
      iApply (wp_cret_s_sconf γ Φ (mword_of_int (PW + 0x6a)) Rra E9 av ltac:(nz)
                with "Hcg Hpc Hi6a [-]").
      iIntros "Hcg Hpc".
      ...
      { unfold callee_saved. split_and!.
        ...
        - rewrite /E9 upd_ne; [| reg_neq]. rewrite /E8 upd_ne; [| reg_neq].
          ...
          rewrite /E5 upd_ne; [| reg_neq]. rewrite /E4 upd_ne; [| reg_neq].
          rewrite /E3 upd_ne; [| reg_neq]. rewrite /E2 upd_ne; [| reg_neq].
          rewrite /E1 upd_ne; [| reg_neq]. rewrite Htpm. symmetry. exact Htp.
        - rewrite /E9 upd_ne; [| reg_neq]. rewrite /E8 upd_ne; [| reg_neq].
          rewrite /E7 upd_ne; [| reg_neq]. rewrite /E6 upd_ne; [| reg_neq].
          rewrite /E5 upd_ne; [| reg_neq]. rewrite /E4 upd_ne; [| reg_neq].
      ...
      iApply ("Hcont" $! E9 P' with "[%] [%] [%] Hcg Hown Hpc Href Hpriv Hctx Hsched").
      - exact HcsE9.
      - exact Hext.
    ...
    (* ================================================================= *)
    iAssert (pw_exits γ γf Φ γs j γl γp w q m av eb C pid V n sp0 pi) with "[EPI]" as "EXITS".
    { rewrite /pw_exits. iSplit.
      - (* ---------------- +0xd8: wakeup(&pi->nread); release ---------------- *)
        rewrite /pw_tail.
        iIntros (M P') "%Hbr %Hs1M %Hrw %Hext HF7 Hhi Hcg Hown Hpay Hlocked Hres Hpc Href Hpriv Hctx Hsched".
        iPoseProof (pwi_d8 with "Htext") as "Hid8".
        iApply (wp_addi4_s_sconf γ Φ (mword_of_int (PW + 0xd8)) Ra0 Rs1
                  (mword_of_int 536 : mword 12) M (av - 14)%nat ltac:(nz) ltac:(nz)
                  with "Hcg Hpc Hid8 [-]").
        iIntros "Hcg Hpc".
        ...
        assert (Hppdc : add_vec_int (mword_of_int (PW + 0xd8) : mword 64) 4 = mword_of_int (PW + 0xdc)) by (apply bv_eq; vm_compute; reflexivity).
        iEval (rewrite Hppdc) in "Hpc".
        iPoseProof (pwi_dc with "Htext") as "Hidc".
        iApply (wp_jal_s_sconf γ Φ (mword_of_int (PW + 0xdc)) Rra (mword_of_int 2087384 : mword 21)
                  T1 (av - 14)%nat ltac:(nz) ltac:(nz) ltac:(vm_compute; reflexivity)
                  with "Hcg Hpc Hidc [-]").
        iIntros "Hcg Hpc".
        set (T2 := <[Regidx Rra := regval_into_reg (add_vec_int (mword_of_int (PW + 0xdc) : mword 64) 4)]> T1).
        change (<[Regidx Rra := regval_into_reg (add_vec_int (mword_of_int (PW + 0xdc) : mword 64) 4)]> T1) with T2.
        assert (Hjwk : add_vec (mword_of_int (PW + 0xdc) : mword 64) (sign_extend' 64 (mword_of_int 2087384 : mword 21))
                       = mword_of_int KernelSyms.wakeup) by (apply bv_eq; vm_compute; reflexivity).
        iEval (rewrite Hjwk) in "Hpc".
        destruct Hbr as (Hsp & Htpm & B6 & B7 & B8 & B9 & B10 & B11).
        assert (HtpT2 : T2 !!! Regidx Rtp = cid_word).
        { rewrite /T2 upd_ne; [| reg_neq]. rewrite /T1 upd_ne; [| reg_neq]. exact Htpm. }
        ...
        assert (Hwa0f : mycpu_ret (T2 !!! Regidx Rtp) = mycpu_ret cid_word) by (rewrite HtpT2; reflexivity).
        assert (Hwnz : eq_vec (zero_reg : mword 64) (mycpu_ret (T2 !!! Regidx Rtp)) = false)
          by (rewrite HtpT2; exact Hcpune).
        assert (Hwlvl : (Z.of_nat 1%nat + 1 < 2 ^ 31)%Z) by (rewrite H31; lia).
        iApply (Wakeup.wp_wakeup_sconf γ Φ T2 γs (mycpu_ret cid_word) (proc_addr j) 1%nat (av - 14)%nat eb C
                  HwK HwdomW Hlen HtpT2 Hwa0f Hwnz Hwlvl
                  with "Hcg Hown Htext Hpc Hpanic Hpinv [-]").
        iIntros (Mw) "[%Hwcs %Hwdom] Hcg Hown Htext2 Hpc".
        assert (HraT2 : T2 !!! Regidx Rra = add_vec_int (mword_of_int (PW + 0xdc) : mword 64) 4)
          by (rewrite /T2; apply upd_eq).
        iEval (rewrite HraT2) in "Hpc".
        assert (Hppe0 : ret_pc (add_vec_int (mword_of_int (PW + 0xdc) : mword 64) 4)
                        = (mword_of_int (PW + 0xe0) : mword 64)) by (apply bv_eq; vm_compute; reflexivity).
        ...
        assert (HtpMw : Mw !!! Regidx Rtp = cid_word).
        { rewrite (callee_saved_lookup HcsMMw (mword_of_int 4) ltac:(vm_compute; reflexivity)). exact Htpm. }
        (* +0xe0 c.mv a0,s1 *)
        iPoseProof (pwi_e0 with "Htext") as "Hie0".
        iApply (wp_cmv_s_sconf γ Φ (mword_of_int (PW + 0xe0)) Ra0 Rs1 Mw (av - 14)%nat
                  ltac:(nz) ltac:(nz) with "Hcg Hpc Hie0 [-]").
        iIntros "Hcg Hpc".
        ...
        assert (Hppe2 : add_vec_int (mword_of_int (PW + 0xe0) : mword 64) 2 = mword_of_int (PW + 0xe2)) by (apply bv_eq; vm_compute; reflexivity).
        iEval (rewrite Hppe2) in "Hpc".
        (* +0xe2 jal release *)
        iPoseProof (pwi_e2 with "Htext") as "Hie2".
        iApply (wp_jal_s_sconf γ Φ (mword_of_int (PW + 0xe2)) Rra (mword_of_int 2082576 : mword 21)
                  T3 (av - 14)%nat ltac:(nz) ltac:(nz) ltac:(vm_compute; reflexivity)
                  with "Hcg Hpc Hie2 [-]").
        iIntros "Hcg Hpc".
        set (T4 := <[Regidx Rra := regval_into_reg (add_vec_int (mword_of_int (PW + 0xe2) : mword 64) 4)]> T3).
        change (<[Regidx Rra := regval_into_reg (add_vec_int (mword_of_int (PW + 0xe2) : mword 64) 4)]> T3) with T4.
        assert (Hjrl : add_vec (mword_of_int (PW + 0xe2) : mword 64) (sign_extend' 64 (mword_of_int 2082576 : mword 21))
        ...
        { rewrite Ha0T4.
          replace (sign_extend' 64 (mword_of_int 0 : mword 12)) with (mword_of_int 0 : mword 64)
            by (apply bv_eq; vm_compute; reflexivity).
          apply kv_addv_zero. }
        assert (HtpT4 : T4 !!! Regidx Rtp = cid_word).
        { rewrite /T4 upd_ne; [| reg_neq]. rewrite /T3 upd_ne; [| reg_neq]. exact HtpMw. }
        assert (HavR : (10 <= av - 14)%nat) by lia.
        iApply (ReleaseGen.wp_release_gen_sconf γ Φ γl pi (pipe_res γp pi) (pipe_dead γl γp) emp%I
                  T4 0%nat eb (proc_addr j) C (av - 14)%nat HlkaT4 HtpT4 HavR
        ...
        assert (HraT4 : T4 !!! Regidx Rra = add_vec_int (mword_of_int (PW + 0xe2) : mword 64) 4)
          by (rewrite /T4; apply upd_eq).
        iEval (rewrite HraT4) in "Hpc".
        assert (Hppe6 : ret_pc (add_vec_int (mword_of_int (PW + 0xe2) : mword 64) 4)
                        = (mword_of_int (PW + 0xe6) : mword 64)) by (apply bv_eq; vm_compute; reflexivity).
        ...
        iApply (wp_cj_s_sconf γ Φ (mword_of_int (PW + 0xe6))
                  (sign_extend' 21 (concat_vec (mword_of_int 1977 : mword 11) ('b"0")))
                  mr (av - 14)%nat ltac:(vm_compute; reflexivity) with "Hcg Hpc Hie6 [-]").
        iNext. iIntros "Hcg Hpc".
        iAssert (▷ sched_vc γ Φ γs (a_cpu_ctx cid_word) (proc_addr j))%I with "[Hsched]" as "Hsched".
        { iNext. iExact "Hsched". }
        assert (Hjep : add_vec (mword_of_int (PW + 0xe6) : mword 64)
                         (sign_extend' 64 (sign_extend' 21 (concat_vec (mword_of_int 1977 : mword 11) ('b"0"))))
                       = mword_of_int (PW + 0x58)) by (apply bv_eq; vm_compute; reflexivity).
        iEval (rewrite Hjep) in "Hpc".
        iApply ("EPI" $! mr P' with "[%] [%] [%] HF7 Hhi Hcg Hown Hpc Href Hpriv Hctx Hsched").
      ...
      - (* ---------------- +0x46: release; i := -1; reload s6..s10 ---------------- *)
        rewrite /pw_minus1.
        iIntros (M P') "%Hmr %Hext HF7 HF5 HCH Hcg Hown Hpay Hlocked Hres Hpc Href Hpriv Hctx Hsched".
        destruct Hmr as (Hsp & Htpm & Hs1M & Hs11M).
        iPoseProof (pwi_46 with "Htext") as "Hi46".
        iApply (wp_cmv_s_sconf γ Φ (mword_of_int (PW + 0x46)) Ra0 Rs1 M (av - 14)%nat
                  ltac:(nz) ltac:(nz) with "Hcg Hpc Hi46 [-]").
        iIntros "Hcg Hpc".
        ...
        assert (Hpp48 : add_vec_int (mword_of_int (PW + 0x46) : mword 64) 2 = mword_of_int (PW + 0x48)) by (apply bv_eq; vm_compute; reflexivity).
        iEval (rewrite Hpp48) in "Hpc".
        iPoseProof (pwi_48 with "Htext") as "Hi48".
        iApply (wp_jal_s_sconf γ Φ (mword_of_int (PW + 0x48)) Rra (mword_of_int 2082730 : mword 21)
                  Q1 (av - 14)%nat ltac:(nz) ltac:(nz) ltac:(vm_compute; reflexivity)
                  with "Hcg Hpc Hi48 [-]").
        iIntros "Hcg Hpc".
        set (Q2 := <[Regidx Rra := regval_into_reg (add_vec_int (mword_of_int (PW + 0x48) : mword 64) 4)]> Q1).
        change (<[Regidx Rra := regval_into_reg (add_vec_int (mword_of_int (PW + 0x48) : mword 64) 4)]> Q1) with Q2.
        assert (Hjrl : add_vec (mword_of_int (PW + 0x48) : mword 64) (sign_extend' 64 (mword_of_int 2082730 : mword 21))
                       = mword_of_int KernelSyms.release) by (apply bv_eq; vm_compute; reflexivity).
        iEval (rewrite Hjrl) in "Hpc".
        ...
        { rewrite Ha0Q2.
          replace (sign_extend' 64 (mword_of_int 0 : mword 12)) with (mword_of_int 0 : mword 64)
            by (apply bv_eq; vm_compute; reflexivity).
          apply kv_addv_zero. }
        assert (HtpQ2 : Q2 !!! Regidx Rtp = cid_word).
        { rewrite /Q2 upd_ne; [| reg_neq]. rewrite /Q1 upd_ne; [| reg_neq]. exact Htpm. }
        assert (HavR : (10 <= av - 14)%nat) by lia.
        iApply (ReleaseGen.wp_release_gen_sconf γ Φ γl pi (pipe_res γp pi) (pipe_dead γl γp) emp%I
                  Q2 0%nat eb (proc_addr j) C (av - 14)%nat HlkaQ2 HtpQ2 HavR
                  ltac:(iApply locked_dead) ltac:(iApply locked_pre_dead)
                  with "Hcg Htext Hpc Hopen Hlocked Hres [] Hown Hpay [-]").
        { iApply lock_finisher_close. }
        iIntros (mr) "_ Hcg Hpc %Hcsr Hown".
        assert (HraQ2 : Q2 !!! Regidx Rra = add_vec_int (mword_of_int (PW + 0x48) : mword 64) 4)
          by (rewrite /Q2; apply upd_eq).
        iEval (rewrite HraQ2) in "Hpc".
        assert (Hpp4c : ret_pc (add_vec_int (mword_of_int (PW + 0x48) : mword 64) 4)
                        = (mword_of_int (PW + 0x4c) : mword 64)) by (apply bv_eq; vm_compute; reflexivity).
        ...
        iApply (wp_cli_s_sconf γ Φ (mword_of_int (PW + 0x4c)) Rs2 (mword_of_int 63 : mword 6)
                  (mword_of_int (-1) : mword 64) mr (av - 14)%nat ltac:(nz) ltac:(nz)
        ...
        assert (Hpp4e : add_vec_int (mword_of_int (PW + 0x4c) : mword 64) 2 = mword_of_int (PW + 0x4e)) by (apply bv_eq; vm_compute; reflexivity).
        ...
        assert (HtpQ3 : Q3 !!! Regidx Rtp = cid_word).
        { rewrite /Q3 upd_ne; [| reg_neq].
          rewrite (callee_saved_lookup HcsMmr (mword_of_int 4) ltac:(vm_compute; reflexivity)). exact Htpm. }
        ...
        assert (E50 : add_vec_int (mword_of_int (PW + 0x4e) : mword 64) 2 = mword_of_int (PW + 0x50)) by (apply bv_eq; vm_compute; reflexivity).
        assert (E52 : add_vec_int (mword_of_int (PW + 0x50) : mword 64) 2 = mword_of_int (PW + 0x52)) by (apply bv_eq; vm_compute; reflexivity).
        assert (E54 : add_vec_int (mword_of_int (PW + 0x52) : mword 64) 2 = mword_of_int (PW + 0x54)) by (apply bv_eq; vm_compute; reflexivity).
        assert (E56 : add_vec_int (mword_of_int (PW + 0x54) : mword 64) 2 = mword_of_int (PW + 0x56)) by (apply bv_eq; vm_compute; reflexivity).
        assert (E58 : add_vec_int (mword_of_int (PW + 0x56) : mword 64) 2 = mword_of_int (PW + 0x58)) by (apply bv_eq; vm_compute; reflexivity).
        iApply (pw_restore5 γ Φ (mword_of_int (PW + 0x4e)) (mword_of_int (PW + 0x50))
                  (mword_of_int (PW + 0x52)) (mword_of_int (PW + 0x54)) (mword_of_int (PW + 0x56))
                  (mword_of_int (PW + 0x58)) Q3 m (av - 14)%nat sp0
        ...
        assert (Htp' : M' !!! Regidx Rtp = cid_word).
        { rewrite (Rrest (mword_of_int 4) ltac:(nz) ltac:(nz) ltac:(nz) ltac:(nz) ltac:(nz)). exact HtpQ3. }
        assert (Hs11' : M' !!! Regidx Rs11 = m !!! Regidx Rs11).
        ...
        iApply ("EPI" $! M' P' with "[%] [%] [%] HF7 [HF5 HCH] Hcg Hown Hpc Href Hpriv Hctx Hsched").
        + unfold pw_base_regs. split_and!; assumption.
        + rewrite Hs2'. by left.
        + exact Hext.
        + iApply (pw_stack7_of m sp0 with "HF5 HCH"). }
    ...
    iApply (wp_caddi16sp_push_s_sconf γ Φ pcE (mword_of_int 57 : mword 6) m av 14%nat ltac:(lia) Hpush
              with "Hcg Hpc Hi00 [-]").
    ...
    assert (Hpp02 : add_vec_int (pcE : mword 64) 2 = mword_of_int (PW + 0x02)) by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (PW + 0x02)) (mword_of_int 13 : mword 6) Rra
              A0 (av - 14)%nat v1 with "Hcg Hpc Hi02 Hc1 [-]").
    iIntros "Hcg Hpc Hc1".
    iEval (rewrite HspA0 Hb1 HraA0) in "Hc1".
    assert (Hpp04 : add_vec_int (mword_of_int (PW + 0x02) : mword 64) 2 = mword_of_int (PW + 0x04)) by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp04) in "Hpc".
    (* +0x04 c.sdsp s0,96(sp) *)
    iPoseProof (pwi_04 with "Htext") as "Hi04".
    iEval (rewrite -Hb2 -HspA0) in "Hc2".
    iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (PW + 0x04)) (mword_of_int 12 : mword 6) Rs0
              A0 (av - 14)%nat v2 with "Hcg Hpc Hi04 Hc2 [-]").
    iIntros "Hcg Hpc Hc2".
    iEval (rewrite HspA0 Hb2 Hs0A0) in "Hc2".
    assert (Hpp06 : add_vec_int (mword_of_int (PW + 0x04) : mword 64) 2 = mword_of_int (PW + 0x06)) by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp06) in "Hpc".
    (* +0x06 c.sdsp s1,88(sp) *)
    iPoseProof (pwi_06 with "Htext") as "Hi06".
    iEval (rewrite -Hb3 -HspA0) in "Hc3".
    iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (PW + 0x06)) (mword_of_int 11 : mword 6) Rs1
              A0 (av - 14)%nat v3 with "Hcg Hpc Hi06 Hc3 [-]").
    iIntros "Hcg Hpc Hc3".
    iEval (rewrite HspA0 Hb3 Hs1A0) in "Hc3".
    assert (Hpp08 : add_vec_int (mword_of_int (PW + 0x06) : mword 64) 2 = mword_of_int (PW + 0x08)) by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp08) in "Hpc".
    (* +0x08 c.sdsp s2,80(sp) *)
    iPoseProof (pwi_08 with "Htext") as "Hi08".
    iEval (rewrite -Hb4 -HspA0) in "Hc4".
    iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (PW + 0x08)) (mword_of_int 10 : mword 6) Rs2
              A0 (av - 14)%nat v4 with "Hcg Hpc Hi08 Hc4 [-]").
    iIntros "Hcg Hpc Hc4".
    iEval (rewrite HspA0 Hb4 Hs2A0) in "Hc4".
    assert (Hpp0a : add_vec_int (mword_of_int (PW + 0x08) : mword 64) 2 = mword_of_int (PW + 0x0a)) by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp0a) in "Hpc".
    (* +0x0a c.sdsp s3,72(sp) *)
    iPoseProof (pwi_0a with "Htext") as "Hi0a".
    iEval (rewrite -Hb5 -HspA0) in "Hc5".
    iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (PW + 0x0a)) (mword_of_int 9 : mword 6) Rs3
              A0 (av - 14)%nat v5 with "Hcg Hpc Hi0a Hc5 [-]").
    iIntros "Hcg Hpc Hc5".
    iEval (rewrite HspA0 Hb5 Hs3A0) in "Hc5".
    assert (Hpp0c : add_vec_int (mword_of_int (PW + 0x0a) : mword 64) 2 = mword_of_int (PW + 0x0c)) by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp0c) in "Hpc".
    (* +0x0c c.sdsp s4,64(sp) *)
    iPoseProof (pwi_0c with "Htext") as "Hi0c".
    iEval (rewrite -Hb6 -HspA0) in "Hc6".
    iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (PW + 0x0c)) (mword_of_int 8 : mword 6) Rs4
              A0 (av - 14)%nat v6 with "Hcg Hpc Hi0c Hc6 [-]").
    iIntros "Hcg Hpc Hc6".
    iEval (rewrite HspA0 Hb6 Hs4A0) in "Hc6".
    assert (Hpp0e : add_vec_int (mword_of_int (PW + 0x0c) : mword 64) 2 = mword_of_int (PW + 0x0e)) by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp0e) in "Hpc".
    (* +0x0e c.sdsp s5,56(sp) *)
    iPoseProof (pwi_0e with "Htext") as "Hi0e".
    iEval (rewrite -Hb7 -HspA0) in "Hc7".
    iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (PW + 0x0e)) (mword_of_int 7 : mword 6) Rs5
              A0 (av - 14)%nat v7 with "Hcg Hpc Hi0e Hc7 [-]").
    iIntros "Hcg Hpc Hc7".
    iEval (rewrite HspA0 Hb7 Hs5A0) in "Hc7".
    assert (Hpp10 : add_vec_int (mword_of_int (PW + 0x0e) : mword 64) 2 = mword_of_int (PW + 0x10)) by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (wp_caddi4spn_s_sconf γ Φ (mword_of_int (PW + 0x10)) (Cregidx (mword_of_int 0))
              (mword_of_int 28 : mword 8) Rs0 A0 (av - 14)%nat
              ltac:(vm_compute; reflexivity) ltac:(nz) ltac:(nz) with "Hcg Hpc Hi10 [-]").
    iIntros "Hcg Hpc".
    ...
    assert (Hpp12 : add_vec_int (mword_of_int (PW + 0x10) : mword 64) 2 = mword_of_int (PW + 0x12)) by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp12) in "Hpc".
    (* +0x12 c.mv s1,a0 *)
    iPoseProof (pwi_12 with "Htext") as "Hi12".
    iApply (wp_cmv_s_sconf γ Φ (mword_of_int (PW + 0x12)) Rs1 Ra0 A1 (av - 14)%nat
    ...
    assert (Hpp14 : add_vec_int (mword_of_int (PW + 0x12) : mword 64) 2 = mword_of_int (PW + 0x14)) by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp14) in "Hpc".
    (* +0x14 c.mv s5,a1 *)
    iPoseProof (pwi_14 with "Htext") as "Hi14".
    iApply (wp_cmv_s_sconf γ Φ (mword_of_int (PW + 0x14)) Rs5 Ra1 A2 (av - 14)%nat
    ...
    assert (Hpp16 : add_vec_int (mword_of_int (PW + 0x14) : mword 64) 2 = mword_of_int (PW + 0x16)) by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp16) in "Hpc".
    (* +0x16 c.mv s4,a2 *)
    iPoseProof (pwi_16 with "Htext") as "Hi16".
    iApply (wp_cmv_s_sconf γ Φ (mword_of_int (PW + 0x16)) Rs4 Ra2 A3 (av - 14)%nat
    ...
    assert (Hpp18 : add_vec_int (mword_of_int (PW + 0x16) : mword 64) 2 = mword_of_int (PW + 0x18)) by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp18) in "Hpc".
    (* +0x18 jal myproc *)
    iPoseProof (pwi_18 with "Htext") as "Hi18".
    iApply (wp_jal_s_sconf γ Φ (mword_of_int (PW + 0x18)) Rra (mword_of_int 2085966 : mword 21)
              A4 (av - 14)%nat ltac:(nz) ltac:(nz) ltac:(vm_compute; reflexivity)
              with "Hcg Hpc Hi18 [-]").
    iIntros "Hcg Hpc".
    set (A5 := <[Regidx Rra := regval_into_reg (add_vec_int (mword_of_int (PW + 0x18) : mword 64) 4)]> A4).
    change (<[Regidx Rra := regval_into_reg (add_vec_int (mword_of_int (PW + 0x18) : mword 64) 4)]> A4) with A5.
    assert (Hjmp : add_vec (mword_of_int (PW + 0x18) : mword 64) (sign_extend' 64 (mword_of_int 2085966 : mword 21))
    ...
    assert (HtpA5 : A5 !!! Regidx Rtp = cid_word).
    { rewrite /A5 upd_ne; [| reg_neq]. rewrite /A4 upd_ne; [| reg_neq].
      rewrite /A3 upd_ne; [| reg_neq]. rewrite /A2 upd_ne; [| reg_neq].
      rewrite /A1 upd_ne; [| reg_neq]. rewrite /A0 upd_ne; [| reg_neq]. exact Htp. }
    ...
    assert (HraA5 : A5 !!! Regidx Rra = add_vec_int (mword_of_int (PW + 0x18) : mword 64) 4)
      by (rewrite /A5; apply upd_eq).
    assert (Hlvl0 : (Z.of_nat 0%nat + 1 < 2 ^ 31)%Z) by (rewrite H31; lia).
    assert (Hav10 : (10 <= av - 14)%nat) by lia.
    iApply (Myproc.wp_myproc_sconf γ Φ A5 (av - 14)%nat 0%nat eb pj C
              HtpA5 Hlvl0 Hav10 with "Hcg Hown Htext Hpc [-]").
    iIntros (ms M0) "%Hms Hcg Hown Hpc %HcsM0".
    destruct HcsM0 as [HcsM0 Ha0M0].
    assert (Hpp1c : ret_pc (A5 !!! Regidx Rra) = (mword_of_int (PW + 0x1c) : mword 64))
      by (rewrite HraA5; apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp1c) in "Hpc".
    (* +0x1c c.mv s3,a0 *)
    iPoseProof (pwi_1c with "Htext") as "Hi1c".
    iApply (wp_cmv_s_sconf γ Φ (mword_of_int (PW + 0x1c)) Rs3 Ra0 M0 (av - 14)%nat
    ...
    assert (Hpp1e : add_vec_int (mword_of_int (PW + 0x1c) : mword 64) 2 = mword_of_int (PW + 0x1e)) by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (wp_cmv_s_sconf γ Φ (mword_of_int (PW + 0x1e)) Ra0 Rs1 B1 (av - 14)%nat
    ...
    assert (Hpp20 : add_vec_int (mword_of_int (PW + 0x1e) : mword 64) 2 = mword_of_int (PW + 0x20)) by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp20) in "Hpc".
    (* +0x20 jal acquire *)
    iPoseProof (pwi_20 with "Htext") as "Hi20".
    iApply (wp_jal_s_sconf γ Φ (mword_of_int (PW + 0x20)) Rra (mword_of_int 2082634 : mword 21)
              B2 (av - 14)%nat ltac:(nz) ltac:(nz) ltac:(vm_compute; reflexivity)
              with "Hcg Hpc Hi20 [-]").
    iIntros "Hcg Hpc".
    set (B3 := <[Regidx Rra := regval_into_reg (add_vec_int (mword_of_int (PW + 0x20) : mword 64) 4)]> B2).
    change (<[Regidx Rra := regval_into_reg (add_vec_int (mword_of_int (PW + 0x20) : mword 64) 4)]> B2) with B3.
    assert (Hjaq : add_vec (mword_of_int (PW + 0x20) : mword 64) (sign_extend' 64 (mword_of_int 2082634 : mword 21))
    ...
    assert (HtpB3 : B3 !!! Regidx Rtp = cid_word).
    { rewrite /B3 upd_ne; [| reg_neq]. rewrite /B2 upd_ne; [| reg_neq].
      rewrite /B1 upd_ne; [| reg_neq].
      rewrite (callee_saved_lookup HcsM0 (mword_of_int 4) ltac:(vm_compute; reflexivity)). exact HtpA5. }
    assert (HraB3 : B3 !!! Regidx Rra = add_vec_int (mword_of_int (PW + 0x20) : mword 64) 4)
    ...
    iApply (AcquireGen.wp_acquire_gen_sconf γ Φ γl (pipe_res γp pi) (pipe_ref γp w q)
              (pipe_dead γl γp) B3 0%nat eb pj C (av - 14)%nat HtpB3 Hlvl0 Hav10
              ltac:(iApply pipe_ref_dead) ltac:(iApply locked_pre_dead)
              with "Hcg Hown Htext Hpc [] Href Hpanic [-]").
    { iEval (rewrite Ha0B3). iExact "Hopen". }
    iIntros (ms2 M1) "%Hms2 Href Hcg Hpc %HcsM1 Hlocked Hres Hown Hpay".
    iEval (rewrite HraB3) in "Hpc".
    assert (Hpp24 : ret_pc (add_vec_int (mword_of_int (PW + 0x20) : mword 64) 4)
                    = (mword_of_int (PW + 0x24) : mword 64)) by (apply bv_eq; vm_compute; reflexivity).
    ...
    assert (HtpM1 : M1 !!! Regidx Rtp = cid_word).
    { rewrite (HthrM1 (mword_of_int 4) ltac:(vm_compute; reflexivity) ltac:(reg_neq)). exact HtpA5. }
    ...
    - (* ======= n <= 0: i := 0 and go straight to the +0xd8 tail ======= *)
      ...
      assert (Hal0 : eq_vec (access_vec_dec (add_vec (mword_of_int (PW + 0x24) : mword 64)
                       (sign_extend' 64 (mword_of_int 196 : mword 13))) 0) ('b"0") = true)
        by (vm_compute; reflexivity).
      iApply (wp_bge_x0_taken_s_sconf γ Φ (mword_of_int (PW + 0x24)) (mword_of_int 196 : mword 13)
                Rs4 M1 (av - 14)%nat ltac:(nz) Hgt0 Hal0 with "Hcg Hpc Hi24 [-]").
      iNext. iIntros "Hcg Hpc".
      iAssert (▷ sched_vc γ Φ γs (a_cpu_ctx cid_word) pj)%I with "[Hsched]" as "Hsched".
      { iNext. iExact "Hsched". }
      assert (Hje8 : add_vec (mword_of_int (PW + 0x24) : mword 64)
                       (sign_extend' 64 (mword_of_int 196 : mword 13))
                     = mword_of_int (PW + 0xe8)) by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hje8) in "Hpc".
      (* +0xe8 c.li s2,0 *)
      iPoseProof (pwi_e8 with "Htext") as "Hie8".
      iApply (wp_cli_s_sconf γ Φ (mword_of_int (PW + 0xe8)) Rs2 (mword_of_int 0 : mword 6)
                (mword_of_int 0 : mword 64) M1 (av - 14)%nat ltac:(nz) ltac:(nz)
      ...
      assert (Hppea : add_vec_int (mword_of_int (PW + 0xe8) : mword 64) 2 = mword_of_int (PW + 0xea)) by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hppea) in "Hpc".
      (* +0xea c.j -> the tail *)
      iPoseProof (pwi_ea with "Htext") as "Hiea".
      iApply (wp_cj_s_sconf γ Φ (mword_of_int (PW + 0xea))
                (sign_extend' 21 (concat_vec (mword_of_int 2039 : mword 11) ('b"0")))
                Z1 (av - 14)%nat ltac:(vm_compute; reflexivity) with "Hcg Hpc Hiea [-]").
      iNext. iIntros "Hcg Hpc".
      iAssert (▷ sched_vc γ Φ γs (a_cpu_ctx cid_word) pj)%I with "[Hsched]" as "Hsched".
      { iNext. iExact "Hsched". }
      assert (Hjd8 : add_vec (mword_of_int (PW + 0xea) : mword 64)
                       (sign_extend' 64 (sign_extend' 21 (concat_vec (mword_of_int 2039 : mword 11) ('b"0"))))
                     = mword_of_int (PW + 0xd8)) by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hjd8) in "Hpc".
      iDestruct "EXITS" as "[TAIL _]".
      iApply ("TAIL" $! Z1 (pv_upt V) with "[%] [%] [%] [%] HF7 Hhi Hcg Hown Hpay Hlocked Hres Hpc Href [Hpriv] Hctx Hsched").
      + (* [callee_saved M1 Z1] is false (s2 is callee-saved), so peel directly *)
        ...
        { rewrite /Z1 upd_ne; [exact HtpM1 | reg_neq]. }
    ...
    - (* ======= n > 0: save s6..s10, set up the loop registers ======= *)
      ...
      iApply (wp_bge_x0_fall_s_sconf γ Φ (mword_of_int (PW + 0x24)) (mword_of_int 196 : mword 13)
                Rs4 M1 (av - 14)%nat ltac:(nz) Hgf0 with "Hcg Hpc Hi24 [-]").
      iIntros "Hcg Hpc".
      assert (Hpp28 : add_vec_int (mword_of_int (PW + 0x24) : mword 64) 4 = mword_of_int (PW + 0x28)) by (apply bv_eq; vm_compute; reflexivity).
      ...
      iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (PW + 0x28)) (mword_of_int 6 : mword 6) Rs6
                M1 (av - 14)%nat u8 with "Hcg Hpc Hi28 G8 [-]").
      iIntros "Hcg Hpc G8".
      iEval (rewrite HspM1 Hb8 Hs6M1) in "G8".
      assert (Hpp2a : add_vec_int (mword_of_int (PW + 0x28) : mword 64) 2 = mword_of_int (PW + 0x2a)) by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hpp2a) in "Hpc".
      iPoseProof (pwi_2a with "Htext") as "Hi2a".
      iEval (rewrite -Hb9 -HspM1) in "G9".
      iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (PW + 0x2a)) (mword_of_int 5 : mword 6) Rs7
                M1 (av - 14)%nat u9 with "Hcg Hpc Hi2a G9 [-]").
      iIntros "Hcg Hpc G9".
      iEval (rewrite HspM1 Hb9 Hs7M1) in "G9".
      assert (Hpp2c : add_vec_int (mword_of_int (PW + 0x2a) : mword 64) 2 = mword_of_int (PW + 0x2c)) by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hpp2c) in "Hpc".
      iPoseProof (pwi_2c with "Htext") as "Hi2c".
      iEval (rewrite -Hb10 -HspM1) in "G10".
      iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (PW + 0x2c)) (mword_of_int 4 : mword 6) Rs8
                M1 (av - 14)%nat u10 with "Hcg Hpc Hi2c G10 [-]").
      iIntros "Hcg Hpc G10".
      iEval (rewrite HspM1 Hb10 Hs8M1) in "G10".
      assert (Hpp2e : add_vec_int (mword_of_int (PW + 0x2c) : mword 64) 2 = mword_of_int (PW + 0x2e)) by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hpp2e) in "Hpc".
      iPoseProof (pwi_2e with "Htext") as "Hi2e".
      iEval (rewrite -Hb11 -HspM1) in "G11".
      iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (PW + 0x2e)) (mword_of_int 3 : mword 6) Rs9
                M1 (av - 14)%nat u11 with "Hcg Hpc Hi2e G11 [-]").
      iIntros "Hcg Hpc G11".
      iEval (rewrite HspM1 Hb11 Hs9M1) in "G11".
      assert (Hpp30 : add_vec_int (mword_of_int (PW + 0x2e) : mword 64) 2 = mword_of_int (PW + 0x30)) by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hpp30) in "Hpc".
      iPoseProof (pwi_30 with "Htext") as "Hi30".
      iEval (rewrite -Hb12 -HspM1) in "G12".
      iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (PW + 0x30)) (mword_of_int 2 : mword 6) Rs10
                M1 (av - 14)%nat u12 with "Hcg Hpc Hi30 G12 [-]").
      iIntros "Hcg Hpc G12".
      iEval (rewrite HspM1 Hb12 Hs10M1) in "G12".
      assert (Hpp32 : add_vec_int (mword_of_int (PW + 0x30) : mword 64) 2 = mword_of_int (PW + 0x32)) by (apply bv_eq; vm_compute; reflexivity).
      ...
      iApply (wp_cli_s_sconf γ Φ (mword_of_int (PW + 0x32)) Rs2 (mword_of_int 0 : mword 6)
                (mword_of_int 0 : mword 64) M1 (av - 14)%nat ltac:(nz) ltac:(nz)
      ...
      assert (Hpp34 : add_vec_int (mword_of_int (PW + 0x32) : mword 64) 2 = mword_of_int (PW + 0x34)) by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hpp34) in "Hpc".
      (* +0x34 addi s8,s0,-97 *)
      iPoseProof (pwi_34 with "Htext") as "Hi34".
      iApply (wp_addi4_s_sconf γ Φ (mword_of_int (PW + 0x34)) Rs8 Rs0
                (mword_of_int 3999 : mword 12) C1 (av - 14)%nat ltac:(nz) ltac:(nz)
                with "Hcg Hpc Hi34 [-]").
      iIntros "Hcg Hpc".
      ...
      assert (Hpp38 : add_vec_int (mword_of_int (PW + 0x34) : mword 64) 4 = mword_of_int (PW + 0x38)) by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hpp38) in "Hpc".
      (* +0x38 c.li s7,1 *)
      iPoseProof (pwi_38 with "Htext") as "Hi38".
      iApply (wp_cli_s_sconf γ Φ (mword_of_int (PW + 0x38)) Rs7 (mword_of_int 1 : mword 6)
                (mword_of_int 1 : mword 64) C2 (av - 14)%nat ltac:(nz) ltac:(nz)
      ...
      assert (Hpp3a : add_vec_int (mword_of_int (PW + 0x38) : mword 64) 2 = mword_of_int (PW + 0x3a)) by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hpp3a) in "Hpc".
      (* +0x3a c.li s6,-1 *)
      iPoseProof (pwi_3a with "Htext") as "Hi3a".
      iApply (wp_cli_s_sconf γ Φ (mword_of_int (PW + 0x3a)) Rs6 (mword_of_int 63 : mword 6)
                (mword_of_int (-1) : mword 64) C3 (av - 14)%nat ltac:(nz) ltac:(nz)
      ...
      assert (Hpp3c : add_vec_int (mword_of_int (PW + 0x3a) : mword 64) 2 = mword_of_int (PW + 0x3c)) by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hpp3c) in "Hpc".
      (* +0x3c addi s10,s1,536 *)
      iPoseProof (pwi_3c with "Htext") as "Hi3c".
      iApply (wp_addi4_s_sconf γ Φ (mword_of_int (PW + 0x3c)) Rs10 Rs1
                (mword_of_int 536 : mword 12) C4 (av - 14)%nat ltac:(nz) ltac:(nz)
                with "Hcg Hpc Hi3c [-]").
      iIntros "Hcg Hpc".
      ...
      assert (Hpp40 : add_vec_int (mword_of_int (PW + 0x3c) : mword 64) 4 = mword_of_int (PW + 0x40)) by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hpp40) in "Hpc".
      (* +0x40 addi s9,s1,540 *)
      iPoseProof (pwi_40 with "Htext") as "Hi40".
      iApply (wp_addi4_s_sconf γ Φ (mword_of_int (PW + 0x40)) Rs9 Rs1
                (mword_of_int 540 : mword 12) C5 (av - 14)%nat ltac:(nz) ltac:(nz)
                with "Hcg Hpc Hi40 [-]").
      iIntros "Hcg Hpc".
      ...
      assert (Hpp44 : add_vec_int (mword_of_int (PW + 0x40) : mword 64) 4 = mword_of_int (PW + 0x44)) by (apply bv_eq; vm_compute; reflexivity).
      ...
      { unfold pw_loop_regs. split_and!.
        ...
        { rewrite /C6 upd_ne; [| reg_neq]. rewrite /C5 upd_ne; [| reg_neq].
          rewrite /C4 upd_ne; [| reg_neq]. rewrite /C3 upd_ne; [| reg_neq].
          rewrite /C2 upd_ne; [| reg_neq]. rewrite /C1 upd_ne; [exact HtpM1 | reg_neq]. }
        { rewrite /C6 upd_ne; [| reg_neq]. rewrite /C5 upd_ne; [| reg_neq].
          rewrite /C4 upd_ne; [| reg_neq]. rewrite /C3 upd_ne; [| reg_neq].
      ...
      (* ================================================================= *)
      iAssert (pw_loop γ γa γf Φ γs j γl γp w q m av eb C pid V n sp0 pi addr) with "[]" as "LOOP".
      { iLöb as "IH". rewrite /pw_loop.
        iIntros (i M Pc) "%Hi %Hext %Hregs HF7 HF5 HCH Hcg Hown Hpay Hlocked Hres Hpc Href Hpriv _ Hctx Hsched HEX".
        pose proof Hregs as Hregs2.
        destruct Hregs2 as (Hsp & Htpm & Hs0 & Hs1 & Hs2 & Hs3 & Hs4 & Hs5 &
                            Hs6 & Hs7 & Hs8 & Hs9 & Hs10 & Hs11).
        ...
        iApply (wp_lw_s_sconf γ Φ (mword_of_int (PW + 0x7e)) Ra5 Rs1 (mword_of_int 544 : mword 12)
                  M (av - 14)%nat ro ltac:(nz) ltac:(nz) with "Hcg Hpc Hi7e [Hro] [-]").
        ...
        assert (Hpp82 : add_vec_int (mword_of_int (PW + 0x7e) : mword 64) 4 = mword_of_int (PW + 0x82)) by (apply bv_eq; vm_compute; reflexivity).
        ...
        - (* ==== readopen == 0 : the -1 arm ==== *)
          ...
          assert (Halz : eq_vec (access_vec_dec (add_vec (mword_of_int (PW + 0x82) : mword 64)
                           (sign_extend' 64 (sign_extend' 13 (concat_vec (mword_of_int 226 : mword 8) ('b"0"))))) 0) ('b"0") = true)
            by (vm_compute; reflexivity).
          iApply (wp_cbeqz_taken_s_sconf γ Φ (mword_of_int (PW + 0x82)) (mword_of_int 226 : mword 8)
                    (Cregidx (mword_of_int 7)) Ra5 L1 (av - 14)%nat
                    ltac:(vm_compute; reflexivity) ltac:(nz) Hcz Halz with "Hcg Hpc Hi82 [-]").
          iNext. iIntros "Hcg Hpc".
          iAssert (▷ sched_vc γ Φ γs (a_cpu_ctx cid_word) (proc_addr j))%I with "[Hsched]" as "Hsched".
          { iNext. iExact "Hsched". }
          assert (Hj46 : add_vec (mword_of_int (PW + 0x82) : mword 64)
                           (sign_extend' 64 (sign_extend' 13 (concat_vec (mword_of_int 226 : mword 8) ('b"0"))))
                         = mword_of_int (PW + 0x46)) by (apply bv_eq; vm_compute; reflexivity).
          iEval (rewrite Hj46) in "Hpc".
          iDestruct "HEX" as "[_ MIN]".
          iApply ("MIN" $! L1 Pc with "[%] [%] HF7 HF5 HCH Hcg Hown Hpay Hlocked [Hnm Hnr Hnw Hro Hwo Hst0 Hst1 Hdat Hslack] Hpc Href Hpriv Hctx Hsched").
          + unfold pw_min_regs. split_and!.
            { rewrite (callee_saved_lookup HcsML1 csp_rs1 ltac:(vm_compute; reflexivity)). exact Hsp. }
            { rewrite (callee_saved_lookup HcsML1 (mword_of_int 4) ltac:(vm_compute; reflexivity)). exact Htpm. }
        ...
        - (* ==== readopen /= 0 : ask killed() ==== *)
          iApply (wp_cbeqz_fall_s_sconf γ Φ (mword_of_int (PW + 0x82)) (mword_of_int 226 : mword 8)
                    (Cregidx (mword_of_int 7)) Ra5 L1 (av - 14)%nat
                    ltac:(vm_compute; reflexivity) ltac:(nz)
                    ltac:(rewrite Ha5L1; exact Hroz) with "Hcg Hpc Hi82 [-]").
          iIntros "Hcg Hpc".
          assert (Hpp84 : add_vec_int (mword_of_int (PW + 0x82) : mword 64) 2 = mword_of_int (PW + 0x84)) by (apply bv_eq; vm_compute; reflexivity).
          iEval (rewrite Hpp84) in "Hpc".
          (* +0x84 c.mv a0,s3 *)
          iPoseProof (pwi_84 with "Htext") as "Hi84".
          iApply (wp_cmv_s_sconf γ Φ (mword_of_int (PW + 0x84)) Ra0 Rs3 L1 (av - 14)%nat
                    ltac:(nz) ltac:(nz) with "Hcg Hpc Hi84 [-]").
          iIntros "Hcg Hpc".
          ...
          assert (Hpp86 : add_vec_int (mword_of_int (PW + 0x84) : mword 64) 2 = mword_of_int (PW + 0x86)) by (apply bv_eq; vm_compute; reflexivity).
          iEval (rewrite Hpp86) in "Hpc".
          (* +0x86 jal killed *)
          iPoseProof (pwi_86 with "Htext") as "Hi86".
          iApply (wp_jal_s_sconf γ Φ (mword_of_int (PW + 0x86)) Rra (mword_of_int 2087966 : mword 21)
                    L2 (av - 14)%nat ltac:(nz) ltac:(nz) ltac:(vm_compute; reflexivity)
                    with "Hcg Hpc Hi86 [-]").
          iIntros "Hcg Hpc".
          set (L3 := <[Regidx Rra := regval_into_reg (add_vec_int (mword_of_int (PW + 0x86) : mword 64) 4)]> L2).
          change (<[Regidx Rra := regval_into_reg (add_vec_int (mword_of_int (PW + 0x86) : mword 64) 4)]> L2) with L3.
          assert (Hjkl : add_vec (mword_of_int (PW + 0x86) : mword 64) (sign_extend' 64 (mword_of_int 2087966 : mword 21))
          ...
          assert (HtpL3 : L3 !!! Regidx Rtp = cid_word).
          { rewrite /L3 upd_ne; [| reg_neq]. rewrite /L2 upd_ne; [| reg_neq].
            rewrite /L1 upd_ne; [exact Htpm | reg_neq]. }
          assert (Hlvl1 : (Z.of_nat 1%nat + 1 < 2 ^ 31)%Z) by (rewrite H31; lia).
          assert (Hav14 : (14 <= av - 14)%nat) by lia.
          iApply (Killed.wp_killed_sconf γ Φ γs j γlp L3 (av - 14)%nat 1%nat eb (proc_addr j) C
                    Ha0L3 HtpL3 Hj Hjlp Hlvl1 Hav14
                    with "Hcg Hown Htext Hpc Hpinv Hpanic [-]").
          iIntros (K0 kl) "%Hkfacts Hcg Hown Hpc".
          destruct Hkfacts as [Hkcs Hka0].
          assert (HraL3 : L3 !!! Regidx Rra = add_vec_int (mword_of_int (PW + 0x86) : mword 64) 4)
            by (rewrite /L3; apply upd_eq).
          iEval (rewrite HraL3) in "Hpc".
          assert (Hpp8a : ret_pc (add_vec_int (mword_of_int (PW + 0x86) : mword 64) 4)
                          = (mword_of_int (PW + 0x8a) : mword 64)) by (apply bv_eq; vm_compute; reflexivity).
          ...
          destruct HregsK0' as (KspF & KtpF & Ks0F & Ks1F & Ks2F & Ks3F & Ks4F & Ks5F &
          ...
          + (* ==== killed: the -1 arm ==== *)
            ...
            assert (Halz : eq_vec (access_vec_dec (add_vec (mword_of_int (PW + 0x8a) : mword 64)
                             (sign_extend' 64 (sign_extend' 13 (concat_vec (mword_of_int 222 : mword 8) ('b"0"))))) 0) ('b"0") = true)
              by (vm_compute; reflexivity).
            iApply (wp_cbnez_taken_s_sconf γ Φ (mword_of_int (PW + 0x8a)) (mword_of_int 222 : mword 8)
                      (Cregidx (mword_of_int 2)) Ra0 K0 (av - 14)%nat
                      ltac:(vm_compute; reflexivity) ltac:(nz) Hcnz Halz with "Hcg Hpc Hi8a [-]").
            iNext. iIntros "Hcg Hpc".
            iAssert (▷ sched_vc γ Φ γs (a_cpu_ctx cid_word) (proc_addr j))%I with "[Hsched]" as "Hsched".
            { iNext. iExact "Hsched". }
            assert (Hj46 : add_vec (mword_of_int (PW + 0x8a) : mword 64)
                             (sign_extend' 64 (sign_extend' 13 (concat_vec (mword_of_int 222 : mword 8) ('b"0"))))
                           = mword_of_int (PW + 0x46)) by (apply bv_eq; vm_compute; reflexivity).
            iEval (rewrite Hj46) in "Hpc".
            iDestruct "HEX" as "[_ MIN]".
            iApply ("MIN" $! K0 Pc with "[%] [%] HF7 HF5 HCH Hcg Hown Hpay Hlocked [Hnm Hnr Hnw Hro Hwo Hst0 Hst1 Hdat Hslack] Hpc Href Hpriv Hctx Hsched").
            * unfold pw_min_regs. split_and!; assumption.
            * exact Hext.
            * iApply (pw_res_intro γp pi nr nw ro wo vname bs Hcnt Hbslen
                        with "Hnm Hnr Hnw Hro Hwo Hst0 Hst1 Hdat Hslack").
          + (* ==== alive: read the counters ==== *)
            iApply (wp_cbnez_fall_s_sconf γ Φ (mword_of_int (PW + 0x8a)) (mword_of_int 222 : mword 8)
                      (Cregidx (mword_of_int 2)) Ra0 K0 (av - 14)%nat
                      ltac:(vm_compute; reflexivity) ltac:(nz)
                      ltac:(rewrite Hka0; exact Hklz) with "Hcg Hpc Hi8a [-]").
            iIntros "Hcg Hpc".
            assert (Hpp8c : add_vec_int (mword_of_int (PW + 0x8a) : mword 64) 2 = mword_of_int (PW + 0x8c)) by (apply bv_eq; vm_compute; reflexivity).
            ...
            iApply (wp_lw_s_sconf γ Φ (mword_of_int (PW + 0x8c)) Ra5 Rs1 (mword_of_int 536 : mword 12)
                      K0 (av - 14)%nat nr ltac:(nz) ltac:(nz) with "Hcg Hpc Hi8c [Hnr] [-]").
            { iEval (rewrite Hnraddr). iExact "Hnr". }
            iIntros "Hcg Hpc Hnr".
            ...
            assert (Hpp90 : add_vec_int (mword_of_int (PW + 0x8c) : mword 64) 4 = mword_of_int (PW + 0x90)) by (apply bv_eq; vm_compute; reflexivity).
            ...
            iApply (wp_lw_s_sconf γ Φ (mword_of_int (PW + 0x90)) Ra4 Rs1 (mword_of_int 540 : mword 12)
                      K1 (av - 14)%nat nw ltac:(nz) ltac:(nz) with "Hcg Hpc Hi90 [Hnw] [-]").
            { iEval (rewrite Hnwaddr). iExact "Hnw". }
            iIntros "Hcg Hpc Hnw".
            ...
            assert (Hpp94 : add_vec_int (mword_of_int (PW + 0x90) : mword 64) 4 = mword_of_int (PW + 0x94)) by (apply bv_eq; vm_compute; reflexivity).
            iEval (rewrite Hpp94) in "Hpc".
            (* +0x94 addiw a5,a5,512 *)
            iPoseProof (pwi_94 with "Htext") as "Hi94".
            iApply (wp_addiw_s_sconf γ Φ (mword_of_int (PW + 0x94)) Ra5 Ra5
                      (mword_of_int 512 : mword 12) K2 (av - 14)%nat ltac:(nz) ltac:(nz)
                      with "Hcg Hpc Hi94 [-]").
            iIntros "Hcg Hpc".
            ...
            assert (Hpp98 : add_vec_int (mword_of_int (PW + 0x94) : mword 64) 4 = mword_of_int (PW + 0x98)) by (apply bv_eq; vm_compute; reflexivity).
            ...
            destruct HregsK3' as (Jsp & Jtp & Js0 & Js1 & Js2 & Js3 & Js4 & Js5 &
            ...
            * (* ======== FULL: wakeup(&nread); sleep(&nwrite, &lock) ======== *)
              assert (Half : eq_vec (access_vec_dec (add_vec (mword_of_int (PW + 0x98) : mword 64)
                               (sign_extend' 64 (mword_of_int 8148 : mword 13))) 0) ('b"0") = true)
                by (vm_compute; reflexivity).
              iApply (wp_beq_taken_s_sconf γ Φ (mword_of_int (PW + 0x98)) (mword_of_int 8148 : mword 13)
                        Ra5 Ra4 K3 (av - 14)%nat ltac:(nz) ltac:(nz) Hfull Half
                        with "Hcg Hpc Hi98 [-]").
              iNext. iIntros "Hcg Hpc".
              iAssert (▷ sched_vc γ Φ γs (a_cpu_ctx cid_word) (proc_addr j))%I with "[Hsched]" as "Hsched".
              { iNext. iExact "Hsched". }
              assert (Hj6c : add_vec (mword_of_int (PW + 0x98) : mword 64)
                               (sign_extend' 64 (mword_of_int 8148 : mword 13))
                             = mword_of_int (PW + 0x6c)) by (apply bv_eq; vm_compute; reflexivity).
              ...
              iApply (wp_cmv_s_sconf γ Φ (mword_of_int (PW + 0x6c)) Ra0 Rs10 K3 (av - 14)%nat
                        ltac:(nz) ltac:(nz) with "Hcg Hpc Hi6c [-]").
              iIntros "Hcg Hpc".
              ...
              assert (Hpp6e : add_vec_int (mword_of_int (PW + 0x6c) : mword 64) 2 = mword_of_int (PW + 0x6e)) by (apply bv_eq; vm_compute; reflexivity).
              iEval (rewrite Hpp6e) in "Hpc".
              (* +0x6e jal wakeup *)
              iPoseProof (pwi_6e with "Htext") as "Hi6e".
              iApply (wp_jal_s_sconf γ Φ (mword_of_int (PW + 0x6e)) Rra (mword_of_int 2087494 : mword 21)
                        F1 (av - 14)%nat ltac:(nz) ltac:(nz) ltac:(vm_compute; reflexivity)
                        with "Hcg Hpc Hi6e [-]").
              iIntros "Hcg Hpc".
              set (F2 := <[Regidx Rra := regval_into_reg (add_vec_int (mword_of_int (PW + 0x6e) : mword 64) 4)]> F1).
              change (<[Regidx Rra := regval_into_reg (add_vec_int (mword_of_int (PW + 0x6e) : mword 64) 4)]> F1) with F2.
              assert (Hjwk : add_vec (mword_of_int (PW + 0x6e) : mword 64) (sign_extend' 64 (mword_of_int 2087494 : mword 21))
                             = mword_of_int KernelSyms.wakeup) by (apply bv_eq; vm_compute; reflexivity).
              iEval (rewrite Hjwk) in "Hpc".
              assert (HtpF2 : F2 !!! Regidx Rtp = cid_word).
              { rewrite /F2 upd_ne; [| reg_neq]. rewrite /F1 upd_ne; [| reg_neq]. exact Jtp. }
              assert (HwK : (18 <= av - 14)%nat) by lia.
              assert (HwdomF : forall r : regidx, r ∈ dom (rf_to_gmap F2)) by (intro r; apply rf_to_gmap_dom).
              assert (Hwa0f : mycpu_ret (F2 !!! Regidx Rtp) = mycpu_ret cid_word) by (rewrite HtpF2; reflexivity).
              assert (Hwnz : eq_vec (zero_reg : mword 64) (mycpu_ret (F2 !!! Regidx Rtp)) = false)
                by (rewrite HtpF2; exact Hcpune).
              assert (Hwlvl : (Z.of_nat 1%nat + 1 < 2 ^ 31)%Z) by (rewrite H31; lia).
              iApply (Wakeup.wp_wakeup_sconf γ Φ F2 γs (mycpu_ret cid_word) (proc_addr j) 1%nat (av - 14)%nat eb C
                        HwK HwdomF Hlen HtpF2 Hwa0f Hwnz Hwlvl
                        with "Hcg Hown Htext Hpc Hpanic Hpinv [-]").
              iIntros (Mw) "[%Hwcs %Hwdom] Hcg Hown Htext2 Hpc".
              assert (HraF2 : F2 !!! Regidx Rra = add_vec_int (mword_of_int (PW + 0x6e) : mword 64) 4)
                by (rewrite /F2; apply upd_eq).
              iEval (rewrite HraF2) in "Hpc".
              assert (Hpp72 : ret_pc (add_vec_int (mword_of_int (PW + 0x6e) : mword 64) 4)
                              = (mword_of_int (PW + 0x72) : mword 64)) by (apply bv_eq; vm_compute; reflexivity).
              ...
              destruct HregsMw' as (Wsp & Wtp & Ws0 & Ws1 & Ws2 & Ws3 & Ws4 & Ws5 &
                                    Ws6 & Ws7 & Ws8 & Ws9 & Ws10 & Ws11).
              (* +0x72 c.mv a1,s1 *)
              iPoseProof (pwi_72 with "Htext") as "Hi72".
              iApply (wp_cmv_s_sconf γ Φ (mword_of_int (PW + 0x72)) Ra1 Rs1 Mw (av - 14)%nat
                        ltac:(nz) ltac:(nz) with "Hcg Hpc Hi72 [-]").
              iIntros "Hcg Hpc".
              ...
              assert (Hpp74 : add_vec_int (mword_of_int (PW + 0x72) : mword 64) 2 = mword_of_int (PW + 0x74)) by (apply bv_eq; vm_compute; reflexivity).
              iEval (rewrite Hpp74) in "Hpc".
              (* +0x74 c.mv a0,s9 *)
              iPoseProof (pwi_74 with "Htext") as "Hi74".
              iApply (wp_cmv_s_sconf γ Φ (mword_of_int (PW + 0x74)) Ra0 Rs9 G1 (av - 14)%nat
                        ltac:(nz) ltac:(nz) with "Hcg Hpc Hi74 [-]").
              iIntros "Hcg Hpc".
              ...
              assert (Hpp76 : add_vec_int (mword_of_int (PW + 0x74) : mword 64) 2 = mword_of_int (PW + 0x76)) by (apply bv_eq; vm_compute; reflexivity).
              iEval (rewrite Hpp76) in "Hpc".
              (* +0x76 jal sleep *)
              iPoseProof (pwi_76 with "Htext") as "Hi76".
              iApply (wp_jal_s_sconf γ Φ (mword_of_int (PW + 0x76)) Rra (mword_of_int 2087410 : mword 21)
                        G2 (av - 14)%nat ltac:(nz) ltac:(nz) ltac:(vm_compute; reflexivity)
                        with "Hcg Hpc Hi76 [-]").
              iIntros "Hcg Hpc".
              set (G3 := <[Regidx Rra := regval_into_reg (add_vec_int (mword_of_int (PW + 0x76) : mword 64) 4)]> G2).
              change (<[Regidx Rra := regval_into_reg (add_vec_int (mword_of_int (PW + 0x76) : mword 64) 4)]> G2) with G3.
              assert (Hjsl : add_vec (mword_of_int (PW + 0x76) : mword 64) (sign_extend' 64 (mword_of_int 2087410 : mword 21))
              ...
              { rewrite Ha1G3.
                replace (sign_extend' 64 (mword_of_int 0 : mword 12)) with (mword_of_int 0 : mword 64)
                  by (apply bv_eq; vm_compute; reflexivity).
                apply kv_addv_zero. }
              assert (HtpG3 : G3 !!! Regidx Rtp = cid_word).
              { rewrite /G3 upd_ne; [| reg_neq]. rewrite /G2 upd_ne; [| reg_neq].
                rewrite /G1 upd_ne; [exact Wtp | reg_neq]. }
              assert (Hav22 : (22 <= av - 14)%nat) by lia.
              iApply (SleepGen.wp_sleep_gen_sconf γ Φ γs j γlp γl pi (pipe_res γp pi)
                        (pipe_ref γp w q) (pipe_dead γl γp) G3 (av - 14)%nat eb C
                        HtpG3 Hj Hjlp HlkaG3 Hav22
              ...
              iIntros (Ms) "%Hscs Hcg Hown Hpay Hpc Href Hlocked Hres Hctx Hsched".
              assert (HraG3 : G3 !!! Regidx Rra = add_vec_int (mword_of_int (PW + 0x76) : mword 64) 4)
                by (rewrite /G3; apply upd_eq).
              iEval (rewrite HraG3) in "Hpc".
              assert (Hpp7a : ret_pc (add_vec_int (mword_of_int (PW + 0x76) : mword 64) 4)
                              = (mword_of_int (PW + 0x7a) : mword 64)) by (apply bv_eq; vm_compute; reflexivity).
              ...
              iApply (pw_guard_step γ γa γf Φ γs j γl γp w q m av eb C pid V n sp0 pi addr i Ms Pc
                        Hn0 Hn31 ltac:(lia) Hext HregsMs
                        with "Htext HF7 HF5 HCH Hcg Hown Hpay Hlocked Hres Hpc Href Hpriv Henv Hctx Hsched HEX IH").
            * (* ======== NOT full: copy one byte in ======== *)
              ...
              iApply (wp_beq_fall_s_sconf γ Φ (mword_of_int (PW + 0x98)) (mword_of_int 8148 : mword 13)
                        Ra5 Ra4 K3 (av - 14)%nat ltac:(nz) ltac:(nz) Hfull
                        with "Hcg Hpc Hi98 [-]").
              iIntros "Hcg Hpc".
              assert (Hpp9c : add_vec_int (mword_of_int (PW + 0x98) : mword 64) 4 = mword_of_int (PW + 0x9c)) by (apply bv_eq; vm_compute; reflexivity).
              iEval (rewrite Hpp9c) in "Hpc".
              (* +0x9c c.mv a3,s7 *)
              iPoseProof (pwi_9c with "Htext") as "Hi9c".
              iApply (wp_cmv_s_sconf γ Φ (mword_of_int (PW + 0x9c)) Ra3 Rs7 K3 (av - 14)%nat
                        ltac:(nz) ltac:(nz) with "Hcg Hpc Hi9c [-]").
              iIntros "Hcg Hpc".
              ...
              assert (Hpp9e : add_vec_int (mword_of_int (PW + 0x9c) : mword 64) 2 = mword_of_int (PW + 0x9e)) by (apply bv_eq; vm_compute; reflexivity).
              iEval (rewrite Hpp9e) in "Hpc".
              (* +0x9e add a2,s2,s5 *)
              iPoseProof (pwi_9e with "Htext") as "Hi9e".
              iApply (wp_add_s_sconf γ Φ (mword_of_int (PW + 0x9e)) Ra2 Rs2 Rs5
                        (add_vec (N1 !!! Regidx Rs2) (N1 !!! Regidx Rs5)) N1 (av - 14)%nat
                        ltac:(nz) ltac:(nz) ltac:(reflexivity) with "Hcg Hpc Hi9e [-]").
              iIntros "Hcg Hpc".
              ...
              assert (Hppa2 : add_vec_int (mword_of_int (PW + 0x9e) : mword 64) 4 = mword_of_int (PW + 0xa2)) by (apply bv_eq; vm_compute; reflexivity).
              iEval (rewrite Hppa2) in "Hpc".
              (* +0xa2 c.mv a1,s8 *)
              iPoseProof (pwi_a2 with "Htext") as "Hia2".
              iApply (wp_cmv_s_sconf γ Φ (mword_of_int (PW + 0xa2)) Ra1 Rs8 N2 (av - 14)%nat
              ...
              assert (Hppa4 : add_vec_int (mword_of_int (PW + 0xa2) : mword 64) 2 = mword_of_int (PW + 0xa4)) by (apply bv_eq; vm_compute; reflexivity).
              ...
              iApply (wp_ld_s_sconf γ Φ (mword_of_int (PW + 0xa4)) Ra0 Rs3
                        (mword_of_int 80 : mword 12) N3 (av - 14)%nat (page_base (ud_root Pc))
                        ltac:(nz) ltac:(nz) with "Hcg Hpc Hia4 Hptc [-]").
              iIntros "Hcg Hpc Hptc".
              iEval (rewrite Hptaddr) in "Hptc".
              ...
              assert (Hppa8 : add_vec_int (mword_of_int (PW + 0xa4) : mword 64) 4 = mword_of_int (PW + 0xa8)) by (apply bv_eq; vm_compute; reflexivity).
              iEval (rewrite Hppa8) in "Hpc".
              (* +0xa8 jal copyin *)
              iPoseProof (pwi_a8 with "Htext") as "Hia8".
              iApply (wp_jal_s_sconf γ Φ (mword_of_int (PW + 0xa8)) Rra (mword_of_int 2085276 : mword 21)
                        N4 (av - 14)%nat ltac:(nz) ltac:(nz) ltac:(vm_compute; reflexivity)
                        with "Hcg Hpc Hia8 [-]").
              iIntros "Hcg Hpc".
              set (N5 := <[Regidx Rra := regval_into_reg (add_vec_int (mword_of_int (PW + 0xa8) : mword 64) 4)]> N4).
              change (<[Regidx Rra := regval_into_reg (add_vec_int (mword_of_int (PW + 0xa8) : mword 64) 4)]> N4) with N5.
              assert (Hjci : add_vec (mword_of_int (PW + 0xa8) : mword 64) (sign_extend' 64 (mword_of_int 2085276 : mword 21))
              ...
              assert (HtpN5 : N5 !!! Regidx Rtp = cid_word).
              { rewrite /N5 upd_ne; [| reg_neq]. rewrite /N4 upd_ne; [| reg_neq].
                rewrite /N3 upd_ne; [| reg_neq]. rewrite /N2 upd_ne; [| reg_neq].
                rewrite /N1 upd_ne; [exact Jtp | reg_neq]. }
              ...
              iApply (Copyin.wp_copyin_sconf γ γa Φ N5 Pc (pv_sz V) 1%nat (fun _ : nat => b0)
                        (av - 14)%nat 1%nat eb (proc_addr j) C (DfracOwn 1) (DfracOwn 1)
                        HK50 HtpN5 Ha0N5 Ha3N5 Hlen1 Hszb Hlvl1
                        with "Hcg Hown Htext Hpc Hszc Hptc Hpt Henv Hbuf [-]").
              iIntros (mr P' dst_new) "Hcg Hown Hpc Hszc Hptc Hpt Hbuf %Hcsr %Hextr %Hret".
              assert (HraN5 : N5 !!! Regidx Rra = add_vec_int (mword_of_int (PW + 0xa8) : mword 64) 4)
                by (rewrite /N5; apply upd_eq).
              iEval (rewrite HraN5) in "Hpc".
              assert (Hppac : ret_pc (add_vec_int (mword_of_int (PW + 0xa8) : mword 64) 4)
                              = (mword_of_int (PW + 0xac) : mword 64)) by (apply bv_eq; vm_compute; reflexivity).
              ...
              destruct Hregsmr' as (Rsp & Rtpf & Rs0f & Rs1f & Rs2f & Rs3f & Rs4f & Rs5f &
              ...
              ** (* ==== copyin failed: break, returning the current i ==== *)
                 assert (Half : eq_vec (access_vec_dec (add_vec (mword_of_int (PW + 0xac) : mword 64)
                                  (sign_extend' 64 (mword_of_int 64 : mword 13))) 0) ('b"0") = true)
                   by (vm_compute; reflexivity).
                 iApply (wp_beq_taken_s_sconf γ Φ (mword_of_int (PW + 0xac)) (mword_of_int 64 : mword 13)
                           Rs6 Ra0 mr (av - 14)%nat ltac:(nz) ltac:(nz) Hfail Half
                           with "Hcg Hpc Hiac [-]").
                 iNext. iIntros "Hcg Hpc".
                 iAssert (▷ sched_vc γ Φ γs (a_cpu_ctx cid_word) (proc_addr j))%I with "[Hsched]" as "Hsched".
                 { iNext. iExact "Hsched". }
                 assert (Hjec : add_vec (mword_of_int (PW + 0xac) : mword 64)
                                  (sign_extend' 64 (mword_of_int 64 : mword 13))
                                = mword_of_int (PW + 0xec)) by (apply bv_eq; vm_compute; reflexivity).
                 ...
                 assert (Eee : add_vec_int (mword_of_int (PW + 0xec) : mword 64) 2 = mword_of_int (PW + 0xee)) by (apply bv_eq; vm_compute; reflexivity).
                 assert (Ef0 : add_vec_int (mword_of_int (PW + 0xee) : mword 64) 2 = mword_of_int (PW + 0xf0)) by (apply bv_eq; vm_compute; reflexivity).
                 assert (Ef2 : add_vec_int (mword_of_int (PW + 0xf0) : mword 64) 2 = mword_of_int (PW + 0xf2)) by (apply bv_eq; vm_compute; reflexivity).
                 assert (Ef4 : add_vec_int (mword_of_int (PW + 0xf2) : mword 64) 2 = mword_of_int (PW + 0xf4)) by (apply bv_eq; vm_compute; reflexivity).
                 assert (Ef6 : add_vec_int (mword_of_int (PW + 0xf4) : mword 64) 2 = mword_of_int (PW + 0xf6)) by (apply bv_eq; vm_compute; reflexivity).
                 iApply (pw_restore5 γ Φ (mword_of_int (PW + 0xec)) (mword_of_int (PW + 0xee))
                           (mword_of_int (PW + 0xf0)) (mword_of_int (PW + 0xf2)) (mword_of_int (PW + 0xf4))
                           (mword_of_int (PW + 0xf6)) mr m (av - 14)%nat sp0
                 ...
                 assert (Htp' : M' !!! Regidx Rtp = cid_word).
                 { rewrite (Rrest (mword_of_int 4) ltac:(nz) ltac:(nz) ltac:(nz) ltac:(nz) ltac:(nz)). exact Rtpf. }
                 assert (Hs1' : M' !!! Regidx Rs1 = pi).
                 ...
                 iApply (wp_cj_s_sconf γ Φ (mword_of_int (PW + 0xf6))
                           (sign_extend' 21 (concat_vec (mword_of_int 2033 : mword 11) ('b"0")))
                           M' (av - 14)%nat ltac:(vm_compute; reflexivity) with "Hcg Hpc Hif6 [-]").
                 iNext. iIntros "Hcg Hpc".
                 iAssert (▷ sched_vc γ Φ γs (a_cpu_ctx cid_word) (proc_addr j))%I with "[Hsched]" as "Hsched".
                 { iNext. iExact "Hsched". }
                 assert (Hjd8 : add_vec (mword_of_int (PW + 0xf6) : mword 64)
                                  (sign_extend' 64 (sign_extend' 21 (concat_vec (mword_of_int 2033 : mword 11) ('b"0"))))
                                = mword_of_int (PW + 0xd8)) by (apply bv_eq; vm_compute; reflexivity).
                 iEval (rewrite Hjd8) in "Hpc".
                 iDestruct "HEX" as "[TAIL _]".
                 iApply ("TAIL" $! M' P' with "[%] [%] [%] [%] HF7 [HF5 HCH] Hcg Hown Hpay Hlocked [Hnm Hnr Hnw Hro Hwo Hst0 Hst1 Hdat Hslack] Hpc Href Hpriv Hctx Hsched").
                 --- unfold pw_base_regs. split_and!; assumption.
                 --- exact Hs1'.
                 --- rewrite Hs2'. right. exists i. split; [reflexivity | lia].
                 --- exact Hext'.
              ...
              ** (* ==== copyin succeeded: nwrite++, store the byte, i++ ==== *)
                 iApply (wp_beq_fall_s_sconf γ Φ (mword_of_int (PW + 0xac)) (mword_of_int 64 : mword 13)
                           Rs6 Ra0 mr (av - 14)%nat ltac:(nz) ltac:(nz) Hfail
                           with "Hcg Hpc Hiac [-]").
                 iIntros "Hcg Hpc".
                 assert (Hppb0 : add_vec_int (mword_of_int (PW + 0xac) : mword 64) 4 = mword_of_int (PW + 0xb0)) by (apply bv_eq; vm_compute; reflexivity).
                 ...
                 iApply (wp_lw_s_sconf γ Φ (mword_of_int (PW + 0xb0)) Ra5 Rs1 (mword_of_int 540 : mword 12)
                           mr (av - 14)%nat nw ltac:(nz) ltac:(nz) with "Hcg Hpc Hib0 [Hnw] [-]").
                 { iEval (rewrite Hnwaddr2). iExact "Hnw". }
                 iIntros "Hcg Hpc Hnw".
                 ...
                 assert (Hppb4 : add_vec_int (mword_of_int (PW + 0xb0) : mword 64) 4 = mword_of_int (PW + 0xb4)) by (apply bv_eq; vm_compute; reflexivity).
                 ...
                 iApply (wp_addiw_s_sconf γ Φ (mword_of_int (PW + 0xb4)) Ra4 Ra5
                           (mword_of_int 1 : mword 12) P1 (av - 14)%nat ltac:(nz) ltac:(nz)
                           with "Hcg Hpc Hib4 [-]").
                 iIntros "Hcg Hpc".
                 ...
                 assert (Hppb8 : add_vec_int (mword_of_int (PW + 0xb4) : mword 64) 4 = mword_of_int (PW + 0xb8)) by (apply bv_eq; vm_compute; reflexivity).
                 ...
                 iApply (wp_sw_s_sconf γ Φ (mword_of_int (PW + 0xb8)) Ra4 Rs1
                           (mword_of_int 540 : mword 12) P2 (av - 14)%nat nw
                           with "Hcg Hpc Hib8 Hnw [-]").
                 iIntros "Hcg Hpc Hnw".
                 iEval (rewrite Hstore Hnwaddr3) in "Hnw".
                 assert (Hppbc : add_vec_int (mword_of_int (PW + 0xb8) : mword 64) 4 = mword_of_int (PW + 0xbc)) by (apply bv_eq; vm_compute; reflexivity).
                 ...
                 iApply (wp_andi_s_sconf γ Φ (mword_of_int (PW + 0xbc)) Ra5 Ra5
                           (mword_of_int 511 : mword 12)
                           (and_vec (sign_extend' 64 nw : mword 64) (sign_extend' 64 (mword_of_int 511 : mword 12)))
                           P2 (av - 14)%nat ltac:(nz) ltac:(nz) ltac:(rewrite Ha5P2; reflexivity)
                           with "Hcg Hpc Hibc [-]").
                 iIntros "Hcg Hpc".
                 ...
                 assert (Hppc0 : add_vec_int (mword_of_int (PW + 0xbc) : mword 64) 4 = mword_of_int (PW + 0xc0)) by (apply bv_eq; vm_compute; reflexivity).
                 iEval (rewrite Hppc0) in "Hpc".
                 (* +0xc0 c.add a5,a5,s1 *)
                 iPoseProof (pwi_c0 with "Htext") as "Hic0".
                 iApply (wp_cadd_s_sconf γ Φ (mword_of_int (PW + 0xc0)) Ra5 Rs1 P3 (av - 14)%nat
                           ltac:(nz) ltac:(nz) with "Hcg Hpc Hic0 [-]").
                 iIntros "Hcg Hpc".
                 set (P4 := <[Regidx Ra5 := regval_into_reg
                     (add_vec (P3 !!! Regidx Ra5) (P3 !!! Regidx Rs1))]> P3).
                 change (<[Regidx Ra5 := regval_into_reg
                     (add_vec (P3 !!! Regidx Ra5) (P3 !!! Regidx Rs1))]> P3) with P4.
                 assert (Hppc2 : add_vec_int (mword_of_int (PW + 0xc0) : mword 64) 2 = mword_of_int (PW + 0xc2)) by (apply bv_eq; vm_compute; reflexivity).
                 ...
                 iApply (wp_lbu_s_sconf γ Φ (mword_of_int (PW + 0xc2)) Ra4 Rs0
                           (mword_of_int 3999 : mword 12) P4 (av - 14)%nat ((dst_new 0%nat) : mword 8)
                           ltac:(nz) ltac:(nz) with "Hcg Hpc Hic2 Hch [-]").
                 iIntros "Hcg Hpc Hch".
                 iEval (rewrite Hchaddr) in "Hch".
                 ...
                 assert (Hppc6 : add_vec_int (mword_of_int (PW + 0xc2) : mword 64) 4 = mword_of_int (PW + 0xc6)) by (apply bv_eq; vm_compute; reflexivity).
                 ...
                 iApply (wp_sb_s_sconf γ Φ (mword_of_int (PW + 0xc6)) Ra4 Ra5
                           (mword_of_int 24 : mword 12) P5 (av - 14)%nat b1
                 ...
                 assert (Hppca : add_vec_int (mword_of_int (PW + 0xc6) : mword 64) 4 = mword_of_int (PW + 0xca)) by (apply bv_eq; vm_compute; reflexivity).
                 iEval (rewrite Hppca) in "Hpc".
                 (* +0xca c.addiw s2,s2,1 *)
                 iPoseProof (pwi_ca with "Htext") as "Hica".
                 iApply (wp_caddiw_s_sconf γ Φ (mword_of_int (PW + 0xca)) Rs2 (mword_of_int 1 : mword 6)
                           P5 (av - 14)%nat ltac:(nz) ltac:(nz) with "Hcg Hpc Hica [-]").
                 iIntros "Hcg Hpc".
                 ...
                 assert (Hppcc : add_vec_int (mword_of_int (PW + 0xca) : mword 64) 2 = mword_of_int (PW + 0xcc)) by (apply bv_eq; vm_compute; reflexivity).
                 ...
                 iApply (wp_cj_s_sconf γ Φ (mword_of_int (PW + 0xcc))
                           (sign_extend' 21 (concat_vec (mword_of_int 2007 : mword 11) ('b"0")))
                           P6 (av - 14)%nat ltac:(vm_compute; reflexivity) with "Hcg Hpc Hicc [-]").
                 iNext. iIntros "Hcg Hpc".
                 iAssert (▷ sched_vc γ Φ γs (a_cpu_ctx cid_word) (proc_addr j))%I with "[Hsched]" as "Hsched".
                 { iNext. iExact "Hsched". }
                 assert (Hj7a : add_vec (mword_of_int (PW + 0xcc) : mword 64)
                                  (sign_extend' 64 (sign_extend' 21 (concat_vec (mword_of_int 2007 : mword 11) ('b"0"))))
                                = mword_of_int (PW + 0x7a)) by (apply bv_eq; vm_compute; reflexivity).
                 iEval (rewrite Hj7a) in "Hpc".
                 iApply (pw_guard_step γ γa γf Φ γs j γl γp w q m av eb C pid V n sp0 pi addr
                           (i + 1)%Z P6 P' Hn0 Hn31 ltac:(lia) Hext' HregsP6
                           with "Htext HF7 HF5 HCH Hcg Hown Hpay Hlocked Hres Hpc Href Hpriv Henv Hctx Hsched HEX IH"). }
      (* ---- +0x44 c.j -> the loop body, with i = 0 ---- *)
      iPoseProof (pwi_44 with "Htext") as "Hi44".
      iApply (wp_cj_s_sconf γ Φ (mword_of_int (PW + 0x44))
                (sign_extend' 21 (concat_vec (mword_of_int 29 : mword 11) ('b"0")))
                C6 (av - 14)%nat ltac:(vm_compute; reflexivity) with "Hcg Hpc Hi44 [-]").
      iNext. iIntros "Hcg Hpc".
      iAssert (▷ sched_vc γ Φ γs (a_cpu_ctx cid_word) pj)%I with "[Hsched]" as "Hsched".
      { iNext. iExact "Hsched". }
      assert (Hj7e : add_vec (mword_of_int (PW + 0x44) : mword 64)
                       (sign_extend' 64 (sign_extend' 21 (concat_vec (mword_of_int 29 : mword 11) ('b"0"))))
                     = mword_of_int (PW + 0x7e)) by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hj7e) in "Hpc".
      iApply ("LOOP" $! 0%Z C6 (pv_upt V) with "[%] [%] [%] HF7 HF5 HCH Hcg Hown Hpay Hlocked Hres Hpc Href [Hpriv] Henv Hctx Hsched EXITS").
      + lia.
      + apply uptd_ext_refl.
      + exact HregsC6.
      + rewrite pw_upd_upt_id. iExact "Hpriv".
