...
Section PwPieces.
  ...
  Proof.
    ...
    iDestruct (slot_bytes_own (KTR := kt) with "H13") as "[%Hal Hb13]".
  ...
  Proof.
    ...
    iDestruct (bytes_own_slot (KTR := kt) (pa_stk sp0 13%nat) Hal with "Hb13") as (w13) "H13".
    rewrite (stack_own_slots (KTR := kt)). cbn [seq].
  ...
  Definition pw_epi `{GEN : GenId} (CID0 : CPU) (γf : gname)  (γs : list gname) (j : nat)
    ...
    (wp_next (CID0 := CID0) true (proc_addr j) (fun (CID : CpuId) =>
     ∀ (M : regfile) (P' : uptd),
       ...
       ⌜uptd_ext (pv_upt V) P'⌝ -∗
       pw_frame7 (kt := kt) m sp0 -∗
       stack_own (KTR := kt) (pa_stk sp0 7%nat) 7%nat -∗
       sie_cap_gpr kt M (av - 14)%nat true (proc_addr j) -∗
  ...
  Definition pw_tail `{GEN : GenId} (CID0 : CPU) (γf : gname)  (γs : list gname) (j : nat)
    ...
    (wp_next (CID0 := CID0) true (proc_addr j) (fun (CID : CpuId) =>
     ∀ (M : regfile) (P' : uptd),
       ...
       pw_frame7 (kt := kt) m sp0 -∗
       stack_own (KTR := kt) (pa_stk sp0 7%nat) 7%nat -∗
       sie_cap_gpr kt M (trap_res true + (av - 14))%nat false (proc_addr j) -∗
       cpu_own 1%nat eb (proc_addr j) false ({["pipe"]} ∪ lks) -∗
       arm_pay kt 0%nat eb (proc_addr j) -∗
       locked γl cpu_id -∗
       pipe_res γp pi -∗
...
Section PwRestore.
  ...
  Lemma pw_restore5 (pme : mword 64) (b : bool)
    ...
    sie_cap_gpr kt M K b pme -∗ pc_is p0 -∗
    ...
    wp_next b pme (fun (CID : CpuId) =>
      ∀ M' : regfile, ⌜ pw_restored m M M' ⌝ -∗
        sie_cap_gpr kt M' K b pme -∗ pc_is p5 -∗ pw_frame5 (kt := kt) m sp0 -∗
...
Section PwGuard.
  ...
  Lemma pw_stack7_of (m : regfile) (sp0 : mword 64) :
    pw_frame5 (kt := kt) m sp0 -∗ pw_chslot (kt := kt) sp0 -∗ stack_own (KTR := kt) (pa_stk sp0 7%nat) 7%nat.
  Proof.
  ...
  Lemma pw_guard_step (CID0 : CPU) (γa γf : gname) (γs : list gname) (j : nat)
    ...
    pw_loop_regs m M (pa_stk sp0 14%nat) sp0 pi (proc_addr j) addr n i ->
    kernel_text -∗
    pw_frame7 (kt := kt) m sp0 -∗ pw_frame5 (kt := kt) m sp0 -∗ pw_chslot (kt := kt) sp0 -∗
    sie_cap_gpr kt M (trap_res true + (av - 14))%nat false (proc_addr j) -∗
    cpu_own 1%nat eb (proc_addr j) false ({["pipe"]} ∪ lks) -∗
    arm_pay kt 0%nat eb (proc_addr j) -∗
    locked γl cpu_id -∗
    ...
    proc_priv_core (proc_addr j) pid (upd_upt V Pc) -∗
    kalloc_env γa None -∗
    pw_exits (kt := kt) CID0 γf γs j γl γp w q m av eb lks pid V n sp0 pi -∗
    pw_loop (kt := kt) CID0 γa γf γs j γl γp w q m av eb lks pid V n sp0 pi addr -∗
    WP (Loop : expr riscv_lang).
  Proof.
    ...
    - (* ==== i >= n : the copy is done, restore s6..s10 and take the tail ==== *)
      ...
      iPoseProof (pwi_fe with "Htext") as "Hj0".
      iPoseProof (pwi_100 with "Htext") as "Hj1".
      iPoseProof (pwi_102 with "Htext") as "Hj2".
      iPoseProof (pwi_104 with "Htext") as "Hj3".
      iPoseProof (pwi_106 with "Htext") as "Hj4".
...
Section ProofPipewrite.
  ...
  Proof.
    ...
    (* ================================================================= *)
    iAssert (pw_epi (kt := kt) CID γf γs j γp w q m av true pid V n sp0) with "[Hcont]" as "EPI".
    { rewrite /pw_epi. iIntros (CIDep Hsep M P') "%Hbr %Hrw %Hext (Hc1 & Hc2 & Hc3 & Hc4 & Hc5 & Hc6 & Hc7) Hhi Hcg Hown Hpc Href Hpriv".
      ...
      iAssert (stack_own (KTR := kt) sp0 14%nat) with "[Hc1 Hc2 Hc3 Hc4 Hc5 Hc6 Hc7 Hhi]" as "Hframe".
      { iApply (stack_own_split_2 (KTR := kt) sp0 7%nat 14%nat ltac:(lia)). iSplitR "Hhi"; [| iExact "Hhi"].
        rewrite (stack_own_slots (KTR := kt)). cbn [seq].
    ...
    (* ================================================================= *)
    iAssert (pw_exits (kt := kt) CID γf γs j γl γp w q m av true lks pid V n sp0 pi) with "[EPI]" as "EXITS".
    { rewrite /pw_exits. iSplit.
      - (* ---------------- +0x108: wakeup(&pi->nread); release ---------------- *)
        ...
        iPoseProof (pwi_108 with "Htext") as "Hi108".
        ...
        iPoseProof (pwi_10c with "Htext") as "Hi10c".
        iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.pipewrite + 0x10c)) Rra (mword_of_int 2087212 : mword 21)
        ...
        assert (Hjwk : add_vec (mword_of_int (KernelSyms.pipewrite + 0x10c) : mword 64) (sign_extend' 64 (mword_of_int 2087212 : mword 21))
        ...
        iApply (Wakeup.wp_wakeup_sconf kt T2 γs (proc_addr j) 1%nat (trap_res true + (av - 14))%nat true false
                  ({["pipe"]} ∪ lks)
        ...
        (* +0x110 c.mv a0,s1 *)
        iPoseProof (pwi_110 with "Htext") as "Hi110".
        ...
        (* +0x112 jal release *)
        iPoseProof (pwi_112 with "Htext") as "Hi112".
        iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.pipewrite + 0x112)) Rra (mword_of_int 2082328 : mword 21)
        ...
        assert (Hjrl : add_vec (mword_of_int (KernelSyms.pipewrite + 0x112) : mword 64) (sign_extend' 64 (mword_of_int 2082328 : mword 21))
        ...
        iApply (ReleaseGen.wp_release_gen_sconf kt γl pi "pipe" (pipe_res γp pi) (pipe_dead γl γp) emp%I
        ...
        (* the pipe lock's release left [({[rank "pipe"]} ∪ lks) ∖ {[rank
           "pipe"]}]; [lks = ∅] at depth 0 makes that the empty set [pw_epi (kt := kt)]'s
           fixed [∅] binder names. *)
      ...
      - (* ---------------- +0x46: release; i := -1; reload s6..s10 ---------------- *)
        ...
        iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.pipewrite + 0x48)) Rra (mword_of_int 2082530 : mword 21)
        ...
        assert (Hjrl : add_vec (mword_of_int (KernelSyms.pipewrite + 0x48) : mword 64) (sign_extend' 64 (mword_of_int 2082530 : mword 21))
                       = mword_of_int KernelSyms.release) by (apply bv_eq; vm_compute; reflexivity).
        iEval (rewrite Hjrl) in "Hpc".
    ...
    iAssert (stack_own (KTR := kt) sp0 7%nat ∗ stack_own (KTR := kt) (pa_stk sp0 7%nat) 7%nat)%I with "[Hframe]" as "[Hlo Hhi]".
    { iApply (stack_own_split_1 (KTR := kt) sp0 7%nat 14%nat ltac:(lia)). iExact "Hframe". }
    iEval (rewrite (stack_own_slots (KTR := kt)); cbn [seq]) in "Hlo".
    ...
    iAssert (pw_frame7 (kt := kt) m sp0) with "[Hc1 Hc2 Hc3 Hc4 Hc5 Hc6 Hc7]" as "HF7".
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.pipewrite + 0x18)) Rra (mword_of_int 2085802 : mword 21)
    ...
    assert (Hjmp : add_vec (mword_of_int (KernelSyms.pipewrite + 0x18) : mword 64) (sign_extend' 64 (mword_of_int 2085802 : mword 21))
    ...
    iDestruct (cpu_own_transport CID CIDp30 0 true pj true ltac:(wp_next_chain)
                 with "Hown") as "Hown".
    iApply (Myproc.wp_myproc_sconf kt A5 (av - 14)%nat 0%nat true pj true _
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.pipewrite + 0x20)) Rra (mword_of_int 2082434 : mword 21)
    ...
    assert (Hjaq : add_vec (mword_of_int (KernelSyms.pipewrite + 0x20) : mword 64) (sign_extend' 64 (mword_of_int 2082434 : mword 21))
    ...
    iDestruct (cpu_own_transport CIDmp CIDp33 0 true pj true ltac:(wp_next_chain)
                 with "Hown") as "Hown".
    iApply (AcquireGen.wp_acquire_gen_sconf kt γl "pipe" (pipe_res γp pi) (pipe_ref γp w q)
    ...
    - (* ======= n <= 0: i := 0 and go straight to the +0x108 tail ======= *)
      ...
      (* +0x118 c.li s2,0 *)
      iPoseProof (pwi_118 with "Htext") as "Hi118".
      iApply (wp_cli_s_sconf (mword_of_int (KernelSyms.pipewrite + 0x118)) Rs2 (mword_of_int 0 : mword 6)
                ...
                ltac:(apply bv_eq; vm_compute; reflexivity) with "Hcg Hpc Hi118").
      iApply wp_next_off_intro. iIntros "Hcg Hpc". rgall.
      ...
      (* +0x11a c.j -> the tail *)
      iPoseProof (pwi_11a with "Htext") as "Hi11a".
      iApply (wp_cj_s_sconf (mword_of_int (KernelSyms.pipewrite + 0x11a))
                (sign_extend' 21 (concat_vec (mword_of_int 2039 : mword 11) ('b"0")))
                Z1 (trap_res true + (av - 14))%nat false ltac:(vm_compute; reflexivity) with "Hcg Hpc Hi11a").
    ...
    - (* ======= n > 0: save s6..s10, set up the loop registers ======= *)
      ...
      iAssert (pw_frame5 (kt := kt) m sp0) with "[G8 G9 G10 G11 G12]" as "HF5".
      ...
      (* ================================================================= *)
      iAssert (pw_loop (kt := kt) CID γa γf γs j γl γp w q m av true lks pid V n sp0 pi addr) with "[]" as "LOOP".
      { iLöb as "IH". rewrite /pw_loop.
        ...
        iApply (wp_lw_s_sconf (kt := kt) (ktd := KT0) (mword_of_int (KernelSyms.pipewrite + 0x8c)) Ra5 Rs1 (mword_of_int 544 : mword 12)
        ...
        - (* ==== readopen /= 0 : ask killed() ==== *)
          ...
          iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.pipewrite + 0x94)) Rra (mword_of_int 2087824 : mword 21)
          ...
          assert (Hjkl : add_vec (mword_of_int (KernelSyms.pipewrite + 0x94) : mword 64) (sign_extend' 64 (mword_of_int 2087824 : mword 21))
          ...
          iApply (Killed.wp_killed_sconf kt γs j γlp L3 (trap_res true + (av - 14))%nat 1%nat true (proc_addr j) false
                    ({["pipe"]} ∪ lks)
          ...
          + (* ==== alive: read the counters ==== *)
            ...
            iApply (wp_lw_s_sconf (kt := kt) (ktd := KT0) (mword_of_int (KernelSyms.pipewrite + 0x9a)) Ra5 Rs1 (mword_of_int 536 : mword 12)
            ...
            iApply (wp_lw_s_sconf (kt := kt) (ktd := KT0) (mword_of_int (KernelSyms.pipewrite + 0x9e)) Ra4 Rs1 (mword_of_int 540 : mword 12)
            ...
            * (* ======== FULL: wakeup(&nread); sleep(&nwrite, &lock) ======== *)
              ...
              iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.pipewrite + 0x6e)) Rra (mword_of_int 2087370 : mword 21)
              ...
              assert (Hjwk : add_vec (mword_of_int (KernelSyms.pipewrite + 0x6e) : mword 64) (sign_extend' 64 (mword_of_int 2087370 : mword 21))
                             = mword_of_int KernelSyms.wakeup) by (apply bv_eq; vm_compute; reflexivity).
              iEval (rewrite Hjwk) in "Hpc".
              assert (HwK : (18 <= trap_res true + (av - 14))%nat) by lia.
              assert (HwdomF : forall r : regidx, r ∈ dom (rf_to_gmap F2)) by (intro r; apply rf_to_gmap_dom).
              assert (Hwlvl : (Z.of_nat 1%nat + 1 < 2 ^ 31)%Z) by (rewrite H31; lia).
              iApply (Wakeup.wp_wakeup_sconf kt F2 γs (proc_addr j) 1%nat (trap_res true + (av - 14))%nat true false
                        ({["pipe"]} ∪ lks)
              ...
              iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.pipewrite + 0x74)) Rra (mword_of_int 2087256 : mword 21)
              ...
              assert (Hjsp : add_vec (mword_of_int (KernelSyms.pipewrite + 0x74) : mword 64) (sign_extend' 64 (mword_of_int 2087256 : mword 21))
              ...
              { rewrite /G2 /G1.
                apply callee_saved_insert_r; [vm_compute; reflexivity|].
                apply callee_saved_insert_r; [vm_compute; reflexivity|]. apply callee_saved_refl. }
              iApply (SleepPrepare.wp_sleep_prepare_sconf kt γs j γlp G2
              ...
              iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.pipewrite + 0x7a)) Rra (mword_of_int 2082480 : mword 21)
              ...
              assert (Hjrl0 : add_vec (mword_of_int (KernelSyms.pipewrite + 0x7a) : mword 64) (sign_extend' 64 (mword_of_int 2082480 : mword 21))
                              = mword_of_int KernelSyms.release) by (apply bv_eq; vm_compute; reflexivity).
              ...
              iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.pipewrite + 0x7e)) Rra (mword_of_int 2087306 : mword 21)
              ...
              assert (Hjsl : add_vec (mword_of_int (KernelSyms.pipewrite + 0x7e) : mword 64) (sign_extend' 64 (mword_of_int 2087306 : mword 21))
              ...
              iApply (Sleep.wp_sleep_sconf kt γs j γlp G5 (av - 14)%nat true lks
              ...
              iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.pipewrite + 0x84)) Rra (mword_of_int 2082334 : mword 21)
              ...
              assert (Hjaq2 : add_vec (mword_of_int (KernelSyms.pipewrite + 0x84) : mword 64) (sign_extend' 64 (mword_of_int 2082334 : mword 21))
                              = mword_of_int KernelSyms.acquire) by (apply bv_eq; vm_compute; reflexivity).
            ...
            * (* ======== NOT full: copy one byte in ======== *)
              ...
              iApply (wp_ld_s_sconf (kt := kt) (ktd := KT0) (mword_of_int (KernelSyms.pipewrite + 0xb2)) Ra1 Rs3
                        (mword_of_int 72 : mword 12) N3 (trap_res true + (av - 14))%nat (pv_sz V) false
                        ltac:(nz) ltac:(rdok) with "Hcg Hpc Hib2 Hszc").
              ...
              iApply (wp_ld_s_sconf (kt := kt) (ktd := KT0) (mword_of_int (KernelSyms.pipewrite + 0xb6)) Ra0 Rs3
                        (mword_of_int 80 : mword 12) N3b (trap_res true + (av - 14))%nat (page_base (ud_root Pc)) false
                        ltac:(nz) ltac:(rdok) with "Hcg Hpc Hib6 Hptc").
              ...
              iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.pipewrite + 0xba)) Rra (mword_of_int 2084872 : mword 21)
              ...
              assert (Hjci : add_vec (mword_of_int (KernelSyms.pipewrite + 0xba) : mword 64) (sign_extend' 64 (mword_of_int 2084872 : mword 21))
              ...
              iDestruct (bytes_own_acc (KTR := kt) (DfracOwn 1) (pa_stk sp0 13%nat) 8%nat 7%nat ltac:(lia) with "Hb13")
                as "[Hchx Hchback]".
              iDestruct "Hchx" as (b0) "Hch".
              iAssert ([∗ list] k ∈ seq 0 1, (pa_add (N5 !!! Regidx Ra2) k) ↦ₘ ((fun _ : nat => b0) k))%I
                with "[Hch]" as "Hbuf".
              { cbn [seq]. iSplitL "Hch"; [| done]. iEval (rewrite Ha2N5 pa_add_0). iExact "Hch". }
              iApply (Copyin.wp_copyin_sconf kt KT0 γa N5 Pc (pv_sz V) 1%nat (fun _ : nat => b0)
              ...
              ** (* ==== copyin failed: break.  SINCE xv6 `76821fc` this arm
                 ...
                 { (* ---- i == 0: beqz taken -> +0xf0, i := a0 = -1 ---- *)
                   iApply (wp_beqz_x0_taken_s_sconf (mword_of_int (KernelSyms.pipewrite + 0xe0)) (mword_of_int 16 : mword 13)
                             ...
                             ltac:(vm_compute; reflexivity) with "Hcg Hpc Hie0").
                   iApply bi.later_intro. iApply wp_next_off_intro. iIntros "Hcg Hpc". rgall.
                   ...
                   iApply (wp_cmv_s_sconf (mword_of_int (KernelSyms.pipewrite + 0xf0)) Rs2 Ra0 mr (trap_res true + (av - 14))%nat false
                             ltac:(nz) ltac:(rdok) with "Hcg Hpc Hif0").
                   iApply wp_next_off_intro. iIntros "Hcg Hpc". rgall.
                   ...
                   iPoseProof (pwi_f2 with "Htext") as "Hj0".
                   iPoseProof (pwi_f4 with "Htext") as "Hj1".
                   iPoseProof (pwi_f6 with "Htext") as "Hj2".
                   iPoseProof (pwi_f8 with "Htext") as "Hj3".
                   iPoseProof (pwi_fa with "Htext") as "Hj4".
                   ...
                   iPoseProof (pwi_fc with "Htext") as "Hifc".
                 ...
                 iPoseProof (pwi_e4 with "Htext") as "Hj0".
                 iPoseProof (pwi_e6 with "Htext") as "Hj1".
                 iPoseProof (pwi_e8 with "Htext") as "Hj2".
                 iPoseProof (pwi_ea with "Htext") as "Hj3".
                 iPoseProof (pwi_ec with "Htext") as "Hj4".
              ...
              ** (* ==== copyin succeeded: nwrite++, store the byte, i++ ==== *)
                 ...
                 iApply (wp_lbu_s_sconf (mword_of_int (KernelSyms.pipewrite + 0xd4)) Ra4 Rs0
                           ...
                           ltac:(nz) ltac:(rdok) with "Hcg Hpc Hid4 Hch").
                 ...
                 iApply (wp_caddiw_s_sconf (mword_of_int (KernelSyms.pipewrite + 0xdc)) Rs2 (mword_of_int 1 : mword 6)
                           P5 (trap_res true + (av - 14))%nat false ltac:(nz) ltac:(rdok) with "Hcg Hpc Hidc").
                 iApply wp_next_off_intro. iIntros "Hcg Hpc". rgall.
