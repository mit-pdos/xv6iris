...
Section ProofBpin.
  ...
  Proof.
    ...
    iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.bpin + 0x10)) Ra0 Ra0 (mword_of_int 0x504 : mword 12)
              R4 (K - 4)%nat b ltac:(vm_compute; discriminate) ltac:(rdok)
              with "Hcg Hpc Hi10").
    iIntros (CID8 Hs8) "Hcg Hpc".
    set (R5 := <[Regidx Ra0 := regval_into_reg
                  (add_vec (R4 !!! Regidx Ra0) (sign_extend' 64 (mword_of_int 1284 : mword 12)))]> R4).
    ...
    iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.bpin + 0x22)) Ra0 Ra0 (mword_of_int 0x4f2 : mword 12)
              D3 (trap_res b + (K - 4))%nat false ltac:(vm_compute; discriminate) ltac:(rdok)
              with "Hcg Hpc Hi22").
    iApply wp_next_off_intro.
    iIntros "Hcg Hpc".
    set (D4 := <[Regidx Ra0 := regval_into_reg
                  (add_vec (D3 !!! Regidx Ra0) (sign_extend' 64 (mword_of_int 1266 : mword 12)))]> D3).
