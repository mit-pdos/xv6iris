...
Section VdrwdLeaves.
  ...
  Proof.
    ...
    (* the class, consumed at [rs1] -- see [IntrDefs.SrcOk] and the note on
       the pre-A6.124 form of this leaf. *)
    ...
    iIntros "Hcg Hpc Hinstr #Hdinv #Hgeom Hpub #Hlb0 Havh Hcont".
    ...
    iApply (wp_load_s_sconf_au_dat (kt := KT1) (ktd := KT0) 2 false true pc rd rs1 imm m n
              ...
              (fun w => (⌜w = wrap16 np⌝ ∗ ⌜(nr <= np)%nat⌝ ∗ disk_pub γd np ∗
                         avail_half pav np)%I)
              (⊤ ∖ ↑minstretN) false
    ...
    { (* peek the invariant for [nr <= np]; the value itself is the holder's *)
      ...
      iIntros "[_ Havh]". iModIntro. iFrame "Hpub Havh".
      iSplitR; [done|]. iPureIntro. lia. }
    iIntros (w). iApply wp_next_off_intro. iIntros "Hcg Hpc Hpost". rgall.
    iDestruct "Hpost" as "(-> & %Hle & Hpub & Havh)".
    iApply ("Hcont" with "[%] Hpub Havh Hcg Hpc"). exact Hle.
  Qed.
  ...
  Lemma wp_vdrwd_sh_publish (γu : uart_names) (γd : disk_names) (pme : Arch.pa) (pd pav pu : mword 64)
    ...
    dev_inv γu γd -∗ disk_geom γd pd pav pu -∗
    disk_pub γd np -∗ avail_half pav np -∗ pin_offer pin -∗ phys_map wrb -∗
  ...
  Proof.
    ...
    iIntros "Hcg Hpc Hinstr #Hdinv #Hgeom Hpub Havh Hpin Hwrb Hpend Hcont".
    iDestruct (sie_cap_gpr_kmap_claims with "Hcg") as "[#Hkm Hcg]".
    iDestruct (disk_geom_static with "Hgeom") as %(_ & Hsta & _).
    iDestruct (disk_geom_canonical with "Hgeom") as %(_ & Hcana & _).
    ...
    iApply (wp_store_s_sconf_au_dat (kt := KT1) (ktd := KT0) 2 false pc rs2 rs1 imm m n
              (wrap16 (S np) : SailStdpp.Values.mword 16)
              (disk_pub γd (S np) ∗ disk_receipt γd np sl pin ∗ avail_half pav (S np) ∗
               pin_back pin)%I
              ...
              with "Hcg Hpc Hinstr [] [Hpub Havh Hpin Hwrb Hpend] [Hcont]").
    { rewrite Hea. iExact "Hcl". }
    { iDestruct (dev_inv_disk with "Hdinv") as "#Hvinv".
      iInv "Hvinv" as ">Hdbody" "Hdclose".
      iDestruct "Hdbody" as (vst) "(Hvf & Hproto & %Hvok)".
      ...
      (* A6.125: the pin is OFFERED (memory at 1, stamp at ½); the lease keeps
         a sealed half and hands the other half of memory back to the caller,
         whose kept halves make it a half ctx cell again *)
      ...
      iMod ("Hclose" with "Hah") as "(Hproto & Hpub & Hrcpt & Hpb)".
      iMod ("Hdclose" with "[Hvf Hproto]") as "_".
      { iNext. iExists vst. iFrame. iPureIntro. exact Hvok. }
      iModIntro. iFrame "Hpub Hrcpt Havh Hpb". }
    iApply wp_next_off_intro. iIntros "Hcg Hpc (Hpub & Hrcpt & Havh & Hpb)". rgall.
    iApply ("Hcont" with "Hcg Hpc Hpub Hrcpt Havh Hpb").
  Qed.
...
Section VdrwdPinBuild.
  ...
  Proof.
    ...
    iDestruct "Hchain" as "(Hops0 & Hops1 & Hops2 & Hda0 & Hdl0 & Hdf0 & Hdn0 &
                            ...
                            Hst & Hib & Hbd & Hrm & Hrt)".
    (* ---- every window into its OFFER and its KEPT halves ---- *)
    iDestruct (vdrwd_o2 (d_ring pav (np `mod` 8)) (Z_to_bv 16 (Z.of_nat h))
                 ltac:(apply (vdrwd_off_static pav _ (4 + 2 * (np `mod` 8))%nat 2 eq_refl
                                ltac:(pose proof (Nat.mod_upper_bound np 8 ltac:(lia)); lia) Hspav))
                 with "Hkm Hring") as "[Hring Kring]".
    ...
    iAssert (∀ mm, keep_map cur_ctx mm -∗
      ...
      with "[Kring Kda0 Kdl0 Kdf0 Kdn0 Kda1 Kdl1 Kdf1 Kdn1 Kda2 Kdl2 Kdf2 Kdn2
             Kops0 Kops1 Kops2]" as "Hkl".
    { iIntros (mm) "Hmm".
      rewrite /kp_list /vdrwd_regions /vdrwd_pinr_regions.
      iFrame "Kring Kda0 Kdl0 Kdf0 Kdn0 Kda1 Kdl1 Kdf1 Kdn1 Kda2 Kdl2 Kdf2 Kdn2
...
Section VdrwdP4.
  ...
  Proof.
    ...
    iApply (wp_vdrwd_lhu_avail γu γd pme pd pav pu
              ...
              ltac:(vm_compute; discriminate) ltac:(rdok)
              with "Hcg Hpc [] Hdinv Hgeom Hpub Hlb Havh").
    { iApply (rwi_178 with "Htext"). }
    iIntros "%Hle Hpub Havh Hcg Hpc".
    ...
    iApply (wp_vdrwd_sh_publish γu γd pme pd pav pu
              ...
              with "Hcg Hpc [] Hdinv Hgeom Hpub Havh Hpin Hwrb [Hdisk Hpend]").
      ...
      - iSplitR.
        ...
        + (* NOTHING HAS DRAINED YET: the whole write is still owed, so the
          ...
          * (* [vs_perm (vdrwd_slot kq …) = kq] by conversion; the entry's
            ...
            iExact "Hpend". }
    iIntros "Hcg Hpc Hpub Hrcpt Havh Hpb".
