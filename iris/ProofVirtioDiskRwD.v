...
(* ===================================================================== *)
(* §0  [fence rw,rw]  (clone of ProofVirtioDiskIntr §0b).                 *)
(* ===================================================================== *)

Lemma vdrwd_exec_FENCE_rw_rw (menvcfg0 : mword 64) s :
...
(* ===================================================================== *)
(* §3  The three leaves P4 needs beyond the plain ones: [fence rw,rw],    *)
(*     and the two dev_inv-OPENING accesses to [avail->idx].             *)
(* ===================================================================== *)
...
Section VdrwdLeaves.
  ...
  (* ---- fence rw,rw (clone of ProofVirtioDiskIntr's leaf) ---- *)
  Lemma wp_vdrwd_fence (γ : gname) (Φ : mval -> iProp Σ)
      (pc : mword 64) (m : regfile) (n : nat) :
    sie_cap_gpr γ m n -∗
    pc_is pc -∗
    instr pc false (FENCE (mword_of_int 0 : mword 4, mword_of_int 3 : mword 4,
                           mword_of_int 3 : mword 4,
                           Regidx (mword_of_int 0), Regidx (mword_of_int 0))) -∗
    ( sie_cap_gpr γ m n -∗
  ...
  Proof.
    ...
    iApply (wp_instr_s_sconf γ m n Φ pc false
              (FENCE (mword_of_int 0 : mword 4, mword_of_int 3 : mword 4,
                      mword_of_int 3 : mword 4,
                      Regidx (mword_of_int 0), Regidx (mword_of_int 0)))
              with "Hcg Hpc Hinstr").
    ...
    { iPureIntro. rewrite Hpceq. fold s_pc.
      apply (vdrwd_exec_FENCE_rw_rw menvcfg0 s_pc Lpriv_pc Lmenv_pc Hfiom). }
  ...
  Lemma wp_vdrwd_lhu_avail (γ : gname) (γu : uart_names) (γd : disk_names)
      (Φ : mval -> iProp Σ) (pd pav pu : mword 64)
      (pc : mword 64) (rd rs1 : mword 5) (imm : mword 12)
      (m : regfile) (n : nat) (np nr : nat) :
    add_vec (m !!! Regidx rs1) (sign_extend' 64 imm) = (pa_add pav 2%nat : mword 64) ->
    ...
    ( ⌜(nr <= np)%nat⌝ -∗
      ...
      sie_cap_gpr γ (<[Regidx rd := regval_into_reg
          (zero_extend' 64 (wrap16 np : SailStdpp.Values.mword 16))]> m) n -∗
      pc_is (add_vec_int pc 4) -∗
      WP (Loop : expr riscv_lang) {{ Φ }}) -∗
    WP (Loop : expr riscv_lang) {{ Φ }}.
  Proof.
  ...
  Lemma wp_vdrwd_sh_publish (γ : gname) (γu : uart_names) (γd : disk_names)
      (Φ : mval -> iProp Σ) (pd pav pu : mword 64)
      (pc : mword 64) (rs2 rs1 : mword 5) (imm : mword 12)
      (m : regfile) (n : nat) (np : nat) (sl : vslot) (pin wrb : _) :
    add_vec (m !!! Regidx rs1) (sign_extend' 64 imm) = (pa_add pav 2%nat : mword 64) ->
    trunc16 (m !!! Regidx rs2) = (wrap16 (S np) : SailStdpp.Values.mword 16) ->
  ...
  Proof.
    ...
    { iInv "Hdinv" as ">Hdbody" "Hdclose".
      ...
      iDestruct (virtio_proto_publish_acc γd vst np sl pin wrb
                   ltac:(rewrite Hceq; exact Hpinok) Hwrbdom Hwrpin
...
Section VdrwdPinBuild.
  ...
  Proof.
    ...
    iDestruct (vdrwd_buf_to_phys (b_data b) bs
                 ltac:(rewrite Hlen; exact Hsbuf) with "Hkm Hbuf") as "Hbuf".
...
Section VdrwdP4.
  ...
  Lemma wp_vdrw_p4 (γ : gname) (γu : uart_names) (γd : disk_names)
    ...
    sie_cap_gpr γ M av -∗
    kernel_text -∗ pc_is (mword_of_int (VRW + 0x162) : mword 64) -∗
    dev_inv γu γd -∗ disk_geom γd pd pav pu -∗
  ...
  Proof.
    ...
    iDestruct "Hgeom2" as "(_ & #Hap & _ & _ & _ & _ & _ & _ & _)".
    ...
    iPoseProof (rwi_162 with "Htext") as "Hi162".
    iPoseProof (rwi_164 with "Htext") as "Hi164".
    iPoseProof (rwi_168 with "Htext") as "Hi168".
    iPoseProof (rwi_16a with "Htext") as "Hi16a".
    iPoseProof (rwi_16c with "Htext") as "Hi16c".
    iPoseProof (rwi_16e with "Htext") as "Hi16e".
    iPoseProof (rwi_172 with "Htext") as "Hi172".
    iPoseProof (rwi_176 with "Htext") as "Hi176".
    iPoseProof (rwi_178 with "Htext") as "Hi178".
    iPoseProof (rwi_17c with "Htext") as "Hi17c".
    iPoseProof (rwi_17e with "Htext") as "Hi17e".
    iPoseProof (rwi_182 with "Htext") as "Hi182".
    (* ---- +0x162  c.ld a3,8(a5) ---- *)
    assert (Hava : add_vec (M !!! Regidx Ra5) (sign_extend' 64 (mword_of_int 8 : mword 12))
                   = (d_avail_ptr : SailStdpp.Values.mword 64))
      by (rewrite Ha5; apply vdrwd_avail_ptr_addr).
    iApply (wp_cld_s_sconf γ Φ (mword_of_int (VRW + 0x162) : mword 64) Ra3 Ra5
              (mword_of_int 8 : mword 12) M av pav (dqm := DfracDiscarded)
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi162 [] [-]").
    { iEval (rewrite Hava). iExact "Hap". }
    iIntros "Hcg Hpc _".
    ...
    assert (Hp164 : add_vec_int (mword_of_int (VRW + 0x162) : mword 64) 2
                    = mword_of_int (VRW + 0x164)) by pcstep.
    iEval (rewrite Hp164) in "Hpc".
    (* ---- +0x164  lhu a4,2(a3)  -- the avail-index read ---- *)
    assert (Hidxa : add_vec (N1 !!! Regidx Ra3) (sign_extend' 64 (mword_of_int 2 : mword 12))
                    = (pa_add pav 2%nat : SailStdpp.Values.mword 64))
      by (rewrite HN1a3; apply vdrwd_idx_addr).
    iApply (wp_vdrwd_lhu_avail γ γu γd Φ pd pav pu
              (mword_of_int (VRW + 0x164) : mword 64) Ra4 Ra3
              (mword_of_int 2 : mword 12) N1 av np nr Hidxa
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi164 Hdinv Hgeom Hpub Hlb [-]").
    ...
    assert (Hp168 : add_vec_int (mword_of_int (VRW + 0x164) : mword 64) 4
                    = mword_of_int (VRW + 0x168)) by pcstep.
    ...
    iApply (wp_candi_s_sconf γ Φ (mword_of_int (VRW + 0x168) : mword 64) Ra4
              (mword_of_int 7 : mword 6) N2 av
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi168 [-]").
    iIntros "Hcg Hpc".
    ...
    assert (Hp16a : add_vec_int (mword_of_int (VRW + 0x168) : mword 64) 2
                    = mword_of_int (VRW + 0x16a)) by pcstep.
    iEval (rewrite Hp16a) in "Hpc".
    (* ---- +0x16a  c.slli a4,a4,1 ---- *)
    iApply (wp_cslli_s_sconf γ Φ (mword_of_int (VRW + 0x16a) : mword 64)
              (Regidx Ra4) Ra4 (mword_of_int 1 : mword 6) N3 av
    ...
    assert (Hp16c : add_vec_int (mword_of_int (VRW + 0x16a) : mword 64) 2
                    = mword_of_int (VRW + 0x16c)) by pcstep.
    iEval (rewrite Hp16c) in "Hpc".
    (* ---- +0x16c  c.add a3,a3,a4 ---- *)
    iApply (wp_cadd_s_sconf γ Φ (mword_of_int (VRW + 0x16c) : mword 64) Ra3 Ra4 N4 av
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi16c [-]").
    iIntros "Hcg Hpc".
    ...
    assert (Hp16e : add_vec_int (mword_of_int (VRW + 0x16c) : mword 64) 2
                    = mword_of_int (VRW + 0x16e)) by pcstep.
    iEval (rewrite Hp16e) in "Hpc".
    (* ---- +0x16e  sh a0,4(a3)   ring[np mod 8] := h ---- *)
    assert (Hring : add_vec (N5 !!! Regidx Ra3)
                      (sign_extend' 64 (mword_of_int 4 : mword 12))
                    = (d_ring pav (np `mod` 8) : SailStdpp.Values.mword 64))
      by (rewrite HN5a3; apply vdrwd_ring_addr).
    iApply (wp_sh_s_sconf γ Φ (mword_of_int (VRW + 0x16e) : mword 64) Ra0 Ra3
              (mword_of_int 4 : mword 12) N5 av w0 with "Hcg Hpc Hi16e [Hcell] [-]").
    { iEval (rewrite Hring). iExact "Hcell". }
    ...
    (* ---- +0x172  fence rw,rw ---- *)
    iApply (wp_vdrwd_fence γ Φ (mword_of_int (VRW + 0x172) : mword 64) N5 av
              with "Hcg Hpc Hi172 [-]").
    iIntros "Hcg Hpc".
    ...
    iApply (wp_cld_s_sconf γ Φ (mword_of_int (VRW + 0x176) : mword 64) Ra4 Ra5
              (mword_of_int 8 : mword 12) N5 av pav (dqm := DfracDiscarded)
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi176 [] [-]").
    { iEval (rewrite Hava2). iExact "Hap". }
    iIntros "Hcg Hpc _".
    ...
    assert (Hp178 : add_vec_int (mword_of_int (VRW + 0x176) : mword 64) 2
                    = mword_of_int (VRW + 0x178)) by pcstep.
    iEval (rewrite Hp178) in "Hpc".
    (* ---- +0x178  lhu a5,2(a4) ---- *)
    assert (Hidxa2 : add_vec (N6 !!! Regidx Ra4)
                       (sign_extend' 64 (mword_of_int 2 : mword 12))
                     = (pa_add pav 2%nat : SailStdpp.Values.mword 64))
      by (rewrite HN6a4; apply vdrwd_idx_addr).
    iApply (wp_vdrwd_lhu_avail γ γu γd Φ pd pav pu
              (mword_of_int (VRW + 0x178) : mword 64) Ra5 Ra4
              (mword_of_int 2 : mword 12) N6 av np nr Hidxa2
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi178 Hdinv Hgeom Hpub Hlb [-]").
    ...
    assert (Hp17c : add_vec_int (mword_of_int (VRW + 0x178) : mword 64) 4
                    = mword_of_int (VRW + 0x17c)) by pcstep.
    iEval (rewrite Hp17c) in "Hpc".
    (* ---- +0x17c  c.addiw a5,a5,1 ---- *)
    iApply (wp_caddiw_s_sconf γ Φ (mword_of_int (VRW + 0x17c) : mword 64) Ra5
              (mword_of_int 1 : mword 6) N7 av
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi17c [-]").
    iIntros "Hcg Hpc".
    ...
    assert (Hp17e : add_vec_int (mword_of_int (VRW + 0x17c) : mword 64) 2
                    = mword_of_int (VRW + 0x17e)) by pcstep.
    ...
    (* ---- +0x17e  sh a5,2(a4)  -- THE PUBLISH ---- *)
    assert (Hidxa3 : add_vec (N8 !!! Regidx Ra4)
                       (sign_extend' 64 (mword_of_int 2 : mword 12))
                     = (pa_add pav 2%nat : SailStdpp.Values.mword 64))
      by (rewrite HN8a4; apply vdrwd_idx_addr).
    iApply (wp_vdrwd_sh_publish γ γu γd Φ pd pav pu
    ...
    (* ---- +0x182  fence rw,rw ---- *)
    iApply (wp_vdrwd_fence γ Φ (mword_of_int (VRW + 0x182) : mword 64) N8 av
              with "Hcg Hpc Hi182 [-]").
    iIntros "Hcg Hpc".
...
Section ProofVirtioDiskRwDSeam.
  ...
  (* ------------------------------------------------------------------- *)
  Definition vdrw_p4_exit (γ γk : gname) (Φ : mval -> iProp Σ)
  ...
  Lemma wp_vdrw_p4_seam (γ γk : gname) (Φ : mval -> iProp Σ)
  ...
  Proof.
    ...
    iApply (wp_vdrw_p4 γ γu γd Φ M (K - 12)%nat pd pav pu b wr (vdrw_sector_raw bno)
    ...
    - destruct Hregs as (Hsp & Hs0 & Hs3 & Hs6 & Hs7 & Htp).
      ...
      + rewrite (Hcs (mword_of_int 4 : mword 5) ltac:(vm_compute; reflexivity)). exact Htp.
