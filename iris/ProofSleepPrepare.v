...
Section ProofSleepPrepare.
  ...
  Proof.
    ...
    iDestruct (cpu_own_transport CID9 CID11 n eb pj b ltac:(wp_next_chain)
                 with "Hcpu") as "Hcpu".
    iApply (Acquire.wp_acquire_sconf KT1 γl "proc"%string (λ ξ : CtxId, proc_lock_res (XI := ξ) γs γl (proc_addr j)) B2 n eb pj (av - 4)%nat b lks
              Hn ltac:(lia) Hno
    ...
    iApply (Release.wp_release_sconf KT1 γl (proc_addr j) "proc"%string (λ ξ : CtxId, proc_lock_res (XI := ξ) γs γl (proc_addr j)) C2 n eb pj (av - 4)%nat
              ({["proc"]} ∪ lks)
              Hlka ltac:(lia)
