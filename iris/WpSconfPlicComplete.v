(* WpSconfPlicComplete.v: whole-function WP for xv6's plic_complete() in S-mode,
   ...
   and the WpSconf{Mem,Ctl,Alu} frame leaves.  The MMIO write goes through
...
Require Import DevModel PlicPlan PlicHart WpUart WpPlic WpPlicExec SpecCpuid SpecPlicComplete.
...
Section WpSconfPlicComplete.
  ...
  Proof.
    ...
    assert (Hpc10 : update_vec_dec (add_vec (add_vec_int (mword_of_int (PC + 0x0c) : mword 64) 4)
                       (sign_extend' 64 (zeros' 12))) 0 ('b"0") = (mword_of_int (PC + 0x10) : mword 64))
