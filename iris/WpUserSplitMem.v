(* WpUserSplitMem.v -- 2-aligned (pc == 2 mod 4) width-4 U-mode LOAD/STORE
   memory arms, riding the data-capable split-fetch engine
   [WpUserSplitData.wp_instr_u_split_data].  Each is the 4-aligned combined
   arm [WpUserMem4.ustep_{lw,sw}_u]'s DATA body spliced onto the 2-aligned
   split-fetch premise bundle; the data body after the callback intro is
   byte-identical to the 4-aligned twin (the 32-bit instruction still advances
   pc by 4, so nextPC handling is unchanged). *)
...
Require Import WpPushOffMem MemData4 UmodeData4.
Require Import UmodeData MemData2 UmodeData2 MemData1 UmodeData1 UmodeAmo4.
...
Require Import WpUserComputeMiss.
Require Import WpUserSplitData.

Section WpUserSplitMem.
  Context `{!riscvGS Σ}.
  Context `{CID : CpuId}.
  Context (U : uctx).
  ...
  Local Notation wp_instr_u_split_data := (WpUserSplitData.wp_instr_u_split_data U).

  (* store-window helpers (Local twins of the WpUserMem2/WpUserMem1 versions,
     which are Local there and hence not importable) *)
  Local Lemma upd_window_2 (mm : _) (pa : Arch.pa) (vnew vold : bv 16) :
    gen_heap_interp (hG:=riscv_memGS) mm -∗ ([∗ list] j ∈ seq 0 2, (pa_add pa j) ↦ₘ nth_byte vold j) ==∗
    gen_heap_interp (hG:=riscv_memGS) (write_bytes mm pa 2 vnew)
      ∗ ([∗ list] j ∈ seq 0 2, (pa_add pa j) ↦ₘ nth_byte vnew j).
  Proof. unfold write_bytes. change (N.to_nat 2) with 2%nat. apply upd_window_bw. Qed.

  Local Lemma upd_window_1 (mm : _) (pa : Arch.pa) (vnew vold : bv 8) :
    gen_heap_interp (hG:=riscv_memGS) mm -∗ ([∗ list] j ∈ seq 0 1, (pa_add pa j) ↦ₘ nth_byte vold j) ==∗
    gen_heap_interp (hG:=riscv_memGS) (write_bytes mm pa 1 vnew)
      ∗ ([∗ list] j ∈ seq 0 1, (pa_add pa j) ↦ₘ nth_byte vnew j).
  Proof. unfold write_bytes. change (N.to_nat 1) with 1%nat. apply upd_window_bw. Qed.

  Lemma ustep_u_split_lw_u
      (va : mword 64) (vpn : mword 27) (ie : uwalk_info) (w : mword 32)
      (vpnD : mword 27) (ieD : uwalk_info)
      (imm : mword 12) (rs1 rd : mword 5) (is_unsigned : bool) (v : mword 32)
  ...
  Proof.
    ...
    assert (Hnlpad : is_lpad_instruction (LOAD (imm, Regidx rs1, Regidx rd, is_unsigned, 4))
                       = false) by reflexivity.
    iApply (wp_instr_u_split_data va vpn ie w (LOAD (imm, Regidx rs1, Regidx rd, is_unsigned, 4))
              ms_v tlbvec E Φ HN Hok Hsome Hchk0 HupdN HcwL HcwH HSXL Hbit0 Hbit1 Hvalign4
    ...
    { unfold s1; lk.
      ...
      vm_compute; reflexivity. }
    (* ---- dispatch the DATA slot on the fetch-filled TLB ---- *)
    ...
    { destruct (match_TLB_Entry entD (mword_of_int 0 : mword 16)
      ...
      { (* data HIT: the stored entry is forced to be [upt_entry vpnD ieD] *)
    ...
    iAssert (⌜addr_is_ram (pa_add paD 3)⌝)%I as %HramD7.
    { iDestruct (big_sepM_lookup_acc (fun a b => (a ↦ₘ b)%I) dm _ _ (Hcwd 3%nat ltac:(lia)) with "Hdm")
        as "[Hb7 _]".
    ...
    destruct (Hpma_all paD 4) as (regionD & HpmamD & _ & HreadD & _ & _).
    ...
    assert (HE : exec (execute (LOAD (imm, Regidx rs1, Regidx rd, is_unsigned, 4))) s1
             = Some (RETIRE_SUCCESS,
                     set_reg s1 (R_bitvector_64 (gpr_of_Z (uint rd)))
                       (regval_into_reg (extend_value is_unsigned
                          (update_subrange_vec_dec (zeros' (4*1*8)) (4*(0+1)*8-1) (4*0*8) v))))).
    { apply (exec_execute_LOAD_4_U (upt_entry vpnD ieD) vpnD rs1 rd imm v regionD s1
               ...
               ltac:(fold eaS paS; rewrite Hpa;
                     exact (within_clint_false paD 4 s1 HncD ltac:(lia)))
               ltac:(fold eaS paS; rewrite Hpa;
                     exact (within_sig_false paD 4 s1 HnsD ltac:(lia)))
               ltac:(fold eaS paS; rewrite Hpa;
                     exact (within_htif_false paD 4 s1 Lhtif1))
               ltac:(fold eaS paS; rewrite Hpa;
                     exact (addr_is_ram_not_dev _ HramD))
               ltac:(fold eaS paS; rewrite Hpa; exact HbfD)
               Hrd is_unsigned). }
    rewrite data2_id_4 in HE.
    (* ---- ghost rd write ---- *)
    ...
    iDestruct ("Hfins" $! (regval_into_reg (extend_value is_unsigned v))
                 with "[Hrdc]") as "Hfmap".
    ...
    assert (Lnpc : register_lookup nextPC
             (set_reg s1 (R_bitvector_64 (gpr_of_Z (uint rd)))
                (regval_into_reg (extend_value is_unsigned v))).(sregs)
             = add_vec_int va 4).
    ...
    iExists ms_v, sc_v, stval_v, sepc_v, (add_vec_int va 4),
            (<[Regidx rd := regval_into_reg (extend_value is_unsigned v)]> g), tlbvec_f.
    ...
    iExact "Hdm".
      ...
      { (* colliding data MISS: the slot holds a non-matching entry *)
    ...
    assert (HE : exec (execute (LOAD (imm, Regidx rs1, Regidx rd, is_unsigned, 4))) s1
             = Some (RETIRE_SUCCESS,
                     set_reg s' (R_bitvector_64 (gpr_of_Z (uint rd)))
                       (regval_into_reg (extend_value is_unsigned
                          (update_subrange_vec_dec (zeros' (4*1*8)) (4*(0+1)*8-1) (4*0*8) v))))).
    { apply (exec_execute_LOAD_4_U_walk_nomatch vpnD root entD rs1 rd imm
    ...
    rewrite /user_frame.
    iExists ms_v, sc_v, stval_v, sepc_v, (add_vec_int va 4),
            (<[Regidx rd := regval_into_reg (extend_value is_unsigned v)]> g), tlbvecD.
    iFrame "Hhs' Hpriv' Hms' Hsc Hstv Hsepc Htlbc Hupt Hcode Hcfg'".
    iSplitR; [iPureIntro; exact HSXL |].
    iSplitR; [iPureIntro;
    ...
    { (* empty data MISS: the slot is None *)
    (* ---- read the DATA PTEs off the owned slots at s1 ---- *)
    iDestruct (upt_walk_read_ptes root slots spec vpnD ieD s1 HsomeD
                 ltac:(rewrite Lpmpc1; exact HpmpA)
                 ltac:(rewrite Lpmpa1; exact Hpmp_ord)
    ...
    assert (HE : exec (execute (LOAD (imm, Regidx rs1, Regidx rd, is_unsigned, 4))) s1
             = Some (RETIRE_SUCCESS,
                     set_reg s' (R_bitvector_64 (gpr_of_Z (uint rd)))
                       (regval_into_reg (extend_value is_unsigned
                          (update_subrange_vec_dec (zeros' (4*1*8)) (4*(0+1)*8-1) (4*0*8) v))))).
    { apply (exec_execute_LOAD_4_U_walk vpnD root rs1 rd imm
               (uw_pte2 ieD) (uw_pte1 ieD) (uw_pte0 ieD) v regionD s1 satp0
               Hrd
               H2i H2nl H1i H1nl H0i H0nl HchkD H0N
               Lmisa1 Lpriv1
  ...
  Lemma ustep_u_split_sw_u
      (va : mword 64) (vpn : mword 27) (ie : uwalk_info) (w : mword 32)
      (vpnD : mword 27) (ieD : uwalk_info)
      (imm : mword 12) (rs2 rs1 : mword 5) (vold : mword 32)
      (ms_v sc_v stval_v sepc_v : mword 64)
      (g : gmap regidx (mword 64))
      (tlbvec : vec (option TLB_Entry) (2 ^ 6))
      E (Φ : mval -> iProp Σ) :
    let eaF := add_vec (g !!! Regidx rs1) (sign_extend' 64 imm) in
    let paD := u_pa (upt_entry vpnD ieD) eaF vpnD in
    let vNew := (autocast (T := mword) (subrange_vec_dec (g !!! Regidx rs2) (Z.sub (Z.mul 4 8) 1) 0) : mword 32) in
    ↑minstretN ⊆ E ->
    upt_tlb_ok spec tlbvec ->
    spec !! vpn = Some ie ->
    uw_check_ok (InstructionFetch tt) ie ->
    ...
    ([∗ list] j ∈ seq 0 4, (pa_add paD j) ↦ₘ nth_byte vold j) -∗
    (* rebuilding [user_data] from the NEW window *)
    (([∗ list] j ∈ seq 0 4, (pa_add paD j) ↦ₘ nth_byte vNew j) -∗ user_data) -∗
    user_cfg -∗
    ▷ (user_frame -∗ WP (Loop : expr riscv_lang) @ E {{ Φ }}) -∗
    WP (Loop : expr riscv_lang) @ E {{ Φ }}.
  Proof.
    intros eaF paD vNew HN Hok Hsome Hchk0 HupdN HcwL HcwH HSXL HMPRV HMXR Hbit0 Hbit1 Hvalign4
           HcanonL Hvpn_defL HalignL HcanonH Hvpn_defH HalignH HnotRVC Hdec
           HsomeD HchkD HupdD HpbmtD
    ...
    iDestruct "Hhwc" as (misa0 mseccfg0 pmar0 elp0)
      "(#Hmisa & #Hmseccfg & #Hpma & #Hhtif & #Help & %HmisaS & %HmisaC &
        %HmisaU & %HmisaM & %Hpma_all & %Hseccfg1 & %Hseccfg2 & %Help_np &
        %HmisaA & %Hmisa_val0 & %Hmseccfg_val0)".
    assert (Hnlpad : is_lpad_instruction (STORE (imm, Regidx rs2, Regidx rs1, 4))
                       = false) by reflexivity.
    iApply (wp_instr_u_split_data va vpn ie w (STORE (imm, Regidx rs2, Regidx rs1, 4))
              ms_v tlbvec E Φ HN Hok Hsome Hchk0 HupdN HcwL HcwH HSXL Hbit0 Hbit1 Hvalign4
              HcanonL Hvpn_defL HalignL HcanonH Hvpn_defH HalignH HnotRVC Hdec Hnlpad
              with "Hhw Hinv Hhs Hpriv Hms Htlbc Hpcr Hcode Hupt Hcfg").
    iIntros (σ Hpceq Hag Hpins) "Htlbc Hupt [Hreg [Hmem Hdev]]".
    ...
    { iDestruct (big_sepL_lookup _ _ 0%nat 0%nat with "Hwin") as "Hb0".
      ...
      iPureIntro. exact Hr0. }
    iAssert (⌜addr_is_ram (pa_add paD 3)⌝)%I as %HramD7.
    { iDestruct (big_sepL_lookup _ _ 3%nat 3%nat with "Hwin") as "Hb7".
      { rewrite lookup_seq_lt; [reflexivity | lia]. }
      iDestruct (mem_ram with "Hb7") as %Hr7. iPureIntro. exact Hr7. }
    pose proof (addr_is_ram_not_in_clint _ HramD) as HncD.
    pose proof (addr_is_ram_not_in_sig _ HramD) as HnsD.
    destruct (Hpma_all paD 4) as (regionD & HpmamD & _ & _ & HwriteD & _).
    assert (HrangeD : pmpRangeMatch (Z.mul (uint (zeros' 64 : mword 64)) 4)
              (Z.mul (uint (vec_access_dec (register_lookup pmpaddr_n s1.(sregs)) 0)) 4)
              (uint paD) (uint (to_bits 64 4)) = PMP_Match).
    { rewrite Lpmpa1.
    ...
    assert (HE : exec (execute (STORE (imm, Regidx rs2, Regidx rs1, 4))) s1
             = Some (RETIRE_SUCCESS,
                     MState s1.(sregs) (write_bytes s1.(mem) paD 4 vNew) s1.(mdev))).
    { pose proof (exec_execute_STORE_4_U (upt_entry vpnD ieD) vpnD rs2 rs1 imm regionD s1
               HchkD' HupdD' HpbmtD' (upt_entry_match vpnD ieD)
               Lpriv1
               ltac:(rewrite Lms1; exact HSXL)
               ltac:(rewrite Lms1; exact HMPRV)
               ...
               ltac:(fold eaS paS; rewrite Hpa;
                     exact (within_sig_false paD 4 s1 HnsD ltac:(lia)))
               ltac:(fold eaS paS; rewrite Hpa;
                     exact (within_htif_writable_false paD 4 s1 Lhtif0))
               ltac:(fold eaS paS; rewrite Hpa;
    ...
    iMod (upd_window_4 σ.(mem) paD vNew vold with "Hmem Hwin") as "[Hmem Hwin]".
    iModIntro.
    iExists (MState s1.(sregs) (write_bytes s1.(mem) paD 4 vNew) s1.(mdev)).
    iSplitR; [iPureIntro; exact HE |].
    iSplitL "Hreg Hmem Hdev".
    ...
    iIntros "Hhs' Hpriv' Hms' Hpc' Hcfg'".
    assert (Lnpc : register_lookup nextPC
             (MState s1.(sregs) (write_bytes s1.(mem) paD 4 vNew) s1.(mdev)).(sregs)
             = add_vec_int va 4).
    ...
    iApply "Hcont".
    rewrite /user_frame.
    iExists ms_v, sc_v, stval_v, sepc_v, (add_vec_int va 4), g, tlbvec_f.
    iFrame "Hhs' Hpriv' Hms' Hsc Hstv Hsepc Htlbc Hupt Hcode Hcfg'".
    iSplitR; [iPureIntro; exact HSXL |].
    iSplitR; [iPureIntro; exact Hok_f |].
    iSplitL "Hpc' Hnpc"; [iFrame "Hpc' Hnpc" |].
    ...
    assert (HE : exec (execute (STORE (imm, Regidx rs2, Regidx rs1, 4))) s1
             = Some (RETIRE_SUCCESS,
                     MState s'.(sregs) (write_bytes s'.(mem) paD 4 vNew) s'.(mdev))).
    { pose proof (exec_execute_STORE_4_U_walk_nomatch vpnD root entD rs2 rs1 imm
    ...
    iMod (upd_window_4 σ.(mem) paD vNew vold with "Hmem Hwin") as "[Hmem Hwin]".
    iModIntro.
    iExists (MState s'.(sregs) (write_bytes s'.(mem) paD 4 vNew) s'.(mdev)).
    iSplitR; [iPureIntro; exact HE |].
    iSplitL "Hreg Hmem Hdev".
    { rewrite /mstate_interp. cbn [sregs mem mdev].
      unfold s', s1, set_reg; cbn [sregs mem mdev]. iFrame "Hreg Hmem Hdev". }
    iIntros "Hhs' Hpriv' Hms' Hpc' Hcfg'".
    assert (Lnpc : register_lookup nextPC
             (MState s'.(sregs) (write_bytes s'.(mem) paD 4 vNew) s'.(mdev)).(sregs)
             = add_vec_int va 4).
    ...
    iApply "Hcont".
    rewrite /user_frame.
    iExists ms_v, sc_v, stval_v, sepc_v, (add_vec_int va 4), g, tlbvecD.
    iFrame "Hhs' Hpriv' Hms' Hsc Hstv Hsepc Htlbc Hupt Hcode Hcfg'".
    iSplitR; [iPureIntro; exact HSXL |].
    iSplitR; [iPureIntro;
              exact (upt_tlb_ok_fill spec (register_lookup tlb s1.(sregs)) vpnD ieD HsomeD
    ...
    assert (HE : exec (execute (STORE (imm, Regidx rs2, Regidx rs1, 4))) s1
             = Some (RETIRE_SUCCESS,
                     MState s'.(sregs) (write_bytes s'.(mem) paD 4 vNew) s'.(mdev))).
    { pose proof (exec_execute_STORE_4_U_walk vpnD root rs2 rs1 imm
               (uw_pte2 ieD) (uw_pte1 ieD) (uw_pte0 ieD) regionD s1 satp0
               H2i H2nl H1i H1nl H0i H0nl HchkD H0N
               Lmisa1 Lpriv1
               ltac:(rewrite Lms1; exact HSXL)
  ...
  Proof.
    ...
    assert (Hnlpad : is_lpad_instruction (LOAD (imm, Regidx rs1, Regidx rd, false, 8))
                       = false) by reflexivity.
    iApply (wp_instr_u_split_data va vpn ie w (LOAD (imm, Regidx rs1, Regidx rd, false, 8))
              ms_v tlbvec E Φ HN Hok Hsome Hchk0 HupdN HcwL HcwH HSXL Hbit0 Hbit1 Hvalign4
              HcanonL Hvpn_defL HalignL HcanonH Hvpn_defH HalignH HnotRVC Hdec Hnlpad
              with "Hhw Hinv Hhs Hpriv Hms Htlbc Hpcr Hcode Hupt Hcfg").
    iIntros (σ Hpceq Hag Hpins) "Htlbc Hupt [Hreg [Hmem Hdev]]".
  ...
  Proof.
    ...
    assert (Hnlpad : is_lpad_instruction (STORE (imm, Regidx rs2, Regidx rs1, 8))
                       = false) by reflexivity.
    iApply (wp_instr_u_split_data va vpn ie w (STORE (imm, Regidx rs2, Regidx rs1, 8))
              ms_v tlbvec E Φ HN Hok Hsome Hchk0 HupdN HcwL HcwH HSXL Hbit0 Hbit1 Hvalign4
              HcanonL Hvpn_defL HalignL HcanonH Hvpn_defH HalignH HnotRVC Hdec Hnlpad
              with "Hhw Hinv Hhs Hpriv Hms Htlbc Hpcr Hcode Hupt Hcfg").
    iIntros (σ Hpceq Hag Hpins) "Htlbc Hupt [Hreg [Hmem Hdev]]".
  ...
  Lemma ustep_u_split_lh_u
      (va : mword 64) (vpn : mword 27) (ie : uwalk_info) (w : mword 32)
      (vpnD : mword 27) (ieD : uwalk_info)
      (imm : mword 12) (rs1 rd : mword 5) (is_unsigned : bool) (v : mword 16)
      (ms_v sc_v stval_v sepc_v : mword 64)
      (g : gmap regidx (mword 64))
      (dm : gmap Arch.pa (bv 8))
      (tlbvec : vec (option TLB_Entry) (2 ^ 6))
  ...
  Proof.
    ...
    iDestruct "Hhwc" as (misa0 mseccfg0 pmar0 elp0)
      "(#Hmisa & #Hmseccfg & #Hpma & #Hhtif & #Help & %HmisaS & %HmisaC &
        %HmisaU & %HmisaM & %Hpma_all & %Hseccfg1 & %Hseccfg2 & %Help_np &
        %HmisaA & %Hmisa_val0 & %Hmseccfg_val0)".
    assert (Hnlpad : is_lpad_instruction (LOAD (imm, Regidx rs1, Regidx rd, is_unsigned, 2))
                       = false) by reflexivity.
    iApply (wp_instr_u_split_data va vpn ie w (LOAD (imm, Regidx rs1, Regidx rd, is_unsigned, 2))
              ms_v tlbvec E Φ HN Hok Hsome Hchk0 HupdN HcwL HcwH HSXL Hbit0 Hbit1 Hvalign4
              HcanonL Hvpn_defL HalignL HcanonH Hvpn_defH HalignH HnotRVC Hdec Hnlpad
              with "Hhw Hinv Hhs Hpriv Hms Htlbc Hpcr Hcode Hupt Hcfg").
    iIntros (σ Hpceq Hag Hpins) "Htlbc Hupt [Hreg [Hmem Hdev]]".
    ...
    { iDestruct (big_sepM_lookup_acc (fun a b => (a ↦ₘ b)%I) dm _ _ (Hcwd 0%nat ltac:(lia)) with "Hdm")
      ...
      iDestruct (mem_ram with "Hb0") as %Hr0. rewrite pa_add_0 in Hr0.
      iPureIntro. exact Hr0. }
    iAssert (⌜addr_is_ram (pa_add paD 1)⌝)%I as %HramD7.
    { iDestruct (big_sepM_lookup_acc (fun a b => (a ↦ₘ b)%I) dm _ _ (Hcwd 1%nat ltac:(lia)) with "Hdm")
        as "[Hb7 _]".
      iDestruct (mem_ram with "Hb7") as %Hr7. iPureIntro. exact Hr7. }
    pose proof (addr_is_ram_not_in_clint _ HramD) as HncD.
    pose proof (addr_is_ram_not_in_sig _ HramD) as HnsD.
    destruct (Hpma_all paD 2) as (regionD & HpmamD & _ & HreadD & _ & _).
    assert (HrangeD : pmpRangeMatch (Z.mul (uint (zeros' 64 : mword 64)) 4)
              (Z.mul (uint (vec_access_dec (register_lookup pmpaddr_n s1.(sregs)) 0)) 4)
              (uint paD) (uint (to_bits 64 2)) = PMP_Match).
    { rewrite Lpmpa1.
      exact (ram_fetch_pmp paD (vec_access_dec pmpaddr00 0) 2 1
               ltac:(lia) ltac:(lia) ltac:(vm_compute; reflexivity) ltac:(reflexivity)
               HramD HramD7 Hpmp_cov). }
    (* ---- the LOAD execute fact at s1 ---- *)
    assert (HE : exec (execute (LOAD (imm, Regidx rs1, Regidx rd, is_unsigned, 2))) s1
             = Some (RETIRE_SUCCESS,
                     set_reg s1 (R_bitvector_64 (gpr_of_Z (uint rd)))
                       (regval_into_reg (extend_value is_unsigned
                          (update_subrange_vec_dec (zeros' (2*1*8)) (2*(0+1)*8-1) (2*0*8) v))))).
    { apply (exec_execute_LOAD_2_U (upt_entry vpnD ieD) vpnD rs1 rd imm v regionD s1
               HchkD' HupdD' HpbmtD' (upt_entry_match vpnD ieD)
               Lpriv1
               ltac:(rewrite Lms1; exact HSXL)
               ltac:(rewrite Lms1; exact HMPRV)
               ...
               HreadD
               ltac:(fold eaS paS; rewrite Hpa;
                     exact (within_clint_false paD 2 s1 HncD ltac:(lia)))
               ltac:(fold eaS paS; rewrite Hpa;
                     exact (within_sig_false paD 2 s1 HnsD ltac:(lia)))
               ltac:(fold eaS paS; rewrite Hpa;
                     exact (within_htif_false paD 2 s1 Lhtif1))
               ltac:(fold eaS paS; rewrite Hpa;
                     exact (addr_is_ram_not_dev _ HramD))
               ltac:(fold eaS paS; rewrite Hpa; exact HbfD)
               Hrd is_unsigned). }
    rewrite data2_id_2 in HE.
    (* ---- ghost rd write ---- *)
    iDestruct (big_sepM_insert_acc _ _ _ _ Hmd with "Hfmap") as "[Hrdc Hfins]".
    rewrite (gpr_pt_nz rd _ Hrd).
    iMod (reg_update _ (R_bitvector_64 (gpr_of_Z (uint rd))) _
    ...
    assert (HE : exec (execute (LOAD (imm, Regidx rs1, Regidx rd, is_unsigned, 2))) s1
             = Some (RETIRE_SUCCESS,
                     set_reg s' (R_bitvector_64 (gpr_of_Z (uint rd)))
                       (regval_into_reg (extend_value is_unsigned
                          (update_subrange_vec_dec (zeros' (2*1*8)) (2*(0+1)*8-1) (2*0*8) v))))).
    { apply (exec_execute_LOAD_2_U_walk_nomatch vpnD root entD rs1 rd imm
               (uw_pte2 ieD) (uw_pte1 ieD) (uw_pte0 ieD) v regionD s1 satp0
               Hrd
               H2i H2nl H1i H1nl H0i H0nl HchkD H0N
               Lmisa1 Lpriv1
    ...
    assert (HE : exec (execute (LOAD (imm, Regidx rs1, Regidx rd, is_unsigned, 2))) s1
             = Some (RETIRE_SUCCESS,
                     set_reg s' (R_bitvector_64 (gpr_of_Z (uint rd)))
                       (regval_into_reg (extend_value is_unsigned
                          (update_subrange_vec_dec (zeros' (2*1*8)) (2*(0+1)*8-1) (2*0*8) v))))).
    { apply (exec_execute_LOAD_2_U_walk vpnD root rs1 rd imm
               (uw_pte2 ieD) (uw_pte1 ieD) (uw_pte0 ieD) v regionD s1 satp0
               Hrd
               H2i H2nl H1i H1nl H0i H0nl HchkD H0N
               Lmisa1 Lpriv1
  ...
  Lemma ustep_u_split_sh_u
      (va : mword 64) (vpn : mword 27) (ie : uwalk_info) (w : mword 32)
      (vpnD : mword 27) (ieD : uwalk_info)
      (imm : mword 12) (rs2 rs1 : mword 5) (vold : mword 16)
      (ms_v sc_v stval_v sepc_v : mword 64)
      (g : gmap regidx (mword 64))
      (tlbvec : vec (option TLB_Entry) (2 ^ 6))
      E (Φ : mval -> iProp Σ) :
    let eaF := add_vec (g !!! Regidx rs1) (sign_extend' 64 imm) in
    let paD := u_pa (upt_entry vpnD ieD) eaF vpnD in
    let vNew := (autocast (T := mword) (subrange_vec_dec (g !!! Regidx rs2) (Z.sub (Z.mul 2 8) 1) 0) : mword 16) in
    ↑minstretN ⊆ E ->
    upt_tlb_ok spec tlbvec ->
    spec !! vpn = Some ie ->
    uw_check_ok (InstructionFetch tt) ie ->
    ...
    user_code -∗
    (* the target window's OLD bytes, owned *)
    ([∗ list] j ∈ seq 0 2, (pa_add paD j) ↦ₘ nth_byte vold j) -∗
    (* rebuilding [user_data] from the NEW window *)
    (([∗ list] j ∈ seq 0 2, (pa_add paD j) ↦ₘ nth_byte vNew j) -∗ user_data) -∗
    user_cfg -∗
    ▷ (user_frame -∗ WP (Loop : expr riscv_lang) @ E {{ Φ }}) -∗
    WP (Loop : expr riscv_lang) @ E {{ Φ }}.
  Proof.
    ...
    iDestruct "Hhwc" as (misa0 mseccfg0 pmar0 elp0)
      "(#Hmisa & #Hmseccfg & #Hpma & #Hhtif & #Help & %HmisaS & %HmisaC &
        %HmisaU & %HmisaM & %Hpma_all & %Hseccfg1 & %Hseccfg2 & %Help_np &
        %HmisaA & %Hmisa_val0 & %Hmseccfg_val0)".
    assert (Hnlpad : is_lpad_instruction (STORE (imm, Regidx rs2, Regidx rs1, 2))
                       = false) by reflexivity.
    iApply (wp_instr_u_split_data va vpn ie w (STORE (imm, Regidx rs2, Regidx rs1, 2))
              ms_v tlbvec E Φ HN Hok Hsome Hchk0 HupdN HcwL HcwH HSXL Hbit0 Hbit1 Hvalign4
              HcanonL Hvpn_defL HalignL HcanonH Hvpn_defH HalignH HnotRVC Hdec Hnlpad
              with "Hhw Hinv Hhs Hpriv Hms Htlbc Hpcr Hcode Hupt Hcfg").
    iIntros (σ Hpceq Hag Hpins) "Htlbc Hupt [Hreg [Hmem Hdev]]".
    ...
    { iDestruct (big_sepL_lookup _ _ 0%nat 0%nat with "Hwin") as "Hb0".
      ...
      iPureIntro. exact Hr0. }
    iAssert (⌜addr_is_ram (pa_add paD 1)⌝)%I as %HramD7.
    { iDestruct (big_sepL_lookup _ _ 1%nat 1%nat with "Hwin") as "Hb7".
      { rewrite lookup_seq_lt; [reflexivity | lia]. }
      iDestruct (mem_ram with "Hb7") as %Hr7. iPureIntro. exact Hr7. }
    pose proof (addr_is_ram_not_in_clint _ HramD) as HncD.
    pose proof (addr_is_ram_not_in_sig _ HramD) as HnsD.
    destruct (Hpma_all paD 2) as (regionD & HpmamD & _ & _ & HwriteD & _).
    assert (HrangeD : pmpRangeMatch (Z.mul (uint (zeros' 64 : mword 64)) 4)
              (Z.mul (uint (vec_access_dec (register_lookup pmpaddr_n s1.(sregs)) 0)) 4)
              (uint paD) (uint (to_bits 64 2)) = PMP_Match).
    { rewrite Lpmpa1.
      exact (ram_fetch_pmp paD (vec_access_dec pmpaddr00 0) 2 1
               ...
               HramD HramD7 Hpmp_cov). }
    (* ---- the STORE execute fact at s1 ---- *)
    assert (HE : exec (execute (STORE (imm, Regidx rs2, Regidx rs1, 2))) s1
             = Some (RETIRE_SUCCESS,
                     MState s1.(sregs) (write_bytes s1.(mem) paD 2 vNew) s1.(mdev))).
    { pose proof (exec_execute_STORE_2_U (upt_entry vpnD ieD) vpnD rs2 rs1 imm regionD s1
               HchkD' HupdD' HpbmtD' (upt_entry_match vpnD ieD)
               Lpriv1
               ltac:(rewrite Lms1; exact HSXL)
               ltac:(rewrite Lms1; exact HMPRV)
               ...
               ltac:(fold eaS paS; rewrite Hpa;
                     exact (within_sig_false paD 2 s1 HnsD ltac:(lia)))
               ltac:(fold eaS paS; rewrite Hpa;
                     exact (within_htif_writable_false paD 2 s1 Lhtif0))
               ltac:(fold eaS paS; rewrite Hpa;
                     exact (addr_is_ram_not_dev _ HramD))) as HE0.
      fold eaS paS in HE0. rewrite Hpa in HE0. rewrite Hrv2 in HE0.
      exact HE0. }
    (* ---- ghost window update in lock-step with write_bytes ---- *)
    iMod (upd_window_2 σ.(mem) paD vNew vold with "Hmem Hwin") as "[Hmem Hwin]".
    iModIntro.
    iExists (MState s1.(sregs) (write_bytes s1.(mem) paD 2 vNew) s1.(mdev)).
    iSplitR; [iPureIntro; exact HE |].
    iSplitL "Hreg Hmem Hdev".
    { rewrite /mstate_interp. cbn [sregs mem mdev].
      unfold s1, set_reg; cbn [sregs mem mdev]. iFrame "Hreg Hmem Hdev". }
    iIntros "Hhs' Hpriv' Hms' Hpc' Hcfg'".
    assert (Lnpc : register_lookup nextPC
             (MState s1.(sregs) (write_bytes s1.(mem) paD 2 vNew) s1.(mdev)).(sregs)
             = add_vec_int va 4).
    { cbn [sregs]. unfold s1, set_reg; cbn [sregs].
      rewrite register_lookup_set. reflexivity. }
    iEval (rewrite Lnpc) in "Hpc'".
    ...
    assert (HE : exec (execute (STORE (imm, Regidx rs2, Regidx rs1, 2))) s1
             = Some (RETIRE_SUCCESS,
                     MState s'.(sregs) (write_bytes s'.(mem) paD 2 vNew) s'.(mdev))).
    { pose proof (exec_execute_STORE_2_U_walk_nomatch vpnD root entD rs2 rs1 imm
               (uw_pte2 ieD) (uw_pte1 ieD) (uw_pte0 ieD) regionD s1 satp0
               H2i H2nl H1i H1nl H0i H0nl HchkD H0N
               Lmisa1 Lpriv1
               ltac:(rewrite Lms1; exact HSXL)
    ...
    iMod (upd_window_2 σ.(mem) paD vNew vold with "Hmem Hwin") as "[Hmem Hwin]".
    iModIntro.
    iExists (MState s'.(sregs) (write_bytes s'.(mem) paD 2 vNew) s'.(mdev)).
    iSplitR; [iPureIntro; exact HE |].
    iSplitL "Hreg Hmem Hdev".
    { rewrite /mstate_interp. cbn [sregs mem mdev].
      unfold s', s1, set_reg; cbn [sregs mem mdev]. iFrame "Hreg Hmem Hdev". }
    iIntros "Hhs' Hpriv' Hms' Hpc' Hcfg'".
    assert (Lnpc : register_lookup nextPC
             (MState s'.(sregs) (write_bytes s'.(mem) paD 2 vNew) s'.(mdev)).(sregs)
             = add_vec_int va 4).
    { cbn [sregs]. unfold s', s1, set_reg; cbn [sregs].
      tmig. rewrite register_lookup_set. reflexivity. }
    iEval (rewrite Lnpc) in "Hpc'".
    ...
    assert (HE : exec (execute (STORE (imm, Regidx rs2, Regidx rs1, 2))) s1
             = Some (RETIRE_SUCCESS,
                     MState s'.(sregs) (write_bytes s'.(mem) paD 2 vNew) s'.(mdev))).
    { pose proof (exec_execute_STORE_2_U_walk vpnD root rs2 rs1 imm
               (uw_pte2 ieD) (uw_pte1 ieD) (uw_pte0 ieD) regionD s1 satp0
               H2i H2nl H1i H1nl H0i H0nl HchkD H0N
               Lmisa1 Lpriv1
               ltac:(rewrite Lms1; exact HSXL)
  ...
  Lemma ustep_u_split_lb_u
      (va : mword 64) (vpn : mword 27) (ie : uwalk_info) (w : mword 32)
      (vpnD : mword 27) (ieD : uwalk_info)
      (imm : mword 12) (rs1 rd : mword 5) (is_unsigned : bool) (v : mword 8)
      (ms_v sc_v stval_v sepc_v : mword 64)
      (g : gmap regidx (mword 64))
      (dm : gmap Arch.pa (bv 8))
      (tlbvec : vec (option TLB_Entry) (2 ^ 6))
  ...
  Proof.
    ...
    iDestruct "Hhwc" as (misa0 mseccfg0 pmar0 elp0)
      "(#Hmisa & #Hmseccfg & #Hpma & #Hhtif & #Help & %HmisaS & %HmisaC &
        %HmisaU & %HmisaM & %Hpma_all & %Hseccfg1 & %Hseccfg2 & %Help_np &
        %HmisaA & %Hmisa_val0 & %Hmseccfg_val0)".
    assert (Hnlpad : is_lpad_instruction (LOAD (imm, Regidx rs1, Regidx rd, is_unsigned, 1))
                       = false) by reflexivity.
    iApply (wp_instr_u_split_data va vpn ie w (LOAD (imm, Regidx rs1, Regidx rd, is_unsigned, 1))
              ms_v tlbvec E Φ HN Hok Hsome Hchk0 HupdN HcwL HcwH HSXL Hbit0 Hbit1 Hvalign4
              HcanonL Hvpn_defL HalignL HcanonH Hvpn_defH HalignH HnotRVC Hdec Hnlpad
              with "Hhw Hinv Hhs Hpriv Hms Htlbc Hpcr Hcode Hupt Hcfg").
    iIntros (σ Hpceq Hag Hpins) "Htlbc Hupt [Hreg [Hmem Hdev]]".
    ...
    { intros s0. exact (upt_entry_pbmt vpnD ieD s0 HpbmtD). }
    (* ---- the physical-side facts at the (frame-form) pa ---- *)
    iAssert (⌜forall j : nat, (N.of_nat j < 1)%N ->
               σ.(mem) !! (pa_add paD j) = Some (nth_byte v j)⌝)%I as %HbfD.
    { iIntros (j Hj).
      iDestruct (big_sepM_lookup_acc (fun a b => (a ↦ₘ b)%I) dm _ _ (Hcwd j ltac:(lia)) with "Hdm")
        as "[Hbj _]".
    ...
    { iDestruct (big_sepM_lookup_acc (fun a b => (a ↦ₘ b)%I) dm _ _ (Hcwd 0%nat ltac:(lia)) with "Hdm")
      ...
      iDestruct (mem_ram with "Hb0") as %Hr0. rewrite pa_add_0 in Hr0.
      iPureIntro. exact Hr0. }
    iAssert (⌜addr_is_ram (pa_add paD 0)⌝)%I as %HramD7.
    { iDestruct (big_sepM_lookup_acc (fun a b => (a ↦ₘ b)%I) dm _ _ (Hcwd 0%nat ltac:(lia)) with "Hdm")
        as "[Hb7 _]".
      iDestruct (mem_ram with "Hb7") as %Hr7. iPureIntro. exact Hr7. }
    pose proof (addr_is_ram_not_in_clint _ HramD) as HncD.
    pose proof (addr_is_ram_not_in_sig _ HramD) as HnsD.
    destruct (Hpma_all paD 1) as (regionD & HpmamD & _ & HreadD & _ & _).
    assert (HrangeD : pmpRangeMatch (Z.mul (uint (zeros' 64 : mword 64)) 4)
              (Z.mul (uint (vec_access_dec (register_lookup pmpaddr_n s1.(sregs)) 0)) 4)
              (uint paD) (uint (to_bits 64 1)) = PMP_Match).
    { rewrite Lpmpa1.
      exact (ram_fetch_pmp paD (vec_access_dec pmpaddr00 0) 1 0
               ltac:(lia) ltac:(lia) ltac:(vm_compute; reflexivity) ltac:(reflexivity)
               HramD HramD7 Hpmp_cov). }
    (* ---- the LOAD execute fact at s1 ---- *)
    assert (HE : exec (execute (LOAD (imm, Regidx rs1, Regidx rd, is_unsigned, 1))) s1
             = Some (RETIRE_SUCCESS,
                     set_reg s1 (R_bitvector_64 (gpr_of_Z (uint rd)))
                       (regval_into_reg (extend_value is_unsigned
                          (update_subrange_vec_dec (zeros' (1*1*8)) (1*(0+1)*8-1) (1*0*8) v))))).
    { apply (exec_execute_LOAD_1_U (upt_entry vpnD ieD) vpnD rs1 rd imm v regionD s1
               HchkD' HupdD' HpbmtD' (upt_entry_match vpnD ieD)
               Lpriv1
               ltac:(rewrite Lms1; exact HSXL)
               ltac:(rewrite Lms1; exact HMPRV)
               ...
               HreadD
               ltac:(fold eaS paS; rewrite Hpa;
                     exact (within_clint_false paD 1 s1 HncD ltac:(lia)))
               ltac:(fold eaS paS; rewrite Hpa;
                     exact (within_sig_false paD 1 s1 HnsD ltac:(lia)))
               ltac:(fold eaS paS; rewrite Hpa;
                     exact (within_htif_false paD 1 s1 Lhtif1))
               ltac:(fold eaS paS; rewrite Hpa;
                     exact (addr_is_ram_not_dev _ HramD))
               ltac:(fold eaS paS; rewrite Hpa; exact HbfD)
               Hrd is_unsigned). }
    rewrite data2_id_1 in HE.
    (* ---- ghost rd write ---- *)
    iDestruct (big_sepM_insert_acc _ _ _ _ Hmd with "Hfmap") as "[Hrdc Hfins]".
    rewrite (gpr_pt_nz rd _ Hrd).
    iMod (reg_update _ (R_bitvector_64 (gpr_of_Z (uint rd))) _
    ...
    assert (HE : exec (execute (LOAD (imm, Regidx rs1, Regidx rd, is_unsigned, 1))) s1
             = Some (RETIRE_SUCCESS,
                     set_reg s' (R_bitvector_64 (gpr_of_Z (uint rd)))
                       (regval_into_reg (extend_value is_unsigned
                          (update_subrange_vec_dec (zeros' (1*1*8)) (1*(0+1)*8-1) (1*0*8) v))))).
    { apply (exec_execute_LOAD_1_U_walk_nomatch vpnD root entD rs1 rd imm
               (uw_pte2 ieD) (uw_pte1 ieD) (uw_pte0 ieD) v regionD s1 satp0
               Hrd
               H2i H2nl H1i H1nl H0i H0nl HchkD H0N
               Lmisa1 Lpriv1
    ...
    assert (HE : exec (execute (LOAD (imm, Regidx rs1, Regidx rd, is_unsigned, 1))) s1
             = Some (RETIRE_SUCCESS,
                     set_reg s' (R_bitvector_64 (gpr_of_Z (uint rd)))
                       (regval_into_reg (extend_value is_unsigned
                          (update_subrange_vec_dec (zeros' (1*1*8)) (1*(0+1)*8-1) (1*0*8) v))))).
    { apply (exec_execute_LOAD_1_U_walk vpnD root rs1 rd imm
               (uw_pte2 ieD) (uw_pte1 ieD) (uw_pte0 ieD) v regionD s1 satp0
               Hrd
               H2i H2nl H1i H1nl H0i H0nl HchkD H0N
               Lmisa1 Lpriv1
  ...
  Lemma ustep_u_split_sb_u
      (va : mword 64) (vpn : mword 27) (ie : uwalk_info) (w : mword 32)
      (vpnD : mword 27) (ieD : uwalk_info)
      (imm : mword 12) (rs2 rs1 : mword 5) (vold : mword 8)
      (ms_v sc_v stval_v sepc_v : mword 64)
      (g : gmap regidx (mword 64))
      (tlbvec : vec (option TLB_Entry) (2 ^ 6))
      E (Φ : mval -> iProp Σ) :
    let eaF := add_vec (g !!! Regidx rs1) (sign_extend' 64 imm) in
    let paD := u_pa (upt_entry vpnD ieD) eaF vpnD in
    let vNew := (autocast (T := mword) (subrange_vec_dec (g !!! Regidx rs2) (Z.sub (Z.mul 1 8) 1) 0) : mword 8) in
    ↑minstretN ⊆ E ->
    upt_tlb_ok spec tlbvec ->
    spec !! vpn = Some ie ->
    uw_check_ok (InstructionFetch tt) ie ->
    ...
    user_code -∗
    (* the target window's OLD bytes, owned *)
    ([∗ list] j ∈ seq 0 1, (pa_add paD j) ↦ₘ nth_byte vold j) -∗
    (* rebuilding [user_data] from the NEW window *)
    (([∗ list] j ∈ seq 0 1, (pa_add paD j) ↦ₘ nth_byte vNew j) -∗ user_data) -∗
    user_cfg -∗
    ▷ (user_frame -∗ WP (Loop : expr riscv_lang) @ E {{ Φ }}) -∗
    WP (Loop : expr riscv_lang) @ E {{ Φ }}.
  Proof.
    ...
    iDestruct "Hhwc" as (misa0 mseccfg0 pmar0 elp0)
      "(#Hmisa & #Hmseccfg & #Hpma & #Hhtif & #Help & %HmisaS & %HmisaC &
        %HmisaU & %HmisaM & %Hpma_all & %Hseccfg1 & %Hseccfg2 & %Help_np &
        %HmisaA & %Hmisa_val0 & %Hmseccfg_val0)".
    assert (Hnlpad : is_lpad_instruction (STORE (imm, Regidx rs2, Regidx rs1, 1))
                       = false) by reflexivity.
    iApply (wp_instr_u_split_data va vpn ie w (STORE (imm, Regidx rs2, Regidx rs1, 1))
              ms_v tlbvec E Φ HN Hok Hsome Hchk0 HupdN HcwL HcwH HSXL Hbit0 Hbit1 Hvalign4
              HcanonL Hvpn_defL HalignL HcanonH Hvpn_defH HalignH HnotRVC Hdec Hnlpad
              with "Hhw Hinv Hhs Hpriv Hms Htlbc Hpcr Hcode Hupt Hcfg").
    iIntros (σ Hpceq Hag Hpins) "Htlbc Hupt [Hreg [Hmem Hdev]]".
    ...
    { iDestruct (big_sepL_lookup _ _ 0%nat 0%nat with "Hwin") as "Hb0".
      ...
      iPureIntro. exact Hr0. }
    iAssert (⌜addr_is_ram (pa_add paD 0)⌝)%I as %HramD7.
    { iDestruct (big_sepL_lookup _ _ 0%nat 0%nat with "Hwin") as "Hb7".
      { rewrite lookup_seq_lt; [reflexivity | lia]. }
      iDestruct (mem_ram with "Hb7") as %Hr7. iPureIntro. exact Hr7. }
    pose proof (addr_is_ram_not_in_clint _ HramD) as HncD.
    pose proof (addr_is_ram_not_in_sig _ HramD) as HnsD.
    destruct (Hpma_all paD 1) as (regionD & HpmamD & _ & _ & HwriteD & _).
    assert (HrangeD : pmpRangeMatch (Z.mul (uint (zeros' 64 : mword 64)) 4)
              (Z.mul (uint (vec_access_dec (register_lookup pmpaddr_n s1.(sregs)) 0)) 4)
              (uint paD) (uint (to_bits 64 1)) = PMP_Match).
    { rewrite Lpmpa1.
      exact (ram_fetch_pmp paD (vec_access_dec pmpaddr00 0) 1 0
               ...
               HramD HramD7 Hpmp_cov). }
    (* ---- the STORE execute fact at s1 ---- *)
    assert (HE : exec (execute (STORE (imm, Regidx rs2, Regidx rs1, 1))) s1
             = Some (RETIRE_SUCCESS,
                     MState s1.(sregs) (write_bytes s1.(mem) paD 1 vNew) s1.(mdev))).
    { pose proof (exec_execute_STORE_1_U (upt_entry vpnD ieD) vpnD rs2 rs1 imm regionD s1
               HchkD' HupdD' HpbmtD' (upt_entry_match vpnD ieD)
               Lpriv1
               ltac:(rewrite Lms1; exact HSXL)
               ltac:(rewrite Lms1; exact HMPRV)
               ...
               ltac:(fold eaS paS; rewrite Hpa;
                     exact (within_sig_false paD 1 s1 HnsD ltac:(lia)))
               ltac:(fold eaS paS; rewrite Hpa;
                     exact (within_htif_writable_false paD 1 s1 Lhtif0))
               ltac:(fold eaS paS; rewrite Hpa;
                     exact (addr_is_ram_not_dev _ HramD))) as HE0.
      fold eaS paS in HE0. rewrite Hpa in HE0. rewrite Hrv2 in HE0.
      exact HE0. }
    (* ---- ghost window update in lock-step with write_bytes ---- *)
    iMod (upd_window_1 σ.(mem) paD vNew vold with "Hmem Hwin") as "[Hmem Hwin]".
    iModIntro.
    iExists (MState s1.(sregs) (write_bytes s1.(mem) paD 1 vNew) s1.(mdev)).
    iSplitR; [iPureIntro; exact HE |].
    iSplitL "Hreg Hmem Hdev".
    { rewrite /mstate_interp. cbn [sregs mem mdev].
      unfold s1, set_reg; cbn [sregs mem mdev]. iFrame "Hreg Hmem Hdev". }
    iIntros "Hhs' Hpriv' Hms' Hpc' Hcfg'".
    assert (Lnpc : register_lookup nextPC
             (MState s1.(sregs) (write_bytes s1.(mem) paD 1 vNew) s1.(mdev)).(sregs)
             = add_vec_int va 4).
    { cbn [sregs]. unfold s1, set_reg; cbn [sregs].
      rewrite register_lookup_set. reflexivity. }
    iEval (rewrite Lnpc) in "Hpc'".
    ...
    assert (HE : exec (execute (STORE (imm, Regidx rs2, Regidx rs1, 1))) s1
             = Some (RETIRE_SUCCESS,
                     MState s'.(sregs) (write_bytes s'.(mem) paD 1 vNew) s'.(mdev))).
    { pose proof (exec_execute_STORE_1_U_walk_nomatch vpnD root entD rs2 rs1 imm
               (uw_pte2 ieD) (uw_pte1 ieD) (uw_pte0 ieD) regionD s1 satp0
               H2i H2nl H1i H1nl H0i H0nl HchkD H0N
               Lmisa1 Lpriv1
               ltac:(rewrite Lms1; exact HSXL)
    ...
    iMod (upd_window_1 σ.(mem) paD vNew vold with "Hmem Hwin") as "[Hmem Hwin]".
    iModIntro.
    iExists (MState s'.(sregs) (write_bytes s'.(mem) paD 1 vNew) s'.(mdev)).
    iSplitR; [iPureIntro; exact HE |].
    iSplitL "Hreg Hmem Hdev".
    { rewrite /mstate_interp. cbn [sregs mem mdev].
      unfold s', s1, set_reg; cbn [sregs mem mdev]. iFrame "Hreg Hmem Hdev". }
    iIntros "Hhs' Hpriv' Hms' Hpc' Hcfg'".
    assert (Lnpc : register_lookup nextPC
             (MState s'.(sregs) (write_bytes s'.(mem) paD 1 vNew) s'.(mdev)).(sregs)
             = add_vec_int va 4).
    { cbn [sregs]. unfold s', s1, set_reg; cbn [sregs].
      tmig. rewrite register_lookup_set. reflexivity. }
    iEval (rewrite Lnpc) in "Hpc'".
    ...
    assert (HE : exec (execute (STORE (imm, Regidx rs2, Regidx rs1, 1))) s1
             = Some (RETIRE_SUCCESS,
                     MState s'.(sregs) (write_bytes s'.(mem) paD 1 vNew) s'.(mdev))).
    { pose proof (exec_execute_STORE_1_U_walk vpnD root rs2 rs1 imm
               (uw_pte2 ieD) (uw_pte1 ieD) (uw_pte0 ieD) regionD s1 satp0
               H2i H2nl H1i H1nl H0i H0nl HchkD H0N
               Lmisa1 Lpriv1
               ltac:(rewrite Lms1; exact HSXL)
  ...
  Lemma ustep_u_split_amoswap_w_u
      (va : mword 64) (vpn : mword 27) (ie : uwalk_info) (w : mword 32)
      (vpnD : mword 27) (ieD : uwalk_info)
      (rs2 rs1 rd : mword 5) (vold : mword 32)
      (ms_v sc_v stval_v sepc_v : mword 64)
      (g : gmap regidx (mword 64))
      (tlbvec : vec (option TLB_Entry) (2 ^ 6))
      E (Φ : mval -> iProp Σ) :
    ...
    let paD := u_pa (upt_entry vpnD ieD) eaF vpnD in
    let vN := sign_extend' (Z.mul 8 (__id 4)) (trunc (Z.mul (__id 4) 8)
                (g !!! Regidx rs2)) : mword 32 in
    ↑minstretN ⊆ E ->
    upt_tlb_ok spec tlbvec ->
    spec !! vpn = Some ie ->
    uw_check_ok (InstructionFetch tt) ie ->
    ...
    user_code -∗
    ([∗ list] j ∈ seq 0 4, (pa_add paD j) ↦ₘ nth_byte vold j) -∗
    (([∗ list] j ∈ seq 0 4, (pa_add paD j) ↦ₘ nth_byte vN j) -∗ user_data) -∗
    user_cfg -∗
    ▷ (user_frame -∗ WP (Loop : expr riscv_lang) @ E {{ Φ }}) -∗
    WP (Loop : expr riscv_lang) @ E {{ Φ }}.
  Proof.
    intros eaF paD vN HN Hok Hsome Hchk0 HupdN HcwL HcwH HSXL HMPRV HMXR Hbit0 Hbit1 Hvalign4
           HcanonL Hvpn_defL HalignL HcanonH Hvpn_defH HalignH HnotRVC Hdec Hrd
           HsomeD HchkD HupdD HpbmtD
           HalignD HcanonD Hvpn_defD HpaalD.
    iIntros "#Hhw #Hinv Hhs Hpriv Hms Hsc Hstv Hsepc Htlbc [Hpcr Hnpc]
    ...
    iDestruct "Hhwc" as (misa0 mseccfg0 pmar0 elp0)
      "(#Hmisa & #Hmseccfg & #Hpma & #Hhtif & #Help & %HmisaS & %HmisaC &
        %HmisaU & %HmisaM & %Hpma_all & %Hseccfg1 & %Hseccfg2 & %Help_np &
        %HmisaA & %Hmisa_val0 & %Hmseccfg_val0)".
    assert (Hnlpad : is_lpad_instruction (AMO (AMOSWAP, true, false, Regidx rs2, Regidx rs1, 4, Regidx rd))
                       = false) by reflexivity.
    iApply (wp_instr_u_split_data va vpn ie w (AMO (AMOSWAP, true, false, Regidx rs2, Regidx rs1, 4, Regidx rd))
              ms_v tlbvec E Φ HN Hok Hsome Hchk0 HupdN HcwL HcwH HSXL Hbit0 Hbit1 Hvalign4
              HcanonL Hvpn_defL HalignL HcanonH Hvpn_defH HalignH HnotRVC Hdec Hnlpad
              with "Hhw Hinv Hhs Hpriv Hms Htlbc Hpcr Hcode Hupt Hcfg").
    iIntros (σ Hpceq Hag Hpins) "Htlbc Hupt [Hreg [Hmem Hdev]]".
    ...
    assert (Hpa : paS = paD)
      by (unfold paS, paD; rewrite Hea; reflexivity).
    (* ---- the data-entry leaf facts at the Atomic access type ---- *)
    assert (HchkD' : forall (mxr do_sum : bool) s0,
      exec (check_PTE_permission (Atomic (AMOSWAP, Data, Data)) User mxr do_sum
        ...
        = Some (PTE_Check_Success tt, s0)).
    { rewrite upt_entry_pte. exact HchkD. }
    assert (HupdD' : update_PTE_Bits (tlb_get_pte 8 (upt_entry vpnD ieD)) (Atomic (AMOSWAP, Data, Data))
                      = (None : option (mword 64))).
    { rewrite upt_entry_pte. exact HupdD. }
    assert (HpbmtD' : forall s0, exec (tlb_get_pbmt (upt_entry vpnD ieD)) s0
                                  = Some (PBMT_PMA, s0)).
    { intros s0. exact (upt_entry_pbmt vpnD ieD s0 HpbmtD). }
    (* ---- the old window bytes are RAM and equal vold in memory ---- *)
    iAssert (⌜forall j : nat, (N.of_nat j < 4)%N ->
               σ.(mem) !! (pa_add paD j) = Some (nth_byte vold j)⌝)%I as %HbfD.
    { iIntros (j Hj).
      iDestruct (big_sepL_lookup _ _ j j with "Hwin") as "Hbj".
      { rewrite lookup_seq_lt; [reflexivity | lia]. }
      iDestruct (mem_valid with "Hmem Hbj") as %Hmj. iPureIntro. exact Hmj. }
    iAssert (⌜addr_is_ram paD⌝)%I as %HramD.
    { iDestruct (big_sepL_lookup _ _ 0%nat 0%nat with "Hwin") as "Hb0".
    ...
    pose proof (addr_is_ram_not_in_clint _ HramD) as HncD.
    pose proof (addr_is_ram_not_in_sig _ HramD) as HnsD.
    destruct (Hpma_all paD 4) as (regionD & HpmamD & _ & HreadD & HwriteD & HatomD).
    assert (HamoD : pma_allows_atomic_op
                      ((override_PMA (PMA_Region_attributes regionD) PBMT_PMA).(PMA_atomic_support))
                      AMOSWAP 4 = true)
      by (rewrite HatomD; vm_compute; reflexivity).
    assert (HrangeD : pmpRangeMatch (Z.mul (uint (zeros' 64 : mword 64)) 4)
              (Z.mul (uint (vec_access_dec (register_lookup pmpaddr_n s1.(sregs)) 0)) 4)
              (uint paD) (uint (to_bits 64 4)) = PMP_Match).
    { rewrite Lpmpa1.
      exact (ram_fetch_pmp paD (vec_access_dec pmpaddr00 0) 4 3
               ltac:(lia) ltac:(lia) ltac:(vm_compute; reflexivity) ltac:(reflexivity)
               HramD HramD7 Hpmp_cov). }
    (* ---- the AMOSWAP execute fact at s1 ---- *)
    assert (HE : exec (execute (AMO (AMOSWAP, true, false, Regidx rs2, Regidx rs1, 4, Regidx rd))) s1
             = Some (RETIRE_SUCCESS,
                     set_reg (MState s1.(sregs) (write_bytes s1.(mem) paD 4 vN) s1.(mdev))
                       (R_bitvector_64 (gpr_of_Z (uint rd)))
                       (regval_into_reg (sign_extend' 64
                          (autocast (T := mword) (vold : mword (8 * 4)) : mword (4 * 8)))))).
    { pose proof (exec_execute_AMOSWAP_4_U (upt_entry vpnD ieD) vpnD rs2 rs1 rd vold regionD s1
               HchkD' HupdD' HpbmtD' (upt_entry_match vpnD ieD)
               Lpriv1
               ltac:(rewrite Lms1; exact HSXL)
               ltac:(rewrite Lms1; exact HMPRV)
               ...
               ltac:(fold eaS paS; rewrite Hpa;
                     exact (within_sig_false paD 4 s1 HnsD ltac:(lia)))
               ltac:(fold eaS paS; rewrite Hpa;
                     exact (within_htif_false paD 4 s1 Lhtif0))
               ltac:(fold eaS paS; rewrite Hpa;
                     exact (within_htif_writable_false paD 4 s1 Lhtif0))
               ltac:(fold eaS paS; rewrite Hpa;
                     exact (addr_is_ram_not_dev _ HramD))
      ...
      exact HE0. }
    (* ---- ghost window update in lock-step with write_bytes ---- *)
    iMod (upd_window_4 σ.(mem) paD vN vold with "Hmem Hwin") as "[Hmem Hwin]".
    (* ---- ghost rd write ---- *)
    iDestruct (big_sepM_insert_acc _ _ _ _ Hmd with "Hfmap") as "[Hrdc Hfins]".
    rewrite (gpr_pt_nz rd _ Hrd).
    iMod (reg_update _ (R_bitvector_64 (gpr_of_Z (uint rd))) _
    ...
    { rewrite (gpr_pt_nz rd _ Hrd). iExact "Hrdc". }
    iModIntro.
    iExists (set_reg (MState s1.(sregs) (write_bytes s1.(mem) paD 4 vN) s1.(mdev))
               (R_bitvector_64 (gpr_of_Z (uint rd)))
               (regval_into_reg (sign_extend' 64
                  (autocast (T := mword) (vold : mword (8 * 4)) : mword (4 * 8))))).
    iSplitR; [iPureIntro; exact HE |].
    ...
    assert (Lnpc : register_lookup nextPC
             (set_reg (MState s1.(sregs) (write_bytes s1.(mem) paD 4 vN) s1.(mdev))
                (R_bitvector_64 (gpr_of_Z (uint rd)))
                (regval_into_reg (sign_extend' 64
                   (autocast (T := mword) (vold : mword (8 * 4)) : mword (4 * 8))))).(sregs)
             = add_vec_int va 4).
    { cbn [sregs]. unfold s1, set_reg; cbn [sregs].
      tmig. rewrite register_lookup_set. reflexivity. }
    iEval (rewrite Lnpc) in "Hpc'".
    ...
    rewrite /user_frame.
    iExists ms_v, sc_v, stval_v, sepc_v, (add_vec_int va 4),
            (<[Regidx rd := regval_into_reg (sign_extend' 64
               (autocast (T := mword) (vold : mword (8 * 4)) : mword (4 * 8)))]> g), tlbvec_f.
    iFrame "Hhs' Hpriv' Hms' Hsc Hstv Hsepc Htlbc Hupt Hcode Hcfg'".
    iSplitR; [iPureIntro; exact HSXL |].
    iSplitR; [iPureIntro; exact Hok_f |].
    iSplitL "Hpc' Hnpc"; [iFrame "Hpc' Hnpc" |].
    ...
    (* the old window bytes are RAM and equal vold in memory *)
    iAssert (⌜forall j : nat, (N.of_nat j < 4)%N ->
               s1.(mem) !! (pa_add paD j) = Some (nth_byte vold j)⌝)%I as %HbfD.
    { iIntros (j Hj).
      iDestruct (big_sepL_lookup _ _ j j with "Hwin") as "Hbj".
      { rewrite lookup_seq_lt; [reflexivity | lia]. }
      iDestruct (mem_valid with "Hmem Hbj") as %Hmj. iPureIntro. unfold s1; exact Hmj. }
    iAssert (⌜addr_is_ram paD⌝)%I as %HramD.
    { iDestruct (big_sepL_lookup _ _ 0%nat 0%nat with "Hwin") as "Hb0".
      { rewrite lookup_seq_lt; [reflexivity | lia]. }
      iDestruct (mem_ram with "Hb0") as %Hr0. rewrite pa_add_0 in Hr0.
    ...
    set (tlbvecD := vec_update_dec (register_lookup tlb s1.(sregs)) (tlb_hash (__id 39) vpnD)
                      (Some (u_walk_entry vpnD (uw_pte2 ieD) (uw_pte1 ieD) (uw_pte0 ieD)
                               (mword_of_int 0)))).
    set (s' := set_reg s1 tlb tlbvecD).
    (* rs2 is read at the POST-fill state s' (after the walk); transport Hrv2
       (keeping the x0 guard so the LHS matches the wrapper's [vrs2]) *)
    assert (Hrv2' : (if Z.eqb (uint rs2) 0 then zero_reg
                     else register_lookup (R_bitvector_64 (gpr_of_Z (uint rs2))) s'.(sregs))
                    = g !!! Regidx rs2)
      by (unfold s', set_reg; cbn [sregs]; rewrite irrelevant_register_set;
          [ exact Hrv2 | vm_compute; reflexivity ]).
    (* ---- ghost tlb fill s1 -> s' ---- *)
    iMod (reg_update _ tlb _ tlbvecD with "Hreg Htlbc") as "[Hreg Htlbc]".
    (* ---- the AMOSWAP execute fact at s1 (walk form, translate built inside) ---- *)
    assert (HE : exec (execute (AMO (AMOSWAP, true, false, Regidx rs2, Regidx rs1, 4, Regidx rd))) s1
             = Some (RETIRE_SUCCESS,
                     set_reg (MState s'.(sregs) (write_bytes s'.(mem) paD 4 vN) s'.(mdev))
                       (R_bitvector_64 (gpr_of_Z (uint rd)))
                       (regval_into_reg (sign_extend' 64
                          (autocast (T := mword) (vold : mword (8 * 4)) : mword (4 * 8)))))).
    { pose proof (exec_execute_AMOSWAP_4_U_walk_nomatch vpnD root rs2 rs1 rd vold regionD
               (uw_pte2 ieD) (uw_pte1 ieD) (uw_pte0 ieD) s1 satp0 entD
               H2i H2nl H1i H1nl H0i H0nl HchkD H0N HupdD
               Hrd2 Hrd1 Hrd0
               Lmisa1 Lpriv1
               ltac:(rewrite Lms1; exact HSXL)
               ltac:(rewrite Lms1; exact HMPRV)
               ltac:(rewrite Lms1; exact HMXR)
               HES1 Lsenv1 Lmenv1 Lsatp1 Hsatpmode Hasid Hroot
               ltac:(rewrite Ltlb1; exact HvaccD)
               HmatchD
               ltac:(fold eaS; rewrite Hea; exact HalignD)
               ltac:(fold eaS; rewrite Hea; exact HcanonD)
               ltac:(fold eaS; rewrite Hea; exact Hvpn_defD)
      ...
      fold eaS paS in HE0. rewrite Hpa in HE0. rewrite Hrv2' in HE0. exact HE0. }
    (* ---- ghost window update in lock-step with write_bytes ---- *)
    iMod (upd_window_4 s1.(mem) paD vN vold with "Hmem Hwin") as "[Hmem Hwin]".
    (* ---- ghost rd write ---- *)
    iDestruct (big_sepM_insert_acc _ _ _ _ Hmd with "Hfmap") as "[Hrdc Hfins]".
    rewrite (gpr_pt_nz rd _ Hrd).
    iMod (reg_update _ (R_bitvector_64 (gpr_of_Z (uint rd))) _
    ...
    { rewrite (gpr_pt_nz rd _ Hrd). iExact "Hrdc". }
    iModIntro.
    iExists (set_reg (MState s'.(sregs) (write_bytes s'.(mem) paD 4 vN) s'.(mdev))
               (R_bitvector_64 (gpr_of_Z (uint rd)))
               (regval_into_reg (sign_extend' 64
                  (autocast (T := mword) (vold : mword (8 * 4)) : mword (4 * 8))))).
    iSplitR; [iPureIntro; exact HE |].
    ...
    iExists ms_v, sc_v, stval_v, sepc_v, (add_vec_int va 4),
            (<[Regidx rd := regval_into_reg (sign_extend' 64
               (autocast (T := mword) (vold : mword (8 * 4)) : mword (4 * 8)))]> g), tlbvecD.
    iFrame "Hhs' Hpriv' Hms' Hsc Hstv Hsepc Htlbc Hupt Hcode Hcfg'".
    iSplitR; [iPureIntro; exact HSXL |].
    iSplitR; [iPureIntro;
              exact (upt_tlb_ok_fill spec (register_lookup tlb s1.(sregs)) vpnD ieD HsomeD
    ...
    assert (HE : exec (execute (AMO (AMOSWAP, true, false, Regidx rs2, Regidx rs1, 4, Regidx rd))) s1
             = Some (RETIRE_SUCCESS,
                     set_reg (MState s'.(sregs) (write_bytes s'.(mem) paD 4 vN) s'.(mdev))
                       ...
                       (regval_into_reg (sign_extend' 64
                          (autocast (T := mword) (vold : mword (8 * 4)) : mword (4 * 8)))))).
    { pose proof (exec_execute_AMOSWAP_4_U_walk vpnD root rs2 rs1 rd vold regionD
               (uw_pte2 ieD) (uw_pte1 ieD) (uw_pte0 ieD) s1 satp0
               H2i H2nl H1i H1nl H0i H0nl HchkD H0N HupdD
               Hrd2 Hrd1 Hrd0
               Lmisa1 Lpriv1
               ltac:(rewrite Lms1; exact HSXL)
