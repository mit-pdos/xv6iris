...
Section ProofFiledup.
  ...
  Lemma wp_filedup_sconf
    ...
    : wp_filedup_sconf_body kt γl γf k q Cf m n eb p K b lks.
  Proof.
    ...
    (* ===== +0x14 jal ra,acquire ===== *)
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.filedup + 0x14)) Rra (mword_of_int 0x1fcb1a : mword 21)
    ...
    assert (Htgtacq : add_vec (mword_of_int (KernelSyms.filedup + 0x14) : mword 64)
                        (sign_extend' 64 (mword_of_int 0x1fcb1a : mword 21))
    ...
    iDestruct (cpu_own_transport CID CID9 n eb p b ltac:(wp_next_chain)
                 with "Hcnt") as "Hcnt".
    iApply (Acquire.wp_acquire_sconf kt γl "ftable"%string (ftable_res γf) mA
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.filedup + 0x2a)) Rra (mword_of_int 0x1fcb8c : mword 21)
    ...
    assert (Htgtrel : add_vec (mword_of_int (KernelSyms.filedup + 0x2a) : mword 64)
                        (sign_extend' 64 (mword_of_int 0x1fcb8c : mword 21))
    ...
    iApply (Release.wp_release_sconf kt γl ftable_addr "ftable"%string (ftable_res γf) D5
              n eb p (K - 4)%nat
              ({["ftable"]} ∪ lks)
