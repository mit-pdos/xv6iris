(* ProofSysOpenAlloc.v -- +0x5e .. +0x84 AND THE +0x140 FD_DEVICE BLOCK,
   ...
   [ProofSysOpenShared.so_arm_fail] at the landed tail's own payout, and
...
Module SysOpenAlloc (Iunlock : IUNLOCK) (Iunlockput : IUNLOCKPUT)
                      (EndOp : END_OP) (Fileclose : FILECLOSE)
                      (Itrunc : ITRUNC) (Filealloc : FILEALLOC)
                      (Fdalloc : FDALLOC).

Module Stores := SysOpenStores Iunlock Iunlockput EndOp Fileclose Itrunc.
Module Tails := SysOpenTails Iunlock Iunlockput EndOp Fileclose.

Section ProofSysOpenAlloc.
