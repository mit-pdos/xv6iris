...
Section IputTail.
  ...
  Lemma ip_tail_exit `{GEN : GenId} `{CID : CpuId} (CID0 : CPU)
      ...
      (sp0 vg4 : mword 64) (lks : gset string) :
    ...
    sie_cap_gpr kt D (trap_res eb + (K - 4))%nat false pj -∗
  ...
  Proof.
    ...
    iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.iput + 0x2a)) Ra0 Ra0
              (mword_of_int 1346 : mword 12) D3 (trap_res eb + (K - 4))%nat false
              ltac:(nz) ltac:(rdok) with "Hcg Hpc Hi2a").
    iApply wp_next_off_intro. iIntros "Hcg Hpc".
    iEval (rgne) in "Hcg".
    set (D4 := <[Regidx Ra0 := regval_into_reg
                  (add_vec (D3 !!! Regidx Ra0)
                     (sign_extend' 64 (mword_of_int 1346 : mword 12)))]> D3).
    ...
    iApply (Release.wp_release_sconf kt gtl itable_lock "itable"%string
              (itable_res2 cn gfs gi cov logstart nib dev) D5
              0%nat eb pj (K - 4)%nat ({["itable"]} ∪ lks)
  ...
  Proof.
    ...
    - (* ---- THE LAST CLOSE: REF-1, and the EVICTION (§13.9) ---- *)
      ...
      iApply (ip_tail_exit CID0 j bn g gfs gi cn gtl cov logstart bmapstart inodestart
                nib size dev used used' Sb Sb' k n n' spf wb crb0 pidv dq dqb dqs m D2 K eb sp0 vg4 lks
...
Section ProofIput.
  ...
  Lemma wp_iput_gen
    ...
    : wp_iput_gen_body kt gs j gl gu gd gk pd pav pu bn g gfs gi cn gtl gil gisl
  ...
  Proof.
    ...
    iDestruct (cpu_own_transport CID CID9 0%nat eb pj eb ltac:(wp_next_chain)
                 with "Hcnt") as "Hcnt".
    iApply (Acquire.wp_acquire_sconf kt gtl "itable"%string
    ...
    iApply (ASL.wp_acquiresleep_nb_sconf kt (dq := dq) j gil gisl "inode"%string
    ...
    iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.iput + 0x58)) Ra0 Ra0
              (mword_of_int 1300 : mword 12) H1 (trap_res eb + (K - 4))%nat false
    ...
    iModIntro.
    iApply (IT.wp_itrunc_gen kt gs j gl gu gd gk pd pav pu bn g gfs gi
              cov logstart bmapstart inodestart nib size dev used
    ...
    iApply (IU.wp_iupdate_credgen kt gs j gl gu gd gk pd pav pu bn g gfs gi
    ...
    iDestruct (cpu_own_transport CIDiu CIDm6 0%nat eb pj eb
                 ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
    iApply (RS.wp_releasesleep_gen_sconf kt gs gil gisl "inode"%string (ic_tok cn k)
              (slh_tok (icfg_isl k)) q
              J6 pidv pj (K - 4)%nat eb eb lks ltac:(lia)
    ...
    iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.iput + 0x7e)) Ra0 Ra0
              (mword_of_int 1262 : mword 12) J7 (K - 4)%nat eb
  ...
  Lemma wp_iput_sconf
      ...
      (m : regfile) (K : nat) (eb : bool)
      (b : bool) (lks : gset string)
    : wp_iput_sconf_body kt gs j gl gu gd gk pd pav pu bn g gfs gi cn gtl gil gisl
                          cov logstart bmapstart inodestart nib size dev used
