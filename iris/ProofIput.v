...
Section IputTail.
  ...
  Definition ip_window `{XI : CurCtx} (cn : ic_names) (γfs : fs_names) (γi : gname)
  ...
  (* the tail's slot rows: the closer's reference minus its fragment, then
     the open window at ref 1 or the whole rows with the fragment otherwise *)
  Definition ip_rows `{XI : CurCtx} (cn : ic_names) (γfs : fs_names) (γi : gname)
      (cov : gset Z) (logstart : Z) (k : nat)
      (Mt : gmap nat (Qp * positive)) (ci : gmap nat (mword 32 * mword 32))
      (q : Qp) (dev inum : mword 32) : iProp Σ :=
  ...
  Lemma ip_rows_one `{XI : CurCtx} cn γfs γi cov logstart k Mt ci q dev inum (q1 : Qp) :
    Mt !! k = Some (q1, 1%positive) ->
    ip_rows cn γfs γi cov logstart k Mt ci q dev inum ⊣⊢
    (iref_frag k q ∗ live_fracc k q ∗ slh_tok (icfg_isl k) q ∗
     inode_ident k (DfracOwn q) dev inum) ∗
  ...
  Lemma ip_rows_ne `{XI : CurCtx} cn γfs γi cov logstart k Mt ci q dev inum (q1 : Qp) (cnt : positive) :
  ...
  Lemma ip_tail_exit `{GEN : GenId} `{CID : CpuId} `{XI : CurCtx} (CID0 : CPU)
    ...
    kernel_text -∗
    is_lock gtl itable_lock "itable"%string (fun ξ => itable_res2 ξ cn gfs gi cov logstart nib dev) -∗
    ...
    cpu_claim_ext eb pj -∗
    locked gtl cpu_id -∗
    itable_res2_llb TsoCtx.cur_ctx cn gfs gi cov logstart nib dev -∗
    iref_slot -∗
  ...
  Proof.
    ...
    iIntros "#Htext #Hlock Hpc Hcg Hcnt Hpay Hextc Hextm Htok HRres Hislot Hgreg
             Hr24 Hr16 Hr8 Hg4 Hg5 Hg6 Hppid Hbms Hins Hbslots Hop Hcont".
    ...
    iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.iput + 0x28)) Ra0 Ra0
              (mword_of_int 1332 : mword 12) D3 (trap_res eb + (K - 6))%nat false
              ltac:(nz) ltac:(rdok) with "Hcg Hpc []").
    ...
    set (D4 := <[Regidx Ra0 := regval_into_reg
                  (add_vec (D3 !!! Regidx Ra0)
                     (sign_extend' 64 (mword_of_int 1332 : mword 12)))]> D3).
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.iput + 0x2c)) Rra
              (mword_of_int 2087054 : mword 21) D4 (trap_res eb + (K - 6))%nat false
    ...
    assert (Htgtrel : add_vec (mword_of_int (KernelSyms.iput + 0x2c) : mword 64)
                        (sign_extend' 64 (mword_of_int 2087054 : mword 21))
    ...
    iApply (RLI.wp_release_in_sconf KT1 gtl itable_lock "itable"%string (fun ξ => itable_res2 ξ cn gfs gi cov logstart nib dev) D5
              0%nat eb pj (K - 6)%nat ({["itable"]} ∪ lks)
              ltac:(rewrite HD5a0; reflexivity) ltac:(lia)
              with "Hcg Htext Hpc [Hlock] Htok [HRres] Hcnt Hpay").
    { iExact "Hlock". }
    { iIntros "Hrun".
      iApply (itable_pay_intro cn gfs gi cov logstart nib dev
                with "Hrun HRres"). }
    ...
    iApply ("Hcont" $! P4 n' Sb' wb
              with "[%] Hcg Hcnt Hextc Hextm Hpc Hppid Hbms Hins Hbslots [%] [%] [%] [%] Hop Hislot Hgreg").
  ...
  Lemma ip_tail `{GEN : GenId} `{CID : CpuId} `{XI : CurCtx} (CID0 : CPU)
    ...
    ip_rows cn gfs gi cov logstart k Mt ci q dev inum -∗
    iref_slots_auth -∗
    isl_pool Mt -∗
  ...
  Proof.
    ...
    iIntros "#Htext #Hlock #Hinv #Hesc #Hireg Hpc Hcg Hcnt Hpay Hextc Hextm Htok
             #Hclaims Hhalf Hrows Hiauth Hipool Hslots Hpool Hru Hgreg Hr24 Hr16 Hr8 Hg4 Hg5 Hg6
    ...
    iDestruct "Hslot" as "(Hrest & Hiu & Hcnt1 & Hpark)".
    ...
    - (* ---- THE LAST CLOSE: REF-1, and the EVICTION (§13.9) ---- *)
      ...
      iApply fupd_wp.
      iMod (frz_park_ref1_off ⊤ k (bv_unsigned inum) qt gip loip
      ...
      { (* the AU: the LAST-CLOSE twin, ∀P pass-through *)
        iMod (IcacheInv.iref_close_last_store_pinw_au
                (⊤ ∖ ↑minstretN) gi gfs inodestart nib
      ...
      { rewrite /ic_hdr /ic_hdr_amb. iSplitR; [done |].
        ...
        iSplitL "Hid"; [iExists dev, inum; iExact "Hid" |]. iExact "Hnlk". }
      iMod (ic_decr cn gfs gi cov logstart k (SlotReg Tb false None None) 0 None ⊤
      ...
      iAssert (ipool_shape gfs gi cov logstart inum) with "[Hcnt0 Hmirf Hnp Hoff]" as "Hbundle".
      { rewrite /ipool_shape. iSplitL "Hcnt0"; [iExact "Hcnt0" |].
        iSplitL "Hmirf"; [iExact "Hmirf" |].
      ...
      iApply (ip_tail_exit CID0 j bn g gfs gi cn gtl cov logstart bmapstart inodestart
                nib size dev Sb Sb' k n n' spf wb crb0 pidv dq dqb dqs m D2 K eb sp0 vg4 vg5 vg6 lks Vpr rg
                HK Hanch Hsp0 HD2regs Hlo Hhi Hssub Hwm Hwc Hfresh
                with "Htext Hlock Hpc Hcg Hcnt Hpay Hextc Hextm Htok [-Hiu Hgreg Hr24 Hr16 Hr8 Hg4 Hg5 Hg6 Hppid Hbms Hins Hbslots Hop Hcont] Hiu Hgreg
                      Hr24 Hr16 Hr8 Hg4 Hg5 Hg6 Hppid Hbms Hins Hbslots Hop Hcont").
    ...
    - (* ---- THE NON-LAST CLOSE: count down, the box's (d) ---- *)
      ...
      iDestruct "Hrows" as "[Hstamps Hrst]".
      (* the payload's slot row: the count store below forfeits its floor
      ...
      { (* the AU: the NOT-LAST close twin *)
        ...
        iMod (IcacheInv.iref_close_store_pinw_au (⊤ ∖ ↑minstretN)
                gi gfs inodestart nib Mt k inum false q qt qrest npred
                gip loip tstk
      ...
      iMod (ic_decr cn gfs gi cov logstart k r (S c0) (Some (dev, inum)) ⊤
      ...
      iDestruct ("Hback" $! (<[k := (qrest, npred)]> Mt) ci
                   with "[%] [%] [Hrest Hiu Hcnt1 Hpark]") as "Hslots".
      { intros i Hi. rewrite lookup_insert_ne; [reflexivity | by apply not_eq_sym]. }
      { intros i Hi. reflexivity. }
      ...
      iApply (ip_tail_exit CID0 j bn g gfs gi cn gtl cov logstart bmapstart inodestart
                nib size dev Sb Sb' k n n' spf wb crb0 pidv dq dqb dqs m D2 K eb sp0 vg4 vg5 vg6 lks Vpr rg
                HK Hanch Hsp0 HD2regs Hlo Hhi Hssub Hwm Hwc Hfresh
                with "Htext Hlock Hpc Hcg Hcnt Hpay Hextc Hextm Htok [-Hislot Hgreg Hr24 Hr16 Hr8 Hg4 Hg5 Hg6 Hppid Hbms Hins Hbslots Hop Hcont] Hislot Hgreg
                      Hr24 Hr16 Hr8 Hg4 Hg5 Hg6 Hppid Hbms Hins Hbslots Hop Hcont").
...
Section IputFreePath.
  ...
  Lemma ip_free_offlock `{GEN : GenId} `{CID0 : CpuId} `{XI : CurCtx}
      ...
      (m : regfile) (K : nat) (eb : bool) (b : bool) (lks : gset string) (Vpr : pprivate) (rg : bool) :
    let pj := proc_addr j in
  ...
  Lemma ip_free_locked `{GEN : GenId} `{CID0 : CpuId} `{XI : CurCtx}
      ...
      (bn : bio_names)
      (γ : log_names) (γfs : fs_names) (γi : gname)
      (cn : ic_names) (gtl gil gisl g1 g2 : gname)
      (cov : gset Z) (logstart bmapstart inodestart : Z) (nib : nat)
      (size : Z) (dev : mword 32)
      (k : nat) (q : Qp) (inum : mword 32)
      ...
      (pidv : mword 32) (dq dqb dqs : dfrac)
      (sp0 vra vs0 vs1 vs2 vs3 vs4 : mword 64)
      (m : regfile) (K : nat) (eb b : bool) (lks : gset string) (Vpr : pprivate) (rg : bool)
      (td T0 Kw : nat) :
    let ip := ientry k in
    let pj := proc_addr j in
    ...
    log_ctx γ bn γfs cov logstart dev -∗
    (* the itable, HELD *)
    is_lock gtl itable_lock "itable"%string (fun ξ => itable_res2 ξ cn γfs γi cov logstart nib dev) -∗
    itable_inv -∗
    ...
    ic_regd k (SlotReg td true (Some (dev, inum)) (Some (IcLoaded g1 dn bm, T0))) -∗
    TsoGhost.llb loglen_name td -∗
    ⌜(T0 <= Kw)%nat⌝ -∗
    TsoCtx.ctx_floor TsoCtx.cur_ctx Kw -∗
    ic_cnt k 1 -∗
    i_valid (ientry k) ↦₄ valid_word true -∗
    IcacheRef.inode_ident k (DfracOwn (1/2)) dev inum -∗
  ...
  Proof.
    ...
    iIntros "Hcg Hcnt Hpay Hextc Hclm #Htext #Hkd Hpc #Hpenv #Hbio #Hlctx
             ...
             Hvld Hid Hnl Hrcpt Hsele Hiauth Hipool Hpool %Hcik Hslots Hrtok
    ...
    iApply (ASL.wp_acquiresleep_nb_genl_llb_sconf j gil gisl "inode"%string (ic_slp cn k)
              (icfg_isl k) q R0 pidv Vpr (trap_res eb + (K - 6))%nat eb 0%nat
              ({["itable"]} ∪ lks) 0%nat
    ...
    iDestruct "Hslp" as (s0) "(Hictok & Hrp & %Hs0 & #Hflp)".
    iApply fupd_wp.
    iDestruct (SieCapCtx.sie_cap_gpr_own_ctx_acc with "Hcg") as "[Hrun Hcgb]".
    iMod (ic_free_take cn γfs γi cov logstart k TsoCtx.cur_ctx
            (SlotReg td true (Some (dev, inum)) (Some (IcLoaded g1 dn bm, T0)))
            dev inum (IcLoaded g1 dn bm) T0 Kw s0 ⊤
            ltac:(solve_ndisj) eq_refl eq_refl eq_refl HTKw Hs0
            with "Hesc Hrun Hflw Hreg Hc Hictok Hrp")
      as "(Hrun & Hrest & Hreg & Hc & Hhold)".
    iDestruct ("Hcgb" with "Hrun") as "Hcg".
    iModIntro.
    ...
    { rewrite /itable_slot_res_llb /ic_slot_row_llb /icM_count HMk1 Hcik.
      iSplitL "Hreg Hc".
      { rewrite /ic_slot_row. iExists td. iSplitL; [| iExact "Hllbd"].
        iExists (SlotReg td false (Some (dev, inum)) None). iFrame "Hreg Hc Hllbd".
        iPureIntro. cbn. split_and!; [done | done | done | lia]. }
      iExact "Hstk". }
    iAssert (itable_res2_llb TsoCtx.cur_ctx cn γfs γi cov logstart nib dev)
    ...
    iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.iput + 0x62)) Ra0 Ra0
              (mword_of_int 1274 : mword 12) H1 (trap_res eb + (K - 6))%nat false
              ltac:(nz) ltac:(rdok) with "Hcg Hpc []").
    ...
    set (H2 := <[Regidx Ra0 := regval_into_reg
                  (add_vec (H1 !!! Regidx Ra0)
                     (sign_extend' 64 (mword_of_int 1274 : mword 12)))]> H1).
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.iput + 0x66)) Rra
              (mword_of_int 2086996 : mword 21) H2 (trap_res eb + (K - 6))%nat false
              ltac:(nz) ltac:(rdok) ltac:(vm_compute; reflexivity)
              with "Hcg Hpc []").
    ...
    assert (Htgtrl : add_vec (mword_of_int (KernelSyms.iput + 0x66) : mword 64)
                       (sign_extend' 64 (mword_of_int 2086996 : mword 21))
                     = mword_of_int KernelSyms.release) by pcw.
    ...
    iApply (RLI.wp_release_in_sconf KT1 gtl itable_lock "itable"%string (fun ξ => itable_res2 ξ cn γfs γi cov logstart nib dev) H3
              0%nat eb pj (K - 6)%nat ({["itable"]} ∪ lks)
              ltac:(rewrite HH3a0; reflexivity) ltac:(lia)
              with "Hcg Htext Hpc [Hitlk] Htok [HRres] Hcnt Hpay").
    { iExact "Hitlk". }
    { iIntros "Hrun".
      iApply (itable_pay_intro cn γfs γi cov logstart nib dev
                with "Hrun HRres"). }
    ...
    (* ---- the bundle in hand, unpacked for itrunc: the rest's cells at the
       ...
       nlink cell, and the ghost's data ---- *)
    iEval (rewrite /ic_rest /=) in "Hrest".
    iDestruct "Hrest" as "(%Hlen13 & Hmetar & Haddrs)".
    iDestruct (ic_loaded_ghost_open with "Hlg") as (data2)
      "(%Hok2 & %Hrl2 & %Hdok2 & %Hddix2 & %Hdoc2 & %Hduq2 & Hdlk2 & Hdat & Hind & Hblks
        & Hdv2 & Hfv2 & Htop2)".
    ...
    iApply fupd_wp.
    iDestruct (SieCapCtx.sie_cap_gpr_own_ctx_acc with "Hcg") as "[Hrun Hcgb]".
    iMod (ic_park_hold cn γfs γi cov logstart k TsoCtx.cur_ctx dev inum 1%Qp ⊤
            ltac:(solve_ndisj)
            with "Hesc Hrun [Hvld Hidv Hinh Hnl Hrcpt Hsele Hty Hmaj Hmin Hsz Haddrs] Hhold")
      as "(Hrun & Hictok & %Tp & Hrp & Href & #HllbT)".
    { iExists (IcUnloaded g2). rewrite /ic_hdr /ic_hdr_amb /ic_pay /ic_rest /=.
      iSplitL "Hvld Hidv Hinh Hnl Hrcpt Hsele".
      { iFrame "Hvld".
        iSplitL "Hidv Hinh"; [rewrite /IcacheRef.inode_ident; iFrame "Hidv Hinh" |].
        iSplitL "Hnl"; [iExists _; iExact "Hnl" |].
        iRight. iFrame "Hrcpt Hsele". }
    ...
    iApply (RS.wp_releasesleep_genin_sconf γs gil gisl "inode"%string (ic_slp cn k)
              (fun _ => ic_slp_dep cn k Tp) (slh_tok (icfg_isl k)) q J6 pidv pj (K - 6)%nat eb eb lks Tp
              ltac:(lia) ltac:(lkbelow)
              (ic_slp_fold cn k Tp)
              with "Hcg Hcnt Htext Hpc [] [Hstok] HllbT [Hictok Hrp] Hprocs").
    all: try lkbelow.
    { iEval (rewrite HJ6a0). iExact "Hslk". }
    { iEval (rewrite HJ6a0). iExact "Hstok". }
    { rewrite /ic_slp_dep. iFrame "Hictok Hrp". }
    ...
    iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.iput + 0x7e)) Ra0 Ra0
              (mword_of_int 1246 : mword 12) J7 (K - 6)%nat eb
              ltac:(nz) ltac:(rdok) with "Hcg Hpc []").
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.iput + 0x82)) Rra
              (mword_of_int 2086832 : mword 21) J8 (K - 6)%nat eb
              ltac:(nz) ltac:(rdok) ltac:(vm_compute; reflexivity)
              with "Hcg Hpc []").
    ...
    assert (Htgtac2 : add_vec (mword_of_int (KernelSyms.iput + 0x82) : mword 64)
                        (sign_extend' 64 (mword_of_int 2086832 : mword 21))
    ...
    iApply (Acquire.wp_acquire_llb_sconf KT1 gtl "itable"%string (fun ξ => itable_res2 ξ cn γfs γi cov logstart nib dev) J9
    ...
    { iEval (rewrite /islot_rest_at) in "Hrest".
      ...
      iApply (inode_ident_agree with "Hrest Hrident"). }
    iApply fupd_wp.
    iMod (icnt_freeze_forces_one ⊤ γi γfs inodestart nib inum (Pos.to_nat cnt2) rg
    ...
    iApply fupd_wp.
    iDestruct (SieCapCtx.sie_cap_gpr_own_ctx_acc with "Hcg") as "[Hrun Hcgb]".
    iMod (ic_guard_withdraw cn γfs γi cov logstart k TsoCtx.cur_ctx r2 dev inum tb2 Kt2 ⊤
            ltac:(solve_ndisj) Hrw2 Hrid2 Hrle2 with "Hesc Hrun Hflb2 Hflt2 Hreg Hc [Href]")
      as "(Hrun & Hc & %x0 & %T0' & %Hx0 & %HT0' & Hreg & Hhdr)".
    { iExists {[(Some (dev, inum), Tp) := 1%Qp]}. iFrame "Href". iPureIntro. split.
      - rewrite CtxBox.qsum_singleton. reflexivity.
      - rewrite CtxBox.max_stamp_singleton. exact HKt2. }
    iDestruct ("Hcgb" with "Hrun") as "Hcg".
    rewrite /ic_hdr /ic_hdr_amb. iDestruct "Hhdr" as "(Hvld & Hid & Hnlk & Hpayl)".
    iAssert (ifreeze_pre rg (bv_unsigned inum) ∗ frzown (bv_unsigned inum) ∗
             frzsel k ((1/2)/2)%Qp true ∗ (∃ n : bv 16, i_nlink (ientry k) ↦₂ n))%I
      with "[Hpayl Hnlk Hpre]" as "(Hpre & Hrcpt & Hsele & Hnlk)".
    { destruct x0 as [| g0 | g0 dn0 bm0]; [exfalso; exact (Hx0 eq_refl) | |]; rewrite /ic_pay.
      - iDestruct "Hpayl" as "[(_ & _ & Hoff & _) | [Hrcpt Hsele]]".
        + rewrite /ifreeze_pre /ifreeze_off.
          iDestruct (IcacheRef.ifreeze_excl with "Hpre Hoff") as %[].
        + iFrame "Hpre Hrcpt Hsele". iExact "Hnlk".
      - iDestruct "Hpayl" as "[(_ & _ & Hoff & _) | [Hrcpt Hsele]]".
        + rewrite /ifreeze_pre /ifreeze_off.
          iDestruct (IcacheRef.ifreeze_excl with "Hpre Hoff") as %[].
        + iFrame "Hpre Hrcpt Hsele". iExists (di_nlink dn0). iExact "Hnlk". }
    iModIntro.
    assert (Hinreg : bv_unsigned inum ∈ region_inums nib).
    { apply region_inums_spec. split; [apply bv_unsigned_in_range |].
    ...
    iApply fupd_wp.
    iDestruct (SieCapCtx.sie_cap_gpr_own_ctx_acc with "Hcg") as "[Hrun Hcgb]".
    iMod (ic_evict_deposit cn γfs γi cov logstart k TsoCtx.cur_ctx
            (SlotReg (sr_td r2) true (Some (dev, inum)) (Some (x0, T0'))) x0 T0' ⊤
            ltac:(solve_ndisj) eq_refl eq_refl
            with "Hesc Hrun Hreg Hc [Hvld Hid Hnlk]")
      as "(Hrun & %Tb & Hreg & Hc & Hst0 & #HllbTb)".
    { rewrite /ic_hdr /ic_hdr_amb. iSplitR; [done |].
      iSplitL "Hvld"; [iExists _; iExact "Hvld" |].
      iSplitL "Hid"; [iExists dev, inum; iExact "Hid" |]. iExact "Hnlk". }
    iMod (ic_decr cn γfs γi cov logstart k (SlotReg Tb false None None) 0 None ⊤
    ...
    iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.iput + 0x90)) Ra0 Ra0
              (mword_of_int 1228 : mword 12) G1 (trap_res eb + (K - 6))%nat false
              ltac:(nz) ltac:(rdok) with "Hcg Hpc []").
    ...
    set (G2 := <[Regidx Ra0 := regval_into_reg
                  (add_vec (G1 !!! Regidx Ra0)
                     (sign_extend' 64 (mword_of_int 1228 : mword 12)))]> G1).
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.iput + 0x94)) Rra
              (mword_of_int 2086950 : mword 21) G2 (trap_res eb + (K - 6))%nat false
              ltac:(nz) ltac:(rdok) ltac:(vm_compute; reflexivity)
              with "Hcg Hpc []").
    ...
    assert (Htgtrl2 : add_vec (mword_of_int (KernelSyms.iput + 0x94) : mword 64)
                        (sign_extend' 64 (mword_of_int 2086950 : mword 21))
                      = mword_of_int KernelSyms.release) by pcw.
    ...
    iApply (RLI.wp_release_in_sconf KT1 gtl itable_lock "itable"%string (fun ξ => itable_res2 ξ cn γfs γi cov logstart nib dev) G3
              0%nat eb pj (K - 6)%nat ({["itable"]} ∪ lks)
    ...
    assert (Hsbadr : add_vec (rget P2 Ra1)
                       (sign_extend' 64 (mword_of_int 1204 : mword 12))
                     = sb_inodestart).
    ...
    iApply (wp_lw_s_sconf (kt := KT1) (ktd := KT0)
              ...
              (mword_of_int 1204 : mword 12) P2 (K - 6)%nat
  ...
  Lemma ip_free_entry `{GEN : GenId} `{CID0 : CpuId} `{XI : CurCtx}
    ...
    IcacheRef.inode_ref_at k q dev inum mst -∗
    ...
    ((* ===== EXIT A: pc +0x20, the ip_tail seam (valid==0 OR nlink!=0);
     ...
     (∀ (M' : regfile) (vg4' vg5' vg6' : mword 64),
       ...
       locked gtl cpu_id -∗
       itable_half Mt -∗
       (* R3 / F28: the guard's OPEN WINDOW rides to the tail *)
       ip_rows cn γfs γi cov logstart k Mt ci q dev inum -∗
       iref_slots_auth -∗
       isl_pool Mt -∗
       ([∗ list] i0 ∈ seq 0 NINODE, islot2 cn Mt ci i0) -∗
    ...
    (∀ (M5 : regfile) (g1 g2 : gname) (dn : dinode) (bm : blkmap)
       ...
       (* R3: the table's slot rows go whole (the frozen park inside) *)
       ([∗ list] i0 ∈ seq 0 NINODE, islot2 cn Mt ci i0) -∗
       (* the REDUCED reference (its live slice froze into the table); no
          fresh fragment -- with (g) the unit's stays in the window (F30) *)
       (iref_frag k q ∗ slh_tok (icfg_isl k) q ∗
        IcacheRef.inode_ident k (DfracOwn q) dev inum) -∗
       (* the LOADED payload's ghost side, in hand (the cells ride the box);
          the old generation's shot and the regenerated one's pending token *)
       ic_loaded_ghost γfs γi cov logstart inum dn bm -∗
       ity_shot g1 (di_type dn) -∗
       IcacheRef.ity_pending g2 -∗
  ...
  Proof.
    ...
    1:{ (* valid == 0 : UNLOADED -- the window stays open into the tail's
      ...
      iDestruct ("Hback" $! Mt ci with "[%] [%] [Hrest Hiu Hcnt1 Hmirf Hself]") as "Hslots";
        [ intros i Hi; reflexivity | intros i Hi; reflexivity | | ].
      { rewrite /islot2 HMk1 Hcik. iFrame "Hiu Hcnt1".
      ...
      iApply ("HcA" $! F1 vg4 vg5 vg6
                with "[%] [%] Hcg Hcnt Hpay Hextc Hclm Hpc Htok Hhalf
                      [Hstampsback Hstk Hreg Hc Hhdr Hrfrg1 Hrlv1 Hrslh1 Hrident] Hiauth Hipool
                      Hslots Hpool Hropen Hbms Hins
                      Hppid Hbslots Hvlb
                      Hcrd Hop Hr1 Hr2 Hr3 Hg4 Hg5 Hg6").
      { exact HF1regs. }
      { exact HF1a5. }
      { rewrite (ip_rows_one cn γfs γi cov logstart k Mt ci q dev inum q HMk1).
    ...
    (* ===== valid == 1: LOADED -- fall through with the header in hand.
       ...
       carries the receipt, refuted by the region against the mirror's
       [false] half this walk holds ([InodeRegion.ireg_frzown_off_absurd]). *)
    rewrite /ic_hdr_amb. iDestruct "Hhdr" as "(Hvld & Hid & Hnl & Hpayh)".
    iApply fupd_wp.
    iDestruct "Hpayh" as "[(Hlg & #Hshot & Hoff & Hlvh) | [Hfo _]]"; last first.
    { iMod (ireg_frzown_off_absurd ⊤ γi γfs inodestart nib inum ltac:(solve_ndisj) Hnib
              with "Hireg Hmirf Hfo") as "[]". }
    iModIntro.
    ...
    iDestruct (ic_loaded_ghost_open with "Hlg") as (data)
      "(%Hok & %Hrl & %Hdok & %Hddix & %Hdoc & %Hduq & Hdlk & Hdat & Hind
        & Hblks & Hdv & Hfv & Htop)".
    ...
    { (* nlink != 0 : re-park at PARKED and take the tail through 0xcc/0xce/0xd0.
      ...
      (* ---- R3: the header re-forms (the ghost re-packs, the nlink cell goes
         ...
         the table's row goes back whole with the mirror's [false] half ---- *)
      iAssert (ic_loaded_ghost γfs γi cov logstart inum dn bm)
        with "[Hdlk Hdat Hind Hblks Hdv Hfv Htop]" as "Hlg".
      { iApply (ic_mk_loaded_ghost _ _ _ _ _ _ _ data Hok Hrl Hdok Hddix Hdoc Hduq
                  with "Hdlk Hdat Hind Hblks Hdv Hfv Htop"). }
      iAssert (IcacheInv.iref_tok_genlo k q gfe lofe)
        with "[Hrfrg Hrlv Hrslh]" as "Hrtok".
      { rewrite /IcacheInv.iref_tok_genlo. iFrame. }
      iAssert (ic_hdr γfs γi cov logstart k (Some (dev, inum)) (IcLoaded ga dn bm) TsoCtx.cur_ctx)
        with "[Hvld Hid Hnl Hlg Hoff Hlvh]" as "Hhdr".
      { rewrite /ic_hdr /ic_hdr_amb /ic_pay. iFrame "Hvld Hid Hnl".
      ...
      iApply ("HcA" $! G2 (m !!! Regidx Rs2) vg5 (m !!! Regidx Rs4)
                with "[%] [%] Hcg Hcnt Hpay Hextc Hclm Hpc Htok Hhalf
                      [Hstampsback Hstk Hreg Hc Hhdr Hrfrg1 Hrlv1 Hrslh1 Hrd Hrn] Hiauth Hipool
                      Hslots Hpool Hropen Hbms Hins
                      Hppid Hbslots Hvlb
                      Hcrd Hop Hr1 Hr2 Hr3 Hg4 Hg5 Hg6").
      { exact HG2regs. }
      ...
      { rewrite (ip_rows_one cn γfs γi cov logstart k Mt ci q dev inum q HMk1).
        ...
        rewrite /ip_window. iFrame "Hstampsback".
        iSplitL "Hstk". { iExists tstk. iFrame "Hstk Hllbk Hflk". }
        iExists (IcLoaded ga dn bm), (sr_td r). iFrame "Hreg Hc".
        iSplitR; [iPureIntro; discriminate |]. iSplitR; [iExact "Hllbr" |].
        iExact "Hhdr". } }
    ...
    iApply fupd_wp.
    iMod (ireg_freeze_au ⊤ γi γfs inodestart nib inum dn rg
    ...
    iDestruct ("Hback" $! Mt ci with "[%] [%] [Hrest Hiu Hcnt1 Hpark]") as "Hslots";
      [ intros i Hi; reflexivity | intros i Hi; reflexivity | | ].
    { rewrite /islot2 HMk1 Hcik. iFrame "Hiu Hcnt1 Hpark". iExact "Hrest". }
    ...
    iApply ("HcB" $! F7 gfe ga' dn bm data (sr_td r) T0 Kw
              with "[%] [%] [%] [%] [%] [%] [%] [%] [%] [%] [%] [%] [%]
                    ...
                    [%] Hflw Hc Hvld Hid Hnl Hrcpt Hsele Hiauth Hipool Hpool
                    [%] Hslots [Hrfrg Hrslh Hrd Hrn] Hlg Hshot Hpend Hpre Hbms Hins
...
Section ProofIput.
  ...
  Proof.
    ...
    (* ===== +0x14 jal ra,acquire ===== *)
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.iput + 0x14)) Rra (mword_of_int 2086942 : mword 21)
    ...
    assert (Htgtacq : add_vec (mword_of_int (KernelSyms.iput + 0x14) : mword 64)
                        (sign_extend' 64 (mword_of_int 2086942 : mword 21))
                      = mword_of_int KernelSyms.acquire) by pcw.
    ...
    iApply (Acquire.wp_acquire_llb_sconf KT1 gtl "itable"%string (fun ξ => itable_res2 ξ cn gfs gi cov logstart nib dev) mA
    ...
    iDestruct (is_itable2_lock with "Hitab") as "#Hitlk0".
    (* THE ADDRESS CLAIM, READ OFF THE CELL ITSELF (the standing per-node
       rule: never from a static bundle).  [WpAu4]'s wrapped leaves take
       [WpSconfMem.wordw_claim] beside the (linear) atomic update, so the
       window's mapping, alignment, canonicality and RAM-ness have to arrive
    ...
    { (* ---- ref != 1: fall through straight into the tail ---- *)
      ...
      iApply (ip_tail (CID := CIDacq) CID j bn g gfs gi cn gtl cov logstart bmapstart
                ...
                with "Htext Hitlk0 Hinv Hesc Hireg Hpc Hcg Hcnt Hpay Hextc Hextm Htok Hclaims0 Hhalf
                      [Hstamps Hrfrag Hrlv Hrslh Hrident Hrefm] Hiauth Hipool Hslots
                      Hpool Hru Hropen Hr24 Hr16 Hr8
                      Hg4 Hg5 Hg6 Hppid Hbms Hins
                      Hbslots Hop Hcont").
      rewrite (ip_rows_ne cn gfs gi cov logstart k Mt ci q dev inum qt cnt HMk Hcnone).
    ...
    iApply (ip_free_entry gs j gl gu gd gk pd pav pu bn g gfs gi cn gtl gil gisl
              ...
              vg4 vg5 vg6 m E2 K eb lks Vpr rg mst Kt
              ...
              with "Hcg Hcnt Hpay Hextc Hextm Htext Hkd Hpc Hpenv Hbio Hlogc
                    Hitlk0 Hinv Hesc Htok Hhalf Hstamps Hiauth Hipool Hslots Hpool
    ...
    - (* ===== EXIT A (+0x20): valid == 0 or nlink != 0 -- into the shared
      ...
      iIntros (M' vg4' vg5' vg6') "%HM'regs %HM'a5 Hcg Hcnt Hpay Hextc Hextm Hpc
                 Htok Hhalf Hrows Hiauth Hipool Hslots Hpool Hropen Hbms Hins
      ...
      iApply (ip_tail (CID := CIDacq) CID j bn g gfs gi cn gtl cov logstart bmapstart
                ...
                ltac:(cbn; lia) ltac:(lia) ltac:(reflexivity)
                ltac:(discriminate) ltac:(intros _; reflexivity) ltac:(lkbelow)
                with "Htext Hitlk0 Hinv Hesc Hireg Hpc Hcg Hcnt Hpay Hextc Hextm Htok
                      Hclaims0 Hhalf Hrows Hiauth Hipool Hslots Hpool Hru Hropen
    ...
    - (* ===== EXIT B (+0x5a): the FREE path proper -- the locked block and,
      ...
      iIntros (M5 g1 g2 dn bm data td T0 Kw) "%Htyne %Hnl0 %Hdnwf %Hbmwf %Hdlen %Hadr
                 ...
                 %HTKw #Hflw Hc Hvld Hid Hnl Hrcpt Hsele Hiauth Hipool Hpool
                 %Hcik5 Hslots5 Href5 Hlg Hshot Hpend Hpre
                 Hbms Hins Hppid Hbslots Hlb Hcrd2 Hop
                 Hr24 Hr16 Hr8 Hg4 Hg5 Hg6".
      ...
      iApply (ip_free_locked gs j gl gu gd gk pd pav pu bn g gfs gi cn gtl gil gisl g1 g2
                cov logstart bmapstart inodestart nib size dev
                ...
                M5 K eb eb lks Vpr rg td T0 Kw
                HK ltac:(lia) Hk ltac:(lia) Hcrb
                ...
                with "Hcg Hcnt Hpay Hextc Hextm Htext Hkd Hpc Hpenv Hbio Hlogc
                      Hitlk0 Hinv Hesc Htok Hhalf Hstampsback Hstk Hreg Hllbd [%] Hflw Hc
                      Hvld Hid Hnl Hrcpt Hsele Hiauth Hipool Hpool [%] Hslots5
                      Href5 Hslk Hclaims Hlg Hshot Hpend Hireg Hpre
      ...
      iIntros (mf n'' Sb'' w) "%Hthr Hcg Hcnt Hextc Hextm Hpc Hppid Hbms Hins
                 Hbslots %Hssub %Hwbm %Hwc %Hbnd Hop Hiu Hgreg
                 Hr24 Hr16 Hr8 Hg4 Hg5 Hg6".
      destruct Hthr as (Hthr5 & Hmfsp & Hmfs2 & Hmfs3 & Hmfs4).
      iEval (rewrite Hf1) in "Hr24". iEval (rewrite Hf2) in "Hr16".
      iEval (rewrite Hf3) in "Hr8".  iEval (rewrite Hf4) in "Hg4".
  ...
  Proof.
    ...
    iApply (wp_iput_gen gs j gl gu gd gk pd pav pu bn g gfs gi cn gtl gil gisl
              cov logstart bmapstart inodestart nib size dev
              k q inum n Sb0 false false false e00 pidv dq dqb dqs m K eb b lks Vpr true
