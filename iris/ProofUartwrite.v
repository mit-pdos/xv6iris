(* ProofUartwrite.v -- the whole-function WP for xv6's uartwrite().
   ...
   - the byte loop is TWO nested loops.  The outer one is bounded by [n], so
     ...
     so it is an iLöb ([SleepLoop], an iAssert inside the induction step); its
...
Require Import WpUartwriteDecode.
...
Section ProofUartwrite.
  ...
  (* ------------------------------------------------------------------ *)
  (*  The two register-state predicates.                                  *)
  (* ------------------------------------------------------------------ *)
  (* what the loop keeps in registers: the three global addresses, the two
     ...
     registers uartwrite itself never touches, which have to survive to the
     return. *)
  ...
  Proof.
    intros Hcs (H1 & H2 & H3 & H4 & H5 & H6 & H7 & H8 & H9 & H10 & H11 & H12 & H13).
    unfold uw_loop_regs.
    repeat first
      [ split
  ...
  (* [0(s1)] is &tx_busy, in both displacement spellings the two accesses use *)
  Lemma uw_busy_addr_c (M : regfile) :
    M !!! Regidx Rs1 = a_tx_busy ->
    add_vec (M !!! Regidx Rs1)
      (sign_extend' 64 (zero_extend' 12 (concat_vec (mword_of_int 0 : mword 5) ('b"00"))))
    = a_tx_busy.
  Proof. intro H. rewrite H /a_tx_busy. apply bv_eq; vm_compute; reflexivity. Qed.

  Lemma uw_busy_addr_b (M : regfile) :
    M !!! Regidx Rs1 = a_tx_busy ->
    add_vec (M !!! Regidx Rs1) (sign_extend' 64 (mword_of_int 0 : mword 12)) = a_tx_busy.
  Proof. intro H. rewrite H. apply uw_addv_0. Qed.
  ...
  (* the loop's two exits, as named continuations (they appear in three
     statements each, so they are worth naming). *)
  Definition uw_next_cont (γ γl : gname) (γu : uart_names) (Φ : mval -> iProp Σ)
      (γs : list gname) (j : nat) (m0 : regfile) (av : nat) (eb : bool) (C : iProp Σ)
      (sp0 buf : mword 64) (n : nat) (f : nat -> bv 8) (dq : dfrac) (i : nat) : iProp Σ :=
    (∀ M' : regfile,
       ⌜ (S i < n)%nat ⌝ -∗
       ⌜ uw_loop_regs m0 M' (pa_stk sp0 10) buf n (S i) ⌝ -∗
       sie_cap_gpr γ M' (av - 10) -∗
       cpu_own γ 1%nat eb (proc_addr j) C -∗ trap_csrs_pay 0%nat eb -∗
       pc_is (mword_of_int (UW + 0x6c)) -∗
  ...
  Definition uw_exit_cont (γ γl : gname) (γu : uart_names) (Φ : mval -> iProp Σ)
      (γs : list gname) (j : nat) (m0 : regfile) (av : nat) (eb : bool) (C : iProp Σ)
  ...
  (* ------------------------------------------------------------------ *)
  Lemma uw_one (γ γl : gname) (γu : uart_names) (γv : disk_names) (Φ : mval -> iProp Σ)
    ...
    ⊢ ∀ M : regfile,
      ...
      own_ctx (p_context (proc_addr j)) -∗
      ▷ sched_vc γ Φ γs (a_cpu_ctx cid_word) (proc_addr j) -∗
      ( uw_next_cont γ γl γu Φ γs j m0 av eb C sp0 buf n f dq i
        ∧ uw_exit_cont γ γl γu Φ γs j m0 av eb C sp0 buf n f dq ) -∗
      WP (Loop : expr riscv_lang) {{ Φ }}.
  Proof.
    intros Hin Hn31 Hj Hjlp Hav.
    ...
    iIntros (M) "%Hregs #Ht #Hdinv #Htxl #Hpinv #Hpanic".
    iIntros "Hcg Hcnt Hpay Hpc Htok Hres #Hsub Hfull Hbuf Hctx Hsched Hcont".
    ...
    assert (P50 : add_vec_int (mword_of_int (UW + 0x4e) : mword 64) 2 = mword_of_int (UW + 0x50)) by pcw.
    assert (P52 : add_vec_int (mword_of_int (UW + 0x50) : mword 64) 2 = mword_of_int (UW + 0x52)) by pcw.
    assert (P56 : ret_pc (add_vec_int (mword_of_int (UW + 0x52) : mword 64) 4) = mword_of_int (UW + 0x56)) by pcw.
    assert (P58 : add_vec_int (mword_of_int (UW + 0x56) : mword 64) 2 = mword_of_int (UW + 0x58)) by pcw.
    assert (P5a : add_vec_int (mword_of_int (UW + 0x58) : mword 64) 2 = mword_of_int (UW + 0x5a)) by pcw.
    assert (P5e : add_vec_int (mword_of_int (UW + 0x5a) : mword 64) 4 = mword_of_int (UW + 0x5e)) by pcw.
    assert (P62 : add_vec_int (mword_of_int (UW + 0x5e) : mword 64) 4 = mword_of_int (UW + 0x62)) by pcw.
    assert (P66 : add_vec_int (mword_of_int (UW + 0x62) : mword 64) 4 = mword_of_int (UW + 0x66)) by pcw.
    assert (P68 : add_vec_int (mword_of_int (UW + 0x66) : mword 64) 2 = mword_of_int (UW + 0x68)) by pcw.
    assert (P6c : add_vec_int (mword_of_int (UW + 0x68) : mword 64) 4 = mword_of_int (UW + 0x6c)) by pcw.
    assert (P6e : add_vec_int (mword_of_int (UW + 0x6c) : mword 64) 2 = mword_of_int (UW + 0x6e)) by pcw.
    assert (P70 : add_vec_int (mword_of_int (UW + 0x6e) : mword 64) 2 = mword_of_int (UW + 0x70)) by pcw.
    assert (Jbody : add_vec (mword_of_int (UW + 0x70) : mword 64)
                      (sign_extend' 64 (sign_extend' 21 (concat_vec (mword_of_int 2037 : mword 11) ('b"0"))))
                    = mword_of_int (UW + 0x5a)) by pcw.
    assert (Jsleep1 : add_vec (mword_of_int (UW + 0x58) : mword 64)
                        (sign_extend' 64 (sign_extend' 13 (concat_vec (mword_of_int 251 : mword 8) ('b"0"))))
                      = mword_of_int (UW + 0x4e)) by pcw.
    assert (Jsleep2 : add_vec (mword_of_int (UW + 0x6e) : mword 64)
                        (sign_extend' 64 (sign_extend' 13 (concat_vec (mword_of_int 240 : mword 8) ('b"0"))))
                      = mword_of_int (UW + 0x4e)) by pcw.
    assert (Jexit : add_vec (mword_of_int (UW + 0x68) : mword 64)
                      (sign_extend' 64 (mword_of_int 10 : mword 13)) = mword_of_int (UW + 0x72)) by pcw.
    assert (Jcall : add_vec (mword_of_int (UW + 0x52) : mword 64)
                      (sign_extend' 64 (mword_of_int 5592 : mword 21)) = mword_of_int KernelSyms.sleep) by pcw.
    (* ================================================================= *)
    (*  THE BODY: +0x5a .. +0x68.                                         *)
    (* ================================================================= *)
    iAssert (∀ (M2 : regfile) (l2 : list (bv 8)) (b2 : mword 32),
      ⌜ uw_loop_regs m0 M2 (pa_stk sp0 10) buf n i ⌝ -∗
      sie_cap_gpr γ M2 (av - 10) -∗
      cpu_own γ 1%nat eb (proc_addr j) C -∗ trap_csrs_pay 0%nat eb -∗
      pc_is (mword_of_int (UW + 0x5a)) -∗
    ...
    { iIntros (M2 l2 b2) "%Hregs2 Hcg Hcnt Hpay Hpc Htok Hcell Hown #Hlb Hfull Hbuf Hctx Hsched Hcont".
      destruct Hregs2 as (Hsp & Htp & Hs1 & Hs2 & Hs3 & Hs4 & Hs5 & Hs6 & Hs7 & H24 & H25 & H26 & H27).
      (* --- +0x5a  lbu a5,0(s4) --- *)
      ...
      assert (Haddrb : add_vec (M2 !!! Regidx Rs4) (sign_extend' 64 (mword_of_int 0 : mword 12)) = pa_add buf i)
        by (rewrite Hs4; apply uw_addv_0).
      iApply (wp_lbu_s_sconf γ Φ (mword_of_int (UW + 0x5a)) Ra5 Rs4 (mword_of_int 0 : mword 12)
                M2 (av - 10)%nat (f i : mword 8) (dqm := dq) ltac:(nz) ltac:(nz)
                with "Hcg Hpc Hi5a [Hbyte] [-]").
      { iEval (rewrite Haddrb). iExact "Hbyte". }
      iIntros "Hcg Hpc Hbyte". iEval (rewrite Haddrb) in "Hbyte".
      ...
      assert (HB1s7 : B1 !!! Regidx Rs7 = uart_pa 0) by (rewrite /B1 upd_ne; [exact Hs7 | reg_neq]).
      assert (HB1a5 : B1 !!! Regidx Ra5 = zero_extend' 64 (f i : mword 8)) by (rewrite /B1 upd_eq; reflexivity).
      iApply (UAcc.wp_uart_thr_write_s_sconf γ γu γv Φ (mword_of_int (UW + 0x5e)) Ra5 Rs7
                B1 (av - 10)%nat l2 HB1s7 with "Hcg Hpc Hi5e Hdinv Hown Hlb Hdlab").
      iIntros "Hcg Hpc Hown #Hsent".
      assert (Hsb : (autocast (T := mword) (subrange_vec_dec (B1 !!! Regidx Ra5)
                       (Z.sub (Z.mul 1 8) 1) 0) : mword 8) = f i)
        by (rewrite HB1a5; apply uw_sub8_zext).
      ...
      assert (Haddrw : add_vec (B1 !!! Regidx Rs1) (sign_extend' 64 (mword_of_int 0 : mword 12)) = a_tx_busy)
        by (apply uw_busy_addr_b; exact HB1s1).
      iApply (wp_sw_s_sconf γ Φ (mword_of_int (UW + 0x62)) Rs6 Rs1 (mword_of_int 0 : mword 12)
                B1 (av - 10)%nat b2 with "Hcg Hpc Hi62 [Hcell] [-]").
      { iEval (rewrite Haddrw). iExact "Hcell". }
      iIntros "Hcg Hpc Hcell".
      iEval (rewrite Haddrw HB1s6) in "Hcell".
      ...
      (* --- +0x66  c.addi s4,s4,1 --- *)
      iApply (wp_caddi_s_sconf γ Φ (mword_of_int (UW + 0x66)) Rs4 (mword_of_int 1 : mword 6)
                B1 (av - 10)%nat ltac:(nz) ltac:(nz) with "Hcg Hpc Hi66 [-]").
      iIntros "Hcg Hpc".
      ...
      assert (Hcmp : eq_vec (B2 !!! Regidx Rs4) (B2 !!! Regidx Rs5) = Nat.eqb (S i) n).
      { rewrite HB2s4 HB2s5. apply pa_add_eqb; [exact HSi64 | exact Hn64]. }
      (* --- +0x68  beq s4,s5 --- *)
      destruct (Nat.eqb (S i) n) eqn:Hend.
      - (* the last byte: leave the loop *)
        assert (Hendn : S i = n) by (apply Nat.eqb_eq; exact Hend).
        iApply (wp_beq_taken_s_sconf γ Φ (mword_of_int (UW + 0x68)) (mword_of_int 10 : mword 13)
                  Rs5 Rs4 B2 (av - 10)%nat ltac:(nz) ltac:(nz) Hcmp
        ...
        iApply ("Hexit" $! B2 with "[%] Hcg Hcnt Hpay Hpc Htok Hres Hsub' Hfull Hbuf Hctx Hsched").
        exact HB2regs.
      - (* more bytes: back to the head *)
        assert (Hendn : S i <> n) by (intro Hc; rewrite Hc Nat.eqb_refl in Hend; discriminate).
        iApply (wp_beq_fall_s_sconf γ Φ (mword_of_int (UW + 0x68)) (mword_of_int 10 : mword 13)
                  Rs5 Rs4 B2 (av - 10)%nat ltac:(nz) ltac:(nz) Hcmp with "Hcg Hpc Hi68 [-]").
        iIntros "Hcg Hpc". iEval (rewrite P6c) in "Hpc".
        iDestruct "Hcont" as "[Hnext _]".
        iApply ("Hnext" $! B2 with "[%] [%] Hcg Hcnt Hpay Hpc Htok Hres Hsub' Hfull Hbuf Hctx Hsched").
        + lia.
        + exact HB2regs. }
    (* ================================================================= *)
    (*  THE SLEEP RETRY LOOP: +0x4e .. +0x58 (iLöb).                      *)
    (* ================================================================= *)
    iAssert (∀ M1 : regfile,
      ⌜ uw_loop_regs m0 M1 (pa_stk sp0 10) buf n i ⌝ -∗
      sie_cap_gpr γ M1 (av - 10) -∗
      cpu_own γ 1%nat eb (proc_addr j) C -∗ trap_csrs_pay 0%nat eb -∗
      pc_is (mword_of_int (UW + 0x4e)) -∗
      locked γl cpu_id -∗ tx_res γu -∗
      uw_full sp0 m0 -∗ uw_buf buf dq f n -∗
      own_ctx (p_context (proc_addr j)) -∗
      ▷ sched_vc γ Φ γs (a_cpu_ctx cid_word) (proc_addr j) -∗
    ...
    { iLöb as "IH".
      iIntros (M1) "%Hregs1 Hcg Hcnt Hpay Hpc Htok Hres Hfull Hbuf Hctx Hsched Hcont".
      pose proof Hregs1 as Hregs1'.
      destruct Hregs1' as (Hsp & Htp & Hs1 & Hs2 & Hs3 & Hs4 & Hs5 & Hs6 & Hs7 & H24 & H25 & H26 & H27).
      (* --- +0x4e  c.mv a1,s3 --- *)
      iApply (wp_cmv_s_sconf γ Φ (mword_of_int (UW + 0x4e)) Ra1 Rs3 M1 (av - 10)%nat
                ltac:(nz) ltac:(nz) with "Hcg Hpc Hi4e [-]").
      iIntros "Hcg Hpc".
      ...
      (* --- +0x50  c.mv a0,s2 --- *)
      iApply (wp_cmv_s_sconf γ Φ (mword_of_int (UW + 0x50)) Ra0 Rs2 S1 (av - 10)%nat
                ltac:(nz) ltac:(nz) with "Hcg Hpc Hi50 [-]").
      iIntros "Hcg Hpc".
      ...
      (* --- +0x52  jal ra,sleep --- *)
      iApply (wp_jal_s_sconf γ Φ (mword_of_int (UW + 0x52)) Rra (mword_of_int 5592 : mword 21)
                S2 (av - 10)%nat ltac:(nz) ltac:(nz) ltac:(vm_compute; reflexivity)
                with "Hcg Hpc Hi52 [-]").
      iIntros "Hcg Hpc".
      set (S3 := <[Regidx Rra := regval_into_reg (add_vec_int (mword_of_int (UW + 0x52) : mword 64) 4)]> S2).
      change (<[Regidx Rra := regval_into_reg (add_vec_int (mword_of_int (UW + 0x52) : mword 64) 4)]> S2) with S3.
      ...
      assert (HS3tp : S3 !!! Regidx Rtp = cid_word).
      { rewrite /S3 upd_ne; [| reg_neq]. rewrite /S2 upd_ne; [| reg_neq].
        rewrite /S1 upd_ne; [| reg_neq]. exact Htp. }
      assert (HS3ra : S3 !!! Regidx Rra = add_vec_int (mword_of_int (UW + 0x52) : mword 64) 4)
        by (rewrite /S3 upd_eq; reflexivity).
      iApply (Sleep.wp_sleep_sconf γ Φ γs j γlp γl a_tx_lock "uart"%string (tx_res γu)
                S3 (av - 10)%nat eb C HS3tp Hj Hjlp
                ltac:(rewrite HS3a1; apply uw_addv_0)
                ltac:(unfold uartwrite_stack in Hav; lia)
                with "Hcg Hcnt Hpay Ht Hpc Hpinv [Hlk] Htok Hres Hpanic Hctx Hsched [-]").
      { iExact "Hlk". }
      iIntros (Ms) "%Hscs Hcg Hcnt Hpay Hpc Htok Hres Hctx Hsched".
      ...
      destruct HregsS' as (Wsp & Wtp & Ws1 & Ws2 & Ws3 & Ws4 & Ws5 & Ws6 & Ws7 & W24 & W25 & W26 & W27).
      (* --- +0x56  c.lw a5,0(s1) --- *)
      iDestruct "Hres" as (b l) "(Hcell & Hown & Hwand)".
      assert (Haddrc : add_vec (Ms !!! Regidx Rs1)
                (sign_extend' 64 (zero_extend' 12 (concat_vec (mword_of_int 0 : mword 5) ('b"00")))) = a_tx_busy)
        by (apply uw_busy_addr_c; exact Ws1).
      iApply (wp_clw_s_sconf γ Φ (mword_of_int (UW + 0x56)) Ra5 Rs1
                (zero_extend' 12 (concat_vec (mword_of_int 0 : mword 5) ('b"00")))
                Ms (av - 10)%nat b (dqm := DfracOwn 1) ltac:(nz) ltac:(nz)
                with "Hcg Hpc Hi56 [Hcell] [-]").
      { iEval (rewrite Haddrc). iExact "Hcell". }
      iIntros "Hcg Hpc Hcell". iEval (rewrite Haddrc) in "Hcell".
      ...
      + (* tx_busy == 0: fall through into the body *)
        ...
        iApply (wp_cbnez_fall_s_sconf γ Φ (mword_of_int (UW + 0x58)) (mword_of_int 251 : mword 8)
                  (Cregidx (mword_of_int 7)) Ra5 S4 (av - 10)%nat uw_cr7 ltac:(nz)
                  ltac:(rewrite HS4a5; unfold neq_vec; rewrite Hbz; reflexivity)
                  with "Hcg Hpc Hi58 [-]").
        iIntros "Hcg Hpc". iEval (rewrite P5a) in "Hpc".
        iDestruct ("Hwand" with "[%]") as "#Hlb"; [exact Hb0|].
        iApply ("Body" $! S4 l b with "[%] Hcg Hcnt Hpay Hpc Htok Hcell Hown Hlb Hfull Hbuf Hctx Hsched Hcont").
        exact HS4regs.
      + (* still busy: sleep again *)
        ...
        iApply (wp_cbnez_taken_s_sconf γ Φ (mword_of_int (UW + 0x58)) (mword_of_int 251 : mword 8)
                  (Cregidx (mword_of_int 7)) Ra5 S4 (av - 10)%nat uw_cr7 ltac:(nz)
                  ltac:(rewrite HS4a5; unfold neq_vec; rewrite Hbz; reflexivity)
        ...
        iApply ("IH" $! S4 with "[%] Hcg Hcnt Hpay Hpc Htok Hres Hfull Hbuf Hctx Hsched Hcont").
        exact HS4regs. }
    (* ================================================================= *)
    (*  THE HEAD: +0x6c .. +0x70.                                         *)
    (* ================================================================= *)
    pose proof Hregs as Hregs'.
    destruct Hregs' as (Hsp & Htp & Hs1 & Hs2 & Hs3 & Hs4 & Hs5 & Hs6 & Hs7 & H24 & H25 & H26 & H27).
    iDestruct "Hres" as (b l) "(Hcell & Hown & Hwand)".
    assert (Haddrc : add_vec (M !!! Regidx Rs1)
              (sign_extend' 64 (zero_extend' 12 (concat_vec (mword_of_int 0 : mword 5) ('b"00")))) = a_tx_busy)
      by (apply uw_busy_addr_c; exact Hs1).
    iApply (wp_clw_s_sconf γ Φ (mword_of_int (UW + 0x6c)) Ra5 Rs1
              (zero_extend' 12 (concat_vec (mword_of_int 0 : mword 5) ('b"00")))
              M (av - 10)%nat b (dqm := DfracOwn 1) ltac:(nz) ltac:(nz)
    ...
    - (* tx_busy == 0: straight to the body via the +0x70 jump *)
      assert (Hb0 : b = (mword_of_int 0 : mword 32)) by (apply uw_sext_zero; exact Hbz).
      iApply (wp_cbnez_fall_s_sconf γ Φ (mword_of_int (UW + 0x6e)) (mword_of_int 240 : mword 8)
                (Cregidx (mword_of_int 7)) Ra5 H1 (av - 10)%nat uw_cr7 ltac:(nz)
                ltac:(rewrite HH1a5; unfold neq_vec; rewrite Hbz; reflexivity)
                with "Hcg Hpc Hi6e [-]").
      iIntros "Hcg Hpc". iEval (rewrite P70) in "Hpc".
      iApply (wp_cj_s_sconf γ Φ (mword_of_int (UW + 0x70))
                (sign_extend' 21 (concat_vec (mword_of_int 2037 : mword 11) ('b"0")))
                H1 (av - 10)%nat ltac:(vm_compute; reflexivity) with "Hcg Hpc Hi70 [-]").
      iNext. iIntros "Hcg Hpc".
      iAssert (▷ sched_vc γ Φ γs (a_cpu_ctx cid_word) (proc_addr j))%I with "[Hsched]" as "Hsched".
      { iNext. iExact "Hsched". }
      iEval (rewrite Jbody) in "Hpc".
      iDestruct ("Hwand" with "[%]") as "#Hlb"; [exact Hb0|].
      iApply ("Body" $! H1 l b with "[%] Hcg Hcnt Hpay Hpc Htok Hcell Hown Hlb Hfull Hbuf Hctx Hsched Hcont").
      exact HH1regs.
    - (* busy: into the sleep loop *)
      assert (Hbne : b <> (mword_of_int 0 : mword 32)) by (apply uw_sext_nonzero; exact Hbz).
      iDestruct (tx_res_busy γu b l Hbne with "Hcell Hown") as "Hres".
      iApply (wp_cbnez_taken_s_sconf γ Φ (mword_of_int (UW + 0x6e)) (mword_of_int 240 : mword 8)
                (Cregidx (mword_of_int 7)) Ra5 H1 (av - 10)%nat uw_cr7 ltac:(nz)
                ltac:(rewrite HH1a5; unfold neq_vec; rewrite Hbz; reflexivity)
                ltac:(vm_compute; reflexivity)
                with "Hcg Hpc Hi6e [-]").
      ...
      iApply ("Sleep" $! H1 with "[%] Hcg Hcnt Hpay Hpc Htok Hres Hfull Hbuf Hctx Hsched Hcont").
      exact HH1regs.
  Qed.
  ...
  (* The head is only ever entered with at least one byte left ([i + S k =
     n]), which is what makes the induction go through: the [beq] at +0x68
     leaves the loop exactly when [S i = n], so the last iteration is the
     [k = 0] case and every other one hands its successor to the IH.  Both
     exits are offered as a CONJUNCTION -- only one is taken, and they must
     share the caller's tail continuation. *)
  Lemma uw_iter (γ γl : gname) (γu : uart_names) (γv : disk_names) (Φ : mval -> iProp Σ)
      (γs : list gname) (j : nat) (γlp : gname)
      (m0 : regfile) (av : nat) (eb : bool) (C : iProp Σ)
    ...
    ⊢ ∀ (i : nat) (M : regfile),
      ...
      WP (Loop : expr riscv_lang) {{ Φ }}.
  Proof.
    intros Hn31 Hj Hjlp Hav.
    induction k as [|k IH].
    - iIntros (i M) "%Hik %Hregs #Ht #Hdinv #Htxl #Hpinv #Hpanic".
      iIntros "Hcg Hcnt Hpay Hpc Htok Hres #Hsub Hfull Hbuf Hctx Hsched Hexit".
      iPoseProof (uw_one γ γl γu γv Φ γs j γlp m0 av eb C sp0 buf n f dq i
                    ltac:(lia) Hn31 Hj Hjlp Hav) as "One".
      iApply ("One" $! M with "[%] Ht Hdinv Htxl Hpinv Hpanic Hcg Hcnt Hpay Hpc Htok Hres Hsub Hfull Hbuf Hctx Hsched [Hexit]").
      + exact Hregs.
      + iSplit.
        * (* the back edge is dead: this was the last byte *)
          rewrite /uw_next_cont. iIntros (M') "%Hlt". exfalso. lia.
        * iExact "Hexit".
    - iIntros (i M) "%Hik %Hregs #Ht #Hdinv #Htxl #Hpinv #Hpanic".
      iIntros "Hcg Hcnt Hpay Hpc Htok Hres #Hsub Hfull Hbuf Hctx Hsched Hexit".
      iPoseProof (uw_one γ γl γu γv Φ γs j γlp m0 av eb C sp0 buf n f dq i
      ...
      + iSplit.
        * rewrite /uw_next_cont.
          iIntros (M') "%Hlt %Hregs' Hcg Hcnt Hpay Hpc Htok Hres #Hsub' Hfull Hbuf Hctx Hsched".
          iApply (IH $! (S i) M' with "[%] [%] Ht Hdinv Htxl Hpinv Hpanic Hcg Hcnt Hpay Hpc Htok Hres Hsub' Hfull Hbuf Hctx Hsched Hexit").
          { lia. }
          { exact Hregs'. }
        * iExact "Hexit".
  Qed.
  ...
  (* ------------------------------------------------------------------ *)
  Lemma uw_tail (γ γl : gname) (γu : uart_names) (Φ : mval -> iProp Σ)
      (γs : list gname) (j : nat)
      (m0 M : regfile) (av : nat) (eb : bool) (C : iProp Σ) (sp0 : mword 64)
      (bs : list (bv 8)) (Rbuf : iProp Σ) :
    uw_tail_regs m0 M (pa_stk sp0 10) ->
    m0 !!! Regidx csp_rs1 = sp0 ->
    m0 !!! Regidx Rtp = cid_word ->
    (uartwrite_stack <= av)%nat ->
    kernel_text -∗ is_txlock γl γu -∗
    sie_cap_gpr γ M (av - 10) -∗
    cpu_own γ 1%nat eb (proc_addr j) C -∗ trap_csrs_pay 0%nat eb -∗
    pc_is (mword_of_int (UW + 0x7c)) -∗
    locked γl cpu_id -∗ tx_res γu -∗
    uart_sent_sub γu bs -∗
    ...
    WP (Loop : expr riscv_lang) {{ Φ }}.
  Proof.
    intros Hregs Hsp0 Hm0tp Hav.
    destruct Hregs as (Hsp & Htp & H18 & H19 & H20 & H22 & H23 & H24 & H25 & H26 & H27).
    iIntros "#Ht #Htxl Hcg Hcnt Hpay Hpc Htok HR #Hsub Hsv Hgap Hs10 Hbuf Hctx Hsched Hcont".
    iDestruct (is_txlock_lock with "Htxl") as "#Hlk".
    set (spd := pa_stk sp0 10).
    (* +0x7c  auipc a0,0x12 *)
    iPoseProof (uwi_7c with "Ht") as "Hi7c".
    iApply (wp_auipc_s_sconf γ Φ (mword_of_int (UW + 0x7c)) Ra0 (mword_of_int 18 : mword 20)
              M (av - 10)%nat ltac:(nz) ltac:(nz) with "Hcg Hpc Hi7c [-]").
    iIntros "Hcg Hpc".
    set (T0 := <[Regidx Ra0 := regval_into_reg
        (add_vec (mword_of_int (UW + 0x7c) : mword 64) (auipc_off (mword_of_int 18 : mword 20)))]> M).
    change (<[Regidx Ra0 := regval_into_reg
        (add_vec (mword_of_int (UW + 0x7c) : mword 64) (auipc_off (mword_of_int 18 : mword 20)))]> M) with T0.
    assert (P80 : add_vec_int (mword_of_int (UW + 0x7c) : mword 64) 4 = mword_of_int (UW + 0x80)) by pcw.
    iEval (rewrite P80) in "Hpc".
    (* +0x80  addi a0,a0,-1608  -> &tx_lock *)
    iPoseProof (uwi_80 with "Ht") as "Hi80".
    iApply (wp_addi4_s_sconf γ Φ (mword_of_int (UW + 0x80)) Ra0 Ra0 (mword_of_int 0x9b8 : mword 12)
              T0 (av - 10)%nat ltac:(nz) ltac:(nz) with "Hcg Hpc Hi80 [-]").
    iIntros "Hcg Hpc".
    ...
    assert (P84 : add_vec_int (mword_of_int (UW + 0x80) : mword 64) 4 = mword_of_int (UW + 0x84)) by pcw.
    iEval (rewrite P84) in "Hpc".
    (* +0x84  jal ra,release *)
    iPoseProof (uwi_84 with "Ht") as "Hi84".
    iApply (wp_jal_s_sconf γ Φ (mword_of_int (UW + 0x84)) Rra (mword_of_int 816 : mword 21)
              T1 (av - 10)%nat ltac:(nz) ltac:(nz) ltac:(vm_compute; reflexivity)
              with "Hcg Hpc Hi84 [-]").
    iIntros "Hcg Hpc".
    set (T2 := <[Regidx Rra := regval_into_reg
        (add_vec_int (mword_of_int (UW + 0x84) : mword 64) 4)]> T1).
    change (<[Regidx Rra := regval_into_reg
        (add_vec_int (mword_of_int (UW + 0x84) : mword 64) 4)]> T1) with T2.
    assert (Hjrel : add_vec (mword_of_int (UW + 0x84) : mword 64)
    ...
    assert (HT2tp : T2 !!! Regidx Rtp = cid_word).
    { rewrite /T2 upd_ne; [| reg_neq]. rewrite /T1 upd_ne; [| reg_neq].
      rewrite /T0 upd_ne; [| reg_neq]. exact Htp. }
    assert (HT2sp : T2 !!! Regidx csp_rs1 = spd).
    { rewrite /T2 upd_ne; [| reg_neq]. rewrite /T1 upd_ne; [| reg_neq].
      rewrite /T0 upd_ne; [| reg_neq]. exact Hsp. }
    assert (HT2ra : T2 !!! Regidx Rra = add_vec_int (mword_of_int (UW + 0x84) : mword 64) 4)
      by (rewrite /T2 upd_eq; reflexivity).
    iApply (Release.wp_release_sconf γ Φ γl a_tx_lock "uart"%string (tx_res γu) T2
              0%nat eb (proc_addr j) C (av - 10)%nat
              ltac:(rewrite HT2a0; apply uw_addv_0)
              HT2tp ltac:(unfold uartwrite_stack in Hav; lia)
              with "Hcg Ht Hpc [Hlk] [Htok] [HR] Hcnt Hpay [-]").
    { iExact "Hlk". }
    { iExact "Htok". }
    { iExact "HR". }
    iIntros (MR) "Hcg Hpc %HcsR Hcnt".
    assert (P88 : ret_pc (add_vec_int (mword_of_int (UW + 0x84) : mword 64) 4)
                  = mword_of_int (UW + 0x88)) by pcw.
    ...
    iApply (wp_cldsp_s_sconf γ Φ (mword_of_int (UW + 0x88)) (mword_of_int 9 : mword 6) Rra
              MR (av - 10)%nat (m0 !!! Regidx Rra) (dqm := DfracOwn 1)
              ltac:(nz) ltac:(nz) with "Hcg Hpc Hi88 [H1] [-]").
    { iEval (rewrite HMRsp Hb1). iExact "H1". }
    iIntros "Hcg Hpc H1". iEval (rewrite HMRsp Hb1) in "H1".
    set (E1 := <[Regidx Rra := regval_into_reg (m0 !!! Regidx Rra)]> MR).
    change (<[Regidx Rra := regval_into_reg (m0 !!! Regidx Rra)]> MR) with E1.
    assert (P8a : add_vec_int (mword_of_int (UW + 0x88) : mword 64) 2 = mword_of_int (UW + 0x8a)) by pcw.
    ...
    iApply (wp_cldsp_s_sconf γ Φ (mword_of_int (UW + 0x8a)) (mword_of_int 8 : mword 6) Rs0
              E1 (av - 10)%nat (m0 !!! Regidx Rs0) (dqm := DfracOwn 1)
              ltac:(nz) ltac:(nz) with "Hcg Hpc Hi8a [H2] [-]").
    { iEval (rewrite HE1sp Hb2). iExact "H2". }
    iIntros "Hcg Hpc H2". iEval (rewrite HE1sp Hb2) in "H2".
    set (E2 := <[Regidx Rs0 := regval_into_reg (m0 !!! Regidx Rs0)]> E1).
    change (<[Regidx Rs0 := regval_into_reg (m0 !!! Regidx Rs0)]> E1) with E2.
    assert (P8c : add_vec_int (mword_of_int (UW + 0x8a) : mword 64) 2 = mword_of_int (UW + 0x8c)) by pcw.
    ...
    iApply (wp_cldsp_s_sconf γ Φ (mword_of_int (UW + 0x8c)) (mword_of_int 7 : mword 6) Rs1
              E2 (av - 10)%nat (m0 !!! Regidx Rs1) (dqm := DfracOwn 1)
              ltac:(nz) ltac:(nz) with "Hcg Hpc Hi8c [H3] [-]").
    { iEval (rewrite HE2sp Hb3). iExact "H3". }
    iIntros "Hcg Hpc H3". iEval (rewrite HE2sp Hb3) in "H3".
    set (E3 := <[Regidx Rs1 := regval_into_reg (m0 !!! Regidx Rs1)]> E2).
    change (<[Regidx Rs1 := regval_into_reg (m0 !!! Regidx Rs1)]> E2) with E3.
    assert (P8e : add_vec_int (mword_of_int (UW + 0x8c) : mword 64) 2 = mword_of_int (UW + 0x8e)) by pcw.
    ...
    iApply (wp_cldsp_s_sconf γ Φ (mword_of_int (UW + 0x8e)) (mword_of_int 3 : mword 6) Rs5
              E3 (av - 10)%nat (m0 !!! Regidx Rs5) (dqm := DfracOwn 1)
              ltac:(nz) ltac:(nz) with "Hcg Hpc Hi8e [H7] [-]").
    { iEval (rewrite HE3sp Hb7). iExact "H7". }
    iIntros "Hcg Hpc H7". iEval (rewrite HE3sp Hb7) in "H7".
    set (E4 := <[Regidx Rs5 := regval_into_reg (m0 !!! Regidx Rs5)]> E3).
    change (<[Regidx Rs5 := regval_into_reg (m0 !!! Regidx Rs5)]> E3) with E4.
    assert (P90 : add_vec_int (mword_of_int (UW + 0x8e) : mword 64) 2 = mword_of_int (UW + 0x90)) by pcw.
    ...
    iApply (wp_caddi16sp_pop_s_sconf γ Φ (mword_of_int (UW + 0x90)) (mword_of_int 5 : mword 6)
              E4 (av - 10)%nat 10%nat Hpop with "Hcg Hpc Hi90 Hframe [-]").
    iIntros "Hcg Hpc".
    ...
    assert (P92 : add_vec_int (mword_of_int (UW + 0x90) : mword 64) 2 = mword_of_int (UW + 0x92)) by pcw.
    ...
    iApply (wp_cret_s_sconf γ Φ (mword_of_int (UW + 0x92)) Rra E5 av ltac:(nz)
              with "Hcg Hpc Hi92 [-]").
    iIntros "Hcg Hpc". iEval (rewrite HE5ra) in "Hpc".
    (* ---- callee_saved m0 E5 ---- *)
    assert (HE5sp : E5 !!! Regidx csp_rs1 = m0 !!! Regidx csp_rs1)
      by (rewrite /E5 upd_eq Hpopv; symmetry; exact Hsp0).
    ...
    iApply ("Hcont" $! E5 with "[%] Hcg Hcnt Hpc Hbuf Hsub Hctx Hsched").
    unfold callee_saved.
    split; [exact HE5sp|].
    split; [rewrite (HMRk (mword_of_int 4) ltac:(vm_compute; reflexivity)
                       ltac:(reg_neq) ltac:(reg_neq) ltac:(reg_neq) ltac:(reg_neq) ltac:(reg_neq) ltac:(reg_neq));
            rewrite (HT2k (mword_of_int 4) ltac:(reg_neq) ltac:(reg_neq) ltac:(reg_neq)); rewrite Hm0tp; exact Htp|].
    split; [exact HE5s0|].
    split; [exact HE5s1|].
  ...
  (*  THE WHOLE FUNCTION.                                                 *)
  (* ------------------------------------------------------------------ *)
  Lemma wp_uartwrite_sconf (γ : gname) (γu : uart_names) (γv : disk_names)
  ...
  Proof.
    ...
    intros pcE pj buf ret_tgt Htp Hj Hjlp Ha1 Hn31 Hav.
    iIntros "Hcg Hcnt #Ht Hpc #Hdinv #Htxl Hbuf #Hpinv #Hpanic Hctx Hsched Hcont".
    iDestruct (is_txlock_lock with "Htxl") as "#Hlk".
    assert (H231 : (2 ^ 31 = 2147483648)%Z) by (vm_compute; reflexivity).
    rewrite H231 in Hn31.
    ...
    assert (P02 : add_vec_int (pcE : mword 64) 2 = mword_of_int (UW + 0x02)) by pcw.
    assert (P04 : add_vec_int (mword_of_int (UW + 0x02) : mword 64) 2 = mword_of_int (UW + 0x04)) by pcw.
    assert (P06 : add_vec_int (mword_of_int (UW + 0x04) : mword 64) 2 = mword_of_int (UW + 0x06)) by pcw.
    assert (P08 : add_vec_int (mword_of_int (UW + 0x06) : mword 64) 2 = mword_of_int (UW + 0x08)) by pcw.
    assert (P0a : add_vec_int (mword_of_int (UW + 0x08) : mword 64) 2 = mword_of_int (UW + 0x0a)) by pcw.
    assert (P0c : add_vec_int (mword_of_int (UW + 0x0a) : mword 64) 2 = mword_of_int (UW + 0x0c)) by pcw.
    assert (P0e : add_vec_int (mword_of_int (UW + 0x0c) : mword 64) 2 = mword_of_int (UW + 0x0e)) by pcw.
    assert (P10 : add_vec_int (mword_of_int (UW + 0x0e) : mword 64) 2 = mword_of_int (UW + 0x10)) by pcw.
    assert (P14 : add_vec_int (mword_of_int (UW + 0x10) : mword 64) 4 = mword_of_int (UW + 0x14)) by pcw.
    assert (P18 : add_vec_int (mword_of_int (UW + 0x14) : mword 64) 4 = mword_of_int (UW + 0x18)) by pcw.
    assert (P1c : ret_pc (add_vec_int (mword_of_int (UW + 0x18) : mword 64) 4) = mword_of_int (UW + 0x1c)) by pcw.
    ...
    iApply (wp_caddi16sp_push_s_sconf γ Φ pcE (mword_of_int 59 : mword 6) m av 10%nat
    ...
    iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (UW + 0x02)) (mword_of_int 9 : mword 6) Rra
              A0 (av - 10)%nat v1 with "Hcg Hpc Hi02 [H1] [-]").
    { iEval (rewrite HcspA0 Hb1). iExact "H1". }
    iIntros "Hcg Hpc H1". iEval (rewrite HcspA0 Hb1) in "H1".
    iEval (rewrite P04) in "Hpc".
    (* +0x04 c.sdsp s0,64(sp) *)
    iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (UW + 0x04)) (mword_of_int 8 : mword 6) Rs0
              A0 (av - 10)%nat v2 with "Hcg Hpc Hi04 [H2] [-]").
    { iEval (rewrite HcspA0 Hb2). iExact "H2". }
    iIntros "Hcg Hpc H2". iEval (rewrite HcspA0 Hb2) in "H2".
    iEval (rewrite P06) in "Hpc".
    (* +0x06 c.sdsp s1,56(sp) *)
    iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (UW + 0x06)) (mword_of_int 7 : mword 6) Rs1
              A0 (av - 10)%nat v3 with "Hcg Hpc Hi06 [H3] [-]").
    { iEval (rewrite HcspA0 Hb3). iExact "H3". }
    iIntros "Hcg Hpc H3". iEval (rewrite HcspA0 Hb3) in "H3".
    iEval (rewrite P08) in "Hpc".
    (* +0x08 c.sdsp s5,24(sp) *)
    iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (UW + 0x08)) (mword_of_int 3 : mword 6) Rs5
              A0 (av - 10)%nat v7 with "Hcg Hpc Hi08 [H7] [-]").
    { iEval (rewrite HcspA0 Hb7). iExact "H7". }
    iIntros "Hcg Hpc H7". iEval (rewrite HcspA0 Hb7) in "H7".
    ...
    iApply (wp_caddi4spn_s_sconf γ Φ (mword_of_int (UW + 0x0a)) (Cregidx (mword_of_int 0))
              (mword_of_int 20 : mword 8) Rs0 A0 (av - 10)%nat
    ...
    (* +0x0c c.mv s5,a0 *)
    iApply (wp_cmv_s_sconf γ Φ (mword_of_int (UW + 0x0c)) Rs5 Ra0 A1 (av - 10)%nat
              ltac:(nz) ltac:(nz) with "Hcg Hpc Hi0c [-]").
    iIntros "Hcg Hpc".
    ...
    (* +0x0e c.mv s1,a1 *)
    iApply (wp_cmv_s_sconf γ Φ (mword_of_int (UW + 0x0e)) Rs1 Ra1 A2 (av - 10)%nat
              ltac:(nz) ltac:(nz) with "Hcg Hpc Hi0e [-]").
    iIntros "Hcg Hpc".
    ...
    iApply (wp_auipc_s_sconf γ Φ (mword_of_int (UW + 0x10)) Ra0 (mword_of_int 18 : mword 20)
              A3 (av - 10)%nat ltac:(nz) ltac:(nz) with "Hcg Hpc Hi10 [-]").
    iIntros "Hcg Hpc".
    set (A4 := <[Regidx Ra0 := regval_into_reg
        (add_vec (mword_of_int (UW + 0x10) : mword 64) (auipc_off (mword_of_int 18 : mword 20)))]> A3).
    change (<[Regidx Ra0 := regval_into_reg
        (add_vec (mword_of_int (UW + 0x10) : mword 64) (auipc_off (mword_of_int 18 : mword 20)))]> A3) with A4.
    iEval (rewrite P14) in "Hpc".
    iApply (wp_addi4_s_sconf γ Φ (mword_of_int (UW + 0x14)) Ra0 Ra0 (mword_of_int 0xa24 : mword 12)
              A4 (av - 10)%nat ltac:(nz) ltac:(nz) with "Hcg Hpc Hi14 [-]").
    iIntros "Hcg Hpc".
    ...
    (* +0x18 jal ra,acquire *)
    iApply (wp_jal_s_sconf γ Φ (mword_of_int (UW + 0x18)) Rra (mword_of_int 788 : mword 21)
              A5 (av - 10)%nat ltac:(nz) ltac:(nz) ltac:(vm_compute; reflexivity)
              with "Hcg Hpc Hi18 [-]").
    iIntros "Hcg Hpc".
    set (A6 := <[Regidx Rra := regval_into_reg (add_vec_int (mword_of_int (UW + 0x18) : mword 64) 4)]> A5).
    change (<[Regidx Rra := regval_into_reg (add_vec_int (mword_of_int (UW + 0x18) : mword 64) 4)]> A5) with A6.
    assert (Hjacq : add_vec (mword_of_int (UW + 0x18) : mword 64)
    ...
    { rewrite /A6 upd_ne; [| reg_neq]. rewrite /A5 upd_eq. rewrite /A4 upd_eq.
      rewrite /a_tx_lock. pcw. }
    assert (HA6tp : A6 !!! Regidx Rtp = cid_word).
    ...
    assert (HA6ra : A6 !!! Regidx Rra = add_vec_int (mword_of_int (UW + 0x18) : mword 64) 4)
    ...
    (* ============ acquire(&tx_lock) ============ *)
    iApply (Acquire.wp_acquire_sconf γ Φ γl "uart"%string (tx_res γu) A6
              0%nat eb pj C (av - 10)%nat HA6tp ltac:(lia)
              ltac:(unfold uartwrite_stack in Hav; lia)
    ...
    assert (HMAtp : MA !!! Regidx Rtp = cid_word)
      by (rewrite (callee_saved_lookup HcsA (mword_of_int 4) ltac:(vm_compute; reflexivity)); exact HA6tp).
    ...
    (* ============ +0x1c  blez s1 ============ *)
    assert (Hcmp0 : zopz0zKzJ_s (zero_reg : mword 64) (MA !!! Regidx Rs1) = Z.geb 0 (Z.of_nat n)).
    { rewrite HMAs1. apply uw_geb_s0. assert (H263 : (2 ^ 63 = 9223372036854775808)%Z)
    ...
    - (* ======== n = 0: straight to the epilogue ======== *)
      ...
      subst n.
      assert (Hal : eq_vec (access_vec_dec (add_vec (mword_of_int (UW + 0x1c) : mword 64)
                      (sign_extend' 64 (mword_of_int 96 : mword 13))) 0) ('b"0") = true)
        by (vm_compute; reflexivity).
      iApply (wp_bge_x0_taken_s_sconf γ Φ (mword_of_int (UW + 0x1c)) (mword_of_int 96 : mword 13)
                Rs1 MA (av - 10)%nat ltac:(nz) ltac:(exact Hcmp0) Hal
                with "Hcg Hpc Hi1c [-]").
      iNext. iIntros "Hcg Hpc".
      iAssert (▷ sched_vc γ Φ γs (a_cpu_ctx cid_word) pj)%I with "[Hsched]" as "Hsched".
      { iNext. iExact "Hsched". }
      assert (Jtail : add_vec (mword_of_int (UW + 0x1c) : mword 64)
                        (sign_extend' 64 (mword_of_int 96 : mword 13)) = mword_of_int (UW + 0x7c)) by pcw.
      ...
      iApply (uw_tail γ γl γu Φ γs j m MA av eb C sp0 ([] : list (bv 8)) (uw_buf buf dq f 0%nat)
                ltac:(unfold uw_tail_regs; split_and!; assumption) Hspm Htp Hav
                with "Ht Htxl Hcg Hcnt Hpay Hpc Htok HR Hsub0 Hsv Hgap Hs10 Hbuf Hctx Hsched [Hcont]").
      iIntros (mf) "%Hcs Hcg Hcnt Hpc Hbuf #Hout Hctx Hsched".
      iApply ("Hcont" $! mf with "[%] Hcg Hcnt Hpc [Hbuf] [Hout] Hctx Hsched").
      + exact Hcs.
    ...
    - (* ======== n > 0: the byte loop ======== *)
      ...
      iApply (wp_bge_x0_fall_s_sconf γ Φ (mword_of_int (UW + 0x1c)) (mword_of_int 96 : mword 13)
                Rs1 MA (av - 10)%nat ltac:(nz) ltac:(exact Hcmp0)
                with "Hcg Hpc Hi1c [-]").
      iIntros "Hcg Hpc".
      assert (P20 : add_vec_int (mword_of_int (UW + 0x1c) : mword 64) 4 = mword_of_int (UW + 0x20)) by pcw.
      ...
      assert (P22 : add_vec_int (mword_of_int (UW + 0x20) : mword 64) 2 = mword_of_int (UW + 0x22)) by pcw.
      assert (P24 : add_vec_int (mword_of_int (UW + 0x22) : mword 64) 2 = mword_of_int (UW + 0x24)) by pcw.
      assert (P26 : add_vec_int (mword_of_int (UW + 0x24) : mword 64) 2 = mword_of_int (UW + 0x26)) by pcw.
      assert (P28 : add_vec_int (mword_of_int (UW + 0x26) : mword 64) 2 = mword_of_int (UW + 0x28)) by pcw.
      assert (P2a : add_vec_int (mword_of_int (UW + 0x28) : mword 64) 2 = mword_of_int (UW + 0x2a)) by pcw.
      iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (UW + 0x20)) (mword_of_int 6 : mword 6) Rs2
                MA (av - 10)%nat v4 with "Hcg Hpc Hi20 [H4] [-]").
      { iEval (rewrite HMAcsp Hb4). iExact "H4". }
      iIntros "Hcg Hpc H4". iEval (rewrite HMAcsp Hb4 HMAs2) in "H4".
      iEval (rewrite P22) in "Hpc".
      iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (UW + 0x22)) (mword_of_int 5 : mword 6) Rs3
                MA (av - 10)%nat v5 with "Hcg Hpc Hi22 [H5] [-]").
      { iEval (rewrite HMAcsp Hb5). iExact "H5". }
      iIntros "Hcg Hpc H5". iEval (rewrite HMAcsp Hb5 HMAs3) in "H5".
      iEval (rewrite P24) in "Hpc".
      iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (UW + 0x24)) (mword_of_int 4 : mword 6) Rs4
                MA (av - 10)%nat v6 with "Hcg Hpc Hi24 [H6] [-]").
      { iEval (rewrite HMAcsp Hb6). iExact "H6". }
      iIntros "Hcg Hpc H6". iEval (rewrite HMAcsp Hb6 HMAs4) in "H6".
      iEval (rewrite P26) in "Hpc".
      iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (UW + 0x26)) (mword_of_int 2 : mword 6) Rs6
                MA (av - 10)%nat v8 with "Hcg Hpc Hi26 [H8] [-]").
      { iEval (rewrite HMAcsp Hb8). iExact "H8". }
      iIntros "Hcg Hpc H8". iEval (rewrite HMAcsp Hb8 HMAs6) in "H8".
      iEval (rewrite P28) in "Hpc".
      iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (UW + 0x28)) (mword_of_int 1 : mword 6) Rs7
                MA (av - 10)%nat v9 with "Hcg Hpc Hi28 [H9] [-]").
      { iEval (rewrite HMAcsp Hb9). iExact "H9". }
      iIntros "Hcg Hpc H9". iEval (rewrite HMAcsp Hb9 HMAs7) in "H9".
      ...
      assert (P2c : add_vec_int (mword_of_int (UW + 0x2a) : mword 64) 2 = mword_of_int (UW + 0x2c)) by pcw.
      assert (P2e : add_vec_int (mword_of_int (UW + 0x2c) : mword 64) 2 = mword_of_int (UW + 0x2e)) by pcw.
      assert (P32 : add_vec_int (mword_of_int (UW + 0x2e) : mword 64) 4 = mword_of_int (UW + 0x32)) by pcw.
      assert (P36 : add_vec_int (mword_of_int (UW + 0x32) : mword 64) 4 = mword_of_int (UW + 0x36)) by pcw.
      assert (P3a : add_vec_int (mword_of_int (UW + 0x36) : mword 64) 4 = mword_of_int (UW + 0x3a)) by pcw.
      assert (P3e : add_vec_int (mword_of_int (UW + 0x3a) : mword 64) 4 = mword_of_int (UW + 0x3e)) by pcw.
      assert (P42 : add_vec_int (mword_of_int (UW + 0x3e) : mword 64) 4 = mword_of_int (UW + 0x42)) by pcw.
      assert (P46 : add_vec_int (mword_of_int (UW + 0x42) : mword 64) 4 = mword_of_int (UW + 0x46)) by pcw.
      assert (P4a : add_vec_int (mword_of_int (UW + 0x46) : mword 64) 4 = mword_of_int (UW + 0x4a)) by pcw.
      assert (P4c : add_vec_int (mword_of_int (UW + 0x4a) : mword 64) 2 = mword_of_int (UW + 0x4c)) by pcw.
      (* +0x2a c.mv s4,s5 *)
      iApply (wp_cmv_s_sconf γ Φ (mword_of_int (UW + 0x2a)) Rs4 Rs5 MA (av - 10)%nat
                ltac:(nz) ltac:(nz) with "Hcg Hpc Hi2a [-]").
      iIntros "Hcg Hpc".
      ...
      (* +0x2c c.add s5,s5,s1 *)
      iApply (wp_cadd_s_sconf γ Φ (mword_of_int (UW + 0x2c)) Rs5 Rs1 B0 (av - 10)%nat
                ltac:(nz) ltac:(nz) with "Hcg Hpc Hi2c [-]").
      iIntros "Hcg Hpc".
      ...
      iApply (wp_auipc_s_sconf γ Φ (mword_of_int (UW + 0x2e)) Rs1 (mword_of_int 10 : mword 20)
                B1 (av - 10)%nat ltac:(nz) ltac:(nz) with "Hcg Hpc Hi2e [-]").
      iIntros "Hcg Hpc".
      set (B2 := <[Regidx Rs1 := regval_into_reg
          (add_vec (mword_of_int (UW + 0x2e) : mword 64) (auipc_off (mword_of_int 10 : mword 20)))]> B1).
      change (<[Regidx Rs1 := regval_into_reg
          (add_vec (mword_of_int (UW + 0x2e) : mword 64) (auipc_off (mword_of_int 10 : mword 20)))]> B1) with B2.
      iEval (rewrite P32) in "Hpc".
      iApply (wp_addi4_s_sconf γ Φ (mword_of_int (UW + 0x32)) Rs1 Rs1 (mword_of_int 0x922 : mword 12)
                B2 (av - 10)%nat ltac:(nz) ltac:(nz) with "Hcg Hpc Hi32 [-]").
      iIntros "Hcg Hpc".
      ...
      iApply (wp_auipc_s_sconf γ Φ (mword_of_int (UW + 0x36)) Rs3 (mword_of_int 18 : mword 20)
                B3 (av - 10)%nat ltac:(nz) ltac:(nz) with "Hcg Hpc Hi36 [-]").
      iIntros "Hcg Hpc".
      set (B4 := <[Regidx Rs3 := regval_into_reg
          (add_vec (mword_of_int (UW + 0x36) : mword 64) (auipc_off (mword_of_int 18 : mword 20)))]> B3).
      change (<[Regidx Rs3 := regval_into_reg
          (add_vec (mword_of_int (UW + 0x36) : mword 64) (auipc_off (mword_of_int 18 : mword 20)))]> B3) with B4.
      iEval (rewrite P3a) in "Hpc".
      iApply (wp_addi4_s_sconf γ Φ (mword_of_int (UW + 0x3a)) Rs3 Rs3 (mword_of_int 0x9fe : mword 12)
                B4 (av - 10)%nat ltac:(nz) ltac:(nz) with "Hcg Hpc Hi3a [-]").
      iIntros "Hcg Hpc".
      ...
      iApply (wp_auipc_s_sconf γ Φ (mword_of_int (UW + 0x3e)) Rs2 (mword_of_int 10 : mword 20)
                B5 (av - 10)%nat ltac:(nz) ltac:(nz) with "Hcg Hpc Hi3e [-]").
      iIntros "Hcg Hpc".
      set (B6 := <[Regidx Rs2 := regval_into_reg
          (add_vec (mword_of_int (UW + 0x3e) : mword 64) (auipc_off (mword_of_int 10 : mword 20)))]> B5).
      change (<[Regidx Rs2 := regval_into_reg
          (add_vec (mword_of_int (UW + 0x3e) : mword 64) (auipc_off (mword_of_int 10 : mword 20)))]> B5) with B6.
      iEval (rewrite P42) in "Hpc".
      iApply (wp_addi4_s_sconf γ Φ (mword_of_int (UW + 0x42)) Rs2 Rs2 (mword_of_int 0x90e : mword 12)
                B6 (av - 10)%nat ltac:(nz) ltac:(nz) with "Hcg Hpc Hi42 [-]").
      iIntros "Hcg Hpc".
      ...
      iApply (wp_lui_s_sconf γ Φ (mword_of_int (UW + 0x46)) Rs7 (mword_of_int 0x10000 : mword 20)
                (uart_pa 0) B7 (av - 10)%nat ltac:(nz) ltac:(nz)
      ...
      (* +0x4a c.li s6,1 *)
      iApply (wp_cli_s_sconf γ Φ (mword_of_int (UW + 0x4a)) Rs6 (mword_of_int 1 : mword 6)
                (mword_of_int 1 : mword 64) B8 (av - 10)%nat ltac:(nz) ltac:(nz)
      ...
      iApply (wp_cj_s_sconf γ Φ (mword_of_int (UW + 0x4c))
                (sign_extend' 21 (concat_vec (mword_of_int 16 : mword 11) ('b"0")))
                B9 (av - 10)%nat ltac:(vm_compute; reflexivity) with "Hcg Hpc Hi4c [-]").
      iNext. iIntros "Hcg Hpc".
      iAssert (▷ sched_vc γ Φ γs (a_cpu_ctx cid_word) pj)%I with "[Hsched]" as "Hsched".
      { iNext. iExact "Hsched". }
      assert (Jhead : add_vec (mword_of_int (UW + 0x4c) : mword 64)
                        (sign_extend' 64 (sign_extend' 21 (concat_vec (mword_of_int 16 : mword 11) ('b"0"))))
                      = mword_of_int (UW + 0x6c)) by pcw.
      ...
      { unfold uw_loop_regs. split_and!.
        ...
        - rewrite /B9 upd_ne; [| reg_neq]. rewrite /B8 upd_ne; [| reg_neq].
          ...
          rewrite /B5 upd_ne; [| reg_neq]. rewrite /B4 upd_ne; [| reg_neq].
          rewrite /B3 upd_ne; [| reg_neq]. rewrite /B2 upd_ne; [| reg_neq].
          rewrite /B1 upd_ne; [| reg_neq]. rewrite /B0 upd_ne; [| reg_neq]. exact HMAtp.
        - rewrite /B9 upd_ne; [| reg_neq]. rewrite /B8 upd_ne; [| reg_neq].
          rewrite /B7 upd_ne; [| reg_neq]. rewrite /B6 upd_ne; [| reg_neq].
          rewrite /B5 upd_ne; [| reg_neq]. rewrite /B4 upd_ne; [| reg_neq].
      ...
      iPoseProof (uw_iter γ γl γu γv Φ γs j γlp m av eb C sp0 buf n f dq (n - 1)%nat
                    ltac:(lia) Hj Hjlp Hav) as "Iter".
      iApply ("Iter" $! 0%nat B9 with "[%] [%] Ht Hdinv Htxl Hpinv Hpanic Hcg Hcnt Hpay Hpc Htok HR Hsub0 Hfull Hbuf Hctx Hsched [Hcont]").
      { lia. }
      { exact HB9regs. }
      (* ============ the loop's exit: +0x72 -> the epilogue ============ *)
      rewrite /uw_exit_cont.
      iIntros (M') "%Hregs' Hcg Hcnt Hpay Hpc Htok HR #Hout Hfull Hbuf Hctx Hsched".
      iDestruct "Hfull" as "(Hsv & Hsv5 & Hs10)".
      rewrite /uw_saved5.
      iDestruct "Hsv5" as "(G4 & G5 & G6 & G8 & G9)".
      pose proof Hregs' as Hregs''.
      destruct Hregs'' as (Wsp & Wtp & Ws1 & Ws2 & Ws3 & Ws4 & Ws5 & Ws6 & Ws7 & W24 & W25 & W26 & W27).
      ...
      assert (P74 : add_vec_int (mword_of_int (UW + 0x72) : mword 64) 2 = mword_of_int (UW + 0x74)) by pcw.
      assert (P76 : add_vec_int (mword_of_int (UW + 0x74) : mword 64) 2 = mword_of_int (UW + 0x76)) by pcw.
      assert (P78 : add_vec_int (mword_of_int (UW + 0x76) : mword 64) 2 = mword_of_int (UW + 0x78)) by pcw.
      assert (P7a : add_vec_int (mword_of_int (UW + 0x78) : mword 64) 2 = mword_of_int (UW + 0x7a)) by pcw.
      assert (P7c : add_vec_int (mword_of_int (UW + 0x7a) : mword 64) 2 = mword_of_int (UW + 0x7c)) by pcw.
      iApply (wp_cldsp_s_sconf γ Φ (mword_of_int (UW + 0x72)) (mword_of_int 6 : mword 6) Rs2
                M' (av - 10)%nat (m !!! Regidx Rs2) (dqm := DfracOwn 1) ltac:(nz) ltac:(nz)
                with "Hcg Hpc Hi72 [G4] [-]").
      { iEval (rewrite Wsp Hb4). iExact "G4". }
      iIntros "Hcg Hpc G4". iEval (rewrite Wsp Hb4) in "G4".
      ...
      iApply (wp_cldsp_s_sconf γ Φ (mword_of_int (UW + 0x74)) (mword_of_int 5 : mword 6) Rs3
                R1 (av - 10)%nat (m !!! Regidx Rs3) (dqm := DfracOwn 1) ltac:(nz) ltac:(nz)
                with "Hcg Hpc Hi74 [G5] [-]").
      { iEval (rewrite HR1sp Hb5). iExact "G5". }
      iIntros "Hcg Hpc G5". iEval (rewrite HR1sp Hb5) in "G5".
      ...
      iApply (wp_cldsp_s_sconf γ Φ (mword_of_int (UW + 0x76)) (mword_of_int 4 : mword 6) Rs4
                R2 (av - 10)%nat (m !!! Regidx Rs4) (dqm := DfracOwn 1) ltac:(nz) ltac:(nz)
                with "Hcg Hpc Hi76 [G6] [-]").
      { iEval (rewrite HR2sp Hb6). iExact "G6". }
      iIntros "Hcg Hpc G6". iEval (rewrite HR2sp Hb6) in "G6".
      ...
      iApply (wp_cldsp_s_sconf γ Φ (mword_of_int (UW + 0x78)) (mword_of_int 2 : mword 6) Rs6
                R3 (av - 10)%nat (m !!! Regidx Rs6) (dqm := DfracOwn 1) ltac:(nz) ltac:(nz)
                with "Hcg Hpc Hi78 [G8] [-]").
      { iEval (rewrite HR3sp Hb8). iExact "G8". }
      iIntros "Hcg Hpc G8". iEval (rewrite HR3sp Hb8) in "G8".
      ...
      iApply (wp_cldsp_s_sconf γ Φ (mword_of_int (UW + 0x7a)) (mword_of_int 1 : mword 6) Rs7
                R4 (av - 10)%nat (m !!! Regidx Rs7) (dqm := DfracOwn 1) ltac:(nz) ltac:(nz)
                with "Hcg Hpc Hi7a [G9] [-]").
      { iEval (rewrite HR4sp Hb9). iExact "G9". }
      iIntros "Hcg Hpc G9". iEval (rewrite HR4sp Hb9) in "G9".
      ...
      { unfold uw_tail_regs. split_and!.
        ...
        - rewrite /R5 upd_ne; [| reg_neq]. rewrite /R4 upd_ne; [| reg_neq].
          ...
          rewrite /R1 upd_ne; [| reg_neq]. exact Wtp.
        - rewrite /R5 upd_ne; [| reg_neq]. rewrite /R4 upd_ne; [| reg_neq].
          rewrite /R3 upd_ne; [| reg_neq]. rewrite /R2 upd_ne; [| reg_neq].
      ...
      iApply (uw_tail γ γl γu Φ γs j m R5 av eb C sp0 (uw_bytes f n) (uw_buf buf dq f n)
                HR5regs Hspm Htp Hav
                with "Ht Htxl Hcg Hcnt Hpay Hpc Htok HR Hout Hsv Hgap Hs10 Hbuf Hctx Hsched [Hcont]").
      iIntros (mf) "%Hcs Hcg Hcnt Hpc Hbuf #Hout2 Hctx Hsched".
      iApply ("Hcont" $! mf with "[%] Hcg Hcnt Hpc [Hbuf] [Hout2] Hctx Hsched").
      + exact Hcs.
      + rewrite /uw_buf. iExact "Hbuf".
      + iExact "Hout2".
  Qed.
