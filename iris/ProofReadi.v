...
Section ReadiRet.
  ...
  Local Lemma rd_ret `{CID0 : CpuId} 
    ...
    rd_dst (ktb := ktb) γf j pidv dq user (upd_upt V P') V
           ...
           (rd_delivered data dst_olds off tot) -∗
    bslot -∗
    rd_cont (ktb := ktb) (CID0 := CID0) bn γf ip bm data dn user off n dst_olds V
...
Section ReadiJoin.
  ...
  Proof.
    ...
    iDestruct (cpu_claim_ext_transport CID0 CID2 eb (proc_addr j)
                 ltac:(rewrite Heb2b; wp_next_chain) with "Hextm") as "Hextm".
    iApply (rd_ret (CID0 := CID2) bn γf ip bm data dn
...
Section ReadiExit.
  ...
  Proof.
    ...
    iDestruct (cpu_claim_ext_transport CID0 CID6 eb (proc_addr j)
                 ltac:(rewrite Heb2b; wp_next_chain) with "Hextm") as "Hextm".
    iApply (rd_join (CID0 := CID6) bn γf ip bm data dn
...
Section ReadiLoop.
  ...
  Proof.
    ...
    iIntros "Hcg Hcnt Hextc Hextm #Htext #Hkd Hpc #Hpenv #Hbio #Hrow #Hkenv #Hprocs
    ...
    iApply (BR.wp_bread_sconf γs j γl γu γd γk pd pav pu bn
              (fs_view fsc_fs γd icfg_dev fsc_cov) pidv icfg_dev (blkmap_get bm fbn)
              (rd_q user dq)
    ...
    iDestruct (inode_blocks_q_acc fsc_fs dq bm data fbn Hfbnlt Hbnzz with "Hblocks")
      as "[Hfsb1 Hblback]".
    iEval (rewrite -Hubno) in "Hfsb1".
    iEval (rewrite /bio_locked) in "Hheld".
    iDestruct (rd_held_k with "Hheld") as %Hkklt.
    iApply fupd_wp.
    iMod (rd_held_content ⊤ bn fsc_fs γd dq icfg_dev fsc_cov _ pidv icfg_dev _
    ...
    { iIntros (CIDb Mb mm) "%Hanch %Hmmd %Hbsp %Hbs10 %Hba5 %Hbs2 %Hbs4 %Hbs7
      ...
      - (* ============ THE COPY SUCCEEDED ============ *)
        ...
        iDestruct (rd_blocks_restore fsc_fs dq bm data fbn with "Hblocks") as "Hblocks".
        ...
        + (* ---------- finished: leave the loop at +0xbe ---------- *)
          ...
          iDestruct (cpu_claim_ext_transport CIDa14 CIDc11 eb (proc_addr j)
                       ltac:(rewrite Heb2b; wp_next_chain) with "Hextm") as "Hextm".
          iApply (rd_exit (CID0 := CIDc11) bn γf ip bm data dn
                    ...
                    with "Hcg Hcnt Hextc Hextm Htext Hpc [] [] [] [] [] []
                          ...
                          Hdst2 Hsl [Hcont]").
          { iApply (rdi_0be with "Htext"). }
          { iApply (rdi_0c0 with "Htext"). }
          { iApply (rdi_0c2 with "Htext"). }
          { iApply (rdi_0c4 with "Htext"). }
          { iApply (rdi_0c6 with "Htext"). }
          { iApply (rdi_0c8 with "Htext"). }
      ...
      - (* ====== THE COPY OUT FAULTED -- only reachable on the USER arm,
        ...
        iDestruct (cpu_claim_ext_transport CIDa14 CIDd8 eb (proc_addr j)
                     ltac:(rewrite Heb2b; wp_next_chain) with "Hextm") as "Hextm".
        iApply (rd_exit (CID0 := CIDd8) bn γf ip bm data dn
                  ...
                  with "Hcg Hcnt Hextc Hextm Htext Hpc [] [] [] [] [] []
                        Hframe Hidev Hmeta Hmap Hblocks
                        Hdst3 Hsl [Hcont]").
        { iApply (rdi_0b2 with "Htext"). }
        { iApply (rdi_0b4 with "Htext"). }
        { iApply (rdi_0b6 with "Htext"). }
        { iApply (rdi_0b8 with "Htext"). }
        { iApply (rdi_0ba with "Htext"). }
        { iApply (rdi_0bc with "Htext"). }
...
Section ReadiMain.
 ...
 ip bm data dn
  ...
  Proof.
    ...
    iAssert (rd_cont (ktb := ktb) (CID0 := CID) bn γf ip bm data dn user off n
    ...
    { iIntros (CIDt Mt nc) "%Hanch %Hncdef %Hncn %Hoffnc %Htsp %Hts5 %Hts6 %Hts7
      ...
      { (* ---------- nc = 0: nothing to read ---------- *)
        ...
        iDestruct (cpu_claim_ext_transport CID CIDz3 eb (proc_addr j)
                     ltac:(rewrite Heb2b; wp_next_chain) with "Hextm") as "Hextm".
        iApply (rd_join (CID0 := CIDz3) bn γf ip bm data dn
 ...
 ip bm data dn user off n nc szn dst_olds V
