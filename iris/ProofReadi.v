...
Section ReadiLoop.
  ...
  (* RULE ONE (claude-notes/optimization.md): the +0x4c chunk body both
     ...
     folded here. *)
  Definition rd_chunk_body `{XI : CurCtx}
 ...
 (γf : gname)
  ...
  Proof using KtierLe0.
    ...
    iApply fupd_wp.
    iMod (rd_held_content ⊤ fsc_bio fsc_fs fsc_disk dq icfg_dev fsc_cov _ pidv icfg_dev _
    ...
    { iIntros (CIDb Mb mm) "%Hanch %Hmmd %Hbsp %Hbs10 %Hba5 %Hbs2 %Hbs4 %Hbs7
      ...
      iAssert ((if user
               ...
               ∗ (if user then True
                  else proc_priv_bare (proc_addr j) pidv U
                       ∗ ([∗ list] jj ∈ seq 0 tot,
      ...
      iApply (wp_jal_s_sconf (mword_of_int (RI + 0x60)) Rra
                (mword_of_int 2092014 : mword 21) D7 (K - 14)%nat b
                ltac:(nz) ltac:(rdok) ltac:(vm_compute; reflexivity)
                with "Hcg Hpc []").
      ...
      assert (Htgtec : add_vec (mword_of_int (RI + 0x60) : mword 64)
                         (sign_extend' 64 (mword_of_int 2092014 : mword 21))
