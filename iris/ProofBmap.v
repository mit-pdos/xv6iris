...
Definition bm_gen_stmt
  ...
  fs_bytes_any γfs -∗
  i_dev ip ↦₄{dqd} dev -∗
  (* the block runs at a SHARE [dq] -- lane B''-blk; see [SpecBmap]'s
     no-alloc contract.  The allocating instantiation passes
     [DfracOwn 1] and [wp_bmap_gen]'s [Hdq1] premise makes that the only
     possibility whenever [ak <> None]. *)
...
Section BmapRelease.
  ...
  Proof.
    ...
    iApply (bm_epilogue (CID0 := CID4)  j γfs bn ak cov logstart dev ip bm bm'
              data data' fbn n n' cr Sb Sb' rv pidv dq dqd m T2 K eb b lks Upr
...
Section BmapTail.
  ...
  Local Lemma bm_indirect_tail `{GEN : GenId} `{CID0 : CpuId} `{XI : CurCtx} 
    ...
    (* THE ALLOCATING ARMS ARE FRACTION-1 ARMS, and only they: balloc hands
       over a FULL byte run, so depositing it into the bundle needs the
       bundle at 1.  A caller that cannot allocate ([ak = None]) supplies
       this vacuously and may hold any share (lane B''-blk). *)
    (ak <> None -> dq = DfracOwn 1) ->
    sie_cap_gpr KT1 M (K - 6)%nat b (proc_addr j) -∗
    cpu_own 0 eb (proc_addr j) b lks -∗
    trap_csrs_ext KT1 eb -∗
    cpu_claim_ext eb (proc_addr j) -∗
    ...
    ind_blk_q γfs dq bmI -∗
    inode_blocks_q γfs dq bmI data -∗
    bslots 1 -∗
  ...
  Proof.
    ...
    iApply (wp_lw_s_sconf (kt := KT1) (ktd := KT0) (mword_of_int (KernelSyms.bmap + 0x64)) Ra0 Rs2
    ...
    iApply (wp_clw_s_sconf (kt := KT1) (ktd := KT0) (mword_of_int (KernelSyms.bmap + 0x7e)) Rs1 Ra5
    ...
    - (* ================= the entry is EMPTY: allocate ================= *)
      ...
      (* AN ALLOCATING ARM IS A FRACTION-1 ARM (lane B''-blk) *)
      pose proof (Hdq1 ltac:(discriminate)) as Hd1.
      ...
      iApply (wp_lw_s_sconf (kt := KT1) (ktd := KT0) (mword_of_int (KernelSyms.bmap + 0x9a)) Ra0 Rs2
      ...
      + (* ---------- balloc FAILED: brelse and return 0 ---------- *)
        ...
        iApply (bm_release (CID0 := CID17)  γs j γfs γd bn (Some (MkBmAlloc γ bms sz dqb dqs γpr)) cov logstart dev
                  ...
                  pidv dq dqd m F0 K eb b lks Upr
      ...
      + (* ---------- balloc SUCCEEDED: install and log ---------- *)
        ...
        iEval (rewrite (ind_blk_q_run γfs dq bmI _ Hindnz Huind)) in "Hindblk".
        iDestruct (inode_fresh_q γfs dq bmI data (bv_unsigned blk)
        ...
        iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.bmap + 0xac)) Rra
                  (mword_of_int 3572 : mword 21) G1 (K - 6)%nat b
                  ltac:(nz) ltac:(rdok) ltac:(vm_compute; reflexivity)
                  with "Hcg Hpc []").
        ...
        assert (Htgtlw : add_vec (mword_of_int (KernelSyms.bmap + 0xac) : mword 64)
                           (sign_extend' 64 (mword_of_int 3572 : mword 21))
        ...
        (* log_write moves the logged view at those bytes, so it takes the
           byte run at fraction 1 -- available here because this is an
           allocating arm (lane B''-blk) *)
        iDestruct (fsblock_q_1_of (fs_bytes γfs) dq _ _ Hd1 with "Hindblk")
          as "Hindblk".
        ...
        iDestruct (inode_blocks_q_1_of γfs dq bmI data Hd1 with "Hblocks")
          as "Hblocks".
        ...
        iDestruct (inode_blocks_q_1_to γfs dq bmJ _ Hd1 with "Hblocks")
          as "Hblocks".
        ...
        iApply (bm_release (CID0 := CID22)  γs j γfs γd bn (Some (MkBmAlloc γ bms sz dqb dqs γpr)) cov logstart dev
                  ...
                  Upr HK HmLsp HmLthr HmLs1 HmLs4 Hkk HwfJ
    ...
    - (* ================= the entry is PRESENT: brelse and return ====== *)
      ...
      iApply (bm_release (CID0 := CID12)  γs j γfs γd bn ak cov logstart dev
                ...
                kk (bm_ind bmI) (ind_bytes (bm_ent bmI)) bsd0 d0
                pidv dq dqd m B7 K eb b lks Upr
...
Section ProofBmapMain.
  ...
  Lemma wp_bmap_gen 
      ...
      (* see [bm_indirect_tail]: only the allocating arms need fraction 1 *)
  ...
  Proof.
    ...
    iAssert (bm_cont (CID0 := CID) γfs bn ak cov logstart dev ip bm data fbn n cr Sb
               pidv dq dqd j m K eb b lks Upr)%I with "[Hcont]" as "Hcont";
    ...
    - (* ================== THE DIRECT ARM (bn < NDIRECT) ================== *)
      ...
      iEval (rewrite -Hcadr) in "Hcell".
      iApply (wp_lw_s_sconf (kt := KT1) (ktd := KT0) (mword_of_int (KernelSyms.bmap + 0x22)) Rs1 Rs3
      ...
      + (* ---------- the slot is EMPTY: balloc and install ---------- *)
        ...
        iEval (rewrite -Hdadr) in "Hidev".
        iApply (wp_clw_s_sconf (kt := KT1) (ktd := KT0) (mword_of_int (KernelSyms.bmap + 0x28)) Ra0 Ra0
        ...
        * (* ------ balloc FAILED: return 0 ------ *)
          ...
          iAssert (inode_map_q γfs dq ip bm) with "[Haddrs Hindblk]" as "Hmap".
          { rewrite /inode_map_q /ind_res_q.
          ...
          iApply (bm_epilogue (CID0 := CID20)  j γfs bn (Some (MkBmAlloc γ bms sz dqb dqs γpr)) cov logstart dev ip bm bm
                    data data fbn n n cr Sb Sb (mword_of_int 0 : mword 32) pidv dq dqd m E0 K eb b lks Upr
        ...
        * (* ------ balloc SUCCEEDED: install into addrs[bn] ------ *)
          iDestruct "Hsucc" as (blk)
            "(%Ha0v & %Hblknz & %Hblkcov & %Hblklog & Hfsb & Hop)".
          iDestruct (inode_fresh_q γfs dq bm data (bv_unsigned blk)
                       with "Hfsb Hindblk Hblocks") as %Hfresh.
          iApply (wp_cmv_s_sconf (mword_of_int (KernelSyms.bmap + 0x2e)) Rs1 Ra0
                    mD (K - 6)%nat b ltac:(nz) ltac:(rdok) with "Hcg Hpc []").
          { iApply (bmi_2e with "Htext"). }
          ...
          iAssert (inode_map_q γfs dq ip bmD) with "[Haddrs Hindblk]" as "Hmap".
          { rewrite /inode_map_q /ind_res_q /ind_blk_q /bmD.
            cbn [bm_ind bm_ent bm_dir].
            iSplitL "Haddrs"; [iExact "Haddrs"|]. iExact "Hindblk". }
          iDestruct (inode_blocks_q_1_of γfs dq bm data Hd1 with "Hblocks")
            as "Hblocks".
          iDestruct (inode_blocks_insert γfs bm bmD data fbn blk
                       (replicate BSIZE (bv_0 8)) Hfbn Hdz HgetDf HgetD
                       with "Hblocks Hfsb") as "Hblocks".
          iDestruct (inode_blocks_q_1_to γfs dq bmD _ Hd1 with "Hblocks")
            as "Hblocks".
          iAssert (bm_frame m) with "[Hf1 Hf2 Hf3 Hf4 Hf5 Hf6]" as "Hframe".
          { rewrite /bm_frame.
            iSplitL "Hf1"; [iExact "Hf1"|]. iSplitL "Hf2"; [iExact "Hf2"|].
          ...
          iApply (bm_epilogue (CID0 := CID22)  j γfs bn (Some (MkBmAlloc γ bms sz dqb dqs γpr)) cov logstart dev ip bm bmD
                    ...
                    pidv dq dqd m N0 K eb b lks Upr
      ...
      + (* ---------- the slot is ALREADY ALLOCATED: return it ---------- *)
        ...
        iApply (bm_epilogue (CID0 := CID15)  j γfs bn ak cov logstart dev ip bm bm
                  data data fbn n n cr Sb Sb (blkmap_get bm fbn) pidv dq dqd m D3 K eb b lks Upr
    ...
    - (* ================ THE INDIRECT ARM (bn >= NDIRECT) ================ *)
      ...
      iApply (wp_lw_s_sconf (kt := KT1) (ktd := KT0) (mword_of_int (KernelSyms.bmap + 0x48)) Rs1 Ra0
      ...
      + (* ------ NO indirect block yet: allocate one ------ *)
        ...
        iEval (rewrite -Hdadr) in "Hidev".
        iApply (wp_clw_s_sconf (kt := KT1) (ktd := KT0) (mword_of_int (KernelSyms.bmap + 0x4e)) Ra0 Ra0
        ...
        * (* ------ balloc FAILED: return 0 ------ *)
          ...
          iApply (bm_epilogue (CID0 := CID22)  j γfs bn (Some (MkBmAlloc γ bms sz dqb dqs γpr)) cov logstart dev ip bm bm
                    data data fbn n n cr Sb Sb (mword_of_int 0 : mword 32) pidv dq dqd m P2 K eb b lks Upr
        ...
        * (* ------ balloc SUCCEEDED: install the indirect block ------ *)
          ...
          iDestruct (ind_blk_q_1_to γfs dq bmI Hd1 with "Hindblk2")
            as "Hindblk2".
          iDestruct (inode_blocks_q_frame γfs dq bm bmI data data
          ...
          iApply (bm_indirect_tail (CID0 := CID25)  γs j γl γu γd γk pd pav pu
                    ...
                    Sb (Sb ∪ {[bms]} ∪ {[bv_unsigned blk]})
                    pidv dq dqd m P2 K eb b lks Upr
                    ...
                    Hbc Hlog ltac:(intros _; exact (Hdq1 ltac:(discriminate)))
      ...
      + (* ------ the indirect block EXISTS ------ *)
        ...
        iApply (bm_indirect_tail (CID0 := CID18)  γs j γl γu γd γk pd pav pu
                  ...
                  cr cr false Sb Sb
                  pidv dq dqd m J4 K eb b lks Upr
...
Section BmapSeal.
  ...
  Proof.
    ...
    (* the core is stated at a SHARE of the block runs (lane B''-blk); the
       ALLOCATING instantiation is the fraction-1 one, and [inode_blocks]
       is sealed against instance search, so the crossing is explicit. *)
...
Section BmapNoallocSeal.
  ...
  Proof.
    ...
    iApply (Core.wp_bmap_gen γs j γl γu γd γk pd pav pu bn None γfs
              cov logstart dev ip bm data fbn 0%nat false ∅ pidv dq dqd m K eb b lks Upr
