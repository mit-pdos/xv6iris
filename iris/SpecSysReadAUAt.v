(* SpecSysReadAUAt.v -- the read AU at the RAW-MAP commit, the sealable form.
   ...
   THE EXACT COUNT RIDES ALONG TOO, and it needs nothing from this file
   but its own re-application: the conjunct

       r = mword_of_int (Z.of_nat d)  \/  r = mword_of_int (-1)

   lives in [wp_sys_read_au_frame], beside the window it is about
   ([SpecSysReadAU]'s "...BUT THE LENGTH IS SAID, AND IT IS THE ANSWER"),
   so BOTH forms below inherit it and both sealed Parameters state it.
   The frame's argument 1 is NAMED for the same reason -- [v1] is what
   the window is stated at -- which is why the parameter lists here carry
   [v v1 v2] and not [v v2]. *)
