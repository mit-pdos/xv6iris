...
(*     "mapped-but-not-executable" fetch-fault arm (MISSING).           *)
(*     FEASIBILITY (traced 2026-07-16): the fault-DELIVERY tower is      *)
(*     already generic -- exec_translateAddr_fetch_walk_u_pagefault      *)
(*     (UmodeWalk.v:134) delivers E_Fetch_Page_Fault from ANY walk error *)
(*     f (it takes the walk-Err and translationException facts as        *)
(*     hyps).  ustep_fetch_unmapped (WpUserFetch.v:216) is the clone     *)
(*     template.  THE BLOCKER: upt_fault_wf (UptInv.v:291) only          *)
(*     quantifies over [spec!!vpn=None] -- it produces the walk error    *)
(*     via upt_unmapped_walk_fault (UptInv.v:446) ONLY for unmapped      *)
(*     vpns; it says NOTHING about a mapped vpn whose fetch-check fails.  *)
(*     So this arm requires STRENGTHENING upt_fault_wf (or a new invar   *)
(*     field: "mapped && ¬uw_check_ok fetch -> the leaf pte's X bit is   *)
(*     clear -> pt_walk returns a permission Err") + a new               *)
(*     upt_noexec_walk_fault lemma feeding the generic deliverer.  This  *)
(*     is an invariant-touching change (wide rebuild), NOT a light clone.*)
(*     Needed because a program may jump into a readable data page.      *)
