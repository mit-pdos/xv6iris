(* WpUserClmul.v -- U-mode USTEP arms for the carryless-multiply ops
   CLMUL / CLMULH / CLMULR.  Each is a retiring two-source compute op, so it
   rides the generic two-source arm ustep_rtype2 as a classification-time
   instance: fix the instruction and the fold f, and discharge ustep_rtype2's
   execute premise with ClmulGpr's exec_execute_CLMUL*_gpr_nz. *)
...
Require Import UptInv ClmulGpr.
...
Section WpUserClmul.
  ...
  Lemma ustep_clmul (rs2 rs1 rd : mword 5)
  ...
  Proof.
    ...
    iApply (ustep_rtype2
              (fun rs2' rs1' rd' => CLMUL (Regidx rs2', Regidx rs1', Regidx rd'))
              (fun a b => subrange_vec_dec (carryless_mul a b) (Z.sub xlen 1) 0)
              va vpn ie w rs2 rs1 rd ms_v sc_v stval_v sepc_v g tlbvec E Φ HN
              (fun rs2' rs1' rd' s Hrd' => exec_execute_CLMUL_gpr_nz rs2' rs1' rd' s Hrd')
              eq_refl Hok Hvec Hchk0 HupdN Hpbmt0 Hcw HSXL Hval Hcanon
              Hvpn_def Hpaal HnotRVC Hdec Hrd).
  Qed.

  Lemma ustep_clmulh (rs2 rs1 rd : mword 5)
      (va : mword 64) (vpn : mword 27) (ie : uwalk_info) (w : mword 32)
      (ms_v sc_v stval_v sepc_v : mword 64)
      (g : gmap regidx (mword 64))
      (tlbvec : vec (option TLB_Entry) (2 ^ 6))
  ...
  Proof.
    intros HN Hok Hvec Hchk0 HupdN Hpbmt0 Hcw HSXL Hval Hcanon Hvpn_def Hpaal HnotRVC Hdec Hrd.
    iApply (ustep_rtype2
              (fun rs2' rs1' rd' => CLMULH (Regidx rs2', Regidx rs1', Regidx rd'))
              (fun a b => subrange_vec_dec (carryless_mul a b) (Z.sub (Z.mul 2 xlen) 1) xlen)
              va vpn ie w rs2 rs1 rd ms_v sc_v stval_v sepc_v g tlbvec E Φ HN
              (fun rs2' rs1' rd' s Hrd' => exec_execute_CLMULH_gpr_nz rs2' rs1' rd' s Hrd')
              eq_refl Hok Hvec Hchk0 HupdN Hpbmt0 Hcw HSXL Hval Hcanon
              Hvpn_def Hpaal HnotRVC Hdec Hrd).
  Qed.

  Lemma ustep_clmulr (rs2 rs1 rd : mword 5)
      (va : mword 64) (vpn : mword 27) (ie : uwalk_info) (w : mword 32)
      (ms_v sc_v stval_v sepc_v : mword 64)
      (g : gmap regidx (mword 64))
      (tlbvec : vec (option TLB_Entry) (2 ^ 6))
  ...
  Proof.
    intros HN Hok Hvec Hchk0 HupdN Hpbmt0 Hcw HSXL Hval Hcanon Hvpn_def Hpaal HnotRVC Hdec Hrd.
    iApply (ustep_rtype2
              (fun rs2' rs1' rd' => CLMULR (Regidx rs2', Regidx rs1', Regidx rd'))
              (fun a b => carryless_mulr a b)
              va vpn ie w rs2 rs1 rd ms_v sc_v stval_v sepc_v g tlbvec E Φ HN
              (fun rs2' rs1' rd' s Hrd' => exec_execute_CLMULR_gpr_nz rs2' rs1' rd' s Hrd')
              eq_refl Hok Hvec Hchk0 HupdN Hpbmt0 Hcw HSXL Hval Hcanon
              Hvpn_def Hpaal HnotRVC Hdec Hrd).
  Qed.
