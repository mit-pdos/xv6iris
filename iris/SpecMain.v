...
Section SpecMain.
  ...
  (* ------------------------------------------------------------------- *)
  (* The eleven locks the init sequence brings up.  TEN are [struct       *)
  (* spinlock]s, each as [SpecProcinit.lk_raw] -- the three cells         *)
  (* [lk ↦₄ _], [lock_name_field lk ↦₈ _], [lk_cpu lk ↦₈ _] bundled with  *)
  (* their values existentially quantified, which is the only shape a     *)
  (* caller can honestly claim about a static global it has never         *)
  (* written.                                                            *)
  (*                                                                     *)
  (* [tx_lock] IS THE ONE EXCEPTION, and it is a SLEEPLOCK: xv6 `b7c25cf` *)
  (* restored uartinit's [initsleeplock(&tx_lock, "uart")] -- the call    *)
  (* `ae96fd0` had deleted without replacing (kernel-defects.md D2) -- so *)
  (* what consoleinit passes down is [SleepLock.sl_raw], SIX cells over   *)
  (* 44 bytes rather than three over 24, and what comes back is           *)
  (* [sl_fresh], [sl_fresh_new]'s raw material.  The carve is             *)
  (* correspondingly wider ([BootCarveMain.boot_sl_raw] in place of       *)
  (* [boot_lk_raw]); the linker leaves 48 bytes between [pr] and [kmem]   *)
  (* for it, which is itself the confirmation that the type changed.      *)
  (* [a_tx_lock] IS [mword_of_int KernelSyms.tx_lock] -- spelled in       *)
  (* UartTxInv's name so it matches consoleinit's premise syntactically.  *)
  (* ------------------------------------------------------------------- *)
  Definition main_locks_raw : iProp Σ :=
    (lk_raw (mword_of_int KernelSyms.cons) ∗       (* consoleinit: cons.lock  *)
     sl_raw a_tx_lock ∗                            (* -> uartinit: uart tx    *)
