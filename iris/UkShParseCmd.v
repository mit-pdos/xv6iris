...
(*ALIASES-END*)
  ...
  (* ...and the FOUR that make the EXEC arm's jump-table entry.  The entry
     is a signed displacement from the table's own base, so 0x13c0 plus it
     is 0x81a -- which is checked by [vm_compute] below, not asserted. *)
  Lemma ushp_jrow_exec :
    shp_rodata γt -∗
    [∗ list] j ∈ seq 0 4,
      utext γt (0x13c4 + Z.of_nat j)
        (nth_byte (mword_of_int 4294964314 : mword 32) j).
  Proof using .
    iIntros "#H". rewrite !big_sepL_cons big_sepL_nil.
    iSplit; [ iApply (ushp_ro_byte (0x13c4 + Z.of_nat 0%nat)
                        (nth_byte (mword_of_int 4294964314 : mword 32) 0%nat)
                        ltac:(vm_compute; f_equal; apply bv_eq;
                              vm_compute; reflexivity) with "H") | ].
    iSplit; [ iApply (ushp_ro_byte (0x13c4 + Z.of_nat 1%nat)
                        (nth_byte (mword_of_int 4294964314 : mword 32) 1%nat)
                        ltac:(vm_compute; f_equal; apply bv_eq;
                              vm_compute; reflexivity) with "H") | ].
    iSplit; [ iApply (ushp_ro_byte (0x13c4 + Z.of_nat 2%nat)
                        (nth_byte (mword_of_int 4294964314 : mword 32) 2%nat)
                        ltac:(vm_compute; f_equal; apply bv_eq;
                              vm_compute; reflexivity) with "H") | ].
    iSplit; [ iApply (ushp_ro_byte (0x13c4 + Z.of_nat 3%nat)
                        (nth_byte (mword_of_int 4294964314 : mword 32) 3%nat)
                        ltac:(vm_compute; f_equal; apply bv_eq;
  ...
  Lemma wp_kshp_nul_loop (s0 p : Z) (len : nat) (nn : nat) :
    ...
    urun N h mc (mword_of_int 0x822) nn -∗
    (ushp_exec_at s0 p toks -∗
  ...
  Proof using .
    ...
    all: rewrite Eslot.
    all: iApply (wp_uk_cld N h mc (mword_of_int 0x822)
    ...
    all: iApply (wp_uk_sb N h1 n1 (mword_of_int 0x824)
    ...
    all: iApply (wp_uk_caddi N h2 n1 (mword_of_int 0x828)
    ...
    - (* ======= the LAST token: argv[i+1] is the NULL cap ============== *)
      ...
      rewrite Eslotn.
      (* ---- 0x82a  ld a4,-8(a5) ---- *)
      iApply (wp_uk_ld N h3 n2 (mword_of_int 0x82a)
      ...
      iApply (wp_uk_cbnez N h4 n3 (mword_of_int 0x82e)
      ...
      iIntros (h5) "Hrun".
      (* ---- 0x830  c.j 0x83e ---- *)
      iApply (wp_uk_cj N h5 n3 (mword_of_int 0x830)
  ...
  Lemma wp_kshp_nul_fin (sp0 spl : mword 64) (vals : nat -> mword 64)
    ...
    ustack γd spl 1 -∗
    urun N h me (mword_of_int 0x83e) nn -∗
    (∀ h' : CpuId,
       urun N h'
  ...
  Proof using .
    ...
    set (m10 := <[Regidx a5_idx
                  := regval_into_reg (mword_of_int 0x13c4 : mword 64)]> m9).
    assert (Hm10 : forall q : mword 5, Regidx q <> Regidx a5_idx ->
                     m10 !!! Regidx q = m9 !!! Regidx q)
      by (intros q Hq; exact (upd_ne m9 (Regidx a5_idx) (Regidx q) _ Hq)).
    assert (Ha5_10 : m10 !!! Regidx a5_idx = mword_of_int 0x13c4)
      by exact (upd_eq m9 (Regidx a5_idx)
                  (regval_into_reg (mword_of_int 0x13c4 : mword 64))).
    ...
    iApply (wp_uk_clw_text N h13 m10 (mword_of_int 0x814)
              ...
              (mword_of_int 7 : mword 3) a5_idx a5_idx 0x13c4
              (mword_of_int 4294964314 : mword 32) nn
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; reflexivity)
              ltac:(vm_compute; reflexivity)
              ltac:(rewrite Ha5_10;
                    rewrite (uint_moi 0x13c4 ltac:(unfold Z64; lia));
                    vm_compute uoff_c4; lia)
              ltac:(vm_compute; reflexivity)
              ltac:(vm_compute; discriminate)
              with "[] [] Hrun").
    ...
    set (m11 := <[Regidx a5_idx
                  := regval_into_reg
                       (sign_extend' 64
                          (mword_of_int 4294964314 : mword 32)
                        : mword 64)]> m10).
