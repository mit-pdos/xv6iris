...
Section TrampFetchPt.
  ...
  Definition tramp_tr_obl (Df : register -> dfrac)
    ...
    (□ (∀ (va pax : mword 64) (tv : type_of_register tlb) (rr : option resv),
          ...
          resv_frag cpu_id rr -∗
          (* A6.61: the walk's A/D write-back on the USER table is a ledger
             append, so the translation obligation carries the trapping
             thread's running token -- a parameter of this ∀ (a
             □-obligation cannot capture a linear token), handed straight
             back beside the residue. *)
          ...
          swp (translateAddr (Virtaddr va) (InstructionFetch tt))
            (fun r => ⌜r = Values.Ok (Physaddr pax, PBMT_PMA, init_ext_ptw)⌝ ∗
                      ∃ tv' : type_of_register tlb,
                        Res tv' ∗ TsoCtx.own_context TsoCtx.cur_ctx ∗
                        resv_any cpu_id ∗
  ...
  Section TrampDispatch.
    ...
    Local Notation RtowW W :=
      (fun rsx : regstate =>
         ((TsoCtx.own_context TsoCtx.cur_ctx ∗ W) ∗ Res (register_lookup tlb rsx) ∗ resv_any cpu_id)%I).
    ...
    Local Lemma tramp_ex_adapt (is_rvc : bool) (i : instruction)
      ...
      spt_ex_obl Df pc ms bmi cy ti ip mst0 pcfg paddr mc micfg misa0
        ...
        (TsoCtx.own_context TsoCtx.cur_ctx ∗ W)%I -∗
      (∀ rsf : regstate, ⌜Qtow rsf⌝ -∗ RtowW W rsf -∗
       ...
       swp (execute i)
         (fun e => ⌜e = RETIRE_SUCCESS⌝ ∗
                   ...
                   hreg_frame rs2 s_Drw ∗ hreg_frame_ro Df rs2 s_Dro ∗ Rr rs2)).
    Proof.
      iIntros "Hex" (rsf) "%HQ ((Hrun & HW) & HRes & Hany) Hrw Hro".
      destruct HQ as (tv & ->). rewrite s_rs_tlb.
      iApply ("Hex" $! tv with "[$Hrun $HW] HRes Hany Hrw Hro").
    Qed.
    ...
    Lemma tramp_run_hart_active_instr_S (tlbv : type_of_register tlb)
      ...
      (* A6.61: the trampoline fetch translates on the USER table, whose
         A/D write-back owes the append -- the trapping thread's token
         comes in here, rides the residue through the dispatch, feeds each
         translation, and leaves through every arm of the post. *)
      TsoCtx.own_context TsoCtx.cur_ctx -∗
      Res tlbv -∗
      ...
      spt_ex_obl Df pc ms bmi cy ti ip mst0 pcfg paddr mc micfg misa0
        ...
        (TsoCtx.own_context TsoCtx.cur_ctx ∗ W)%I -∗
      swp (run_hart_active 0)
        (spt_run_post Df Q Rr (fun ii pr => (TsoCtx.own_context TsoCtx.cur_ctx ∗ Qi ii pr)%I)).
    Proof.
      ...
      iIntros "#Hcert Hinstr HW Hfrag0 Hrun HRes Hrw Hro Hdisp #Htr Hex".
      iAssert (((TsoCtx.own_context TsoCtx.cur_ctx ∗ W) ∗ Res tlbv ∗ resv_frag cpu_id None) -∗
               hreg_frame (srs tlbv) s_Drw -∗
               hreg_frame_ro Df (srs tlbv) s_Dro -∗
               swp (dispatchInterrupt Supervisor)
                 (fun o => match o with
                           | Some (ii, pr) => (TsoCtx.own_context TsoCtx.cur_ctx ∗ Qi ii pr)%I
                           | None => ((TsoCtx.own_context TsoCtx.cur_ctx ∗ W) ∗ Res tlbv ∗ resv_frag cpu_id None) ∗
                                     hreg_frame (srs tlbv) s_Drw ∗
                                     hreg_frame_ro Df (srs tlbv) s_Dro
                           end))%I with "[Hdisp]" as "Hdisp'".
      { iIntros "((Hrun & HW) & HRes & Hfrag) Hrw Hro".
        iApply (swp_mono with "[Hrun] [-]");
          [| iApply ("Hdisp" with "HW HRes Hfrag Hrw Hro") ].
        iIntros (o). destruct o as [[ii pr] |].
        - iIntros "H". iFrame "Hrun". iExact "H".
      ...
      - (* ======================= F_Base w ========================== *)
        ...
        + (* ---- 4-aligned: one 4-byte read ---- *)
          ...
          iApply (tramp_ex_w Q Rr (fun ii pr => (TsoCtx.own_context TsoCtx.cur_ctx ∗ Qi ii pr)%I) _).
          iApply (swp_run_hart_active_gen_exf s_Drw s_Dro Df (srs tlbv)
                    Qtow Q (RtowW W) ((TsoCtx.own_context TsoCtx.cur_ctx ∗ W) ∗ Res tlbv ∗ resv_frag cpu_id None)%I Supervisor pc w i 8 Rr (fun ii pr => (TsoCtx.own_context TsoCtx.cur_ctx ∗ Qi ii pr)%I)
                    ...
                    with "Hcert Hrw Hro [$Hrun $HW $HRes $Hfrag0] Hdisp' [] [Hex]").
          2:{ iApply (tramp_ex_adapt false i Q Rr W with "Hex"). }
          iIntros "((Hrun & HW) & HRes & Hany) Hrw Hro".
          iApply (swp_mono with "[] [-]");
            [| iApply (spt_fetch_S_P s_Drw s_Dro Df (srs tlbv) Qtow (RtowW W) pc
                         ...
                         Hbit0 Hbit1 Hpv
                         with "Hcert Hrw Hro [Hany HRes HW Hrun] []") ].
          ...
          * iIntros "Hrw Hro". iRename "Hany" into "Hfrag".
            iApply (swp_mono with "[HW] [-]");
              [| iApply ("Htr" $! pc (pa_of ppn pa) tlbv None with
                           "[%] [%] [%] Hfrag Hrun HRes Hrw Hro") ].
            2:{ exact Hcanon. }
            2:{ exact Hvpn. }
            2:{ rewrite Hident. exact (eq_sym Hpaid). }
            iIntros (v) "(-> & Hf)". iSplitR; [done|].
            iDestruct "Hf" as (tv') "(HRes & Hrun & Hany & Hrw & Hro)".
        ...
        + (* ---- 2 mod 4: two halfword reads, two translations ---- *)
          ...
          iApply (tramp_ex_w Q Rr (fun ii pr => (TsoCtx.own_context TsoCtx.cur_ctx ∗ Qi ii pr)%I) _).
          iApply (swp_run_hart_active_gen_exf s_Drw s_Dro Df (srs tlbv)
                    Qtow Q (RtowW W) ((TsoCtx.own_context TsoCtx.cur_ctx ∗ W) ∗ Res tlbv ∗ resv_frag cpu_id None)%I Supervisor pc
                    (concat_vec (subrange_vec_dec w 31 16)
                       (subrange_vec_dec w 15 0)) i 8 Rr (fun ii pr => (TsoCtx.own_context TsoCtx.cur_ctx ∗ Qi ii pr)%I)
                    s_disj s_in_priv s_in_PC s_w_nPC ltac:(srs_lk_g)
                    ltac:(intros rsf (tv & ->); srs_lk_g)
                    ltac:(intros rsf (tv & ->);
          ...
          * iIntros (rs1) "%HQ Hrw Hro". destruct HQ as (tv & ->).
            ...
            iApply (s_text_obl pa pa 0%nat 4%nat 2%N (nth_byte w) ppnl
                      ...
                      with "Hkl Hb").
          * iIntros (rs1) "%HQ ((Hrun & HW) & HRes & Hany) Hrw Hro".
            destruct HQ as (tv & ->). rewrite s_rs_tlb.
      ...
      - (* ======================= F_RVC h =========================== *)
        ...
        + (* ---- 4-aligned: the halfword sits in a 4-byte word ---- *)
          ...
          iApply (swp_run_hart_active_gen_rvc_exf s_Drw s_Dro Df (srs tlbv)
                    Qtow Q (RtowW W) ((TsoCtx.own_context TsoCtx.cur_ctx ∗ W) ∗ Res tlbv ∗ resv_frag cpu_id None)%I Supervisor pc h i0 i 8 Rr (fun ii pr => (TsoCtx.own_context TsoCtx.cur_ctx ∗ Qi ii pr)%I)
  ...
  Lemma wp_instr_tramp_pt
    ...
    tlb ↦ᵣ tlbv -∗ Res tlbv -∗
    (* A6.61: the trampoline instruction translates on the user table, so
       the block threads the trapping thread's running token *)
    TsoCtx.own_context TsoCtx.cur_ctx -∗
    pc_is pc -∗
    instr pa is_rvc i -∗
  ...
  Proof.
    ...
    iIntros "#Hhw #Hminv Hhs Hpriv Hmst Hmie Hmdl Hmenv Hsatp Hpcfg Hpaddr
             Htlbc HRes Hrun Hpc Hinstr Htr Hex Hcont".
    ...
    iApply (spt_cycle (s_Df_mix dq) pc
              (fun rs2 => (TsoCtx.own_context TsoCtx.cur_ctx ∗
                           Res1 (register_lookup tlb rs2) ∗ resv_any cpu_id
              ...
              ltac:(intros rs2 (npc & ms1 & mdv1 & cy1 & ti1 & ip1 & tv & ->);
                    apply s_rs_p_mi)
              with "Hcert Hfrag Hrw Hro [Hex HRes Hinstr Hrun] [Hcont]").
    2:{ (* ---- the continuation ---- *)
        iNext. iIntros (rs3 rs2 mi) "[%HQ %Hag] Hrw Hro (Hrun & HRes & Hfrag & HRl)".
        ...
        iApply ("Hcont" $! npc ms1 mdv1 tv with "Hhs Hpriv Hmst Hmie Hmdl Hmenv
                  Hsatp Hpcfg Hpaddr Htlbc HRes Hrun Hpc HRl"). }
    (* ---- the body ---- *)
    ...
    iApply (swp_mono with "[] [-]");
      [| iApply (tramp_run_hart_active_instr_S (s_Df_mix dq) pc pa ms
                   ...
                   HA Hord HX Hcov
                   Hcanon Hvpn Hident Hcanon2 Hvpn2 Hident2 Hva2 Hpa4va4
                   with "Hcert Hinstr [] Hfrag Hrun HRes Hrw Hro [] Htr0
                         [Hex]") ].
    - iIntros (st) "[Hi | Hr]".
      + iDestruct "Hi" as (ii pr) "(_ & Hf)". iDestruct "Hf" as "[_ Hf]".
        iDestruct "Hf" as %[].
    ...
    - (* THE LEAF, at the file the fetch landed on *)
      iIntros (tv') "[Hrun' _] HRes' Hany Hrw Hro".
      ...
      iApply (swp_mono with "[Hms Hmi Hhs] [-]");
        [| iApply ("Hex" with "Hpriv Hmst Hmie Hmdl Hmenv Hsatp Hpcfg
                     Hpaddr Htlbc HRes' Hclk HPC HnPC Hany Hrun'") ].
      iIntros (e) "(-> & Hpriv & Hmie & Hmenv & Hsatp & Hpcfg &
                    Hpaddr & Htlbr & Hclk & Hcfg & Hrun & Hany)".
      ...
      iFrame "Hrw Hro HRes' Hrun Hany HRl".
  Qed.
  ...
  Proof.
    ...
    iIntros (va pax tv rr) "%Hcanon %Hvpn %Hident Hfrag Hrun HRes Hrw Hro".
    ...
    - iExists tv. iFrame "Hrun Hany Hrw Hro".
    ...
    - assert (Lsatp : register_lookup satp
      ...
      iExists tvx. iFrame "Hrun Hany Hrw Hro".
      iEval (rewrite -(sr_swp_res_agree R
                (register_set tlb tvx
                   (s_rs pc pc ms bmi cy ti ip mst0 pcfg paddr mc micfg misa0
                      mseccfg0 senv0 pmar0 elp0 satp0 mie0 mdv0 menv0 tv)))
  ...
  Proof.
    ...
    iApply ("Hobl" $! va pax tv rr with "[%] [%] [%] Hfrag Hrun HRes Hrw Hro");
  ...
  Proof.
    intros HSIE HMPRV HSXL Hmm HPBMTE Hmenvval
           Hcanon Hvpn Hident Hcanon2 Hvpn2 Hident2 Hva2 Hpa4va4.
    iIntros "#Hclaim #Hhw #Hminv Hhs Hpriv Hmstatus Hmiec Hmdlc Hmenvc Hinv Hrun Hpc
             Hinstr Hex Hcont".
    ...
    iApply (wp_instr_tramp_pt (kpt_res_at root_ppn satp0)
              ...
              with "Hhw Hminv Hhs Hpriv Hmstatus Hmiec Hmdlc Hmenvc Hsatp
                    Hpcfg Hpaddr Htlbc HRes Hrun Hpc Hinstr [] [Hex] [Hcont]").
    ...
    - iNext. iIntros (npc ms1 mdv1 tv1)
      ...
      iApply ("Hcont" $! npc ms1 mdv1 with
                "Hhs Hpriv Hmstatus Hmiec Hmdlc Hmenvc
                 [Hsatp Htlbc Hpcfg Hpaddr HRes] Hrun Hpc HRl").
