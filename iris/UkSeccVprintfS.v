(* ===================================================================== *)
(* UkSeccVprintfS.v -- vprintf's '%s' arm, at seccomp's addresses.       *)
(* A PORT of [UkGrepVprintfS.v] (itself cat's): the same C, the same     *)
(* instructions, every pc 0x1d0 further down; the comments below still   *)
(* speak of cat where they did.  seccomp's one '%s' is the exec          *)
(* diagnostic [fprintf(2, “seccomp: exec %s failed\n”, argv[1])].        *)
(* ===================================================================== *)
...
Require Import UkSeccPutc.
Require Import UkSeccVprintf.
...
Section UkSeccVprintfS.
  ...
  (* --------------------------------------------------------------------- *)
  (* A RUN OF PLAIN CHARACTERS, 0x50e -> 0x50e.                             *)
  (*                                                                        *)
  (* [wp_ksecc_vprintf_loop] walks a format string with no '%' from an index *)
  ...
  (* --------------------------------------------------------------------- *)
  Lemma wp_ksecc_vprintf_seg (m0 : regfile) (sp0 fd ap : mword 64) (a : Z)
    ...
    forall (i0 : nat) (h : CpuId) (m : regfile) (n : nat)
      ...
      ksecc_pay_seq N fd f i0 k Ci Co -∗
      seccomp_code γt -∗
      utext_str γt a len f -∗
      Ci -∗
      urun N h m (mword_of_int 0x50e) (4 + n) -∗
      (∀ (h' : CpuId) (m' : regfile),
  ...
  Proof using .
    ...
    - (* one plain round, then the rest *)
      ...
      iApply (wp_uk_btype0 N h1 m1 (mword_of_int 0x50a)
                (mword_of_int 468 : mword 13) s1_idx BEQ false
                (add_vec (mword_of_int 0x50a : mword 64)
                   (sign_extend' 64 (mword_of_int 468 : mword 13)))
                (4 + n) Hnt eq_refl ltac:(discriminate)
                with "[] Hrun").
      { iApply (uis_seccomp_50a with "Hcode"). }
      assert (E562 : add_vec_int (mword_of_int 0x50a : mword 64) 4
                     = mword_of_int 0x50e)
  ...
  (* [vp_inv] WITH s3 FREE.                                                 *)
  (*                                                                        *)
  (* From 0x69a to 0x6ba the '%s' arm parks the BUMPED va_list in s3 -- the  *)
  (* same register the loop uses for its state -- and only the [li s3,0] at  *)
  (* 0x6ba puts the state back.  For those thirty instructions the loop      *)
  ...
  (* --------------------------------------------------------------------- *)
  (* ONE TURN OF THE ARGUMENT STRING'S LOOP, 0x6aa -> 0x6b6:                *)
  (*                                                                        *)
  ...
  (* ...AND THE BYTE IT PRINTS IS THE ONE ALREADY IN a1 (lane CAT-WALK,
     ...
     that costs. *)
  Lemma wp_ksecc_vprintf_sstep (m0 : regfile) (sp0 fd ap v3 : mword 64) (a : Z)
    ...
    ksecc_wb N fd b0 Ci Co -∗
    seccomp_code γt -∗
    ubyteq γd dq (p + 1) b1 -∗
    Ci -∗
    urun N h m (mword_of_int 0x6aa) (4 + n) -∗
    (∀ (h' : CpuId) (m' : regfile),
       ubyteq γd dq (p + 1) b1 -∗
       ⌜ vp_inv3 m0 m' sp0 a fd ap v3 i ⌝ -∗
       ...
       Co -∗
       urun N h' m' (mword_of_int 0x6b6) (4 + n) -∗
       mWP (Loop : expr riscv_lang)) -∗
    mWP (Loop : expr riscv_lang).
  Proof using .
    ...
    destruct seccomp_syms_pins
      as (_ & _ & _ & _ & Hputc & _ & _ & _ & _ & _ & _).
    (* ---- 0x6aa  c.mv a0,s6 ---- *)
    iApply (wp_uk_cmv N h m (mword_of_int 0x6aa) a0_idx s6_idx
              (add_vec zero_reg (m !!! Regidx s6_idx)) (4 + n)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate) eq_refl
              with "[] Hrun").
    { iApply (uis_seccomp_6aa with "Hcode"). }
    assert (E702 : add_vec_int (mword_of_int 0x6aa : mword 64) 2
                   = mword_of_int 0x6ac)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E702.
    iIntros (h1) "Hrun".
    ...
    (* ---- 0x6ac  jal ra,0x3fc <putc> ---- *)
    iApply (wp_uk_jal N h1 m1 (mword_of_int 0x6ac)
              (mword_of_int 2096464 : mword 21) ra_idx
              (mword_of_int SeccompSyms.putc) (mword_of_int 0x6b0) (4 + n)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ...
              with "[] Hrun").
    { iApply (uis_seccomp_6ac with "Hcode"). }
    iIntros (h2) "Hrun".
    set (m2 := <[Regidx ra_idx
                 := regval_into_reg (mword_of_int 0x6b0 : mword 64)]> m1).
    assert (Hra2 : m2 !!! Regidx ra_idx = (mword_of_int 0x6b0 : mword 64))
    ...
    { rewrite /m2 (upd_ne m1 (Regidx ra_idx) (Regidx a1_idx) _
      ...
      rewrite /m1 (upd_ne m (Regidx a0_idx) (Regidx a1_idx) _
                     ltac:(vm_compute; discriminate)).
      rewrite Ha1. apply ksecc_nth_byte0_zext. }
    iApply (wp_ksecc_putc N h2 m2 n Ci Co with "[Hw] Hcode HCi Hrun").
    { rewrite Ha0m2 Hb0m2. iExact "Hw". }
    iIntros (h3 m3) "%Hcs HCo Hrun".
    assert (Eret : ret_pc (m2 !!! Regidx ra_idx)
                   = (mword_of_int 0x6b0 : mword 64))
    ...
    iApply (wp_uk_caddi N h3 m3 (mword_of_int 0x6b0)
              (mword_of_int 1 : mword 6) s1_idx (mword_of_int (p + 1)) (4 + n)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate)
              ltac:(exact (eq_sym Eadd))
              with "[] Hrun").
    { iApply (uis_seccomp_6b0 with "Hcode"). }
    assert (E708 : add_vec_int (mword_of_int 0x6b0 : mword 64) 2
                   = mword_of_int 0x6b2)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E708.
    iIntros (h4) "Hrun".
    ...
    { rewrite Hs1_4 (uint_moi (p + 1) Hp1r).
      ...
      lia. }
    iApply (wp_uk_lbu N h4 m4 (mword_of_int 0x6b2)
              (mword_of_int 0 : mword 12) s1_idx a1_idx dq (p + 1) b1 (4 + n)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              Haddr ltac:(vm_compute; discriminate)
              with "[] Hb1 Hrun").
    { iApply (uis_seccomp_6b2 with "Hcode"). }
    assert (E70a : add_vec_int (mword_of_int 0x6b2 : mword 64) 4
                   = mword_of_int 0x6b6)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E70a.
    iIntros "Hb1" (h5) "Hrun".
  ...
  (* [sf j], and the [c.bnez a1] at 0x6b6 decides -- exactly as the format  *)
  ...
  (* --------------------------------------------------------------------- *)
  Lemma wp_ksecc_vprintf_sloop (m0 : regfile) (sp0 fd ap v3 : mword 64) (a : Z)
    ...
    forall (j : nat) (h : CpuId) (m : regfile) (n : nat)
      ...
      ksecc_pay_seq N fd sf j (S k) Ci Co -∗
      seccomp_code γt -∗
      ...
      (∀ (h' : CpuId) (m' : regfile),
         ⌜ vp_inv3 m0 m' sp0 a fd ap v3 i ⌝ -∗
         Co -∗
         urun N h' m' (mword_of_int 0x6b8) (4 + n) -∗
         mWP (Loop : expr riscv_lang)) -∗
      mWP (Loop : expr riscv_lang).
  Proof using .
    intros Hsa0 Hsahi.
    ...
    - (* the LAST byte: what 0x6b2 loads is the terminator *)
      iDestruct (ustr_nul with "Hstr") as "[#Hnul _]".
      cbn [ksecc_pay_seq].
      iDestruct "Hpay" as (Cm) "[Hw Hend]".
      iApply (wp_ksecc_vprintf_sstep m0 sp0 fd ap v3 a i
      ...
      (* ---- 0x6b6  c.bnez a1,0x6aa -- NOT taken: the terminator ---- *)
      ...
      { rewrite Ha11 zext8_moi.
        ...
        reflexivity. }
      iApply (wp_uk_cbnez N h1 m1 (mword_of_int 0x6b6)
                (mword_of_int 250 : mword 8) (mword_of_int 3 : mword 3) a1_idx
                false
                (add_vec (mword_of_int 0x6b6 : mword 64)
                   (sign_extend' 64
                      (sign_extend' 13
                ...
                ltac:(discriminate)
                with "[] Hrun").
      { iApply (uis_seccomp_6b6 with "Hcode"). }
      assert (E70e : add_vec_int (mword_of_int 0x6b6 : mword 64) 2
                     = mword_of_int 0x6b8)
        by (apply bv_eq; vm_compute; reflexivity).
      rewrite E70e.
      iIntros (h2) "Hrun".
    ...
    - (* a BODY byte follows: round again *)
      ...
      { replace (sa + Z.of_nat j + 1)%Z with (sa + Z.of_nat (S j))%Z by lia.
        iExact "Hb1". }
      iIntros (h1 m1) "_ %Hinv1 %Hs11 %Ha11 HCm Hrun".
      (* ---- 0x6b6  c.bnez a1,0x6aa -- TAKEN: a body byte is not NUL ---- *)
      ...
      { rewrite Ha11 zext8_moi. unfold neq_vec.
        ...
        destruct (Z.eqb_spec (bv_unsigned (sf (S j))) 0) as [He | _];
          [ exfalso; exact (Hnz He) | reflexivity ]. }
      assert (Etgt : add_vec (mword_of_int 0x6b6 : mword 64)
                       (sign_extend' 64
                          (sign_extend' 13
                             (concat_vec (mword_of_int 250 : mword 8) ('b"0"))))
                     = mword_of_int 0x6aa)
  ...
  (* --------------------------------------------------------------------- *)
  (* THE TAIL OF EVERY ROUND, 0x4fc -> 0x50a:                               *)
  (*                                                                        *)
  ...
  (* --------------------------------------------------------------------- *)
  Lemma wp_ksecc_vprintf_bump (m0 : regfile) (sp0 fd ap v3 : mword 64) (a : Z)
  ...
  Proof using .
    ...
    { rewrite Hs2 Es1_12.
      ...
      f_equal. lia. }
    iApply (wp_uk_addiw N h m (mword_of_int 0x4fc)
              ...
              with "[] Hrun").
    { iApply (uis_seccomp_4fc with "Hcode"). }
    assert (E554 : add_vec_int (mword_of_int 0x4fc : mword 64) 4
                   = mword_of_int 0x500)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E554.
    iIntros (h6) "Hrun".
    ...
    (* ---- 0x500  c.mv s2,a5 -- the index moves ---- *)
    iApply (wp_uk_cmv N h6 m3 (mword_of_int 0x500) s2_idx a5_idx
              (mword_of_int (Z.of_nat (S i))) (4 + n)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate)
              ltac:(rewrite Ha53; symmetry; apply add_vec_zero_l)
              with "[] Hrun").
    { iApply (uis_seccomp_500 with "Hcode"). }
    assert (E558 : add_vec_int (mword_of_int 0x500 : mword 64) 2
                   = mword_of_int 0x502)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E558.
    iIntros (h7) "Hrun".
    ...
    { rewrite <- Ha53.
      exact (upd_ne m3 (Regidx s2_idx) (Regidx a5_idx) _
               ltac:(vm_compute; discriminate)). }
    (* ---- 0x502  c.mv a4,a5 ---- *)
    iApply (wp_uk_cmv N h7 m4 (mword_of_int 0x502) a4_idx a5_idx
              (mword_of_int (Z.of_nat (S i))) (4 + n)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate)
              ltac:(rewrite Ha54; symmetry; apply add_vec_zero_l)
              with "[] Hrun").
    { iApply (uis_seccomp_502 with "Hcode"). }
    assert (E55a : add_vec_int (mword_of_int 0x502 : mword 64) 2
                   = mword_of_int 0x504)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E55a.
    iIntros (h8) "Hrun".
    ...
    { rewrite <- Ha54.
      exact (upd_ne m4 (Regidx a4_idx) (Regidx a5_idx) _
               ltac:(vm_compute; discriminate)). }
    (* ---- 0x504  c.add a5,a5,s4 -- the pointer ---- *)
    assert (Eadd5 : add_vec (m5 !!! Regidx a5_idx) (m5 !!! Regidx s4_idx)
                    = mword_of_int (a + Z.of_nat (S i))).
    { rewrite Ha55 Hs45' moi_add. f_equal. lia. }
    iApply (wp_uk_cadd N h8 m5 (mword_of_int 0x504) a5_idx s4_idx
              (mword_of_int (a + Z.of_nat (S i))) (4 + n)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate)
              ltac:(exact (eq_sym Eadd5))
              with "[] Hrun").
    { iApply (uis_seccomp_504 with "Hcode"). }
    assert (E55c : add_vec_int (mword_of_int 0x504 : mword 64) 2
                   = mword_of_int 0x506)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E55c.
    iIntros (h9) "Hrun".
    ...
    (* ---- 0x506  lbu s1,0(a5) -- fmt[i+1], out of .rodata ---- *)
    ...
    { rewrite Ha56 (uint_moi (a + Z.of_nat (S i)) Hb64a).
      replace (uoff_i12 (mword_of_int 0 : mword 12)) with 0
        by (vm_compute; reflexivity).
      lia. }
    iApply (wp_uk_lbu_text N h9 m6 (mword_of_int 0x506)
              (mword_of_int 0 : mword 12) a5_idx s1_idx
              (a + Z.of_nat (S i)) b1 (4 + n)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              Haddr ltac:(vm_compute; discriminate)
              with "[] Hb1 Hrun").
    { iApply (uis_seccomp_506 with "Hcode"). }
    assert (E55e : add_vec_int (mword_of_int 0x506 : mword 64) 4
                   = mword_of_int 0x50a)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E55e.
    iIntros (h10) "Hrun".
  ...
  (* --------------------------------------------------------------------- *)
  (* THE ROUND THAT SEES THE '%', 0x50e -> 0x50a.                           *)
  (*                                                                        *)
  (* It differs from a plain round at one instruction: at 0x516 the         *)
  (* character IS s5, so the branch to the putc arm is NOT taken and 0x51a  *)
  ...
  (* round, one turn later, can find fmt[i+1] at 0x51e.                     *)
  (* --------------------------------------------------------------------- *)
  Lemma wp_ksecc_vprintf_pct (m0 : regfile) (sp0 fd ap : mword 64) (a : Z)
      (i : nat) (b1 : mword 8) (h : CpuId) (m : regfile) (n : nat) :
    0 <= a -> a + Z.of_nat i + 2 < 2 ^ 31 ->
  ...
  Proof using .
    ...
    iApply (wp_uk_addiw N h m (mword_of_int 0x50e)
              (mword_of_int 0 : mword 12) s1_idx a5_idx (mword_of_int 37) (4 + n)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate)
              ltac:(exact (eq_sym Ea5c))
              with "[] Hrun").
    { iApply (uis_seccomp_50e with "Hcode"). }
    assert (E566 : add_vec_int (mword_of_int 0x50e : mword 64) 4
                   = mword_of_int 0x512)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E566.
    iIntros (h1) "Hrun".
    ...
    assert (Ha5_1 : m1 !!! Regidx a5_idx = mword_of_int 37)
      by exact (upd_eq m (Regidx a5_idx) (regval_into_reg _)).
    (* ---- 0x512  bnez s3,0x4f8 -- NOT taken, the state register is 0 ---- *)
    assert (Hnt : false = uv_btaken BNE (m1 !!! Regidx s3_idx) zero_reg)
      by (rewrite Hs31; vm_compute; reflexivity).
    iApply (wp_uk_btype0 N h1 m1 (mword_of_int 0x512)
              (mword_of_int 8166 : mword 13) s3_idx BNE false
              (add_vec (mword_of_int 0x512 : mword 64)
                 (sign_extend' 64 (mword_of_int 8166 : mword 13)))
              (4 + n) Hnt eq_refl ltac:(discriminate)
              with "[] Hrun").
    { iApply (uis_seccomp_512 with "Hcode"). }
    assert (E56a : add_vec_int (mword_of_int 0x512 : mword 64) 4
                   = mword_of_int 0x516)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E56a.
    iIntros (h2) "Hrun".
    (* ---- 0x516  bne a5,s5,0x4ee -- NOT taken: the character IS '%' ---- *)
    assert (Hnt2 : false
    ...
    { rewrite Ha5_1 Hs51. cbn [uv_btaken].
      ...
      reflexivity. }
    iApply (wp_uk_btype N h2 m1 (mword_of_int 0x516)
              (mword_of_int 8152 : mword 13) s5_idx a5_idx BNE false
              (add_vec (mword_of_int 0x516 : mword 64)
                 (sign_extend' 64 (mword_of_int 8152 : mword 13)))
              (4 + n) Hnt2 eq_refl ltac:(discriminate)
              with "[] Hrun").
    { iApply (uis_seccomp_516 with "Hcode"). }
    assert (E56e : add_vec_int (mword_of_int 0x516 : mword 64) 4
                   = mword_of_int 0x51a)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E56e.
    iIntros (h3) "Hrun".
    (* ---- 0x51a  c.mv s3,a5 -- state := '%' ---- *)
    iApply (wp_uk_cmv N h3 m1 (mword_of_int 0x51a) s3_idx a5_idx
              (mword_of_int 37) (4 + n)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate)
              ltac:(rewrite Ha5_1; symmetry; apply add_vec_zero_l)
              with "[] Hrun").
    { iApply (uis_seccomp_51a with "Hcode"). }
    assert (E572 : add_vec_int (mword_of_int 0x51a : mword 64) 2
                   = mword_of_int 0x51c)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E572.
    iIntros (h4) "Hrun".
    ...
    destruct Hd2 as (Hsp2 & Hs02 & Hs22 & Hs32 & Hs42 & Hs52 & Hs62 & Hs72
                     & Hs82 & Hfr2).
    (* ---- 0x51c  c.j 0x4fc ---- *)
    assert (Etgt554 : (mword_of_int 0x4fc : mword 64)
                          ...
                          (sign_extend' 64
                             (sign_extend' 21
                                (concat_vec (mword_of_int 2032 : mword 11)
                                   ('b"0")))))
      by (apply bv_eq; vm_compute; reflexivity).
    iApply (wp_uk_cj N h4 m2 (mword_of_int 0x51c)
              (mword_of_int 2032 : mword 11) (mword_of_int 0x4fc) (4 + n)
              Etgt554 ltac:(vm_compute; reflexivity)
              with "[] Hrun").
    { iApply (uis_seccomp_51c with "Hcode"). }
    iIntros (h5) "Hrun".
    iApply (wp_ksecc_vprintf_bump m0 sp0 fd ap (mword_of_int 37) a i b1
              h5 m2 n Ha0 Habnd Hinv2 with "Hcode Hb1 Hrun Hcont").
  Qed.

  (* --------------------------------------------------------------------- *)
  (* THE '%s' ARM PROPER, 0x69a -> 0x50a.                                   *)
  (*                                                                        *)
  ...
  (*   beqz s1,0x6be  the "(null)" arm, excluded by [sa <> 0]                *)
  (*   lbu  a1,0(s1) ; beqz a1,0x6d8   the empty-string arm, which is the    *)
  (*                  same three instructions as the loop's exit             *)
  (*   0x6aa..0x6b6   one putc per byte                                      *)
  (*   mv s7,s3 ; li s3,0 ; j 0x4fc    the bumped list is installed and the  *)
  (*                  state goes back to 0                                   *)
  (* --------------------------------------------------------------------- *)
  Lemma wp_ksecc_vprintf_pcs3 (m0 : regfile) (sp0 fd : mword 64) (a : Z)
    ...
    (* the argument's OWN bytes, in order -- at [slen = 0] this is the
       ...
       CAT-WALK, W1) *)
    ksecc_pay_seq N fd sf 0%nat slen Ci Co -∗
    seccomp_code γt -∗
    utext γt (a + Z.of_nat (S i)) c1 -∗
    uwordq γd dq apz (mword_of_int sa) -∗
    ustr γd DfracDiscarded sa slen sf -∗
    Ci -∗
    urun N h m (mword_of_int 0x69a) (4 + n) -∗
    (∀ (h' : CpuId) (m' : regfile),
       uwordq γd dq apz (mword_of_int sa) -∗
  ...
  Proof using .
    ...
    assert (Ezr : (sign_extend' 64 (mword_of_int 0 : mword 6) : mword 64)
      ...
      by (apply bv_eq; vm_compute; reflexivity).
    assert (Etgt554 : (mword_of_int 0x4fc : mword 64)
                      = add_vec (mword_of_int 0x6bc : mword 64)
                          (sign_extend' 64
                             (sign_extend' 21
      ...
      by (apply bv_eq; vm_compute; reflexivity).
    assert (Etgt554' : (mword_of_int 0x4fc : mword 64)
                       = add_vec (mword_of_int 0x6dc : mword 64)
                           (sign_extend' 64
                              (sign_extend' 21
                                 (concat_vec (mword_of_int 1808 : mword 11)
                                    ('b"0")))))
      by (apply bv_eq; vm_compute; reflexivity).
    (* ---- 0x69a  addi s3,s7,8 -- the BUMPED va_list, parked in s3 ---- *)
    ...
    iApply (wp_uk_addi N h m (mword_of_int 0x69a)
              ...
              with "[] Hrun").
    { iApply (uis_seccomp_69a with "Hcode"). }
    assert (E6f2 : add_vec_int (mword_of_int 0x69a : mword 64) 4
                   = mword_of_int 0x69e)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E6f2.
    iIntros (h1) "Hrun".
    ...
    destruct Hd1 as (Hsp1 & Hs01 & Hs21 & Hs31 & Hs41 & Hs51 & Hs61 & Hs71
                     & Hs81 & Hfr1).
    (* ---- 0x69e  ld s1,0(s7) -- the char* out of the caller's frame ---- *)
    ...
    { rewrite Hs71 (uint_moi apz Hapr).
      replace (uoff_i12 (mword_of_int 0 : mword 12)) with 0
        by (vm_compute; reflexivity).
      lia. }
    iApply (wp_uk_ld N h1 m1 (mword_of_int 0x69e)
              (mword_of_int 0 : mword 12) s7_idx s1_idx dq apz
              (mword_of_int sa) (4 + n)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              Haddr Hapal ltac:(vm_compute; discriminate)
              with "[] Hw Hrun").
    { iApply (uis_seccomp_69e with "Hcode"). }
    assert (E6f6 : add_vec_int (mword_of_int 0x69e : mword 64) 4
                   = mword_of_int 0x6a2)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E6f6.
    iIntros "Hw" (h2) "Hrun".
    ...
    (* ---- 0x6a2  c.beqz s1,0x6be -- NOT taken: the pointer is not null -- *)
    ...
    iApply (wp_uk_cbeqz N h2 m2 (mword_of_int 0x6a2)
              (mword_of_int 14 : mword 8) (mword_of_int 1 : mword 3) s1_idx
              false
              (add_vec (mword_of_int 0x6a2 : mword 64)
                 (sign_extend' 64
                    (sign_extend' 13
              ...
              ltac:(discriminate)
              with "[] Hrun").
    { iApply (uis_seccomp_6a2 with "Hcode"). }
    assert (E6fa : add_vec_int (mword_of_int 0x6a2 : mword 64) 2
                   = mword_of_int 0x6a4)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E6fa.
    iIntros (h3) "Hrun".
    (* ---- 0x6a4  lbu a1,0(s1) -- the string's first byte ---- *)
    ...
    - (* THE EMPTY STRING.  0x6a4 reads the terminator, 0x6a8 is taken, and
         the arm at 0x6d8 is the loop's exit written out a second time. *)
      iDestruct (ustr_nul with "Hstr") as "[#Hnul _]".
      iApply (wp_uk_lbu N h3 m2 (mword_of_int 0x6a4)
                (mword_of_int 0 : mword 12) s1_idx a1_idx DfracDiscarded
                (sa + Z.of_nat 0%nat)%Z ubyte0 (4 + n)
                ltac:(unfold unot_sp; vm_compute; discriminate)
                Haddr0 ltac:(vm_compute; discriminate)
                with "[] [] Hrun").
      { iApply (uis_seccomp_6a4 with "Hcode"). }
      { iExact "Hnul". }
      assert (E6fc : add_vec_int (mword_of_int 0x6a4 : mword 64) 4
                     = mword_of_int 0x6a8)
        by (apply bv_eq; vm_compute; reflexivity).
      rewrite E6fc.
      iIntros "_" (h4) "Hrun".
      ...
      assert (Etgt730 : add_vec (mword_of_int 0x6a8 : mword 64)
                          (sign_extend' 64
                             (sign_extend' 13
                        ...
                        = mword_of_int 0x6d8)
        by (apply bv_eq; vm_compute; reflexivity).
      iApply (wp_uk_cbeqz N h4 m3 (mword_of_int 0x6a8)
                (mword_of_int 24 : mword 8) (mword_of_int 3 : mword 3) a1_idx
                true (mword_of_int 0x6d8) (4 + n)
                ltac:(vm_compute; reflexivity) Ht700 (eq_sym Etgt730)
                ltac:(intros _; vm_compute; reflexivity)
                with "[] Hrun").
      { iApply (uis_seccomp_6a8 with "Hcode"). }
      iIntros (h5) "Hrun".
      (* ---- 0x6d8  c.mv s7,s3 ---- *)
      iApply (wp_uk_cmv N h5 m3 (mword_of_int 0x6d8) s7_idx s3_idx
                (mword_of_int (apz + 8)) (4 + n)
                ltac:(unfold unot_sp; vm_compute; discriminate)
                ltac:(vm_compute; discriminate)
                ...
                with "[] Hrun").
      { iApply (uis_seccomp_6d8 with "Hcode"). }
      assert (E730 : add_vec_int (mword_of_int 0x6d8 : mword 64) 2
                     = mword_of_int 0x6da)
        by (apply bv_eq; vm_compute; reflexivity).
      rewrite E730.
      iIntros (h6) "Hrun".
      ...
      pose proof (vp_inv3_s7 m0 m3 sp0 a fd (mword_of_int apz)
                    ...
                    Hinv3) as Hinv4.
      (* ---- 0x6da  c.li s3,0 -- the state goes back ---- *)
      iApply (wp_uk_cli N h6 m4 (mword_of_int 0x6da)
                (mword_of_int 0 : mword 6) s3_idx (4 + n)
                ltac:(unfold unot_sp; vm_compute; discriminate)
                ltac:(vm_compute; discriminate)
                with "[] Hrun").
      { iApply (uis_seccomp_6da with "Hcode"). }
      assert (E732 : add_vec_int (mword_of_int 0x6da : mword 64) 2
                     = mword_of_int 0x6dc)
        by (apply bv_eq; vm_compute; reflexivity).
      rewrite E732.
      iIntros (h7) "Hrun".
      ...
      (* ---- 0x6dc  c.j 0x4fc ---- *)
      iApply (wp_uk_cj N h7 _ (mword_of_int 0x6dc)
                (mword_of_int 1808 : mword 11) (mword_of_int 0x4fc) (4 + n)
                Etgt554' ltac:(vm_compute; reflexivity)
                with "[] Hrun").
      { iApply (uis_seccomp_6dc with "Hcode"). }
      iIntros (h8) "Hrun".
      iApply (wp_ksecc_vprintf_bump m0 sp0 fd (mword_of_int (apz + 8)) zero_reg
      ...
      + cbn [ksecc_pay_seq] in *. iApply ("Hpay" with "HCi").
    - (* AT LEAST ONE BYTE.  0x6a8 falls through into the loop. *)
      ...
      assert (Hinv3 : vp_inv3 m0 m3 sp0 a fd (mword_of_int apz) (mword_of_int (apz + 8)) i)
        by exact (vp_inv3_upd m0 m2 sp0 a fd (mword_of_int apz) (mword_of_int (apz + 8)) i a1_idx _
                    ltac:(vm_compute; reflexivity) Hinv2).
      (* ---- 0x6a8  c.beqz a1,0x6d8 -- NOT taken ---- *)
      ...
      iApply (wp_uk_cbeqz N h4 m3 (mword_of_int 0x6a8)
                (mword_of_int 24 : mword 8) (mword_of_int 3 : mword 3) a1_idx
                false
                (add_vec (mword_of_int 0x6a8 : mword 64)
                   (sign_extend' 64
                      (sign_extend' 13
                ...
                with "[] Hrun").
      { iApply (uis_seccomp_6a8 with "Hcode"). }
      assert (E700 : add_vec_int (mword_of_int 0x6a8 : mword 64) 2
                     = mword_of_int 0x6aa)
        by (apply bv_eq; vm_compute; reflexivity).
      rewrite E700.
      iIntros (h5) "Hrun".
      ...
      iApply (wp_ksecc_vprintf_sloop m0 sp0 fd (mword_of_int apz)
      ...
      (* ---- 0x6b8  c.mv s7,s3 ---- *)
      iApply (wp_uk_cmv N h6 m6 (mword_of_int 0x6b8) s7_idx s3_idx
                (mword_of_int (apz + 8)) (4 + n)
                ltac:(unfold unot_sp; vm_compute; discriminate)
                ltac:(vm_compute; discriminate)
                ltac:(rewrite Hs36; symmetry; apply add_vec_zero_l)
                with "[] Hrun").
      { iApply (uis_seccomp_6b8 with "Hcode"). }
      assert (E710 : add_vec_int (mword_of_int 0x6b8 : mword 64) 2
                     = mword_of_int 0x6ba)
        by (apply bv_eq; vm_compute; reflexivity).
      rewrite E710.
      iIntros (h7) "Hrun".
      ...
      pose proof (vp_inv3_s7 m0 m6 sp0 a fd (mword_of_int apz)
                    ...
                    Hinv6) as Hinv7.
      (* ---- 0x6ba  c.li s3,0 ---- *)
      iApply (wp_uk_cli N h7 m7 (mword_of_int 0x6ba)
                (mword_of_int 0 : mword 6) s3_idx (4 + n)
                ltac:(unfold unot_sp; vm_compute; discriminate)
                ltac:(vm_compute; discriminate)
                with "[] Hrun").
      { iApply (uis_seccomp_6ba with "Hcode"). }
      assert (E712 : add_vec_int (mword_of_int 0x6ba : mword 64) 2
                     = mword_of_int 0x6bc)
        by (apply bv_eq; vm_compute; reflexivity).
      rewrite E712.
      iIntros (h8) "Hrun".
      ...
      (* ---- 0x6bc  c.j 0x4fc ---- *)
      iApply (wp_uk_cj N h8 _ (mword_of_int 0x6bc)
                (mword_of_int 1824 : mword 11) (mword_of_int 0x4fc) (4 + n)
                Etgt554 ltac:(vm_compute; reflexivity)
                with "[] Hrun").
      { iApply (uis_seccomp_6bc with "Hcode"). }
      iIntros (h9) "Hrun".
      iApply (wp_ksecc_vprintf_bump m0 sp0 fd (mword_of_int (apz + 8)) zero_reg
  ...
  (* --------------------------------------------------------------------- *)
  (* THE REST OF THE CHAIN, 0x70a -> 0x69a.  Eight more tests: 'u', then    *)
  (* the two long forms of it, then 'x' and its two, then 'p', then 'c',    *)
  (* and finally 's'.  a0, a2, a1 and a4 are scratch throughout.            *)
  (* --------------------------------------------------------------------- *)
  Lemma wp_ksecc_vprintf_pcs2 (m0 : regfile) (sp0 fd : mword 64) (a : Z)
  ...
  Proof using .
    ...
    assert (Em120 : (sign_extend' 64 (mword_of_int 3976 : mword 12) : mword 64)
      ...
      by (apply bv_eq; vm_compute; reflexivity).
    (* ---- 0x70a  li a0,117 ---- *)
    iApply (wp_uk_li N h m (mword_of_int 0x70a)
              (mword_of_int 117 : mword 12) a0_idx (mword_of_int 117) (4 + n)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate)
              ltac:(apply bv_eq; vm_compute; reflexivity)
              with "[] Hrun").
    { iApply (uis_seccomp_70a with "Hcode"). }
    assert (E762 : add_vec_int (mword_of_int 0x70a : mword 64) 4
                   = mword_of_int 0x70e)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E762.
    iIntros (h1) "Hrun".
    ...
    iApply (wp_uk_btype N h1 m1 (mword_of_int 0x70e)
              ...
              with "[] Hrun").
    { iApply (uis_seccomp_70e with "Hcode"). }
    assert (E766 : add_vec_int (mword_of_int 0x70e : mword 64) 4
                   = mword_of_int 0x712)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E766.
    iIntros (h2) "Hrun".
    ...
    iApply (wp_uk_addi N h2 m1 (mword_of_int 0x712)
              ...
              with "[] Hrun").
    { iApply (uis_seccomp_712 with "Hcode"). }
    assert (E76a : add_vec_int (mword_of_int 0x712 : mword 64) 4
                   = mword_of_int 0x716)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E76a.
    iIntros (h3) "Hrun".
    ...
    iApply (wp_uk_addi N h4 m2 (mword_of_int 0x71c)
              ...
              with "[] Hrun").
    { iApply (uis_seccomp_71c with "Hcode"). }
    assert (E774 : add_vec_int (mword_of_int 0x71c : mword 64) 4
                   = mword_of_int 0x720)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E774.
    iIntros (h5) "Hrun".
    ...
    { rewrite <- Ha5_2.
      exact (upd_ne m2 (Regidx a0_idx) (Regidx a5_idx) _
               ltac:(vm_compute; discriminate)). }
    (* ---- 0x720  c.bnez a0,0x726 -- TAKEN ---- *)
    assert (Ht778 : true = neq_vec (m3 !!! Regidx a0_idx) zero_reg)
      by (rewrite Ha0_3;
          exact (eq_sym (moi_sub_ne_zero (bv_unsigned c2) 117 Hr2
                           ltac:(lia) Hc2u))).
    assert (Et778 : add_vec (mword_of_int 0x720 : mword 64)
                     (sign_extend' 64
                        (sign_extend' 13
                           (concat_vec (mword_of_int 3 : mword 8) ('b"0"))))
                   = mword_of_int 0x726)
      by (apply bv_eq; vm_compute; reflexivity).
    iApply (wp_uk_cbnez N h5 m3 (mword_of_int 0x720)
              (mword_of_int 3 : mword 8) (mword_of_int 2 : mword 3) a0_idx
              true (mword_of_int 0x726) (4 + n)
              ltac:(vm_compute; reflexivity) Ht778 (eq_sym Et778)
              ltac:(intros _; vm_compute; reflexivity)
              with "[] Hrun").
    { iApply (uis_seccomp_720 with "Hcode"). }
    iIntros (h6) "Hrun".
    (* ---- 0x726  li a0,120 ---- *)
    iApply (wp_uk_li N h6 m3 (mword_of_int 0x726)
              (mword_of_int 120 : mword 12) a0_idx (mword_of_int 120) (4 + n)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate)
              ltac:(apply bv_eq; vm_compute; reflexivity)
              with "[] Hrun").
    { iApply (uis_seccomp_726 with "Hcode"). }
    assert (E77e : add_vec_int (mword_of_int 0x726 : mword 64) 4
                   = mword_of_int 0x72a)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E77e.
    iIntros (h7) "Hrun".
    ...
    iApply (wp_uk_addi N h8 m4 (mword_of_int 0x72e)
              ...
              with "[] Hrun").
    { iApply (uis_seccomp_72e with "Hcode"). }
    assert (E786 : add_vec_int (mword_of_int 0x72e : mword 64) 4
                   = mword_of_int 0x732)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E786.
    iIntros (h9) "Hrun".
    ...
    { rewrite <- Ha5_4.
      exact (upd_ne m4 (Regidx a2_idx) (Regidx a5_idx) _
               ltac:(vm_compute; discriminate)). }
    (* ---- 0x732  c.bnez a2,0x738 -- TAKEN ---- *)
    assert (Ht78a : true = neq_vec (m5 !!! Regidx a2_idx) zero_reg)
      by (rewrite Ha2_5;
          exact (eq_sym (moi_sub_ne_zero (bv_unsigned c1) 120 Hr1
                           ltac:(lia) Hc1x))).
    assert (Et78a : add_vec (mword_of_int 0x732 : mword 64)
                     (sign_extend' 64
                        (sign_extend' 13
                           (concat_vec (mword_of_int 3 : mword 8) ('b"0"))))
                   = mword_of_int 0x738)
      by (apply bv_eq; vm_compute; reflexivity).
    iApply (wp_uk_cbnez N h9 m5 (mword_of_int 0x732)
              ...
              ltac:(intros _; vm_compute; reflexivity)
              with "[] Hrun").
    { iApply (uis_seccomp_732 with "Hcode"). }
    iIntros (h10) "Hrun".
    ...
    iApply (wp_uk_addi N h10 m5 (mword_of_int 0x738)
              ...
              with "[] Hrun").
    { iApply (uis_seccomp_738 with "Hcode"). }
    assert (E790 : add_vec_int (mword_of_int 0x738 : mword 64) 4
                   = mword_of_int 0x73c)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E790.
    iIntros (h11) "Hrun".
    ...
    { rewrite <- Ha5_5.
      exact (upd_ne m5 (Regidx a1_idx) (Regidx a5_idx) _
               ltac:(vm_compute; discriminate)). }
    (* ---- 0x73c  c.bnez a1,0x742 -- TAKEN ---- *)
    assert (Ht794 : true = neq_vec (m6 !!! Regidx a1_idx) zero_reg)
      by (rewrite Ha1_6;
          exact (eq_sym (moi_sub_ne_zero (bv_unsigned c2) 120 Hr2
                           ltac:(lia) Hc2x))).
    assert (Et794 : add_vec (mword_of_int 0x73c : mword 64)
                     (sign_extend' 64
                        (sign_extend' 13
                           (concat_vec (mword_of_int 3 : mword 8) ('b"0"))))
                   = mword_of_int 0x742)
      by (apply bv_eq; vm_compute; reflexivity).
    iApply (wp_uk_cbnez N h11 m6 (mword_of_int 0x73c)
              (mword_of_int 3 : mword 8) (mword_of_int 3 : mword 3) a1_idx
              true (mword_of_int 0x742) (4 + n)
              ltac:(vm_compute; reflexivity) Ht794 (eq_sym Et794)
              ltac:(intros _; vm_compute; reflexivity)
              with "[] Hrun").
    { iApply (uis_seccomp_73c with "Hcode"). }
    iIntros (h12) "Hrun".
    (* ---- 0x742  li a4,112 ---- *)
    iApply (wp_uk_li N h12 m6 (mword_of_int 0x742)
              (mword_of_int 112 : mword 12) a4_idx (mword_of_int 112) (4 + n)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate)
              ltac:(apply bv_eq; vm_compute; reflexivity)
              with "[] Hrun").
    { iApply (uis_seccomp_742 with "Hcode"). }
    assert (E79a : add_vec_int (mword_of_int 0x742 : mword 64) 4
                   = mword_of_int 0x746)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E79a.
    iIntros (h13) "Hrun".
    ...
    { rewrite <- Ha5_6.
      exact (upd_ne m6 (Regidx a4_idx) (Regidx a5_idx) _
               ltac:(vm_compute; discriminate)). }
    (* ---- 0x746  beq a5,a4 -- NOT taken ---- *)
    assert (Hn79e : false
                    = uv_btaken BEQ (m7 !!! Regidx a5_idx)
                        (m7 !!! Regidx a4_idx))
      by (rewrite Ha5_7 Ha4_7; vm_compute; reflexivity).
    iApply (wp_uk_btype N h13 m7 (mword_of_int 0x746)
              (mword_of_int 7928 : mword 13) a4_idx a5_idx BEQ false
              (add_vec (mword_of_int 0x746 : mword 64)
                 (sign_extend' 64 (mword_of_int 7928 : mword 13)))
              (4 + n) Hn79e eq_refl ltac:(discriminate)
              with "[] Hrun").
    { iApply (uis_seccomp_746 with "Hcode"). }
    assert (E79e : add_vec_int (mword_of_int 0x746 : mword 64) 4
                   = mword_of_int 0x74a)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E79e.
    iIntros (h14) "Hrun".
    (* ---- 0x74a  li a4,99 ---- *)
    iApply (wp_uk_li N h14 m7 (mword_of_int 0x74a)
              (mword_of_int 99 : mword 12) a4_idx (mword_of_int 99) (4 + n)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate)
              ltac:(apply bv_eq; vm_compute; reflexivity)
              with "[] Hrun").
    { iApply (uis_seccomp_74a with "Hcode"). }
    assert (E7a2 : add_vec_int (mword_of_int 0x74a : mword 64) 4
                   = mword_of_int 0x74e)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E7a2.
    iIntros (h15) "Hrun".
    ...
    { rewrite <- Ha5_7.
      exact (upd_ne m7 (Regidx a4_idx) (Regidx a5_idx) _
               ltac:(vm_compute; discriminate)). }
    (* ---- 0x74e  beq a5,a4 -- NOT taken ---- *)
    assert (Hn7a6 : false
                    = uv_btaken BEQ (m8 !!! Regidx a5_idx)
                        (m8 !!! Regidx a4_idx))
      by (rewrite Ha5_8 Ha4_8; vm_compute; reflexivity).
    iApply (wp_uk_btype N h15 m8 (mword_of_int 0x74e)
              (mword_of_int 7992 : mword 13) a4_idx a5_idx BEQ false
              (add_vec (mword_of_int 0x74e : mword 64)
                 (sign_extend' 64 (mword_of_int 7992 : mword 13)))
              (4 + n) Hn7a6 eq_refl ltac:(discriminate)
              with "[] Hrun").
    { iApply (uis_seccomp_74e with "Hcode"). }
    assert (E7a6 : add_vec_int (mword_of_int 0x74e : mword 64) 4
                   = mword_of_int 0x752)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E7a6.
    iIntros (h16) "Hrun".
    (* ---- 0x752  li a4,115 ---- *)
    iApply (wp_uk_li N h16 m8 (mword_of_int 0x752)
              (mword_of_int 115 : mword 12) a4_idx (mword_of_int 115) (4 + n)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate)
              ltac:(apply bv_eq; vm_compute; reflexivity)
              with "[] Hrun").
    { iApply (uis_seccomp_752 with "Hcode"). }
    assert (E7aa : add_vec_int (mword_of_int 0x752 : mword 64) 4
                   = mword_of_int 0x756)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E7aa.
    iIntros (h17) "Hrun".
    ...
    { rewrite <- Ha5_8.
      exact (upd_ne m8 (Regidx a4_idx) (Regidx a5_idx) _
               ltac:(vm_compute; discriminate)). }
    (* ---- 0x756  beq a5,a4,0x69a -- TAKEN: the directive is 's' ---- *)
    assert (Ht7ae : true
                    = uv_btaken BEQ (m9 !!! Regidx a5_idx)
                        (m9 !!! Regidx a4_idx))
      by (rewrite Ha5_9 Ha4_9; vm_compute; reflexivity).
    assert (Et7ae : add_vec (mword_of_int 0x756 : mword 64)
                      (sign_extend' 64 (mword_of_int 8004 : mword 13))
                    = mword_of_int 0x69a)
      by (apply bv_eq; vm_compute; reflexivity).
    iApply (wp_uk_btype N h17 m9 (mword_of_int 0x756)
              (mword_of_int 8004 : mword 13) a4_idx a5_idx BEQ true
              (mword_of_int 0x69a) (4 + n) Ht7ae (eq_sym Et7ae)
              ltac:(intros _; vm_compute; reflexivity)
              with "[] Hrun").
    { iApply (uis_seccomp_756 with "Hcode"). }
    iIntros (h18) "Hrun".
    iApply (wp_ksecc_vprintf_pcs3 m0 sp0 fd a i apz sa dq c1 slen sf
              m9 h18 n Ci Co Ha0 ltac:(lia) Hap0 Haphi Hapal Hsanz Hinv9
              with "Hpay Hcode Hc1 Hw Hstr HCi Hrun Hcont").
  Qed.

  (* ===================================================================== *)
  (* THE DIRECTIVE'S ROUND, 0x50e -> 0x50a, for c0 = 's'.                   *)
  (*                                                                        *)
  (* The state register is '%', so 0x512 and 0x4f8 both go to 0x51e and the *)
  ...
  (* that read them (0x718 and 0x73e) are reached only when c1 or c2 IS the *)
  (* character just excluded, so no step in this path depends on them.      *)
  (* ===================================================================== *)
  Lemma wp_ksecc_vprintf_pcs (m0 : regfile) (sp0 fd : mword 64) (a : Z)
      (i : nat) (apz sa : Z) (dq : dfrac) (c1 c2 : mword 8)
  ...
  Proof using .
    ...
    { rewrite <- Ha4.
      exact (upd_ne m (Regidx a5_idx) (Regidx a4_idx) _
               ltac:(vm_compute; discriminate)). }
    (* ---- 0x512  bnez s3,0x4f8 -- TAKEN: a directive is pending ---- *)
    assert (Ht56a : true = uv_btaken BNE (m1 !!! Regidx s3_idx) zero_reg)
      by (rewrite Hs31; vm_compute; reflexivity).
    assert (Etgt550 : add_vec (mword_of_int 0x512 : mword 64)
                        (sign_extend' 64 (mword_of_int 8166 : mword 13))
                      = mword_of_int 0x4f8)
      by (apply bv_eq; vm_compute; reflexivity).
    iApply (wp_uk_btype0 N h1 m1 (mword_of_int 0x512)
              (mword_of_int 8166 : mword 13) s3_idx BNE true
              (mword_of_int 0x4f8) (4 + n) Ht56a (eq_sym Etgt550)
              ltac:(intros _; vm_compute; reflexivity)
              with "[] Hrun").
    { iApply (uis_seccomp_512 with "Hcode"). }
    iIntros (h2) "Hrun".
    (* ---- 0x4f8  beq s3,s5,0x51e -- TAKEN: the state IS '%' ---- *)
    assert (Ht550 : true
                    = uv_btaken BEQ (m1 !!! Regidx s3_idx) (m1 !!! Regidx s5_idx))
      by (rewrite Hs31 Hs51; vm_compute; reflexivity).
    assert (Etgt576 : add_vec (mword_of_int 0x4f8 : mword 64)
                        (sign_extend' 64 (mword_of_int 38 : mword 13))
                      = mword_of_int 0x51e)
      by (apply bv_eq; vm_compute; reflexivity).
    iApply (wp_uk_btype N h2 m1 (mword_of_int 0x4f8)
              (mword_of_int 38 : mword 13) s5_idx s3_idx BEQ true
              (mword_of_int 0x51e) (4 + n) Ht550 (eq_sym Etgt576)
              ltac:(intros _; vm_compute; reflexivity)
              with "[] Hrun").
    { iApply (uis_seccomp_4f8 with "Hcode"). }
    iIntros (h3) "Hrun".
    (* ---- 0x51e  add a3,s4,a4 -- &fmt[i] ---- *)
    assert (Eadd3 : add_vec (m1 !!! Regidx s4_idx) (m1 !!! Regidx a4_idx)
                    = mword_of_int (a + Z.of_nat i))
      by (rewrite Hs41 Ha41 moi_add; reflexivity).
    iApply (wp_uk_add N h3 m1 (mword_of_int 0x51e) s4_idx a4_idx a3_idx
              (mword_of_int (a + Z.of_nat i)) (4 + n)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate)
              ltac:(exact (eq_sym Eadd3))
              with "[] Hrun").
    { iApply (uis_seccomp_51e with "Hcode"). }
    assert (E576 : add_vec_int (mword_of_int 0x51e : mword 64) 4
                   = mword_of_int 0x522)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E576.
    iIntros (h4) "Hrun".
    ...
    { rewrite Ha32 (uint_moi (a + Z.of_nat i) Hai).
      replace (uoff_i12 (mword_of_int 1 : mword 12)) with 1
        by (vm_compute; reflexivity).
      lia. }
    iApply (wp_uk_lbu_text N h4 m2 (mword_of_int 0x522)
              (mword_of_int 1 : mword 12) a3_idx a2_idx
              (a + Z.of_nat (S i)) c1 (4 + n)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              Haddr1 ltac:(vm_compute; discriminate)
              with "[] Hc1 Hrun").
    { iApply (uis_seccomp_522 with "Hcode"). }
    assert (E57a : add_vec_int (mword_of_int 0x522 : mword 64) 4
                   = mword_of_int 0x526)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E57a.
    iIntros (h5) "Hrun".
    ...
    { rewrite <- Ha42.
      exact (upd_ne m2 (Regidx a2_idx) (Regidx a4_idx) _
               ltac:(vm_compute; discriminate)). }
    (* ---- 0x526  beqz a2,0x6f6 -- NOT taken: c1 is not the terminator ---- *)
    ...
    iApply (wp_uk_btype0 N h5 m3 (mword_of_int 0x526)
              (mword_of_int 464 : mword 13) a2_idx BEQ false
              (add_vec (mword_of_int 0x526 : mword 64)
                 (sign_extend' 64 (mword_of_int 464 : mword 13)))
              (4 + n) Hn57e eq_refl ltac:(discriminate)
              with "[] Hrun").
    { iApply (uis_seccomp_526 with "Hcode"). }
    assert (E57e : add_vec_int (mword_of_int 0x526 : mword 64) 4
                   = mword_of_int 0x52a)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E57e.
    iIntros (h6) "Hrun".
    (* ---- 0x52a  beq a5,s8,0x558 -- NOT taken: 's' is not 'd' ---- *)
    assert (Hn582 : false
                    = uv_btaken BEQ (m3 !!! Regidx a5_idx) (m3 !!! Regidx s8_idx))
      by (rewrite Ha53 Hs83; vm_compute; reflexivity).
    iApply (wp_uk_btype N h6 m3 (mword_of_int 0x52a)
              (mword_of_int 46 : mword 13) s8_idx a5_idx BEQ false
              (add_vec (mword_of_int 0x52a : mword 64)
                 (sign_extend' 64 (mword_of_int 46 : mword 13)))
              (4 + n) Hn582 eq_refl ltac:(discriminate)
              with "[] Hrun").
    { iApply (uis_seccomp_52a with "Hcode"). }
    assert (E582 : add_vec_int (mword_of_int 0x52a : mword 64) 4
                   = mword_of_int 0x52e)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E582.
    iIntros (h7) "Hrun".
    (* ---- 0x52e  addi a3,a5,-108 ; 0x532  seqz a3,a3 -- a3 is dead ---- *)
    iApply (wp_uk_addi N h7 m3 (mword_of_int 0x52e)
              ...
              ltac:(vm_compute; discriminate) eq_refl
              with "[] Hrun").
    { iApply (uis_seccomp_52e with "Hcode"). }
    assert (E586 : add_vec_int (mword_of_int 0x52e : mword 64) 4
                   = mword_of_int 0x532)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E586.
    iIntros (h8) "Hrun".
    ...
    assert (Hinv4 : vp_inv3 m0 m4 sp0 a fd (mword_of_int apz) (mword_of_int 37) i)
      by exact (vp_inv3_upd m0 m3 sp0 a fd (mword_of_int apz) (mword_of_int 37) i a3_idx _
                  ltac:(vm_compute; reflexivity) Hinv3).
    iApply (wp_uk_sltiu N h8 m4 (mword_of_int 0x532)
              ...
              ltac:(vm_compute; discriminate) eq_refl
              with "[] Hrun").
    { iApply (uis_seccomp_532 with "Hcode"). }
    assert (E58a : add_vec_int (mword_of_int 0x532 : mword 64) 4
                   = mword_of_int 0x536)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E58a.
    iIntros (h9) "Hrun".
    ...
    iApply (wp_uk_addi N h9 m5 (mword_of_int 0x536)
              ...
              with "[] Hrun").
    { iApply (uis_seccomp_536 with "Hcode"). }
    assert (E58e : add_vec_int (mword_of_int 0x536 : mword 64) 4
                   = mword_of_int 0x53a)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E58e.
    iIntros (h10) "Hrun".
    ...
    iApply (wp_uk_cadd N h11 m6 (mword_of_int 0x570) a4_idx s4_idx
              (mword_of_int (a + Z.of_nat i)) (4 + n)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate)
              ltac:(exact (eq_sym Ea4c))
              with "[] Hrun").
    { iApply (uis_seccomp_570 with "Hcode"). }
    assert (E5c8 : add_vec_int (mword_of_int 0x570 : mword 64) 2
                   = mword_of_int 0x572)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E5c8.
    iIntros (h12) "Hrun".
    ...
    { rewrite Ha47 (uint_moi (a + Z.of_nat i) Hai).
      replace (uoff_i12 (mword_of_int 2 : mword 12)) with 2
        by (vm_compute; reflexivity).
      lia. }
    iApply (wp_uk_lbu_text N h12 m7 (mword_of_int 0x572)
              (mword_of_int 2 : mword 12) a4_idx a1_idx
              (a + Z.of_nat (S (S i))) c2 (4 + n)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              Haddr2 ltac:(vm_compute; discriminate)
              with "[] Hc2 Hrun").
    { iApply (uis_seccomp_572 with "Hcode"). }
    assert (E5ca : add_vec_int (mword_of_int 0x572 : mword 64) 4
                   = mword_of_int 0x576)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E5ca.
    iIntros (h13) "Hrun".
    ...
    { rewrite <- Ha57.
      exact (upd_ne m7 (Regidx a1_idx) (Regidx a5_idx) _
               ltac:(vm_compute; discriminate)). }
    (* ---- 0x576  addi a4,a2,-108 ; 0x57a  seqz a4,a4 ; 0x57e  and a4,a4,a3
           -- a4 is dead on this path, so its value is left as written ---- *)
    iApply (wp_uk_addi N h13 m8 (mword_of_int 0x576)
              ...
              ltac:(vm_compute; discriminate) eq_refl
              with "[] Hrun").
    { iApply (uis_seccomp_576 with "Hcode"). }
    assert (E5ce : add_vec_int (mword_of_int 0x576 : mword 64) 4
                   = mword_of_int 0x57a)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E5ce.
    iIntros (h14) "Hrun".
    ...
    assert (Hinv9 : vp_inv3 m0 m9 sp0 a fd (mword_of_int apz) (mword_of_int 37) i)
      by exact (vp_inv3_upd m0 m8 sp0 a fd (mword_of_int apz) (mword_of_int 37) i a4_idx _
                  ltac:(vm_compute; reflexivity) Hinv8).
    iApply (wp_uk_sltiu N h14 m9 (mword_of_int 0x57a)
              ...
              ltac:(vm_compute; discriminate) eq_refl
              with "[] Hrun").
    { iApply (uis_seccomp_57a with "Hcode"). }
    assert (E5d2 : add_vec_int (mword_of_int 0x57a : mword 64) 4
                   = mword_of_int 0x57e)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E5d2.
    iIntros (h15) "Hrun".
    ...
    iApply (wp_uk_cand N h15 m10 (mword_of_int 0x57e)
              ...
              ltac:(vm_compute; discriminate) eq_refl
              with "[] Hrun").
    { iApply (uis_seccomp_57e with "Hcode"). }
    assert (E5d6 : add_vec_int (mword_of_int 0x57e : mword 64) 2
                   = mword_of_int 0x580)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E5d6.
    iIntros (h16) "Hrun".
    ...
    iApply (wp_uk_addi N h16 m11 (mword_of_int 0x580)
              ...
              with "[] Hrun").
    { iApply (uis_seccomp_580 with "Hcode"). }
    assert (E5d8 : add_vec_int (mword_of_int 0x580 : mword 64) 4
                   = mword_of_int 0x584)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E5d8.
    iIntros (h17) "Hrun".
    ...
    (* ---- 0x584  bnez a0,0x70a -- TAKEN: c2 is not 'd' ---- *)
    ...
    { rewrite Ha0_12. cbn [uv_btaken].
      exact (eq_sym (moi_sub_ne_zero (bv_unsigned c2) 100 Hr2
                       ltac:(lia) Hc2d)). }
    assert (Etgt762 : add_vec (mword_of_int 0x584 : mword 64)
                        (sign_extend' 64 (mword_of_int 390 : mword 13))
                      = mword_of_int 0x70a)
      by (apply bv_eq; vm_compute; reflexivity).
    iApply (wp_uk_btype0 N h17 m12 (mword_of_int 0x584)
              (mword_of_int 390 : mword 13) a0_idx BNE true
              (mword_of_int 0x70a) (4 + n) Ht5dc (eq_sym Etgt762)
              ltac:(intros _; vm_compute; reflexivity)
              with "[] Hrun").
    { iApply (uis_seccomp_584 with "Hcode"). }
    iIntros (h18) "Hrun".
    iApply (wp_ksecc_vprintf_pcs2 m0 sp0 fd a i apz sa dq c1 c2 slen sf
  ...
  (* ===================================================================== *)
  Lemma wp_ksecc_vprintf_s (a : Z) (len q : nat) (f : nat -> mword 8)
    ...
    (* THE THREE RUNS THIS WALK PRINTS (lane CAT-WALK, W1), in the order
       ...
       them as one run joins them with [ksecc_pay_seq_join]. *)
    ksecc_pay_seq N (m !!! Regidx a0_idx) f 0%nat q Ci Cm1 -∗
    ksecc_pay_seq N (m !!! Regidx a0_idx) sf 0%nat slen Cm1 Cm2 -∗
    ksecc_pay_seq N (m !!! Regidx a0_idx) f (S (S q))
    ...
    ustr γd DfracDiscarded sa slen sf -∗
    Ci -∗
    urun N h m (mword_of_int SeccompSyms.vprintf) (12 + (4 + n)) -∗
    (∀ (h' : CpuId) (m' : regfile),
       uwordq γd dq apz (mword_of_int sa) -∗
       ⌜ ucallee_saved m m' ⌝ -∗
       Co -∗
  ...
  Proof using .
    ...
    (* ---- the prologue ---- *)
    iApply (wp_ksecc_vprintf_pro N a len f h m n Ha0 Habnd ltac:(lia) Ha1
    ...
    iApply (wp_ksecc_vprintf_seg m (m !!! Regidx csp_rs1) fd
    ...
    iDestruct (utext_str_byte γt a len f (S q) ltac:(lia) with "Hstr")
      as "#Hbsq".
    iApply (wp_ksecc_vprintf_pct m (m !!! Regidx csp_rs1) fd
    ...
    (* ---- 0x50a, not taken: 's' is not the terminator ---- *)
    ...
    iApply (wp_uk_btype0 N h2 mC (mword_of_int 0x50a)
              (mword_of_int 468 : mword 13) s1_idx BEQ false
              (add_vec (mword_of_int 0x50a : mword 64)
                 (sign_extend' 64 (mword_of_int 468 : mword 13)))
    ...
    iDestruct (utext_str_byte γt a len f (S (S q)) ltac:(lia) with "Hstr")
      as "#Hbssq".
    iApply (wp_ksecc_vprintf_pcs m (m !!! Regidx csp_rs1) fd a (S q) apz sa dq
    ...
    (* ---- 0x50a, not taken again: there IS a character after the "%s" ---- *)
    ...
    iApply (wp_uk_btype0 N h4 mD (mword_of_int 0x50a)
              (mword_of_int 468 : mword 13) s1_idx BEQ false
              (add_vec (mword_of_int 0x50a : mword 64)
                 (sign_extend' 64 (mword_of_int 468 : mword 13)))
              (4 + n) Hnt2 eq_refl ltac:(discriminate)
    ...
    iApply (wp_ksecc_vprintf_loop N m (m !!! Regidx csp_rs1) fd
