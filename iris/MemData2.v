(* MemData2.v -- mode-neutral width-2 DATA memory leaf lemmas (clone of MemData4).

   The width-2 physical read/write core, factored out of the per-mode
...
(* ===================================================================== *)
(* Pure width-2 leaf helpers (mode-neutral).                             *)
(* ===================================================================== *)

Lemma avi0_mul2 (a : mword 64) : add_vec_int a (0 * 2) = a.
Proof. change (0 * 2) with 0. apply avi0. Qed.

Lemma exec_split_misaligned_aligned_2 (vaddr : virtaddr) s :
  is_aligned_vaddr vaddr 2 = true ->
  exec (split_misaligned vaddr 2) s = Some ((1, 2), s).
Proof. intro H. unfold split_misaligned. rewrite H. cbn [orb]. apply exec_returnm. Qed.

Lemma data2_id_2 (v : mword 16) :
  update_subrange_vec_dec (zeros' (2*1*8)) (2*(0+1)*8-1) (2*0*8) v = v.
Proof.
...
Lemma exec_mem_write_ea_2 (addr : mword 64) s :
  exec (mem_write_ea (Physaddr addr) 2 false false false) s = Some (Ok tt, s).
Proof.
...
(* ===================================================================== *)
(* Privilege-generic checked_mem_read / mem_read at width 2 (Load Data). *)
(* The pmpCheck grant is supplied by the caller (Hpmp), which is the      *)
(* only privilege-dependent step.                                         *)
(* ===================================================================== *)

Lemma exec_checked_mem_read_ram_load_2 (p : Privilege) (pbmt : page_based_mem_type)
      (addr : mword 64) (region : PMA_Region) (w : bv 16) s :
  exec (pmpCheck (Physaddr addr) 2 (Load Data) p) s = Some (None, s) ->
  matching_pma_region (register_lookup pma_regions s.(sregs)) (Physaddr addr) 2 = Some region ->
  is_aligned_paddr (Physaddr addr) 2 = true ->
  (override_PMA (PMA_Region_attributes region) pbmt).(PMA_readable) = true ->
  ...
  exec (checked_mem_read (Load Data) pbmt p (Physaddr addr) 2 false false false false)
...
Lemma exec_mem_read_load_2 (p : Privilege) (pbmt : page_based_mem_type)
      (addr : mword 64) (region : PMA_Region) (w : bv 16) (m : mword 64) s :
  exec (pmpCheck (Physaddr addr) 2 (Load Data) p) s = Some (None, s) ->
  matching_pma_region (register_lookup pma_regions s.(sregs)) (Physaddr addr) 2 = Some region ->
  is_aligned_paddr (Physaddr addr) 2 = true ->
  (override_PMA (PMA_Region_attributes region) pbmt).(PMA_readable) = true ->
...
Proof.
  ...
  2:{ unfold mem_read_priv_meta. cbn [orb andb].
      ...
      2:{ cbn match. apply exec_checked_mem_read_ram_load_2 with (p := p) (region := region); assumption. }
      cbn match. unfold mem_read_callback. apply exec_returnM. }
  cbn [MemoryOpResult_drop_meta]. apply exec_returnM.
Qed.

(* ===================================================================== *)
(* Privilege-generic checked_mem_write / mem_write_value at width 2.      *)
(* ===================================================================== *)

Lemma exec_checked_mem_write_ram_2 (p : Privilege) (pbmt : page_based_mem_type)
      (addr : mword 64) (region : PMA_Region) (data : bv 16) s :
  exec (pmpCheck (Physaddr addr) 2 (Store Data) p) s = Some (None, s) ->
  matching_pma_region (register_lookup pma_regions s.(sregs)) (Physaddr addr) 2 = Some region ->
  is_aligned_paddr (Physaddr addr) 2 = true ->
  (override_PMA (PMA_Region_attributes region) pbmt).(PMA_writable) = true ->
  exec (within_clint (Physaddr addr) 2) s = Some (false, s) ->
  ...
  dev_addr addr = false ->
  exec (checked_mem_write (Physaddr addr) 2 data (Store Data) pbmt p tt false false false) s
...
Lemma exec_mem_write_value_2 (p : Privilege) (pbmt : page_based_mem_type)
      (addr : mword 64) (region : PMA_Region) (data : bv 16) (m : mword 64) s :
  exec (pmpCheck (Physaddr addr) 2 (Store Data) p) s = Some (None, s) ->
  matching_pma_region (register_lookup pma_regions s.(sregs)) (Physaddr addr) 2 = Some region ->
  is_aligned_paddr (Physaddr addr) 2 = true ->
  (override_PMA (PMA_Region_attributes region) pbmt).(PMA_writable) = true ->
...
Proof.
  ...
  rewrite (exec_bind_Some _ _ _ _ _ (exec_checked_mem_write_ram_2 p pbmt addr region data s Hpmp Hmatch Halign Hwrite Hc Hsig Hh Hdev)).
  cbn match. unfold mem_write_callback. apply exec_returnm.
Qed.

(* ===================================================================== *)
(* Generic width-2 LOAD tower (state-preserving, privilege- and          *)
...
Section GenVMemReadAddr4.
  ...
  Hypothesis Hpmp : exec (pmpCheck (Physaddr pa) 2 (Load Data) p) s = Some (None, s).
  Hypothesis Hmatch : matching_pma_region (register_lookup pma_regions s.(sregs)) (Physaddr pa) 2 = Some region.
  ...
  Hypothesis Hbytes : forall j : nat, (N.of_nat j < 2)%N -> s.(mem) !! (pa_add pa j) = Some (nth_byte v j).

  Lemma exec_vmem_read_addr_2 :
    ...
    { eapply execR_untilMT_1.
      ...
      - cbn match.
        ...
        { rewrite (execR_liftR_seq _ _ _ _ _
            (exec_mem_read_load_2 p PBMT_PMA pa region v (register_lookup mstatus s.(sregs)) s
...
Section GenVMemRead4.
  ...
  Lemma exec_vmem_read_2_gpr :
    exec (vmem_read (Regidx rs1) offset 2 (Load Data) false false false) s = Some (Ok data2, s).
  Proof.
    ...
    rewrite execR_liftR.
    rewrite (exec_vmem_read_addr_2 p a v region s pa Halign Hcp Hmprv Htr Hpmp Hmatch Hpalign Hread Hc Hsig Hh Hdev Hbytes).
    reflexivity.
  Qed.
End GenVMemRead4.
...
Section GenExecLoad4.
  ...
  Proof.
    ...
    unfold execute_LOAD.
    replace (2 <=? xlen_bytes) with true by (vm_compute; reflexivity).
    ...
    rewrite (exec_bind_Some _ _ _ _ _
      (exec_vmem_read_2_gpr p rs1 offset a v region s pa Htea Halign Hcp Hmprv Htr Hpmp Hmatch Hpalign Hread Hc Hsig Hh Hdev Hbytes)).
    cbn match.
...
(* ===================================================================== *)
(* Generic width-2 STORE tower (state-preserving, privilege-generic).     *)
(* ===================================================================== *)
...
Section GenVMemWriteAddr4.
  ...
  Hypothesis Htr : exec (translateAddr (Virtaddr (add_vec_int (bits_of_virtaddr (Virtaddr a)) (0 * 2))) (Store Data)) s
                   = Some (Ok (Physaddr pa, PBMT_PMA, init_ext_ptw), s).
  Hypothesis Hpmp : exec (pmpCheck (Physaddr pa) 2 (Store Data) p) s = Some (None, s).
  Hypothesis Hmatch : matching_pma_region (register_lookup pma_regions s.(sregs)) (Physaddr pa) 2 = Some region.
  Hypothesis Hpalign : is_aligned_paddr (Physaddr pa) 2 = true.
  ...
  Lemma exec_vmem_write_addr_2 :
    exec (vmem_write_addr (Virtaddr a) 2 data (Store Data) false false false) s
      = Some (Ok true, MState s.(sregs) (write_bytes s.(mem) pa 2 data) s.(mdev)).
  Proof.
    ...
    | |- context [ Defs.bind (Defs.untilMT ?vs ?m ?c ?b) ?post ] =>
      assert (Hu : execR (Defs.untilMT vs m c b) s
                   = Some (inr (true, 0%Z, true), MState s.(sregs) (write_bytes s.(mem) pa 2 data) s.(mdev)))
    end.
    { eapply execR_untilMT_1.
      - reflexivity.
      - cbn match.
        ...
        | |- context [ Defs.bind (Defs.bind0 (Defs.liftR ?asrt) ?Nbody) ?post ] =>
            assert (Hwrloop : execR (Defs.bind0 (Defs.liftR asrt) Nbody) s
                             = Some (inr true, MState s.(sregs) (write_bytes s.(mem) pa 2 data) s.(mdev)))
        end.
        { match goal with
          ...
          rewrite (execR_liftR_seq _ _ _ _ _ (exec_mem_write_ea_2 pa s)).
          cbn match.
          match goal with
          | |- context [ mem_write_value ?pp 2 ?D (Store Data) ?pb false false false ] =>
              replace D with data
          end.
          2: { symmetry.
               change (2*(0+1)*8-1) with 15. change (2*0*8) with 0. change (2*8) with 16.
               change (15 - 0 + 1) with 16. rewrite autocast_id.
          ...
          rewrite (execR_liftR_seq _ _ _ _ _
            (exec_mem_write_value_2 p PBMT_PMA pa region data
...
Section GenVMemWrite4.
  ...
  Lemma exec_vmem_write_2_gpr :
    exec (vmem_write (Regidx rs1) offset 2 data (Store Data) false false false) s
      = Some (Ok true, MState s.(sregs) (write_bytes s.(mem) pa 2 data) s.(mdev)).
  Proof.
    ...
    rewrite (execR_bind_Some _ _ _ _ _ (execR_returnR_fwd (Virtaddr a) s)).
    rewrite execR_liftR.
    rewrite (exec_vmem_write_addr_2 p a data region s pa Halign Hcp Hmprv Htr Hpmp Hmatch Hpalign Hwrite Hc Hsig Hh Hdev).
    reflexivity.
  Qed.
End GenVMemWrite4.
...
Section GenExecStore4.
  ...
  Lemma exec_execute_STORE_2_gpr :
  ...
  Proof.
    ...
    cbn match.
    rewrite (exec_bind_Some _ _ _ _ _
      (exec_vmem_write_2_gpr p rs1 offset a _ region s pa Htea Halign Hcp Hmprv Htr Hpmp Hmatch Hpalign Hwrite Hc Hsig Hh Hdev)).
    cbn match.
    apply exec_returnM.
  Qed.
End GenExecStore4.
...
Section GenVMemReadAddr4Walk.
  ...
  Lemma exec_vmem_read_addr_2_walk :
    exec (vmem_read_addr (Virtaddr a) 2 (Load Data) false false false) s
      = Some (Ok data2, s').
  Proof.
    unfold vmem_read_addr.
...
Section GenVMemRead4Walk.
  ...
  Lemma exec_vmem_read_2_gpr_walk :
    exec (vmem_read (Regidx rs1) offset 2 (Load Data) false false false) s = Some (Ok data2, s').
  Proof.
    unfold vmem_read. rewrite exec_catch_early_return.
    assert (Hgta : exec (get_transformed_data_addr (Regidx rs1) offset (Load Data) 2) s
    ...
    rewrite (execR_bind_Some _ _ _ _ _ (execR_returnR_fwd (Virtaddr a) s)).
    rewrite execR_liftR.
    rewrite (exec_vmem_read_addr_2_walk p a v region s s' pa Halign Htr Hcp Hmprv Hpmp Hmatch Hpalign Hread Hc Hsig Hh Hdev Hbytes).
    reflexivity.
  Qed.
...
Section GenExecLoad4Walk.
  ...
  Lemma exec_execute_LOAD_2_gpr_walk :
    exec (execute (LOAD (imm, Regidx rs1, Regidx rd, is_unsigned, 2))) s
      = Some (RETIRE_SUCCESS,
              set_reg s' (R_bitvector_64 (gpr_of_Z (uint rd))) (regval_into_reg (extend_value is_unsigned data2))).
  Proof.
    ...
    rewrite (exec_bind_Some _ _ _ _ _ Hass).
    rewrite (exec_bind_Some _ _ _ _ _
      (exec_vmem_read_2_gpr_walk p rs1 offset a v region s s' pa Htea Halign Htr Hcp Hmprv Hpmp Hmatch Hpalign Hread Hc Hsig Hh Hdev Hbytes)).
    cbn match.
    assert (Hw : exec (wX_bits (Regidx rd) (extend_value is_unsigned data2)) s'
                 = Some (tt, set_reg s' (R_bitvector_64 (gpr_of_Z (uint rd)))
                                (regval_into_reg (extend_value is_unsigned data2)))).
...
Section GenVMemWriteAddr4Walk.
  ...
  Lemma exec_vmem_write_addr_2_walk :
    exec (vmem_write_addr (Virtaddr a) 2 data (Store Data) false false false) s
      = Some (Ok true, MState s'.(sregs) (write_bytes s'.(mem) pa 2 data) s'.(mdev)).
  Proof.
    unfold vmem_write_addr.
...
Section GenVMemWrite4Walk.
  ...
  Lemma exec_vmem_write_2_gpr_walk :
    exec (vmem_write (Regidx rs1) offset 2 data (Store Data) false false false) s
      = Some (Ok true, MState s'.(sregs) (write_bytes s'.(mem) pa 2 data) s'.(mdev)).
  Proof.
    unfold vmem_write. rewrite exec_catch_early_return.
    ...
    rewrite (execR_bind_Some _ _ _ _ _ (execR_returnR_fwd (Virtaddr a) s)).
    rewrite execR_liftR.
    rewrite (exec_vmem_write_addr_2_walk p a data region s s' pa Halign Htr Hcp Hmprv Hpmp Hmatch Hpalign Hwrite Hc Hsig Hh Hdev).
    reflexivity.
  Qed.
...
Section GenExecStore4Walk.
  ...
  Lemma exec_execute_STORE_2_gpr_walk :
    exec (execute (STORE (imm, Regidx rs2, Regidx rs1, 2))) s
      = Some (RETIRE_SUCCESS,
              MState s'.(sregs) (write_bytes s'.(mem) pa 2
                (autocast (T := mword) (subrange_vec_dec vrs2 (Z.sub (Z.mul 2 8) 1) 0) : mword 16)) s'.(mdev)).
  Proof.
    ...
    cbn match.
    rewrite (exec_bind_Some _ _ _ _ _
      (exec_vmem_write_2_gpr_walk p rs1 offset a _ region s s' pa Htea Halign Htr Hcp Hmprv Hpmp Hmatch Hpalign Hwrite Hc Hsig Hh Hdev)).
    cbn match.
    apply exec_returnM.
  Qed.
