...
Section ProofFilealloc.
  ...
  Definition fa_epi_body {kt : ktier}
    ...
    (∀ (mj : regfile) (res : mword 64),
        ...
        sie_cap_gpr kt (CID := CID0) mj (K - 4)%nat b p -∗
  ...
  Lemma wp_filealloc_sconf
    ...
    : wp_filealloc_sconf_body kt γl γf m n eb p K b lks.
  Proof.
    ...
    iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.filealloc + 0x0e)) Ra0 Ra0 (mword_of_int 0x498 : mword 12)
              R3 (K - 4)%nat b ltac:(vm_compute; discriminate) ltac:(rdok)
              with "Hcg Hpc Hi0e").
    iIntros (CID7 Hs7) "Hcg Hpc".
    iEval (rgne) in "Hcg".
    set (R4 := <[Regidx Ra0 := regval_into_reg
                  (add_vec (R3 !!! Regidx Ra0) (sign_extend' 64 (mword_of_int 1176 : mword 12)))]> R3).
    ...
    iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.filealloc + 0x1a)) Rs1 Rs1 (mword_of_int 0x4a4 : mword 12)
              R6 (trap_res b + (K - 4))%nat false ltac:(vm_compute; discriminate) ltac:(rdok)
              with "Hcg Hpc Hi1a").
    iApply wp_next_off_intro. iIntros "Hcg Hpc".
    iEval (rgne) in "Hcg".
    set (R7 := <[Regidx Rs1 := regval_into_reg
                  (add_vec (R6 !!! Regidx Rs1) (sign_extend' 64 (mword_of_int 1188 : mword 12)))]> R6).
    ...
    iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.filealloc + 0x22)) Ra4 Ra4 (mword_of_int 0x43c : mword 12)
              R8 (trap_res b + (K - 4))%nat false ltac:(vm_compute; discriminate) ltac:(rdok)
              with "Hcg Hpc Hi22").
    iApply wp_next_off_intro. iIntros "Hcg Hpc".
    iEval (rgne) in "Hcg".
    set (R9 := <[Regidx Ra4 := regval_into_reg
                  (add_vec (R8 !!! Regidx Ra4) (sign_extend' 64 (mword_of_int 1084 : mword 12)))]> R8).
    ...
    iAssert (∀ (CID0 : CpuId), fa_epi_body (kt := kt) γf m spr K b p n eb ret_tgt CID0 lks)%I
    ...
    set (Cfound := (∀ (i : nat) (Mi : regfile),
        ...
        sie_cap_gpr kt Mi (trap_res b + (K - 4))%nat false p -∗
    ...
    set (Cfull := (∀ (Mf : regfile),
        ...
        sie_cap_gpr kt Mf (trap_res b + (K - 4))%nat false p -∗
    ...
    { iIntros (fuel). iInduction fuel as [|fuel IH] "IH";
      ...
      - (* ---- entry IS in use: ref reads nonzero, keep scanning ---- *)
        ...
        iApply (wp_clw_s_sconf (kt := kt) (ktd := KT0) (mword_of_int (KernelSyms.filealloc + 0x26)) Ra5 Rs1 (mword_of_int 4 : mword 12)
    ...
    { iIntros (i Mi) "(%Hi & %HMgi & %Mis1 & %Mithr) Hcg Hpc Hslots".
      ...
      iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.filealloc + 0x4a)) Ra0 Ra0 (mword_of_int 0x45c : mword 12)
                F2 (trap_res b + (K - 4))%nat false ltac:(vm_compute; discriminate) ltac:(rdok)
                with "Hcg Hpc Hi4a").
      iApply wp_next_off_intro. iIntros "Hcg Hpc".
      iEval (rgne) in "Hcg".
      set (F3 := <[Regidx Ra0 := regval_into_reg
                    (add_vec (F2 !!! Regidx Ra0) (sign_extend' 64 (mword_of_int 1116 : mword 12)))]> F2).
      ...
      iApply (Release.wp_release_sconf kt γl ftable_addr "ftable"%string (ftable_res γf) F4
                n eb p (K - 4)%nat
                ({["ftable"]} ∪ lks)
    ...
    { iIntros (Mf) "%Mfthr Hcg Hpc Hslots".
      ...
      iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.filealloc + 0x36)) Ra0 Ra0 (mword_of_int 0x470 : mword 12)
                G1 (trap_res b + (K - 4))%nat false ltac:(vm_compute; discriminate) ltac:(rdok)
                with "Hcg Hpc Hi36").
      iApply wp_next_off_intro. iIntros "Hcg Hpc".
      iEval (rgne) in "Hcg".
      set (G2 := <[Regidx Ra0 := regval_into_reg
                    (add_vec (G1 !!! Regidx Ra0) (sign_extend' 64 (mword_of_int 1136 : mword 12)))]> G1).
      ...
      (* the acquire handed the window index out as [trap_res b + N]; release
         ...
         acquire/release pair compose back to [N]. *)
      iEval (rewrite Houtb) in "Hcg".
      iApply (Release.wp_release_sconf kt γl ftable_addr "ftable"%string (ftable_res γf) G3
                n eb p (K - 4)%nat
                ({["ftable"]} ∪ lks)
