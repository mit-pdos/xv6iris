(* ProofUartwriteLoc.v -- the whole-function proof of uartwrite AT THE
   ...
   the rotated loop, the [iLoeb] at +0x4a inside the [nat] induction, the
   index discipline, the THR store's licence.  Read that file first;
...
Require Import UartSentLoc.   (* the located receipt and its two producer lemmas *)
...
Section UwProps.
  ...
  (* THE EMPTY CLAIM IS THE SEED ITSELF.  The frozen proof opens [dev_inv]
     here for a free [uart_sent_sub gu []] ([uw_sent_sub_empty]); the
     located contract HANDS one in, and [UartSentLoc.uart_sent_from_refl]
     turns it into the entry receipt with no modality at all.  Hence no
     lemma in this slot. *)
  ...
  Definition uw_next_cont `{CID0 : CpuId} `{XI : CurCtx} (γu : uart_names)
      ...
      (pidv : mword 32) (dqp : dfrac) (i : nat) (lks : gset string) (tr0 : list (bv 8)) : iProp Σ :=
  ...
  Definition uw_exit_cont `{CID0 : CpuId} `{XI : CurCtx} (γu : uart_names)
      (j : nat) (m0 : regfile) (av : nat) (eb : bool)
      (sp0 buf : mword 64) (n : nat) (f : nat -> bv 8) (dq : dfrac)
      (pidv : mword 32) (dqp : dfrac) (lks : gset string) (tr0 : list (bv 8)) : iProp Σ :=
    (wp_next (CID0 := CID0) true (proc_addr j) (fun (CID : CpuId) =>
       ∀ M' : regfile,
  ...
  Definition uw_ret `{CID0 : CpuId} `{XI : CurCtx} (γu : uart_names)
      (j : nat) (m0 : regfile) (av : nat) (eb : bool)
      (bs : list (bv 8)) (Rbuf : iProp Σ) (pidv : mword 32) (dqp : dfrac) (lks : gset string) (tr0 : list (bv 8)) : iProp Σ :=
    (wp_next (CID0 := CID0) true (proc_addr j) (fun (CID : CpuId) =>
       ∀ mf : regfile,
         ⌜ callee_saved m0 mf ⌝ -∗
...
Section UwBodies.
  ...
  Proof.
    ...
    iAssert (wp_next (CID0 := CID) true pj (fun (CIDh : CpuId) =>
      ...
      p_pid pj ↦₄{dqp} pidv -∗
      uw_full sp0 m0 -∗ uw_buf buf dq f n -∗
      ( uw_next_cont (CID0 := CID0) γu j m0 av true sp0 buf n f dq pidv dqp i lks tr0
        ∧ uw_exit_cont (CID0 := CID0) γu j m0 av true sp0 buf n f dq pidv dqp lks tr0 ) -∗
  ...
  Lemma uw_iter `{GEN : GenId} `{XI : CurCtx} (CID0 : CPU)
    ...
    ⊢ kernel_text -∗ dev_inv γu γv -∗ is_txlock γl γu -∗
      procs_inv γs -∗
      uw_head (CID0 := CID0) γu j m0 av eb sp0 buf n f dq pidv dqp i lks tr0.
  Proof.
...
Section ProofUartwriteLoc.
  ...
  Lemma wp_uartwrite_loc_sconf (γu : uart_names) (γv : disk_names)
      (γs : list gname) (j : nat) (γlp : gname) (γl : gname)
      (m : regfile) (av : nat) (eb : bool)
      (n : nat) (f : nat -> bv 8) (dq : dfrac) (b : bool)
      (pidv : mword 32) (dqp : dfrac) (lks : gset string) (tr0 : list (bv 8))
  ...
  Proof.
    ...
    iIntros "Hcg Hcnt #Ht Hpc #Hdinv #Htxl Hpid Hbuf #Hpinv #Hseed Hcont".
