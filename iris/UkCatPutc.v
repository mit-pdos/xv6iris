...
Section UkCatPutc.
  ...
  (* --------------------------------------------------------------------- *)
  (* putc(fd, c) @0x45c -- ulib's one-byte write.                            *)
  (*                                                                        *)
  ...
  Proof using .
    ...
    assert (Ho16 : uoff_sdsp (mword_of_int 2 : mword 6) = 16)
      by (vm_compute; reflexivity).
    (* ---- 0x45c  c.addi sp,sp,-32 -- THE PUSH ---- *)
    iApply (wp_uk_caddi_sp_dn N h m (mword_of_int 0x45c)
              (mword_of_int 32 : mword 6) 4 n
              ltac:(apply bv_eq; vm_compute; reflexivity)
              with "[] Hrun").
    { iApply (uis_cat_45c with "Hcode"). }
    iIntros "Hframe".
    assert (E41a : add_vec_int (mword_of_int 0x45c : mword 64) 2
                   = mword_of_int 0x45e)
    ...
    (* ---- 0x45e  c.sdsp ra,24(sp) ---- *)
    iApply (wp_uk_csdsp N h1 m1 (mword_of_int 0x45e)
              (mword_of_int 3 : mword 6) ra_idx (uint sp0 - 8) vra n
              ltac:(rewrite Hsp1 Hsp32 Ho24; lia)
              ltac:(rewrite Zminus_mod Hal8; reflexivity)
              with "[] Hwra Hrun").
    { iApply (uis_cat_45e with "Hcode"). }
    iIntros "Hwra".
    assert (E41c : add_vec_int (mword_of_int 0x45e : mword 64) 2
                   = mword_of_int 0x460)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E41c.
    iIntros (h2) "Hrun".
    (* ---- 0x460  c.sdsp s0,16(sp) ---- *)
    iApply (wp_uk_csdsp N h2 m1 (mword_of_int 0x460)
    ...
    iIntros "Hws0".
    assert (E41e : add_vec_int (mword_of_int 0x460 : mword 64) 2
                   = mword_of_int 0x462)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E41e.
    iIntros (h3) "Hrun".
    ...
    assert (E420 : add_vec_int (mword_of_int 0x462 : mword 64) 2
                   = mword_of_int 0x464)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E420.
    iIntros (h4) "Hrun".
    ...
    { rewrite <- Hsp1.
      exact (upd_ne m1 (Regidx s0_idx) (Regidx csp_rs1) (regval_into_reg sp0)
               ltac:(vm_compute; discriminate)). }
    (* ---- 0x464  sb a1,-17(s0) -- the byte, into BYTE 7 of the third word ---- *)
    ...
    iApply (wp_uk_sb N h4 m2 (mword_of_int 0x464)
              (mword_of_int 4079 : mword 12) s0_idx a1_idx
              (uint sp0 - 17) (nth_byte vb 7%nat) n
              ltac:(rewrite Hs02 Hoff17; lia)
              with "[] Hb7 Hrun").
    { iApply (uis_cat_464 with "Hcode"). }
    iIntros "Hb7".
    assert (E422 : add_vec_int (mword_of_int 0x464 : mword 64) 4
                   = mword_of_int 0x468)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E422.
    iIntros (h5) "Hrun".
    (* THE FRAME WORD STAYS OPEN over the write (lane CAT-WALK, W1): the
       ...
       returns it. *)
    (* ---- 0x468  c.li a2,1 ---- *)
    iApply (wp_uk_cli N h5 m2 (mword_of_int 0x468)
              (mword_of_int 1 : mword 6) a2_idx n
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate) with "[] Hrun").
    { iApply (uis_cat_468 with "Hcode"). }
    assert (E426 : add_vec_int (mword_of_int 0x468 : mword 64) 2
                   = mword_of_int 0x46a)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E426.
    iIntros (h6) "Hrun".
    ...
    { rewrite <- Hs02.
      exact (upd_ne m2 (Regidx a2_idx) (Regidx s0_idx) _
               ltac:(vm_compute; discriminate)). }
    (* ---- 0x46a  addi a1,s0,-17 ---- *)
    iApply (wp_uk_addi N h6 m3 (mword_of_int 0x46a)
              (mword_of_int 4079 : mword 12) s0_idx a1_idx
    ...
    assert (E428 : add_vec_int (mword_of_int 0x46a : mword 64) 4
                   = mword_of_int 0x46e)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E428.
    iIntros (h7) "Hrun".
    ...
    (* ---- 0x46e  jal ra,0x392 <write> ---- *)
    iApply (wp_uk_jal N h7 m4 (mword_of_int 0x46e)
              (mword_of_int 2096990 : mword 21) ra_idx
              (mword_of_int CatSyms.write) (mword_of_int 0x472) n
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate)
    ...
    set (m5 := <[Regidx ra_idx
                 := regval_into_reg (mword_of_int 0x472 : mword 64)]> m4).
    assert (Hra5 : m5 !!! Regidx ra_idx = (mword_of_int 0x472 : mword 64))
    ...
    assert (Eret : ret_pc (m5 !!! Regidx ra_idx) = (mword_of_int 0x472 : mword 64))
    ...
    { intros r Hr Hrsp Hrs0.
      ...
      rewrite /m5 (upd_ne m4 (Regidx ra_idx) (Regidx r)
                     (regval_into_reg (mword_of_int 0x472 : mword 64)) Nra).
      ...
      rewrite /m1 (upd_ne m (Regidx csp_rs1) (Regidx r)
                     ...
                     Hrsp).
      reflexivity. }
    (* ---- 0x472  c.ldsp ra,24(sp) ---- *)
    iApply (wp_uk_cldsp N h9 m6 (mword_of_int 0x472)
              ...
              ltac:(vm_compute; discriminate)
              with "[] Hwra Hrun").
    { iApply (uis_cat_472 with "Hcode"). }
    iIntros "Hwra".
    assert (E430 : add_vec_int (mword_of_int 0x472 : mword 64) 2
                   = mword_of_int 0x474)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E430.
    iIntros (h10) "Hrun".
    ...
    { rewrite <- Hsp6.
      exact (upd_ne m6 (Regidx ra_idx) (Regidx csp_rs1) _
               ltac:(vm_compute; discriminate)). }
    (* ---- 0x474  c.ldsp s0,16(sp) ---- *)
    iApply (wp_uk_cldsp N h10 m7 (mword_of_int 0x474)
              ...
              ltac:(vm_compute; discriminate)
              with "[] Hws0 Hrun").
    { iApply (uis_cat_474 with "Hcode"). }
    iIntros "Hws0".
    assert (E432 : add_vec_int (mword_of_int 0x474 : mword 64) 2
                   = mword_of_int 0x476)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E432.
    iIntros (h11) "Hrun".
    ...
    { rewrite <- Hsp7.
      exact (upd_ne m7 (Regidx s0_idx) (Regidx csp_rs1) _
               ltac:(vm_compute; discriminate)). }
    (* ---- 0x476  c.addi16sp sp,sp,32 -- THE POP: the frame goes back ---- *)
    iApply (wp_uk_caddi16sp_up N h11 m8 (mword_of_int 0x476)
              (mword_of_int 2 : mword 6) 4 n
              ltac:(apply bv_eq; vm_compute; reflexivity)
              with "[] [Hwra Hws0 Hwb Hw32] Hrun").
    { iApply (uis_cat_476 with "Hcode"). }
    { rewrite Hsp8 Hup.
    ...
    assert (E434 : add_vec_int (mword_of_int 0x476 : mword 64) 2
                   = mword_of_int 0x478)
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    { rewrite /m9 (upd_ne m8 (Regidx csp_rs1) (Regidx ra_idx) _
      ...
      exact Hra1. }
    (* ---- 0x478  c.jr ra ---- *)
    iApply (wp_uk_cjr N h12 m9 (mword_of_int 0x478) ra_idx
              (ret_pc (m !!! Regidx ra_idx)) (4 + n)
              ltac:(vm_compute; discriminate)
              ltac:(rewrite Hra9; reflexivity)
              with "[] Hrun").
    { iApply (uis_cat_478 with "Hcode"). }
    iIntros (h13) "Hrun".
