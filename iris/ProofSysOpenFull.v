(* ProofSysOpenFull.v -- sys_open's walk, O_CREATE ARM, and THE SEAL:
   ...
   The plain arm is NOT re-proved here.  [ProofSysOpen]'s
   ...
   [wp_sys_open_create] is this file's own walk.  It is [ProofSysOpen]'s
   ...
   than proved, and control reaches [ProofSysOpenEntryC.so_entry_c_au] at
   ...
   BINDERS: [ProofSysOpen]'s list verbatim. *)
...
Require Import ProofSysOpen.       (* [SysOpenPlainProof]: the plain arm *)
Require Import ProofSysOpenEntryC. (* [so_entry_c_au], [so_cont0_au_create] *)
...
Module EntryC := SysOpenEntryC Create Iunlock Iunlockput EndOp
                                 Fileclose Itrunc Filealloc Fdalloc.


Section ProofSysOpenFullBody.
  ...
  Proof.
    ...
    2:{ (* ---- ARM 0: the string did not fetch.  No begin_op, no s1 save ---- *)
      ...
      { (* ARM 0: argstr refused, so nothing fs-visible happened and the
           ...
           bundle, so the arm is a frame and nothing else. *)
 ...
 ns dqb dqs dqbs dqn (proc_addr j) pid vom
    ...
    (* THE MIRROR IMAGE of the plain walk's entry.  [om_create vom = true]
       makes the [andi]'s mask leave a NONZERO word
       ([ProofSysOpenBits.soau_and_pow2_zero], read in the other
       direction), so the [c.beqz] FALLS THROUGH and the else arm is
       REFUTED rather than proved: exclusion by premise, at the machine. *)
    assert (Hocne : eq_vec (so_and (arg_int32 vom) 512)
                      (zero_reg : mword 64) = false).
    { apply (proj2 (eq_vec_false_iff _ _)). intros Heq.
      assert (Hz : so_and (arg_int32 vom) 512 = (mword_of_int 0 : mword 64)).
      { rewrite Heq. apply bv_eq; vm_compute; reflexivity. }
      apply (proj1 (soau_and_pow2_zero (arg_int32 vom) 9 512 ltac:(lia)
                      ltac:(vm_compute; reflexivity)
                      ltac:(vm_compute; reflexivity))) in Hz.
      rewrite soau_om_arg in Hz. unfold om_create in Hcr. congruence. }
