(* SpecFileinit.v -- the public interface of Fileinit, stated independently of
...
(* The address of the string literal "ftable" that fileinit passes as initlock's
   ...
   its own, so it is spelled out here (see kernel.asm: 80007558 <etext+0x558>). *)
Definition ftable_name_str : Z := 0x80007558%Z.
...
(* fileinit() = initlock(&ftable.lock, "ftable").  The lock is the FIRST member
   ...
   touches no state beyond the three fields of that lock: the caller hands the
   word and the cpu field in raw and gets them back zeroed, and the name field
   comes back sealed as the persistent [lock_name lk "ftable"].  Whether the
...
Definition wp_fileinit_sconf_body `{!riscvGS Σ} `{!sieG Σ} `{GEN : GenId} `{CID : CpuId}
