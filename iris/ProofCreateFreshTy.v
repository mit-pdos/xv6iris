...
Definition create_fresh_ty_body
  ...
  (forall `{CIDa : CpuId}
     ...
     wp_ialloc_gen_body (CID := CIDa) γs' j' γl' γu' γd' γk' pd' pav' pu' bn'
                        γ' γi' gtl' γpr' cov' logstart' inodestart'
  ...
  (forall `{CIDl : CpuId}
     ...
     wp_ilock_dep_sconf_body (CID := CIDl) γs' j' γl' γu' γd' γk' pd' pav' pu' bn'
                             γi' gil' gisl' cov' logstart' inodestart'
    ...
    (gil gisl : gname) (dn : dinode) (bm : blkmap),
      ...
      (if alloc
       then
         ...
         ic_loaded fsc_fs γi cov logstart kslot inum dn bm ∗
...
Proof.
  ...
  iApply (Hia CID3 γs j γl γu γd γk pd pav pu bn γ γi gtl γpr cov logstart
  ...
  - (* ============================================================== *)
    ...
    iDestruct (cft_esc_acc γi cov logstart kslot Hkslt with "Hesc")
      as "#Hescc".
    ...
    iDestruct (cpu_own_transport CID4 CID7 0%nat eb (proc_addr j) b
                 ltac:(rewrite Hb; wp_next_chain) with "Hcnt") as "Hcnt".
    iApply (Hil CID7 γs j γl γu γd γk pd pav pu bn γi gilc gislc cov logstart
