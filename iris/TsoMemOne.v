(** * TsoMemOne.v — the ONE-LOG view machine, kept for the discovery record

    This is the spike machine as it stood after the load–load relaxation
    ([claude-notes/completed/relaxed-rr.md]) and BEFORE the store–store
    relaxation: one global write log appended at issue, whose order is both
    the store order and the coherence order; per hart a floor, a read
    watermark and per-byte coherence floors, all log positions.

    It is NOT the model of record.  That is [TsoMem.v], which splits the
    log into an issue log and a drain log ([claude-notes/projects/relaxed-ww.md]).
    This file exists so that the TSO port's discovery-record twins
    ([TsoCtxTwin.v], [TsoCtxRehearsal.v], [TsoCtxTwin2.v] — "kept, do not
    extend") keep meaning exactly what they meant when they were written:
    they prove a context machinery over the one-log machine, and their
    two-log successor is the relaxed-ww project's own twin.  Nothing else
