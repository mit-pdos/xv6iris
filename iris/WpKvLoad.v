...
(* Load forward engine: RVC load (c.ldsp) with a state-preserving fetch (the
   superpage TLB hit). Reuses the RVC step engine; the post-execute state has a
   REGISTER changed (vs the store's memory). *)
Section ForwardCldsp.
  Context (s : mstate) (pc : mword 64) (b : bool) (w16 : mword 16)
          (cinstr base : instruction) (rd : mword 5) (value : mword 64).
  Let sAl := set_reg s (R_bool minstret_increment) b.
  Let s_pc := set_reg sAl nextPC (add_vec_int pc 2).
  Let sXsg := set_reg s_pc (R_bitvector_64 (gpr_of_Z (uint rd))) (regval_into_reg value).
  ...
  Definition sTlsg : mstate := set_reg sXsg PC (register_lookup nextPC sXsg.(sregs)).
  Definition sFlsg : mstate :=
    if b then set_reg sTlsg minstret (add_vec_int (register_lookup minstret sTlsg.(sregs)) 1)
         else sTlsg.

  Lemma forward_exec_cldsp_super :
    ...
    exec riscv_step s = Some (tt, sFlsg).
  Proof using All.
  ...
  Definition base_upd_lsg_super : mstate := set_reg sXsg PC (add_vec_int pc 2).
  Definition sFclsg_super : mstate :=
    if b then set_reg base_upd_lsg_super minstret (add_vec_int mst0 1) else base_upd_lsg_super.

  Lemma sFl_eq_super : register_lookup minstret s.(sregs) = mst0 -> sFlsg = sFclsg_super.
  Proof using All.
    intro Lmst_s.
    ...
    { unfold sXsg, s_pc, sAl; cbn [sregs].
      ...
      rewrite register_lookup_set. reflexivity. }
    unfold sFlsg, sTlsg, sFclsg_super, base_upd_lsg_super. rewrite Enpc. destruct b; [|reflexivity].
    assert (Emst : register_lookup minstret (set_reg sXsg PC (add_vec_int pc 2)).(sregs) = mst0).
    { unfold sXsg, s_pc, sAl, set_reg; cbn [sregs].
      do 4 (rewrite irrelevant_register_set; [ | reg_ne ]). exact Lmst_s. }
