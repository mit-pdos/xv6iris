(* WpUserZicond.v -- U-mode USTEP arm for Zicond (CZERO.EQZ / CZERO.NEZ).

   ZICOND_RTYPE is a retiring two-source compute op, so it rides the generic
   two-source arm ustep_rtype2 as a classification-time instance: fix the
   instruction to ZICOND_RTYPE and the fold function to [zicond_f op], and
   discharge ustep_rtype2's execute premise with ZicondGpr's
   exec_execute_ZICOND_RTYPE_gpr_nz. *)
...
(* fold function so that [zicond_f op] applied to the two source-reads equals
   [zicond_rd_val op]: this is the shape ustep_rtype2 expects (f rs1v rs2v). *)
Definition zicond_f (op : zicondop) : mword 64 -> mword 64 -> mword 64 :=
  fun a b => if (match op with
                 | CZERO_EQZ => eq_vec b (zeros' 64)
                 | CZERO_NEZ => neq_vec b (zeros' 64)
                 end) then zeros' 64 else a.

Section WpUserZicond.
  Context `{!riscvGS Σ}.
  Context `{CID : CpuId}.
  Context (U : uctx).
  ...
  Lemma ustep_zicond (op : zicondop) (rs2 rs1 rd : mword 5)
