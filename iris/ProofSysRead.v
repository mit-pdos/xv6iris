(* ProofSysRead.v -- whole-function WP for sys_read(), the second syscall
   ...
   Twenty-five instructions; CodeSysRead.v has the listing and SpecSysRead.v
   the shape notes.  ProofSysFstat.v is the worked template for the whole
   shape -- the B1 seam ([ProcInv.proc_priv_lend] at the descriptor argfd
   resolved, [proc_priv_core] down to the file.c callee, [proc_ofiles_repay]
   + [proc_priv_join] on the way back, with the [upd_upt] crossing free by
   [cbn]) is verbatim.  THREE THINGS DIFFER, and sys_write repeats all
   three:
   ...
   * ONE EXTRA CALLEE, argint.  It hands back [ip ↦₄ arg_int32 v2], the
     ...
     reaches fileread's a2 is [sign_extend' 64 (trunc32 v2)], which
     [SpecSysRead.sys_rw_count_reg] says is [mword_of_int (sys_rw_count v2)]
     exactly.  That is the whole numeric story: the two owed premises
     ([0 <= n] and readi's MAXFILE row) stay premises of this shell, about
     the trapframe word the user wrote. *)
...
Require Import SpecReadi.       (* [K_readi] -- the stack budget unfolds to it *)
...
Lemma sr_upd_upt_id (V : pprivate) : upd_upt V (pv_upt V) = V.
Proof. destruct V; reflexivity. Qed.

(* [addi a1,s0,-40] / [ld a1,-40(s0)] : &p, the whole of frame slot 5 *)
Lemma sr_addr_p (X : mword 64) :
...
Lemma sr_addr_f_base (X : mword 64) :
  pa_stk X 3 = add_vec_int (pa_stk X 6) 24.
Proof. unfold pa_stk. rewrite avi_assoc. f_equal; lia. Qed.

Module SysReadProof (Argaddr : ARGADDR) (Argint : ARGINT) (Argfd : ARGFD)
...
Section ProofSysRead.
  ...
  Proof.
    ...
    iPoseProof (sri_40 with "Htext") as "Hi40".
    iPoseProof (sri_42 with "Htext") as "Hi42".
    iPoseProof (sri_44 with "Htext") as "Hi44".
    iPoseProof (sri_46 with "Htext") as "Hi46".
  ...
  Proof.
    ...
    iPoseProof (sri_00 with "Htext") as "Hi00".
    iPoseProof (sri_02 with "Htext") as "Hi02".
    iPoseProof (sri_04 with "Htext") as "Hi04".
    iPoseProof (sri_06 with "Htext") as "Hi06".
    iPoseProof (sri_08 with "Htext") as "Hi08".
    iPoseProof (sri_0c with "Htext") as "Hi0c".
    iPoseProof (sri_0e with "Htext") as "Hi0e".
    iPoseProof (sri_12 with "Htext") as "Hi12".
    iPoseProof (sri_16 with "Htext") as "Hi16".
    iPoseProof (sri_18 with "Htext") as "Hi18".
    iPoseProof (sri_1c with "Htext") as "Hi1c".
    iPoseProof (sri_20 with "Htext") as "Hi20".
    iPoseProof (sri_22 with "Htext") as "Hi22".
    iPoseProof (sri_24 with "Htext") as "Hi24".
    iPoseProof (sri_28 with "Htext") as "Hi28".
    iPoseProof (sri_2a with "Htext") as "Hi2a".
    iPoseProof (sri_2c with "Htext") as "Hi2c".
    iPoseProof (sri_30 with "Htext") as "Hi30".
    iPoseProof (sri_34 with "Htext") as "Hi34".
    iPoseProof (sri_38 with "Htext") as "Hi38".
    iPoseProof (sri_3c with "Htext") as "Hi3c".
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.sys_read + 0x0e))
              Rra (mword_of_int 2087720 : mword 21) M4 (av - 6)%nat b
    ...
    assert (Hjaa : add_vec (mword_of_int (KernelSyms.sys_read + 0x0e) : mword 64)
                     (sign_extend' 64 (mword_of_int 2087720 : mword 21)) = mword_of_int KernelSyms.argaddr)
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.sys_read + 0x18))
              Rra (mword_of_int 2087682 : mword 21) B2 (av - 6)%nat b
              ltac:(vm_compute; discriminate) ltac:(rdok) ltac:(vm_compute; reflexivity)
              with "Hcg Hpc Hi18").
    ...
    assert (Hjai : add_vec (mword_of_int (KernelSyms.sys_read + 0x18) : mword 64)
                     (sign_extend' 64 (mword_of_int 2087682 : mword 21)) = mword_of_int KernelSyms.argint)
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    - (* ================= FAILURE: argfd returned -1 ================= *)
      ...
      iApply (wp_blt_x0_taken_s_sconf (mword_of_int (KernelSyms.sys_read + 0x2c))
    ...
    - (* ================= SUCCESS: the descriptor resolved ============= *)
      ...
      iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.sys_read + 0x3c))
                Rra (mword_of_int 2094268 : mword 21) S3 (av - 6)%nat b
                ltac:(vm_compute; discriminate) ltac:(rdok) ltac:(vm_compute; reflexivity)
                with "Hcg Hpc Hi3c").
      ...
      assert (Hjfr : add_vec (mword_of_int (KernelSyms.sys_read + 0x3c) : mword 64)
                       (sign_extend' 64 (mword_of_int 2094268 : mword 21)) = mword_of_int KernelSyms.fileread)
        by (apply bv_eq; vm_compute; reflexivity).
      ...
      iDestruct (cpu_own_transport CID17 CID24 0%nat eb pj b 
                   ltac:(rewrite Hb; wp_next_chain) with "Hcpu") as "Hcpu".
      iApply (Fileread.wp_fileread_sconf kt γa γf γs j γlp kk qq Cf fn pidv V
