...
(* INTERRUPTS MUST BE DISABLED at holding() -- it [jal mycpu]s at +0x16, and
   ...
   [lk->cpu] read below). *)
Definition wp_holding_lockinv_s_sconf_body `{!riscvGS Σ, !xv6G Σ} `{GEN : GenId} `{CID : CpuId} `{XI : TsoCtx.CurCtx} (kt : ktier) (γl : gname) (lka : mword 64) (s : string) (R : TsoCtx.CtxId -> iProp Σ) (Tc Dc : iProp Σ) (m : regfile) (n : nat) (p : mword 64) (lks : gset string) :=
...
(* INTERRUPTS MUST BE DISABLED -- see the note above [wp_holding_lockinv_s_sconf_body];
   ...
   ambient hart. *)
Definition wp_holding_lockinv_locked_s_sconf_body `{!riscvGS Σ, !xv6G Σ} `{GEN : GenId} `{CID : CpuId} `{XI : TsoCtx.CurCtx} (kt : ktier) (γl : gname) (lka : mword 64) (s : string) (R : TsoCtx.CtxId -> iProp Σ) (Dc : iProp Σ) (m : regfile) (n : nat) (p : mword 64) :=
  let pcE : mword 64 := mword_of_int KernelSyms.holding in
  let lk := m !!! Regidx (mword_of_int 10 : mword 5) in
  let ret_tgt := ret_pc (m !!! Regidx (mword_of_int 1 : mword 5)) in
...
Module Type HOLDING.
  Parameter wp_holding_lockinv_s_sconf :
    forall `{!riscvGS Σ, !xv6G Σ} `{GEN : GenId} `{CID : CpuId} `{XI : TsoCtx.CurCtx} (kt : ktier) (γl : gname) (lka : mword 64) (s : string) (R : TsoCtx.CtxId -> iProp Σ) (Tc Dc : iProp Σ) (m : regfile) (n : nat) (p : mword 64) (lks : gset string),
      wp_holding_lockinv_s_sconf_body kt γl lka s R Tc Dc m n p lks.
  Parameter wp_holding_lockinv_locked_s_sconf :
    forall `{!riscvGS Σ, !xv6G Σ} `{GEN : GenId} `{CID : CpuId} `{XI : TsoCtx.CurCtx} (kt : ktier) (γl : gname) (lka : mword 64) (s : string) (R : TsoCtx.CtxId -> iProp Σ) (Dc : iProp Σ) (m : regfile) (n : nat) (p : mword 64),
