...
Section ProofSysMkdirM1Tail.
 ...
 (gfs : fs_names)
      ...
      (b : bool) (lks : gset string) (bf : nat -> bv 8) (Upr : ustate) :
...
Section ProofSysMkdirBody.
 ...
 ns dqb dqs dqbs dqn v
  ...
  Proof.
    ...
    iDestruct (cpu_own_transport CID6 CID10 0 eb pj b ltac:(wp_next_chain)
                 with "Hown") as "Hown".
    iApply (Argstr.wp_argstr_sconf (CID := CID10) fsc_kalloc gf M7 (K - 18)%nat 0%nat eb pj
      ...
      + (* ---------- create SUCCEEDED: the LOCKED inode ---------- *)
        ...
        { destruct made; [reflexivity | exfalso].
          ...
          assert (Hz : bv_unsigned (SpecDirlookup.T_DIR : mword 16) = 2)
            by (rewrite Hft; exact FsAbsCreateFire.T_FILE_value).
 ...
 kk qi ss gy loc tlc inum dn bm un1
                  ...
                  with "Hcg Hown [] [] Htext Hdata Hpc Hpe Hbio Hlog Hitab Hitinv
                        Hesc Hireg [] Hslk Hslkd [%] Hflc Hclaims Hdep Hoffd Hidev Hiinum Hivalid
        ...
        { (* the legs' receipts at a zero return: the directory was MADE *)
          ...
          iExists (bview pk bf), (bv_unsigned inum). iExact "Harms". }
 ...
 un1 pid (DfracOwn (1/4))
        ...
        { (* create refused: the walk died, or the cursor comes home with
             nothing fired or the do-then-undo PAIR *)
          rewrite /mkdir_arms. iRight.
          iSplitR; [iPureIntro; exact Ha0f |].
          iRight. iExists (bview pk bf). iExact "Harms". }
 ...
 MAXOPBLOCKS pid (DfracOwn (1/4))
                ...
                with "Hcg Hown [] [] Htext Hdata Hpc Hpe Hbio Hlog Hseam Hgen
                      ...
                      [Hpback Hbsl Hsbn Hsbi Hsbs Hsbb Hir Hwp Hdlkc Hcre Hcont]").
      { rewrite Heb /trap_csrs_ext. done. }
      { rewrite Heb /cpu_claim_ext. done. }
      iEval (rewrite /wp_next).
      iIntros (CIDz) "%Hqz". iIntros (mf) "%Hcsf %Ha0f Hcg Hown _ _ Hpc Hpbare".
