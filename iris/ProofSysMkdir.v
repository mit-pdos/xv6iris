...
Section ProofSysMkdirM1Tail.
  ...
  Lemma md_m1_tail `{GEN : GenId} `{CID0 : CpuId}
    ...
    log_ctx g bn gfs fsc_cov logstart dev -∗
  ...
  Proof.
    ...
    iApply (EndOp.wp_end_op_sconf (CID := CID1) gs jx gl gu gd gk pd pav pu bn
              g gfs fsc_cov logstart dev u pidv dq M1 (K - 18)%nat eb b lks
...
Section ProofSysMkdirBody.
  ...
  Lemma wp_sys_mkdir_sconf `{GEN : GenId} `{CID0 : CpuId}
    ...
    wp_sys_mkdir_sconf_body gf ga gpr gs j gl gu gd gk pd pav pu bn g gi
  ...
  Proof.
    ...
    iDestruct (cpu_own_transport CID0 CID5 0 eb pj b ltac:(wp_next_chain)
                 with "Hown") as "Hown".
    iApply (BeginOp.wp_begin_op_sconf (CID := CID5) gs j gl bn g fsc_fs fsc_cov logstart
    ...
    - (* ---- the string fetched: the [bltz] FALLS THROUGH ---- *)
      ...
      iApply (Create.wp_create_sconf (CID := CID17) gs j gl gu gd gk pd pav pu
                bn g gi gtl ga gf gpr logstart bmapstart inodestart
      ...
      + (* ---------- create SUCCEEDED: the LOCKED inode ---------- *)
        ...
        iDestruct (md_esc_acc gi logstart kk ltac:(lia)
        ...
        iApply (EndOp.wp_end_op_sconf (CID := CID22) gs j gl gu gd gk pd pav pu
                  bn g fsc_fs fsc_cov logstart dev n2 pid (DfracOwn (1/4))
      ...
      + (* ---------- ARM B: create returned 0 ---------- *)
        ...
        iDestruct (cpu_own_transport CID18 CID19 0 eb pj b
                     ltac:(wp_next_chain) with "Hown") as "Hown".
        iApply (md_m1_tail (CID0 := CID19) gs j gl gu gd gk pd pav pu bn g fsc_fs
                  logstart dev un1 pid (DfracOwn (1/4))
    ...
    - (* ================= ARM A: argstr returned -1 =================
      ...
      iDestruct (cpu_own_transport CID11 CID12 0 eb pj b
                   ltac:(wp_next_chain) with "Hown") as "Hown".
      iApply (md_m1_tail (CID0 := CID12) gs j gl gu gd gk pd pav pu bn g fsc_fs
                logstart dev MAXOPBLOCKS pid (DfracOwn (1/4))
