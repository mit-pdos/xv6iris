...
(*                                                                        *)
(* FOUR INSTRUCTIONS -- 0x80e c.ld a0,8(a0); 0x810 jal nulterminate;      *)
(* 0x814 c.ld a5,24(s1); 0x816 sb zero,0(a5) -- and then the same 0x81a   *)
...
(* instead of 0x13b4 and the row there sends control to 0x80e instead of  *)
(* 0x7f6.  [ushp_jrow_redir] is those four .rodata bytes, read off the    *)
...
(*ALIASES-BEGIN*)
  ...
  (* plus it is 0x80e -- which [vm_compute] checks below rather than this  *)
  ...
  Proof using .
    ...
    iApply (wp_uk_clw_text N h13 m10 (mword_of_int 0x7f0)
              ...
              (mword_of_int 4294964318 : mword 32) (4 + nn)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; reflexivity)
              ltac:(vm_compute; reflexivity)
              ltac:(rewrite Ha5_10;
    ...
    set (m11 := <[Regidx a5_idx
                  := regval_into_reg
                       (sign_extend' 64
                          (mword_of_int 4294964318 : mword 32)
                        : mword 64)]> m10).
    ...
    iApply (wp_uk_cadd N h14 m11 (mword_of_int 0x7f2) a5_idx
              a4_idx (mword_of_int 0x80e) (4 + nn)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate)
              ltac:(rewrite Ha4_11
    ...
    set (m12 := <[Regidx a5_idx
                  := regval_into_reg (mword_of_int 0x80e : mword 64)]> m11).
    ...
    iApply (wp_uk_cjr N h15 m12 (mword_of_int 0x7f4) a5_idx
              (mword_of_int 0x80e) (4 + nn)
              ltac:(vm_compute; discriminate)
              ltac:(rewrite (upd_eq m11 (Regidx a5_idx)
                               (regval_into_reg
                                  (mword_of_int 0x80e : mword 64)));
                    apply bv_eq; vm_compute; reflexivity)
              with "[] Hrun").
    { iApply (uis_shp_7f4 with "Hcode"). }
    iIntros (h16) "Hrun".
    (* ---- 0x80e  c.ld a5,8(a0) -- argv[0] ---- *)
    ...
    iApply (wp_uk_jal N h17 n1 (mword_of_int 0x810)
              ...
              (mword_of_int 0x7ca) (mword_of_int 0x814) (4 + nn)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate)
              ltac:(apply bv_eq; vm_compute; reflexivity)
              ltac:(apply bv_eq; vm_compute; reflexivity)
