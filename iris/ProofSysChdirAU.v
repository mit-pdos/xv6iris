(* ProofSysChdirAU.v -- sys_chdir on its AU contract ([SpecSysChdirAU]):
   [ProofSysChdir] re-walked at [SpecNameiEra.wp_namei_era] instead of the
   SET form, with the walk premise handed DOWN unfired (the walk picks the
   start inum -- ROOTINO on an absolute path, the block's [pv_cwi] on a
   relative one -- and fires it there, as [ProofSysOpenAUWalk] does), the
   observation commit fired under the landed node's lock exactly where the
   [T_DIR] test reads the type ([FsAbsOpenFire.opf_open_fire_1]), and the
   three arms of [chdir_arms] paid: the era refund on the dead arm, the
   cursor and the receipt at a non-directory on the refused arm, and the
   cursor, the receipt at [MkAnode (ADir e) nl] and the block at the
   cursor's inum on the success arm.

   COPY-ADAPT (the family's rule): everything below that is not the walk
   call, its post's destructuring, the fire after [ilock] and the four
   tails is [ProofSysChdir.v] verbatim -- the landed proof is untouched
   and stays linked ([LinkSysChdir]); this file seals [SYSCHDIR_AU] and
   [LinkSysChdirAU] closes it over [LinkNameiEra].  The landed header
   (below, kept) is the reading of the code.
...
(* ===================================================================== *)
Module SysChdirAUProof (Myproc : MYPROC) (BeginOp : BEGIN_OP) (Argstr : ARGSTR)
...
Section ProofSysChdirBody.
  ...
  Lemma wp_sys_chdir_au `{GEN : GenId} `{CID0 : CpuId} `{XI : CurCtx}
      (gf : gname)
    ...
    wp_sys_chdir_au_body gf gs j gl pd pav pu dqb dqs v pid U m K eb b lks
      P Pmiss Φo.
  Proof.
    cbv beta delta [wp_sys_chdir_au_body wp_sys_chdir_au_frame].
