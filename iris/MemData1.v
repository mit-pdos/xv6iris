(* MemData1.v -- mode-neutral width-1 DATA memory leaf lemmas (clone of MemData4; includes the width-1 read_ram primitive).

   The width-1 physical read/write core, factored out of the per-mode
...
(* width-1 physical read primitive (clone of RiscvFetchExec's width-2 one) *)
Lemma run_read_ram_plain_1_pin (addr : mword 64) (w : bv 8) s :
  dev_addr addr = false ->
  (forall j : nat, (N.of_nat j < 1)%N ->
     s.(mem) !! (pa_add addr j) = Some (nth_byte w j)) ->
  run (read_ram rv64d_types.Read_plain (Physaddr addr) 1 false) s (w, default_meta) s.
Proof.
  intros Hdev Hbytes.
...
(* ===================================================================== *)
(* Pure width-1 leaf helpers (mode-neutral).                             *)
(* ===================================================================== *)

Lemma avi0_mul1 (a : mword 64) : add_vec_int a (0 * 1) = a.
Proof. change (0 * 1) with 0. apply avi0. Qed.
...
Lemma data2_id_1 (v : mword 8) :
  update_subrange_vec_dec (zeros' (1*1*8)) (1*(0+1)*8-1) (1*0*8) v = v.
Proof.
...
Lemma exec_pmaCheck_ram_load_1 (addr : mword 64) (pbmt : page_based_mem_type)
      (region : PMA_Region) s :
  ...
  exec (pmaCheck (Physaddr addr) 1 (Load Data) pbmt false) s = Some (None, s).
Proof.
...
(* ===================================================================== *)
(* Privilege-generic checked_mem_read / mem_read at width 1 (Load Data). *)
(* The pmpCheck grant is supplied by the caller (Hpmp), which is the      *)
(* only privilege-dependent step.                                         *)
(* ===================================================================== *)

Lemma exec_checked_mem_read_ram_load_1 (p : Privilege) (pbmt : page_based_mem_type)
      (addr : mword 64) (region : PMA_Region) (w : bv 8) s :
  exec (pmpCheck (Physaddr addr) 1 (Load Data) p) s = Some (None, s) ->
  matching_pma_region (register_lookup pma_regions s.(sregs)) (Physaddr addr) 1 = Some region ->
  is_aligned_paddr (Physaddr addr) 1 = true ->
  (override_PMA (PMA_Region_attributes region) pbmt).(PMA_readable) = true ->
  exec (within_clint (Physaddr addr) 1) s = Some (false, s) ->
  exec (within_sig (Physaddr addr) 1) s = Some (false, s) ->
  exec (within_htif_readable (Physaddr addr) 1) s = Some (false, s) ->
  dev_addr addr = false ->
  (forall j : nat, (N.of_nat j < 1)%N -> s.(mem) !! (pa_add addr j) = Some (nth_byte w j)) ->
  exec (checked_mem_read (Load Data) pbmt p (Physaddr addr) 1 false false false false)
...
Proof.
  ...
  2:{ unfold phys_access_check.
      ...
      rewrite (exec_bind_Some _ _ _ _ _ (exec_pmaCheck_ram_load_1 addr pbmt region s Hmatch Halign Hread)).
      cbn match. apply exec_returnM. }
  rewrite (exec_bind_Some _ _ _ _ _
            (_ : exec (within_mmio_readable (Physaddr addr) 1) s = Some (false, s))).
  ...
  rewrite (exec_bind_Some _ _ _ _ _ (exec_read_ram_plain_1 addr w s Hdev Hbytes)).
  apply exec_returnM.
Qed.

Lemma exec_mem_read_load_1 (p : Privilege) (pbmt : page_based_mem_type)
      (addr : mword 64) (region : PMA_Region) (w : bv 8) (m : mword 64) s :
  exec (pmpCheck (Physaddr addr) 1 (Load Data) p) s = Some (None, s) ->
  matching_pma_region (register_lookup pma_regions s.(sregs)) (Physaddr addr) 1 = Some region ->
  is_aligned_paddr (Physaddr addr) 1 = true ->
  (override_PMA (PMA_Region_attributes region) pbmt).(PMA_readable) = true ->
  ...
  exec (mem_read (Load Data) pbmt (Physaddr addr) 1 false false false)
       s = Some (Ok w, s).
Proof.
  ...
  rewrite (exec_bind_Some _ _ _ _ _
            (_ : exec (mem_read_priv_meta _ _ _ _ 1 _ _ _ _) s = Some (Ok (w, default_meta), s))).
  2:{ unfold mem_read_priv_meta. cbn [orb andb].
      rewrite (exec_bind_Some _ _ _ _ _
                (_ : exec (checked_mem_read _ _ _ _ 1 _ _ _ _) s = Some (Ok (w, default_meta), s))).
      2:{ cbn match. apply exec_checked_mem_read_ram_load_1 with (p := p) (region := region); assumption. }
      cbn match. unfold mem_read_callback. apply exec_returnM. }
  cbn [MemoryOpResult_drop_meta]. apply exec_returnM.
Qed.

(* ===================================================================== *)
(* Privilege-generic checked_mem_write / mem_write_value at width 1.      *)
(* ===================================================================== *)

Lemma exec_checked_mem_write_ram_1 (p : Privilege) (pbmt : page_based_mem_type)
      (addr : mword 64) (region : PMA_Region) (data : bv 8) s :
  exec (pmpCheck (Physaddr addr) 1 (Store Data) p) s = Some (None, s) ->
  matching_pma_region (register_lookup pma_regions s.(sregs)) (Physaddr addr) 1 = Some region ->
  is_aligned_paddr (Physaddr addr) 1 = true ->
  (override_PMA (PMA_Region_attributes region) pbmt).(PMA_writable) = true ->
  exec (within_clint (Physaddr addr) 1) s = Some (false, s) ->
  ...
  dev_addr addr = false ->
  exec (checked_mem_write (Physaddr addr) 1 data (Store Data) pbmt p tt false false false) s
...
Lemma exec_mem_write_value_1 (p : Privilege) (pbmt : page_based_mem_type)
      (addr : mword 64) (region : PMA_Region) (data : bv 8) (m : mword 64) s :
  exec (pmpCheck (Physaddr addr) 1 (Store Data) p) s = Some (None, s) ->
  matching_pma_region (register_lookup pma_regions s.(sregs)) (Physaddr addr) 1 = Some region ->
  is_aligned_paddr (Physaddr addr) 1 = true ->
  (override_PMA (PMA_Region_attributes region) pbmt).(PMA_writable) = true ->
...
Proof.
  ...
  rewrite (exec_bind_Some _ _ _ _ _ (exec_checked_mem_write_ram_1 p pbmt addr region data s Hpmp Hmatch Halign Hwrite Hc Hsig Hh Hdev)).
  cbn match. unfold mem_write_callback. apply exec_returnm.
Qed.

(* ===================================================================== *)
(* Generic width-1 LOAD tower (state-preserving, privilege- and          *)
...
Section GenVMemReadAddr4.
  ...
  Hypothesis Hpmp : exec (pmpCheck (Physaddr pa) 1 (Load Data) p) s = Some (None, s).
  ...
  Hypothesis Hbytes : forall j : nat, (N.of_nat j < 1)%N -> s.(mem) !! (pa_add pa j) = Some (nth_byte v j).

  Lemma exec_vmem_read_addr_1 :
    ...
    { eapply execR_untilMT_1.
      ...
      - cbn match.
        ...
        { rewrite (execR_liftR_seq _ _ _ _ _
            (exec_mem_read_load_1 p PBMT_PMA pa region v (register_lookup mstatus s.(sregs)) s
...
Section GenVMemRead4.
  ...
  Lemma exec_vmem_read_1_gpr :
    exec (vmem_read (Regidx rs1) offset 1 (Load Data) false false false) s = Some (Ok data2, s).
  Proof.
    ...
    rewrite execR_liftR.
    rewrite (exec_vmem_read_addr_1 p a v region s pa Halign Hcp Hmprv Htr Hpmp Hmatch Hpalign Hread Hc Hsig Hh Hdev Hbytes).
    reflexivity.
  Qed.
End GenVMemRead4.
...
Section GenExecLoad4.
  ...
  Proof.
    ...
    rewrite (exec_bind_Some _ _ _ _ _
      (exec_vmem_read_1_gpr p rs1 offset a v region s pa Htea Halign Hcp Hmprv Htr Hpmp Hmatch Hpalign Hread Hc Hsig Hh Hdev Hbytes)).
    cbn match.
...
(* ===================================================================== *)
(* Generic width-1 STORE tower (state-preserving, privilege-generic).     *)
(* ===================================================================== *)
...
Section GenVMemWriteAddr4.
  ...
  Hypothesis Htr : exec (translateAddr (Virtaddr (add_vec_int (bits_of_virtaddr (Virtaddr a)) (0 * 1))) (Store Data)) s
                   = Some (Ok (Physaddr pa, PBMT_PMA, init_ext_ptw), s).
  Hypothesis Hpmp : exec (pmpCheck (Physaddr pa) 1 (Store Data) p) s = Some (None, s).
  Hypothesis Hmatch : matching_pma_region (register_lookup pma_regions s.(sregs)) (Physaddr pa) 1 = Some region.
  Hypothesis Hpalign : is_aligned_paddr (Physaddr pa) 1 = true.
    ...
    { eapply execR_untilMT_1.
      ...
      - cbn match.
        ...
        { match goal with
          ...
          rewrite (execR_liftR_seq _ _ _ _ _ (exec_mem_write_ea_1 pa s)).
          cbn match.
          match goal with
          ...
          rewrite (execR_liftR_seq _ _ _ _ _
            (exec_mem_write_value_1 p PBMT_PMA pa region data
...
Section GenVMemWrite4.
  ...
  Proof.
    ...
    rewrite (execR_bind_Some _ _ _ _ _ (execR_returnR_fwd (Virtaddr a) s)).
    rewrite execR_liftR.
    rewrite (exec_vmem_write_addr_1 p a data region s pa Halign Hcp Hmprv Htr Hpmp Hmatch Hpalign Hwrite Hc Hsig Hh Hdev).
    reflexivity.
  Qed.
End GenVMemWrite4.
...
Section GenExecStore4.
  ...
  Lemma exec_execute_STORE_1_gpr :
  ...
  Proof.
    ...
    cbn match.
    rewrite (exec_bind_Some _ _ _ _ _
      (exec_vmem_write_1_gpr p rs1 offset a _ region s pa Htea Halign Hcp Hmprv Htr Hpmp Hmatch Hpalign Hwrite Hc Hsig Hh Hdev)).
    cbn match.
    apply exec_returnM.
  Qed.
End GenExecStore4.
...
Section GenVMemReadAddr4Walk.
  ...
  Lemma exec_vmem_read_addr_1_walk :
    exec (vmem_read_addr (Virtaddr a) 1 (Load Data) false false false) s
      = Some (Ok data2, s').
  Proof.
    unfold vmem_read_addr.
...
Section GenVMemRead4Walk.
  ...
  Lemma exec_vmem_read_1_gpr_walk :
    exec (vmem_read (Regidx rs1) offset 1 (Load Data) false false false) s = Some (Ok data2, s').
  Proof.
    unfold vmem_read. rewrite exec_catch_early_return.
    assert (Hgta : exec (get_transformed_data_addr (Regidx rs1) offset (Load Data) 1) s
    ...
    rewrite (execR_bind_Some _ _ _ _ _ (execR_returnR_fwd (Virtaddr a) s)).
    rewrite execR_liftR.
    rewrite (exec_vmem_read_addr_1_walk p a v region s s' pa Halign Htr Hcp Hmprv Hpmp Hmatch Hpalign Hread Hc Hsig Hh Hdev Hbytes).
    reflexivity.
  Qed.
...
Section GenExecLoad4Walk.
  ...
  Lemma exec_execute_LOAD_1_gpr_walk :
    exec (execute (LOAD (imm, Regidx rs1, Regidx rd, is_unsigned, 1))) s
      = Some (RETIRE_SUCCESS,
              set_reg s' (R_bitvector_64 (gpr_of_Z (uint rd))) (regval_into_reg (extend_value is_unsigned data2))).
  Proof.
    ...
    rewrite (exec_bind_Some _ _ _ _ _ Hass).
    rewrite (exec_bind_Some _ _ _ _ _
      (exec_vmem_read_1_gpr_walk p rs1 offset a v region s s' pa Htea Halign Htr Hcp Hmprv Hpmp Hmatch Hpalign Hread Hc Hsig Hh Hdev Hbytes)).
    cbn match.
    assert (Hw : exec (wX_bits (Regidx rd) (extend_value is_unsigned data2)) s'
                 = Some (tt, set_reg s' (R_bitvector_64 (gpr_of_Z (uint rd)))
                                (regval_into_reg (extend_value is_unsigned data2)))).
...
Section GenVMemWriteAddr4Walk.
  ...
  Lemma exec_vmem_write_addr_1_walk :
    exec (vmem_write_addr (Virtaddr a) 1 data (Store Data) false false false) s
      = Some (Ok true, MState s'.(sregs) (write_bytes s'.(mem) pa 1 data) s'.(mdev)).
  Proof.
    unfold vmem_write_addr.
...
Section GenVMemWrite4Walk.
  ...
  Lemma exec_vmem_write_1_gpr_walk :
    exec (vmem_write (Regidx rs1) offset 1 data (Store Data) false false false) s
      = Some (Ok true, MState s'.(sregs) (write_bytes s'.(mem) pa 1 data) s'.(mdev)).
  Proof.
    unfold vmem_write. rewrite exec_catch_early_return.
    ...
    rewrite (execR_bind_Some _ _ _ _ _ (execR_returnR_fwd (Virtaddr a) s)).
    rewrite execR_liftR.
    rewrite (exec_vmem_write_addr_1_walk p a data region s s' pa Halign Htr Hcp Hmprv Hpmp Hmatch Hpalign Hwrite Hc Hsig Hh Hdev).
    reflexivity.
  Qed.
...
Section GenExecStore4Walk.
  ...
  Lemma exec_execute_STORE_1_gpr_walk :
    exec (execute (STORE (imm, Regidx rs2, Regidx rs1, 1))) s
      = Some (RETIRE_SUCCESS,
              MState s'.(sregs) (write_bytes s'.(mem) pa 1
                (autocast (T := mword) (subrange_vec_dec vrs2 (Z.sub (Z.mul 1 8) 1) 0) : mword 8)) s'.(mdev)).
  Proof.
    ...
    cbn match.
    rewrite (exec_bind_Some _ _ _ _ _
      (exec_vmem_write_1_gpr_walk p rs1 offset a _ region s s' pa Htea Halign Htr Hcp Hmprv Hpmp Hmatch Hpalign Hwrite Hc Hsig Hh Hdev)).
    cbn match.
    apply exec_returnM.
  Qed.
