  ...
  Proof.
    ...
    { iIntros (fuel). iInduction fuel as [|fuel IHf] "IHf".
      ...
      iDestruct (cpu_own_transport CIDk CIDl2 lvl eb pme b ltac:(wp_next_chain)
                   with "Hcpu") as "Hcpu".
      iApply (Acquire.wp_acquire_sconf KT1 (CID := CIDl2) γl "proc"%string (λ ξ : CtxId, proc_lock_res (XI := ξ) γs γl (proc_addr k)) L2 lvl eb pme (K - 4)%nat b lks
      ...
      - (* ============ FOUND: p->state == UNUSED ============ *)
        ...
        { (* ================== TAIL 1: kalloc failed, +0x86 ==================
          ...
          iApply (FP.wp_freeproc_sconf (CID := CIDf) γa T2 k γl V pidn USED ch None None
                    ...
                    with "Hcg Hcpu Htext Hpc [Hlocked Hstate Hpg Hchan Hkilled Hxstate Hpidinv] [Hpidown Hfields Hofc Hofs Hspare Hirsp Hbsp Hkst Hctx] [Hpgcell] [Htfcell] Henvb").
          all: try lkbelow.
          ...
          iApply (Release.wp_release_sconf KT1 (CID := CIDf) γl (proc_addr k) "proc"%string (λ ξ : CtxId, proc_lock_res (XI := ξ) γs γl (proc_addr k)) T4 lvl eb pme (K - 4)%nat
                    ({["proc"]} ∪ lks)
      ...
      by (apply locks_add_del_below; lkbelow).
        ...
        2: { (* ============ TAIL 2: proc_pagetable failed, +0x96 ============
          ...
          iApply (FP.wp_freeproc_sconf (CID := CIDf) γa U2 k γl V pidn USED ch None (Some (tfp, tfws))
                    ...
                    with "Hcg Hcpu Htext Hpc [Hlocked Hstate Hpg Hchan Hkilled Hxstate Hpidinv] [Hpidown Hfields Hofc Hofs Hspare Hirsp Hbsp Hkst Hctx] [Hpgcell] [Htfcell Htfpage] Henvb").
          all: try lkbelow.
          { rewrite /proc_held. iFrame "Hlocked Hstate Hpg Hchan".
            iExists kl, xs, pidn. iFrame "Hkilled Hxstate Hpidinv". }
          { rewrite /fp_rest. iSplitR.
          ...
          (* [b] IS [outb] ([cpu_own] forces it); pure re-spelling for release. *)
          iEval (rewrite Hbmatch) in "Hcg".
          iApply (Release.wp_release_sconf KT1 (CID := CIDf) γl (proc_addr k) "proc"%string (λ ξ : CtxId, proc_lock_res (XI := ξ) γs γl (proc_addr k)) U4 lvl eb pme (K - 4)%nat
                    ({["proc"]} ∪ lks)
      ...
      - (* ============ NOT FREE: release and step ============ *)
        ...
        (* [b] IS [outb] ([cpu_own] forces it); pure re-spelling for release. *)
        iEval (rewrite Hbmatch) in "Hcg".
        iApply (Release.wp_release_sconf KT1 (CID := CIDf) γl (proc_addr k) "proc"%string (λ ξ : CtxId, proc_lock_res (XI := ξ) γs γl (proc_addr k)) R2 lvl eb pme (K - 4)%nat
                  ({["proc"]} ∪ lks)
