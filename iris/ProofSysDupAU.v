(* ProofSysDupAU.v -- the whole-function WP proof of xv6's sys_dup() at the
   SHARPENED fd contract: [SpecSysDupAU.wp_sys_dup_au_body].

   Contract: SpecSysDupAU.v.  Decode: CodeSysDup.v.  Epilogue:
   ProofSysDupAUTail.v.  Linked: LinkSysDupAU.v.  The landed walk this one
   is a re-targeting of is ProofSysDup.v, whose header owns the resource
   story (argfd / LEND / fdalloc / filedup / REPAY, the ledger closing with
   ZERO allowance).  Nothing about the MACHINE changes here: same three
   calls, same three exits, same epilogue.  What changes is the four things
   SpecSysDupAU's "WHAT THE PROVER OWES" names, and they are all fd-side:

   1. THE BUNDLE IS EXPLICIT.  The landed walk holds [fd_frags_any] and
      opens the destination row with [fd_frags_any_acc], which loses the
      list; here the walk holds [fd_frags _ sts] and opens with
      [fd_frags_acc], whose give-back is [<[fd1 := _]> sts] -- exactly the
      shape the success arm names.  This is the settle-site delta and it is
      the whole of the difference at the ghost step
      (ProofSysOpenAUPub.so_tail_pub_au makes the same move for sys_open).
   2. THE DESTINATION ROW'S OLD STATE IS LEARNED, NOT GUESSED.  fdalloc
      hands its authority back still at [FdClosed]; [fd_st_agree] against
      the row the bundle just yielded turns that into
      [sts !! fd1 = Some FdClosed], which the arm exposes for free.
   3. THE SOURCE ROW'S AGREEMENT.  [proc_priv_lend] reports an ABSTRACT
      [stf] with only [stf <> FdClosed] known; the premise says row [fd0] of
      the caller's own bundle reads [FdOpen rb wb t], and [fd_st_agree]
      between the loan's authority and that row identifies the two.  The
      fragment goes straight back at the state it came out at
      ([list_insert_id]), so the source row reaches BOTH postconditions
      untouched -- which is what "the source's row is still [sts]'s" means.
   4. THE BAD-FD ARM IS REFUTED, NOT PROVED.  argfd's failure disjunct says
      [arg_fd v _ = None] and the frame's premise says [= Some (fd0, fv)];
      the walk stops at the [c.beqz] with a [discriminate] and the landed
      three arms become two.

   EXPLICIT-CPUID: three calls, so the [cpu_own] transports are the usual
   [cpu_own_transport ... ltac:(wp_next_chain)] before each -- the landed
   walk's own, unchanged. *)
...
Require Import ProofSysDupAUTail.  (* [sda_sp_bounds], [sda_tail] *)
...
Module SysDupAUProof (Argfd : ARGFD) (Fdalloc : FDALLOC) (Filedup : FILEDUP) : SYSDUP_AU.

Section ProofSysDupAU.
  ...
  (*  THE CAPSTONE.                                                       *)
  (* =================================================================== *)
  Lemma wp_sys_dup_au (γl γf : gname)
      (m : regfile) (av : nat) (n : nat) (eb : bool) (p : mword 64)
      (v : mword 64) (pid : mword 32) (U : ustate) (b : bool) (lks : gset string)
      (fd0 : nat) (fv : mword 64) (rb wb : bool) (t : fdtype)
      (sts : list fdstate)
    : wp_sys_dup_au_body γl γf m av n eb p v pid U b lks fd0 fv rb wb t sts.
  Proof.
    cbv beta delta [wp_sys_dup_au_body].
    intros pcE ret_tgt Harg Hn Hav Hftno Hsome0 Hrow0.
    ...
    iDestruct (sda_sp_bounds _ _ _ _ Hkpos with "Hcg") as %Hspb.
    ...
    { (* ===== THE BAD-FD ARM IS REFUTED BY PREMISE (delta 4) =====
         [arg_fd v _] is [Some (fd0, fv)] by the frame's own premise and
         [None] by argfd's failure disjunct; the walk never reaches +0x3c. *)
      iDestruct "Hfail" as "([%Hr %Hnone] & _ & Hs5)".
      rewrite Hsome0 in Hnone. discriminate. }
    (* ===================== argfd found the descriptor ===================== *)
    iDestruct "Hsucc" as (fda fva) "([%Hrv0 %Hsome] & _ & Hs5)".
    (* argfd's report and the premise name the SAME descriptor and word *)
    rewrite Hsome0 in Hsome. injection Hsome as Hfde Hfve.
    subst fda. subst fva.
    iEval (rewrite HM6a2) in "Hs5".
    destruct (arg_fd_lookup v (pv_ofile (us_V U)) fd0 fv Hsome0) as (Hfd0lt & Hlk0 & Hfvnz & _).
    ...
    (* ===== THE SOURCE ROW'S AGREEMENT (delta 3) =====
       the loan reports an ABSTRACT [stf]; the premise says the caller's own
       row [fd0] reads [FdOpen rb wb t], and the authority the loan took out
       is that row's, so the two are the same state.  The fragment goes
    ...
    { (* ============ the table was full: return -1, undo nothing ============ *)
      ...
      (* the reference goes straight back where it came from.
         NOT [set_solver]: it runs naive_solver over the WHOLE context --
         see claude-notes/optimization.md. *)
      iDestruct (proc_ofiles_repay γf (pv_fdg (us_V U)) p (pv_ofile (us_V U)) ∅ fd0 k q
                   (FdOpen rb wb t)
      ...
      iApply (sda_tail (CID0 := CID23) m F2 av (mword_of_int (-1) : mword 64)
      ...
      (* THE -1 ARM: the table was full, so everything comes back exactly as
         handed in -- the bundle at the caller's OWN [sts]. *)
      rewrite /sys_dup_au_post. iLeft. iSplitR; [| iFrame "Hpriv Hfrag"].
      iPureIntro. split; [exact HFa0 | exact Hnone2]. }
    ...
    (* ===== THE SETTLE SITE, AT THE EXPLICIT BUNDLE (deltas 1 and 2) =====
       THE DESTINATION IS THE ONE THAT CHANGES STATE.  fdalloc handed out its
       authority at [FdClosed]; it has to arrive at the source file's state,
       and that move is what this syscall spends the fragment bundle on.
       [fd_frags_acc] where the landed walk takes [fd_frags_any_acc], so the
       row the move writes SURVIVES into the arm; and [fd_st_agree] against
       fdalloc's own authority is what tells us the row it overwrites was
       [FdClosed] -- the fact the arm exposes. *)
    iDestruct (fd_frags_len with "Hfrag") as %Hlensts.
    assert (Hstq1 : is_Some (sts !! fd1))
    ...
    iDestruct (proc_ofiles_repay γf (pv_fdg (us_V U)) p (pv_ofile (upd_ofile (us_V U) fd1 (fnode k)))
                 ∅ fd0 k (q/2)%Qp (FdOpen rb wb t) ltac:(apply not_elem_of_empty) Hlk0' Hklt Hty
    ...
    iApply (sda_tail (CID0 := CID26) m G6 av (mword_of_int (Z.of_nat fd1) : mword 64)
              sp0 ra0 s00 (m !!! Regidx Rs1) (m !!! Regidx Rs2) fv w6 p b
    ...
    (* THE SUCCESS ARM: the least free descriptor names the SAME file, and
       the bundle comes back with that row at the source's own state. *)
    rewrite /sys_dup_au_post. iRight.
    iExists fd1, l. iSplitR.
    { iPureIntro. split; [exact HFa0|]. split; [exact Hfr1 | exact Hstq]. }
