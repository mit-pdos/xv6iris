...
Section PtHit.
  ...
  Lemma exec_translate_pt (vpn : mword 27) (root : mword 44)
        (p2 p1 p0 : mword 64) (menvcfg0 : mword 64)
    ...
    exists s',
      ...
      = Some (Ok (autocast (T := mword) ((autocast (T := mword) (PPN_of_PTE p0)) : mword 44), PBMT_PMA, tt), s')
      /\ (s' = s \/
  ...
  Proof.
    ...
    - eexists. split.
      + rewrite (exec_bind_Some _ _ _ _ _
                   (exec_lookup_TLB_miss vpn (mword_of_int 0) tlbvec s Htlb Hnone)).
        cbn match. rewrite <- Htlb.
        apply (exec_translate_TLB_miss_user vpn root p2 p1 p0 acc p mxr do_sum
                 Hv2 Hn2 Hv1 Hn1 Hv0 Hl0 Hchk Hnap
                 (mword_of_int 0) menvcfg0 s Hmisa Hupd Hrd2 Hrd1 Hrd0 Hmenv HPBMTE).
      + right. rewrite Htlb. reflexivity.
    - eexists. split.
      + rewrite (exec_bind_Some _ _ _ _ _
                   (exec_lookup_TLB_nomatch vpn (mword_of_int 0) ent tlbvec s Htlb Hent Hnm)).
        cbn match. rewrite <- Htlb.
        apply (exec_translate_TLB_miss_user vpn root p2 p1 p0 acc p mxr do_sum
                 Hv2 Hn2 Hv1 Hn1 Hv0 Hl0 Hchk Hnap
                 (mword_of_int 0) menvcfg0 s Hmisa Hupd Hrd2 Hrd1 Hrd0 Hmenv HPBMTE).
    ...
    - eexists. split.
      + rewrite (exec_bind_Some _ _ _ _ _
                   (exec_lookup_TLB_hit_ent vpn (mword_of_int 0) tlbvec _ s Htlb Hhit
                      (uwe_match_self vpn q2 q1 q0))).
        cbn match.
        rewrite (exec_translate_TLB_hit_pt vpn q2 q1 q0 (mword_of_int 0) _ s Hchk' Hupd' Hpb').
