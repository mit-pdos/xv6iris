...
(*  [auipc a0,0x4 / addi a0,a0,1368] at +0xf6 resolves to 0x80007400.     *)
...
Definition ba_msg_addr : Z := 0x80007400.
...
Section BallocOut.
  ...
  Proof.
    ...
    iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.balloc + 0xfa)) Ra0 Ra0
              (mword_of_int 1356 : mword 12) Q8 (K - 10)%nat b
              ltac:(nz) ltac:(rdok) with "Hcg Hpc Hifa").
    iIntros (CID9 Hq9) "Hcg Hpc".
    set (Q9 := <[Regidx Ra0 := regval_into_reg
                  (add_vec (rget Q8 Ra0)
                     (sign_extend' 64 (mword_of_int 1356 : mword 12)))]> Q8).
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.balloc + 0xfe)) Rra
              (mword_of_int 2086470 : mword 21) Q9 (K - 10)%nat b
    ...
    assert (Htgtpk : add_vec (mword_of_int (KernelSyms.balloc + 0xfe) : mword 64)
                       (sign_extend' 64 (mword_of_int 2086470 : mword 21))
    ...
    iDestruct (cpu_own_transport CID11 CID13 0 eb (proc_addr j) C b lks
                 ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
...
Section BallocExhaust.
  ...
  Proof.
    ...
    iDestruct (cpu_own_transport CID0 CID2 0 eb (proc_addr j) C b lks
                 ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
    ...
    iEval (rewrite Hjt) in "Hpc".
    iDestruct (cpu_own_transport CID3 CID6 0 eb (proc_addr j) C b lks
                 ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
...
Section BallocRestore.
  ...
  Proof.
    ...
    iDestruct (cpu_own_transport CID0 CID7 0 eb (proc_addr j) C b lks
                 ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
...
Section BallocBzero.
  ...
  Proof.
    ...
    iIntros "Hcg Hcnt Hextc Hextm #Htext Hpc #Hpanic #Hbio #Hlctx #Hprocs Hframe Hppid Hsbsz Hsbbm #Hdevi #Hdgeom #Hdlock Hsl Hop
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.balloc + 0x60)) Rra
              (mword_of_int 2088540 : mword 21) Z6 (K - 10)%nat b
              ltac:(nz) ltac:(rdok) ltac:(vm_compute; reflexivity)
              with "Hcg Hpc Hi60").
    iIntros (CID9 Hq9) "Hcg Hpc".
    ...
    assert (Htgtms : add_vec (mword_of_int (KernelSyms.balloc + 0x60) : mword 64)
                       (sign_extend' 64 (mword_of_int 2088540 : mword 21))
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.balloc + 0x66)) Rra
              (mword_of_int 4002 : mword 21) Z8 (K - 10)%nat b
              ltac:(nz) ltac:(rdok) ltac:(vm_compute; reflexivity)
              with "Hcg Hpc Hi66").
    iIntros (CID12 Hq12) "Hcg Hpc".
    ...
    assert (Htgtlw2 : add_vec (mword_of_int (KernelSyms.balloc + 0x66) : mword 64)
                        (sign_extend' 64 (mword_of_int 4002 : mword 21))
    ...
    iDestruct (cpu_own_transport CID13 CID15 0 eb (proc_addr j) C b lks
                 ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
...
Section BallocAlloc.
  ...
  Proof.
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.balloc + 0x42)) Rra
              (mword_of_int 4038 : mword 21) A2 (K - 10)%nat b
              ltac:(nz) ltac:(rdok) ltac:(vm_compute; reflexivity)
              with "Hcg Hpc Hi42").
    iIntros (CID5 Hq5) "Hcg Hpc".
    ...
    assert (Htgtlw : add_vec (mword_of_int (KernelSyms.balloc + 0x42) : mword 64)
                       (sign_extend' 64 (mword_of_int 4038 : mword 21))
                     = mword_of_int KernelSyms.log_write) by pcw.
    ...
    iDestruct (cpu_own_transport CID6 CID8 0 eb (proc_addr j) C b lks
                 ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
...
Section BallocScan.
  ...
  Proof.
    ...
    induction fuel as [|fuel IH];
      ...
      iIntros "Hcg Hcnt Hextc Hextm #Htext #Hkdata Hpc #Hpanic #Hpenv #Hbio #Hlctx #Hprocs Hframe Hppid Hsbsz Hsbbm #Hdevi #Hdgeom
    ...
    - (* ---- FUEL 0: the scan is at BPB, so [bgeu] is taken ---- *)
      ...
      iDestruct (cpu_own_transport CIDx CID1 0 eb (proc_addr j) C b lks
                   ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
      ...
      iApply (ba_exhaust (CID0 := CID1)  γs j γfs γd bn γ γpr γu cov logstart
                ...
                pidv dq dqb dqs m M K eb C b HK Hpk Hsize Hsp Hthr Hs2 Hs5 Hs6 Hs8 Hkk
      ...
      + (* +0xb6 FALL-THROUGH: bi < sb.size, so this bit is in range *)
        ...
        * (* the bit is SET: fall through and keep scanning *)
          ...
          -- (* bi + 1 = BPB: fall through to the +0xe6 jump *)
             ...
             iDestruct (bitmap_res_close γfs bmapstart cov logstart size used Hok
                          with "Hfsbm Hpool") as "Hbmr".
             iDestruct (cpu_own_transport CIDx CID15 0 eb (proc_addr j) C b lks
                          ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
          ...
          -- (* bi + 1 < BPB: branch back to +0xb6, one fuel unit down *)
             ...
             iDestruct (cpu_own_transport CIDx CID14 0 eb (proc_addr j) C b lks
                          ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
             ...
             iApply (IH CID14 (bi + 1) SA
                       ...
                       with "Hcg Hcnt Hextc Hextm Htext Hkdata Hpc Hpanic Hpenv Hbio Hlctx Hprocs Hframe Hppid Hsbsz Hsbbm Hdevi Hdgeom
                             Hdlock Hsl Hop Hfsbm Hpool Hlk [Hcont]").
        ...
        * (* the bit is CLEAR: take the branch to +0x38 and allocate *)
          ...
          iDestruct (cpu_own_transport CIDx CID11 0 eb (proc_addr j) C b lks
                       ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
...
Section BallocMain.
  ...
  (* [ba_main] IS THE ONE CORE both top-level lemmas below build on: the
     ...
     and [wp_balloc_sconf] is the set-forgetting instance at [cr := false] lks.
  ...
  Lemma ba_main
    ...
    : let pcE : mword 64 := mword_of_int KernelSyms.balloc in
      ...
      WP (Loop : expr riscv_lang).
  Proof.
    intros pcE pj ret_tgt HK Hgeom Hpk Hsize Hbm0 Hbmcov Hbmlog Hcred Hj Hgl Ha0.
    pose proof HK as HK'. unfold K_balloc in HK'.
    pose proof Hsize as Hsz'. rewrite BPB_value in Hsz'.
    pose proof Hgeom as [Hcovok Hlogsub].
    ...
    iIntros "Hcg Hcnt Hextc Hextm #Htext Hpc #Hpanic #Hkdata #Hpenv #Hbio #Hlctx Hppid
    ...
    assert (Hszadr1 : add_vec (rget R3 Ra5)
                        (sign_extend' 64 (mword_of_int 2796 : mword 12))
                      = sb_size).
    ...
    iApply (wp_lw_s_sconf (mword_of_int (KernelSyms.balloc + 0xe)) Ra5 Ra5
              (mword_of_int 2796 : mword 12) R3 (K - 10)%nat
              (mword_of_int size : mword 32) b
    ...
    iDestruct (cpu_own_transport CID CIDb28 0 eb (proc_addr j) C b lks
                 ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
    ...
    iDestruct (cpu_own_transport CIDb29 CIDb33 0 eb (proc_addr j) C b lks
                 ltac:(wp_next_chain) with "Hcnt") as "Hcnt".
  ...
  Proof.
    ...
    intros pcE pj ret_tgt HK Hgeom Hpk Hsize Hbm0 Hbmcov Hbmlog Hj Hgl Ha0.
    iIntros "Hcg Hcnt Hextc Hextm #Htext Hpc #Hpanic #Hkdata #Hpenv #Hbio #Hlctx Hppid
