...
(* ===================================================================== *)
  ...
  Proof.
    ...
    rewrite (exec_bind_Some _ _ _ _ _
      (exec_vmem_write_1_gpr_S_walk_dev_pt rs1 offset data_byte d' region satp0 s s' Hcp HSXL Hsatp Hmode Hmprv Hmxr Hpmm Halign Htr Hcp' Hmprv' HA Hord Hrange HW Hmatch Hwrite Hc Hsig Hh Hdev Hwr)).
    cbn match.
    apply exec_returnM.
  Qed.
...
Proof.
  ...
  iApply (wp_instr_s_config_tlbinv_pt root_ppn Φ pc is_rvc
 ...
 HSIE HMPRV HSXL Hmm HPBMTE Hmenvval0
  ...
  assert (Lsatp_pc : register_lookup satp s_pc.(sregs) = satp1) by (unfold s_pc; tmig; exact Lsatp).
  ...
  iMod (tlb_inv_pt_translateAddr_store_dev root_ppn a8 s_pc
          Hdevvpn Hcanon Hident_pt
  ...
  { rewrite (exec_execute_STORE_1_gpr_S_walk_dev_pt rs2 rs1 imm region_st satp1 s_pc s_tr d'
               Lpriv_pc LSXL_pc Lsatp_pc Hmode
               ...
               ltac:(rewrite Lpmpaddr_tr !Lva Hpa; apply uart_pmp_match1; [exact Hoff | exact Hcov])
...
Proof.
 ...
 HSIE HMPRV HSXL Hmm HPBMTE Hmenvval0
  ...
  assert (LSXL_pc : _get_Mstatus_SXL (register_lookup mstatus s_pc.(sregs)) = 'b"10") by (rewrite Lms_pc; exact HSXL).
  assert (Lpma_pc' : pma_allows_all (register_lookup pma_regions s_pc.(sregs))) by (rewrite Lpma_pc; exact Hpma_all).
  iMod (tlb_inv_pt_translateAddr_load_dev root_ppn a8 s_pc
          Hdevvpn Hcanon Hident_pt
          Lmisa_pc' Lmenv_pc' Lhtif_pc Lpriv_pc LSXL_pc
  ...
  { subst s_x. unfold ldval.
    apply (exec_execute_LOAD_1_gpr_S_walk_dev rs1 rd imm is_unsigned b d' region_ld satp1 s_pc s_tr
             Hrd Lpriv_pc LSXL_pc Lsatp_pc Hmode
             ltac:(rewrite Lms_pc; exact HMPRV) ltac:(rewrite Lms_pc; exact HMXR)
             ltac:(rewrite Lmenv_pc; exact Hpmm)
             ltac:(unfold is_aligned_vaddr; rewrite Z.rem_1_r; reflexivity)
             ltac:(cbn [bits_of_virtaddr]; rewrite !Lva Hpa; change (0 * 1)%Z with 0%Z; rewrite avi0; exact Htr_uart)
             ...
             ltac:(rewrite Lpmpc_tr; exact HA0) ltac:(rewrite Lpmpaddr_tr; exact Hord0)
             ltac:(rewrite Lpmpaddr_tr !Lva Hpa; apply uart_pmp_match1; [exact Hoff | exact Hcov])
             ltac:(rewrite Lpmpc_tr; exact HR)
