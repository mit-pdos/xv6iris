...
Section ProofNamexMain.
  ...
  Definition nx_mid_body
    ...
    (∀ (a : nat) (Ms : regfile),
     ...
     wp_next (CID0 := CIDs) true (proc_addr j) (fun CIDb : CpuId =>
       ∀ Ms' : regfile,
         ⌜(a < plen)%nat⌝ -∗
         ⌜pfun a <> NUL⌝ -∗
         ⌜Ms' !!! Regidx Rs1 = pa_add pv a⌝ -∗
  ...
  Proof.
            ...
            -- iIntros (CIDe Hse e Me) "%E1 %E2 %E3 %E4 %E5 %E6
               ...
               ++ (* ================ LONG: 14 <= len ================= *)
                  ...
                  assert (Htgmm : add_vec
                            ...
                            (sign_extend' 64 (mword_of_int 2085928 : mword 21))
                          = mword_of_int KernelSyms.memmove) by pcw.
                  iApply (wp_jal_s_sconf (mword_of_int (NX + 0x9e)) Rra
                            (mword_of_int 2085928 : mword 21) T3
                            (K - 12)%nat b
                            ltac:(nz) ltac:(rdok)
                            ltac:(vm_compute; reflexivity)
                            with "Hcg Hpc Hj9e").
    ...
    + (* ---------------- THE RELATIVE ARM: idup(myproc()->cwd) ---------- *)
      ...
      assert (Htgtmp : add_vec (mword_of_int (NX + 0x2e) : mword 64)
                         (sign_extend' 64 (mword_of_int 2089078 : mword 21))
                       = mword_of_int KernelSyms.myproc) by pcw.
      iApply (wp_jal_s_sconf (mword_of_int (NX + 0x2e)) Rra
                (mword_of_int 2089078 : mword 21) R7 (K - 12)%nat b
                ltac:(nz) ltac:(rdok) ltac:(vm_compute; reflexivity)
                with "Hcg Hpc Hi02e").
      iIntros (CID21 Hq21) "Hcg Hpc".
