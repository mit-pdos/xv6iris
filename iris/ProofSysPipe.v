...
(* the same at the record *)
Lemma sp_us_upt_ofile_comm (U : ustate) (P' : uptd) (M' : gmap Z (bv 8))
    (fd0 fd1 : nat) (x0 x1 : mword 64) :
  us_ofile (us_ofile (upd_usM (us_upt U P') M') fd0 x0) fd1 x1
  = upd_usM (us_upt (us_ofile (us_ofile U fd0 x0) fd1 x1) P') M'.
...
(* the same at the record: [proc_priv]'s argument is a [ustate], and the
   image the descriptor move was rebuilt at is the one that survives. *)
Lemma sp_us_restore_upt (U : ustate) (P' : uptd) (M' : gmap Z (bv 8))
    (fd0 fd1 : nat) (x0 x1 : mword 64) :
  fd0 <> fd1 ->
  pv_ofile (us_V U) !! fd0 = Some (zero_reg : mword 64) ->
  pv_ofile (us_V U) !! fd1 = Some (zero_reg : mword 64) ->
  us_ofile (us_ofile (upd_usM (us_upt (us_ofile (us_ofile U fd0 x0) fd1 x1) P') M')
                     fd0 (zero_reg : mword 64)) fd1 (zero_reg : mword 64)
  = upd_usM (us_upt U P') M'.
Proof.
  intros Hne H0 H1.
  rewrite /us_ofile /us_upt /upd_usV /upd_usM /=
          (sp_restore_upt (us_V U) P' fd0 fd1 x0 x1 Hne H0 H1). reflexivity.
Qed.

(* the same at the record -- [proc_priv]'s argument is a [ustate] now. *)
Lemma sp_us_ofile_restore (U : ustate) (fd : nat) (x : mword 64) :
  pv_ofile (us_V U) !! fd = Some (zero_reg : mword 64) ->
  us_ofile (us_ofile U fd x) fd (zero_reg : mword 64) = U.
Proof.
  intro Hlk. rewrite /us_ofile /upd_usV /=.
  rewrite (sp_ofile_restore (us_V U) fd x Hlk). by destruct U.
Qed.
...
Section ProofSysPipe.
  ...
  (* THE PID QUARTER, LENT FOR THE DURATION OF ONE [sp_close2].
     ...
     call, and take it back.  Nothing about the block bitmap rides with it
     any more: it is a persistent invariant inside [FsReady.fs_ready], so
     the bundle goes out and comes back at the same index. *)
  (* THE LOAN, AT THE BLOCK.  It USED TO splice a quarter of [p->pid] into
     the fs bundle, because [fileclose_fs_env] carried that cell; the bundle
     does not carry it any more -- fileclose asks for [proc_priv_bare] at the
  ...
  Lemma sp_close2 `{CID0 : CpuId}  (γfl γf : gname)
    ...
    (* THE PROCESS BLOCK, THREADED THROUGH BOTH CLOSES.  fileclose takes it at
  ...
  Proof using .
    ...
    iDestruct (cpu_own_transport CID7 CID9 0%nat eb p b ltac:(wp_next_chain)
                 with "Hcpu") as "Hcpu".
    ...
    iDestruct (cpu_own_transport CID10 CID11 0%nat eb p b ltac:(wp_next_chain)
                 with "Hcpu") as "Hcpu".
  ...
  Proof using .
    ...
    rewrite /kalloc_env.
    iDestruct "Henv" as (γk) "(#Hkmem & #Hkav)".
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.sys_pipe + 0xa)) Rra
              (mword_of_int 2081698 : mword 21) R2 (av - 8)%nat b
              ltac:(vm_compute; discriminate) ltac:(rdok)
    ...
    assert (Hjmp : add_vec (mword_of_int (KernelSyms.sys_pipe + 0xa) : mword 64)
                     (sign_extend' 64 (mword_of_int 2081698 : mword 21))
    ...
    iDestruct (cpu_own_transport CID CID24 0%nat eb p b ltac:(wp_next_chain)
                 with "Hcpu") as "Hcpu".
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.sys_pipe + 0x16)) Rra
              (mword_of_int 2085708 : mword 21) P3 (av - 8)%nat b
              ltac:(vm_compute; discriminate) ltac:(rdok)
              ltac:(vm_compute; reflexivity) with "Hcg Hpc []").
    ...
    assert (Hjaa : add_vec (mword_of_int (KernelSyms.sys_pipe + 0x16) : mword 64)
                     (sign_extend' 64 (mword_of_int 2085708 : mword 21))
    ...
    iDestruct (cpu_own_transport CID25 CID29 0%nat eb p b ltac:(wp_next_chain)
                 with "Hcpu") as "Hcpu".
    iApply (Argaddr.wp_argaddr_sconf P4 (av - 8)%nat 0%nat eb p 0%nat
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.sys_pipe + 0x22)) Rra
              (mword_of_int 2092952 : mword 21) Q2 (av - 8)%nat b
              ltac:(vm_compute; discriminate) ltac:(rdok)
              ltac:(vm_compute; reflexivity) with "Hcg Hpc []").
    ...
    assert (Hjpa : add_vec (mword_of_int (KernelSyms.sys_pipe + 0x22) : mword 64)
                     (sign_extend' 64 (mword_of_int 2092952 : mword 21))
    ...
    iDestruct (cpu_own_transport CID30 CID33 0%nat eb p b ltac:(wp_next_chain)
                 with "Hcpu") as "Hcpu".
    ...
    { (* ============ pipealloc failed: straight to the epilogue ========= *)
      ...
      iDestruct (cpu_own_transport CID34 CID36 0%nat eb p b ltac:(wp_next_chain)
                   with "Hcpu") as "Hcpu".
    ...
    assert (Hcc4f : add_vec (mword_of_int (KernelSyms.sys_pipe + 0xcc) : mword 64)
                      (sign_extend' 64 (mword_of_int 2091962 : mword 21))
                    = mword_of_int KernelSyms.fileclose)
      by (apply bv_eq; vm_compute; reflexivity).
    assert (Hcc4g : add_vec (mword_of_int (KernelSyms.sys_pipe + 0xd4) : mword 64)
                      (sign_extend' 64 (mword_of_int 2091954 : mword 21))
                    = mword_of_int KernelSyms.fileclose)
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.sys_pipe + 0x34)) Rra
              (mword_of_int 2094752 : mword 21) X0 (av - 8)%nat b
              ltac:(vm_compute; discriminate) ltac:(rdok)
              ltac:(vm_compute; reflexivity) with "Hcg Hpc []").
    ...
    assert (Hjfd1 : add_vec (mword_of_int (KernelSyms.sys_pipe + 0x34) : mword 64)
                      (sign_extend' 64 (mword_of_int 2094752 : mword 21))
    ...
    iDestruct (cpu_own_transport CID34 CID40 0%nat eb p b ltac:(wp_next_chain)
                 with "Hcpu") as "Hcpu".
    ...
    { (* ===== the ftable had no free descriptor: close both, return -1 ===== *)
      ...
      iDestruct (cpu_own_transport CID41 CID43 0%nat eb p b ltac:(wp_next_chain)
                   with "Hcpu") as "Hcpu".
      ...
      iDestruct (cpu_claim_ext_transport CID34 CID43 eb p
                   ltac:(rewrite Hb; wp_next_chain) with "Hextm") as "Hextm".
      (* the process BLOCK, lent for the span of the two closes -- see
         [sp_lend_pid] *)
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.sys_pipe + 0x44)) Rra
              (mword_of_int 2094736 : mword 21) Z0 (av - 8)%nat b
              ltac:(vm_compute; discriminate) ltac:(rdok)
              ltac:(vm_compute; reflexivity) with "Hcg Hpc []").
    ...
    assert (Hjfd2 : add_vec (mword_of_int (KernelSyms.sys_pipe + 0x44) : mword 64)
                      (sign_extend' 64 (mword_of_int 2094736 : mword 21))
                    = mword_of_int KernelSyms.fdalloc)
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    { (* ===== no second descriptor: undo the first, close both ===== *)
      ...
      iDestruct "Href" as (k0' q0' st0') "((%Hfv0 & %Hk0'lt & %Hty0) & Href0 & Hst0)".
    ...
    assert (Hc9cg : add_vec (mword_of_int (KernelSyms.sys_pipe + 0xac) : mword 64)
                      (sign_extend' 64 (mword_of_int 2091994 : mword 21))
                    = mword_of_int KernelSyms.fileclose)
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.sys_pipe + 0x5e)) Rra
              (mword_of_int 2080648 : mword 21) A5 (av - 8)%nat b
              ltac:(vm_compute; discriminate) ltac:(rdok)
              ltac:(vm_compute; reflexivity) with "Hcg Hpc []").
    ...
    assert (Hjco1 : add_vec (mword_of_int (KernelSyms.sys_pipe + 0x5e) : mword 64)
                      (sign_extend' 64 (mword_of_int 2080648 : mword 21))
    ...
    { iSplit; [iExact "Hepi"|]. rewrite /T7C.
      ...
      iDestruct "Hslot" as "[Hcell [[%Hz _] | Href]]";
        [by exfalso; apply (fnode_ne_zero k0 Hk0lt)|].
      iDestruct "Href" as (k0' q0' st0') "((%Hfv0 & %Hk0'lt & %Hty0) & Hrf0 & Hst0)".
      ...
      iDestruct "Href1'" as (k1' q1' st1') "((%Hfv1 & %Hk1'lt & %Hty1) & Hrf1 & Hst1)".
      ...
      iDestruct (cpu_own_transport CIDT CID65 0%nat eb p b ltac:(wp_next_chain)
                   with "Hcpu") as "Hcpu".
      ...
      iDestruct (cpu_own_transport CID66 CID67 0%nat eb p b ltac:(wp_next_chain)
                   with "Hcpu") as "Hcpu".
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.sys_pipe + 0x76)) Rra
              (mword_of_int 2080624 : mword 21) C6 (av - 8)%nat b
              ltac:(vm_compute; discriminate) ltac:(rdok)
              ltac:(vm_compute; reflexivity) with "Hcg Hpc []").
    ...
    assert (Hjco2 : add_vec (mword_of_int (KernelSyms.sys_pipe + 0x76) : mword 64)
                      (sign_extend' 64 (mword_of_int 2080624 : mword 21))
                    = mword_of_int KernelSyms.copyout)
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    - (* ============ SUCCESS: return 0 ============ *)
      ...
      iDestruct (cpu_own_transport CID76 CID78 0%nat eb p b ltac:(wp_next_chain)
                   with "Hcpu") as "Hcpu".
    ...
    - (* ============ copyout(&fd1) failed: the shared tail ============ *)
      ...
      iDestruct "HK" as "[_ Ht7c]".
      iDestruct (cpu_own_transport CID76 CID79 0%nat eb p b ltac:(wp_next_chain)
                   with "Hcpu") as "Hcpu".
