(* ProofSysPipe.v -- the whole-function WP for sys_pipe(), over the
   ...
   * THE FRAME IS THE STATE.  Like pipealloc, every branch after a call
     ...
     [InstrBytes.ctx_word_pointsto_split4] and is only rejoined for the epilogue.
...
Section ProofSysPipe.
  ...
  Lemma sp_lend_pid (γf : gname) (fn : fclose_names) (n : nat)
    ...
    proc_priv γf p pid V -∗ fileclose_fs_env_nopid fn n eb p -∗
    proc_priv_bare p pid V ∗ fileclose_fs_env fn n eb p ∗
    (proc_priv_bare p pid V -∗ fileclose_fs_env fn n eb p -∗
       proc_priv γf p pid V ∗ fileclose_fs_env_nopid fn n eb p).
  Proof.
    iIntros "Hpriv Hfe".
  ...
  Proof.
    ...
    pose (EPI := (wp_next true p (fun (CIDe : CpuId) =>
      ∀ (mj : regfile) (P' : uptd) (res : mword 64),
        ...
        (∃ w5 w6 w7 : mword 64,
           ...
           ctx_word_pointsto (KTR := KT1) cur_ctx (pa_stk sp0 7) (DfracOwn 1) w7) -∗
        (∃ lo hi : mword 32,
           TsoCtx.ctx_word4_pointsto (KTR := KT1) cur_ctx (pa_stk sp0 8) (DfracOwn 1) lo ∗
           TsoCtx.ctx_word4_pointsto (KTR := KT1) cur_ctx (pa_add (pa_stk sp0 8) 4) (DfracOwn 1) hi) -∗
    ...
    (* A6.61: [↦₄] IS the context tower, so its byte run already IS
       copyout's context-indexed window -- the crossing was an identity. *)
    ...
    pose (T7C := (wp_next true p (fun (CIDt : CpuId) =>
      ∀ (Mt : regfile) (P' : uptd),
        ...
        ctx_word_pointsto (KTR := KT1) cur_ctx (pa_stk sp0 6) (DfracOwn 1) (fnode k0) -∗
        ctx_word_pointsto (KTR := KT1) cur_ctx (pa_stk sp0 7) (DfracOwn 1) (fnode k1) -∗
        TsoCtx.ctx_word4_pointsto (KTR := KT1) cur_ctx (pa_stk sp0 8) (DfracOwn 1)
          (trunc32 (mword_of_int (Z.of_nat fd1) : mword 64)) -∗
        TsoCtx.ctx_word4_pointsto (KTR := KT1) cur_ctx (pa_add (pa_stk sp0 8) 4) (DfracOwn 1)
