(* ProofSysUnlinkW5D.v -- the unlink AU walk's BLOCK W5-DIR: the same
   span at the +0x8a seam's [isdir = true] -- the zeroing writei, the
   PARENT's own [dp->nlink--] and its iupdate, then ip->nlink--,
   iupdate(ip) and the success epilogue.

     W5-DIR  +0x8a .. +0xd8

   [ProofSysUnlinkPure.su_w5_dir]'s copy-adapt, at [SysUnlinkDefs]'s
   contract.  The two fires are W5-FILE's at the DIRECTORY instantiation,
   and that instantiation is the whole of the difference:

     INSTANT 1 is FUSED with [dp->nlink--].  W5-DIR retags [dp] ONCE, after
       [iupdate(dp)], covering the entry-delete AND the count together --
       legal because dp's lock is held across both writes -- so
       [uf_uent_fire] covers both too, at [dec = 1].  That [dec] is
       [unl_dec] of the TARGET's row, read off the target's own fragment
       ([su_au_dir_dec]), which is exactly why ONE fire lemma serves both W5
       arms rather than two.  [unl_pre]'s dots-only conjunct is the
       isdirempty harvest [dir_dots_only] this arm's seam carries, read
       through [su_au_dir_dots].
     INSTANT 2 is W5-FILE's verbatim: [uf_utgt_fire] after
       ...
       [SysUnlinkDefs.delta_unlink_last_dir] is the pure statement of
       it.
   ...
   [dp->nlink--] ([DirView.dir_dots_ix] + [FsStateEra.ent_toks_era_borrow_at]
   name it, [FsStateLink.link_toks_reps_S] / [link_reps_1] split it) and
   hands back [2 <= dir_nrec (di_size ip)].  The AU fires sit BESIDE that
   whole ledger; nothing about the tokens crosses the abstraction. *)
...
Module SysUnlinkW5D (Memset : MEMSET) (Writei : WRITEI) (Iupdate : IUPDATE)
...
Section ProofSysUnlinkW5D.
  ...
  (* ================================================================== *)
  Lemma su_w5_dir_au `{GEN : GenId} `{CID0 : CpuId} `{XI : CurCtx}
      (gf : gname)
 ...
 (ientry kd) dinum
    ...
    { apply (inode_local_of_ok_rec (bv_unsigned dinum) fsc_cov fsc_logst _ bm' data').
      ...
      - exact HduqF2.
      - exact HddixF2. }
    (* ===== INSTANT 1: THE PARENT'S ROW, ENTRY AND COUNT TOGETHER =====
       W5-DIR retags [dp] ONCE, after [iupdate(dp)], covering the
       entry-delete AND the [dp->nlink--] -- legal because dp's lock is held
       across both writes -- so the fire covers both too, at [dec = 1].
       That [dec] is [unl_dec] of the TARGET's row, read off the target's own
    ...
    (* [lia] does NOT close this: the two spellings of the stored halfword
       ...
       syntactically.  The landed walk hits the same wall at its own
       [HdWnd] site and uses the same one-rewrite route. *)
    { rewrite su_setnl_nlink -Hnl'v HdecrW Z.add_simpl_r. reflexivity. }
 ...
 ks qsi si gyi loyi tlyi
    ...
    (* ===== ret 0, the DIR arm.  [unl_pre]'s dots-only conjunct is the
       isdirempty harvest, and [unl_dec] of the target's row is 1, so the
       parent's own count is down by one in the same instant. ===== *)
