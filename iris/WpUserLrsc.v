...
Require Import UptInv MemData4 UmodeLrsc.
...
Section WpUserLrsc.
  ...
  (* ------------------------------------------------------------------ *)
  (* USTEP case: SC.W fault (prohibited).  As LR.W, but StoreConditional  *)
  (* -- no reservation exists (match_reservation = false), so SC takes    *)
  (* the phys_access_check branch, which faults on reservability=RsrvNone  *)
  (* with E_SAMO_Access_Fault, delivered by the same Trap arm.             *)
  (* ------------------------------------------------------------------ *)
  Lemma ustep_sc_fault
      (va : mword 64) (vpn : mword 27) (ie : uwalk_info) (w : mword 32)
      (vpnD : mword 27) (ieD : uwalk_info)
  ...
  Proof.
    ...
    assert (Htr_x : exec (translateAddr (Virtaddr (add_vec_int (bits_of_virtaddr (Virtaddr eaF)) (0 * 4))) (StoreConditional Data)) s_x
                    = Some (Ok (Physaddr paD, PBMT_PMA, init_ext_ptw), s_x)).
    { rewrite Heaeq.
      exact (exec_translateAddr_storecon_hit_u (upt_entry vpnD ieD) vpnD eaF satp0 tlbvec s_x
               HchkD' HupdD' HpbmtD' (upt_entry_match vpnD ieD)
               LprivX HSXLX HMPRVX LsatpX Hsatpmode Hasid LtlbX HvecD HcanonD Hvpn_defD). }
    assert (Htea_x : exec (transform_effective_address
              (Virtaddr (add_vec (if Z.eqb (uint rs1) 0 then zero_reg
    ...
    { unfold s_x; lk. rewrite Lmedl. exact Hdel_samoaf. }
    pose proof (exec_exception_handler_ne_M s_x sc_cause User va
