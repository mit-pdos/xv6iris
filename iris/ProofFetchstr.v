(* ProofFetchstr.v -- the whole-function WP for xv6's fetchstr().
   ...
   Thirty-three instructions, a 48-byte (6-slot) frame with five slots used,
   two arms joining at the epilogue (+0x30).  The contract is SpecFetchstr.v.

   *** WHAT THE [psz] BUMP COST THIS PROOF, xv6 `4f2fc8b`. ***  gcc inserted
   [ld a1,72(a0)] at +0x1e, between the argument [c.mv]s and the
   [ld a0,80(a0)] that follows, so the whole tail from +0x1e on sits two
   bytes higher and the three argument moves shifted a register each
   (a3/a2/a1 -> a4/a3/a2).  That much is bookkeeping.  What is NOT is that
   copyinstr changed ALTITUDE with the same commit -- its [pa0 == 0] arm now
   calls vmfault (SpecCopyinstr.v) -- and fetchstr had to follow it up:

   - the kalloc tier is threaded through ([γa], [kalloc_env γa None]);
   - [fetchstr_stack] went 26 -> 56, because copyinstr's own budget went
     20 -> 50 and fetchstr's frame is 6 slots;
   - the borrow no longer closes at the descriptor it opened with.

   [szv := pv_sz V] and its [uint szv <= 2^38] premise are paid out of
   [ProcInv.proc_priv_sz_bound], and the [p->sz] cell comes out of the SAME
   [proc_priv_copy] borrow that already served [p->pagetable] -- so fetchstr
   still does no range test of its own.
   ...
   and closed on both arms -- at [P'], the descriptor copyinstr hands back,
   NOT at [pv_upt V].  That is the one place where this proof stopped being
   the cheap member of the family: copyinstr faults pages in now, so the
   record-eta step that used to turn [upd_upt V (pv_upt V)] back into [V] is
   gone, and the postcondition carries [upd_upt V P'] under
   [uptd_ext (pv_upt V) P'] exactly as fetchaddr's does.
...
Section ProofFetchstr.
  ...
  Lemma fs_tail `{CID0 : CpuId}
    ...
    ctx_word_pointsto (KTR := KT1) cur_ctx (pa_stk sp0 6) (DfracOwn 1) gap -∗
  ...
  Proof.
    ...
    iApply (wp_cldsp_s_sconf (mword_of_int (KernelSyms.fetchstr + 0x38))
    ...
    iApply (wp_caddi16sp_pop_s_sconf (mword_of_int (KernelSyms.fetchstr + 0x3a))
    ...
    assert (Hpp3c : add_vec_int (mword_of_int (KernelSyms.fetchstr + 0x3a) : mword 64) 2
                    = mword_of_int (KernelSyms.fetchstr + 0x3c)) by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (wp_cret_s_sconf (mword_of_int (KernelSyms.fetchstr + 0x3c))
  ...
  Proof.
    ...
    iIntros "Hcg Hcpu #Htext Hpc Hpriv #Henv Hbuf Hcont".
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.fetchstr + 0x14))
              Rra (mword_of_int 2093224 : mword 21) M5 (av - 6)%nat b
    ...
    assert (Hjmp : add_vec (mword_of_int (KernelSyms.fetchstr + 0x14) : mword 64)
                     (sign_extend' 64 (mword_of_int 2093224 : mword 21)) = mword_of_int KernelSyms.myproc)
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    assert (HA3a2 : A3 !!! Regidx Ra2 = buf)
    ...
    (* ---- +0x1e: c.ld a1,72(a0) -- a1 := p->sz, copyinstr's NEW psz ---- *)
    assert (Hszaddr : add_vec (A3 !!! Regidx Ra0) (sign_extend' 64 (mword_of_int 72 : mword 12))
                      = p_sz p)
    ...
    assert (HAza0 : Az !!! Regidx Ra0 = p)
      by (rewrite /Az upd_ne; [exact HA3a0 | reg_neq]).
    (* ---- +0x20: c.ld a0,80(a0) -- a0 := p->pagetable ---- *)
    assert (Hptaddr : add_vec (Az !!! Regidx Ra0) (sign_extend' 64 (mword_of_int 80 : mword 12))
                      = p_pagetable p)
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.fetchstr + 0x22))
              Rra (mword_of_int 2092598 : mword 21) A4 (av - 6)%nat b
              ltac:(vm_compute; discriminate) ltac:(rdok) ltac:(vm_compute; reflexivity)
              with "Hcg Hpc []").
    ...
    assert (Hjcis : add_vec (mword_of_int (KernelSyms.fetchstr + 0x22) : mword 64)
                      (sign_extend' 64 (mword_of_int 2092598 : mword 21)) = mword_of_int KernelSyms.copyinstr)
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    { rewrite /A5 upd_ne; [| reg_neq]. rewrite /A4 upd_ne; [| reg_neq].
      rewrite /Az upd_ne; [| reg_neq]. exact HA3a2. }
    assert (HA5a4 : A5 !!! Regidx Ra4 = (mword_of_int (Z.of_nat maxn) : mword 64)).
    { rewrite /A5 upd_ne; [| reg_neq]. rewrite /A4 upd_ne; [| reg_neq].
    ...
    - (* ======= copyinstr returned 0: fall through to strlen ======= *)
      ...
      iApply (wp_blt_x0_fall_s_sconf (mword_of_int (KernelSyms.fetchstr + 0x26))
      ...
      iApply (wp_cmv_s_sconf (mword_of_int (KernelSyms.fetchstr + 0x2a))
      ...
      assert (Hpp2c : add_vec_int (mword_of_int (KernelSyms.fetchstr + 0x2a) : mword 64) 2
                      = mword_of_int (KernelSyms.fetchstr + 0x2c)) by (apply bv_eq; vm_compute; reflexivity).
      ...
      iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.fetchstr + 0x2c))
                Rra (mword_of_int 2090426 : mword 21) B1 (av - 6)%nat b
                ltac:(vm_compute; discriminate) ltac:(rdok) ltac:(vm_compute; reflexivity)
                with "Hcg Hpc []").
      ...
      assert (Hjsl : add_vec (mword_of_int (KernelSyms.fetchstr + 0x2c) : mword 64)
                       (sign_extend' 64 (mword_of_int 2090426 : mword 21)) = mword_of_int KernelSyms.strlen)
        by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hjsl) in "Hpc".
      assert (HB2ra : B2 !!! Regidx Rra
                      = add_vec_int (mword_of_int (KernelSyms.fetchstr + 0x2c) : mword 64) 4)
    ...
    - (* ======= copyinstr returned -1: take the branch ======= *)
      ...
      iApply (wp_blt_x0_taken_s_sconf (mword_of_int (KernelSyms.fetchstr + 0x26))
                (mword_of_int 24 : mword 13) Ra0 mr (av - 6)%nat b
      ...
      (* ---- +0x3c: c.li a0,-1 ---- *)
      iApply (wp_cli_s_sconf (mword_of_int (KernelSyms.fetchstr + 0x3e))
      ...
      assert (Hpp40 : add_vec_int (mword_of_int (KernelSyms.fetchstr + 0x3e) : mword 64) 2
                      = mword_of_int (KernelSyms.fetchstr + 0x40)) by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hpp40) in "Hpc".
      (* ---- +0x3e: c.j -0x0c ---- *)
      iApply (wp_cj_s_sconf (mword_of_int (KernelSyms.fetchstr + 0x40))
