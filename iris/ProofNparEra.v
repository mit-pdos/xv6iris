(* ProofNparEra.v -- the whole-function proof of namex AT THE NAMEIPARENT
   ERA-FRAGMENT TRACE CONTRACT ([SpecNparEra.wp_npar_era_body]).

   Worklist: claude-notes/projects/fs-syscall-specs.md, lane A item (iii),
   REMAINING item ("the NAMEIPARENT walk").

   THIS FILE IS A RE-DERIVATION FROM [ProofNamex.v], AND THAT IS THE
   MEASURED CHOICE.  [ProofNamexTr] / [ProofNamexEra] are [ProofNamex] with
   the npar arms and the relative start CUT (1432 deleted lines of 1766
   changed), so patching one of them back up is a 74-hunk merge; the lane
   before this one refused it and was right.  Starting from [ProofNamex]
   instead, the npar arms are already there, in their own bullets, and what
   this file adds is (a) the trace rows -- copied from [ProofNamexTr]'s
   additions and [ProofNamexEra]'s fire, both verbatim -- and (b) three
   nameiparent-specific facts.  Review it as the diff against
   [ProofNamex.v].

   THE DIFF, EXHAUSTIVELY.

   (1) THE FLAG IS PINNED, NOT ELIMINATED.  The contract fixes [a1 <> 0],
       but namex's text tests the register at three places and the landed
       proof case-splits on a [npar : bool] variable there.  Rather than
       re-bullet those splits (and re-indent thousands of lines), the proof
       opens with

           pose (npar := true). assert (Hnpe : npar = true) by reflexivity.
           clearbody npar.

       so [destruct npar] still works and every NAMEI branch closes with
       [discriminate Hnpe] -- three arms, three lines.  The two NAMEIPARENT
       arms ([L_par] at +0x84, the +0x140 iput) are [ProofNamex]'s own,
       instruction for instruction, with a trace row added at the exit.

   (2) THE HOP FAMILY IS THE PARENT PREFIX.  The loop invariant carries
       [FsAbsNpar.ep_hops_from fsc_fs P Pmiss pl (length es0)] --
       [FsAbs.ax_hops_from] at [FsAbsEra.elend] over
       [removelast (path_elems pl)] -- where the namei walks carry the same
       big-op over [path_elems pl].  Consequences, and there are exactly
       three:

         - THE PEEL MOVED.  [Hdrop] (the namei walks' "the family's head at
           [length es0] is the name the memmove wrote") becomes [Hdropp],
           over the shortened list.  It needs [removelast_app], hence it
           needs the remaining string to have an element -- which is
           available exactly where the peel is used and NOT before it: the
           walk reaches dirlookup only after deciding [*s1 <> 0].

         - THE DEATH INDEX'S BOUND SPLIT IN TWO.  [Hklep] ([<=], the type
           test and the nlink guard, which run at the PARENT's own level
           too) and [Hkltp] ([<], the dirlookup miss, which cannot).  See
           [FsAbsNpar.np_dead].

         - +0x140 NEEDED A NEW INVARIANT CONJUNCT.  On the namei side that
           exit is the SUCCESS return and [P L dcur] is exactly what it
           owes.  On this side it is the "nameiparent of /" failure, and
           the failure arm is indexed by the PARENT prefix -- so the proof
           has to know that [es0] is empty there, i.e. that a level which
           consumed an element always leaves another one behind (a level
           whose element is the last one exits at [L_par] instead).  That
           is the second conjunct of the invariant's [Hes0], carried inside
           the existing pure premise so that no call site's [%] count or
           bullet list moves.

   (3) THE FIRE IS [ProofNamexEra]'s, VERBATIM.  Split [Hfview] into
       [Hfv Htop] once before the [found] split, fire off [Htop] with
       [FsAbsEra.elend_fire_hit] / [elend_fire_miss], re-frame [Hfv Htop],
       and read [Htyd] as [Htydz]. [Hdview] is never touched.  The fire is
       shared by both sides of namex's [a1] test, which is why this lane
       was never a question about the lend.

   (4) THE RELATIVE START IS STILL REFUTED, from [pfun 0 = SLASH], exactly
       as [ProofNamexEra] refutes it.  See [SpecNparEra]'s header for why
       it is a contract-shape question and not an arm.

   (5) [ProofNamex]'s counted seal ([wp_namex_sconf]) has no twin here, for
       the same reason the trace walks have none: the counted contract
       cannot carry a trace.
...
(* ===================================================================== *)
(*  THE TRACE'S GHOST LEMMAS ARE NOT HERE.                                *)
(*                                                                        *)
(*  The peel is [FsAbsNpar.ep_hops_cons], the two fires are               *)
(*  [FsAbsEra.elend_fire_hit] / [elend_fire_miss], and the three death    *)
(*  introductions are [FsAbsNpar.np_dead_unfired] / [np_dead_missed] /    *)
(*  [np_dead_noelems].  They live in the spec layer because the lend has  *)
(*  clients (the path syscalls' contracts) that need them without this   *)
(*  file's cone.                                                          *)
(*  So this file's own "genuinely new proof content" is the three         *)
(*  nameiparent facts of the header's item (2), and nothing else.         *)
(* ===================================================================== *)

Module NparEraProof (MP : MYPROC) (ID : IDUP) (IG : IGET) (MM : MEMMOVE)
                    (IL : ILOCK) (IU : IUNLOCK) (IUP : IUNLOCKPUT)
                    (DL : DIRLOOKUP) (IP : IPUT) : NPAR_ERA.
...
Section ProofNparEraMain.
  ...
  Definition nx_loop_body
    ...
    (∀ (off : nat) (ipv : mword 64) (Ml : regfile) (ncur : nat)
     ...
     ⌜(plen - off < fuel)%nat⌝ -∗
     ⌜(off <= plen)%nat⌝ -∗
     (* THE PREFIX -- AND, NEW ON THE NAMEIPARENT SIDE, THE FACT THAT A
        CONSUMED ELEMENT LEAVES ANOTHER ONE BEHIND.  With [npar] set, a
        level whose element is the LAST one exits at [L_par]; so the loop
        head's "nothing left" exit (+0x140) is reachable ONLY on the very
        first entry, and that is what the second conjunct records.  It
        rides INSIDE the existing pure premise rather than as a wand of its
        own, so no call site's [%] count and no bullet list moves. *)
     ⌜path_elems pl = es0 ++ path_elems (drop off pl)
      /\ (es0 <> [] -> path_elems (drop off pl) <> [])⌝ -∗
     ...
     bslots 3 -∗
     (* the set form beside the transaction's token: the walk's [ilock]
        takes the write arm (durable-disk B''-tx) *)
     log_opSt icfg_log ncur Scur -∗
     P (length es0) dcur -∗
     ep_hops_from fsc_fs P Pmiss pl (length es0) -∗
     ...
     wp_next (CID0 := CIDl) true (proc_addr j) (fun CIDc : CpuId =>
       npar_era_post (CID := CIDc) (proc_addr j) pv nb ret_tgt pl m K b eb lks
  ...
  Definition nx_rest_body
    ...
    (∀ (Mt : regfile) (nf' : nat -> bv 8),
     ...
     log_opSt icfg_log ncur Scur -∗
     P kk dcur -∗
     ep_hops_from fsc_fs P Pmiss pl kk -∗
     WP (Loop : expr riscv_lang))%I.

  (* THE WALK, AT THE NAMEIPARENT ERA TRACE CONTRACT.  Instruction for
     instruction the landed [ProofNamex.wp_namex_gen]; what moves is the
     loop invariant's two trace rows, the parent-prefix index arithmetic,
     and the five exits that read them.  BOTH STARTS ARE PROVED (lane
     A-iii): the relative arm is back, restored from [ProofNamex]'s own
     text, and fires the caller's deferred one shot at idup's inum.  The
     arms the contract still kills -- the three namei branches -- are
     refuted from [Hnpe] rather than proved. *)
  Lemma wp_npar_era
 ...
 plen pfun nfun n Sb P Pmiss
  ...
  Proof.
    cbv beta delta [wp_npar_era_body].
    intros pcE pjv pv nb ret_tgt pl L
           HK Hroot Hnib0 Hlg Hsize Hbmap0 Hbmapcov
           Hbmaplog Hinos0 Hcovb Hiregb Hcstr Hplen Hbud Hj Hgs Ha1o
           Hbelow.
    (* THE FLAG, AS AN OPAQUE BOOLEAN (header item (1)).  Pinned rather
       than eliminated, so the three [destruct npar] splits below are the
       landed proof's and each namei branch closes in one line. *)
    pose (npar := true).
    assert (Hnpe : npar = true) by reflexivity.
    assert (Ha1 : eq_vec (m !!! Regidx (mword_of_int 11 : mword 5)) zero_reg
                  = negb npar) by (rewrite Hnpe; exact Ha1o).
    clearbody npar. clear Ha1o.
      ...
      - iIntros (CIDl Hsl off ipv Ml ncur Scur es0 nf wc dcur)
        ...
        + (* ================= EXIT A: nothing but separators left ======= *)
          ...
          * (* ---- the +0x140 tail ---- *)
            ...
            rewrite Hlr in HbC.
            (* THE WALK NEVER RAN.  With [npar] set, a level whose element
               is the last one leaves at [L_par]; so reaching +0x140 means
               the loop head found nothing on its FIRST look, and the path
               has no elements at all -- "nameiparent of /".  [Hesne] is
               the invariant conjunct that says so. *)
            assert (Hes0nil : es0 = []).
            { destruct es0 as [|x es0']; [reflexivity |].
              exfalso. exact (Hesne ltac:(discriminate) Hpel). }
            assert (Hlen0 : length es0 = 0%nat)
              by (rewrite Hes0nil; reflexivity).
            ...
            -- (* nameiparent of a path with no elements left: iput, a0 = 0 *)
               ...
               assert (HT2ra : T2 !!! Regidx Rra
                 ...
                 by (rewrite /T2; apply upd_eq).
               iDestruct "Hip" as (pk pq pinum)
                 "(%Hpe & %Hpk & %Hpb & %Hppos & %Hpz & Href & Hru)".

               assert (HT2a0 : T2 !!! Regidx Ra0 = ientry pk).
               { rewrite /T2 upd_ne; [| nz]. rewrite HT1a0. exact Hpe. }
               assert (Hpb' : bv_unsigned pinum < 16 * Z.of_nat icfg_nib)
                 by (exact Hpb).
               destruct (Hiregb pinum Hpb') as [Hibc Hibl].
               iDestruct (nx_esc_acc pk Hpk with "Hesc")
                 as "#Hescp".
               iDestruct (ic_sleeplocks_lookup fsc_ic pk Hpk with "Hslks")
                 as (gilp gislp) "#Hslkp".
 ...
 pk pq pinum ncur Scur wc false
                         ...
                         T2 (K - 12)%nat eb b
                         _ Upr true Kip Hpk HbW ltac:(discriminate)
               ...
               ++ iIntros (CIDf Hsf mf) "%Hcsf %Hfa0 Hcg Hcnt Hextc Hclmc Hpc".
                  ...
                  ** (* "nameiparent of /": nothing fired, and the cursor at
                        index 0 IS the whole refund.  This is the arm the
                        frozen [k < L] death shape cannot express -- at
                        [L = 0] there is no such [k]. *)
                     iSplitR; [iPureIntro; exact Hfa0 |].
                     iSplitL "Hisl"; [iExact "Hisl" |].
                     iEval (rewrite Hlen0) in "HP".
                     iEval (rewrite Hlen0) in "Hhops".
                     iApply (np_dead_unfired fsc_fs P Pmiss pl 0%nat dcur
                               (Nat.le_0_l _) with "HP Hhops").
            -- (* namei: DEAD.  This contract is the nameiparent side,
 ...
 pidv dq dqb dqs dqpv
               ...
               { iIntros (CIDt Hst Mt nf') "%Hregt %Hviewt Hcg Hcnt Hextc Hclmc Hpc
                 ...
                 - iIntros (CIDr Hsr o2 Mr) "%J1 %J2 %J3 %J4 %J5 %J6
                   ...
                   rewrite Hlrs in HbC.
                   (* ---- THE LEVEL'S TRACE FACTS, AT THE PARENT PREFIX ----
                      [Hklep]: this level's index is inside the prefix OR AT
                      ITS END, the latter when the element is the last one
                      -- i.e. when the directory this level locks IS the
                      parent.  That is the [<=] the LEFT death disjunct
                      owes, and the reason it is not strict; the strict
                      bound [Hkltp] is derived further down, in the branch
                      that has decided the element is not the last one.
                      [Hlen1]: the back edge's index arithmetic. *)
                   assert (Hrne0 : path_elems (drop off pl) <> []).
                   { intro Hc. rewrite Hc in Hlrs. discriminate. }
                   assert (Hklep : (length es0
                            <= length (np_elems pl))%nat)
                     by exact (np_removelast_len_ge (path_elems pl) es0
                                 (path_elems (drop off pl)) Hes0 Hrne0).
                   ...
                   (* ---- THE WRITE ARM (durable-fs-plan.md section 3,
                      [ilock]; durable-disk B''-tx).  namex holds this
                      directory's lock from here to its [iunlock] /
                      [iunlockput], and half the transaction's element sits
 ...
 gilk gislk
                     ...
                     * iIntros (CIDf Hsf mf) "%Hcsf %Hfa0 Hcg Hcnt Hextc Hclmc Hpc".
                       ...
                       ** (* THE LEFT DISJUNCT.  [L_nlink] runs AT the guard,
                             ...
                             [length es0] never fired -- and it runs at the
                             PARENT's own level too, which is exactly why
                             the bound is [<=] and not [<]. *)
                          iSplitR; [iPureIntro; exact Hfa0 |].
                          iSplitL "Hisl"; [iExact "Hisl" |].
                          iApply (np_dead_unfired fsc_fs P Pmiss pl
                                    (length es0) dcur Hklep with "HP Hhops").
                     }
                     ...
                     * (* ---- NAMEIPARENT stops one level early: +0x84 ---- *)
                       (* THE ELEMENT WAS THE LAST ONE, and both halves of
                          this exit's postcondition follow from that one
                          fact.  [Hnl] is [*s1 = 0], so the remaining string
                          normalises to nothing and the path decomposes as
                          [es0 ++ [e]] ([Hpes]) -- which IS the landed name
                          clause [nameiparent_of pl es0 e], and which on the
                          trace side says the PARENT PREFIX is exactly [es0]
                          ([Hlp]).  So the cursor the invariant carries is
                          already at the contract's index, and the hop family
                          from there is empty: the walk owes no further fire.
                          (The derivation is [ProofNamex]'s own, hoisted from
                          the exit to the head of the arm because the trace
                          rows need it too.) *)
                       assert (Hnil : path_elems (drop o2 pl) = []).
                       { apply (proj2 (path_elems_nil_iff (drop o2 pl))).
                       ...
                       assert (Hpes : path_elems pl = es0 ++
                                 [take 14 (bview (e - a)%nat
                                    (fun i => pfun (a + i)%nat))]).
                       { rewrite Hes2 Hnil app_nil_r. reflexivity. }
                       assert (Hlp : length (np_elems pl) = length es0)
                         by (rewrite /np_elems Hpes np_removelast_snoc; reflexivity).
                       ...
                       iDestruct (cpu_claim_ext_transport CIDil CIDP5 eb (proc_addr j)
                                    ltac:(try rewrite Hebb; wp_next_chain) with "Hclmc") as "Hclmc".
                       iDestruct (off_rows_to_dep with "Hoffr") as "Hoffd".
                       iApply (IU.wp_iunlock_tx_sconf gs gilk gislk
                                 ik (iq/2)%Qp gkp lokp tlkp icfg_dev iinum dnl bml
                                 pidv dq NP3 (K - 12)%nat eb (proc_addr j) b lks Upr
                                 Kiu Hik HP3a0
                                 ltac:(lkbelow)
                                 with "Hcg Hcnt Htext Hpc Hitbl Hesck
                                       Hslkk Hslkd Hppid Hprocs [%] Hflkp Hclaimsnx Hdep Hoffd
...
"[%] [%] Hcg Hcnt Hextc Hclmc Hpc
                       ...
                       ** iIntros (CIDf Hsf mf) "%Hcsf %Hfa0 Hcg Hcnt Hextc Hclmc Hpc".
                          ...
                          --- (* THE SUCCESS ARM: the parent, typed and
                                 pinned, and the cursor at the parent index.
                                 [Hlp] is what re-indexes the invariant's
                                 [P (length es0) dcur] to the contract's
                                 [P (length (np_elems pl)) iL]; nothing else
                                 about this exit's resources moved. *)
                              iExists dcur, es0,
                                (take 14 (bview (e - a)%nat
                                   (fun i => pfun (a + i)%nat))).
                              iSplitR; [iPureIntro; exact Hfa0 |].
                              iSplitR;
                                [iPureIntro; exact (conj Hpes Hviewt) |].
                              rewrite Hlp. iFrame "Hip HP Hisl".
                     * (* ---- everything else reaches dirlookup at +0xde ---- *)
                       (* THE ELEMENT IS NOT THE LAST ONE, and every
                          nameiparent-specific fact this branch owes follows
                          from that.  [Hoth] with [npar] set says
                          [*s1 <> 0]; the rest body's [J4] says
                          [*s1 <> '/']; so the remaining string is its own
                          normalisation and has an element.  Hence the
                          level's index is STRICTLY inside the parent prefix
                          ([Hkltp], the miss arm's bound) and the prefix's
                          own head at that index is the name the memmove
                          wrote ([Hdropp], the peel the fire runs on). *)
                       assert (Hnl : pfun o2 <> NUL).
                       { intro Hc. apply Hoth. split; [exact Hnpe | exact Hc]. }
                       assert (Ho2lt : (o2 < plen)%nat)
                         by exact (Hnult o2 J2 Hnl).
                       assert (HRne : path_elems (drop o2 pl) <> []).
                       { intro Hc.
                         assert (Hps : pe_skip (drop o2 pl) = [])
                           by exact (proj1 (path_elems_nil_iff (drop o2 pl)) Hc).
                         assert (Hdc : drop o2 pl = pfun o2 :: drop (S o2) pl)
                           by exact (nx_drop_cons o2 plen pfun Ho2lt).
                         rewrite Hdc (pe_skip_ne (pfun o2) _ J4) in Hps.
                         discriminate. }
                       assert (Hkltp : (length es0
                                < length (np_elems pl))%nat)
                         by exact (np_removelast_len_gt (path_elems pl) es0
                                     (take 14 (bview (e - a)%nat
                                        (fun i => pfun (a + i)%nat)))
                                     (path_elems (drop o2 pl)) Hes2 HRne).
                       assert (Hdropp : drop (length es0) (np_elems pl)
                                = bname 14 nf'
                                  :: removelast (path_elems (drop o2 pl))).
                       { rewrite /np_elems Hes2 (np_removelast_app _ _ HRne)
                           -app_assoc Hviewt drop_app_length. reflexivity. }
 ...
 (ientry ik) iinum
                         ...
                         (* ================== THE FIRE POINT ==================
                            [ProofNamexEra]'s, verbatim -- the fire is SHARED
                            by both sides of namex's [a1] test.  dirlookup
                            has returned and the walk still holds the locked
                            directory's payload, so [Hfview] is
                            [fv_ride * top_frag] at [DfracOwn 1]; split it
                            once here, before the [found] case split, so both
                            arms fire off [Htop] and both re-pack
                            [Hfv Htop].  [Hdview] is never touched.  Peel hop
                            [length es0] off the PARENT-PREFIX family --
                            legal HERE and not at the [L_par] level, because
                            [Hdropp] needed the element not to be the last
                            one -- and lend half the leg through the caller's
                            single [={⊤}=∗]; the mask is eliminated by
                            [fupd_wp] at this instruction boundary, which is
                            legal exactly because nothing of the walk's is
                            open here. *)
                         iEval (rewrite -Hiz) in "HP".
                         iDestruct (ep_hops_cons fsc_fs P Pmiss pl (length es0)
                                      (bname 14 nf')
                                      (removelast (path_elems (drop o2 pl)))
                                      Hdropp with "Hhops") as "[Hhop Hhops]".
 ...
 ik (iq/2)%Qp (iq/2)%Qp gkp lokp tlkp
                           ...
                           -- lia.
                           -- exact J2.
                           -- (* THE BACK EDGE RE-ESTABLISHES THE SECOND
                                 CONJUNCT: this level consumed an element and
                                 [HRne] says another one is left. *)
                              split; [exact Hes2 | intros _; exact HRne].
                           -- exact Hnb1.
                           -- exact Hnb2.
                           -- exact Hnb2b.
                           -- exact Hnb3.
 ...
 ik (iq/2)%Qp (iq/2)%Qp gkp lokp tlkp
                           ...
                           -- iIntros (CIDf Hsf mf) "%Hcsf %Hfa0 Hcg Hcnt Hextc Hclmc Hpc".
                              ...
                              ++ (* THE RIGHT DISJUNCT.  Hop [length es0] DID
                                    fire, and the name was absent.  The bound
                                    is STRICT here: dirlookup is reached only
                                    after the walk has decided the element is
                                    not the last one ([Hkltp]). *)
                                 iSplitR; [iPureIntro; exact Hfa0 |].
                                 iSplitL "Hisl"; [iExact "Hisl" |].
                                 iApply (np_dead_missed fsc_fs P Pmiss pl
                                           (length es0) (bv_unsigned iinum)
                                           Hkltp with "HP Hhops"). }
                       (* ---- the two routes into +0xde ---- *)
                       destruct npar.
                       ** (* nameiparent, but the path continues.  [Hnl] is
                             already in scope: this branch's whole point --
                             that the element is not the last one -- was
                             derived at the head of the [Hoth] arm, because
                             the trace facts need it too. *)
                          iApply (wp_beqz_x0_fall_s_sconf
                                    (mword_of_int (NX + 0xd4))
                                    (mword_of_int 10 : mword 13) Rs6 W0
                                    (K - 12)%nat b ltac:(nz)
 ...
 gilk gislk
                     ...
                     * iIntros (CIDf Hsf mf) "%Hcsf %Hfa0 Hcg Hcnt Hextc Hclmc Hpc".
                       ...
                       ** (* THE LEFT DISJUNCT.  [L_notdir] runs before the
                             ...
                             [length es0] never fired -- and, again, this
                             test runs at the parent's own level too. *)
                          iSplitR; [iPureIntro; exact Hfa0 |].
                          iSplitL "Hisl"; [iExact "Hisl" |].
                          iApply (np_dead_unfired fsc_fs P Pmiss pl
                                    (length es0) dcur Hklep
                                    with "HP Hhops"). }
    ...
    + (* ---------------- THE ABSOLUTE ARM: iget(ROOTDEV, ROOTINO) ------- *)
      ...
      iDestruct (wp_next_shift (b := true) (CIDa := CID23) (CIDb := CIDK5)
                   ltac:(wp_next_chain) with "Hcont") as "Hcont".
      (* THE ONE SHOT, FIRED AT THE ROOT.  [FsAbsStart.ep_start] is
         universally quantified over the start inum; this arm's is
         ROOTINO, and the tie is discharged by [reflexivity] because [r]
         IS ROOTINO here.  The relative arm below fires the same premise
      ...
      - lia.
      - lia.
      - (* the prefix is empty, so the second conjunct is vacuous *)
        split; [rewrite drop_0; reflexivity | intros Hc; exfalso; exact (Hc eq_refl)].
  ...
  (* NO COUNTED SEAL.  [ProofNamex] closes with [wp_namex_sconf], the
     [log_op] form of the same walk; a trace contract has no counted twin
     (the counted continuation has no [P] to hand the cursor to), which is
     why [ProofNamexTr] and [ProofNamexEra] have none either. *)

End ProofNparEraMain.
