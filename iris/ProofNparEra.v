...
Section ProofNparEraMain.
  ...
  (* THE WALK, AT THE NAMEIPARENT ERA TRACE CONTRACT.  Instruction for
     ...
     and the five exits that read them.  The arms the contract kills -- the
     relative start and the three namei branches -- are refuted from
     [Hpfun0] / [Hnpe] rather than proved. *)
  Lemma wp_npar_era
 ...
 plen pfun nfun n Sb P Pmiss
  ...
  Proof.
    ...
    intros pcE pjv pv nb ret_tgt pl L
           ...
           Hbmaplog Hinos0 Hcovb Hiregb Hcstr Hplen Hpfun0 Hbud Hj Hgs Ha1o
           Hbelow.
...
"[%] [%] Hcg Hcnt Hextc Hclmc Hpc
 ...
 gilk gislk
    ...
    + (* --------- THE RELATIVE ARM IS DEAD: ABSOLUTE PATHS ONLY --------- *)
      (* The contract is scoped to [pfun 0 = SLASH], so the [beq a4,a5] at
         +0x2a is decided by a PREMISE and myproc / idup are never reached.
         When the relative form lands it needs an inum-exposed cwd, which is
         a different contract's business ([SpecNparEra]'s header). *)
