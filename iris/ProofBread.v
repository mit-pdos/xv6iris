...
Section ProofBread.
  ...
  (* ---------------------------------------------------------------- *)
  Definition bd_cont (γ : gname) (Φ : mval -> iProp Σ)
    ...
    ( ∀ (mf : regfile) (k : nat) (bs_out : list (bv 8)),
        ⌜callee_saved m mf /\ mf !!! Regidx Ra0 = bnode k⌝ -∗
        sie_cap_gpr γ mf K -∗
        cpu_own γ 0 eb pj C -∗
        trap_csrs_pay 0 eb -∗
  ...
  Local Lemma bread_epi (γ : gname) (Φ : mval -> iProp Σ)
    ...
    bio_locked bn k pidv dev bno bs_out -∗
    disk_block γd (uint bno) bs_disk -∗
    bd_cont γ Φ γs γd bn pidv dev bno dq m K eb (proc_addr j) C bs_disk -∗
  ...
  Proof.
    ...
    set (spr := add_vec (m !!! Regidx csp_rs1 : mword 64)
                  (sign_extend' 64 (caddi16sp_imm (mword_of_int 61 : mword 6)))).
    iIntros "Hcg #Htext Hpc Hframe Hcnt Hpay Hoctx Hvc Hppid Hlk Hdisk Hcont".
    rewrite /bd_frame.
    ...
    rewrite /bd_cont.
    iApply ("Hcont" $! E7 k bs_out with "[%] Hcg Hcnt Hpay Hpc Hoctx Hvc Hppid Hlk Hdisk").
    split; [| exact HE7a0].
    (* callee_saved m E7 *)
    ...
    { intros c Hcs N2 N8 N9 N18 N19.
      ...
      exact (HMthr c Hcs N2 N8 N9 N18 N19). }
    unfold callee_saved.
    ...
    assert (Hc4 : E7 !!! Regidx Rtp = m !!! Regidx Rtp)
      by (apply Hthread; vm_compute; first [reflexivity | discriminate]).
    repeat split;
      first [ exact Hc2 | exact Hc4 | exact Hc8 | exact Hc9 | exact Hc18 | exact Hc19
  ...
  Local Lemma bread_tail (γ : gname) (Φ : mval -> iProp Σ)
    ...
    WP (Loop : expr riscv_lang) {{ Φ }}.
  Proof.
    intros HK Hbno Hk Htp Hj Hgl Hregs HMs1.
    pose proof Hregs as (HMsp & HMs2 & HMs3 & HMthr).
    iIntros "Hcg #Htext Hpc #Hpanic #Hesc Hframe Hcnt Hpay #Hprocs Hoctx Hvc Hppid".
    ...
    - (* ============ b->valid = 0: the disk read ============ *)
      ...
      assert (HmRs1 : mR !!! Regidx Rs1 = bnode k)
        by (rewrite (callee_saved_lookup Hcs2_cs Rs1 ltac:(vm_compute; reflexivity)); exact HT4s1).
      (* +0xd0 c.li a5,1 *)
      assert (Hli1 : add_vec zero_reg (sign_extend' 64 (sign_extend' 12 (mword_of_int 1 : mword 6)))
                     = (mword_of_int 1 : mword 64))
        by (apply bv_eq; vm_compute; reflexivity).
      iApply (wp_cli_s_sconf γ Φ (mword_of_int (BD + 0xd0)) Ra5 (mword_of_int 1 : mword 6)
      ...
      iApply (wp_csw_s_sconf γ Φ (mword_of_int (BD + 0xd2)) Ra5 Rs1 (mword_of_int 0 : mword 12)
      ...
      iApply (wp_cj_s_sconf γ Φ (mword_of_int (BD + 0xd4))
      ...
      { rewrite /bd_regs. split_and!.
        - rewrite /T5 upd_ne; [| vm_compute; discriminate].
          rewrite (callee_saved_lookup Hcs2_cs csp_rs1 ltac:(vm_compute; reflexivity)).
          rewrite (HT4thr csp_rs1 ltac:(vm_compute; reflexivity)).
          rewrite /T1 upd_ne; [exact HMsp | vm_compute; discriminate].
        - rewrite /T5 upd_ne; [| vm_compute; discriminate].
          rewrite (callee_saved_lookup Hcs2_cs Rs2 ltac:(vm_compute; reflexivity)).
          rewrite (HT4thr Rs2 ltac:(vm_compute; reflexivity)).
          rewrite /T1 upd_ne; [exact HMs2 | vm_compute; discriminate].
        - rewrite /T5 upd_ne; [| vm_compute; discriminate].
          rewrite (callee_saved_lookup Hcs2_cs Rs3 ltac:(vm_compute; reflexivity)).
        ...
        - intros c Hcs N2 N8 N9 N18 N19.
          ...
          rewrite /T1 upd_ne; [| apply not_eq_sym; apply is_cs_idx_true_neq;
                                 [vm_compute; reflexivity | assumption]].
          exact (HMthr c Hcs N2 N8 N9 N18 N19). }
      iApply (bread_epi γ Φ γs j γd bn k pidv dev bno dq m T5 K eb C bs_disk bs_disk
                HK HT5regs HT5s1
                with "Hcg Htext Hpc Hframe Hcnt Hpay Hoctx Hvc Hppid [Hstok Hpid Hvld Hbdev Hbuf] Hdisk Hcont").
  ...
  Local Lemma bread_hit (γ : gname) (Φ : mval -> iProp Σ)
    ...
    WP (Loop : expr riscv_lang) {{ Φ }}.
  Proof.
    intros HK Hbno Hk Hdevs Hbnos Htp Hj Hgl Hregs HMs1.
    pose proof Hregs as (HMsp & HMs2 & HMs3 & HMthr).
    iIntros "Hcg #Htext Hpc #Hpanic #Hbio Hframe Hcnt Hpay Hpay0 Htok Hscan Hbslot".
    ...
    iApply (ASL.wp_acquiresleep_sconf (dq := dq) γ Φ γs j (fst (bn_slk bn k)) (snd (bn_slk bn k))
              ...
              HH7tp Hj ltac:(unfold K_bread in HK; lia)
              with "Hcg Hcnt Htext Hpc [] Hpanic Hppid Hprocs Hoctx Hvc [-]").
    { iEval (rewrite HH7a0). iExact "Hslk". }
    iIntros (mf) "%Hcsasl Hcg Hcnt Hpc Hstok Hpid Hbown Hppid Hoctx Hvc".
    ...
    pose proof Hcsasl as Hcsasl_cs.
    assert (HmfX : forall c : mword 5, is_cs_idx c = true ->
                     mf !!! Regidx c = M !!! Regidx c).
    { intros c Hcs. rewrite (callee_saved_lookup Hcsasl_cs c Hcs). exact (HH7thr c Hcs). }
    ...
    assert (Hmftp : mf !!! Regidx Rtp = cid_word)
      by (rewrite (HmfX Rtp ltac:(vm_compute; reflexivity)); exact Htp).
    ...
    { rewrite /bd_regs. split_and!.
      ...
      - intros c Hcs N2 N8 N9 N18 N19.
        rewrite (HmfX c Hcs). exact (HMthr c Hcs N2 N8 N9 N18 N19). }
    (* ---- +0x62 c.j +0x52 : into the shared tail ---- *)
    iApply (wp_cj_s_sconf γ Φ (mword_of_int (BD + 0x62))
    ...
    iApply (bread_tail γ Φ γs j γl γu γd γk pd pav pu bn k qref pidv dev bno dq
              m mf K eb C bs_disk HK Hbno Hk Hmftp Hj Hgl Hmfregs Hmfs1
              with "Hcg Htext Hpc Hpanic Hesc Hframe Hcnt Hpay0 Hprocs Hoctx Hvc Hppid
  ...
  Local Lemma bread_recyc (γ : gname) (Φ : mval -> iProp Σ)
    ...
    WP (Loop : expr riscv_lang) {{ Φ }}.
  Proof.
    intros HK Hbno Hk HMk Ha0 Ha1 Htp Hj Hgl Hregs HMs1.
    pose proof Hregs as (HMsp & HMs2 & HMs3 & HMthr).
    iIntros "Hcg #Htext Hpc #Hpanic #Hbio Hframe Hcnt Hpay Hpay0 Htok Hscan Hbslot".
    iIntros "#Hprocs Hoctx Hvc Hppid #Hdev #Hgeom #Hdlock Hdisk Hcont".
    iDestruct (bio_ctx_lock with "Hbio") as "#Hlock".
    ...
    iApply (ASL.wp_acquiresleep_sconf (dq := dq) γ Φ γs j (fst (bn_slk bn k)) (snd (bn_slk bn k))
              ...
              HC6tp Hj ltac:(unfold K_bread in HK; lia)
              with "Hcg Hcnt Htext Hpc [] Hpanic Hppid Hprocs Hoctx Hvc [-]").
    ...
    assert (HmfX : forall c : mword 5, is_cs_idx c = true ->
                     mf !!! Regidx c = M !!! Regidx c).
    { intros c Hcs. rewrite (callee_saved_lookup Hcsasl_cs c Hcs). exact (HC6thr c Hcs). }
    assert (Hmfs1 : mf !!! Regidx Rs1 = bnode k)
      by (rewrite (HmfX Rs1 ltac:(vm_compute; reflexivity)); exact HMs1).
    assert (Hmftp : mf !!! Regidx Rtp = cid_word)
      by (rewrite (HmfX Rtp ltac:(vm_compute; reflexivity)); exact Htp).
    ...
    { rewrite /bd_regs. split_and!.
      ...
      - intros c Hcs N2 N8 N9 N18 N19.
        rewrite (HmfX c Hcs). exact (HMthr c Hcs N2 N8 N9 N18 N19). }
    iApply (bread_tail γ Φ γs j γl γu γd γk pd pav pu bn k (1/4)%Qp pidv dev bno dq
              m mf K eb C bs_disk HK Hbno Hk Hmftp Hj Hgl Hmfregs Hmfs1
              with "Hcg Htext Hpc Hpanic Hesc Hframe Hcnt Hpay0 Hprocs Hoctx Hvc Hppid
                    Hdev Hgeom Hdlock Hstok Hpid Hbown Href Hdisk Hcont").
  Qed.
  ...
  Proof.
    ...
    induction n as [|n IH];
      intros pre post M Hlen Hord Hne Hregs Htp HMs1 HMa4.
    ...
    - (* ======== refcnt == 0: this buffer is free, RECYCLE it ======== *)
      ...
      iApply (bread_recyc γ Φ γs j γl γu γd γk pd pav pu bn kk Mg ord devs bnos
                ...
                HK Hbno Hkk HMkNone Ha0 Ha1 HB1tp Hj Hgl HB1regs HB1s1
                with "Hcg Htext Hpc Hpanic Hbio Hframe Hcnt Hpay Hpay0 Htok Hscan Hbslot
    ...
    - (* ======== refcnt <> 0: advance to b->prev ======== *)
      ...
      + (* ---- the prefix is exhausted: fall through to panic ---- *)
        ...
        iApply ("Hpanic" with "Htext Hpc Hcg").
      + (* ---- the prefix still has a node: take the back edge ---- *)
        ...
        iApply (IH d (kk :: post) B2 Hlen' Hord2 Hdne HB2regs HB2tp HB2s1 HB2a4
                  with "Hcg Htext Hpc Hpanic Hbio Hframe Hcnt Hpay Hpay0 Htok Hscan Hbslot
                        Hprocs Hoctx Hvc Hppid Hdev Hgeom Hdlock Hdisk Hcont").
  Qed.
  ...
  Local Lemma bread_miss (γ : gname) (Φ : mval -> iProp Σ)
    ...
    WP (Loop : expr riscv_lang) {{ Φ }}.
  Proof.
    intros HK Hbno Ha0 Ha1 Hj Hgl Hordp Hregs Htp.
    pose proof Hregs as (HMsp & HMs2 & HMs3 & HMthr).
    ...
    { rewrite /bd_regs. split_and!.
      ...
      - rewrite (HQ5thr Rs3 ltac:(vm_compute; reflexivity) ltac:(vm_compute; discriminate)).
        exact HMs3.
      - intros c Hcs N2 N8 N9 N18 N19.
        rewrite (HQ5thr c Hcs N9). exact (HMthr c Hcs N2 N8 N9 N18 N19). }
    iAssert (bcache_scan bn Mg ord devs bnos)
      with "[Hauth Hsauth Hlru Hslots]" as "Hscan".
    { rewrite /bcache_scan. iFrame "Hauth Hsauth".
      iSplitR; [iPureIntro; exact Hdom|].
    ...
    iApply (bread_bloop γ Φ γs j γl γu γd γk pd pav pu bn Mg ord devs bnos
              ...
              (bd_ord_nonnil ord Hordp) HQ5regs HQ5tp HQ5s1 HQ5a4
              with "Hcg Htext Hpc Hpanic Hbio Hframe Hcnt Hpay Hpay0 Htok Hscan Hbslot
                    Hprocs Hoctx Hvc Hppid Hdev Hgeom Hdlock Hdisk Hcont").
  Qed.
  ...
  Proof.
    ...
    induction n as [|n IH];
      intros done rest M Hlen Hord Hne Hregs Htp HMs1 HMa4.
    ...
    { iIntros (Mx (Hxregs & Hxtp & Hxs1 & Hxa4)).
      iIntros "Hcg Hpc Hframe Hcnt Hpay Hpay0 Htok Hscan Hbslot Hoctx Hvc Hppid Hdisk Hcont".
      ...
      { rewrite /bd_regs. split_and!.
        ...
        - intros c Hcs N2 N8 N9 N18 N19.
          ...
          exact (Hxthr c Hcs N2 N8 N9 N18 N19). }
      iAssert (bcache_scan bn Mg ord devs bnos)
        with "[Hauth Hsauth Hlru Hslots]" as "Hscan".
      { rewrite /bcache_scan. iFrame "Hauth Hsauth".
        iSplitR; [iPureIntro; exact Hdom|].
      ...
      + (* the scan has wrapped: on to the backward scan *)
        ...
        iApply (bread_miss γ Φ γs j γl γu γd γk pd pav pu bn Mg ord devs bnos
                  ...
                  HK Hbno Ha0 Ha1 Hj Hgl Hordp HG1regs HG1tp
                  with "Hcg Htext Hpc Hpanic Hbio Hframe Hcnt Hpay Hpay0 Htok Hscan Hbslot
                        Hprocs Hoctx Hvc Hppid Hdev Hgeom Hdlock Hdisk Hcont").
      + (* another buffer to test: back to the loop head *)
        ...
        iApply (IH (done ++ [kk])%list r G1 Hlen'
                  ltac:(rewrite Hord2 -app_assoc; reflexivity) Hrne HG1regs HG1tp HG1s1 HG1a4
                  with "Hcg Htext Hpc Hpanic Hbio Hframe Hcnt Hpay Hpay0 Htok Hscan Hbslot
                        Hprocs Hoctx Hvc Hppid Hdev Hgeom Hdlock Hdisk Hcont"). }
    ...
    - (* ---- dev matches: test the block number ---- *)
      ...
      + (* ======== THE HIT ======== *)
        ...
        iApply (bread_hit γ Φ γs j γl γu γd γk pd pav pu bn kk Mg ord devs bnos
                  ...
                  HK Hbno Hkk Hdeq Hbeq2 HF2tp Hj Hgl HF2regs HF2s1
                  with "Hcg Htext Hpc Hpanic Hbio Hframe Hcnt Hpay Hpay0 Htok Hscan Hbslot
                        Hprocs Hoctx Hvc Hppid Hdev Hgeom Hdlock Hdisk Hcont").
      + (* blockno mismatch: advance *)
        ...
        iApply ("HADV" $! F2 with "[%] Hcg Hpc Hframe Hcnt Hpay Hpay0 Htok Hscan Hbslot
    ...
    - (* ---- dev mismatch: advance ---- *)
      ...
      { rewrite /bcache_scan. iFrame "Hauth Hsauth".
        ...
        iSplitR; [iPureIntro; exact Hordp2|].
        iFrame "Hlru Hslots". }
      iApply ("HADV" $! F1 with "[%] Hcg Hpc Hframe Hcnt Hpay Hpay0 Htok Hscan Hbslot
                                 Hoctx Hvc Hppid Hdisk Hcont").
  ...
  Proof.
    ...
    intros pcE pj ret_tgt HK Hbno Htp Hj Hgl Ha0 Ha1.
    pose (sp0 := (m !!! Regidx csp_rs1 : mword 64)).
    set (spr := add_vec (m !!! Regidx csp_rs1 : mword 64)
                  (sign_extend' 64 (caddi16sp_imm (mword_of_int 61 : mword 6)))).
    iIntros "Hcg Hcnt Hpay0 #Htext Hpc #Hpanic #Hbio Hppid #Hprocs Hoctx Hvc".
    iIntros "#Hdev #Hgeom #Hdlock Hdisk Hbslot Hcont".
    iAssert (bd_cont γ Φ γs γd bn pidv dev bno dq m K eb (proc_addr j) C bs_disk)
      with "[Hcont]" as "Hcont".
    ...
    iApply (bread_floop γ Φ γs j γl γu γd γk pd pav pu bn Mg ord devs bnos
              ...
              (bd_ord_nonnil ord Hordp) HW5regs HW5tp HW5s1 HW5a4
              with "Hcg Htext Hpc Hpanic Hbio Hframe Hcnt Hpay Hpay0 Htok Hscan Hbslot
                    Hprocs Hoctx Hvc Hppid Hdev Hgeom Hdlock Hdisk Hcont").
  Qed.
