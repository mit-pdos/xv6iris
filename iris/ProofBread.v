(* ProofBread.v -- bread (with bget INLINED) over the SIE-agnostic sconf world.
   ...
   * THE SCANS CARRY [BioInv.bcache_scan], the bcache resource with
     ...
     was established and the minted [bref] could not be at the requested key.
   ...
   * THE FOUR ESCROW OPENS are each around ONE instruction, with the
     ...
     three field stores ([escrow_free_dev]/[_bno]/[_valid]) and the tail's
     [lw a5,0(s1)] ([BioInv.escrow_swap_checkout]).  So the parked bundle is
     whole at every instruction boundary, which is what makes the
     recycler-vs-hit-thread race safe for free.
   ...
   * THE VALID PIN.  [buf_parked] pins b->valid to {0,1}, so the tail's
     [c.beqz] fall-through arm KNOWS the cell holds 1 -- which is exactly what
     [bio_locked] states -- and the taken arm knows it holds 0 and may
     therefore be overwritten with 1 after the read. *)
...
(* [is_cs_idx] minus tp is exactly [is_cs_idx_notp]: the bridge a bd_regs
   clause needs to feed [callee_saved_notp_lookup] across a park. *)
Lemma bd_notp_of (c : mword 5) :
  is_cs_idx c = true -> c <> (mword_of_int 4 : mword 5) -> is_cs_idx_notp c = true.
Proof.
  unfold is_cs_idx, is_cs_idx_notp. cbn [existsb].
  intros Hc Hne. rewrite !orb_true_iff in Hc. rewrite !orb_true_iff.
  destruct Hc as [Hc|Hc]; [tauto|].
  destruct Hc as [Hc|Hc]; [apply bool_decide_eq_true in Hc; congruence | tauto].
Qed.

(* ===================================================================== *)
(*  THE HART-FREE PIECES: bread's own continuation, its frame bundle and  *)
(*  the register invariant every block carries.  [bd_cont] must be        *)
(*  hart-INDEPENDENT -- it is established before the first park and       *)
(*  consumed after the second, on a hart neither side knows -- so it is   *)
(*  quantified over the returning hart and its SIE ghost, and lives       *)
(*  OUTSIDE any section that fixes [CID].                                 *)
(* ===================================================================== *)
Section BreadDefs.
  Context `{!riscvGS Σ, !sieG Σ, !lockG Σ, !fdslotG Σ, !bioG Σ, !diskGhostG Σ, !uartGhostG Σ}.

  (* ---------------------------------------------------------------- *)
  (*  bread's own continuation, NAMED (claude-notes/optimization.md:   *)
  (*  a whole-function proof's post must not be spelled inline -- it   *)
  (*  is re-traversed by every proofmode operation, and here it also   *)
  (*  appears in seven block-lemma statements).                        *)
  (* ---------------------------------------------------------------- *)
  Definition bd_cont (Φ : mval -> iProp Σ)
      (γs : list gname) (γd : disk_names) (bn : bio_names)
      (pidv dev bno : mword 32) (dq : dfrac)
      (m : regfile) (K : nat) (eb : bool) (pj : mword 64) (C : iProp Σ)
      (bs_disk : list (bv 8)) : iProp Σ :=
    ( ∀ (h : CPU) (g : gname) (mf : regfile) (k : nat) (bs_out : list (bv 8)),
        ⌜callee_saved_notp m mf
         /\ mf !!! Regidx Rtp = cid_word_of h
         /\ mf !!! Regidx Ra0 = bnode k⌝ -∗
        sie_cap_gpr (CID := h) g mf K -∗
        cpu_own (CID := h) g 0 eb pj C -∗
        pc_is (CID := h) (ret_pc (m !!! Regidx Rra)) -∗
...
(* ===================================================================== *)
(*  THE TWO POST-PARK STRETCHES, EACH WITH [CID] AS A BINDER.             *)
(*                                                                        *)
(*  acquiresleep parks, and so does virtio_disk_rw, so the shared tail    *)
(*  runs at the hart the FIRST park resumed on and the epilogue at the    *)
(*  hart the SECOND one did (or the same hart, on the valid-hit arm that  *)
(*  skips rw).  A [Context {CID : CpuId}] cannot be instantiated from     *)
(*  inside its own section, so both live here, applied at [(CID := h)].   *)
(* ===================================================================== *)
Section BreadBlocks.
  Context `{!riscvGS Σ, !sieG Σ, !lockG Σ, !fdslotG Σ, !bioG Σ, !diskGhostG Σ, !uartGhostG Σ}.
  ...
  Local Lemma bread_epi `{CID : CpuId} (γ : gname) (Φ : mval -> iProp Σ)
      (γs : list gname) (j : nat) (γd : disk_names) (bn : bio_names) (k : nat)
      (pidv dev bno : mword 32) (dq : dfrac)
      (m M : regfile) (K : nat) (eb : bool) (C : iProp Σ)
      (bs_out bs_disk : list (bv 8)) :
    (K_bread <= K)%nat ->
    bd_regs m M ->
    M !!! Regidx Rs1 = bnode k ->
    (* THE HART-FLAVOURED PREMISE: the arrival map's tp is THIS hart's -- the
       resuming hart's, if a park lies between bread's entry and here. *)
    M !!! Regidx Rtp = cid_word ->
    sie_cap_gpr γ M (K - 6)%nat -∗
    kernel_text -∗
    pc_is (mword_of_int (BD + 0xb8) : mword 64) -∗
    bd_frame m -∗
    ...
    bio_locked bn k pidv dev bno bs_out -∗
    disk_block γd (uint bno) bs_disk -∗
    bd_cont Φ γs γd bn pidv dev bno dq m K eb (proc_addr j) C bs_disk -∗
    WP (Loop : expr riscv_lang) {{ Φ }}.
  Proof.
    intros HK (HMsp & HMs2 & HMs3 & HMthr) HMs1 HMtp.
    pose (sp0 := (m !!! Regidx csp_rs1 : mword 64)).
    set (spr := add_vec (m !!! Regidx csp_rs1 : mword 64)
                  (sign_extend' 64 (caddi16sp_imm (mword_of_int 61 : mword 6)))).
    iIntros "Hcg #Htext Hpc Hframe Hcnt Hoctx Hvc Hppid Hlk Hdisk Hcont".
    rewrite /bd_frame.
    ...
    { unfold spr, sp0, pa_stk, add_vec_int. rewrite add_vec_off2.
      f_equal; try (apply bv_eq; vm_compute; reflexivity). }
    (* +0xb8 c.mv a0,s1 *)
    iApply (wp_cmv_s_sconf γ Φ (mword_of_int (BD + 0xb8)) Ra0 Rs1
    ...
    assert (Hppba : add_vec_int (mword_of_int (BD + 0xb8) : mword 64) 2 = mword_of_int (BD + 0xba))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hppba) in "Hpc".
    (* +0xba c.ldsp ra,40(sp) *)
    iApply (wp_cldsp_s_sconf γ Φ (mword_of_int (BD + 0xba)) (mword_of_int 5 : mword 6) Rra
              E1 (K - 6)%nat (m !!! Regidx Rra)
    ...
    assert (Hppbc : add_vec_int (mword_of_int (BD + 0xba) : mword 64) 2 = mword_of_int (BD + 0xbc))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hppbc) in "Hpc".
    (* +0xbc c.ldsp s0,32(sp) *)
    iApply (wp_cldsp_s_sconf γ Φ (mword_of_int (BD + 0xbc)) (mword_of_int 4 : mword 6) Rs0
              E2 (K - 6)%nat (m !!! Regidx Rs0)
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
    ...
    assert (Hppbe : add_vec_int (mword_of_int (BD + 0xbc) : mword 64) 2 = mword_of_int (BD + 0xbe))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hppbe) in "Hpc".
    (* +0xbe c.ldsp s1,24(sp) *)
    iApply (wp_cldsp_s_sconf γ Φ (mword_of_int (BD + 0xbe)) (mword_of_int 3 : mword 6) Rs1
              E3 (K - 6)%nat (m !!! Regidx Rs1)
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
    ...
    assert (Hppc0 : add_vec_int (mword_of_int (BD + 0xbe) : mword 64) 2 = mword_of_int (BD + 0xc0))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hppc0) in "Hpc".
    (* +0xc0 c.ldsp s2,16(sp) *)
    iApply (wp_cldsp_s_sconf γ Φ (mword_of_int (BD + 0xc0)) (mword_of_int 2 : mword 6) Rs2
              E4 (K - 6)%nat (m !!! Regidx Rs2)
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
    ...
    assert (Hppc2 : add_vec_int (mword_of_int (BD + 0xc0) : mword 64) 2 = mword_of_int (BD + 0xc2))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hppc2) in "Hpc".
    (* +0xc2 c.ldsp s3,8(sp) *)
    iApply (wp_cldsp_s_sconf γ Φ (mword_of_int (BD + 0xc2)) (mword_of_int 1 : mword 6) Rs3
              E5 (K - 6)%nat (m !!! Regidx Rs3)
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
    ...
    assert (Hppc4 : add_vec_int (mword_of_int (BD + 0xc2) : mword 64) 2 = mword_of_int (BD + 0xc4))
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (wp_caddi16sp_pop_s_sconf γ Φ (mword_of_int (BD + 0xc4)) (mword_of_int 3 : mword 6)
              E6 (K - 6)%nat 6 Hpop with "Hcg Hpc Hic4 Hframe6 [-]").
    iIntros "Hcg Hpc".
    ...
    assert (Hppc6 : add_vec_int (mword_of_int (BD + 0xc4) : mword 64) 2 = mword_of_int (BD + 0xc6))
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    (* +0xc6 c.ret *)
    iApply (wp_cret_s_sconf γ Φ (mword_of_int (BD + 0xc6)) Rra E7 K
    ...
    rewrite /bd_cont.
    (* tp is untouched by the whole epilogue (E1..E7 write a0/ra/s0/s1/s2/s3/sp) *)
    assert (HE7tp : E7 !!! Regidx Rtp = cid_word_of cpu_id).
    { rewrite /E7 upd_ne; [| vm_compute; discriminate].
      rewrite /E6 upd_ne; [| vm_compute; discriminate].
      rewrite /E5 upd_ne; [| vm_compute; discriminate].
      rewrite /E4 upd_ne; [| vm_compute; discriminate].
      ...
      rewrite /E1 upd_ne; [| vm_compute; discriminate]. exact HMtp. }
    iApply ("Hcont" $! cpu_id γ E7 k bs_out with "[%] Hcg Hcnt Hpc Hoctx Hvc Hppid Hlk Hdisk").
    split; [| split; [exact HE7tp | exact HE7a0]].
    (* callee_saved_notp m E7 *)
  ...
  Local Lemma bread_tail `{CID : CpuId} (γ : gname) (Φ : mval -> iProp Σ)
      (γs : list gname) (j : nat) (γl : gname)
      (γu : uart_names) (γd : disk_names) (γk : gname)
      (pd pav pu : mword 64)
      (bn : bio_names) (k : nat) (q : Qp)
      (pidv dev bno : mword 32) (dq : dfrac)
      (m M : regfile) (K : nat) (eb : bool) (C : iProp Σ)
      (bs_disk : list (bv 8)) :
    (K_bread <= K)%nat ->
    ...
    sie_cap_gpr γ M (K - 6)%nat -∗
    kernel_text -∗
    pc_is (mword_of_int (BD + 0xb4) : mword 64) -∗
    panic_wp_any -∗
    ...
    WP (Loop : expr riscv_lang) {{ Φ }}.
  Proof.
    intros HK Hbno Hk Htp Hj Hgl Hregs HMs1 Heb.
    pose proof Hregs as (HMsp & HMs2 & HMs3 & HMthr).
    iIntros "Hcg #Htext Hpc #Hpanic #Hesc Hframe Hcnt #Hprocs Hoctx Hvc Hppid".
    iIntros "#Hdev #Hgeom #Hdlock Hstok Hpid Hbown Hbref Hdisk Hcont".
    iDestruct "Hbref" as "(Hrtok & Hrdev & Hrbno)".
    iPoseProof (bdi_b4 with "Htext") as "Hib4".
    iPoseProof (bdi_b6 with "Htext") as "Hib6".
    assert (Hva : add_vec (M !!! Regidx Rs1) (sign_extend' 64 (mword_of_int 0 : mword 12))
                  = b_valid (bpa k)).
    { rewrite HMs1 bd_s0. rewrite /b_valid /bpa. apply kv_addv_zero. }
    (* ---- +0xb4 c.lw a5,0(s1): the CHECKOUT, at this one instruction ---- *)
    iApply (wp_lw_au_s_sconf (dqm := DfracOwn 1) γ Φ true
              (mword_of_int (BD + 0xb4)) Ra5 Rs1 (mword_of_int 0 : mword 12)
              M (K - 6)%nat
              (fun v : mword 32 =>
                 (⌜v = (mword_of_int 0 : mword 32) \/ v = (mword_of_int 1 : mword 32)⌝ ∗
                  b_valid (bpa k) ↦₄ v ∗
                  b_dev (bpa k) ↦₄{DfracOwn (1/2)} dev ∗
                  ∃ bs : list (bv 8), buf_own (bpa k) bno (mword_of_int 0 : mword 32) bs)%I)
              (⊤ ∖ ↑minstretN ∖ ↑bioN)
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              ltac:(solve_ndisj)
              with "Hcg Hpc Hib4 [Hbown Hrtok Hrdev Hrbno] [-]").
    { iInv "Hesc" as ">Hbody" "Hclose".
      iDestruct (escrow_swap_checkout bn k q dev bno
                   with "Hbody Hbown Hrtok Hrdev Hrbno") as "[Hbody Hpark]".
      iDestruct "Hpark" as (vld bs) "(%Hpin & Hvld & Hbdev & Hbuf)".
      ...
      iModIntro. iSplitR; [by iPureIntro|]. iFrame "Hvld Hbdev". by iExists bs. }
    iIntros (vld) "Hcg Hpc (%Hpin & Hvld & Hbdev & Hbuf)".
    iDestruct "Hbuf" as (bs) "Hbuf".
    ...
    { rewrite /bd_regs. split_and!.
      ...
      - intros c Hcs N2 N8 N9 N18 N19 N4.
        rewrite /T1 upd_ne; [| apply not_eq_sym; apply is_cs_idx_true_neq;
                               [vm_compute; reflexivity | assumption]].
        exact (HMthr c Hcs N2 N8 N9 N18 N19 N4). }
    assert (Hppb6 : add_vec_int (mword_of_int (BD + 0xb4) : mword 64) 2 = mword_of_int (BD + 0xb6))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hppb6) in "Hpc".
    destruct Hpin as [Hz | Ho].
    - (* ============ b->valid = 0: the disk read ============ *)
      ...
      iApply (wp_cbeqz_taken_s_sconf γ Φ (mword_of_int (BD + 0xb6)) (mword_of_int 9 : mword 8)
                ...
                Hbeqz ltac:(vm_compute; reflexivity)
                with "Hcg Hpc Hib6 [-]").
      iNext. iIntros "Hcg Hpc".
      assert (Htgtc8 : add_vec (mword_of_int (BD + 0xb6) : mword 64)
                         (sign_extend' 64 (sign_extend' 13
                            (concat_vec (mword_of_int 9 : mword 8) ('b"0"))))
                       = mword_of_int (BD + 0xc8))
        by (apply bv_eq; vm_compute; reflexivity).
      ...
      assert (Hli0 : add_vec zero_reg (sign_extend' 64 (sign_extend' 12 (mword_of_int 0 : mword 6)))
        ...
        by (apply bv_eq; vm_compute; reflexivity).
      iApply (wp_cli_s_sconf γ Φ (mword_of_int (BD + 0xc8)) Ra1 (mword_of_int 0 : mword 6)
                (mword_of_int 0 : mword 64) T1 (K - 6)%nat
                ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate) Hli0
                with "Hcg Hpc Hic8 [-]").
      iIntros "Hcg Hpc".
      ...
      assert (Hppca : add_vec_int (mword_of_int (BD + 0xc8) : mword 64) 2 = mword_of_int (BD + 0xca))
        by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hppca) in "Hpc".
      (* +0xca c.mv a0,s1 *)
      iApply (wp_cmv_s_sconf γ Φ (mword_of_int (BD + 0xca)) Ra0 Rs1
      ...
      assert (Hppcc : add_vec_int (mword_of_int (BD + 0xca) : mword 64) 2 = mword_of_int (BD + 0xcc))
        by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hppcc) in "Hpc".
      (* ===== +0xcc jal ra,virtio_disk_rw ===== *)
      iApply (wp_jal_s_sconf γ Φ (mword_of_int (BD + 0xcc)) Rra (mword_of_int 11086 : mword 21)
      ...
      set (T4 := <[Regidx Rra := regval_into_reg
                    (add_vec_int (mword_of_int (BD + 0xcc) : mword 64) 4)]> T3).
      assert (Htgtrw : add_vec (mword_of_int (BD + 0xcc) : mword 64)
                         (sign_extend' 64 (mword_of_int 11086 : mword 21))
      ...
      assert (HT4tp : T4 !!! Regidx Rtp = cid_word).
      { rewrite (HT4thr Rtp ltac:(vm_compute; reflexivity)).
        rewrite /T1 upd_ne; [exact Htp | vm_compute; discriminate]. }
      assert (HT4ra : T4 !!! Regidx Rra = add_vec_int (mword_of_int (BD + 0xcc) : mword 64) 4)
      ...
      iApply (RW.wp_virtio_disk_rw_sconf γ Φ γs j γl γu γd γk pd pav pu T4
                (K - 6)%nat eb C bno (mword_of_int 0 : mword 32) bs bs_disk
                HKrw Hbno Hkdata HT4tp Hj Hgl Heb
                with "Hcg Hcnt Htext Hpc Hpanic Hprocs Hoctx Hvc Hdev Hgeom Hdlock [Hbuf] Hdisk [-]").
      { iEval (rewrite HT4a0). rewrite /bpa. iExact "Hbuf". }
      (* rw PARKS: it returns on hart [h2] under ghost [g2], so every leaf from
         +0xd0 on -- and the epilogue -- is applied at [(CID := h2)]. *)
      iIntros (h2 g2 mR) "%Hcs2 %Htp2 Hcg Hcnt Hpc Hoctx Hvc Hbuf Hdisk".
      (* [instr] is CID-indexed, so the post-park leaves need their decode
         facts AT the resuming hart. *)
      iPoseProof (bdi_d0 (CID := h2) with "Htext") as "Hid0".
      iPoseProof (bdi_d2 (CID := h2) with "Htext") as "Hid2".
      iPoseProof (bdi_d4 (CID := h2) with "Htext") as "Hid4".
      iEval (rewrite (bd_wr_false _ bs bs_disk HT4a1)) in "Hbuf".
      iEval (rewrite (bd_wr_false _ bs bs_disk HT4a1)) in "Hdisk".
      iEval (rewrite HT4a0) in "Hbuf".
      assert (Hpcd0 : ret_pc (T4 !!! Regidx Rra) = mword_of_int (BD + 0xd0)).
      { rewrite HT4ra. apply bv_eq; vm_compute; reflexivity. }
      iEval (rewrite Hpcd0) in "Hpc".
      assert (HmRs1 : mR !!! Regidx Rs1 = bnode k)
        by (rewrite (callee_saved_notp_lookup Hcs2 Rs1 ltac:(vm_compute; reflexivity)); exact HT4s1).
      (* +0xd0 c.li a5,1 *)
      assert (Hli1 : add_vec zero_reg (sign_extend' 64 (sign_extend' 12 (mword_of_int 1 : mword 6)))
                     = (mword_of_int 1 : mword 64))
        by (apply bv_eq; vm_compute; reflexivity).
      iApply (wp_cli_s_sconf (CID := h2) g2 Φ (mword_of_int (BD + 0xd0)) Ra5 (mword_of_int 1 : mword 6)
      ...
      assert (Hppd2 : add_vec_int (mword_of_int (BD + 0xd0) : mword 64) 2 = mword_of_int (BD + 0xd2))
        by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hppd2) in "Hpc".
      (* +0xd2 c.sw a5,0(s1) : b->valid = 1 (we own the cell outright) *)
      assert (Hva5 : add_vec (T5 !!! Regidx Rs1) (sign_extend' 64 (mword_of_int 0 : mword 12))
                     = b_valid (bpa k)).
      { rewrite HT5s1 bd_s0. rewrite /b_valid /bpa. apply kv_addv_zero. }
      iEval (rewrite -Hva5) in "Hvld".
      iApply (wp_csw_s_sconf (CID := h2) g2 Φ (mword_of_int (BD + 0xd2)) Ra5 Rs1 (mword_of_int 0 : mword 12)
                T5 (K - 6)%nat (vld : mword 32)
                with "Hcg Hpc Hid2 Hvld [-]").
      iIntros "Hcg Hpc Hvld".
      iEval (rewrite Hva5) in "Hvld".
      assert (Hstv : trunc32 (T5 !!! Regidx Ra5) = (mword_of_int 1 : mword 32)).
      { rewrite HT5a5. apply bv_eq; vm_compute; reflexivity. }
      iEval (rewrite Hstv) in "Hvld".
      assert (Hppd4 : add_vec_int (mword_of_int (BD + 0xd2) : mword 64) 2 = mword_of_int (BD + 0xd4))
        by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hppd4) in "Hpc".
      (* +0xd4 c.j -0x1c : back to the epilogue join *)
      iApply (wp_cj_s_sconf (CID := h2) g2 Φ (mword_of_int (BD + 0xd4))
                (sign_extend' 21 (concat_vec (mword_of_int 2034 : mword 11) ('b"0")))
                T5 (K - 6)%nat ltac:(vm_compute; reflexivity)
                with "Hcg Hpc Hid4 [-]").
      iApply bi.later_intro. iIntros "Hcg Hpc".
      assert (Htgtb8 : add_vec (mword_of_int (BD + 0xd4) : mword 64)
                         (sign_extend' 64 (sign_extend' 21
                            (concat_vec (mword_of_int 2034 : mword 11) ('b"0"))))
                       = mword_of_int (BD + 0xb8))
        by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Htgtb8) in "Hpc".
      assert (HT5regs : bd_regs m T5).
      { rewrite /bd_regs. split_and!.
        - rewrite /T5 upd_ne; [| vm_compute; discriminate].
          rewrite (callee_saved_notp_lookup Hcs2 csp_rs1 ltac:(vm_compute; reflexivity)).
          rewrite (HT4thr csp_rs1 ltac:(vm_compute; reflexivity)).
          rewrite /T1 upd_ne; [exact HMsp | vm_compute; discriminate].
        - rewrite /T5 upd_ne; [| vm_compute; discriminate].
          rewrite (callee_saved_notp_lookup Hcs2 Rs2 ltac:(vm_compute; reflexivity)).
          rewrite (HT4thr Rs2 ltac:(vm_compute; reflexivity)).
          rewrite /T1 upd_ne; [exact HMs2 | vm_compute; discriminate].
        - rewrite /T5 upd_ne; [| vm_compute; discriminate].
          rewrite (callee_saved_notp_lookup Hcs2 Rs3 ltac:(vm_compute; reflexivity)).
          rewrite (HT4thr Rs3 ltac:(vm_compute; reflexivity)).
          rewrite /T1 upd_ne; [exact HMs3 | vm_compute; discriminate].
        - intros c Hcs N2 N8 N9 N18 N19 N4.
          rewrite /T5 upd_ne; [| apply not_eq_sym; apply is_cs_idx_true_neq;
                                 [vm_compute; reflexivity | assumption]].
          rewrite (callee_saved_notp_lookup Hcs2 c (bd_notp_of c Hcs N4)).
          rewrite (HT4thr c Hcs).
          rewrite /T1 upd_ne; [| apply not_eq_sym; apply is_cs_idx_true_neq;
                                 [vm_compute; reflexivity | assumption]].
          exact (HMthr c Hcs N2 N8 N9 N18 N19 N4). }
      assert (HT5tp : T5 !!! Regidx Rtp = cid_word_of h2)
        by (rewrite /T5 upd_ne; [exact Htp2 | vm_compute; discriminate]).
      iApply (bread_epi (CID := h2) g2 Φ γs j γd bn k pidv dev bno dq m T5 K eb C bs_disk bs_disk
                HK HT5regs HT5s1 HT5tp
                with "Hcg Htext Hpc Hframe Hcnt Hoctx Hvc Hppid [Hstok Hpid Hvld Hbdev Hbuf] Hdisk Hcont").
      rewrite /bio_locked /bpa.
      iSplitR; [by iPureIntro|].
      iFrame "Hstok Hpid Hvld Hbdev Hbuf".
    - (* ============ b->valid = 1: straight to the epilogue ============ *)
      iEval (rewrite Ho) in "Hvld".
      assert (Hbeqz : eq_vec (T1 !!! Regidx Ra5) zero_reg = false).
      { rewrite HT1a5 Ho. exact bd_valid1_nonzero. }
      iApply (wp_cbeqz_fall_s_sconf γ Φ (mword_of_int (BD + 0xb6)) (mword_of_int 9 : mword 8)
                (Cregidx (mword_of_int 7)) Ra5 T1 (K - 6)%nat
                ltac:(vm_compute; reflexivity) ltac:(vm_compute; discriminate) Hbeqz
                with "Hcg Hpc Hib6 [-]").
      iIntros "Hcg Hpc".
      assert (Hppb8 : add_vec_int (mword_of_int (BD + 0xb6) : mword 64) 2 = mword_of_int (BD + 0xb8))
        by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hppb8) in "Hpc".
      assert (HT1tp : T1 !!! Regidx Rtp = cid_word)
        by (rewrite /T1 upd_ne; [exact Htp | vm_compute; discriminate]).
      iApply (bread_epi γ Φ γs j γd bn k pidv dev bno dq m T1 K eb C bs bs_disk
                HK HT1regs HT1s1 HT1tp
                with "Hcg Htext Hpc Hframe Hcnt Hoctx Hvc Hppid [Hstok Hpid Hvld Hbdev Hbuf] Hdisk Hcont").
      rewrite /bio_locked /bpa.
      iSplitR; [by iPureIntro|].
      iFrame "Hstok Hpid Hvld Hbdev Hbuf".
...
Section ProofBread.
  ...
  Local Lemma bread_hit (γ : gname) (Φ : mval -> iProp Σ)
      (γs : list gname) (j : nat) (γl : gname)
      (γu : uart_names) (γd : disk_names) (γk : gname)
      (pd pav pu : mword 64)
      (bn : bio_names) (k : nat)
    ...
    sie_cap_gpr γ M (K - 6)%nat -∗
    kernel_text -∗
    pc_is (mword_of_int (BD + 0x48) : mword 64) -∗
    panic_wp_any -∗
    bio_ctx bn -∗
    bd_frame m -∗
    ...
    bcache_scan bn Mg ord devs bnos -∗
    bslot bn -∗
    procs_inv Φ γs -∗
    own_ctx (p_context (proc_addr j)) -∗
    ▷ sched_vc γ Φ γs (a_cpu_ctx cid_word) (proc_addr j) -∗
    ...
    WP (Loop : expr riscv_lang) {{ Φ }}.
  Proof.
    intros HK Hbno Hk Hdevs Hbnos Htp Hj Hgl Hregs HMs1 Heb.
    pose proof Hregs as (HMsp & HMs2 & HMs3 & HMthr).
    iIntros "Hcg #Htext Hpc #Hpanic #Hbio Hframe Hcnt Hpay Htok Hscan Hbslot".
    iIntros "#Hprocs Hoctx Hvc Hppid #Hdev #Hgeom #Hdlock Hdisk Hcont".
    ...
    iDestruct (bcache_scan_incr bn Mg ord devs bnos k Hk with "Hscan Hbslot")
      as (cw) "[Hcell Hclose]".
    (* ---- +0x48 c.lw a5,64(s1) ---- *)
    assert (Hpa : add_vec (M !!! Regidx Rs1) (sign_extend' 64 (mword_of_int 64 : mword 12))
                  = brefcnt k).
    { rewrite HMs1 bd_s64. rewrite /brefcnt /bpa /pa_add /add_vec_int. reflexivity. }
    iEval (rewrite -Hpa) in "Hcell".
    iApply (wp_clw_s_sconf γ Φ (mword_of_int (BD + 0x48)) Ra5 Rs1 (mword_of_int 64 : mword 12)
              M (K - 6)%nat (cw : mword 32)
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
    ...
    assert (Hpp4a : add_vec_int (mword_of_int (BD + 0x48) : mword 64) 2 = mword_of_int (BD + 0x4a))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp4a) in "Hpc".
    (* ---- +0x4a c.addiw a5,a5,1 ---- *)
    iApply (wp_caddiw_s_sconf γ Φ (mword_of_int (BD + 0x4a)) Ra5 (mword_of_int 1 : mword 6)
              H1 (K - 6)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi4a [-]").
    iIntros "Hcg Hpc".
    ...
    assert (Hpp4c : add_vec_int (mword_of_int (BD + 0x4a) : mword 64) 2 = mword_of_int (BD + 0x4c))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp4c) in "Hpc".
    (* ---- +0x4c c.sw a5,64(s1) ---- *)
    assert (Hpa2 : add_vec (H2 !!! Regidx Rs1) (sign_extend' 64 (mword_of_int 64 : mword 12))
                   = brefcnt k).
    { rewrite HH2s1 bd_s64. rewrite /brefcnt /bpa /pa_add /add_vec_int. reflexivity. }
    iEval (rewrite -Hpa2) in "Hcell".
    iApply (wp_csw_s_sconf γ Φ (mword_of_int (BD + 0x4c)) Ra5 Rs1 (mword_of_int 64 : mword 12)
              H2 (K - 6)%nat (cw : mword 32)
              with "Hcg Hpc Hi4c Hcell [-]").
    iIntros "Hcg Hpc Hcell".
    iEval (rewrite Hpa2) in "Hcell".
    assert (Hstv : trunc32 (H2 !!! Regidx Ra5) = incr32 (cw : mword 32)).
    { rewrite /H2 upd_eq. unfold regval_into_reg. rewrite HH1a5. reflexivity. }
    ...
    assert (Hpp4e : add_vec_int (mword_of_int (BD + 0x4c) : mword 64) 2 = mword_of_int (BD + 0x4e))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp4e) in "Hpc".
    (* ---- +0x4e / +0x52 : a0 := &bcache ; +0x56 jal release ---- *)
    iApply (wp_auipc_s_sconf γ Φ (mword_of_int (BD + 0x4e)) Ra0 (mword_of_int 0x15 : mword 20)
              H2 (K - 6)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi4e [-]").
    iIntros "Hcg Hpc".
    set (H3 := <[Regidx Ra0 := regval_into_reg
                  (add_vec (mword_of_int (BD + 0x4e) : mword 64)
                     (auipc_off (mword_of_int 0x15 : mword 20)))]> H2).
    assert (Hpp52 : add_vec_int (mword_of_int (BD + 0x4e) : mword 64) 4 = mword_of_int (BD + 0x52))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp52) in "Hpc".
    iApply (wp_addi4_s_sconf γ Φ (mword_of_int (BD + 0x52)) Ra0 Ra0 (mword_of_int 1548 : mword 12)
              H3 (K - 6)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi52 [-]").
    iIntros "Hcg Hpc".
    set (H4 := <[Regidx Ra0 := regval_into_reg
                  (add_vec (H3 !!! Regidx Ra0) (sign_extend' 64 (mword_of_int 1548 : mword 12)))]> H3).
    assert (HH4a0 : H4 !!! Regidx Ra0 = bcache_addr).
    { rewrite /H4 upd_eq /H3 upd_eq. rewrite /bcache_addr.
      apply bv_eq; vm_compute; reflexivity. }
    assert (Hpp56 : add_vec_int (mword_of_int (BD + 0x52) : mword 64) 4 = mword_of_int (BD + 0x56))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp56) in "Hpc".
    iApply (wp_jal_s_sconf γ Φ (mword_of_int (BD + 0x56)) Rra (mword_of_int 2089220 : mword 21)
              H4 (K - 6)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              ltac:(vm_compute; reflexivity) with "Hcg Hpc Hi56 [-]").
    iIntros "Hcg Hpc".
    set (H5 := <[Regidx Rra := regval_into_reg
                  (add_vec_int (mword_of_int (BD + 0x56) : mword 64) 4)]> H4).
    assert (Htgtrel : add_vec (mword_of_int (BD + 0x56) : mword 64)
                        (sign_extend' 64 (mword_of_int 2089220 : mword 21))
    ...
    assert (HH5tp : H5 !!! Regidx Rtp = cid_word)
      by (rewrite (HH5thr Rtp ltac:(vm_compute; reflexivity)); exact Htp).
    assert (HH5ra : H5 !!! Regidx Rra = add_vec_int (mword_of_int (BD + 0x56) : mword 64) 4)
      by (rewrite /H5; apply upd_eq).
    iApply (R.wp_release_sconf γ Φ (bn_lk bn) bcache_addr "bcache"%string (bcache_res bn) H5
              0%nat eb (proc_addr j) C (K - 6)%nat
              ltac:(rewrite HH5a0; apply bv_eq; vm_compute; reflexivity)
              HH5tp ltac:(unfold K_bread in HK; lia)
              with "Hcg Htext Hpc [Hlock] Htok HRres Hcnt Hpay [-]").
    { iExact "Hlock". }
    iIntros (mr) "Hcg Hpc %Hrelpins Hcnt".
    assert (Hpc5a : ret_pc (H5 !!! Regidx Rra) = mword_of_int (BD + 0x5a)).
    ...
    iApply (wp_addi4_s_sconf γ Φ (mword_of_int (BD + 0x5a)) Ra0 Rs1 (mword_of_int 16 : mword 12)
              mr (K - 6)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi5a [-]").
    iIntros "Hcg Hpc".
    ...
    assert (Hpp5e : add_vec_int (mword_of_int (BD + 0x5a) : mword 64) 4 = mword_of_int (BD + 0x5e))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp5e) in "Hpc".
    (* ---- +0x5e jal ra,acquiresleep ---- *)
    iApply (wp_jal_s_sconf γ Φ (mword_of_int (BD + 0x5e)) Rra (mword_of_int 4920 : mword 21)
              H6 (K - 6)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              ltac:(vm_compute; reflexivity) with "Hcg Hpc Hi5e [-]").
    iIntros "Hcg Hpc".
    set (H7 := <[Regidx Rra := regval_into_reg
                  (add_vec_int (mword_of_int (BD + 0x5e) : mword 64) 4)]> H6).
    assert (Htgtasl : add_vec (mword_of_int (BD + 0x5e) : mword 64)
    ...
    assert (HH7tp : H7 !!! Regidx Rtp = cid_word)
      by (rewrite (HH7thr Rtp ltac:(vm_compute; reflexivity)); exact Htp).
    assert (HH7ra : H7 !!! Regidx Rra = add_vec_int (mword_of_int (BD + 0x5e) : mword 64) 4)
      by (rewrite /H7; apply upd_eq).
    iApply (ASL.wp_acquiresleep_sconf (dq := dq) γ Φ γs j (fst (bn_slk bn k)) (snd (bn_slk bn k))
              "buffer"%string (bown bn k) H7 pidv (K - 6)%nat eb C
              HH7tp Hj ltac:(unfold K_bread in HK; lia) Heb
              with "Hcg Hcnt Htext Hpc [] Hpanic Hppid Hprocs Hoctx Hvc [-]").
    { iEval (rewrite HH7a0). iExact "Hslk". }
    (* acquiresleep PARKS: it returns on hart [h] under ghost [g], so the c.j
       into the shared tail -- and the tail itself -- run there. *)
    iIntros (h g mf) "%Hcsasl %Hmftp Hcg Hcnt Hpc Hstok Hpid Hbown Hppid Hoctx Hvc".
    iEval (rewrite HH7a0) in "Hpid".
    iPoseProof (bdi_62 (CID := h) with "Htext") as "Hi62".
    assert (Hpc62 : ret_pc (H7 !!! Regidx Rra) = mword_of_int (BD + 0x62)).
    { rewrite HH7ra. apply bv_eq; vm_compute; reflexivity. }
    iEval (rewrite Hpc62) in "Hpc".
    assert (HmfX : forall c : mword 5, is_cs_idx c = true -> c <> Rtp ->
                     mf !!! Regidx c = M !!! Regidx c).
    { intros c Hcs Hne.
      rewrite (callee_saved_notp_lookup Hcsasl c (bd_notp_of c Hcs Hne)).
      exact (HH7thr c Hcs). }
    assert (Hmfs1 : mf !!! Regidx Rs1 = bnode k)
      by (rewrite (HmfX Rs1 ltac:(vm_compute; reflexivity) ltac:(vm_compute; discriminate)); exact HMs1).
    assert (Hmfregs : bd_regs m mf).
    { rewrite /bd_regs. split_and!.
      - rewrite (HmfX csp_rs1 ltac:(vm_compute; reflexivity) ltac:(vm_compute; discriminate)). exact HMsp.
      - rewrite (HmfX Rs2 ltac:(vm_compute; reflexivity) ltac:(vm_compute; discriminate)). exact HMs2.
      - rewrite (HmfX Rs3 ltac:(vm_compute; reflexivity) ltac:(vm_compute; discriminate)). exact HMs3.
      - intros c Hcs N2 N8 N9 N18 N19 N4.
        rewrite (HmfX c Hcs N4). exact (HMthr c Hcs N2 N8 N9 N18 N19 N4). }
    (* ---- +0x62 c.j +0x52 : into the shared tail ---- *)
    iApply (wp_cj_s_sconf (CID := h) g Φ (mword_of_int (BD + 0x62))
              (sign_extend' 21 (concat_vec (mword_of_int 41 : mword 11) ('b"0")))
              mf (K - 6)%nat ltac:(vm_compute; reflexivity)
              with "Hcg Hpc Hi62 [-]").
    iApply bi.later_intro. iIntros "Hcg Hpc".
    assert (Htgtb4 : add_vec (mword_of_int (BD + 0x62) : mword 64)
                       (sign_extend' 64 (sign_extend' 21
                          (concat_vec (mword_of_int 41 : mword 11) ('b"0"))))
                     = mword_of_int (BD + 0xb4))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Htgtb4) in "Hpc".
    iApply (bread_tail (CID := h) g Φ γs j γl γu γd γk pd pav pu bn k qref pidv dev bno dq
              m mf K eb C bs_disk HK Hbno Hk Hmftp Hj Hgl Hmfregs Hmfs1 Heb
              with "Hcg Htext Hpc Hpanic Hesc Hframe Hcnt Hprocs Hoctx Hvc Hppid
                    Hdev Hgeom Hdlock Hstok Hpid Hbown Href Hdisk Hcont").
  Qed.
  ...
  Local Lemma bread_recyc (γ : gname) (Φ : mval -> iProp Σ)
      (γs : list gname) (j : nat) (γl : gname)
      (γu : uart_names) (γd : disk_names) (γk : gname)
      (pd pav pu : mword 64)
      (bn : bio_names) (k : nat)
    ...
    sie_cap_gpr γ M (K - 6)%nat -∗
    kernel_text -∗
    pc_is (mword_of_int (BD + 0x90) : mword 64) -∗
    panic_wp_any -∗
    bio_ctx bn -∗
    bd_frame m -∗
    cpu_own γ 1 eb (proc_addr j) C -∗
    ...
    WP (Loop : expr riscv_lang) {{ Φ }}.
  Proof.
    intros HK Hbno Hk HMk Ha0 Ha1 Htp Hj Hgl Hregs HMs1 Heb.
    pose proof Hregs as (HMsp & HMs2 & HMs3 & HMthr).
    iIntros "Hcg #Htext Hpc #Hpanic #Hbio Hframe Hcnt Hpay Htok Hscan Hbslot".
    iIntros "#Hprocs Hoctx Hvc Hppid #Hdev #Hgeom #Hdlock Hdisk Hcont".
    iDestruct (bio_ctx_lock with "Hbio") as "#Hlock".
    ...
    assert (Hadev : add_vec (M !!! Regidx Rs1) (sign_extend' 64 (mword_of_int 8 : mword 12))
                    = b_dev (bpa k)).
    { rewrite HMs1 bd_s8. rewrite /b_dev /bpa /pa_add /add_vec_int. reflexivity. }
    assert (Habno : add_vec (M !!! Regidx Rs1) (sign_extend' 64 (mword_of_int 12 : mword 12))
                    = b_blockno (bpa k)).
    { rewrite HMs1 bd_s12. rewrite /b_blockno /bpa /pa_add /add_vec_int. reflexivity. }
    assert (Hava : add_vec (M !!! Regidx Rs1) (sign_extend' 64 (mword_of_int 0 : mword 12))
                   = b_valid (bpa k)).
    { rewrite HMs1 bd_s0. rewrite /b_valid /bpa. apply kv_addv_zero. }
    (* the stored words: the two arguments arrive sign-extended, so the
       truncation the store performs gives the arguments back *)
    assert (Hsvdev : trunc32 (M !!! Regidx Rs2) = dev)
      by (rewrite HMs2 Ha0; apply trunc32_sext).
    assert (Hsvbno : trunc32 (M !!! Regidx Rs3) = bno)
      by (rewrite HMs3 Ha1; apply trunc32_sext).
    iDestruct (sie_cap_gpr_x0 γ M (K - 6)%nat Rz ltac:(vm_compute; reflexivity) with "Hcg")
      as "[%Hx0 Hcg]".
    assert (Hsvz : trunc32 (M !!! Regidx Rz) = (mword_of_int 0 : mword 32))
      by (rewrite Hx0; exact bd_trunc32_zero).
    ...
    iDestruct (bcache_scan_recycle bn Mg ord devs bnos k Hk HMk with "Hscan Hbslot")
      as "(Hauth & Hcell & Hdevs & Hbnos & Hclose)".
    (* ---- +0x90 sw s2,8(s1) : b->dev = dev ---- *)
    iApply (wp_sw_au_s_sconf γ Φ false (mword_of_int (BD + 0x90)) Rs2 Rs1
              (mword_of_int 8 : mword 12) M (K - 6)%nat
              (own (bn_auth bn) (● Mg) ∗ b_dev (bpa k) ↦₄{DfracOwn (1/2)} dev)%I
              (⊤ ∖ ↑minstretN ∖ ↑bioN) ltac:(solve_ndisj)
              with "Hcg Hpc Hi90 [Hauth Hdevs] [-]").
    { iInv "Hesc" as ">Hbody" "Hclose2".
      iDestruct (escrow_free_dev bn k Mg (devs k) HMk with "Hauth Hbody Hdevs")
      ...
      iDestruct ("Hback" $! dev with "Hfull") as "[Hbody Hhalf]".
      iMod ("Hclose2" with "[Hbody]") as "_". { iNext. iExact "Hbody". }
      iModIntro. iFrame "Hauth Hhalf". }
    iIntros "Hcg Hpc [Hauth Hdevs]".
    assert (Hpp94 : add_vec_int (mword_of_int (BD + 0x90) : mword 64) 4 = mword_of_int (BD + 0x94))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp94) in "Hpc".
    (* ---- +0x94 sw s3,12(s1) : b->blockno = blockno ---- *)
    iApply (wp_sw_au_s_sconf γ Φ false (mword_of_int (BD + 0x94)) Rs3 Rs1
              (mword_of_int 12 : mword 12) M (K - 6)%nat
              (own (bn_auth bn) (● Mg) ∗ b_blockno (bpa k) ↦₄{DfracOwn (1/2)} bno)%I
              (⊤ ∖ ↑minstretN ∖ ↑bioN) ltac:(solve_ndisj)
              with "Hcg Hpc Hi94 [Hauth Hbnos] [-]").
    { iInv "Hesc" as ">Hbody" "Hclose2".
      iDestruct (escrow_free_bno bn k Mg (bnos k) HMk with "Hauth Hbody Hbnos")
        as "(Hauth & Hfull & Hback)".
      iModIntro. iExists (bnos k).
      iEval (rewrite -Habno) in "Hfull". iFrame "Hfull".
      iIntros "Hfull". iEval (rewrite Habno Hsvbno) in "Hfull".
      iDestruct ("Hback" $! bno with "Hfull") as "[Hbody Hhalf]".
      iMod ("Hclose2" with "[Hbody]") as "_". { iNext. iExact "Hbody". }
      iModIntro. iFrame "Hauth Hhalf". }
    iIntros "Hcg Hpc [Hauth Hbnos]".
    assert (Hpp98 : add_vec_int (mword_of_int (BD + 0x94) : mword 64) 4 = mword_of_int (BD + 0x98))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp98) in "Hpc".
    (* ---- +0x98 sw zero,0(s1) : b->valid = 0 ---- *)
    iApply (wp_sw_au_s_sconf γ Φ false (mword_of_int (BD + 0x98)) Rz Rs1
              (mword_of_int 0 : mword 12) M (K - 6)%nat
              (own (bn_auth bn) (● Mg))%I
              (⊤ ∖ ↑minstretN ∖ ↑bioN) ltac:(solve_ndisj)
              with "Hcg Hpc Hi98 [Hauth] [-]").
    { iInv "Hesc" as ">Hbody" "Hclose2".
      iDestruct (escrow_free_valid bn k Mg HMk with "Hauth Hbody")
        as "(Hauth & Hvld & Hback)".
      iDestruct "Hvld" as (vld) "Hvld".
      iModIntro. iExists vld.
      iEval (rewrite -Hava) in "Hvld". iFrame "Hvld".
      iIntros "Hvld". iEval (rewrite Hava Hsvz) in "Hvld".
      iDestruct ("Hback" with "Hvld") as "Hbody".
      iMod ("Hclose2" with "[Hbody]") as "_". { iNext. iExact "Hbody". }
      iModIntro. iFrame "Hauth". }
    iIntros "Hcg Hpc Hauth".
    assert (Hpp9c : add_vec_int (mword_of_int (BD + 0x98) : mword 64) 4 = mword_of_int (BD + 0x9c))
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    assert (Hli1 : add_vec zero_reg (sign_extend' 64 (sign_extend' 12 (mword_of_int 1 : mword 6)))
                   = (mword_of_int 1 : mword 64))
      by (apply bv_eq; vm_compute; reflexivity).
    iApply (wp_cli_s_sconf γ Φ (mword_of_int (BD + 0x9c)) Ra5 (mword_of_int 1 : mword 6)
              (mword_of_int 1 : mword 64) M (K - 6)%nat
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate) Hli1
              with "Hcg Hpc Hi9c [-]").
    iIntros "Hcg Hpc".
    ...
    assert (Hpp9e : add_vec_int (mword_of_int (BD + 0x9c) : mword 64) 2 = mword_of_int (BD + 0x9e))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp9e) in "Hpc".
    assert (Harc : add_vec (C1 !!! Regidx Rs1) (sign_extend' 64 (mword_of_int 64 : mword 12))
                   = brefcnt k).
    { rewrite HC1s1 bd_s64. rewrite /brefcnt /bpa /pa_add /add_vec_int. reflexivity. }
    iEval (rewrite -Harc) in "Hcell".
    iApply (wp_csw_s_sconf γ Φ (mword_of_int (BD + 0x9e)) Ra5 Rs1 (mword_of_int 64 : mword 12)
              C1 (K - 6)%nat (mword_of_int 0 : mword 32)
              with "Hcg Hpc Hi9e Hcell [-]").
    iIntros "Hcg Hpc Hcell".
    iEval (rewrite Harc) in "Hcell".
    assert (Hstv : trunc32 (C1 !!! Regidx Ra5) = (mword_of_int 1 : mword 32)).
    { rewrite HC1a5. apply bv_eq; vm_compute; reflexivity. }
    iEval (rewrite Hstv) in "Hcell".
    iMod ("Hclose" $! dev bno with "Hauth Hcell Hdevs Hbnos") as "[HRres Href]".
    assert (Hppa0 : add_vec_int (mword_of_int (BD + 0x9e) : mword 64) 2 = mword_of_int (BD + 0xa0))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hppa0) in "Hpc".
    (* ---- +0xa0 / +0xa4 : a0 := &bcache ; +0xa8 jal release ---- *)
    iApply (wp_auipc_s_sconf γ Φ (mword_of_int (BD + 0xa0)) Ra0 (mword_of_int 0x15 : mword 20)
              C1 (K - 6)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hia0 [-]").
    iIntros "Hcg Hpc".
    set (C2 := <[Regidx Ra0 := regval_into_reg
                  (add_vec (mword_of_int (BD + 0xa0) : mword 64)
                     (auipc_off (mword_of_int 0x15 : mword 20)))]> C1).
    assert (Hppa4 : add_vec_int (mword_of_int (BD + 0xa0) : mword 64) 4 = mword_of_int (BD + 0xa4))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hppa4) in "Hpc".
    iApply (wp_addi4_s_sconf γ Φ (mword_of_int (BD + 0xa4)) Ra0 Ra0 (mword_of_int 1466 : mword 12)
              C2 (K - 6)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hia4 [-]").
    iIntros "Hcg Hpc".
    set (C3 := <[Regidx Ra0 := regval_into_reg
                  (add_vec (C2 !!! Regidx Ra0) (sign_extend' 64 (mword_of_int 1466 : mword 12)))]> C2).
    assert (HC3a0 : C3 !!! Regidx Ra0 = bcache_addr).
    { rewrite /C3 upd_eq /C2 upd_eq. rewrite /bcache_addr.
      apply bv_eq; vm_compute; reflexivity. }
    assert (Hppa8 : add_vec_int (mword_of_int (BD + 0xa4) : mword 64) 4 = mword_of_int (BD + 0xa8))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hppa8) in "Hpc".
    iApply (wp_jal_s_sconf γ Φ (mword_of_int (BD + 0xa8)) Rra (mword_of_int 2089138 : mword 21)
              C3 (K - 6)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              ltac:(vm_compute; reflexivity) with "Hcg Hpc Hia8 [-]").
    iIntros "Hcg Hpc".
    set (C4 := <[Regidx Rra := regval_into_reg
                  (add_vec_int (mword_of_int (BD + 0xa8) : mword 64) 4)]> C3).
    assert (Htgtrel : add_vec (mword_of_int (BD + 0xa8) : mword 64)
                        (sign_extend' 64 (mword_of_int 2089138 : mword 21))
                      = mword_of_int KernelSyms.release)
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Htgtrel) in "Hpc".
    ...
    assert (HC4tp : C4 !!! Regidx Rtp = cid_word)
      by (rewrite (HC4thr Rtp ltac:(vm_compute; reflexivity)); exact Htp).
    assert (HC4ra : C4 !!! Regidx Rra = add_vec_int (mword_of_int (BD + 0xa8) : mword 64) 4)
      by (rewrite /C4; apply upd_eq).
    iApply (R.wp_release_sconf γ Φ (bn_lk bn) bcache_addr "bcache"%string (bcache_res bn) C4
              0%nat eb (proc_addr j) C (K - 6)%nat
              ltac:(rewrite HC4a0; apply bv_eq; vm_compute; reflexivity)
              HC4tp ltac:(unfold K_bread in HK; lia)
              with "Hcg Htext Hpc [Hlock] Htok HRres Hcnt Hpay [-]").
    { iExact "Hlock". }
    iIntros (mr) "Hcg Hpc %Hrelpins Hcnt".
    assert (Hpcac : ret_pc (C4 !!! Regidx Rra) = mword_of_int (BD + 0xac)).
    ...
    iApply (wp_addi4_s_sconf γ Φ (mword_of_int (BD + 0xac)) Ra0 Rs1 (mword_of_int 16 : mword 12)
              mr (K - 6)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hiac [-]").
    iIntros "Hcg Hpc".
    ...
    assert (Hppb0 : add_vec_int (mword_of_int (BD + 0xac) : mword 64) 4 = mword_of_int (BD + 0xb0))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hppb0) in "Hpc".
    iApply (wp_jal_s_sconf γ Φ (mword_of_int (BD + 0xb0)) Rra (mword_of_int 4838 : mword 21)
              C5 (K - 6)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              ltac:(vm_compute; reflexivity) with "Hcg Hpc Hib0 [-]").
    iIntros "Hcg Hpc".
    set (C6 := <[Regidx Rra := regval_into_reg
                  (add_vec_int (mword_of_int (BD + 0xb0) : mword 64) 4)]> C5).
    assert (Htgtasl : add_vec (mword_of_int (BD + 0xb0) : mword 64)
                        (sign_extend' 64 (mword_of_int 4838 : mword 21))
                      = mword_of_int KernelSyms.acquiresleep)
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Htgtasl) in "Hpc".
    ...
    assert (HC6tp : C6 !!! Regidx Rtp = cid_word)
      by (rewrite (HC6thr Rtp ltac:(vm_compute; reflexivity)); exact Htp).
    assert (HC6ra : C6 !!! Regidx Rra = add_vec_int (mword_of_int (BD + 0xb0) : mword 64) 4)
      by (rewrite /C6; apply upd_eq).
    iApply (ASL.wp_acquiresleep_sconf (dq := dq) γ Φ γs j (fst (bn_slk bn k)) (snd (bn_slk bn k))
              "buffer"%string (bown bn k) C6 pidv (K - 6)%nat eb C
              HC6tp Hj ltac:(unfold K_bread in HK; lia) Heb
              with "Hcg Hcnt Htext Hpc [] Hpanic Hppid Hprocs Hoctx Hvc [-]").
    { iEval (rewrite HC6a0). iExact "Hslk". }
    (* acquiresleep PARKS: it returns on hart [h] under ghost [g]. *)
    iIntros (h g mf) "%Hcsasl %Hmftp Hcg Hcnt Hpc Hstok Hpid Hbown Hppid Hoctx Hvc".
    iEval (rewrite HC6a0) in "Hpid".
    assert (Hpcb4 : ret_pc (C6 !!! Regidx Rra) = mword_of_int (BD + 0xb4)).
    ...
    { rewrite /bd_regs. split_and!.
      ...
      - intros c Hcs N2 N8 N9 N18 N19 N4.
        rewrite (HmfX c Hcs N4). exact (HMthr c Hcs N2 N8 N9 N18 N19 N4). }
    iApply (bread_tail (CID := h) g Φ γs j γl γu γd γk pd pav pu bn k (1/4)%Qp pidv dev bno dq
              m mf K eb C bs_disk HK Hbno Hk Hmftp Hj Hgl Hmfregs Hmfs1 Heb
              with "Hcg Htext Hpc Hpanic Hesc Hframe Hcnt Hprocs Hoctx Hvc Hppid
                    Hdev Hgeom Hdlock Hstok Hpid Hbown Href Hdisk Hcont").
  Qed.
  ...
  Local Lemma bread_bloop (γ : gname) (Φ : mval -> iProp Σ)
      (γs : list gname) (j : nat) (γl : gname)
      (γu : uart_names) (γd : disk_names) (γk : gname)
      (pd pav pu : mword 64)
      (bn : bio_names)
    ...
    sie_cap_gpr γ M (K - 6)%nat -∗
    kernel_text -∗
    pc_is (mword_of_int (BD + 0x7a) : mword 64) -∗
    panic_wp_any -∗
    bio_ctx bn -∗
    bd_frame m -∗
    cpu_own γ 1 eb (proc_addr j) C -∗
    ...
    WP (Loop : expr riscv_lang) {{ Φ }}.
  Proof.
    intros HK Hbno Ha0 Ha1 Hj Hgl.
    induction n as [|n IH];
      intros pre post M Hlen Hord Hne Hregs Htp HMs1 HMa4 Heb.
    ...
    rewrite /bcache_scan.
    iDestruct "Hscan" as "(Hauth & Hsauth & %Hdom & %Hordp & Hlru & Hslots)".
    ...
    iDestruct (bio_slot_refcnt_acc bn Mg kk (devs kk) (bnos kk) with "Hslot")
      as (cw) "(Hcell & %Htie & Hbackslot)".
    assert (Hpa : add_vec (M !!! Regidx Rs1) (sign_extend' 64 (mword_of_int 64 : mword 12))
                  = brefcnt kk).
    { rewrite Hcur bd_s64. rewrite /brefcnt /bpa /pa_add /add_vec_int. reflexivity. }
    iEval (rewrite -Hpa) in "Hcell".
    iApply (wp_clw_s_sconf γ Φ (mword_of_int (BD + 0x7a)) Ra5 Rs1 (mword_of_int 64 : mword 12)
              M (K - 6)%nat (cw : mword 32)
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
    ...
    assert (HB1tp : B1 !!! Regidx Rtp = cid_word)
      by (rewrite /B1 upd_ne; [exact Htp | vm_compute; discriminate]).
    ...
    { rewrite /bd_regs. split_and!.
      ...
      - intros c Hcs N2 N8 N9 N18 N19 N4.
        rewrite /B1 upd_ne; [| apply not_eq_sym; apply is_cs_idx_true_neq;
                               [vm_compute; reflexivity | assumption]].
        exact (HMthr c Hcs N2 N8 N9 N18 N19 N4). }
    assert (Hpp7c : add_vec_int (mword_of_int (BD + 0x7a) : mword 64) 2 = mword_of_int (BD + 0x7c))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp7c) in "Hpc".
    destruct Htie as [[Hwz HMkNone] | [Hwnz _]].
    - (* ======== refcnt == 0: this buffer is free, RECYCLE it ======== *)
      assert (Hbeqz : eq_vec (B1 !!! Regidx Ra5) zero_reg = true)
        by (rewrite HB1a5; exact Hwz).
      iApply (wp_cbeqz_taken_s_sconf γ Φ (mword_of_int (BD + 0x7c)) (mword_of_int 10 : mword 8)
                (Cregidx (mword_of_int 7)) Ra5 B1 (K - 6)%nat
                ltac:(vm_compute; reflexivity) ltac:(vm_compute; discriminate)
                Hbeqz ltac:(vm_compute; reflexivity)
                with "Hcg Hpc Hi7c [-]").
      iNext. iIntros "Hcg Hpc".
      assert (Htgt90 : add_vec (mword_of_int (BD + 0x7c) : mword 64)
                         (sign_extend' 64 (sign_extend' 13
                            (concat_vec (mword_of_int 10 : mword 8) ('b"0"))))
                       = mword_of_int (BD + 0x90))
        by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Htgt90) in "Hpc".
      iAssert (bcache_scan bn Mg ord devs bnos)
        with "[Hauth Hsauth Hlru Hslots]" as "Hscan".
      { rewrite /bcache_scan. iFrame "Hauth Hsauth".
        iSplitR; [iPureIntro; exact Hdom|].
        iSplitR; [iPureIntro; exact Hordp|].
        iFrame "Hlru Hslots". }
      iApply (bread_recyc γ Φ γs j γl γu γd γk pd pav pu bn kk Mg ord devs bnos
                pidv dev bno dq m B1 K eb C bs_disk
                HK Hbno Hkk HMkNone Ha0 Ha1 HB1tp Hj Hgl HB1regs HB1s1 Heb
                with "Hcg Htext Hpc Hpanic Hbio Hframe Hcnt Hpay Htok Hscan Hbslot
                      Hprocs Hoctx Hvc Hppid Hdev Hgeom Hdlock Hdisk Hcont").
    - (* ======== refcnt <> 0: advance to b->prev ======== *)
      assert (Hbeqz : eq_vec (B1 !!! Regidx Ra5) zero_reg = false)
        by (rewrite HB1a5; exact Hwnz).
      iApply (wp_cbeqz_fall_s_sconf γ Φ (mword_of_int (BD + 0x7c)) (mword_of_int 10 : mword 8)
                (Cregidx (mword_of_int 7)) Ra5 B1 (K - 6)%nat
                ltac:(vm_compute; reflexivity) ltac:(vm_compute; discriminate) Hbeqz
                with "Hcg Hpc Hi7c [-]").
      iIntros "Hcg Hpc".
      assert (Hpp7e : add_vec_int (mword_of_int (BD + 0x7c) : mword 64) 2 = mword_of_int (BD + 0x7e))
        by (apply bv_eq; vm_compute; reflexivity).
      ...
      iApply (wp_cld_s_sconf γ Φ (mword_of_int (BD + 0x7e)) Rs1 Rs1 (mword_of_int 72 : mword 12)
                B1 (K - 6)%nat (List.last (map bnode d) bhead)
                ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                with "Hcg Hpc Hi7e [Hprev] [-]").
      { iEval (rewrite HB1s1). iExact "Hprev". }
      iIntros "Hcg Hpc Hprev".
      iEval (rewrite HB1s1) in "Hprev".
      ...
      assert (HB2tp : B2 !!! Regidx Rtp = cid_word)
        by (rewrite /B2 upd_ne; [exact HB1tp | vm_compute; discriminate]).
      ...
      { rewrite /bcache_scan. iFrame "Hauth Hsauth".
        ...
        iSplitR; [iPureIntro; exact Hordp|].
        iFrame "Hlru Hslots". }
      assert (Hpp80 : add_vec_int (mword_of_int (BD + 0x7e) : mword 64) 2 = mword_of_int (BD + 0x80))
        by (apply bv_eq; vm_compute; reflexivity).
      ...
      + (* ---- the prefix is exhausted: fall through to panic ---- *)
        ...
        iApply (wp_bne_fall_s_sconf γ Φ (mword_of_int (BD + 0x80)) (mword_of_int 8186 : mword 13)
                  Ra4 Rs1 B2 (K - 6)%nat
                  ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate) Hbne
                  with "Hcg Hpc Hi80 [-]").
        iIntros "Hcg Hpc".
        assert (Hpp84 : add_vec_int (mword_of_int (BD + 0x80) : mword 64) 4 = mword_of_int (BD + 0x84))
          by (apply bv_eq; vm_compute; reflexivity).
        ...
        iApply (wp_auipc_s_sconf γ Φ (mword_of_int (BD + 0x84)) Ra0 (mword_of_int 4 : mword 20)
                  B2 (K - 6)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                  with "Hcg Hpc Hi84 [-]").
        iIntros "Hcg Hpc".
        set (Q1 := <[Regidx Ra0 := regval_into_reg
                      (add_vec (mword_of_int (BD + 0x84) : mword 64)
                         (auipc_off (mword_of_int 4 : mword 20)))]> B2).
        assert (Hpp88 : add_vec_int (mword_of_int (BD + 0x84) : mword 64) 4 = mword_of_int (BD + 0x88))
          by (apply bv_eq; vm_compute; reflexivity).
        iEval (rewrite Hpp88) in "Hpc".
        iApply (wp_addi4_s_sconf γ Φ (mword_of_int (BD + 0x88)) Ra0 Ra0 (mword_of_int 2022 : mword 12)
                  Q1 (K - 6)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                  with "Hcg Hpc Hi88 [-]").
        iIntros "Hcg Hpc".
        set (Q2 := <[Regidx Ra0 := regval_into_reg
                      (add_vec (Q1 !!! Regidx Ra0)
                         (sign_extend' 64 (mword_of_int 2022 : mword 12)))]> Q1).
        assert (Hpp8c : add_vec_int (mword_of_int (BD + 0x88) : mword 64) 4 = mword_of_int (BD + 0x8c))
          by (apply bv_eq; vm_compute; reflexivity).
        iEval (rewrite Hpp8c) in "Hpc".
        iApply (wp_jal_s_sconf γ Φ (mword_of_int (BD + 0x8c)) Rra (mword_of_int 2088036 : mword 21)
                  Q2 (K - 6)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                  ltac:(vm_compute; reflexivity) with "Hcg Hpc Hi8c [-]").
        iIntros "Hcg Hpc".
        assert (Htgtpanic : add_vec (mword_of_int (BD + 0x8c) : mword 64)
                              (sign_extend' 64 (mword_of_int 2088036 : mword 21))
      ...
      + (* ---- the prefix still has a node: take the back edge ---- *)
        ...
        iApply (wp_bne_taken_s_sconf γ Φ (mword_of_int (BD + 0x80)) (mword_of_int 8186 : mword 13)
                  Ra4 Rs1 B2 (K - 6)%nat
                  ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate) Hbne
                  ltac:(vm_compute; reflexivity)
                  with "Hcg Hpc Hi80 [-]").
        iNext. iIntros "Hcg Hpc".
        assert (Htgt7a : add_vec (mword_of_int (BD + 0x80) : mword 64)
                           (sign_extend' 64 (mword_of_int 8186 : mword 13))
                         = mword_of_int (BD + 0x7a))
          by (apply bv_eq; vm_compute; reflexivity).
        iEval (rewrite Htgt7a) in "Hpc".
        assert (Hlen' : (length d <= n)%nat).
        { rewrite Hpre length_app in Hlen. cbn in Hlen. lia. }
        iApply (IH d (kk :: post) B2 Hlen' Hord2 Hdne HB2regs HB2tp HB2s1 HB2a4 Heb
                  with "Hcg Htext Hpc Hpanic Hbio Hframe Hcnt Hpay Htok Hscan Hbslot
                        Hprocs Hoctx Hvc Hppid Hdev Hgeom Hdlock Hdisk Hcont").
  Qed.
  ...
  Local Lemma bread_miss (γ : gname) (Φ : mval -> iProp Σ)
      (γs : list gname) (j : nat) (γl : gname)
      (γu : uart_names) (γd : disk_names) (γk : gname)
      (pd pav pu : mword 64)
      (bn : bio_names)
    ...
    sie_cap_gpr γ M (K - 6)%nat -∗
    kernel_text -∗
    pc_is (mword_of_int (BD + 0x64) : mword 64) -∗
    ...
    WP (Loop : expr riscv_lang) {{ Φ }}.
  Proof.
    intros HK Hbno Ha0 Ha1 Hj Hgl Hordp Hregs Htp Heb.
    pose proof Hregs as (HMsp & HMs2 & HMs3 & HMthr).
    ...
    rewrite /bcache_scan.
    iDestruct "Hscan" as "(Hauth & Hsauth & %Hdom & %Hordp2 & Hlru & Hslots)".
    (* ---- +0x64 auipc s1,0x1e ---- *)
    iApply (wp_auipc_s_sconf γ Φ (mword_of_int (BD + 0x64)) Rs1 (mword_of_int 30 : mword 20)
              M (K - 6)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi64 [-]").
    iIntros "Hcg Hpc".
    set (Q1 := <[Regidx Rs1 := regval_into_reg
                  (add_vec (mword_of_int (BD + 0x64) : mword 64)
                     (auipc_off (mword_of_int 30 : mword 20)))]> M).
    assert (Hpp68 : add_vec_int (mword_of_int (BD + 0x64) : mword 64) 4 = mword_of_int (BD + 0x68))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp68) in "Hpc".
    (* ---- +0x68 ld s1,-1882(s1) : s1 := bcache.head.prev ---- *)
    assert (Hhp : add_vec (Q1 !!! Regidx Rs1) (sign_extend' 64 (mword_of_int 2214 : mword 12))
                  = bprev bhead).
    ...
    iApply (wp_ld_s_sconf γ Φ (mword_of_int (BD + 0x68)) Rs1 Rs1 (mword_of_int 2214 : mword 12)
              Q1 (K - 6)%nat (List.last (map bnode ord) bhead)
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
    ...
    assert (Hpp6c : add_vec_int (mword_of_int (BD + 0x68) : mword 64) 4 = mword_of_int (BD + 0x6c))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp6c) in "Hpc".
    (* ---- +0x6c / +0x70 : a5 := &bcache.head ---- *)
    iApply (wp_auipc_s_sconf γ Φ (mword_of_int (BD + 0x6c)) Ra5 (mword_of_int 30 : mword 20)
              Q2 (K - 6)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi6c [-]").
    iIntros "Hcg Hpc".
    set (Q3 := <[Regidx Ra5 := regval_into_reg
                  (add_vec (mword_of_int (BD + 0x6c) : mword 64)
                     (auipc_off (mword_of_int 30 : mword 20)))]> Q2).
    assert (Hpp70 : add_vec_int (mword_of_int (BD + 0x6c) : mword 64) 4 = mword_of_int (BD + 0x70))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp70) in "Hpc".
    iApply (wp_addi4_s_sconf γ Φ (mword_of_int (BD + 0x70)) Ra5 Ra5 (mword_of_int 2134 : mword 12)
              Q3 (K - 6)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi70 [-]").
    iIntros "Hcg Hpc".
    set (Q4 := <[Regidx Ra5 := regval_into_reg
                  (add_vec (Q3 !!! Regidx Ra5)
                     (sign_extend' 64 (mword_of_int 2134 : mword 12)))]> Q3).
    ...
    assert (Hpp74 : add_vec_int (mword_of_int (BD + 0x70) : mword 64) 4 = mword_of_int (BD + 0x74))
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (wp_beq_fall_s_sconf γ Φ (mword_of_int (BD + 0x74)) (mword_of_int 16 : mword 13)
              Ra5 Rs1 Q4 (K - 6)%nat
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate) Hbeq
              with "Hcg Hpc Hi74 [-]").
    iIntros "Hcg Hpc".
    assert (Hpp78 : add_vec_int (mword_of_int (BD + 0x74) : mword 64) 4 = mword_of_int (BD + 0x78))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp78) in "Hpc".
    (* ---- +0x78 c.mv a4,a5 : the loop's sentinel ---- *)
    iApply (wp_cmv_s_sconf γ Φ (mword_of_int (BD + 0x78)) Ra4 Ra5
              Q4 (K - 6)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi78 [-]").
    iIntros "Hcg Hpc".
    ...
    assert (HQ5tp : Q5 !!! Regidx Rtp = cid_word)
      by (rewrite (HQ5thr Rtp ltac:(vm_compute; reflexivity) ltac:(vm_compute; discriminate)); exact Htp).
    ...
    { rewrite /bcache_scan. iFrame "Hauth Hsauth".
      ...
      iFrame "Hlru Hslots". }
    assert (Hpp7a : add_vec_int (mword_of_int (BD + 0x78) : mword 64) 2 = mword_of_int (BD + 0x7a))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp7a) in "Hpc".
    iApply (bread_bloop γ Φ γs j γl γu γd γk pd pav pu bn Mg ord devs bnos
              pidv dev bno dq m K eb C bs_disk (length ord)
              HK Hbno Ha0 Ha1 Hj Hgl ord [] Q5
              ltac:(reflexivity) ltac:(rewrite app_nil_r; reflexivity)
              (bd_ord_nonnil ord Hordp) HQ5regs HQ5tp HQ5s1 HQ5a4 Heb
              with "Hcg Htext Hpc Hpanic Hbio Hframe Hcnt Hpay Htok Hscan Hbslot
                    Hprocs Hoctx Hvc Hppid Hdev Hgeom Hdlock Hdisk Hcont").
  Qed.
  ...
  (*  arbitrary arrival map) rather than written twice.                    *)
  (* ================================================================== *)

  Local Lemma bread_floop (γ : gname) (Φ : mval -> iProp Σ)
      (γs : list gname) (j : nat) (γl : gname)
      (γu : uart_names) (γd : disk_names) (γk : gname)
      (pd pav pu : mword 64)
      (bn : bio_names)
    ...
    sie_cap_gpr γ M (K - 6)%nat -∗
    kernel_text -∗
    pc_is (mword_of_int (BD + 0x3c) : mword 64) -∗
    panic_wp_any -∗
    bio_ctx bn -∗
    bd_frame m -∗
    cpu_own γ 1 eb (proc_addr j) C -∗
    ...
    WP (Loop : expr riscv_lang) {{ Φ }}.
  Proof.
    intros HK Hbno Ha0 Ha1 Hj Hgl Hordp.
    induction n as [|n IH];
      intros done rest M Hlen Hord Hne Hregs Htp HMs1 HMa4 Heb.
    ...
    iAssert (∀ Mx : regfile,
               ⌜bd_regs m Mx /\ Mx !!! Regidx Rtp = cid_word
                /\ Mx !!! Regidx Rs1 = bnode kk /\ Mx !!! Regidx Ra4 = bhead⌝ -∗
               sie_cap_gpr γ Mx (K - 6)%nat -∗
               pc_is (mword_of_int (BD + 0x36) : mword 64) -∗
               bd_frame m -∗
               cpu_own γ 1 eb (proc_addr j) C -∗
               trap_csrs_pay 0 eb -∗
               locked (bn_lk bn) cpu_id -∗
    ...
    { iIntros (Mx (Hxregs & Hxtp & Hxs1 & Hxa4)).
      iIntros "Hcg Hpc Hframe Hcnt Hpay Htok Hscan Hbslot Hoctx Hvc Hppid Hdisk Hcont".
      ...
      iApply (wp_cld_s_sconf γ Φ (mword_of_int (BD + 0x36)) Rs1 Rs1 (mword_of_int 80 : mword 12)
                Mx (K - 6)%nat (List.hd bhead (map bnode r))
                ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                with "Hcg Hpc Hi36 [Hnextc] [-]").
      { iEval (rewrite Hxs1). iExact "Hnextc". }
      iIntros "Hcg Hpc Hnextc".
      iEval (rewrite Hxs1) in "Hnextc".
      ...
      assert (HG1tp : G1 !!! Regidx Rtp = cid_word)
        by (rewrite /G1 upd_ne; [exact Hxtp | vm_compute; discriminate]).
      ...
      { rewrite /bcache_scan. iFrame "Hauth Hsauth".
        ...
        iSplitR; [iPureIntro; exact Hordp2|].
        iFrame "Hlru Hslots". }
      assert (Hpp38 : add_vec_int (mword_of_int (BD + 0x36) : mword 64) 2 = mword_of_int (BD + 0x38))
        by (apply bv_eq; vm_compute; reflexivity).
      ...
      + (* the scan has wrapped: on to the backward scan *)
        ...
        iApply (wp_beq_taken_s_sconf γ Φ (mword_of_int (BD + 0x38)) (mword_of_int 44 : mword 13)
                  Ra4 Rs1 G1 (K - 6)%nat
                  ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate) Hbeq
                  ltac:(vm_compute; reflexivity)
                  with "Hcg Hpc Hi38 [-]").
        iNext. iIntros "Hcg Hpc".
        assert (Htgt64 : add_vec (mword_of_int (BD + 0x38) : mword 64)
                           (sign_extend' 64 (mword_of_int 44 : mword 13))
                         = mword_of_int (BD + 0x64))
          by (apply bv_eq; vm_compute; reflexivity).
        iEval (rewrite Htgt64) in "Hpc".
        iApply (bread_miss γ Φ γs j γl γu γd γk pd pav pu bn Mg ord devs bnos
                  pidv dev bno dq m G1 K eb C bs_disk
                  HK Hbno Ha0 Ha1 Hj Hgl Hordp HG1regs HG1tp Heb
                  with "Hcg Htext Hpc Hpanic Hbio Hframe Hcnt Hpay Htok Hscan Hbslot
                        Hprocs Hoctx Hvc Hppid Hdev Hgeom Hdlock Hdisk Hcont").
      + (* another buffer to test: back to the loop head *)
        ...
        iApply (wp_beq_fall_s_sconf γ Φ (mword_of_int (BD + 0x38)) (mword_of_int 44 : mword 13)
                  Ra4 Rs1 G1 (K - 6)%nat
                  ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate) Hbeq
                  with "Hcg Hpc Hi38 [-]").
        iIntros "Hcg Hpc".
        assert (Hpp3c : add_vec_int (mword_of_int (BD + 0x38) : mword 64) 4 = mword_of_int (BD + 0x3c))
          by (apply bv_eq; vm_compute; reflexivity).
        iEval (rewrite Hpp3c) in "Hpc".
        assert (Hlen' : (length r <= n)%nat).
        { rewrite Hrest in Hlen. cbn in Hlen. lia. }
        iApply (IH (done ++ [kk])%list r G1 Hlen'
                  ltac:(rewrite Hord2 -app_assoc; reflexivity) Hrne HG1regs HG1tp HG1s1 HG1a4 Heb
                  with "Hcg Htext Hpc Hpanic Hbio Hframe Hcnt Hpay Htok Hscan Hbslot
                        Hprocs Hoctx Hvc Hppid Hdev Hgeom Hdlock Hdisk Hcont"). }
    ...
    assert (Hadev : add_vec (M !!! Regidx Rs1) (sign_extend' 64 (mword_of_int 8 : mword 12))
                    = b_dev (bpa kk)).
    { rewrite Hcur bd_s8. rewrite /b_dev /bpa /pa_add /add_vec_int. reflexivity. }
    (* ---- +0x3c c.lw a5,8(s1) : b->dev ---- *)
    iEval (rewrite -Hadev) in "Hdevc".
    iApply (wp_clw_s_sconf (dqm := DfracOwn qc) γ Φ (mword_of_int (BD + 0x3c)) Ra5 Rs1
              (mword_of_int 8 : mword 12) M (K - 6)%nat (devs kk)
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
    ...
    assert (HF1tp : F1 !!! Regidx Rtp = cid_word)
      by (rewrite /F1 upd_ne; [exact Htp | vm_compute; discriminate]).
    ...
    { rewrite /bd_regs. split_and!;
      ...
      rewrite /F1 upd_ne; [| apply not_eq_sym; apply is_cs_idx_true_neq;
                             [vm_compute; reflexivity | assumption]].
      exact (HMthr c Hcs N2 N8 N9 N18 N19 N4). }
    assert (Hpp3e : add_vec_int (mword_of_int (BD + 0x3c) : mword 64) 2 = mword_of_int (BD + 0x3e))
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    - (* ---- dev matches: test the block number ---- *)
      ...
      iApply (wp_bne_fall_s_sconf γ Φ (mword_of_int (BD + 0x3e)) (mword_of_int 8184 : mword 13)
                Rs2 Ra5 F1 (K - 6)%nat
                ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate) Hbne
                with "Hcg Hpc Hi3e [-]").
      iIntros "Hcg Hpc".
      assert (Hpp42 : add_vec_int (mword_of_int (BD + 0x3e) : mword 64) 4 = mword_of_int (BD + 0x42))
        by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hpp42) in "Hpc".
      (* ---- +0x42 c.lw a5,12(s1) : b->blockno ---- *)
      assert (Habno : add_vec (F1 !!! Regidx Rs1) (sign_extend' 64 (mword_of_int 12 : mword 12))
                      = b_blockno (bpa kk)).
      { rewrite HF1s1 bd_s12. rewrite /b_blockno /bpa /pa_add /add_vec_int. reflexivity. }
      iEval (rewrite -Habno) in "Hbnoc".
      iApply (wp_clw_s_sconf (dqm := DfracOwn qc) γ Φ (mword_of_int (BD + 0x42)) Ra5 Rs1
                (mword_of_int 12 : mword 12) F1 (K - 6)%nat (bnos kk)
                ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
      ...
      assert (HF2tp : F2 !!! Regidx Rtp = cid_word)
        by (rewrite /F2 upd_ne; [exact HF1tp | vm_compute; discriminate]).
      ...
      { rewrite /bcache_scan. iFrame "Hauth Hsauth".
        ...
        iSplitR; [iPureIntro; exact Hordp2|].
        iFrame "Hlru Hslots". }
      assert (Hpp44 : add_vec_int (mword_of_int (BD + 0x42) : mword 64) 2 = mword_of_int (BD + 0x44))
        by (apply bv_eq; vm_compute; reflexivity).
      ...
      + (* ======== THE HIT ======== *)
        ...
        iApply (wp_bne_fall_s_sconf γ Φ (mword_of_int (BD + 0x44)) (mword_of_int 8178 : mword 13)
                  Rs3 Ra5 F2 (K - 6)%nat
                  ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate) Hbne3
                  with "Hcg Hpc Hi44 [-]").
        iIntros "Hcg Hpc".
        assert (Hpp48 : add_vec_int (mword_of_int (BD + 0x44) : mword 64) 4 = mword_of_int (BD + 0x48))
          by (apply bv_eq; vm_compute; reflexivity).
        iEval (rewrite Hpp48) in "Hpc".
        iApply (bread_hit γ Φ γs j γl γu γd γk pd pav pu bn kk Mg ord devs bnos
                  pidv dev bno dq m F2 K eb C bs_disk
                  HK Hbno Hkk Hdeq Hbeq2 HF2tp Hj Hgl HF2regs HF2s1 Heb
                  with "Hcg Htext Hpc Hpanic Hbio Hframe Hcnt Hpay Htok Hscan Hbslot
                        Hprocs Hoctx Hvc Hppid Hdev Hgeom Hdlock Hdisk Hcont").
      + (* blockno mismatch: advance *)
        ...
        iApply (wp_bne_taken_s_sconf γ Φ (mword_of_int (BD + 0x44)) (mword_of_int 8178 : mword 13)
                  Rs3 Ra5 F2 (K - 6)%nat
                  ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate) Hbne3
                  ltac:(vm_compute; reflexivity)
                  with "Hcg Hpc Hi44 [-]").
        iNext. iIntros "Hcg Hpc".
        assert (Htgt36 : add_vec (mword_of_int (BD + 0x44) : mword 64)
                           (sign_extend' 64 (mword_of_int 8178 : mword 13))
                         = mword_of_int (BD + 0x36))
          by (apply bv_eq; vm_compute; reflexivity).
        iEval (rewrite Htgt36) in "Hpc".
        iApply ("HADV" $! F2 with "[%] Hcg Hpc Hframe Hcnt Hpay Htok Hscan Hbslot
                                   Hoctx Hvc Hppid Hdisk Hcont").
        split_and!; [exact HF2regs | exact HF2tp | exact HF2s1 | exact HF2a4].
    - (* ---- dev mismatch: advance ---- *)
      ...
      iApply (wp_bne_taken_s_sconf γ Φ (mword_of_int (BD + 0x3e)) (mword_of_int 8184 : mword 13)
                Rs2 Ra5 F1 (K - 6)%nat
                ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate) Hbne
                ltac:(vm_compute; reflexivity)
                with "Hcg Hpc Hi3e [-]").
      iNext. iIntros "Hcg Hpc".
      assert (Htgt36 : add_vec (mword_of_int (BD + 0x3e) : mword 64)
      ...
      { rewrite /bcache_scan. iFrame "Hauth Hsauth".
        ...
        iSplitR; [iPureIntro; exact Hordp2|].
        iFrame "Hlru Hslots". }
      iApply ("HADV" $! F1 with "[%] Hcg Hpc Hframe Hcnt Hpay Htok Hscan Hbslot
                                 Hoctx Hvc Hppid Hdisk Hcont").
      split_and!; [exact HF1regs | exact HF1tp | exact HF1s1 | exact HF1a4].
  Qed.

  Lemma wp_bread_sconf (γ : gname) (Φ : mval -> iProp Σ)
      (γs : list gname) (j : nat) (γl : gname)
      (γu : uart_names) (γd : disk_names) (γk : gname)
      (pd pav pu : mword 64)
      (bn : bio_names)
  ...
  Proof.
    ...
    intros pcE pj ret_tgt HK Hbno Htp Hj Hgl Ha0 Ha1 Heb.
    pose (sp0 := (m !!! Regidx csp_rs1 : mword 64)).
    set (spr := add_vec (m !!! Regidx csp_rs1 : mword 64)
                  (sign_extend' 64 (caddi16sp_imm (mword_of_int 61 : mword 6)))).
    iIntros "Hcg Hcnt #Htext Hpc #Hpanic #Hbio Hppid #Hprocs Hoctx Hvc".
    iIntros "#Hdev #Hgeom #Hdlock Hdisk Hbslot Hcont".
    iAssert (bd_cont Φ γs γd bn pidv dev bno dq m K eb (proc_addr j) C bs_disk)
      with "[Hcont]" as "Hcont".
    ...
    assert (Hpp02 : add_vec_int (pcE : mword 64) 2 = mword_of_int (BD + 0x02))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp02) in "Hpc".
    iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (BD + 0x02)) (mword_of_int 5 : mword 6) Rra
              R1 (K - 6)%nat v40 with "Hcg Hpc Hi02 Hr40 [-]").
    iIntros "Hcg Hpc Hr40".
    iEval (rewrite Hb1 HR1ra) in "Hr40".
    assert (Hpp04 : add_vec_int (mword_of_int (BD + 0x02) : mword 64) 2 = mword_of_int (BD + 0x04))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp04) in "Hpc".
    iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (BD + 0x04)) (mword_of_int 4 : mword 6) Rs0
              R1 (K - 6)%nat v32 with "Hcg Hpc Hi04 Hr32 [-]").
    iIntros "Hcg Hpc Hr32".
    iEval (rewrite Hb2 HR1s0) in "Hr32".
    assert (Hpp06 : add_vec_int (mword_of_int (BD + 0x04) : mword 64) 2 = mword_of_int (BD + 0x06))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp06) in "Hpc".
    iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (BD + 0x06)) (mword_of_int 3 : mword 6) Rs1
              R1 (K - 6)%nat v24 with "Hcg Hpc Hi06 Hr24 [-]").
    iIntros "Hcg Hpc Hr24".
    iEval (rewrite Hb3 HR1s1) in "Hr24".
    assert (Hpp08 : add_vec_int (mword_of_int (BD + 0x06) : mword 64) 2 = mword_of_int (BD + 0x08))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp08) in "Hpc".
    iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (BD + 0x08)) (mword_of_int 2 : mword 6) Rs2
              R1 (K - 6)%nat v16 with "Hcg Hpc Hi08 Hr16 [-]").
    iIntros "Hcg Hpc Hr16".
    iEval (rewrite Hb4 HR1s2) in "Hr16".
    assert (Hpp0a : add_vec_int (mword_of_int (BD + 0x08) : mword 64) 2 = mword_of_int (BD + 0x0a))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp0a) in "Hpc".
    iApply (wp_csdsp_s_sconf γ Φ (mword_of_int (BD + 0x0a)) (mword_of_int 1 : mword 6) Rs3
              R1 (K - 6)%nat v8 with "Hcg Hpc Hi0a Hr8 [-]").
    iIntros "Hcg Hpc Hr8".
    ...
    assert (Hpp0c : add_vec_int (mword_of_int (BD + 0x0a) : mword 64) 2 = mword_of_int (BD + 0x0c))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp0c) in "Hpc".
    (* +0x0c c.addi4spn s0,sp,48 *)
    iApply (wp_caddi4spn_s_sconf γ Φ (mword_of_int (BD + 0x0c)) (Cregidx (mword_of_int 0))
    ...
    assert (Hpp0e : add_vec_int (mword_of_int (BD + 0x0c) : mword 64) 2 = mword_of_int (BD + 0x0e))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp0e) in "Hpc".
    (* +0x0e c.mv s2,a0 : s2 := dev *)
    iApply (wp_cmv_s_sconf γ Φ (mword_of_int (BD + 0x0e)) Rs2 Ra0
    ...
    assert (Hpp10 : add_vec_int (mword_of_int (BD + 0x0e) : mword 64) 2 = mword_of_int (BD + 0x10))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp10) in "Hpc".
    (* +0x10 c.mv s3,a1 : s3 := blockno *)
    iApply (wp_cmv_s_sconf γ Φ (mword_of_int (BD + 0x10)) Rs3 Ra1
    ...
    assert (Hpp12 : add_vec_int (mword_of_int (BD + 0x10) : mword 64) 2 = mword_of_int (BD + 0x12))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp12) in "Hpc".
    (* +0x12 / +0x16 : a0 := &bcache ; +0x1a jal acquire *)
    iApply (wp_auipc_s_sconf γ Φ (mword_of_int (BD + 0x12)) Ra0 (mword_of_int 0x15 : mword 20)
              R4 (K - 6)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi12 [-]").
    iIntros "Hcg Hpc".
    set (R5 := <[Regidx Ra0 := regval_into_reg
                  (add_vec (mword_of_int (BD + 0x12) : mword 64)
                     (auipc_off (mword_of_int 0x15 : mword 20)))]> R4).
    assert (Hpp16 : add_vec_int (mword_of_int (BD + 0x12) : mword 64) 4 = mword_of_int (BD + 0x16))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp16) in "Hpc".
    iApply (wp_addi4_s_sconf γ Φ (mword_of_int (BD + 0x16)) Ra0 Ra0 (mword_of_int 1608 : mword 12)
              R5 (K - 6)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi16 [-]").
    iIntros "Hcg Hpc".
    set (R6 := <[Regidx Ra0 := regval_into_reg
                  (add_vec (R5 !!! Regidx Ra0) (sign_extend' 64 (mword_of_int 1608 : mword 12)))]> R5).
    assert (HR6a0 : R6 !!! Regidx Ra0 = bcache_addr).
    { rewrite /R6 upd_eq /R5 upd_eq. rewrite /bcache_addr.
      apply bv_eq; vm_compute; reflexivity. }
    assert (Hpp1a : add_vec_int (mword_of_int (BD + 0x16) : mword 64) 4 = mword_of_int (BD + 0x1a))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp1a) in "Hpc".
    iApply (wp_jal_s_sconf γ Φ (mword_of_int (BD + 0x1a)) Rra (mword_of_int 2089144 : mword 21)
    ...
    set (R7 := <[Regidx Rra := regval_into_reg
                  (add_vec_int (mword_of_int (BD + 0x1a) : mword 64) 4)]> R6).
    assert (Htgtacq : add_vec (mword_of_int (BD + 0x1a) : mword 64)
                        (sign_extend' 64 (mword_of_int 2089144 : mword 21))
    ...
    { rewrite (HR7thr Rtp ltac:(vm_compute; reflexivity)
                 ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                 ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
                 ltac:(vm_compute; discriminate)).
      exact Htp. }
    assert (HR7ra : R7 !!! Regidx Rra = add_vec_int (mword_of_int (BD + 0x1a) : mword 64) 4)
      by (rewrite /R7; apply upd_eq).
    iApply (A.wp_acquire_sconf γ Φ (bn_lk bn) "bcache"%string (bcache_res bn) R7
              0%nat eb (proc_addr j) C (K - 6)%nat
              HR7tp ltac:(vm_compute; reflexivity) ltac:(unfold K_bread in HK; lia)
              with "Hcg Hcnt Htext Hpc [Hlock] [] [-]").
    { iEval (rewrite HR7a0). iExact "Hlock". }
    { iApply (panic_wp_any_at cpu_id with "Hpanic"). }
    iIntros (ms mq) "%Hmsfacts Hcg Hpc %Hacqpins Htok HRres Hcnt Hpay".
    assert (Hpc1e : ret_pc (R7 !!! Regidx Rra) = mword_of_int (BD + 0x1e)).
    ...
    iDestruct (bcache_res_to_scan bn with "HRres") as (Mg ord devs bnos) "Hscan".
    rewrite /bcache_scan.
    iDestruct "Hscan" as "(Hauth & Hsauth & %Hdom & %Hordp & Hlru & Hslots)".
    ...
    iApply (wp_auipc_s_sconf γ Φ (mword_of_int (BD + 0x1e)) Rs1 (mword_of_int 30 : mword 20)
              mq (K - 6)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi1e [-]").
    iIntros "Hcg Hpc".
    set (W1 := <[Regidx Rs1 := regval_into_reg
                  (add_vec (mword_of_int (BD + 0x1e) : mword 64)
                     (auipc_off (mword_of_int 30 : mword 20)))]> mq).
    assert (Hpp22 : add_vec_int (mword_of_int (BD + 0x1e) : mword 64) 4 = mword_of_int (BD + 0x22))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp22) in "Hpc".
    assert (Hhn : add_vec (W1 !!! Regidx Rs1) (sign_extend' 64 (mword_of_int 2292 : mword 12))
                  = bnext bhead).
    ...
    iApply (wp_ld_s_sconf γ Φ (mword_of_int (BD + 0x22)) Rs1 Rs1 (mword_of_int 2292 : mword 12)
              W1 (K - 6)%nat (List.hd bhead (map bnode ord))
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
    ...
    assert (Hpp26 : add_vec_int (mword_of_int (BD + 0x22) : mword 64) 4 = mword_of_int (BD + 0x26))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp26) in "Hpc".
    (* +0x26 / +0x2a : a5 := &bcache.head *)
    iApply (wp_auipc_s_sconf γ Φ (mword_of_int (BD + 0x26)) Ra5 (mword_of_int 30 : mword 20)
              W2 (K - 6)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi26 [-]").
    iIntros "Hcg Hpc".
    set (W3 := <[Regidx Ra5 := regval_into_reg
                  (add_vec (mword_of_int (BD + 0x26) : mword 64)
                     (auipc_off (mword_of_int 30 : mword 20)))]> W2).
    assert (Hpp2a : add_vec_int (mword_of_int (BD + 0x26) : mword 64) 4 = mword_of_int (BD + 0x2a))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp2a) in "Hpc".
    iApply (wp_addi4_s_sconf γ Φ (mword_of_int (BD + 0x2a)) Ra5 Ra5 (mword_of_int 2204 : mword 12)
              W3 (K - 6)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi2a [-]").
    iIntros "Hcg Hpc".
    set (W4 := <[Regidx Ra5 := regval_into_reg
                  (add_vec (W3 !!! Regidx Ra5)
                     (sign_extend' 64 (mword_of_int 2204 : mword 12)))]> W3).
    ...
    assert (Hpp2e : add_vec_int (mword_of_int (BD + 0x2a) : mword 64) 4 = mword_of_int (BD + 0x2e))
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (wp_beq_fall_s_sconf γ Φ (mword_of_int (BD + 0x2e)) (mword_of_int 54 : mword 13)
              Ra5 Rs1 W4 (K - 6)%nat
              ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate) Hbeq
              with "Hcg Hpc Hi2e [-]").
    iIntros "Hcg Hpc".
    assert (Hpp32 : add_vec_int (mword_of_int (BD + 0x2e) : mword 64) 4 = mword_of_int (BD + 0x32))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp32) in "Hpc".
    (* +0x32 c.mv a4,a5 : the scan's sentinel *)
    iApply (wp_cmv_s_sconf γ Φ (mword_of_int (BD + 0x32)) Ra4 Ra5
              W4 (K - 6)%nat ltac:(vm_compute; discriminate) ltac:(vm_compute; discriminate)
              with "Hcg Hpc Hi32 [-]").
    iIntros "Hcg Hpc".
    ...
    assert (HW5tp : W5 !!! Regidx Rtp = cid_word).
    { rewrite (HW5thr Rtp ltac:(vm_compute; reflexivity) ltac:(vm_compute; discriminate)).
      rewrite (callee_saved_lookup Hacqpins_cs Rtp ltac:(vm_compute; reflexivity)).
      exact HR7tp. }
    ...
    { rewrite /bcache_scan. iFrame "Hauth Hsauth".
      ...
      iSplitR; [iPureIntro; exact Hordp|].
      iFrame "Hlru Hslots". }
    assert (Hpp34 : add_vec_int (mword_of_int (BD + 0x32) : mword 64) 2 = mword_of_int (BD + 0x34))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp34) in "Hpc".
    (* +0x34 c.j +0x8 : into the loop's test *)
    iApply (wp_cj_s_sconf γ Φ (mword_of_int (BD + 0x34))
              (sign_extend' 21 (concat_vec (mword_of_int 4 : mword 11) ('b"0")))
              W5 (K - 6)%nat ltac:(vm_compute; reflexivity)
              with "Hcg Hpc Hi34 [-]").
    iApply bi.later_intro. iIntros "Hcg Hpc".
    assert (Htgt3c : add_vec (mword_of_int (BD + 0x34) : mword 64)
                       (sign_extend' 64 (sign_extend' 21
                          (concat_vec (mword_of_int 4 : mword 11) ('b"0"))))
                     = mword_of_int (BD + 0x3c))
      by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Htgt3c) in "Hpc".
    iApply (bread_floop γ Φ γs j γl γu γd γk pd pav pu bn Mg ord devs bnos
              pidv dev bno dq m K eb C bs_disk (length ord)
              HK Hbno Ha0 Ha1 Hj Hgl Hordp [] ord W5
              ltac:(reflexivity) ltac:(reflexivity)
              (bd_ord_nonnil ord Hordp) HW5regs HW5tp HW5s1 HW5a4 Heb
              with "Hcg Htext Hpc Hpanic Hbio Hframe Hcnt Hpay Htok Hscan Hbslot
                    Hprocs Hoctx Hvc Hppid Hdev Hgeom Hdlock Hdisk Hcont").
  Qed.
