...
Section WpUserSplitCompute.
  ...
  (* ------------------------------------------------------------------ *)
  (* 2-aligned 32-bit COMBINED (fetch-unconditional) GENERIC rd=x0 NO-OP  *)
  (* (HINT/nop) arm.  The SPLICE of [ustep_u_split_compute_u]'s 2-aligned  *)
  (* split-fetch premise front + engine [wp_instr_u_split_combined] with   *)
  (* [ustep_nop_u]'s generic nop execute premise + state-preserving        *)
  (* continuation (no gpr write).  Covers a state-preserving retiring       *)
  (* instruction (e.g. [addi x0,x1,5]) at an odd-halfword pc.               *)
  (* ------------------------------------------------------------------ *)
  Lemma ustep_u_split_nop
      (va : mword 64) (vpn : mword 27) (ie : uwalk_info) (w : mword 32)
      (ii : instruction)
      (ms_v sc_v stval_v sepc_v : mword 64)
      (g : gmap regidx (mword 64))
    ...
    is_aligned_paddr (Physaddr (u_pa (upt_entry vpn ie) (add_vec_int va 2) vpn)) 2 = true ->
    isRVC (subrange_vec_dec w 15 0) = false ->
    (* decode: w is this generic instruction *)
    (forall s0, agree_on D_u s0 dstateU ->
    ...
    WP (Loop : expr riscv_lang) @ E {{ Φ }}.
  Proof.
    intros HN Hexec_nop Hnlpad Hok Hsome Hchk0 HupdN HcwL HcwH
           HSXL Hbit0 Hbit1 Hvalign4
           HcanonL Hvpn_defL HalignL
           HcanonH Hvpn_defH HalignH
           HnotRVC Hdec.
