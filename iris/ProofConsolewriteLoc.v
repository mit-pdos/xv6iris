(* ProofConsolewriteLoc.v -- the whole-function proof of consolewrite AT
   THE LOCATED TRACE CONTRACT
   ...
   THREADED THROUGH THE CHUNK LOOP and NOTHING ELSE: same block
   decomposition, same rotated loop and fuel induction, same three exits,
   same frame and bounce-buffer carving, same budget, line for line.
   Review it as the diff against [ProofConsolewrite.v], which is what it was
   produced as.  The duplication is TRANSITIONAL BY DESIGN; when the
   consumers have moved, the campaign's retirement step deletes the
   receipt-free original.
   ...
   (2) THE COUNT CARRIES A RECEIPT.  [cw_ret]'s post gains
       [SpecConsolewriteLoc.cons_sent_cnt gu tr0 r] -- "[r] bytes were
   ...
   (4) THE CHUNK STEP IS THE ONE PLACE WITH NEW CONTENT, at the
       ...
       returns (the landed contract has nowhere to put it); this one
         ...
         - reads a seed off the accumulated receipt
           ([SpecConsolewriteLoc.cons_sent_cnt_seed]: its trace witness
...
Require Import SpecConsolewriteLoc.
...
Module ConsolewriteLocProof (EitherCopyin : EITHER_COPYIN)
                            (Uartwrite : UARTWRITE_LOC) : CONSOLEWRITE_LOC.
...
Section CwBodies.
  ...
  Definition cw_ret `{CID0 : CpuId} `{XI : CurCtx} (jp : nat) (m0 : regfile) (av : nat)
      ...
      (γu : uart_names) (tr0 : list (bv 8)) : iProp Σ :=
  ...
  Proof.
    ...
    iApply (cw_epi (CID := CIDj) CID0 jp m0 R8 av eb sp0 pid U n r lks γu tr0
  ...
  Proof.
    ...
    iDestruct (cw_rest_of sp0 m0 Hal with "[S4 S5 S6 S7 S8 S9 S10 S11 S12] Hbuf")
      as "Hrest".
    { rewrite /cw_spill. iFrame "S4 S5 S6 S7 S8 S9 S10 S11 S12". }
    iApply (cw_epi (CID := CIDl8) CID0 jp m0 R8 av eb sp0 pid U n r lks γu tr0
              Hm0sp HR8sp Hs1v Hhi Hr Hav Heb ltac:(wp_next_chain)
              with "Ht Hcg Hcnt Hpc Hpriv Hsaved Hrest Hrcpt Hcont").
  Qed.
  (* =================================================================== *)
  ...
  Proof.
    ...
    { iIntros (CIDb Mb) "%Hregb %Hs2 %Hs11b %Hcrb Hcg Hpc".
      ...
      iApply (wp_jal_s_sconf (mword_of_int (CW + 0x46)) Rra
                (mword_of_int 8624 : mword 21) B5 (av - 16)%nat true
     ...
     iDestruct (cpu_own_transport CIDb CIDc6 0%nat eb pj true 
      ...
      - (* it succeeded: the branch is NOT taken *)
        ...
        iDestruct (cons_sent_cnt_seed γu tr0 i with "Hrcpt")
          as (tracc bsacc) "(%Hbsacc & %Hpacc & %Hsacc & #Htracc)".
        ...
        iAssert (cons_sent_cnt γu tr0 (nn + i)) as "#Hrcpt'".
        { assert (Hcnt : (nn + i)%Z
                         = (i + Z.of_nat (length ((fb' <$> seq 0 nnN) : list (bv 8))))%Z).
          { rewrite length_fmap length_seq. lia. }
          rewrite Hcnt.
          iApply (cons_sent_cnt_chunk γu tr0 tracc i bsacc (fb' <$> seq 0 nnN)
                    Hbsacc Hpacc Hsacc with "Hsent"). }
        ...
        + (* the loop is done: i + nn = n *)
          ...
          iDestruct (cw_ret_weaken (CID0 := CID0) jp m0 av eb pid U P1 n lks γu tr0 Hext1
                       with "Hcont") as "Hcont".
          iApply (cw_exit_done (CID := CIDce) CID0 jp m0 F1 av eb sp0 pid
                    (us_upt U P1) n (nn + i)%Z lks γu tr0
        ...
        + (* another turn: fall through to the head at +0x5e *)
          ...
          iDestruct (cw_ret_weaken (CID0 := CID0) jp m0 av eb pid U P1 n _ γu tr0 Hext1
                       with "Hcont") as "Hcont".
  ...
  (* =================================================================== *)
  Lemma wp_consolewrite_loc_sconf `{CID : CpuId} `{XI : CurCtx}
      ...
      (tr0 : list (bv 8))
    : wp_consolewrite_loc_sconf_body γa γf γs jp γlp γu γv γl m av eb pid U n b lks tr0.
  Proof.
    cbv beta delta [wp_consolewrite_loc_sconf_body].
    ...
    iDestruct (cons_sent_cnt_zero γu tr0 with "Hseed") as "#Hrcpt0".
