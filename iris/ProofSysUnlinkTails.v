...
Section ProofSysUnlinkTails.
  ...
  Proof.
    ...
    iApply (wp_addi4_s_sconf (CID := CID1) (mword_of_int (SU + 0x132)) Ra0 Ra0
              (mword_of_int 1344 : mword 12) P1 K b
              ltac:(nz) ltac:(rdok) with "Hcg Hpc []").
    { iApply (suli_132 with "Htext"). }
    iIntros (CID2 Hq2) "Hcg Hpc".
    pose (P2 := <[Regidx Ra0 := regval_into_reg
                   (add_vec (rget P1 Ra0)
                      (sign_extend' 64 (mword_of_int 1344 : mword 12)))]> P1).
    ...
    iApply (wp_jal_s_sconf (CID := CID2) (mword_of_int (SU + 0x136)) Rra
              (mword_of_int 2078532 : mword 21) P2 K b
              ltac:(nz) ltac:(rdok) ltac:(vm_compute; reflexivity)
              with "Hcg Hpc []").
    { iApply (suli_136 with "Htext"). }
    iIntros (CID3 Hq3) "Hcg Hpc".
    assert (Htgt : add_vec (mword_of_int (SU + 0x136) : mword 64)
                     (sign_extend' 64 (mword_of_int 2078532 : mword 21))
                   = mword_of_int KernelSyms.panic) by pcw.
    iEval (rewrite Htgt) in "Hpc".
    (* the regfile the spec wants is the POST-JAL one: [wp_jal_s_sconf] wrote
       [ra].  a0 is untouched, but prove it on P3 directly -- deriving it
  ...
  Proof.
    ...
    iApply (wp_addi4_s_sconf (CID := CID1) (mword_of_int (SU + 0x13e)) Ra0 Ra0
              (mword_of_int 1356 : mword 12) P1 K b
              ltac:(nz) ltac:(rdok) with "Hcg Hpc []").
    { iApply (suli_13e with "Htext"). }
    iIntros (CID2 Hq2) "Hcg Hpc".
    pose (P2 := <[Regidx Ra0 := regval_into_reg
                   (add_vec (rget P1 Ra0)
                      (sign_extend' 64 (mword_of_int 1356 : mword 12)))]> P1).
    ...
    iApply (wp_jal_s_sconf (CID := CID2) (mword_of_int (SU + 0x142)) Rra
              (mword_of_int 2078520 : mword 21) P2 K b
              ltac:(nz) ltac:(rdok) ltac:(vm_compute; reflexivity)
              with "Hcg Hpc []").
    { iApply (suli_142 with "Htext"). }
    iIntros (CID3 Hq3) "Hcg Hpc".
    assert (Htgt : add_vec (mword_of_int (SU + 0x142) : mword 64)
                     (sign_extend' 64 (mword_of_int 2078520 : mword 21))
                   = mword_of_int KernelSyms.panic) by pcw.
    iEval (rewrite Htgt) in "Hpc".
    (* the regfile the spec wants is the POST-JAL one: [wp_jal_s_sconf] wrote
       [ra].  a0 is untouched, but prove it on P3 directly -- deriving it
  ...
  Proof.
    ...
    iApply (EndOp.wp_end_op_sconf (CID := CID1) gs jx gl gu gd gk pd pav pu bn
              g fsc_fs fsc_cov fsc_logst dev u pidv dq M1 (K - 30)%nat eb b lks
  ...
  Proof.
    ...
    iApply (EndOp.wp_end_op_sconf (CID := CID4) gs jx gl gu gd gk pd pav pu bn
              g fsc_fs fsc_cov fsc_logst dev n2 pidv dq Q1 (K - 30)%nat eb b lks
              Vpr HKeo Hgeom Hj Hgl ltac:(rewrite Hlkempty; apply locks_below_empty)
              with "Hcg Hown Htce Hcce Htext Hkd Hpc Hpenv Hbio Hlog Hseam Hgen
                    Hpid Hprocs Hdev Hgeo Hdlk Hop").
  ...
  Proof.
    ...
    iDestruct (cpu_claim_ext_transport CID0 CID1 eb (proc_addr jx)
                 ltac:(rewrite Hb; wp_next_chain) with "Hcce") as "Hcce".
    iApply (su_tail_bad (CID0 := CID1) gs jx gl gu gd gk pd pav pu bn g gi
  ...
  Lemma su_tail_e `{GEN : GenId} `{CID0 : CpuId}
    ...
    ~ (IBLOCK inum inodestart ∈ log_region_set fsc_logst) ->
    bv_unsigned inum < 16 * Z.of_nat nib ->
    IBLOCK inumi inodestart ∈ fsc_cov ->
    ~ (IBLOCK inumi inodestart ∈ log_region_set fsc_logst) ->
    ...
    itable_inv -∗
    ic_escrow fsc_ic fsc_fs gi fsc_cov fsc_logst kk -∗
    ic_escrow fsc_ic fsc_fs gi fsc_cov fsc_logst ki -∗
    ireg_inv gi fsc_fs inodestart nib -∗
    ireg_open -∗
    ...
    ic_loaded fsc_fs gi fsc_cov fsc_logst ki inumi dni bmi -∗
  ...
  Proof.
    ...
    iApply (Iunlockput.wp_iunlockput_dep_gen (CID := CID2) gs jx gl gu gd gk
              pd pav pu bn g gi gtl gili gisli bmapstart
    ...
    iApply fupd_wp.
    iMod (ic_grow_tx ⊤ fsc_ic fsc_fs gi fsc_cov fsc_logst kk s dev inum gy true
    ...
    iDestruct (wp_next_shift (b := true) (CIDa := CID0) (CIDb := CID6)
                 ltac:(wp_next_chain) with "Hcont") as "Hcont".
    iApply (su_tail_bad (CID0 := CID6) gs jx gl gu gd gk pd pav pu bn g gi
              gtl gil gisl bmapstart inodestart nib size dev
