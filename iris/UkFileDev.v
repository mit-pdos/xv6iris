...
Section UkFileDev.
  ...
  Definition file_in (i : Z) (γo : gname) (q : Qp) (content S : list (bv 8))
      : iProp Σ :=
    (∃ p : nat, ⌜S = drop p content⌝ ∗ ⌜sf !! fname_f = Some (i, content)⌝
  ...
  Definition file_out (i : Z) (γo : gname) (ws : wordline) (b : nat) : iProp Σ :=
    efany c r fname_f sf i γo ws b.
  ...
  Proof using Heq Hsr.
    ...
    iPoseProof (wp_uk_read_deed_learns_held_at N h1 m1
                  ...
                  fd wb i γo c r q jo content fname_f sf p D HsN Heq Hnum Hcnt Hcnt0 Hcapk
  ...
  Lemma fdev_out_of_cur (i : Z) (γo : gname) (ws : wordline) (sel : list nat)
    ...
    efcur c r fname_f sf i γo ws sel jx k -∗ file_out i γo ws (S jx).
  Proof using .
    intros Hlt. iIntros "Hc". rewrite /file_out.
    destruct k as [| k'].
    - iApply (efany_of c r fname_f sf i γo ws (S jx) sel with "Hc").
      exact (fdev_forall_lt_weaken sel jx (S jx) ltac:(lia) Hlt).
    - iApply (efany_of c r fname_f sf i γo ws (S jx) (sel ++ [jx]) with "Hc").
  ...
  Proof using Heq Hsw.
    ...
    iPoseProof (wp_uk_ecall_write_at N h1 m1 (mword_of_int (up_write P + 2))
                  avail
                  (write_file_fam
                     (fun k => efcur c r fname_f sf i γo ws sel jx k
    ...
    { (* ---- THE POST: the arm names the answer, the chain's stop the
      ...
      iAssert (∃ k : nat,
                 ...
                 ∗ efcur c r fname_f sf i γo ws sel jx k
                 ∗ usrc_at N tx dq2 ua (length bs) f)%I
    ...
    iApply (ef_chain c r fname_f sf Heq i γo ws sel jx M pm sz Pt (m1 !!! Regidx a1_idx)
  ...
  Proof using Heq Hso.
    ...
    iPoseProof (wp_uk_ecall_open_read_deed_v N OffHeld h1 m1
                  (mword_of_int (up_open P + 2)) l avail c r q1 q2 i content fname_f sf cw
                  (fdev_fimg pv f) (mword_of_int pv) fname_f HsN Heq Hnum Hal4
  ...
  Proof using Heq Hso.
    ...
    iPoseProof (wp_uk_ecall_open_miss_deed_v N h1 m1
                  (mword_of_int (up_open P + 2)) l avail c r q fname_f sf cw
                  (fdev_fimg pv f) (mword_of_int pv) fname_f FileName.uname_fname_f HsN
                  Heq Hnum Hal4
...
Section UkFileDevCat.
  ...
  Context `{PS : UexecSG.uprogSG Σ}.
  Context `{!echoOutG Σ, !inG Σ (mono_listR (leibnizO Z)), !fileAppG Σ}.
  Context (c : file_fixed) (r : file_names) (sf : dst).
  Context (Heq : file_app = MkAppcfg file_names (file_pred c) r).
  Context (N : uk_names Σ).

  Definition file_read_cat :=
    file_read c r sf Heq N (cat_prog N) (cat_stub_read N).
  Definition file_write_cat :=
    file_write c r sf Heq N (cat_prog N) (cat_stub_write N).
  ...
  Definition file_close_in_cat :=
    file_close_in r sf N (cat_prog N) (cat_stub_close N).
  Definition file_open_present_cat :=
    file_open_present c r sf Heq N (cat_prog N) (cat_stub_open N).
  Definition file_open_absent_cat :=
    file_open_absent c r sf Heq N (cat_prog N) (cat_stub_open N).
End UkFileDevCat.
