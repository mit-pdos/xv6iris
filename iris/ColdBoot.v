...
(* [reset_sys] calls the platform hook [cancel_reservation], and the boot cone *)
(* runs the chain at a copy with that hook lifted to a PARAMETER rather than   *)
(* at whatever the hook happens to be realised as.  (Until the `coq:`-binding  *)
(* change the hook was a generated *Axiom* and this was forced -- [run]/[exec] *)
(* are stuck on an opaque element of the monad; it is now a deliberate         *)
(* statement of independence.  See ArchReset.v's header.)  That                *)
