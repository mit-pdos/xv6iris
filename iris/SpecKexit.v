...
Definition wp_kexit_sconf_body
  ...
  (* THE STACK DEPOSIT, threaded down the diverging chain.  kexit is where
     ...
     assemble by itself is only the region from ITS sp down; the frames
     above it belong to usertrap, syscall and sys_exit, each of which is
     equally never returning.  So each of them hands its callee a closer
     that has captured its own frame, and this is the one that reaches the
     bottom ([ProcDefs.kstack_closer]).  Nothing comes back on the arms that
     RETURN: the wand is affine and those paths drop it. *)
