(* ProofCreateAUF.v -- create AT ITS ATOMIC-UPDATE CONTRACT, PINNED AT
   [T_FILE] ([SpecCreateAUF.wp_create_auf_body]).

   Worklist: claude-notes/projects/fs-syscall-specs.md, lane W (the T_FILE
   create-AU carry, sys_open's O_CREATE arm).

   THIS FILE IS A COPY-ADAPT of [ProofCreateAU.v] -- which is itself the
   ruled copy-adapt of [ProofCreate.v] its own header describes -- with ONE
   arm put back.  Review it as the diff against [ProofCreateAU.v], which is
   what it was produced as.  The duplication is TRANSITIONAL BY DESIGN,
   exactly as the era walk's is; the consolidation these three files are
   waiting for is a NON-DIRECTORY-generic create-AU ([ty <> T_DIR], both
   sides of the [bne s2,2] walked), which subsumes this file and
   [ProofCreateAU] and is a strictly larger job than either.

   THE DIFF AGAINST [ProofCreateAU.v], EXHAUSTIVELY.

   (1) THE PIN MOVES FROM [T_DEVICE] TO [T_FILE].  Two of the three things
       the device pin bought are UNCHANGED, because they are refutations of
       the DIRECTORY arm and [T_FILE <> T_DIR] just as [T_DEVICE <> T_DIR]:
       the [beq s4,a4] at +0xca still cannot be taken, so [cr_mkdir_body] /
       [cr_mkdir_half] / [cr_fail_mkdir_*] are absent from this file too,
       and [cr_alloc_half] still takes one parked body instead of two.

   (2) ARM F-OK COMES BACK, and it is the point of the file.  At
       [ty = T_DEVICE] the [bne s2,2] at +0x5c is always TAKEN and the
       found half's only exit is ARM F-BAD; at [T_FILE] it always FALLS
       THROUGH, and the found node's own type is inspected by the
       [lhu]/[addiw]/[slli]/[srli]/[bltu] span at +0x60..+0x6c.  So this
       file restores, verbatim from [ProofCreate.v], the whole
       +0x5c..+0x70 F-OK path and ARM F-BAD's SECOND entry -- about 250
       lines -- and refutes the OTHER bullet ([ty <> T_FILE]) instead.

   (3) THE TWO FIRES ARE THE SAME TWO INSTANTS, at the file child.

       [FIRE 1] is unchanged as an instant: [FsAbsMknodFire.
       mkf_dlookup_fire] at the exists-lookup's FOUND instant, off the
       parent's own era fragment.  What changed is WHO CONSUMES IT.  In
       [ProofCreateAU] the [cau_fail] payout was assembled ONE LINE after
       the fire, because ARM F-BAD was the found half's only exit; here the
       receipt has TWO possible consumers -- ARM F-BAD (which folds it into
       [cauf_fail]) and ARM F-OK (which hands it out as [cauf_ok]'s
       [made = false] arm) -- so the assembly is DEFERRED to each of the
       two leaves and [HFex] is carried down the +0x4a..+0x6c walk instead.

       [FIRE 2] is [FsAbsCreateFire.caf_acre_fire_file] in place of
       [FsAbsMknodFire.mkf_acre_fire]: the same fused parent-row retag, at
       the child [AFile []] rather than [ADev ma mi].  The new leaf exists
       because [mkf_acre_fire] is device-pinned only through
       [SpecSysMknodAU.delta_create_dev], whose proof uses nothing about a
       device but that it is not a directory -- see that file's header.
       Its reading bridge is [caf_child_file] ([mkf_child_dev]'s twin off
       [cr_setf_fresh_made]).

   (4) THE PAYOUTS ARE KEYED ON [made].  [SpecCreateAU]'s [cau_ok] said
       "[made] is forced"; [SpecCreateAUF.cauf_ok] takes [made] as an
       index, and its two arms are ARM C-OK (the delta fired, the exists
       commit refunded) and ARM F-OK (the observation fired, the create
       commit refunded).  [cauf_fail] is [cau_fail] at the file child.
       The seven exits are otherwise where they were: [cr_cont_body],
       [cr_alloc_body], [cr_fail_body], [cr_found_half], [cr_alloc_half]
       and [cr_fail_half] carry the same four parameters
       [P Pmiss Φarm Φun Φok Φex].

   ==== the ProofCreateAU header's own diff note follows =================
...
Module CreateAUFProof (NP : NPAR_WRAP_ERA) (IL : ILOCK) (IUP : IUNLOCKPUT)
                      (DL : DIRLOOKUP) (IA : IALLOC) (IU : IUPDATE)
                      (DLK : DIRLINK) : CREATE_AUF.
...
Section ProofCreateMain.
  ...
  (* ...THE ARM IS A LEG OF create's AU (round E2, lane E2-C; the twin of
     ...
     back beside the moved fragment. *)
    ...
    (∀ (mf : regfile) (ok made : bool)
       ...
       (if ok
        then ⌜mf !!! Regidx Ra0 = ientry k
          ...
          cauf_ok (fs_gamma_L fsc_fs) P Φarm Φun Φok Φex (bview plen pfun) made
                  (bv_unsigned inum)
        else ⌜mf !!! Regidx Ra0 = (mword_of_int 0 : mword 64)⌝ ∗ log_tx icfg_log ∗
          cauf_fail (fs_gamma_L fsc_fs) fsc_fs P Pmiss Φarm Φun Φok Φex (bview plen pfun)) -∗
  ...
  Definition cr_alloc_body
    ...
    (∀ (Ma : regfile) (w5 : mword 64)
       ...
       cre_child_unfired (fs_gamma_L fsc_fs) (AFile []) Φarm Φun -∗
 ...
 dind
      ...
      + (* ========== ARM G: the guard FIRES -- nlink == 0 ============= *)
        ...
        iAssert (cauf_fail (fs_gamma_L fsc_fs) fsc_fs P Pmiss Φarm Φun Φok Φex (bview plen pfun))
          with "[HPpar Hacre Hdlkc Hclegs]" as "Hcf".
        { rewrite /cauf_fail. iRight. iExists (bv_unsigned dind).
 ...
 γf (ientry kd) dind bml datl
        ...
        * (* ========================================================== *)
          ...
          (* THE RECEIPT IS NOT FOLDED HERE.  [ProofCreateAU] assembles
             [cau_fail] on this line, because at [T_DEVICE] ARM F-BAD is
             the found half's only exit.  At [T_FILE] the [bne] at +0x5c
             falls through and the type span at +0x60..+0x6c decides
             between ARM F-BAD (a directory: fold into [cauf_fail]) and ARM
             F-OK (a file or a device: hand out as [cauf_ok]'s
             [made = false] arm), so [HFex] -- with the cursor [HPpar] and
             the unfired create commit [Hacre] -- rides down to whichever
             leaf is reached and is assembled there. *)
 ...
 cinum pidv (DfracOwn (1/4)) dqs F5 (K - 10)%nat eb b lks
          ...
          iAssert (□ wp_next (CID0 := CID) true (proc_addr j) (fun CIDb : CpuId =>
                     ∀ Mb : regfile,
                       ...
                       cauf_fail (fs_gamma_L fsc_fs) fsc_fs P Pmiss Φarm Φun Φok Φex
                         (bview plen pfun) -∗
                       wp_next (CID0 := CID) true (proc_addr j)
                         (fun CIDc : CpuId =>
                            cr_cont_body γf
 ...
 kslot (qq/2)%Qp (qq/2)%Qp gc lock tlc (DepTx (qq/2)%Qp icfg_dev cinum gc lock t (1/2)%Qp) cinum dnc bmc
          ...
          -- (* ===== THE PIN'S OWN BULLET: [ty = T_FILE], so the [bne]
                 at +0x5c FALLS THROUGH and the FOUND NODE's type decides
                 between ARM F-BAD and ARM F-OK.  Everything from here to
                 the end of this bullet is [ProofCreate.v]'s +0x5c..+0x70
                 verbatim, with three AU seams marked below. ===== *)
      ...
      iDestruct "Hcmeta" as "(Hcity & Hcimaj & Hcimin & Hcinl & Hcisz)".
             ...
             ++ (* ===== ARM F-BAD (second entry): the type is out of
                ...
                iDestruct (cpu_own_transport CIDic CID37 0%nat eb
                  ...
                  as "Hcnt".
                (* THE AU PAYOUT ON ARM F-BAD, assembled HERE rather than
                   above the type tests (see the note at FIRE 1): the exists
                   observation FIRED and the create commit goes home unfired
                   -- [cauf_fail]'s right disjunct, LEFT alternative, which
                   is where [ProofCreateAU]'s single F-BAD entry puts it. *)
                iAssert (cauf_fail (fs_gamma_L fsc_fs) fsc_fs P Pmiss
                           Φarm Φun Φok Φex (bview plen pfun))
                ...
                { rewrite /cauf_fail. iRight. iExists (bv_unsigned dind).
                  ...
                  { iLeft.
                    ...
                    iSplitR;
                      [iPureIntro; exact (cr_last_of_npar _ nfp Hnpname) |].
             ...
             ++ (* ===== ARM F-OK: the found inode is a file or a device *)
                ...
                { iPureIntro. split; [rewrite Ha0f; exact HFBs2 |].
                  ...
                  (* [made = false] keys the post to ARM F-OK's reading;
                     [ty = T_FILE] is the contract's PREMISE here, not a
                     payout, so the F-OK conjunct is the type disjunction
                     alone. *)
                  exact (cr_trange_in (di_type dnc) Hrng). }
                (* ============================================================ *)
                (*  ARM F-OK's AU PAYOUT.  NOTHING MOVED ON THIS ARM: xv6      *)
                (*  opens the file that was already there, so the create        *)
                (*  commit goes home UNFIRED and the receipt handed out is the  *)
                (*  EXISTS observation FIRE 1 took at the lookup, off the       *)
                (*  parent's own era fragment under the parent's lock.  This    *)
                (*  is the arm [SpecCreateAU] refutes and the one                *)
                (*  [SpecSysOpenAU]'s create arm calls EXISTS-OPENS.            *)
                (* ============================================================ *)
                iSplitR "HPpar Hacre HFex Hclegs".
                ...
                iExists avx, (dir_entries (era_node dnl bml datl)),
                        (fn_nlink (era_node dnl bml datl)).
                iSplitR; [by iPureIntro |]. iSplitR; [by iPureIntro |].
                (* ARM F-OK moved nothing: the child's two legs come home *)
                iFrame "Hacre HFex". iExact "Hclegs".
          -- (* [ty = T_FILE] REFUTES ARM F-BAD's FIRST ENTRY: the [bne
                s2,2] at +0x5c compares the REQUESTED type against T_FILE,
                and the pin makes it fall through.  This is the bullet
                [ProofCreateAU] proves and this file cannot reach, the
                mirror image of its own F-OK refutation. *)
             exfalso. exact (Htyf Hty).
        * (* ========================================================== *)
    ...
    - (* ============================================================== *)
      ...
      { iDestruct (np_dead_to_mknod fsc_fs P Pmiss (bview plen pfun)
                     with "Hdead") as "[Hd | Hp]".
        - rewrite /cauf_fail. iLeft. iFrame "Hd Hacre Hdlkc Hclegs".
        - iDestruct "Hp" as (dpar) "HPd". rewrite /cauf_fail. iRight.
  ...
  Lemma cr_alloc_half
    ...
    (* ---- ARM FAIL's NON-DIRECTORY ENTRY, PARKED.  The T_DIR sub-branch's
       premise is GONE: at [ty = T_FILE] the +0xca [beq] is never taken. ---- *)
    (∀ (kd : nat) (qd : Qp) (gd γil γisl : gname) (dind : mword 32)
       (dn : dinode) (bm : blkmap) (data : nat -> list (bv 8))
       (nf nsl : nat -> bv 8) (t : nat),
 ...
 ty kd (DfracOwn (1/2))
    ...
    - (* ============================================================== *)
      ...
      (* THE ARM FIRES HERE (round E2, lane E2-C, site #23): the claim box
         has no row ([fresh_shape]: count 0), and the record the count store
         leaves reads as the empty FILE at count 1 ([caf_child_file] off
         [cr_setf_fresh_made]). *)
      ...
      assert (Hrowc : abs_of (era_node (cr_setf dnc major minor
                      ...
                      = Some (MkAnode (AFile []) 1%nat)).
      { apply (caf_child_file _ bmc datc major minor).
        rewrite (cr_setf_fresh_made dnc ty major minor Hfresh Htyc).
        by rewrite Hty. }
      iApply fupd_wp.
      iMod (cr_dirty_arm ⊤ t (bv_unsigned cinum) (AFile []) Φarm
 ...
 (ientry kslot) cinum
      ...
      + (* [ty = T_FILE] REFUTES THE WHOLE T_DIR SUB-BRANCH, exactly as
           [T_DEVICE] does in [ProofCreateAU]: the [beq s4,a4] at +0xca
           cannot be taken, so [cr_mkdir_body] / [cr_mkdir_half] /
           [cr_fail_mkdir_*] are gone from this file rather than
           re-proved. *)
 ...
 γf
        ...
        * destruct Harm as (_ & Hwf' & Hholes' & Haddr' & Hsz31' &
          ...
          -- (* ======================================================== *)
             ...
             iMod (caf_acre_fire_file fsc_fs ⊤ Φok (bv_unsigned dind)
             ...
             iAssert (cauf_ok (fs_gamma_L fsc_fs) P Φarm Φun Φok Φex
                        (bview plen pfun) true (bv_unsigned cinum))
             ...
             { rewrite /cauf_ok.
               ...
               iFrame "Hdlkc Harmr Hun". iExact "HFok". }
 ...
 kd (qd/2)%Qp (qd/2)%Qp gd lodc tldc (DepTx (qd/2)%Qp icfg_dev dind gd lodc t (1/4)%Qp) dind dn' bm'
             ...
             { iPureIntro. split; [rewrite Ha0f; exact HY4s2 |].
               ...
               (* [made = true] keys the post to ARM C-OK's reading *)
 ...
 (ientry kslot) cinum
    ...
    { apply (inode_local_of_ok_rec (bv_unsigned cinum) fsc_cov fsc_logst _ bmc datc).
      ...
      - apply dir_uniq_not_dir. rewrite cr_setf_type. exact Htdirz.
      - apply dir_dots_ix_not_dir. rewrite cr_setf_type. exact Htdirz. }
    (* THE UNARM FIRES HERE (round E2, lane E2-C, site #26): the file child
       the arm minted goes to count 0, so its row DISAPPEARS -- the second
       half of ruling Q-h's do-then-undo pair, at a PLAIN fragment (this
       walk disarmed at the count). *)
    ...
    { rewrite /cauf_fail. iRight. iExists (bv_unsigned dind).
      iFrame "HPpar Hacre". iSplitL "Hdlkc"; [iRight; iExact "Hdlkc" |].
      (* ruling Q-h: the row APPEARED at +0xc4 and DISAPPEARED at +0x146 *)
 ...
 plen pfun ty major minor
  ...
  Proof.
    ...
    assert (Htyw : InodeRegion.ireg_ty_ok_w ty)
      by (rewrite Hty; exact T_FILE_ty_ok).
