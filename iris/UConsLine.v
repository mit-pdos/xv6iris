...
(*                                                                        *)
(*  (a) THE EXIT PAYLOAD CANNOT CROSS exec TODAY.  A run's payload         *)
(*      [UkRun.ukn_pay N (-1)] lives INSIDE [UkRun.urun]; the exec'd       *)
(*      image's own run needs it ([UkRun.uslot_of_urun_all] takes [Q (-1)] *)
(*      linearly), and the only linear thing that crosses                  *)
(*      [SpecKexec.exec_slot_pre]'s wands is the slot piece's [Pay], which *)
(*      the exec'ing PROGRAM supplies -- and no program can take its own   *)
(*      payload out of its run.  The kernel does hold it at the trap       *)
(*      ([SpecSyscall.sysc_pay_in], returned unconditionally as            *)
(*      [sysc_pay_out]) and DROPS it on the success arm, where             *)
(*      [SpecSyscall.sysc_exec_out] hands back a finished [uslot].  So     *)
(*      "sh's run at [UserConsole.ucons_pay]" needs the kernel to feed     *)
(*      [sexit_pay f (-1)] into [exec_slot_pre] -- CONS-ROUTE's own move,  *)
(*      one syscall over.  §5's supply is stated at that shape and takes   *)
(*      the payload as an explicit premise, so it is honest today.         *)
(*                                                                        *)
...
Section UConsLine.
  ...
  (*                                                                      *)
  (*  THE EXIT PAYLOAD IS AN EXPLICIT PREMISE, and that is the seam this   *)
  (*  lane found (header (a)): the exec'd image's run needs [Q (-1)] and   *)
  (*  nothing in [SpecKexec.exec_slot_pre] delivers it, so a supply that   *)
  (*  did not ask for it could not build sh's entry.  Asking for it here   *)
  (*  makes the shape honest and localises what the kernel still owes.     *)
  (* =================================================================== *)
  Definition init_exec_sup_lin (cn : cons_names) (γp : gname) (T : iProp Σ)
    ...
    (∀ (N' : uk_names Σ) (m : regfile) (pc : mword 64),
       ...
       upos γp n -∗
       (* ...and the payload itself, which the KERNEL holds at the exec
          trap and does not hand on today -- header (a) *)
       ucons_pay cn γp T (-1) -∗
