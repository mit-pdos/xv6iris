...
Require Import UserPerm UsysMemOk UexecWp UexecRet UkStep UkStore.
...
Section UkLoadPostFetch.
  ...
  Lemma uk_load_post_fetch (R : iProp Σ) (Rut : uptd -> iProp Σ) (sz : Z)
    ...
    (R -∗ (TsoCtx.own_context XI -∗ Rut pt) ∗ Rfd fdv ∗ ukb C pt Rfd Rut sz π fdv cw gn cs pidv false ProcDefs.secc_all ∗
          (uvb C pt Rfd Rut sz π fdv cw gn cs pidv false ProcDefs.secc_all M (<[Regidx lrd := regval_into_reg wval]> m)
             (add_vec_int pc dpc) -∗
  ...
  Proof using .
    ...
    iSplitR;
      [ iApply (uexec_pay_dep_ne _ (uvis_of_run m pc M π sz fdv cw gn cs pidv false ProcDefs.secc_all) _ (sfam_at Qp sfam_pt)
...
Section UkLoad.
  ...
  Lemma wp_uk_cldsp (M : gmap Z (bv 8)) (m : regfile)
      (pc : mword 64) (fdv : list fdstate) (cw : Z) (gn : gname) (cs : gset gname) (pidv : mword 32) (uimm : mword 6) (rd : mword 5)
      (va wval : mword 64) :
