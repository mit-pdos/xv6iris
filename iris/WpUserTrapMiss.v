...
Section WpUserTrapMiss.
  ...
  (* ================================================================== *)
  (* The TLB-MISS breakpoint (EBREAK) trap-fetch engine: same walk-fill + *)
  (* fetch + progress scaffold as the illegal engine, but the execute      *)
  (* traps to a software breakpoint, so the exception tower runs with       *)
  (* cause E_Breakpoint (stval := va) via exec_exception_handler_ne_M       *)
  (* followed by a separate set_next_pc.                                    *)
  (* ================================================================== *)
  Lemma ustep_ebreak_miss (ii : instruction)
      (va : mword 64) (vpn : mword 27) (ie : uwalk_info) (w : mword 32)
      (ms_v sc_v stval_v sepc_v : mword 64)
      (g : gmap regidx (mword 64))
      (tlbvec : vec (option TLB_Entry) (2 ^ 6))
    ...
    WP (Loop : expr riscv_lang) @ E {{ Φ }}.
  Proof.
    intros HN Hexec_bp Hnlpad Hok Hsome Hmiss Hchk0 HupdN Hcw HSXL Hval Hcanon
           Hvpn_def Hpaal HnotRVC Hdec.
    iIntros "#Hhw #Hinv Hhs Hpriv Hms Hsc Hstv Hsepc Htlbc Hpc Hgpr Hupt
             #Hcode Hdata Hcfg Hcont".
    iPoseProof "Hhw" as "#Hhwc".
    ...
    assert (Hlpad : eq_vec (register_lookup elp σ'.(sregs))
                      (landing_pad_bits_backwards LP_EXPECTED) = false)
      by (rewrite LelpS; exact Help_np).
    (* ---- execute -> Trap(Breakpoint), at the filled state ---- *)
    set (s_x := set_reg σ' nextPC (add_vec_int va 4)).
    ...
    { apply (exec_hart_active_progress_base_gen User σ σ' s_x w ii va
               ...
               Lpriv Hdisp Hfetch Hdec' Hlpad Hnlpad LpcS).
      - exact (Hexec_bp s_x LprivXe LpcXe).
      - exact I. }
    (* ---- the dispatch arm: exception_handler at s_x ---- *)
    assert (LprivX : register_lookup cur_privilege s_x.(sregs) = User)
      by (unfold s_x; lk; exact Lpriv).
    assert (LmsX : register_lookup mstatus s_x.(sregs) = ms_v)
      by (unfold s_x; lk; exact Lms).
    ...
    (* ---- ghost updates: fill the tlb, then the tower ---- *)
    iMod (reg_update _ tlb _ tlbvec' with "Hreg Htlbc") as "[Hreg Htlbc]".
    iMod (reg_update _ nextPC _ (add_vec_int va 4) with "Hreg Hnpc") as "[Hreg Hnpc]".
    iMod (reg_update _ mstatus _ (update_subrange_vec_dec ms_v 23 23 elp0)
            with "Hreg Hms") as "[Hreg Hms]".
  ...
  (* Compressed EBREAK trap-fetch, TLB-miss path (walk-fill + 2-alignment F_RVC). *)
  Lemma ustep_c_ebreak_miss (ii : instruction)
    ...
    WP (Loop : expr riscv_lang) @ E {{ Φ }}.
  Proof.
    intros HN Hexp Hok Hsome Hmiss Hchk0 HupdN HSXL Hcanon Hvpn_def
           Hmode HisRVC Hdec.
    iIntros "#Hhw #Hinv Hhs Hpriv Hms Hsc Hstv Hsepc Htlbc Hpc Hgpr Hupt
             #Hcode Hdata Hcfg Hcont".
    iPoseProof "Hhw" as "#Hhwc".
    ...
    { apply exec_currentlyEnabled_Zca. unfold σ'; lk. rewrite Lmisa. exact HmisaC. }
    pose proof (agree_u_set_tlb σ tlbvec'
                  (agree_u σ Lpriv Lmenv' Lsenv Lmst0 Lsst0 Lmisa')) as HagX.
    pose proof (Hdec σ' HagX) as Hdec'.
    assert (Hlpad : eq_vec (register_lookup elp σ'.(sregs))
                      (landing_pad_bits_backwards LP_EXPECTED) = false)
      by (rewrite LelpS; exact Help_np).
    (* ---- fetch: unified 2-alignment F_RVC over the filled state ---- *)
    iAssert (⌜exec (fetch tt) σ = Some (F_RVC h, σ')⌝)%I as %Hfetch.
    { destruct Hmode as
      ...
      - iAssert (⌜forall j : nat, (N.of_nat j < 2)%N ->
        ...
        exact (exec_fetch_F_RVC_2_U_gen va pa h σ σ' region
                 ...
                 (within_htif_false pa 2 σ' LhtifS)
                 (addr_is_ram_not_dev _ Hram)
                 Hbf LprivS HisRVC). }
    (* ---- execute -> Trap(Breakpoint), at the filled state ---- *)
    ...
    { apply (exec_hart_active_progress_RVC_gen User σ σ' s_x h ii (EBREAK tt) va
               (rv64d_types.Trap (User, make_sync_exception (E_Breakpoint Brk_Software) va, va))
               Lpriv Hdisp Hfetch Hdec' Hlpad LpcS HZca').
      - apply Hexp.
      - exact HexecB. }
    (* ---- the dispatch arm: exception_handler at s_x ---- *)
    assert (LprivX : register_lookup cur_privilege s_x.(sregs) = User)
  ...
  (* Compressed illegal-instruction trap-fetch, TLB-miss path. *)
  Lemma ustep_c_illegal_miss (ii : instruction)
      (va : mword 64) (vpn : mword 27) (ie : uwalk_info) (h : mword 16)
      (ms_v sc_v stval_v sepc_v : mword 64)
      (g : gmap regidx (mword 64))
      (tlbvec : vec (option TLB_Entry) (2 ^ 6))
      E (Φ : mval -> iProp Σ) :
    ↑minstretN ⊆ E ->
    (forall s : mstate, exec (execute ii) s = Some (Illegal_Instruction tt, s)) ->
    upt_tlb_ok spec tlbvec ->
    spec !! vpn = Some ie ->
    (vec_access_dec tlbvec (tlb_hash (__id 39) vpn) = None \/
     (exists ent', vec_access_dec tlbvec (tlb_hash (__id 39) vpn) = Some ent' /\
    ...
    WP (Loop : expr riscv_lang) @ E {{ Φ }}.
  Proof.
    intros HN Hill Hok Hsome Hmiss Hchk0 HupdN HSXL Hcanon Hvpn_def
           Hmode HisRVC Hdec.
    iIntros "#Hhw #Hinv Hhs Hpriv Hms Hsc Hstv Hsepc Htlbc Hpc Hgpr Hupt
             #Hcode Hdata Hcfg Hcont".
    iPoseProof "Hhw" as "#Hhwc".
    ...
    { exact (exec_hart_active_progress_RVC_direct_gen User σ σ' s_x h ii va
               (Illegal_Instruction tt) Lpriv Hdisp Hfetch Hdec' Hlpad LpcS HZca'
               (Hill s_x) I). }
    (* ---- the dispatch arm: handle_exception at s_x ---- *)
    assert (LprivX : register_lookup cur_privilege s_x.(sregs) = User)
      by (unfold s_x; lk; exact Lpriv).
  ...
  (* Combined EBREAK trap-fetch engine: dispatch on the TLB slot. *)
  Lemma ustep_ebreak_u (ii : instruction)
      (va : mword 64) (vpn : mword 27) (ie : uwalk_info) (w : mword 32)
      (ms_v sc_v stval_v sepc_v : mword 64)
      (g : gmap regidx (mword 64))
      (tlbvec : vec (option TLB_Entry) (2 ^ 6))
    ...
    WP (Loop : expr riscv_lang) @ E {{ Φ }}.
  Proof.
    intros HN Hexec_bp Hnlpad Hok Hsome Hchk0 HupdN Hcw HSXL Hval Hcanon
           Hvpn_def Hpaal HnotRVC Hdec.
    destruct (Hspec vpn ie Hsome) as (_ & _ & _ & Hwf).
    destruct Hwf as (_ & _ & _ & _ & _ & _ & _ & _ & Hpbmt0).
    destruct (vec_access_dec tlbvec (tlb_hash (__id 39) vpn)) as [ent|] eqn:Hvacc.
    - destruct (match_TLB_Entry ent (mword_of_int 0 : mword 16)
      ...
      + destruct (Hok vpn ent Hvacc) as (vpn'' & i & Hspec'' & _ & Hent).
        ...
        pose proof (upt_entry_match_inj vpn'' vpn i Hmatch) as Hvv. subst vpn''.
        rewrite Hsome in Hspec''. inversion Hspec''. subst i.
        iApply (WpUserTrap.ustep_ebreak U ii va vpn ie w ms_v sc_v stval_v sepc_v g tlbvec E Φ
                  HN Hexec_bp Hnlpad Hok Hvacc Hchk0 HupdN Hpbmt0 Hcw HSXL Hval Hcanon
                  Hvpn_def Hpaal HnotRVC Hdec).
      + iApply (ustep_ebreak_miss ii va vpn ie w ms_v sc_v stval_v sepc_v g tlbvec E Φ
                  HN Hexec_bp Hnlpad Hok Hsome
                  (or_intror (ex_intro _ ent (conj Hvacc Hmatch)))
                  Hchk0 HupdN
                  ltac:(intros j Hj; rewrite <- (WpUserComputeMiss.u_pa_upt_entry_walk vpn ie va); exact (Hcw j Hj))
                  HSXL Hval Hcanon Hvpn_def
                  ...
                  HnotRVC Hdec).
    - iApply (ustep_ebreak_miss ii va vpn ie w ms_v sc_v stval_v sepc_v g tlbvec E Φ
                HN Hexec_bp Hnlpad Hok Hsome (or_introl Hvacc)
                Hchk0 HupdN
                ltac:(intros j Hj; rewrite <- (WpUserComputeMiss.u_pa_upt_entry_walk vpn ie va); exact (Hcw j Hj))
                HSXL Hval Hcanon Hvpn_def
                ltac:(rewrite <- (WpUserComputeMiss.u_pa_upt_entry_walk vpn ie va); exact Hpaal)
  ...
  (* Combined compressed-EBREAK trap-fetch engine: dispatch on the TLB slot. *)
  Lemma ustep_c_ebreak_u (ii : instruction)
      (va : mword 64) (vpn : mword 27) (ie : uwalk_info) (h : mword 16)
      (ms_v sc_v stval_v sepc_v : mword 64)
      (g : gmap regidx (mword 64))
      (tlbvec : vec (option TLB_Entry) (2 ^ 6))
    ...
    WP (Loop : expr riscv_lang) @ E {{ Φ }}.
  Proof.
    intros HN Hexp Hok Hsome Hchk0 HupdN HSXL Hcanon Hvpn_def Hmode HisRVC Hdec.
    destruct (Hspec vpn ie Hsome) as (_ & _ & _ & Hwf).
    destruct Hwf as (_ & _ & _ & _ & _ & _ & _ & _ & Hpbmt0).
    ...
    - destruct (match_TLB_Entry ent (mword_of_int 0 : mword 16)
      ...
      + destruct (Hok vpn ent Hvacc) as (vpn'' & i & Hspec'' & _ & Hent).
        ...
        pose proof (upt_entry_match_inj vpn'' vpn i Hmatch) as Hvv. subst vpn''.
        rewrite Hsome in Hspec''. inversion Hspec''. subst i.
        iApply (WpUserTrap.ustep_c_ebreak U ii va vpn ie h ms_v sc_v stval_v sepc_v g tlbvec E Φ
                  HN Hexp Hok Hvacc Hchk0 HupdN Hpbmt0 HSXL Hcanon Hvpn_def Hmode HisRVC Hdec).
      + iApply (ustep_c_ebreak_miss ii va vpn ie h ms_v sc_v stval_v sepc_v g tlbvec E Φ
                  HN Hexp Hok Hsome
                  (or_intror (ex_intro _ ent (conj Hvacc Hmatch)))
                  Hchk0 HupdN HSXL Hcanon Hvpn_def Hmode_w HisRVC Hdec).
    - iApply (ustep_c_ebreak_miss ii va vpn ie h ms_v sc_v stval_v sepc_v g tlbvec E Φ
                HN Hexp Hok Hsome (or_introl Hvacc)
                Hchk0 HupdN HSXL Hcanon Hvpn_def Hmode_w HisRVC Hdec).
  Qed.

  (* Combined compressed illegal-instruction trap-fetch engine. *)
  Lemma ustep_c_illegal_u (ii : instruction)
      (va : mword 64) (vpn : mword 27) (ie : uwalk_info) (h : mword 16)
      (ms_v sc_v stval_v sepc_v : mword 64)
      (g : gmap regidx (mword 64))
      (tlbvec : vec (option TLB_Entry) (2 ^ 6))
    ...
    WP (Loop : expr riscv_lang) @ E {{ Φ }}.
  Proof.
    intros HN Hill Hok Hsome Hchk0 HupdN HSXL Hcanon Hvpn_def Hmode HisRVC Hdec.
    destruct (Hspec vpn ie Hsome) as (_ & _ & _ & Hwf).
    destruct Hwf as (_ & _ & _ & _ & _ & _ & _ & _ & Hpbmt0).
    pose proof Hmode as Hmode_w.
    rewrite (WpUserComputeMiss.u_pa_upt_entry_walk vpn ie va) in Hmode_w.
    ...
    - destruct (match_TLB_Entry ent (mword_of_int 0 : mword 16)
      ...
      + destruct (Hok vpn ent Hvacc) as (vpn'' & i & Hspec'' & _ & Hent).
        ...
        pose proof (upt_entry_match_inj vpn'' vpn i Hmatch) as Hvv. subst vpn''.
        rewrite Hsome in Hspec''. inversion Hspec''. subst i.
        iApply (WpUserTrap.ustep_c_illegal U ii va vpn ie h ms_v sc_v stval_v sepc_v g tlbvec E Φ
                  HN Hill Hok Hvacc Hchk0 HupdN Hpbmt0 HSXL Hcanon Hvpn_def Hmode HisRVC Hdec).
      + iApply (ustep_c_illegal_miss ii va vpn ie h ms_v sc_v stval_v sepc_v g tlbvec E Φ
                  HN Hill Hok Hsome
                  (or_intror (ex_intro _ ent (conj Hvacc Hmatch)))
                  Hchk0 HupdN HSXL Hcanon Hvpn_def Hmode_w HisRVC Hdec).
    - iApply (ustep_c_illegal_miss ii va vpn ie h ms_v sc_v stval_v sepc_v g tlbvec E Φ
                HN Hill Hok Hsome (or_introl Hvacc)
                Hchk0 HupdN HSXL Hcanon Hvpn_def Hmode_w HisRVC Hdec).
  Qed.

  (* ================================================================== *)
  (* The TLB-MISS ECALL trap-fetch engine: the ecall word (ecall_w) walks  *)
  (* + fills the TLB, fetches, decodes to ECALL, and the execute traps to   *)
  (* an environment call, running the U trap tower with cause E_U_EnvCall   *)
  (* (stval := 0) via exec_exception_handler_ne_M + set_next_pc.  Decode,    *)
  (* is_lpad and isRVC are all baked (ECALL / ecall_w are concrete).         *)
  (* ================================================================== *)
  Lemma ustep_ecall_miss
    ...
    update_PTE_Bits (uw_pte0 ie) (InstructionFetch tt) = None ->
    (forall j : nat, (j < 4)%nat ->
       code !! pa_add (u_walk_pa (uw_pte0 ie) va) j = Some (nth_byte ecall_w j)) ->
    _get_Mstatus_SXL ms_v = 'b"10" ->
    is_aligned_vaddr (Virtaddr va) 4 = true ->
    neq_vec (bits_of_virtaddr (Virtaddr va))
       (sign_extend' 64 (subrange_vec_dec (bits_of_virtaddr (Virtaddr va)) (Z.sub 39 1) 0)) = false ->
    ...
    WP (Loop : expr riscv_lang) @ E {{ Φ }}.
  Proof.
    intros HN Hok Hsome Hmiss Hchk0 HupdN Hcw HSXL Hval Hcanon Hvpn_def Hpaal.
    iIntros "#Hhw #Hinv Hhs Hpriv Hms Hsc Hstv Hsepc Htlbc Hpc Hgpr Hupt
             #Hcode Hdata Hcfg Hcont".
    iPoseProof "Hhw" as "#Hhwc".
    iDestruct "Hhwc" as (misa0 mseccfg0 pmar0 elp0)
    ...
    pose proof (agree_u_set_tlb σ tlbvec'
                  (agree_u σ Lpriv Lmenv' Lsenv Lmst0 Lsst0 Lmisa')) as HagX.
    pose proof (decode_ecall_u σ' HagX) as Hdec'.
    assert (Hlpad : eq_vec (register_lookup elp σ'.(sregs))
                      (landing_pad_bits_backwards LP_EXPECTED) = false)
      by (rewrite LelpS; exact Help_np).
    (* ---- execute -> Trap(E_U_EnvCall), at the filled state ---- *)
    set (s_x := set_reg σ' nextPC (add_vec_int va 4)).
    assert (LprivXe : register_lookup cur_privilege s_x.(sregs) = User)
      by (unfold s_x; lk; exact Lpriv).
    assert (LpcXe : register_lookup PC s_x.(sregs) = va)
      by (unfold s_x; lk; exact Lpc).
    pose proof (exec_execute_ECALL_U va s_x LprivXe LpcXe) as Hexec.
    assert (Hha : exec (run_hart_active 0) σ
    ...
    { apply (exec_hart_active_progress_base_gen User σ σ' s_x ecall_w (ECALL tt) va
               (rv64d_types.Trap (User, ecall_u_exc, va))
               Lpriv Hdisp Hfetch Hdec' Hlpad ltac:(vm_compute; reflexivity) LpcS
               Hexec I). }
    (* ---- the dispatch arm: exception_handler at s_x ---- *)
    assert (LprivX : register_lookup cur_privilege s_x.(sregs) = User)
      by (unfold s_x; lk; exact Lpriv).
    ...
    { unfold s_x; lk. rewrite Lmisa. exact HmisaS. }
    assert (LmedlX : bit_to_bool (access_vec_dec (register_lookup medeleg s_x.(sregs))
                       (uint (exceptionType_bits_forwards (ecall_u_exc.(sync_exception_trap))))) = true).
    { unfold s_x; lk. rewrite Lmedl. exact Hdel_ecall. }
    pose proof (exec_exception_handler_ne_M s_x ecall_cause User va None
                    ms_v sc_v stvec_v elp0
                    (or_introl eq_refl) LprivX LmsX LscX LstvecX LelpX LmisaSX Htvd
                    ecall_u_exc eq_refl eq_refl eq_refl LmedlX) as Hhe.
    match type of Hhe with _ = Some (_, ?T) => set (s9 := T) in Hhe end.
    set (s_trap := set_reg s9 nextPC (stvec_base stvec_v)).
    assert (Hdispb : exec (try_step_dispatch
    ...
    { unfold try_step_dispatch. cbn match.
      rewrite (exec_bind_Some _ _ _ _ _ Hhe). apply exec_set_next_pc. }
    (* ---- ghost updates: fill the tlb, then the U trap tower ---- *)
    iMod (reg_update _ tlb _ tlbvec' with "Hreg Htlbc") as "[Hreg Htlbc]".
    iMod (reg_update _ nextPC _ (add_vec_int va 4) with "Hreg Hnpc") as "[Hreg Hnpc]".
    iMod (reg_update _ mstatus _ (u_ms_e ms_v elp0) with "Hreg Hms") as "[Hreg Hms]".
    assert (Hlkelp : register_lookup elp
              (register_set mstatus (u_ms_e ms_v elp0) s_x.(sregs))
            = landing_pad_bits_backwards NO_LP_EXPECTED).
    { rewrite irrelevant_register_set; [ | vm_compute; reflexivity ].
      rewrite LelpX. exact Help0. }
    iDestruct (reg_interp_set_same _ elp _ Hlkelp with "Hreg") as "Hreg".
    iMod (reg_update _ scause _ (u_c1 sc_v) with "Hreg Hsc") as "[Hreg Hsc]".
    iMod (reg_update _ scause _ (u_trap_cause sc_v) with "Hreg Hsc") as "[Hreg Hsc]".
    ...
    iMod (reg_update _ stval _ (tval None) with "Hreg Hstv") as "[Hreg Hstv]".
    iMod (reg_update _ sepc _ va with "Hreg Hsepc") as "[Hreg Hsepc]".
    iMod (reg_update _ cur_privilege _ Supervisor with "Hreg Hpriv") as "[Hreg Hpriv]".
    iMod (reg_update _ nextPC _ (stvec_base stvec_v) with "Hreg Hnpc") as "[Hreg Hnpc]".
    iModIntro.
    ...
    iSplitR; [iPureIntro; reflexivity |].
    assert (LpcT : register_lookup PC s_trap.(sregs) = va).
    { unfold s_trap, s9, s_x. lk. exact Lpc. }
    rewrite LpcT.
    iSplitL "Hpcr"; [iExact "Hpcr" |].
    iSplitL "Hreg Hmem Hdev".
    { unfold s_trap, s9, s_x, set_reg; cbn [sregs mem mdev].
      unfold u_trap_ms, u_ms_b, u_ms_a, u_ms_e, u_trap_cause, u_c1.
      iFrame "Hreg Hmem Hdev". }
    iNext.
    iIntros "Hhs Hpcr".
    assert (LnT : register_lookup nextPC s_trap.(sregs) = stvec_base stvec_v).
    { unfold s_trap. cbn [sregs]. rewrite register_lookup_set. reflexivity. }
    rewrite LnT.
    rewrite Help0.
    iApply "Hcont".
    rewrite /user_trap_frame.
  ...
  (* Combined ECALL trap-fetch engine: dispatch on the TLB slot. *)
  Lemma ustep_ecall_u
      (va : mword 64) (vpn : mword 27) (ie : uwalk_info)
      (ms_v sc_v stval_v sepc_v : mword 64)
      (g : gmap regidx (mword 64))
    ...
    update_PTE_Bits (uw_pte0 ie) (InstructionFetch tt) = None ->
    (forall j : nat, (j < 4)%nat ->
       code !! pa_add (u_pa (upt_entry vpn ie) va vpn) j = Some (nth_byte ecall_w j)) ->
    _get_Mstatus_SXL ms_v = 'b"10" ->
    is_aligned_vaddr (Virtaddr va) 4 = true ->
    neq_vec (bits_of_virtaddr (Virtaddr va))
       (sign_extend' 64 (subrange_vec_dec (bits_of_virtaddr (Virtaddr va)) (Z.sub 39 1) 0)) = false ->
  ...
  Proof.
    ...
    - destruct (match_TLB_Entry ent (mword_of_int 0 : mword 16)
      ...
      + destruct (Hok vpn ent Hvacc) as (vpn'' & i & Hspec'' & _ & Hent).
        ...
        pose proof (upt_entry_match_inj vpn'' vpn i Hmatch) as Hvv. subst vpn''.
        rewrite Hsome in Hspec''. inversion Hspec''. subst i.
        iApply (WpUserTrap.ustep_ecall U va vpn ie ms_v sc_v stval_v sepc_v g tlbvec E Φ
                  HN Hsome Hok Hvacc Hchk0 HupdN Hcw HSXL Hval Hcanon Hvpn_def Hpaal).
      + iApply (ustep_ecall_miss va vpn ie ms_v sc_v stval_v sepc_v g tlbvec E Φ
                  ...
                  ltac:(rewrite <- (WpUserComputeMiss.u_pa_upt_entry_walk vpn ie va); exact Hpaal)).
    - iApply (ustep_ecall_miss va vpn ie ms_v sc_v stval_v sepc_v g tlbvec E Φ
                HN Hok Hsome (or_introl Hvacc) Hchk0 HupdN
                ltac:(intros j Hj; rewrite <- (WpUserComputeMiss.u_pa_upt_entry_walk vpn ie va); exact (Hcw j Hj))
                HSXL Hval Hcanon Hvpn_def
                ltac:(rewrite <- (WpUserComputeMiss.u_pa_upt_entry_walk vpn ie va); exact Hpaal)).
  Qed.
