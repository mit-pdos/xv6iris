(* ===================================================================== *)
(* UkSeccPutc.v -- ulib's [putc(fd, c)] at seccomp's addresses, the      *)
(* bottom of seccomp's fprintf cone, and the WRITE OBLIGATIONS it spends.*)
(*                                                                       *)
(* A PORT of [UkGrepPutc.v] to seccomp's image: the same C, the same     *)
(* instructions, every pc 0x1d0 further down (putc 0x5cc -> 0x3fc, the   *)
(* write stub 0x53c -> 0x36c); the comments still speak of cat and grep  *)
(* where they did.  [ksecc_w] / [ksecc_wb] / [ksecc_pay_seq] are         *)
(* [UkCat.kcat_w] / [kcat_wb] / [kcat_pay_seq] at seccomp's code and     *)
(* write stub, so every walk above states its output exactly as cat's.   *)
(* The duplication across programs is program-specs.md SS1's item 2.     *)
(* ===================================================================== *)
...
Section UkSeccW.
  ...
  (* ===================================================================== *)
  Definition ksecc_w (fdw ua : mword 64) (nb : nat) (Ci Co : iProp Σ)
    : iProp Σ :=
    (∀ (h : CpuId) (m : regfile) (avail : nat),
       ...
       seccomp_code γt -∗
       Ci -∗
       urun N h m (mword_of_int SeccompSyms.write) avail -∗
  ...
  Lemma ksecc_w_mono (fdw ua : mword 64) (nb : nat) (Ci Co Co' : iProp Σ) :
    (Co -∗ Co') -∗ ksecc_w fdw ua nb Ci Co -∗ ksecc_w fdw ua nb Ci Co'.
  Proof using .
  ...
  Lemma ksecc_w_mono_in (fdw ua : mword 64) (nb : nat) (Ci Ci' Co : iProp Σ) :
    (Ci' -∗ Ci) -∗ ksecc_w fdw ua nb Ci Co -∗ ksecc_w fdw ua nb Ci' Co.
  Proof using .
    iIntros "Hm Hw" (h m avail) "%Ha0 %Ha1 %Ha2 #Hcode HCi Hrun Hcont".
    iApply ("Hw" $! h m avail
  ...
  (* ...AND IT FRAMES: a caller that HOLDS the input half a payment asks
     ...
     meet here. *)
  Lemma ksecc_w_frame (fdw ua : mword 64) (nb : nat) (Ci Co C : iProp Σ) :
    C -∗ ksecc_w fdw ua nb (Ci ∗ C) Co -∗ ksecc_w fdw ua nb Ci Co.
  Proof using .
  ...
  Definition ksecc_wb (fdw : mword 64) (b : bv 8) (Ci Co : iProp Σ) : iProp Σ :=
    (∀ ua : mword 64,
       ksecc_w fdw ua 1%nat
         (Ci ∗ ubyte γd (uint ua) b) (Co ∗ ubyte γd (uint ua) b))%I.

  Lemma ksecc_wb_mono (fdw : mword 64) (b : bv 8) (Ci Co Co' : iProp Σ) :
    (Co -∗ Co') -∗ ksecc_wb fdw b Ci Co -∗ ksecc_wb fdw b Ci Co'.
  Proof using .
  ...
  Lemma ksecc_wb_mono_in (fdw : mword 64) (b : bv 8) (Ci Ci' Co : iProp Σ) :
    (Ci' -∗ Ci) -∗ ksecc_wb fdw b Ci Co -∗ ksecc_wb fdw b Ci' Co.
  Proof using .
    iIntros "Hm Hw" (ua h m avail)
  ...
  Lemma ksecc_wb_frame (fdw : mword 64) (b : bv 8) (Ci Co C : iProp Σ) :
    C -∗ ksecc_wb fdw b (Ci ∗ C) Co -∗ ksecc_wb fdw b Ci Co.
  Proof using .
  ...
  Lemma ksecc_nth_byte0_moi (b : bv 8) :
  ...
  Lemma ksecc_nth_byte0_zext (b : mword 8) :
    nth_byte (zero_extend' 64 b : mword 64) 0%nat = b.
  Proof using . rewrite zext8_moi. apply ksecc_nth_byte0_moi. Qed.

  Fixpoint ksecc_pay_seq (fdw : mword 64) (fb : nat -> bv 8) (i k : nat)
      (Ci Cend : iProp Σ) : iProp Σ :=
    match k with
    | O => (Ci -∗ Cend)%I
    | S k' => (∃ Cm : iProp Σ,
                 ksecc_wb fdw (fb i) Ci Cm
                 ∗ ksecc_pay_seq fdw fb (S i) k' Cm Cend)%I
    end.

  (* the output side is MONOTONE, as one write's is *)
  Lemma ksecc_pay_seq_mono (fdw : mword 64) (fb : nat -> bv 8) (k : nat) :
    forall (i : nat) (Ci Cend Cend' : iProp Σ),
      (Cend -∗ Cend') -∗
      ksecc_pay_seq fdw fb i k Ci Cend -∗ ksecc_pay_seq fdw fb i k Ci Cend'.
  Proof using .
    induction k as [| k IH]; intros i Ci Cend Cend'; iIntros "Hm Hc".
    - cbn [ksecc_pay_seq]. iIntros "HCi".
      iApply "Hm". iApply ("Hc" with "HCi").
    - cbn [ksecc_pay_seq]. iDestruct "Hc" as (Cm) "[Hw Hc]".
  ...
  Lemma ksecc_pay_seq_frame (fdw : mword 64) (fb : nat -> bv 8) (k : nat)
      (i : nat) (Ci Cend C : iProp Σ) :
    C -∗ ksecc_pay_seq fdw fb i k (Ci ∗ C) Cend -∗
    ksecc_pay_seq fdw fb i k Ci Cend.
  Proof using .
    destruct k as [| k]; iIntros "HC Hc".
    - cbn [ksecc_pay_seq]. iIntros "HCi". iApply ("Hc" with "[$HCi $HC]").
    - cbn [ksecc_pay_seq]. iDestruct "Hc" as (Cm) "[Hw Hc]".
      iExists Cm. iFrame "Hc". iApply (ksecc_wb_frame with "HC Hw").
  Qed.
  ...
  (* ...AND IT SPLITS AND JOINS AT ANY POINT, which is what a format string
     ...
     chain. *)
  Lemma ksecc_pay_seq_in (fdw : mword 64) (fb : nat -> bv 8) (k : nat) :
  ...
  Proof using .
    ...
    - cbn [ksecc_pay_seq]. iDestruct "Hc" as (Cm) "[Hw Hc]".
      iExists Cm. iFrame "Hc". iApply (ksecc_wb_mono_in with "Hm Hw").
  Qed.

  Lemma ksecc_pay_seq_split (fdw : mword 64) (fb : nat -> bv 8) (k1 : nat) :
    forall (i k2 : nat) (Ci Cend : iProp Σ),
      ksecc_pay_seq fdw fb i (k1 + k2) Ci Cend -∗
      ∃ Cm : iProp Σ,
        ksecc_pay_seq fdw fb i k1 Ci Cm
        ∗ ksecc_pay_seq fdw fb (i + k1) k2 Cm Cend.
  Proof using .
    induction k1 as [| k1 IH]; intros i k2 Ci Cend.
    - rewrite Nat.add_0_r. cbn [Nat.add]. iIntros "Hc".
      iExists Ci. iSplitR "Hc"; [ cbn [ksecc_pay_seq]; by iIntros "$" | ].
      iExact "Hc".
    - cbn [Nat.add]. iIntros "Hc". cbn [ksecc_pay_seq].
  ...
  Lemma ksecc_pay_seq_join (fdw : mword 64) (fb : nat -> bv 8) (k1 : nat) :
  ...
  Proof using .
    ...
    - rewrite Nat.add_0_r. cbn [Nat.add]. iIntros "H1 H2".
      iApply (ksecc_pay_seq_in fdw fb k2 i Cm Ci Cend with "[H1] H2").
      cbn [ksecc_pay_seq]. iExact "H1".
    - cbn [Nat.add]. iIntros "H1 H2". cbn [ksecc_pay_seq] in *.
  ...
  Lemma ksecc_pay_seq_ext (fdw : mword 64) (fb fb' : nat -> bv 8) (k : nat) :
    forall (i : nat) (Ci Cend : iProp Σ),
      (forall j : nat, (i <= j)%nat -> (j < i + k)%nat -> fb j = fb' j) ->
      ksecc_pay_seq fdw fb i k Ci Cend -∗ ksecc_pay_seq fdw fb' i k Ci Cend.
  Proof using .
    induction k as [| k IH]; intros i Ci Cend Heq; iIntros "Hc".
    - cbn [ksecc_pay_seq] in *. iExact "Hc".
    - cbn [ksecc_pay_seq] in *. iDestruct "Hc" as (Cm) "[Hw Hc]".
...
Section UkSeccPutc.
  ...
  (* --------------------------------------------------------------------- *)
  (* putc(fd, c) @0x3fc -- ulib's one-byte write.                            *)
  (*                                                                        *)
  ...
  (* ...AND WHAT IT SPENDS (lane CAT-WALK, W1): ONE per-call obligation at
     ...
     [ksecc_wb], because it is one frame below the caller's sp and no
     caller can name it. *)
  Lemma wp_ksecc_putc (h : CpuId) (m : regfile) (n : nat)
      (Ci Co : iProp Σ) :
    ksecc_wb N (m !!! Regidx a0_idx)
      (nth_byte (m !!! Regidx a1_idx) 0) Ci Co -∗
    seccomp_code γt -∗
    Ci -∗
    urun N h m (mword_of_int SeccompSyms.putc) (4 + n) -∗
    (∀ (h' : CpuId) (m' : regfile),
       ⌜ ucallee_saved m m' ⌝ -∗
       Co -∗
  ...
  Proof using .
    ...
    destruct seccomp_syms_pins
      as (_ & _ & _ & _ & Hputc & _ & _ & _ & _ & _ & Hwrite).
    rewrite Hputc.
    ...
    assert (Ho16 : uoff_sdsp (mword_of_int 2 : mword 6) = 16)
      by (vm_compute; reflexivity).
    (* ---- 0x3fc  c.addi sp,sp,-32 -- THE PUSH ---- *)
    iApply (wp_uk_caddi_sp_dn N h m (mword_of_int 0x3fc)
              (mword_of_int 32 : mword 6) 4 n
              ltac:(apply bv_eq; vm_compute; reflexivity)
              with "[] Hrun").
    { iApply (uis_seccomp_3fc with "Hcode"). }
    iIntros "Hframe".
    assert (E41a : add_vec_int (mword_of_int 0x3fc : mword 64) 2
                   = mword_of_int 0x3fe)
    ...
    (* ---- 0x3fe  c.sdsp ra,24(sp) ---- *)
    iApply (wp_uk_csdsp N h1 m1 (mword_of_int 0x3fe)
              (mword_of_int 3 : mword 6) ra_idx (uint sp0 - 8) vra n
              ltac:(rewrite Hsp1 Hsp32 Ho24; lia)
              ltac:(rewrite Zminus_mod Hal8; reflexivity)
              with "[] Hwra Hrun").
    { iApply (uis_seccomp_3fe with "Hcode"). }
    iIntros "Hwra".
    assert (E41c : add_vec_int (mword_of_int 0x3fe : mword 64) 2
                   = mword_of_int 0x400)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E41c.
    iIntros (h2) "Hrun".
    (* ---- 0x400  c.sdsp s0,16(sp) ---- *)
    iApply (wp_uk_csdsp N h2 m1 (mword_of_int 0x400)
              (mword_of_int 2 : mword 6) s0_idx (uint sp0 - 16) vs0 n
              ltac:(rewrite Hsp1 Hsp32 Ho16; lia)
              ltac:(rewrite Zminus_mod Hal8; reflexivity)
              with "[] Hws0 Hrun").
    { iApply (uis_seccomp_400 with "Hcode"). }
    iIntros "Hws0".
    assert (E41e : add_vec_int (mword_of_int 0x400 : mword 64) 2
                   = mword_of_int 0x402)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E41e.
    iIntros (h3) "Hrun".
    ...
    iApply (wp_uk_caddi4spn N h3 m1 (mword_of_int 0x402)
              ...
              with "[] Hrun").
    { iApply (uis_seccomp_402 with "Hcode"). }
    assert (E420 : add_vec_int (mword_of_int 0x402 : mword 64) 2
                   = mword_of_int 0x404)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E420.
    iIntros (h4) "Hrun".
    ...
    { rewrite <- Hsp1.
      exact (upd_ne m1 (Regidx s0_idx) (Regidx csp_rs1) (regval_into_reg sp0)
               ltac:(vm_compute; discriminate)). }
    (* ---- 0x404  sb a1,-17(s0) -- the byte, into BYTE 7 of the third word ---- *)
    ...
    iApply (wp_uk_sb N h4 m2 (mword_of_int 0x404)
              (mword_of_int 4079 : mword 12) s0_idx a1_idx
              (uint sp0 - 17) (nth_byte vb 7%nat) n
              ltac:(rewrite Hs02 Hoff17; lia)
              with "[] Hb7 Hrun").
    { iApply (uis_seccomp_404 with "Hcode"). }
    iIntros "Hb7".
    assert (E422 : add_vec_int (mword_of_int 0x404 : mword 64) 4
                   = mword_of_int 0x408)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E422.
    iIntros (h5) "Hrun".
    (* THE FRAME WORD STAYS OPEN over the write (lane CAT-WALK, W1): the
       ...
       [ksecc_wb] and the word is closed again only after the call
       returns it. *)
    (* ---- 0x408  c.li a2,1 ---- *)
    iApply (wp_uk_cli N h5 m2 (mword_of_int 0x408)
              (mword_of_int 1 : mword 6) a2_idx n
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate) with "[] Hrun").
    { iApply (uis_seccomp_408 with "Hcode"). }
    assert (E426 : add_vec_int (mword_of_int 0x408 : mword 64) 2
                   = mword_of_int 0x40a)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E426.
    iIntros (h6) "Hrun".
    ...
    { rewrite <- Hs02.
      exact (upd_ne m2 (Regidx a2_idx) (Regidx s0_idx) _
               ltac:(vm_compute; discriminate)). }
    (* ---- 0x40a  addi a1,s0,-17 ---- *)
    iApply (wp_uk_addi N h6 m3 (mword_of_int 0x40a)
              (mword_of_int 4079 : mword 12) s0_idx a1_idx
              ...
              with "[] Hrun").
    { iApply (uis_seccomp_40a with "Hcode"). }
    assert (E428 : add_vec_int (mword_of_int 0x40a : mword 64) 4
                   = mword_of_int 0x40e)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E428.
    iIntros (h7) "Hrun".
    ...
    (* ---- 0x40e  jal ra,0x502 <write> ---- *)
    iApply (wp_uk_jal N h7 m4 (mword_of_int 0x40e)
              (mword_of_int 2096990 : mword 21) ra_idx
              (mword_of_int SeccompSyms.write) (mword_of_int 0x412) n
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate)
              ...
              with "[] Hrun").
    { iApply (uis_seccomp_40e with "Hcode"). }
    iIntros (h8) "Hrun".
    set (m5 := <[Regidx ra_idx
                 := regval_into_reg (mword_of_int 0x412 : mword 64)]> m4).
    assert (Hra5 : m5 !!! Regidx ra_idx = (mword_of_int 0x412 : mword 64))
    ...
    assert (Eret : ret_pc (m5 !!! Regidx ra_idx) = (mword_of_int 0x412 : mword 64))
    ...
    { intros r Hr Hrsp Hrs0.
      ...
      rewrite /m5 (upd_ne m4 (Regidx ra_idx) (Regidx r)
                     (regval_into_reg (mword_of_int 0x412 : mword 64)) Nra).
      ...
      rewrite /m1 (upd_ne m (Regidx csp_rs1) (Regidx r)
                     ...
                     Hrsp).
      reflexivity. }
    (* ---- 0x412  c.ldsp ra,24(sp) ---- *)
    iApply (wp_uk_cldsp N h9 m6 (mword_of_int 0x412)
              ...
              ltac:(vm_compute; discriminate)
              with "[] Hwra Hrun").
    { iApply (uis_seccomp_412 with "Hcode"). }
    iIntros "Hwra".
    assert (E430 : add_vec_int (mword_of_int 0x412 : mword 64) 2
                   = mword_of_int 0x414)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E430.
    iIntros (h10) "Hrun".
    ...
    { rewrite <- Hsp6.
      exact (upd_ne m6 (Regidx ra_idx) (Regidx csp_rs1) _
               ltac:(vm_compute; discriminate)). }
    (* ---- 0x414  c.ldsp s0,16(sp) ---- *)
    iApply (wp_uk_cldsp N h10 m7 (mword_of_int 0x414)
              ...
              ltac:(vm_compute; discriminate)
              with "[] Hws0 Hrun").
    { iApply (uis_seccomp_414 with "Hcode"). }
    iIntros "Hws0".
    assert (E432 : add_vec_int (mword_of_int 0x414 : mword 64) 2
                   = mword_of_int 0x416)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E432.
    iIntros (h11) "Hrun".
    ...
    { rewrite <- Hsp7.
      exact (upd_ne m7 (Regidx s0_idx) (Regidx csp_rs1) _
               ltac:(vm_compute; discriminate)). }
    (* ---- 0x416  c.addi16sp sp,sp,32 -- THE POP: the frame goes back ---- *)
    iApply (wp_uk_caddi16sp_up N h11 m8 (mword_of_int 0x416)
              (mword_of_int 2 : mword 6) 4 n
              ltac:(apply bv_eq; vm_compute; reflexivity)
              with "[] [Hwra Hws0 Hwb Hw32] Hrun").
    { iApply (uis_seccomp_416 with "Hcode"). }
    { rewrite Hsp8 Hup.
    ...
    assert (E434 : add_vec_int (mword_of_int 0x416 : mword 64) 2
                   = mword_of_int 0x418)
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    { rewrite /m9 (upd_ne m8 (Regidx csp_rs1) (Regidx ra_idx) _
      ...
      exact Hra1. }
    (* ---- 0x418  c.jr ra ---- *)
    iApply (wp_uk_cjr N h12 m9 (mword_of_int 0x418) ra_idx
              (ret_pc (m !!! Regidx ra_idx)) (4 + n)
              ltac:(vm_compute; discriminate)
              ltac:(rewrite Hra9; reflexivity)
              with "[] Hrun").
    { iApply (uis_seccomp_418 with "Hcode"). }
    iIntros (h13) "Hrun".
  ...
  (* --------------------------------------------------------------------- *)
  (* vprintf's SHARED TAIL @0x6b2: restore ra, s0, s1; pop the 96-byte      *)
  (* frame; return.  The empty-string arm jumps straight here from 0x48c    *)
