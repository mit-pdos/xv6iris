...
Section WpSwtchVc.
  ...
  Section SwtchSpec.
    ...
    Context (mstatus0 mie_v mdv0 menvcfg0 : mword 64).
    Context (pmpcfg0 : type_of_register pmpcfg_n) (pmpaddr00 : type_of_register pmpaddr_n).
    Context (region_pte : PMA_Region) (dq : dfrac).
    ...
    Hypothesis HX : eq_vec (_get_Pmpcfg_ent_X (vec_access_dec pmpcfg0 0)) ('b"1") = true.
    Hypothesis Hramcov : (ram_base + ram_size <= uint (vec_access_dec pmpaddr00 0) * 4)%Z.
    Hypothesis Hpmpp : pmp_tor0_pte_read pmpcfg0 pmpaddr00 (pte_paddr root_ppn).
    Hypothesis Hpteregion : forall pmar0, pma_allows_all pmar0 ->
       matching_pma_region pmar0 (Physaddr (pte_paddr root_ppn)) 8 = Some region_pte /\
       (override_PMA (PMA_Region_attributes region_pte) PBMT_PMA).(PMA_supports_pte_read) = true.
    Hypothesis HW : eq_vec (_get_Pmpcfg_ent_W (vec_access_dec pmpcfg0 0)) ('b"1") = true.
    Hypothesis HR : eq_vec (_get_Pmpcfg_ent_R (vec_access_dec pmpcfg0 0)) ('b"1") = true.
    Hypothesis Hlpe : bool_bit_backwards (_get_MEnvcfg_LPE menvcfg0) = false.

    (* the fixed S-mode machine configuration, bundled. *)
    Definition sconf : iProp Σ :=
      (hart_state ↦ᵣ{dq} HART_ACTIVE tt ∗ cur_privilege ↦ᵣ{dq} Supervisor ∗
       ...
       menvcfg ↦ᵣ{dq} menvcfg0 ∗ pmpcfg_n ↦ᵣ{dq} pmpcfg0 ∗
       pmpaddr_n ↦ᵣ{dq} pmpaddr00 ∗ tlb_inv root_ppn)%I.
    ...
    Proof.
      ...
      iDestruct "Hconf" as
        "(Hhs & Hpriv & Hms & Hmie & Hmdl & Hmenv & Hpmpc & Hpmpa & Htlbinv)".
      ...
      iApply (wp_vc_block_s_den root_ppn swtch_prog E Phi
                ...
                rho mstatus0 mie_v mdv0 menvcfg0 pmpcfg0 pmpaddr00 region_pte (dq:=dq)
      ...
      iApply (wp_cret_s_zca root_ppn E Phi
                ...
                HN HSIE HMPRV HSXL Hmm HPBMTE HX Hramcov Hpmpp Hpteregion
      ...
      iAssert sconf with
        "[Hhs Hpriv Hms Hmie Hmdl Hmenv Hpmpc Hpmpa Htlbinv]" as "Hconf".
