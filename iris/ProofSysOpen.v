(* ProofSysOpen.v -- sys_open's walk, PLAIN ARM.  The theorem is
   ...
   functor carries no signature of its own: [ProofSysOpenFull] puts the
   ...
   ==== THE FIVE BLOCKS, AND WHICH ONE OWES WHAT =======================
     ...
     [ProofSysOpenWalk]    the else arm: the ERA namei walk and THE
                             TERMINAL OBSERVATION, fired the instant the
                             child is locked; ARMs B-FAIL and C-FAIL.
     [ProofSysOpenJoin]    the T_DEVICE test and the major bound; ARM
                             D-FAIL.
     [ProofSysOpenAlloc]   filealloc / fdalloc and the descriptor's TYPE;
                             ARMs E-FAIL and F-FAIL.
     [ProofSysOpenStores]  the field stores and THE O_TRUNC FIRE, fused
                             with the retag; the three success arms.
     [ProofSysOpenPub]     ARM S: the publication, with the descriptor
   ...
   ([ProofSysOpenBits.soau_create_zero]), so the [c.beqz] is TAKEN and
   ...
   The O_CREATE arm, which is [ProofSysOpenFull]'s. *)
...
Require Import ProofSysOpenWalk.
...
Module Walk := SysOpenWalk Iunlock Iunlockput EndOp Fileclose Itrunc
...
Section ProofSysOpenBody.
 ...
 pk bf (arg_int32 vom) (word_lo u23) ns Sb0
    ...
    (* ---- O_CREATE: REFUTED BY THE PREMISE.  [om_create vom = false]
       ...
       ([ProofSysOpenBits.soau_create_zero]), so this branch's own key is
       ...
       makes this arm's proof 300 lines shorter than the create arm's
       ([ProofSysOpenEntryC]). ---- *)
    exfalso.
