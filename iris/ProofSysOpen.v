...
Section ProofSysOpenBody.
  ...
  Lemma so_tail_pub `{GEN : GenId} `{CID0 : CpuId}
    ...
    wp_next true (proc_addr jx)
      (so_cont gf bn cov logstart bmapstart inodestart size nsj
  ...
  Proof.
    ...
    iApply (ic_mk_loaded fsc_fs gi cov logstart k inum dn bm data Hok Hrl Hdok Hddix
  ...
  Proof.
    ...
    { (* ---- no O_TRUNC: straight to ARM S ---- *)
      ...
      iApply (so_tail_pub (CID0 := CID10) gf gs jx gl gu gd gk pd pav pu bn g
                gi gil gisl cov logstart bmapstart inodestart size
    ...
    iApply (Itrunc.wp_itrunc_gen (CID := CID15) gs jx gl gu gd gk pd pav pu
              bn g gi cov logstart bmapstart inodestart icfg_nib size
    ...
    iModIntro.
    iDestruct (so_trunc_loaded gi cov logstart kk inum dn Htynz Htynd Hrl
  ...
  Proof.
    ...
    { (* ---- filealloc refused ---- *)
      ...
      iApply (Tails.so_tail_e (CID0 := CID5) gs jx gl gu gd gk pd pav pu bn g
                gi gtl gil gisl cov logstart bmapstart inodestart
    ...
    { (* ---- fdalloc refused ---- *)
      ...
      iApply (Tails.so_tail_f (CID0 := CID10) gfl gf gs jx gl gu gd gk pd pav
                pu bn g gi gtl gil gisl cov logstart bmapstart
    ...
    { (* ---- T_DEVICE: the +0x140 block ---- *)
      ...
      iApply (so_stores (CID0 := CID17) gf gs jx gl gu gd gk pd pav pu bn g
                gi gil gisl cov logstart bmapstart inodestart icfg_nib
  ...
  Lemma so_entry_c `{GEN : GenId} `{CID0 : CpuId}
    ...
    (pa_stk sp0 24) ↦₈[KT1] w24 -∗
    wp_next true (proc_addr jx)
      (so_cont0 gf bn cov logstart bmapstart inodestart size ninodes ns
  ...
  Proof.
    ...
    iApply (Create.wp_create_sconf (CID := CID5) gs jx gl gu gd gk pd pav pu bn
              g gi gtl ga gf gpr cov logstart bmapstart inodestart nib
    ...
    2:{ (* ---- create refused: nothing is locked and nothing is held ---- *)
      ...
      iApply (Tails.so_tail_a (CID0 := CID8) gs jx gl gu gd gk pd pav pu bn g
                cov logstart dev u1 pidv (DfracOwn (1/4)) m P1 sp0 K eb b
    ...
    iDestruct (so_esc_acc gi cov logstart kk ltac:(lia)
    ...
    iAssert (wp_next true (proc_addr jx)
               (so_cont gf bn cov logstart bmapstart inodestart size
    ...
    iApply (so_join (CID0 := CID8) gfl gf gs jx gl gu gd gk pd pav pu bn g
              gi gtl gil gisl cov logstart bmapstart inodestart nib
  ...
  Proof.
    ...
    iApply (Namei.wp_namei_gen (CID := CID2) gs jx gl gu gd gk pd pav pu bn
              g gi gtl ga gf cov logstart bmapstart inodestart nib
    ...
    iEval (rewrite -proc_priv_nocwd_bare) in "Hpnc".
      ...
      iApply (Tails.so_tail_b (CID0 := CID5) gs jx gl gu gd gk pd pav pu bn g
                cov logstart dev n1 pidv (DfracOwn (1/4)) m P1 sp0 K eb b
                lks w4 w5 w6 (word_of_words lo om) w24 bp1 V
                HKeo HK24 Kpop Hgeom Hj Hgl Hlkempty Hsp0 HP1sp HP1thr HP1s2
                HP1s3 Hal
                with "Hcg Hown [] [] Htext Hdata Hpc Hpe Hbio Hlog Hseam Hgen
    ...
    iDestruct (so_esc_acc gi cov logstart kk Hkk with "Hescrows")
      as "#Hesck".
    ...
    iApply (Ilock.wp_ilock_dep_sconf (CID := CID6) gs jx gl gu gd gk pd pav pu bn
              gi gil gisl cov logstart inodestart nib
  ...
  Proof.
    ...
    iDestruct (cpu_own_transport CID13 CID18 0 eb (proc_addr j) b ltac:(wp_next_chain)
                 with "Hown") as "Hown".
    iApply (BeginOp.wp_begin_op_sconf (CID := CID18) gs j gl bn g fsc_fs cov
    ...
    iAssert (wp_next (CID0 := CID21) true (proc_addr j)
               (so_cont0 gf bn cov logstart bmapstart inodestart size
    ...
    { (* ---- no O_CREATE: the else arm at +0xdc ---- *)
      ...
      iApply (so_entry_n (CID0 := CID22) gfl gf ga gpr gs j gl gu gd gk pd pav
                pu bn g gi gtl cov logstart bmapstart inodestart nib
