...
Section ProofSysOpenBody.
  ...
  (* [so_wf_dev] -- the DEVICE arm's vacuous [off_wf] bound -- moved to
     [ProofSysOpenParts], beside [so_deposit], so the AU lane shares it. *)
  ...
  Proof.
    ...
    (* ---- THE BIRTH OF THE FD'S OFF BOX (r25 item 24), under the lock:
           on the FD_INODE arm the stored [f->off] word is deposited into a
           fresh box registered in the inode's rows -- the rows ride the
           holder's handle ([ic_tx_dep], item 33) and the running token is
           borrowed from the cap ([sie_cap_gpr_own_ctx_acc]); on the
           FD_DEVICE arm the word simply joins the visibility-free tier.
           Either way the payload's off conjunct comes out at SOME box
           names [γb] (the device arm's are immaterial). ---- *)
    iApply fupd_wp.
    iDestruct (sie_cap_gpr_own_ctx_acc with "Hcg") as "[Hrun Hcgb]".
    iDestruct (ic_tx_dep_off_rows_acc with "Hdep") as "[Hrows Hdepb]".
    ...
    (* THE ONE GHOST STEP: the descriptor fdalloc opened is now OPEN, at the
       ...
       sys_open's contract takes the bundle at all.

       AT THE NAMED TABLE, so the post can say WHICH row moved:
       [fd_frags_acc]'s closer hands back [<[fd := stpub]> sts]. *)
 ...
 kk qi s gy loy tly inum dn bm kf fd l C pn om voff nsj
    ...
    iDestruct (so_loaded_open with "Hload")
      as (data) "(%Hok & %Hrl & %Hdok & Hlnk & Hat & Hmeta & Hmap & Hblk 
                  & Htop)".
 ...
 kk qi ss gy loy tly inum dn bm om lo ns1 u1 pidv dqb dqs
  ...
  Proof.
    ...
    iDestruct (cpu_own_transport CID0 CID2 0 eb (proc_addr jx) b
                 ltac:(wp_next_chain) with "Hown") as "Hown".
    iApply (Namei.wp_namei_gen (CID := CID2) gs jx gl pd pav pu
 gf
 ...
 ns dqb dqs dqbs dqn v vom
  ...
  Proof.
    ...
    2:{ (* ---- ARM 0: the string did not fetch.  No begin_op, no s1 save ---- *)
      ...
      { split; lia. }
      { rewrite /sys_open_post. iSplitR "Hfds"; [| iFrame "Hfds"].
        iLeft. iSplitR; [| iFrame "Hpriv Hfrag"]. iPureIntro.
