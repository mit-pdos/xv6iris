(* ===================================================================== *)
(* UkSeccVprintf.v -- ulib's [vprintf(fd, fmt, ap)] for a format string  *)
(* containing no '%', at seccomp's addresses.                            *)
(* A PORT of [UkGrepVprintf.v] (itself cat's): the same C, the same      *)
(* instructions, every pc 0x1d0 further down (vprintf 0x688 -> 0x4b8);   *)
...
Section UkSeccVprintf.
  ...
  Lemma wp_ksecc_vprintf_epi0 (h : CpuId) (m : regfile)
    ...
    urun N h m (mword_of_int 0x6ec) n -∗
    (∀ (h' : CpuId) (m' : regfile),
  ...
  Proof using .
    ...
    assert (Ho72 : uoff_sdsp (mword_of_int 9 : mword 6) = 72)
      by (vm_compute; reflexivity).
    (* ---- 0x6ec  c.ldsp ra,88(sp) ---- *)
    iApply (wp_uk_cldsp N h m (mword_of_int 0x6ec)
              ...
              with "[] Hwra Hrun").
    { iApply (uis_seccomp_6ec with "Hcode"). }
    iIntros "Hwra".
    assert (E70a : add_vec_int (mword_of_int 0x6ec : mword 64) 2
                 = mword_of_int 0x6ee)
    ...
    { rewrite <- Hsp.
      exact (upd_ne m (Regidx ra_idx) (Regidx csp_rs1) (regval_into_reg vra)
               ltac:(vm_compute; discriminate)). }
    (* ---- 0x6ee  c.ldsp s0,80(sp) ---- *)
    iApply (wp_uk_cldsp N h1 mm1 (mword_of_int 0x6ee)
              ...
              ltac:(vm_compute; discriminate)
              with "[] Hws0 Hrun").
    { iApply (uis_seccomp_6ee with "Hcode"). }
    iIntros "Hws0".
    assert (E70c : add_vec_int (mword_of_int 0x6ee : mword 64) 2
                 = mword_of_int 0x6f0)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E70c.
    iIntros (h2) "Hrun".
    ...
    { rewrite <- Hsp1.
      exact (upd_ne mm1 (Regidx s0_idx) (Regidx csp_rs1) (regval_into_reg vs0)
               ltac:(vm_compute; discriminate)). }
    (* ---- 0x6f0  c.ldsp s1,72(sp) ---- *)
    iApply (wp_uk_cldsp N h2 mm2 (mword_of_int 0x6f0)
              ...
              ltac:(vm_compute; discriminate)
              with "[] Hws1 Hrun").
    { iApply (uis_seccomp_6f0 with "Hcode"). }
    iIntros "Hws1".
    assert (E70e : add_vec_int (mword_of_int 0x6f0 : mword 64) 2
                 = mword_of_int 0x6f2)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E70e.
    iIntros (h3) "Hrun".
    ...
    { rewrite <- Hsp2.
      exact (upd_ne mm2 (Regidx s1_idx) (Regidx csp_rs1) (regval_into_reg vs1)
               ltac:(vm_compute; discriminate)). }
    (* ---- 0x6f2  c.addi16sp sp,sp,96 -- THE POP ---- *)
    iApply (wp_uk_caddi16sp_up N h3 mm3 (mword_of_int 0x6f2)
              (mword_of_int 6 : mword 6) 12 n
              ltac:(apply bv_eq; vm_compute; reflexivity)
              with "[] [Hwra Hws0 Hws1 Hw4 Hw5 Hw6 Hw7 Hw8 Hw9 Hw10 Hw11 Hw12] Hrun").
    { iApply (uis_seccomp_6f2 with "Hcode"). }
    { rewrite Hsp3 Hup.
    ...
    assert (E710 : add_vec_int (mword_of_int 0x6f2 : mword 64) 2
                   = mword_of_int 0x6f4)
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    (* ---- 0x6f4  c.jr ra ---- *)
    iApply (wp_uk_cjr N h4 mm4 (mword_of_int 0x6f4) ra_idx
              (ret_pc vra) (12 + n)
              ltac:(vm_compute; discriminate)
              ltac:(rewrite Hra4; reflexivity)
              with "[] Hrun").
    { iApply (uis_seccomp_6f4 with "Hcode"). }
    iIntros (h5) "Hrun".
  ...
  (* --------------------------------------------------------------------- *)
  (* vprintf's FULL EPILOGUE @0x6de: restore s2..s8, then fall into the      *)
  ...
  (* sixteenth.                                                              *)
  (* --------------------------------------------------------------------- *)
  Lemma wp_ksecc_vprintf_epi (h : CpuId) (m m0 : regfile) (sp0 : mword 64)
      (n : nat) :
    m !!! Regidx csp_rs1 = add_vec_int sp0 (- (8 * Z.of_nat 12)) ->
    ...
    (∃ w : mword 64, uword γd (uint sp0 - 88) w) -∗
    (∃ w : mword 64, uword γd (uint sp0 - 96) w) -∗
    urun N h m (mword_of_int 0x6de) n -∗
    (∀ (h' : CpuId) (m' : regfile),
       ⌜ ucallee_saved m0 m' ⌝ -∗
  ...
  Proof using .
    ...
    assert (Ho16 : uoff_sdsp (mword_of_int 2 : mword 6) = 16)
      by (vm_compute; reflexivity).
    (* ---- 0x6de  c.ldsp s2,64(sp) ---- *)
    iApply (wp_uk_cldsp N h m (mword_of_int 0x6de)
              ...
              ltac:(vm_compute; discriminate)
              with "[] Hw2 Hrun").
    { iApply (uis_seccomp_6de with "Hcode"). }
    iIntros "Hw2".
    assert (E6fc : add_vec_int (mword_of_int 0x6de : mword 64) 2
                 = mword_of_int 0x6e0)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E6fc.
    iIntros (h1) "Hrun".
    ...
    { rewrite <- Hsp.
      exact (upd_ne m (Regidx s2_idx) (Regidx csp_rs1)
               ...
               ltac:(vm_compute; discriminate)). }
    (* ---- 0x6e0  c.ldsp s3,56(sp) ---- *)
    iApply (wp_uk_cldsp N h1 me1 (mword_of_int 0x6e0)
              ...
              ltac:(vm_compute; discriminate)
              with "[] Hw3 Hrun").
    { iApply (uis_seccomp_6e0 with "Hcode"). }
    iIntros "Hw3".
    assert (E6fe : add_vec_int (mword_of_int 0x6e0 : mword 64) 2
                 = mword_of_int 0x6e2)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E6fe.
    iIntros (h2) "Hrun".
    ...
    { rewrite <- Hsp1.
      exact (upd_ne me1 (Regidx s3_idx) (Regidx csp_rs1)
               ...
               ltac:(vm_compute; discriminate)). }
    (* ---- 0x6e2  c.ldsp s4,48(sp) ---- *)
    iApply (wp_uk_cldsp N h2 me2 (mword_of_int 0x6e2)
              ...
              ltac:(vm_compute; discriminate)
              with "[] Hw4 Hrun").
    { iApply (uis_seccomp_6e2 with "Hcode"). }
    iIntros "Hw4".
    assert (E700 : add_vec_int (mword_of_int 0x6e2 : mword 64) 2
                 = mword_of_int 0x6e4)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E700.
    iIntros (h3) "Hrun".
    ...
    { rewrite <- Hsp2.
      exact (upd_ne me2 (Regidx s4_idx) (Regidx csp_rs1)
               ...
               ltac:(vm_compute; discriminate)). }
    (* ---- 0x6e4  c.ldsp s5,40(sp) ---- *)
    iApply (wp_uk_cldsp N h3 me3 (mword_of_int 0x6e4)
              ...
              ltac:(vm_compute; discriminate)
              with "[] Hw5 Hrun").
    { iApply (uis_seccomp_6e4 with "Hcode"). }
    iIntros "Hw5".
    assert (E702 : add_vec_int (mword_of_int 0x6e4 : mword 64) 2
                 = mword_of_int 0x6e6)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E702.
    iIntros (h4) "Hrun".
    ...
    { rewrite <- Hsp3.
      exact (upd_ne me3 (Regidx s5_idx) (Regidx csp_rs1)
               ...
               ltac:(vm_compute; discriminate)). }
    (* ---- 0x6e6  c.ldsp s6,32(sp) ---- *)
    iApply (wp_uk_cldsp N h4 me4 (mword_of_int 0x6e6)
              ...
              ltac:(vm_compute; discriminate)
              with "[] Hw6 Hrun").
    { iApply (uis_seccomp_6e6 with "Hcode"). }
    iIntros "Hw6".
    assert (E704 : add_vec_int (mword_of_int 0x6e6 : mword 64) 2
                 = mword_of_int 0x6e8)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E704.
    iIntros (h5) "Hrun".
    ...
    { rewrite <- Hsp4.
      exact (upd_ne me4 (Regidx s6_idx) (Regidx csp_rs1)
               ...
               ltac:(vm_compute; discriminate)). }
    (* ---- 0x6e8  c.ldsp s7,24(sp) ---- *)
    iApply (wp_uk_cldsp N h5 me5 (mword_of_int 0x6e8)
              ...
              ltac:(vm_compute; discriminate)
              with "[] Hw7 Hrun").
    { iApply (uis_seccomp_6e8 with "Hcode"). }
    iIntros "Hw7".
    assert (E706 : add_vec_int (mword_of_int 0x6e8 : mword 64) 2
                 = mword_of_int 0x6ea)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E706.
    iIntros (h6) "Hrun".
    ...
    { rewrite <- Hsp5.
      exact (upd_ne me5 (Regidx s7_idx) (Regidx csp_rs1)
               ...
               ltac:(vm_compute; discriminate)). }
    (* ---- 0x6ea  c.ldsp s8,16(sp) ---- *)
    iApply (wp_uk_cldsp N h6 me6 (mword_of_int 0x6ea)
              ...
              ltac:(vm_compute; discriminate)
              with "[] Hw8 Hrun").
    { iApply (uis_seccomp_6ea with "Hcode"). }
    iIntros "Hw8".
    assert (E708 : add_vec_int (mword_of_int 0x6ea : mword 64) 2
                 = mword_of_int 0x6ec)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E708.
    iIntros (h7) "Hrun".
    ...
    { rewrite <- Hsp6.
      exact (upd_ne me6 (Regidx s8_idx) (Regidx csp_rs1)
               ...
               ltac:(vm_compute; discriminate)). }
    (* ---- 0x6ec..0x6f4: the shared tail ---- *)
    iApply (wp_ksecc_vprintf_epi0 h7 me7 sp0 (m0 !!! Regidx ra_idx)
  ...
  (*                                                                        *)
  (*   0x50e  sext.w a5,s1        ; a5 := the current character             *)
  (*   0x512  bnez  s3,0x4f8      ; NOT taken: s3 is the pending-% state    *)
  (*   0x516  bne   a5,s5,0x4ee   ; taken: the character is not '%'          *)
  (*   0x4ee  mv a1,s1 ; mv a0,s6 ; jal putc ; j 0x4fc                       *)
  (*   0x4fc  addiw a5,s2,1 ; mv s2,a5 ; mv a4,a5 ; add a5,a5,s4             *)
  (*   0x506  lbu   s1,0(a5)      ; the NEXT character                       *)
  (*   0x50a  beqz  s1,0x6de      ; taken at the terminator                  *)
  (*                                                                        *)
  ...
  (* --------------------------------------------------------------------- *)
  (* ONE ROUND, 0x50e -> 0x50a.  It owns no frame word: putc's four are     *)
  (* BELOW sp, and the twelve vprintf spilled are untouched here.            *)
  (* --------------------------------------------------------------------- *)
  (* ...AND ITS ONE PAYMENT (lane CAT-WALK, W1): the round's character,
     at the descriptor the invariant parks in s6.  Not the flagged
     deposit -- see [UkCat.wp_ksecc_write_chain]'s header. *)
  Lemma wp_ksecc_vprintf_step (m0 : regfile) (sp0 fd ap : mword 64) (a : Z)
  ...
  Proof using .
    ...
    assert (E52c : add_vec_int (mword_of_int 0x50e : mword 64) 4
                   = mword_of_int 0x512)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E52c.
    iIntros (h1) "Hrun".
    ...
    assert (E530 : add_vec_int (mword_of_int 0x512 : mword 64) 4
                   = mword_of_int 0x516)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E530.
    iIntros (h2) "Hrun".
    ...
    assert (Etgt534 : add_vec (mword_of_int 0x516 : mword 64)
    ...
    iApply (wp_uk_btype N h2 m1 (mword_of_int 0x516)
              ...
              (mword_of_int 0x4ee) (4 + n) Ht (eq_sym Etgt534)
              ltac:(intros _; vm_compute; reflexivity)
              with "[] Hrun").
    { iApply (uis_seccomp_516 with "Hcode"). }
    iIntros (h3) "Hrun".
    (* ---- 0x4ee  c.mv a1,s1 ---- *)
    iApply (wp_uk_cmv N h3 m1 (mword_of_int 0x4ee) a1_idx s1_idx
              (add_vec zero_reg (m1 !!! Regidx s1_idx)) (4 + n)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate) eq_refl
              with "[] Hrun").
    { iApply (uis_seccomp_4ee with "Hcode"). }
    assert (E50c : add_vec_int (mword_of_int 0x4ee : mword 64) 2
                   = mword_of_int 0x4f0)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E50c.
    iIntros (h4) "Hrun".
    ...
    (* ---- 0x4f0  c.mv a0,s6 ---- *)
    iApply (wp_uk_cmv N h4 m2 (mword_of_int 0x4f0) a0_idx s6_idx
              (add_vec zero_reg (m2 !!! Regidx s6_idx)) (4 + n)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate) eq_refl
              with "[] Hrun").
    { iApply (uis_seccomp_4f0 with "Hcode"). }
    assert (E50e : add_vec_int (mword_of_int 0x4f0 : mword 64) 2
                   = mword_of_int 0x4f2)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E50e.
    iIntros (h5) "Hrun".
    ...
    (* ---- 0x4f2  jal ra,0x3fc <putc> ---- *)
    destruct seccomp_syms_pins
      as (_ & _ & _ & _ & Hputc & _ & _ & _ & _ & _ & _).
    iApply (wp_uk_jal N h5 m3 (mword_of_int 0x4f2)
              (mword_of_int 2096906 : mword 21) ra_idx
              (mword_of_int SeccompSyms.putc) (mword_of_int 0x4f6) (4 + n)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate)
              ...
              with "[] Hrun").
    { iApply (uis_seccomp_4f2 with "Hcode"). }
    iIntros (h6) "Hrun".
    set (m4 := <[Regidx ra_idx
                 := regval_into_reg (mword_of_int 0x4f6 : mword 64)]> m3).
    assert (Hra4 : m4 !!! Regidx ra_idx = (mword_of_int 0x4f6 : mword 64))
      by exact (upd_eq m3 (Regidx ra_idx) (regval_into_reg _)).
    (* ---- putc(fd, c): the round's own per-call obligation ---- *)
    assert (Ha0m4 : m4 !!! Regidx a0_idx = fd).
    ...
    assert (Hb0m4 : nth_byte (m4 !!! Regidx a1_idx) 0%nat = b0).
    { rewrite /m4 (upd_ne m3 (Regidx ra_idx) (Regidx a1_idx) _
      ...
      rewrite Hs1_1 add_vec_zero_l. apply ksecc_nth_byte0_moi. }
    iApply (wp_ksecc_putc N h6 m4 n Ci Co with "[Hw] Hcode HCi Hrun").
    { rewrite Ha0m4 Hb0m4. iExact "Hw". }
    iIntros (h7 m5) "%Hcs HCo Hrun".
    assert (Eret : ret_pc (m4 !!! Regidx ra_idx) = (mword_of_int 0x4f6 : mword 64))
    ...
    (* ---- 0x4f6  c.j 0x4fc ---- *)
    assert (Etgt514 : (mword_of_int 0x4fc : mword 64)
                      = add_vec (mword_of_int 0x4f6 : mword 64)
                          (sign_extend' 64
                             (sign_extend' 21
                                (concat_vec (mword_of_int 3 : mword 11) ('b"0")))))
      by (apply bv_eq; vm_compute; reflexivity).
    iApply (wp_uk_cj N h7 m5 (mword_of_int 0x4f6)
              (mword_of_int 3 : mword 11) (mword_of_int 0x4fc) (4 + n)
              Etgt514 ltac:(vm_compute; reflexivity)
              with "[] Hrun").
    { iApply (uis_seccomp_4f6 with "Hcode"). }
    iIntros (h8) "Hrun".
    (* ---- 0x4fc  addiw a5,s2,1 ---- *)
    ...
    { rewrite Hs25 Es1_12.
      ...
      f_equal. lia. }
    iApply (wp_uk_addiw N h8 m5 (mword_of_int 0x4fc)
    ...
    assert (E51a : add_vec_int (mword_of_int 0x4fc : mword 64) 4
                   = mword_of_int 0x500)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E51a.
    iIntros (h9) "Hrun".
    ...
    iApply (wp_uk_cmv N h9 m6 (mword_of_int 0x500) s2_idx a5_idx
              (add_vec zero_reg (m6 !!! Regidx a5_idx)) (4 + n)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate) eq_refl
              with "[] Hrun").
    { iApply (uis_seccomp_500 with "Hcode"). }
    assert (E51e : add_vec_int (mword_of_int 0x500 : mword 64) 2
                   = mword_of_int 0x502)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E51e.
    iIntros (h10) "Hrun".
    ...
    { rewrite <- Ha56.
      exact (upd_ne m6 (Regidx s2_idx) (Regidx a5_idx) _
               ltac:(vm_compute; discriminate)). }
    (* ---- 0x502  c.mv a4,a5 ---- *)
    iApply (wp_uk_cmv N h10 m7 (mword_of_int 0x502) a4_idx a5_idx
              (add_vec zero_reg (m7 !!! Regidx a5_idx)) (4 + n)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate) eq_refl
              with "[] Hrun").
    { iApply (uis_seccomp_502 with "Hcode"). }
    assert (E520 : add_vec_int (mword_of_int 0x502 : mword 64) 2
                   = mword_of_int 0x504)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E520.
    iIntros (h11) "Hrun".
    ...
    iApply (wp_uk_cadd N h11 m8 (mword_of_int 0x504) a5_idx s4_idx
              (add_vec (m8 !!! Regidx a5_idx) (m8 !!! Regidx s4_idx)) (4 + n)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate) eq_refl
              with "[] Hrun").
    { iApply (uis_seccomp_504 with "Hcode"). }
    assert (E522 : add_vec_int (mword_of_int 0x504 : mword 64) 2
                   = mword_of_int 0x506)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E522.
    iIntros (h12) "Hrun".
    ...
    (* ---- 0x506  lbu s1,0(a5) -- the NEXT character, out of .rodata ---- *)
    ...
    { rewrite Ha59.
      ...
      replace (uoff_i12 (mword_of_int 0 : mword 12)) with 0
        by (vm_compute; reflexivity).
      lia. }
    iApply (wp_uk_lbu_text N h12 m9 (mword_of_int 0x506)
    ...
    assert (E524 : add_vec_int (mword_of_int 0x506 : mword 64) 4
                   = mword_of_int 0x50a)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E524.
    iIntros (h13) "Hrun".
  ...
  (* the current one is [f i], and the round's own [beqz s1] at 0x50a is     *)
  ...
  (* is a body byte, the branch is not taken, and the head at 0x50e comes    *)
  ...
  (* unbounded ones.                                                         *)
  (* --------------------------------------------------------------------- *)
  Lemma wp_ksecc_vprintf_loop (m0 : regfile) (sp0 fd ap : mword 64) (a : Z)
    ...
    forall (i : nat) (h : CpuId) (m : regfile) (n : nat)
      ...
      (* THE RUN THE LOOP SPENDS (lane CAT-WALK, W1): the [S k] characters
         still to print, in order, at the descriptor in s6. *)
      ksecc_pay_seq N fd f i (S k) Ci Co -∗
      seccomp_code γt -∗
  ...
  Proof using .
    ...
    induction k as [| k IH ];
      intros i h m n Ci Co Hlo_i Hik Hinv Hs1;
      iIntros "Hpay #Hcode #Hstr HCi Hwra Hws0 Hws1 Hw2 Hw3 Hw4 Hw5 Hw6 Hw7 Hw8 Hw11 Hw12 Hrun Hcont";
    ...
    - (* the LAST character: the byte after it is the terminator *)
      ...
      iApply (wp_ksecc_vprintf_step m0 sp0 fd ap a i (f i) ubyte0 h m n
                Ci Cm Ha0 ltac:(lia) (Hpct i ltac:(lia)) Hinv Hs1
                with "Hw Hcode [] HCi Hrun").
      { rewrite Ei. iExact "Hnul". }
      iIntros (h1 m1) "%Hinv1 %Hs11 HCm Hrun".
      (* ---- 0x50a  beqz s1,0x6de -- TAKEN: this was the terminator ---- *)
      ...
      assert (Etgt : add_vec (mword_of_int 0x50a : mword 64)
      ...
      iApply (wp_uk_btype0 N h1 m1 (mword_of_int 0x50a)
                ...
                (mword_of_int 0x6de) (4 + n) Ht (eq_sym Etgt)
                ltac:(intros _; vm_compute; reflexivity)
                with "[] Hrun").
      { iApply (uis_seccomp_50a with "Hcode"). }
      iIntros (h2) "Hrun".
      destruct Hinv1 as (Hsp1 & _ & _ & _ & _ & _ & _ & _ & _ & Hfr1).
      iApply (wp_ksecc_vprintf_epi h2 m1 m0 sp0 (4 + n)
                Hsp1 Hsp0 Hal8 Hlo Hfr1
                with "Hcode Hwra Hws0 Hws1 Hw2 Hw3 Hw4 Hw5 Hw6 Hw7 Hw8 Hw11 Hw12 Hrun [Hend HCm Hcont]").
      iIntros (h3 m2) "%Hcs Hrun".
      iApply ("Hcont" $! h3 m2 with "[%] [Hend HCm] Hrun");
        [ exact Hcs | iApply ("Hend" with "HCm") ].
    - (* a BODY character: the byte after it is one too *)
      ...
      iApply (wp_ksecc_vprintf_step m0 sp0 fd ap a i (f i) (f (S i)) h m n
                Ci Cm Ha0 ltac:(lia) (Hpct i ltac:(lia)) Hinv Hs1
                with "Hw Hcode Hb1 HCi Hrun").
      iIntros (h1 m1) "%Hinv1 %Hs11 HCm Hrun".
      ...
      iApply (wp_uk_btype0 N h1 m1 (mword_of_int 0x50a)
                ...
                with "[] Hrun").
      { iApply (uis_seccomp_50a with "Hcode"). }
      assert (E528 : add_vec_int (mword_of_int 0x50a : mword 64) 4
                     = mword_of_int 0x50e)
        by (apply bv_eq; vm_compute; reflexivity).
      rewrite E528.
      iIntros (h2) "Hrun".
      iApply (IH (S i) h2 m1 n Cm Co ltac:(lia) ltac:(lia) Hinv1 Hs11
                with "Hpay Hcode Hstr HCm Hwra Hws0 Hws1 Hw2 Hw3 Hw4 Hw5 Hw6 Hw7 Hw8 Hw11 Hw12 Hrun Hcont").
  Qed.


  (* --------------------------------------------------------------------- *)
  (* THE PROLOGUE, 0x4b8 -> 0x50e.  Forty instructions that say nothing     *)
  ...
  (* 0x4c6 arm -- the one that jumps straight to the shared tail with s2..s8 *)
  (* never spilled.  [wp_ksecc_vprintf_epi0] still states that arm's shape,  *)
  (* so re-admitting it later is a branch, not a rewrite.                    *)
  (* --------------------------------------------------------------------- *)
  Lemma wp_ksecc_vprintf_pro (a : Z) (len : nat) (f : nat -> mword 8)
    ...
    (∀ (h' : CpuId) (m' : regfile) (fd ap : mword 64),
       ...
       ⌜ ap = m !!! Regidx a2_idx ⌝ -∗
       (* ...AND WHICH DESCRIPTOR IT PARKED (lane CAT-WALK, W1): the
          prologue's [c.mv s6,a0] is what makes the loop's payment
          statable at the caller's own a0. *)
       ⌜ fd = m !!! Regidx a0_idx ⌝ -∗
  ...
  Proof using .
    ...
    iDestruct (utext_str_nonul with "Hstr") as %Hnn.
    destruct seccomp_syms_pins
      as (_ & _ & _ & Hvprintf & _ & _ & _ & _ & _ & _ & _).
    rewrite Hvprintf.
    ...
    assert (Ho16 : uoff_sdsp (mword_of_int 2 : mword 6) = 16)
      by (vm_compute; reflexivity).
    (* ---- 0x4b8  c.addi16sp sp,sp,-96 -- THE PUSH ---- *)
    iApply (wp_uk_caddi16sp_dn N h m (mword_of_int 0x4b8)
              (mword_of_int 58 : mword 6) 12 (4 + n)
              ltac:(apply bv_eq; vm_compute; reflexivity)
              with "[] Hrun").
    { iApply (uis_seccomp_4b8 with "Hcode"). }
    iIntros "Hframe".
    assert (E4d6 : add_vec_int (mword_of_int 0x4b8 : mword 64) 2
                   = mword_of_int 0x4ba)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite Hsp E4d6.
    iIntros (h0) "Hrun".
    ...
    assert (Ec4 : (sign_extend' 64 (caddi4spn_imm (mword_of_int 24 : mword 8))
      ...
      by (apply bv_eq; vm_compute; reflexivity).
    iApply (wp_uk_caddi4spn N h3 mp1 (mword_of_int 0x4c0)
              ...
              with "[] Hrun").
    { iApply (uis_seccomp_4c0 with "Hcode"). }
    assert (E4de : add_vec_int (mword_of_int 0x4c0 : mword 64) 2
                   = mword_of_int 0x4c2)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E4de.
    iIntros (h4) "Hrun".
    set (mp2 := <[Regidx s0_idx := regval_into_reg sp0]> mp1).
    (* ---- 0x4c2  lbu s1,0(a1) -- the FIRST character, out of .rodata ---- *)
    ...
    iApply (wp_uk_lbu_text N h4 mp2 (mword_of_int 0x4c2)
              (mword_of_int 0 : mword 12) a1_idx s1_idx a (f 0%nat) (4 + n)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              Haddr0 ltac:(vm_compute; discriminate)
              with "[] Hb0 Hrun").
    { iApply (uis_seccomp_4c2 with "Hcode"). }
    assert (E4e0 : add_vec_int (mword_of_int 0x4c2 : mword 64) 4
                   = mword_of_int 0x4c6)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E4e0.
    iIntros (h5) "Hrun".
    ...
    (* ---- 0x4c6  beqz s1,0x6ec -- NOT taken: the string is non-empty ---- *)
    ...
    iApply (wp_uk_btype0 N h5 mp3 (mword_of_int 0x4c6)
              (mword_of_int 550 : mword 13) s1_idx BEQ false
              (add_vec (mword_of_int 0x4c6 : mword 64)
                 (sign_extend' 64 (mword_of_int 550 : mword 13)))
              (4 + n) Hnt0 eq_refl ltac:(discriminate)
              with "[] Hrun").
    { iApply (uis_seccomp_4c6 with "Hcode"). }
    assert (E4e4 : add_vec_int (mword_of_int 0x4c6 : mword 64) 4
                   = mword_of_int 0x4ca)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E4e4.
    iIntros (h6) "Hrun".
    ...
    { rewrite /mp3 (upd_ne mp2 (Regidx s1_idx) (Regidx s2_idx) _
      ...
      rewrite /mp1. exact (upd_ne m (Regidx csp_rs1) (Regidx s2_idx) _
               ltac:(vm_compute; discriminate)). }
    iApply (wp_uk_csdsp N h6 mp3 (mword_of_int 0x4ca)
              (mword_of_int 8 : mword 6) s2_idx (uint sp0 - 32) w4 (4 + n)
              ltac:(rewrite Hspp3 Hsp96 Ho64; lia)
              ltac:(rewrite Zminus_mod Hal8; reflexivity)
              with "[] Hw4 Hrun").
    { iApply (uis_seccomp_4ca with "Hcode"). }
    iIntros "Hw4". rewrite Hrs2_idx.
    assert (E4e8 : add_vec_int (mword_of_int 0x4ca : mword 64) 2
                 = mword_of_int 0x4cc)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E4e8.
    iIntros (h7) "Hrun".
    (* ---- 0x4cc  c.sdsp s3,56(sp) ---- *)
    ...
    { rewrite /mp3 (upd_ne mp2 (Regidx s1_idx) (Regidx s3_idx) _
      ...
      rewrite /mp1. exact (upd_ne m (Regidx csp_rs1) (Regidx s3_idx) _
               ltac:(vm_compute; discriminate)). }
    iApply (wp_uk_csdsp N h7 mp3 (mword_of_int 0x4cc)
              (mword_of_int 7 : mword 6) s3_idx (uint sp0 - 40) w5 (4 + n)
              ltac:(rewrite Hspp3 Hsp96 Ho56; lia)
              ltac:(rewrite Zminus_mod Hal8; reflexivity)
              with "[] Hw5 Hrun").
    { iApply (uis_seccomp_4cc with "Hcode"). }
    iIntros "Hw5". rewrite Hrs3_idx.
    assert (E4ea : add_vec_int (mword_of_int 0x4cc : mword 64) 2
                 = mword_of_int 0x4ce)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E4ea.
    iIntros (h8) "Hrun".
    (* ---- 0x4ce  c.sdsp s4,48(sp) ---- *)
    ...
    { rewrite /mp3 (upd_ne mp2 (Regidx s1_idx) (Regidx s4_idx) _
      ...
      rewrite /mp1. exact (upd_ne m (Regidx csp_rs1) (Regidx s4_idx) _
               ltac:(vm_compute; discriminate)). }
    iApply (wp_uk_csdsp N h8 mp3 (mword_of_int 0x4ce)
              (mword_of_int 6 : mword 6) s4_idx (uint sp0 - 48) w6 (4 + n)
              ltac:(rewrite Hspp3 Hsp96 Ho48; lia)
              ltac:(rewrite Zminus_mod Hal8; reflexivity)
              with "[] Hw6 Hrun").
    { iApply (uis_seccomp_4ce with "Hcode"). }
    iIntros "Hw6". rewrite Hrs4_idx.
    assert (E4ec : add_vec_int (mword_of_int 0x4ce : mword 64) 2
                 = mword_of_int 0x4d0)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E4ec.
    iIntros (h9) "Hrun".
    (* ---- 0x4d0  c.sdsp s5,40(sp) ---- *)
    ...
    { rewrite /mp3 (upd_ne mp2 (Regidx s1_idx) (Regidx s5_idx) _
      ...
      rewrite /mp1. exact (upd_ne m (Regidx csp_rs1) (Regidx s5_idx) _
               ltac:(vm_compute; discriminate)). }
    iApply (wp_uk_csdsp N h9 mp3 (mword_of_int 0x4d0)
              (mword_of_int 5 : mword 6) s5_idx (uint sp0 - 56) w7 (4 + n)
              ltac:(rewrite Hspp3 Hsp96 Ho40; lia)
              ltac:(rewrite Zminus_mod Hal8; reflexivity)
              with "[] Hw7 Hrun").
    { iApply (uis_seccomp_4d0 with "Hcode"). }
    iIntros "Hw7". rewrite Hrs5_idx.
    assert (E4ee : add_vec_int (mword_of_int 0x4d0 : mword 64) 2
                 = mword_of_int 0x4d2)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E4ee.
    iIntros (h10) "Hrun".
    (* ---- 0x4d2  c.sdsp s6,32(sp) ---- *)
    ...
    { rewrite /mp3 (upd_ne mp2 (Regidx s1_idx) (Regidx s6_idx) _
      ...
      rewrite /mp1. exact (upd_ne m (Regidx csp_rs1) (Regidx s6_idx) _
               ltac:(vm_compute; discriminate)). }
    iApply (wp_uk_csdsp N h10 mp3 (mword_of_int 0x4d2)
              (mword_of_int 4 : mword 6) s6_idx (uint sp0 - 64) w8 (4 + n)
              ltac:(rewrite Hspp3 Hsp96 Ho32; lia)
              ltac:(rewrite Zminus_mod Hal8; reflexivity)
              with "[] Hw8 Hrun").
    { iApply (uis_seccomp_4d2 with "Hcode"). }
    iIntros "Hw8". rewrite Hrs6_idx.
    assert (E4f0 : add_vec_int (mword_of_int 0x4d2 : mword 64) 2
                 = mword_of_int 0x4d4)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E4f0.
    iIntros (h11) "Hrun".
    (* ---- 0x4d4  c.sdsp s7,24(sp) ---- *)
    ...
    { rewrite /mp3 (upd_ne mp2 (Regidx s1_idx) (Regidx s7_idx) _
      ...
      rewrite /mp1. exact (upd_ne m (Regidx csp_rs1) (Regidx s7_idx) _
               ltac:(vm_compute; discriminate)). }
    iApply (wp_uk_csdsp N h11 mp3 (mword_of_int 0x4d4)
              (mword_of_int 3 : mword 6) s7_idx (uint sp0 - 72) w9 (4 + n)
              ltac:(rewrite Hspp3 Hsp96 Ho24; lia)
              ltac:(rewrite Zminus_mod Hal8; reflexivity)
              with "[] Hw9 Hrun").
    { iApply (uis_seccomp_4d4 with "Hcode"). }
    iIntros "Hw9". rewrite Hrs7_idx.
    assert (E4f2 : add_vec_int (mword_of_int 0x4d4 : mword 64) 2
                 = mword_of_int 0x4d6)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E4f2.
    iIntros (h12) "Hrun".
    (* ---- 0x4d6  c.sdsp s8,16(sp) ---- *)
    ...
    { rewrite /mp3 (upd_ne mp2 (Regidx s1_idx) (Regidx s8_idx) _
      ...
      rewrite /mp1. exact (upd_ne m (Regidx csp_rs1) (Regidx s8_idx) _
               ltac:(vm_compute; discriminate)). }
    iApply (wp_uk_csdsp N h12 mp3 (mword_of_int 0x4d6)
              (mword_of_int 2 : mword 6) s8_idx (uint sp0 - 80) w10 (4 + n)
              ltac:(rewrite Hspp3 Hsp96 Ho16; lia)
              ltac:(rewrite Zminus_mod Hal8; reflexivity)
              with "[] Hw10 Hrun").
    { iApply (uis_seccomp_4d6 with "Hcode"). }
    iIntros "Hw10". rewrite Hrs8_idx.
    assert (E4f4 : add_vec_int (mword_of_int 0x4d6 : mword 64) 2
                 = mword_of_int 0x4d8)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E4f4.
    iIntros (h13) "Hrun".
    (* ---- 0x4d8  c.mv s6,a0 ---- *)
    iApply (wp_uk_cmv N h13 mp3 (mword_of_int 0x4d8) s6_idx a0_idx
              (add_vec zero_reg (mp3 !!! Regidx a0_idx)) (4 + n)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate) eq_refl
              with "[] Hrun").
    { iApply (uis_seccomp_4d8 with "Hcode"). }
    assert (E4f6 : add_vec_int (mword_of_int 0x4d8 : mword 64) 2
                 = mword_of_int 0x4da)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E4f6.
    iIntros (h14) "Hrun".
    ...
    (* ---- 0x4da  c.mv s4,a1 ---- *)
    iApply (wp_uk_cmv N h14 mp4 (mword_of_int 0x4da) s4_idx a1_idx
              (add_vec zero_reg (mp4 !!! Regidx a1_idx)) (4 + n)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate) eq_refl
              with "[] Hrun").
    { iApply (uis_seccomp_4da with "Hcode"). }
    assert (E4f8 : add_vec_int (mword_of_int 0x4da : mword 64) 2
                 = mword_of_int 0x4dc)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E4f8.
    iIntros (h15) "Hrun".
    ...
    (* ---- 0x4dc  c.mv s7,a2 ---- *)
    iApply (wp_uk_cmv N h15 mp5 (mword_of_int 0x4dc) s7_idx a2_idx
              (add_vec zero_reg (mp5 !!! Regidx a2_idx)) (4 + n)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate) eq_refl
              with "[] Hrun").
    { iApply (uis_seccomp_4dc with "Hcode"). }
    assert (E4fa : add_vec_int (mword_of_int 0x4dc : mword 64) 2
                 = mword_of_int 0x4de)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E4fa.
    iIntros (h16) "Hrun".
    ...
    (* ---- 0x4de  c.li s3,0 ---- *)
    iApply (wp_uk_cli N h16 mp6 (mword_of_int 0x4de)
              (mword_of_int 0 : mword 6) s3_idx (4 + n)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate) with "[] Hrun").
    { iApply (uis_seccomp_4de with "Hcode"). }
    assert (E4fc : add_vec_int (mword_of_int 0x4de : mword 64) 2
                 = mword_of_int 0x4e0)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E4fc.
    iIntros (h17) "Hrun".
    ...
    (* ---- 0x4e0  c.li s2,0 ---- *)
    iApply (wp_uk_cli N h17 mp7 (mword_of_int 0x4e0)
              (mword_of_int 0 : mword 6) s2_idx (4 + n)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate) with "[] Hrun").
    { iApply (uis_seccomp_4e0 with "Hcode"). }
    assert (E4fe : add_vec_int (mword_of_int 0x4e0 : mword 64) 2
                 = mword_of_int 0x4e2)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E4fe.
    iIntros (h18) "Hrun".
    ...
    (* ---- 0x4e2  c.li a4,0 ---- *)
    iApply (wp_uk_cli N h18 mp8 (mword_of_int 0x4e2)
              (mword_of_int 0 : mword 6) a4_idx (4 + n)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate) with "[] Hrun").
    { iApply (uis_seccomp_4e2 with "Hcode"). }
    assert (E500 : add_vec_int (mword_of_int 0x4e2 : mword 64) 2
                 = mword_of_int 0x4e4)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E500.
    iIntros (h19) "Hrun".
    ...
    (* ---- 0x4e4  li s5,37 ---- *)
    iApply (wp_uk_li N h19 mp9 (mword_of_int 0x4e4)
    ...
    { iApply (uis_seccomp_4e4 with "Hcode"). }
    assert (E502 : add_vec_int (mword_of_int 0x4e4 : mword 64) 4
                 = mword_of_int 0x4e8)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E502.
    iIntros (h20) "Hrun".
    ...
    (* ---- 0x4e8  li s8,100 ---- *)
    iApply (wp_uk_li N h20 mp10 (mword_of_int 0x4e8)
              (mword_of_int 100 : mword 12) s8_idx
              (add_vec zero_reg (sign_extend' 64 (mword_of_int 100 : mword 12))) (4 + n)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate) eq_refl with "[] Hrun").
    { iApply (uis_seccomp_4e8 with "Hcode"). }
    assert (E506 : add_vec_int (mword_of_int 0x4e8 : mword 64) 4
                 = mword_of_int 0x4ec)
      by (apply bv_eq; vm_compute; reflexivity).
    rewrite E506.
    iIntros (h21) "Hrun".
    set (mp11 := <[Regidx s8_idx := regval_into_reg (add_vec zero_reg (sign_extend' 64 (mword_of_int 100 : mword 12)))]> mp10).
    (* ---- 0x4ec  c.j 0x50e -- into the loop ---- *)
    assert (Etgt50a : (mword_of_int 0x50e : mword 64)
                      = add_vec (mword_of_int 0x4ec : mword 64)
                          (sign_extend' 64
                             (sign_extend' 21
                                (concat_vec (mword_of_int 17 : mword 11) ('b"0")))))
      by (apply bv_eq; vm_compute; reflexivity).
    iApply (wp_uk_cj N h21 mp11 (mword_of_int 0x4ec)
              (mword_of_int 17 : mword 11) (mword_of_int 0x50e) (4 + n)
              Etgt50a ltac:(vm_compute; reflexivity)
              with "[] Hrun").
    { iApply (uis_seccomp_4ec with "Hcode"). }
    iIntros (h22) "Hrun".
  ...
  (* --------------------------------------------------------------------- *)
  (* vprintf(fd, fmt, ap) @0x4b8, for a format string with no '%'.           *)
  (*                                                                        *)
  (* The contract asks for a NON-EMPTY string.  cat prints four literals    *)
  (* and none of them is empty, and taking [0 < len] deletes the whole       *)
  (* 0x4c6 arm -- the one that jumps straight to the shared tail with s2..s8 *)
  ...
  (* so re-admitting it later is a branch, not a rewrite.                    *)
  (* --------------------------------------------------------------------- *)
  Lemma wp_ksecc_vprintf (a : Z) (len : nat) (f : nat -> mword 8)
      (h : CpuId) (m : regfile) (n : nat) (Ci Co : iProp Σ) :
    0 <= a -> a + Z.of_nat len + 2 < 2 ^ 31 ->
    (0 < len)%nat ->
    (forall j : nat, (j < len)%nat -> bv_unsigned (f j) <> 37) ->
    m !!! Regidx a1_idx = mword_of_int a ->
    ksecc_pay_seq N (m !!! Regidx a0_idx) f 0%nat len Ci Co -∗
    seccomp_code γt -∗
    utext_str γt a len f -∗
    Ci -∗
    urun N h m (mword_of_int SeccompSyms.vprintf) (12 + (4 + n)) -∗
  ...
  Proof using .
    ...
    iApply (wp_ksecc_vprintf_pro a len f h m n Ha0 Habnd Hlen Ha1
    ...
    assert (Hslen : S (len - 1)%nat = len) by lia.
    iApply (wp_ksecc_vprintf_loop m (m !!! Regidx csp_rs1) fd ap a len f
              0%nat (len - 1)%nat Ha0 Habnd ltac:(intros j Hj; apply Hpct; lia)
              eq_refl Hal8 Hlo 0%nat h' m' n Ci Co ltac:(lia) Hk0 Hinv0 Hs1z
              with "[Hpay] Hcode Hstr HCi Hw1 Hw2 Hw3 Hw4 Hw5 Hw6 Hw7 Hw8 Hw9 Hw10 Hw11 Hw12 Hrun Hcont").
    rewrite Hslen Hfd. iExact "Hpay".
  Qed.
