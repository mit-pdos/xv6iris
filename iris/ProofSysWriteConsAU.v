(* ProofSysWriteConsAU.v -- sys_write's CONSOLE walk: the whole-function WP
   that seals [SpecSysWriteConsAU.SYSWRITE_CONS_AU], over
   [SpecFilewriteCons]'s located device-arm form of filewrite.

   Worklist: claude-notes/projects/fs-syscall-specs.md, the console-write
   lane.  A PARALLEL FORM beside [ProofSysWrite.SysWriteProof] -- R10: the
   landed proof and its contract do not move -- and, more immediately, a
   COPY-ADAPT of [ProofSysWriteAU.v], the inode arm's shell, which is that
   proof's walk instruction for instruction.  Review it as the diff against
   ProofSysWriteAU.v, which is what it was produced as.

   ==== WHY THE SHELL IS ALMOST UNCHANGED ==============================

   Because sys_write itself does not know what a console is.  Twenty-five
   instructions push a six-slot frame, call argaddr / argint / argfd,
   reload the three arguments and call filewrite; the DISPATCH on
   [f->type], the [devsw[major].write] indexing and the call through the
   cell are all filewrite's ([SpecFilewriteCons]'s domain, one shell
   down).  So this file's job is the FRAME and the RELAY, and the diff
   against the AU shell is five lines.

   THE DIFF, EXHAUSTIVELY.

   1. THE DESCRIPTOR IS A DEVICE.  The contract's fragment pins
      [FdOpen rb true (FdDevice ma)] where the AU shell's pins
      [FdOpen rb true (FdInode i)], and the extra premise [ma = CONSOLE]
      rides down to the callee.  The B1 seam ([ProcInv.proc_priv_lend] +
      [FdSlots.fd_st_agree]) is the SAME bridge at the other constructor:
      the lend hands the reference out at an existential [stf], the
      fragment pins it, and that equation is exactly the callee's domain
      premise.  Nothing else in the fd plumbing moves -- a console write
      touches neither the fd table nor an offset, so the fragment is
      threaded through and returned.

   2. THE CALLEE IS [SpecFilewriteCons.FILEWRITE_CONS].  Same arguments as
      the AU call with [rb], [ma] and the seed [tr0] in place of [rb], [i]
      and the receipt family.

   3. THE EXTRA IS A SEED, NOT A BUNDLE.  [uart_sent fsc_uart tr0] is
      PERSISTENT (a mono_list lower bound), so it is introduced with [#]
      and handed to the callee without being spent -- where the AU shell's
      [awrite_commits] is linear and moves.  That is the whole difference
      the "no commit machinery" ruling makes at this altitude
      (SpecSysWriteConsAU.v: WHY THERE IS NO COMMIT BUNDLE).

   4. THE POST IS [write_cons_arms].  Like the AU shell, this walk fires
      nothing -- every byte's instant is inside consolewrite's chunk loop,
      and the receipts are history anyway -- so the armed post comes back
      from the callee and is handed on exactly as it came.

   5. THE argfd FAILURE ARM IS DEAD, for the AU shell's own reason: the
      contract's [arg_fd v (pv_ofile (us_V U)) = Some (fd, fv)] premise
      contradicts [SpecArgfd.argfd_post]'s failure disjunct.  That is what
      leaves the arms THREE (OK / SHORT / the sign guard's NEG) rather
      than the landed blanket, and the NEG arm is filewrite's own guard at
      +0x1c -- below this shell, in the callee's contract.

   Everything else -- the six-slot frame, the [word_pointsto_split4] carve
   of the [int n] out of the upper half of slot 4, the three callee seams,
   the tail -- is [ProofSysWriteAU.v]'s text, which is [ProofSysWrite.v]'s.
   Read those two headers first; nothing of them is restated here.

   BINDERS: [ProofSysWriteAU]'s section list VERBATIM. *)
...
(* ---- the console side ---- *)
Require Import SpecSysWriteConsAU.  (* the contract this file seals         *)
Require Import SpecFilewriteCons.   (* the callee's located device form     *)
...
Module SysWriteConsAUProof (Argaddr : ARGADDR) (Argint : ARGINT) (Argfd : ARGFD)
                      (FilewriteCons : FILEWRITE_CONS) : SYSWRITE_CONS_AU.

Section ProofSysWriteConsAU.
  ...
  Proof.
    ...
    intros nn pcE pj ret_tgt Hav Hj Hgs Hlens Hfj Hfprocs
           Harg0 Harg1 Harg2 Hwp Hdq Heb Hargfd Hma.
    ...
    (* THE SEED IS PERSISTENT (diff item 3): a mono_list lower bound, so it
       is introduced with [#] and handed to the callee without being spent. *)
    ...
    iDestruct (swau_sp_bounds _ _ _ _ Hkpos with "Hcg") as %Hspb.
    ...
    - (* ================= SUCCESS: the descriptor resolved ============= *)
      ...
      (* ---- THE FD BRIDGE.  The lend hands the reference out at an
         ...
         no new carrier, no new agreement law -- and it is what puts the
         call inside [FILEWRITE_CONS]'s domain (open, WRITABLE, a DEVICE
      ...
      (* THE ARMED POST goes straight through: this shell fires nothing --
         every byte's instant is inside consolewrite's chunk loop, and the
         receipts are history anyway -- so [Harms] is handed on exactly as
         it came back, and the descriptor's fragment with it. *)
...
End ProofSysWriteConsAU.

End SysWriteConsAUProof.
