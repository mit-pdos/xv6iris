...
(*                                                                        *)
(*   gettoken @0x2ec  -- 104 instructions, an EIGHT-word frame, THREE      *)
...
(*ALIASES-END*)
  ...
  (* ===================================================================== *)
  (* §9 gettoken @0x2ec -- 104 instructions, an EIGHT-word frame, THREE      *)
  (* scans and a switch.                                                    *)
  (*                                                                       *)
  ...
  (* are the SAME CODE as peek's, at 0x316 and 0x378 rather than 0x44a --   *)
  ...
  (* times.  The token-body scan at 0x3dc is the new one: the same loop     *)
  ...
  (* THE WHITESPACE SCAN, ONCE, FOR ALL THREE OF ITS COPIES.
     ...
     peek's is at p = 0x44a and gettoken's two at p = 0x316 and p = 0x378 --
  ...
  Proof using .
    ...
    iApply (wp_uk_jal N h2 m2 (mword_of_int (p + 6))
              ji ra_idx (mword_of_int 0xa5e) (mword_of_int (p + 10)) (2 + nn)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate)
              Hjt
              ltac:(symmetry;
  ...
  (* ---- the token-body scan, 0x3dc..0x3f6 -------------------------------- *)


  (* WHERE THE TOKEN SCAN COMES OUT.  Its two live exits are the SAME
     instruction pair -- [li s5,97 ; c.j 0x364] at 0x40e and again at 0x414,
     gcc having duplicated the tail -- so a byte that ends the token (either
     table) leaves the scan at 0x364 with the answer already in s5, and only
     running off the end of the line leaves it anywhere else (0x400, past
     the third copy of [li s5,97] at 0x3fc).  Folding the three stubs into
  ...
  Lemma wp_kshp_tok_scan (dq dw dv : dfrac) (s0 : Z) (len : nat)
    ...
    urun N h mc (mword_of_int 0x3dc) (2 + nn) -∗
    (ustr γd dq s0 len f -∗
     ustr γd dw ushp_whitespace 5 ushp_ws_f -∗
     ustr γd dv ushp_symbols 7 ushp_sym_f -∗
       ∀ (h' : CpuId) (mc' : regfile),
  ...
  Proof using .
    ...
    (* ---- 0x3dc  lbu a1,0(s1) ---- *)
    iDestruct (ustr_byte γd dq s0 len f j Hj with "Hstr") as "[Hb Hcl]".
    iApply (wp_uk_lbu N h mc (mword_of_int 0x3dc)
              (mword_of_int 0 : mword 12) s1_idx a1_idx dq
              (s0 + Z.of_nat j) (f j) (2 + nn)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(rewrite Hs1 (uint_moi (s0 + Z.of_nat j)
              ...
              ltac:(vm_compute; discriminate)
              with "[] Hb Hrun").
    { iApply (uis_shp_3dc with "Hcode"). }
    iIntros "Hb". iDestruct ("Hcl" with "Hb") as "Hstr".
    rewrite (ushp_pc_step 0x3dc 4). iIntros (h1) "Hrun".
    set (m1 := <[Regidx a1_idx
                 := regval_into_reg (zero_extend' 64 ((f j) : mword 8)
                                     : mword 64)]> mc).
    ...
    { rewrite (upd_eq mc (Regidx a1_idx)
      ...
      exact (zext8_moi (f j)). }
    (* ---- 0x3e0  c.mv a0,s3 ---- *)
    iApply (wp_uk_cmv N h1 m1 (mword_of_int 0x3e0) a0_idx s3_idx
              (mword_of_int ushp_whitespace) (2 + nn)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate)
              ltac:(rewrite (Hm1 s3_idx ltac:(vm_compute; discriminate)) Hs3;
    ...
    rewrite (ushp_pc_step 0x3e0 2). iIntros (h2) "Hrun".
    set (m2 := <[Regidx a0_idx
                 := regval_into_reg (mword_of_int ushp_whitespace
                                     : mword 64)]> m1).
    ...
    (* ---- 0x3e2  jal a5e <strchr> -- the WHITESPACE table ---- *)
    iApply (wp_uk_jal N h2 m2 (mword_of_int 0x3e2)
              (mword_of_int 1660 : mword 21) ra_idx
              (mword_of_int 0xa5e) (mword_of_int 0x3e6) (2 + nn)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate)
    ...
    iIntros (h3) "Hrun".
    set (m3 := <[Regidx ra_idx
                 := regval_into_reg (mword_of_int 0x3e6 : mword 64)]> m2).
    ...
    { rewrite (Hm3 a1_idx ltac:(vm_compute; discriminate)).
      rewrite (Hm2 a1_idx ltac:(vm_compute; discriminate)). exact Ha1_1. }
    assert (Eret3 : ret_pc (m3 !!! Regidx ra_idx) = mword_of_int 0x3e6).
    { rewrite (upd_eq m2 (Regidx ra_idx)
                 (regval_into_reg (mword_of_int 0x3e6 : mword 64))).
    ...
    (* ---- 0x3e6  c.bnez a0,0x414 -- a whitespace byte ENDS the token ---- *)
    destruct (ushp_is_ws (f j)) eqn:Ews.
    { destruct (ushp_ws_chr_nz (f j) Ews) as [ k [ Hk Hchr ] ].
      ...
      iApply (wp_uk_cbnez N h4 m4 (mword_of_int 0x3e6)
                (mword_of_int 23 : mword 8) (mword_of_int 2 : mword 3)
                a0_idx true (mword_of_int 0x414) (2 + nn)
                ltac:(vm_compute; reflexivity) Htk
                ltac:(apply bv_eq; vm_compute; reflexivity)
                ltac:(intros _; vm_compute; reflexivity)
                with "[] Hrun").
      { iApply (uis_shp_3e6 with "Hcode"). }
      iIntros (h5) "Hrun".
      (* ---- 0x414  li s5,97 ---- *)
      iApply (wp_uk_li N h5 m4 (mword_of_int 0x414)
                (mword_of_int 97 : mword 12) s5_idx (mword_of_int 97) (2 + nn)
                ltac:(unfold unot_sp; vm_compute; discriminate)
                ltac:(vm_compute; discriminate)
                ltac:(apply bv_eq; vm_compute; reflexivity)
                with "[] Hrun").
      { iApply (uis_shp_414 with "Hcode"). }
      rewrite (ushp_pc_step 0x414 4). iIntros (h6) "Hrun".
      (* ---- 0x418  c.j 0x364 ---- *)
      iApply (wp_uk_cj N h6
                (<[Regidx s5_idx
                   := regval_into_reg (mword_of_int 97 : mword 64)]> m4)
                (mword_of_int 0x418) (mword_of_int 1958 : mword 11)
                (mword_of_int 0x364) (2 + nn)
                ltac:(apply bv_eq; vm_compute; reflexivity)
                ltac:(vm_compute; reflexivity)
                with "[] Hrun").
      ...
      assert (Eex : ushp_tok_exit len f j = 0x364).
    ...
    iApply (wp_uk_cbnez N h4 m4 (mword_of_int 0x3e6)
              (mword_of_int 23 : mword 8) (mword_of_int 2 : mword 3)
              a0_idx false (mword_of_int 0x414) (2 + nn)
              ltac:(vm_compute; reflexivity) Htk
              ltac:(apply bv_eq; vm_compute; reflexivity)
              ltac:(discriminate)
              with "[] Hrun").
    { iApply (uis_shp_3e6 with "Hcode"). }
    rewrite (ushp_pc_step 0x3e6 2). iIntros (h5) "Hrun".
    (* ---- 0x3e8  lbu a1,0(s1) -- the same byte, read again ---- *)
    iDestruct (ustr_byte γd dq s0 len f j Hj with "Hstr") as "[Hb Hcl]".
    iApply (wp_uk_lbu N h5 m4 (mword_of_int 0x3e8)
              (mword_of_int 0 : mword 12) s1_idx a1_idx dq
              (s0 + Z.of_nat j) (f j) (2 + nn)
              ltac:(unfold unot_sp; vm_compute; discriminate)
    ...
    iIntros "Hb". iDestruct ("Hcl" with "Hb") as "Hstr".
    rewrite (ushp_pc_step 0x3e8 4). iIntros (h6) "Hrun".
    ...
    { rewrite (upd_eq m4 (Regidx a1_idx)
                 (regval_into_reg (zero_extend' 64 ((f j) : mword 8)
                                   : mword 64))).
      exact (zext8_moi (f j)). }
    (* ---- 0x3ec  c.mv a0,s5 -- the SYMBOLS table ---- *)
    iApply (wp_uk_cmv N h6 n1 (mword_of_int 0x3ec) a0_idx s5_idx
    ...
    rewrite (ushp_pc_step 0x3ec 2). iIntros (h7) "Hrun".
    ...
    iApply (wp_uk_jal N h7 n2 (mword_of_int 0x3ee)
              (mword_of_int 1648 : mword 21) ra_idx
              (mword_of_int 0xa5e) (mword_of_int 0x3f2) (2 + nn)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate)
              ltac:(apply bv_eq; vm_compute; reflexivity)
              ltac:(apply bv_eq; vm_compute; reflexivity)
              ltac:(vm_compute; reflexivity)
              with "[] Hrun").
    { iApply (uis_shp_3ee with "Hcode"). }
    iIntros (h8) "Hrun".
    set (n3 := <[Regidx ra_idx
                 := regval_into_reg (mword_of_int 0x3f2 : mword 64)]> n2).
    ...
    assert (Eret7 : ret_pc (n3 !!! Regidx ra_idx) = mword_of_int 0x3f2).
    { rewrite (upd_eq n2 (Regidx ra_idx)
                 (regval_into_reg (mword_of_int 0x3f2 : mword 64))).
      apply bv_eq; vm_compute; reflexivity. }
    ...
    (* ---- 0x3f2  c.bnez a0,0x40e -- a symbol byte ENDS the token too ---- *)
    ...
    { destruct (ushp_sym_chr_nz (f j) Esy) as [ k [ Hk Hchr ] ].
      ...
      { rewrite Hc0_4 Hchr.
        ...
        rewrite Hz. reflexivity. }
      iApply (wp_uk_cbnez N h9 n4 (mword_of_int 0x3f2)
                (mword_of_int 14 : mword 8) (mword_of_int 2 : mword 3)
                a0_idx true (mword_of_int 0x40e) (2 + nn)
                ltac:(vm_compute; reflexivity) Htk2
                ltac:(apply bv_eq; vm_compute; reflexivity)
                ltac:(intros _; vm_compute; reflexivity)
                with "[] Hrun").
      { iApply (uis_shp_3f2 with "Hcode"). }
      iIntros (h10) "Hrun".
      (* ---- 0x40e  li s5,97 ---- *)
      iApply (wp_uk_li N h10 n4 (mword_of_int 0x40e)
                (mword_of_int 97 : mword 12) s5_idx (mword_of_int 97) (2 + nn)
                ltac:(unfold unot_sp; vm_compute; discriminate)
                ltac:(vm_compute; discriminate)
                ltac:(apply bv_eq; vm_compute; reflexivity)
                with "[] Hrun").
      { iApply (uis_shp_40e with "Hcode"). }
      rewrite (ushp_pc_step 0x40e 4). iIntros (h11) "Hrun".
      (* ---- 0x412  c.j 0x364 ---- *)
      iApply (wp_uk_cj N h11
                (<[Regidx s5_idx
                   := regval_into_reg (mword_of_int 97 : mword 64)]> n4)
                (mword_of_int 0x412) (mword_of_int 1961 : mword 11)
                (mword_of_int 0x364) (2 + nn)
                ltac:(apply bv_eq; vm_compute; reflexivity)
                ltac:(vm_compute; reflexivity)
                with "[] Hrun").
    ...
    iApply (wp_uk_cbnez N h9 n4 (mword_of_int 0x3f2)
              (mword_of_int 14 : mword 8) (mword_of_int 2 : mword 3)
              a0_idx false (mword_of_int 0x40e) (2 + nn)
              ltac:(vm_compute; reflexivity) Htk2
              ltac:(apply bv_eq; vm_compute; reflexivity)
              ltac:(discriminate)
              with "[] Hrun").
    { iApply (uis_shp_3f2 with "Hcode"). }
    rewrite (ushp_pc_step 0x3f2 2). iIntros (h10) "Hrun".
    (* ---- 0x3f4  c.addi s1,s1,1 ---- *)
    assert (E1 : (sign_extend' 64 (mword_of_int 1 : mword 6) : mword 64)
                 = mword_of_int 1)
      by (apply bv_eq; vm_compute; reflexivity).
    iApply (wp_uk_caddi N h10 n4 (mword_of_int 0x3f4)
              (mword_of_int 1 : mword 6) s1_idx
              (mword_of_int (s0 + Z.of_nat (S j))) (2 + nn)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate)
    ...
    rewrite (ushp_pc_step 0x3f4 2). iIntros (h11) "Hrun".
    ...
    { (* the token ran to [es] *)
      ...
      { cbn [uv_btaken]. rewrite Hs2_5 Hs1_5 Hend.
        rewrite (ushp_moi_neq (s0 + Z.of_nat len) (s0 + Z.of_nat len)
                   ltac:(lia) ltac:(lia)).
        rewrite Z.eqb_refl. reflexivity. }
      iApply (wp_uk_btype N h11 n5 (mword_of_int 0x3f6)
                (mword_of_int 8166 : mword 13) s1_idx s2_idx BNE false
                (mword_of_int 0x3dc) (2 + nn)
                Htk3
                ltac:(apply bv_eq; vm_compute; reflexivity)
                ltac:(discriminate)
                with "[] Hrun").
      { iApply (uis_shp_3f6 with "Hcode"). }
      rewrite (ushp_pc_step 0x3f6 4). iIntros (h12) "Hrun".
      (* ---- 0x3fa  c.mv s1,s2 ---- *)
      iApply (wp_uk_cmv N h12 n5 (mword_of_int 0x3fa)
                s1_idx s2_idx (mword_of_int (s0 + Z.of_nat len)) (2 + nn)
                ltac:(unfold unot_sp; vm_compute; discriminate)
                ltac:(vm_compute; discriminate)
                ltac:(rewrite Hs2_5; symmetry;
                      exact (ushp_mv_val (s0 + Z.of_nat len)))
                with "[] Hrun").
      { iApply (uis_shp_3fa with "Hcode"). }
      rewrite (ushp_pc_step 0x3fa 2). iIntros (h13) "Hrun".
      set (n6 := <[Regidx s1_idx
                   := regval_into_reg (mword_of_int (s0 + Z.of_nat len)
                                       : mword 64)]> n5).
      (* ---- 0x3fc  li s5,97 ---- *)
      iApply (wp_uk_li N h13 n6 (mword_of_int 0x3fc)
                (mword_of_int 97 : mword 12) s5_idx (mword_of_int 97) (2 + nn)
                ltac:(unfold unot_sp; vm_compute; discriminate)
                ltac:(vm_compute; discriminate)
                ltac:(apply bv_eq; vm_compute; reflexivity)
  ...
  (* THE TRAILING WHITESPACE SCAN, 0x36c..0x38a, with its table setup.
  ...
  Lemma wp_kshp_gtk_ws2 (dq dw : dfrac) (s0 : Z) (len j : nat)
    ...
    ustr γd dq s0 len f -∗
    ustr γd dw ushp_whitespace 5 ushp_ws_f -∗
    urun N h mc (mword_of_int 0x36c) (2 + nn) -∗
    (ustr γd dq s0 len f -∗
     ustr γd dw ushp_whitespace 5 ushp_ws_f -∗
       ∀ (h' : CpuId) (mc' : regfile),
         ⌜ forall t : mword 5, ucallee_saved_idx t = true ->
  ...
  Proof using .
    ...
    (* ---- 0x36c  auipc s3,0x2 ---- *)
    iApply (wp_uk_auipc N h mc (mword_of_int 0x36c)
              (mword_of_int 2 : mword 20) s3_idx (mword_of_int 0x236c)
              (2 + nn)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate)
              ltac:(apply bv_eq; vm_compute; reflexivity)
              with "[] Hrun").
    { iApply (uis_shp_36c with "Hcode"). }
    rewrite (ushp_pc_step 0x36c 4). iIntros (h1) "Hrun".
    set (m1 := <[Regidx s3_idx
                 := regval_into_reg (mword_of_int 0x236c : mword 64)]> mc).
    assert (Hm1 : forall t : mword 5, Regidx t <> Regidx s3_idx ->
                    m1 !!! Regidx t = mc !!! Regidx t)
      by (intros t Ht; exact (upd_ne mc (Regidx s3_idx) (Regidx t) _ Ht)).
    (* ---- 0x370  addi s3,s3,-868  -- s3 = &whitespace ---- *)
    iApply (wp_uk_addi N h1 m1 (mword_of_int 0x370)
              (mword_of_int 3228 : mword 12) s3_idx s3_idx
              (mword_of_int ushp_whitespace) (2 + nn)
              ltac:(unfold unot_sp; vm_compute; discriminate)
              ltac:(vm_compute; discriminate)
              ltac:(rewrite (upd_eq mc (Regidx s3_idx)
                               (regval_into_reg (mword_of_int 0x236c
                                                 : mword 64)));
                    unfold ushp_whitespace;
                    apply bv_eq; vm_compute; reflexivity)
              with "[] Hrun").
    { iApply (uis_shp_370 with "Hcode"). }
    rewrite (ushp_pc_step 0x370 4). iIntros (h2) "Hrun".
    ...
    assert (Hs3_2 : m2 !!! Regidx s3_idx = mword_of_int ushp_whitespace)
      by exact (upd_eq m1 (Regidx s3_idx)
                  (regval_into_reg (mword_of_int ushp_whitespace : mword 64))).
    (* ---- 0x374..0x38a  the entry test and the scan ---- *)
    iApply (wp_kshp_ws_enter 0x374 s2_idx (mword_of_int 1760 : mword 21)
    ...
    { iApply (uis_shp_37e with "Hcode"). }
    { iApply (uis_shp_382 with "Hcode"). }
    { iApply (uis_shp_384 with "Hcode"). }
    { iApply (uis_shp_386 with "Hcode"). }
    { iApply (uis_shp_38a with "Hcode"). }
  ...
  (* [*eq = s], then the trailing scan.  0x368 is reached TWO ways -- from
     0x364's [beqz s6] falling through, and from 0x400's [bnez s6] being
  ...
  Lemma wp_kshp_gtk_eqst (dq dw : dfrac) (s0 eqp : Z) (len j : nat)
    ...
    ustr γd dq s0 len f -∗
    ustr γd dw ushp_whitespace 5 ushp_ws_f -∗
    urun N h mc (mword_of_int 0x368) (2 + nn) -∗
  ...
  Proof using .
    ...
    (* ---- 0x368  sd s1,0(s6)  --  *eq = s ---- *)
    iApply (wp_uk_sd N h mc (mword_of_int 0x368)
              (mword_of_int 0 : mword 12) s6_idx s1_idx eqp v0 (2 + nn)
              ltac:(rewrite Hs6 (uint_moi eqp ltac:(unfold Z64 in *; lia));
                    vm_compute uoff_i12; lia)
              Heq8
              with "[] Hcell Hrun").
    { iApply (uis_shp_368 with "Hcode"). }
    iIntros "Hcell". rewrite Hs1.
    rewrite (ushp_pc_step 0x368 4). iIntros (h1) "Hrun".
  ...
  (* 0x364: [if(eq) *eq = s], the NUL and symbol-free arms' way in. *)
  ...
  Proof using .
    ...
    { (* eq == 0: the store is skipped *)
      ...
      { cbn [uv_btaken]. rewrite Hs6 Hx0 Hnull.
        rewrite (moi_eq_zero 0 ltac:(unfold Z64; lia)). reflexivity. }
      iApply (wp_uk_btype N h mc (mword_of_int 0x364)
                (mword_of_int 8 : mword 13) x0_idx s6_idx BEQ true
                (mword_of_int 0x36c) (2 + nn)
                Htk
                ltac:(apply bv_eq; vm_compute; reflexivity)
                ltac:(intros _; vm_compute; reflexivity)
                with "[] Hrun").
      { iApply (uis_shp_364 with "Hcode"). }
      iIntros (h1) "Hrun".
      iApply (wp_kshp_gtk_ws2 dq dw s0 len j f nn h1 mc
                Hjle Hs0 Hs64 Hs1 Hs2 with "Hcode Hstr Hws Hrun").
      iIntros "Hstr Hws" (h2 mc') "%Hpres %Hret Hrun".
    ...
    iApply (wp_uk_btype N h mc (mword_of_int 0x364)
              ...
              with "[] Hrun").
    { iApply (uis_shp_364 with "Hcode"). }
    rewrite (ushp_pc_step 0x364 4). iIntros (h1) "Hrun".
  ...
  (* 0x400: the SAME [if(eq)] test, at the copy the default arm reaches when
     the token ran to [es].  The cursor is already at the end there, so the
     [c.j 0x38c] that skips the trailing scan skips a no-op -- which is why
  ...
  Proof using .
    ...
    { (* eq == 0: straight to 0x38c ---- *)
      ...
      (* ---- 0x404  c.j 0x38c ---- *)
      iApply (wp_uk_cj N h1 mc (mword_of_int 0x404)
                (mword_of_int 1988 : mword 11) (mword_of_int 0x38c) (2 + nn)
                ltac:(apply bv_eq; vm_compute; reflexivity)
                ltac:(vm_compute; reflexivity)
                with "[] Hrun").
  ...
  (* gettoken's epilogue, 0x392..0x3a4: the EIGHT restores, the pop and the
  ...
  Lemma wp_kshp_gtk_epi (sp0 spl : mword 64) (vals : nat -> mword 64)
    ...
    ustack γd spl 0 -∗
    urun N h me (mword_of_int 0x392) (2 + nn) -∗
    (∀ h' : CpuId,
       urun N h'
  ...
  Proof using .
    ...
    (* ---- 0x392..0x3a0  the eight restores ---- *)
    iApply (wp_kshp_restore spn (2 + nn)
              ...
              (fun i : nat => match i with
                              | 0%nat => 0x392 | 1%nat => 0x394
                              | 2%nat => 0x396 | 3%nat => 0x398
                              | 4%nat => 0x39a | 5%nat => 0x39c
                              | 6%nat => 0x39e | 7%nat => 0x3a0
                              | _ => 0x3a2 end)
    ...
    { rewrite !big_sepL_cons big_sepL_nil.
      iSplit; [ iApply (uis_shp_392 with "Hcode") | ].
      iSplit; [ iApply (uis_shp_394 with "Hcode") | ].
      iSplit; [ iApply (uis_shp_396 with "Hcode") | ].
      iSplit; [ iApply (uis_shp_398 with "Hcode") | ].
      iSplit; [ iApply (uis_shp_39a with "Hcode") | ].
      iSplit; [ iApply (uis_shp_39c with "Hcode") | ].
      iSplit; [ iApply (uis_shp_39e with "Hcode") | ].
      iSplit; [ iApply (uis_shp_3a0 with "Hcode") | done ]. }
    ...
    (* ---- 0x3a2  c.addi16sp sp,sp,64 -- THE POP ---- *)
    iApply (wp_uk_caddi16sp_up N h1 mr (mword_of_int 0x3a2)
  ...
  Lemma wp_kshp_gtk_qst (s0 qp : Z) (k : nat) (v0 : mword 64)
    ...
    shp_code γt -∗
    ushp_cell qp v0 -∗
    urun N h mc (mword_of_int 0x32a) (2 + nn) -∗
  ...
  Proof using .
    ...
    { assert (Htk : true = uv_btaken BEQ (mc !!! Regidx s5_idx)
      ...
      { cbn [uv_btaken]. rewrite Hs5 Hx0 Hnull.
        rewrite (moi_eq_zero 0 ltac:(unfold Z64; lia)). reflexivity. }
      iApply (wp_uk_btype N h mc (mword_of_int 0x32a)
                (mword_of_int 8 : mword 13) x0_idx s5_idx BEQ true
                (mword_of_int 0x332) (2 + nn)
                Htk
                ltac:(apply bv_eq; vm_compute; reflexivity)
                ltac:(intros _; vm_compute; reflexivity)
                with "[] Hrun").
    ...
    iApply (wp_uk_btype N h mc (mword_of_int 0x32a)
              ...
              with "[] Hrun").
    { iApply (uis_shp_32a with "Hcode"). }
    rewrite (ushp_pc_step 0x32a 4). iIntros (h1) "Hrun".
    (* ---- 0x32e  sd s1,0(s5)  --  *q = s ---- *)
    iApply (wp_uk_sd N h1 mc (mword_of_int 0x32e)
    ...
    rewrite (ushp_pc_step 0x32e 4). iIntros (h2) "Hrun".
  ...
  (* THE SWITCH, 0x332..0x362 and its two out-of-line halves 0x3a6..0x3c4.
  ...
  (* ---- the common landing, 0x38c..0x3a4 --------------------------------- *)
  ...
  Lemma wp_kshp_gtk_fin (m : regfile) (sp0 spl : mword 64)
    ...
    ([∗ list] i ↦ _ ∈ [(ra_idx, mword_of_int 7 : mword 6);
       ...
       uword γd (uint sp0 - 8 * (Z.of_nat i + 1)) (vals i)) -∗
    ustack γd spl 0 -∗
    urun N h me (mword_of_int 0x38c) (2 + nn) -∗
  ...
  Proof using .
    ...
    (* ---- 0x38c  sd s1,0(s4)  --  *ps = s ---- *)
    iApply (wp_uk_sd N h me (mword_of_int 0x38c)
    ...
    (* ---- 0x390  c.mv a0,s5  -- the return value ---- *)
    iApply (wp_uk_cmv N h1 me (mword_of_int 0x390) a0_idx s5_idx
    ...
    rewrite (ushp_pc_step 0x390 2). iIntros (h2) "Hrun".
