...
Section UkStoreTrapWrap.
  ...
  Lemma uk_swp_exec_trap (dq : dfrac) (pt : uptd) (Mp : gmap Z (bv 8)) (t : ptree)
      (rsx : regstate)
    ...
    goodmb Du_r Du_w (execute (uv_exp i o)) (u_state rsx (uv_mmd pt Mp t))
      (uv_mmd pt Mp t) = true ->
    exec (execute (uv_exp i o)) (u_state rsx (uv_mmd pt Mp t))
      = Some (er, u_state rsx (uv_mmd pt Mp t)) ->
  ...
  Proof.
    ...
    destruct o as [j | ].
    - iApply (uv_swp_walk pt Mp Mp t t dq rsx rsx rsx (execute i) (ExecuteAs j) _
                ...
                with "Hcert Hany Hrw Hro Hrun Hmm [Hk]").
      iIntros (rs1) "%Hag1 Hrw Hro Hrun Hmm Hany".
      iApply run_exec_post_redirect.
      iApply (uv_swp_walk pt Mp Mp t t dq rs1 rsx rsx (execute j) er _
                Hinj Hinj eq_refl Htok Htok eq_refl Hag1 He Hg2
                with "Hcert Hany Hrw Hro Hrun Hmm [Hk]").
      iIntros (rs2) "%Hag Hrw Hro Hrun Hmm Hany".
      iApply ("Hk" $! rs2 with "[%] Hrw Hro Hrun Hmm Hany"). exact Hag.
    - iApply (uv_swp_walk pt Mp Mp t t dq rsx rsx rsx (execute i) er _
                Hinj Hinj eq_refl Htok Htok eq_refl
                ltac:(intros q _; reflexivity) He Hg2
                with "Hcert Hany Hrw Hro Hrun Hmm [Hk]").
      iIntros (rs2) "%Hag Hrw Hro Hrun Hmm Hany".
...
Section UkStorePostFetch.
  ...
  Lemma uk_store_post_fetch (R : iProp Σ) (Rut : uptd -> iProp Σ) (sz : Z)
    ...
    (R -∗ (TsoCtx.own_context XI -∗ Rut pt) ∗ Rfd fdv ∗ ukb C pt Rfd Rut sz π fdv cw ∗
          (uvb C pt Rfd Rut sz π fdv cw (uM_store M (uint va) kk wval) m (add_vec_int pc dpc) -∗
  ...
  Proof.
    ...
    assert (HMdb : forall j : nat, (j < Z.to_nat kk)%nat ->
              is_Some (uM_data pt Mp !! (uint va + Z.of_nat j))).
    { intros j Hj. destruct (HMb j Hj) as (bb & Hbb). exists bb.
      apply uM_data_lookup. exact (conj Hbb (Hntxj j Hj)). }
    assert (Hmdw : forall j : nat, (j < Z.to_nat kk)%nat ->
              is_Some (md !! pa_add pa j)).
    { intros j Hj. destruct (HMdb j Hj) as (bb & Hbb). exists bb.
      rewrite <- (Hwin j Hj). exact (upa_map_lookup pt _ _ bb Hinjd Hbb). }
    (* ---- the store, pure ---- *)
    destruct (uv_store_mm kk Hk Hk8 Hkdvd Huintk Hwrite_plain pt t' md rsx
                w_st va (ustore_data kk wval)
                Hl Hchk Hcanon Hal (uinpage_one va kk Hpg) Hmdw Hcfgx Hpinsx Htokd')
    ...
    assert (Htext' : uM_text pt Mp' = uM_text pt Mp)
      by exact (uM_text_store pt Mp (uint va) kk wval Hntxj).
    assert (Hdata' : uM_data pt Mp' = uM_store (uM_data pt Mp) (uint va) kk wval)
      by exact (uM_data_store pt Mp (uint va) kk wval Hntxj).
    assert (Hddom : dom (uM_data pt Mp') = dom (uM_data pt Mp))
      by (rewrite Hdata'; apply uM_store_dom; exact HMdb).
    assert (Hmdeq : upa_map pt (uM_data pt Mp') = write_bytes md pa (Z.to_N kk) wval)
      by (rewrite Hdata'; apply upa_map_store;
          [ exact Hinjd | exact HMdb | exact Hwin ]).
    ...
    assert (Hmmeq : write_bytes (uv_mm t'' md) pa (Z.to_N kk) (ustore_data kk wval)
                    = uv_mmd pt Mp' t'').
    { rewrite /uv_mmd /uv_mm (write_bytes_union_r (ptree_bytes 2 t'') md pa (Z.to_N kk)
    ...
    assert (Hdomimg : (dom (upa_map pt (uM_data pt Mp')) : gset Arch.pa) = dom md)
      by (unfold md; exact (upa_map_dom_eq pt _ _ Hddom)).
    assert (Htokn : uv_tree_ok pt (upa_map pt Mp') t'').
    { apply (uv_tree_ok_dom_eq pt Mp Mp' t'' (upa_map_dom_eq pt Mp' Mp HMdom)).
      exact (uv_tree_ok_of_data pt Mp t' t'' Htok' Htok'' Hshape). }
    (* ---- the domain of the data half does not move ---- *)
    assert (Hdomall : (dom (uv_mmd pt Mp' t'') : gset Arch.pa)
                      = dom (uv_mmd pt Mp t')).
    { rewrite /uv_mmd (uv_mm_dom_img t'' (upa_map pt (uM_data pt Mp')) md Hdomimg).
    ...
    iApply (uv_swp_exec_mem (uc_dqc C) pt Mp Mp' t' t'' rsx rsw i o ib _ Hred Hg1
              Hinj Hinj' Htext' Htok' Htokn Hdomall Hexg Hex
  ...
  Lemma uk_store_fault_post_fetch (R : iProp Σ) (Rut : uptd -> iProp Σ) (sz : Z)
    ...
    uk_pt_pure pt sz M Mp ->
    gen_cert -∗ uv_amb -∗
    (R -∗ (TsoCtx.own_context XI -∗ Rut pt) ∗ Rfd fdv ∗ ukb C pt Rfd Rut sz π fdv cw ∗ uslot (uvis_of_run m pc M π sz fdv cw)) -∗
    resv_any cpu_id -∗
    TsoCtx.own_context XI -∗
    uv_bytes pt Mp t' -∗
    uv_res pt Mp t' usatp pcfg paddr -∗
...
Section UkStoreObl.
  ...
  Lemma uk_store_obl_base (R : iProp Σ) (Rut : uptd -> iProp Σ) (sz : Z)
    ...
    (R -∗ (TsoCtx.own_context XI -∗ Rut pt) ∗ Rfd fdv ∗ ukb C pt Rfd Rut sz π fdv cw ∗
          ((uvb C pt Rfd Rut sz π fdv cw (uM_store M (uint va) kk wval) m (add_vec_int pc 4) -∗
            WP (Loop : expr riscv_lang))
  ...
  Lemma uk_store_obl_rvc (R : iProp Σ) (Rut : uptd -> iProp Σ) (sz : Z)
    ...
    (R -∗ (TsoCtx.own_context XI -∗ Rut pt) ∗ Rfd fdv ∗ ukb C pt Rfd Rut sz π fdv cw ∗
          ((uvb C pt Rfd Rut sz π fdv cw (uM_store M (uint va) kk wval) m (add_vec_int pc 2) -∗
            WP (Loop : expr riscv_lang))
           ∧ uslot (uvis_of_run m pc M π sz fdv cw))) -∗
    resv_any cpu_id -∗
    hreg_frame rsA u_Drw -∗ hreg_frame_ro (u_Df (uc_dqc C)) rsA u_Dro -∗
...
Section UkStore.
  ...
  Lemma wp_uk_store_later (M : gmap Z (bv 8)) (m : regfile)
    ...
    ▷ ukc π (uM_store M (uint va) k wval) sz fdv cw m (add_vec_int pc (if is_rvc then 2 else 4)) -∗
    WP (Loop : expr riscv_lang).
  Proof.
    ...
    iAssert (R -∗ (TsoCtx.own_context (CID := CIDo) XIo -∗ Rut' pt') ∗ Rfd' fdv ∗ ukb C' pt' Rfd' Rut' sz π fdv cw ∗
             ((uvb (CID := CIDo) C' pt' Rfd' Rut' sz π fdv cw (uM_store M (uint va) k wval) m
    ...
    - iDestruct "Hf" as (h) "[[%HisRVC %Hdecrvc] Hbridge]".
      iApply (uk_store_obl_rvc C' pt' Rfd' R Rut' sz π M Mp' m pc h i o k imm rs1 rs2 va wval
                t usatp pcfg paddr rs1s rsA fdv cw Hpre Hpure Hdecrvc Hkw Hred Hg1 Hexp
    ...
    - iDestruct "Hf" as (w) "[[%HnRVC %Hdecbase] Hbridge]".
      iApply (uk_store_obl_base C' pt' Rfd' R Rut' sz π M Mp' m pc w i o k imm rs1 rs2 va wval
                t usatp pcfg paddr rs1s rsA fdv cw Hpre Hpure Hdecbase Hkw Hred Hg1 Hexp
                Hva Hwval Hdisp Hcanon Hpg Hal
                with "Hcert Hamb Hbridge Hk Hany Hrw Hro Hctx Hmm Hres").
  Qed.
  ...
  Proof.
    intros Hkw Hui Hred Hg1 Hlpad Hexp Hva Hwval Hsok Hcanon Hpg Hal HMb.
    iIntros "Hb Hcont".
    iApply (wp_uk_store_later M m pc fdv cw is_rvc i o imm rs1 rs2 k va wval
  ...
  Lemma wp_uk_sd (M : gmap Z (bv 8)) (m : regfile)
      (pc : mword 64) (fdv : list fdstate) (cw : Z) (imm : mword 12) (rs1 rs2 : mword 5)
      (va wval : mword 64) :
    ...
    uvb C pt Rfd Rut sz π fdv cw M m pc -∗
    ukc π (uM_store8 M (uint va) wval) sz fdv cw m (add_vec_int pc 4) -∗
    WP (Loop : expr riscv_lang).
  Proof.
    intros Hui Hva Hwval Hsok Hcanon Hpg Hal HMb.
    iIntros "Hb Hcont".
    iApply (wp_uk_store M m pc fdv cw false
  ...
  Lemma wp_uk_sw (M : gmap Z (bv 8)) (m : regfile)
    ...
    uvb C pt Rfd Rut sz π fdv cw M m pc -∗
    ukc π (uM_store M (uint va) 4 wval) sz fdv cw m (add_vec_int pc 4) -∗
    WP (Loop : expr riscv_lang).
  Proof.
    intros Hui Hva Hwval Hsok Hcanon Hpg Hal HMb.
    iIntros "Hb Hcont".
  ...
  Lemma wp_uk_sb (M : gmap Z (bv 8)) (m : regfile)
    ...
    M !! (uint va) = Some bb ->
    uvb C pt Rfd Rut sz π fdv cw M m pc -∗
    ukc π (uM_store M (uint va) 1 wval) sz fdv cw m (add_vec_int pc 4) -∗
    WP (Loop : expr riscv_lang).
  Proof.
  ...
  Lemma wp_uk_csdsp (M : gmap Z (bv 8)) (m : regfile)
      (pc : mword 64) (fdv : list fdstate) (cw : Z) (uimm : mword 6) (rs2 : mword 5)
      (tgt wval : mword 64) :
    ...
    uvb C pt Rfd Rut sz π fdv cw M m pc -∗
    ukc π (uM_store8 M (uint tgt) wval) sz fdv cw m (add_vec_int pc 2) -∗
    WP (Loop : expr riscv_lang).
  Proof.
    intros Hui Htgt Hwval Hsok Hcanon Hpg Hal HMb.
    iIntros "Hb Hcont".
    iApply (wp_uk_store M m pc fdv cw true (C_SDSP (uimm, Regidx rs2))
  ...
  Lemma wp_uk_csd (M : gmap Z (bv 8)) (m : regfile)
      (pc : mword 64) (fdv : list fdstate) (cw : Z) (uimm : mword 5) (cr1 cr2 : mword 3) (rs1 rs2 : mword 5)
      (va wval : mword 64) :
    ...
    (forall j : nat, (j < 8)%nat -> exists bb : bv 8, M !! (uint va + Z.of_nat j) = Some bb) ->
    uvb C pt Rfd Rut sz π fdv cw M m pc -∗
    ukc π (uM_store8 M (uint va) wval) sz fdv cw m (add_vec_int pc 2) -∗
    WP (Loop : expr riscv_lang).
  Proof.
    intros Hui Hcr1 Hcr2 Hva Hwval Hsok Hcanon Hpg Hal HMb.
    iIntros "Hb Hcont".
    iApply (wp_uk_store M m pc fdv cw true (C_SD (uimm, Cregidx cr1, Cregidx cr2))
              (Some (STORE (zero_extend' 12 (concat_vec uimm ('b"000")),
  ...
  Lemma wp_uk_csw (M : gmap Z (bv 8)) (m : regfile)
    ...
    (forall j : nat, (j < 4)%nat -> exists bb : bv 8, M !! (uint va + Z.of_nat j) = Some bb) ->
    uvb C pt Rfd Rut sz π fdv cw M m pc -∗
    ukc π (uM_store M (uint va) 4 wval) sz fdv cw m (add_vec_int pc 2) -∗
    WP (Loop : expr riscv_lang).
  Proof.
    intros Hui Hcr1 Hcr2 Hva Hwval Hsok Hcanon Hpg Hal HMb.
    iIntros "Hb Hcont".
    iApply (wp_uk_store M m pc fdv cw true (C_SW (uimm, Cregidx cr1, Cregidx cr2))
