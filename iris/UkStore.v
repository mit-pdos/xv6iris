...
Section UkStorePostFetch.
  ...
  Lemma uk_store_post_fetch (R : iProp Σ) (Rut : uptd -> iProp Σ) (sz : Z)
    ...
    (R -∗ (TsoCtx.own_context XI -∗ Rut pt) ∗ Rfd fdv ∗ ukb C pt Rfd Rut sz π fdv cw gn cs pidv false secc_all ∗
          (uvb C pt Rfd Rut sz π fdv cw gn cs pidv false secc_all (uM_store M (uint va) kk wval) m (add_vec_int pc dpc) -∗
  ...
  Lemma uk_store_fault_post_fetch (R : iProp Σ) (Rut : uptd -> iProp Σ) (sz : Z)
    ...
    (⊢ (Kcx ∗ ChildTok.my_pay gn Qp -∗
        (app_taint ∨ (ChildTok.kill_owed gn ∗
            sbundle_at uslot USYS_exit fx
              (uvis_of_run m pc M π sz fdv cw gn cs pidv false secc_all))) : iProp Σ)) ->
    Z.rem (uint va) 4096 <= 4096 - kk ->
    uva_inj pt Mp ->
    match o with
    | Some _ => forall (s : mstate) (mb : PtBytes.pamap),
    ...
    (R -∗ (TsoCtx.own_context XI -∗ Rut pt) ∗ Rfd fdv ∗ ukb C pt Rfd Rut sz π fdv cw gn cs pidv false secc_all ∗
          UkStep.uk_paycont Qp gn
            (Kcx ∧ uslot (uvis_of_run m pc M π sz fdv cw gn cs pidv false secc_all))) -∗
    resv_any cpu_id -∗
    TsoCtx.own_context XI -∗
    uv_bytes pt Mp t' -∗
    uv_res pt Mp t' usatp pcfg paddr -∗
  ...
  Proof using .
    ...
    iExists fx.
    iSplitR;
      [ iApply (uexec_pay_dep_ne _ (uvis_of_run m pc M π sz fdv cw gn cs pidv false secc_all) _ fx
...
Section UkStoreObl.
  ...
  Lemma uk_store_obl_base (R : iProp Σ) (Rut : uptd -> iProp Σ) (sz : Z)
    ...
    (R -∗ (TsoCtx.own_context XI -∗ Rut pt) ∗ Rfd fdv ∗ ukb C pt Rfd Rut sz π fdv cw gn cs pidv false secc_all ∗
          ((⌜uk_store_retires pt Mp va kk⌝ -∗
            uvb C pt Rfd Rut sz π fdv cw gn cs pidv false secc_all (uM_store M (uint va) kk wval) m (add_vec_int pc 4) -∗
            mWP (Loop : expr riscv_lang))
           ∧ UkStep.uk_paycont Qp gn
               (Kcx ∧ uslot (uvis_of_run m pc M π sz fdv cw gn cs pidv false secc_all)))) -∗
    resv_any cpu_id -∗
  ...
  Lemma uk_store_obl_rvc (R : iProp Σ) (Rut : uptd -> iProp Σ) (sz : Z)
    ...
    (R -∗ (TsoCtx.own_context XI -∗ Rut pt) ∗ Rfd fdv ∗ ukb C pt Rfd Rut sz π fdv cw gn cs pidv false secc_all ∗
          ((⌜uk_store_retires pt Mp va kk⌝ -∗
            uvb C pt Rfd Rut sz π fdv cw gn cs pidv false secc_all (uM_store M (uint va) kk wval) m (add_vec_int pc 2) -∗
            mWP (Loop : expr riscv_lang))
           ∧ UkStep.uk_paycont Qp gn
               (Kcx ∧ uslot (uvis_of_run m pc M π sz fdv cw gn cs pidv false secc_all)))) -∗
    resv_any cpu_id -∗
...
Section UkStore.
  ...
  Proof using HRut Hlf0 Hlo Hpm.
                      ...
                      (app_taint ∨
                       (ChildTok.kill_owed gn ∗
                        sbundle_at uslot USYS_exit (sfam_at Qp sfam_pt)
                          (uvis_of_run m pc M π sz fdv cw gn cs pidv false secc_all)))
                      : iProp Σ)).
    { intros Hfl. exfalso.
      destruct (lazy_free_wmapped pt' sz (svpn_of va) q Hwf' Hlf'
    ...
    iAssert (R -∗ (TsoCtx.own_context (CID := CIDo) XIo -∗ Rut' pt') ∗ Rfd' fdv ∗ ukb C' pt' Rfd' Rut' sz π fdv cw gn cs pidv false secc_all ∗
             ((⌜uk_store_retires pt' Mp' va k⌝ -∗
               uvb (CID := CIDo) C' pt' Rfd' Rut' sz π fdv cw gn cs pidv false secc_all (uM_store M (uint va) k wval) m
                 (add_vec_int pc (if is_rvc then 2 else 4)) -∗
               mWP (Loop : expr riscv_lang))
              ∧ UkStep.uk_paycont Qp gn
                  (True ∧ uslot (uvis_of_run m pc M π sz fdv cw gn cs pidv false secc_all))))%I with "[Hk]" as "Hk".
    { iIntros "HR". iDestruct ("Hk" with "HR") as "(Hrut & Hfdr & Hkb & Hkc)".
  ...
  Proof using HRut Hlf0 Hlo Hpm.
    ...
    iAssert (▷ (ChildTok.my_pay gn Qp ∗
                (Qp (-1) ∗ sbundle_at uslot USYS_exit fx
                             (uvis_of_run m pc M π sz fdv cw gn cs pidv false secc_all))))%I
    ...
    iAssert (R -∗ (TsoCtx.own_context (CID := CIDo) XIo -∗ Rut' pt') ∗ Rfd' fdv ∗ ukb C' pt' Rfd' Rut' sz π fdv cw gn cs pidv false secc_all ∗
             ((⌜uk_store_retires pt' Mp' va k⌝ -∗
              ...
              ∧ UkStep.uk_paycont Qp gn
                  ((Qp (-1) ∗ sbundle_at uslot USYS_exit fx
                                (uvis_of_run m pc M π sz fdv cw gn cs pidv false secc_all))
                   ∧ uslot (uvis_of_run m pc M π sz fdv cw gn cs pidv false secc_all))))%I with "[Hk]" as "Hk".
    { iIntros "HR". iDestruct ("Hk" with "HR") as "(Hrut & Hfdr & Hkb & Hkc)".
      iFrame "Hrut Hfdr Hkb". iSplit.
  ...
  Lemma wp_uk_csdsp (M : gmap Z (bv 8)) (m : regfile)
      (pc : mword 64) (fdv : list fdstate) (cw : Z) (gn : gname) (cs : gset gname) (pidv : mword 32) (uimm : mword 6) (rs2 : mword 5)
      (tgt wval : mword 64) :
  ...
  Lemma wp_uk_csd (M : gmap Z (bv 8)) (m : regfile)
      (pc : mword 64) (fdv : list fdstate) (cw : Z) (gn : gname) (cs : gset gname) (pidv : mword 32) (uimm : mword 5) (cr1 cr2 : mword 3) (rs1 rs2 : mword 5)
      (va wval : mword 64) :
