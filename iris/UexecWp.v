...
(* because its proof uses no linear hypothesis, which is what lets a        *)
(* generic slot be MINTED wherever a [UEXEC_GEN] is in scope.               *)
(* THE KERNEL MINTS NOWHERE.  Every slot the kernel handles arrives from    *)
(* outside it: userinit parks the first process with the exec bundle the    *)
(* system theorem's [Hinit_boot] hands down, whose slot piece answers at    *)
(* the key forkret's kexec("/init") builds, and kfork's child gets a copy   *)
(* of the parent's ([ParkCap.park_token_park_steady]).  So the one reader   *)
(* left is the GENERIC APPLICATION's own discharge of that hypothesis       *)
(* ([SystemAdequacy.init_boot_of_sup], through [UexecExecMint.uslot_mint]   *)
(* and [LinkUserinit.UG]), which eliminates the [box] once into the slot    *)
(* family the trivial bundle's two wands answer with.  Nothing PERSISTENT   *)
(* carries a WP -- [SyscParkEnv.park_world] used to, which made one         *)
(* duplicable from inside every trap round.  See                            *)
(* claude-notes/design/user-wp-slot.md.                                     *)
(* ===================================================================== *)
(* THE APPLICATION'S SUPPLY IS NOT A FIELD OF THIS MODULE TYPE, and cannot   *)
(* be.  A generic slot's syscall bundles are paid out of                     *)
...
(* stays where it is payable -- inside the application's own discharge of     *)
(* [Hinit_boot] -- and [UEXEC_GEN] stays what it was: the program-generic     *)
(* WP and nothing about the file system.                                      *)
