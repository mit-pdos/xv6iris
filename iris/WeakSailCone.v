...
Definition pf_xsolo (next : bool → M unit) (i : agent)
  ...
  ∃ l : wlabel,
    wp_pf_step (pstep_unit (pstep_ni_xv6 next)) lbl_class_p i l c c' ∧
    xcls_canon i l c c' ∧ xrmw_tight i l c.

Definition pf_xquiet (next : bool → M unit) (i : agent)
    (c c' : wpcfg pxv6 unit) : Prop :=
  ∃ l : wlabel, lbl_quiet l ∧ wp_pf_step (pstep_unit (pstep_ni_xv6 next)) lbl_class_p i l c c'.
...
Lemma pf_xquiet_nolog next i l (c c' : wpcfg pxv6 unit) :
  lbl_quiet l → wp_pf_step (pstep_unit (pstep_ni_xv6 next)) lbl_class_p i l c c' →
  pc_log c' = pc_log c.
Proof.
  intros Hq Hstep. destruct Hstep as
    [cfg ag st' Hlk Hps
    |cfg ag aq lat base tvs asrc st' Hlk Hps Hr
    |cfg ag rl base data asrc vsrc kk st' Hlk Hps Hne Hkc
...
Lemma pf_xsolo_run next i c c' :
  pf_xsolo next i c c' → wp_pf_run (pstep_unit (pstep_xv6 next)) lbl_class_p c c'.
Proof.
  ...
  destruct Hstep as
    ...
    | by eapply PFRmw; [done|apply pstep_ni_xv6_step|done|done|done|done|done]
...
Lemma pf_xsolo_rtc_run next i c c' :
  rtc (pf_xsolo next i) c c' → rtc (wp_pf_run (pstep_unit (pstep_xv6 next)) lbl_class_p) c c'.
Proof.
...
Section lift.
  ...
  Lemma pf_step_lift (i : agent) (l : wlabel) (c : wpcfg pxv6 unit)
    ...
    wp_pf_step (pstep_unit (sail_step_ni next)) lbl_class_p i l cs cs' →
    cls_canon i l cs cs' → rmw_tight i l cs → hlink i c cs →
    ∃ c', wp_pf_step (pstep_unit (pstep_ni_xv6 next)) lbl_class_p i l c c' ∧
  ...
  Proof.
    ...
    - (* silent *)
      exists (WPCfgU (pc_img c) (pc_log c)
               (<[i := WPAgent (PHart st') (pa_ws agx) (pa_prom agx)]>
                  (pc_ags c))).
      split_and!.
      + apply (PFSilent (pstep_unit (pstep_ni_xv6 next)) lbl_class_p i c agx (PHart st') tt Hax).
    ...
    - (* load *)
      have Hr' : read_ok_d (pc_img c) (pc_log c) (pa_ws agx) aq lat base tvs
                   (srcs_view (pa_ws agx) asrc)
        by rewrite Himg Hlog Hws.
      exists (WPCfgU (pc_img c) (pc_log c)
               (<[i := WPAgent (PHart st')
                         (load_post_run_d (pa_ws agx) aq
                            (srcs_view (pa_ws agx) asrc) base tvs.*1)
                         (pa_prom agx)]> (pc_ags c))).
      split_and!.
      + apply (PFLoad (pstep_unit (pstep_ni_xv6 next)) lbl_class_p i c agx aq
                 lat base tvs asrc (PHart st') tt
      ...
      + intros j Hj. simpl. by apply lookup_insert_other.
    - (* store *)
      have Hkk : kk = lbl_class (WeakPromise.LStore rl base data asrc vsrc) (pa_ws ag)
      ...
      exists (WPCfgU (pc_img c) (pc_log c ++ [WMsg base data (Some i) kk])
               (<[i := WPAgent (PHart st')
                  ...
                  (pc_ags c))).
      split_and!.
      + apply (PFStore (pstep_unit (pstep_ni_xv6 next)) lbl_class_p i c agx rl
                 base data asrc vsrc kk (PHart st') tt Hax);
          [rewrite Hst /=; exact Hps|exact Hne|by rewrite Hws].
      ...
      exists (WPCfgU (pc_img c) (pc_log c ++ [WMsg base data (Some i) kk])
               (<[i := WPAgent (PHart st')
                         (store_post_run_d
                            (load_post_run_d (pa_ws agx) aq
                               (srcs_view (pa_ws agx) asrc) base tvs.*1)
                            rl (srcs_view (pa_ws agx) asrc)
                               (srcs_view (pa_ws agx) vsrc)
                            base (length data) (S (length (pc_log c))))
                         (pa_prom agx)]> (pc_ags c))).
      split_and!.
      + apply (PFRmw (pstep_unit (pstep_ni_xv6 next)) lbl_class_p i c agx aq rl
                 base tvs data asrc vsrc kk (PHart st') tt Hax);
    ...
    - (* fence *)
      exists (WPCfgU (pc_img c) (pc_log c)
               (<[i := WPAgent (PHart st')
                         ...
                         (pa_prom agx)]> (pc_ags c))).
      split_and!.
      + apply (PFFence (pstep_unit (pstep_ni_xv6 next)) lbl_class_p i c agx pr pw sr sw (PHart st') tt
    ...
    - (* dev: the silent case verbatim *)
      exists (WPCfgU (pc_img c) (pc_log c)
               (<[i := WPAgent (PHart st') (pa_ws agx) (pa_prom agx)]>
                  (pc_ags c))).
      split_and!.
      + apply (PFDev (pstep_unit (pstep_ni_xv6 next)) lbl_class_p i c agx (PHart st') tt Hax).
        rewrite Hst /=. exact Hps.
      + by apply xcls_canon_nolog.
      + exact I.
      + split_and!; [exact Himg|exact Hlog|].
    ...
    - (* regw *)
      exists (WPCfgU (pc_img c) (pc_log c)
               (<[i := WPAgent (PHart st') (regw_post (pa_ws agx) rdw (srcs_view (pa_ws agx) wsrc)) (pa_prom agx)]>
                  (pc_ags c))).
      split_and!.
      + apply (PFRegW (pstep_unit (pstep_ni_xv6 next)) lbl_class_p i c agx
                 rdw wsrc (PHart st') tt Hax).
        rewrite Hst /=. exact Hps.
      + by apply xcls_canon_nolog.
      + exact I.
      + split_and!; [exact Himg|exact Hlog|].
    ...
    - (* ctrl *)
      exists (WPCfgU (pc_img c) (pc_log c)
               (<[i := WPAgent (PHart st') (ctrl_post (pa_ws agx) (srcs_view (pa_ws agx) csrc)) (pa_prom agx)]>
                  (pc_ags c))).
      split_and!.
      + apply (PFCtrl (pstep_unit (pstep_ni_xv6 next)) lbl_class_p i c agx
                 csrc (PHart st') tt Hax).
        rewrite Hst /=. exact Hps.
      + by apply xcls_canon_nolog.
      + exact I.
      + split_and!; [exact Himg|exact Hlog|].
    ...
    - (* instr *)
      exists (WPCfgU (pc_img c) (pc_log c)
               (<[i := WPAgent (PHart st') (instr_post (pa_ws agx)) (pa_prom agx)]>
                  (pc_ags c))).
      split_and!.
      + apply (PFInstr (pstep_unit (pstep_ni_xv6 next)) lbl_class_p i c agx
                 (PHart st') tt Hax).
        rewrite Hst /=. exact Hps.
      + by apply xcls_canon_nolog.
      + exact I.
...
Proof.
  intros (l & Hstep & _ & _).
  destruct (wp_pf_step_shape (pstep_unit (pstep_ni_xv6 next)) lbl_class_p i l c c' Hstep)
    as (Himg & _ & (ms & Hlog & Hall)).
  split_and!; [by exists ms|done| |].
  - intros j Hj.
    exact (wp_pf_step_frame (pstep_unit (pstep_ni_xv6 next)) lbl_class_p i l c c' j Hstep Hj).
  - intros j a.
    exact (wp_pf_step_obs_flr (pstep_unit (pstep_ni_xv6 next)) lbl_class_p i l c c' j a Hstep).
Qed.
...
Proof.
  ...
  intros Hb. apply IH.
  exact (wp_pf_step_bnd (pstep_unit (pstep_ni_xv6 next)) lbl_class_p i l x y Hxy Hb).
Qed.
...
Section cone.
  ...
  Context (nh : nat).
  Context (Hsplit : edges_split_cyc nh TS (PDevs tt [])).
  ...
  Lemma cut_pquiet (order : list gev) (b2 : gev) (cf : wpcfg pxv6 unit) j ag :
    (∀ e, e ∈ order ↔ (gev_wf TS e ∧ (e = b2 ∨ tc (gdep2 TS) e b2))) →
    NoDup order →
    qcfg (pstep_unit (pstep_xv6 riscv_step)) lbl_class_p TS (PDevs tt []) img tt ps order cf →
...
Lemma chained_run (next : bool → M unit) segs c cf :
  ...
  rtc (wp_pf_run (pstep_unit (pstep_xv6 next)) lbl_class_p) c cf.
Proof.
...
Lemma pf_run_bnd {P D : Type} (pstep : P → D → wlabel → P → D → Prop)
    (pcls : P → wlabel → wstate → wm_class) (c c' : wpcfg P D) :
  rtc (wp_pf_run pstep pcls) c c' → cfg_bnd c → cfg_bnd c'.
Proof.
  induction 1 as [|x y z (i & l & Hxy) _ IH]; [by intros Hb|].
  intros Hb. apply IH. exact (wp_pf_step_bnd pstep pcls i l x y Hxy Hb).
Qed.

Lemma pf_run_ags_len {P D : Type} (pstep : P → D → wlabel → P → D → Prop)
    (pcls : P → wlabel → wstate → wm_class)
    (c c' : wpcfg P D) :
  rtc (wp_pf_run pstep pcls) c c' → length (pc_ags c') = length (pc_ags c).
Proof.
  induction 1 as [|x y z (i & l & Hxy) _ IH]; [done|].
  by rewrite IH (wp_pf_step_ags_len pstep pcls i l x y Hxy).
Qed.
...
Proof.
  (* D2: this is [WeakRobustProv.w_relp_aev_post_indep] verbatim — the local
     copy predates it and is kept only for the name. *)
  apply w_relp_aev_post_indep.
Qed.
...
Lemma xrmw_tight_own_coh (pstep : pxv6 → unit → wlabel → pxv6 → unit → Prop)
    (pcls : pxv6 → wlabel → wstate → wm_class)
    (i : agent) (l : wlabel) (c c' : wpcfg pxv6 unit) :
  own_coh c → wp_pf_step pstep pcls i l c c' → xrmw_tight i l c.
Proof.
  intros Hoc Hstep.
  destruct Hstep as
    [cfg ag0 st' dd Hlk0 Hps
    |cfg ag0 aq0 lat0 base0 tvs0 asrc0 st' dd Hlk0 Hps Hr
    |cfg ag0 rl0 base0 data0 asrc0 vsrc0 kk st' dd Hlk0 Hps Hne
    |cfg ag0 aq0 rl0 base0 tvs0 data0 asrc0 vsrc0 kk st' dd
  ...
  split_and!;
    [exact H1
    |eapply readable_anti_vpre; [apply load_vpre_load_vpre_d|exact H2]
    |intros _].
...
Lemma pf_xirq_pf_step (next : bool → M unit) i c c' :
  ...
  wp_pf_step (pstep_unit (pstep_xv6 next)) lbl_class_p i WeakPromise.LSilent c c'.
Proof.
  intros Hf (ag & q & q' & Hlk & Hst & Hdel & ->).
  apply (PFSilent (pstep_unit (pstep_xv6 next)) lbl_class_p i c ag (PHart q') tt Hlk).
...
Lemma pf_xirq_run (next : bool → M unit) i c c' :
  xfence_free i c → pf_xirq i c c' → wp_pf_run (pstep_unit (pstep_xv6 next)) lbl_class_p c c'.
...
Lemma xstep_run i c c' : xstep i c c' → wp_pf_run (pstep_unit (pstep_xv6 riscv_step)) lbl_class_p c c'.
Proof.
...
Lemma xstep_rtc_run i c c' :
  rtc (xstep i) c c' → rtc (wp_pf_run (pstep_unit (pstep_xv6 riscv_step)) lbl_class_p) c c'.
Proof.
  induction 1 as [|x y z Hxy _ IH]; [apply rtc_refl|].
...
Lemma wp_pf_step_agent {P D : Type} (pstep : P → D → wlabel → P → D → Prop)
    (pcls : P → wlabel → wstate → wm_class)
    i l (c c' : wpcfg P D) :
  wp_pf_step pstep pcls i l c c' → ∃ ag, pc_ags c !! i = Some ag.
Proof.
...
Proof.
  ...
  destruct (wp_pf_step_agent (pstep_unit (pstep_ni_xv6 riscv_step)) lbl_class_p i l c c' Hstep)
    as (ag & Hlk).
  destruct (xcfg_eqv_lookup c d i ag Heq Hlk)
    as (agd & Hlkd & Hws & Hprom & Hst).
  destruct (wp_pf_step_inv (pstep_unit (pstep_ni_xv6 riscv_step)) lbl_class_p i l c c' ag Hstep Hlk)
  ...
  split.
  - exists l. split_and!.
    + refine (Hre d agd stD Hlkd Hws Hprom Himgd Hlogd HpsD _).
...
Proof.
  ...
  destruct (wp_pf_step_inv (pstep_unit (pstep_ni_xv6 riscv_step)) lbl_class_p i l _ c2 ag1 Hstep Hlk1)
...
Lemma pf_step_ni_or_irq next i l (c c' : wpcfg pxv6 unit) :
  wp_pf_step (pstep_unit (pstep_xv6 next)) lbl_class_p i l c c' →
  wp_pf_step (pstep_unit (pstep_ni_xv6 next)) lbl_class_p i l c c' ∨
...
Lemma seg2_run (s : seg2) :
  seg2_ok s → rtc (wp_pf_run (pstep_unit (pstep_xv6 riscv_step)) lbl_class_p) (seg2_src s) (seg2_dst s).
Proof.
...
Lemma chained2_run segs c cf :
  ...
  rtc (wp_pf_run (pstep_unit (pstep_xv6 riscv_step)) lbl_class_p) c cf.
Proof.
  revert c. induction segs as [|s segs IH]; intros c Hch Hall; simpl in Hch.
  { by rewrite Hch. }
  destruct Hch as (Hs & Hch).
...
Lemma w_relp_ag_store_d (st' : pxv6) ws rl va vd base n t pr :
  (0 < n)%nat →
  w_relp (pa_ws (WPAgent st' (store_post_run_d ws rl va vd base n t) pr))
  = false.
Proof. apply store_post_run_d_relp. Qed.
...
Proof.
  ...
  intros [(-> & _)|(m & r & _ & _ & _ & ->)]; [by left|].
  right. by eexists _, _, _.
Qed.

Lemma disk_relp_step next i l (c c' : wpcfg pxv6 unit) :
  disk_relp c → wp_pf_step (pstep_unit (pstep_xv6 next)) lbl_class_p i l c c' → disk_relp c'.
Proof.
  intros Hdr Hstep.
  destruct Hstep as
  ...
  - (* store *)
    apply w_relp_ag_store_d. destruct data; [done|]. simpl. lia.
Qed.

Lemma disk_relp_run next (c c' : wpcfg pxv6 unit) :
  disk_relp c → rtc (wp_pf_run (pstep_unit (pstep_xv6 next)) lbl_class_p) c c' → disk_relp c'.
Proof.
...
Proof.
  ...
  destruct Hstep as
    ...
    |cfg ag0 aq lat base tvs asrc st' dv Hlk0 Hps Hr
    |cfg ag0 rl base data asrc vsrc kk st' dv Hlk0 Hps Hne Hkc
...
Section package.
  ...
  (** (B5a-2) every logged message carries the class its own author's
      pre-state computes ([WeakSailLTS2.lbl_class]).  This IS
      [WeakRobustTrace.cls_canonical lbl_class_p TS], and since G6a it is
      needed EARLIER than stage B5a needed it: the promise-free fragment
      pins the class it appends, so [WeakRobustCone.bad_edge_violates_blocks]
      — the cone replay [cone_segments] forwards — takes it as a premise. *)
  Context (Hcls : cls_canonical lbl_class_p TS).
  ...
  Theorem cone_segments (b1 b2 : gev) :
    ...
    ∃ (order : list gev) (cfs : list (wpcfg pxv6 unit)) (cmid : wpcfg pxv6 unit)
      ...
      (∀ i e c c', order !! i = Some e → cfs !! i = Some c →
         cfs !! S i = Some c' → cstep (pstep_unit (pstep_xv6 riscv_step)) lbl_class_p e.1 c c') ∧
      rtc (wp_pf_run (pstep_unit (pstep_xv6 riscv_step)) lbl_class_p) (wp_init img tt ps) cmid ∧
      ...
      rtc (wp_pf_run (pstep_unit (pstep_xv6 riscv_step)) lbl_class_p) cmid cf ∧
  ...
  Proof.
    ...
    destruct (bad_edge_violates_blocks (pstep_unit (pstep_xv6 riscv_step))
                lbl_class_p (λ _ _ _, false) TS img tt ps
                Hwf Hwsi Hco Hwfl (xv6_lat_free riscv_step)
                (xv6_ts_oblivious riscv_step) Hcls lbl_class_obl
                Himg Hnag Hdata Hdf Hps0 Hfo Hee
    ...
    have Hqcm : qcfg (pstep_unit (pstep_xv6 riscv_step)) lbl_class_p TS (PDevs tt []) img tt ps order cmid.
    ...
    have Hbndm : cfg_bnd cmid
      by apply (pf_run_bnd _ _ _ _ Hrunm), cfg_bnd_init.
    (* ---- the reader's record ---- *)
    ...
    (* ---- the assembled runs ---- *)
    have Hrun1r : rtc (wp_pf_run (pstep_unit (pstep_xv6 riscv_step)) lbl_class_p) cmid c1
      := pf_xsolo_rtc_run riscv_step b2.1 cmid c1 Hrun1.
    have Hrun2r : rtc (wp_pf_run (pstep_unit (pstep_xv6 riscv_step)) lbl_class_p) c1 cf
      := chained_run riscv_step segs c1 cf Hch Hallseg.
    have Hrunmf : rtc (wp_pf_run (pstep_unit (pstep_xv6 riscv_step)) lbl_class_p) cmid cf
      by etrans; [exact Hrun1r|exact Hrun2r].
    have Hrunall : rtc (wp_pf_run (pstep_unit (pstep_xv6 riscv_step)) lbl_class_p) (wp_init img tt ps) cf
    ...
    { intros j ag Hlkj.
      ...
      - apply (Hbdf j ag); [|exact Hlkj]. apply Hksmem. split; [|exact Hjne].
        ...
        rewrite (pf_run_ags_len _ _ _ _ Hrunall) wp_init_ags_len in Hlkj. lia. }
    ...
    - apply list_relations.Forall_cons. split; [|exact Hallseg].
      ...
      rewrite Hst1. by split.
    - by exists p, mv, av.
    - destruct Hviolf as (Hlogp & Htid & Hclsv & Hpub & Hne' & Hbyte & Hobs).
  ...
  Lemma pf_step_writes (i : agent) l (c c' : wpcfg pxv6 unit) msg :
    wp_pf_step (pstep_unit (pstep_xv6 riscv_step)) lbl_class_p i l c c' →
  ...
  Lemma pf_step_pstep (i : agent) l (c c' : wpcfg pxv6 unit) ag :
    wp_pf_step (pstep_unit (pstep_xv6 riscv_step)) lbl_class_p i l c c' → pc_ags c !! i = Some ag →
  ...
  Lemma chain_cls (order : list gev) n (e : gev) (T : atrace pxv6 unit) ev
    ...
    qcfg (pstep_unit (pstep_xv6 riscv_step)) lbl_class_p TS (PDevs tt []) img tt ps (take n order) c →
    qcfg (pstep_unit (pstep_xv6 riscv_step)) lbl_class_p TS (PDevs tt []) img tt ps (take (S n) order) c' →
    pt_trs TS !! e.1 = Some T → at_evs T !! e.2 = Some ev →
    pc_ags c !! e.1 = Some ag →
    wp_pf_step (pstep_unit (pstep_xv6 riscv_step)) lbl_class_p e.1 l c c' →
  ...
  Definition seg_inv (order : list gev) (n : nat) (c : wpcfg pxv6 unit) : Prop :=
    ∃ segs dstart d op,
      ...
      rtc (wp_pf_run (pstep_unit (pstep_xv6 riscv_step)) lbl_class_p) (wp_init img tt ps) d ∧
      (match op with
       | None => d = dstart
  ...
  Lemma seg_build (order : list gev) (cfs : list (wpcfg pxv6 unit)) (b2 : gev) :
    ...
    (∀ n e c c', order !! n = Some e → cfs !! n = Some c →
       cfs !! S n = Some c' → cstep (pstep_unit (pstep_xv6 riscv_step)) lbl_class_p e.1 c c') →
    (∀ n c, cfs !! n = Some c →
       qcfg (pstep_unit (pstep_xv6 riscv_step)) lbl_class_p TS (PDevs tt []) img tt ps (take n order) c) →
  ...
  Proof.
    ...
    destruct (wp_pf_step_transplant (pstep_unit (pstep_xv6 riscv_step)) lbl_class_p e.1 l c0 c d
    ...
    have Hrun2 : rtc (wp_pf_run (pstep_unit (pstep_xv6 riscv_step)) lbl_class_p) (wp_init img tt ps) d2
    ...
    have Hrmwd : xrmw_tight e.1 l d
      by exact (xrmw_tight_own_coh (pstep_unit (pstep_xv6 riscv_step)) lbl_class_p e.1 l d d2 Hocd Hstd).
      ...
      + (* the block continues *)
        ...
        * destruct (decide (unfin order (S n) e.1)) as [Hunf1|Hunf1].
          ...
          -- (* the agent is finished: splice its silent epilogue in HERE *)
             ...
             have Hrunall : rtc (wp_pf_run (pstep_unit (pstep_xv6 riscv_step)) lbl_class_p) d2 d3
             ...
             ++ by rewrite (pf_run_ags_len _ _ _ _ Hrunall).
             ++ intros j Hj. destruct (decide (j = e.1)) as [->|Hjne];
  ...
  Proof.
    ...
    { destruct op as [i|]; last first.
      ...
      { have Hbndd : cfg_bnd d
          by (apply (pf_run_bnd _ _ _ _ Hrun); apply cfg_bnd_init).
        destruct (sp_m qr) as [mr|] eqn:Hm.
        - have Hr := Hres b2.1 T2 (S b2.2) agn2 qr HT2 Hagn2 Hqr.
          rewrite Hm in Hr. destruct Hr as (Hsh & Hlv).
