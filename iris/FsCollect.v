...
(*  [IcacheEscrow.ipool_transit] do.  (E) is CLOSED (C-5) and so is (F)    *)
(*  (C-6).  WHAT REMAINS is not a residue of the collection's arithmetic   *)
(*  but a missing POOL-SIDE WITNESS for the partition's third part [X] --  *)
(*  section 5d below is that wall, machine-checked.                        *)
(*                                                                        *)
...
(*      [col_slot_unfrozen]).                                              *)
(*                                                                        *)
(*      IT DOES NOT FINISH [X], and section 5d is that wall,               *)
(*      machine-checked: a MARKED slot at [FrzOff] is every cached or      *)
(*      pooled inode, and the pool-side witness that would rule the        *)
(*      combination out cannot be minted as things stand.                  *)
(*                                                                        *)
