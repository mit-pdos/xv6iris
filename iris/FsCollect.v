...
(*                                                                        *)
(*  FOUR SUPPLIERS DO NOT EXIST IN THE TREE TODAY.  Two were already       *)
(*  named by lane B''-join; two are this lane's findings and neither is a  *)
(*  proof difficulty -- both are era-side invariants that FORGET a fact    *)
(*  their producer holds.                                                  *)
(*                                                                        *)
(*  (A) THE PARTITION (B''-join's open item).  [ipool_inv]'s index [O]     *)
(*      together with the fifty [ic_escrow_body_ident] identities must     *)
(*      EXHAUST [region_inums nib]; today that is [ic_ci_wf]'s [dom ci =   *)
(*      dom M] plus [ipool]'s domain, both LOCK-HELD.  Fix as recommended: *)
(*      a quarter of [ic_id] parked in [ipool_body] beside the pure row,   *)
(*      [ipool_inv]/[ipool_body]/[ipool] gaining [cn]; movers are          *)
(*      [ProofIget]'s recycle and [ProofIput]'s two evictions.             *)
(*      MEASURED HERE, and it is what makes the increment delicate: the    *)
(*      row is FALSE between [ipool_take] and the escrow deposit, and at   *)
(*      iget's recycle those are two separate ghost steps -- [ipool_take]  *)
(*      runs under [fupd_wp] just before the [+0x72] store while the       *)
(*      deposit into the MID arm happens inside that store's atomic        *)
(*      update ([ProofIget.v], "THE WITHDRAW").  Nothing is UNSOUND about  *)
(*      that today (the leading fupd fuses with the step's own mask        *)
(*      change, so no other hart's ghost step interleaves and the          *)
(*      commit never observes the gap), but the ROW cannot be re-           *)
(*      established at the intermediate closing, so the take has to move   *)
(*      INSIDE the store's atomic update, beside the identity flip.        *)
(*      [ipoolN] is outside that update's mask already.                    *)
(*                                                                        *)
...
(*      superblock's block at FULL fraction, plus its parse.  The era      *)
(*      hands [fsblock (fs_bytes γfs) 1 bs_sb] to [fsinit]                 *)
(*      ([FsCfgBoot.fs_kit_fsinit_ghost]), [SpecFsinit] returns it, and    *)
(*      [ProofForkret] DROPS it (the name "Hb1" occurs three times in that *)
(*      file: bound, passed, returned -- never spent).  So at a commit no  *)
(*      resource says what block 1 holds: [sk_parse] and [sk_meta_used]    *)
(*      at [SB_BNO] have no source.  [FsCollectImg.img_sb_home] shows the  *)
(*      GEOMETRY half is free (block 1 is a home block, so [sk_sb] is      *)
(*      satisfied by taking [fss_sbb S] to be the view's own value); only  *)
(*      the OWNERSHIP is missing.  It must be at fraction 1 and NOT        *)
(*      discarded: [sk_own_used] refutes a node owning block 1 through     *)
(*      [blk_owned_ne_full], and a discarded share does not refute 3/4     *)
(*      ([DfracDiscarded ⋅ DfracOwn (3/4)] is valid).  It has to reach the *)
(*      commit through [LogInv.log_ctx] -- the plan's parked law -- whose  *)
(*      only producer is [initlog], whose only caller is [fsinit], which   *)
(*      already holds block 1's run and every other invariant the law      *)
(*      needs.  MEASURED: [log_ctx] is built at exactly ONE site           *)
(*      ([ProofInitlog.v:2697]) and [wp_initlog] has exactly ONE caller,   *)
(*      so the law costs [log_ctx] no arity and [initlog] one premise.     *)
(*                                                                        *)
...
(*                                                                        *)
(*  NONE of the four is visible from inside this file, which is the point  *)
(*  of stating [col_hand] as a named predicate: the collection is CLOSED   *)
(*  (see [col_snap_ok]), and what remains is exactly four suppliers.       *)
(* ====================================================================== *)
