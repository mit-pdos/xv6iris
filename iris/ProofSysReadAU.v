(* ProofSysReadAU.v -- sys_read's ATOMIC-UPDATE walk: the whole-function WP
   that seals [SpecSysReadAUAt.SYSREAD_AU_AT]'s first Parameter, over
   [SpecFilereadAU]'s AU form of fileread.
   ...
   1. THE argfd FAILURE ARM IS DEAD.  The contract's pure premise
      [arg_fd v (pv_ofile (us_V U)) = Some (fd, fv)] names the slot, and
      [SpecArgfd.argfd_post]'s failure disjunct carries
      [arg_fd v (pv_ofile (us_V U)) = None].  One [congruence] deletes the
      whole -1-without-a-call branch, which is why the armed post has TWO
      arms and not the landed contract's blanket.

   2. THE DESCRIPTOR'S STATE IS PINNED BY THE CALLER'S FRAGMENT.  The B1 seam
      ([ProcInv.proc_priv_lend]) hands out the reference at an EXISTENTIAL
      [stf] together with [FdSlots.fd_st_auth] at that [stf]; the contract's
      own [fd_st] fragment plus [FdSlots.fd_st_agree] pins
      [stf = FdOpen true wb (FdInode i)].  That is the whole fd bridge -- no
      new carrier, no new agreement law -- and it is what lets the AU form of
      fileread be called at all (its domain premise IS that equation).  The
      fragment is threaded through untouched and returned: a read moves the
      offset, never the descriptor.

   3. THE CALLEE IS [SpecFilereadAU.FILEREAD_AU].  Same arguments as the
      landed call plus [wb], [i] and the receipt [Φr]; the caller's commit
      goes down unchanged (this shell fires nothing itself -- the one instant
   ...
   [sr_tail] -- is copied because [ProofSysRead]'s module is SEALED and its
   [Local] helpers are therefore not reachable; the four pure address lemmas
   ARE top-level there and are reused by [Require].

   BINDERS: [ProofSysRead]'s section list VERBATIM.  NO [!icacheG Σ]:
   [fileG] bundles it. *)
...
(* UNSEALED, and deliberately: [SYSREAD_AU_AT] has TWO Parameters and the
   second one is a DERIVATION, not a walk (SpecSysReadAUAt's header).  So this
   file proves the walk and [ProofSysReadAUStable.v] [Include]s it, adds the
   corollary and seals the pair -- the write lane's split, adapted to a module
   type that carries both fields instead of two module types carrying one
   each. *)
Module SysReadAUWalk (Argaddr : ARGADDR) (Argint : ARGINT) (Argfd : ARGFD)
                     (FilereadAU : FILEREAD_AU).

Section ProofSysReadAU.
  ...
  Lemma sr_sp_bounds `{CID0 : CpuId} (mm : regfile) (kk : nat)
      (bb : bool) (pp : mword 64) :
    (0 < kk)%nat ->
  ...
  Proof.
    ...
    iApply (wp_cldsp_s_sconf (mword_of_int (KernelSyms.sys_read + 0x40))
    ...
    assert (Hpp42 : add_vec_int (mword_of_int (KernelSyms.sys_read + 0x40) : mword 64) 2
                    = mword_of_int (KernelSyms.sys_read + 0x42)) by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (wp_cldsp_s_sconf (mword_of_int (KernelSyms.sys_read + 0x42))
    ...
    assert (Hpp44 : add_vec_int (mword_of_int (KernelSyms.sys_read + 0x42) : mword 64) 2
                    = mword_of_int (KernelSyms.sys_read + 0x44)) by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (wp_caddi16sp_pop_s_sconf (mword_of_int (KernelSyms.sys_read + 0x44))
    ...
    assert (Hpp46 : add_vec_int (mword_of_int (KernelSyms.sys_read + 0x44) : mword 64) 2
                    = mword_of_int (KernelSyms.sys_read + 0x46)) by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (wp_cret_s_sconf (mword_of_int (KernelSyms.sys_read + 0x46))
  ...
  Proof.
    ...
    iApply (wp_csdsp_s_sconf (mword_of_int (KernelSyms.sys_read + 0x02))
    ...
    iApply (wp_csdsp_s_sconf (mword_of_int (KernelSyms.sys_read + 0x04))
    ...
    iApply (wp_caddi4spn_s_sconf (mword_of_int (KernelSyms.sys_read + 0x06))
    ...
    assert (Hpp08 : add_vec_int (mword_of_int (KernelSyms.sys_read + 0x06) : mword 64) 2
                    = mword_of_int (KernelSyms.sys_read + 0x08)) by (apply bv_eq; vm_compute; reflexivity).
    ...
    assert (Hpp0c : add_vec_int (mword_of_int (KernelSyms.sys_read + 0x08) : mword 64) 4
                    = mword_of_int (KernelSyms.sys_read + 0x0c)) by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp0c) in "Hpc".
    assert (HM3a1 : M3 !!! Regidx Ra1 = pa_stk sp0 5).
    { rewrite /M3 upd_eq. rgne. rewrite HM2s0. apply sr_addr_p. }
    ...
    iApply (wp_cli_s_sconf (mword_of_int (KernelSyms.sys_read + 0x0c))
    ...
    assert (Hpp0e : add_vec_int (mword_of_int (KernelSyms.sys_read + 0x0c) : mword 64) 2
                    = mword_of_int (KernelSyms.sys_read + 0x0e)) by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp0e) in "Hpc".
    (* ---- +0x0e: jal ra,argaddr ---- *)
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.sys_read + 0x0e))
              Rra (mword_of_int 2087634 : mword 21) M4 (av - 6)%nat b
    ...
    change (<[Regidx Rra := regval_into_reg
              (add_vec_int (mword_of_int (KernelSyms.sys_read + 0x0e) : mword 64) 4)]> M4) with M5.
    assert (Hjaa : add_vec (mword_of_int (KernelSyms.sys_read + 0x0e) : mword 64)
                     (sign_extend' 64 (mword_of_int 2087634 : mword 21)) = mword_of_int KernelSyms.argaddr)
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    assert (HM5ra : M5 !!! Regidx Rra
                    = add_vec_int (mword_of_int (KernelSyms.sys_read + 0x0e) : mword 64) 4)
    ...
    assert (Hpc12 : ret_pc (M5 !!! Regidx Rra)
                    = mword_of_int (KernelSyms.sys_read + 0x12))
    ...
    iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.sys_read + 0x12))
    ...
    assert (Hpp16 : add_vec_int (mword_of_int (KernelSyms.sys_read + 0x12) : mword 64) 4
                    = mword_of_int (KernelSyms.sys_read + 0x16)) by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp16) in "Hpc".
    assert (HB1a1 : B1 !!! Regidx Ra1 = pa_add (pa_stk sp0 4) 4).
    { rewrite /B1 upd_eq. rgne. rewrite HA0s0. apply sr_addr_n. }
    ...
    iApply (wp_cli_s_sconf (mword_of_int (KernelSyms.sys_read + 0x16))
    ...
    assert (Hpp18 : add_vec_int (mword_of_int (KernelSyms.sys_read + 0x16) : mword 64) 2
                    = mword_of_int (KernelSyms.sys_read + 0x18)) by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp18) in "Hpc".
    (* ---- +0x18: jal ra,argint ---- *)
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.sys_read + 0x18))
              Rra (mword_of_int 2087596 : mword 21) B2 (av - 6)%nat b
              ltac:(vm_compute; discriminate) ltac:(rdok) ltac:(vm_compute; reflexivity)
              with "Hcg Hpc []").
    ...
    change (<[Regidx Rra := regval_into_reg
              (add_vec_int (mword_of_int (KernelSyms.sys_read + 0x18) : mword 64) 4)]> B2) with B3.
    assert (Hjai : add_vec (mword_of_int (KernelSyms.sys_read + 0x18) : mword 64)
                     (sign_extend' 64 (mword_of_int 2087596 : mword 21)) = mword_of_int KernelSyms.argint)
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    assert (HB3ra : B3 !!! Regidx Rra
                    = add_vec_int (mword_of_int (KernelSyms.sys_read + 0x18) : mword 64) 4)
    ...
    assert (Hpc1c : ret_pc (B3 !!! Regidx Rra)
                    = mword_of_int (KernelSyms.sys_read + 0x1c))
    ...
    iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.sys_read + 0x1c))
    ...
    assert (Hpp20 : add_vec_int (mword_of_int (KernelSyms.sys_read + 0x1c) : mword 64) 4
                    = mword_of_int (KernelSyms.sys_read + 0x20)) by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp20) in "Hpc".
    assert (HN1a2 : N1 !!! Regidx Ra2 = pa_stk sp0 3).
    { rewrite /N1 upd_eq. rgne. rewrite HA1s0. apply sr_addr_f. }
    ...
    iApply (wp_cli_s_sconf (mword_of_int (KernelSyms.sys_read + 0x20))
    ...
    assert (Hpp22 : add_vec_int (mword_of_int (KernelSyms.sys_read + 0x20) : mword 64) 2
                    = mword_of_int (KernelSyms.sys_read + 0x22)) by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp22) in "Hpc".
    (* ---- +0x22: c.li a0,0 -- the argument index ---- *)
    iApply (wp_cli_s_sconf (mword_of_int (KernelSyms.sys_read + 0x22))
    ...
    assert (Hpp24 : add_vec_int (mword_of_int (KernelSyms.sys_read + 0x22) : mword 64) 2
                    = mword_of_int (KernelSyms.sys_read + 0x24)) by (apply bv_eq; vm_compute; reflexivity).
    iEval (rewrite Hpp24) in "Hpc".
    (* ---- +0x24: jal ra,argfd ---- *)
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.sys_read + 0x24))
              Rra (mword_of_int 2096530 : mword 21) N3 (av - 6)%nat b
              ltac:(vm_compute; discriminate) ltac:(rdok) ltac:(vm_compute; reflexivity)
              with "Hcg Hpc []").
    ...
    change (<[Regidx Rra := regval_into_reg
              (add_vec_int (mword_of_int (KernelSyms.sys_read + 0x24) : mword 64) 4)]> N3) with N4.
    assert (Hjafd : add_vec (mword_of_int (KernelSyms.sys_read + 0x24) : mword 64)
                      (sign_extend' 64 (mword_of_int 2096530 : mword 21)) = mword_of_int KernelSyms.argfd)
      by (apply bv_eq; vm_compute; reflexivity).
    ...
    assert (HN4ra : N4 !!! Regidx Rra
                    = add_vec_int (mword_of_int (KernelSyms.sys_read + 0x24) : mword 64) 4)
    ...
    iDestruct (sr_sp_bounds _ _ _ _ Hkpos with "Hcg") as %Hspb.
    rewrite HN4sp in Hspb.
    assert (Hnzf : N4 !!! Regidx Ra2 <> (zero_reg : mword 64)).
    { rewrite HN4a2 sr_addr_f_base. apply stack_off_nonzero; [exact Hspb | lia]. }
    ...
    (* ---- +0x28: c.mv a5,a0 ---- *)
    iApply (wp_cmv_s_sconf (mword_of_int (KernelSyms.sys_read + 0x28))
    ...
    assert (Hpp2a : add_vec_int (mword_of_int (KernelSyms.sys_read + 0x28) : mword 64) 2
                    = mword_of_int (KernelSyms.sys_read + 0x2a)) by (apply bv_eq; vm_compute; reflexivity).
    ...
    iApply (wp_cli_s_sconf (mword_of_int (KernelSyms.sys_read + 0x2a))
    ...
    assert (Hpp2c : add_vec_int (mword_of_int (KernelSyms.sys_read + 0x2a) : mword 64) 2
                    = mword_of_int (KernelSyms.sys_read + 0x2c)) by (apply bv_eq; vm_compute; reflexivity).
    ...
    - (* ============ FAILURE: argfd returned -1 -- A DEAD ARM ==========
         The contract's own premise NAMES the slot, and this disjunct says
         there is none.  One [congruence] deletes the whole
         -1-without-a-call branch, which is what makes the armed post TWO
         arms rather than the landed contract's blanket. *)
      iDestruct "Hfail" as "([%Hr %Hnone] & _ & Hfcell)".
      exfalso. rewrite Hargfd in Hnone. discriminate Hnone.
    - (* ================= SUCCESS: the descriptor resolved ============= *)
      iDestruct "Hsucc" as (fd' fv') "([%Hr %Hsome] & _ & Hfcell)".
      (* THE SLOT IS THE CONTRACT'S OWN: [arg_fd] is a function, so argfd's
         witness and the premise's are the same pair. *)
      ...
      iApply (wp_blt_x0_fall_s_sconf (mword_of_int (KernelSyms.sys_read + 0x2c))
      ...
      { intros CID'; rgne. rewrite HA3s0. apply sr_addr_n. }
      iEval (rewrite -(Haddrn CID20)) in "Hs4hi".
      iApply (wp_lw_s_sconf (CID := CID20) (mword_of_int (KernelSyms.sys_read + 0x30))
      ...
      assert (Hpp34 : add_vec_int (mword_of_int (KernelSyms.sys_read + 0x30) : mword 64) 4
                      = mword_of_int (KernelSyms.sys_read + 0x34)) by (apply bv_eq; vm_compute; reflexivity).
      ...
      { intros CID'; rgne. rewrite HS1s0. apply sr_addr_p. }
      iEval (rewrite -(Haddrp CID21)) in "Hs5".
      iApply (wp_ld_s_sconf (CID := CID21) (mword_of_int (KernelSyms.sys_read + 0x34))
      ...
      { intros CID'; rgne. rewrite HS2s0. apply sr_addr_f. }
      iEval (rewrite HN4a2) in "Hfcell".
      iEval (rewrite -(Haddrf CID22)) in "Hfcell".
      iApply (wp_ld_s_sconf (CID := CID22) (mword_of_int (KernelSyms.sys_read + 0x38))
      ...
      (* ---- +0x3c: jal ra,fileread ---- *)
      iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.sys_read + 0x3c))
                Rra (mword_of_int 2094254 : mword 21) S3 (av - 6)%nat b
                ltac:(vm_compute; discriminate) ltac:(rdok) ltac:(vm_compute; reflexivity)
                with "Hcg Hpc []").
      ...
      change (<[Regidx Rra := regval_into_reg
                (add_vec_int (mword_of_int (KernelSyms.sys_read + 0x3c) : mword 64) 4)]> S3) with S4.
      assert (Hjfr : add_vec (mword_of_int (KernelSyms.sys_read + 0x3c) : mword 64)
                       (sign_extend' 64 (mword_of_int 2094254 : mword 21)) = mword_of_int KernelSyms.fileread)
        by (apply bv_eq; vm_compute; reflexivity).
      iEval (rewrite Hjfr) in "Hpc".
      assert (HS4ra : S4 !!! Regidx Rra
                      = add_vec_int (mword_of_int (KernelSyms.sys_read + 0x3c) : mword 64) 4)
      ...
      (* ---- THE FD BRIDGE (difference 2).  The lend hands the reference out
         at an EXISTENTIAL state together with the slot's AUTHORITY; the
         contract's own fragment pins it.  No new carrier, no new agreement
         law -- and it is what puts the call inside [FILEREAD_AU]'s domain
      ...
      (* THE ARMED POST goes straight through: this shell fires nothing -- the
         one instant is inside fileread's lock window -- so [Harms] is handed
