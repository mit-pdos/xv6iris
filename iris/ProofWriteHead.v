...
Lemma wh_align_arith (kk qq : Z) :
  ...
  (2147583168 + 1112 * kk + (88 + 4 * qq)) `mod` 4 = 0
  /\ 0 <= 2147583168 + 1112 * kk + (88 + 4 * qq)
  /\ 2147583168 + 1112 * kk + (88 + 4 * qq) < 18446744073709551616.
Proof.
  intros H1 H2 H3 H4. split_and!; [| lia | lia].
  replace (2147583168 + 1112 * kk + (88 + 4 * qq))
    with ((536895814 + 278 * kk + qq) * 4) by lia.
  apply Z_mod_mult.
Qed.
...
Proof.
  ...
  destruct (wh_align_arith (Z.of_nat k) (Z.of_nat q)
    ...
    as (Hm & Hlo & Hhi).
  replace (0x800184a8 + 24 + 1112 * Z.of_nat k + Z.of_nat (88 + 4 * q))
    with (2147583168 + 1112 * Z.of_nat k + (88 + 4 * Z.of_nat q)) by lia.
...
Section ProofWriteHead.
  ...
  Proof using .
    ...
    iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.write_head + 0x10)) Rs2 Rs2
              (mword_of_int 2600 : mword 12) R3 (K - 4)%nat b
              ltac:(vm_compute; discriminate) ltac:(rdok)
              with "Hcg Hpc []").
    ...
    - (* ---- n > 0: set the cursors up and enter the loop ---- *)
      ...
      iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.write_head + 0x30)) Ra4 Ra4
                (mword_of_int 2616 : mword 12) B3 (K - 4)%nat b
                ltac:(vm_compute; discriminate) ltac:(rdok)
                with "Hcg Hpc []").
