...
Lemma wh_align_arith (kk qq : Z) :
  ...
  (2147582504 + 1112 * kk + (88 + 4 * qq)) `mod` 4 = 0
  /\ 0 <= 2147582504 + 1112 * kk + (88 + 4 * qq)
  /\ 2147582504 + 1112 * kk + (88 + 4 * qq) < 18446744073709551616.
Proof.
  intros H1 H2 H3 H4. split_and!; [| lia | lia].
  replace (2147582504 + 1112 * kk + (88 + 4 * qq))
    with ((536895648 + 278 * kk + qq) * 4) by lia.
  apply Z_mod_mult.
Qed.
...
Proof.
  ...
  destruct (wh_align_arith (Z.of_nat k) (Z.of_nat q)
    ...
    as (Hm & Hlo & Hhi).
  replace (0x80018210 + 24 + 1112 * Z.of_nat k + Z.of_nat (88 + 4 * q))
    with (2147582504 + 1112 * Z.of_nat k + (88 + 4 * Z.of_nat q)) by lia.
...
Section WriteHeadBlocks.
  ...
  Local Lemma wh_tail `{GEN : GenId} `{CID0 : CpuId} 
      ...
      (f : nat -> bv 8)
      (m M : regfile) (K : nat) (eb : bool) (b : bool) (lks : gset string) (Q : iProp Σ) :
    (K_write_head <= K)%nat ->
  ...
  Proof.
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.write_head + 0x48)) Rra
              (mword_of_int 2093368 : mword 21) T1 (K - 4)%nat b
    ...
    assert (Htgtbw : add_vec (mword_of_int (KernelSyms.write_head + 0x48) : mword 64)
                       (sign_extend' 64 (mword_of_int 2093368 : mword 21))
   ...
   iDestruct (cpu_own_transport CID0 CID2 0 eb (proc_addr j) b 
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.write_head + 0x4e)) Rra
              (mword_of_int 2093412 : mword 21) T3 (K - 4)%nat b
              ltac:(vm_compute; discriminate) ltac:(rdok)
    ...
    assert (Htgtbl : add_vec (mword_of_int (KernelSyms.write_head + 0x4e) : mword 64)
                       (sign_extend' 64 (mword_of_int 2093412 : mword 21))
  ...
  Proof.
    ...
    iApply (wp_clw_s_sconf (kt := KT1) (ktd := KT0) (mword_of_int (KernelSyms.write_head + 0x3a)) Ra3 Ra4
    ...
    - (* the last entry has been copied: fall through to +0x46 *)
      ...
      iApply (wh_tail (CID0 := CID5)  γs j γl γu γd γk pd pav pu bn γfs cov logstart
                dev n W L pidv dq kk bno bsh bs0 bsd0 d0 f' m S3 K eb b lks Q
...
Section ProofWriteHead.
  ...
  Proof.
    ...
    iApply (wp_lw_s_sconf (kt := KT1) (ktd := KT0) (mword_of_int (KernelSyms.write_head + 0x14)) Ra1 Rs2
    ...
    iApply (wp_lw_s_sconf (kt := KT1) (ktd := KT0) (mword_of_int (KernelSyms.write_head + 0x18)) Ra0 Rs2
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.write_head + 0x1c)) Rra
              (mword_of_int 2093198 : mword 21) R6 (K - 4)%nat b
              ltac:(vm_compute; discriminate) ltac:(rdok)
    ...
    assert (Htgtbr : add_vec (mword_of_int (KernelSyms.write_head + 0x1c) : mword 64)
                       (sign_extend' 64 (mword_of_int 2093198 : mword 21))
    ...
    iDestruct (cpu_own_transport CID CID11 0 eb pj b ltac:(wp_next_chain)
                 with "Hcnt") as "Hcnt".
    ...
    iApply (wp_lw_s_sconf (kt := KT1) (ktd := KT0) (mword_of_int (KernelSyms.write_head + 0x22)) Ra2 Rs2
    ...
    - (* ---- n = 0: the loop is skipped entirely ---- *)
      ...
      iDestruct (cpu_own_transport CID12 CID16 0 eb pj b ltac:(wp_next_chain)
                   with "Hcnt") as "Hcnt".
      ...
      iApply (wh_tail (CID0 := CID16)  γs j γl γu γd γk pd pav pu bn γfs cov logstart
                ...
                bsh bs0 bsd0 d0 f1 m B2 K eb b lks Q
    ...
    - (* ---- n > 0: set the cursors up and enter the loop ---- *)
      ...
      iDestruct (cpu_own_transport CID12 CID21 0 eb pj b ltac:(wp_next_chain)
                   with "Hcnt") as "Hcnt".
      ...
      iApply (wh_loop (CID0 := CID21)  γs j γl γu γd γk pd pav pu bn γfs cov logstart
                ...
                bsh bs0 bsd0 d0 m K eb b lks (S n') Q
