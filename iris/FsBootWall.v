...
(*                                                                        *)
(*  WHAT REMAINS IS THE MINT'S VALUE, AND IT IS A THIRD WALL.              *)
(*  [FsCfgSnap.fs_cfg_alloc_snap] still takes [snap_ok S] at the RAW home  *)
(*  blocks.  Reading [D] instead means taking [S] from the crash           *)
(*  predicate's [SystemAdequacy.fs_boot_pure], which already delivers      *)
(*  "[exists S, snap_ok S D]" and [FsCrash.hdr_wf] at every era -- but     *)
(*  [FsDurSnap.snap_ok S D] is a statement about [S] and [D] ALONE.        *)
(*  Nothing in it ties [fss_sb S] -- hence the era's logstart,             *)
(*  inodestart, bmapstart, ninodes and [nib] -- to the era-invariant       *)
(*  superblock that the crash predicate's [cov]/[ls] and the boot chain's  *)
(*  configuration are stated at.  [SystemAdequacy.fs_boot_image_eras]      *)
(*  ([Himg]) is what supplies that tie today, and it is why it cannot be   *)
(*  deleted yet.  The fix is a STRENGTHENING OF THE CRASH PREDICATE:       *)
(*  [FsCrash.P_fs] already carries [ls], so the clause is                  *)
...
(*  snapshot at the era's OWN state, whose superblock is the era's).       *)
(* ====================================================================== *)
