...
(*                                                                        *)
(*  WHAT REMAINS IS THE MINT'S VALUE, AND IT IS NOT A WALL.                *)
(*  [FsCfgSnap.fs_cfg_alloc_snap] now TAKES the byte view's value [Pb] and *)
(*  the exception set [Xexc]; era 0 passes the raw disk and the empty set. *)
(*  Passing [FsCrash.fr_D] is what the boot chain has still to do, and the *)
(*  pure channel already exists: [SystemAdequacy.fs_boot_pure] delivers    *)
(*  [fs_extent], the recovery record, [FsCrash.hdr_wf] and                 *)
(*  "[exists S, snap_ok S D]" at EVERY era through                        *)
(*  [RiscvAdequacy.riscv_power_adequacy]'s [Hproj]/[Ppure].  Four things   *)
(*  are left, measured:                                                    *)
(*                                                                        *)
(*   1. THE ERA'S CONFIGURATION COMES FROM [fss_sb S], not from the        *)
(*      theorem's [sb].  [FsDurSnap.snap_ok S D] is about [S] and [D]      *)
(*      alone, so it cannot say [fss_sb S = sb] -- and it does not have    *)
(*      to.  [FsImg.fs_sb_ok] (which [sk_sbok] delivers) already fixes     *)
(*      [logstart = 2], [nlog = 31] and [inodestart]/[bmapstart]/[size]    *)
(*      as functions of [ninodes]/[nblocks], so the era's logstart IS the  *)
(*      crash predicate's [ls] for free.  What must change is that         *)
(*      [BootShared.boot_shared_alloc]'s [sb]/[nib] become EXISTENTIAL in  *)
(*      its post, bound by [SystemAdequacy.xv6_boot_era].                  *)
(*   2. THE TWO COVERAGE CORNERS need a NEW pure reading of [snap_ok]:     *)
(*      every block below [sb_size (fss_sb S)] is in [dom D] (free by      *)
(*      [sk_pool], metadata by [sk_sb]/[sk_reg]/[sk_bmap], a node's by     *)
(*      [sk_slot]) or in the log region.  With [dom D ⊆ home ⊆ cov] and    *)
(*      [log_region_set ls ⊆ cov] that IS the corner.  This is the only    *)
(*      genuine proof work left.                                           *)
(*   3. [FirstTok.first_fsinit_pures] needs a SNAPSHOT-side producer.  Its *)
(*      [hdr_n = 0] clause is replaced by [hdr_wf]'s three; its            *)
(*      parse/[fs_sb_ok] pair is [FsCrash.fs_recovery_sb_parse] plus       *)
(*      [sk_sbok] (lane E-blk1 landed both); its block-1 BYTE SHAPE is     *)
(*      [FirstTok.first_sb_image_of_le], which needs only [32 <= length],  *)
(*      so it costs nothing; and [FsCollect.col_geom] is                   *)
(*      [sk_reg]/[sk_regdom]/[sk_sbok] against the width tie the mint      *)
(*      itself establishes.                                                *)
(*   4. [boot_shared_alloc] then spends [FsCfgBoot.fs_boot_image_wf] only  *)
(*      at era 0, and [SystemAdequacy.fs_boot_image_eras] ([Himg]) goes    *)
(*      with [boot_hart_primary]'s use of it.                              *)
(* ====================================================================== *)
