...
(*                                                                        *)
(*  AFTER LANE G5, AND NARROWED AGAIN -- BUT NOT CLOSED.  G5 replaced the  *)
(*  parent register with the TYPE REGISTER ([FsStateLink]'s               *)
(*  [auth (gmultiset ity)], one per inum) and built the per-directory     *)
(*  EXACTNESS clause the ruling asked for                                 *)
(*  ([FsStateInode.node_exact]: a live directory's count is the size of   *)
(*  its marker set plus one).  Both of S7-unlink's readings now come off  *)
(*  the register and the marker set alone: (D1) is                        *)
(*  [IregLinkNz.ireg_toks_agree] between the parent's name record and the *)
(*  child's own ["."] fragment, and (D2) is [node_exact] read off [dp]'s  *)
(*  own bundle.  NOTHING IN THE WALKS READS [DirLinks] FOR EITHER any     *)
(*  more.                                                                 *)
(*                                                                        *)
(*  WHAT THAT DOES NOT DO is retire (a)/(b)/(c).  [DirLinks.dir_links] is *)
(*  still the FIRST conjunct of [IcacheEscrow.dlinks] and the region's    *)
(*  [wl/wdu/wdt/g/p] columns and the [fl] index are still carried through *)
(*  every flush, so the boot still has to build them and                  *)
(*  [dir_links_of_plain] is still the only constructor.  The wall is      *)
(*  therefore unchanged in substance and now waits on the DEMOLITION      *)
(*  alone -- slice 6's [DirLinks.v] removal -- rather than on any missing *)
(*  reading.  The exactness clause is what makes that demolition          *)
(*  possible: it is the replacement for [DirView.dlc_lower], which was    *)
(*  the one thing [dir_links] carried that nothing else did.              *)
(*                                                                        *)
