...
Definition wp_trapinithart_sconf_body `{!riscvGS Σ, !sieG Σ} `{GEN : GenId} `{CID : CpuId} (kt : ktier) (mm : regfile) (K : nat)
...
Module Type TRAPINITHART.
  Parameter wp_trapinithart_sconf :
    forall `{!riscvGS Σ, !sieG Σ} `{GEN : GenId} `{CID : CpuId} (kt : ktier) (mm : regfile) (K : nat)
      (tv0 : mword 64) (p : mword 64),
