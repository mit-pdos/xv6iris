...
Section ctx.
  ...
  (* ================================================================== *)
  (* §0.35'(i)+(iii) AND §0.38': THE BUY / CARRY / CASH TRIPLE.          *)
  (*                                                                    *)
  (* Landed at slice 1 on the T-leg's interface, NOT the M-leg's -- the  *)
  (* M-leg's first draft of this module had none of the three, and the   *)
  (* T-leg had to retrofit them (A6.96/A6.97) after measuring that every *)
  (* tier of the proof eventually consumes the triple: locks (§0.35'),   *)
  (* the kernel page table (§0.36'), the park protocol (§0.27'), and the *)
  (* trap-handler contract (§0.39').  main-tso-readiness.md §4 makes     *)
  (* them MANDATORY for exactly that reason, and Amendment 1 §1.2        *)
  (* records where they actually live: origin/tso-flip, never origin/tso.*)
  (*                                                                    *)
  (* CARRY -- [ctx_floor xi lo]: a persistent lower-bound witness on a   *)
  (* context's bound.  At TSO ([tso-flip TsoCtx.ctx_floor]) it is        *)
  (* [llb (ctx_bound_name xi) lo]; here it is trivial.  The point of the *)
  (* SHAPE is what A6.89/A6.92 paid for: it names NO HART.  A floor is a *)
  (* fact about the THREAD OF CONTROL xi, and a thread carries its       *)
  (* context across every migration, so a floor survives a [wp_next]     *)
  (* crossing, a park and a resume.  Both refuted spellings -- the       *)
  (* strengthened [locked] token (A6.89 §(5), and see §0.34''s           *)
  (* refutation note) and the lock word's author receipt (A6.92 §(3)) -- *)
  (* were HART-relative and both died on a crossing.                     *)
  (* ================================================================== *)
  Definition ctx_floor_def (xi : CtxId) (lo : nat) : iProp Σ := True%I.
  Lemma ctx_floor_aux : { f | f = ctx_floor_def }.
  Proof. by eexists. Qed.
  Definition ctx_floor (xi : CtxId) (lo : nat) : iProp Σ :=
    proj1_sig ctx_floor_aux xi lo.
  Lemma ctx_floor_unseal (xi : CtxId) (lo : nat) :
    ctx_floor xi lo = ctx_floor_def xi lo.
  Proof. unfold ctx_floor. by rewrite (proj2_sig ctx_floor_aux). Qed.

  Global Instance ctx_floor_persistent xi lo : Persistent (ctx_floor xi lo).
  Proof. rewrite ctx_floor_unseal /ctx_floor_def. apply _. Qed.
  Global Instance ctx_floor_timeless xi lo : Timeless (ctx_floor xi lo).
  Proof. rewrite ctx_floor_unseal /ctx_floor_def. apply _. Qed.

  (* The era-image floor: every context is above position 0.  This is the
     left-hand source of §0.38''s [lk_floor] disjunction for the .bss
     locks -- their [initlock] precedes the started flag's write, so the
     handle they mint is floored at 0 and A6.100's "two floor sources,
     not two distributions" is the reading. *)
  Lemma ctx_floor_0 xi : ⊢ ctx_floor xi 0.
  Proof. rewrite ctx_floor_unseal /ctx_floor_def. auto. Qed.

  Lemma ctx_floor_le xi lo lo' :
  ...
  (*                                                                    *)
  (* A thread holding a view receipt may RAISE ITS OWN CONTEXT'S BOUND   *)
  (* to it, and what it gets back is the floor a lock handle carries.    *)
  (* At TSO the proof is one [mono_nat_own_update] plus the observation  *)
  (* that nothing in the token is weakened by raising B: the clean facts *)
  (* claim [t <= B] and only get easier, and the dirty entries'          *)
  (* justification is monotone in B.                                     *)
  (*                                                                    *)
  (* THE AUTHOR'S FLOOR IS BOUGHT, NOT FREE, and that is the point of    *)
  (* the creator-side use (§0.38', A6.101/A6.103): [initlock]'s          *)
  (* store-then-mint leaves the lock's window floored at its own append, *)
  (* which the writer's own bound has NOT passed -- a plain store is     *)
  (* buffered.  The creator buys the floor at its NEXT acquire, whose    *)
  (* AMO takes the view to the log top: the same receipt every other     *)
  (* hart pays with.  This is why the creator bootstrap is structural    *)
  (* rather than a distribution (A6.100), and why [initlock]'s spec is   *)
  (* floor-parametric.                                                   *)
  (* ------------------------------------------------------------------ *)
  Lemma ctx_bound_raise `{CID : CpuId} (xi : CtxId) (K' : nat) :
    own_context xi -∗ hart_view_lb K' ==∗ own_context xi ∗ ctx_floor xi K'.
  Proof.
    iIntros "Hrun #HK'". iModIntro. iFrame "Hrun".
    rewrite ctx_floor_unseal /ctx_floor_def. auto.
  Qed.

  (* ------------------------------------------------------------------ *)
  (* CASH -- the read-side, and it is the object four sites wanted.      *)
  (*                                                                    *)
  ...
  (* and the relation [lo <= K] against a floor that was existential     *)
  (* inside an invariant.  With the floor carried on the HANDLE and the  *)
  (* running token in hand, both fall out.                               *)
  (*                                                                    *)
  (* The token is THREADED, NOT CONSUMED: the view receipt is persistent *)
  (* and the token comes straight back.  Callers rely on that -- a       *)
  (* read gate discharges its floor mid-proof and keeps running as xi.   *)
  (* ------------------------------------------------------------------ *)
  Lemma own_context_floor_view `{CID : CpuId} (xi : CtxId) (lo : nat) :
    own_context xi -∗ ctx_floor xi lo -∗
    own_context xi ∗ ∃ K : nat, hart_view_lb K ∗ ⌜(lo <= K)%nat⌝.
  Proof.
  ...
  (* THE FLOOR TRAVELS ALONG DOMINATION, and it is the ten-line law
     A6.116/A6.117 found closes the expensive branch: a floor the SENDER
     can discharge is one the RECEIVER can discharge, so payload transport
     needs no [CtxMorph] strengthening.  Stated here rather than beside
     [ctx_floor] because it is where [ctx_dom] first exists.  (T-leg
     [TsoCtx.ctx_floor_dom]; the floor is persistent, so [ctx_dom] comes
     back untouched.) *)
