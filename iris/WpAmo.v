(* WpAmo.v -- S-mode support for xv6's amoswap.w.aq (acquire()'s atomic
   test-and-set).  Three layers:
     ...
     3. wp: the capstone [wp_amoswap_w_s] on the stock [instr] step engine
        (whose decode field now also provides misa.A, threaded from
        hw_config); mirrors wp_csw_s (TLB hit and walk data paths) plus
        wp_gpr_write_s_config's rd update. *)
...
(* ===================================================================== *)
(* Part 7 -- the WP: one S-mode step of [amoswap.w.aq rd, rs2, (rs1)] on  *)
(* a 4-byte RAM cell in the identity superpage.  Mirrors wp_csw_s (data   *)
(* TLB hit + walk) and additionally writes rd (mirroring                  *)
(* wp_gpr_write_s_config's register-file update).                         *)
(* ===================================================================== *)
...
Section WpAmoswap.
  ...
  Lemma wp_amoswap_w_s (root_ppn : mword 44) E (Φ : mval -> iProp Σ)
      (pc : mword 64) (rd rs2 rs1 : mword 5) (svpn : mword 27)
      (m : gmap regidx (mword 64)) (w : mword 32)
    ...
    ([∗ list] j ∈ seq 0 4, (pa_add pa j) ↦ₘ nth_byte w j) -∗
    ( hart_state ↦ᵣ{ dq } HART_ACTIVE tt -∗
      cur_privilege ↦ᵣ{ dq } Supervisor -∗
      mstatus ↦ᵣ{ dq } mstatus0 -∗
      mie ↦ᵣ{ dq } mie_v -∗
      ...
      ([∗ list] j ∈ seq 0 4, (pa_add pa j) ↦ₘ nth_byte (amoswap_stored (m !!! Regidx rs2)) j) -∗
  ...
  Proof.
    ...
    (* the exec lemmas' effective address shapes collapse to [pa] *)
