(* UmodeData1.v -- U-mode width-1 DATA load/store execute lemmas (clone of UmodeData4).

   Thin U-mode instantiation of the mode-neutral width-1 tower in
   ...
   generic exec_execute_LOAD_1_gpr / exec_execute_STORE_1_gpr.  The LOAD
...
Require Import UmodeFetch MemData1 UmodeData.
Require Import CommonWalk.
Local Open Scope Z_scope.
Import Defs.

(* ===================================================================== *)
(* U-mode width-1 register-generic LOAD (LW / LWU), rd <> x0.            *)
(* ===================================================================== *)
Section VRU4.
  ...
  Hypothesis Halign : is_aligned_vaddr (Virtaddr ea) 1 = true.
  ...
  Hypothesis Hpmam : matching_pma_region (register_lookup pma_regions s.(sregs)) (Physaddr pa) 1 = Some region.
  ...
  Hypothesis Hbytes : forall j : nat, (N.of_nat j < 1)%N ->
    s.(mem) !! (pa_add pa j) = Some (nth_byte v j).
  Hypothesis Hrd : uint rd <> 0.

  Lemma exec_execute_LOAD_1_U (is_unsigned : bool) :
  ...
  Proof.
    ...
    assert (Htr : exec (translateAddr (Virtaddr (add_vec_int (bits_of_virtaddr (Virtaddr ea)) (0 * 1))) (Load Data)) s
                  = Some (Ok (Physaddr pa, PBMT_PMA, init_ext_ptw), s)).
    { cbn [bits_of_virtaddr]. rewrite avi0_mul1.
    ...
    assert (Hpmp : exec (pmpCheck (Physaddr pa) 1 (Load Data) User) s = Some (None, s)).
    { exact (exec_pmpCheck_user_grant_load pa 1 s HpmpA Hpmp_ord Hrange HpmpR). }
    exact (exec_execute_LOAD_1_gpr User is_unsigned rs1 rd imm ea v region s pa
             Hrd Htea Halign Hcp Hmprv Htr Hpmp Hpmam Hpalign Hread Hc Hsig Hh Hdev Hbytes).
  Qed.
End VRU4.

(* ===================================================================== *)
(* U-mode width-1 register-generic STORE (SW).                          *)
(* ===================================================================== *)
Section VWU4.
  Context (ent : TLB_Entry) (vpn : mword 27).
  ...
  Proof.
    ...
    assert (Htr : exec (translateAddr (Virtaddr (add_vec_int (bits_of_virtaddr (Virtaddr ea)) (0 * 1))) (Store Data)) s
                  = Some (Ok (Physaddr pa, PBMT_PMA, init_ext_ptw), s)).
    { cbn [bits_of_virtaddr]. rewrite avi0_mul1.
      exact (exec_translateAddr_store_hit_u ent vpn ea
               ...
               Hchk Hupd Hpbmt Hmatch_tlb Hcp HSXL Hmprv eq_refl Hsatpmode Hasid
               eq_refl Hvec Hcanon Hvpn_def). }
    assert (Hpmp : exec (pmpCheck (Physaddr pa) 1 (Store Data) User) s = Some (None, s)).
    { exact (exec_pmpCheck_user_grant_store pa 1 s HpmpA Hpmp_ord Hrange HpmpW). }
    exact (exec_execute_STORE_1_gpr User rs2 rs1 imm ea region s pa
...
Section VRU4Walk.
  ...
  Lemma exec_execute_LOAD_1_U_walk (is_unsigned : bool) :
    exec (execute (LOAD (imm, Regidx rs1, Regidx rd, is_unsigned, 1))) s
      = Some (RETIRE_SUCCESS,
              set_reg s' (R_bitvector_64 (gpr_of_Z (uint rd)))
                (regval_into_reg (extend_value is_unsigned data2))).
  Proof.
    ...
    assert (Hpmam' : matching_pma_region (register_lookup pma_regions s'.(sregs)) (Physaddr pa) 1 = Some region)
    ...
    assert (Hbytes' : forall j : nat, (N.of_nat j < 1)%N ->
              s'.(mem) !! (pa_add pa j) = Some (nth_byte v j)).
    { intros j Hj. unfold s', set_reg; cbn [mem]. exact (Hbytes j Hj). }
    exact (exec_execute_LOAD_1_gpr_walk User is_unsigned rs1 rd imm ea v region s s' pa
             Hrd Htea Halign Htr Hcp' Hmprv' Hpmp Hpmam' Hpalign Hread
             (within_clint_false pa 1 s' Hnc ltac:(lia))
             (within_sig_false pa 1 s' Hns ltac:(lia))
             (within_htif_false pa 1 s' Lhtif')
             (addr_is_ram_not_dev _ Hram) Hbytes').
  Qed.
End VRU4Walk.

(* nomatch load *)
Section VRU4WalkNomatch.
  Context (vpn : mword 27) (root : mword 44).
  ...
  Lemma exec_execute_LOAD_1_U_walk_nomatch (is_unsigned : bool) :
    exec (execute (LOAD (imm, Regidx rs1, Regidx rd, is_unsigned, 1))) s
      = Some (RETIRE_SUCCESS,
              set_reg s' (R_bitvector_64 (gpr_of_Z (uint rd)))
                (regval_into_reg (extend_value is_unsigned data2))).
...
Section VWU4Walk.
  ...
  Lemma exec_execute_STORE_1_U_walk :
    exec (execute (STORE (imm, Regidx rs2, Regidx rs1, 1))) s
      = Some (RETIRE_SUCCESS,
              MState s'.(sregs) (write_bytes s'.(mem) pa 1
                (autocast (T := mword) (subrange_vec_dec vrs2 (Z.sub (Z.mul 1 8) 1) 0) : mword 8)) s'.(mdev)).
  Proof.
    ...
    assert (Lhtif' : register_lookup htif_tohost_base s'.(sregs) = None)
      by (unfold s', set_reg; cbn [sregs]; rewrite irrelevant_register_set; [ exact Hhtif | vm_compute; reflexivity ]).
    exact (exec_execute_STORE_1_gpr_walk User rs2 rs1 imm ea region s s' pa
             Htea Halign Htr Hcp' Hmprv' Hpmp Hpmam' Hpalign Hwrite
             (within_clint_false pa 1 s' Hnc ltac:(lia))
             (within_sig_false pa 1 s' Hns ltac:(lia))
             (within_htif_false pa 1 s' Lhtif' : exec (within_htif_writable (Physaddr pa) 1) s' = Some (false, s'))
             (addr_is_ram_not_dev _ Hram)).
  Qed.
End VWU4Walk.

(* nomatch store *)
Section VWU4WalkNomatch.
  Context (vpn : mword 27) (root : mword 44).
  Variable ent' : TLB_Entry.
  Variable rs2 rs1 : mword 5.
  Variable imm : mword 12.
  ...
  Lemma exec_execute_STORE_1_U_walk_nomatch :
    exec (execute (STORE (imm, Regidx rs2, Regidx rs1, 1))) s
      = Some (RETIRE_SUCCESS,
              MState s'.(sregs) (write_bytes s'.(mem) pa 1
                (autocast (T := mword) (subrange_vec_dec vrs2 (Z.sub (Z.mul 1 8) 1) 0) : mword 8)) s'.(mdev)).
