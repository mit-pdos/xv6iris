...
Section ProofCopyout.
  ...
  Proof.
    ...
    { iIntros (CIDh Pd Md pa0)
      ...
      { iIntros (CIDc Mn nn)
        ...
        iApply (Memmove.wp_memmove_sconf U5 (K - 14)%nat nn
    ...
    { (* ===== walkaddr missed: fault the page in ===== *)
      ...
      iDestruct (cpu_own_transport CID0 CIDm5 lvl eb p C b ltac:(wp_next_chain)
                   with "Hcnt") as "Hcnt".
      iApply (Vmfault.wp_vmfault_sconf γa (tp_pin F5) Pc szv (K - 14)%nat lvl eb p C b
  ...
  Proof.
    ...
    iApply (wp_cj_s_sconf (mword_of_int (KernelSyms.copyout + 0x34))
              ...
              with "Hcg Hpc Hi34 [-]").
    iIntros (CIDpr25 Hspr25). iNext. iIntros "Hcg Hpc".
    ...
    { iIntros (CIDe0 Hse0 mj res P')
      ...
      iDestruct (cpu_own_transport CIDe0 CIDe15 lvl eb p C b ltac:(wp_next_chain)
                   with "Hcnt") as "Hcnt".
    ...
    (* ---- into the loop ---- *)
    iDestruct (cpu_own_transport CID CIDpr25 lvl eb p C b ltac:(wp_next_chain)
                 with "Hcnt") as "Hcnt".
    iApply (co_loop γa mm P szv len src_bytes K lvl eb p C src spr b
