...
Section ProofIget.
  ...
  (* ---- THE SCAN'S BLOCK CONTINUATIONS, [Hloop]/[Hstep]: RULE ONE FOLD
     ...
     lemma-binder/proof-local name each body mentions -- [γl fsc_ic fsc_fs γi cov
  ...
  Proof.
    ...
    iApply (Acquire.wp_acquire_sconf KT1 γl "itable"%string
              (itable_res2 fsc_ic fsc_fs γi cov logstart nib dev) mA
        ...
        - (* ===== THE SENTINEL: the branch is TAKEN, to +0x6a ===== *)
          ...
          + (* ===== THE RECYCLE, four stores wide ===== *)
            ...
            iDestruct (big_sepL_lookup
                         (fun (_ : nat) (i0 : nat) => ic_escrow fsc_ic fsc_fs γi cov logstart i0)
                         (seq 0 NINODE) e e
    ...
    (* Here the peek is [ic_open_empty_dev] run at [devN := devT]: it hands
       ...
       the ghost re-tag is the identity. *)
            ...
            iDestruct (ic_open_empty_dev_peek fsc_ic fsc_fs γi cov logstart e (1/4)
            ...
            { rewrite Hpa6e Hsv6e.
              ...
              iMod (ipool_id_lend (⊤ ∖ ↑minstretN) fsc_ic fsc_fs γi cov logstart nib
                      e devT inumT ltac:(solve_ndisj) He with "Hpinv Hgid")
                as "[Hgid Hidback]".
              iInv "Hesc" as ">Hbody" "Hclose2".
              iMod (ic_open_empty_dev fsc_ic fsc_fs γi cov logstart e devT inumT dev
    ...
    (* Here no peek is needed at all: the caller already OWNS half of the
       inum cell ([HinT]), which is what it is about to put in the update. *)
            ...
            { rewrite Hpa72 Hsv72.
              ...
              iMod (ipool_take_lend (⊤ ∖ ↑minstretN) fsc_ic fsc_fs γi inodestart cov
                ...
                as "(Hlic & Hbundle & Hicnt0 & Hmir0 & Hfoff & Hpool & Hgid &
                     Hidback)".
              iInv "Hesc" as ">Hbody" "Hclose2".
              iMod (ic_open_empty_free fsc_ic fsc_fs γi cov logstart e dev inumT dev inum
              ...
              { iApply bi.later_intro. iApply ic_close_mid.
                iApply (ic_mk_mid_arm fsc_ic fsc_fs γi cov logstart e dev inum wv
    ...
    (* [ic_open_mid] / [ic_close_mid] are an open/close pair with no ghost
       move, so the mid arm's valid cell can be looked at and put back. *)
            iApply fupd_wp.
            iInv "Hesc" as ">Hbodyp" "Hclosep".
            iDestruct (ic_open_mid fsc_ic fsc_fs γi cov logstart e with "Hmt Hbodyp")
              as "[Hmt Harmp]".
            ...
            { iNext. iApply (ic_close_mid fsc_ic fsc_fs γi cov logstart e).
            ...
            { rewrite Hpa7c Hsv7c.
              iInv "Hesc" as ">Hbody" "Hclose2".
              iDestruct (ic_open_mid fsc_ic fsc_fs γi cov logstart e with "Hmt Hbody")
                as "[Hmt Harm]".
              ...
              iDestruct (ic_close_mid_to_parked fsc_ic fsc_fs γi cov logstart e dev inum gnew
            ...
            iApply (Release.wp_release_sconf KT1 γl itable_lock "itable"%string
                      (itable_res2 fsc_ic fsc_fs γi cov logstart nib dev) V4
    ...
    (* THE ADDRESS CLAIM, READ OFF THE CELL ITSELF.  The per-node form takes
       ...
       VALUE, so a RESTORING peek of the same cell delivers it. *)
        ...
        iEval (rewrite Houtb) in "Hcg".
        iApply (Release.wp_release_sconf KT1 γl itable_lock "itable"%string
                  (itable_res2 fsc_ic fsc_fs γi cov logstart nib dev) L7
                  n eb p (K - 6)%nat ({["itable"]} ∪ lks)
