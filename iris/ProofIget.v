...
Section ProofIget.
  ...
  (* ---- THE SCAN'S BLOCK CONTINUATIONS, [Hloop]/[Hstep]: RULE ONE FOLD
     ...
     lemma-binder/proof-local name each body mentions -- [γl fsc_ic fsc_fs γi fsc_cov
  ...
  Proof.
    ...
    iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.iget + 0x18)) Ra0 Ra0 (mword_of_int 2660 : mword 12)
              R5 (K - 6)%nat b ltac:(nz) ltac:(rdok) with "Hcg Hpc []").
    { iApply (igi_18 with "Htext"). }
    iIntros (CID12 Hs12) "Hcg Hpc".
    iEval (rgne) in "Hcg".
    set (R6 := <[Regidx Ra0 := regval_into_reg
                  (add_vec (R5 !!! Regidx Ra0) (sign_extend' 64 (mword_of_int 2660 : mword 12)))]> R5).
    ...
    iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.iget + 0x1c)) Rra (mword_of_int 2088246 : mword 21)
    ...
    assert (Htgtacq : add_vec (mword_of_int (KernelSyms.iget + 0x1c) : mword 64)
                        (sign_extend' 64 (mword_of_int 2088246 : mword 21))
    ...
    iApply (Acquire.wp_acquire_sconf KT1 γl "itable"%string
              (itable_res2 fsc_ic fsc_fs γi fsc_cov fsc_logst nib dev) mA
    ...
    iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.iget + 0x26)) Rs1 Rs1 (mword_of_int 2670 : mword 12)
              D2 (trap_res b + (K - 6))%nat false ltac:(nz) ltac:(rdok) with "Hcg Hpc []").
    { iApply (igi_26 with "Htext"). }
    iApply wp_next_off_intro. iIntros "Hcg Hpc".
    iEval (rgne) in "Hcg".
    set (D3 := <[Regidx Rs1 := regval_into_reg
                  (add_vec (D2 !!! Regidx Rs1) (sign_extend' 64 (mword_of_int 2670 : mword 12)))]> D2).
    ...
    iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.iget + 0x2e)) Ra3 Ra3 (mword_of_int 1270 : mword 12)
              D4 (trap_res b + (K - 6))%nat false ltac:(nz) ltac:(rdok) with "Hcg Hpc []").
    { iApply (igi_2e with "Htext"). }
    iApply wp_next_off_intro. iIntros "Hcg Hpc".
    iEval (rgne) in "Hcg".
    set (D5 := <[Regidx Ra3 := regval_into_reg
                  (add_vec (D4 !!! Regidx Ra3) (sign_extend' 64 (mword_of_int 1270 : mword 12)))]> D4).
        ...
        - (* ===== THE SENTINEL: the branch is TAKEN, to +0x6a ===== *)
          ...
          + (* ===== "iget: no inodes".  THE PANIC IS LIVE: a full table is
            ...
            iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.iget + 0xa2)) Ra0 Ra0
                      (mword_of_int 1274 : mword 12) PA1 (trap_res b + (K - 6))%nat false
                      ltac:(nz) ltac:(rdok) with "Hcg Hpc []").
            { iApply (igi_a2 with "Htext"). }
            iApply wp_next_off_intro. iIntros "Hcg Hpc".
            set (PA2 := <[Regidx Ra0 := regval_into_reg
                           (add_vec (rget PA1 Ra0)
                              (sign_extend' 64 (mword_of_int 1274 : mword 12)))]> PA1).
            ...
            iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.iget + 0xa6)) Rra
                      (mword_of_int 2087174 : mword 21) PA2 (trap_res b + (K - 6))%nat false
                      ltac:(nz) ltac:(rdok) ltac:(vm_compute; reflexivity)
                      with "Hcg Hpc []").
            { iApply (igi_a6 with "Htext"). }
            iApply wp_next_off_intro. iIntros "Hcg Hpc".
            assert (Htgtpn : add_vec (mword_of_int (KernelSyms.iget + 0xa6) : mword 64)
                               (sign_extend' 64 (mword_of_int 2087174 : mword 21))
          ...
          + (* ===== THE RECYCLE, four stores wide ===== *)
            ...
            iDestruct (big_sepL_lookup
                         (fun (_ : nat) (i0 : nat) => ic_escrow fsc_ic fsc_fs γi fsc_cov fsc_logst i0)
                         (seq 0 NINODE) e e
    ...
    (* Here the peek is [ic_open_empty_dev] run at [devN := devT]: it hands
       ...
       the ghost re-tag is the identity. *)
            (* durable-disk C-3b: the peek RE-TAGGED nothing even before,
               so it now runs at the table's quarter ([ic_open_empty_dev_peek])
               rather than joining the pool's in for a full unit. *)
            iApply fupd_wp.
            iInv "Hesc" as ">Hbodyp" "Hclosep".
            iDestruct (ic_open_empty_dev_peek fsc_ic fsc_fs γi fsc_cov fsc_logst e (1/4)
            ...
            { rewrite Hpa6e Hsv6e.
              (* THE POOL'S QUARTER (durable-disk C-3b): the identification
                 ghost is re-tagged here, and the pool holds a quarter of it
              ...
              iMod (ipool_id_lend (⊤ ∖ ↑minstretN) fsc_ic fsc_fs γi fsc_cov fsc_logst nib
                      e devT inumT ltac:(solve_ndisj) He with "Hpinv Hgid")
                as "[Hgid Hidback]".
              iInv "Hesc" as ">Hbody" "Hclose2".
              iMod (ic_open_empty_dev fsc_ic fsc_fs γi fsc_cov fsc_logst e devT inumT dev
       ...
       inum cell ([HinT]), which is what it is about to put in the update. *)
            ...
            iApply (wp_sw_au_s_sconf false (mword_of_int (KernelSyms.iget + 0x72)) Rs4 Rs3
                      ...
                      (i_dev (ientry e) ↦₄{DfracOwn (1/2)} dev ∗
                       ...
                       ipool fsc_fs γi fsc_cov fsc_logst
            ...
            { rewrite Hpa72 Hsv72.
              ...
              iMod (ipool_take_lend (⊤ ∖ ↑minstretN) fsc_ic fsc_fs γi inodestart fsc_cov
                ...
                as "(Hlic & Hbundle & Hicnt0 & Hmir0 & Hfoff & Hpool & Hgid &
                     Hidback)".
              iInv "Hesc" as ">Hbody" "Hclose2".
              iMod (ic_open_empty_free fsc_ic fsc_fs γi fsc_cov fsc_logst e dev inumT dev inum
              ...
              { iApply bi.later_intro. iApply ic_close_mid.
                iApply (ic_mk_mid_arm fsc_ic fsc_fs γi fsc_cov fsc_logst e dev inum wv
    ...
    (* THE ADDRESS CLAIM, READ OFF THE CELL ITSELF.  The per-node form takes
       ...
       VALUE, so a RESTORING peek of the same cell delivers it. *)
            ...
            { rewrite Hpa78 Hsv78.
              ...
              iMod (iref_alloc_store_au (⊤ ∖ ↑minstretN) γi fsc_fs inodestart nib
                      M e inum l (1/2/2)%Qp
    ...
    (* [ic_open_mid] / [ic_close_mid] are an open/close pair with no ghost
       move, so the mid arm's valid cell can be looked at and put back. *)
            iApply fupd_wp.
            iInv "Hesc" as ">Hbodyp" "Hclosep".
            iDestruct (ic_open_mid fsc_ic fsc_fs γi fsc_cov fsc_logst e with "Hmt Hbodyp")
              as "[Hmt Harmp]".
            ...
            { iNext. iApply (ic_close_mid fsc_ic fsc_fs γi fsc_cov fsc_logst e).
            ...
            { rewrite Hpa7c Hsv7c.
              iInv "Hesc" as ">Hbody" "Hclose2".
              iDestruct (ic_open_mid fsc_ic fsc_fs γi fsc_cov fsc_logst e with "Hmt Hbody")
                as "[Hmt Harm]".
              ...
              iDestruct (ic_close_mid_to_parked fsc_ic fsc_fs γi fsc_cov fsc_logst e dev inum gnew
            ...
            iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.iget + 0x84)) Ra0 Ra0
                      (mword_of_int 2552 : mword 12) V2 (trap_res b + (K - 6))%nat false
                      ltac:(nz) ltac:(rdok) with "Hcg Hpc []").
            ...
            set (V3 := <[Regidx Ra0 := regval_into_reg
                          (add_vec (V2 !!! Regidx Ra0)
                             (sign_extend' 64 (mword_of_int 2552 : mword 12)))]> V2).
            ...
            iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.iget + 0x88)) Rra
                      (mword_of_int 2088274 : mword 21) V3 (trap_res b + (K - 6))%nat false
                      ltac:(nz) ltac:(rdok) ltac:(vm_compute; reflexivity)
                      with "Hcg Hpc []").
            ...
            assert (Htgtrel2 : add_vec (mword_of_int (KernelSyms.iget + 0x88) : mword 64)
                                 (sign_extend' 64 (mword_of_int 2088274 : mword 21))
            ...
            iApply (Release.wp_release_sconf KT1 γl itable_lock "itable"%string
                      (itable_res2 fsc_ic fsc_fs γi fsc_cov fsc_logst nib dev) V4
    ...
    (* THE ADDRESS CLAIM, READ OFF THE CELL ITSELF.  The per-node form takes
      ...
      - (* ===== A LIVE SLOT: [bge x0,a5] falls through ===== *)
        ...
        iApply fupd_wp.
        iMod (frz_park_lic_off ⊤ γi fsc_fs inodestart nib inum l j
    ...
    (* THE ADDRESS CLAIM, READ OFF THE CELL ITSELF.  The per-node form takes
       ...
       VALUE, so a RESTORING peek of the same cell delivers it. *)
        ...
        { rewrite Hpa58 Hstv.
          ...
          iMod (iref_incr_store_au (⊤ ∖ ↑minstretN) γi fsc_fs inodestart nib M j inum l
                  qj (qj'/2)%Qp nj
                  ltac:(solve_ndisj) ltac:(solve_ndisj)
                  ltac:(apply subseteq_difference_r;
                        [solve_ndisj | apply logN_top])
        ...
        iApply (wp_addi4_s_sconf (mword_of_int (KernelSyms.iget + 0x5e)) Ra0 Ra0
                  (mword_of_int 2590 : mword 12) L5 (trap_res b + (K - 6))%nat false
                  ltac:(nz) ltac:(rdok) with "Hcg Hpc []").
        ...
        set (L6 := <[Regidx Ra0 := regval_into_reg
                      (add_vec (L5 !!! Regidx Ra0)
                         (sign_extend' 64 (mword_of_int 2590 : mword 12)))]> L5).
        ...
        iApply (wp_jal_s_sconf (mword_of_int (KernelSyms.iget + 0x62)) Rra
                  (mword_of_int 2088312 : mword 21) L6 (trap_res b + (K - 6))%nat false
                  ltac:(nz) ltac:(rdok) ltac:(vm_compute; reflexivity)
                  with "Hcg Hpc []").
        ...
        assert (Htgtrel : add_vec (mword_of_int (KernelSyms.iget + 0x62) : mword 64)
                            (sign_extend' 64 (mword_of_int 2088312 : mword 21))
                          = mword_of_int KernelSyms.release) by pcw.
        ...
        iEval (rewrite Houtb) in "Hcg".
        iApply (Release.wp_release_sconf KT1 γl itable_lock "itable"%string
                  (itable_res2 fsc_ic fsc_fs γi fsc_cov fsc_logst nib dev) L7
                  n eb p (K - 6)%nat ({["itable"]} ∪ lks)
