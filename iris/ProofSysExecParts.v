(* ProofSysExecParts.v -- sys_exec's blocks, everything but the kexec call.
   sys_exec is the marshaller in front of kexec and the last function of
   sysfile.c.
   ...
   EVERY BLOCK OF sys_exec EXCEPT THE TWO THAT CALL kexec.  The pure side
   conditions, the zero round trip, the sixty-slot frame carve and join,
   the epilogue, and -- inside [SysExecParts], a functor over the SEVEN
...
(* ===================================================================== *)
(*  THE BODY'S BLOCKS.  Seven functor arguments, one per callee; the       *)
(*  eighth callee, [Kexec], is the one this file does not have.            *)
(* ===================================================================== *)
Module SysExecParts (Argaddr : ARGADDR) (Argstr : ARGSTR) (Memset : MEMSET)
...
Section SysExecHead.
  ...
  Proof using .
    ...
    assert (Htaa : add_vec (mword_of_int (SX + 0xe) : mword 64)
                     (sign_extend' 64 (mword_of_int 2085984 : mword 21))
                   = mword_of_int KernelSyms.argaddr) by pcw.
    iApply (wp_jal_s_sconf (mword_of_int (SX + 0xe)) Rra
              (mword_of_int 2085984 : mword 21) M4 (K - 60)%nat b
              ltac:(nz) ltac:(rdok)
    ...
    assert (Htas : add_vec (mword_of_int (SX + 0x1c) : mword 64)
                     (sign_extend' 64 (mword_of_int 2085998 : mword 21))
                   = mword_of_int KernelSyms.argstr) by pcw.
    iApply (wp_jal_s_sconf (mword_of_int (SX + 0x1c)) Rra
              (mword_of_int 2085998 : mword 21) M9 (K - 60)%nat b
              ltac:(nz) ltac:(rdok)
...
Section SysExecSetup.
  ...
  Proof using .
    ...
    assert (Htms : add_vec (mword_of_int (SX + 0x42) : mword 64)
                     (sign_extend' 64 (mword_of_int 2078726 : mword 21))
                   = mword_of_int KernelSyms.memset) by pcw.
    iApply (wp_jal_s_sconf (mword_of_int (SX + 0x42)) Rra
              (mword_of_int 2078726 : mword 21) N4 (K - 60)%nat b
              ltac:(nz) ltac:(rdok)
...
Section SysExecFree.
  ...
  (* not [Local]: the break block, which lives with the composition that
     names its kexec, uses both of these. *)
  Lemma sx_off0 `{XI : CurCtx} (x : mword 64) :
  ...
  Lemma sx_zreg0 `{XI : CurCtx} : (zero_reg : mword 64) = mword_of_int 0.
  ...
  Proof using . rewrite seq_app big_sepL_app. iIntros "A B". iSplitL "A"; done. Qed.
...
Section SysExecLoop.
  ...
  Lemma sx_upd_eq `{XI : CurCtx} {A : Type} (f : nat -> A) (i : nat) (v : A) :
    sx_upd f i v i = v.
  Proof using . rewrite /sx_upd. destruct (Nat.eq_dec i i); [reflexivity | lia]. Qed.
...
Section SysExecStep.
  ...
  Proof using .
    ...
    iApply (wp_jal_s_sconf (mword_of_int (SX + 0x62) : mword 64) Rra
              (mword_of_int 2085732 : mword 21) N4 (K - 60)%nat b
              ltac:(nz) ltac:(rdok)
    ...
    iApply (wp_jal_s_sconf (mword_of_int (SX + 0x70) : mword 64) Rra
              (mword_of_int 2078270 : mword 21) Q1 (K - 60)%nat b
              ltac:(nz) ltac:(rdok)
    ...
    iApply (wp_jal_s_sconf (mword_of_int (SX + 0x82) : mword 64) Rra
              (mword_of_int 2085774 : mword 21) Q5 (K - 60)%nat b
              ltac:(nz) ltac:(rdok)
...
(* ===================================================================== *)
(*  +0x0b6 .. +0x0cc -- THE BREAK'S PURE SIDE.                            *)
(*                                                                        *)
...
Section SysExecBreakParts.
  ...
  Proof using . intro Hj. rewrite /sx_avf. apply sx_upd_lt. exact Hj. Qed.

  Lemma sx_avf_eq `{XI : CurCtx} (pg : nat -> mword 64) (i : nat) :
    sx_avf pg i i = (mword_of_int 0 : mword 64).
  Proof using . rewrite /sx_avf. apply sx_upd_eq. Qed.
  ...
  Proof using . apply bv_eq. rewrite !add_vec64_unsigned. f_equal. lia. Qed.
