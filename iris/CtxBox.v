(* CtxBox.v -- THE TRANSIT BOX, v2 (claude-notes/design/tso-escrow-endgame.md
   ...
   statements and writing the case split BEFORE building is the fix.
   ...
   THE GHOSTS (one per box; §3.2):
     ...
     slot_d   ghost_var slot_reg -- THE L1 SLOT REGISTER {| td; win; ident |},
              half in the box, half in L1's payload row.  The only state L1
              and the box share.
...
Record slot_reg (id : Type) := SlotReg {
...
Arguments SlotReg {id} _ _ _.
Arguments sr_td {id} _.
Arguments sr_win {id} _.
Arguments sr_ident {id} _.
...
Class boxG (id : Type) `{Countable id} (Σ : gFunctors) := BoxG {
  box_stampsG :: inG Σ (stampsR id);
  box_cntG    :: ghost_varG Σ nat;
  box_slotdG  :: ghost_varG Σ (slot_reg id);
...
Section box.
  Context `{!riscvGS Σ}.
  Context {id : Type} `{Countable id} `{!boxG id Σ}.
  Context {X : Type}.
  ...
  Definition slotd_half γ (r : slot_reg id) : iProp Σ := ghost_var (bx_slotd γ) (1/2) r.
  ...
  Definition out_l2 γ (s : l2_reg id) : iProp Σ :=
    (Q ∗ tok ∗
     ∃ i m, ⌜lr_hold s = Some (i, m)⌝ ∗ ⌜m ≠ ∅⌝ ∗ stamps_frag γ m)%I.
  ...
  Definition box_body γ : iProp Σ :=
    (∃ (T : nat) (ξb : CtxId) m (c : nat) (r : slot_reg id) (s : l2_reg id),
       ...
       (if sr_win r
        then hdr_out γ m ∗ ∃ x, P_rest x ξb
  ...
  Definition l1_row γ (r : slot_reg id) (ξ : CtxId) : iProp Σ :=
    (slotd_half γ r ∗ ⌜sr_win r = false⌝ ∗
  ...
  (* Caller holds: L1's register half (window shut), the cnt half at c,
     ...
     Proof skeleton:
       ...
       close: win := true (both halves in hand); hdr_out := ◯ mD;
              rows (Σ),(I),(C),(D) unchanged (m, T, r.td, r.ident, s unchanged). *)
  Lemma box_withdraw_L1 `{CID : CpuId} (N : namespace) γ (ξ : CtxId) (r : slot_reg id)
    ...
    stamps_frag γ mD ={E}=∗
    own_context ξ ∗
    slotd_half γ (SlotReg (sr_td r) true (sr_ident r)) ∗
    cnt_half γ c ∗
    ∃ x, P_hdr (sr_ident r) x ξ.
  Proof.
  Admitted.
  ...
  (* ================================================================== *)
  (* Caller holds: L1's register half with the window OPEN, the cnt half,
     and the header at the NEW identity id', x-GENERIC (F8: the arm's
     P_rest sits at a box-bound x the caller cannot see; the bcache
     discharges ∀ x at v = false, the icache at valid = 0).
     Proof skeleton:
       open; agree slot_d ⇒ win = true ⇒ hdr_out ∗ ∃ x, P_rest x ξb.
       ...
       (both halves).
       close in_arm at id' with the arm's x; rows: (Σ) by construction,
       (I) singleton at id', (C) left disjunct (T' ≤ T'), (D) via td = T'.
       export llb T' (ctx_parked_llb). *)
  Lemma box_deposit_L1 `{CID : CpuId} (N : namespace) γ (ξ : CtxId) (r : slot_reg id)
      (c : nat) (i' : id) (E : coPset) :
    ↑N ⊆ E →
    sr_win r = true →
    is_box N γ -∗
    ...
    own_context ξ ∗
    ∃ T' : nat,
      slotd_half γ (SlotReg T' false i') ∗
  ...
  Lemma box_ref_incr (N : namespace) γ (r : slot_reg id) (c : nat) (E : coPset) :
    ↑N ⊆ E →
    sr_win r = false →
    is_box N γ -∗
    slotd_half γ r -∗
  ...
  (* Caller holds: L1's register half (window shut), the cnt half at S c,
     and ONE UNIT (qsum = 1) as a reference.  It pays the unit's debt:
     td := max td (max_stamp mD); the release folds the new td by R2
     (llb of the max from the row's llb and the reference's).
     Proof skeleton:
       open; agree slot_d ⇒ win = false; the arm is untouched.
  ...
  Lemma box_ref_decr (N : namespace) γ (r : slot_reg id) (c : nat) (i : id)
    ...
    cnt_half γ (S c) -∗
    reference γ i mD ={E}=∗
    slotd_half γ (SlotReg (Nat.max (sr_td r) (max_stamp mD)) false (sr_ident r)) ∗
    cnt_half γ c ∗
  ...
  (* Caller holds: its reference (mass q > 0, keyed at i, stamps ≤ Kt by
     ...
     half with nothing held, its floor Kp ≥ tp), and floors.
     Proof skeleton:
       ...
       close out_l2 with the caller's WHOLE fragment parked and
       slot_p := {| tp; Some (i, mh) |} (both halves in hand); the
       caller keeps the register half for its handle (l2_hold).
  ...
  (* Caller holds: the bundle at identity i (the handle's cells), and the
     ...
     Proof skeleton:
       ...
       win = false, out_l2 : agree slot_p ⇒ the arm's fragment IS (i, mh);
                    take Q, tok, ◯ mh.  (I) on mh ⇒ i = sr_ident r.
  ...
  Lemma box_alloc `{CID : CpuId} (N : namespace) (ξ : CtxId) (i0 : id) (E : coPset) :
    ...
    ∃ (γ : box_names) (T_boot : nat),
      is_box N γ ∗
      slotd_half γ (SlotReg T_boot false i0) ∗ llb loglen_name T_boot ∗
      cnt_half γ 0 ∗
  ...
  Lemma l1_row_fold γ (r : slot_reg id) (ξ : CtxId) :
    sr_win r = false →
    slotd_half γ r -∗ ctx_floor ξ (sr_td r) -∗ llb loglen_name (sr_td r) -∗
    l1_row γ r ξ.
  Proof. iIntros (Hw) "Hd #Hfl #Hllb". iFrame "Hd Hfl Hllb". by iPureIntro. Qed.
