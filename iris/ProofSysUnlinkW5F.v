(* ProofSysUnlinkW5F.v -- the unlink AU walk's BLOCK W5-FILE: the
   zeroing writei, ip->nlink--, iupdate(ip) and the success epilogue, on
   the +0x8a seam's [isdir = false] arm.

     W5-FILE  +0x8a .. +0xe0

   [ProofSysUnlinkPure.su_w5_file]'s copy-adapt, at [SysUnlinkDefs]'s
...
Module SysUnlinkW5F (Ilock : ILOCK) (Memset : MEMSET) (Writei : WRITEI)
...
Section ProofSysUnlinkW5F.
  ...
  Proof using .
    ...
    iApply (wp_jal_s_sconf (CID := D4) (mword_of_int (SU + 0x94)) Rra
              (mword_of_int 2079666 : mword 21) A4 (K - 30)%nat b
    ...
    assert (Hjms : add_vec (mword_of_int (SU + 0x94) : mword 64)
                     (sign_extend' 64 (mword_of_int 2079666 : mword 21))
