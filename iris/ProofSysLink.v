...
Section ProofSysLinkBody.
  ...
  Lemma sl_esc_acc (gi : gname)
      (logstart : Z) (k : nat) :
    (k < NINODE)%nat ->
    (ic_escrows fsc_ic fsc_fs gi fsc_cov logstart -∗ ic_escrow fsc_ic fsc_fs gi fsc_cov logstart k
     : iProp Σ).
  Proof.
    ...
    - (* ------------ the FIRST string fetched: fall through ------------ *)
      ...
      + (* ========== BOTH strings fetched: on to begin_op ========== *)
        ...
        iApply (wp_jal_s_sconf (CID := CID19) (mword_of_int (SL + 0x32)) Rra
                  (mword_of_int 2092488 : mword 21) N4 (K - 38)%nat b
                  ltac:(nz) ltac:(rdok) ltac:(vm_compute; reflexivity)
                  with "Hcg Hpc []").
        ...
        iApply (BeginOp.wp_begin_op_sconf (CID := CID20) gs j gl bn g fsc_fs fsc_cov
        ...
        iApply (wp_jal_s_sconf (CID := CID22) (mword_of_int (SL + 0x3a)) Rra
                  (mword_of_int 2092002 : mword 21) Q1 (K - 38)%nat b
                  ltac:(nz) ltac:(rdok) ltac:(vm_compute; reflexivity)
                  with "Hcg Hpc []").
        ...
        iApply (Namei.wp_namei_gen (CID := CID23) gs j gl gu gd gk pd pav pu bn
                  g gi gtl γa γf logstart bmapstart inodestart nib
        ...
        * (* ---------- the path RESOLVED ---------- *)
          ...
          iDestruct (sl_esc_acc gi logstart kk Hkk with "Hescrows")
            as "#Hesck".
          ...
          iApply (wp_jal_s_sconf (CID := CID26) (mword_of_int (SL + 0x42)) Rra
                    (mword_of_int 2089806 : mword 21) Q3 (K - 38)%nat b
                    ltac:(nz) ltac:(rdok) ltac:(vm_compute; reflexivity)
                    with "Hcg Hpc []").
          ...
          assert (Hjil : add_vec (mword_of_int (SL + 0x42) : mword 64)
                           (sign_extend' 64 (mword_of_int 2089806 : mword 21))
          ...
          iApply (Ilock.wp_ilock_tx_sconf (CID := CID27) gs j gl gu gd gk pd pav pu
                    bn gi gil gisl logstart inodestart nib
          ...
          -- (* ======== ARM C: it IS a directory ======== *)
             ...
             iAssert (ic_loaded fsc_fs gi fsc_cov logstart kk inum dn bm)
             ...
             iApply (Tails.sl_tail_c (CID0 := CID31) gs j gl gu gd gk pd pav pu
                       bn g gi gtl gil gisl logstart bmapstart
          ...
          -- (* ======== not a directory: on to the NLINK_MAX guard ======== *)
             ...
             ++ (* ======== the guard PASSES: the mint ======== *)
                ...
                iAssert (inode_map fsc_fs (ientry kk) bm)
                ...
                iApply (wp_jal_s_sconf (CID := CID39) (mword_of_int (SL + 0x66)) Rra
                          (mword_of_int 2089590 : mword 21) S1 (K - 38)%nat b
                          ltac:(nz) ltac:(rdok) ltac:(vm_compute; reflexivity)
                          with "Hcg Hpc []").
                ...
                assert (Hjiu : add_vec (mword_of_int (SL + 0x66) : mword 64)
                                 (sign_extend' 64 (mword_of_int 2089590 : mword 21))
                ...
                assert (Hdirlen : length (bm_dir bm) = NDIRECT)
                  by exact (blkmap_wf_dir_len fsc_cov logstart bm (proj1 Hiok)).
                sl_own_transport CID28 CID40 eb pj b.
                iApply (Iupdate.wp_iupdate_link (CID := CID40) gs j gl gu gd gk
                          pd pav pu bn g gi logstart inodestart nib dev
                          ...
                          (* NO REGISTER VALUE IS CHOSEN HERE (lane G5).  The
                             ...
                             value, and only create's fresh child does; every
                             other raising flush stands at [None] and takes
                             the value the register already holds.  Which is
                             the whole reason the old re-valuation step is
                             gone: [ip]'s value is already whatever its own
                             record says, and ARM C refused [T_DIR] at +0x4c,
                             so the fragment that comes back is [TFile] --
                             exactly what a NAME record must carry. *)
                          true None
                          pid
                ...
                { destruct vtok as [| pv]; [reflexivity |].
                  exfalso. rewrite /InodeRegion.ireg_reg_ok in Hvok.
                  rewrite /sl_incnl sl_setnl_type in Hvok.
                  exact (sl_tdir_zne _ Hty Hvok). }
                (* ...and the pile is ONE unit: [ip] is not a directory. *)
                assert (Hdelta1 : InodeRegion.ireg_dot_delta
                                    (bv_unsigned (di_type dn))
                                    (bv_unsigned (di_nlink dn)) = 1%nat)
                  by exact (InodeRegion.ireg_dot_delta_not_dir _ _
                              (sl_tdir_zne _ Hty)).
                iEval (rewrite Hdelta1 FsStateLink.link_reps_1) in "Htoken".
                ...
                iAssert (dlinks fsc_fs (bv_unsigned inum) (sl_incnl dn) bm dat)
                  as "Hdlnk2".
                { iApply (dlinks_not_dir fsc_fs (bv_unsigned inum) (sl_incnl dn) bm dat).
                ...
                { apply (inode_local_of_ok_rec (bv_unsigned inum) fsc_cov logstart
                           (sl_incnl dn) bm dat
                           (sl_setnl_inode_ok fsc_cov logstart dn bm dat _ Hiok)
                ...
                iModIntro.
                iAssert (ic_loaded fsc_fs gi fsc_cov logstart kk inum (sl_incnl dn) bm)
                  with "[Hdlnk2 Hdiat Hmeta Hmap Hblocks Hdview Hfview Htop]"
                  as "Hload".
                { iApply ic_loaded_flat; rewrite /ic_loaded_flat_body. iExists dat.
                  iSplitR;
                    [iPureIntro; exact (sl_setnl_inode_ok fsc_cov logstart dn bm dat _ Hiok) |].
                  iSplitR; [iPureIntro;
                ...
                iApply (wp_jal_s_sconf (CID := CID42) (mword_of_int (SL + 0x6c)) Rra
                          (mword_of_int 2089938 : mword 21) S3 (K - 38)%nat b
                          ltac:(nz) ltac:(rdok) ltac:(vm_compute; reflexivity)
                          with "Hcg Hpc []").
                ...
                assert (Hjul : add_vec (mword_of_int (SL + 0x6c) : mword 64)
                                 (sign_extend' 64 (mword_of_int 2089938 : mword 21))
                ...
                iApply (Iunlock.wp_iunlock_tx_sconf (CID := CID43) gs gi gil
                          gisl logstart kk (qq/2)%Qp gsh dev inum
                ...
                iApply (wp_jal_s_sconf (CID := CID46) (mword_of_int (SL + 0x78)) Rra
                          (mword_of_int 2091966 : mword 21) T1 (K - 38)%nat b
                          ltac:(nz) ltac:(rdok) ltac:(vm_compute; reflexivity)
                          with "Hcg Hpc []").
                ...
                assert (Hjnp : add_vec (mword_of_int (SL + 0x78) : mword 64)
                                 (sign_extend' 64 (mword_of_int 2091966 : mword 21))
                ...
                iApply (Nameiparent.wp_nameiparent_gen (CID := CID47) gs j gl gu
                          gd gk pd pav pu bn g gi gtl γa γf logstart
                ...
                ** (* ---------- the parent RESOLVED ---------- *)
                   ...
                   iDestruct (sl_esc_acc gi logstart kd Hkd
                   ...
                   iApply (wp_jal_s_sconf (CID := CID50) (mword_of_int (SL + 0x80))
                             Rra (mword_of_int 2089744 : mword 21) T3 (K - 38)%nat b
                             ltac:(nz) ltac:(rdok) ltac:(vm_compute; reflexivity)
                             with "Hcg Hpc []").
                   ...
                   assert (Hjild : add_vec (mword_of_int (SL + 0x80) : mword 64)
                                     (sign_extend' 64 (mword_of_int 2089744 : mword 21))
                                   = mword_of_int KernelSyms.ilock) by pcw.
                   ...
                   iApply (Ilock.wp_ilock_tx_sconf (CID := CID51) gs j gl gu gd gk pd
                             pav pu bn gi gild gisld logstart inodestart
                   ...
                   { (* ===== ARM E2: THE PARENT IS ORPHANED -- goto bad ===== *)
                     ...
                     iDestruct (ic_mk_loaded fsc_fs gi fsc_cov logstart kd dinum dnd bmd
                   ...
                   iApply (wp_lw_s_sconf (kt := KT1) (ktd := KT0) (CID := CID53) (mword_of_int (SL + 0x8c))
                             ...
                             dev b ltac:(nz) ltac:(rdok) with "Hcg Hpc [] [Hidevd]").
                   ...
                   iApply (wp_clw_s_sconf (kt := KT1) (ktd := KT0) (CID := CID54) (mword_of_int (SL + 0x90))
                             ...
                             dev b ltac:(nz) ltac:(rdok) with "Hcg Hpc [] [Hipdev]").
                   ...
                   iApply (wp_jal_s_sconf (CID := CID58) (mword_of_int (SL + 0x9c))
                             Rra (mword_of_int 2091734 : mword 21) U5 (K - 38)%nat b
                             ltac:(nz) ltac:(rdok) ltac:(vm_compute; reflexivity)
                             with "Hcg Hpc []").
                   ...
                   assert (Hjdl : add_vec (mword_of_int (SL + 0x9c) : mword 64)
                                    (sign_extend' 64 (mword_of_int 2091734 : mword 21))
                   ...
                   iDestruct (sl_bs3 with "[Hbs1d Hbs2d]") as "Hbsl";
                     [iSplitL "Hbs1d"; [iExact "Hbs1d" | iExact "Hbs2d"] |].
                   iAssert (inode_map fsc_fs (ientry kd) bmd)
                   ...
                   iApply (Dirlink.wp_dirlink_gen (CID := CID59) gs j gl gu gd gk pd
                             pav pu bn g gi gtl γa γf γpr logstart
                   ...
                   --- (* ============ ARM F-FOUND: the name was there ======= *)
                       ...
                       iAssert (ic_loaded fsc_fs gi fsc_cov logstart kd dinum dnd bmd)
                   ...
                   --- (* ============ the APPEND arm ========================= *)
                       ...
                       assert (Hdiok' : inode_ok fsc_cov logstart dnd' bmd' datd').
                       ...
                       ++++ (* ====== ARM G: the whole record went in ====== *)
                            ...
                            (* THE DOT NAMES ARE MISSED (durable-disk G5).
                               [dirlink]'s own [dirlookup] came back empty and
                               a live directory's records 0 and 1 are the two
                               dot names ([DirView.dir_dots_miss_not_dots]) --
                               so the appended name is a NAME name, which is
                               what both the marker set and the register value
                               below need. *)
                            ...
                            assert (Hnfd' : bname 14 nf <> DOT)
                              by (rewrite FsStateEra.DOT_dot; exact Hnd1).
                            assert (Hnfdd' : bname 14 nf <> DOTDOT)
                              by (rewrite FsStateEra.DOTDOT_dotdot; exact Hnd2).
                            assert (Hdzd : bv_unsigned (di_type dnd) = T_DIR_z)
                              by (clear -Htyd; rewrite Htyd;
                                  vm_compute; reflexivity).
                            (* the appended name is not MARKED: it is not an
                               entry at all yet. *)
                            assert (HsD : bname 14 nf ∉ Dd).
                            { intros Hin. destruct (Hdokd _ Hin) as ([tt Htt] & _ & _).
                            ...
                            iDestruct (ent_toks_dirlink_arm
                                         (fs_gamma_L fsc_fs) (bv_unsigned dinum)
                            ...
                            { iEval (rewrite -Hli) in "Htoken".
                              iApply (ent_tok_of_link (fs_gamma_L fsc_fs)
                                        (bv_unsigned dinum)
                                        (fn_dd (era_node dnd bmd datd))
                                        (fn_orphan (era_node dnd bmd datd))
                                        false
                                        (bname 14 nf)
                                        (bv_unsigned (sl_low16 inum))
                                        vtok
                                        ltac:(apply FsStateInode.ent_ty_ok_name;
                                              [exact Hnfd' | exact Hnfdd'
                                               | exact Hvfile])
                                        with "Htoken"). }
                            assert (Hgrowd := dir_entries_dirlink_grow
                                       dnd dnd' bmd bmd' datd datd'
                                       (sl_low16 inum) (bname 14 nf)
                                       (dir_nrec (bv_unsigned (di_size dnd)))
                                       (dir_slot datd
                                       ...
                                       (proj1 (proj2 (proj2 (proj2 (proj2
                                          Hdiok')))))).
                            iDestruct (dlinks_intro _ _ _ _ _ Dd
                                         ltac:(exact (FsStateInode.ent_dset_ok_grow
                                                        _ _ Dd Hgrowd Hdokd))
                                         ltac:(exact (FsStateInode.node_exact_cong
                                                        (era_node dnd bmd datd)
                                                        (era_node dnd' bmd' datd')
                                                        Dd
                                                        ltac:(rewrite /fn_is_dir
                                                                /fn_type
                                                                !era_node_rec
                                                                Htyeq //)
                                                        ltac:(rewrite /fn_nlink
                                                                !era_node_rec
                                                                Hnleq //)
                                                        Hxactd))
                                         with "Hetkd'")
                            ...
                            iApply (wp_jal_s_sconf (CID := CID62)
                                      ...
                                      (mword_of_int 2090302 : mword 21) W0
                                      (K - 38)%nat b ltac:(nz) ltac:(rdok)
                                      ltac:(vm_compute; reflexivity)
                                      with "Hcg Hpc []").
                            ...
                            assert (Hjupd : add_vec
                                      ...
                                      (sign_extend' 64 (mword_of_int 2090302 : mword 21))
                            ...
                            iApply (Iunlockput.wp_iunlockput_tx_gen (CID := CID63) gs j
                                      ...
                                      gisld logstart bmapstart inodestart nib
                            ...
                            iApply (wp_jal_s_sconf (CID := CID65)
                                      ...
                                      (mword_of_int 2090086 : mword 21) W2
                                      (K - 38)%nat b ltac:(nz) ltac:(rdok)
                                      ltac:(vm_compute; reflexivity)
                                      with "Hcg Hpc []").
                            ...
                            assert (Hjip : add_vec
                                      ...
                                      (sign_extend' 64 (mword_of_int 2090086 : mword 21))
                            ...
                            iApply (wp_jal_s_sconf (CID := CID67)
                                      ...
                                      (mword_of_int 2092502 : mword 21) mip
                                      (K - 38)%nat b ltac:(nz) ltac:(rdok)
                                      ltac:(vm_compute; reflexivity)
                                      with "Hcg Hpc []").
                            ...
                            iApply (EndOp.wp_end_op_sconf (CID := CID68) gs j gl gu gd
                                      gk pd pav pu bn g fsc_fs fsc_cov logstart dev n5 pid
                       ...
                       ++++ (* ====== ARM F-0: the EMPTY append ====== *)
                            ...
                            iDestruct (ent_toks_dirlink_nop
                                         (fs_gamma_L fsc_fs) (bv_unsigned dinum)
                                         dnd dnd' bmd bmd' datd datd'
                                         (fun j => dirent_bytes
                                            (de_of_name (sl_low16 inum)
                                               (bname 14 nf)) !!! j)
                                         (dir_nrec (bv_unsigned (di_size dnd)))
                                         (dir_slot datd
                                            (dir_nrec (bv_unsigned (di_size dnd))))
                                         tot Dd eq_refl (dir_slot_le _ _) Htot0
                                         Htyeq Hnleq Hszmax Hrng
                                         (proj1 (proj2 (proj2 (proj2 (proj2
                                            (proj2 Hdiok))))))
                                         Hholes3
                                         (proj1 (proj2 (proj2 (proj2 (proj2
                                         ...
                                         (proj1 (proj2 (proj2 (proj2 (proj2
                                            Hdiok')))))
                                         with "Hetkd") as "Hetkd'".
                            assert (Heqentd := dir_entries_dirlink_nop_eq
                                       dnd dnd' bmd bmd' datd datd'
                                       (fun j => dirent_bytes
                                          (de_of_name (sl_low16 inum)
                                             (bname 14 nf)) !!! j)
                                       ...
                                       (dir_slot datd
                                          (dir_nrec (bv_unsigned (di_size dnd))))
                                       tot eq_refl (dir_slot_le _ _) Htot0
                                       Htyeq Hszmax Hrng
                                       (proj1 (proj2 (proj2 (proj2 (proj2
                                          (proj2 Hdiok))))))
                                       Hholes3
                                       ...
                                       (proj1 (proj2 (proj2 (proj2 (proj2
                                          Hdiok')))))).
                            assert (Hgrowd0 : forall s',
                                       is_Some (dir_entries
                                                  (era_node dnd bmd datd) !! s')
                                       -> is_Some (dir_entries
                                                     (era_node dnd' bmd' datd')
                                                     !! s'))
                              by (intros s' Hs'; rewrite Heqentd; exact Hs').
                            iDestruct (dlinks_intro _ _ _ _ _ Dd
                                         ltac:(exact (FsStateInode.ent_dset_ok_grow
                                                        _ _ Dd Hgrowd0 Hdokd))
                                         ltac:(exact (FsStateInode.node_exact_cong
                                                        (era_node dnd bmd datd)
                                                        (era_node dnd' bmd' datd')
                                                        Dd
                ...
                ** (* ---------- ARM E: no parent -- goto bad ---------- *)
                   ...
                   iApply (Tails.sl_tail_bad (CID0 := CID50) gs j gl gu gd gk pd
                             pav pu bn g gi gtl gil gisl logstart
        ...
        * (* ---------- ARM B: the path did not resolve ---------- *)
          ...
          iApply (Tails.sl_tail_b (CID0 := CID26) gs j gl gu gd gk pd pav pu bn
                    g logstart dev n1 pid (DfracOwn (1/4)) m Q3 sp0 K eb
