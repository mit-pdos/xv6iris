...
(* the same at the record: two descriptor moves collapse, and the image the
...
Section ProofSysLinkBody.
 ...
 dqb dqs dqbs v0 v1 pid U
  ...
  Proof.
    ...
    iApply (Argstr.wp_argstr_sconf (CID := CID8) fsc_kalloc γf M6 (K - 38)%nat 0%nat eb
              pj 0%nat v0 pid U 128%nat bo0 b lks
    ...
    - (* ------------ the FIRST string fetched: fall through ------------ *)
      ...
      + (* ========== BOTH strings fetched: on to begin_op ========== *)
        ...
        iApply (Namei.wp_namei_gen (CID := CID23) gs j gl pd pav pu
 γf
 ...
 kk (qq/2)%Qp (qq/2)%Qp gsh losh tlsh
             ...
             ++ (* ======== the guard PASSES: the mint ======== *)
                ...
                iApply (Iupdate.wp_iupdate_link (CID := CID40) gs j gl
                          pd pav pu
                ...
                { apply (inode_local_of_ok_rec (bv_unsigned inum) fsc_cov fsc_logst
                           (sl_incnl dn) bm dat
                           (sl_setnl_inode_ok fsc_cov fsc_logst dn bm dat _ Hiok)
                ...
                { iApply ic_loaded_flat; rewrite /ic_loaded_flat_body. iExists dat.
                  iSplitR;
                    [iPureIntro; exact (sl_setnl_inode_ok fsc_cov fsc_logst dn bm dat _ Hiok) |].
                  iSplitR; [iPureIntro;
                ...
                iApply (Nameiparent.wp_nameiparent_gen (CID := CID47) gs j gl
 pd pav pu γf
                ...
                assert (Hmem2 : IBLOCK inum icfg_ist
                                ∈ (Sb1 ∪ {[IBLOCK inum icfg_ist]}))
                  by (apply elem_of_union_r, elem_of_singleton; reflexivity).
                assert (Hmem2' : IBLOCK inum icfg_ist ∈ Sb2)
 ...
 kd (qd/2)%Qp gyd lod tld PlainK dinum pid
                   ...
                   assert (HU2a4 : (U2 !!! Regidx Ra4 : mword 64)
                                   = (sign_extend' 64 icfg_dev : mword 64))
                   ...
                   assert (Hcrok2 : sl_crok (bool_decide (fsc_bmapstart ∈ Sb2)) w1 w2).
                   { intro Hf. apply bool_decide_eq_false in Hf. split.
                     ...
                     - destruct w2; [| reflexivity]. exfalso. apply Hf.
                       exact (Hw2 eq_refl). }
                   assert (Hlow16 : bv_unsigned (sl_low16 inum) < 16 * Z.of_nat icfg_nib).
                   ...
                   iApply (Dirlink.wp_dirlink_gen (CID := CID59) gs j gl pd
                             pav pu γf
 (ientry kd)
                   ...
                   assert (Hcrok3 : sl_crok (bool_decide (fsc_bmapstart ∈ Sb3)) w1 w2).
                   { intro Hf. apply bool_decide_eq_false in Hf. split.
                     - destruct w1; [| reflexivity]. exfalso. apply Hf.
                   ...
                   assert (Hcrimp : bool_decide (fsc_bmapstart ∈ Sb2) = true ->
                                    bool_decide (fsc_bmapstart ∈ Sb3) = true).
                   { intro Ht. apply bool_decide_eq_true.
                     apply HSb3. exact (proj1 (bool_decide_eq_true _) Ht). }
                   assert (Hmem3 : IBLOCK inum icfg_ist ∈ Sb3)
                     by exact (HSb3 _ Hmem2').
                   iAssert (inode_ref_short kk (qq/2 + qq/2)%Qp (qq/2)%Qp icfg_dev inum)
                   ...
                   --- (* ============ the APPEND arm ========================= *)
                       ...
                       assert (Hdiok' : inode_ok fsc_cov fsc_logst dnd' bmd' datd').
