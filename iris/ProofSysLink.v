  ...
  Proof.
    ...
    - (* ------------ the FIRST string fetched: fall through ------------ *)
      ...
      + (* ========== BOTH strings fetched: on to begin_op ========== *)
        ...
        iApply (BeginOp.wp_begin_op_sconf (CID := CID20) gs j gl bn g fsc_fs cov
        ...
        iApply (Namei.wp_namei_gen (CID := CID23) gs j gl gu gd gk pd pav pu bn
                  g gi gtl γa γf cov logstart bmapstart inodestart nib
        ...
        * (* ---------- the path RESOLVED ---------- *)
          ...
          iDestruct (sl_esc_acc gi cov logstart kk Hkk with "Hescrows")
            as "#Hesck".
          ...
          iApply (Ilock.wp_ilock_tx_sconf (CID := CID27) gs j gl gu gd gk pd pav pu
                    bn gi gil gisl cov logstart inodestart nib
          ...
          -- (* ======== ARM C: it IS a directory ======== *)
             ...
             iAssert (ic_loaded fsc_fs gi cov logstart kk inum dn bm)
             ...
             iApply (Tails.sl_tail_c (CID0 := CID31) gs j gl gu gd gk pd pav pu
                       bn g gi gtl gil gisl cov logstart bmapstart
          ...
          -- (* ======== not a directory: on to the NLINK_MAX guard ======== *)
             ...
             ++ (* ======== the guard PASSES: the mint ======== *)
                ...
                iApply (Iupdate.wp_iupdate_link (CID := CID40) gs j gl gu gd gk
                          pd pav pu bn g gi cov logstart inodestart nib dev
                ...
                iModIntro.
                iAssert (ic_loaded fsc_fs gi cov logstart kk inum (sl_incnl dn) bm)
                ...
                iApply (Nameiparent.wp_nameiparent_gen (CID := CID47) gs j gl gu
                          gd gk pd pav pu bn g gi gtl γa γf cov logstart
                ...
                ** (* ---------- the parent RESOLVED ---------- *)
                   ...
                   iDestruct (sl_esc_acc gi cov logstart kd Hkd
                   ...
                   iApply (Ilock.wp_ilock_tx_sconf (CID := CID51) gs j gl gu gd gk pd
                             pav pu bn gi gild gisld cov logstart inodestart
                   ...
                   { (* ===== ARM E2: THE PARENT IS ORPHANED -- goto bad ===== *)
                     ...
                     iDestruct (ic_mk_loaded fsc_fs gi cov logstart kd dinum dnd bmd
                     ...
                     iApply (Tails.sl_tail_e2 (CID0 := CIDg1) gs j gl gu gd gk pd
                               pav pu bn g gi gtl gil gisl gild gisld cov
                   ...
                   iApply (Dirlink.wp_dirlink_gen (CID := CID59) gs j gl gu gd gk pd
                             pav pu bn g gi gtl γa γf γpr cov logstart
                   ...
                   --- (* ============ the APPEND arm ========================= *)
                       ...
                       ++++ (* ====== ARM G: the whole record went in ====== *)
                            ...
                            iApply (Iput.wp_iput_sconf (CID := CID66) gs j gl gu gd gk
                                      pd pav pu bn g gi gtl gil gisl cov
                                      logstart bmapstart inodestart nib size dev
                            ...
                            iApply (EndOp.wp_end_op_sconf (CID := CID68) gs j gl gu gd
                                      gk pd pav pu bn g fsc_fs cov logstart dev n5 pid
                ...
                ** (* ---------- ARM E: no parent -- goto bad ---------- *)
                   ...
                   iApply (Tails.sl_tail_bad (CID0 := CID50) gs j gl gu gd gk pd
                             pav pu bn g gi gtl gil gisl cov logstart
        ...
        * (* ---------- ARM B: the path did not resolve ---------- *)
          ...
          iApply (Tails.sl_tail_b (CID0 := CID26) gs j gl gu gd gk pd pav pu bn
                    g cov logstart dev n1 pid (DfracOwn (1/4)) m Q3 sp0 K eb
