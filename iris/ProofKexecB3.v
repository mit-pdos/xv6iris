...
Section KexecB3Seam.
  ...
  Definition kxc_at_11a
    ...
    (⌜ M !!! Regidx csp_rs1 = pa_stk sp0 68 /\
     ...
     kalloc_env ga None ∗
     kxc_res jp bn g gi gf logstart bmapstart inodestart size dev
...
Section KexecB3Incr.
  ...
  Lemma kxc_incr `{CID0 : CpuId}
    ...
    kernel_text -∗
    kxc_at_11a jp bn g gi ga gf logstart bmapstart inodestart nib
    ...
    wp_next true (proc_addr jp) (fun (CID : CpuId) =>
      ∀ (M' : regfile),
        ( kxc_at_12c jp bn g gi ga gf logstart bmapstart inodestart
          ...
          ∨ kxc_at_1a4 jp bn g gi ga gf logstart bmapstart inodestart
                       nib size dev kf qf sf gyf inumf dnf bmf
                       gilf gislf n2 plen pfun na avf aslen afun pidv V eb
...
Section KexecB3Body.
  ...
  Lemma kxc_ph_step `{CID0 : CpuId}
    ...
    kxc_at_12c jp bn g gi ga gf logstart bmapstart inodestart nib
               size dev kf qf sf gyf inumf dnf bmf gilf gislf n2
  ...
  Proof.
    ...
    iApply (Readi.wp_readi_sconf KT1 gs jp gl gu gd gk pd pav pu bn ga gf
              logstart dev (ientry kf) bmf datl dnf false offn 56%nat phb V
    ...
    - (* ================ THE HEADER ARRIVED ======================== *)
      ...
      + (* ---- NOT PT_LOAD: nothing to do, take the back edge ---- *)
        ...
        iDestruct (cpu_claim_ext_transport CIDrd CIDg4 eb (proc_addr jp)
                     ltac:(try rewrite Hebb; wp_next_chain) with "Hclmc") as "Hclmc".
        iApply (kxc_incr (CID0 := CIDg4) jp bn g gi ga gf logstart
      ...
      + (* ================ PT_LOAD: load the segment ================ *)
        ...
        * (* memsz < filesz -- malformed *)
          ...
          iApply (B2.kxc_bad324 (CID0 := CIDy2) Q gs jp gl gu gd gk pd pav pu bn g
                    gi gtl gilf gislf ga gf logstart bmapstart
             ...
             ++ iApply (wp_bnez_x0_fall_s_sconf (mword_of_int (KXB + 0x168))
                ...
                ** (* ---- kalloc failed: [bad:] at +0x352 ---- *)
                   ...
                   iApply (B2.kxc_bad324 (CID0 := CIDw3) Q gs jp gl gu gd gk pd
                             pav pu bn g gi gtl gilf gislf ga gf
                   ...
                   { iApply (A.fs_fabric_mk with "[%] Hkd Hpenv Hbio Hlogc Hcrash Hcert Hitab
                                                  ...
                                                  Hdevi Hdgeom Hdlock"). exact Hclogf. }
                ** (* ---- the table grew ---- *)
                   ...
                   --- (* ---- AN EMPTY SEGMENT: skip loadseg ---- *)
                       ...
                       iApply (kxc_incr (CID0 := CIDv3) jp bn g gi ga gf
                                 logstart bmapstart inodestart nib size dev
                   ...
                   --- (* ---- A NON-EMPTY SEGMENT: run the loadseg loop ---- *)
                       ...
                       iApply (B2.kxc_ls (CID0 := CIDv5) Q gs jp gl gu gd gk pd pav
                                 pu bn g gi gtl gilf gislf ga gf
                                 logstart bmapstart inodestart nib size dev
                                 kf qf sf gyf inumf dnf bmf n2 plen pfun na
                       ...
                       iDestruct (cpu_claim_ext_transport CIDq1 CIDq2 eb (proc_addr jp)
                                    ltac:(try rewrite Hebb; wp_next_chain) with "Hclmc") as "Hclmc".
                       iApply (kxc_incr (CID0 := CIDq2) jp bn g gi ga gf
                                 logstart bmapstart inodestart nib size dev
                                 kf qf sf gyf inumf dnf bmf gilf
                                 gislf n2 plen pfun na avf aslen afun pidv V eb
...
Section KexecB3Close.
  ...
  Lemma kxc_seam1a2 `{CID0 : CpuId}
      ...
      (ef : nat -> bv 8) (P : uptd) :
    kernel_text -∗
    kxc_at_1a2 jp bn g gi ga gf logstart bmapstart inodestart nib
               size dev kf qf sf gyf inumf dnf bmf gilf gislf n2
               plen pfun na avf aslen afun pidv V eb dqb dqs dqa dqpv dqas m M K
               sp0 ra0 s00 s10 s20 pv av
  ...
  Lemma kxc_close `{CID0 : CpuId}
      ...
      (sp0 ra0 s00 s10 s20 pv av : mword 64)
      (w5 w6 w7 w8 w9 w10 w11 w12 w13 w67 : mword 64)
      (ef : nat -> bv 8) (P : uptd) (szv sv11 : mword 64) :
    (K_kexec <= K)%nat ->
    log_geom_ok fsc_cov logstart ->
    0 < size <= BPB ->
    0 <= bmapstart ->
    ...
    fs_fabric gs gu gd gk pd pav pu bn g gi gtl
              logstart inodestart nib dev -∗
    kxc_at_1a4 jp bn g gi ga gf logstart bmapstart inodestart nib
               size dev kf qf sf gyf inumf dnf bmf gilf gislf n2
               plen pfun na avf aslen afun pidv V eb dqb dqs dqa dqpv dqas M K
               sp0 ra0 s00 s10 s20 pv av
    ...
    wp_next true (proc_addr jp) (fun (CID : CpuId) =>
      ∀ (M' : regfile),
        kxc_at_1ae jp bn ga gf logstart bmapstart inodestart size
  ...
  Proof.
    ...
    iDestruct (proc_priv_bare_acc gf (proc_addr jp) pidv V with "Hpriv")
      as "[Hppid Hpvbk]".
    iDestruct (A.kxa_esc_acc gi logstart kf Hk with "Hesc")
      as "#Hesck".
    (* ---- +0x1a4: c.mv a0,s4 ---- *)
    ...
    iApply (Iunlockput.wp_iunlockput_tx_sconf gs jp gl gu gd gk pd pav pu bn g
              gi gtl gilf gislf logstart bmapstart inodestart nib size
    ...
    iApply (EndOp.wp_end_op_sconf gs jp gl gu gd gk pd pav pu bn g fsc_fs
              fsc_cov logstart dev n3 pidv (DfracOwn (1/4)) B3 (K - 68)%nat
...
Section KexecB3Main.
  ...
  Lemma kxc_b2
      ...
      (sp0 ra0 s00 s10 s20 pv av w67 : mword 64)
      (ef : nat -> bv 8) (P : uptd) (i : nat) (szv : mword 64) :
    kxc_b2_body Q gs jp gl gu gd gk pd pav pu bn g gi gtl gilf gislf
  ...
  Proof.
    ...
    iApply (kxc_phdr (CID0 := CID0) Q gs jp gl gu gd gk pd pav pu bn g gi
              gtl gilf gislf ga gf logstart bmapstart inodestart nib size
    ...
    iDestruct (wp_next_retarget CID0 CIDn true (proc_addr jp) _ Hcr
                 with "Hc1ae") as "Hc1ae".
    iApply (kxc_close (CID0 := CIDn) gs jp gl gu gd gk pd pav pu bn g gi
              gtl gilf gislf ga gf logstart bmapstart inodestart nib size
              dev kf qf sf gyf inumf dnf bmf n2 plen pfun na avf
  ...
  Lemma kxc_b2z
      ...
      (ef : nat -> bv 8) (P : uptd) :
    kxc_b2z_body gs jp gl gu gd gk pd pav pu bn g gi gtl gilf gislf
      ga gf logstart bmapstart inodestart nib size dev
      kf qf sf gyf inumf dnf bmf n2 plen pfun na avf alen aslen afun
  ...
  Proof.
    ...
    iApply (kxc_seam1a2 (CID0 := CID0) jp bn g gi ga gf logstart
