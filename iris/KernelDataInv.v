...
(* NO SEAM ANY MORE.  This file used to hold the tree's one "flip ↦ₛ"
   worklist marker -- [kernel_data_string]'s conclusion was the RAW string
   tower, so its bytes left the ledger here through
   [TsoCtx.ctx_pointsto_forget].  M1 stage 3 (tso-port.md §0.21′/§0.22′)
   flipped [↦ₛ], and the crossing simply ceased to exist: [kernel_data] is
   ∀-context already and [kernel_data_string_all] hands the ∀ straight
   through.  The junk witness [MkCtxId inhabitant inhabitant] that used to
   discharge it is gone with it -- and it was the diagnostic all along
   (a lemma that instantiates a ∀ at a junk witness is a lemma whose
   conclusion is in the wrong tier). *)
...
Section KernelDataInv.
  ...
  (* Extract a NUL-terminated STRING literal from the image, AT EVERY
     ...
     the ∀ just comes along -- and [kernel_data]'s own ∀ was minted by the
     boot carve out of [TsoCtx.ctx_pointsto_of_pristine_va_all], i.e. out
     of the clean arm's [t = 0] disjunct.  It is what the persistent lock
     handles ([WpLock.lock_name] / [SleepLock.sl_name]) carry, and what
