(* ===================================================================== *)
(*  ProofKexecA.v -- PHASE A OF kexec, AT ITS CONTRACT.                 *)
(*  (SpecKexec.v section 2; design/fs-syscall-specs.md)                 *)
(* ===================================================================== *)
...
(*  PHASE A AT THE CALLER'S OWN WALK PREMISE AND THE CALLER'S OWN
    ...
    costs phase A:
      ...
      * [kxc_a1_au] is [kxc_a1p] with the pin removed: the era walk fires
        [SysOpenDefs.namei_walk_pre_era] -- the caller's one-shot,
        ...
        [namei_walk_dead_era] by conversion, [FsAbsEra.ex_hops_is_ax_hops])
      ...
      * [kxc_phaseA_au] does not RELAY a header oracle and does not ANSWER
        ...
        [ProofKexecACode]'s oracle is fired -- ilock's payload open, readi not
        ...
        [Fo.(pf_recv) av zi (abs_of (era_node dn bm data))].  A persistent claim
        cannot carry a linear receipt, which is why [ProofKexecACode] grew
        [kxc_a2_r] (the oracle's payout generic in [R]) and why the landed
        [kxc_a2] is now that lemma's corollary at the header claim.

    EVERYTHING ELSE IS ProofKexecACode's.  Its blocks are opened as [LA] and
...
Require Import ProofKexecACode.    (* the LANDED blocks, opened as [LA]  *)
...
Require SysOpenDefs.   (* [namei_walk_pre_era] / [namei_walk_dead_era]
                            / [aopen_commit_at]                      *)
...
Module T := ProofKexecTail.KexecTailProof Myproc BeginOp Namei Ilock Readi
                                          Iunlockput EndOp.
Module LA := ProofKexecACode.KexecACodeProof Myproc BeginOp Namei Ilock Readi
                                     Iunlockput EndOp.
 ...
 gf
    ...
    - (* ============ namei FAILED: the +0x088 tail ============ *)
      ...
      (* ...and kexec's AU caller wants exactly that suffix back: the era
         arm IS [namei_walk_dead_era] by conversion
...
(*  [ProofKexecACode.kxc_phaseA]'s reason exactly (the chain applies          *)
...
Section KexecAUAMain.
  ...
  (*                                                                      *)
  (*  [ProofKexecACode]'s oracle is fired with ilock's payload open and readi  *)
  ...
  (*  the WHOLE abstract node -- which is exactly [SysOpenDefs]'s        *)
  ...
  Definition kxa_receipt (Fs : pfam Σ (UexecSlot.uvis -> iProp Σ)) (P : nat -> Z -> iProp Σ)
      ...
      (sts : list fdstate)
      (* ...and the caller's children set and pid, the two identity rows of
         [SpecKexec.exec_slot_pre] (lane EXEC-SEAM), threaded like [cw] *)
  ...
  Lemma kxa_fail_dead (Fs : pfam Σ (UexecSlot.uvis -> iProp Σ))
      ...
      (Fo : pfam Σ (aview -> Z -> anode -> iProp Σ))
      (na : nat) (alen : nat -> nat) (afun : nat -> nat -> bv 8)
      (sts : list fdstate) (cs : gset gname) (pidv : mword 32) (pl : list (bv 8)) :
  ...
  (* ---- THE TWO TAILS' HONESTY (2026-09-04) --------------------------
     [SpecKexec.exec_fail_ok]'s [EfNoMem] now DEMANDS the magic passed, so
  ...
  Lemma kxc_phaseA_au
      ...
      (* the caller's children set, for [exec_slot_pre]'s identity rows
         (lane EXEC-SEAM); the pid is the process's own above *)
      (cs : gset gname)
      (dqb dqs dqa dqpv dqas : dfrac)
      (m : regfile) (K : nat) (eb : bool) (b : bool) (lks : gset string)
      (sp0 ra0 s00 s10 s20 pv av : mword 64)
      (* THE AU BUNDLE'S PARAMETERS (SpecKexec sect. 2): the walk's
 ...
 -∗
  ...
  Proof using .
    ...
    iDestruct (KexecDefs.fs_fabric_all with "Hfab")
    ...
    { (* ==== THE ORACLE'S INSTANT: the caller's commit, spent off the
      ...
      iMod (FsAbsOpenFire.opf_open_fire fsc_fs ⊤ (DfracOwn 1) Fo zi
