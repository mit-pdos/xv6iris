...
Section KexecABody.
  ...
  Lemma kxc_a1
    ...
    wp_next true (proc_addr jp) (fun (CID : CpuId) =>
      ∀ (M32 : regfile) (ipv : mword 64) (zi : Z) (n1 : nat),
        kxc_at_a2 jp bn g ga gf logstart bmapstart inodestart size
  ...
  Proof.
    ...
    iDestruct (cpu_claim_ext_transport CIDj1 CIDj2 eb (proc_addr jp)
                 ltac:(try rewrite Hebb; wp_next_chain) with "Hclmc") as "Hclmc".
    iApply (BeginOp.wp_begin_op_sconf gs jp gl bn g fsc_fs fsc_cov logstart dev
    ...
    - (* ============ namei FAILED: the +0x088 tail ============ *)
      ...
      iApply (EndOp.wp_end_op_sconf gs jp gl gu gd gk pd pav pu bn g fsc_fs
                fsc_cov logstart dev n1 pidv (DfracOwn (1/4)) P1 (K - 68)%nat
  ...
  Lemma kxc_a2
    ...
    wp_next true (proc_addr jp) (fun (CID : CpuId) =>
      ∀ (M90 : regfile) (kf : nat) (qf sf : Qp) (inumf : mword 32)
        ...
        ic_loaded fsc_fs gi fsc_cov logstart kf inumf dnf bmf -∗
  ...
  Proof.
    ...
    iDestruct (T.kxa_esc_acc gi logstart k Hk with "Hesc") as "#Hesck".
    ...
    iApply (Ilock.wp_ilock_tx_sconf gs jp gl gu gd gk pd pav pu bn gi
              gilk gislk logstart inodestart nib k (q/2)%Qp gy PlainK
              dev inum
    ...
    iApply (Readi.wp_readi_sconf KT1 gs jp gl gu gd gk pd pav pu bn ga gf
              logstart dev (ientry k) bml datl dnl false 0%nat 64%nat fb V
    ...
    - (* the read was the full 64 bytes: fall through *)
      ...
      + (* the magic matched: on to PHASE B at +0x090 *)
        ...
        iDestruct (cpu_claim_ext_transport CIDrd CID15 eb (proc_addr jp)
                     ltac:(try rewrite Hebb; wp_next_chain) with "Hclmc") as "Hclmc".
        iAssert (ic_loaded fsc_fs gi fsc_cov logstart k inum dnl bml)
      ...
      iApply (T.kxc_bad64 Q gs jp gl gu gd gk pd pav pu bn g gi gtl
                  gilk gislk ga gf logstart bmapstart inodestart nib size
