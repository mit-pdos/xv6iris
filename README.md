...
├── tools/           # dump_elf.py (ELF -> Rocq), gen_decode.py, proof_coverage.py, proof_profile.py, regen_sail_model.sh
...
0x14`) and so survive a relayout, but each decode site also states the raw
...
targets and linker relaxation. `make check-decode` compares every site (and
every decode lemma) against the tracked dump, and `make update-decode` rewrites
the stale literals, refusing any site where the instruction itself changed.
**Run `make check-decode` after every re-dump.**
...
- **`tools/gen_decode.py`** — checks/refreshes `iris/`'s instruction-decode
  layer against the tracked image (`make check-decode` / `make update-decode`).
