...
| 34 | **the I-cache is not coherent with the data side.**  `core_icache` executes a one-instruction probe, overwrites its word with an ordinary store (no `fence.i`), executes it three more times, then once more after a `fence.i` | BOTH executions (claude-notes/design/icache.md): a fetch reads at any view at or above the hart's INSTRUCTION view, which only `fence.i` raises.  `VIcache.v` exhibits them as the two fetch schedules of `CoreIcacheSched.v`: `IFresh` (`1 2 2 2 2`) and `IStale` (`1 1 1 1 2`) | QEMU: the NEW instruction at once, every run (TCG invalidates its translation of a written page) -- `1 2 2 2 2`.  **The U74: the OLD instruction on every fetch until the `fence.i`, 5 runs of 5** -- `1 1 1 1 2` | the model is RIGHT to be non-coherent, and an xv6 `exec` without its `fence.i` would run stale text on this board | `core_icache` |
| 35 | **the board does not hand the payload a power-on register file, and the model's reset assumes it does.**  Two register files on the VisionFive 2 hold state at `_vtest_body` that no reset produced: the fp control file (`fcsr` = 0x44, i.e. `frm` = 2 and `fflags` = OF -- firmware executed floating point) and the low PMP entries (`pmpcfg0` = 0x0F, entry 0 TOR with RWX, and `pmpaddr0..7` -- OpenSBI programs them before it enters the payload).  QEMU is started with `-bios none`, so both files are zero there, which is what the model's `reset_pmp` and cold `fcsr` also produce | 0 in both files | QEMU 0 in both; the board as above | **NOT an unsoundness** -- `core_regs_pmp`'s header already says `reset_pmp` establishes its values over ARBITRARY garbage and that `pmpcfg` is not a board obligation.  The model's reset is more SPECIFIC than the machine a payload actually lands on, so these words can never be an `agree` proof on a board | `core_regs_fcsr`, `core_regs_pmp_fw` |
| 36 | **the U74 implements two programmable HPM counters, and they are running.**  `mhpmcounter3`/`4` (and their unprivileged aliases `hpmcounter3`/`4`) answer large, boot-varying values on the board; counters 5..18 read 0 there as everywhere, and all sixteen `mhpmevent` selectors agree | all sixteen counters read 0 | QEMU's default rv64 CPU also 0; the board's first two are live (measured, one boot: `0x64_8490_0744` and `0x19_0201_0210`) | incompleteness, and unfixable by construction -- a free-running counter cannot agree between two machines that execute a different number of cycles | `core_regs_hpm_live` |
...
| 5 | **the completion ORDER of two in-flight requests** | publication order ONLY -- `virtio_req_step` read the chain at `v_seen` and handed back `v_seen + 1`, so no schedule could reorder | EITHER order, and the model now has both: the device POPS available-ring entries in order (`virtio_pop_step`, `v_inflight` = the popped heads) and completes ANY in-flight head (`virtio_complete_step` takes the head), `VSched` picks which, and `DiskOrder.v` exhibits the two executions against the two captures.  The Iris driver proofs are ported to it (design/virtio-driver.md) | **UNSOUNDNESS**, RESOLVED | `disk_order` |
...
status byte for a read.  `VirtioModel.virtio_used_elem_writes` writes the data
...
and serves any in-flight head one bus transaction at a time
(`virtio_fetch_step` / `virtio_capture_step` / `virtio_write_step` /
`virtio_complete_step` take the head).  `VSched`'s eager schedules differ only
