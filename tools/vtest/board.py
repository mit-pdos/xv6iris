...
def gen(r, alts=None):
    """Write vtest-rocq/JH7110/<Name>Gen.v -- the board's capture, beside
    the QEMU one under QEMU/.  Same shape as vtest.py's gen() so the test .v can compare
    them with the same combinators; the names carry _hw_ so one file can
    Require both."""
    vtest.rocq_mkdirs()
    mod, low = vtest.modname(r["name"]), r["name"]
    path = vtest.rp(mod + "Gen.v", "jh7110")
    ...
    p = r["board"]
    open(path, "w").write(f"""(* JH7110/{mod}Gen.v -- GENERATED by tools/vtest/board.py from
    ...
    try:
        for n in names:
            ...
            if a.cmd == "gen":
                ...
                runp = vtest.rp(vtest.modname(n) + "Run.v", "jh7110")
