...
def emit_passes():
    ...
    THE PROOF IS SELF-SELECTING.  A run passes either because the model
    exhibits every observation the platform produced, or because the model
    reaches a state the RELATION cannot step from -- and the generator does
    not know which, so it emits both and lets the one that holds close the
    goal.  Both are one computation over [VExecStep]'s bridge, which is
    what makes them a line each.

    A file that does NOT compile is a run that does not pass -- the table's
    "no proof" column, which is a finding and not a build error to paper
    over.  A HAND-WRITTEN Pass module is left alone; see [hand_written]."""
    made, kept = [], []
    for n in all_tests():
        mod = modname(n)
        cfg = config(n)
  ...
  Proof.
    first
      [ solve [ left; intros o Ho;
                ...
                destruct Ho ]
      | solve [ right; apply (run_no_step {tick} lowest_head {budget});
