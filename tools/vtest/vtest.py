...
# though their files are now both <Case>Gen.v.
# ---------------------------------------------------------------------------
...
From VTest.{PLATDIR[pl]} Require Import {mod}Run.

Module {mod}Pass <: TEST_PASSES {mod}.
  Lemma passes : run_passes {mod}.observed {mod}.outcome.
  Proof. apply run_passes_b_sound. vm_cast_no_check (eq_refl true). Qed.
End {mod}Pass.
""")
            made.append(rrel(mod, pl))
    if kept:
...
def write_project(from_build=False):
    ...
    for pl in PLATFORMS:
        ...
        gens = sorted(f for f in here if f.endswith("Gen.v"))
...
def _run_state(n, pl):
    ...
    mod = modname(n)
    if not os.path.exists(rp(mod + "Run.v", pl)):
        if not os.path.exists(rp(mod + "Gen.v", pl)):
            return "uncaptured"
        return "no-builder"
...
def runs_from_captures():
    ...
    so this needs neither QEMU nor the board: it reads QEMU/<Name>Gen.v
    and JH7110/<Name>Gen.v and writes <Platform>/<Name>Run.v.  That keeps
    the captures authoritative and makes the port mechanical."""
    import re as _re
    ...
    wanted = {pl: set() for pl in PLATFORMS}
    for n in all_tests():
        b = config(n).get("builder", "single")
        for pl in platforms_of(n):
            if b in ("multi", "icache") and not os.path.exists(
                    rp(modname(n) + "Sched.v")):
                continue
            wanted[pl].add(modname(n))
    for n in all_tests():          # a hand-written run exists regardless of
        for pl in platforms_of(n):  # whether a builder could have made it
            if hand_written(modname(n) + "Run.v", pl):
                wanted[pl].add(modname(n))
    stale = 0
    for pl in PLATFORMS:
        for f in rocq_listdir(pl):
            # A PLATFORM DIRECTORY HOLDS ONLY GENERATED FILES, so matching is
            # now just the suffix -- the old pattern had to spell the platform
            # out to avoid deleting VRun.v, which it once did.  The harness
            # lives at the top and is not reachable from here at all.
            m = _re.match(r"(\w+)(Run|Pass)\.v$", f)
            if (m and m.group(1) not in wanted[pl]
                    and not hand_written(f, pl)):
                os.remove(rp(f, pl))
    ...
    for n in all_tests():
        mod = modname(n)
        # THE CAPTURE FILE IS <Case>Gen.v ON BOTH PLATFORMS -- the platform
        # is the directory.  Its DEFINITIONS still carry the board's _hw_
        # infix, so the two files' globals remain distinct.
        for platform, pfx, hart_def in [
                ("qemu",   n,         None),
                ("jh7110", n + "_hw", n + "_hw_primary_hart")]:
            if platform not in platforms_of(n):
                continue
            builder = config(n).get("builder", "single")
            if builder in ("multi", "icache") and not os.path.exists(
                    rp(mod + "Sched.v")):
                continue                       # no interleavings written yet
            src = rp(mod + "Gen.v", platform)
...
def emit_run_for(name, platform, text, observed, hart, path):
    ...
    if hand_written(os.path.basename(path), platform):
        return True                     # a run exists; it is just not ours
    b = config(name).get("builder", "single")
    if b == "multi":
        if not os.path.exists(rp(modname(name) + "Sched.v")):
            return False                # no interleavings written yet
        emit_run_conc(name, platform, text, observed, path, hart)
    elif b == "icache":
        if not os.path.exists(rp(modname(name) + "Sched.v")):
            return False                # no fetch schedules written yet
        emit_run_icache(name, platform, text, observed, hart, path)
    elif b == "sched":
...
def emit_run_sched(name, platform, text, observed, hart, path):
    ...
    cfg = config(name)
    mod = modname(name)          # the platform is the DIRECTORY
    rx = [b for b in cfg.get("serial_in", "").split(",") if b]
    prefix = "; ".join("SUartRx %s" % b for b in rx)
    results = ";\n     ".join("[%s]" % lit(o) for o in observed)
    open(path, "w").write(f"""(* {PLATDIR[platform]}/{os.path.basename(path)} -- GENERATED by tools/vtest.  Do not edit.
...
From Stdlib Require Import List ZArith String.
Import ListNotations.
From VTest Require Import VTest VConc VRun VRunConc {sched}.
Local Open Scope Z_scope.
...
def emit_run_icache(name, platform, text, observed, hart, path):
    """The RUN MODULE for a case that stores over its own code.  Same
    [TEST_RUN] as any other run; [outcome] is one model execution per FETCH
    schedule of the hand-written <Case>Sched.v -- which fetch view
    reproduces which observed outcome is the thing a human works out.
    [VIcache.IcacheRun]."""
    cfg = config(name)
    mod = modname(name)          # the platform is the DIRECTORY
    sched = modname(name) + "Sched"
   ...
   The test RUN produced by executing the self-modifying-code case [{name}]
   on platform [{platform}].  The fetch schedules are {sched}.schedules. *)
From Stdlib Require Import List ZArith String.
Import ListNotations.
From VTest Require Import VTest VRun VIcache {sched}.
Local Open Scope Z_scope.

Module {mod}Case <: ICACHE_CASE.
  Definition case      := "{name}"%string.
  Definition platform  := "{platform}"%string.
  Definition hart      : Z := {hart}.
  Definition regions   : list region := {regions_of(name)}.
