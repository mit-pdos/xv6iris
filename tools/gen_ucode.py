...
def emit(dump, prog, pcs, groups, dropped, skipfuncs, skipdefault, notes, asts,
    ...
    a('')
    a('  (* The byte side condition is discharged as a GOAL, not spliced into the')
    a('     term as an [ltac:] ARGUMENT -- claude-notes/optimization.md\'s "unshelve')
    a('     hoist", under "Inline [ltac:] in argument position".  It is worth')
    a('     spelling out here because it was essentially the WHOLE cost of this')
    a('     section: the inline form measured 0.85 s per instruction against 0.02 s')
    a('     for this one, and the tactic runs exactly ONCE either way, so the')
    a('     difference is the elaboration of the spliced term, not repeated')
    a('     [vm_compute]s. *)')
    a('  Ltac uis_run g off n w :=')
    a('    unshelve iApply (utext_img_run g %sInstrs.%s_bytes' % (M, D))
    a('              (uint (mword_of_int off : mword 64)) n w _);')
    a('    [ first [ uis_bytes2 | uis_bytes4 ] | ];')
