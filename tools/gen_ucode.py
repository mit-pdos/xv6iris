...
set of program counters, it emits a file in four sections:
  ...
  §3  <prog>_code / <prog>_rodata and their introductions from [utext_all] --
      the one place the program's bytes meet the process image.
...
Invocation:
  ...
  make gen-ucode                 regenerate EVERY catalog (the normal way)
  make check-ucode               ... and fail if any of them moved

  tools/gen_ucode.py --manifest tools/ucode_manifest.json
  tools/gen_ucode.py --module Init --spec tools/ucode_init.txt \
      --out iris/UCodeInit.v
  tools/gen_ucode.py --module Echo --out /tmp/UCodeEcho.v \
      --funcs main,start,strlen,exit,write --omit '0x338:exit never returns'

tools/ucode_manifest.json is THE RECORD of which catalogs exist: a catalog
that is not a row there is regenerated by nothing and drifts.  Each generated
file's own header prints the command that reproduces it.
...
/shared/xv6rocq)] must be in effect -- and that coqc needs the iris/ tree
BUILT, since the probe imports WpDecodeBridge and friends.

WHAT THE GENERATOR WILL NOT EMIT.  The catalog is the TEXT half: the dumped
instruction bytes, the read-only image sharing their pages, and one [uinstr]
fact per pc.  A predicate over the program's WRITABLE data -- its argument
vector, its buffers -- is a hand-written file's ([UInitArgv.v] is the worked
example), because whether those bytes may be persisted is a claim about the
program, not about the dump.
...
def invocation_lines(dump, out_path, args):
    """The command that regenerates this file, as the header prints it.

    The --out is quoted as [iris/<basename>], not as the path actually
    written, so that generating to a scratch directory and diffing against
    the tracked file still compares equal -- the check this whole layer
    rests on."""
    cmd = ['tools/gen_ucode.py', '--module', dump.module]
    if args.prefix:
    ...
    if args.all:
        cmd += ['--all']
    if args.funcs:
        cmd += ['--funcs', args.funcs]
    if args.pcs:
        cmd += ['--pcs', args.pcs]
    for o in (args.omit or []):
        cmd += ['--omit', "'%s'" % o]
    cmd += ['--out', 'iris/' + os.path.basename(out_path)]
    # wrap at the same width the header comment uses, continuing with a
    # backslash so the printed line can be pasted into a shell
    lines, cur = [], ''
    for tok in cmd:
...
def emit(dump, prog, pcs, groups, dropped, skipfuncs, skipdefault, notes, asts,
    ...
    a('')
    a('   Regenerate the whole layer with `make gen-ucode`; `make check-ucode`')
    a('   regenerates and then FAILS if anything moved.  This file alone is')
    a('')
    for l in invocation_lines(dump, out_path, args):
        a('     ' + l)
    a('')
    for l in wrap_comment(
            ...
            'data half, say -- belongs in a hand-written file beside this one, not '
            'in it.', '   '):
        a(l)
    a('')
    for l in wrap_comment(
    ...
    a('                          read-WRITE, and they, the stack and the argument area')
    a('                          belong to the program\'s own hand-written files')
    a('                          (U%s*.v / Uk%s*.v), which is also where a predicate'
      % (P[0].upper() + P[1:], P[0].upper() + P[1:]))
    a('                          over the writable half goes -- [UInitArgv.v] is the')
    a('                          worked example.')
    a('')
    ...
    a('Proof using . intros [ H _ ]. exact H. Qed.')
    a('Lemma %s_img_data (M : gmap Z (bv 8)) : %s_img_sub M -> %s_data_sub M.' % (P, P, P))
    a('Proof using . intros [ _ H ]. exact H. Qed.')
    a('')
    ...
    for width, w in order:
        ...
        if width == 16:
            ...
            a('Proof using . udec_rvc_oneshot. Qed.')
        else:
            a('  udecode_base (mword_of_int 0x%08x) (%s).' % (w, term))
            a('Proof using . udec_base_bridge. Qed.')
        a('')
    ...
    a('  Proof using . rewrite /%s_code. iIntros "#H". iExact "H". Qed.' % P)
    a('')
    ...
    for l in comment_block(
        ...
        a('  ' + l if l else l)
    a('')
    rokeys = sorted(a for a in dump.data if a < ntext * PAGE)
    rolo, rohi = (rokeys[0], rokeys[-1] + 1) if rokeys else (0, 0)
    for l in comment_block(
            ...
            'the code.  This program\'s literals land at 0x%x..0x%x, inside the R-X '
            'segment, '
            'and a printf-family walk LOADS them one byte at a time.  This is the '
            'part of [%sData.%s_data] that lands there -- everything below the end '
            'of the executable segment -- and [UserHeap.utext_str_of_img] cuts a '
...
def run(args):
    """Generate ONE catalog from an already-parsed argument set."""
...
# ---------------------------------------------------------------------------
# The manifest: the RECORD of which catalogs exist and how each is generated.
#
# `make gen-ucode` walks it, so a catalog that is not listed is not
# regenerated by anything and will drift.  Adding a program means a row here,
# a tools/ucode_<prog>.txt spec, and a line in iris/_CoqProject.
# ---------------------------------------------------------------------------

MANIFEST_KEYS = ('module', 'prefix', 'prog', 'spec', 'out')


def run_manifest(path, args):
    import json
    entries = json.load(open(path))
    for i, e in enumerate(entries):
        bad = set(e) - set(MANIFEST_KEYS)
        if bad:
            sys.exit('gen_ucode: %s entry %d: unknown key(s) %s'
                     % (path, i, ', '.join(sorted(bad))))
        for k in ('module', 'spec', 'out'):
            if k not in e:
                sys.exit('gen_ucode: %s entry %d: no %r' % (path, i, k))
        sub = argparse.Namespace(**vars(args))
        sub.manifest = None
        sub.module, sub.spec, sub.out = e['module'], e['spec'], e['out']
        sub.prefix, sub.prog = e.get('prefix'), e.get('prog')
        sub.funcs = sub.pcs = None
        sub.all, sub.omit = False, None
        print('==> %s' % sub.out)
        run(sub)
...
def main():
    ...
    ap.add_argument('--manifest', help='regenerate every catalog listed in this '
                                       'JSON record (tools/ucode_manifest.json)')
    ap.add_argument('--module', help='dump module prefix, e.g. Sh')
    ...
    if args.manifest:
        for k in ('module', 'out', 'spec', 'funcs', 'pcs', 'prog', 'prefix'):
            if getattr(args, k):
                sys.exit('gen_ucode: --manifest takes the whole layer; --%s is a '
                         'single-catalog flag' % k.replace('_', '-'))
        run_manifest(args.manifest, args)
        return 0
    if not args.module or not args.out:
        sys.exit('gen_ucode: --module and --out are required without --manifest')
    run(args)
    return 0
