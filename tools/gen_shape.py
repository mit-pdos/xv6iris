...
RETPAT = re.compile(r':\s*M\s*\(\s*ExecutionResult\s*\)\s*(:=|$)')


def returns_exres(body):
    """Monadic [M (ExecutionResult)] -- the value the sweep constrains."""
    head = ' ' + ' '.join(body.split(':=')[0].split())
    return ' M (ExecutionResult)' in head or ' M ExecutionResult' in head


def returns_exres_pure(body):
    """A PURE [ExecutionResult] producer: the ~54 compressed expansions.
    They are what [execute]'s [returnM (execute_C_* ...)] arms return, and
    the [ExecuteAs] payloads they build are where [ast_wf] has to come from
    for the redirection [run_hart_active] applies [execute] to a second
    time."""
    head = ' ' + ' '.join(body.split(':=')[0].split())
    return (not returns_exres(body)) and ': ExecutionResult' in head


def emit_exec(mon, pure, model, outdir, shard_sizes, dry_run=False):
    names = mon + pure
    shards = split_shards(names, shard_sizes)
    written = []
    for idx, shard in enumerate(shards, start=1):
        fname = 'WeakShapeExecGen%02d.v' % idx
        reqs = ['From stdpp Require Import gmap finite list relations.',
                ...
                + ' '.join('WeakShapeGen%02d' % j for j in range(1, 16)) + '.']
        if idx > 1:
            reqs.append('From xv6iris Require Import %s.'
                        % ' '.join('WeakShapeExecGen%02d' % j
                                   for j in range(1, idx)))
        reqs.append('Require Import Riscv.rv64d.')
        body = [EXEC_HEADER % {'file': fname, 'idx': idx,
                               'requires': '\n'.join(reqs)}]
        for n in shard:
            gs = binder_groups(binder_region(model[n][1]))
            arity = len(gs)
            ...
            body.append('')
            if n in pure:
                stmt = ('forall %s, exres_wf (@%s %s)' % (args, n, args)
                        if arity else 'exres_wf %s' % n)
                body.append('Lemma ew_%s : %s.' % (n, stmt))
                body.append('Proof. intros; %scbv [exres_wf %s]; ew_solve. Qed.'
                            % (pre, n))
                body.append('#[export] Hint Resolve ew_%s : gwpost.' % n)
            else:
                stmt = ('forall %s, gwx (@%s %s)' % (args, n, args)
                        if arity else 'gwx %s' % n)
                body.append('Lemma gx_%s : %s.' % (n, stmt))
                body.append('Proof. intros; %scbv [%s]; gwx_solve. Qed.'
                            % (pre, n))
                body.append('#[export] Hint Resolve gx_%s : gwexec.' % n)
        body.append('')
        text = '\n'.join(body)
        path = os.path.join(outdir, fname)
        if not dry_run:
    ...
    if not dry_run:
        ...
        while True:
            idx += 1
            stale = os.path.join(outdir, 'WeakShapeExecGen%02d.v' % idx)
            if not os.path.exists(stale):
                break
            os.remove(stale)
    return written
...
def main():
    ...
    if args.mode == 'exec':
        ...
        # cone is excluded exactly as in the shape mode.
        exres = {n for n in creach if n in model and returns_exres(model[n][1])}
        exmon = sorted((exres - impure) - {'execute'})
        expure = sorted(n for n in creach
                        if n in model and returns_exres_pure(model[n][1]))
        mon_order = [n for n in toposort(ccalls, set(exmon)) if n in set(exmon)]
        pure_order = [n for n in toposort(ccalls, set(expure))
                      if n in set(expure)]
        if args.report:
            print('exec mode: %d monadic (%d excluded as memory cone), '
                  '%d pure' % (len(mon_order), len(exres) - len(exmon) - 1,
                               len(pure_order)))
            print('excluded: %s' % ' '.join(sorted((exres & impure)
                                                   | {'execute'})))
        written = emit_exec(mon_order, pure_order, model, args.outdir, sizes,
                            args.dry_run)
