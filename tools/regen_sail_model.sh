...
# $SAIL_RISCV_REV; the fork's delta against riscv/sail-riscv upstream is the
# atomic PTE A/D-bit update (see README.md).  A checkout that is not at the
...
# generated by the build, so the source tree plus the two files above is
...
SAIL_RISCV_REV="${SAIL_RISCV_REV:-c32fbf4111b849061db1812355d6da9df8c2e396}"
