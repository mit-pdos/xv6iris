...
def emit_rocq(items: list[object], out_path: str, kernel: str, objdump: str,
    ...
    w("Require Import List ZArith String.")
    w("Import ListNotations.")
    w("")
    w("(* One decoded instruction of the kernel image.  [ki_enc] is exactly the")
    w("   integer Sail's fetch assembles from memory bytes (little-endian), so it")
    w("   is what you hand to the Sail RISC-V decoder.  [ki_asm] is for humans. *)")
    w("Record kinstr := MkKInstr {")
    w("  ki_addr  : Z;")
    w("  ki_width : nat;")
    w("  ki_enc   : Z;")
    w("  ki_asm   : string")
    w("}.")
    w("")
    w("(* Coq elaborates one huge list literal in super-linear time, so emit")
    w("   fixed-size chunks and concatenate them with List.concat. *)")
    w("")
    CHUNK = 200
    chunk_idx = 0
    count_in_chunk = 0
    ...
    def open_chunk(idx: int) -> None:
        chunk_names.append(f"{name}_chunk{idx}")
        w(f"Definition {name}_chunk{idx} : list kinstr := [")

    open_chunk(chunk_idx)
    # Emit each instruction as a fully scope-annotated record so the file
    # compiles regardless of which numeric/string scope is open.
    cur_label = None
    first_in_chunk = True
    for it in items:
        if isinstance(it, Label):
        ...
        if cur_label is not None:
            ...
            cur_label = None
        w(
            f"{sep}MkKInstr (0x{it.addr:x})%Z {it.width}%nat "
            f"(0x{it.enc:x})%Z {coq_string(it.asm)}%string"
        )
        count_in_chunk += 1
    w("].")
    w("")
    w(f"Definition {name} : list kinstr := List.concat [")
    for i, cn in enumerate(chunk_names):
        sep = "  " if i == 0 else "; "
        w(f"{sep}{cn}")
    w("].")
...
def emit_rocq_data(items: list[object], out_path: str, kernel: str,
                   objdump: str, name: str) -> tuple[int, int]:
    """Rocq mirror of emit_lean_data: the loadable DATA of the kernel image (every
    PT_LOAD byte NOT covered by a dumped instruction), grouped into 8-byte words
    (stray bytes solo), as a chunked `list kinstr` (reusing the [kinstr] record
    from Kernel.KernelInstrs; width is in bits, enc is the value).  Plus
    [kernelMemBase] and [kernelMemEnd] (= vaddr+memsz, INCLUDING zero-init .bss).
    Code (kernel_instrs) + this data = the full on-disk image; the BSS zero-init
    region is [kernelMemBase, kernelMemEnd) minus the loaded bytes."""
    insns = [it for it in items if isinstance(it, Insn)]
    covered: set[int] = set()
    for it in insns:
        for k in range(it.width // 8):
    ...
    n = len(entries)
    lo = min((a for a, _, _ in entries), default=0)
    hi = max((a + wd // 8 for a, wd, _ in entries), default=0)
    base, end = image_extent(kernel)
    ...
    w("Require Import List ZArith String.")
    w("Import ListNotations.")
    w("Require Import Kernel.KernelInstrs.")
    w("")
    ...
    w("")
    w("(* Chunked to keep Coq's list-literal elaboration shallow. *)")
    CHUNK = 200
    chunk_names: list[str] = []
    for ci in range(0, n, CHUNK):
        cn = f"{name}_chunk{ci // CHUNK}"
        chunk_names.append(cn)
        body = "; ".join(
            f"MkKInstr (0x{a:x})%Z {wd}%nat (0x{v:x})%Z \"\"%string"
            for a, wd, v in entries[ci:ci + CHUNK])
        w(f"Definition {cn} : list kinstr := [{body}].")
    w("")
    if chunk_names:
        w(f"Definition {name} : list kinstr := List.concat [{'; '.join(chunk_names)}].")
    else:
        w(f"Definition {name} : list kinstr := [].")
    w("")
    w(f"(* Total data words = {n} (in {len(chunk_names)} chunks of <= {CHUNK}). *)")
    w("")
...
def emit_rocq_syms(items: list[object], out_path: str, kernel: str,
    ...
    for ci in range(0, n, CHUNK):
        ...
        chunk_names.append(cn)
        body = "; ".join(
            f"({coq_string(s)}%string, 0x{a:x}%Z)" for s, a in syms[ci:ci + CHUNK])
        w(f"Definition {cn} : list (string * Z) := [{body}].")
    w("")
    if chunk_names:
        w(f"Definition {name} : list (string * Z) := List.concat [{'; '.join(chunk_names)}].")
    else:
        w(f"Definition {name} : list (string * Z) := [].")
    w("")
    w("(* Address of a kernel symbol by name (0 if not found). *)")
    w("Definition sym (q : string) : Z :=")
    w(f"  match List.find (fun p => String.eqb (fst p) q) {name} with")
    w("  | Some p => snd p")
    w("  | None => 0%Z")
    w("  end.")
    w("")
    w(f"(* Total symbols = {n} (in {len(chunk_names)} chunks of <= {CHUNK}). *)")
    w("")
...
def main() -> None:
    ...
    if args.format == "lean":
        ...
        name = args.name or "kernel_instrs"
        emit_fn = emit_rocq
