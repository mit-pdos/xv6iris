...
We disassemble the executable sections with ``objdump`` and emit:
  ...
  ``--format rocq-raw``   the WHOLE debug-stripped ELF, hex-encoded as one
                          ``<prefix>_elf_hex : PrimString.string`` (Rocq 9
                          primitive string), plus ``<prefix>_elf_size : Z``.
                          Unlike the three formats above (which split the
                          image into code/data by this dumper's own
                          instruction/section reasoning), this is the raw file
                          -- the ground truth a general ELF64 semantics can be
                          instantiated against and checked consistent with the
                          other dumps.
  ``--format rocq-bin``   Like ``rocq-raw`` but for an ARBITRARY file, not
                          necessarily an ELF: no magic check, no ELF parsing at
                          all, and (see ``main()``) no disassembly is attempted
                          either.  Used for ``xv6-riscv/fs.img``, the initial
                          disk image ``mkfs`` builds.  Emits ``<prefix>_hex``
                          (chunked the same way) and ``<prefix>_size``.
...
class Names:
    ...
    @property
    def elf_hex(self) -> str:  return f"{self.prefix}_elf_hex"
    @property
    def elf_size(self) -> str: return f"{self.prefix}_elf_size"
    def elf_hex_chunk(self, i: int) -> str: return f"{self.prefix}_elf_hex_chunk{i}"
    # `--format rocq-bin`: same shape as elf_hex/elf_size/elf_hex_chunk above,
    # but for an arbitrary (non-ELF) file -- kept as separate names/properties
    # so a kernel/user ELF dump and e.g. the fs.img dump can be `Require`d
    # side by side without clashing (mirrors why elf_* itself is prefixed).
    @property
    def hex(self) -> str:  return f"{self.prefix}_hex"
    @property
    def size(self) -> str: return f"{self.prefix}_size"
    def hex_chunk(self, i: int) -> str: return f"{self.prefix}_hex_chunk{i}"
...
def emit_rocq_raw(items: list[object], out_path: str, elf: str, objdump: str,
    ...
    CHUNK = 8192
    chunk_strs = [hexstr[i:i + CHUNK] for i in range(0, len(hexstr), CHUNK)]
    chunk_names = [names.elf_hex_chunk(i) for i in range(len(chunk_strs))]
    ...
    w("From Stdlib.Strings Require Import PString.")
    w("Import ListNotations.")
    w("")
    w("(* LOWERCASE hex, no separators, chunked so Rocq never elaborates one")
    w(f"   giant string literal.  [{names.elf_hex}] below is the full")
    w("   [PrimString.string] (a Rocq 9 primitive string: compact literal,")
    w("   O(1) [PrimString.get], native in [vm_compute]). *)")
    for cn, cs in zip(chunk_names, chunk_strs):
        w(f'Definition {cn} : PrimString.string := "{cs}"%pstring.')
    w("")
    cat_list = "; ".join(chunk_names)
    w(f"Definition {names.elf_hex} : PrimString.string :=")
    w(f'  List.fold_left PrimString.cat [{cat_list}] ""%pstring.')
    w("")
    w(f"(* Size in BYTES of the (debug-stripped) ELF file; hex length = "
      f"2 * this. *)")
    w(f"Definition {names.elf_size} : Z := {nbytes}%Z.")
    w("")

    write_if_changed(out_path, "\n".join(lines))
    return nbytes, len(lines)


def emit_rocq_bin(items: list[object], out_path: str, elf: str, objdump: str,
                  names: Names) -> tuple[int, int]:
    """The WHOLE file, byte for byte, hex-encoded as a single Rocq 9 primitive
    string (`PrimString.string`) -- `--format rocq-raw` generalized to a file
    that is not an ELF at all: xv6's ``fs.img``, the literal initial disk
    image ``mkfs`` built, packing the (deterministic, ``-ffile-prefix-map``)
    user binaries.  ``iris/FsImgCheck.v`` proves it well-formed and that
    ``/init /sh /echo /sync`` hold exactly the tracked ``*ElfRaw`` binaries.

    Unlike `emit_rocq_raw`, this does NO file-format reasoning at all: no ELF
    magic check, no section/instruction parsing -- just the raw bytes.
    (`items` and `objdump` are accepted only to keep the same emitter call
    signature as every other `--format`; `main()` skips `find_objdump()` and
    `disassemble()` entirely for this format -- objdump has nothing to
    disassemble in a filesystem image, and may not even be installed.)

    The only check kept is the same build-directory guard as `rocq-raw`, and
    for the same reason: the packed user binaries are expected to be
    byte-identical across build trees (that is what the `rocq-raw`/`ElfUser`
    dumps of those same binaries already rely on), so a dump that embedded
    its own build directory would silently produce a tree-dependent proof
    target.  `fs.img` sits directly at the xv6 build root, so *its own*
    directory (one level up from the file, not two as for a nested ELF like
    `kernel/kernel`) IS the build directory being guarded against.
    """
    path = elf
    ...
    if build_dir.encode() in data:
        sys.exit(
            f"rocq-bin: {path} embeds its own build directory ({build_dir}), "
            "so this dump would differ between build trees.  The xv6 build is "
            "expected to pass -ffile-prefix-map=$(CURDIR)=. (see DETFLAGS in "
            "its Makefile) so the packed user binaries are build-dir-free; "
            "rebuild xv6 at a revision with that flag rather than dumping "
            "this file."
        )
    ...
    CHUNK = 8192
    chunk_strs = [hexstr[i:i + CHUNK] for i in range(0, len(hexstr), CHUNK)]
    chunk_names = [names.hex_chunk(i) for i in range(len(chunk_strs))]

    lines: list[str] = []
    w = lines.append
    w("(* ------------------------------------------------------------------ *)")
    w(f"(* AUTO-GENERATED by {GEN_BY} (--format rocq-bin).             *)")
    w(f"(* Source file (literal, byte for byte): {os.path.relpath(path, REPO)}")
    w("   xv6's fs.img: the literal initial disk image mkfs built, packing")
    w("   the (deterministic, -ffile-prefix-map) user binaries.")
    w("   iris/FsImgCheck.v proves it well-formed and that /init /sh /echo")
    w("   /sync hold exactly the tracked *ElfRaw binaries.")
    w(f"   Bytes : {nbytes}   Hex chars : {len(hexstr)}   "
      f"Chunks : {len(chunk_strs)} (<= {CHUNK} hex chars each)")
    w("")
    w("   Unlike the ELF dumps in this file (--format rocq/rocq-data/rocq-syms/")
    w("   rocq-raw), fs.img is not an ELF and gets NO file-format reasoning at")
    w("   all here -- no magic check, no section or instruction parsing, just")
    w("   the raw bytes mkfs wrote.  The guard below refuses a file that embeds")
    w("   its own build directory, the same rationale as --format rocq-raw's")
    w("   guard: fs.img sits at the xv6 build root, so its own directory IS the")
    w("   build directory. *)")
    w("(* ------------------------------------------------------------------ *)")
    w("")
    w("From Stdlib Require Import ZArith List.")
    ...
    w("")
    w("(* LOWERCASE hex, no separators, chunked so Rocq never elaborates one")
    w(f"   giant string literal.  [{names.hex}] below is the full")
    w("   [PrimString.string] (a Rocq 9 primitive string: compact literal,")
    w("   O(1) [PrimString.get], native in [vm_compute]). *)")
    for cn, cs in zip(chunk_names, chunk_strs):
        w(f'Definition {cn} : PrimString.string := "{cs}"%pstring.')
    w("")
    cat_list = "; ".join(chunk_names)
    w(f"Definition {names.hex} : PrimString.string :=")
    w(f'  List.fold_left PrimString.cat [{cat_list}] ""%pstring.')
    w("")
    w("(* Size in BYTES of the file; hex length = 2 * this. *)")
    w(f"Definition {names.size} : Z := {nbytes}%Z.")
    w("")
...
# format -> (emitter, default output directory, default file suffix, whether
# main() needs to run find_objdump()+disassemble() before calling the
# emitter).  The default file name is <Module><suffix>, e.g.
# KernelInstrs.v / SyncInstrs.v.  `rocq-bin` is the one format that dumps an
# arbitrary non-ELF file (fs.img): it does no disassembly at all, so it is
# the only entry with needs_disasm=False -- every other format's behavior is
# unchanged (main() still runs find_objdump()+disassemble() for them exactly
# as before).
FORMATS = {
    "lean":        (emit_lean,        "lean",        "Instrs.lean", True),
    "lean-data":   (emit_lean_data,   "lean",        "Data.lean",   True),
    "lean-syms":   (emit_lean_syms,   "lean",        "Syms.lean",   True),
    "lean-decode": (emit_lean_decode, "lean",        "Decode.lean", True),
    "rocq":        (emit_rocq,        "kernel-rocq", "Instrs.v",    True),
    "rocq-data":   (emit_rocq_data,   "kernel-rocq", "Data.v",      True),
    "rocq-syms":   (emit_rocq_syms,   "kernel-rocq", "Syms.v",      True),
    "rocq-raw":    (emit_rocq_raw,    "kernel-rocq", "ElfRaw.v",    True),
    "rocq-bin":    (emit_rocq_bin,    "kernel-rocq", "Raw.v",       False),
}
...
def main() -> None:
    ...
    ap.add_argument("--format", choices=list(FORMATS), default="lean",
                    help="output: 'rocq' (text image + decode index), "
                         ...
                         "'rocq-syms' (symbol table), 'rocq-raw' (the whole "
                         "debug-stripped ELF, hex-encoded as one PrimString), "
                         "'rocq-bin' (like rocq-raw but for an arbitrary "
                         "non-ELF file, e.g. fs.img; no disassembly), "
                         "or their 'lean' mirrors "
    ...
    emit_fn, out_dir, suffix, needs_disasm = FORMATS[args.format]
    ...
    if needs_disasm:
        ...
        # e.g. rocq-bin: the file (fs.img) is not an ELF, so there is nothing
        # to disassemble and objdump may not even be installed -- don't look
        # for it.
        objdump = ""
        items = []
