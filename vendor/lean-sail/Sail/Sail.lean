...
/-- How to turn the architecture's physical-address type into a byte address,
needed only by the reference interpreter. -/
class PaToNat [Arch] where
  toNat : Arch.pa → Nat

def main_of_sail_main [Arch] [PaToNat] (initialState : SequentialState RegisterType trivialChoiceSource)
    (main : Unit → PreSailM RegisterType trivialChoiceSource ue Unit) : IO UInt32 := do
  let res := (main ()).interp PaToNat.toNat |>.run initialState
  match res with
  | .ok _ s => do
